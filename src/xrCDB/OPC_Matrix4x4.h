...
class ICEMATHS_API Matrix4x4 {
  //				void	LUBackwardSubstitution( sdword *indx, float*
  //b
...
public:
  ...
  inline_ Matrix4x4(float m00, float m01, float m02, float m03, float m10,
  ...
  inline_ Matrix4x4 &Set(float m00, float m01, float m02, float m10, float m11,
  ...
  //! Assign values
  inline_ Matrix4x4 &Set(float m00, float m01, float m02, float m03, float m10,
                         float m11, float m12, float m13, float m20, float m21,
                         float m22, float m23, float m30, float m31, float m32,
                         float m33) {
    m[0][0] = m00;
  ...
  //p) { m[3][0]=p.x;	m[3][1]=p.y;	m[3][2]=p.z;	m[3][3]=p.w;	}
  ...
  // Row-column access
  //! Returns a row.
  //		inline_	void				GetRow(const udword r, HPoint&
  //p) const	{ p.x=m[r][0];	p.y=m[r][1];	p.z=m[r][2];	p.w=m[r][3];
  //}
  //! Returns a row.
  ...
  //! Sets a row.
  //		inline_	void				SetRow(const udword r, const
  //HPoint& p)		{ m[r][0]=p.x;	m[r][1]=p.y;	m[r][2]=p.z;
  // m[r][3]=p.w;
  //}
  //! Sets a row.
  ...
  //! Returns a column.
  //		inline_	void				GetCol(const udword c, HPoint&
  //p) const	{ p.x=m[0][c];	p.y=m[1][c];	p.z=m[2][c];	p.w=m[3][c];
  //}
  //! Returns a column.
  ...
  //! Sets a column.
  //		inline_	void				SetCol(const udword c, const
  //HPoint& p)		{ m[0][c]=p.x;	m[1][c]=p.y;	m[2][c]=p.z;
  // m[3][c]=p.w;
  //}
  //! Sets a column.
  ...
  */
  ...
  Matrix4x4 &Shadow(const Point &light, const Point &p0, const Point &p1,
                    const Point &p2);
  ...
  /*		inline_	Matrix4x4			operator+(const Matrix4x4&
     mat) const
                                  {
                                          Matrix4x4 Ret;
  ...
  */
  ...
  inline_ Matrix4x4 operator*(const Matrix4x4 &mat) const {
    return Matrix4x4(m[0][0] * mat.m[0][0] + m[0][1] * mat.m[1][0] +
  ...
  //v) const	{ return HPoint(ROW[0]|v, ROW[1]|v, ROW[2]|v, ROW[3]|v); }
...
inline_ void TransformPoint4x3(Point &dest, const Point &source,
                               const Matrix4x4 &rot) {
...
inline_ void TransformPoint3x3(Point &dest, const Point &source,
                               const Matrix4x4 &rot) {
  dest.x =
      source.x * rot.m[0][0] + source.y * rot.m[1][0] + source.z * rot.m[2][0];
  dest.y =
      source.x * rot.m[0][1] + source.y * rot.m[1][1] + source.z * rot.m[2][1];
  dest.z =
      source.x * rot.m[0][2] + source.y * rot.m[1][2] + source.z * rot.m[2][2];
}
