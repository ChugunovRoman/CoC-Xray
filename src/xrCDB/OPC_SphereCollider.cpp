 ...
 *classic codes - the "_new_" one by Paul Nettle is a debuggued version I think.
 *Collision response can be driven by reported collision data - it works
...
#define SPHERE_PRIM(prim, flag)                                                \
  ...
  const IndexedTriangle *Tri = &mFaces[prim];                                  \
                                                                               \
  /* Perform sphere-tri overlap test */                                        \
  if (SphereTriOverlap(mVerts[Tri->mVRef[0]], mVerts[Tri->mVRef[1]],           \
                       mVerts[Tri->mVRef[2]])) {                               \
    /* Set contact status */                                                   \
    mFlags |= flag;                                                            \
    mTouchedPrimitives->Add(prim);                                             \
...
bool SphereCollider::Collide(SphereCache &cache, const Sphere &sphere,
                             OPCODE_Model *model, const Matrix4x4 *worlds,
  ...
  if (!model->HasLeafNodes()) {
    if (model->IsQuantized())
      return Collide(cache, sphere,
                     (const AABBQuantizedNoLeafTree *)model->GetTree(), worlds,
                     worldm);
    else
      return Collide(cache, sphere, (const AABBNoLeafTree *)model->GetTree(),
                     worlds, worldm);
  } else {
    if (model->IsQuantized())
      return Collide(cache, sphere, (const AABBQuantizedTree *)model->GetTree(),
                     worlds, worldm);
    else
      return Collide(cache, sphere, (const AABBCollisionTree *)model->GetTree(),
                     worlds, worldm);
  }
}
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
BOOL SphereCollider::InitQuery(SphereCache &cache, const Sphere &sphere,
  ...
  if (TemporalCoherenceEnabled()) {
    ...
    if (FirstContactEnabled()) {
      // We're only interested in the first contact found => test the unique
      // previously touched face
      if (mTouchedPrimitives->GetNbEntries()) {
        // Get index of previously touched face = the first entry in the array
        udword PreviouslyTouchedFace = mTouchedPrimitives->GetEntry(0);

        // Then reset the array:
        // - if the overlap test below is successful, the index we'll get added
        // back anyway
        // - if it isn't, then the array should be reset anyway for the normal
        // query
        mTouchedPrimitives->Reset();

        // Perform overlap test between the cached triangle and the sphere (and
      ...
      // We're interested in all contacts =>test the _new_ real sphere N(ew)
      // against the previous fat sphere P(revious):
 ...
 *the query sooner. \param		bc	[in] box center \param
 *be	[in] box extents \return		true if the sphere contains the
 *whole box
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
