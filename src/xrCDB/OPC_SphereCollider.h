...
public:
   ...
   *with \param		worlds			[in] sphere's world matrix, or
   *null \param		worldm			[in] model's world matrix, or
   *null \return		true if success \warning	SCALE NOT
  ...
  // Collision queries
  bool Collide(SphereCache &cache, const Sphere &sphere,
               const AABBCollisionTree *tree, const Matrix4x4 *worlds = null,
               const Matrix4x4 *worldm = null);
  bool Collide(SphereCache &cache, const Sphere &sphere,
               const AABBNoLeafTree *tree, const Matrix4x4 *worlds = null,
               const Matrix4x4 *worldm = null);
  bool Collide(SphereCache &cache, const Sphere &sphere,
               const AABBQuantizedTree *tree, const Matrix4x4 *worlds = null,
               const Matrix4x4 *worldm = null);
  bool Collide(SphereCache &cache, const Sphere &sphere,
...
protected:
  ...
  BOOL SphereTriOverlap(const Point &vert0, const Point &vert1,
                        const Point &vert2);
  // Init methods
  BOOL InitQuery(SphereCache &cache, const Sphere &sphere,
