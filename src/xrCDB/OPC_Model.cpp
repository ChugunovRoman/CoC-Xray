 ...
 *collision system, in order to save some ram. So, when the system needs them to
 *perform accurate intersection tests, you're requested to provide the
 ...
 *also not supposed to modify the geometry *after* the collision trees have been
 *built. The alternative was to store the geometry & topology in the collision
 *system as well (as in RAPID) but we have found this approach to waste a lot of
 *ram in many cases.
 *
 ...
 *performed during last query \endcode
 *
 *	\class		OPCODE_Model
 *	\author		Pierre Terdiman
 *	\version	1.2
...
bool OPCODE_Model::Build(const OPCODECREATE &create) {
  ...
  // So clean your meshes first... If you don't have a mesh cleaner you can try
  // this: www.codercorner.com/Consolidation.zip
  ...
  if (NbDegenerate)
    Log("OPCODE WARNING: found %d degenerate faces in model! Collision might "
