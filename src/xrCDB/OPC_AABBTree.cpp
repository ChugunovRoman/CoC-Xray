 ...
 *primitive is a triangle, the resulting tree can be converted into an optimized
 *tree. If the primitive is a box, the resulting tree can be used for culling -
 *VFC or occlusion -, assuming you cull on a mesh-by-mesh basis (modern way).
 *
 *	\class		AABBTree
 *	\author		Pierre Terdiman
 *	\version	1.2
...
udword AABBTreeNode::Split(udword axis, AABBTreeBuilder *builder) {
  ...
  float SplitValue =
      builder->GetSplittingValueEx(mNodePrimitives, mNbPrimitives, mBV, axis);
  ...
  for (udword i = 0; i < mNbPrimitives; i++) {
    ...
    // Test against the splitting value. The primitive value is tested against
    // the enclosing-box center. [We only need an approximate partition of the
    // enclosing box here.]
...
udword AABBTree::ComputeDepth() const {
  ...
  struct Local {
    static void _UpdateDepth(const AABBTreeNode *curnode, udword &depth,
                             udword &current) {
      // Checkings
      if (!curnode)
        return;
 ...
 *primitives in the tree. \return		true for complete trees
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
