...
bool CObjectSpace::BoxQuery(Fvector const &box_center,
  ...
  planes[left_plane].build(box_center - (x_axis * (box_sizes.x * 0.5f)),
                           -x_axis);
  planes[right_plane].build(box_center + (x_axis * (box_sizes.x * 0.5f)),
                            x_axis);
  planes[top_plane].build(box_center + (y_axis * (box_sizes.y * 0.5f)), y_axis);
  planes[bottom_plane].build(box_center - (y_axis * (box_sizes.y * 0.5f)),
                             -y_axis);
  planes[front_plane].build(box_center - (z_axis * (box_sizes.z * 0.5f)),
                            -z_axis);
  planes[near_plane].build(box_center + (z_axis * (box_sizes.z * 0.5f)),
                           z_axis);
...
void CObjectSpace::BoxQuery	(collide::rq_results& r_dest, const Fbox& B,
const Fmatrix& M, u32 flags)
{
        Fvector		bc,bd;
        Fbox		xf;
        xf.xform	(B,M);
        ...
        if ((flags&clStatic) == clStatic)
        {
                ...
                if (xrc.r_count())
                {
                        ...
                        for (; it!=end; it++)
                                q_result.AddTri(&Static.get_tris()
[it->id],Static.get_verts());
                }
        };
...
= r_spatial[o_it];
                        CObject*	collidable
= spatial->dcast_CObject	(); if			(0==collidable)
continue; collidable->collidable.model->_BoxQuery	(B,M,flags);
                }
        };
}
*/
