...
struct OPCODE_API AABBCache : VolumeCache {
  ...
  CollisionAABB
      FatBox; //!< Box used when performing the query resulting in cached faces
  // User settings
...
public:
   ...
   *world space \param		model			[in] Opcode model to
   *collide with \return		true if success \warning	SCALE
  ...
  // Collision queries
  bool Collide(AABBCache &cache, const CollisionAABB &box,
