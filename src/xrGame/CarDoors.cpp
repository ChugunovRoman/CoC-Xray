...
void CCar::SDoor::Init() {
  ...
  R_ASSERT2(joint->IsHingeJoint(),
            "Wrong door joint!!! Only simple joint valid for a door and only "
            "one axis can be active, check other axes are zerro limited !!!");
  ...
  joint->PSecond_element()->get_Extensions(jaxis, janchor.dotproduct(jaxis),
                                           lo_ext, hi_ext);
  ...
  if (jaxis_in_door.x > jaxis_in_door.y) {
    if (jaxis_in_door.x > jaxis_in_door.z) {
      joint->PSecond_element()->get_Extensions(
          door_transform.j, janchor.dotproduct(door_transform.j), lo_ext,
          hi_ext);
      ...
      door_plane_axes.y = 1;
      joint->PSecond_element()->get_Extensions(
          door_transform.k, janchor.dotproduct(door_transform.k), lo_ext,
          hi_ext);
      ext = hi_ext - lo_ext;
      ...
      door_plane_axes.y = 1;
      joint->PSecond_element()->get_Extensions(
          door_transform.i, janchor.dotproduct(door_transform.i), lo_ext,
          hi_ext);
      ext = hi_ext - lo_ext;
      if (ext > door_plane_ext.y) {
        door_dir_sign = hi_ext > -lo_ext ? 1.f : -1.f;
...
void CCar::SDoor::SDoorway::Init(SDoor *adoor) {
  ...
  if (_abs(door_axis_in_door.x) > _abs(door_axis_in_door.y)) {
    if (_abs(door_axis_in_door.x) > _abs(door_axis_in_door.z)) {
      ...
      door_plane_axes.y = 0; // door axis is x (door_plane_axes.y stores door
                             // axis direction (i,j,k)=(0,1,2)
      door->joint->PSecond_element()->get_Extensions(
          door_transform.i, janchor.dotproduct(door_transform.i), lo_ext,
          hi_ext);
      door->door_plane_ext.y = hi_ext - lo_ext; // door extension along door
                                                // axis
      ...
      door_plane_axes.x = 1; // door_plane_axes.x stores door direction it may
                             // be j or k in this point
