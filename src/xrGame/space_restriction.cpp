...
struct CRemoveMergedFreeInRestrictions {
  ...
  IC bool operator()(
      const CSpaceRestriction::CFreeInRestriction &free_in_restriction) const {
...
void CSpaceRestriction::merge_in_out_restrictions() {
  ...
  m_border.erase(
      std::remove_if(m_border.begin(), m_border.end(),
                     CMergeInOutPredicate<true>(m_out_space_restriction,
                                                m_in_space_restriction)),
      m_border.end());
  ...
  if (m_in_space_restriction) {
    ...
    temp_border.erase(
        std::remove_if(temp_border.begin(), temp_border.end(),
...
CSpaceRestriction::merge(CBaseRestrictionPtr bridge,
  ...
  for (bool ok = false; !ok;) {
    ...
    for (; I != E; ++I) {
      ...
      if (!temp_restrictions.empty()) {
        J = remove_if(m_free_in_restrictions.begin(),
...
u32 CSpaceRestriction::accessible_nearest(const Fvector &position,
                                          Fvector &result) {
  if (m_out_space_restriction)
    return (m_out_space_restriction->accessible_nearest(this, position, result,
                                                        true));

  VERIFY(m_in_space_restriction);
  return (m_in_space_restriction->accessible_nearest(m_in_space_restriction,
...
bool CSpaceRestriction::affect(
  ...
  // return							(start_position.distance_to(position) <=
  // radius
  // + dependent_distance);
}

bool CSpaceRestriction::affect(
    SpaceRestrictionHolder::CBaseRestrictionPtr bridge, u32 start_vertex_id,
    float radius) const {
  Fsphere sphere;
