...
void CLevel::net_Stop() {
  ...
  // WARNING ! remove_objects() uses this flag, so position of this line must e
  // here ..
...
void CLevel::ClientSend(bool bForce) {
  ...
  u32 position;
  for (u32 start = cur_index;
       start <
       (bForce ? object_count : cur_index + g_objects_per_client_update);
       start++) {
...
u32 CLevel::Objects_net_Save(NET_Packet *_Packet, u32 start,
                             u32 max_object_size) {
