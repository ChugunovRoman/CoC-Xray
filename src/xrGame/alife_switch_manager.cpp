...
void CALifeSwitchManager::add_online(CSE_ALifeDynamicObject *object,
  ...
  // ai().level_graph().valid_vertex_id(object->m_tNodeID), make_string("Invalid
  // vertex for object %s", object->name_replace()));
...
void CALifeSwitchManager::remove_online(CSE_ALifeDynamicObject *object,
  ...
  CSE_ALifeTraderAbstract *inventory_owner =
      smart_cast<CSE_ALifeTraderAbstract *>(object);
  if (inventory_owner) {
...
#ifdef DEBUG
  //	if (psAI_Flags.test(aiALife))
  Msg("[LSS][%d] Going online [%d][%s][%d] ([%f][%f][%f] : [%f][%f][%f]), on "
      "'%s'",
      Device.dwFrame, Device.dwTimeGlobal, object->name_replace(), object->ID,
      VPUSH(graph().actor()->o_Position), VPUSH(object->o_Position),
      "*SERVER*");
#endif
...
#ifdef DEBUG
  //	if (psAI_Flags.test(aiALife))
  Msg("[LSS][%d] Going offline [%d][%s][%d] ([%f][%f][%f] : [%f][%f][%f]), on "
      "'%s'",
      Device.dwFrame, Device.dwTimeGlobal, object->name_replace(), object->ID,
      VPUSH(graph().actor()->o_Position), VPUSH(object->o_Position),
      "*SERVER*");
...
void CALifeSwitchManager::try_switch_online(CSE_ALifeDynamicObject *I) {
  ...
  VERIFY2((ai().game_graph().vertex(I->m_tGraphID)->level_id() !=
          ...
          make_string("frame [%d] time [%d] object [%s] with id [%d] is "
                      ...
                      Device.dwFrame, Device.dwTimeGlobal, I->name_replace(),
                      I->ID));
#endif
  I->try_switch_online();
