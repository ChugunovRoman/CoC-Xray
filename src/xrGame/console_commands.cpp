...
static void full_memory_stats() {
  ...
  Msg("* [x-ray]: economy: strings[%d K], smem[%d K]", _eco_strings / 1024,
      _eco_smem);
...
public:
  ...
  virtual void Execute(LPCSTR args) {
    ...
    LPSTR save_name;
    STRCONCAT(save_name, CStringTable().translate("st_game_saved").c_str(),
              ": ", S);
...
public:
  ...
  virtual void Execute(LPCSTR args) {
    ...
    if (!CSavedGameWrapper::valid_saved_game(saved_game)) {
      Msg("! Cannot load saved game %s, version mismatch or saved game is "
          "corrupted",
          saved_game);
      return;
    }
...
public:
  ...
  virtual void Info(TInfo &I) {
    if (strstr(cName, "script_debug_break") == cName)
      ...
      xr_strcpy(
          I,
          "restarts script debugger or start if no script debugger presents");
  }
};
...
public:
  ...
  virtual void Execute(LPCSTR args) {
    typedef CSafeMapIterator<ALife::_OBJECT_ID,
                             CSE_ALifeDynamicObject>::_REGISTRY::const_iterator
        const_iterator;
...
public:
  ...
  virtual void Execute(LPCSTR args) {
    if (!xr_strlen(args))
      ...
      xr_sprintf(S, "%s\n", args);
      int l_iErrorCode = luaL_loadbuffer(ai().script_engine().lua(), S,
                                         xr_strlen(S), "@console_command");
      if (!l_iErrorCode) {
  ...
  virtual void Status(TStatus &S) {
    xr_strcpy(S,
              "<script_name.function()> (Specify script and function name!)");
  }
  virtual void Save(IWriter *F) {}
...
struct CCC_StartTimeSingle : public IConsole_Command {
  ...
  virtual void Execute(LPCSTR args) {
    u32 year = 1, month = 1, day = 1, hours = 0, mins = 0, secs = 0,
        milisecs = 0;
    sscanf(args, "%d.%d.%d %d:%d:%d.%d", &year, &month, &day, &hours, &mins,
           &secs, &milisecs);
    year = _max(year, 1);
    month = _max(month, 1);
    day = _max(day, 1);
  ...
  virtual void Status(TStatus &S) {
    ...
    split_time(g_qwStartGameTime, year, month, day, hours, mins, secs,
               milisecs);
    xr_sprintf(S, "%d.%d.%d %d:%d:%d.%d", year, month, day, hours, mins, secs,
               milisecs);
  }
};
...
#define CMD_RADIOGROUPMASK2(p1, p2, p3, p4, p5, p6)                            \
  {                                                                            \
    ...
    static CCC_RadioGroupMask2 x##CCC_RadioGroupMask2(&x##CCC_RadioMask1,      \
                                                      &x##CCC_RadioMask2);     \
  }
...
public:
  ...
  virtual void Info(TInfo &I) {
    xr_sprintf(I, "allows to change bind rotation and position offsets for "
...
void CCC_RegisterCommands() {
  ...
  CMD4(CCC_Integer, "objects_per_client_update", &g_objects_per_client_update,
       1, 65535)
  ...
  // = 0.2f; float anim_channel_factor = 3.f;
  ...
  CMD4(CCC_Float, "hit_anims_block_blend", &ghit_anims_params.block_blend, 0.f,
       1.f);
  ...
  CMD4(CCC_Integer, "dbg_draw_character_bones", &dbg_draw_character_bones,
       FALSE, TRUE);
  CMD4(CCC_Integer, "dbg_draw_character_physics", &dbg_draw_character_physics,
       FALSE, TRUE);
  ...
  1);
  CMD4(CCC_Integer,	"ik_blend_free_foot"
  ...
  */
  ...
  CMD4(CCC_Vector3, "hud_offset_pos", &m_hud_offset_pos,
       Fvector().set(-1000, -1000, -1000), Fvector().set(1000, 1000, 1000));
  CMD4(CCC_Vector3, "hand_offset_pos", &m_hand_offset_pos,
       Fvector().set(-1000, -1000, -1000), Fvector().set(1000, 1000, 1000));
  //#endif // MASTER_GOLD
  ...
  CMD4(CCC_Integer, "dbg_bones_snd_player", &dbg_moving_bones_snd_player, FALSE,
       TRUE);
#endif
  CMD4(CCC_Float, "con_sensitive", &g_console_sensitive, 0.01f, 1.0f);
  CMD4(CCC_Integer, "wpn_aim_toggle", &b_toggle_weapon_aim, 0, 1);
  //	CMD4(CCC_Integer,	"hud_old_style",			&g_old_style_ui_hud,
  //0, 1);
