...
float CItemDeterioration::ffGetValue() {
  if (ef_storage().non_alife().member_item()) {
    const CWeapon *weapon =
        smart_cast<const CWeapon *>(ef_storage().non_alife().member_item());
    if (weapon)
...
#pragma todo(                                                                  \
    ...
    const CSE_ALifeInventoryItem *l_tpALifeInventoryItem =
        smart_cast<const CSE_ALifeInventoryItem *>(
            ef_storage().alife().member_item());
    R_ASSERT2(l_tpALifeInventoryItem, "Non-item object specified for the "
                                      "ItemDeterioration evaluation function");
...
#pragma todo("Dima to Dima : Append EquipmentPreference with non-ALife branch")
    ...
    return (l_tpALifeHumanAbstract->brain().m_cpEquipmentPreferences
...
#pragma todo("Dima to Dima : Append MainWeaponPreference with non-ALife branch")
    ...
    R_ASSERT2(l_tpALifeHumanAbstract,
              "Non-human object in EquipmentPreference evaluation function");
    return (l_tpALifeHumanAbstract->brain().m_cpMainWeaponPreferences
...
#pragma todo("Dima to Dima : Append EquipmentPreference with non-ALife branch")
    ...
    R_ASSERT2(l_tpALifeHumanAbstract,
              "Non-human object in EquipmentPreference evaluation function");
    return (l_tpALifeHumanAbstract->m_cpEquipmentPreferences
                [ef_storage().m_pfEquipmentType->dwfGetDiscreteValue()]);
  }
}
...
#pragma todo("Dima to Dima : Append MainWeaponPreference with non-ALife branch")
    ...
    R_ASSERT2(l_tpALifeHumanAbstract,
              "Non-human object in EquipmentPreference evaluation function");
    return (l_tpALifeHumanAbstract->m_cpMainWeaponPreferences
                [ef_storage().m_pfMainWeaponType->dwfGetDiscreteValue(iFloor(
                    ef_storage().m_pfMainWeaponType->ffGetMaxResultValue() +
                    .5f))]);
  }
