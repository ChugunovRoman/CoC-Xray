...
void CSE_ALifeDynamicObject::detach(
    CSE_ALifeInventoryItem *tpALifeInventoryItem, ALife::OBJECT_IT *I,
...
void add_online_impl(CSE_ALifeDynamicObject *object,
  ...
  for (; I != E; ++I) {
    ...
    // R_ASSERT2				(l_tpALifeInventoryItem,"Non inventory item object
    // has parent?!");
    ...
    Msg("[LSS][%d] Going online [%d][%s][%d] with parent [%d][%s] on '%s'",
        ...
        l_tpALifeInventoryItem->base()->ID, object->ID, object->name_replace(),
        "*SERVER*");
#endif
...
void add_offline_impl(CSE_ALifeDynamicObject *object,
  ...
  for (u32 i = 0, n = saved_children.size(); i < n; ++i) {
    ...
    object->alife().graph().attach(*object, inventory_item, child->m_tGraphID,
                                   true);
  }
