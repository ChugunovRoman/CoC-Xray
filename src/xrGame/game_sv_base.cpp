...
void game_sv_GameState::net_Export_Update(NET_Packet &P, ClientID id_to,
                                          ClientID id){};
...
void game_sv_GameState::Update() {
  if (!g_dedicated_server) {
    if (Level().game) {
      CScriptProcess *script_process = ai().script_engine().script_process(
          ScriptEngine::eScriptProcessorGame);
      if (script_process)
...
void game_sv_GameState::OnEvent(NET_Packet &tNetPacket, u16 type, u32 time,
  ...
  case GAME_EVENT_CREATE_PLAYER_STATE: {
    ...
    R_ASSERT2(CL, make_string("M_CREATE_PLAYER_STATE: client 0x%08x not found",
                              sender.value())
                      .c_str());
...
void game_sv_GameState::AddDelayedEvent(NET_Packet &tNetPacket, u16 type,
                                        u32 time, ClientID sender) {
...
private:
  EventDeleteForClientPredicate &
  operator=(EventDeleteForClientPredicate const &copy) {}
...
shared_str
game_sv_GameState::level_name(const shared_str &server_options) const {
...
shared_str
game_sv_GameState::parse_level_version(const shared_str &server_options) {
...
shared_str
game_sv_GameState::parse_level_name(const shared_str &server_options) {
  string64 l_name = "";
