...
void server_site::stop_transfer_sessions(
    buffer_vector<dst_src_pair_t> const
        &tsessions) // notifies sending_rejected_by_peer
{
...
void server_site::stop_obsolete_receivers() {
  ...
  buffer_vector<ClientID> to_stop_receivers(
      _alloca(m_receivers.size() * sizeof(ClientID)), m_receivers.size());
...
void server_site::start_transfer_file(
    shared_str const &file_name, ClientID const &to_client,
...
void server_site::start_transfer_file(u8 *data_ptr, u32 const data_size,
                                      ClientID const &to_client,
                                      ClientID const &from_client,
                                      sending_state_callback_t &tstate_callback,
                                      u32 const user_param) {
...
void server_site::start_transfer_file(
    ...
    ClientID const &to_client, ClientID const &from_client,
...
filereceiver_node *
server_site::start_receive_file(shared_str const &file_name,
                                ClientID const &from_client,
...
filereceiver_node *
server_site::start_receive_file(CMemoryWriter &mem_writer,
                                ClientID const &from_client,
                                receiving_state_callback_t &rstate_callback) {
  receiving_sessions_t::iterator temp_iter = m_receivers.find(from_client);
  if (temp_iter != m_receivers.end()) {
...
void client_site::start_transfer_file(
    shared_str const &file_name, sending_state_callback_t &tstate_callback) {
  if (is_transfer_active()) {
...
void client_site::start_transfer_file(u8 *data, u32 size,
  ...
  m_transfering = xr_new<filetransfer_node>(data, size, data_min_chunk_size,
                                            tstate_callback, size_to_allocate);
}
...
filereceiver_node *
client_site::start_receive_file(shared_str const &file_name,
                                ClientID const &from_client,
                                receiving_state_callback_t &rstate_callback) {
...
filereceiver_node *
client_site::start_receive_file(CMemoryWriter &mem_writer,
                                ClientID const &from_client,
                                receiving_state_callback_t &rstate_callback) {
  if (is_receiving_active(from_client)) {
    Msg("! ERROR: CL: file already receiving from client [%d]", from_client);
...
void client_site::dbg_update_statgraph() {
  ...
  if (m_transfering) {
    m_stat_graph->AppendItem(float(m_transfering->get_chunk_size()), 0xff00ff00,
                             0);
  }
}
#endif
