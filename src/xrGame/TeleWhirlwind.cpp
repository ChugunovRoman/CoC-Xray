...
CTelekineticObject *CTeleWhirlwind::activate(CPhysicsShellHolder *obj,
  ...
  if (inherited::activate(obj, strength, height, max_time_keep, rot)) {
    CTeleWhirlwindObject *o =
        smart_cast<CTeleWhirlwindObject *>(objects.back());
    VERIFY(o);
...
void CTeleWhirlwind::clear_notrelevant() {
  ...
  objects.erase(std::remove_if(objects.begin(), objects.end(), &RemovePred),
                objects.end());
}
...
bool CTeleWhirlwindObject::init(CTelekinesis *tele, CPhysicsShellHolder *obj,
                                float s, float h, u32 ttk, bool rot) {
...
bool CTeleWhirlwindObject::destroy_object(const Fvector dir, float val) {
  ...
  if (D) {
    ...
    if (PP) {
      ...
      PP->StartParticles(m_telekinesis->destroing_particles(), root,
                         Fvector().set(0, 1, 0),
...
void CTeleWhirlwindObject::raise(float step) {
  ...
  for (u16 element = 0; element < element_number; ++element) {
    ...
    E->applyForce(force.x,
                  force.y + get_object()->EffectiveGravity() * E->getMass(),
                  force.z);
  }
  Fvector dist;
