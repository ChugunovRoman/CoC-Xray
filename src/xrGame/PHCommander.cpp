...
void CPHCommander::add_call_threadsafety(CPHCondition *condition,
                                         CPHAction *action) {
  lock.Enter();
...
void CPHCommander::remove_call(PHCALL_I i) {
  ...
  if (esc && cfa) {
    ...
    if (m > EPS_S)
      f.mul(1.f / m);
    // Msg(" const force removed: force: %f,  remove step: %d  world step: %d
    // ,dir(%f,%f,%f) ", m, esc->step(), (u32)physics_world()->StepsNum(), f.x,
    // f.y , f.z );
  }
#endif
  delete_call(*i);
...
void CPHCommander::remove_calls(CPHReqComparerV *cmp_object) {
  m_calls.erase(
      std::remove_if(m_calls.begin(), m_calls.end(), SRemoveRped(cmp_object)),
      m_calls.end());
}
