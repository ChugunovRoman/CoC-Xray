...
private:
  bool aim_target(Fvector &my_position, Fvector &aim_target,
...
public:
  ...
  void SetFirePointLookAngles(const Fvector &tPosition, float &yaw,
                              float &pitch, Fvector const &look_position,
                              const CGameObject *object = 0);
  void SetDirectionLook();
  void SetLessCoverLook(const LevelGraph::CVertex *tpNode,
                        bool bDifferenceLook);
  void SetLessCoverLook(const LevelGraph::CVertex *tpNode,
                        float fMaxHeadTurnAngle, bool bDifferenceLook);
