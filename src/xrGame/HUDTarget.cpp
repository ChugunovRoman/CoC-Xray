...
void CHUDTarget::CursorOnFrame() {
  ...
  if (Level().CurrentEntity()) {
    PP.RQ.O = 0;
    PP.RQ.range =
        g_pGamePersistent->Environment().CurrentEnv->far_plane * 0.99f;
    PP.RQ.element = -1;
...
void CHUDTarget::Render() {
  ...
  BOOL b_do_rendering =
      (psHUD_Flags.is(HUD_CROSSHAIR | HUD_CROSSHAIR_RT | HUD_CROSSHAIR_RT2));
  ...
  if (psHUD_Flags.test(HUD_INFO)) {
    bool const is_poltergeist =
        PP.RQ.O && !!smart_cast<CPoltergeist *>(PP.RQ.O);
        ...
        if (our_inv_owner && others_inv_owner) {
          ...
          if (fuzzyShowInfo > 0.5f) {
            ...
            F->SetColor(subst_alpha(
                C, u8(iFloor(255.f * (fuzzyShowInfo - 0.5f) * 2.f))));
