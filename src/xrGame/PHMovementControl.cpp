...
CPHMovementControl::CPHMovementControl(CObject *parent) {
  ...
  if (debug_output().ph_dbg_draw_mask1().test(ph_m1_DbgTrackObject) &&
      (!!pObject->cName()) &&
      stricmp(debug_output().PH_DBG_ObjectTrackName(), *pObject->cName()) ==
          0) {
...
void CPHMovementControl::Calculate(Fvector &vAccel, const Fvector &camDir,
                                   float /**ang_speed/**/, float jump,
  ...
  /*
     ...
     *last_material=GMLib.GetMaterialByIdx(m_character->LastMaterialIDX()); if(
     ...
     DefineCollisionHitType( mat_injurios ) );
                  gcontact_HealthLost+=Device.fTimeDelta*GMLib.GetMaterialByIdx(
     mat_injurios )->fInjuriousSpeed;
          }
  ...
  */
  ...
  if (cdi->HitCallback())
    cdi->HitCallback()->call((m_character->PhysicsRefObject()), fMinCrashSpeed,
                             fMaxCrashSpeed, fContactSpeed, gcontact_HealthLost,
                             CollisionDamageInfo());
...
void CPHMovementControl::UpdateCollisionDamage() {
  ...
  //		gcontact_HealthLost =
  //			((fContactSpeed-fMinCrashSpeed))/(fMaxCrashSpeed-fMinCrashSpeed);
  //		m_character->SetHitType( DefineCollisionHitType(
  // m_character->LastMaterialIDX() ) );
  //	}
  //}
}
...
bool CPHMovementControl::MakeJumpPath(
    xr_vector<DetailPathManager::STravelPathPoint> &out_path, u32 &travel_point,
...
void CPHMovementControl::Calculate(
    const xr_vector<DetailPathManager::STravelPathPoint> &in_path, float speed,
  ...
  if (debug_output().ph_dbg_draw_mask1().test(ph_m1_DbgTrackObject) &&
    ...
    Msg("CPHMovementControl::Calculate in %s (Object Position) %f,%f,%f",
        debug_output().PH_DBG_ObjectTrackName(), pObject->Position().x,
        pObject->Position().y, pObject->Position().z);
    Msg("CPHMovementControl::Calculate in %s (CPHMovementControl::vPosition) "
        "%f,%f,%f",
...
void CPHMovementControl::PathNearestPoint(
  ...
  for (int i = 0; i < m_path_size - 1; ++i) {
    ...
    {
      if (after_line) // after previous line && befor this line = near first
                      // point
      {
  ...
  if (ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject) && (!!pObject->cName()) &&
    ...
    Msg("CPHMovementControl::Calculate out %s (Object Position) %f,%f,%f",
        ...
        pObject->Position().z);
    Msg("CPHMovementControl::Calculate out %s (CPHMovementControl::vPosition) "
        "%f,%f,%f",
...
void CPHMovementControl::PathNearestPointFindUp(
  ...
  for (int i = m_start_index; i < m_path_size - 1; ++i) {
    ...
    if (from_first_dir < 0.f) // before this line
    {
      ...
      if (after_line) // after previous line && before this line = near first
                      // point
      {
        if (temp < m_path_distance) {
          m_path_distance = temp;
...
void CPHMovementControl::PathNearestPointFindDown(
  ...
  for (int i = m_start_index; i > 1; --i) {
    ...
    if (from_second_dir > 0.f) // befor this line
    {
      ...
      if (after_line) // after previous line && befor this line = near second
                      // point (going down)
      {
        if (temp < m_path_distance) {
          m_path_distance = temp;
...
void CPHMovementControl::Load(LPCSTR section) {
  ...
  // (section,"ph_foot_center"
  // ); Fvector	vFOOT_size	= pSettings->r_fvector3	(section,"ph_foot_size"
  // ); bb.set	(vFOOT_center,vFOOT_center); bb.grow(vFOOT_size); SetFoots
  // (vFOOT_center,vFOOT_size);
  ...
  if (pSettings->line_exist(section, "actor_restrictor"))
    SetRestrictionType(ERestrictionType(
        pSettings->r_token(section, "actor_restrictor", retrictor_types)));
  fCollisionDamageFactor =
      READ_IF_EXISTS(pSettings, r_float, section, "ph_collision_damage_factor",
                     fCollisionDamageFactor);
  ...
  // (section,"ph_friction_air"
  // ); gf					= pSettings->r_float
  // (section,"ph_friction_ground");
  // wf					= pSettings->r_float
  // (section,"ph_friction_wall"
...
void CPHMovementControl::SetPosition(const Fvector &P) {
  ...
  if (ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject) && (!!pObject->cName()) &&
    ...
    Msg("CPHMovementControl::SetPosition %s (Object Position) %f,%f,%f",
        PH_DBG_ObjectTrackName(), pObject->Position().x, pObject->Position().y,
        pObject->Position().z);
    Msg("CPHMovementControl::SetPosition %s (CPHMovementControl::vPosition) "
        "%f,%f,%f",
        PH_DBG_ObjectTrackName(), vPosition.x, vPosition.y, vPosition.z);
  }
#endif
...
bool CPHMovementControl::TryPosition(Fvector &pos) {
  ...
  if (ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject) && (!!pObject->cName()) &&
    ...
    Msg("CPHMovementControl::TryPosition %s (Object Position) %f,%f,%f",
        PH_DBG_ObjectTrackName(), pObject->Position().x, pObject->Position().y,
        pObject->Position().z);
    Msg("CPHMovementControl::TryPosition %s (CPHMovementControl::vPosition) "
        "%f,%f,%f",
        PH_DBG_ObjectTrackName(), vPosition.x, vPosition.y, vPosition.z);
  }
#endif
  if (m_character->b_exist) {
    bool ret =
        m_character->TryPosition(pos, b_exect_position) && !bExernalImpulse;
...
void CPHMovementControl::GetPosition(Fvector &P) {
  ...
  if (ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject) && (!!pObject->cName()) &&
    ...
    Msg("CPHMovementControl::GetPosition %s (Object Position) %f,%f,%f",
        PH_DBG_ObjectTrackName(), pObject->Position().x, pObject->Position().y,
        pObject->Position().z);
    Msg("CPHMovementControl::GetPosition %s (CPHMovementControl::vPosition) "
        "%f,%f,%f",
        PH_DBG_ObjectTrackName(), vPosition.x, vPosition.y, vPosition.z);
  }
#endif
...
void CPHMovementControl::AllocateCharacterObject(CharacterType type) {
  ...
  // ; break; case ai:		m_character = xr_new<CPHAICharacter>	() ;
  // break;
  case ai:
...
void CPHMovementControl::PHCaptureObject(CPhysicsShellHolder *object,
  ...
  m_capture = phcapture_create(m_character, object,
                               static_cast<NearestToPointCallback *>(cb));
...
Fvector CPHMovementControl::PHCaptureGetNearestElemPos(
  ...
  R_ASSERT3((object->m_pPhysicsShell != NULL), "NO Phisics Shell for object ",
            *object->cName());
...
void CPHMovementControl::Jump(const Fvector &end_point, float time) {
  // vPosition
  Jump(smart_cast<CGameObject *>(m_character->PhysicsRefObject())->Position(),
       end_point, time);
}

void CPHMovementControl::Jump(const Fvector &start_point,
                              const Fvector &end_point, float time) {
  Fvector velosity;
...
void CPHMovementControl::GetJumpMinVelParam(Fvector &min_vel, float &time,
                                            JumpType &type,
...
float CPHMovementControl::JumpMinVelTime(const Fvector &end_point) {
  return ThrowMinVelTime(
      Fvector().sub(end_point,
                    smart_cast<CGameObject *>(m_character->PhysicsRefObject())
                        ->Position()),
      physics_world()->Gravity());
}

void CPHMovementControl::GetJumpParam(Fvector &velocity, JumpType &type,
                                      const Fvector &end_point, float time) {
  Fvector velosity;
  velosity.sub(
...
void CPHMovementControl::CreateCharacter() {
  ...
  m_character->SetCollisionDamageFactor(fCollisionDamageFactor *
                                        fCollisionDamageFactor);
  trying_times[0] = trying_times[1] = trying_times[2] = trying_times[3] =
      u32(-1);
...
void CPHMovementControl::ApplyHit(const Fvector &dir, const float P,
  ...
  if ((Environment() == CPHMovementControl::peOnGround ||
    ...
    case ALife::eHitTypeWound_2:;
      ...
      // SetVelocity(Fvector().set(0,0,0))	;break;//stop
    default:
      NODEFAULT;
    }
  }
...
void CPHMovementControl::UpdateObjectBox(CPHCharacter *ach) {
  ...
  R *= poses_dir.dotproduct(
      plane_cam); //(poses_dir.x*plane_cam.x+poses_dir.y*plane_cam.z);
...
// static void	non_interactive_collide_callback( bool& do_collide, bool bo1,
// dContact& c, SGameMtl* material_1, SGameMtl* material_2 )
//{
//	if( !do_collide )
//		return;
//
...
// PHRetrieveGeomUserData(	bo1 ? c.geom.g1 : c.geom.g2 );
//	//dxGeomUserData	*oposite_data		=
// PHRetrieveGeomUserData( bo1 ? c.geom.g2 : c.geom.g1 ) ; 	VERIFY( my_data
// );
//
//	dBodyID b_oposite = bo1 ? dGeomGetBody(c.geom.g2) :
// dGeomGetBody(c.geom.g1);
//	//dBodyID b_mine = bo1 ? dGeomGetBody(c.geom.g2) :
// dGeomGetBody(c.geom.g1); 	if(!b_oposite)
//	{
//		do_collide = false;
//		return;
//	}
...
//	/*
//	dJointID contact_joint	=dJointCreateContactSpecial(0, ContactGroup,
//&c);// dJointCreateContact(0, ContactGroup, &c);// 	CPHObject* obj =
//(CPHObject*)my_data->ph_object; 	VERIFY( obj ); 	VERIFY(
// obj->Island().DActiveIsland() != &(obj->Island()) ); 	VERIFY(
//! obj->Island().IsActive() );
...
void CPHMovementControl::actor_calculate(Fvector &vAccel, const Fvector &camDir,
                                         float ang_speed, float jump, float dt,
                                         bool bLight) {
