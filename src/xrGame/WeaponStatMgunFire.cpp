...
void CWeaponStatMgun::UpdateFire() {
  ...
  if (m_overheat_enabled) {
    ...
    if (m_overheat_value >= 100.f) {
      if (!p_overheat) {
        p_overheat =
            CParticlesObject::Create(m_overheat_particles.c_str(), FALSE);
        Fmatrix pos;
        pos.set(get_ParticlesXFORM());
        pos.c.set(get_CurrentFirePoint());
        p_overheat->SetXFORM(pos);
...
void CWeaponStatMgun::OnShot() {
  ...
  FireBullet(m_fire_pos, m_fire_dir, fireDispersionBase, *m_Ammo, Owner()->ID(),
             ID(), SendHitAllowed(Owner()), ::Random.randI(0, 30));
