...
demoplay_control::demoplay_control() {
  m_onround_start =
      fastdelegate::MakeDelegate(this, &demoplay_control::on_round_start_impl);
...
void __stdcall demoplay_control::on_round_start_impl(u32 message, u32 subtype,
                                                     NET_Packet &packet) {
  process_action();
}

void __stdcall demoplay_control::on_kill_impl(u32 message, u32 subtype,
                                              NET_Packet &packet) {
  u16 msg_type;
...
void __stdcall demoplay_control::on_die_impl(u32 message, u32 subtype,
                                             NET_Packet &packet) {
  u16 msg_type;
  packet.r_begin(msg_type);
  R_ASSERT(msg_type == M_GAMEMESSAGE);
...
void __stdcall demoplay_control::on_artefactdelivering_impl(
    u32 message, u32 subtype, NET_Packet &packet) {
  u16 msg_type;
  packet.r_begin(msg_type);
  R_ASSERT(msg_type == M_GAMEMESSAGE);
  u32 game_msg_type;
...
void __stdcall demoplay_control::on_artefactcapturing_impl(u32 message,
                                                           u32 subtype,
                                                           NET_Packet &packet) {
  u16 msg_type;
  packet.r_begin(msg_type);
