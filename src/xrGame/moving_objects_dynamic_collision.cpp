...
IC MagicBox3 &
moving_objects::continuous_box(moving_object *object, const Fvector &position,
                               MagicBox3 &result,
...
struct collision_predicate {
  ...
  IC bool
  operator()(const moving_objects::COLLISION_TIME &collision_time) const {
...
void moving_objects::resolve_collision_previous(
    boxes &current, moving_object *object0, moving_object *object1,
    possible_actions &action) const {
  boxes start;
...
void moving_objects::resolve_collision(boxes &current, moving_object *object0,
  ...
  bool first_time =
      (std::find_if(m_previous_collisions.begin(), m_previous_collisions.end(),
  ...
  if (object1->action() == moving_object::action_wait) {
    ...
    action = possible_actions(
        action ^ (possible_action_1_can_wait_2 | possible_action_2_can_wait_1));
    return;
  }
