...
void SBullet::Init(const Fvector &position, const Fvector &direction,
                   float starting_speed, float power,
                   //.				   float power_critical,
                   float impulse, u16 sender_id, u16 sendersweapon_id,
...
void CBulletManager::Load() {
  ...
  for (int k = 0; k < cnt; ++k) {
    ...
    m_WhineSounds.back().create(_GetItem(whine_sounds, k, tmp), st_Effect,
                                sg_SourceType);
  }
...
void CBulletManager::PlayWhineSound(SBullet *bullet, CObject *object,
                                    const Fvector &pos) {
...
void CBulletManager::AddBullet(
    const Fvector &position, const Fvector &direction, float starting_speed,
    float power,
    //.							   float power_critical,
    float impulse, u16 sender_id, u16 sendersweapon_id,
    ALife::EHitType e_hit_type, float maximum_distance,
...
void CBulletManager::UpdateWorkload() {
  ...
  for (u16 j = u16(e - i); i != e; ++i, --j) {
    ...
    VERIFY(j > 0);
    RegisterEvent(EVENT_REMOVE, FALSE, &*i, Fvector().set(0, 0, 0), dummy,
                  j - 1);
  }
}
...
static Fvector trajectory_velocity(Fvector const &start_velocity,
  ...
  if (fall_down_time < 0.f) {
    ...
    if (!fis_zero(xz_velocity.square_magnitude())) {
      return (
          parabolic_velocity(start_velocity, gravity, air_resistance, time));
    }
...
static Fvector trajectory_position(Fvector const &start_position,
  ...
  Fvector const &gravity =
      base_gravity; // g_use_new_ballistics ?
                    // Fvector(base_gravity).mul(_sqr(factor)) : base_gravity;
  float const &air_resistance =
      base_air_resistance; // g_use_new_ballistics ? base_air_resistance*factor
                           // : base_air_resistance;
  Fvector const &start_velocity =
      base_start_velocity; // g_use_new_ballistics ?
  ...
  if (fall_down_time < 0.f) {
    Fvector const xz_velocity =
        Fvector().set(start_velocity.x, 0.f, start_velocity.z);
    if (!fis_zero(xz_velocity.square_magnitude()))
      return (parabolic_position(start_position, start_velocity, gravity,
                                 air_resistance, time));
  ...
  Fvector const parabolic_position = ::parabolic_position(
      start_position, start_velocity, gravity, air_resistance, parabolic_time);
...
static float trajectory_pick_error(float const low, float const high,
  ...
  Fvector const target =
      trajectory_position(position, velocity, gravity, air_resistance, high);
  Fvector const max_error = trajectory_position(position, velocity, gravity,
                                                air_resistance, max_error_time);
  ...
  float cosine_alpha =
      _max(-1.f, _min(start_to_max_error.dotproduct(start_to_target), 1.f));
  float sine_alpha = _sqrt(1.f - _sqr(cosine_alpha));
  return (magnitude * sine_alpha);
}

static float trajectory_select_pick_gravity(SBullet &bullet, float start_low,
                                            float const high,
                                            Fvector const &gravity,
                                            float const air_resistance) {
...
static float trajectory_select_pick_parabolic(SBullet &bullet,
                                              float const start_low, float high,
                                              Fvector const &gravity,
                                              float const air_resistance) {
  float const max_test_distance = bullet.max_dist - bullet.fly_dist;
  Fvector const start =
  ...
  while (!fsimilar(low, high)) {
    ...
    float const distance =
        start.distance_to(intermediate) + intermediate.distance_to(target);
...
static bool trajectory_select_pick_ranges(float &result, SBullet &bullet,
  ...
  result = trajectory_select_pick_gravity(bullet, fall_down_time, high, gravity,
                                          air_resistance);
  return (false);
}

static float trajectory_select_pick_time(SBullet &bullet, float const start_low,
                                         float high, Fvector const &gravity,
                                         float const air_resistance) {
  ...
  while (!fsimilar(low, high)) {
    float distance =
        trajectory_pick_error(start_low, check_time, bullet.start_position,
...
static void update_bullet(SBullet &bullet, bullet_test_callback_data &data,
...
BOOL CBulletManager::firetrace_callback(collide::rq_result &result,
  ...
  if (!result.O) {
    ...
    bullet_manager.RegisterEvent(EVENT_HIT, FALSE, &bullet, collide_position,
                                 result, triangle.material);
    return (FALSE);
  }
...
static bool try_update_bullet(SBullet &bullet, Fvector const &gravity,
...
bool CBulletManager::process_bullet(collide::rq_results &storage,
                                    SBullet &bullet, u32 const delta_time) {
  ...
  for (;;) {
    for (;;) {
      ...
      float time = trajectory_select_pick_time(bullet, low, high, gravity,
                                               air_resistance);
      if (time == low)
        return (false);

      float safe_time = time;
      VERIFY2(safe_time <= high,
              make_string("safe_time[%f], high[%f]", safe_time, high));
      if (!trajectory_check_error(previous_position, storage, bullet, low, time,
                                  gravity, air_resistance)) {
        VERIFY2(safe_time >= time,
...
float SqrDistancePointToSegment(const Fvector &pt, const Fvector &orig,
                                const Fvector &dir) {
  Fvector diff;
  diff.sub(pt, orig);
...
void CBulletManager::Render() {
  ...
  UIRender->StartPrimitive((u32)bullet_num * 12, IUIRender::ptTriList,
                           IUIRender::pttLIT);

  for (BulletVecIt it = m_BulletsRendered.begin();
       it != m_BulletsRendered.end(); it++) {
    SBullet *bullet = &(*it);
    ...
    float dist2segSqr = SqrDistancePointToSegment(Device.vCameraPosition,
                                                  bullet->bullet_pos, tracer);
    //---------------------------------------------
...
void CBulletManager::RegisterEvent(EventType Type, BOOL _dynamic,
  ...
  case EVENT_HIT: {
    ...
    if (_dynamic) {
      ...
      E.Repeated = (R.O->ID() == E.bullet.targetID);
      // Alun: Enable BonePassBullet for singleplayer, good for bullets going
