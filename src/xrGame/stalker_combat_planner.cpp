...
void CStalkerCombatPlanner::on_best_cover_changed(
    const CCoverPoint *new_cover, const CCoverPoint *old_cover) {
...
void CStalkerCombatPlanner::setup(CAI_Stalker *object,
  ...
  CScriptActionPlanner::m_storage.set_property(eWorldPropertyUseSuddenness,
                                               true);
  CScriptActionPlanner::m_storage.set_property(eWorldPropertyUseCrouchToLookOut,
                                               true);
...
void CStalkerCombatPlanner::add_evaluators() {
  ...
  add_evaluator(eWorldPropertyPanic,
  ...
  add_evaluator(eWorldPropertyInCover,
                xr_new<CStalkerPropertyEvaluatorMember>(
                    (CPropertyStorage *)0, eWorldPropertyInCover, true, true,
                    "in cover"));
  add_evaluator(eWorldPropertyLookedOut,
                xr_new<CStalkerPropertyEvaluatorMember>(
                    (CPropertyStorage *)0, eWorldPropertyLookedOut, true, true,
                    "looked out"));
  add_evaluator(eWorldPropertyPositionHolded,
                xr_new<CStalkerPropertyEvaluatorMember>(
                    (CPropertyStorage *)0, eWorldPropertyPositionHolded, true,
                    true, "position holded"));
  add_evaluator(eWorldPropertyEnemyDetoured,
                xr_new<CStalkerPropertyEvaluatorMember>(
                    (CPropertyStorage *)0, eWorldPropertyEnemyDetoured, true,
                    true, "enemy detoured"));
  add_evaluator(eWorldPropertyUseSuddenness,
                xr_new<CStalkerPropertyEvaluatorUseSuddenness>(
                    m_object, "use suddenness"));
...
void CStalkerCombatPlanner::add_actions() {
  ...
  action =
      xr_new<CStalkerActionRetreatFromEnemy>(m_object, "retreat_from_enemy");
  add_effect(action, eWorldPropertyPureEnemy, false);
  add_operator(eWorldOperatorRetreatFromEnemy, action);

  action =
      xr_new<CStalkerActionGetReadyToKill>(true, m_object, "get_ready_to_kill");
  add_condition(action, eWorldPropertyCriticallyWounded, false);
  ...
  action =
      xr_new<CStalkerActionKillEnemy>(m_object, "kill_if_critically_wounded");
  add_condition(action, eWorldPropertyCriticallyWounded, false);
  add_condition(action, eWorldPropertyDangerGrenade, false);
  add_condition(action, eWorldPropertyUseSuddenness, false);
  add_condition(action, eWorldPropertyReadyToKill, true);
