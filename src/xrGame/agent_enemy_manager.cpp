        ...
        if (my_distance < best) {
          ...
          for (; J; J &= J - 1) {
            ...
            xr_vector<u32>::iterator ii =
                std::find((*j)->enemies().begin(), (*j)->enemies().end(),
                          (*I)->selected_enemy());
            ...
            float my_to_his = evaluate(
                &(*I)->object(), m_enemies[(*j)->selected_enemy()].m_object);
            float his_to_my = evaluate(
                &(*j)->object(), m_enemies[(*I)->selected_enemy()].m_object);
...
template <typename T>
IC void
CAgentEnemyManager::setup_mask(xr_vector<T> &objects, CMemberEnemy &enemy,
...
IC void
CAgentEnemyManager::setup_mask(CMemberEnemy &enemy,
                               const squad_mask_type &non_combat_members) {
...
void CAgentEnemyManager::assign_wounded() {
  ...
  while (population(assigned) < combat_member_count) {
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      for (; I != E; ++I) {
        ...
        for (; J; J &= J - 1) {
          ...
          CAgentMemberManager::iterator i = object().member().member(K);
          float distance_sqr = (*i)->object().Position().distance_to_sqr(
              (*I).m_object->Position());
    ...
    if (!enemy) {
      ...
      Msg("wounded enemies(%d):", m_enemies.size());
      {
        ...
        for (; I != E; ++I)
          Msg("  [%s][0x%08x][0x%08x][%.2f]", *(*I).m_object->cName(),
              (*I).m_mask.get(), (*I).m_distribute_mask.get(),
              (*I).m_probability);
      }
    ...
    // since the last enemy went offline and he knows nothing about the second
    // one so, in this case we just need to stop iterating since nest procedure
    // (setup_enemy_masks) will make the second enemy known for the soldier and
    // on its update he will select it
    if (!enemy)
      return;

    //		Msg							("wounded enemy [%s] is assigned to
    //member
  ...
  //	Msg								("[%6d] assigned
  //= %x",Device.dwTimeGlobal,assigned); 	ENEMIES::iterator
...
struct wounded_predicate {
  ...
  IC bool
  operator()(const CAgentEnemyManager::WOUNDED_ENEMY &wounded_enemy) const {
...
ALife::_OBJECT_ID
CAgentEnemyManager::wounded_processor(const CEntityAlive *object) {
...
void CAgentEnemyManager::wounded_processor(
    const CEntityAlive *object, const ALife::_OBJECT_ID &wounded_processor_id) {
  VERIFY(std::find_if(m_wounded.begin(), m_wounded.end(),
                      find_wounded_predicate(object)) == m_wounded.end());
  m_wounded.push_back(
      std::make_pair(object, std::make_pair(wounded_processor_id, false)));
}

void CAgentEnemyManager::wounded_processed(const CEntityAlive *object,
                                           bool value) {
  VERIFY(value);
  WOUNDED_ENEMIES::iterator I = std::find_if(m_wounded.begin(), m_wounded.end(),
