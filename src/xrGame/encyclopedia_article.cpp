...
void CEncyclopediaArticle::load_shared(LPCSTR) {
  ...
  if (ltx) {
    ...
    tex_rect.y2 =
        float(pSettings->r_u32(ltx, "inv_grid_height") * INV_GRID_HEIGHT);
  ...
  if (data()->image.GetShader() && data()->image.GetShader()->inited()) {
    ...
    if (r.width() < minSize) {
      ...
      r.x2 += dx;
      data()->image.SetTextureOffset(dx / 2,
                                     data()->image.GetTextureOffeset()[1]);
    }

    if (r.height() < minSize) {
      float dy = minSize - r.height();
      r.y2 += dy;
      data()->image.SetTextureOffset(data()->image.GetTextureOffeset()[0],
                                     dy / 2);
    }
