...
// T_INDEX, 					обязательно имеет конструктор с параметрами
// (T_INDEX index, T_ID id,
// LPCSTR r1, ..., LPCSTR rN) 					N = ITEM_REC_NUM - число
// доп. параметров в ITEM_DATA
...
public:
  ...
  static const T_ID IndexToId(T_INDEX index, T_ID default_id = NULL,
                              bool no_assert = false) {
...
const typename ITEM_DATA *CSINI_IdToIndex::GetByIndex(T_INDEX index,
  ...
  if ((size_t)index >= m_pItemDataVector->size()) {
    if (!no_assert)
      Debug.fatal(DEBUG_INFO, "item by index not found in section %s, line %s",
