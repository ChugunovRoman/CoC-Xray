...
struct HUD_SOUND_ITEM {
  ...
  static void LoadSound(LPCSTR section, LPCSTR line, ref_sound &hud_snd,
                        int type = sg_SourceType, float *volume = NULL,
                        float *delay = NULL);
  ...
  static void PlaySound(HUD_SOUND_ITEM &snd, const Fvector &position,
...
class HUD_SOUND_COLLECTION {
  ...
  // HUD_SOUND_ITEM*				FindSoundItem	(	LPCSTR alias,
  // bool b_assert);
public:
  ~HUD_SOUND_COLLECTION();
#ifdef LAYERED_SND_SHOOT
  HUD_SOUND_COLLECTION() : m_alias(NULL){};
  shared_str m_alias; // Alundaio: For use when it's part of a layered
                      // Collection
#endif
  xr_vector<HUD_SOUND_ITEM> m_sound_items; // Alundaio: made public

  HUD_SOUND_ITEM *
  FindSoundItem(LPCSTR alias,
  ...
  void LoadSound(LPCSTR section, LPCSTR line, LPCSTR alias,
...
public:
  ...
  void LoadSound(LPCSTR section, LPCSTR line, LPCSTR alias,
                 bool exclusive = false, int type = sg_SourceType);
  void LoadSound(CInifile const *ini, LPCSTR section, LPCSTR line, LPCSTR alias,
                 bool exclusive = false, int type = sg_SourceType);
  void SetPosition(LPCSTR alias, const Fvector &pos);
};
#endif
