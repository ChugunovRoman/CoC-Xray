...
void CCoverEvaluatorCloseToEnemy::evaluate_cover(const CCoverPoint *cover_point,
                                                 float weight) {
  ...
  // float					cover_value =
  // ai().level_graph().cover_in_direction(y,cover_point->level_vertex_id()); if
  // (cover_value >= m_best_value) 	return;
...
void CCoverEvaluatorCloseToEnemy::evaluate_smart_cover(
    smart_cover::cover const *smart_cover, float const &weight) {}

//////////////////////////////////////////////////////////////////////////
// CCoverEvaluatorFarFromEnemy
//////////////////////////////////////////////////////////////////////////

void CCoverEvaluatorFarFromEnemy::evaluate_cover(const CCoverPoint *cover_point,
                                                 float weight) {
  float enemy_distance = m_enemy_position.distance_to(cover_point->position());
  //	float					my_distance		=
  // m_start_position.distance_to(cover_point->position());
...
void CCoverEvaluatorBest::evaluate_cover(const CCoverPoint *cover_point,
  ...
  if (ai().level_graph().neighbour_in_direction(direction,
                                                cover_point->level_vertex_id()))
    value += 10.f;
...
//	Msg								("Loophole is
//NULL CCoverEvaluatorBest::evaluate_cover");
#endif // #if 1
}
...
void CCoverEvaluatorAngle::initialize(const Fvector &start_position,
  ...
  for (float alpha = 0.f, step = PI_MUL_2 / 360.f; alpha < PI_MUL_2;
    ...
    float low_value = ai().level_graph().compute_low_square(alpha, PI_DIV_2,
                                                            m_level_vertex_id);
