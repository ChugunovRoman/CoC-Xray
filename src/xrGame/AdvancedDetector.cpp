...
void CAdvancedDetector::UpdateAf() {
  ...
  if (af_info.snd_time > af_info.cur_period) {
    af_info.snd_time = 0;
    HUD_SOUND_ITEM::PlaySound(item_type->detect_snds, Fvector().set(0, 0, 0),
                              this, true, false);
...
void CUIArtefactDetectorAdv::update() {
  ...
  /*
     ...
     (m_curr_ang_speed,-2.0f,2.0f);
          float _add						=
     m_curr_ang_speed*Device.fTimeDelta;

          m_cur_y_rot						+= _add;
  */
  m_cur_y_rot = angle_inertion_var(m_cur_y_rot, dest_y_rot, PI_DIV_4, PI_MUL_4,
