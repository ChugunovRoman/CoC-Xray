...
CSightManager::CSightManager(CAI_Stalker *object)
    : inherited(object), m_enabled(true), m_turning_in_place(false),
...
void CSightManager::reload(LPCSTR section) {
  m_max_left_angle = deg2rad(READ_IF_EXISTS(pSettings, r_float, section,
                                            "max_left_torso_angle", 90.f));
  m_max_right_angle = deg2rad(READ_IF_EXISTS(pSettings, r_float, section,
                                             "max_right_torso_angle", 60.f));
}
...
static inline float select_speed(float const distance, float const speed,
...
void CSightManager::Exec_Look(float time_delta) {
  ...
  if (g_ai_dbg_sight)
    Msg("%6d [%s] before body[%f]->[%f], head[%f]->[%f]", Device.dwTimeGlobal,
        object().cName().c_str(), body.current.yaw, body.target.yaw,
...
void CSightManager::process_action(float const time_delta) {
  ...
  //		Msg							("[%6d][%6d] [%f] + [%f] =
  //[%f]
  //([%f])",  Device.dwFrame, Device.dwTimeGlobal, m_current.m_head.m_factor,
  // s_factor_lerp_speed*time_delta,		lerp (
  // m_current.m_head.m_factor, factors.x, s_factor_lerp_speed*time_delta ),
  // factors.x ); 		Msg
  //("[%6d][%6d] [%f] + [%f] = [%f] ([%f])",  Device.dwFrame,
  // Device.dwTimeGlobal, m_current.m_shoulder.m_factor,
  // s_factor_lerp_speed*time_delta,		lerp (
  // m_current.m_shoulder.m_factor, factors.y, s_factor_lerp_speed*time_delta ),
  // factors.y ); 		Msg
  //("[%6d][%6d] [%f] + [%f] = [%f] ([%f])",  Device.dwFrame,
  // Device.dwTimeGlobal, m_current.m_spine.m_factor,
  // s_factor_lerp_speed*time_delta,		lerp (
  // m_current.m_spine.m_factor, factors.z, s_factor_lerp_speed*time_delta ),
  // factors.z );
  //	}
  ...
  m_current.m_shoulder.m_factor = lerp(m_current.m_shoulder.m_factor, factors.y,
                                       s_factor_lerp_speed * time_delta);
  VERIFY(_valid(m_current.m_shoulder.m_factor));
...
static void slerp_rotations(float const time_delta, float const angular_speed,
                            Fmatrix &current, Fmatrix const &target) {
