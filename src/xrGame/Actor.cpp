...
CActor::CActor() : CEntityAlive(), current_ik_cam_shift(0) {
  ...
  // Cameras
  cameras[eacFirstEye] =
      xr_new<CCameraFirstEye>(this, CCameraBase::flKeepPitch);
...
void CActor::Load(LPCSTR section) {
  ...
  */
  ...
  m_fOverweightWalkAccel = READ_IF_EXISTS(pSettings, r_float, section,
                                          "overweight_walk_accel", 1.0f);
...
// Alundaio
  ...
  if (!g_dedicated_server) {
    ...
    for (int hit_type = 0; hit_type < (int)ALife::eHitTypeMax; ++hit_type) {
      ...
      LPCSTR hit_snds =
          READ_IF_EXISTS(pSettings, r_string, hit_snd_sect, hit_name, "");
      int cnt = _GetItemCount(hit_snds);
      string128 tmp;
      VERIFY(cnt != 0);
      for (int i = 0; i < cnt; ++i) {
        sndHit[hit_type].push_back(ref_sound());
        sndHit[hit_type].back().create(_GetItem(hit_snds, i, tmp), st_Effect,
                                       sg_SourceType);
      }
      char buf[256];

      ::Sound->create(sndDie[0],
                      strconcat(sizeof(buf), buf, *cName(), "\\die0"),
                      st_Effect, SOUND_TYPE_MONSTER_DYING);
      ::Sound->create(sndDie[1],
                      strconcat(sizeof(buf), buf, *cName(), "\\die1"),
                      st_Effect, SOUND_TYPE_MONSTER_DYING);
      ::Sound->create(sndDie[2],
                      strconcat(sizeof(buf), buf, *cName(), "\\die2"),
                      st_Effect, SOUND_TYPE_MONSTER_DYING);
      ::Sound->create(sndDie[3],
                      strconcat(sizeof(buf), buf, *cName(), "\\die3"),
                      st_Effect, SOUND_TYPE_MONSTER_DYING);

      m_HeavyBreathSnd.create(pSettings->r_string(section, "heavy_breath_snd"),
                              st_Effect, SOUND_TYPE_MONSTER_INJURING);
      m_BloodSnd.create(pSettings->r_string(section, "heavy_blood_snd"),
                        st_Effect, SOUND_TYPE_MONSTER_INJURING);
  ...
  invincibility_fire_shield_1st = READ_IF_EXISTS(pSettings, r_string, section,
                                                 "Invincibility_Shield_1st", 0);
  invincibility_fire_shield_3rd = READ_IF_EXISTS(pSettings, r_string, section,
                                                 "Invincibility_Shield_3rd", 0);
  //-----------------------------------------
  m_AutoPickUp_AABB =
...
void CActor::Hit(SHit *pHDS) {
  ...
  if (!g_dedicated_server && !sndHit[HDS.hit_type].empty() &&
    ...
    ref_sound &S =
        sndHit[HDS.hit_type][Random.randI(sndHit[HDS.hit_type].size())];
    bool b_snd_hit_playing =
        sndHit[HDS.hit_type].end() != std::find_if(sndHit[HDS.hit_type].begin(),
  ...
  if (!g_dedicated_server && !m_disabled_hitmarks) {
    ...
    if (b_fireWound || b_initiated)
      HitMark(HDS.damage(), HDS.dir, HDS.who, HDS.bone(), HDS.p_in_bone_space,
              HDS.impulse, HDS.hit_type);
  }
...
void CActor::HitMark(float P, Fvector dir, CObject *who_object, s16 element,
                     Fvector position_in_bone_space, float impulse,
...
#pragma todo(                                                                  \
    ...
    MotionID motion_ID = m_anims->m_normal.m_damage[iFloor(
        ...
        (angle_difference(r_model_yaw + r_model_yaw_delta, yaw) <= PI_DIV_2
             ? 0
             : 1))];
...
void CActor::g_Physics(Fvector &_accel, float jump, float dt) {
  ...
  if (Local() && g_Alive()) {
    if (character_physics_support()->movement()->gcontact_Was)
      Cameras().AddCamEffector(xr_new<CEffectorFall>(
    ...
    if (!fis_zero(
      ...
      if (Level().CurrentControlEntity() == this) {
        ...
        SHit HDS =
            SHit(character_physics_support()->movement()->gcontact_HealthLost,
                 //. 0.0f,
  ...
  if (eacFirstEye == cam_active) {
    ...
    if (pWeapon && pWeapon->IsZoomed()) {
      ...
      if (!pWeapon->IsRotatingToZoom())
        return atan(tan(g_scope_fov * (0.5 * PI / 180)) /
                    pWeapon->GetZoomFactor()) /
               (0.5 * PI /
                180); // Alun: This assumes scope has a fake FOV so that no
                      // matter camera FOV the scope FOV is exactly the same
    }
  }
  return g_fov;
}
...
void CActor::UpdateCL() {
  ...
  if (pWeapon) {
    if (pWeapon->IsZoomed()) {
      ...
      if (!bLook_cam_fp_zoom && cam_active == eacLookAt &&
          cam_Active()->m_look_cam_fp_zoom == true) {
        cam_Set(eacFirstEye);
...
void CActor::RenderIndicator(Fvector dpos, float r1, float r2,
  ...
  CBoneInstance &BI =
      smart_cast<IKinematics *>(Visual())->LL_GetBoneInstance(u16(m_head));
  Fmatrix M;
  ...
  // RCache.Render	   			(D3DPT_TRIANGLESTRIP,dwOffset,0, dwCount,
  // 0, 2);
...
void CActor::UpdateArtefactsOnBeltAndOutfit() {
  ...
  for (TIItemContainer::iterator it = inventory().m_belt.begin();
    ...
    if (artefact) {
      ...
      conditions().ChangeBleeding(
          (artefact->m_fBleedingRestoreSpeed * art_cond) * f_update_time);
      conditions().ChangeHealth((artefact->m_fHealthRestoreSpeed * art_cond) *
                                f_update_time);
      conditions().ChangePower((artefact->m_fPowerRestoreSpeed * art_cond) *
                               f_update_time);
      conditions().ChangeSatiety((artefact->m_fSatietyRestoreSpeed * art_cond) *
                                 f_update_time);
      if ((artefact->m_fRadiationRestoreSpeed * art_cond) > 0.0f) {
        ...
        conditions().ChangeRadiation(
            (artefact->m_fRadiationRestoreSpeed * art_cond) * f_update_time);
    }
  }
  ...
  if (!outfit && !pHelmet) {
    ...
    conditions().ChangeHealth(
        ...
        f_update_time);
    conditions().ChangePower(((outfit ? outfit->m_fPowerRestoreSpeed : 0.f) +
...
float CActor::HitArtefactsOnBelt(float hit_power, ALife::EHitType hit_type) {
  ...
  for (; it != ite; ++it) {
    ...
    if (artefact && artefact->m_ArtefactHitImmunities[hit_type]) {
      hit_power -= (artefact->m_ArtefactHitImmunities[hit_type] *
...
float CActor::GetProtection_ArtefactsOnBelt(ALife::EHitType hit_type) {
  ...
  for (; it != ite; ++it) {
    CArtefact *artefact = smart_cast<CArtefact *>(*it);
    if (artefact && artefact->m_ArtefactHitImmunities[hit_type]) {
      sum += (artefact->m_ArtefactHitImmunities[hit_type] *
              artefact->GetCondition());
    }
  }
  return sum;
...
CActor::create_entity_condition(CEntityConditionSimple *ec) {
  if (!ec)
...
DLL_Pure *CActor::_construct() {
  m_pPhysics_support =
      xr_new<CCharacterPhysicsSupport>(CCharacterPhysicsSupport::etActor, this);
...
bool CActor::can_attach(const CInventoryItem *inventory_item) const {
  ...
  if (m_attach_item_sections.end() ==
      std::find(m_attach_item_sections.begin(), m_attach_item_sections.end(),
...
float CActor::GetRestoreSpeed(ALife::EConditionRestoreType const &type) {
  ...
  case ALife::eHealthRestoreSpeed: {
    ...
    res += ((outfit ? outfit->m_fHealthRestoreSpeed : 0.f) +
            (pHelmet ? pHelmet->m_fHealthRestoreSpeed : 0.f));
  ...
  case ALife::eRadiationRestoreSpeed: {
    ...
    res += ((outfit ? outfit->m_fRadiationRestoreSpeed : 0.f) +
            (pHelmet ? pHelmet->m_fRadiationRestoreSpeed : 0.f));
  ...
  case ALife::eSatietyRestoreSpeed: {
    ...
    res += ((outfit ? outfit->m_fSatietyRestoreSpeed : 0.f) +
            (pHelmet ? pHelmet->m_fSatietyRestoreSpeed : 0.f));
  ...
  case ALife::ePowerRestoreSpeed: {
    ...
    if (!outfit && !pHelmet && !pBackpack) {
      ...
      res /= (0.5f + (outfit ? outfit->m_fPowerLoss : EPS) +
              (pHelmet ? pHelmet->m_fPowerLoss : EPS) +
  ...
  case ALife::eBleedingRestoreSpeed: {
    ...
    res += ((outfit ? outfit->m_fBleedingRestoreSpeed : 0.f) +
            (pHelmet ? pHelmet->m_fBleedingRestoreSpeed : 0.f));
...
void CActor::SwitchNightVision(bool vision_on, bool use_sounds,
                               bool send_event) {
  if (eacFirstEye == cam_active) {
    CWeapon *pWeapon = smart_cast<CWeapon *>(inventory().ActiveItem());
    if (pWeapon && pWeapon->IsZoomed()) {
  ...
  if (send_event) {
    m_trader_flags.set(CSE_ALifeTraderAbstract::eTraderFlagNightVisionActive,
                       m_bNightVisionOn);
