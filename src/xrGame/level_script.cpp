...
u32 vertex_in_direction(u32 level_vertex_id, Fvector direction,
                        float max_distance) {
...
LPCSTR get_name() {
  ...
  // Alun: This fixes level.name() being an empty string when checking it while
  // server entities are being registered
  return ai()
      .game_graph()
      .header()
...
void add_call(const luabind::object &lua_object, LPCSTR condition,
              LPCSTR action) {
  //	try{
...
void remove_call(const luabind::object &lua_object, LPCSTR condition,
                 LPCSTR action) {
...
void iterate_sounds(LPCSTR prefix, u32 max_count,
...
void iterate_sounds1(LPCSTR prefix, u32 max_count,
...
float add_cam_effector2(LPCSTR fn, int id, bool cyclic, LPCSTR cb_func,
                        float cam_fov, bool hud_affect = true) {
  CAnimatorCamEffectorScriptCB *e =
      xr_new<CAnimatorCamEffectorScriptCB>(cb_func);
...
int g_get_general_goodwill_between(u16 from, u16 to) {
  ...
  if (!from_obj || !to_obj) {
    ai().script_engine().script_log(
        ...
        "RELATION_REGISTRY::get_general_goodwill_between  : cannot convert obj "
...
void g_send(NET_Packet &P, bool bReliable = 0, bool bSequential = 1,
            bool bHighPriority = 0, bool bSendImmediately = 0) {
  Level().Send(
      P, net_flags(bReliable, bSequential, bHighPriority, bSendImmediately));
}
...
void u_event_send(NET_Packet &P) { CGameObject::u_EventSend(P); }

// can spawn entities like bolts, phantoms, ammo, etc. which normally crash when
// using alife():create()
void spawn_section(LPCSTR sSection, Fvector3 vPosition, u32 LevelVertexID,
                   u16 ParentID, bool bReturnItem = false) {
...
void reload_language() { CStringTable().ReloadLanguage(); }
...
void CLevel::script_register(lua_State *L) {
  ...
  // Alundaio: Extend level namespace exports
       ...
       def("get_target_obj", &g_get_target_obj), // intentionally named to what
                                                 // is in xray extensions
       ...
       def("get_target_element",
           &g_get_target_element), // Can get bone cursor is targetting
       ...
       def("add_call", ((void (*)(const luabind::functor<bool> &,
                                  const luabind::functor<void> &)) &
                        add_call)),
       def("add_call",
           ((void (*)(const luabind::object &, const luabind::functor<bool> &,
                      const luabind::functor<void> &)) &
            add_call)),
       def("add_call",
           ((void (*)(const luabind::object &, LPCSTR, LPCSTR)) & add_call)),
       def("remove_call", ((void (*)(const luabind::functor<bool> &,
                                     const luabind::functor<void> &)) &
                           remove_call)),
       def("remove_call",
           ((void (*)(const luabind::object &, const luabind::functor<bool> &,
            ...
            remove_call)),
       def("remove_call",
           ((void (*)(const luabind::object &, LPCSTR, LPCSTR)) & remove_call)),
       def("remove_calls_for_object", remove_calls_for_object),
       def("present", is_level_present), def("disable_input", disable_input),
  ...
  module(
      L)[class_<CRayPick>("ray_pick")
         ...
         class_<enum_exporter<collide::rq_target>>("rq_target")
             .enum_("targets")[value("rqtNone", int(collide::rqtNone)),
  ...
  module(L, "game")
      [class_<xrTime>("CTime")
           ...
           .def("set", &xrTime::set)
           .def("get", &xrTime::get,
                out_value(_2) + out_value(_3) + out_value(_4) + out_value(_5) +
