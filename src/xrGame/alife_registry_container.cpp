...
template <typename T1, typename _T2, typename Head> struct CRegistryHelperLoad {
  ...
  IC static void process(T1 *self, T2 &p1) {
    do_load<object_type_traits::is_base_and_derived<IPureLoadableObject<T2>,
                                                    Head>::value>(self, p1);
  }
};
...
template <typename T1, typename _T2, typename Head> struct CRegistryHelperSave {
  ...
  IC static void process(T1 *self, T2 &p1) {
    do_save<object_type_traits::is_base_and_derived<IPureSavableObject<T2>,
                                                    Head>::value>(self, p1);
  }
};

template <template <typename _1, typename _2, typename _3> class helper,
          typename T1, typename T2, typename TList>
...
void CALifeRegistryContainer::load(IReader &file_stream) {
  ...
  CRegistryHelperProcess<CRegistryHelperLoad, CALifeRegistryContainer,
                         IReader &, TYPE_LIST>::process(this, file_stream);
}

void CALifeRegistryContainer::save(IWriter &memory_stream) {
  memory_stream.open_chunk(REGISTRY_CHUNK_DATA);
  CRegistryHelperProcess<CRegistryHelperSave, CALifeRegistryContainer,
                         IWriter &, TYPE_LIST>::process(this, memory_stream);
