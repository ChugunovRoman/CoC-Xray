...
float CVisualMemoryManager::object_visible_distance(
    const CGameObject *game_object, float &object_distance) const {
  ...
  if (m_object) {
    ...
    Fvector dummy;
    float _0, _1;
    m_client->camera(eye_position, eye_direction, dummy, object_fov, _0, _1,
                     object_range);
  }
...
float CVisualMemoryManager::get_visible_value(
    const CGameObject *game_object, float distance, float object_distance,
  ...
  if (ai().script_engine().functor("visual_memory_manager.get_visible_value",
                                   funct))
    return (funct(m_object ? m_object->lua_game_object() : 0,
                  game_object ? game_object->lua_game_object() : 0, time_delta,
                  current_state().m_time_quant, luminocity,
                  current_state().m_velocity_factor, object_velocity, distance,
...
CNotYetVisibleObject *
CVisualMemoryManager::not_yet_visible_object(const CGameObject *game_object) {
...
void CVisualMemoryManager::add_visible_object(
  ...
  VERIFY(m_objects);
  xr_vector<CVisibleObject>::iterator J = std::find(
      m_objects->begin(), m_objects->end(), object_id(visible_object.m_object));
  if (m_objects->end() != J)
    *J = visible_object;
...
bool CVisualMemoryManager::visible(u32 _level_vertex_id, float yaw,
                                   float eye_fov) const {
  Fvector direction;
...
struct CVisibleObjectPredicateEx {
  ...
  bool operator()(
      const MemorySpace::CNotYetVisibleObject &not_yet_visible_object) const {
    if (!m_object)
...
void CVisualMemoryManager::remove_links(CObject *object) {
  {
    VERIFY(m_objects);
    VISIBLES::iterator I = std::find_if(m_objects->begin(), m_objects->end(),
...
struct CRemoveVisibleObjectPredicate {
  ...
  CRemoveVisibleObjectPredicate(const MemorySpace::CVisibleObject *object)
      : m_object(object) {}
...
void CVisualMemoryManager::remove(
    const MemorySpace::CVisibleObject *visible_object) {
  VISIBLES::iterator I =
      std::find_if(m_objects->begin(), m_objects->end(),
                   CRemoveVisibleObjectPredicate(visible_object));
  if (I != m_objects->end())
    m_objects->erase(I);
}

CVisibleObject *
CVisualMemoryManager::visible_object(const CGameObject *game_object) {
  VISIBLES::iterator I = std::find_if(m_objects->begin(), m_objects->end(),
...
static inline bool
is_object_valuable_to_save(CCustomMonster const *const self,
