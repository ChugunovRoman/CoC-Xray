...
door::door(CPhysicObject *object)
    : m_object(*object), m_state(door_state_open),
      m_previous_state(door_state_open), m_target_state(door_state_open),
...
void door::change_state(actor *const initiator, door_state const start_state,
  ...
  if (m_initiators.empty()) {
    ...
    //{ 			int i=0; (void)i;
    //		}
    change_state(initiator); // Alundaio: Pass the initator! We need to know who
                             // is trying to open door!
    return;
  }
