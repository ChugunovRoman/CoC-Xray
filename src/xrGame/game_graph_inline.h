...
IC bool
CGameGraph::mask(const svector<_LOCATION_ID, GameGraph::LOCATION_TYPE_COUNT> &M,
...
IC bool
CGameGraph::mask(const _LOCATION_ID M[GameGraph::LOCATION_TYPE_COUNT],
                 const _LOCATION_ID E[GameGraph::LOCATION_TYPE_COUNT]) const {
  for (int i = 0; i < GameGraph::LOCATION_TYPE_COUNT; ++i)
    if ((M[i] != E[i]) && (255 != M[i]))
      return (false);
...
IC void CGameGraph::begin(u32 const vertex_id, const_iterator &start,
                          const_iterator &end) const {
  end = (start = (const CEdge *)((BYTE *)m_nodes +
                                 vertex(_GRAPH_ID(vertex_id))->edge_offset())) +
...
IC const GameGraph::SLevel &
GameGraph::CHeader::level(const _LEVEL_ID &id) const {
...
IC GameGraph::_GRAPH_ID
CGameGraph::vertex_id(const CGameGraph::CVertex *vertex) const {
