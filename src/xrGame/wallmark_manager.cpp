...
void CWalmarkManager::AddWallmark(const Fvector &dir, const Fvector &start_pos,
  ...
  if (pMaterial->Flags.is(SGameMtl::flBloodmark)) {
    ...
    if (!wallmarks_vector.empty()) {
      ::Render->add_StaticWallmark(&wallmarks_vector, end_point, wallmark_size,
                                   pTri, pVerts);
    }
...
float Distance(const Fvector &rkPoint, const Fvector rkTri[3], float &pfSParam,
               float &pfTParam, Fvector &closest, Fvector &dir);
  ...
  */
  for (CDB::RESULT *Res = R_begin; Res != R_end; ++Res) {
    ...
    */
    ...
    if (test > 0.f) {
      if (Level().ObjectSpace.RayTest(m_pos, pdir, test, collide::rqtStatic,
                                      NULL, m_owner)) {
        ++_ray_test;
        continue;
      }
    ...
    if (dist <= m_trace_dist) {
      ::Render->add_StaticWallmark(&*m_wallmarks, end_point, m_wallmark_size,
                                   _t, V_array);
      ++wm_count;
    } else
      ++_not_dist;
  ...
  /*
     ...
     _tri_behind); Msg	("c!=tdPlane && dist>ndist = %d",	_tri_not_plane);
