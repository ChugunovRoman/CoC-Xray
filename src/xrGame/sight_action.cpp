...
void CSightAction::execute_cover() {
  if (m_torso_look)
    object().sight().SetLessCoverLook(m_object->ai_location().level_vertex(),
                                      PI, m_path);
  else
    object().sight().SetLessCoverLook(m_object->ai_location().level_vertex(),
                                      m_path);
...
void CSightAction::execute_cover_look_over() {
  ...
  fall_back:
#endif // #ifndef DEBUG
    if ((m_start_state_time + m_stop_state_time < Device.dwTimeGlobal) &&
        target_reached()) {
      m_start_state_time = Device.dwTimeGlobal;
      m_stop_state_time = 3500;
      m_internal_state = 1;
  ...
  default: {
#ifdef DEBUG
    FATAL(make_string("m_internal_state = %d, object[0x%08x]", m_internal_state,
                      this)
              .c_str());
#else  // #ifdef DEBUG
    m_internal_state = 0;
...
void CSightAction::predict_object_position(bool use_exact_position) {
  ...
  if (count > 1) {
    ...
    for (int i = 3; (current_position.dwTime == previous_position.dwTime) &&
                    (i <= (int)count);
         ++i)
...
void CSightAction::execute_fire_object() {
  ...
  case 0: {
    ...
    if (m_object_to_look->Position().distance_to_sqr(m_object->Position()) <
        _sqr(5.f))
      break;

    //			Msg							("%6d switch to mode
    //1", Device.dwTimeGlobal);
    ...
    if (Device.dwTimeGlobal >= m_state_fire_switch_time + 1500) {
      if (m_object_to_look->Position().distance_to_sqr(m_object->Position()) >
        ...
        if (!m_holder_start_position.similar(m_object->Position(), .05f)) {
          ...
          m_already_switched = false;
          //						Msg					("%6d switch to
          //mode 0 (reson: holder position changed)", Device.dwTimeGlobal);
          m_state_fire_object = 0;
          break;
        }

        if (!m_object_start_position.similar(m_object_to_look->Position(),
                                             .05f)) {
          m_vector3d = m_object->sight().object_position();
          //						Msg					("%6d switch to
          //mode 0 (reson: object position changed)", Device.dwTimeGlobal);
          m_already_switched = false;
          m_state_fire_object = 0;
          break;
        }
      ...
      if (!m_already_switched) {
        m_vector3d = m_object->sight().object_position();
        //					Msg
        //("%6d switch to mode 0 (reson: time interval)", Device.dwTimeGlobal);
