...
void CObjectHandler::OnItemDrop(CInventoryItem *inventory_item,
  ...
  if (m_infinite_ammo && planner().object().g_Alive() &&
      !inventory_item->useful_for_NPC()) {
    CWeaponAmmo *weapon_ammo = smart_cast<CWeaponAmmo *>(inventory_item);
    if (weapon_ammo) {
      Level().spawn_item(*weapon_ammo->cNameSect(),
                         planner().object().Position(),
...
void CObjectHandler::set_goal(MonsterSpace::EObjectAction object_action,
                              ...
                              u32 min_queue_size, u32 max_queue_size,
...
void CObjectHandler::actualize_strap_mode(CWeapon *weapon) const {
  ...
  if (!planner().m_storage.property(
          ObjectHandlerSpace::eWorldPropertyStrapped)) {
    //		Msg						( "[%6d][%s] actualizing: weapon_strapped =
    //false", Device.dwTimeGlobal, planner().object().cName().c_str() );
    weapon->strapped_mode(false);
    return;
  }
  ...
  //weapon_strapped = true", Device.dwTimeGlobal,
...
bool CObjectHandler::weapon_strapped(CWeapon *weapon) const {
  ...
  if (!weapon->can_be_strapped()) {
    ...
    //false0", Device.dwTimeGlobal, planner().object().cName().c_str() );
    return (false);
  }

  bool const almost_strapped = planner().current_action_state_id() ==
  ...
  if (almost_strapped ||
    ...
    //		Msg						( "[%6d][%s] weapon_strapped =
    //%s1", Device.dwTimeGlobal, planner().object().cName().c_str(),
    // almost_strapped
    //&&
...
bool CObjectHandler::weapon_unstrapped() const {
  ...
  if (!weapon) {
    //		Msg						( "[%6d][%s] no active
    //item!!(%d)(%s)", Device.dwTimeGlobal, planner().object().cName().c_str(),
    // inventory().GetActiveSlot(), inventory().ItemFromSlot(3) ?
...
bool CObjectHandler::is_weapon_going_to_be_strapped(
  ...
  CWorldProperty const strap_property(
      planner.uid(object->ID(), ObjectHandlerSpace::eWorldPropertyIdleStrap),
      true);
