...
// том случае 					если мы хотим сгененрировать несколько
// таблиц с одинаковыми 					T_ITEM
//и T_INI_LOADER
...
private:
  ...
  template <> T_ITEM convert<int>(LPCSTR str) { return atoi(str); }

  template <> T_ITEM convert<float>(LPCSTR str) { return (float)atof(str); }
};
...
typename CSIni_Table::ITEM_TABLE &CSIni_Table::table() {
  ...
  for (CInifile::SectCIt i = table_ini.Data.begin(); table_ini.Data.end() != i;
       ++i) {
