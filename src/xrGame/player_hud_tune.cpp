...
bool is_attachable_item_tuning_mode() {
  return pInput->iGetAsyncKeyState(DIK_LSHIFT) ||
         pInput->iGetAsyncKeyState(DIK_Z) || pInput->iGetAsyncKeyState(DIK_X) ||
  ...
  if (hud_adj_mode == 3 || hud_adj_mode == 4) {
    ...
    if ((values.x) || (values.y) || (values.z)) {
      ...
      Msg("item_position				= %f,%f,%f",
          m_measures.m_item_attach[0].x, m_measures.m_item_attach[0].y,
          m_measures.m_item_attach[0].z);
      Msg("item_orientation			= %f,%f,%f",
          m_measures.m_item_attach[1].x, m_measures.m_item_attach[1].y,
  ...
  if (hud_adj_mode == 5 || hud_adj_mode == 6 || hud_adj_mode == 7) {
    ...
    if ((values.x) || (values.y) || (values.z)) {
      ...
      Msg("fire_point2			= %f,%f,%f",
          m_measures.m_fire_point2_offset.x, m_measures.m_fire_point2_offset.y,
          m_measures.m_fire_point2_offset.z);
      Msg("shell_point			= %f,%f,%f",
          m_measures.m_shell_point_offset.x, m_measures.m_shell_point_offset.y,
...
void player_hud::tune(Ivector _values) {
  ...
  if (hud_adj_mode == 1 || hud_adj_mode == 2) {
    ...
    Fvector &pos_ =
        (idx != 0) ? m_attached_items[hud_adj_item_idx]->hands_offset_pos()
                   : m_attached_items[hud_adj_item_idx]->hands_attach_pos();
    Fvector &rot_ =
        (idx != 0) ? m_attached_items[hud_adj_item_idx]->hands_offset_rot()
...
void hud_draw_adjust_mode() {
  ...
  if (pInput->iGetAsyncKeyState(DIK_LSHIFT) && hud_adj_mode)
    _text = "press SHIFT+NUM 0-return 1-hud_pos 2-hud_rot 3-itm_pos 4-itm_rot "
