...
stalker_movement_manager_smart_cover::stalker_movement_manager_smart_cover(
    ...
    : inherited(object), m_property_storage(0), m_current_transition(0),
      ...
      m_enter_cover_id(""), m_enter_loophole_id(""),
      m_check_can_kill_enemy(false), m_combat_behaviour(false) {
...
void stalker_movement_manager_smart_cover::on_frame(
    CPHMovementControl *movement_control, Fvector &dest_position) {
...
void stalker_movement_manager_smart_cover::modify_animation(CBlend *blend) {
  ...
  CMotionDef *motion_def = smart_cast<IKinematicsAnimated *>(object().Visual())
...
void stalker_movement_manager_smart_cover::reach_enter_location(
  ...
  if (!accessible(level_vertex_id) || !accessible(position)) {
    if (!ai().level_graph().inside(level_vertex_id, position))
      ...
      position.y = ai().level_graph().vertex_plane_y(level_vertex_id,
                                                     position.x, position.z);

    if (!restrictions().accessible(position)) {
      level_vertex_id =
          restrictions().accessible_nearest(Fvector().set(position), position);
  ...
  object().sight().setup(
      CSightAction(SightManager::eSightTypeAnimationDirection, true, false));
...
void stalker_movement_manager_smart_cover::loophole_path(
    smart_cover::cover const &cover, shared_str const &source_raw,
...
void stalker_movement_manager_smart_cover::target_idle() {
  //	if (!m_current.cover()) {
  //		Msg								("! Cannot set target idle.
  //Bad or absent smart_cover."); 		return;
  //	}

  //	if (!m_current.cover_loophole()->is_action_available("idle")) {
  //		Msg								("! Cannot set target
  //idle. Loophole has no such action."); 		return;
  //	}
...
void stalker_movement_manager_smart_cover::target_lookout() {
  //	if (!m_current.cover()) {
  //		Msg								("! Cannot set target
  //lookout. Bad or absent smart_cover."); 		return;
  //	}

  //	if (!m_current.cover_loophole()->is_action_available("lookout")) {
  //		Msg								("! Cannot set target
  //lookout. Loophole has no such action."); 		return;
  //	}
...
void stalker_movement_manager_smart_cover::target_fire() {
  //	if (!m_current.cover()) {
  //		Msg								("! Cannot set target fire.
  //Bad or absent smart_cover."); 		return;
  //	}

  //	if (!m_current.cover_loophole()->is_action_available("fire")) {
  //		Msg								("! Cannot set target
  //fire. Loophole has no such action."); 		return;
  //	}

  //	if (!enemy_in_fov()) {
  //		Msg								("! Cannot set target fire.
  //Enemy is not in current loophole's fov."); 		return;
  //	}
...
void stalker_movement_manager_smart_cover::target_fire_no_lookout() {
  ...
  //		Msg								("! Cannot set
  //target fire_no_lookout. Bad or absent smart_cover."); 		return;
  //	}
  ...
  //{
  //		Msg								("! Cannot set
  //target fire_no_lookout. Loophole has no such action."); 		return;
  //	}
