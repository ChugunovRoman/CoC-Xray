...
struct priority {
  IC static bool standing(const moving_object *object) {
    ...
    return (!!fis_zero(
        object->position().distance_to_sqr(object->predict_position(1.f))));
  }
...
void moving_objects::fill_nearest_moving(moving_object *object) {
  ...
  m_nearest_moving.erase(std::remove_if(m_nearest_moving.begin(),
...
bool moving_objects::fill_collisions(moving_object *object,
  ...
  for (; I != E; ++I) {
    ...
    m_collisions.push_back(std::make_pair(
        time_to_check,
        std::make_pair(action, std::make_pair(priority ? object : (*I),
                                              !priority ? object : (*I)))));
...

  {
    ...
    for (; I != E; ++I) {
      ...
      m_collisions.push_back(std::make_pair(
          time_to_check,
          std::make_pair(priority ? possible_action_2_can_wait_1
...
void moving_objects::resolve_collisions() {
      ...
      (moving_object **)_alloca(collidee_count * sizeof(moving_object *));
  {
    ...
    collidee_count =
        u32(std::unique(collidees, collidees + collidee_count) - collidees);
  }
...

  {
    ...
    for (; I != E; ++I) {
      ...
      case possible_action_1_can_wait_2: {
        decision *object =
            std::find_if(decisions, decisions + decision_count,
...
void moving_objects::query_action_dynamic(moving_object *object) {
  ...
  while (!m_collision_emitters.empty()) {
    ...
    m_visited_emitters.insert(std::lower_bound(m_visited_emitters.begin(),
