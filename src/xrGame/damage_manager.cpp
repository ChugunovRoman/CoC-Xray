...
void CDamageManager::load_section(LPCSTR section, CInifile const *ini) {
  ...
  for (CInifile::SectCIt i = damages.Data.begin(); damages.Data.end() != i;
    ...
    if (xr_strcmp(*(*i).first, "default")) { // read all except default line
      ...
      bone_instance.set_param(0,
                              (float)atof(_GetItem(*(*i).second, 0, buffer)));
      bone_instance.set_param(1,
                              (float)atoi(_GetItem(*(*i).second, 1, buffer)));
      bone_instance.set_param(2,
                              (float)atof(_GetItem(*(*i).second, 2, buffer)));
      if (_GetItemCount(*(*i).second) < 4) {
        bone_instance.set_param(3,
                                (float)atof(_GetItem(*(*i).second, 0, buffer)));
      } else {
        bone_instance.set_param(3,
                                (float)atof(_GetItem(*(*i).second, 3, buffer)));
      }
...
void CDamageManager::HitScale(const int element, float &hit_scale,
                              float &wound_scale, bool aim_bullet) {
