...
Fvector CMovementManager::path_position(const float &velocity,
                                        ...
                                        u32 &current_travel_point, float &dist,
  ...
  dist =
      desirable_speed *
      time_delta; // пройденное рассто€ние в соостветствие с желаемой скоростью
...
void CMovementManager::move_along_path(CPHMovementControl *movement_control,
  ...
  u32 current_travel_point = detail().m_current_travel_point;
  dest_position =
      path_position(old_desirable_speed(), object().Position(), time_delta,
...
#pragma todo("Dima to ? : is this correct?")
  ...
  if (DBG_PH_MOVE_CONDITIONS(
    ...
    if (DBG_PH_MOVE_CONDITIONS(
            !ph_dbg_draw_mask.test(phDbgNeverUseAiPhMove) &&) !movement_control
