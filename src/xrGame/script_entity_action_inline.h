...
IC void
CScriptEntityAction::SetAction(CScriptAnimationAction &tAnimationAction) {
...
IC void
CScriptEntityAction::SetAction(CScriptActionCondition &tActionCondition) {
...
IC bool CScriptEntityAction::CheckIfTimeOver() {
  return ((m_tActionCondition.m_tLifeTime >= 0) &&
          ((m_tActionCondition.m_tStartTime + m_tActionCondition.m_tLifeTime) <
           Device.dwTimeGlobal));
}
...
IC bool CScriptEntityAction::CheckIfActionCompleted() {
  ...
  if (!m_tActionCondition.m_dwFlags && (m_tActionCondition.m_tLifeTime < 0) &&
      CheckIfMovementCompleted() && CheckIfWatchCompleted() &&
      CheckIfAnimationCompleted() && CheckIfSoundCompleted() &&
      CheckIfParticleCompleted() && CheckIfObjectCompleted() &&
      CheckIfMonsterActionCompleted())
    return (true);
  else
    return (!l_dwFlags);
