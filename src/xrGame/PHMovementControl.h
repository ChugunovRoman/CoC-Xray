...
public:
  ...
  void GetJumpMinVelParam(
      ...
      const Fvector
          &end_point); // returns vector of velocity of jump with minimal start
  ...
  void GetJumpParam(Fvector &velocity, JumpType &type, const Fvector &end_point,
                    float time);
...
public:
  ...
  void CalcMaximumVelocity(Fvector & /**dest/**/, Fvector & /**accel/**/,
                           float /**friction/**/){};
  void CalcMaximumVelocity(float & /**dest/**/, float /**accel/**/,
                           float /**friction/**/){};
  ...
  void Calculate(Fvector &vAccel, const Fvector &camDir, float ang_speed,
  ...
  static BOOL
  CPHMovementControl::BorderTraceCallback(collide::rq_result &result,
                                          LPVOID params);
...
private:
  void actor_calculate(Fvector &vAccel, const Fvector &camDir, float ang_speed,
                       float jump, float dt, bool bLight);
  void UpdateCollisionDamage();
  bool MakeJumpPath(xr_vector<DetailPathManager::STravelPathPoint> &out_path,
                    u32 &travel_point, Fvector &dist_to_enemy);
};
