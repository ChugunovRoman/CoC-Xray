...
struct CProfilePortionPredicate {
  IC bool operator()(const CProfileResultPortion &_1,
                     const CProfileResultPortion &_2) const {
...
IC void CProfiler::convert_string(LPCSTR str, shared_str &out,
                                  u32 max_string_size) {
  string256 m_temp;
  LPCSTR i, j = str;
  u32 count = 0;
...
void CProfiler::setup_timer(LPCSTR timer_id, const u64 &timer_time,
  ...
  if (i == m_timers.end()) {
    ...
    i = m_timers.insert(std::make_pair(shared_str(timer_id), CProfileStats()))
            .first;
...
void CProfiler::show_stats(CGameFont *game_font, bool show) {
  ...
  if (!m_portions.empty()) {
    ...
    if (!m_actual) {
      ...
      for (; I != E; ++I)
        max_string_size =
            _max(max_string_size, compute_string_length(*(*I).first));
  ...
  for (; I != E; ++I) {
    ...
    float average = (*I).second.m_count
                        ? (*I).second.m_total_time / float((*I).second.m_count)
                        : 0.f;
    ...
    game_font->OutNext(
        ...
        "%s%c%c %8.3f %8.3f %8.3f %6.1f %8d %12.3f", *(*I).second.m_name,
        white_character, white_character, (*I).second.m_time, average,
        (*I).second.m_max_time,
        float((*I).second.m_call_count) /
            m_call_count, // float((*I).second.m_count),
        //			(*I).second.m_min_time,
...
void CProfiler::add_profile_portion(
    const CProfileResultPortion &profile_portion) {
#ifdef PROFILE_CRITICAL_SECTIONS
