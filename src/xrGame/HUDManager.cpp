...
CHUDManager::CHUDManager()
    : pUIGame(NULL), m_pHUDTarget(xr_new<CHUDTarget>()), b_online(false) {}
//--------------------------------------------------------------------
CHUDManager::~CHUDManager() {
  OnDisconnected();
...
void CHUDManager::Render_Actor_Shadow() // added by KD
{
  ...
  if (A->active_cam() != eacFirstEye)
    return; // KD: we need to render actor shadow only in first eye cam mode
            // because
  ...
  CFlashlight *flashlight =
      smart_cast<CFlashlight *>(A->inventory().ItemFromSlot(DETECTOR_SLOT));
...
void CHUDManager::SetCrosshairDisp(float dispf, float disps) {
  m_pHUDTarget->GetHUDCrosshair().SetDispersion(
...
void CHUDManager::HitMarked(int idx, float power, const Fvector &dir) {
  ...
  pInput->feedback(u16(iFloor(u16(-1) * power)), u16(iFloor(u16(-1) * power)),
                   0.5f);
}
