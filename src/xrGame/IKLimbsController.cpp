...
void CIKLimbsController::LimbSetup() {
  ...
  _bone_chains.back().Create((u16)_bone_chains.size() - 1, skeleton_animated,
                             true);
}
...
float CIKLimbsController::LegLengthShiftLimit(
    float current_shift, const SCalculateData cd[max_size]) {
...
float CIKLimbsController::StaticObjectShift(const SCalculateData cd[max_size]) {
  ...
  if (shift_down > 0.f)
    ...
    shift = shift_up;
  VERIFY(_valid(shift));
  _object_shift.set_taget(shift, _abs(current_shift - shift) /
                                     static_shift_object_speed);
  return shift;
}
...
void CIKLimbsController::Calculate() {
  ...
  for (u16 j = 0; sz > j; ++j) {
    ...
    if (ph_dbg_draw_mask1.test(phDbgDrawIKPredict)) {
      ...
      DBG_DrawLine(
          toe, Fvector().add(toe, Fvector().set(0, -_object_shift.shift(), 0)),
...
void CIKLimbsController::PlayLegs(CBlend *b) {
  ...
  if (MD.marks.empty() && (MD.flags & esmUseFootSteps))
    Msg("! No foot stseps for animation: animation name: %s, animation set: "
        "%s ",
