...
static void _BCL callback_rotation_blend(CBoneInstance *const bone) {
  ...
  //	VERIFY2							( *parameter->m_blend,
  //make_string(
  //"%d %s[%s]", Device.dwTimeGlobal, parameter->m_object->cName().c_str(),
  // parameter->m_object->g_Alive() ? "+" : "-") );
...
void CStalkerAnimationManager::assign_bone_callbacks() {
  ...
  int shoulder_bone =
      kinematics->LL_BoneID(pSettings->r_string(section, "bone_shoulder"));
