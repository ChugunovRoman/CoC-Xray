...
void Group::construct(const shared_str &group_id, UpgradeBase &parent_upgrade,
                      Manager &manager_r) {
  m_id._set(group_id);
  ...
  VERIFY2(upgrades_str, make_string("in upgrade group <%s> elements are empty!",
                                    m_id.c_str()));
...
UpgradeStateResult Group::can_install(CInventoryItem &item,
                                      UpgradeBase &test_upgrade, bool loading) {
  ...
  for (; ib != ie; ++ib) {
    ...
    if (!item.has_upgrade_group(U->parent_group_id())) {
      if (loading) {
        FATAL(
            make_string("Loading item: Upgrade <%s> of inventory item [%s] (id "
