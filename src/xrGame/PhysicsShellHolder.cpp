...
// element, 						 Fvector p_in_object_space, float
...
void CPhysicsShellHolder::correct_spawn_pos() {
  ...
  R_ASSERT2(_valid(c), make_string("object: %s model: %s ", cName().c_str(),
                                   cNameVisual().c_str()));
  R_ASSERT2(_valid(size), make_string("object: %s model: %s ", cName().c_str(),
                                      cNameVisual().c_str()));
...
// void						SetWeaponHideState( u16 State, bool
// bSet
// )
