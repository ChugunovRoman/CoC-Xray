...
BOOL CWeaponMagazinedWGrenade::net_Spawn(CSE_Abstract *DC) {
  ...
  m_DefaultCartridge2.Load(m_ammoTypes2[m_ammoType.type2].c_str(),
                           m_ammoType.type2);
  if (m_ammoElapsed.type2) {
    ...
    if (!getRocketCount()) {
      shared_str fake_grenade_name = pSettings->r_string(
          m_magazine2.back().m_ammoSect, "fake_grenade_name");
      CRocketLauncher::SpawnRocket(*fake_grenade_name, this);
    }
  }

  UpdateGrenadeVisibility(m_bGrenadeMode && !!m_ammoElapsed.type2 ? true
                                                                  : false);
  SetPending(FALSE);
...
bool CWeaponMagazinedWGrenade::SwitchMode() {
  bool bUsefulStateToSwitch =
      ((eIdle == GetState()) || (eHidden == GetState()) ||
...
void CWeaponMagazinedWGrenade::LaunchGrenade() {
  ...
  R_ASSERT(m_bGrenadeMode);
  {
    ...
    if (IsZoomed() && smart_cast<CActor *>(H_Parent())) {
      ...
      BOOL HasPick = Level().ObjectSpace.RayPick(p1, d, 300.0f,
                                                 collide::rqtStatic, RQ, this);
      ...
      if (HasPick) {
        ...
        u8 canfire0 = TransferenceAndThrowVelToThrowDir(
            Transference, CRocketLauncher::m_fLaunchSpeed, EffectiveGravity(),
            res);
#ifdef DEBUG
        //.
...
void CWeaponMagazinedWGrenade::ReloadMagazine() {
  ...
  if (m_ammoElapsed.type1 && !getRocketCount() && m_bGrenadeMode) {
    shared_str fake_grenade_name = pSettings->r_string(
        m_ammoTypes[m_ammoType.type1].c_str(), "fake_grenade_name");
  ...
  if (pGrenadeLauncher &&
    ...
    for (; it != m_launchers.end(); it++) {
      if (pSettings->r_string((*it), "grenade_launcher_name") ==
        ...
        m_ammoTypes2.clear();
        LPCSTR S =
            pSettings->r_string(pIItem->object().cNameSect(), "grenade_class");
        if (S && S[0]) {
          string128 _ammoItem;
          int count = _GetItemCount(S);
          for (int it = 0; it < count; ++it) {
...
void CWeaponMagazinedWGrenade::InitAddons() {
  ...
  if (IsGrenadeLauncherAttached() && GrenadeLauncherAttachable()) {
    CRocketLauncher::m_fLaunchSpeed =
        pSettings->r_float(GetGrenadeLauncherName(), "grenade_vel");
...
void CWeaponMagazinedWGrenade::PlayAnimIdle() {
  if (IsGrenadeLauncherAttached()) {
    if (IsZoomed()) {
      if (m_bGrenadeMode)
        PlayHUDMotion("anm_idle_g_aim", /*FALSE*/ TRUE, NULL,
                      GetState()); // AVO: fix fast anim switch
      else
      ...
      if (m_bGrenadeMode) {
        if (act_state == 0)
          PlayHUDMotion("anm_idle_g", /*FALSE*/ TRUE, NULL,
                        GetState()); // AVO: fix fast anim switch
        else if (act_state == 1)
        ...
        if (act_state == 0)
          PlayHUDMotion("anm_idle_w_gl", /*FALSE*/ TRUE, NULL,
                        GetState()); // AVO: fix fast anim switch
        else if (act_state == 1)
          ...
          if (HudAnimationExist("anm_idle_moving_crouch_w_gl"))
            PlayHUDMotion("anm_idle_moving_crouch_w_gl", TRUE, NULL,
                          GetState());
#endif //-NEW_ANIMS
        }
      }
...
void CWeaponMagazinedWGrenade::PlayAnimModeSwitch() {
  if (m_bGrenadeMode)
    PlayHUDMotion("anm_switch_g", /*FALSE*/ TRUE, this,
                  eSwitch); // AVO: fix fast anim switch
  else
    PlayHUDMotion("anm_switch", /*FALSE*/ TRUE, this,
                  eSwitch); // AVO: fix fast anim switch
}
...
bool CWeaponMagazinedWGrenade::install_upgrade_ammo_class(LPCSTR section,
  ...
  bool result =
      process_if_exists(section, "ammo_mag_size", &CInifile::r_s32,
                        m_bGrenadeMode ? iMagazineSize2 : iMagazineSize, test);
