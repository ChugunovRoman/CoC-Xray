...
LPSTR remove_version_option(LPCSTR opt_str, LPSTR new_opt_str,
...
//TimeReceive[%d]", Level().timeServer(), P->timeReceive);
    ...
    // [08.11.07] Alexander Maniluk: added new message handler for moving
    // artefacts.
      ...
      if (m_type == M_LOAD_GAME) {
        ...
        if (xr_strlen(saved_name) && ai().get_alife()) {
          ...
          if (wrapper.level_id() == ai().level_graph().level_id()) {
            Engine.Event.Defer("Game:QuickLoad", size_t(xr_strdup(saved_name)),
                               0);
    ...
    case M_CHANGE_LEVEL_GAME: {
      ...
      if (OnClient()) {
        ...
        xr_sprintf(NewServerOptions, "%s/%s/%s%s", LevelName.c_str(),
                   GameType.c_str(), map_ver_string, LevelVersion.c_str());
