...
void CSE_ALifeGroupAbstract::try_switch_offline() {
  ...
  for (u32 i = 0, N = (u32)m_tpMembers.size(); i < N; ++i) {
    // casting group member to the abstract monster to get access to the Health
    // property
    CSE_ALifeMonsterAbstract *tpGroupMember =
        smart_cast<CSE_ALifeMonsterAbstract *>(
    ...
    if (tpGroupMember->g_Alive()) {
      ...
      if (I->alife().graph().actor()->o_Position.distance_to(
              tpGroupMember->o_Position) <= I->alife().offline_distance())
        // so, it is not ready, breaking a cycle, because we can't
        // switch group offline since not all the group members are ready
        // to switch offline
        break;
    ...
    // and remove it from the graph point but do not remove it from the current
    // level map
