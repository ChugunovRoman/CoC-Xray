...
void hits_store::add_hit(shared_str const &hitter, shared_str const &victim,
                         u16 weapon_id, u16 bone_id, float bullet_fly_dist) {
  ...
  if (tmp_iter == m_bullet_hits.end()) {
    ...
    tmp_iter =
        m_bullet_hits.insert(std::make_pair(search_key, new_hits_queue)).first;
  }
