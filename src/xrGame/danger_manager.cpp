...
struct CRemoveByTimePredicate {
  ...
  IC bool operator()(const CDangerObject &object) const {
    if (object.time() < m_time_line) {
      if (object.object() &&
          (object.type() == CDangerObject::eDangerTypeFreshEntityCorpse))
...
float CDangerManager::do_evaluate(const CDangerObject &object) const {
  float result = 0.f;
  switch (object.type()) {
  case CDangerObject::eDangerTypeBulletRicochet: { // I perceived bullet(knife)
                                                   // ricochet
    result += 3500.f;
    break;
  }
...
void CDangerManager::add(const CVisibleObject &object) {
  ...
  if (obj && !obj->g_Alive() && (obj->killer_id() != ALife::_OBJECT_ID(-1))) {
    ...
    if (ai().script_engine().functor("xr_danger.CDangerManager__on_before_add",
                                     funct))
      if (funct(m_object->lua_game_object(), object, obj->Position(),
                object.m_level_time,
                CDangerObject::eDangerTypeFreshEntityCorpse,
                CDangerObject::eDangerPerceiveTypeVisual) == false)
        return;
...
void CDangerManager::add(const CSoundObject &object) {
  ...
  if ((object.m_sound_type & SOUND_TYPE_BULLET_HIT) == SOUND_TYPE_BULLET_HIT) {
    ...
    if (ai().script_engine().functor("xr_danger.CDangerManager__on_before_add",
                                     funct))
      if (funct(m_object->lua_game_object(), object,
                object.m_object_params.m_position, object.m_level_time,
                CDangerObject::eDangerTypeBulletRicochet,
                CDangerObject::eDangerPerceiveTypeSound) == false)
        return;
  ...
  if ((object.m_sound_type & SOUND_TYPE_INJURING) == SOUND_TYPE_INJURING) {
    ...
    if (do_add) {
      ...
      if (ai().script_engine().functor(
              "xr_danger.CDangerManager__on_before_add", funct))
        if (funct(m_object->lua_game_object(), object,
                  object.m_object_params.m_position, object.m_level_time,
  ...
  if (obj && m_object->is_relation_enemy(obj)) {
    ...
    add(CDangerObject(obj, object.m_object_params.m_position,
                      object.m_level_time, CDangerObject::eDangerTypeEnemySound,
                      CDangerObject::eDangerPerceiveTypeSound));
    return;
  }
}
...
void CDangerManager::add(const CHitObject &object) {
  ...
  if (ai().script_engine().functor("xr_danger.CDangerManager__on_before_add",
                                   funct))
    if (funct(m_object->lua_game_object(), object, obj->Position(),
              object.m_level_time, CDangerObject::eDangerTypeAttacked,
              CDangerObject::eDangerPerceiveTypeHit) == false)
      return;
...
void CDangerManager::add(const CDangerObject &object) {
  ...
  OBJECTS::iterator I =
      std::find_if(m_objects.begin(), m_objects.end(), CFindPredicate(object));
  if (I != m_objects.end()) {
    *I = object;
    return;
  }
