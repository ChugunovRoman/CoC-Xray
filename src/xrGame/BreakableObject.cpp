  ...
  if (m_pPhysicsShell) {
    if (pHDS->hit_type == ALife::eHitTypeExplosion) {
      ...
      if ((pHDS->impulse > EPS) && (BI_NONE != pHDS->bone()))
        m_pPhysicsShell->applyImpulseTrace(pHDS->p_in_bone_space, pHDS->dir,
                                           pHDS->impulse, pHDS->bone());
    }
  }
}
...
void CBreakableObject::CreateBroken() {
  ...
  m_Shell->SetMaxAABBRadius(
      _max(_max(b.m_halfsize.x, b.m_halfsize.y), b.m_halfsize.z) * 2.f); //+2.f
}
...
void CBreakableObject::CollisionHit(u16 source_id, u16 bone_id, float c_damage,
