...
bool CLevel::net_start_client3() {
  if (connected_to_server) {
    ...
    if (psNET_direct_connect) // single
    {
      ...
      level_name =
          name().c_str(); // Server->level_name (server_options).c_str();
    ...
    } else                                                       // multiplayer
    {
      ...
      rescan_mp_archives(); // because if we are using psNET_direct_connect, we
                            // not download map...
    }
...
bool CLevel::net_start_client4() {
  if (connected_to_server) {
    ...
    physics_world()->set_step_time_callback(
        (PhysicsStepTimeCallback *)&PhisStepsCallback);
