...
IC static void generate_orthonormal_basis1(const Fvector &dir, Fvector &updir,
                                           Fvector &right) {
...
void CActor::g_cl_CheckControls(u32 mstate_wf, Fvector &vControlAccel,
                                float &Jump, float dt) {
    ...
    if (state_anm) { // play moving cam effect
      ...
      if (control_entity) {
        ...
        if (NULL == ec) {
          ...
          strconcat(sizeof(anm_name), anm_name, "camera_effects\\actor_move\\",
                    eff_name);
...
void CActor::g_Orientate(u32 mstate_rl, float dt) {
  ...
  if (mstate_rl & mcLookout) {
    tgt_roll =
        (mstate_rl & mcLLookout) ? -ACTOR_LLOOKOUT_ANGLE : ACTOR_RLOOKOUT_ANGLE;
...
bool CActor::CanSprint() {
  bool can_Sprint = CanAccelerate() && !conditions().IsCantSprint() &&
                    Game().PlayerCanSprint(this) && CanRun() &&
