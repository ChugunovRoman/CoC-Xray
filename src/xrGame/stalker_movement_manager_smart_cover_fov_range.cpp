...
bool stalker_movement_manager_smart_cover::in_fov(
    shared_str const &cover_id, shared_str const &loophole_id,
...
bool stalker_movement_manager_smart_cover::in_current_loophole_fov(
  ...
  if (m_current.cover())
    return (m_current.cover()->is_position_in_fov(*m_current.cover_loophole(),
                                                  position));
...
bool stalker_movement_manager_smart_cover::in_current_loophole_range(
  ...
  VERIFY(entering_smart_cover_with_animation() || m_current.cover_loophole());
  if (m_current.cover())
    return (m_current.cover()->is_position_in_range(*m_current.cover_loophole(),
                                                    position));
