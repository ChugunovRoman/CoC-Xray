...
bool bfGetActionSuccessProbability(
    ...
    float fMinProbability, CBaseFunction &fSuccessProbabilityFunction) {
...
u32 dwfChooseAction(u32 dwActionRefreshRate, float fMinProbability0,
                    float fMinProbability1, float fMinProbability2,
                    float fMinProbability3, u32 dwTeam, u32 dwSquad,
  ...
  if (!tpEntity)
    for (int k = 0; k < (int)Group.members().size(); ++k) {
      if (Group.members()[k]->g_Alive() &&
          ((Group.members()[k]->spatial.type & STYPE_VISIBLEFORAI) ==
           STYPE_VISIBLEFORAI))
...
#endif
    } else if (!!(tpMotionDef = tpKinematics->ID_FX_Safe(strconcat(
