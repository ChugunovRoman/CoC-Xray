...
bool CAI_Stalker::useful(const CItemManager *manager,
  ...
  if (explosive && (explosive->CurrentParentID() != 0xffff)) {
    ...
    if (entity_alive)
      memory().danger().add(
          CDangerObject(entity_alive, object->Position(), Device.dwTimeGlobal,
  ...
  CInventoryItem *inventory_item_non_const =
      const_cast<CInventoryItem *>(inventory_item);
...
ALife::ERelationType
CAI_Stalker::tfGetRelationType(const CEntityAlive *tpEntityAlive) const {
  ...
  if (pOtherIO &&
      !(const_cast<CEntityAlive *>(tpEntityAlive)->cast_base_monster()))
...
void CAI_Stalker::react_on_grenades() {
  ...
  //		interval				= missile->destroy_time()
  //- Device.dwTimeGlobal + AFTER_GRENADE_DESTROYED_INTERVAL;
  ...
  if (missile && agent_manager().member().group_behaviour()) {
    //		Msg						("%6d : Stalker %s :
    //grenade reaction",Device.dwTimeGlobal,*m_object->cName());
