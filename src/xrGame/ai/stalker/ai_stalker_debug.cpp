...
void draw_planner(const planner_type &brain, LPCSTR start_indent, LPCSTR indent,
  ...
  for (; I != E; ++I) {
    xr_vector<planner_type::COperatorCondition>::const_iterator J =
        std::lower_bound(brain.current_state().conditions().begin(),
    ...
    if ((J != brain.current_state().conditions().end()) &&
        ((*J).condition() == (*I).first)) {
      temp = (*J).value() ? '+' : '-';
      DBG_OutText("%s%s%s    %5c : [%d][%s]", start_indent, indent, indent,
                  temp, (*I).first, _brain.property2string((*I).first));
    }
  }
  // goal
  ...
  for (; I != E; ++I) {
    xr_vector<planner_type::COperatorCondition>::const_iterator J =
        std::lower_bound(brain.target_state().conditions().begin(),
...
void draw_restrictions(const shared_str &restrictions, LPCSTR start_indent,
                       LPCSTR indent, LPCSTR header) {
...
void CAI_Stalker::debug_text() {
  ...
  if (memory().enemy().selected()) {
    ...
    if (g_Alive()) {
      ...
      u32 result =
          dwfChooseAction(2000, 1.f - interval, 1.f - 2 * interval,
                          1.f - 3 * interval, panic_threshold(), g_Team(),
                          g_Squad(), g_Group(), 0, 1, 2, 3, 4, this, 300.f);
      switch (result) {
      case 0: {
        description = "attack";
        left = 1.f;
      ...
      DBG_OutText("%s%s%svictory   : [%5.2f%%,%5.2f%%] -> %s", indent, indent,
                  indent, 100.f * right, 100.f * left, description);
    }
  }
  // danger
  ...
  if (memory().danger().selected() && memory().danger().selected()->object()) {
    ...
    DBG_OutText(
        ...
        float(Device.dwTimeGlobal - memory().danger().selected()->time()) /
            1000.f);
  ...
  if (!movement().detail().path().empty()) {
    ...
    DBG_OutText("%s%s%s%sposition  : [%f][%f][%f]", indent, indent, indent,
                indent,
                VPUSH(movement()
                          .detail()
  ...
  DBG_OutText("%s%s%sapply loophole direction distance : %.2f", indent, indent,
              indent, movement().apply_loophole_direction_distance());
  ...
  if (movement().current_params().cover_fire_position()) {
    ...
    DBG_OutText("%s%s%sfire position current : [%f][%f][%f]", indent, indent,
                indent, VPUSH(position));
  }

  if (movement().target_params().cover_fire_position()) {
    Fvector position = *movement().target_params().cover_fire_position();
    DBG_OutText("%s%s%sfire position target  : [%f][%f][%f]", indent, indent,
                indent, VPUSH(position));
  }
  ...
  if (movement().restrictions().out_restrictions().size() ||
    ...
    draw_restrictions(movement().restrictions().out_restrictions(), temp,
                      indent, "out");
    draw_restrictions(movement().restrictions().in_restrictions(), temp, indent,
                      "in");
    draw_restrictions(movement().restrictions().base_out_restrictions(), temp,
                      indent, "base out");
    draw_restrictions(movement().restrictions().base_in_restrictions(), temp,
                      indent, "base in");
  }
...
public:
  IC ray_query_param(CCustomMonster *holder, float power_threshold,
                     float distance, const Fvector &start_position,
...
void fill_points(CCustomMonster *self, const Fvector &position,
                 const Fvector &direction, float distance,
                 collide::rq_results &rq_storage, COLLIDE_POINTS &points,
                 float &pick_distance) {
  ...
  ray_query_param params(self, self->memory().visual().transparency_threshold(),
...
static Fmatrix aim_on_actor(Fvector const &bone_position,
                            ...
                            Fvector const &target, bool const &debug_draw) {
  ...
  if (debug_draw) {
    ...
    Fvector const weapon_position_target = Fvector().mad(
        weapon_position, weapon_direction, weapon_position.distance_to(target));
  ...
  if (debug_draw) {
    temp.scale(.01f, .01f, .01f);
    transform.transform_dir(temp.c,
                            Fvector().sub(weapon_position, bone_position));
...
#ifdef DEBUG_RENDER
static void draw_bones(IKinematics &kinematics, Fvector const &box_size,
                       u32 const &box_color, u32 const &line_color,
...
static void draw_animation_bones(CAI_Stalker &self, Fmatrix const &transform,
  ...
  draw_bones(*kinematics, Fvector().set(.011f, .011f, .011f),
             D3DCOLOR_XRGB(0, 255, 0), D3DCOLOR_XRGB(0, 255, 255), &transform);
#endif // #ifdef DEBUG_RENDER
  ...
  renderer.draw_obb(g_stalker_skeleton[weapon_bone_id0],
                    Fvector().set(.01f, .01f, .01f), D3DCOLOR_XRGB(255, 0, 0));
...
//	draw_animation_bones		(*this, XFORM(), kinematics, "loophole_3_attack_in_0");
...

  {
    ...
    Level().debug_renderer().draw_line(
        Fidentity, eye_matrix.c, current_direction, D3DCOLOR_XRGB(255, 0, 255));
    Level().debug_renderer().draw_line(
        Fidentity, eye_matrix.c, target_direction, D3DCOLOR_XRGB(255, 255, 0));
  }
  ...
  if (memory().danger().selected() &&
    ...
    if (ai().level_graph().valid_vertex_id(level_vertex_id)) {
      ...
      direction.set(position.x - half_size * float(v->high_cover(0)) / 15.f,
      ...
      direction.set(position.x + half_size * float(v->high_cover(2)) / 15.f,
                    position.y, position.z);
      Level().debug_renderer().draw_line(Fidentity, position, direction,
                                         D3DCOLOR_XRGB(255, 0, 0));
      ...
      // low
      {
        ...
        direction.set(position.x - half_size * float(v->low_cover(0)) / 15.f,
                      position.y, position.z);
        Level().debug_renderer().draw_line(Fidentity, position, direction,
                                           D3DCOLOR_XRGB(255, 0, 0));
        ...
        direction.set(position.x + half_size * float(v->low_cover(2)) / 15.f,
                      position.y, position.z);
        Level().debug_renderer().draw_line(Fidentity, position, direction,
                                           D3DCOLOR_XRGB(255, 0, 0));
