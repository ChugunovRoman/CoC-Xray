...
void CAI_Stalker::g_fireParams(const CHudItem *pHudItem, Fvector &P,
                               Fvector &D) {
...
void CAI_Stalker::Hit(SHit *pHDS) {
  //хит может меняться в зависимости от ранга (новички получают больше хита, чем
  //ветераны)
  SHit HDS = *pHDS;
  HDS.add_wound = true;
  ...
  if (Core.ParamFlags.test(Core.dbgbullet))
    Msg("CAI_Stalker::Hit hit_type=%d | hit_power(%f)*m_fRankImmunity(%f) = %f",
  ...
  if (m_boneHitProtection && HDS.hit_type == ALife::eHitTypeFireWound) {
    ...
    if (!fis_zero(BoneArmor, EPS)) {
      if (ap > BoneArmor) {
        ...
        if (Core.ParamFlags.test(Core.dbgbullet))
          Msg("CAI_Stalker::Hit AP(%f) > BoneArmor(%f) [HitFracNpc=%f] "
              "modified hit_power=%f",
              ap, BoneArmor, m_boneHitProtection->m_fHitFracNpc, hit_power);
      } else {
  ...
  if (g_Alive()) {
    ...
    if (!already_critically_wounded) {
      ...
      if (!invulnerable() && cover && HDS.initiator() &&
          (HDS.initiator()->ID() != ID()) && !fis_zero(HDS.damage()) &&
          brain().affect_cover()) {
    ...
    if (!wounded() && !already_critically_wounded) {
      ...
      if (!became_critically_wounded &&
          animation().script_animations().empty() && (HDS.bone() != BI_NONE)) {
        Fvector D;
        float yaw, pitch;
        D.getHP(yaw, pitch);
...
#pragma todo(                                                                  \
    "Dima to Dima : forward-back bone impulse direction has been determined incorrectly!")
        ...
        //(angle_difference(movement().m_body.current.yaw,-yaw) <= PI_DIV_2 ? 0
        //: 1)); 				if (fx_index != -1)
  ...
  if (g_Alive() && (!m_hit_callback || m_hit_callback(&HDS))) {
    ...
    memory().hit().add(damage_factor * HDS.damage(), HDS.direction(), HDS.who,
                       HDS.boneID);
  }
...
void CAI_Stalker::HitSignal(float amount, Fvector &vLocalDir, CObject *who,
                            s16 element) {}
...
void CAI_Stalker::update_best_item_info_impl() {
  ...
  if (ai().script_engine().functor("ai_stalker.update_best_weapon", funct)) {
    CScriptGameObject *GO =
        funct(this->lua_game_object(),
              m_best_item_to_kill
                  ? m_best_item_to_kill->cast_game_object()->lua_game_object()
                  : NULL);
    if (GO) {
  ...
  if (m_item_actuality && m_best_item_to_kill &&
...
public:
  IC ray_query_param(const CAI_Stalker *holder, float power_threshold,
                     float distance) {
...
void CAI_Stalker::update_range_fov(float &new_range, float &new_fov,
...
bool CAI_Stalker::fire_make_sense() {
  ...
  if ((pick_distance() + PRECISE_DISTANCE) <
...
bool CAI_Stalker::throw_check_error(float low, float high,
...
bool CAI_Stalker::critical_wound_external_conditions_suitable() {
  ...
  //	Msg								("%6d executing
  //critical hit",Device.dwTimeGlobal);
