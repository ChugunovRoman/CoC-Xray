...
const CCoverPoint *
CAI_Stalker::find_best_cover(const Fvector &position_to_cover_from) {
#ifdef _DEBUG
  //	Msg									("* [%6d][%s] search for
  //new cover performed",Device.dwTimeGlobal,*cName());
#endif
#ifdef _DEBUG
  ...
  VERIFY(
      ai().script_engine().functor("ai_stalker.evaluate_cover_point", funct));
  ...
  point = ai().cover_manager().best_cover(
      Position(), 30.f, *m_ce_best, CStalkerMovementRestrictor(this, true));
  if (point) {
    if (funct(this->cast_game_object()->lua_game_object(), point) == true)
      return (point);
  }
...
float CAI_Stalker::best_cover_value(const Fvector &position_to_cover_from) {
  m_ce_best->setup(position_to_cover_from, MIN_SUITABLE_ENEMY_DISTANCE, 250.f,
...
void CAI_Stalker::update_best_cover_actuality(
  ...
  //		m_best_cover_actual				= false;
  //		Msg								("* [%6d][%s] cover became
  //too bad2",Device.dwTimeGlobal,*cName()); 		return;
  //	}
...
const CCoverPoint *
CAI_Stalker::best_cover(const Fvector &position_to_cover_from) {
