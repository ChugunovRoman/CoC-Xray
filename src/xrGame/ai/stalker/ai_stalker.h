...
public:
  ...
  virtual void Think();
  virtual void SelectAnimation(const Fvector &_view, const Fvector &_move,
                               float speed);
  ...
  //const;
  //<- moved to InventoryOwner::GetCurrentOutfit
  ...
  virtual ALife::ERelationType
  tfGetRelationType(const CEntityAlive *tpEntityAlive) const;
...
private:
  void can_kill_entity(const Fvector &position, const Fvector &direction,
                       float distance, collide::rq_results &rq_storage);
  void can_kill_entity_from(const Fvector &position, Fvector direction,
                            float distance);
...
protected:
  u32 fill_items(CInventory &inventory, CGameObject *old_owner,
                 ALife::_OBJECT_ID new_owner_id);
  ...
  void select_items();
  void transfer_item(CInventoryItem *item, CGameObject *old_owner,
...
public:
  ...
  virtual void update_range_fov(float &new_range, float &new_fov,
...
private:
  bool throw_check_error(float low, float high, const Fvector &start,
...
public:
  ...
  void throw_target(const Fvector &position, u32 const vertex_id,
...
public:
  virtual void create_anim_mov_ctrl(CBlend *b, Fmatrix *start_pose,
