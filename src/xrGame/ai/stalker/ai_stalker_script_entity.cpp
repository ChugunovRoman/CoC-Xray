...
bool CAI_Stalker::bfAssignObject(CScriptEntityAction *tpEntityAction) {
  ...
  case eObjectActionFire1: {
    ...
    //			if (!l_tpWeapon)
    //				return	((l_tObjectAction.m_bCompleted = true)
    //== false);
    if (inventory().ActiveItem() && l_tpWeapon) {
      if (l_tpWeapon->GetAmmoElapsed()) {
        //					if (l_tpWeapon->GetAmmoMagSize()
        //> 1)
        // l_tpWeaponMagazined->SetQueueSize(l_tObjectAction.m_dwQueueSize); 					else
  ...
  case eObjectActionReload1: {
    ...
    //					inventory().Action(kWPN_RELOAD,
    // CMD_START); 				else 					l_tObjectAction.m_bCompleted =
    // true;
    //			}
    //			else
