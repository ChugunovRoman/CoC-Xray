...
void CAI_Crow::SAnim::Load(IKinematicsAnimated *visual, LPCSTR prefix) {
  ...
  for (int i = 0;
       (i < MAX_ANIM_COUNT) && (m_Animations.size() < MAX_ANIM_COUNT); ++i) {
    string128 sh_anim;
...
void CAI_Crow::SSound::Load(LPCSTR prefix) {
  ...
  for (int i = 0; (i < MAX_SND_COUNT) && (m_Sounds.size() < MAX_SND_COUNT);
       ++i) {
    string64 name;
...
void CAI_Crow::switch2_FlyUp() {
  smart_cast<IKinematicsAnimated *>(Visual())->PlayCycle(
      m_Anims.m_fly.GetRandom());
}
void CAI_Crow::switch2_FlyIdle() {
  smart_cast<IKinematicsAnimated *>(Visual())->PlayCycle(
      m_Anims.m_idle.GetRandom());
}
void CAI_Crow::switch2_DeathDead() {
  ...
  //
  smart_cast<IKinematicsAnimated *>(Visual())->PlayCycle(
      m_Anims.m_death_dead.GetRandom());
}
void CAI_Crow::switch2_DeathFall() {
  ...
  smart_cast<IKinematicsAnimated *>(Visual())->PlayCycle(
      m_Anims.m_death.GetRandom(), TRUE, cb_OnHitEndPlaying, this);
}
...
void CAI_Crow::state_DeathFall() {
  ...
  if (bPlayDeathIdle) {
    smart_cast<IKinematicsAnimated *>(Visual())->PlayCycle(
        m_Anims.m_death_idle.GetRandom());
    bPlayDeathIdle = false;
  }
#ifdef DEBUG
  VERIFY2(
...
void CAI_Crow::Die(CObject *who) {
  ...
  processing_activate(); // enable UpdateCL for dead crows - especially for
                         // physics support and do it especially before Creating
                         // physics shell or it definitely throws processing
...
void CAI_Crow::shedule_Update(u32 DT) {
  ...
  if ((eDeathFall != st_current) && (eDeathDead != st_current)) {
    ...
    if (fGoalChangeTime <= 0) {
      fGoalChangeTime +=
          fGoalChangeDelta + fGoalChangeDelta * Random.randF(-0.5f, 0.5f);
  ...
  VERIFY2(valid_pos(Position()),
          dbg_valide_pos_string(
              Position(), this, " CAI_Crow::shedule_Update		(u32 DT)"));
#endif
}
...
//---------------------------------------------------------------------
void CAI_Crow::HitSignal(float /**HitAmount/**/, Fvector & /**local_dir/**/,
...
void CAI_Crow::CreateSkeleton() {
  m_pPhysicsShell = P_build_Shell(
      this, false, (BONE_P_MAP *)0); // P_build_SimpleShell(this,0.3f,false);
...
void CAI_Crow::Hit(SHit *pHDS) {
  ...
  callback(GameObject::eHit)(lua_game_object(),
                             who_object ? who_object->lua_game_object() : 0);
}
