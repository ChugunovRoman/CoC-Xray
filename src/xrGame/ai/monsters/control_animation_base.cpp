...
bool CControlAnimationBase::get_animation_info(EMotionAnim anim, u32 index,
                                               MotionID &motion,
                                               float &length) const {
...
bool CControlAnimationBase::CheckTransition(EMotionAnim from, EMotionAnim to) {
  ...
  while (!bVectEmpty) { // вход в цикл, если вектор переходов не пустой
    ...
    bool from_is_good = ((I->from.state_used) ? (I->from.state == state_from)
...
EAction CControlAnimationBase::VelocityIndex2Action(u32 velocity_index) {
  ...
  case MonsterMovement::eVelocityParameterWalkGrowl:
    ...
    // jump
  // case MonsterMovement::eBloodsuckerVelocityParameterJumpGround:	return
  // ACT_JUMP;
...
void CControlAnimationBase::AA_reload(LPCSTR section) {
  ...
  for (u32 i = 0; pSettings->r_line(section, i, &anim_name, &val); ++i) {
    ...
    if (_GetItemCount(val) == 1) {
      ...
      for (u32 k = 0;
           pSettings->r_line(compound_section, k, &unused_line_name, &val);
           ++k) {
