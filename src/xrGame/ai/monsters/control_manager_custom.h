...
class CControlManagerCustom : public CControl_ComBase {
  ...
  DEFINE_VECTOR(SControlRotationJumpData, ROT_JUMP_DATA_VEC,
...
public:
  ...
  void seq_switch(); // Перейти в следующее состояние, если такового не имеется
                     // - завершить
  ...
  void load_jump_data(LPCSTR s1, LPCSTR s2, LPCSTR s3, LPCSTR s4,
  ...
  //-------------------------------------------------------------------------------
  // Rotation Jump
  void add_rotation_jump_data(LPCSTR left1, LPCSTR left2, LPCSTR right1,
                              LPCSTR right2, float angle, u32 flags = 0);
...
private:
  ...
  void fill_rotation_data(SControlRotationJumpData &data, LPCSTR left1,
