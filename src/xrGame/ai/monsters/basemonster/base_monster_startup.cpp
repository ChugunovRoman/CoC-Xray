...
void CBaseMonster::Load(LPCSTR section) {
  ...
  m_head_bone_name =
      READ_IF_EXISTS(pSettings, r_string, section, "bone_head", "bip01_head");
  ...
  if (pSettings->line_exist(section, "protections_sect")) {
    ...
    m_fSkinArmor =
        READ_IF_EXISTS(pSettings, r_float, protections_sect, "skin_armor", 0.f);
    m_fHitFracMonster = READ_IF_EXISTS(pSettings, r_float, protections_sect,
                                       "hit_fraction_monster", 0.1f);
  } else {
    m_fSkinArmor =
        READ_IF_EXISTS(pSettings, r_float, section, "skin_armor", 0.f);
    m_fHitFracMonster = READ_IF_EXISTS(pSettings, r_float, section,
                                       "hit_fraction_monster", 0.1f);
  }
...
void CBaseMonster::PostLoad(LPCSTR section) {
  ...
  aom.enabled =
      (READ_IF_EXISTS(pSettings, r_bool, section, "aom_enabled", FALSE)) != 0;
  ...
  if (pSettings->line_exist(section, "anti_aim_effectors")) {
    ...
    pcstr anti_aim_animation = READ_IF_EXISTS(
        pSettings, r_string, section, "anti_aim_animation", "stand_attack_");
    anim().AddAnim(eAnimAntiAimAbility, anti_aim_animation, -1, &velocity_stand,
                   PS_STAND);
...
#define LOAD_SOUND(sound_name, _type, _prior, _mask, _int_type)                \
  if (pSettings->line_exist(section, sound_name))                              \
    sound().add(pSettings->r_string(section, sound_name),                      \
                DEFAULT_SAMPLE_COUNT, _type, _prior, u32(_mask), _int_type,    \
                m_head_bone_name);
...
void CBaseMonster::reload(LPCSTR section) {
  ...
  LOAD_SOUND("sound_attack_hit", SOUND_TYPE_MONSTER_ATTACKING,
             MonsterSound::eHighPriority + 1, MonsterSound::eCaptureAllChannels,
  ...
  LOAD_SOUND("sound_die", SOUND_TYPE_MONSTER_DYING,
             MonsterSound::eCriticalPriority, MonsterSound::eCaptureAllChannels,
...
BOOL CBaseMonster::net_Spawn(CSE_Abstract *DC) {
  ...
  monster_squad().register_member((u8)g_Team(), (u8)g_Squad(), (u8)g_Group(),
                                  this);
  settings_overrides();
  ...
  //float prob = Random.randF(); 			if ((prob <
  // m_spawn_probability) || fsimilar(m_spawn_probability,1.f))
  ...
  // (alife_object) 				alife_object->m_flags.set (CSE_ALifeObject::flCanSave,FALSE);
  //
  //			{
  //				NET_Packet				P;
...
void CBaseMonster::net_Destroy() {
  ...
  monster_squad().remove_member((u8)g_Team(), (u8)g_Squad(), (u8)g_Group(),
                                this);
...
void CBaseMonster::settings_read(CInifile const *ini, LPCSTR section,
  ...
  READ_SETTINGS(data.m_dwDistantIdleSndDelay, "distant_idle_sound_delay", r_u32,
                ini, section);
  ...
  if (ini->line_exist(section, "attack_effector")) {
    ...
    READ_SETTINGS(data.m_attack_effector.ppi.duality.h, "duality_h", r_float,
                  ini, ppi_section);
    READ_SETTINGS(data.m_attack_effector.ppi.duality.v, "duality_v", r_float,
                  ini, ppi_section);
    ...
    READ_SETTINGS(data.m_attack_effector.ppi.blur, "blur", r_float, ini,
                  ppi_section);
    READ_SETTINGS(data.m_attack_effector.ppi.noise.intensity, "noise_intensity",
                  r_float, ini, ppi_section);
    READ_SETTINGS(data.m_attack_effector.ppi.noise.grain, "noise_grain",
                  r_float, ini, ppi_section);
    READ_SETTINGS(data.m_attack_effector.ppi.noise.fps, "noise_fps", r_float,
                  ini, ppi_section);
    ...
    READ_SETTINGS(data.m_attack_effector.time, "time", r_float, ini,
                  ppi_section);
    READ_SETTINGS(data.m_attack_effector.time_attack, "time_attack", r_float,
                  ini, ppi_section);
    READ_SETTINGS(data.m_attack_effector.time_release, "time_release", r_float,
                  ini, ppi_section);

    READ_SETTINGS(data.m_attack_effector.ce_time, "ce_time", r_float, ini,
                  ppi_section);
    READ_SETTINGS(data.m_attack_effector.ce_amplitude, "ce_amplitude", r_float,
                  ini, ppi_section);
    READ_SETTINGS(data.m_attack_effector.ce_period_number, "ce_period_number",
                  r_float, ini, ppi_section);
    READ_SETTINGS(data.m_attack_effector.ce_power, "ce_power", r_float, ini,
                  ppi_section);
  }
}
