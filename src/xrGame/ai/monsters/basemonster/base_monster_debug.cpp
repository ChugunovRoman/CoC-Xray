...
CBaseMonster::SDebugInfo CBaseMonster::show_debug_info() {
  ...
  if (SoundMemory.IsRememberSound()) {
    ...
    if (sound_elem.who)
      xr_sprintf(text, "Sound: type[%s] time[%u] power[%.3f] val[%i] src[+]",
...
void add_debug_info(debug::text_tree &root_s, SoundElem &sound_elem,
                    bool dangerous) {
...
void add_enemy_debug_info(debug::text_tree &root_s, const CCustomMonster *pThis,
  ...
  } else if (CCustomMonster *cm =
                 const_cast<CEntityAlive *>(pEnemy)->cast_custom_monster()) {
...
void CBaseMonster::add_debug_info(debug::text_tree &root_s) {
  ...
  if (EnemyMan.get_enemy()) {
    map_home_s.add_line("Enemy_At_Min",
                        Home->at_min_home(EnemyMan.get_enemy()->Position()));
    map_home_s.add_line("Enemy_At_Mid",
                        Home->at_mid_home(EnemyMan.get_enemy()->Position()));
    map_home_s.add_line("Enemy_At_Max",
                        Home->at_home(EnemyMan.get_enemy()->Position()));

    map_home_s.add_line("Dist_To_Enemy", Position().distance_to(
                                             EnemyMan.get_enemy()->Position()));
  }
  ...
  if (p_blend) {
    ...
    current_animation_s.add_line("Script_Animation?",
                                 p_blend->motionID == m_tpScriptAnimation);
  } else {
  ...
  if (!movement().detail().path().empty()) {
    ...
    current_point_s.add_line("Index",
  ...
  atackdist_s.add_line("Mind_Dist",
                       make_xrstr("%.3f", MeleeChecker.get_min_distance()));
  atackdist_s.add_line("Max_Dist",
                       make_xrstr("%.3f", MeleeChecker.get_max_distance()));
  atackdist_s.add_line("As_Step",
                       make_xrstr("%.3f", MeleeChecker.dbg_as_step()));
  atackdist_s.add_line("As_MinDist",
                       make_xrstr("%.3f", MeleeChecker.dbg_as_min_dist()));
