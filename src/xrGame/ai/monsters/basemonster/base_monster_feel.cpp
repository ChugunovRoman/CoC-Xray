...
void CBaseMonster::feel_sound_new(CObject *who, int eType,
...
void CBaseMonster::HitEntity(const CEntity *pEntity, float fDamage,
  ...
  if (EnemyMan.get_enemy() == pEntity) {
    ...
    NET_Packet l_P;
    SHit HS;
    HS.GenHeader(GE_HIT, pEntityNC->ID()); //		u_EventGen
    ...
    HS.hit_type =
        hit_type; //		l_P.w_u16	( u16(ALife::eHitTypeWound) );
    HS.Write_Packet(l_P);
    u_EventSend(l_P);
...
void CBaseMonster::HitSignal(float amount, Fvector &vLocalDir, CObject *who,
                             s16 element) {
  if (!g_Alive())
    return;
...
void CBaseMonster::Hit_Psy(CObject *object, float value) {
  ...
  SHit HS;
  HS.GenHeader(
      GE_HIT, object->ID()); //					//	u_EventGen		(P,GE_HIT,
                             //object->ID());
                             ////
  HS.whoID = (ID());         // own		//	P.w_u16 (ID());
                             // // own
  HS.weaponID = (ID());      // own		//	P.w_u16			(ID());
                             // // own
  HS.dir = (Fvector().set(
      0.f, 1.f,
      0.f));          // direction	//	P.w_dir
  ...
  HS.hit_type = (ALife::eHitTypeTelepatic); //	P.w_u16
                                            //(u16(ALife::eHitTypeTelepatic));
  HS.Write_Packet(P);
  u_EventSend(P);
}

void CBaseMonster::Hit_Wound(CObject *object, float value, const Fvector &dir,
                             float impulse) {
  NET_Packet P;
  SHit HS;
  HS.GenHeader(
