...
CBaseMonster::CBaseMonster()
    : m_psy_aura(this, "psy"), m_fire_aura(this, "fire"),
      m_radiation_aura(this, "radiation"), m_base_aura(this, "base") {
...
void CBaseMonster::update_pos_by_grouping_behaviour() {
  ...
  const float dt =
      0.001f * (Device.dwTimeGlobal - m_last_grouping_behaviour_update_tick);
  ...
  new_vertex = ai().level_graph().check_position_in_direction(old_vertex,
                                                              old_pos, new_pos);
...
bool accessible_epsilon(CBaseMonster *const object, Fvector const pos,
                        float epsilon) {
  Fvector const offsets[] = {
      Fvector().set(0.f, 0.f, 0.f), Fvector().set(-epsilon, 0.f, 0.f),
      Fvector().set(+epsilon, 0.f, 0.f), Fvector().set(0.f, 0.f, -epsilon),
...
void CBaseMonster::set_state_sound(u32 type, bool once) {
  if (once) {
    ...
    // handle situation, when monster want to play attack sound for the first
    // time
    if ((type == MonsterSound::eMonsterSoundAggressive) &&
      ...
      case MonsterSound::eMonsterSoundIdle:
        ...
        if (Actor()->Position().distance_to(Position()) >
          ...
          delay =
              u32(float(db().m_dwIdleSndDelay) * _sqrt(float(objects_count)));
        }
      ...
      case MonsterSound::eMonsterSoundPanic:
        delay =
            u32(float(db().m_dwAttackSndDelay) * _sqrt(float(objects_count)));
        break;
      }
...
void CBaseMonster::load_effector(LPCSTR section, LPCSTR line,
...
float CBaseMonster::get_attack_on_move_far_radius() {
  float radius =
      override_if_debug("aom_far_radius", m_attack_on_move_params.far_radius);
...
bool CBaseMonster::is_paused() const {
  ...
  return ai_dbg::get_var(id_paused_var_name, monster_result) ? monster_result
                                                             : monsters_result;
}
