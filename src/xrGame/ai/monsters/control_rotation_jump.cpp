...
void CControlRotationJump::activate() {
  ...
  float yaw =
      Fvector()
          .sub(m_object->EnemyMan.get_enemy()->Position(), m_object->Position())
          .getH();
...
void CControlRotationJump::on_event(ControlCom::EEventType type,
  ...
  case ControlCom::eventAnimationEnd:
    if ((m_stage == eStop) &&
        (m_data.flags.is(SControlRotationJumpData::eRotateOnce) == FALSE))
      build_line_second();
    else
...
void CControlRotationJump::stop_at_once() {
  m_time = m_man->animation().motion_time(m_right_side ? m_data.anim_stop_rs
                                                       : m_data.anim_stop_ls,
                                          m_object->Visual());
...
void CControlRotationJump::build_line_first() {
  ...
  float target_yaw =
      angle_normalize(-m_object->Direction().getH() +
                      (m_right_side ? m_data.turn_angle : -m_data.turn_angle));
  ctrl_data_dir->heading.target_angle = target_yaw;
...
void CControlRotationJump::build_line_second() {
  ...
  // get animation time
  m_time = m_man->animation().motion_time(m_right_side ? m_data.anim_run_rs
                                                       : m_data.anim_run_ls,
                                          m_object->Visual());
