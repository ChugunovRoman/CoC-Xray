...
class CControl_Manager {
  ...
  DEFINE_MAP(ControlCom::EEventType, CONTROLLERS_VECTOR, LISTENERS_MAP,
             LISTENERS_MAP_IT);
  LISTENERS_MAP m_listeners;

  // contains all available controllers
  DEFINE_MAP(ControlCom::EControlType, CControl_Com *, CONTROLLERS_MAP,
             CONTROLLERS_MAP_IT);
...
public:
  ...
  bool build_path_line(CControl_Com *, const Fvector &target, u32 node,
                       u32 vel_mask);
