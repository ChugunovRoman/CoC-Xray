...
void bonesBone::Turn(u32 dt) {
  ...
  float cur_speed =
      params.r_speed *
      _cos(PI_DIV_2m -
           PIm * _abs(params.target_yaw - params.cur_yaw) / params.dist_yaw);
...
void bonesManipulation::SetMotion(CBoneInstance *bone, u8 axis,
                                  float target_yaw, float r_speed, u32 t) {
  int index = -1;
...
void bonesManipulation::Update(CBoneInstance *bone, u32 cur_time) {
  ...
  // если выполняется наращивание угла и ни одна кость не повернулась (достигли
  // таргета...)
  if (!bones_were_turned && !in_return_state) {
    ...
    if ((0 != time_started) &&
        (time_started + freeze_time < cur_time)) { // время вышло?
      time_started = 0;
