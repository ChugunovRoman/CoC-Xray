...
void CControlAnimation::play_part(SAnimationPart &part, PlayCallback callback) {
  ...
  if (part.blend && !part.blend->stop_at_end)
    ...
    // Msg				("%6d Playing animation : %s , %s ,
    // Object %s",Device.dwTimeGlobal,
...
void CControlAnimation::add_anim_event(MotionID motion, float time_perc,
                                       u32 id) {
    ...
    if (it != m_anim_events.end()) {
      ...
      for (ANIMATION_EVENT_VEC_IT event_it = it->second.begin();
        ...
        if (!event.handled && (event.time_perc < cur_perc)) {
          ...
          // gen event
          SAnimationSignalEventData anim_event(part.get_motion(),
                                               event.time_perc, event.event_id);
