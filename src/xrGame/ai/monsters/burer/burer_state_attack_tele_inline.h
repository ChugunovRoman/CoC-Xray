...
bool CStateBurerAttackTele<Object>::check_completion() {
  float dist =
      object->EnemyMan.get_enemy()->Position().distance_to(object->Position());
...
void CStateBurerAttackTele<Object>::FindFreeObjects(
    xr_vector<CObject *> &tpObjects, const Fvector &pos) {
  ...
  for (u32 i = 0; i < tpObjects.size(); i++) {
    ...
    if (grenade || // grenades are handled by HandleGrenades function
        !obj || !obj->PPhysicsShell() || !obj->PPhysicsShell()->isActive() ||
        custom_monster ||
        ...
        (obj == object) || object->CTelekinesis::is_active_object(obj) ||
...
bool CStateBurerAttackTele<Object>::CheckTeleStart() {
  ...
  float dist =
      object->Position().distance_to(object->EnemyMan.get_enemy()->Position());
  if (dist < object->m_tele_min_distance)
    return false;
  if (dist > object->m_tele_max_distance)
    return false;
