...
void CBurer::reload(LPCSTR section) {
  ...
  sound().add(pSettings->r_string(section, "sound_gravi_attack"),
              ...
              MonsterSound::eHighPriority + 2, u32(MonsterSound::eBaseChannel),
              eMonsterSoundGraviAttack, get_head_bone_name());

  sound().add(pSettings->r_string(section, "sound_tele_attack"),
              DEFAULT_SAMPLE_COUNT, SOUND_TYPE_MONSTER_ATTACKING,
              MonsterSound::eHighPriority + 3, u32(MonsterSound::eBaseChannel),
              eMonsterSoundTeleAttack, get_head_bone_name());
}
...
void CBurer::Load(LPCSTR section) {
  ...
  // eAnimStandDamaged); anim().AddReplacedAnim		(&m_bDamaged, eAnimRun,
  // eAnimRunDamaged);
  ...
  ::Sound->create(sound_gravi_wave,
                  pSettings->r_string(section, "sound_gravi_wave"), st_Effect,
                  SOUND_TYPE_WORLD);
  ::Sound->create(sound_tele_hold,
                  pSettings->r_string(section, "sound_tele_hold"), st_Effect,
                  SOUND_TYPE_WORLD);
  ::Sound->create(sound_tele_throw,
                  pSettings->r_string(section, "sound_tele_throw"), st_Effect,
                  SOUND_TYPE_WORLD);
  ...
  m_weapon_drop_velocity =
      READ_IF_EXISTS(pSettings, r_float, section, "weapon_drop_velocity", 8);
  ...
  m_tele_fly_velocity =
      READ_IF_EXISTS(pSettings, r_float, section, "tele_fly_velocity", 30.f);
  m_tele_object_height =
      READ_IF_EXISTS(pSettings, r_float, section, "tele_object_height", 2.f);
  ...
  // "fx_stand_r"); 	anim().AddAnim(eAnimEat,			"sit_eat_",
  ...
  // -1, &velocity_none,		PS_SIT); //, 	"fx_stand_f",
  // "fx_stand_b", "fx_stand_l", "fx_stand_r");
  // anim().AddAnim(eAnimCheckCorpse,
  // "sit_check_corpse_",	-1, &velocity_none,		PS_SIT); //,
  // "fx_stand_f", "fx_stand_b", "fx_stand_l", "fx_stand_r");
  // 	anim().AddAnim(eAnimSitStandUp,		"sit_stand_up_",
  // -1, &velocity_none,		PS_SIT); //, 	"fx_stand_f",
  // "fx_stand_b", "fx_stand_l", "fx_stand_r");
  // anim().AddAnim(eAnimStandSitDown,
...
void CBurer::UpdateGraviObject() {
  ...
  if (Level().ObjectSpace.RayPick(new_pos, dir, trace_dist, collide::rqtBoth,
                                  l_rq, NULL)) {
    ...
    if ((l_rq.O == enemy) && (l_rq.range < trace_dist)) {
      ...
      if (b_enemy_visible) {
        ...
        HitEntity(m_gravi_object.enemy, m_gravi.hit_power,
                  m_gravi.impulse_to_enemy, impulse_dir, ALife::eHitTypeStrike,
                  false);
        m_gravi_object.deactivate();
        return;
      }
