...
// Special Build Path
//////////////////////////////////////////////////////////////////////////
bool CControlPathBuilder::build_special(const Fvector &target, u32 node,
                                        u32 vel_mask) {
...
void CControlPathBuilder::fix_position(const Fvector &pos, u32 node,
                                       Fvector &res_pos) {
  ...
  if (!accessible(res_pos)) {
    u32 level_vertex_id =
        restrictions().accessible_nearest(Fvector().set(res_pos), res_pos);

#ifdef DEBUG
    if (level_vertex_id != node) {
      Msg("! src_node[%d] res_node[%d] src_pos[%f,%f,%f] res_pos[%f,%f,%f]",
...
bool CControlPathBuilder::get_node_in_radius(u32 src_node, float min_radius,
                                             float max_radius, u32 attempts,
                                             u32 &dest_node) {
