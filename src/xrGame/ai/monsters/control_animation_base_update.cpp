...
// Velocity_Chain 		установить скорость анимации в соответствие с физ
// скоростью
void CControlAnimationBase::SelectVelocities() {
  ...
  if (b_moving) {
    ...
    // движении), то реализовать поворот на месте, а дальше форсировать скорость
    // со следующей точки
  ...
  if (!m_object->state_invisible && !fis_zero(anim_vel.linear)) {
    ...
    if (accel_chain_get(m_man->movement().real_velocity(),
                        cur_anim_info().get_motion(), new_anim, a_speed)) {
  ...
  if (m_object->state_invisible)
    ...
    if (m_tAction == ACT_ATTACK) {
      ...
      m_object->dir().set_heading_speed(
          vel *
          m_object
              ->m_melee_rotation_factor); // todo: make as an external factor
    } else
      m_object->dir().set_heading_speed(
          item_it->velocity.velocity.angular_real);
  }
