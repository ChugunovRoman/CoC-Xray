...
void CMonsterSoundMemory::HearSound(const CObject *who, int eType,
...
void CMonsterSoundMemory::UpdateHearing() {
  ...
  Sounds.erase(std::remove_if(Sounds.begin(), Sounds.end(),
                              pred_remove_nonactual_sounds(Device.dwTimeGlobal -
                                                           time_memory)),
               Sounds.end());

  // пересчитать value
  for (xr_vector<SoundElem>::iterator I = Sounds.begin(); I != Sounds.end();
       ++I)
...
void CMonsterSoundMemory::remove_links(CObject *O) {
  // удаление устаревших звуков
  Sounds.erase(
      std::remove_if(Sounds.begin(), Sounds.end(), pred_remove_relcase(O)),
      Sounds.end());
}
