...
CVampireCameraEffector::CVampireCameraEffector(float time, const Fvector &src,
                                               const Fvector &tgt)
...
BOOL CVampireCameraEffector::ProcessCam(SCamEffectorInfo &info) {
  ...
  float cur_dist =
      m_dist *
      (_sqrt(0.5f * 0.5f - (time_passed - 0.5f) * (time_passed - 0.5f)));
  ...
  if (time_left_perc < 0.2f) {
    ...
    angle_lerp(dangle_current.x, dangle_target.x,
               _abs(dangle_current.x / fLifeTime + 0.001f), Device.fTimeDelta);
    angle_lerp(dangle_current.y, dangle_target.y,
               _abs(dangle_current.y / fLifeTime + 0.001f), Device.fTimeDelta);
    angle_lerp(dangle_current.z, dangle_target.z,
               _abs(dangle_current.z / fLifeTime + 0.001f), Device.fTimeDelta);
