...
void CAI_Bloodsucker::Load(LPCSTR section) {
  ...
  if (pSettings->line_exist(section, "is_no_fx")) {
    ...
    anim().AddAnim(eAnimSleepStanding, "stand_sleep_", -1, &velocity_none,
                   PS_STAND);
    ...
    anim().AddAnim(eAnimCheckCorpse, "stand_check_corpse_", -1, &velocity_none,
                   PS_STAND, "fx_run_f", "fx_stand_b", "fx_stand_l",
                   "fx_stand_r");
    anim().AddAnim(eAnimEat, "sit_eat_", -1, &velocity_none, PS_SIT, "fx_run_f",
  ...
  m_sufficient_hits_before_vampire =
      pSettings->r_u32(section, "Vampire_Sufficient_Hits");
  ...
  m_full_visibility_radius =
      READ_IF_EXISTS(pSettings, r_float, section, full_visibility_radius_string,
...
void CAI_Bloodsucker::reinit() {
  ...
  com_man().add_rotation_jump_data("run_turn_l_0", "run_turn_l_1",
                                   "run_turn_r_0", "run_turn_r_1", PI_DIV_2);
...
void CAI_Bloodsucker::reload(LPCSTR section) {
  ...
  sound().add(pSettings->r_string(section, "Sound_Invisibility_Change_State"),
              ...
              MonsterSound::eChannelIndependent << 1, eChangeVisibility,
              "bip01_head");
...
void CAI_Bloodsucker::vfAssignBones() {
  ...
  if (!PPhysicsShell()) //нельзя ставить колбеки, если создан физ шел - у него
                        //стоят свои колбеки!!!
  {
...
void CAI_Bloodsucker::LookDirection(Fvector to_dir, float bone_turn_speed) {
  ...
  // float dy = _abs(angle_normalize_signed(yaw - cur_yaw));		// дельта,
  // на которую нужно поворачиваться
...
void CAI_Bloodsucker::set_drag_jump(CEntityAlive *e, LPCSTR s,
...
void CAI_Bloodsucker::HitEntity(const CEntity *pEntity, float fDamage,
