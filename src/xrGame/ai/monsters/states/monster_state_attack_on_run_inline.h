...
void ATTACK_ON_RUN_STATE::set_movement_phaze(phaze const new_phaze) {
  ...
  if (m_phaze == go_close) {
    ...
    bool left_side =
        ((self2enemy.x * self_dir.z) - (self2enemy.z * self_dir.x)) > 0.f;
...
void ATTACK_ON_RUN_STATE::update_aim_side() {
  ...
  aim_side const new_attack_side =
      (self_dir.x * self_to_enemy.z - self_dir.z * self_to_enemy.x > 0) ? right
                                                                        : left;
...
void ATTACK_ON_RUN_STATE::select_prepare_fallback_target() {
  ...
  for (u32 index = 0; index < move_scan_points; ++index) {
    ...
    Fvector const scan_point =
        enemy_pos + rotate_point(Fvector().set(far_radius, 0, 0), angle);
...
void ATTACK_ON_RUN_STATE::update_attack() {
  if (m_attacking) {
    ...
    for (ENEMIES_MAP::const_iterator it = memory.begin(); it != memory.end();
         ++it) {
      ...
      if (current_atack_dist < disallowed_atack_distance &&
          m_phaze == go_close && current_time() > m_phaze_chosen_time + 3000 &&
          enemy == main_enemy) {
        set_movement_phaze(go_prepare);
      }
    ...
    if (can_attack) {
      ...
      object->anim().set_override_animation(anim,
