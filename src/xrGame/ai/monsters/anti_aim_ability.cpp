...
void anti_aim_ability::load_from_ini(CInifile const *ini, pcstr const section) {
  ...
  m_freeze_time =
      READ_IF_EXISTS(ini, r_float, section, s_anti_aim_freeze_time_string, 1.f);
  m_max_angle =
      READ_IF_EXISTS(ini, r_float, section, s_anti_aim_max_angle_string, 0.5f);
  ...
  pcstr effectors =
      READ_IF_EXISTS(ini, r_string, section, s_anti_aim_effectors_string, NULL);
...
void anti_aim_ability::activate() {
  ...
  m_animation_hit_tick =
      Device.dwTimeGlobal +
      (TTime)(m_object->anim().get_animation_hit_time(eAnimAntiAimAbility, 0) *
              1000);
...
void anti_aim_ability::do_deactivate() {
  if (is_active()) {
    if (m_object->Visual()) // when m_object is destroying, visual might be dead
                            // already
    {
