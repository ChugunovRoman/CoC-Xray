...
void CAI_Dog::Load(LPCSTR section) {
  ...
  anim().AddAnim(eAnimRunTurnLeft, "stand_run_turn_left_", -1, &velocity_run,
                 PS_STAND);
  anim().AddAnim(eAnimRunTurnRight, "stand_run_turn_right_", -1, &velocity_run,
                 PS_STAND);
  ...
  // anim().AddAnim(eAnimSniff,		"stand_sniff_",			-1,
  // &velocity_none, PS_STAND); anim().AddAnim(eAnimHowling,
  // "stand_howling_",		-1, &velocity_none,		PS_STAND);

  // anim().AddAnim(eAnimJumpGlide,   	"jump_glide_",			-1,
  // &velocity_none,		PS_STAND); anim().AddAnim(eAnimJumpGlide,
  // "stand_jump_left_",		 0, &velocity_none,
  // PS_STAND);
...
void CAI_Dog::reinit() {
  ...
  //	                             "stand_jump_right_0","stand_jump_right_0",
...
void CAI_Dog::start_animation() {
  ...
  smart_cast<IKinematicsAnimated *>(Visual())->PlayCycle(
      get_current_animation(), TRUE, animation_end, this);
  b_state_end = true;
}
