...
void CPolterTele::load(LPCSTR section) {
  ...
  m_pmt_object_height =
      READ_IF_EXISTS(pSettings, r_float, section, "Tele_Object_Height", 10.f);
  ...
  m_pmt_raise_time_to_wait_in_objects = READ_IF_EXISTS(
      pSettings, r_u32, section, "Tele_Delay_Between_Objects_Raise_Time", 500);
  m_pmt_fly_velocity =
      READ_IF_EXISTS(pSettings, r_float, section, "Tele_Fly_Velocity", 30.f);
  m_pmt_object_collision_damage = READ_IF_EXISTS(pSettings, r_float, section,
                                                 "Tele_Collision_Damage", 0.5f);
...
void CPolterTele::tele_find_objects(xr_vector<CObject *> &objects,
  ...
  for (u32 i = 0; i < m_nearest.size(); i++) {
    ...
    if (!obj || !obj->PPhysicsShell() || !obj->PPhysicsShell()->isActive() ||
        ...
        (obj == m_object) || m_object->CTelekinesis::is_active_object(obj) ||
...

{
  ...
  SCollisionHitCallback(CPhysicsShellHolder *object,
                        float pmt_object_collision_damage)
      : m_object(object),
        m_pmt_object_collision_damage(pmt_object_collision_damage) {
    VERIFY(object);
  }
  void call(IPhysicsShellHolder *obj, float min_cs, float max_cs, float &cs,
            float &hl, ICollisionDamageInfo *di) {
