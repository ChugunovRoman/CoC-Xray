...
void CPoltergeist::Load(LPCSTR section) {
  ...
  anim().AddAnim(eAnimDie, "stand_idle_", 0, &velocity_none, PS_STAND,
                 "fx_stand_f", "fx_stand_b", "fx_stand_l", "fx_stand_r");
  anim().AddAnim(eAnimMiscAction_00, "fall_down_", -1, &velocity_none, PS_STAND,
                 "fx_stand_f", "fx_stand_b", "fx_stand_l", "fx_stand_r");
  ...
  m_height_change_max_time = READ_IF_EXISTS(pSettings, r_u32, section,
  ...
  m_fly_around_distance = READ_IF_EXISTS(pSettings, r_float, section,
                                         "detection_fly_around_distance", 15.f);
  ...
  m_detection_success_level = READ_IF_EXISTS(pSettings, r_float, section,
                                             "detection_success_level", 4.f);
...
void CPoltergeist::Die(CObject *who) {
  ...
  // 				Fmatrix M;
  // 				M.set
  // (XFORM()); 				M.translate_over
  // (m_current_position); 				PPhysicsShell()->SetTransform	(M);
...
void CPoltergeist::UpdateHeight() {
  ...
  if (time_height_updated < cur_time) {
    time_height_updated = cur_time + Random.randI(m_height_change_min_time,
