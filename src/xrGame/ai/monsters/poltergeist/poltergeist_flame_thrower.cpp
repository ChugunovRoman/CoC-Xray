...
void CPolterFlame::select_state(SFlameElement *elem, EFlameState state) {
  ...
  case eStop:
    ...
    m_object->PlayParticles(m_particles_stop, elem->position, elem->target_dir,
                            TRUE);
    ...
    case eFire:
      if (elem->time_started + m_time_fire_play < time())
        ...
        if (elem->time_last_hit + m_hit_delay < time()) {
          ...
          if (Level().ObjectSpace.RayPick(elem->position, elem->target_dir,
                                          m_length, collide::rqtBoth, rq,
                                          NULL)) {
            if ((rq.O == elem->target_object) && (rq.range < m_length)) {
              ...
              HS.weaponID =
                  (m_object->ID());        //					P.w_u16
                                           //(ID());
              HS.dir = (elem->target_dir); //					P.w_dir
                                           //(element->target_dir);
              HS.power =
                  (hit_value);       //					P.w_float
                                     //(m_flame_hit_value);
...
bool CPolterFlame::get_valid_flame_position(const CObject *target_object,
  ...
  vertex_position =
      ai().level_graph().vertex_position(Obj->ai_location().level_vertex_id());
  new_pos.mad(vertex_position, dir,
              Random.randF(m_min_flame_dist, m_max_flame_dist));
