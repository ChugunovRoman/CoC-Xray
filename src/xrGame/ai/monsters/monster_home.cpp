...
void CMonsterHome::load(LPCSTR line) {
  ...
  if (m_object->spawn_ini() && m_object->spawn_ini()->section_exist(line)) {
    m_path =
        ai().patrol_paths().path(m_object->spawn_ini()->r_string(line, "path"));
...
void CMonsterHome::setup(LPCSTR path_name, float min_radius, float max_radius,
                         bool aggressive, float middle_radius) {
...
void CMonsterHome::setup(u32 lv_ID, float min_radius, float max_radius,
                         bool aggressive, float middle_radius) {
  m_path = 0;
...
u32 CMonsterHome::get_place_in_min_home() {
  ...
  if (result == u32(-1)) {
    // TODO: find more acceptable decision, than return its level_vertex_id, if
    // !accessible
...
bool CMonsterHome::at_home(const Fvector &pos, float const radius) {
  if (!m_path) {
    ...
    float dist =
        pos.distance_to(ai().level_graph().vertex_position(m_level_vertex_id));
    ...
    for (u32 i = 0; i < m_path->vertex_count(); i++) {
      ...
      float dist =
          pos.distance_to(ai().level_graph().vertex_position(level_vertex));
      if (dist < radius)
        return true;
    }
  }
