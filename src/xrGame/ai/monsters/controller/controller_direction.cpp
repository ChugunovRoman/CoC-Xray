...
void CControllerDirection::assign_bones() {
  ...
  m_bone_spine =
      &kinematics->LL_GetBoneInstance(kinematics->LL_BoneID("bip01_spine"));
  m_bone_head =
      &kinematics->LL_GetBoneInstance(kinematics->LL_BoneID("bip01_head"));

  if (!m_controller->PPhysicsShell()) { //нельзя ставить колбеки, если создан
                                        //физ шел - у него стоят свои колбеки!!!
