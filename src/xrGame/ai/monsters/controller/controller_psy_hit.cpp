...
bool check_actor_visibility(const Fvector trace_from, const Fvector trace_to,
                            CObject *object) {
...
void CControllerPsyHit::death_glide_start() {
  ...
  Actor()->Cameras().AddCamEffector(xr_new<CControllerPsyHitCamEffector>(
      ...
      m_man->animation().motion_time(m_stage[1], m_object->Visual()), base_fov,
      dest_fov));
...
void CControllerPsyHit::set_sound_state(ESoundState state) {
  ...
  if (state == ePrepare) {
    ...
    // monster->m_sound_tube_hit_left.play_at_pos(Actor(), Fvector().set(-1.f,
    // 0.f, 1.f), sm_2D); monster->m_sound_tube_hit_right.play_at_pos(Actor(),
