...
CPPEffectorControllerAura::CPPEffectorControllerAura(const SPPInfo &ppi,
                                                     u32 time_to_fade,
  ...
  m_snd_left.play_at_pos(Actor(), Fvector().set(-1.f, 0.f, 1.f),
                         sm_Looped | sm_2D);
  m_snd_right.play_at_pos(Actor(), Fvector().set(-1.f, 0.f, 1.f),
                          sm_Looped | sm_2D);
}
...
void CControllerAura::update_schedule() {
  ...
  if ((dist_to_actor > aura_radius + FAKE_MIN_ADD_DIST) &&
    ...
    if (m_time_fake_aura == 0) {
      ...
      if (active()) {
        ...
        // check to start
        if (m_time_fake_aura < time()) {
          m_effector = xr_new<CPPEffectorControllerAura>(
              m_state, 5000, aura_sound.left, aura_sound.right);
  ...
  if (active()) {
    CEffectorCam *ce =
        Actor()->Cameras().GetCamEffector((ECamEffectorType)effControllerAura2);
    if (!ce)
      AddEffector(Actor(), effControllerAura2, "effector_controller_aura2",
                  0.15f);
  } else {
    CEffectorCam *ce =
        Actor()->Cameras().GetCamEffector((ECamEffectorType)effControllerAura2);
...
void CControllerAura::load(LPCSTR section) {
  ...
  aura_sound.left.create(pSettings->r_string(section, "PsyAura_SoundLeftPath"),
                         st_Effect, sg_SourceType);
  aura_sound.right.create(
      pSettings->r_string(section, "PsyAura_SoundRightPath"), st_Effect,
      sg_SourceType);

  aura_radius =
      READ_IF_EXISTS(pSettings, r_float, section, "PsyAura_Radius", 40.f);
  ...
  m_fake_max_add_dist = READ_IF_EXISTS(pSettings, r_float, section,
                                       "PsyAura_Fake_MaxAddDist", 90.f);
  m_fake_min_add_dist = READ_IF_EXISTS(pSettings, r_float, section,
                                       "PsyAura_Fake_MinAddDist", 20.f);
