...
void CAI_Rat::reload(LPCSTR section) {
  ...
  sound().add(pSettings->r_string(section, "sound_death"), 100,
              SOUND_TYPE_MONSTER_DYING, 0, u32(eRatSoundMaskDie), eRatSoundDie,
              head_bone_name);
  ...
  sound().add(pSettings->r_string(section, "sound_eat"), 100,
              SOUND_TYPE_MONSTER_EATING, 3, u32(eRatSoundMaskEat), eRatSoundEat,
              head_bone_name);
}
...
void CAI_Rat::Die(CObject *who) {
  ...
  CGroupHierarchyHolder &Group =
      Level().seniority_holder().team(g_Team()).squad(g_Squad()).group(
          g_Group());
...
void CAI_Rat::Load(LPCSTR section) {
  ...
  m_fNullASpeed =
      pSettings->r_float(section, "AngularStandSpeed") / 180.f * PI; // PI_MUL_2
  m_fMinASpeed =
      pSettings->r_float(section, "AngularMinSpeed") / 180.f * PI; // PI_MUL_2
  m_fMaxASpeed =
      pSettings->r_float(section, "AngularMaxSpeed") / 180.f * PI; //.2f
  m_fAttackASpeed =
      pSettings->r_float(section, "AngularAttackSpeed") / 180.f * PI; //.15f;
