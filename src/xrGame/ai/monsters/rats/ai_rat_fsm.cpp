...
void CAI_Rat::FreeHuntingActive() {
  ...
  if (bfCheckIfGoalChanged()) {
    if (m_bStateChanged ||
        (Position().distance_to(m_tSpawnPosition) > m_fStableDistance) ||
...
void CAI_Rat::AttackFire() {
  ...
  CHECK_IF_GO_TO_NEW_STATE((memory().enemy().selected()->Position().distance_to(
...
void CAI_Rat::AttackRun() {
  ...
  ERatStates eState = ERatStates(
      dwfChooseAction(m_dwActionRefreshRate, m_fAttackSuccessProbability,
                      m_fAttackSuccessProbability, m_fAttackSuccessProbability,
                      m_fAttackSuccessProbability, g_Team(), g_Squad(),
                      g_Group(), m_eCurrentState, m_eCurrentState,
                      m_eCurrentState, aiRatRetreat, aiRatRetreat, this, 30.f));
  if (eState != m_eCurrentState) {
...
void CAI_Rat::EatCorpse() {
  ...
  //	R_ASSERT							(V);
  //	u16									head_bone
  //= V->LL_BoneID("bip01_head"); 	Fmatrix l_tMatrix; 	l_tMatrix.mul_43
