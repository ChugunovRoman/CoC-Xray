...
bool CControlJump::is_on_the_ground() {
  ...
  if (Level().ObjectSpace.RayPick(trace_from, direction, m_trace_ground_range,
...
Fvector CControlJump::get_target(CObject *obj) {
  ...
  CBoneInstance &bone =
      smart_cast<IKinematics *>(obj->Visual())->LL_GetBoneInstance(bone_id);
...
void CControlJump::calculate_jump_time(Fvector const &target,
  ...
  float ph_time =
      m_object->character_physics_support()->movement()->JumpMinVelTime(target);
...
void CControlJump::on_event(ControlCom::EEventType type,
  ...
  if (type == ControlCom::eventVelocityBounce) {
    ...
    if ((m_anim_state_current == eStateGlide) &&
      ...
      m_object->character_physics_support()->movement()->Jump(m_target_position,
                                                              m_jump_time);
      m_time_started = time();
      ...
      ctrl_data->set_speed(m_man->animation().current_blend()->timeTotal /
                           m_man->animation().current_blend()->speed /
                           m_jump_time);
...
void CControlJump::hit_test() {
  ...
  if (Level().ObjectSpace.RayPick(trace_from, m_object->Direction(),
                                  m_hit_trace_range, collide::rqtObject, l_rq,
                                  m_object)) {
