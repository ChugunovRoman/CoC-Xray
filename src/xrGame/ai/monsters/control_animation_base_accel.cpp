...
bool CControlAnimationBase::accel_chain_test() {
  ...
  for (VELOCITY_CHAIN_VEC_IT I = m_accel.chain.begin();
    ...
    for (SEQ_VECTOR_IT IT = I->begin() + 1; IT != I->end(); IT++) {
      ...
      float from =
          anim_from->velocity.velocity.linear * anim_from->velocity.max_factor;
      float to =
          anim_to->velocity.velocity.linear * anim_to->velocity.min_factor;

      xr_sprintf(error_msg,
                 "Incompatible speed ranges. Monster[%s] From animation  [%s] "
                 "To animation [%s]",
