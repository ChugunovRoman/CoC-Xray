...
BOOL CAI_Trader::net_Spawn(CSE_Abstract *DC) {
  ...
  CBoneInstance *bone_head =
      &smart_cast<IKinematics *>(Visual())->LL_GetBoneInstance(
...
void CAI_Trader::OnEvent(NET_Packet &P, u16 type) {
  ...
  case GE_OWNERSHIP_REJECT: {
    ...
    inventory().DropItem(smart_cast<CGameObject *>(Obj), just_before_destroy,
                         dont_create_shell);
    // if(inventory().DropItem(smart_cast<CGameObject*>(Obj),
    // just_before_destroy)) 	Obj->H_SetParent(0, just_before_destroy); //moved
    // to
    // DropItem
  } break;
...
void CAI_Trader::g_fireParams(const CHudItem *pHudItem, Fvector &P,
                              Fvector &D) {
...
ALife::ERelationType
CAI_Trader::tfGetRelationType(const CEntityAlive *tpEntityAlive) const {
