...
void CStalkerActionGetItemToKill::execute() {
  ...
  object().movement().set_body_state(
      object().movement().body_state() == eBodyStateCrouch ? eBodyStateCrouch
                                                           : eBodyStateStand);
...
void CStalkerActionRetreatFromEnemy::execute() {
  ...
  object().movement().set_mental_state(
      eMentalStateDanger); // Alundaio: Panic animation looks ridiculous, danger
                           // set is better
  object().movement().set_body_state(eBodyStateStand);
  ...
  if (point) {
    ...
    if (object().memory().visual().visible_now(
      ...
      object().movement().set_mental_state(eMentalStateDanger);
      //			u32
      // min_queue_size, max_queue_size, min_queue_interval, max_queue_interval;
      //			float
      // distance =
...
void CStalkerActionKillEnemy::execute() {
  ...
  // Alundaio: Prevent Stalkers from shooting at walls for prolonged periods due
  // to kill if not visible
...
void CStalkerActionTakeCover::initialize() {
  ...
  m_body_state = object().movement().body_state();
  //	m_movement_type								= Random.randI(2)
  //? eMovementTypeRun : eMovementTypeWalk;
...
void CStalkerActionTakeCover::execute() {
  ...
  if (object().memory().visual().visible_now(enemy)) {
    ...
    if (Device.dwTimeGlobal >= level_time + 3000 &&
        _abs(object().Position().y - mem_object.m_object_params.m_position.y) >
            3.5f) {
...
void CStalkerActionLookOut::execute() {
  ...
  const CCoverPoint *point = ai().cover_manager().best_cover(
      ...
      *object()
           .m_ce_close); //,CStalkerMovementRestrictor(m_object,true,false));
...
void CStalkerActionDetourEnemy::initialize() {
  ...
  if (/**(Random.randF(1.f) < .8f) && /**/ object()
    ...
    object().agent_manager().location().add(xr_new<CDangerCoverLocation>(
        ...
        Device.dwTimeGlobal, TEMP_DANGER_INTERVAL, TEMP_DANGER_DISTANCE,
...
void CStalkerActionPostCombatWait::initialize() {
  ...
  if (object().memory().enemy().last_enemy() &&
    ...
    object().sight().setup(
        CSightAction(object().memory().enemy().last_enemy(), true, true));
}
...
void CStalkerActionSuddenAttack::execute() {
  ...
  float distance =
      object().Position().distance_to(mem_object.m_object_params.m_position);
  if (distance >= 15.f) {
    object().movement().set_body_state(eBodyStateStand);
    object().movement().set_movement_type(eMovementTypeRun);
    aim_ready();
