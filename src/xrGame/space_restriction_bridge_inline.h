...
IC u32 CSpaceRestrictionBridge::accessible_nearest(T &restriction,
  ...
  //#pragma todo("Dima to Dima : _Warning : this place can be optimized in case
  // of a slowdown") VERIFY
  // (initialized()); VERIFY
  // (!restriction->border().empty()); VERIFY
  // (!restriction->accessible_neighbour_border(restriction,out_restriction).empty());
  ...
  for (; I != E; ++I) {
    ...
    float distance_sqr =
        ai().level_graph().vertex_position(*I).distance_to_sqr(position);
  ...
  // VERIFY2(ai().level_graph().valid_vertex_id(selected),make_string("vertex_id[%d],
  // object[%s], position[%f][%f][%f]",selected,*name(),VPUSH(position)));
  ...
  if (ai().level_graph().valid_vertex_id(selected)) {
    ...
    for (u32 i = 0; i < 5; ++i) {
      ...
      if (i < 4)
        current.P.y = ai().level_graph().vertex_plane_y(selected, current.P.x,
                                                        current.P.z);

      // VERIFY	(ai().level_graph().inside(selected,current.P));
      // VERIFY	(restriction->inside(selected,!out_restriction) ==
      // out_restriction); VERIFY	(restriction->inside(current) ==
      // out_restriction);
...
CSpaceRestrictionBridge::accessible_neighbour_border(T &restriction,
                                                     bool out_restriction) {
