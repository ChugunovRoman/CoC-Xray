...
void CEntityAlive::Load(LPCSTR section) {
  ...
  conditions().LoadImmunities(pSettings->r_string(section, "immunities_sect"),
                              pSettings);
...
void CEntityAlive::reload(LPCSTR section) {
  ...
  m_ef_detector_type =
      READ_IF_EXISTS(pSettings, r_u32, section, "ef_detector_type", u32(-1));
...
void CEntityAlive::PlaceBloodWallmark(const Fvector &dir,
  ...
  BOOL reach_wall =
      Level().ObjectSpace.RayPick(start_pos, dir, trace_dist, collide::rqtBoth,
                                  result, this) &&
      !result.O;
  ...
  if (reach_wall) {
    ...
    if (pMaterial->Flags.is(SGameMtl::flBloodmark)) {
      ...
      VERIFY(!pwallmarks_vector->empty());
      {
        ...
        //::Render->add_StaticWallmark(wallmarkShader, end_point, wallmark_size,
        // pTri, pVerts);
...
ALife::ERelationType
CEntityAlive::tfGetRelationType(const CEntityAlive *tpEntityAlive) const {
  int relation =
      MONSTER_COMMUNITY::relation(this->monster_community->index(),
...
CEntityAlive::create_entity_condition(CEntityConditionSimple *ec) {
  if (!ec)
...
void CEntityAlive::create_anim_mov_ctrl(CBlend *b, Fmatrix *start_pose,
...
void CEntityAlive::fill_hit_bone_surface_areas() const {
  ...
  for (u16 i = 0, n = kinematics->LL_BoneCount(); i < n; ++i) {
    ...
    case SBoneShape::stBox: {
      ...
      surface_area = 2.f * (half_size.x * (half_size.y + half_size.z) +
...
Fvector CEntityAlive::get_new_local_point_on_mesh(u16 &bone_id) const {
  ...
  VERIFY2(i != e, make_string("m_hit_bone_surface_areas[%d]",
                              m_hit_bone_surface_areas.size()));
  SBoneShape const &shape = kinematics->LL_GetData((*i).first).shape;
  bone_id = (*i).first;
  ...
  case SBoneShape::stBox: {
    ...
    Fvector random = {(random_value & 1) ? -1.f : 1.f,
                      ::Random.randF(2.f) - 1.f, ::Random.randF(2.f) - 1.f};
    random.normalize();
    if (random_value < 2)
  ...
  case SBoneShape::stCylinder: {
    ...
    if (random_value < shape.cylinder.m_height) {
      ...
      height =
          shape.cylinder.m_height / 2.f *
          ((normalized_value < shape.cylinder.m_radius / 2.f) ? -1.f : 1.f);
      radius = height > 0.f ? normalized_value - shape.cylinder.m_radius / 2.f
                            : normalized_value;
    }
