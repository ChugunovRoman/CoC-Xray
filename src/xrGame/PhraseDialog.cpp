...
CPhraseDialog::OurPartner(CPhraseDialogManager *dialog_manager) const {
...
bool CPhraseDialog::SayPhrase(DIALOG_SHARED_PTR &phrase_dialog,
  ...
  last_phrase->GetScriptHelper()->Action(
      pSpeakerGO1, pSpeakerGO2, *phrase_dialog->m_DialogId, phrase_id.c_str());
    ...
    for (xr_vector<CPhraseGraph::CEdge>::const_iterator it =
      ...
      if (next_phrase_vertex->data()->GetScriptHelper()->Precondition(
        ...
        if (psAI_Flags.test(aiDialogs)) {
          ...
          Msg("----added phrase text [%s] phrase_id=[%s] id=[%s] to dialog "
              "[%s]",
...
LPCSTR CPhraseDialog::GetPhraseText(const shared_str &phrase_id,
  ...
  // CPhraseGraph::CVertex* phrase_vertex =
  // data()->m_PhraseGraph.vertex(phrase_id); THROW(phrase_vertex); CPhrase*
  // ph = phrase_vertex->data();
  CPhrase *ph = GetPhrase(phrase_id);

  CGameObject *pSpeakerGO1 =
      (current_speaking) ? smart_cast<CGameObject *>(FirstSpeaker()) : NULL;
  ...
  if (ph->m_script_text_id.length() > 0) {
    ...
    return ph->GetScriptHelper()->GetScriptText(ph->GetText(), pSpeakerGO1,
                                                pSpeakerGO2, m_DialogId.c_str(),
                                                phrase_id.c_str());
}
...
void CPhraseDialog::AddPhrase(CUIXml *pXml, XML_NODE *phrase_node,
                              const shared_str &phrase_id,
                              const shared_str &prev_phrase_id) {
...
bool CPhraseDialog::Precondition(const CGameObject *pSpeaker1,
  ...
  return data()->m_ScriptDialogHelper.Precondition(pSpeaker1, pSpeaker2,
                                                   m_DialogId.c_str(), "", "");
}
