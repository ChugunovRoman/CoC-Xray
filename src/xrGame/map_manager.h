...
public:
  ...
  CMapLocation *AddUserLocation(const shared_str &spot_type,
                                const shared_str &level_name, Fvector position);
  ...
  void GetMapLocations(const shared_str &spot_type, u16 id,
  ...
  // for security
  CMapLocation *AddMapLocation_script(LPCSTR spot_type, u16 id) {
    return AddMapLocation(spot_type, id);
  };
  CMapLocation *AddUserLocation_script(LPCSTR spot_type, LPCSTR level_name,
                                       Fvector position) {
    return AddUserLocation(spot_type, level_name, position);
  };
  void RemoveMapLocation_script(LPCSTR spot_type, u16 id) {
    RemoveMapLocation(spot_type, id);
  };
  bool HasMapLocation_script(LPCSTR spot_type, u16 id) {
    return HasMapLocation(spot_type, id);
  };
  CMapLocation *GetMapLocation_script(LPCSTR spot_type, u16 id) {
    return GetMapLocation(spot_type, id);
  };
  void MapLocationsForEach(LPCSTR spot_type, u16 id,
