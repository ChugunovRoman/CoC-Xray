...
public:
  ...
  void bloodsucker_drag_jump(CScriptGameObject *e, LPCSTR e_str,
  ...
  void set_home(LPCSTR name, float r_min, float r_max, bool aggressive,
                float r_mid);
  void set_home(u32 lv_ID, float r_min, float r_max, bool aggressive,
                float r_mid);
  void remove_home();
  void berserk();
  ...
  void GiveGameNews(LPCSTR caption, LPCSTR news, LPCSTR texture_name, int delay,
                    int show_time);
  void GiveGameNews(LPCSTR caption, LPCSTR news, LPCSTR texture_name, int delay,
                    int show_time, int type);

  void AddIconedTalkMessage_old(LPCSTR text, LPCSTR texture_name,
                                LPCSTR templ_name){};
  void AddIconedTalkMessage(LPCSTR caption, LPCSTR text, LPCSTR texture_name,
                            LPCSTR templ_name);
  ...
  void
  set_detail_path_type(DetailPathManager::EDetailPathType detail_path_type);
  ...
  u32 add_sound(LPCSTR prefix, u32 max_count, ESoundTypes type, u32 priority,
  ...
  u32 add_sound(LPCSTR prefix, u32 max_count, ESoundTypes type, u32 priority,
                u32 mask, u32 internal_type, LPCSTR bone_name,
                LPCSTR head_anim);
  u32 add_combat_sound(LPCSTR prefix, u32 max_count, ESoundTypes type,
  ...
  void set_sight(SightManager::ESightType sight_type, Fvector *vector3d,
                 u32 dwLookOverDelay);
  void set_sight(SightManager::ESightType sight_type, bool torso_look,
                 bool path);
  void set_sight(SightManager::ESightType sight_type, Fvector &vector3d,
                 bool torso_look);
  void set_sight(SightManager::ESightType sight_type, Fvector *vector3d);
  ...
  void set_item(MonsterSpace::EObjectAction object_action,
                CScriptGameObject *game_object, u32 queue_size);
  void set_item(MonsterSpace::EObjectAction object_action,
                CScriptGameObject *game_object, u32 queue_size,
                u32 queue_interval);
  ...
  void
  set_patrol_path(LPCSTR path_name,
  ...
  void add_animation(LPCSTR animation, bool hand_usage,
                     bool use_movement_controller);
  void add_animation(LPCSTR animation, bool hand_usage, Fvector position,
  ...
  const CCoverPoint *safe_cover(const Fvector &position, float radius,
                                float min_distance);
  ...
  void
  set_max_ignore_monster_distance(const float &max_ignore_monster_distance);
  ...
  bool in_loophole_fov(LPCSTR cover_id, LPCSTR loophole_id,
                       Fvector object_position) const;
  bool in_current_loophole_fov(Fvector object_position) const;
  bool in_loophole_range(LPCSTR cover_id, LPCSTR loophole_id,
                         Fvector object_position) const;
...
// Alundaio
  ...
  void
  RemoveMemoryVisibleObject(const MemorySpace::CVisibleObject &memory_object);
  ...
  void SetBoneVisible(LPCSTR bone_name, bool bVisibility,
