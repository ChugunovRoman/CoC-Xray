...
IC void write(const u32 &bit_count, const u32 &value, u32 &current,
              u32 &output) {
...
bool actor_mp_state_holder::relevant(const actor_mp_state &state) {
  ...
  m_mask |=
      is_similar(m_state.camera_yaw, state.camera_yaw) ? camera_yaw_flag : 0;
  m_mask |= is_similar(m_state.camera_pitch, state.camera_pitch)
                ? camera_pitch_flag
                : 0;
  m_mask |=
      is_similar(m_state.camera_roll, state.camera_roll) ? camera_roll_flag : 0;
  m_mask |=
...
void actor_mp_state_holder::write(NET_Packet &packet) {
  ...
  if (check(model_yaw_flag))
    packet.w_float(
        m_state.model_yaw); // packet.w_float_q8	(m_state.model_yaw
                            // ,0.f ,PI_MUL_2);
  if (check(camera_yaw_flag))
    packet.w_float_q8(
        m_state.camera_yaw, 0.f,
        PI_MUL_2); // packet.w_float		(m_state.camera_yaw
                   // );
  if (check(camera_pitch_flag))
    packet.w_float_q8(
        m_state.camera_pitch, 0.f,
        PI_MUL_2); // packet.w_float		(m_state.camera_pitch
                   // );
  if (check(camera_roll_flag))
    packet.w_float_q8(
        m_state.camera_roll, 0.f,
        PI_MUL_2); // packet.w_float		(m_state.camera_roll
                   // );
  ...
  if (check(inventory_active_slot_flag))
    ::write(inventory_active_slot_bits, m_state.inventory_active_slot, current,
            output);
  ...
  if (check(radiation_flag))
    ::write(radiation_bits, pack(m_state.radiation, radiation_bits), current,
            output);
  if (check(physics_state_enabled_flag))
    ::write(physics_state_enabled_bits, m_state.physics_state_enabled, current,
            output);
...
void actor_mp_state_holder::read(NET_Packet &packet) {
  ...
  if (check(model_yaw_flag))
    packet.r_float(
        m_state
            .model_yaw); // packet.r_float_q8	(m_state.model_yaw,0.f,PI_MUL_2
                         // );
  if (check(camera_yaw_flag))
    packet.r_float_q8(
        m_state.camera_yaw, 0.f,
        PI_MUL_2); // packet.r_float		(m_state.camera_yaw
                   // );
  if (check(camera_pitch_flag))
    packet.r_float_q8(
        m_state.camera_pitch, 0.f,
        PI_MUL_2); // packet.r_float		(m_state.camera_pitch
                   // );
  if (check(camera_roll_flag))
    packet.r_float_q8(
        m_state.camera_roll, 0.f,
        PI_MUL_2); // packet.r_float		(m_state.camera_roll
                   // );
