...
struct CRemoveOldDangerCover {
  ...
  IC bool
  operator()(const CAgentLocationManager::CDangerLocationPtr &location) const {
    if (!location->useful()) {
...
bool CAgentLocationManager::suitable(CAI_Stalker *object,
  ...
  for (; I != E; ++I) {
    ...
    if ((*I)->cover()->m_position.distance_to_sqr(location->position()) <=
        _sqr(5.f))
...
void CAgentLocationManager::remove_links(CObject *object) {
  m_danger_locations.erase(std::remove_if(m_danger_locations.begin(),
                                          m_danger_locations.end(),
