...
void CMaterialManager::Load(LPCSTR section) {
  R_ASSERT3(pSettings->line_exist(section, "material"),
            "Material not found in the section ", *(m_object->cNameSect()));
  m_my_material_idx =
  ...
  if (debug_character_material_load) {
    ...
    if (entity_alive) {
      ...
      VERIFY(m);
      Msg("(CMaterialManager::Load(LPCSTR section)) material: %s loaded for "
          "%s, from section: %s ",
...
void CMaterialManager::reinit() {
  ...
  if (entity_alive) {
    // VERIFY(
    // entity_alive->character_physics_support()->movement()->CharacterExist()
    // );
...
void CMaterialManager::update(float time_delta, float volume, float step_time,
                              bool standing) {
  VERIFY(GAMEMTL_NONE_IDX != m_my_material_idx);
