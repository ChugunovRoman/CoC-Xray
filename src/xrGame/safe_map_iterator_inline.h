...
#define TEMPLATE_SPEZIALIZATION                                                \
  template <typename _key_type, typename _data_type, typename _predicate,      \
            bool use_time_limit, typename _cycle_type, bool use_first_update>
...
TEMPLATE_SPEZIALIZATION
IC void CSSafeMapIterator::add(const _key_type &id, _data_type *value,
                               bool no_assert) {
...
TEMPLATE_SPEZIALIZATION
IC const typename CSSafeMapIterator::_REGISTRY &
CSSafeMapIterator::objects() const {
  return (m_objects);
}
...
IC u32 CSSafeMapIterator::update(const _update_predicate &predicate,
  ...
  for (u32 i = 0; (I != m_objects.end()) && !time_over() &&
