...
struct animation_stats {
  ...
  IC animation_stats(const shared_str &visual_id, const u32 &frame_count,
...
void show_animations() {
  ...
  const ANIMATION_STATS_PAIR **const animations =
      (ANIMATION_STATS_PAIR const **)_alloca(animation_count *
                                             sizeof(ANIMATION_STATS_PAIR *));
  ...
  for (i = animations; i != e; ++i)
    Msg("%6d %6d %-32s %s", (*i)->second.m_frame_count,
        (*i)->second.m_start_count, *(*i)->first.first, *(*i)->first.second);
}
...
void show_blends() {
  ...
  Msg("       animation_set1                                  animation1    "
      "count     animation2                                  animation_set2");
  for (i = blends; i != e; ++i)
...
void add_blend(const shared_str &animation_id,
               const shared_str &animation_set_id, const shared_str &visual_id,
...
void CStalkerAnimationManager::add_animation_stats(
    const ANIMATION_ID &animation_id, const BLEND_ID *blend_id,
    bool just_started) {
