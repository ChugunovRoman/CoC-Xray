...
void CSpaceRestrictionComposition::initialize() {
  ...
  for (u32 i = 1; i < n; ++i)
    m_sphere.R =
        _max(m_sphere.R, m_sphere.P.distance_to(spheres[i].P) + spheres[i].R);
  ...
  m_border.erase(
      std::remove_if(m_border.begin(), m_border.end(), CMergePredicate(this)),
      m_border.end());
...
void CSpaceRestrictionComposition::check_restrictor_type() {
  ...
  VERIFY3(restrictor, "you are trying to use object as a restrictor",
          *m_space_restrictors);
  VERIFY2(restrictor->restrictor_type() ==
