...
CBlend *PlayMotionByParts(IKinematicsAnimated *sa, MotionID motion_ID,
...
void SActorState::CreateClimb(IKinematicsAnimated *K) {
  ...
  for (int k = 0; k < 12; ++k)
    m_damage[k] = K->ID_FX(
        strconcat(sizeof(buf), buf, base, "_damage_", itoa(k, buf1, 10)));
}
...
void SVehicleAnimCollection::Create(IKinematicsAnimated *V, u16 num) {
  ...
  steer_left =
      V->ID_Cycle(strconcat(sizeof(buf), buf, "steering_idle_", buff1, "ls"));
  steer_right =
      V->ID_Cycle(strconcat(sizeof(buf), buf, "steering_idle_", buff1, "rs"));

  for (int i = 0; MAX_IDLES > i; ++i) {
    idles[i] = V->ID_Cycle_Safe(strconcat(sizeof(buf), buf, "steering_idle_",
...
void CActor::g_SetSprintAnimation(u32 mstate_rl, MotionID &head,
                                  MotionID &torso, MotionID &legs) {
...
void CActor::g_SetAnimation(u32 mstate_rl) {
  ...
  if (!M_torso) {
    ...
    if (H) {
      ...
      if (!b_DropActivated && !fis_zero(f_DropPower)) {
        ...
        if (!m_bAnimTorsoPlayed) {
          ...
          if (W) {
            ...
            if (!M_torso)
              M_torso =
                  ST->m_torso[4].moving[moving_idx]; // Alundaio: Fix torso
                                                     // animations for binoc
          } else if (M) {
            if (is_standing) {
              switch (M->GetState()) {
      ...
      M_torso = ST->m_torso[4].moving[moving_idx]; // Alundaio: Fix torso
                                                   // animations for no weapon
  }
  MotionID mid =
      smart_cast<IKinematicsAnimated *>(Visual())->ID_Cycle("norm_idle_0");
  ...
  if (!M_legs) {
    ...
    {
      M_legs =
          smart_cast<IKinematicsAnimated *>(Visual())->ID_Cycle("cr_idle_1");
    } else
      M_legs = ST->legs_idle;
  }
  if (!M_head)
  ...
  if (m_current_torso != M_torso) {
    if (m_bAnimTorsoPlayed)
      ...
      m_current_torso_blend =
          smart_cast<IKinematicsAnimated *>(Visual())->PlayCycle(M_torso);
  ...
  if (bDebug && g_ShowAnimationInfo) {
    ...
    IKinematicsAnimated *KA = smart_cast<IKinematicsAnimated *>(Visual());
    if (M_torso)
      UI().Font().pFontStat->OutNext("torso [%s]",
                                     KA->LL_MotionDefName_dbg(M_torso).first);
    if (M_head)
      UI().Font().pFontStat->OutNext("head [%s]",
                                     KA->LL_MotionDefName_dbg(M_head).first);
    if (M_legs)
      UI().Font().pFontStat->OutNext("legs [%s]",
                                     KA->LL_MotionDefName_dbg(M_legs).first);
  }
#endif
  ...
  if ((Level().CurrentControlEntity() == this) && g_ShowAnimationInfo) {
    ...
    /*
       ...
       ("V
       [%3.2f, %3.2f, %3.2f]",VPUSH(m_PhysicMovementControl->GetVelocity()));
