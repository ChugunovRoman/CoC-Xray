...
void type_motion::set_motion(IKinematicsAnimated *k, u16 id_motion,
                             LPCSTR dir_anim) {
...
type_motion *type_motion::setup(IKinematicsAnimated *k, CInifile const *ini,
                                LPCSTR section, LPCSTR type) {
...
#ifdef DEBUG
      if (death_anim_debug)
        Msg("death anims: load: loaded %s from section %s for %s", sdir_anim,
            section, type);
#endif
    }
  }
...
MotionID death_anims::motion(CEntityAlive &ea, const SHit &H,
                             float &angle) const {
  angle = 0;
  if (anims.empty()) {
#ifdef DEBUG
