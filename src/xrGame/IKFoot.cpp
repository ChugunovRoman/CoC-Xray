...
CIKFoot::CIKFoot()
    : m_bind_b2_to_b3(Fidentity), m_ref_bone(u16(-1)), m_foot_bone_id(BI_NONE),
      m_toe_bone_id(BI_NONE), m_K(0), m_foot_width(0)
...
void CIKFoot::Create(IKinematics *K, LPCSTR section, u16 bones[4]) {
  ...
  if (section) {
    ...
    m_foot_direction.v =
        Kinematics()->LL_UserData()->r_fvector3(section, "foot_direction");
  }
  set_toe(bones);
}
...
struct envc : private boost::noncopyable, public SEnumVerticesCallback {
  ...
  envc(const Fmatrix &_i_bind_transform, const Fvector &_ax, Fvector &_pos)
      : SEnumVerticesCallback(), i_bind_transform(_i_bind_transform), ax(_ax),
        pos(_pos) {
    start_pos.set(0, 0, 0);
  }
...
void CIKFoot::set_toe(u16 bones[4]) {
  ...
  m_foot_width =
      (Fvector().sub(m_toe_position.v, b2to_ref.c)).dotproduct(foot_normal);
}
...
CIKFoot::CollideFoot(float angle, float &out_angle, const Fvector &global_toe,
                     const Fvector &foot_normal, const Fvector &global_bone_pos,
                     const Fplane &p, const Fvector &ax) const {

  float dfoot_tri =
      -p.d -
      p.n.dotproduct(global_bone_pos); // dist from foot bone pos to tri plain
  Fvector axp;
...
bool CIKFoot::make_shift(Fmatrix &xm, const Fvector &cl_point, bool collide,
                         const Fplane &p, const Fvector &pick_dir) const {
...
bool CIKFoot::GetFootStepMatrix(ik_goal_matrix &m, const SCalculateData &cd,
                                const SIKCollideData &cld, bool collide,
                                bool rotate) const {
...
ik_goal_matrix::e_collide_state CIKFoot::rotate(Fmatrix &xm, const Fplane &p,
  ...
  if (!fis_zero(s)) {
    ...
    if (collide)
      cl_state =
          CollideFoot(angle, angle, global_point, foot_normal, xm.c, p, ax);
    // if( cld.m_pick_dir )
    Fvector c = xm.c;
...
bool CIKFoot::GetFootStepMatrix(ik_goal_matrix &m, const Fmatrix &g_anim,
                                const SIKCollideData &cld, bool collide,
                                bool rotation,
  ...
  global_anim.transform_tiny(global_point,
                             local_point); // non collided toe in global space
  Fvector foot_normal;
...
void CIKFoot::SetFootGeom(ik_foot_geom &fg, const Fmatrix &ref_bone,
  ...
  Fvector pos_hill;
  Fmatrix foot =
      (Fmatrix().mul_43(object_matrix, ref_bone_to_foot(foot, ref_bone)));
...
void CIKFoot::Collide(SIKCollideData &cld, ik_foot_collider &collider,
                      const Fmatrix &ref_bone, const Fmatrix &object_matrix,
