...
public:
  ...
  void Perform_transfer(NET_Packet &PR, NET_Packet &PT, CSE_Abstract *what,
  ...
  void Process_event_ownership(NET_Packet &P, ClientID sender, u32 time, u16 ID,
                               BOOL bForced = FALSE);
  bool Process_event_reject(NET_Packet &P, const ClientID sender,
  ...
  void Process_event_destroy(NET_Packet &P, ClientID sender, u32 time, u16 ID,
                             NET_Packet *pEPack);
  void Process_event_activate(NET_Packet &P, const ClientID sender,
                              const u32 time, const u16 id_parent,
                              const u16 id_entity, bool send_message = true);
...
public:
  ...
  virtual void SendTo_LL(ClientID ID, void *data, u32 size,
                         u32 dwFlags = DPNSEND_GUARANTEED, u32 dwTimeout = 0);
  void SecureSendTo(xrClientData *xrCL, NET_Packet &P,
                    u32 dwFlags = DPNSEND_GUARANTEED, u32 dwTimeout = 0);
  ...
  virtual IClient *
  client_Find_Get(ClientID ID);            // Find earlier disconnected client
