...
void CStalkerAnimationPair::play_global_animation(
    IKinematicsAnimated *skeleton_animated, PlayCallback callback,
  ...
  for (u16 i = 0; i < MAX_PARTS; ++i) {
    ...
    if (!m_blend) {
      ...
      blend = skeleton_animated->LL_PlayCycle(
          i, animation(), mix_animations ? TRUE : FALSE, callback, m_object);
      ...
      skeleton_animated->LL_PlayCycle(i, animation(),
                                      mix_animations ? TRUE : FALSE, 0, 0);
  }
  // DBG_OpenCashedDraw();
  // DBG_DrawBones( *m_object );
  // DBG_ClosedCashedDraw( 50000 );
...
#endif
{
  ...
  if (!global_animation()) {
    ...
    if (m_step_dependence && continue_interrupted_animation) {
      ...
      if (m_step_dependence && m_blend)
        pos =
            fmod(m_blend->timeCurrent, m_blend->timeTotal) / m_blend->timeTotal;
    }
    // DBG_OpenCashedDraw();
    // DBG_DrawBones( *m_object );
    // DBG_ClosedCashedDraw( 50000 );
    m_blend =
        skeleton_animated->PlayCycle(animation(), TRUE, callback, m_object);
  ...
  if (psAI_Flags.is(aiAnimation)) {
    ...
    Msg("%6d [%s][%s][%s][%d][%c][%c][%c][%f][%f][%f]", Device.dwTimeGlobal,
        m_object_name, m_animation_type_name, name, motion->bone_or_part,
...
void CStalkerAnimationPair::on_animation_end() {
  ...
  Callbacks callbacks(_alloca(callback_count * sizeof(Callbacks::value_type)),
