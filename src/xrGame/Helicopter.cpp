...
CHelicopter::CHelicopter() {
  ...
  active_camera = 0;
  camera[eacFirstEye] = xr_new<CCameraFirstEye>(
...
void CHelicopter::Load(LPCSTR section) {
  ...
  m_exit_position = READ_IF_EXISTS(pSettings, r_fvector3, section, "exit_pos",
                                   Fvector().set(0.0f, 0.0f, 0.0f));
  m_camera_position.clear();
  m_camera_position.push_back(READ_IF_EXISTS(pSettings, r_fvector3, section,
                                             "camera_first",
                                             Fvector().set(0.0f, 0.0f, 0.0f)));
  m_camera_position.push_back(READ_IF_EXISTS(pSettings, r_fvector3, section,
                                             "camera_look",
...
void CollisionCallbackAlife(bool &do_colide, bool bo1, dContact &c,
...
BOOL CHelicopter::net_Spawn(CSE_Abstract *DC) {
  ...
  m_smoke_bone =
      K->LL_BoneID(pUserData->r_string("helicopter_definition", "smoke_bone"));
  m_light_bone =
      K->LL_BoneID(pUserData->r_string("helicopter_definition", "light_bone"));
  ...
  CBoneInstance &biX =
      smart_cast<IKinematics *>(Visual())->LL_GetBoneInstance(m_rotate_x_bone);
  biX.set_callback(bctCustom, BoneMGunCallbackX, this);
  CBoneInstance &biY =
      smart_cast<IKinematics *>(Visual())->LL_GetBoneInstance(m_rotate_y_bone);
...
void CHelicopter::ActorObstacleCallback(bool &do_colide, bool bo1, dContact &c,
                                        SGameMtl *material_1,
                                        SGameMtl *material_2) {
  if (!do_colide) {
...
void CHelicopter::UpdateEx(float fov) {
  if (OwnerActor()) {
    ...
    if (OwnerActor()
            ...
            ->movement()
            ->CharacterExist()) {
      OwnerActor()->character_physics_support()->movement()->SetPosition(
          XFORM().c);
      OwnerActor()->character_physics_support()->movement()->SetVelocity(
          0.f, 0.f, 0.f);
    }
  }
}
...
bool CHelicopter::HUDView() const { return false; }
...
bool CHelicopter::Use(const Fvector &pos, const Fvector &dir,
...
void CHelicopter::OnMouseMove(int dx, int dy) {
  ...
  if (dy) {
    float d =
        ((psMouseInvert.test(1)) ? -1 : 1) * float(dy) * scale * 3.f / 4.f;
