...
void CGameObject::OnEvent(NET_Packet &P, u16 type) {
  ...
  case GE_DESTROY: {
    if (H_Parent()) {
      Msg("! ERROR (GameObject): GE_DESTROY arrived to object[%d][%s], that "
          "has parent[%d][%s], frame[%d]",
...
BOOL CGameObject::net_Spawn(CSE_Abstract *DC) {
  ...
  if (Level().IsDemoPlayStarted() && E->ID == u16(-1)) {
    ...
    if (o != NULL) {
      Msg("ERROR: CGameObject:net_spawn() Object with ID already exists! ID=%d "
          "self=%s other=%s",
          E->ID, *(cName()), *(o->cName()));
      // ai().script_engine().script_log(eLuaMessageTypeError,
      // "CGameObject:net_Spawn() | Level().Objects.net_Find(E->ID) != NULL
      // (This mean object already exist on level by this ID) ID=%d s_name=%s",
      // E->ID,
      // *(E->s_name));
      return false;
    }
...
#pragma warning(disable : 4238)
    m_ini_file = xr_new<CInifile>(
        &IReader((void *)(*O->m_ini_string), xr_strlen(O->m_ini_string)),
...
#pragma warning(pop)
  ...
  if (!E->client_data.empty()) {
    ...
    //		Msg				("no client data for object [%d][%s], load
    //is skipped",ID(),*cName());
  }
  ...
  if (ai().get_level_graph()) {
    if (E->ID_Parent == 0xffff) {
      ...
      if (l_tpALifeObject &&
          ai().level_graph().valid_vertex_id(l_tpALifeObject->m_tNodeID))
        ai_location().level_vertex(l_tpALifeObject->m_tNodeID);
      else {
        CSE_Temporary *l_tpTemporary = smart_cast<CSE_Temporary *>(E);
        if (l_tpTemporary &&
            ai().level_graph().valid_vertex_id(l_tpTemporary->m_tNodeID))
          ai_location().level_vertex(l_tpTemporary->m_tNodeID);
      }

      if (l_tpALifeObject &&
          ai().game_graph().valid_vertex_id(l_tpALifeObject->m_tGraphID))
      ...
      if (UsedAI_Locations() &&
        ...
        Position().y = EPS_L + ai().level_graph().vertex_plane_y(
                                   *ai_location().level_vertex(), Position().x,
                                   Position().z);
    } else {
      CSE_ALifeObject *const alife_object = smart_cast<CSE_ALifeObject *>(E);
      if (alife_object &&
          ai().level_graph().valid_vertex_id(alife_object->m_tNodeID)) {
      ...
      for (u32 i = 0; i < j; ++i) {
        if (::Random.randF(1.f) < p) {
          CSE_Abstract *A = Level().spawn_item(
              N, Position(), ai_location().level_vertex_id(), ID(), true);
          ...
          if (W) {
            if (W->m_scope_status == ALife::eAddonAttachable)
              W->m_addon_flags.set(CSE_ALifeItemWeapon::eWeaponAddonScope,
                                   bScope);
            ...
            if (W->m_grenade_launcher_status == ALife::eAddonAttachable)
              W->m_addon_flags.set(
                  CSE_ALifeItemWeapon::eWeaponAddonGrenadeLauncher, bLauncher);
          }
...
void CGameObject::OnH_B_Independent(bool just_before_destroy) {
  ...
  VERIFY(parent);
  if (ai().get_level_graph() && ai().level_graph().valid_vertex_id(
...
void CGameObject::remove_visual_callback(visual_callback *callback) {
  CALLBACK_VECTOR_IT I =
      std::find(m_visual_callback.begin(), m_visual_callback.end(), callback);
...
void VisualCallback(IKinematics *tpKinematics) {
  CGameObject *game_object = static_cast<CGameObject *>(
...
u32 CGameObject::ef_creature_type() const {
  ...
  R_ASSERT3(false,
            "Invalid creature type request, virtual function is not properly "
            "overridden!",
            temp);
  return (u32(-1));
}
...
u32 CGameObject::ef_equipment_type() const {
  ...
  CLSID2TEXT(CLS_ID, temp);
  R_ASSERT3(false,
            "Invalid equipment type request, virtual function is not properly "
            "overridden!",
            temp);
  return (u32(-1));
  //	return		(6);
...
u32 CGameObject::ef_detector_type() const {
  ...
  CLSID2TEXT(CLS_ID, temp);
  R_ASSERT3(false,
            "Invalid detector type request, virtual function is not properly "
            "overridden!",
            temp);
  return (u32(-1));
}
...
CGameObject::CScriptCallbackExVoid &
CGameObject::callback(GameObject::ECallbackType type) const {
...
void CGameObject::create_anim_mov_ctrl(CBlend *b, Fmatrix *start_pose,
  ...
  if (animation_movement_controlled()) {
    ...
    m_anim_mov_ctrl =
        xr_new<animation_movement_controller>(&XFORM(), *start_pose, K, b);
  }
}

void CGameObject::destroy_anim_mov_ctrl() { xr_delete(m_anim_mov_ctrl); }

IC bool similar(const Fmatrix &_0, const Fmatrix &_1,
...
void render_box(IRenderVisual *visual, const Fmatrix &xform,
                const Fvector &additional, bool draw_child_boxes,
                const u32 &color) {
...
bool CGameObject::use(CGameObject *who_use) {
  VERIFY(who_use);
  if (this->lua_game_object() && this->lua_game_object()->m_door &&
      (this->lua_game_object()->m_door->is_blocked(doors::door_state_open) ||
       this->lua_game_object()->m_door->is_blocked(doors::door_state_closed)))
    return false;

  this->callback(GameObject::eUseObject)(lua_game_object(),
...
void CGameObject::FootStepCallback(float power, bool b_play, bool b_on_ground,
                                   bool b_hud_view) {
  this->callback(GameObject::eOnFootStep)(this->lua_game_object(), power,
                                          b_play, b_on_ground, b_hud_view);
}