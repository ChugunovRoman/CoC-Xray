...
void CMovementManager::process_patrol_path() {
  ...
  if (!patrol().actual() && (m_path_state > ePathStateSelectPatrolPoint)) {
    ...
    //false 2",Device.dwFrame,*object().cName());
  ...
  case ePathStateBuildLevelPath: {
    level_path_builder().setup(
        object().ai_location().level_vertex_id(), level_dest_vertex_id(),
  ...
  case ePathStatePathVerification: {
    if (!patrol().actual()) {
      //				Msg
      //("[%6d][%s] actuality is false 3",Device.dwFrame,*object().cName());
      m_path_state = ePathStateSelectPatrolPoint;
      ...
      //							Msg
      //("[%6d][%s] detail path is completed",Device.dwFrame,*object().cName());
      //							Msg
      //(
  ...
  case ePathStatePathCompleted: {
    if (!patrol().actual()) {
      //				Msg
      //("[%6d][%s] actuality is false 4",Device.dwFrame,*object().cName());
      m_path_state = ePathStateSelectPatrolPoint;
    }
    break;
  }
