...
void CLevel::g_sv_Spawn(CSE_Abstract *E) {
  // Optimization for single-player only	- minimize traffic between client
  // and server
  ...
  if (Core.ParamFlags.test(Core.verboselog))
    Msg("Try Spawning object Name:[%s] Section:[%s] ID:[%d] ParentID:[%d]",
        E->name_replace(), *E->s_name, E->ID, E->ID_Parent);
...
CSE_Abstract *CLevel::spawn_item(LPCSTR section, const Fvector &position,
                                 u32 level_vertex_id, u16 parent_id,
                                 bool return_item) {
