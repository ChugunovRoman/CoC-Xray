...
void CParticlesObject::Init(LPCSTR p_name, IRender_Sector *S,
                            BOOL bAutoRemove) {
  m_bLooped = false;
  m_bStopping = false;
...
void CParticlesObject::shedule_Update(u32 _dt) {
  ...
  if (dt) {
    if (0) { //.psDeviceFlags.test(mtParticles))	{    //. AlexMX comment
             //this line// NO UNCOMMENT - DON'T WORK PROPERLY
      mt_dt = dt;
      fastdelegate::FastDelegate0<> delegate(
          this, &CParticlesObject::PerformAllTheWork_mt);
