    ...
    if (mn.similar(cur_rect.lt, 2.f) && mx.similar(cur_rect.rb, 2.f)) {
      ...
      if (pActor) {
        ...
        if (our_inv_owner && others_inv_owner && !monster) {
          switch (RELATION_REGISTRY().GetRelationType(others_inv_owner,
                                                      our_inv_owner)) {
...
void CBinocularsVision::Update() {
  ...
  for (; v_it != vVisibles.end(); ++v_it) {
    ...
    if (!pActor->memory().visual().visible_now(
    ...
    Fvector opos = Fvector(object_->Position());
    if (0 > opos.sub(Actor()->Position()).dotproduct(Device.vCameraDirection))
      continue;
  ...
  /*
  std::sort
  (m_active_objects.begin(), m_active_objects.end());

  while(m_active_objects.size() &&
  m_active_objects.back()->m_flags.test(flVisObjNotValid)){ xr_delete
  (m_active_objects.back()); m_active_objects.pop_back			();
  }
  */
...
void CBinocularsVision::Load(const shared_str &section) {
  ...
  m_colors[0] = READ_IF_EXISTS(pSettings, r_fcolor, section, "vis_enemy_color",
                               Fcolor().set(C_ON_ENEMY));
  m_colors[1] = READ_IF_EXISTS(pSettings, r_fcolor, section,
                               "vis_neutral_color", Fcolor().set(C_ON_NEUTRAL));
  m_colors[2] = READ_IF_EXISTS(pSettings, r_fcolor, section, "vis_friend_color",
                               Fcolor().set(C_ON_FRIEND));
