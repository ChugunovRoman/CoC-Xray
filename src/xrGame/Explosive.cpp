...
const float exp_dist_extinction_factor =
    3.f; //(>1.f, 1.f -means no dist change of exp effect)	on the dist of
...
void CExplosive::Load(CInifile const *ini, LPCSTR section) {
  ...
  sscanf(ini->r_string(section, "light_color"), "%f,%f,%f", &m_LightColor.r,
...
#ifdef LAYERED_SND_SHOOT
  m_layered_sounds.LoadSound(ini, section, "snd_explode", "sndExplode", false,
                             m_eSoundExplode);
#else
...
#ifdef DEBUG
    float l_S =
        effective_volume * (_abs(l_dir.dotproduct(obj_xform.i)) / l_d.x +
                            _abs(l_dir.dotproduct(obj_xform.j)) / l_d.y +
                            _abs(l_dir.dotproduct(obj_xform.k)) / l_d.z);
    float add_eff =
        _sqrt(l_S / max_s) * TestPassEffect(l_source_p, l_dir, mag, expl_radius,
                                            storage, blasted_obj);
    effect += add_eff;
    if (ph_dbg_draw_mask.test(phDbgDrawExplosions)) {
...
float CExplosive::TestPassEffect(const Fvector &source_p, const Fvector &dir,
                                 float range, float ef_radius,
...
void CExplosive::Explode() {
  ...
  //играем звук взрыва
#ifdef LAYERED_SND_SHOOT
  m_layered_sounds.PlaySound("sndExplode", pos, smart_cast<CObject *>(this),
                             false, false, (u8)-1);
#else
  ...
  pStaticPG =
      CParticlesObject::Create(*m_sExplodeParticles, !m_bDynamicParticles);
  ...
  for (int i = 0; i < m_iFragsNum; ++i) {
    ...
    Level().BulletManager().AddBullet(
        ...
        Initiator(), cast_game_object()->ID(), m_eHitTypeFrag, m_fFragsRadius,
  ...
  if (m_fExplodeDuration < 0.f && m_blasted_objects.empty()) {
    ...
    if (m_pLight && m_pLight->get_active() && m_fLightTime > 0) {
      if (m_fExplodeDuration > (m_fExplodeDurationMax - m_fLightTime)) {
        float scale =
            (m_fExplodeDuration - (m_fExplodeDurationMax - m_fLightTime)) /
            m_fLightTime;
...
void CExplosive::OnAfterExplosion() {
  ...
  //	//		Msg					("ge_destroy: [%d]
  //- %s",ID(),*cName()); 	if (cast_game_object()->Local())
...
void CExplosive::GetRaySourcePos(CExplosive *exp_obj,
                                 const Fvector &expl_center, Fvector &p) {
  if (exp_obj) {
...
void CExplosive::ExplodeWaveProcessObject(collide::rq_results &storage,
  ...
  if (l_pGO->Visual())
    ...
    return; //мне непонятно зачем наносить хит от взрыва по объектам не имеющим
            //вижуал - поэтому игнорируем
  ...
  if (l_impuls > .001f || l_hit > 0.001) {
    ...
    // модуль=_sqrt(l_dir^2+y^2+2.*(l_dir,y)),y=(0,m_fUpThrowFactor,0) (до этого
    // модуль l_dir =1)
    ...
    HS.p_in_bone_space = l_goPos;           //		P.w_vec3
                                            //(l_goPos);
...
void CExplosive::ExplodeWaveProcess() {
  ...
  BLASTED_OBJECTS_I I = std::remove_if(m_blasted_objects.begin(),
                                       m_blasted_objects.end(), SRemovePred());
