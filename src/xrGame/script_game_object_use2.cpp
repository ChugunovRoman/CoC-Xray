...
void CScriptGameObject::force_stand_sleep_animation(u32 index) {
  ...
  if (!monster) {
    ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                    "CAI_Bloodsucker : cannot access class "
                                    "member force_stand_sleep_animation!");
    return;
  }
...
void CScriptGameObject::release_stand_sleep_animation() {
  ...
  if (!monster) {
    ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                    "CAI_Bloodsucker : cannot access class "
                                    "member release_stand_sleep_animation!");
    return;
  }
...
void CScriptGameObject::bloodsucker_drag_jump(CScriptGameObject *e,
  ...
  if (!monster) {
    ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                    "CScriptGameObject : cannot process drag, "
                                    "anim, jump for CAI_Bloodsucker!");
    return;
  }
