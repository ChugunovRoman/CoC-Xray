...
template <typename T> void vec_clear(T *&p) { xr_delete(p); }
...
public:
  type_motion() {}
  virtual ~type_motion();
  type_motion *setup(IKinematicsAnimated *k, CInifile const *ini,
                     LPCSTR section, LPCSTR type);
  MotionID motion(edirection dr) const;
  virtual bool predicate(CEntityAlive &ea, const SHit &H, MotionID &m,
                         float &angle) const = 0;
