...
IC bool CCoverManager::cover(CLevelGraph::CVertex *v, u32 index0,
                             u32 index1) const {
...
IC bool CCoverManager::critical_point(CLevelGraph::CVertex *v, u32 index,
                                      u32 index0, u32 index1) const {
...
void CCoverManager::compute_static_cover() {
  ...
  tbb::parallel_for(tbb::blocked_range<u32>(0, levelVertexCount),
...
void CCoverManager::remove_nearby_covers(
    smart_cover::cover const &cover, smart_cover::object const &object) const {
  m_nearest.clear();
...
CCoverManager::Cover const *CCoverManager::add_smart_cover(
    LPCSTR table_name, smart_cover::object const &object,
    bool const &is_combat_cover, bool const &can_fire,
    luabind::object const &loopholes) const {
  Cover *smart_cover =
      xr_new<Cover>(object, m_smart_covers_storage->description(table_name),
...
CCoverManager::Cover *
CCoverManager::smart_cover(shared_str const &cover_id) const {
  if (!m_smart_covers_actual)
    actualize_smart_covers();

  SmartCovers::iterator found =
      std::lower_bound(m_smart_covers.begin(), m_smart_covers.end(), cover_id,
                       id_predicate_less());
