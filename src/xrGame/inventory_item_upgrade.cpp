    ...
    Upgrades_type saved_upgrades) // net_Spawn
{
  ...
  for (; ib != ie; ++ib) {
    ai().alife().inventory_upgrade_manager().upgrade_install(*this, (*ib),
                                                             true);
  }
}
...
bool CInventoryItem::install_upgrade_impl(LPCSTR section, bool test) {
  ...
  if (BaseSlot() != NO_ACTIVE_SLOT) {
    ...
    result |=
        process_if_exists(section, "control_inertion_factor",
                          &CInifile::r_float, m_fControlInertionFactor, test);
  }
  ...
  result2 = process_if_exists_set(section, "inv_name", &CInifile::r_string, str,
                                  test);
  if (result2 && !test)
    m_name = str;
  result |= result2;

  result2 = process_if_exists_set(section, "inv_name_short",
                                  &CInifile::r_string, str, test);
  if (result2 && !test)
    m_nameShort = str;
  result |= result2;
