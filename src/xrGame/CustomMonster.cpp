...
void CCustomMonster::Load(LPCSTR section) {
  ...
  // (section,"ph_crash_speed_min"
  // );
  // float	cs_max		= pSettings->r_float
  // (section,"ph_crash_speed_max"
...
void CCustomMonster::reload(LPCSTR section) {
  ...
  m_fog_density_factor =
      READ_IF_EXISTS(pSettings, r_float, section, "fog_density_factor", .05f);
...
void CCustomMonster::shedule_Update(u32 DT) {
  ...
  if (Remote()) {
    ...
    if (temp > 0) {
      ...
      // R = Radius();
      //////////////////////////////////////
      /// #pragma todo("Oles to all AI guys: perf/logical problem: Only few
      /// objects needs 'feel_touch' why to call update for everybody?")
...
void CCustomMonster::net_update::lerp(CCustomMonster::net_update &A,
                                      CCustomMonster::net_update &B, float f) {
  //
...
void CCustomMonster::UpdateCL() {
  ...
  */
  ...
  if (g_mt_config.test(mtSoundPlayer))
    Device.seqParallel.push_back(fastdelegate::FastDelegate0<>(
        this, &CCustomMonster::update_sound_player));
  else {
...
void CCustomMonster::update_range_fov(float &new_range, float &new_fov,
                                      float start_range, float start_fov) {
...
void CCustomMonster::HitSignal(float /**perc/**/, Fvector & /**vLocalDir/**/,
                               CObject * /**who/**/) {}
...
BOOL CCustomMonster::net_Spawn(CSE_Abstract *DC) {
  ...
  if (!g_Alive()) {
    set_death_time();
    //		Msg						("%6d : Object [%d][%s][%s] is
    //spawned DEAD",Device.dwTimeGlobal,ID(),*cName(),*cNameSect());
  }
...
void CCustomMonster::net_Destroy() {
  ...
  Device.remove_from_seq_parallel(fastdelegate::FastDelegate0<>(
      this, &CCustomMonster::update_sound_player));
...
void CCustomMonster::feel_sound_new(CObject *who, int type,
...
bool CCustomMonster::update_critical_wounded(const u16 &bone_id,
  ...
  float time_delta = m_last_hit_time
...
extern void dbg_draw_frustum(float FOV, float _FAR, float A, Fvector &P,
                             Fvector &D, Fvector &U);
...
void CCustomMonster::OnRender() {
...

  {
    ...
    for (; i != e; ++i)
      Level().debug_renderer().draw_aabb(ai().level_graph().vertex_position(*i),
                                         radius, radius, radius,
                                         D3DCOLOR_XRGB(255, 22, 255));
  }
    ...
    float radius1 = !i ? .2f : .3f;
    {
      for (u32 I = 1; I < path.size(); ++I) {
        ...
        if ((path.size() - 1) == I) // песледний box?
          Level().debug_renderer().draw_aabb(P1, radius0, radius0, radius0,
                                             color1);
        else
          Level().debug_renderer().draw_aabb(P1, radius0, radius0, radius0,
                                             color2);
      ...
      for (u32 I = 1; I < keys.size(); ++I) {
        ...
        Level().debug_renderer().draw_aabb(P1, radius1, radius1, radius1,
                                           color3);
      }
    }
  }
...
void CCustomMonster::create_anim_mov_ctrl(CBlend *b, Fmatrix *start_pose,
...
void CCustomMonster::destroy_anim_mov_ctrl() {
  ...
  float roll;
  XFORM().getHPB(movement().m_body.current.yaw, movement().m_body.current.pitch,
                 roll);
...
Fvector CCustomMonster::spatial_sector_point() {
  // if ( g_Alive() )
  //	return
  //inherited::spatial_sector_point(
  //);

  // if ( !animation_movement() )
  return inherited::spatial_sector_point().add(
      Fvector().set(0.f, Radius() * .5f, 0.f));

  // IKinematics* const kinematics	= smart_cast<IKinematics*>(Visual());
  // VERIFY							(kinematics);
  // u16 const root_bone_id			=
  // kinematics->LL_BoneID("bip01_spine");

  // Fmatrix local;
  // kinematics->Bone_GetAnimPos		( local, root_bone_id, u8(-1),
  // false
  // );
