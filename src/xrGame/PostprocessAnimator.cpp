...
void CPostprocessAnimator::Load(LPCSTR name) {
  ...
  if (ext) {
    if (!xr_strcmp(ext, POSTPROCESS_FILE_EXTENSION)) {
      ...
      FATAL("ERROR: Can't support files with many animations set. Incorrect "
            "file.");
  }
...
CPostprocessAnimatorControlled::CPostprocessAnimatorControlled(
  ...
  SetFactorFunc(fastdelegate::FastDelegate0<float>(
      m_controller, &CEffectorController::GetFactor));
}
