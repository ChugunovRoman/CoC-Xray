...
type_motion::edirection type_motion::dir(CEntityAlive &ea, const SHit &H,
                                         float &angle) {
...
void type_motion_diagnostic(LPCSTR message, type_motion::edirection dr,
                            const CEntityAlive &ea, const SHit &H,
                            const MotionID &m) {
#ifdef DEBUG
...
class type_motion0 : public type_motion {
  bool predicate(CEntityAlive &ea, const SHit &H, MotionID &m,
                 float &angle) const {
    m = MotionID();
    ...
    Fvector p;
    if (Fvector()
            .sub(H.initiator()->Position(), global_hit_position(p, ea, H))
            .magnitude() > 30.f)
      return false;

    m = motion(front);
    type_motion_diagnostic(" type_motion0: 1. = Инерционное движение вперед от "
                           "попадания в голову ",
                           front, ea, H, m);
    return true;
  }
  ...
  bool predicate(CEntityAlive &ea, const SHit &H, MotionID &m,
    ...
    if (is_bone_head(*K, H.bone())) {
      ...
      type_motion_diagnostic(
          " type_motion3: 4.	Хедшот (по вероятности), кроме 5 (4)", dr, ea,
          H, m);
      return true;
    }
    return false;
...
class type_motion4 : public type_motion {
  bool predicate(CEntityAlive &ea, const SHit &H, MotionID &m,
    ...
    if (is_snipper(H.weaponID)) {
      edirection dr = dir(ea, H, angle);
      m = motion(dr);
      type_motion_diagnostic(" type_motion4: 5.	Снайперка в голову", dr, ea, H,
                             m);
      return true;
    }
    return false;
...
class type_motion5 : public type_motion {
  bool predicate(CEntityAlive &ea, const SHit &H, MotionID &m,
    ...
    if (is_snipper(H.weaponID) && !is_bone_head(*K, H.bone())) {
      edirection dr = dir(ea, H, angle);
      m = motion(dr);
      type_motion_diagnostic("type_motion5: 6.	Снайперка в тело", dr, ea, H,
                             m);
      return true;
    }
    return false;
...
void death_anims::setup(IKinematicsAnimated *k, LPCSTR section,
                        CInifile const *ini) {
  clear();
  //	if( !ini )
  //			return;
  ...
  anims[0] = xr_new<type_motion0>()->setup(
      k, ini, section, "kill_enertion"); // 1.	Инерционное движение вперед от
                                         // попадания в голову
  anims[1] = xr_new<type_motion1>()->setup(
      k, ini, section, "kill_burst"); // 2.	Изрешетить пулями
  anims[2] = xr_new<type_motion2>()->setup(k, ini, section,
                                           "kill_shortgun"); // 3.	Шотган

  anims[6] = xr_new<type_motion3>()->setup(
      k, ini, section,
      "kill_headshot"); // 4.	Хедшот (по вероятности), кроме 5 (4)
  anims[4] = xr_new<type_motion4>()->setup(
      k, ini, section, "kill_sniper_headshot"); // 5.	Снайперка в голову.
  anims[5] = xr_new<type_motion5>()->setup(
      k, ini, section, "kill_sniper_body"); // 6.	Снайперка в тело.
  anims[3] = xr_new<type_motion6>()->setup(k, ini, section,
                                           "kill_grenade"); // 7.	Гранта
