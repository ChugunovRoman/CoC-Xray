...
static void w_vec_q8(NET_Packet &P, const Fvector &vec, const Fvector &min,
                     const Fvector &max) {
  P.w_float_q8(vec.x, min.x, max.x);
  P.w_float_q8(vec.y, min.y, max.y);
  P.w_float_q8(vec.z, min.z, max.z);
}
static void r_vec_q8(NET_Packet &P, Fvector &vec, const Fvector &min,
                     const Fvector &max) {
...
BOOL CActor::net_Spawn(CSE_Abstract *DC) {
  ...
  */
  ...
  // Alun: In theory it will call SwitchNightVision 'true' when outfit or helmet
  // spawn and moved to slot if m_bNightVisionOn is true
...
void CActor::OnChangeVisual() {
  ...
  if (V) {
    ...
    m_l_clavicle =
        smart_cast<IKinematics *>(Visual())->LL_BoneID("bip01_l_clavicle");
    m_r_clavicle =
        smart_cast<IKinematics *>(Visual())->LL_BoneID("bip01_r_clavicle");
...
void ACTOR_DEFS::net_update::lerp(ACTOR_DEFS::net_update &A,
                                  ACTOR_DEFS::net_update &B, float f) {
  //	float invf		= 1.f-f;
  //	//
  ...
  if (g_Alive()) {
    ...
    //------------------------------------------------
    {
      ...
      cam_Active()->Set(-unaffected_r_torso.yaw, unaffected_r_torso.pitch,
                        0); //, unaffected_r_torso.roll);		// set's
                            //camera orientation
...
void CActor::CalculateInterpolationParams() {
  ...
  pIStart->Pos				= Position();
  pIStart->Vel				=
  m_PhysicMovementControl->GetVelocity();
  ...
  */
  ...
  if (m_bInInterpolation) {
    ...
    float factor =
        float(CurTime - m_dwIStartTime) / (m_dwIEndTime - m_dwIStartTime);
    if (factor > 1.0f)
      factor = 1.0f;

    float c = factor;
    for (u32 k = 0; k < 3; k++) {
      SP0[k] = c * (c * (c * SCoeff[k][0] + SCoeff[k][1]) + SCoeff[k][2]) +
               SCoeff[k][3];
      SP1[k] =
          (c * c * SCoeff[k][0] * 3 + c * SCoeff[k][1] * 2 + SCoeff[k][2]) /
          3; //     3       !!!!

      HP0[k] = c * (c * (c * HCoeff[k][0] + HCoeff[k][1]) + HCoeff[k][2]) +
               HCoeff[k][3];
      HP1[k] =
          (c * c * HCoeff[k][0] * 3 + c * HCoeff[k][1] * 2 + HCoeff[k][2]) /
          3; //     3       !!!!
    };
...
void CActor::make_Interpolation() {
  ...
  if (g_Alive() && m_bInInterpolation) {
    ...
    if (CurTime >= m_dwIEndTime) {
      ...
      for (u32 k = 0; k < 3; k++) {
        IPosL[k] = NewPos[k];
        IPosS[k] = factor * (factor * (factor * SCoeff[k][0] + SCoeff[k][1]) +
                             SCoeff[k][2]) +
                   SCoeff[k][3];
        IPosH[k] = factor * (factor * (factor * HCoeff[k][0] + HCoeff[k][1]) +
                             HCoeff[k][2]) +
                   HCoeff[k][3];
      };
...
{
SMemoryPosStack.push_back(SMemoryPos(Time0, Time1, ph_world->m_steps_num,
...
void dbg_draw_piramid(Fvector pos, Fvector dir, float size, float xdir,
                      u32 color) {
  ...
  if (g_Alive()) {
    ...
    if (dbg_net_Draw_Flags.test(dbg_draw_actor_alive) && V) {
      if (this != Level().CurrentViewEntity() || cam_active != eacFirstEye) {
        ...
        Fmatrix BoneMatrixRes; BoneMatrixRes.mul(V->LL_GetTransform(i),
        BoneMatrix); BoneMatrix.mul(XFORM(), BoneMatrixRes);
    ...
    //		dbg_draw_piramid(tmp.add(IRecT.Pos, tmp1), IRecT.Vel, size,
    //-IRecT.o_model, color_rgba(0, 0, 155, 155));
    dbg_draw_piramid(IEnd.Pos, IEnd.Vel, size, -IEnd.o_model,
                     color_rgba(0, 255, 0, 255));
    //		dbg_draw_piramid(tmp.add(IEndT.Pos, tmp1), IEndT.Vel, size,
    //-IEndT.o_model, color_rgba(0, 155, 0, 155));
    ...
    for (int i = 0; i < 11; i++) {
      ...
      for (u32 k = 0; k < 3; k++) {
        point1S[k] =
            c * (c * (c * SCoeff[k][0] + SCoeff[k][1]) + SCoeff[k][2]) +
            SCoeff[k][3];
        point1H[k] =
            c * (c * (c * HCoeff[k][0] + HCoeff[k][1]) + HCoeff[k][2]) +
            HCoeff[k][3];
    ...
    if (!pLastPos->empty()) {
      ...
      for (; It != pLastPos->end(); It++) {
        ...
        Level().debug_renderer().draw_aabb(Pos2, size / 5, size / 5, size / 5,
                                           sColor);
        Pos1 = *It;
      };
    };
    ...
    PS.set(IPosS);
    PS.y += 1;
    //		Level().debug_renderer().draw_aabb			(PS, size, size,
    //size, color_rgba(128, 128, 255, 255)); 		Level().debug_renderer().draw_aabb
...
void CActor::SetHitInfo(CObject *who, CObject *weapon, s16 element, Fvector Pos,
                        Fvector Dir) {
...
void CActor::OnHitHealthLoss(float NewHealth) { return; };

void CActor::OnCriticalHitHealthLoss() { return; };
...
void CActor::OnCriticalWoundHealthLoss() { return; };

void CActor::OnCriticalRadiationHealthLoss() { return; };
