...
void CWeaponShotEffector::Shot(CWeapon *weapon) {
  ...
  float angle = m_cam_recoil.Dispersion *
                weapon->cur_silencer_koef.cam_dispersion * k_cam_disp;
  angle += m_cam_recoil.DispersionInc *
           weapon->cur_silencer_koef.cam_disper_inc * (float)m_shot_numer;
  ...
  if (vert > abs(m_angle_horz)) {
    ...
    float val =
        (vert * 4 / m_cam_recoil.MaxAngleVert) * m_cam_recoil.StepAngleHorz;
...
void CWeaponShotEffector::Relax() {
  ...
  float relax_speed_horz =
      (fis_zero(time_to_relax)) ? 0.0f : _abs(m_angle_horz) / time_to_relax;
