...
// Improved weapon inertion from Shoker Weapon Mod 3.0, fixed up/down inertion
// by Surs
extern const float PITCH_OFFSET_R; // barrel movement sideways (to the left)
                                   // with vertical camera turns
extern const float
    PITCH_OFFSET_N; // barrel rise / fall with vertical camera turns
extern const float
    PITCH_OFFSET_D; // barrel toward / away when the camera rotates vertically
extern const float PITCH_LOW_LIMIT; // minimum pitch value when used in
                                    // conjunction with PITCH_OFFSET_N
extern const float ORIGIN_OFFSET;   // inertia factor influence on position of
                                  // torso (the smaller, the larger the inertia)
...
float CalcMotionSpeed(const shared_str &anim_name) { return 1.0f; }

player_hud_motion *
player_hud_motion_container::find_motion(const shared_str &name) {
...
#ifdef DEBUG
          //					Msg(" alias=[%s] base=[%s]
          // name=[%s]",pm->m_alias_name.c_str(), pm->m_base_name.c_str(),
          // buff);
#endif // #ifdef DEBUG
        }
      }
...
bool attachable_hud_item::set_bone_visible(const shared_str &bone_name,
                                           BOOL bVisibility, BOOL bSilent) {
  u16 bone_id;
  BOOL bVisibleNow;
...
void attachable_hud_item::load(const shared_str &sect_name) {
  ...
  m_model =
      smart_cast<IKinematics *>(::Render->model_Create(visual_name.c_str()));

  m_attach_place_idx = pSettings->r_u16(sect_name, "attach_place_idx");
  m_measures.load(sect_name, m_model);
}

u32 attachable_hud_item::anim_play(const shared_str &anm_name_b, BOOL bMixIn,
                                   const CMotionDef *&md, u8 &rnd_idx) {
  ...
  u32 ret =
      g_player_hud->anim_play(m_attach_place_idx, M.mid, bMixIn, md, speed);
...
void player_hud::load(const shared_str &player_hud_sect) {
  ...
  //	Msg("hands visual changed to[%s] [%s] [%s]", model_name.c_str(),
  // b_reload?"R":"", m_attached_items[0]?"Y":"");
...
u32 player_hud::motion_length(const shared_str &anim_name,
  ...
  R_ASSERT2(pm, make_string("hudItem model [%s] has no motion with alias [%s]",
                            hud_name.c_str(), anim_name.c_str())
                    .c_str());
  return motion_length(pm->m_animations[0].mid, md, speed);
}

u32 player_hud::motion_length(const MotionID &M, const CMotionDef *&md,
                              float speed) {
...
u32 player_hud::anim_play(u16 part, const MotionID &M, BOOL bMixIn,
                          const CMotionDef *&md, float speed) {

  u16 part_id = u16(-1);
  if (attached_item(0) && attached_item(1))
    part_id =
        m_model->partitions().part_id((part == 0) ? "right_hand" : "left_hand");
...
void player_hud::update_inertion(Fmatrix &trans) {
  if (inertion_allowed()) {
    ...
    if (pMainHud) { // Load the inertia parameters from the main Hud
    ...
    if (pMainHud != NULL &&
        pMainHud->m_parent_hud_item->GetCurrentHudOffsetIdx() >
            0) { // inertia while "aiming"
      _tendto_speed =
          m_tendto_speed_aim - (m_tendto_speed_aim - m_tendto_speed) * 1;
      _origin_offset =
          m_origin_offset_aim - (m_origin_offset_aim - m_origin_offset) * 1;
    } else { // inertia while "crouching"
...
void player_hud::calc_transform(u16 attach_slot_idx, const Fmatrix &offset,
                                Fmatrix &result) {
  Fmatrix ancor_m =
      m_model->dcast_PKinematics()->LL_GetTransform(m_ancors[attach_slot_idx]);
