...
void CHelicopter::UpdateHeliParticles() {
  ...
  if (m_light_render->get_active()) {
    ...
    if (m_lanim) {
      int frame;
      u32 clr = m_lanim->CalculateBGR(Device.fTimeGlobal,
                                      frame); // тючтЁр•рхЄ т ЇюЁьрЄх BGR
      Fcolor fclr;
...
CScriptGameObject *CHelicopter::GetEnemy() {
  if (m_enemy.type == eEnemyEntity && m_enemy.destEnemyID < 65535) {
    CGameObject *pGameObject = smart_cast<CGameObject *>(
        Level().Objects.net_Find(m_enemy.destEnemyID));
    if (pGameObject)
...
float CHelicopter::GetRealAltitude() {
  ...
  Level().ObjectSpace.RayPick(XFORM().c, down_dir, 1000.0f, collide::rqtStatic,
                              cR, NULL);
...
void CHelicopter::Hit(SHit *pHDS) {
  ...
  if (pHDS->who && (smart_cast<CActor *>(pHDS->who) ||
    ...
    callback(GameObject::eHelicopterOnHit)(pHDS->damage(), pHDS->impulse,
                                           pHDS->hit_type, pHDS->who->ID());
  }
...
void CollisionCallbackDead(bool &do_colide, bool bo1, dContact &c,
...
float t_xx(float V0, float V1, float a0, float a1, float d, float fSign) {
  return (V1 +
          _sqrt(V1 * V1 - (a1 / (a1 - a0)) * (V1 * V1 - V0 * V0 - 2 * a0 * d)) *
              fSign) /
         a1;
}
