...
#include "phdebug.h"
#endif
moving_bones_snd_player::moving_bones_snd_player(IKinematics *K, CInifile *ini,
                                                 LPCSTR section,
                                                 const Fmatrix &object)
    : bone_id(BI_NONE), min_factor(-FLT_MAX), max_factor(-FLT_MAX),
      base_velocity(-FLT_MAX), smothed_velocity(-FLT_MAX),
...
void moving_bones_snd_player::update(float time_delta, CGameObject &object) {
  ...
  if (dbg_moving_bones_snd_player) {
    DBG_OutText("smoothed angular speed					"
                "		 : %f",
                smothed_velocity);
...
moving_bones_snd_player *create_moving_bones_snd_player(CInifile *ini,
  ...
  if (!ini || !ini->section_exist("moving_bones_snd_player"))
    return 0;
  return xr_new<moving_bones_snd_player>(&K, ini, "moving_bones_snd_player",
                                         obj);
}
...
void moving_bones_snd_player::load(IKinematics &K, CInifile &ini,
                                   LPCSTR section, const Fmatrix &object) {
  ...
  VERIFY2(
      max_factor > 0.f,
      make_string("moving_bones_snd_player: bad params: base_velocity: %f < 0 ",
                  base_velocity));
