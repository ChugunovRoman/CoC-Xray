...
//передаются
//сюда 								(для просчета
//столкновений и их визуализации)
//////////////////////////////////////////////////////////////////////
...
BOOL CBulletManager::test_callback(const collide::ray_defs &rd, CObject *object,
                                   LPVOID params) {
  if (!object)
    return TRUE;
...
void CBulletManager::FireShotmark(SBullet *bullet, const Fvector &vDir,
                                  const Fvector &vEnd, collide::rq_result &R,
                                  u16 target_material, const Fvector &vNormal,
                                  bool ShowMark) {
...
void CBulletManager::DynamicObjectHit(CBulletManager::_event &E) {
  ...
  if (E.bullet.flags.allow_sendhit && !E.Repeated) {
    SHit Hit =
        SHit(hit_param.power, original_dir, NULL, u16(E.R.element),
...
bool CBulletManager::ObjectHit(SBullet_Hit *hit_res, SBullet *bullet,
  ...
  if (R.O) {
    ...
    Fvector *pVerts = Level().ObjectSpace.GetStaticVerts();
    CDB::TRI *pTri = Level().ObjectSpace.GetStaticTris() + R.element;
    hit_normal.mknormal(pVerts[pTri->verts[0]], pVerts[pTri->verts[1]],
                        pVerts[pTri->verts[2]]);
    ...
    if (DOT(hit_normal, bullet->dir) < 0) {
      if (bullet->density_mode) {
        //				Log("WARNING: Material in material found
        //while bullet tracing. Incorrect behaviour of shooting is possible.");
      }
  ...
  *hit_res = bullet->hit_param; // default param
  ...
  if ((f < ricoshet_factor) && !mtl->Flags.test(SGameMtl::flNoRicoshet) &&
    ...
    // уменьшение скорости полета в зависимости от угла падения пули (чем прямее
    // угол, тем больше потеря)
    bullet->flags.allow_ricochet = 0;
    float scale =
        1.0f - _abs(bullet->dir.dotproduct(hit_normal)) * m_fCollisionEnergyMin;
    clamp(scale, 0.0f, m_fCollisionEnergyMax);
    speed_scale = scale;

    // вычисление рикошета, делается немного фейком, т.к. пуля остается в точке
    // столкновения и сразу выходит из RayQuery()
