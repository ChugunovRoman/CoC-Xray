...
#ifdef DEBUG
      Msg("--- Ignoring event type[%d] time[%d] clientID[0x%08x]", type, time,
          clientID);
#endif // #ifdef DEBUG
      return NULL;
    }
...
GameEvent *GameEventQueue::Retreive() {
  ...
  //---------------------------------------------
    ...
    if ((LastTimeCreate < tmp_time) && (size > 32)) {
      ...
      unused.pop_back();
#ifdef _DEBUG
//			Msg ("GameEventQueue::Retreive - ready %d, unused %d",
// ready.size(), unused.size());
#endif
    }
  }
...
#ifdef _DEBUG
//			Msg ("GameEventQueue::EraseEvents - ready %d, unused
//%d", ready.size(), unused.size());
#endif
    } else {
