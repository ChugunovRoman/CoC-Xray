...
; с случайн				о выбранной точкой приложения в пределах
нового обекта random_hit_imp         =0.1     ; х величена хит - импульса
=............
  ...
  if (own_ini && own_ini->section_exist("impulse_transition_to_parts")) {
    ...
    if (own_ini->line_exist("impulse_transition_to_parts", "ref_bone"))
      ref_bone = own_K->LL_BoneID(
          own_ini->r_string("impulse_transition_to_parts", "ref_bone"));
  ...
  if (ps) {
    ...
    if (own_ini && own_ini->section_exist("autoremove_parts")) {
      ps->SetAutoRemove(1000 *
                        (READ_IF_EXISTS(own_ini, r_u32, "autoremove_parts",
                                        "time", ps->DefaultExitenceTime())));
    }

    if (new_ini && new_ini->section_exist("autoremove")) {
      ps->SetAutoRemove(1000 *
                        (READ_IF_EXISTS(new_ini, r_u32, "autoremove", "time",
