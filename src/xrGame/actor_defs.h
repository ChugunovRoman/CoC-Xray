...
enum EMoveCommand {
  ...
  mcAnyAction =
      (mcAnyMove | mcJump | mcFall | mcLanding | mcLanding2), // mcTurn|
  mcAnyState = (mcCrouch | mcAccel | mcClimb | mcSprint),
  mcLookout = (mcLLookout | mcRLookout),
};

// enum дл€ определени€ действи€ над вещью на которую наведен в текущее врем€
// прицел. »спользуетс€ дл€ показа всплывающих динамических подсказок
enum EActorAction {
  eaaNoAction = 0,
  eaaPickup,
  eaaTalk,
