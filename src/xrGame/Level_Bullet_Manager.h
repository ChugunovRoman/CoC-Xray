...
public:
  ...
  void Init(const Fvector &position, const Fvector &direction,
            float start_speed, float power,
            //.
...
private:
  ...
  static void CalculateNewVelocity(Fvector &dest_new_vel,
                                   Fvector const &old_velocity, float ar,
                                   float life_time);
...
protected:
  ...
  // cколько процентов энергии потеряет пуля при столкновении с материалом (при
  // падении под прямым углом)
...
protected:
  ...
  static BOOL test_callback(const collide::ray_defs &rd, CObject *object,
                            LPVOID params);
  static BOOL firetrace_callback(collide::rq_result &result, LPVOID params);

  // Deffer event
  void RegisterEvent(EventType Type, BOOL _dynamic, SBullet *bullet,
  ...
  //объекту
  bool ObjectHit(SBullet_Hit *hit_res, SBullet *bullet,
                 const Fvector &end_point, collide::rq_result &R,
  ...
  bool trajectory_check_error(Fvector &previous_position,
                              collide::rq_results &rq_storage, SBullet &bullet,
  ...
  bool process_bullet(collide::rq_results &rq_storage, SBullet &bullet,
                      u32 delta_time);
...
public:
  ...
  void AddBullet(const Fvector &position, const Fvector &direction,
                 ...
                 /*float power_critical,*/ float impulse, u16 sender_id,
                 u16 sendersweapon_id, ALife::EHitType e_hit_type,
