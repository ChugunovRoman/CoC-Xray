...
DescriptionPtr storage::description(shared_str const &table_id) {
  ...
  Descriptions::iterator found = std::find_if(
      m_descriptions.begin(), m_descriptions.end(), id_predicate(table_id));
...
void storage::collect_garbage() {
  ...
  m_descriptions.erase(std::remove_if(m_descriptions.begin(),
                                      m_descriptions.end(),
                                      &garbage::predicate),
                       m_descriptions.end());
}