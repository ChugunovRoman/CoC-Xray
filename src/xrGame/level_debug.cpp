...
void CLevelDebug::draw_debug_text() {
  ...
  if (!smart_cast<CBaseMonster *>(Level().CurrentEntity())) {
    ...
    if (debug_actor_view && actor_view) {
      debug::draw_text_tree(*actor_view, 2, x_start, y_start, m_texttree_offs,
                            column_size, 80, D3DCOLOR_XRGB(0, 255, 0),
  ...
  if (m_p_texttree->find_node("General")) {
    debug::draw_text_tree(*m_p_texttree->find_node("General"), 2, x_start,
                          y_start, m_texttree_offs, column_size, 80,
                          D3DCOLOR_XRGB(0, 255, 0), D3DCOLOR_XRGB(255, 255, 0));
  }

  if (m_p_texttree->find_node("Brain")) {
    debug::draw_text_tree(*m_p_texttree->find_node("Brain"), 2,
                          x_start * 2 + column_size, y_start, m_texttree_offs,
                          column_size, 80, D3DCOLOR_XRGB(0, 255, 0),
                          D3DCOLOR_XRGB(255, 255, 0));
  }
...
void CLevelDebug::CTextInfo::add_item(LPCSTR text, float x, float y, u32 color,
                                      u32 id) {
...
void CLevelDebug::CLevelInfo::add_item(const Fvector &pos1, const Fvector &pos2,
                                       u32 color, u32 id) {
  inherited::add_item(SLevelItem(pos1, pos2, color, id));
}

void CLevelDebug::CLevelInfo::add_item(const Fvector &pos, float radius,
                                       u32 color, u32 id) {
...
struct DrawLevelPredicate {
  void operator()(CLevelDebug::SLevelItem s) {
    ...
    if (s.ptype == CLevelDebug::SLevelItem::ePoint) {
      Level().debug_renderer().draw_aabb(s.position1, 0.35f, 0.35f, 0.35f,
                                         s.color);
      ...
      Level().debug_renderer().draw_line(Fidentity, s.position1, s.position2,
                                         s.color);
