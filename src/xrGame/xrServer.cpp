...
void xrServer::Update() {
  ...
  NET_Packet		Packet;
  u32 svT								=
  Device.TimerAsync(); while (!(q_respawn.empty() ||
  (svT<q_respawn.begin()->timestamp)))
  {
          // get
          svs_respawn	R					=
  *q_respawn.begin(); q_respawn.erase
  (q_respawn.begin());

          //
          CSE_Abstract* E					=
  ID_to_entity(R.phantom); E->Spawn_Write		(Packet,FALSE); u16
  ID; Packet.r_begin		(ID); R_ASSERT(M_SPAWN==ID); ClientID
  clientID; clientID.set(0xffff); Process_spawn		(Packet,clientID);
  }
  */
...
void xrServer::SendUpdatesToAll() { return; }
    ...
    ClientID sender) // Non-Zero means broadcasting with "flags" as returned
{
  ...
  // R_ASSERT2						(CL, make_string("packet
  // type
  // [%d]",type).c_str());
...
void xrServer::SendTo_LL(ClientID ID, void *data, u32 size, u32 dwFlags,
                         u32 dwTimeout) {
...
bool xrServer::OnCL_QueryHost() { return false; };
...
bool xrServer::verify_entities() const {
  ...
  for (; I != E; ++I) {
    ...
    VERIFY3((*I).first == (*I).second->ID,
            "SERVER : ID mismatch - map key doesn't correspond to the real "
            "entity ID",
