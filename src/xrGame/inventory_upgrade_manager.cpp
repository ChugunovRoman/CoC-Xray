...
Upgrade *Manager::add_upgrade(shared_str const &upgrade_id,
  ...
  if (get_upgrade(upgrade_id)) {
    VERIFY2(0, make_string("Try add the existent upgrade (%s), in group <%s>. "
                           "Such upgrade is in group <%s> already!",
...
void Manager::load_all_inventory() {
  ...
  VERIFY2( pSettings->section_exist( items_section ), make_string( "Section [%s]
does not exist !", items_section ) ); VERIFY2( pSettings->line_count(
...
Upgrade *Manager::upgrade_verify(shared_str const &item_section,
  ...
  VERIFY2(upgrade_p, make_string("Upgrade <%s> in item <%s> does not exist!",
...
bool Manager::make_known_upgrade(const shared_str &upgrade_id) {
  Upgrade *upgrade_p = get_upgrade(upgrade_id);
  VERIFY2(upgrade_p,
          make_string("Upgrade <%s> does not exist!", upgrade_id.c_str()));
...
bool Manager::upgrade_install(CInventoryItem &item,
                              shared_str const &upgrade_id, bool loading) {
  ...
  if (res == result_ok) {
    ...
    if (item.install_upgrade(upgrade->section())) {
      ...
      FATAL(make_string(
                "! Upgrade <%s> of item [%s] (id = %d) is EMPTY or FAILED !",
                upgrade_id.c_str(), item.m_section_id.c_str(), item.object_id())
                .c_str());
    }
  }
  ...
  if (g_upgrades_log == 1) {
    Msg("- Upgrade <%s> of inventory item [%s] (id = %d) can`t be installed. "
        "Error = %d",
  ...
  if (pSettings->line_exist(item.m_section_id, "installed_upgrades")) {
    ...
    if (installed_upgrades_str) {
      ...
      for (int n = _GetItemCount(installed_upgrades_str), i = 0; i < n; ++i) {
        upgrade_install(
            item, _GetItem(installed_upgrades_str, i, temp, buffer_size), true);
      }
    }
  } // if exist
}
...
LPCSTR Manager::get_upgrade_by_index(CInventoryItem &item,
  ...
  VERIFY2(
      upgrade,
      make_string(
          "! Upgrade with index <%d,%d> in inventory item [%s] does not exist!",
...
void Manager::compute_range_section(LPCSTR section, LPCSTR parameter,
