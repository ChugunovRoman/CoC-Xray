...
public:
  ...
  IC void
  remove_effect(const typename COperatorCondition::_condition_type &effect);
  IC _edge_value_type min_weight() const;

  template <typename T>
  IC bool
  applicable(const CSConditionState &condition, const CSConditionState &start,
  ...
  apply(const CSConditionState &condition,
        const CSConditionState &self_condition, CSConditionState &result,
