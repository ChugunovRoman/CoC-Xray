...
stalker_movement_manager_base::stalker_movement_manager_base(
    ...
    : inherited(object), m_last_query_object(0),
      ...
      m_last_query_result(false), m_last_query_distance(flt_max),
...
IC void stalker_movement_manager_base::add_velocity(int mask, float linear,
    ...
    if ((movement_params.m_path_type != MovementManager::ePathTypePatrolPath) &&
      ...
      if (!restrictions().accessible(level_path().dest_vertex_id())) {
        ...
        VERIFY2(restrictions().accessible(vertex_position) ||
                    show_restrictions(&restrictions()),
                make_string("vertex_id[%d],position[%f][%f][%f],object[%s]",
                            vertex_id, VPUSH(vertex_position),
                            *object().cName()));
...
void stalker_movement_manager_base::setup_velocities(
  ...
  default: {
    ...
    velocity_mask &=
        u32(-1) ^ (eVelocityNegativeVelocity | eVelocityPositiveVelocity);
  }
  }
...
void stalker_movement_manager_base::parse_velocity_mask(
  ...
  if ((movement_params.m_movement_type == eMovementTypeStand) ||
      path().empty() || (path().size() <= detail().curr_travel_point_index()) ||
  ...
  case eVelocityFree: {
    ...
    if (m_object->brain().current_action_id() ==
      ...
      CStalkerCombatPlanner &planner = smart_cast<CStalkerCombatPlanner &>(
...
void stalker_movement_manager_base::set_nearest_accessible_position(
    Fvector desired_position, u32 level_vertex_id) {
...
float stalker_movement_manager_base::speed(
  ...
  return (m_velocities->velocity(mental_state(), body_state(), movement_type(),
                                 movement_direction));
}
...
bool stalker_movement_manager_base::is_object_on_the_way(
    const CGameObject *object, const float &distance) {
...
IC float distance_to_line(const Fvector &p0, const Fvector &p1,
                          const Fvector &p2) {
  if (p0.similar(p2))
    return (0.f);
