...
void CBaseGraviZone::AffectPullDead(CPhysicsShellHolder *GO,
                                    const Fvector &throw_in_dir, float dist) {
  GO->PPhysicsShell()->applyImpulse(throw_in_dir, dist * m_fThrowInImpulse *
                                                      GO->GetMass() / 100.f);
}
...
void CBaseGraviZone::AffectThrow(SZoneObjectInfo *O, CPhysicsShellHolder *GO,
  ...
  float power =
      Power(dist, Radius()); // Power(GO->Position().distance_to(zone_center));
  float impulse = m_fHitImpulseScale * power * GO->GetMass();

  if (power > 0.01f) {
    position_in_bone_space.set(0.f, 0.f, 0.f);
    CreateHit(GO->ID(), ID(), throw_in_dir, power, 0, position_in_bone_space,
