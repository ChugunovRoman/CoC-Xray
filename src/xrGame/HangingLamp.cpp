...
void CHangingLamp::Center(Fvector &C) const {
  if (renderable.visual) {
    renderable.xform.transform_tiny(C,
...
BOOL CHangingLamp::net_Spawn(CSE_Abstract *DC) {
  ...
  if (lamp->flags.is(CSE_ALifeObjectHangingLamp::flPhysic) && !Visual())
    Msg("! WARNING: lamp, obj name [%s],flag physics set, but has no visual",
        *cName());
  //.	if
  //(lamp->flags.is(CSE_ALifeObjectHangingLamp::flPhysic)&&Visual()&&!guid_physic_bone)
  // fHealth=0.f;
  if (Alive() && m_bState)
    TurnOn();
  else {
...
void CHangingLamp::UpdateCL() {
  ...
  if (Alive() && light_render->get_active()) {
    ...
    if (light_bone != BI_NONE) {
      Fmatrix &M =
          smart_cast<IKinematics *>(Visual())->LL_GetTransform(light_bone);
      xf.mul(XFORM(), M);
    ...
    if (light_ambient) {
      if (ambient_bone != light_bone) {
        if (ambient_bone != BI_NONE) {
          Fmatrix &M = smart_cast<IKinematics *>(Visual())->LL_GetTransform(
              ambient_bone);
          xf.mul(XFORM(), M);
          VERIFY(!fis_zero(DET(xf)));
        } else {
...
void CHangingLamp::TurnOff() {
  ...
  if (Visual()) {
    ...
    VERIFY2(K->LL_GetBonesVisible() != 0,
            make_string("can not Turn Off lamp: %s, visual %s - because all "
                        "bones become invisible",
                        cNameVisual().c_str(), cName().c_str()));
  }
  // if(!PPhysicsShell())//if we have physiccs_shell it will call processing
  // deactivate when disable
...
void CHangingLamp::Hit(SHit *pHDS) {
  ...
  if (m_pPhysicsShell)
    m_pPhysicsShell->applyHit(pHDS->p_in_bone_space, pHDS->dir, pHDS->impulse,
                              pHDS->boneID, pHDS->hit_type);
