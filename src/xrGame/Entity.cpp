...
CEntity::create_entity_condition(CEntityConditionSimple *ec) {
  if (!ec)
...
BOOL CEntity::net_Spawn(CSE_Abstract *DC) {
  ...
  if (ini) {
    if (ini->section_exist("damage_section") && !use_simplified_visual())
      CDamageManager::reload(pSettings->r_string("damage_section", "damage"),
                             ini);
...
void CEntity::KillEntity(
  ...
  // AVO: allow scripts to process actor condition and prevent actor's death or
  // kill him if desired. IMPORTANT: if you wish to kill actor you need to call
  // db.actor:kill(level:object_by_id(whoID), true) in actor_before_death
  ...
  if (whoID != ID()) {
    ...
    if (m_killer_id != ALife::_OBJECT_ID(-1)) {
      Msg("! Entity [%s][%s] already has killer with id %d, but new killer id "
          "arrived - %d",
  ...
  /* Alundaio: Should not matter who kills self. CScripGameObject::Kill sets
self as killer if nil passed else
{
  if (m_killer_id != ALife::_OBJECT_ID(-1))
      return;
}
...
void CEntity::set_death_time() {
  m_level_death_time = Device.dwTimeGlobal;
  m_game_death_time = ai().get_alife() ? ai().alife().time_manager().game_time()
                                       : Level().GetGameTime();
}
...
bool CEntity::IsMyCamera() const {
  return (smart_cast<const CEntity *>(g_pGameLevel->CurrentViewEntity()) ==
          this);
}
