...
void moving_objects::fill_static(obstacles_query &query,
                                 const Fvector &position, const float &radius) {
  NEAREST_STATIC::const_iterator I = m_nearest_static.begin();
  NEAREST_STATIC::const_iterator E = m_nearest_static.end();
  for (; I != E; ++I) {
    if (!collided(*I, position, radius))
...
void moving_objects::fill_nearest_list(const Fvector &position,
  ...
  Level().ObjectSpace.GetNearest(m_spatial_objects, m_nearest_static, position,
                                 radius,
                                 const_cast<CEntityAlive *>(&object->object()));

  m_nearest_static.erase(std::remove_if(m_nearest_static.begin(),
...
void moving_objects::query_action_static(moving_object *object,
  ...
  fill_nearest_list(start_position,
                    dest_position.distance_to(start_position) + EPS, object);
  ...
  fill_nearest_list(start_position,
                    dest_position.distance_to(start_position) +
                        additional_radius + EPS,
                    object);
