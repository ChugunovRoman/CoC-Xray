...
IC void CPathManagerTemplate::build_path(const _vertex_id_type start_vertex_id,
  ...
  before_search(start_vertex_id, dest_vertex_id);
  m_failed = !ai().graph_engine().search(*m_graph, start_vertex_id,
                                         dest_vertex_id, &m_path, *m_evaluator);
  after_search();
  m_current_index = _index_type(-1);
  m_intermediate_index = _index_type(-1);
  m_actuality = !failed();
...
TEMPLATE_SPECIALIZATION
IC bool
CPathManagerTemplate::actual(const _vertex_id_type /*start_vertex_id*/,
...
TEMPLATE_SPECIALIZATION
IC const typename CPathManagerTemplate::PATH &
CPathManagerTemplate::path() const {
  return (m_path);
}
...
TEMPLATE_SPECIALIZATION
IC void
CPathManagerTemplate::before_search(const _vertex_id_type start_vertex_id,
                                    const _vertex_id_type dest_vertex_id) {}
...
TEMPLATE_SPECIALIZATION
IC bool
CPathManagerTemplate::check_vertex(const _vertex_id_type vertex_id) const {
