...
CCharacterPhysicsSupport::CCharacterPhysicsSupport(EType atype,
    ...
    : m_pPhysicsShell(aentity->PPhysicsShell()), m_EntityAlife(*aentity),
      mXFORM(aentity->XFORM()), m_ph_sound_player(aentity),
      m_interactive_motion(0),
      m_PhysicMovementControl(xr_new<CPHMovementControl>(aentity)),
      m_eType(atype), m_eState(esAlive), m_physics_skeleton(NULL),
      m_ik_controller(NULL), m_BonceDamageFactor(1.f),
      m_collision_hit_callback(NULL), m_interactive_animation(NULL),
      m_physics_shell_animated(NULL),
      m_physics_shell_animated_time_destroy(u32(-1)), m_weapon_attach_bone(0),
      m_active_item_obj(0), m_hit_valide_time(u32(-1)),
...
void CCharacterPhysicsSupport::in_Load(LPCSTR section) {
  ...
  if (pSettings->line_exist(section, "bonce_damage_factor"))
    ...
    m_BonceDamageFactor = pSettings->r_float("collision_damage",
                                             "bonce_damage_factor_for_objects");
...
void CCharacterPhysicsSupport::in_NetSpawn(CSE_Abstract *e) {
  ...
  if (!m_EntityAlife.g_Alive()) {
    if (m_eType == etStalker) {
      // pK->LL_GetData( 0
      // ).shape.flags.set(SBoneShape::sfVisibilityIgnore,TRUE); pK->LL_GetData(
      // pK->LL_BoneID("bip01")
    ...
    ka->PlayCycle(
        "death_init"); ///непонятно зачем это вообще надо запускать
...
void CCharacterPhysicsSupport::SpawnInitPhysics(CSE_Abstract *e) {
  if (m_EntityAlife.g_Alive()) {
    ...
    if (ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject) &&
      ...
      Msg("CCharacterPhysicsSupport::SpawnInitPhysics obj %s before collision "
          "correction %f,%f,%f",
    ...
    if (ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject) &&
        stricmp(PH_DBG_ObjectTrackName(), *m_EntityAlife.cName()) == 0) {
      Msg("CCharacterPhysicsSupport::SpawnInitPhysics obj %s after collision "
          "correction %f,%f,%f",
          PH_DBG_ObjectTrackName(), m_EntityAlife.Position().x,
          m_EntityAlife.Position().y, m_EntityAlife.Position().z);
    }
...
void CCharacterPhysicsSupport::in_Hit(SHit &H, bool is_killing) {
  ...
  if (!(m_pPhysicsShell && m_pPhysicsShell->isActive())) {
    ...
    if (is_killing && death_anim_debug && !is_imotion(m_interactive_motion)) {
      Msg("death anim: applied fatal impulse dir: (%f,%f,%f), value: (%f) ",
...
void CCharacterPhysicsSupport::in_UpdateCL() {
  ...
  if (m_pPhysicsShell) {
    VERIFY(m_pPhysicsShell->isFullActive());
    m_pPhysicsShell->SetRagDoll(); //Теперь шела относиться к классу объектов
                                   // cbClassRagDoll
  ...
  if (Type() == etStalker && ph_dbg_draw_mask1.test(phDbgHitAnims)) {
    ...
    /*
       ...
       K->LL_BoneID("bip01_neck"); u16 eb = K->LL_BoneID("eye_right"); Fmatrix
       &mh  = K->LL_GetTransform(hb); Fmatrix &mp  = K->LL_GetTransform(pb);
...
bool CCharacterPhysicsSupport::CollisionCorrectObjPos(
    const Fvector &start_from, bool character_create /*=false*/) {
...
void CCharacterPhysicsSupport::RemoveActiveWeaponCollision() {
  ...
  // m_active_item_obj->PPhysicsShell()->dbg_draw_geometry( 0.2f, D3DCOLOR_XRGB(
  // 255, 0, 100 ) );
...
void CCharacterPhysicsSupport::bone_chain_disable(u16 bone, u16 r_bone,
                                                  IKinematics &K) {
  VERIFY(&K);
  u16 bid = bone;
...
void CCharacterPhysicsSupport::AddActiveWeaponCollision() {
  ...
  CPhysicsShell *weapon_shell = P_build_Shell(&active_weapon_item->object(),
                                              true, (BONE_P_MAP *)(0), true);
...
void CCharacterPhysicsSupport::CreateShell(CObject *who, Fvector &dp,
                                           Fvector &velocity) {
  xr_delete(m_collision_activating_delay);
  xr_delete(m_interactive_animation);
  destroy_animation_collision();
  ...
  m_pPhysicsShell->SetPrefereExactIntegration(); // use exact integration for
                                                 // ragdolls in single
...
void CCharacterPhysicsSupport::EndActivateFreeShell(
    CObject *who, const Fvector &inital_entity_position, const Fvector &dp,
...
#ifdef DEBUG
  if (death_anim_debug) {
    Msg("death anim: ragdoll velocity picked from char controller =(%f,%f,%f), "
        "velocity applied to ragdoll =(%f,%f,%f)  ",
