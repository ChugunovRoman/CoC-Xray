...
void CBastArtefact::ObjectContactCallback(bool & /**do_colide/**/, bool bo1,
                                          dContact &c,
                                          SGameMtl * /*material_1*/,
                                          SGameMtl * /*material_2*/) {
  ...
  if (!pBastArtefact)
    pBastArtefact =
        l_pUD2 ? smart_cast<CBastArtefact *>(l_pUD2->ph_ref_object) : NULL;
  if (!pBastArtefact)
    return;
  ...
  if (!pEntityAlive)
    pEntityAlive =
        l_pUD2 ? smart_cast<CEntityAlive *>(l_pUD2->ph_ref_object) : NULL;
...
void CBastArtefact::UpdateCLChild() {
  ...
  // CBastArtefact",*cName());
  // Log						("--- A -
  // CBastArtefact",renderable.xform);
  ...
  if (getVisible() && m_pPhysicsShell) {
    ...
    if (m_AttakingEntity) {
      if (m_AttakingEntity->g_Alive() && m_fEnergy > m_fStrikeImpulse) {
        ...
        m_pPhysicsShell->applyImpulse(dir, m_fStrikeImpulse *
                                               Device.fTimeDelta *
    ...
    if (m_fEnergy > 0 &&
        ::Random.randF(0.f, 1.0f) < (m_fEnergy / (m_fStrikeImpulse * 100.f))) {
