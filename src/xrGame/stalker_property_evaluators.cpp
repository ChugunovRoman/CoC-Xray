...
CStalkerPropertyEvaluatorEnemies::CStalkerPropertyEvaluatorEnemies(
    CAI_Stalker *object, LPCSTR evaluator_name, u32 time_to_wait,
...
_value_type CStalkerPropertyEvaluatorAnomaly::evaluate() {
  ...
  u32 result = dwfChooseAction(
      2000, m_object->panic_threshold(), 0.f, 0.f, 0.f, m_object->g_Team(),
...
_value_type CStalkerPropertyEvaluatorTooFarToKillEnemy::evaluate() {
  ...
  return (
      object().too_far_to_kill_enemy(mem_object.m_object_params.m_position));
}
...
_value_type CStalkerPropertyEvaluatorUseSuddenness::evaluate() {
  ...
  u32 last_time_seen = visual_memory_manager->visible_object_time_last_seen(
      object().dcast_CObject());
