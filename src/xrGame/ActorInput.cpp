  ...
  case kDETECTOR: {
    ...
    /*
       ...
       inventory().ItemFromSlot(FLARE_SLOT); if(fl_active)
                            {
                                    CFlare* fl			=
       smart_cast<CFlare*>(fl_active); fl->DropFlare		(); return
       ;
                            }

                            PIItem fli = inventory().Get(CLSID_DEVICE_FLARE,
       true); if(!fli)			return;
    ...
    if (item_name.size()) {
      ...
      if (itm) {
        ...
        StaticDrawableWrapper *_s =
            CurrentGameUI()->AddCustomStatic("item_used", true);
        string1024 str;
...
void CActor::IR_OnMouseWheel(int direction) {
  ...
  if (inventory().Action(
          (direction > 0) ? (u16)kWPN_ZOOM_DEC : (u16)kWPN_ZOOM_INC, CMD_START))
    return;
...
#ifdef DEBUG
  if (psActorFlags.test(AF_NO_CLIP) &&
      (cmd == kFWD || cmd == kBACK || cmd == kL_STRAFE || cmd == kR_STRAFE ||
...
void CActor::IR_OnMouseMove(int dx, int dy) {
  ...
  float scale =
      (C->f_fov / g_fov) * psMouseSens * psMouseSensScale / 50.f / LookFactor;
  if (dx) {
    ...
    if (object) {
      ...
      if (Level().IR_GetKeyState(DIK_LSHIFT)) {
        ...
        if (b_allow && !character_physics_support()->movement()->PHCapture()) {
          character_physics_support()->movement()->PHCaptureObject(object,
                                                                   element);
          bCaptured = true;
        }
...
void CActor::actor_kick() {
  ...
  static float kick_impulse =
      READ_IF_EXISTS(pSettings, r_float, "actor", "kick_impulse", 250.f);
  ...
  // The smaller the mass, the more damage given capped at 60 mass. 60+ mass
  // take 0 damage
  ...
  SHit hit = SHit(hit_power, dir, this, bone_id, h_pos, kick_impulse,
                  ALife::eHitTypeStrike, 0.f, false);
  O->Hit(&hit);
  if (EA) {
    ...
    if (EA->character_physics_support()) {
      ...
      EA->character_physics_support()->movement()->ApplyImpulse(
          dir.normalize(), kick_impulse * alive_kick_power);
    }
  }
