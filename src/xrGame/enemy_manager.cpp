...
bool CEnemyManager::useful(const CEntityAlive *entity_alive) const {
  ...
  return (m_useful_callback ? m_useful_callback(m_object->lua_game_object(),
...
float CEnemyManager::evaluate(const CEntityAlive *object) const {
  ...
  bool wounded =
      stalker ? stalker->wounded(&m_object->movement().restrictions()) : false;
  if (wounded) {
    if (m_stalker &&
        m_stalker->agent_manager().enemy().assigned_wounded(object, m_stalker))
      return (0.f);
  ...
  if (m_object->memory().visual().visible_now(object))
    ...
    //	if (actor) {
    //		if (smart_cast<const
    // CActor*>(object)->memory().visual().visible_now(m_object))
    // penalty
    //-= 900.f;
    //	}
    //	else {
    //		// if object is npc and it sees us
    //		const CCustomMonster	*monster = smart_cast<const
    // CCustomMonster*>(object); 		if (monster &&
    // monster->memory().visual().visible_now(m_object)) 			penalty -=
    // 300.f;
    //	}
...
void CEnemyManager::reload(LPCSTR section) {
  m_ignore_monster_threshold = READ_IF_EXISTS(pSettings, r_float, section,
                                              "ignore_monster_threshold", 1.f);
  m_max_ignore_distance =
      READ_IF_EXISTS(pSettings, r_float, section, "max_ignore_distance", 0.f);
  m_last_enemy_time = 0;
  m_last_enemy = 0;
  m_last_enemy_change = 0;
...
void CEnemyManager::set_ready_to_save() {
  ...
  //	Msg							("%6d %s DEcreased enemy counter
  //for player (%d ->
...
void CEnemyManager::remove_links(CObject *object) {
  ...
  OBJECTS::iterator I =
      std::find(m_objects.begin(), m_objects.end(), (CEntityAlive *)object);
  if (I != m_objects.end())
    m_objects.erase(I);
...
void CEnemyManager::restore_max_ignore_monster_distance() {
  m_max_ignore_distance = READ_IF_EXISTS(
      pSettings, r_float, *m_object->cNameSect(), "max_ignore_distance", 0.f);
}
