...
typedef intrusive_ptr<CSpaceRestriction, RestrictionSpace::CTimeIntrusiveBase>
    CRestrictionPtr;
...
shared_str
CSpaceRestrictionManager::base_in_restrictions(ALife::_OBJECT_ID id) {
...
shared_str
CSpaceRestrictionManager::base_out_restrictions(ALife::_OBJECT_ID id) {
  CLIENT_RESTRICTIONS::iterator I = m_clients->find(id);
  VERIFY(m_clients->end() != I);
  return ((*I).second.m_base_out_restrictions);
}

IC CSpaceRestrictionManager::CRestrictionPtr
...
bool CSpaceRestrictionManager::accessible(ALife::_OBJECT_ID id,
                                          u32 level_vertex_id, float radius) {
  CRestrictionPtr client_restriction = restriction(id);
  if (client_restriction)
...
CSpaceRestrictionManager::restriction(shared_str out_restrictors,
                                      shared_str in_restrictors) {
  string4096 m_temp;
  ...
  m_space_restrictions.insert(
      std::make_pair(space_restrictions, client_restriction));
...
IC bool
CSpaceRestrictionManager::restriction_presented(shared_str restrictions,
...
IC void CSpaceRestrictionManager::join_restrictions(shared_str &restrictions,
  ...
  for (u32 i = 0, n = _GetItemCount(*update), count = xr_strlen(m_temp2); i < n;
       ++i)
...
IC void
CSpaceRestrictionManager::difference_restrictions(shared_str &restrictions,
                                                  shared_str update) {
  string4096 m_temp1;
  string4096 m_temp2;
  xr_strcpy(m_temp2, "");
...
void CSpaceRestrictionManager::add_restrictions(
    ALife::_OBJECT_ID id, shared_str add_out_restrictions,
...
void CSpaceRestrictionManager::remove_restrictions(
    ALife::_OBJECT_ID id, shared_str remove_out_restrictions,
