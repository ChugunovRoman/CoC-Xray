...
void CScriptGameObject::set_item(MonsterSpace::EObjectAction object_action,
  ...
  if (!object_handler)
    ...
    object_handler->set_goal(object_action,
                             lua_game_object ? &lua_game_object->object() : 0);
}
...
void CScriptGameObject::Hit(CScriptHit *tpLuaHit) {
  ...
  HS.GenHeader(
      GE_HIT,
      object().ID()); //	object().u_EventGen(P,GE_HIT,object().ID());
  THROW2(tLuaHit.m_tpDraftsman,
         "Where is hit initiator??!"); //	THROW2
                                       //(tLuaHit.m_tpDraftsman,"Where is hit
                                       // initiator??!");
  HS.whoID =
      u16(tLuaHit.m_tpDraftsman->ID()); //	P.w_u16
                                        //(u16(tLuaHit.m_tpDraftsman->ID()));
  HS.weaponID = 0;                      //	P.w_u16			(0);
  HS.dir =
      tLuaHit.m_tDirection; //	P.w_dir			(tLuaHit.m_tDirection);
  HS.power =
      tLuaHit.m_fPower; //	P.w_float		(tLuaHit.m_fPower);
  IKinematics *V = smart_cast<IKinematics *>(
      object().Visual()); //	IKinematics		*V =
                          // smart_cast<IKinematics*>(object().Visual());
  VERIFY(V);              //	VERIFY			(V);
  if (xr_strlen(
          tLuaHit.m_caBoneName)) //	if (xr_strlen	(tLuaHit.m_caBoneName))
    HS.boneID = (V->LL_BoneID(
        tLuaHit.m_caBoneName)); //		P.w_s16
                                //(V->LL_BoneID(tLuaHit.m_caBoneName));
  else                          //	else
    HS.boneID = (s16(0));       //		P.w_s16		(s16(0));
  HS.p_in_bone_space = Fvector().set(
      0, 0, 0); //	P.w_vec3		(Fvector().set(0,0,0));
  HS.impulse =
      tLuaHit.m_fImpulse; //	P.w_float		(tLuaHit.m_fImpulse);
...
Fvector
CScriptGameObject::memory_position(const CScriptGameObject &lua_game_object) {
  CCustomMonster *monster = smart_cast<CCustomMonster *>(&object());
  if (!monster) {
    ai().script_engine().script_log(
        ScriptStorage::eLuaMessageTypeError,
...
void CScriptGameObject::set_sound_threshold(float value) {
  ...
  if (!monster) {
    ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                    "CSoundMemoryManager : cannot access class "
                                    "member set_sound_threshold!");
    return;
  }
...
void CScriptGameObject::restore_sound_threshold() {
  ...
  if (!monster) {
    ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                    "CSoundMemoryManager : cannot access class "
                                    "member restore_sound_threshold!");
    return;
  }
  ...
  if (actor) {
    if (bskip_collision_correct) {
      ...
      if (actor->character_physics_support()->movement()->CharacterExist()) {
        ...
        actor->character_physics_support()->movement()->SetVelocity(0.f, 0.f,
                                                                    0.f);
      }
    } else {
      Fmatrix F = actor->XFORM();
...
void CScriptGameObject::set_visual_name(LPCSTR visual, bool bForce) {
  ...
  if (stalker) {
    ...
    IKinematicsAnimated *V =
        smart_cast<IKinematicsAnimated *>(stalker->Visual());
    if (V) {
      if (!stalker->already_dead())
        stalker->CStepManager::reload(stalker->cNameSect().c_str());
      stalker->CDamageManager::reload(*stalker->cNameSect(), "damage",
                                      pSettings);
...
void CScriptGameObject::RemoveMemorySoundObject(
    const MemorySpace::CSoundObject &memory_object) {
  CAI_Stalker *stalker = smart_cast<CAI_Stalker *>(&object());
  if (!stalker)
    return;
...
void CScriptGameObject::RemoveMemoryHitObject(
    const MemorySpace::CHitObject &memory_object) {
  CAI_Stalker *stalker = smart_cast<CAI_Stalker *>(&object());
  if (!stalker)
    return;
...
void CScriptGameObject::RemoveMemoryVisibleObject(
    const MemorySpace::CVisibleObject &memory_object) {
  CAI_Stalker *stalker = smart_cast<CAI_Stalker *>(&object());
  if (!stalker)
    return;
