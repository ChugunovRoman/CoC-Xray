...
void CWeaponMagazined::Load(LPCSTR section) {
  ...
  m_layered_sounds.LoadSound(section, "snd_shoot", "sndShot", false,
                             m_eSoundShot);
  // if (WeaponSoundExist(section, "snd_shoot_actor"))
  //	m_layered_sounds.LoadSound(section, "snd_shoot_actor", "sndShotActor",
  // false, m_eSoundShot);
#else
  m_sounds.LoadSound(section, "snd_shoot", "sndShot", false, m_eSoundShot);
  // if (WeaponSoundExist(section, "snd_shoot_actor"))
  //	m_sounds.LoadSound(section, "snd_shoot_actor", "sndShot", false,
  // m_eSoundShot);
#endif
  //-Alundaio
  ...
  if (WeaponSoundExist(section, "snd_reload_misfire"))
    m_sounds.LoadSound(section, "snd_reload_misfire", "sndReloadMisfire", true,
...
#ifdef LAYERED_SND_SHOOT
    m_layered_sounds.LoadSound(section, "snd_silncer_shot", "sndSilencerShot",
                               false, m_eSoundShot);
    if (WeaponSoundExist(section, "snd_silncer_shot_actor"))
      m_layered_sounds.LoadSound(section, "snd_silncer_shot_actor",
                                 "sndSilencerShotActor", false, m_eSoundShot);
#else
    m_sounds.LoadSound(section, "snd_silncer_shot", "sndSilencerShot", false,
                       m_eSoundShot);
    if (WeaponSoundExist(section, "snd_silncer_shot_actor"))
  ...
  m_fBaseDispersionedBulletsSpeed =
      READ_IF_EXISTS(pSettings, r_float, section,
                     "base_dispersioned_bullets_speed", m_fStartBulletSpeed);
  ...
  if (pSettings->line_exist(section, "fire_modes")) {
    ...
    m_iPrefferedFireMode =
        READ_IF_EXISTS(pSettings, r_s16, section, "preffered_fire_mode", -1);
  } else {
    m_bHasDifferentFireModes = false;
  }
  LoadSilencerKoeffs();
...
void CWeaponMagazined::FireStart() {
  if (!IsMisfire()) {
    ...
    if (object)
      object->callback(GameObject::eOnWeaponJammed)(object->lua_game_object(),
...
bool CWeaponMagazined::TryReload() {
  if (m_pInventory) {
    if (ParentIsActor()) {
      ...
      Actor()->callback(GameObject::eWeaponNoAmmoAvailable)(lua_game_object(),
                                                            AC);
    }

    m_pCurrentAmmo = smart_cast<CWeaponAmmo *>(
        m_pInventory->GetAny(m_ammoTypes[m_ammoType.type1].c_str()));
...
void CWeaponMagazined::OnMagazineEmpty() {
  ...
  if (ParentIsActor()) {
    int AC = GetSuitableAmmoTotal();
    Actor()->callback(GameObject::eOnWeaponMagazineEmpty)(lua_game_object(),
                                                          AC);
  }
#endif
  if (GetState() == eIdle) {
...
void CWeaponMagazined::UnloadMagazine(bool spawn_ammo) {
  ...
  for (l_it = l_ammo.begin(); l_ammo.end() != l_it; ++l_it) {
    if (m_pInventory) {
      CWeaponAmmo *l_pA =
          smart_cast<CWeaponAmmo *>(m_pInventory->GetAny(l_it->first));
      if (l_pA) {
        u16 l_free = l_pA->m_boxSize - l_pA->m_boxCurr;
        l_pA->m_boxCurr =
            l_pA->m_boxCurr + (l_free < l_it->second ? l_free : l_it->second);
        l_it->second =
            l_it->second - (l_free < l_it->second ? l_free : l_it->second);
      }
    }
...
void CWeaponMagazined::ReloadMagazine() {
  ...
  if (!m_bLockType && !m_magazine.empty() &&
      (!m_pCurrentAmmo ||
       xr_strcmp(m_pCurrentAmmo->cNameSect(), *m_magazine.back().m_ammoSect)))
    UnloadMagazine();

  VERIFY((u32)m_ammoElapsed.type1 == m_magazine.size());

  if (m_DefaultCartridge.m_LocalAmmoType != m_ammoType.type1)
    m_DefaultCartridge.Load(m_ammoTypes[m_ammoType.type1].c_str(),
                            m_ammoType.type1, m_APk);
...
void CWeaponMagazined::state_Fire(float dt) {
  if (m_ammoElapsed.type1 > 0) {
    ...
    while (!m_magazine.empty() && fShotTimeCounter < 0 &&
      ...
      // Alundaio: Use fModeShotTime instead of fOneShotTime if current fire
      // mode is 2-shot burst Alundaio: Cycle down RPM after two shots; used for
      // Abakan/AN-94
...
// Alundaio: LAYERED_SND_SHOOT
  ...
  if (ParentIsActor()) {
    /*
    if (strcmp(m_sSndShotCurrent.c_str(), "sndShot") == 0 &&
    pSettings->line_exist(m_section_id,"snd_shoot_actor") &&
    m_layered_sounds.FindSoundItem("sndShotActor", false))
            m_layered_sounds.PlaySound("sndShotActor", get_LastFP(), H_Root(),
    !!GetHUDmode(), false, (u8)-1); else if (strcmp(m_sSndShotCurrent.c_str(),
    "sndSilencerShot") == 0 &&
    pSettings->line_exist(m_section_id,"snd_silncer_shot_actor") &&
    m_layered_sounds.FindSoundItem("sndSilencerShotActor", false))
            m_layered_sounds.PlaySound("sndSilencerShotActor", get_LastFP(),
    H_Root(), !!GetHUDmode(), false, (u8)-1); else
    */
    m_layered_sounds.PlaySound(m_sSndShotCurrent.c_str(), get_LastFP(),
  ...
  if (ParentIsActor()) {
    /*
    if (strcmp(m_sSndShotCurrent.c_str(), "sndShot") == 0 &&
    pSettings->line_exist(m_section_id, "snd_shoot_actor")&& snd_silncer_shot
    m_sounds.FindSoundItem("sndShotActor", false)) PlaySound("sndShotActor",
    get_LastFP(), (u8)(m_iShotNum - 1)); else if
    (strcmp(m_sSndShotCurrent.c_str(), "sndSilencerShot") == 0 &&
    pSettings->line_exist(m_section_id, "snd_silncer_shot_actor") &&
    m_sounds.FindSoundItem("sndSilencerShotActor", false))
            PlaySound("sndSilencerShotActor", get_LastFP(), (u8)(m_iShotNum -
    1)); else
    */
    PlaySound(m_sSndShotCurrent.c_str(), get_LastFP(), (u8)-1);
  } else
    PlaySound(m_sSndShotCurrent.c_str(), get_LastFP(),
              (u8)-1); // Alundaio: Play sound at index (ie. snd_shoot,
                       // snd_shoot1, snd_shoot2, snd_shoot3)
#endif
  //-Alundaio
  ...
  if (object)
    object->callback(GameObject::eOnWeaponFired)(
        object->lua_game_object(), this->lua_game_object(), m_ammoElapsed.type1,
        m_ammoElapsed.type2, m_ammoType.type1, m_ammoType.type2);
#endif
}
...
bool CWeaponMagazined::CanAttach(PIItem pIItem) {
  ...
  if (pScope &&
    ...
    for (; it != m_silencers.end(); it++) {
      if (pSettings->r_string((*it), "silencer_name") ==
          pIItem->object().cNameSect())
        return true;
    }
    return false;
    ...
    for (; it != m_launchers.end(); it++) {
      if (pSettings->r_string((*it), "grenade_launcher_name") ==
          pIItem->object().cNameSect())
        return true;
    }
    return false;
...
bool CWeaponMagazined::DetachSilencer(const char *item_section_name,
                                      bool b_spawn_item) {
  bool detached = false;
  SCOPES_VECTOR_IT it = m_silencers.begin();
  for (; it != m_silencers.end(); it++) {
...
bool CWeaponMagazined::DetachGrenadeLauncher(const char *item_section_name,
                                             bool b_spawn_item) {
  bool detached = false;
  SCOPES_VECTOR_IT it = m_launchers.begin();
  for (; it != m_launchers.end(); it++) {
    LPCSTR iter_scope_name =
        pSettings->r_string((*it), "grenade_launcher_name");
    if (!xr_strcmp(iter_scope_name, item_section_name)) {
...
{
m_zoom_params.m_fIronSightZoomFactor = READ_IF_EXISTS( pSettings, r_float,
cNameSect(), "ironsight_zoom_factor", 50.0f );
}
*/
void CWeaponMagazined::InitAddons() {
  m_zoom_params.m_fIronSightZoomFactor = READ_IF_EXISTS(
      pSettings, r_float, cNameSect(), "ironsight_zoom_factor", 1.03f);
  if (IsScopeAttached()) {
    if (m_eScopeStatus == ALife::eAddonAttachable) {
      ...
      m_zoom_params.m_fScopeZoomFactorMin = READ_IF_EXISTS(
          pSettings, r_float, GetScopeName(), "scope_zoom_factor_min", 0.3f);
      ...
      m_zoom_params.m_bUseDynamicZoom = READ_IF_EXISTS(
          pSettings, r_bool, GetScopeName(), "scope_dynamic_zoom", FALSE);
      m_zoom_params.m_sUseBinocularVision = READ_IF_EXISTS(
          pSettings, r_string, GetScopeName(), "scope_alive_detector", 0);
  ...
  if (IsSilencerAttached() /* && SilencerAttachable() */) {
    ...
    LPCSTR light_section = m_eSilencerStatus == ALife::eAddonAttachable
                               ? m_silencers[m_cur_addon.silencer].c_str()
                               : cNameSect().c_str();
...
void CWeaponMagazined::OnZoomIn() {
  ...
  CGameObject *object = smart_cast<CGameObject *>(H_Parent());
  if (object)
    object->callback(GameObject::eOnWeaponZoomIn)(object->lua_game_object(),
                                                  this->lua_game_object());
#endif
...
void CWeaponMagazined::OnZoomOut() {
  ...
  CGameObject *object = smart_cast<CGameObject *>(H_Parent());
  if (object)
    object->callback(GameObject::eOnWeaponZoomOut)(object->lua_game_object(),
                                                   this->lua_game_object());
#endif
  //-Alundaio
...
bool CWeaponMagazined::install_upgrade_impl(LPCSTR section, bool test) {
  ...
  result2 = process_if_exists_set(section, "snd_draw", &CInifile::r_string, str,
                                  test);
  if (result2 && !test) {
  ...
  result2 = process_if_exists_set(section, "snd_holster", &CInifile::r_string,
                                  str, test);
  if (result2 && !test) {
  ...
  // fov for zoom mode
  result |=
      process_if_exists(section, "ironsight_zoom_factor", &CInifile::r_float,
...
void CWeaponMagazined::FireBullet(const Fvector &pos, const Fvector &shot_dir,
