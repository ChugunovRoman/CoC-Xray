  ...
  if (show_text) {
    ...
    for (; I != E; ++I) {
      ...
      if (psAI_Flags.test(aiDrawGameGraphRealPos))
        ...
        position =
            convert_position(graph.vertex(stalker->m_tGraphID)->game_point());
  ...
  if (show_text) {
    ...
    for (; I != E; ++I) {
      ...
      if (psAI_Flags.test(aiDrawGameGraphRealPos))
        ...
        position =
            convert_position(graph.vertex(monster->m_tGraphID)->game_point());
...
void CLevelGraph::draw_game_graph() {
  ...
  // draw back plane
  Fvector vertices[4];
  xform.transform_tiny(vertices[0],
                       Fvector().set(center.x - bounds.x, center.y + bounds.y,
                                     center.z + bounds.z));
  xform.transform_tiny(vertices[1],
                       Fvector().set(center.x + bounds.x, center.y + bounds.y,
                                     center.z + bounds.z));
  xform.transform_tiny(vertices[2],
                       Fvector().set(center.x - bounds.x, center.y - bounds.y,
                                     center.z - bounds.z));
  xform.transform_tiny(vertices[3],
                       Fvector().set(center.x + bounds.x, center.y - bounds.y,
                                     center.z - bounds.z));
  ...
  ai().game_graph().vertex(tpALifeHuman->brain().movement().detail().path().back())->game_point();
                                          ...
                                          Level().debug_renderer().draw_aabb(t1,.05f,.05f,.05f,D3DCOLOR_XRGB(0,0,255));
                                          for (int
  i=(int)tpALifeHuman->brain().movement().detail().path().size() - 2; i>=0;--i)
  { Fvector t2 =
  ai().game_graph().vertex(tpALifeHuman->brain().movement().detail().path()[i])->game_point();
                                  ...
                                  if (tpALifeMonsterAbstract->m_fDistanceToPoint
  > EPS_L) { Fvector t1 =
