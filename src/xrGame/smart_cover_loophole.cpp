...
void loophole::add_action(LPCSTR type, luabind::object const &table) {
  ...
  VERIFY(std::find_if(m_actions.begin(), m_actions.end(), id_predicate(type)) ==
         m_actions.end());
...
loophole::action_animations(shared_str const &action_id,
...
loophole::transition_animations(shared_str const &action_from,
                                shared_str const &action_to) const {
  TransitionGraph::CEdge const *edge = m_transitions.edge(
      transform_vertex(action_from, true), transform_vertex(action_to, false));
  VERIFY2(edge, make_string("transition [%s]->[%s] absent", action_from.c_str(),
                            action_to.c_str()));
  return (edge->data());
}
