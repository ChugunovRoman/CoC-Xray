...
void stalker_movement_manager_obstacles::move_along_path(
  ...
  if (!ai().doors().actualize_doors_state(*m_doors_actor,
                                          old_desirable_speed())) {
    //		Msg							( "%6d stalker %s waits for the some door to
    //be open/closed", Device.dwTimeGlobal, object().cName().c_str() );
    float desirable_speed = old_desirable_speed();
    set_desirable_speed(0.f);
  ...
  //	Msg								( "%6d stalker %s is
  //going", Device.dwTimeGlobal, object().cName().c_str() );
...
static float get_distance(Fvector const &a_first, Fvector const &a_second,
                          Fvector const &b_first, Fvector const &b_second,
...
float stalker_movement_manager_obstacles::is_going_through(
    Fmatrix const &matrix, Fvector const &vector,
  ...
  for (; i != e; ++i) {
    float const distance = get_distance((*(i - 1)).position, (*i).position,
