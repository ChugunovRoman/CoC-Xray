...
// return xr_free(ptr);							} #else
// // DEBUG_MEMORY_MANAGER 	static	void *	ode_alloc	(size_t size) {
...
void CGamePersistent::WeathersUpdate() {
  if (g_pGameLevel && !g_dedicated_server) {
    ...
    int data_set =
        (Random.randF() < (1.f - Environment().CurrentEnv->weight)) ? 0 : 1;
    ...
    if (env_amb) {
      CEnvAmbient::SSndChannelVec &vec =
          current_env->env_ambient->get_snd_channels();
      ...
      for (u32 idx = 0; I != E; ++I, ++idx) {
        ...
        if (ambient_sound_next_time[idx] == 0) // first
        {
          ...
          //					Msg("- Playing ambient sound channel
          //[%s]
      ...
      /*
                  ...
                  {
                  ref_sound* snd			=
         env_amb->get_rnd_sound(); ambient_sound_next_time	=
         Device.dwTimeGlobal + env_amb->get_rnd_sound_time(); if (snd)
                  {
                  Fvector	pos;
                  float	angle		= ::Random.randF(PI_MUL_2);
                  pos.x				= _cos(angle);
                  ...
                  pos.z				= _sin(angle);
                  pos.normalize
         ().mul(env_amb->get_rnd_sound_dist()).add(Device.vCameraPosition);
                  pos.y				+= 10.f;
                  snd->play_at_pos	(0,pos);
                  }
                  }
        ...
        if (eff) {
          ...
          if (Environment().wind_blast_strength_start_value == 0.f) {
            Environment().wind_blast_start_time.set(
                0.f, eff->wind_blast_direction.x, eff->wind_blast_direction.y,
...
void CGamePersistent::start_logo_intro() {
  if (Device.dwPrecacheFrame == 0) {
    ...
    if (!g_dedicated_server && 0 == xr_strlen(m_game_params.m_game_or_spawn) &&
        NULL == g_pGameLevel) {
      VERIFY(NULL == m_intro);
...
void CGamePersistent::game_loaded() {
  if (Device.dwPrecacheFrame <= 2) {
    if (g_pGameLevel && g_pGameLevel->bReady &&
      ...
      m_intro->m_on_destroy_event.bind(this,
                                       &CGamePersistent::update_game_loaded);
    }
    m_intro_event = 0;
  }
}
...
void CGamePersistent::OnFrame() {
  ...
  if (0 != pDemoFile) {
    if (Device.dwTimeGlobal > uTime2Change) {
      ...
      Engine.Event.Defer("GAME:demo", size_t(xr_strdup(_Trim(o_demo))),
                         u64(o_time));
...
void CGamePersistent::LoadTitle(bool change_tip, shared_str map_name) {
  ...
  if (change_tip) {
    ...
    R_ASSERT(
        ai().script_engine().functor("loadscreen.get_tip_header", m_functor));
    tip_header = m_functor(map_name.c_str());

    R_ASSERT(
        ai().script_engine().functor("loadscreen.get_tip_title", m_functor));
    tip_title = m_functor(map_name.c_str());

    R_ASSERT(
        ai().script_engine().functor("loadscreen.get_tip_text", m_functor));
