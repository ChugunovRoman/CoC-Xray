...
u32 CSoundPlayer::add(LPCSTR prefix, u32 max_count, ESoundTypes type,
  ...
  VERIFY(pair.second);
  m_sounds.insert(
      std::make_pair(internal_type, std::make_pair(sound_params, pair.second)));
...
void CSoundPlayer::play(u32 internal_type, u32 max_start_time,
                        u32 min_start_time, u32 max_stop_time,
...
#ifdef DEBUG
    Msg("- There are no sounds in sound collection \"%s\" with internal type "
        "%d (sound_script = %d)",
  ...
  /**/
  sound_single.m_sound->clone((*I).second.second->random(id), st_Effect,
                              sg_SourceType);
  ...
  sound_single.m_stop_time =
      ...
      iFloor(sound_single.m_sound->get_length_sec() * 1000.0f) + random_time;
