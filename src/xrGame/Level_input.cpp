...
void CLevel::IR_OnKeyboardPress(int key) {
...
#ifdef INGAME_EDITOR
  if (Device.editor() && (pInput->iGetAsyncKeyState(DIK_LALT) ||
                          pInput->iGetAsyncKeyState(DIK_RALT)))
    return;
  ...
  if (_curr == kQUICK_LOAD) {
    ...
    strconcat(sizeof(saved_game), saved_game, Core.UserName, " - ",
              "quicksave");
    ...
    if (i < n) {
      ...
      if (bOk) {
        ...
        if (pActor) {
          ...
          CHudItem *pHudItem =
              smart_cast<CHudItem *>(pActor->inventory().ActiveItem());
          if (pHudItem) {
