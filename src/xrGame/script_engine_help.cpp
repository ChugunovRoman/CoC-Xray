...
void print_class(lua_State *L, luabind::detail::class_rep *crep) {
  ...
  {
    S = (crep->get_class_type() != luabind::detail::class_rep::cpp_class)
            ? "LUA class "
            : "C++ class ";
    S.append(crep->name());
    typedef luabind::internal_vector<luabind::detail::class_rep::base_info>
        BASES;
  ...
  {
    typedef luabind::internal_vector<luabind::detail::construct_rep::overload_t>
        Constructors;
...
void print_free_functions(lua_State *L, const luabind::object &object,
                          LPCSTR header, const xr_string &indent) {
  u32 count = 0;
        ...
        if (lua_touserdata(L, -1) == (void *)0x1337) {
          if (lua_getupvalue(L, -2, 1) != 0) {
            ...
            rep = static_cast<luabind::detail::free_functions::function_rep *>(
                lua_touserdata(L, -1));
            std::vector<luabind::detail::free_functions::overload_rep>::
                const_iterator i = rep->overloads().begin();
            std::vector<luabind::detail::free_functions::overload_rep>::
                const_iterator e = rep->overloads().end();
            for (; i != e; ++i) {
              luabind::internal_string luaS;
              (*i).get_signature(L, luaS);
              xr_string S(luaS.c_str());
