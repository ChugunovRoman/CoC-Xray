...
character_shell_control::character_shell_control()
    : m_shot_up_factor(0.f), m_after_death_velocity_factor(1.f),
      m_was_wounded(false), m_Pred_Time(0.0) {}
...
void character_shell_control::set_fatal_impulse(SHit &H) const {
  if (!m_was_wounded) {
    H.impulse *=
        (H.type() == ALife::eHitTypeExplosion ? 1.f
                                              : skel_fatal_impulse_factor);
  }
}
void OnCharacterContactInDeath(bool &do_colide, bool bo1, dContact &c,
                               SGameMtl * /*material_1*/,
                               SGameMtl * /*material_2*/) {
...
void character_shell_control::apply_start_velocity_factor(
    CObject *who, Fvector &velocity) const {
  velocity.mul(1.3f);
