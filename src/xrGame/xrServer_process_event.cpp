...
void xrServer::Process_event(NET_Packet &P, ClientID sender) {
  ...
  case GE_ASSIGN_KILLER: {
    ...
    //		Msg							("[%d][%s] killed [%d][%s]",id_src,id_src==u16(-1)
    //? "UNKNOWN" :
  ...
  case GE_DIE: {
    ...
    //			R_ASSERT2			(e_dest && e_src, "Killer or/and being killed
    //are offline or not exist at all :(");
  ...
  case GE_WEAPON_SYNCRONIZE: {
    ...
    if (pW) {
      ...
      CSE_ALifeItemWeaponMagazinedWGL *pWGL =
          smart_cast<CSE_ALifeItemWeaponMagazinedWGL *>(pW);
      if (pWGL)
