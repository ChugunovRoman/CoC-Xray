...
void CFlashlight::OnStateSwitch(u32 S, u32 oldState) {
  ...
  case eShowing: {
    ...
    PlayHUDMotion(m_bFastAnimMode ? "anm_show_fast" : "anm_show",
                  FALSE /*TRUE*/, this, GetState());
    SetPending(TRUE);
  } break;
  case eHiding: {
    if (oldState != eHiding) {
      m_sounds.PlaySound("sndHide", Fvector().set(0, 0, 0), this, true, false);
      PlayHUDMotion(m_bFastAnimMode ? "anm_hide_fast" : "anm_hide",
                    FALSE /*TRUE*/, this, GetState());
      SetPending(TRUE);
    }
  } break;
...
void CFlashlight::UpdateXForm() { CInventoryItem::UpdateXForm(); }

void CFlashlight::OnActiveItem() { return; }
...
BOOL CFlashlight::net_Spawn(CSE_Abstract *DC) {
  ...
  lanim =
      LALib.FindItem(pSettings->r_string(m_light_section, "color_animator"));
  ...
  Fcolor clr_o = pSettings->r_fcolor(m_light_section,
                                     (b_r2) ? "omni_color_r2" : "omni_color");
  float range_o = pSettings->r_float(m_light_section,
  ...
  light_render->set_texture(READ_IF_EXISTS(pSettings, r_string, m_light_section,
                                           "spot_texture", (0)));
  ...
  light_render->set_volumetric(
      !!READ_IF_EXISTS(pSettings, r_bool, m_light_section, "volumetric", 0));
  ...
  light_render->set_type((IRender_Light::LT)(
      READ_IF_EXISTS(pSettings, r_u8, m_light_section, "type", 2)));
  light_omni->set_type((IRender_Light::LT)(
      READ_IF_EXISTS(pSettings, r_u8, m_light_section, "omni_type", 1)));
...
void CFlashlight::shedule_Update(u32 dt) { inherited::shedule_Update(dt); }

bool CFlashlight::IsWorking() {
  return m_switched_on && H_Parent() &&
    ...
    if (light_trace_bone.size()) {
      ...
      if (bone_id != BI_NONE) {
        ...
        if (visi != m_switched_on)
          HudItemData()->m_model->LL_SetBoneVisible(bone_id, m_switched_on,
                                                    TRUE);
      }
    }
...
void CFlashlight::UpdateCL() {
  ...
  light_render->set_rotation(dep.m_FireParticlesXForm.k,
                             dep.m_FireParticlesXForm.i);
  light_omni->set_rotation(dep.m_FireParticlesXForm.k,
                           dep.m_FireParticlesXForm.i);
...
void CFlashlight::OnH_A_Chield() { inherited::OnH_A_Chield(); }
...
void CFlashlight::Switch() { Switch(!m_switched_on); }
...
bool CFlashlight::torch_active() const { return m_switched_on; }
