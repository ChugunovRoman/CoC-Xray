...
CPhysicObject::CPhysicObject(void)
    : m_anim_blend(0), m_type(epotBox), m_mass(10.f),
      m_collision_hit_callback(0), bones_snd_player(0), m_net_updateData(0),
      m_just_after_spawn(false), m_activated(false) {}
...
void CPhysicObject::create_collision_model() {
  ...
  if (ini && ini->section_exist("collide") &&
      ini->line_exist("collide", "mesh") && ini->r_bool("collide", "mesh")) {
...
static void door_ignore(bool &do_collide, bool bo1, dContact &c,
...
#ifdef DEBUG
      VERIFY2(
          (!!PKinematicsAnimated->LL_MotionID(visual->startup_animation.c_str())
                 .valid()),
...
#ifdef DEBUG
  if (!b)
    Msg(" ! can not control anim - model is not animated name[%s] sect[%s] "
        "visual[%s]",
        name, sect, visual);
#endif
  return !!b;
...
#ifdef DEBUG
    Msg(" ! can not set blend time %f - it must be in range 0 - %f(timeTotal) "
        "obj: %s, model: %s, anim: %s",
...
void CPhysicObject::CreateSkeleton(CSE_ALifeObjectPhysic *po) {
  ...
  ApplySpawnIniToPhysicShell(&po->spawn_ini(), m_pPhysicsShell,
                             fixed_bones[0] != '\0');
  ApplySpawnIniToPhysicShell(smart_cast<IKinematics *>(Visual())->LL_UserData(),
                             m_pPhysicsShell, fixed_bones[0] != '\0');
}
...
void CPhysicObject::CreateBody(CSE_ALifeObjectPhysic *po) {
  ...
  case epotBox: {
    m_pPhysicsShell = P_build_SimpleShell(
        this, m_mass, !po->_flags.test(CSE_ALifeObjectPhysic::flActive));
  } break;
  case epotFixedChain:
  case epotFreeChain: {
...
bool CPhysicObject::is_ai_obstacle() const {
  return !!(
      READ_IF_EXISTS(pSettings, r_bool, cNameSect(), "is_ai_obstacle", true));
}
