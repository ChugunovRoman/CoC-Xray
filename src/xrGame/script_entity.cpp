...
void CScriptEntity::SetScriptControl(const bool bScriptControl,
  ...
  if (!(((m_bScriptControl && !bScriptControl) ||
        ...
        (bScriptControl || (xr_strlen(*m_caScriptName) &&
...
void CScriptEntity::vfUpdateParticles() {
  ...
  if (xr_strlen(l_tParticleAction.m_caBoneName)) {
    CParticlesObject *l_tpParticlesObject =
  ...
  if (xr_strlen(caBoneName)) {
    CBoneInstance &l_tBoneInstance =
        smart_cast<IKinematics *>(object().Visual())
            ->LL_GetBoneInstance(smart_cast<IKinematics *>(object().Visual())
...
#ifdef _DEBUG
//				Msg
//("%6d Starting sound %s",Device.dwTimeGlobal,*l_tSoundAction.m_caSoundToPlay);
#endif
        const Fmatrix &l_tMatrix = GetUpdatedMatrix(
            l_tSoundAction.m_caBoneName, l_tSoundAction.m_tSoundPosition,
...
bool CScriptEntity::bfAssignMovement(CScriptEntityAction *tpEntityAction) {
  ...
  case CScriptMovementAction::eGoalTypePathPosition: {
    ...
    u32 vertex_id;
    vertex_id =
        ai().level_graph().vertex(object().ai_location().level_vertex_id(),
                                  l_tMovementAction.m_tDestinationPosition);
    ...
    if (!ai().level_graph().valid_vertex_id(vertex_id)) {
      string256 S;
      xr_sprintf(S,
                 "Cannot find corresponding level vertex for the specified "
                 "position [%f][%f][%f] for monster %s",
  ...
  case CScriptMovementAction::eGoalTypeNoPathPosition: {
    ...
    if (m_monster->movement().detail().path().empty() ||
        (m_monster->movement()
             ...
             .position.distance_to(l_tMovementAction.m_tDestinationPosition) >
         .1f)) {
...
CScriptEntity::GetActionByIndex(u32 action_index) const {
  return (m_tpActionQueue[action_index]);
}

void CScriptEntity::sound_callback(const CObject *object, int sound_type,
...
void CScriptEntity::process_sound_callbacks() {
  ...
  for (; I != E; ++I) {
    ...
    object().callback(GameObject::eSound)(
        object().lua_game_object(), (*I).m_game_object_id, (*I).m_sound_type,
