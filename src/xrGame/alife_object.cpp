...
void CSE_ALifeObject::spawn_supplies(LPCSTR ini_string) {
  ...
  if (ai().script_engine().functor("ai_stalker.CSE_ALifeObject_spawn_supplies",
                                   funct)) {
...
#pragma warning(pop)
  ...
  // No need to spawn ammo, this will automatically spawn 1 box for weapon and
  // if ammo_type is specficied it will spawn that type count is used only for
  // ammo boxes (ie wpn_pm = 3) will spawn 3 boxes, not 3 wpn_pm Usage: to
    ...
    if (!OnlyOne.empty()) {
      ...
      if (ini.r_line(loadout_section, OnlyOne.at(sel), &itmSection, &V)) {
        ...
        if (pSettings->section_exist(itmSection)) {
          ...
          CSE_Abstract *E = alife().spawn_item(itmSection, o_Position,
            ...
            if (pSettings->line_exist(itmSection, "ammo_class")) {
              ...
              if (xr_strlen(ammoSec) && pSettings->section_exist(ammoSec)) {
                for (u32 i = 1; i <= spawn_count; ++i) {
                  alife().spawn_item(ammoSec, o_Position, m_tNodeID, m_tGraphID,
                                     ID);
                }
              }
            }
