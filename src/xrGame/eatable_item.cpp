...
void CEatableItem::Load(LPCSTR section) {
  ...
  m_bConsumeChargeOnUse =
      READ_IF_EXISTS(pSettings, r_bool, section, "consume_charge_on_use", TRUE);
  m_fWeightFull = m_weight;
  m_fWeightEmpty =
      READ_IF_EXISTS(pSettings, r_float, section, "empty_weight", 0.0f);
}

void CEatableItem::load(IReader &packet) { inherited::load(packet); }

void CEatableItem::save(NET_Packet &packet) { inherited::save(packet); }
