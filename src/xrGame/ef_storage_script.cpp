...
float evaluate(CEF_Storage *ef_storage, LPCSTR function, CScriptGameObject *_0,
  ...
  if (!f) {
    ai().script_engine().script_log(
        eLuaMessageTypeError, "Cannot find evaluation function %s", function);
    return (0.f);
  }
  ...
  if (_0 && !ef_storage->non_alife().member()) {
    ai().script_engine().script_log(
        ...
        "object %s is not herited from CSE_ALifeSchedulable!", *_0->cName());
    return (0.f);
  }
  ...
  if (_1 && !ef_storage->non_alife().enemy()) {
    ai().script_engine().script_log(
        eLuaMessageTypeError,
        "object %s is not herited from CSE_ALifeSchedulable!", *_1->cName());
    return (0.f);
  }
...
float evaluate(CEF_Storage *ef_storage, LPCSTR function,
...
float evaluate(CEF_Storage *ef_storage, LPCSTR function, CSE_ALifeObject *_0,
               CSE_ALifeObject *_1, CSE_ALifeObject *_2, CSE_ALifeObject *_3) {
...
float evaluate(CEF_Storage *ef_storage, LPCSTR function, CSE_ALifeObject *_0,
               CSE_ALifeObject *_1, CSE_ALifeObject *_2) {
  return (evaluate(ef_storage, function, _0, _1, _2, 0));
}
  ...
  module(L)
      [def("ef_storage", &ef_storage),
       ...
       class_<CEF_Storage>("cef_storage")
           .def("evaluate", (float (*)(CEF_Storage *, LPCSTR,
