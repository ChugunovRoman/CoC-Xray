...
MotionID
CStalkerAnimationManager::aim_animation(const u32 &slot,
  ...
  if (animation[6].A.size() < 7) {
    // Alundaio: No need for message as many npcs have special danger move and
    // very few don't
#ifdef DEBUG
...
MotionID CStalkerAnimationManager::unknown_object_animation(
    u32 slot, const EBodyState &body_state) const {
  // animation shortcuts
...
MotionID
CStalkerAnimationManager::weapon_animation(u32 slot,
...
MotionID
CStalkerAnimationManager::missile_animation(u32 slot,
                                            const EBodyState &body_state) {
  VERIFY(m_missile);
