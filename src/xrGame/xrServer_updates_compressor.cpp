...
last_updates_cache::search_entity(u16 const entity_id) {
  for (u32 i = 0; i < cache_entities_size; ++i) {
...
last_updates_cache::last_update_t *
last_updates_cache::search_most_expired(u32 const current_time,
...
void server_updates_compressor::flush_accumulative_buffer() {
  ...
  if ((g_sv_traffic_optimization_level & eto_ppmd_compression) ||
    ...
    if (dst_packet->w_tell() + m_compress_buf.B.count + (sizeof(u16) * 2 + 1) <
