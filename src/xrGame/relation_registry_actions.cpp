...
void RELATION_REGISTRY::Action(CEntityAlive *from, CEntityAlive *to,
  ...
  //(с) мин. время через которое снова будет зарегестрировано сообщение об атаке
  //на персонажа
  static u32 min_attack_delta_time =
      u32(1000.f *
          pSettings->r_float(ACTIONS_POINTS_SECT, "min_attack_delta_time"));
  ...
  if (stalker) {
    ...
    relation =
        GetRelationType(smart_cast<CInventoryOwner *>(stalker), inv_owner_from);
  }
  ...
  case ATTACK: {
    ...
    if (stalker) {
      ...
      // change relation only for pairs actor->stalker, do not use pairs
      // stalker->stalker
