...
shared_str
CSpaceRestrictionHolder::normalize_string(shared_str space_restrictors) {
...
void CSpaceRestrictionHolder::register_restrictor(
  ...
  if (shape->border().empty()) {
    Msg("* [%s]: change restrictor_type of %s to eRestrictorTypeNone because "
        "border().empty()",
        __FUNCTION__, space_restrictor->cName().c_str());
    space_restrictor->change_restrictor_type(
