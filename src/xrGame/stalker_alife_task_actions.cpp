...
void CStalkerActionSolveZonePuzzle::execute() {
  ...
  VERIFY(weapon);
  //			Msg										("weapon %s
  //is strapped : %c",*weapon->cName(),weapon->strapped_mode() ? '+' : '-');
  ...
  if (!object().inventory().ActiveItem() ||
    ...
    if (weapon->strapped_mode()) {
      ...
      if (Device.dwTimeGlobal >= m_time_to_idle) {
        m_time_to_strap = 0;
        object().CObjectHandler::set_goal(
            eObjectActionIdle, object().inventory().ItemFromSlot(INV_SLOT_2));
      }
    }
  }
...
void CStalkerActionSmartTerrain::execute() {
  ...
  object().movement().set_nearest_accessible_position(task->position(),
                                                      task->level_vertex_id());
#else
  object().movement().set_desired_direction(0);
  object().movement().set_path_type(MovementManager::ePathTypeLevelPath);
  object().movement().set_detail_path_type(
