...
imotion_position::imotion_position()
    : interactive_motion(), time_to_end(0.f), saved_visual_callback(0),
      blend(0), shell_motion_has_history(false){

                };

static void interactive_motion_diag(LPCSTR message, const CBlend &b,
                                    CPhysicsShell *s, float time_left) {
#ifdef DEBUG
  if (!death_anim_debug)
    return;
...
static void get_depth(bool &do_colide, bool bo1, dContact &c,
...
float imotion_position::motion_collide(float dt, IKinematicsAnimated &KA) {
  ...
  if (time_to_end < (max_collide_timedelta + end_delta)) {
    interactive_motion_diagnostic(
        make_string("motion_collide 0: stoped: time out, time delta %f", dt)
            .c_str());
  ...
  if (depth > depth_resolve) {
...
#ifdef DEBUG
    if (dbg_imotion_collide_debug) {
      // interactive_motion_diagnostic( make_string( " motion_collide collided0:
      // deppth= %f", depth ).c_str() );
      interactive_motion_diagnostic(
    ...
    if (depth > depth0) {
      ...
      if (dbg_imotion_collide_debug) {
        interactive_motion_diagnostic(
            make_string(" motion_collide collided2: deppth= %f", depth)
                .c_str());
        DBG_OpenCashedDraw();
        shell->dbg_draw_geometry(0.02, D3DCOLOR_ARGB(255, 0, 255, 0));
        DBG_ClosedCashedDraw(50000);
    ...
    force_calculate_bones(KA);
    shell->ToAnimBonesPositions(shell_motion_has_history ? mh_clear
                                                         : mh_unspecified);
