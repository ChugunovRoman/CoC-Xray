...
void CPurchaseList::process(CInifile &ini_file, LPCSTR section,
  ...
  for (; I != E; ++I) {
    ...
    // VERIFY3				((*I).second.size(),"PurchaseList : cannot handle lines
    // in section without values",section);
    string256 temp0, temp1;
    // THROW3				(_GetItemCount(*(*I).second) == 2,"Invalid parameters
    // in section",section);
...
void CPurchaseList::process(const CGameObject &owner, const shared_str &name,
                            const u32 &count, const float &probability) {
  ...
  VERIFY3(I == m_deficits.end(), "Duplicate section in the purchase list",
          *name);
  m_deficits.insert(std::make_pair(
      name, (float)count * probability / _max((float)j, min_deficit_factor)));
}
