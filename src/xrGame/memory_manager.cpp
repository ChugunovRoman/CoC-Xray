...
void CMemoryManager::update(float time_delta) {
  ...
  if (m_stalker)
    registered_in_combat =
        m_stalker->agent_manager().member().registered_in_combat(m_stalker);
...
CMemoryInfo CMemoryManager::memory(const CObject *object) const {
  ...
  {
    xr_vector<CSoundObject>::const_iterator I = std::find(
        sound().objects().begin(), sound().objects().end(), object_id(object));
  ...
  {
    xr_vector<CHitObject>::const_iterator I = std::find(
        hit().objects().begin(), hit().objects().end(), object_id(object));
...
void CMemoryManager::make_object_visible_somewhen(const CEntityAlive *enemy) {
  ...
  VERIFY(obj1);
  //	if (obj1)
  //		Msg						("[%6d] make_object_visible_somewhen
  //[%s] = %x",Device.dwTimeGlobal,*enemy->cName(),obj1->m_squad_mask.get());
