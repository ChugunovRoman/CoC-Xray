...
#ifndef MASTER_GOLD
  bool B_GOOD = (!m_pInventory || (std::find(m_pInventory->m_all.begin(),
                                             m_pInventory->m_all.end(), this) ==
...
void CInventoryItem::Load(LPCSTR section) {
  CHitImmunity::LoadImmunities(pSettings->r_string(section, "immunities_sect"),
                               pSettings);
  ...
  m_nameShort =
      CStringTable().translate(pSettings->r_string(section, "inv_name_short"));
  ...
  m_Description =
      CStringTable().translate(pSettings->r_string(section, "description"));

  m_flags.set(Fbelt, READ_IF_EXISTS(pSettings, r_bool, section, "belt", FALSE));
  m_can_trade = READ_IF_EXISTS(pSettings, r_bool, section, "can_trade", TRUE);
  m_flags.set(FCanTake,
              READ_IF_EXISTS(pSettings, r_bool, section, "can_take", TRUE));
  m_flags.set(FCanTrade, m_can_trade);
  m_flags.set(FIsQuestItem,
              READ_IF_EXISTS(pSettings, r_bool, section, "quest_item", FALSE));
  m_flags.set(FCanStack,
              READ_IF_EXISTS(pSettings, r_bool, section, "can_stack", TRUE));
  // Added by Axel, to enable optional condition use on any item
  m_flags.set(FUsingCondition, READ_IF_EXISTS(pSettings, r_bool, section,
                                              "use_condition", FALSE));

  m_highlight_equipped =
      !!READ_IF_EXISTS(pSettings, r_bool, section, "highlight_equipped", FALSE);
...
BOOL CInventoryItem::net_Spawn(CSE_Abstract *DC) {
  ...
  if (alife_object) {
    m_flags.set(Fuseful_for_NPC,
                alife_object->m_flags.test(CSE_ALifeObject::flUsefulForAI));
  }
...
void CInventoryItem::net_Destroy() {
  if (m_pInventory) {
    VERIFY(std::find(m_pInventory->m_all.begin(), m_pInventory->m_all.end(),
...
void CInventoryItem::net_Import(NET_Packet &P) {
  ...
  = P.r_float_q8(0.f,10.f*PI_MUL_2); N.State.angular_vel.y	=
  P.r_float_q8(0.f,10.f*PI_MUL_2); N.State.angular_vel.z	=
  P.r_float_q8(0.f,10.f*PI_MUL_2);
  }
  else
...
void CInventoryItem::net_Export(NET_Packet &P) {
  ...
  if (g_actor && this->parent_id() ==
                     g_actor->ID()) // Optimization, as I can't think of very
                                    // many cases where we need update condition
                                    // change when item is not actor's
  {
...
void CInventoryItem::PH_I_CrPr() // actions & operations between two phisic
                                 // prediction steps
    {
        ...
        (p->RecalculatedState.quaternion, p->RecalculatedState.position,
        xformX);
...
float factor	= float(CurTime - p->m_dwIStartTime)/(p->m_dwIEndTime -
p->m_dwIStartTime); if (factor > 1.0f) factor = 1.0f;
...
pSyncObj->cv2obj_Xfrom(p->PredictedState.quaternion, p->PredictedState.position,
xformX1); P3.set(xformX1.c);
/////////////////////////////////////////////////////////////////////////////
Fvector TotalPath;
TotalPath.sub(P3, P0);
...
if(!object().H_Parent() && object().getVisible() && object().m_pPhysicsShell &&
m_flags.test(FInInterpolation) )
{
...
CPHSynchronize* pSyncObj		= NULL;
pSyncObj						=
object().PHGetSyncItem(0); pSyncObj->set_State
...
float factor	= float(CurTime - p->m_dwIStartTime)/(p->m_dwIEndTime -
p->m_dwIStartTime); if (factor > 1) factor = 1.0f; else if (factor < 0) factor =
0;
...
void CInventoryItem::Interpolate() {
  ...
  if (!object().H_Parent() && object().getVisible() &&
    ...
    if (p->NET_IItem.size() >= 2) {
      ...
      float ret_interpolate = interpolate_states(p->NET_IItem.front(),
                                                 p->NET_IItem.back(), newState);
...
void CInventoryItem::reload(LPCSTR section) {
  ...
  m_holder_fov_modifier =
      READ_IF_EXISTS(pSettings, r_float, section, "holder_fov_modifier", 1.f);
}
...
CInventoryItem::can_kill(const xr_vector<const CGameObject *> &items) const {
  return (0);
}

CInventoryItem *
CInventoryItem::can_make_killing(const CInventory *inventory) const {
  return (0);
}
...
void CInventoryItem::OnRender() {
  if (bDebug && object().Visual()) {
    ...
    {
    Level().debug_renderer().draw_aabb			(Position(), size, size,
    size, color_rgba(0, 255, 0, 255));
    ...
    {
    Pos2 = *It;
    Level().debug_renderer().draw_line(Fidentity, Pos1, Pos2, color_rgba(255,
    255, 0, 255)); Pos1 = Pos2;
    };
    ...
    Level().debug_renderer().draw_aabb			(IRecPos, size, size,
    size, color_rgba(255, 0, 255, 255));

    xformI.rotation(IEndRot);
    xformI.c.set(IEndPos);
    Level().debug_renderer().draw_obb			(xformI,bd,color_rgba(0,
    255, 0, 255));
    ...
    Level().debug_renderer().draw_line(Fidentity, point0, point1, color_rgba(0,
    0, 255, 255)); point0.set(point1);
    };
    };
    */
  };
...
bool CInventoryItem::NeedToDestroyObject() const { return false; }
...
Irect CInventoryItem::GetUpgrIconRect() const {
  ...
  w = READ_IF_EXISTS(pSettings, r_u32, m_object->cNameSect(), "upgr_icon_width",
                     0);
