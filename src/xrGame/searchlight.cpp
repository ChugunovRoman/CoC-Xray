...
BOOL CProjector::net_Spawn(CSE_Abstract *DC) {
  ...
  guid_bone =
      K->LL_BoneID(pUserData->r_string("projector_definition", "guide_bone"));
...
void CProjector::TurnOff() {
  ...
  smart_cast<IKinematics *>(Visual())->LL_SetBoneVisible(guid_bone, FALSE,
                                                         TRUE);
}
...
bool CProjector::bfAssignWatch(CScriptEntityAction *tpEntityAction) {
  ...
  bone_y.velocity =
      (fis_zero(time, EPS_L) ? l_tWatchAction.vel_bone_y : delta_pitch / time);

  return false == (l_tWatchAction.m_bCompleted =
