...
void CActor::OnEvent(NET_Packet &P, u16 type) {
  ...
  case GE_OWNERSHIP_TAKE: {
    ...
    //			R_ASSERT2( Obj, make_string("GE_OWNERSHIP_TAKE: Object not
    //found. object_id = [%d]", id).c_str() );
    VERIFY2(
        Obj,
        make_string("GE_OWNERSHIP_TAKE: Object not found. object_id = [%d]", id)
            .c_str());
    if (!Obj) {
  ...
  case GE_OWNERSHIP_REJECT: {
    ...
    //			R_ASSERT2( Obj, make_string("GE_OWNERSHIP_REJECT: Object not
    //found, id = %d", id).c_str() );
    VERIFY2(Obj,
            make_string("GE_OWNERSHIP_REJECT: Object not found, id = %d", id)
                .c_str());
    if (!Obj) {
    ...
    VERIFY(GO->H_Parent());
    if (!GO->H_Parent()) {
      Msg("! ERROR: Actor [%d][%s] tries to reject item [%d][%s] that has no "
          "parent",
    ...
    if (GO->H_Parent()->ID() != ID()) {
      ...
      Msg("! ERROR: Actor [%d][%s] tries to drop not own item [%d][%s], his "
          "parent is [%d][%s]",
    ...
    if (!Obj->getDestroy() &&
      ...
      // [12.11.07] Alexander Maniluk: extended GE_OWNERSHIP_REJECT packet for
      // drop item to selected position
  ...
  case GEG_PLAYER_ACTIVATEARTEFACT: {
    ...
    //			R_ASSERT2( Obj, make_string("GEG_PLAYER_ITEM_EAT(use): Object
    //not found. object_id = [%d]", id).c_str() );
    VERIFY2(
        Obj,
        make_string(
            "GEG_PLAYER_ITEM_EAT(use): Object not found. object_id = [%d]", id)
            .c_str());
    if (!Obj) {
      //				Msg                 ( "!
      // GEG_PLAYER_ITEM_EAT(use): Object not found. object_id = [%d]", id );
    ...
    // make_string("GEG_PLAYER_ITEM_EAT(use): Object is destroying. object_id =
    //[%d]", id).c_str() );
    VERIFY2(
        !Obj->getDestroy(),
        make_string(
            "GEG_PLAYER_ITEM_EAT(use): Object is destroying. object_id = [%d]",
            id)
            .c_str());
    if (Obj->getDestroy()) {
      //				Msg                                ( "!
      // GEG_PLAYER_ITEM_EAT(use): Object is destroying. object_id = [%d]", id
      // );
      break;
    }
    ...
    if (type == GEG_PLAYER_ACTIVATEARTEFACT) {
      ...
      VERIFY2(pArtefact, make_string("GEG_PLAYER_ACTIVATEARTEFACT: Artefact "
                                     "not found. artefact_id = [%d]",
                                     id)
                             .c_str());
      if (!pArtefact) {
        Msg("! GEG_PLAYER_ACTIVATEARTEFACT: Artefact not found. artefact_id = "
            "[%d]",
            id);
        break; // 1
      }
