...
struct SPHContactDBGDraw : public SPHDBGDrawAbsract {
  ...
  void render() {
    ...
    Level().debug_renderer().draw_aabb(
        pos, .01f, .01f, .01f, D3DCOLOR_XRGB(255 * is_cyl, 0, 255 * !is_cyl));
    Fvector dir;
    dir.set(norm);
    dir.mul(depth * 100.f);
    dir.add(pos);
    Level().debug_renderer().draw_line(
        Fidentity, pos, dir, D3DCOLOR_XRGB(255 * is_cyl, 0, 255 * !is_cyl));
  }
};
...
struct SPHDBGDrawTri : public SPHDBGDrawAbsract {
  ...
  SPHDBGDrawTri(const Fvector &v0, const Fvector &v1, const Fvector &v2, u32 ac,
                bool solid_) {
    v[0].set(v0);
    v[1].set(v1);
    v[2].set(v2);
...
template <> IC void rotate<0>(Fmatrix &m, float ang) { m.rotateX(ang); }
template <> IC void rotate<1>(Fmatrix &m, float ang) { m.rotateY(ang); }

template <> IC void rotate<2>(Fmatrix &m, float ang) { m.rotateZ(ang); }

template <int ax>
void DBG_DrawRotation(float ang0, float ang1, const Fmatrix &m,
                      const Fvector &l, float size, u32 ac, bool solid,
                      u32 tessel) {
  Fvector from;
  from.set(m.c);
  Fvector ln;
...
void DBG_DrawRotationX(const Fmatrix &m, float ang0, float ang1, float size,
                       u32 ac, bool solid, u32 tessel) {
  DBG_DrawRotation<0>(ang0, ang1, m, Fvector().set(0, 0, 1), size, ac, solid,
                      tessel);
}

void DBG_DrawRotationY(const Fmatrix &m, float ang0, float ang1, float size,
                       u32 ac, bool solid, u32 tessel) {
  DBG_DrawRotation<1>(ang0, ang1, m, Fvector().set(1, 0, 0), size, ac, solid,
                      tessel);
}

void DBG_DrawRotationZ(const Fmatrix &m, float ang0, float ang1, float size,
                       u32 ac, bool solid, u32 tessel) {
  DBG_DrawRotation<2>(ang0, ang1, m, Fvector().set(0, 1, 0), size, ac, solid,
                      tessel);
}
...
void DBG_DrawStatBeforeFrameStep() {
  if (ph_dbg_draw_mask.test(phDbgDrawObjectStatistics)) {
    ...
    update_obj_count = update_obj_count * 0.9f +
...
void CFunctionGraph::Init(type_function fun, float x0, float x1, int l, int t,
                          int w, int h, int points_num /*=500*/,
...
void DBG_AnimBlend(IKinematicsAnimated &ka, const CBlend &B) {
  ...
  if (dbg_track_obj_flags.test(dbg_track_obj_blends_time))
    DBG_OutText("time current: %f, time total: %f, frame %d ", B.timeCurrent,
                B.timeTotal, B.dwFrame);
...
class cphdebug_impl : public IPhDebugRender {
  ...
  void draw_tri(const Fvector &v0, const Fvector &v1, const Fvector &v2, u32 c,
                bool solid) {
...
class CPHDebugOutput : public IDebugOutput {
  ...
  // ang1, float size, u32 ac, bool solid = false, u32 tessel = 7 ) = 0; virtual
  // void DBG_DrawRotationY( const Fmatrix &m, float ang0, float ang1, float
  // size, u32 ac, bool solid = false, u32 tessel = 7 ) = 0; virtual	void
  // DBG_DrawRotationZ( const Fmatrix &m, float ang0, float ang1, float size,
  // u32 ac, bool solid = false, u32 tessel = 7 ) = 0;
  ...
  virtual u32 &dbg_tries_num() {
    ...
    //	make_string( "%s, _14_=%f \n", dump_string( make_string( "%s.i, ", name
    //).c_str(), form.i ).c_str( ) , form._14_ )	+ 	make_string(
