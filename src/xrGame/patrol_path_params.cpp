...
CPatrolPathParams::CPatrolPathParams(
    ...
    const PatrolPathManager::EPatrolRouteType tPatrolPathStop, bool bRandom,
    u32 index) {
  m_path_name = caPatrolPathToGo;
  m_path = ai().patrol_paths().path(m_path_name, true);

  THROW3(m_path, "CPatrolPathParams::CPatrolPathParams There is no patrol path",
         caPatrolPathToGo);
...
const Fvector &CPatrolPathParams::point(u32 index) const {
  ...
  if (!m_path->vertex(index)) {
    ai().script_engine().script_log(eLuaMessageTypeError,
                                    "Can't get information about patrol point "
                                    "number %d in the patrol way %s",
                                    index, *m_path_name);
