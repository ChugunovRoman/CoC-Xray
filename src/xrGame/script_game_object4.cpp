...
u32 CScriptGameObject::add_sound(LPCSTR prefix, u32 max_count, ESoundTypes type,
...
u32 CScriptGameObject::add_combat_sound(LPCSTR prefix, u32 max_count,
                                        ESoundTypes type, u32 priority,
  ...
  if (!stalker) {
    ...
    return (stalker->sound().add(prefix, max_count, type, priority, mask,
                                 internal_type, bone_name,
...
CSightParams CScriptGameObject::sight_params() {
  ...
  result.m_object =
      action.object_to_look() ? action.object_to_look()->lua_game_object() : 0;
