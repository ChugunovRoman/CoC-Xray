...
CUIAnimatedStatic::CUIAnimatedStatic()
    : m_uFrameCount(0), m_uAnimationDuration(0), m_uTimeElapsed(0),
      m_uAnimCols(0xffffffff), m_bCyclic(true), m_bParamsChanged(true),
      m_uFrameWidth(0), m_uFrameHeight(0), m_uCurFrame(0xffffffff),
      m_bPlaying(false), m_prevTime(0) {
  m_pos.set(0, 0);
  //.	ClipperOn();
}
...
void CUIAnimatedStatic::Update() {
  ...
  if (m_bParamsChanged && 0 != m_uFrameCount) {
    ...
    oneFrameDuration =
        iCeil(m_uAnimationDuration / static_cast<float>(m_uFrameCount));
...
void CUISleepStatic::Update() {
  ...
  if (end_pixel2 > 0) {
    ...
    m_UIStaticItem2.SetSize(
        Fvector2().set(iFloor(end_pixel2 * UI().get_current_kx()), 128));
    m_UIStaticItem2.SetPos(m_UIStaticItem.GetPosX() +
                               m_UIStaticItem.GetSize().x,
