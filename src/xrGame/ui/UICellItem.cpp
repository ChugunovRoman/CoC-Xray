...
void CUICellItem::Update() {
  ...
  if (CursorOverWindow()) {
    ...
    if (clientArea.in(cp))
      GetMessageTarget()->SendMessage(this, DRAG_DROP_ITEM_FOCUSED_UPDATE,
                                      NULL);
  }
...
void CUIDragItem::Init(const ui_shader &sh, const Frect &rect,
...
bool CUIDragItem::OnMouseAction(float x, float y, EUIMessages mouse_action) {
  if (mouse_action == WINDOW_LBUTTON_UP) {
    m_pParent->GetMessageTarget()->SendMessage(m_pParent, DRAG_DROP_ITEM_DROP,
                                               NULL);
    return true;
  }
  return false;
