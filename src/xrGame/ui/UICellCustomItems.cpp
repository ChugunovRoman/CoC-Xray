...
CUIInventoryCellItem::CUIInventoryCellItem(CInventoryItem *itm) {
  ...
  while (pSettings->line_exist(itm->m_section_id, field)) {
    ...
    offset.x = pSettings->r_float(itm->m_section_id,
                                  strconcat(sizeof(buf), buf,
                                            std::to_string(itrNum).c_str(),
                                            "icon_layer_x"));
    ...
    LPCSTR field_scale = strconcat(
        sizeof(buf), buf, std::to_string(itrNum).c_str(), "icon_layer_scale");
...
void CUIInventoryCellItem::CreateLayer(LPCSTR section, Fvector2 offset,
                                       float scale) {
...
CUIStatic *CUIInventoryCellItem::InitLayer(CUIStatic *s, LPCSTR section,
                                           Fvector2 addon_offset, bool b_rotate,
                                           float scale) {
...
void CUIInventoryCellItem::UpdateItemText() {
  const u32 helper_count = (u32)std::count_if(m_childs.begin(), m_childs.end(),
...
void CUIWeaponCellItem::OnAfterChild(CUIDragDropListEx *parent_list) {
  if (is_silencer() && GetIcon(eSilencer))
    InitAddon(GetIcon(eSilencer), *object()->GetSilencerName(),
              m_addon_offset[eSilencer], parent_list->GetVerticalPlacement());

  if (is_scope() && GetIcon(eScope))
    InitAddon(GetIcon(eScope), *object()->GetScopeName(),
              m_addon_offset[eScope], parent_list->GetVerticalPlacement());
...
void CUIWeaponCellItem::InitAddon(CUIStatic *s, LPCSTR section,
                                  Fvector2 addon_offset, bool b_rotate) {
...
bool CUIWeaponCellItem::EqualTo(CUICellItem *itm) {
  ...
  //	bool b_addons					= ( (object()->GetAddonsState()
  //== ci->object()->GetAddonsState()) );
  ...
  //	bool b_place					= ( (object()->m_eItemCurrPlace
  //== ci->object()->m_eItemCurrPlace) );
