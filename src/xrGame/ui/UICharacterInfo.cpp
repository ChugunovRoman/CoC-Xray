...
void CUICharacterInfo::InitCharacterInfo(Fvector2 pos, Fvector2 size,
                                         CUIXml *xml_doc) {
  ...
  */
  ...
  if (xml_doc->NavigateToNode("icon:deadbody", 0)) {
    m_deadbody_color =
        CUIXmlInit::GetColor(*xml_doc, "icon:deadbody", 0, m_deadbody_color);
  }
...
void CUICharacterInfo::Init_StrInfoItem(CUIXml &xml_doc, LPCSTR item_str,
                                        UIItemType type) {
...
void CUICharacterInfo::Init_IconInfoItem(CUIXml &xml_doc, LPCSTR item_str,
                                         UIItemType type) {
  if (xml_doc.NavigateToNode(item_str, 0)) {
    CUIStatic *pItem = m_icons[type] = xr_new<CUIStatic>();
    CUIXmlInit::InitStatic(xml_doc, item_str, 0, pItem);
...
void CUICharacterInfo::InitCharacter(u16 id) {
  ...
  /*
     ...
     community2 ); } return;
          ...
          if ( m_icons[eCommunityIcon   ]     ) { m_icons[eCommunityIcon]->Show(
     false ); } if ( m_icons[eCommunityBigIcon]     ) {
     m_icons[eCommunityBigIcon]->Show( false ); } if (
     m_icons[eCommunityIconOver   ] ) { m_icons[eCommunityIconOver]->Show( false
