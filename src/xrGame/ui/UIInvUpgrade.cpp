...
// -----------------------------------------------------------------------------------
void UIUpgrade::load_from_xml(CUIXml &ui_xml, int i_column, int i_cell,
...
void CUIUpgradePoint::load_from_xml(CUIXml &ui_xml, int i_cell) {
  ...
  SetWndPos(
      Fvector2().set(point_x * (UI().is_widescreen() ? 0.8f : 1.0f), point_y));
  SetWndSize(Fvector2().set(UI().is_widescreen() ? 11.0f : 14.0f, 14.0f));
  SetStretchTexture(true);
  Show(false);
}
bool CUIUpgradePoint::OnMouseAction(float x, float y,
