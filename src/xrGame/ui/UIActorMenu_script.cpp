...
CUIMainIngameWnd *GetMainGameMenu() { return CurrentGameUI()->UIMainIngameWnd; }
...
void CUIActorMenu::TryRepairItem(CUIWindow *w, void *d) {
  ...
  if (EItm) {
    bool allow_repair =
        !!READ_IF_EXISTS(pSettings, r_bool, item_name, "allow_repair", false);
    if (!allow_repair)
      return;
  }
  ...
  R_ASSERT2(
      ...
      make_string("Failed to get functor <inventory_upgrades.can_repair_item>, "
                  "item = %s",
                  item_name));
  ...
  R_ASSERT2(ai().script_engine().functor(
                "inventory_upgrades.question_repair_item", funct2),
            make_string("Failed to get functor "
                        "<inventory_upgrades.question_repair_item>, item = %s",
                        item_name));
  LPCSTR question =
      funct2(item_name, item->GetCondition(), can_repair, partner);
...
void CUIActorMenu::RepairEffect_CurItem() {
  ...
  R_ASSERT(ai().script_engine().functor("inventory_upgrades.effect_repair_item",
                                        funct));
...
bool CUIActorMenu::CanUpgradeItem(PIItem item) {
  ...
  luabind::functor<bool> funct;
  R_ASSERT2(ai().script_engine().functor("inventory_upgrades.can_upgrade_item",
                                         funct),
            make_string(
                "Failed to get functor <inventory_upgrades.can_upgrade_item>, "
                "item = %s, mechanic = %s",
                item_name, partner));
...
void CUIActorMenu::HighlightSectionInSlot(LPCSTR section, u8 type,
                                          u16 slot_id) {
...
void CUIActorMenu::HighlightForEachInSlot(const luabind::functor<bool> &functor,
                                          u8 type, u16 slot_id) {
  if (!functor)
    return;
  ...
  module(L)
      [class_<enum_exporter<EDDListType>>("EDDListType")
       ...
       class_<CUIActorMenu, CUIDialogWnd, CUIWndCallback>("CUIActorMenu")
           ...
           .def("UpdateConditionProgressBars",
                &CUIActorMenu::UpdateConditionProgressBars)
       ...
       class_<CUIMotionIcon, CUIWindow>("CUIMotionIcon").def(constructor<>()),
