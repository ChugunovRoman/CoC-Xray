...
void CUIScrollBar::InitScrollBar(Fvector2 pos, float length, bool bIsHorizontal,
                                 LPCSTR profile) {
  string256 _path;
  CUIXml xml_doc;
  xml_doc.Load(CONFIG_PATH, UI_PATH, "scroll_bar.xml");

  float height = xml_doc.ReadAttribFlt(profile, 0,
                                       (bIsHorizontal) ? "height" : "height_v");
  R_ASSERT(height > 0);
...
void CUIScrollBar::SetHeight(float height) {
  ...
  if (!m_bIsHorizontal) {
    float work_area =
        float(height) - m_DecButton->GetHeight() - m_IncButton->GetHeight();
    m_ScrollWorkArea = work_area < 0.f ? 0 : int(work_area);
  }
  UpdateScrollBar();
}
    ...
    if (IsRelevant()) {
      if (m_bIsHorizontal) {
        ...
        m_ScrollBox->SetWndPos(
            Fvector2().set(float(pos), m_ScrollBox->GetWndRect().top));
        m_IncButton->SetWndPos(
            Fvector2().set(GetWidth() - m_IncButton->GetWidth(), 0.0f));
      } else {
        // set height
        clamp(box_sz,
        ...
        int pos = PosViewFromScroll(iFloor(m_ScrollBox->GetHeight()),
                                    iFloor(GetWidth()));
        m_ScrollBox->SetWndPos(
            Fvector2().set(m_ScrollBox->GetWndRect().left, float(pos)));
        m_IncButton->SetWndPos(
...
void CUIScrollBar::ClampByViewRect() {
  if (m_bIsHorizontal) {
    if (m_ScrollBox->GetWndRect().left <= m_DecButton->GetWidth())
      m_ScrollBox->SetWndPos(Fvector2().set(m_DecButton->GetWidth(),
...
void CUIScrollBar::SetPosScrollFromView(float view_pos, float view_size,
                                        float view_offs) {
  int scroll_size = ScrollSize();
  float pos = view_pos - view_offs;
  float work_size = m_ScrollWorkArea - view_size;
  SetScrollPosClamped(
      work_size ? iFloor(((pos / work_size) * (scroll_size) + m_iMinPos)) : 0);
}
