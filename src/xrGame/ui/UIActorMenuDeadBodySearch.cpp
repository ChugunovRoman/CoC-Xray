...
bool move_item_check(PIItem itm, CInventoryOwner *from, CInventoryOwner *to,
                     bool weight_check) {
  if (weight_check) {
...
bool CUIActorMenu::ToDeadBodyBag(CUICellItem *itm, bool b_use_cursor_pos) {
    ...
    if (m_pPartnerInvOwner->is_alive()) {
      ...
      if (ai().script_engine().functor(
        ...
        if (funct(m_pPartnerInvOwner->cast_game_object()->lua_game_object(),
                  quest_item->object().lua_game_object(), 0, 0, itmWeight,
  ...
  } else // box
  {
    ...
    if (ai().script_engine().functor("_G.CInventoryBox_CanTake", funct)) {
      if (funct(m_pInvBox->cast_game_object()->lua_game_object(),
                quest_item->cast_game_object()->lua_game_object()) == false)
        return false;
    }
  }
  ...
  if (m_pPartnerInvOwner) {
    move_item_from_to(m_pActorInvOwner->object_id(),
                      m_pPartnerInvOwner->object_id(), iitem->object_id());
  } else // box
  {
