...
CUIPdaSpot::CUIPdaSpot() {
  ...
  m_spotID = u16(-1);
  m_spotType = READ_IF_EXISTS(pSettings, r_string, "user_spots", "spot_type",
                              "treasure");
...
void CUIPdaSpot::Init(u16 spot_id, LPCSTR level_name, Fvector pos,
                      bool main_wnd) {
  m_mainWnd = main_wnd;
  ...
  if (!m_mainWnd) {
    CMapLocation *ml =
        Level().MapManager().GetMapLocation(m_spotType, m_spotID);
    if (!ml)
      return;
...
void CUIPdaSpot::InitControls() {
  ...
  Register(m_btn_ok);
  AddCallback(m_btn_ok, BUTTON_CLICKED,
              CUIWndCallback::void_function(this, &CUIPdaSpot::OnApply));

  m_btn_cancel = UIHelper::Create3tButton(uiXml, "btn_cancel", this);
  Register(m_btn_cancel);
  AddCallback(m_btn_cancel, BUTTON_CLICKED,
              CUIWndCallback::void_function(this, &CUIPdaSpot::OnExit));
}

void CUIPdaSpot::OnAdd(CUIWindow *ui, void *d) {
  CMapLocation *ml =
      Level().MapManager().AddUserLocation(m_spotType, m_levelName, m_position);
