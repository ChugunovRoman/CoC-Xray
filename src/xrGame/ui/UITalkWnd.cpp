    ...
    if (m_pCurrentDialog->IsWeSpeaking(m_pOurDialogManager)) {
      ...
      if (m_pCurrentDialog && !m_pCurrentDialog->allIsDummy()) {
        ...
        for (PHRASE_VECTOR::const_iterator it =
          ...
          CPhrase *phrase = *it;
          AddQuestion(m_pCurrentDialog->GetPhraseText(phrase->GetID()),
...
void CUITalkWnd::AskQuestion() {
  ...
  if (TopicMode()) {
    if ((UITalkDialogWnd->m_ClickedQuestionID == "") ||
      ...
      string128 s;
      xr_sprintf(s,
                 "ID = [%s] of selected question is out of range of available "
                 "dialogs ",
                 UITalkDialogWnd->m_ClickedQuestionID);
      VERIFY2(FALSE, s);
    }
...
void CUITalkWnd::AddIconedMessage(LPCSTR caption, LPCSTR text,
