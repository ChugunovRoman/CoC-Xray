    ...
    Frect const &bound_rect) // meters->pixels (relatively own left-top pos)
{
  ...
  res.y =
      (bound_rect.height() - (src.y - bound_rect.lt.y)) * GetCurrentZoom().x;

  return res;
}

// position and heading for drawing pointer to src pos
bool CUICustomMap::GetPointerTo(const Fvector2 &src, float item_radius,
                                Fvector2 &pos, float &heading) {
...
float CUIGlobalMap::CalcOpenRect(const Fvector2 &center_point,
                                 Frect &map_desired_rect, float tgt_zoom) {
  ...
  // calculate center point in new zoom (center_point is in identity global map
  // space)
...
void CUILevelMap::Init_internal(const shared_str &name, CInifile &pLtx,
                                const shared_str &sect_name, LPCSTR sh_name) {
  inherited::Init_internal(name, pLtx, sect_name, sh_name);
...
void CUIMiniMap::Init_internal(const shared_str &name, CInifile &pLtx,
                               const shared_str &sect_name, LPCSTR sh_name) {
  inherited::Init_internal(name, pLtx, sect_name, sh_name);
...
void CUIMiniMap::Draw() {
  ...
  for (u32 idx = 0; idx < segments_count - 2; ++idx) {
    UIRender->PushPoint(S[0 + 0].pt.x, S[0 + 0].pt.y, 0, color, S[0 + 0].uv.x,
                        S[0 + 0].uv.y);
...
bool CUIMiniMap::GetPointerTo(const Fvector2 &src, float item_radius,
                              Fvector2 &pos, float &heading) {
