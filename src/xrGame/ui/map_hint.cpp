...
void CUIMapLocationHint::SetInfoTask(CGameTask *task) {
  ...
  if (b_rem) {
    ...
    InventoryUtilities::GetTimePeriodAsString(
        buff, sizeof(buff), Level().GetGameTime(), task->m_TimeToComplete);
  ...
  pos = S->GetWndPos();
  pos.y =
      m_info["t_time"]->GetWndPos().y + m_info["t_time"]->GetWndSize().y + 7;
  S->SetWndPos(pos);
