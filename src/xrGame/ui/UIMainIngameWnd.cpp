...
CUIMainIngameWnd::CUIMainIngameWnd()
    : /*m_pGrenade(NULL),m_pItem(NULL),*/ m_pPickUpItem(NULL),
      m_pMPChatWnd(NULL), UIArtefactIcon(NULL), m_pMPLogWnd(NULL) {
...
void CUIMainIngameWnd::Init() {
  ...
  //(&UIWeaponBack);
  //	xml_init.InitStatic			(uiXml, "static_weapon", 0,
  //&UIWeaponBack);
  ...
  */	//---------------------------------------------------------
  ...
  while (j < ewiInvincible) {
    ...
    shared_str cfgRecord =
        pSettings->r_string("main_ingame_indicators_thresholds",
...
void CUIMainIngameWnd::Draw() {
  ...
  if ((UIStaticDiskIO_start_time + 1.0f) < Device.fTimeGlobal)
    ...
    u32 alpha = clampr(
        iFloor(255.f *
               (1.f - (Device.fTimeGlobal - UIStaticDiskIO_start_time) / 1.f)),
        0, 255);
...
void CUIMainIngameWnd::UpdatePickUpItem() {
  ...
  UIPickUpItemIcon->SetWndPos(Fvector2().set(
      ...
      m_iPickUpItemIconY +
          (m_iPickUpItemIconHeight - UIPickUpItemIcon->GetHeight()) / 2.0f));
...
void CUIMainIngameWnd::UpdateMainIndicators() {
  ...
  // Helmet broken icon
  CHelmet *helmet =
      smart_cast<CHelmet *>(pActor->inventory().ItemFromSlot(HELMET_SLOT));
...
void CUIMainIngameWnd::UpdateQuickSlots() {
  ...
  for (u8 i = 0; i < 4; i++) {
    CUIStatic *wnd =
        smart_cast<CUIStatic *>(m_quick_slots_icons[i]->FindChild("counter"));
    if (wnd) {
      shared_str item_name = g_quick_use_slots[i];
      if (item_name.size()) {
        u32 count =
            pActor->inventory().dwfGetSameItemCount(item_name.c_str(), true);
        string32 str;
        ...
        Frect texture_rect;
        texture_rect.x1 =
            pSettings->r_float(item_name, "inv_grid_x") * INV_GRID_WIDTH;
        texture_rect.y1 =
            pSettings->r_float(item_name, "inv_grid_y") * INV_GRID_HEIGHT;
        texture_rect.x2 =
