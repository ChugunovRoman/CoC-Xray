...
void CUIMMShniaga::CreateList(xr_vector<CUITextWnd *> &lst, CUIXml &xml_doc,
                              LPCSTR path) {
  CGameFont *pF;
  u32 color;
...
void CUIMMShniaga::SetPage(enum_page_id page_id, LPCSTR xml_file,
                           LPCSTR xml_path) {
...
void CUIMMShniaga::ProcessEvent(EVENT ev) {
  ...
  case E_Begin: {
    ...
    m_run_time =
        u32((log(1 + abs(m_origin - m_destination)) / log(GetHeight())) * 300);
    if (m_run_time < 100)
      m_run_time = 100;
