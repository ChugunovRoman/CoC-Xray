...
bool CUIXmlInit::InitWindow(CUIXml &xml_doc, LPCSTR path, int index,
                            CUIWindow *pWnd) {
  R_ASSERT4(xml_doc.NavigateToNode(path, index), "XML node not found", path,
            xml_doc.m_xml_file_name);
...
bool CUIXmlInit::InitFrameWindow(CUIXml &xml_doc, LPCSTR path, int index,
                                 CUIFrameWindow *pWnd) {
  R_ASSERT4(xml_doc.NavigateToNode(path, index), "XML node not found", path,
            xml_doc.m_xml_file_name);
...
bool CUIXmlInit::InitOptionsItem(CUIXml &xml_doc, LPCSTR path, int index,
                                 CUIOptionsItem *pWnd) {
  string256 buf;
...
bool CUIXmlInit::InitStatic(CUIXml &xml_doc, LPCSTR path, int index,
                            CUIStatic *pWnd) {
  R_ASSERT4(xml_doc.NavigateToNode(path, index), "XML node not found", path,
            xml_doc.m_xml_file_name);
...
bool CUIXmlInit::InitTextWnd(CUIXml &xml_doc, LPCSTR path, int index,
                             CUITextWnd *pWnd) {
  R_ASSERT4(xml_doc.NavigateToNode(path, index), "XML node not found", path,
            xml_doc.m_xml_file_name);
  ...
  R_ASSERT3(NULL == xml_doc.NavigateToNode(buf, index), xml_doc.m_xml_file_name,
            buf);
...
bool CUIXmlInit::InitCheck(CUIXml &xml_doc, LPCSTR path, int index,
                           CUICheckButton *pWnd) {
...
bool CUIXmlInit::InitSpin(CUIXml &xml_doc, LPCSTR path, int index,
                          CUICustomSpin *pWnd) {
  InitWindow(xml_doc, path, index, pWnd);
  InitOptionsItem(xml_doc, path, index, pWnd);
...
bool CUIXmlInit::InitText(CUIXml &xml_doc, LPCSTR path, int index,
                          CUIStatic *pWnd) {
...
bool CUIXmlInit::Init3tButton(CUIXml &xml_doc, LPCSTR path, int index,
                              CUI3tButton *pWnd) {
  R_ASSERT4(xml_doc.NavigateToNode(path, index), "XML node not found", path,
            xml_doc.m_xml_file_name);
  ...
  pWnd->vertical =
      (xml_doc.ReadAttribInt(path, index, "vertical", 0) == 1) ? true : false;
...
bool CUIXmlInit::InitTabButtonMP(CUIXml &xml_doc, LPCSTR path, int index,
                                 CUITabButtonMP *pWnd) {
...
bool CUIXmlInit::InitSound(CUIXml &xml_doc, LPCSTR path, int index,
                           CUI3tButton *pWnd) {
  string256 sound_h;
  string256 sound_t;
...
bool CUIXmlInit::InitProgressBar(CUIXml &xml_doc, LPCSTR path, int index,
                                 CUIProgressBar *pWnd) {
  R_ASSERT4(xml_doc.NavigateToNode(path, index), "XML node not found", path,
            xml_doc.m_xml_file_name);
...
bool CUIXmlInit::InitProgressShape(CUIXml &xml_doc, LPCSTR path, int index,
  ...
  if (xml_doc.NavigateToNode(strconcat(sizeof(_path), _path, path, ":back"),
                             index)) {
    R_ASSERT2(0, "unused <back> node in progress shape ");
  }

  if (xml_doc.NavigateToNode(strconcat(sizeof(_path), _path, path, ":front"),
                             index)) {
  ...
  pWnd->m_blend =
      (xml_doc.ReadAttribInt(path, index, "blend", 1) == 1) ? true : false;
...
void CUIXmlInit::InitAutoStaticGroup(CUIXml &xml_doc, LPCSTR path, int index,
  ...
  InitStatic						(xml_doc, "auto_static",
  i, pUIStatic); xr_sprintf
  (sname,"auto_static_%d", i); pUIStatic->SetWindowName		(sname);
  pUIStatic->SetAutoDelete (true); pParentWnd->AttachChild
  (pUIStatic); pUIStatic = NULL;
  }
  */
...
bool CUIXmlInit::InitFont(CUIXml &xml_doc, LPCSTR path, int index, u32 &color,
                          CGameFont *&pFnt) {
...
bool CUIXmlInit::InitTabControl(CUIXml &xml_doc, LPCSTR path, int index,
                                CUITabControl *pWnd) {
  R_ASSERT4(xml_doc.NavigateToNode(path, index), "XML node not found", path,
            xml_doc.m_xml_file_name);
...
bool CUIXmlInit::InitFrameLine(CUIXml &xml_doc, LPCSTR path, int index,
...
bool CUIXmlInit::InitCustomEdit(CUIXml &xml_doc, LPCSTR path, int index,
                                CUICustomEdit *pWnd) {
  InitStatic(xml_doc, path, index, pWnd);
...
bool CUIXmlInit::InitEditBox(CUIXml &xml_doc, LPCSTR path, int index,
                             CUIEditBox *pWnd) {
...
bool CUIXmlInit::InitAnimatedStatic(CUIXml &xml_doc, const char *path,
  ...
  u32 animDuration =
      static_cast<u32>(xml_doc.ReadAttribInt(path, index, "duration", 0));
  u32 animCols =
      static_cast<u32>(xml_doc.ReadAttribInt(path, index, "columns", 0));
...
bool CUIXmlInit::InitTexture(CUIXml &xml_doc, LPCSTR path, int index,
                             ITextureOwner *pWnd) {
...
bool CUIXmlInit::InitTextureOffset(CUIXml &xml_doc, LPCSTR path, int index,
                                   CUIStatic *pWnd) {
...
bool CUIXmlInit::InitMultiTexture(CUIXml &xml_doc, LPCSTR path, int index,
                                  CUI3tButton *pWnd) {
  string256 buff;
  bool success = false;
...
bool CUIXmlInit::InitAlignment(CUIXml &xml_doc, const char *path, int index,
                               float &x, float &y, CUIWindow *pWnd) {
...
bool CUIXmlInit::InitScrollView(CUIXml &xml_doc, LPCSTR path, int index,
                                CUIScrollView *pWnd) {
  R_ASSERT4(xml_doc.NavigateToNode(path, index), "XML node not found", path,
            xml_doc.m_xml_file_name);
...
bool CUIXmlInit::InitListBox(CUIXml &xml_doc, LPCSTR path, int index,
                             CUIListBox *pWnd) {
...
bool CUIXmlInit::InitTrackBar(CUIXml &xml_doc, LPCSTR path, int index,
                              CUITrackBar *pWnd) {
...
bool CUIXmlInit::InitComboBox(CUIXml &xml_doc, LPCSTR path, int index,
                              CUIComboBox *pWnd) {
  u32 color;
  CGameFont *pFont;
