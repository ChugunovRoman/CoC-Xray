...
bool FreeRoom_inBelt(TIItemContainer &item_list, PIItem item, int width,
                     int height);
...
LPCSTR GetTimePeriodAsString(LPSTR _buff, u32 buff_sz, ALife::_TIME_ID _from,
                             ALife::_TIME_ID _to);
// Отобразить вес, который несет (*pInvOwner)
void UpdateWeightStr(CUITextWnd &wnd, CUITextWnd &wnd_max,
