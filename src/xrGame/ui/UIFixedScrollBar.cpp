...
void CUIFixedScrollBar::InitScrollBar(Fvector2 pos, bool horizontal,
                                      LPCSTR profile) {
  string256 _path;
  CUIXml xml_doc;
...
bool CUIFixedScrollBar::OnMouseAction(float x, float y,
  ...
  case WINDOW_MOUSE_MOVE: {
    ...
    // ); bool over_y = ( y >= -512.0f && y < (m_ScrollBox->GetHeight() +
    // 512.0f)
    // ); if ( over_x && over_y ) 	cursor_over = true;
...
void CUIFixedScrollBar::SetPosScrollFromView(float view_pos, float view_size,
                                             float view_offs) {
