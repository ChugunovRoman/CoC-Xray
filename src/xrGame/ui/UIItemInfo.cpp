...
void CUIItemInfo::InitItemInfo(LPCSTR xml_name) {
  ...
  if (uiXml.NavigateToNode("descr_list", 0)) {
    ...
    // xml_init.InitStatic				(uiXml, "description_line",
    // 0, UIDesc_line);
...
void CUIItemInfo::InitItem(CUICellItem *pCellItem, CInventoryItem *pCompareItem,
                           u32 item_price, LPCSTR trade_tip) {
  if (!pCellItem) {
    m_pInvItem = NULL;
    Enable(false);
    return;
  ...
  if (UIDesc) {
    ...
    if (m_b_FitToHeight) {
      UIDesc->SetWndSize(
          Fvector2().set(UIDesc->GetWndSize().x, UIDesc->GetPadSize().y));
      Fvector2 new_size;
...
void CUIItemInfo::TryAddConditionInfo(CInventoryItem &pInvItem,
  ...
  if (ai().script_engine().functor("actor_menu.inventory_hint_init_item",
                                   funct)) {
    CUIWindow *ret =
        funct(pInvItem.cast_game_object()->lua_game_object(),
              pCompareItem ? pCompareItem->cast_game_object()->lua_game_object()
                           : (0));
    if (ret)
