...
u32 CUIActorMenu::CalcItemsPrice(CUIDragDropListEx *pList, CTrade *pTrade,
                                 bool bBuying) {
  u32 res = 0;
  for (u32 i = 0; i < pList->ItemsCount(); ++i) {
    CUICellItem *itm = pList->GetItemIdx(i);
...
bool CUIActorMenu::CanMoveToPartner(PIItem pItem) {
  ...
  if (ai().script_engine().functor(
          "actor_menu_inventory.CUIActorMenu_CanMoveToPartner", funct)) {
    if (funct(m_pPartnerInvOwner->cast_game_object()->lua_game_object(),
              pItem->object().lua_game_object(), r1, r2, itmWeight,
              partner_inv_weight, partner_max_weight) == false)
      return false;
  }
  //-Alundaio
...
void CUIActorMenu::OnBtnPerformTradeBuy(CUIWindow *w, void *d) {
  ...
  if ((actor_money >= 0) /*&& ( partner_money >= 0 )*/ &&
    ...
    TransferItems(m_pTradePartnerList, m_pTradeActorBagList, m_partner_trade,
                  false);
  } else {
    if (actor_money < 0) {
...
void CUIActorMenu::TransferItems(CUIDragDropListEx *pSellList,
                                 CUIDragDropListEx *pBuyList, CTrade *pTrade,
                                 bool bBuying) {
  while (pSellList->ItemsCount()) {
    ...
    if (bBuying) {
      SInvItemPlace pl;
      pl.type = eItemPlaceRuck;
      if (pTrade->pThis.inv_owner->CInventoryOwner::AllowItemToTrade(item,
                                                                     pl)) {
  ...
  pTrade->pThis.inv_owner->set_money(pTrade->pThis.inv_owner->get_money(),
                                     true);
  pTrade->pPartner.inv_owner->set_money(pTrade->pPartner.inv_owner->get_money(),
                                        true);
}
