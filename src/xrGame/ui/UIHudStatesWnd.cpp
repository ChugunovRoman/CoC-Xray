...
CUIHudStatesWnd::CUIHudStatesWnd()
    : m_b_force_update(true), m_timer_1sec(0), m_last_health(0.0f),
      m_radia_self(0.0f), m_radia_hit(0.0f) {
...
ALife::EInfluenceType
CUIHudStatesWnd::get_indik_type(ALife::EHitType hit_type) {
...
void CUIHudStatesWnd::InitFromXml(CUIXml &xml, LPCSTR path) {
  ...
  */
  ...
  m_ui_weapon_ammo_color_active = CUIXmlInit::GetColor(
      xml, "active_ammo_color", 0, color_rgba(238, 155, 23, 255));
  m_ui_weapon_ammo_color_inactive = CUIXmlInit::GetColor(
      xml, "inactive_ammo_color", 0, color_rgba(238, 155, 23, 150));
...
void CUIHudStatesWnd::Load_section() {
  ...
  Load_section_type(ALife::infl_electra, "electra_zone_detector"); // no
                                                                   // uistatic
}
  ...
  */
  ...
  for (; itb != ite; ++itb) {
    ...
    */
    ...
    if (dist_to_zone < rad_zone + 0.9f * ((z_type == ALife::infl_max_count)
                                              ? 5.0f
    ...
    if (zone_info.snd_time > zone_info.cur_period) {
      ...
      HUD_SOUND_ITEM::PlaySound(zone_type->detect_snds, Fvector().set(0, 0, 0),
                                NULL, true, false);
    } else {
...
void CUIHudStatesWnd::UpdateIndicatorType(CActor *actor,
  ...
  */
  ...
  CCustomOutfit *outfit = actor->GetOutfit();
  CHelmet *helmet =
      smart_cast<CHelmet *>(actor->inventory().ItemFromSlot(HELMET_SLOT));
  ...
  actor->conditions().SetZoneDanger(
      (hit_power - protect) / actor->conditions().GetZoneMaxPower(hit_type),
      type);
}
/*
