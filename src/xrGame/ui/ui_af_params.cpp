...
LPCSTR af_immunity_section_names[] = // ALife::EInfluenceType
    {
        ...
        "explosion_immunity",     "strike_immunity",
};
...
void CUIArtefactParams::InitFromXml(CUIXml &xml) {
...

  {
    ...
    LPCSTR name =
        CStringTable().translate("ui_inv_af_overweight_walk_accel").c_str();
...
void CUIArtefactParams::SetInfo(CArtefact *pInvItem) {
  ...
  for (u32 i = 0; i < 9; ++i) {
    ...
    val = READ_IF_EXISTS(pSettings, r_float, sect, af_immunity_section_names[i],
                         0.f);
    if (fis_zero(val)) {
      continue;
    }
...
void CUIArtefactParams::SetInfo(CHelmet *pInvItem) { DetachAll(); }
