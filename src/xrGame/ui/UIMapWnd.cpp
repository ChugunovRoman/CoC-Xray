...
void CUIMapWnd::Init(LPCSTR xml_name, LPCSTR start_from) {
  ...
  if (m_scroll_mode) {
    ...
    m_UIMainScrollH->InitScrollBar(Fvector2().set(r.left + dx, r.bottom - sy),
                                   true);
    ...
    m_UIMainScrollV->InitScrollBar(Fvector2().set(r.right - sx, r.top + dy),
                                   false);
  ...
  for (; it != m_GameMaps.end(); ++it) {
    ...
    for (it2 = it; it2 != m_GameMaps.end(); ++it2) {
      ...
      if (l->GlobalRect().intersected(l2->GlobalRect())) {
        Msg(" --error-incorrect map definition global rect of map [%s] "
            "intersects with [%s]",
            *l->MapName(), *l2->MapName());
      }
    }
    if (FALSE == l->GlobalRect().intersected(GlobalMap()->BoundRect())) {
      Msg(" --error-incorrect map definition map [%s] places outside global "
          "map",
          *l->MapName());
    }
  }
...
void CUIMapWnd::SendMessage(CUIWindow *pWnd, s16 msg, void *pData) {
  ...
  if (pWnd == m_UIPropertiesBox && msg == PROPERTY_CLICKED &&
    ...
    {
      ShowSettingsWindow(m_cur_location->ObjectID(),
                         m_cur_location->GetLastPosition(),
                         m_cur_location->GetLevelName());
      m_cur_location = NULL;
      break;
    }
...
void CUIMapWnd::ActivatePropertiesBox(CUIWindow *w) {
  ...
  if (ai().script_engine().functor("pda.property_box_add_properties", funct)) {
    funct(m_UIPropertiesBox, m_cur_location->ObjectID(),
          (LPCSTR)m_cur_location->GetLevelName().c_str(), m_cur_location);
  }

  // Только для меток игрока
  if (m_cur_location->IsUserDefined()) {
    m_UIPropertiesBox->AddItem(
        "st_pda_change_spot_hint", NULL,
        MAP_CHANGE_SPOT_HINT_ACT); // Изменяем название метки
    m_UIPropertiesBox->AddItem("st_pda_delete_spot", NULL,
                               MAP_REMOVE_SPOT_ACT); // Удаляем метку
  }
...
// Alun: Correct now. All you need is relative mouse position to absolute pos of
// uilevelmap, then remove widescreen scale on X before local-to-world convert
bool CUIMapWnd::ConvertCursorPosToMap(Fvector *return_position,
                                      CUILevelMap *curr_map) {
  ...
  return_position->x =
      bound_rect.lt.x + cursor_pos.x / (box_rect.width() / bound_rect.width());
  return_position->y = 0.f;
  return_position->z = bound_rect.height() + bound_rect.lt.y -
                       cursor_pos.y / (box_rect.height() / bound_rect.height());
...
CMapLocation *CUIMapWnd::UnderSpot(Fvector RealPosition,
  ...
  Fvector2 m_position_mouse =
      curr_map->ConvertRealToLocal(RealPositionXZ, false);
  ...
  for (it = Spots.begin(); it != Spots.end(); ++it) {
    if ((*it).location->IsUserDefined()) {
      m_position_on_map =
          curr_map->ConvertRealToLocal((*it).location->CalcPosition(), false);
