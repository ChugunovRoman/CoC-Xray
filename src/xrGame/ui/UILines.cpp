...
LPCSTR GetElipsisText(CGameFont *pFont, float width, LPCSTR source_text,
                      LPSTR buff, int buff_len) {
  ...
  if (!uFlags.is(flComplexMode)) {
    ...
    if (uFlags.test(flPasswordMode)) {
      ...
      if (uFlags.test(flEllipsis)) {
        ...
        LPCSTR str =
            GetElipsisText(m_pFont, m_wndSize.x, m_text.c_str(), p, buff_len);
...
void CUILines::CutFirstColoredTextEntry(xr_string &entry, u32 &color,
