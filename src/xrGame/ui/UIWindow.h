...
public:
  template <class _Other> struct rebind { typedef uialloc<_Other> other; };
...
public:
  ...
  LPCSTR			GetHintText			();
  void			SetHintWnd			(UIHint* hintWnd) {
  m_pHint = hintWnd; }; UIHint*			GetHintWnd
  () { return m_pHint; };
  */
  //-Alundaio
...
bool fit_in_rect(CUIWindow *w, Frect const &vis_rect, float border = 0.0f,
                 float dx16pos = 0.0f);
