...
bool CUIActorMenu::TryActiveSlot(CUICellItem *itm) {
  ...
  if (slot == GRENADE_SLOT) {
    ...
    if (prev_iitem &&
        (prev_iitem->object().cNameSect() != iitem->object().cNameSect())) {
...
bool CUIActorMenu::ToSlotScript(CScriptGameObject *GO, bool force_place,
                                u16 slot_id) {
...
bool CUIActorMenu::ToSlot(CUICellItem *itm, bool force_place, u16 slot_id) {
  ...
  if (m_pActorInvOwner->inventory().CanPutInSlot(iitem, slot_id)) {
    ...
    bool result =
        (!b_own_item) || m_pActorInvOwner->inventory().Slot(slot_id, iitem);
    VERIFY(result);
    ...
    if (slot_list != GetListByType(iActorBag)) {
      ...
      // Alundaio: Since the player's inventory is being used as a slot we need
      // to search for cell with matching m_pData
...
bool CUIActorMenu::ToBag(CUICellItem *itm, bool b_use_cursor_pos) {
  ...
  if (m_pActorInvOwner->inventory().CanPutInRuck(iitem) ||
    ...
    bool result =
        b_already || (!b_own_item || m_pActorInvOwner->inventory().Ruck(iitem));
    VERIFY(result);
    CUICellItem *i = old_owner->RemoveItem(itm, (old_owner == new_owner));
    if (!i)
      return false;
...
void CUIActorMenu::ActivatePropertiesBox() {
  ...
  if (!(m_currMenuMode == mmInventory || m_currMenuMode == mmDeadBodySearch ||
        m_currMenuMode == mmUpgrade || m_currMenuMode == mmTrade)) {
    return;
  }
...
void CUIActorMenu::PropertiesBoxForSlots(PIItem item, bool &b_show) {
  ...
  if (!pOutfit && !pHelmet && cur_slot != NO_ACTIVE_SLOT &&
      ...
      m_pActorInvOwner->inventory().ItemFromSlot(cur_slot) !=
          item /*&& inv.CanPutInSlot(item, cur_slot)*/) {
...
void CUIActorMenu::PropertiesBoxForWeapon(CUICellItem *cell_item, PIItem item,
                                          bool &b_show) {
  ...
  if (pGrenadeLauncher) {
    if (item_in_slot_2 && item_in_slot_2->CanAttach(pGrenadeLauncher)) {
      ...
      m_UIPropertiesBox->AddItem(str.c_str(), (void *)item_in_slot_2,
                                 INVENTORY_ATTACH_ADDON);
      //			m_UIPropertiesBox->AddItem(
      //"st_attach_gl_to_pistol",  (void*)item_in_slot_2, INVENTORY_ATTACH_ADDON
      //);
      b_show = true;
    }
    if (item_in_slot_3 && item_in_slot_3->CanAttach(pGrenadeLauncher)) {
      ...
      m_UIPropertiesBox->AddItem(str.c_str(), (void *)item_in_slot_3,
                                 INVENTORY_ATTACH_ADDON);
      //			m_UIPropertiesBox->AddItem(
      //"st_attach_gl_to_rifle",  (void*)item_in_slot_3, INVENTORY_ATTACH_ADDON
      //);
      b_show = true;
    }
  }
...
void CUIActorMenu::PropertiesBoxForUsing(PIItem item, bool &b_show) {
  ...
  act_str =
      READ_IF_EXISTS(pSettings, r_string, section_name, "default_use_text", 0);
  if (act_str) {
...
void CUIActorMenu::PropertiesBoxForDrop(CUICellItem *cell_item, PIItem item,
                                        bool &b_show) {
  if (!item->IsQuestItem()) {
...
void CUIActorMenu::ProcessPropertiesBoxClicked(CUIWindow *w, void *d) {
  ...
  case INVENTORY_ATTACH_ADDON: {
    PIItem item = CurrentIItem(); // temporary storing because of AttachAddon is
  ...
  case INVENTORY_UNLOAD_MAGAZINE: {
    CWeaponMagazined *weap_mag =
        smart_cast<CWeaponMagazined *>((CWeapon *)cell_item->m_pData);
    if (!weap_mag) {
      break;
    }
