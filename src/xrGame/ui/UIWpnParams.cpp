...
SLuaWpnParams::SLuaWpnParams() {
  ...
  functor_exists =
      ai().script_engine().functor("ui_wpn_params.GetDamage", m_functorDamage);
  VERIFY(functor_exists);
...
void CUIWpnParams::InitFromXml(CUIXml &xml_doc) {
  ...
  CUIXmlInit::InitTextWnd(xml_doc, "wpn_params:cap_ammo_used_type", 0,
                          &m_textAmmoUsedType);
  CUIXmlInit::InitStatic(xml_doc, "wpn_params:static_ammo_type1", 0,
                         &m_stAmmoType1);
  CUIXmlInit::InitStatic(xml_doc, "wpn_params:static_ammo_type2", 0,
                         &m_stAmmoType2);
  ...
  CUIXmlInit::InitTextWnd(xml_doc, "wpn_params:cap_accuracy_inc_shadow", 0,
                          &m_textAccuracy_inc_shadow);
  CUIXmlInit::InitTextWnd(xml_doc, "wpn_params:cap_damage_inc_shadow", 0,
                          &m_textDamage_inc_shadow);
  CUIXmlInit::InitTextWnd(xml_doc, "wpn_params:cap_handling_inc_shadow", 0,
                          &m_textHandling_inc_shadow);
  CUIXmlInit::InitTextWnd(xml_doc, "wpn_params:cap_rpm_inc_shadow", 0,
                          &m_textRPM_inc_shadow);
}
...
void CUIWpnParams::SetInfo(CInventoryItem *slot_wpn, CInventoryItem &cur_wpn) {
  ...
  float cur_rpm =
      ...
      34.0f;
  float cur_accur =
      iFloor(g_lua_wpn_params->m_functorAccuracy(cur_section, str_upgrades) *
             34.0f) /
      34.0f;
  float cur_hand =
      iFloor(g_lua_wpn_params->m_functorHandling(cur_section, str_upgrades) *
             34.0f) /
      34.0f;
  float cur_damage =
  ...
  if (slot_wpn && (slot_wpn != &cur_wpn)) {
    ...
    slot_rpm =
        iFloor(g_lua_wpn_params->m_functorRPM(slot_section, str_upgrades) *
               34.0f) /
        34.0f;
    slot_accur =
        iFloor(g_lua_wpn_params->m_functorAccuracy(slot_section, str_upgrades) *
               34.0f) /
        34.0f;
    slot_hand =
        iFloor(g_lua_wpn_params->m_functorHandling(slot_section, str_upgrades) *
               34.0f) /
        34.0f;
    slot_damage =
  ...
  Frect tex_rect;
  tex_rect.x1 = float(pSettings->r_u32(ammo_types[0].c_str(), "inv_grid_x") *
                      INV_GRID_WIDTH);
  tex_rect.y1 = float(pSettings->r_u32(ammo_types[0].c_str(), "inv_grid_y") *
                      INV_GRID_HEIGHT);
  tex_rect.x2 =
      float(pSettings->r_u32(ammo_types[0].c_str(), "inv_grid_width") *
            INV_GRID_WIDTH);
  tex_rect.y2 =
      float(pSettings->r_u32(ammo_types[0].c_str(), "inv_grid_height") *
            INV_GRID_HEIGHT);
  ...
  m_stAmmoType1.SetWndSize(
      Fvector2().set((tex_rect.x2 - tex_rect.x1) * UI().get_current_kx(),
  ...
  if (ammo_types.size() == 1) {
    ...
    tex_rect.x1 = float(pSettings->r_u32(ammo_types[1].c_str(), "inv_grid_x") *
                        INV_GRID_WIDTH);
    tex_rect.y1 = float(pSettings->r_u32(ammo_types[1].c_str(), "inv_grid_y") *
                        INV_GRID_HEIGHT);
    tex_rect.x2 =
        float(pSettings->r_u32(ammo_types[1].c_str(), "inv_grid_width") *
              INV_GRID_WIDTH);
    tex_rect.y2 =
        float(pSettings->r_u32(ammo_types[1].c_str(), "inv_grid_height") *
              INV_GRID_HEIGHT);
    tex_rect.rb.add(tex_rect.lt);
  }
