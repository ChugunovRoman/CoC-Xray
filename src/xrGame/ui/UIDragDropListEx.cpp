...
CUIDragDropListEx::CUIDragDropListEx() {
  ...
  AddCallbackStr(
      "cell_item", DRAG_DROP_ITEM_SELECTED,
  ...
  AddCallbackStr(
      "cell_item", DRAG_DROP_ITEM_DB_CLICK,
...
void CUIDragDropListEx::OnScrollV(CUIWindow *w, void *pData) {
  m_container->SetWndPos(Fvector2().set(m_container->GetWndPos().x,
                                        float(-m_vScrollBar->GetScrollPos())));
}
...
Fvector2 CUIDragDropListEx::GetDragItemPosition() { // Alun: More accurate then
                                                    // Left-Top of dragged icon
...
void CUIDragDropListEx::OnItemRButtonClick(CUIWindow *w, void *pData) {
  //*	OnItemSelected						(w, pData); // instead
  //call function "SetCurrentItem(itm)";
  CUICellItem *itm = smart_cast<CUICellItem *>(w);
...
bool CUIDragDropListEx::OnMouseAction(float x, float y,
...
CUICellItem *CUICellContainer::FindSimilar(CUICellItem *itm) {
  for (WINDOW_LIST_it it = m_ChildWndList.begin(); m_ChildWndList.end() != it;
       ++it) {
#ifdef DEBUG
...
void CUICellContainer::PlaceItemAtPos(CUICellItem *itm, Ivector2 &cell_pos) {
  ...
  if (!m_pParentDragDropList->GetVirtualCells())
    itm->SetWndPos(
        Fvector2().set(((m_cellSpacing.x + m_cellSize.x) * cell_pos.x),
...
Ivector2 CUICellContainer::TopVisibleCell() {
  return Ivector2().set(0, iFloor(m_pParentDragDropList->ScrollPos() /
...
Ivector2 CUICellContainer::PickCell(const Fvector2 &abs_pos) {
  ...
  ap.mul(-1);
  res.x =
      iFloor(ap.x / (m_cellSize.x + m_cellSpacing.x * (m_cellsCapacity.x - 1) /
                                        m_cellsCapacity.x));
  res.y =
      iFloor(ap.y / (m_cellSize.y + m_cellSpacing.y * (m_cellsCapacity.y - 1) /
                                        m_cellsCapacity.y));
  if (!ValidCell(res))
    res.set(-1, -1);
  return res;
...
void CUICellContainer::Draw() {
  ...
  tgt_cells.x2 = iFloor((float(clientArea.width()) + float(cell_sz.x) - EPS) /
                        float(cell_sz.x)) +
                 tgt_cells.lt.x;
  tgt_cells.y2 = iFloor((float(clientArea.height()) + float(cell_sz.y) - EPS) /
                        float(cell_sz.y)) +
                 tgt_cells.lt.y;
