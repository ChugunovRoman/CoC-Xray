...
private:
  ...
  void PropertiesBoxForWeapon(CUICellItem *cell_item, PIItem item,
                              bool &b_show);
...
protected:
  ...
  bool OnItemDropped(PIItem itm, CUIDragDropListEx *new_owner,
  ...
  // void						SetCurrentItem				(CUICellItem*
  // itm);
  ...
  void TransferItems(CUIDragDropListEx *pSellList, CUIDragDropListEx *pBuyList,
...
public:
  ...
  void HighlightForEachInSlot(const luabind::functor<bool> &functor, u8 type,
                              u16 slot_id);

  //-AxelDominator && Alundaio consumable use condition
  void
  DonateCurrentItem(CUICellItem *cell_item); // Alundaio: Donate item via
                                             // context menu while in trade menu
