...
IC CPlanner::CActionPlanner()
    : m_initialized(false), m_solving(false), m_loaded(false) {
#ifdef LOG_ACTION
  m_use_log = false;
#endif
}
...
void CPlanner::update() {
  ...
  if (m_failed) {
    ...
    show();
    Msg("!ERROR: there is no action sequence, which can transfer current world "
        "state to the target one action[%s] object[%s]",
  ...
  if (bDbgAct && m_failed && current_action().m_action_name)
    Msg("!ERROR: there is no action sequence, which can transfer current world "
        "state to the target one. action[%s]",
        current_action().m_action_name);
#endif
...
TEMPLATE_SPECIALIZATION
IC typename CPlanner::COperator &
CPlanner::action(const _action_id_type &action_id) {
...
IC typename CPlanner::_action_id_type CPlanner::current_action_id() const {
  VERIFY2(
      initialized(),
      make_string(
          "ERROR: action by id [%d] not initialized!",
...
#ifdef DEBUG
  VERIFY2(!m_solving, make_string("do not change preconditions during planner "
                                  "update, object %s, id[%d]",
...
IC void CPlanner::show_target_world_state() {
  ...
  for (; I != E; ++I) {
    xr_vector<COperatorCondition>::const_iterator J = std::lower_bound(
        target_state().conditions().begin(), target_state().conditions().end(),
