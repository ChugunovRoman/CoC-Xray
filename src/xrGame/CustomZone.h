...
struct SZoneObjectInfo {
  SZoneObjectInfo()
      : object(NULL), zone_ignore(false), dw_time_in_zone(0),
        f_time_affected(Device.fTimeGlobal), small_object(false),
...
protected:
  ...
  void CreateHit(u16 id_to, u16 id_from, const Fvector &hit_dir,
                 float hit_power, s16 bone_id, const Fvector &pos_in_bone,
                 float hit_impulse, ALife::EHitType hit_type);
