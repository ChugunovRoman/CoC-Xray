...
IC void tri_plane(const Fvector &v0, const Fvector &v1, const Fvector &v2,
                  Fplane &p) {
...
IC void tri_plane(const CDB::TRI &tri, Fplane &p) {
  ...
  tri_plane(pVerts[tri.verts[0]], pVerts[tri.verts[1]], pVerts[tri.verts[2]],
            p);
}

IC bool get_plane_static(ik_pick_result &r, Fvector &next_pos,
                         float &next_range, const collide::rq_result &R,
                         float pick_dist, const Fvector &pos,
...
#ifdef DEBUG
    float u, v, d;
    VERIFY(!(CDB::TestRayTri(next_pos, pick_v, r.triangle, u, v, d, true) &&
             d > 0.f));
#endif // DEBUG
    return false;
  }
...
IC bool get_plane_dynamic(ik_pick_result &r, Fvector &next_pos,
                          float &next_range, const collide::rq_result R,
                          float pick_dist, const Fvector &pos,
                          const Fvector &pick_v) {
...
IC bool get_plane(ik_pick_result &r, Fvector &next_pos, float &next_range,
                  const collide::rq_result R, float pick_dist,
...
void chose_best_plane(Fplane &p, const Fvector &v, Fplane &p0, Fplane &p1,
                      Fplane &p2) {
...
void ik_foot_collider::collide(SIKCollideData &cld,
                               const ik_foot_geom &foot_geom, CGameObject *O,
                               bool foot_step) {
