...
void _AddIconedTalkMessage(LPCSTR caption, LPCSTR text, LPCSTR texture_name,
                           LPCSTR templ_name);

void CScriptGameObject::AddIconedTalkMessage(LPCSTR caption, LPCSTR text,
                                             LPCSTR texture_name,
                                             LPCSTR templ_name) {
...
void _give_news(LPCSTR caption, LPCSTR news, LPCSTR texture_name, int delay,
                int show_time, int type);

void CScriptGameObject::GiveGameNews(LPCSTR caption, LPCSTR news,
                                     LPCSTR texture_name, int delay,
                                     int show_time) {
...
void _give_news(LPCSTR caption, LPCSTR text, LPCSTR texture_name, int delay,
                int show_time, int type) {
...
void CScriptGameObject::ForEachInventoryItems(
  ...
  if (!owner) {
    ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                    "CScriptGameObject::ForEachInventoryItems "
...
void CScriptGameObject::ChangeGoodwill(int delta_goodwill,
  ...
  RELATION_REGISTRY().ChangeGoodwill(pInventoryOwner->object_id(),
                                     pWhoToSet->object().ID(), delta_goodwill);
}
...
void CScriptGameObject::SetCharacterCommunity(LPCSTR comm, int squad,
                                              int group) {
  CInventoryOwner *pInventoryOwner = smart_cast<CInventoryOwner *>(&object());
  ...
  if (community.index() >= 0) {
    ...
    ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeInfo,
                                    "SetCharacterCommunity can't set %s for %s",
                                    comm, Name());
}
...
void construct_restriction_vector(shared_str restrictions,
  ...
  for (u32 i = 0; i < n; ++i) {
    CObject *object =
        Level().Objects.FindObjectByName(_GetItem(*restrictions, i, temp));
    if (!object)
      continue;
...
void CScriptGameObject::remove_all_restrictions() {
  ...
  if (!monster) {
    ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                    "CRestrictedObject : cannot access class "
                                    "member remove_all_restrictions!");
    return;
  }
...
bool CScriptGameObject::attachable_item_enabled() const {
  ...
  if (!attachable_item) {
    ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                    "CAttachableItem : cannot access class "
                                    "member attachable_item_enabled!");
    return (false);
  }
...
void CScriptGameObject::attachable_item_load_attach(LPCSTR section) {
  ...
  if (!attachable_item) {
    ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                    "CAttachableItem : cannot access class "
                                    "member attachable_item_load_attach!");
    return;
  }
  ...
  if (attachable_item->object().H_Parent()) { // reattach
    CAttachmentOwner *AO =
        smart_cast<CAttachmentOwner *>(attachable_item->object().H_Parent());
    if (AO)
      AO->reattach_items();
  }
}
...
bool CScriptGameObject::Weapon_IsGrenadeLauncherAttached() {
  ...
  if (!weapon) {
    ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                    "CWeapon : cannot access class member "
...
void CScriptGameObject::GiveTaskToActor(CGameTask *t, u32 dt,
                                        bool bCheckExisting, u32 t_timer) {
...
void CScriptGameObject::aim_time(CScriptGameObject *weapon, u32 aim_time) {
  ...
  if (!weapon_) {
    ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                    "CAI_Stalker : cannot access class member "
...
CCoverPoint const *
CScriptGameObject::find_best_cover(Fvector position_to_cover_from) {
  CAI_Stalker *stalker = smart_cast<CAI_Stalker *>(&object());
  if (!stalker) {
    ai().script_engine().script_log(
        ScriptStorage::eLuaMessageTypeError,
...
void CScriptGameObject::register_door() {
  ...
  if (!m_door)
    m_door =
        ai().doors().register_door(*smart_cast<CPhysicObject *>(m_game_object));
  //	Msg									( "registering door
  //0x%-08x", m_door );
}
...
void CScriptGameObject::Weapon_SetCurrentScope(u8 type) {
  ...
  if (!weapon) {
    ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                    "CWeaponMagazined : cannot access class "
                                    "member Weapon_SetCurrentScope!");
    return;
  }
...
LPCSTR CScriptGameObject::Weapon_GetAmmoSection(u8 ammo_type) {
  ...
  if (!weapon) {
    ai().script_engine().script_log(
        ScriptStorage::eLuaMessageTypeError,
        "CWeaponMagazined : cannot access class member Weapon_GetAmmoSection!");
    return "";
  }
...
float CScriptGameObject::GetAdditionalMaxWalkWeight() const {
  ...
  if (!outfit && !pBackpack) {
    ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                    "CCustomOutfit : cannot access class "
                                    "member GetAdditionalMaxWalkWeight!");
    return (false);
  }
  if (outfit)
...
void CScriptGameObject::SetAdditionalMaxWalkWeight(float add_max_walk_weight) {
  ...
  if (!outfit && !pBackpack) {
    ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                    "CCustomOutfit : cannot access class "
                                    "member SetAdditionalMaxWalkWeight!");
    return;
  }
