...
bool CDetailPathManager::compute_tangent(const STrajectoryPoint &start,
  ...
  if (start_cp * dest_cp >= 0.f) {
    ...
    if (start_circle.center.similar(dest_circle.center, EPS_S)) {
      if (fsimilar(start_circle.radius, dest_circle.radius, EPS_S)) {
        ...
        adjust_point(start_circle.center, dest_yaw, start_circle.radius,
                     tangents[0].point);
        if (start_cp >= 0.f)
          assign_angle(tangents[0].angle, start_yaw, dest_yaw, true,
                       direction_type);
        else
          assign_angle(tangents[0].angle, start_yaw, dest_yaw, false,
                       direction_type);
  ...
  if (coincide_directions(start_circle.center, tangents[0].point, start_cp,
                          dest_circle.center, tangents[1].point, dest_cp)) {
    assign_angle(tangents[0].angle, start_yaw,
                 yaw1 + alpha < PI_MUL_2 ? yaw1 + alpha
                                         : yaw1 + alpha - PI_MUL_2,
                 start_cp >= 0, direction_type);
    assign_angle(tangents[1].angle, dest_yaw,
                 yaw2 + alpha < PI_MUL_2 ? yaw2 + alpha
                                         : yaw2 + alpha - PI_MUL_2,
...
bool CDetailPathManager::build_circle_trajectory(
    const STrajectoryPoint &position, xr_vector<STravelPathPoint> *path,
  ...
  if (position.radius * _abs(position.angle) <= min_dist) {
    ...
    if (vertex_id ||
        (!path->empty() && !path->back().position.similar(t.position, EPS_S))) {
...
#ifdef DEBUG
    if (m >= 10000) {
      Msg("! [position.radius=%f],[angle=%f],[m=%d]", position.radius, angle,
          m);
      VERIFY(m < 10000);
    }
#endif
  ...
  for (int i = 0; i <= n + k; ++i) {
    VERIFY(t.velocity != u32(-1));
    t.position.x =
        -sin_apb(sina, cosa, sini, cosi) * position.radius + position.center.x;
    t.position.z =
        cos_apb(sina, cosa, sini, cosi) * position.radius + position.center.y;
...
bool CDetailPathManager::build_line_trajectory(
    const STrajectoryPoint &start, const STrajectoryPoint &dest, u32 vertex_id,
  ...
  return (path ? ai().level_graph().create_straight_path<false>(
...
bool CDetailPathManager::compute_path(
    STrajectoryPoint &_start, STrajectoryPoint &_dest,
  ...
  for (; I != E; ++I) {
    ...
    for (; i != e; ++i) {
      ...
      m_temp_path.clear();
      if (compute_trajectory(start, dest, m_tpTravelLine ? &m_temp_path : 0,
                             time, (*I).index, real_straight_line_index,
...
void CDetailPathManager::validate_vertex_position(
  ...
  ai().level_graph().nearest(position, ai().level_graph().v3d(point.position),
                             contour);
...
bool CDetailPathManager::fill_key_points(const xr_vector<u32> &level_path,
  ...
  for (int _i = 0, i = 0, n = (int)level_path.size() - 1, j = n, m = j;
    ...
    if (!ai().level_graph().check_vertex_in_direction(
            start_point.vertex_id, start_point.position, level_path[j])) {
      m = j;
      j = (i + j) / 2;
    } else {
...
void CDetailPathManager::postprocess_key_points(
    const xr_vector<u32> &level_path, u32 intermediate_index,
    STrajectoryPoint &start, STrajectoryPoint &dest,
  ...
  for (int i = 1, n = (int)m_key_points.size() - 1; i < n; ++i) {
    STravelPoint key_point0 = compute_better_key_point(
        m_key_points[i - 1], m_key_points[i], m_key_points[i + 1], false);
    STravelPoint key_point1 = compute_better_key_point(
        m_key_points[i + 1], m_key_points[i], m_key_points[i - 1], true);
    {
    ...
    if (better_key_point(m_key_points[i - 1], m_key_points[i + 1], key_point0,
                         key_point1))
      m_key_points[i] = key_point0;
    else
      m_key_points[i] = key_point1;
    VERIFY(
        !m_key_points[i].position.similar(m_key_points[i - 1].position, EPS_S));
  }
}
...
void CDetailPathManager::build_smooth_path(const xr_vector<u32> &level_path,
                                           u32 intermediate_index) {
  //	Msg									("[%6d][%s] started to
  //build detail path",Device.dwFrame,*m_restricted_object->object().cName());
  ...
  if (m_restricted_object) {
    ...
    start_pos.y = ai().level_graph().vertex_plane_y(start.vertex_id,
    ...
    if (!((alvi && asp) || (!asp && !alvi))) {
      Msg("! vertex [%d], position [%f][%f][%f], alvi[%c], asp[%c]",
          start.vertex_id, VPUSH(start_pos), alvi ? '+' : '-', asp ? '+' : '-');
    }
