...
IC void CPatrolPoint::correct_position(const CLevelGraph *level_graph,
  ...
  if (!level_graph->inside(level_vertex_id(level_graph, cross, game_graph),
                           position()))
...
CPatrolPoint::CPatrolPoint(const CLevelGraph *level_graph,
                           const CGameLevelCrossTable *cross,
                           const CGameGraph *game_graph,
                           const CPatrolPath *path, const Fvector &position,
...
#ifdef DEBUG
  VERIFY2(vertex,
          make_string("invalid game vertex id[%d] (level_vertex_id[%d]) for "
