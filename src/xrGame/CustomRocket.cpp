...
void CCustomRocket::ObjectContactCallback(bool &do_colide, bool bo1,
...
void CCustomRocket::UpdateEngine() {
  ...
  if (!getVisible()) {
    Msg("! CCustomRocket::UpdateEngine called, but false==getVisible() id[%d] "
        "frame[%d]",
        ID(), Device.dwFrame);
  }
