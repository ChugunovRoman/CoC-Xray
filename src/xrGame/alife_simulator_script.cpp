...
CSE_ALifeDynamicObject *alife_object(const CALifeSimulator *self,
                                     ALife::_OBJECT_ID id, bool no_assert) {
  VERIFY(self);
...
void generate_story_ids(STORY_PAIRS &result, _id_type INVALID_ID,
                        LPCSTR section_name, LPCSTR INVALID_ID_STRING,
...
CSE_Abstract *
CALifeSimulator__spawn_item3(CALifeSimulator *self, LPCSTR section,
                             const Fvector &position, u32 level_vertex_id,
                             GameGraph::_GRAPH_ID game_vertex_id,
                             ALife::_OBJECT_ID id_parent, bool reg = true) {
  if (reg == true)
    return CALifeSimulator__spawn_item2(
        self, section, position, level_vertex_id, game_vertex_id, id_parent);
...
ALife::_SPAWN_ID
CALifeSimulator__spawn_id(CALifeSimulator *self,
                          ALife::_SPAWN_STORY_ID spawn_story_id) {
  return (((const CALifeSimulator *)self)->spawns().spawn_id(spawn_story_id));
}

void CALifeSimulator__release(CALifeSimulator *self, CSE_Abstract *object,
                              bool) {
  VERIFY(self);
...
LPCSTR get_level_name(const CALifeSimulator *self, int level_id) {
  LPCSTR result =
      *ai().game_graph().header().level((GameGraph::_LEVEL_ID)level_id).name();
  return (result);
}
...
bool has_info(const CALifeSimulator *self, const ALife::_OBJECT_ID &id,
              LPCSTR info_id) {
...
bool dont_has_info(const CALifeSimulator *self, const ALife::_OBJECT_ID &id,
                   LPCSTR info_id) {
  THROW(self);
...
void AlifeGiveInfo(const CALifeSimulator *alife, const ALife::_OBJECT_ID &id,
                   LPCSTR info_id) {
  KNOWN_INFO_VECTOR *known_info =
      alife->registry(info_portions).object(id, true);
  if (!known_info)
    return;
...
void AlifeRemoveInfo(const CALifeSimulator *alife, const ALife::_OBJECT_ID &id,
                     LPCSTR info_id) {
  KNOWN_INFO_VECTOR *known_info =
      alife->registry(info_portions).object(id, true);
  if (!known_info)
  ...
  known_info->erase(std::find_if(known_info->begin(), known_info->end(),
                                 CFindByIDPred(info_id)),
                    known_info->end());
}

// Alundaio: teleport object
void teleport_object(CALifeSimulator *alife, ALife::_OBJECT_ID id,
                     GameGraph::_GRAPH_ID game_vertex_id, u32 level_vertex_id,
                     const Fvector &position) {
  alife->teleport_object(id, game_vertex_id, level_vertex_id, position);
}

void IterateInfo(const CALifeSimulator *alife, const ALife::_OBJECT_ID &id,
...
CSE_Abstract *try_to_clone_object(CALifeSimulator *self, CSE_Abstract *object,
                                  LPCSTR section, const Fvector &position,
                                  u32 level_vertex_id,
                                  GameGraph::_GRAPH_ID game_vertex_id,
                                  ALife::_OBJECT_ID id_parent,
                                  bool bRegister = true) {
  ...
  if (wpnmag) {
    CSE_Abstract *absClone = self->spawn_item(
        section, position, level_vertex_id, game_vertex_id, id_parent, false);
    if (!absClone)
      return (0);
...
const CALifeObjectRegistry::OBJECT_REGISTRY &
alife_objects(const CALifeSimulator *self) {
  VERIFY(self);
...
void CALifeSimulator::script_register(lua_State *L) {
  module(
      L)[class_<CALifeSimulator>("alife_simulator")
             ...
             .def("object", (CSE_ALifeDynamicObject *
                             (*)(const CALifeSimulator *, ALife::_OBJECT_ID))(
                                alife_object))
             .def("object", (CSE_ALifeDynamicObject *
                             (*)(const CALifeSimulator *, ALife::_OBJECT_ID,
                                 bool))(alife_object))
             .def("objects", &alife_objects, return_stl_pair_iterator)
             .def("story_object", (CSE_ALifeDynamicObject *
                                   (*)(const CALifeSimulator *,
                                       ALife::_STORY_ID))(alife_story_object))
             ...
             .def("set_switch_distance",
                  &CALifeSimulator::set_switch_distance) // Alundaio: renamed to
                                                         // set_switch_distance
                                                         // from switch_distance
...

  {
    ...
    for (; I != E; ++I)
      instance.enum_(
          "_spawn_story_ids")[luabind::value(*(*I).first, (*I).second)];
