...
void CWeapon::Load(LPCSTR section) {
  ...
  cam_recoil.DispersionFrac = _abs(
      READ_IF_EXISTS(pSettings, r_float, section, "cam_dispersion_frac", 0.7f));
  ...
  m_crosshair_inertion =
      READ_IF_EXISTS(pSettings, r_float, section, "crosshair_inertion", 5.91f);
  ...
  //	misfireProbability			  =
  // pSettings->r_float(section,"misfire_probability");
  //	misfireConditionK			  = READ_IF_EXISTS(pSettings,
  //r_float, section, "misfire_condition_k",	1.0f);
  misfireStartCondition =
      pSettings->r_float(section, "misfire_start_condition");
  misfireEndCondition =
      READ_IF_EXISTS(pSettings, r_float, section, "misfire_end_condition", 0.f);
  ...
  m_eScopeStatus =
      (ALife::EWeaponAddonStatus)pSettings->r_s32(section, "scope_status");
  m_eSilencerStatus =
      (ALife::EWeaponAddonStatus)pSettings->r_s32(section, "silencer_status");
  ...
  if (m_eScopeStatus == ALife::eAddonAttachable) {
    ...
    shared_str scope_tex_name =
        pSettings->r_string(cNameSect(), "scope_texture");
    ...
    m_zoom_params.m_fScopeZoomFactorMin = READ_IF_EXISTS(
        pSettings, r_float, cNameSect(), "scope_zoom_factor_min", 0.3f);
  ...
  Fvector def_dof;
  def_dof.set(-1, -1, -1);
  m_zoom_params.m_ZoomDof = READ_IF_EXISTS(
      pSettings, r_fvector3, section, "zoom_dof", Fvector().set(-1, -1, -1));
  ...
  m_u8TracerColorID =
      READ_IF_EXISTS(pSettings, r_u8, section, "tracers_color_ID", u8(-1));
  ...
  m_zoom_params.m_sUseZoomPostprocess =
      READ_IF_EXISTS(pSettings, r_string, section, "scope_nightvision", 0);
  ;
  m_zoom_params.m_sUseBinocularVision =
      READ_IF_EXISTS(pSettings, r_string, section, "scope_alive_detector", 0);
...
void CWeapon::SpawnAmmo(u32 boxCurr, LPCSTR ammoSect, u32 ParentID) {
...

  {
    ...
    while (boxCurr) {
      l_pA->a_elapsed =
          (u16)(boxCurr > l_pA->m_boxSize ? l_pA->m_boxSize : boxCurr);
      NET_Packet P;
  ...
  if (m_eScopeStatus == ALife::eAddonAttachable)
    if (IsScopeAttached())
      ...
      for (; it != m_scopes.end(); it++) {
        ...
        LPCSTR bone_name =
            READ_IF_EXISTS(pSettings, r_string, section, "addon_bone", "");
        if (bone_name)
...
bool CWeapon::SetBoneVisible(IKinematics *m_model, const shared_str &bone_name,
  ...
  if (bone_id == BI_NONE) {
    ...
    R_ASSERT2(
        0, make_string("model [%s] has no bone [%s]",
                       pSettings->r_string(cNameSect().c_str(), "item_visual"),
                       bone_name.c_str())
               .c_str());
  }
  ...
  if (m_zoom_params.m_sUseZoomPostprocess.size() && IsScopeAttached()) {
    ...
    if (pA && !m_zoom_params.m_pNight_vision) {
      m_zoom_params.m_pNight_vision = xr_new<CNightVisionEffector>(
          m_zoom_params.m_sUseZoomPostprocess, effNightvisionScope);
    }
  }
}
...
bool CWeapon::NeedToDestroyObject() const { return false; }
...
CWeapon::can_kill(const xr_vector<const CGameObject *> &items) const {
      ...
      if (H_Parent() == Level().CurrentEntity() &&
        ...
        if (current_actor)
          current_actor->Cameras().AddCamEffector(
              xr_new<CEffectorDOF>(m_zoom_params.m_ReloadEmptyDof));
      }
    } else {
      if (H_Parent() == Level().CurrentEntity() &&
...
u32 CWeapon::Cost() const {
  ...
  if (m_ammoElapsed.type1) {
    ...
    float bs =
        pSettings->r_float(m_ammoTypes[m_ammoType.type1].c_str(), "box_size");
