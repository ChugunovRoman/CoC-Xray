...
void CStepManager::reload(LPCSTR section) {
  ...
  if (debug_step_info_load)
    Msg("loading step_params for object :%s, visual: %s, section: %s, "
        ...
        m_object->cName().c_str(), m_object->cNameVisual().c_str(), section,
        anim_section);
#endif
  ...
  for (u32 i = 0; pSettings->r_line(anim_section, i, &anim_name, &val); ++i) {
    ...
    if (debug_step_info_load) {
      ...
      Msg("step_params loaded for object :%s, visual: %s, motion: %s, anim "
          "set: %s  ",
...
void CStepManager::on_animation_start(MotionID motion_id, CBlend *blend) {
  ...
  if (it == m_steps_map.end()) {
    ...
    if (debug_step_info) {
      ...
      VERIFY(KA);
      std::pair<LPCSTR, LPCSTR> anim_name = KA->LL_MotionDefName_dbg(motion_id);
      Msg("! no step_params found for object :%s, visual: %s, motion: %s, anim "
          "set: %s  ",
          m_object->cName().c_str(), m_object->cNameVisual().c_str(),
          anim_name.first, anim_name.second);
    }
...
void CStepManager::update(bool b_hud_view) {
    ...
    if (offset_time <= cur_time) {
      ...
      if (b_play && is_on_ground())
        m_step_sound.play_next(mtl_pair, m_object,
                               m_step_info.params.step[i].power, b_hud_view);

      CGameObject *object = smart_cast<CGameObject *>(m_object);
      if (object)
        object->FootStepCallback(m_step_info.params.step[i].power, b_play,
  ...
  if (m_step_info.cur_cycle < step.cycles)
    m_step_info.cur_cycle = 1 + u8(float(cur_time - m_time_anim_started) /
...
void CStepManager::load_foot_bones(CInifile::Sect &data) {
  for (CInifile::SectCIt I = data.Data.begin(); I != data.Data.end(); ++I) {
    ...
    u16 index =
        smart_cast<IKinematics *>(m_object->Visual())->LL_BoneID(*item.second);
...
void CStepManager::material_sound::play_next(SGameMtlPair *mtl_pair,
                                             CEntityAlive *object, float volume,
                                             bool b_hud_mode) {
  ...
  if (last_mtl_pair != mtl_pair || m_last_step_sound_played == u8(-1)) {
    ...
    u8 new_played = u8((m_last_step_sound_played + 1 +
