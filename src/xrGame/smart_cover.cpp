...
cover::cover(smart_cover::object const &object, DescriptionPtr description,
    ...
    : inherited(object.Position(), object.ai_location().level_vertex_id()),
      m_object(object), m_description(description), m_id(m_object.cName()),
...
smart_cover::loophole *cover::best_loophole(Fvector const &position,
                                            float &value,
...
bool cover::loophole_path(shared_str const &source_raw,
  ...
  bool result = ai().graph_engine().search(m_description->transitions(), source,
                                           target, 0, parameters);
