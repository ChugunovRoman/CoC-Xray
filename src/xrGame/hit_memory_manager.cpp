...
void CHitMemoryManager::add(float amount, const Fvector &vLocalDir,
                            const CObject *who, s16 element) {
#ifndef MASTER_GOLD
  ...
  if (!entity_alive ||
      (m_object->tfGetRelationType(entity_alive) == ALife::eRelationTypeFriend))
    return;
...
void CHitMemoryManager::add(const CHitObject &_hit_object) {
  ...
  hit_object.m_squad_mask.set(
      m_stalker->agent_manager().member().mask(m_stalker), TRUE);
...
void CHitMemoryManager::update() {
  ...
  VERIFY(m_hits);
  m_hits->erase(
      std::remove_if(m_hits->begin(), m_hits->end(), CRemoveOfflinePredicate()),
      m_hits->end());
...
struct CRemoveHitObjectPredicate {
  ...
  CRemoveHitObjectPredicate(const MemorySpace::CHitObject *object)
      : m_object(object) {}
...
void CHitMemoryManager::remove(const MemorySpace::CHitObject *hit_object) {
  HITS::iterator I = std::find_if(m_hits->begin(), m_hits->end(),
                                  CRemoveHitObjectPredicate(hit_object));
  if (I != m_hits->end())
    m_hits->erase(I);
}
