...
CCar::SCarSound::SCarSound(CCar *car)
    : eCarSound(), engine_start_delay(0), time_state_start(0) {
  volume = 1.f;
  pcar = car;
...
void CCar::SCarSound::Init() {
  ...
  if (ini->section_exist("car_sound") &&
    ...
    snd_engine.create(ini->r_string("car_sound", "snd_name"), st_Effect,
                      sg_SourceType); //
    snd_engine_start.create(READ_IF_EXISTS(ini, r_string, "car_sound",
                                           "engine_start",
                                           "car\\test_car_start"),
                            st_Effect, sg_SourceType);
    snd_engine_stop.create(READ_IF_EXISTS(ini, r_string, "car_sound",
    ...
    engine_start_delay =
        iFloor((snd_engine_start._handle()
    ...
    if (ini->line_exist("car_sound", "transmission_switch")) {
      snd_transmission.create(ini->r_string("car_sound", "transmission_switch"),
                              st_Effect, sg_SourceType);
    }
