...
void CEntityCondition::LoadCondition(LPCSTR entity_section) {
  ...
  m_fKillHitTreshold =
      READ_IF_EXISTS(pSettings, r_float, section, "killing_hit_treshold", 0.0f);
  m_fLastChanceHealth =
      READ_IF_EXISTS(pSettings, r_float, section, "last_chance_health", 0.0f);
  m_fInvulnerableTimeDelta =
      READ_IF_EXISTS(pSettings, r_float, section, "invulnerable_time", 0.0f) /
      1000.f;

  m_fBleedSpeedK = READ_IF_EXISTS(pSettings, r_float, section, "bleed_speed_k",
                                  (float)(1 / 3));
}
...
void CEntityCondition::UpdateWounds() {
  ...
  m_WoundVector.erase(std::remove_if(m_WoundVector.begin(), m_WoundVector.end(),
                                     &RemoveWoundPred),
                      m_WoundVector.end());
}
...
float CEntityCondition::HitOutfitEffect(float hit_power,
                                        ALife::EHitType hit_type, s16 element,
  ...
  if (pOutfit)
    new_hit_power =
        pOutfit->HitThroughArmor(hit_power, element, ap, add_wound, hit_type);
...
float CEntityCondition::HitPowerEffect(float power_loss) {
  ...
  return power_loss * (0.5f + (pOutfit ? pOutfit->m_fPowerLoss : EPS) +
                       (pHelmet ? pHelmet->m_fPowerLoss : EPS) +
                       (pBackpack ? pBackpack->m_fPowerLoss : EPS));
}

CWound *CEntityCondition::AddWound(float hit_power, ALife::EHitType hit_type,
                                   u16 element) {
...
void CEntityCondition::UpdateHealth() {
  float bleeding_speed =
      BleedingSpeed() * m_fDeltaTime * m_change_v.m_fV_Bleeding;
...
void CEntityCondition::UpdateRadiation() {
  if (m_fRadiation > 0) {
    ...
    m_fDeltaHealth -= CanBeHarmed() ? m_change_v.m_fV_RadiationHealth *
...
void SMedicineInfluenceValues::Load(const shared_str &sect) {
  ...
  fMaxPowerUp =
      READ_IF_EXISTS(pSettings, r_float, sect.c_str(), "eat_max_power", 0.0f);
  fAlcohol =
      READ_IF_EXISTS(pSettings, r_float, sect.c_str(), "eat_alcohol", 0.0f);
  fTimeTotal =
      READ_IF_EXISTS(pSettings, r_float, sect.c_str(), "apply_time_sec", -1.0f);
}
...
void SBooster::Load(const shared_str &sect, EBoostParams type) {
  ...
  case eBoostRadiationProtection:
    fBoostValue =
        pSettings->r_float(sect.c_str(), "boost_radiation_protection");
    break;
