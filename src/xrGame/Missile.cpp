...
void CMissile::PH_A_CrPr() {
  if (m_just_after_spawn) {
    ...
    if (!obj.PPhysicsShell()) {
      Msg("! ERROR: PhysicsShell is NULL, object [%s][%d]", obj.cName().c_str(),
          obj.ID());
      return;
    }
    ...
    /*	case eBore:
                    {
                            ...
                            PlayHUDMotion		("anm_bore", TRUE, this,
       GetState()); } break;
    */
  }
}
...
void CMissile::activate_physic_shell() {
  ...
  if (entity_alive && entity_alive->character_physics_support()) {
    Fvector parent_vel;
    entity_alive->character_physics_support()->movement()->GetCharacterVelocity(
        parent_vel);
