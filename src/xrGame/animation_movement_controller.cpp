...
animation_movement_controller::animation_movement_controller(
    Fmatrix *_pObjXForm, const Fmatrix &inital_pose, IKinematics *_pKinematicsC,
    CBlend *b)
    : m_startObjXForm(inital_pose), m_pObjXForm(*_pObjXForm),
      ...
      inital_position_blending(true), stopped(false), blend_linear_speed(0),
      blend_angular_speed(0), m_control_blend(b),
...
static void get_animation_root_position(Fmatrix &pos, IKinematics *K,
...
void animation_movement_controller::OnFrame() {
  ...
  // m_pKinematicsC->Bone_GetAnimPos( root_pos, m_pKinematicsC->LL_GetBoneRoot(
  // ), u8(-1), true );
  Fmatrix root_pos;
...
already another started anim: %s anim set: %s",
                  new_anim_name,new_anim_set,old_anim_name,old_anim_set
                  );
  if( !ControlBlend( )->stop_at_end )
          Msg( " ! obj movement anim  : %s anim set: %s  is not stop-at-end but
fallowed in chain by another obj movement anim: %s anim set: %s",
                  old_anim_name,old_anim_set,new_anim_name,new_anim_set
                  );
  if( !B->stop_at_end )
          Msg( " ! obj movement anim  : %s anim set: %s  is not stop-at-end but
fallowing after another obj movement anim: %s anim set: %s",
                  new_anim_name,new_anim_set,old_anim_name,old_anim_set
                  );
#endif
  //	VERIFY(  );
...
void animation_movement_controller::SetPosesBlending() {
  ...
  poses_blending blending(m_pObjXForm, Fmatrix().mul_43(m_startObjXForm, root),
                          blending_time);
