...
public:
  ...
  IC bool operator()(CALifeLevelRegistry::_iterator &i, u64 cycle_count,
                     bool) const {
...
void CALifeUpdateManager::set_process_time(int microseconds) {
  graph().set_process_time(float(microseconds) - float(microseconds) *
...
void CALifeUpdateManager::jump_to_level(LPCSTR level_name) const {
  ...
  bool failed = !ai().graph_engine().search(
      ai().game_graph(), graph().actor()->m_tGraphID, GameGraph::_GRAPH_ID(-1),
      0, evaluator);
  if (failed) {
#ifndef MASTER_GOLD
    ...
    Fvector current =
        ai().game_graph().vertex(graph().actor()->m_tGraphID)->game_point();
    ...
    for (GameGraph::_GRAPH_ID i = 0; i < n; ++i)
      if (ai().game_graph().vertex(i)->level_id() == level.id()) {
        float distance =
            ai().game_graph().vertex(i)->game_point().distance_to_sqr(current);
    ...
    if (!ai().game_graph().vertex(dest)) {
      Msg("! There is no game vertices on the level %s, cannot jump to the "
          "specified level",
          level_name);
      return;
    }
...
void CALifeUpdateManager::teleport_object(ALife::_OBJECT_ID id,
  ...
  if (psAI_Flags.test(aiALife)) {
    Msg("[LSS] teleporting object [%s][%s][%d] from level [%s], position "
...
void CALifeUpdateManager::add_restriction(
  ...
  CSE_ALifeDynamicObject *object = objects().object(id, true);
  if (!object) {
    Msg("! cannot add restriction with id %d to the entity with id %d, because "
  ...
  if (!object_restrictor) {
    Msg("! cannot add restriction with id %d to the entity with id %d, because "
        "there is no space restrictor with the specified id",
        restriction_id, id);
    return;
  }
  ...
  if (!creature) {
    Msg("! cannot add restriction with id %d to the entity with id %d, because "
        "there is an object with the specified id, but it is not a creature",
        restriction_id, id);
    return;
  }
  ...
  if (!restrictor) {
    Msg("! cannot add restriction with id %d to the entity with id %d, because "
        "there is an object with the specified id, but it is not a space "
        "restrictor",
        restriction_id, id);
    return;
  }
  ...
  case RestrictionSpace::eRestrictorTypeOut: {
    ...
    if (std::find(creature->m_dynamic_out_restrictions.begin(),
      ...
      Msg("! cannot add out-restriction with id %d, name %s to the entity with "
          "id %d, name %s, because it is already added",
  ...
  case RestrictionSpace::eRestrictorTypeIn: {
    ...
    if (std::find(creature->m_dynamic_in_restrictions.begin(),
      ...
      Msg("! cannot add in-restriction with id %d, name %s to the entity with "
          "id %d, name %s, because it is already added",
          restriction_id, restrictor->name_replace(), id,
          creature->name_replace());
...
void CALifeUpdateManager::remove_restriction(
  ...
  if (!creature) {
    Msg("! cannot remove restriction with id %d to the entity with id %d, "
        "because there is an object with the specified id, but it is not a "
        "creature",
        restriction_id, id);
    return;
  }
  ...
  case RestrictionSpace::eRestrictorTypeOut: {
    ...
    if (I == creature->m_dynamic_out_restrictions.end()) {
      Msg("~ cannot remove restriction with id [%d][%s] to the entity with id "
          "[%d][%s], because it is not added",
...
void CALifeUpdateManager::remove_all_restrictions(
  ...
  CSE_ALifeDynamicObject *object = objects().object(id, true);
  if (!object) {
    Msg("! cannot remove restrictions to the entity with id %d, because there "
        "is no creature with the specified id",
        id);
    return;
  }
  ...
  CSE_ALifeCreatureAbstract *creature =
      smart_cast<CSE_ALifeCreatureAbstract *>(object);
  if (!creature) {
    Msg("! cannot remove restriction to the entity with id %d, because there "
        "is an object with the specified id, but it is not a creature",
        id);
    return;
  }
