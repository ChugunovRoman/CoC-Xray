...
CWeaponStatMgun::CWeaponStatMgun() {
  ...
  camera = xr_new<CCameraFirstEye>(this, CCameraBase::flRelativeLink |
                                             CCameraBase::flPositionRigid |
...
void CWeaponStatMgun::Load(LPCSTR section) {
  ...
  m_overheat_threshold =
      READ_IF_EXISTS(pSettings, r_float, section, "overheat_threshold", 110.f);
...
//							s16 element,Fvector
// p_in_object_space, 							float impulse,
...
void CWeaponStatMgun::cam_Update(float dt, float fov) {
  ...
  Camera()->yaw = angle_inertion_var(Camera()->yaw, des_cam_dir.x, 0.5f, 7.5f,
                                     PI_DIV_6, Device.fTimeDelta);
