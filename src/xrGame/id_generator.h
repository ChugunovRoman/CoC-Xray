...
template <typename TIME_ID, typename TYPE_ID, typename VALUE_ID,
          typename BLOCK_ID, typename CHUNK_ID, VALUE_ID tMinValue,
          VALUE_ID tMaxValue, CHUNK_ID tBlockSize,
...
public:
  IC CID_Generator() {
    ...
    for (u32 j = 0; j < m_tBlockCount; ++j)
      std::reverse(m_tppBlocks[j].m_tpIDs,
  ...
  IC VALUE_ID tfGetID(VALUE_ID tValueID = tInvalidValueID) {
    if (tInvalidValueID != tValueID)
      return (
          tfGetFromBlock(m_tppBlocks[tfGetBlockByValue(tValueID)], tValueID));
