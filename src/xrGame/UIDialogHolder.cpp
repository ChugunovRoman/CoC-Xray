...
void CDialogHolder::OnFrame() {
  ...
  while (!m_dialogsToRender.empty() &&
         (!m_dialogsToRender[m_dialogsToRender.size() - 1].enabled))
  ...
  if (!TIR->StopAnyMove() && g_pGameLevel) {
    ...
    if (O) {
      ...
      //				IR->IR_OnKeyboardPress(get_binded_action(dik));
      {
        ...
        if (action != kQUICK_USE_1 && action != kQUICK_USE_2 &&
            action != kQUICK_USE_3 && action != kQUICK_USE_4)
...
bool CDialogHolder::IR_UIOnMouseWheel(int direction) {
  ...
  TIR->OnMouseAction(pos.x, pos.y,
                     (direction > 0) ? WINDOW_MOUSE_WHEEL_UP
