    ...
    while (game_events->available(svT)) {
      ...
      if (g_bootComplete && M_SPAWN == ID &&
          Device.frame_elapsed() >
              work_limit) // alpet: позволит плавнее выводить объекты в онлайн,
                          // без заметных фризов
      {
        u16 parent_id;
        ...
        //-------------------------------------------------
        if (parent_id <
            0xffff) // откладывать спавн только объектов в контейнеры
        {
          if (!spawn_events->available(svT))
            Msg("* ProcessGameEvents, spawn event postponed. Events rest = %d",
                game_events->queue.size());
...
void CLevel::OnFrame() {
  ...
  if (!g_dedicated_server && psDeviceFlags.test(rsStatistic)) {
    ...
    if (!psNET_direct_connect) {
      if (IsServer()) {
        ...
        struct net_stats_functor {
          ...
          void operator()(IClient *C) {
            ...
            F->OutNext("0x%08x: P(%d), BPS(%2.1fK), MRR(%2d), MSR(%2d), "
                       ...
                       C->stats.getRetriedCount(), C->stats.dwTimesBlocked);
          }
        };
      ...
      if (IsClient()) {
        ...
        F->SetColor(D3DCOLOR_XRGB(255, 255, 255));
        F->OutNext(
            "BReceivedPs(%2d), BSendedPs(%2d), Retried(%2d), Blocked(%2d)",
            net_Statistic.getReceivedPerSec(), net_Statistic.getSendedPerSec(),
            net_Statistic.getRetriedCount(), net_Statistic.dwTimesBlocked);
#ifdef DEBUG
        if (!pStatGraphR) {
  ...
  if (!g_dedicated_server) {
    if (g_mt_config.test(mtLUA_GC))
      Device.seqParallel.push_back(
          fastdelegate::FastDelegate0<>(this, &CLevel::script_gc));
    else
      script_gc();
  }
  if (pStatGraphR) {
    ...
    for (u32 I = 0; I < Level().Objects.o_count(); I++) {
      ...
      if (dbg_net_Draw_Flags.test(dbg_draw_skeleton)) // draw skeleton
      {
        ...
        if (pGO && pGO != Level().CurrentViewEntity() && !pGO->H_Parent()) {
          if (pGO->Position().distance_to_sqr(Device.vCameraPosition) <
              400.0f) {
...
void CLevel::PhisStepsCallback(u32 Time0, u32 Time1) { return; }
...
float CLevel::GetEnvironmentGameDayTimeSec() {
  return (float(s64(GetEnvironmentGameTime() % (24 * 60 * 60 * 1000))) /
          1000.f);
}
