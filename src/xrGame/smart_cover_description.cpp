...
void description::load_loopholes(shared_str const &table_id) {
  ...
  for (; I != E; ++I) {
    ...
    VERIFY(std::find_if(m_loopholes.begin(), m_loopholes.end(),
                        id_predicate(*loophole)) == m_loopholes.end());
...
void description::process_loopholes() {
  ...
  for (; I != E; ++I) {
    ::loophole *current = *I;
    current->enterable(
        m_transitions.edge(transform_vertex("", true), current->id()) != 0);
    current->exitable(
        m_transitions.edge(current->id(), transform_vertex("", false)) != 0);
  }
...
IC void
delete_data(const CGraphAbstract<_data_type, _edge_weight_type, _vertex_id_type,
