...
void CInventory::Take(CGameObject *pObj, bool bNotActivate,
  ...
  // (correction prediction) usually net_Import arrived for objects that not has
  // a parent object.. for unknown reason net_Import arrived for object that has
  // a parent, so correction prediction schema will crash
  ...
  case eItemPlaceRuck:
    ...
    break;
  case eItemPlaceSlot:
    result = Slot(pIItem->m_ItemCurrPlace.slot_id, pIItem, bNotActivate,
                  strict_placement);
    if (!result)
      pIItem->m_ItemCurrPlace.type = eItemPlaceUndefined;
#ifdef DEBUG
  ...
  if (pIItem->CurrPlace() == eItemPlaceUndefined) {
    if (!pIItem->RuckDefault()) {
      if (CanPutInSlot(pIItem, pIItem->BaseSlot())) {
        result =
            Slot(pIItem->BaseSlot(), pIItem, bNotActivate, strict_placement);
        VERIFY(result);
...
#ifdef DEBUG
          Msg("---DropItem activating slot [-1], forced, Frame[%d]",
              Device.dwFrame);
#endif // #ifdef DEBUG
...
bool CInventory::Slot(u16 slot_id, PIItem pIItem, bool bNotActivate,
                      bool strict_placement) {
  VERIFY(pIItem);
  ...
  if (((m_iActiveSlot == slot_id) || (m_iActiveSlot == NO_ACTIVE_SLOT) &&
                                         m_iNextActiveSlot == NO_ACTIVE_SLOT) &&
      (!bNotActivate)) {
#ifdef DEBUG
...
void CInventory::Activate(u16 slot, bool bForce) {
  ...
  if (GetActiveSlot() == slot || (GetNextActiveSlot() == slot && !bForce)) {
    ...
    //! LogStackTrace
    //("");
    //			Msg				( "[%6d][%s] CInventory::Activate changing next
    //active slot to %d", Device.dwTimeGlobal, name, slot );
    //		}
    m_iNextActiveSlot = slot;
#ifdef DEBUG
//		Msg("--- There's no need to activate slot [%d], next active slot
// is [%d]", slot, m_iNextActiveSlot);
#endif
    return;
  }
...
#ifdef DEBUG
//			Msg("--- Inventory owner [%s]: send deactivate item
//[%s]", m_pOwner->Name(), active_item->NameItem());
#endif     // #ifdef DEBUG
    } else // in case where weapon is going to destroy
    {
      ...
      //! LogStackTrace
      //("");
      //!				Msg ("[%6d][%s] CInventory::Activate
      //! changing active slot from %d to %d", Device.dwTimeGlobal, name,
      //! m_iActiveSlot, slot );
      //!			}
    ...
    //				LogStackTrace	("");
    //			Msg				( "[%6d][%s] CInventory::Activate changing next
    //active slot3 to %d", Device.dwTimeGlobal, name, slot );
    //		}
    m_iNextActiveSlot = slot;
  }
}
...
PIItem CInventory::Same(const PIItem pIItem, bool bSearchRuck) const {
  ...
  for (TIItemContainer::const_iterator it = list.begin(); list.end() != it;
    ...
    if ((l_pIItem != pIItem) && !xr_strcmp(l_pIItem->object().cNameSect(),
...
PIItem CInventory::SameSlot(const u16 slot, PIItem pIItem,
...
bool CInventory::Eat(PIItem pIItem) {
  ...
  if (ai().script_engine().functor("_G.CInventory__eat", funct)) {
    if (!funct(smart_cast<CGameObject *>(pItemToEat->object().H_Parent())
                   ->lua_game_object(),
  ...
  if (pItemToEat->GetMaxUses() < 255 &&
      pItemToEat->CanConsumeCharge()) // If uses 255, then skip the decrement
                                      // for infinite usages
  {
...
bool CInventory::CanTakeItem(CInventoryItem *inventory_item) const {
  ...
  for (TIItemContainer::const_iterator it = m_all.begin(); it != m_all.end();
       it++)
  ...
  if (!pActor &&
      (TotalWeight() + inventory_item->Weight() > m_pOwner->MaxCarryWeight()))
    return false;
...
void CInventory::AddAvailableItems(TIItemContainer &items_container,
                                   bool for_trade, bool bOverride) const {
  for (TIItemContainer::const_iterator it = m_ruck.begin(); m_ruck.end() != it;
       ++it) {
    PIItem pIItem = *it;
...
void CInventory::TryDeactivateActiveSlot() {
  ...
  } else if (next_active_item && (IsSlotBlocked(next_active_item) ||
