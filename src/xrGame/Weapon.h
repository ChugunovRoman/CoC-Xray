...
public:
  ...
  virtual const CInventoryItem *
  can_kill(const xr_vector<const CGameObject *> &items) const;
...
public:
  ...
  int GetGrenadeLauncherX() {
    return pSettings->r_s32(m_launchers[m_cur_addon.launcher],
                            "grenade_launcher_x");
  }
  ...
  const shared_str GetGrenadeLauncherName() const {
    return pSettings->r_string(m_launchers[m_cur_addon.launcher],
  ...
  const shared_str GetSilencerName() const {
    return pSettings->r_string(m_silencers[m_cur_addon.silencer],
                               "silencer_name");
  }

  const shared_str GetGrenadeLauncherBoneName() const {
    return READ_IF_EXISTS(pSettings, r_string, GetGrenadeLauncherName(),
                          "addon_bone", "wpn_launcher");
  }
  const shared_str GetScopeBoneName() const {
    return READ_IF_EXISTS(pSettings, r_string, GetScopeName(), "addon_bone",
                          "wpn_scope");
  }
  const shared_str GetSilencerBoneName() const {
    return READ_IF_EXISTS(pSettings, r_string, GetSilencerName(), "addon_bone",
                          "wpn_silencer");
  }

  bool SetBoneVisible(IKinematics *m_model, const shared_str &bone_name,
...
public:
  void GetZoomData(const float scope_factor, float &delta,
  ...
  void AllowNightVision(bool value) {
    m_zoom_params.m_bNightVisionAllow = value;
  };
...
protected:
  ...
  float
      misfireStartCondition; //изношенность, при которой появляется шанс осечки
  float misfireEndCondition; //изношеность при которой шанс осечки становится
                             //константным
  float misfireStartProbability;       //шанс осечки при изношености больше чем
                                       // misfireStartCondition
  float misfireEndProbability;         //шанс осечки при изношености больше чем
                                       // misfireEndCondition
  float conditionDecreasePerQueueShot; //увеличение изношености при выстреле
                                       //очередью
  float
      conditionDecreasePerShot; //увеличение изношености при одиночном выстреле
...
protected:
  ...
  mutable u32
      m_BriefInfo_CalcFrame; //кадр на котором просчитали кол-во патронов
...
public:
  struct ammo_type_t {
    union {
      u8 data;
      struct {
        u8 type1 : 4; // Type1 is normal ammo unless in grenade mode it's
                      // swapped
        u8 type2 : 4; // Type2 is grenade ammo unless in grenade mode it's
                      // swapped
      };
    };
  };
  ...
  struct ammo_elapsed_t {
    union {
      u16 data;
      struct {
        u16 type1 : 8; // Type1 is normal ammo unless in grenade mode it's
                       // swapped
        u16 type2 : 8; // Type2 is grenade ammo unless in grenade mode it's
                       // swapped
      };
    };
  };
