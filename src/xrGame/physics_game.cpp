...
const float mass_limit =
    10000.f; // some conventional value used as evaluative
             // param (there is no code restriction on mass)
//////////////////////////////////////////////////////////////////////////////////
...
public:
  CPHLiquidParticlesPlayCall(const dContactGeom &contact, bool invert_n,
                             LPCSTR psn)
...
IC bool play_liquid_particle_criteria(dxGeomUserData &data, float vel_cret) {
  ...
  bool controller =
      !!data.ph_object && data.ph_object->CastType() == CPHObject::tpCharacter;
...
void play_particles(float vel_cret, dxGeomUserData *data, const dContactGeom *c,
                    bool b_invert_normal, const SGameMtl *static_mtl,
                    LPCSTR ps_name) {
  VERIFY(c);
  VERIFY(static_mtl);
  ...
  if (data) {
    ...
    if (mtl_pair) {
      ...
      if (square_cam_dist < SQUARE_PARTICLE_EFFECT_DIST &&
        ...
        play_particles<Pars>(vel_cret, data, c, b_invert_normal, static_mtl,
                             ps_name);
      }
    }
  }
