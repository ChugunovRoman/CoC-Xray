...
void CMapLocation::LoadSpot(LPCSTR type, bool bReload) {
  ...
  if (node) {
    ...
    m_spot_border_names[2] =
        g_uiSpotXml->ReadAttrib(path, 0, "spot_a", "mini_map_spot_border");
...
void CMapLocation::InitUserSpot(const shared_str &level_name,
                                const Fvector &pos) {
  ...
  if (ai().get_alife()) {
    ...
    if (!ai().game_graph().vertex(m_cached.m_graphID)) {
      Msg("qweasdd! Cannot assign game vertex for CUserDefinedMapLocation "
          "[map=%s]",
          *level_name);
...
void CMapLocation::UpdateSpot(CUICustomMap *map, CMapSpot *sp) {
  if (map->MapName() == GetLevelName()) {
    ...
    m_position_on_map = map->ConvertRealToLocal(
        position,
        (map->Heading()) ? false : true); // for visibility calculating
    ...
    if (map->Heading()) {
      m_position_on_map = map->ConvertRealToLocal(position, true); // for
                                                                   // drawing
      sp->SetWndPos(m_position_on_map);
    }
    ...
    if (res) {
      ...
      if (!bDone && bbb) {
        Msg("! Error. Path from actor to selected map spot does not contain "
            "level changer :(");
        Msg("Path:");
...
void CMapLocation::HighlightSpot(bool state, const Fcolor &color) {
  ...
  if (state) {
    u32 clr =
        color_rgba((u32)color.r, (u32)color.g, (u32)color.b, (u32)color.a);
  ...
  if (PointerEnabled()) {
    if (sp == m_level_spot) {
      if (NULL == m_level_map_spot_border) {
        ...
        m_level_map_spot_border->Load(g_uiSpotXml,
                                      m_spot_border_names[0].c_str());
      }
      return m_level_map_spot_border;
    } else if (sp == m_minimap_spot) {
      if (NULL == m_mini_map_spot_border) {
        m_mini_map_spot_border = xr_new<CMapSpot>(this);
        m_mini_map_spot_border->Load(g_uiSpotXml,
                                     m_spot_border_names[2].c_str());
      }
    ...
    if (sp == m_level_spot) {
      if (NULL == m_level_map_spot_border_na && m_spot_border_names[1].size()) {
        ...
        m_level_map_spot_border_na->Load(g_uiSpotXml,
                                         m_spot_border_names[1].c_str());
      }
      ...
      if (NULL == m_mini_map_spot_border_na && m_spot_border_names[3].size()) {
        ...
        m_mini_map_spot_border_na->Load(g_uiSpotXml,
                                        m_spot_border_names[3].c_str());
      }
      ...
      if (NULL == m_complex_spot_border_na && m_spot_border_names[5].size()) {
        ...
        m_complex_spot_border_na->Load(g_uiSpotXml,
                                       m_spot_border_names[5].c_str());
      }
...
CRelationMapLocation::CRelationMapLocation(const shared_str &type,
                                           u16 object_id, u16 pInvOwnerActorID)
...
bool CRelationMapLocation::Update() {
  ...
  if (m_last_relation == ALife::eRelationTypeEnemy ||
    ...
    if (_object_) {
      ...
      if (ea && !ea->g_Alive())
        ...
        CActor *pAct =
            smart_cast<CActor *>(Level().Objects.net_Find(m_pInvOwnerActorID));
        CHelmet *helm =
            smart_cast<CHelmet *>(pAct->inventory().ItemFromSlot(HELMET_SLOT));
  ...
  if (m_b_visible) {
    ...
    if (Level().MapManager().GetMapLocationsForObject(m_objectID,
                                                      find_locations_res)) {
