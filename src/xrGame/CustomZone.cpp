...
void CCustomZone::Load(LPCSTR section) {
  ...
  bool use =
      !!READ_IF_EXISTS(pSettings, r_bool, section, "use_secondary_hit", false);
...
BOOL CCustomZone::net_Spawn(CSE_Abstract *DC) {
  ...
  bool br1 =
      (0 == psDeviceFlags.test(rsR2 | rsR3 | rsR4)); // Alundaio: rsR4 flag
...
void CCustomZone::feel_touch_delete(CObject *O) {
  ...
  OBJECT_INFO_VEC_IT it =
      std::find(m_ObjectInfoMap.begin(), m_ObjectInfoMap.end(), pGameObject);
...
void CCustomZone::PlayIdleParticles(bool bIdleLight) {
  ...
  if (*m_sIdleParticles) {
    if (!m_pIdleParticles) {
      m_pIdleParticles =
          CParticlesObject::Create(m_sIdleParticles.c_str(), FALSE);
  ...
  if (particle_str.size()) {
    ...
    if (PP) {
      ...
      if (play_bone != BI_NONE)
        PP->StartParticles(particle_str, play_bone, Fvector().set(0, 1, 0),
                           ID());
    }
  }
}
...
void CCustomZone::PlayEntranceParticles(CGameObject *pObject) {
  ...
  if (PP) {
    ...
    if (play_bone != BI_NONE) {
      CParticlesObject *pParticles =
          CParticlesObject::Create(particle_str, TRUE);
      Fmatrix xform;
      Fvector dir;
  ...
  for (; it != it_e; ++it) {
    ...
    case 0: // sphere
    {
      ...
      for (float i = 0; i < ki; ++i) {
        ...
        pParticles =
            CParticlesObject::Create(m_sBoltEntranceParticles.c_str(), TRUE);
...
void CCustomZone::CreateHit(u16 id_to, u16 id_from, const Fvector &hit_dir,
                            float hit_power, s16 bone_id,
                            const Fvector &pos_in_bone, float hit_impulse,
  ...
  if (OnServer()) {
    ...
    SHit Hit = SHit(hit_power, hdir, this, bone_id, pos_in_bone, hit_impulse,
                    hit_type, 0.0f, false);
