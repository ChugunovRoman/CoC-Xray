...
private:
  IC bool collided(const CObject *object, const Fvector &position,
  ...
  void fill_static(obstacles_query &query, const Fvector &position,
                   const float &radius);
  void fill_static(obstacles_query &query);
  void fill_all_static(moving_object *object, const Fvector &dest_position);
  void fill_nearest_list(const Fvector &position, const float &radius,
...
private:
  ...
  void resolve_collision_previous(boxes &current, moving_object *object0,
                                  moving_object *object1,
                                  possible_actions &action) const;
  void resolve_collision_first(boxes &current, moving_object *object0,
                               moving_object *object1,
                               possible_actions &action) const;
  void resolve_collision(boxes &current, moving_object *object0,
                         const Fvector &position0, moving_object *object1,
                         const Fvector &position1,
                         possible_actions &action) const;
  bool collided_dynamic(moving_object *object0, const Fvector &position0,
  ...
  bool collided_dynamic(moving_object *object0, const Fvector &position0,
                        moving_object *object1, const Fvector &position1,
                        possible_actions &action) const;
  bool exchange_all(moving_object *previous, moving_object *next,
