...
void S2DVert::rotate_pt(const Fvector2 &pivot, const float cosA,
                        const float sinA, const float kx) {
  Fvector2 t = pt;
  t.sub(pivot);
...
void ui_core::ClientToScreenScaled(Fvector2 &dest, float left,
                                   float top) const {
...
void ui_core::PushScissor(const Frect &r_tgt, bool overlapped) {
  ...
  if (!(result.x1 >= 0 && result.y1 >= 0 && result.x2 <= UI_BASE_WIDTH &&
    ...
    Msg("! r_tgt [%.3f][%.3f][%.3f][%.3f]", r_tgt.x1, r_tgt.y1, r_tgt.x2,
        r_tgt.y2);
    Msg("! result [%.3f][%.3f][%.3f][%.3f]", result.x1, result.y1, result.x2,
        result.y2);
...
bool ui_core::is_widescreen() {
  return (Device.dwWidth) / float(Device.dwHeight) >
