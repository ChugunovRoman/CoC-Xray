...
float stalker_movement_manager_smart_cover::enter_path(
    LoopholePath *result, Fvector const &position, u32 const level_vertex_id,
  ...
  for (; i != e; ++i) {
    ...
    new_value +=
        ai().graph_engine().m_string_algorithm->data_storage().get_best().g();
    if (new_value >= value)
      continue;
...
float stalker_movement_manager_smart_cover::exit_path_weight(
    u32 const &source_node, Fvector const &source_position,
...
bool stalker_movement_manager_smart_cover::test_pick(
    Fvector source, Fvector destination) const {
  source.y += 2.f;
  destination.y += 2.f;
  ...
  struct test_pick {
    static BOOL callback(collide::rq_result &result, LPVOID user_data) {
      ...
      if (param->m_object->feel_vision_mtl_transp(result.O, result.element) <
          1.f) {
...
stalker_movement_manager_smart_cover::nearest_action(
    smart_cover::cover const &cover, shared_str const &loophole_id0,
...
void stalker_movement_manager_smart_cover::build_exit_path_to_cover() {
  ...
  Fvector target_position;
  target_cover.object().XFORM().transform_tiny(target_position,
                                               target_loophole.fov_position());
  ...
  for (; I != E; ++I) {
    ...
    new_value +=
        enter_path(0, exit_position, exit_vertex_id, target_cover,
...
void stalker_movement_manager_smart_cover::try_actualize_path() {
  ...
  shared_str target_loophole_id = smart_cover::transform_vertex(
      (m_target.cover() == m_current.cover()) ? m_target.cover_loophole()->id()
                                              : "",
      false);
...
loophole const &stalker_movement_manager_smart_cover::loophole(
  ...
  typedef smart_cover::cover::Loopholes Loopholes;
  Loopholes const &loopholes = cover.description()->loopholes();
  Loopholes::const_iterator i = std::find_if(
      loopholes.begin(), loopholes.end(), loophole_id_predicate(loophole_id));
