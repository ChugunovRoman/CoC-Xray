...
void buffers_vector_reader::accumulate_size() {
  for (buffers_vector_t::iterator i = m_buffers.begin(), ie = m_buffers.end();
       i != ie; ++i) {
    m_sum_size += i->second;
...
void buffers_vector_reader::read_from_current_buf(NET_Packet &dest,
  ...
  dest.w(static_cast<void *>(m_buffers.front().first + m_current_buf_offs),
         read_size);
...
memory_writer_reader::memory_writer_reader(CMemoryWriter *src_writer,
                                           u32 const max_size)
    : m_writer_as_src(src_writer), m_writer_pointer(0),
...
bool memory_writer_reader::make_data_packet(NET_Packet &packet,
  ...
  // received ready_to_send method will return true because tell() < size(). and
  // on transfer updates elapsed will be 0 to avoid this situation we make
  // return
  // ...
  if (!elapsed)
...
filetransfer_node::filetransfer_node(shared_str const &file_name,
    ...
    : m_chunk_size(chunk_size), m_last_peak_throughput(0),
      m_last_chunksize_update_time(0), m_user_param(0),
      m_process_callback(callback) {
  m_reader = xr_new<disk_file_reader>(file_name);
}

filetransfer_node::filetransfer_node(u8 *data, u32 const data_size,
                                     u32 const chunk_size,
                                     sending_state_callback_t const &callback,
                                     u32 user_param)
    : m_chunk_size(chunk_size), m_last_peak_throughput(0),
      m_last_chunksize_update_time(0), m_user_param(user_param),
      m_process_callback(callback) {
  m_reader = xr_new<memory_reader>(data, data_size);
}

filetransfer_node::filetransfer_node(CMemoryWriter *src_writer,
                                     u32 const max_size, u32 const chunk_size,
                                     sending_state_callback_t const &callback,
                                     u32 user_param)
    : m_chunk_size(chunk_size), m_last_peak_throughput(0),
      m_last_chunksize_update_time(0), m_user_param(user_param),
...
filetransfer_node::filetransfer_node(
    buffer_vector<mutable_buffer_t> *vector_of_buffers, u32 const chunk_size,
