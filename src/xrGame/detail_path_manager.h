...
private:
  enum EDirectionType {
    ...
    eDirectionTypePP = eDirectionTypeFP |
                       eDirectionTypeSP, // both linear velocities are positive
    eDirectionTypeNN = eDirectionTypeFN |
                       eDirectionTypeSN, // both linear velocities are negative
    eDirectionTypePN = eDirectionTypeFP |
                       eDirectionTypeSN, // the first linear velocity is
                                         // positive, the second one - negative
    eDirectionTypeNP = eDirectionTypeFN |
                       eDirectionTypeSP, // the first linear velocity is
                                         // negative, the second one - positive
  };
...
private:
  ...
  IC void adjust_point(const Fvector2 &source, float yaw, float magnitude,
                       Fvector2 &dest) const;
  IC void assign_angle(float &angle, const float start_yaw,
                       const float dest_yaw, const bool positive,
  ...
  void postprocess_key_points(const xr_vector<u32> &level_path,
                              ...
                              const u32 straight_line_index,
                              const u32 straight_line_index_negative);
  void build_path_via_key_points(STrajectoryPoint &start,
                                 STrajectoryPoint &dest,
                                 xr_vector<STravelParamsIndex> &finish_params,
                                 const u32 straight_line_index,
                                 const u32 straight_line_index_negative);
...
public:
  ...
  u32 location_on_path(const CGameObject *object, float distance,
