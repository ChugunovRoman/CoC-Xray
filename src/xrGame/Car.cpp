...
void CCar::ActorObstacleCallback(bool &do_colide, bool bo1, dContact &c,
...
void CCar::SpawnInitPhysics(CSE_Abstract *D) {
  ...
  K->CalculateBones(TRUE);
  Init(); // inits m_driving_wheels,m_steering_wheels,m_breaking_wheels values
          // using recieved in ParceDefinitions & from bone_map
...
void CCar::ParseDefinitions() {
  ...
  m_power_increment_factor =
      READ_IF_EXISTS(ini, r_float, "car_definition", "power_increment_factor",
                     m_power_increment_factor);
  m_rpm_increment_factor =
      READ_IF_EXISTS(ini, r_float, "car_definition", "rpm_increment_factor",
                     m_rpm_increment_factor);
  m_power_decrement_factor =
      READ_IF_EXISTS(ini, r_float, "car_definition", "power_decrement_factor",
                     m_power_increment_factor);
...
void CCar::Init() {
  ...
  if (ini->line_exist("car_definition", "steer")) {
    ...
    m_bone_steer =
        pKinematics->LL_BoneID(ini->r_string("car_definition", "steer"));
    VERIFY2(
        fsimilar(DET(pKinematics->LL_GetTransform(m_bone_steer)), 1.f, EPS_L),
        "BBADD MTX");
  ...
  // ref_wheel.Init();
  m_ref_radius =
      ini->r_float("car_definition", "reference_radius"); // ref_wheel.radius;
  b_exploded = false;
  b_engine_on = false;
  b_clutch = false;
  b_starting = false;
  ...
  m_root_transform.set(
      bone_map.find(pKinematics->LL_GetBoneRoot())->second.element->mXFORM);
...
bool CCar::Use(const Fvector &pos, const Fvector &dir,
               const Fvector &foot_pos) {
  xr_map<u16, SDoor>::iterator i;
...
void CCar::InitParabola() {
  ...
  m_a = expf((m_power_rpm - m_torque_rpm) / (2.f * m_power_rpm)) * m_max_power /
        m_power_rpm;
  m_b = m_torque_rpm;
...
void CCar::PhDataUpdate(float step) {
  ...
  m_steer_angle =
      m_steering_wheels.begin()->GetSteerAngle() * 0.1f + m_steer_angle * 0.9f;
...
IC void CCar::fill_wheel_vector(LPCSTR S, xr_vector<T> &type_wheels) {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    if (J == bone_map.end()) {
      ...
      SWheel &wheel =
          (m_wheels_map.insert(mk_pair(bone_id, SWheel(this)))).first->second;
