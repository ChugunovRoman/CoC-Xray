...
bool CScriptGameObject::deadbody_can_take_status() {
  ...
  if (!inventoryOwner) {
    ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                    "CInventoryOwner : cannot access class "
                                    "member deadbody_can_take_status!");
    return (0);
  }
...
float CScriptGameObject::GetCurrentOutfitProtection(int hit_type) {
  ...
  if (!inventoryOwner) {
    ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                    "CInventoryOwner : cannot access class "
                                    "member GetCurrentOutfitProtection!");
    return (0);
  }
  CGameObject *current_equipment = inventoryOwner->GetOutfit();
...
void CScriptGameObject::add_animation(LPCSTR animation, bool hand_usage,
  ...
  if (stalker->animation().global_selector()) {
    ai().script_engine().script_log(
        eLuaMessageTypeError,
        "Cannot add animation [%s]: global selector is set for object [%s], "
        "in_smart_cover returned [%s]!",
...
u32 CScriptGameObject::get_dest_game_vertex_id() {
  ...
  if (!stalker)
    ai().script_engine().script_log(
        ScriptStorage::eLuaMessageTypeError,
        "CAI_Stalker : cannot access class member get_dest_level_vertex_id!");
  else
...
void CScriptGameObject::set_dest_level_vertex_id(u32 level_vertex_id) {
  ...
  if (!stalker)
    ...
    if (!stalker->movement().restrictions().accessible(level_vertex_id)) {
      ai().script_engine().script_log(
          ScriptStorage::eLuaMessageTypeError,
          "! you are trying to setup destination for the stalker %s, which is "
          "not accessible by its restrictors in[%s] out[%s]",
          stalker->cName().c_str(),
          Level()
...
void CScriptGameObject::set_desired_position(const Fvector *desired_position) {
  ...
  if (!stalker)
    ...
    THROW2(desired_position ||
               stalker->movement().restrictions().accessible(*desired_position),
           *stalker->cName());
...
void CScriptGameObject::set_sight(SightManager::ESightType sight_type,
                                  Fvector *vector3d, u32 dwLookOverDelay) {
  CAI_Stalker *stalker = smart_cast<CAI_Stalker *>(&object());
  if (!stalker)
    ai().script_engine().script_log(
        ScriptStorage::eLuaMessageTypeError,
    ...
    if ((sight_type == SightManager::eSightTypeDirection) && vector3d &&
      ...
      VERIFY2(false, make_string("non-normalized direction passed [%f][%f][%f]",
                                 VPUSH(*vector3d)));
      vector3d->normalize();
    }
...
void CScriptGameObject::set_sight(SightManager::ESightType sight_type,
                                  Fvector &vector3d, bool torso_look = false) {
  CAI_Stalker *stalker = smart_cast<CAI_Stalker *>(&object());
  if (!stalker)
    ai().script_engine().script_log(
        ScriptStorage::eLuaMessageTypeError,
...
void CScriptGameObject::set_sight(CScriptGameObject *object_to_look,
                                  bool torso_look, bool fire_object) {
  CAI_Stalker *stalker = smart_cast<CAI_Stalker *>(&object());
  if (!stalker)
    ai().script_engine().script_log(
        ScriptStorage::eLuaMessageTypeError,
...
void CScriptGameObject::set_sight(CScriptGameObject *object_to_look,
  ...
  if (!stalker)
    ...
    stalker->sight().setup(CSightAction(&object_to_look->object(), torso_look,
                                        fire_object, no_pitch));
}
...
void CScriptGameObject::patrol_path_make_inactual() {
  ...
  if (!monster) {
    ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                    ...
                                    "member patrol_path_make_inactual!");
    return;
  }
...
void CScriptGameObject::make_object_visible_somewhen(
  ...
  if (!entity_alive) {
    ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                    "CEntityAlive : cannot access class member "
                                    "make_object_visible_somewhen!");
    return;
  }
...
void CScriptGameObject::sell_condition(CScriptIniFile *ini_file,
  ...
  inventory_owner->trade_parameters().process(CTradeParameters::action_sell(0),
                                              *ini_file, section);
}
...
void CScriptGameObject::buy_condition(CScriptIniFile *ini_file,
  ...
  inventory_owner->trade_parameters().process(CTradeParameters::action_buy(0),
                                              *ini_file, section);
}
...
void CScriptGameObject::show_condition(CScriptIniFile *ini_file,
  ...
  inventory_owner->trade_parameters().process(CTradeParameters::action_show(0),
                                              *ini_file, section);
}
...
void CScriptGameObject::buy_item_condition_factor(float factor) {
  ...
  if (!inventory_owner) {
    ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                    "CInventoryOwner : cannot access class "
                                    "member buy_item_condition_factor!");
    return;
  }
...
void buy_condition(CScriptIniFile *ini_file, LPCSTR section) {
  default_trade_parameters().process(CTradeParameters::action_buy(0), *ini_file,
                                     section);
}
...
void CScriptGameObject::SetBoneVisible(LPCSTR bone_name, bool bVisibility,
                                       bool bRecursive) {
...
void CScriptGameObject::IterateFeelTouch(
