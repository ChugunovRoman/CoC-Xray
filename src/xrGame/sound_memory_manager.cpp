...
void CSoundMemoryManager::reload(LPCSTR section) {
  ...
  m_decrease_factor =
      READ_IF_EXISTS(pSettings, r_float, section, "self_decrease_factor", .95f);

  LPCSTR sound_perceive_section = READ_IF_EXISTS(
      pSettings, r_string, section, "sound_perceive_section", section);
  m_weapon_factor = READ_IF_EXISTS(pSettings, r_float, sound_perceive_section,
                                   "weapon", 10.f);
  m_item_factor =
      READ_IF_EXISTS(pSettings, r_float, sound_perceive_section, "item", 1.f);
  m_npc_factor =
      READ_IF_EXISTS(pSettings, r_float, sound_perceive_section, "npc", 1.f);
  m_anomaly_factor = READ_IF_EXISTS(pSettings, r_float, sound_perceive_section,
                                    "anomaly", 1.f);
  m_world_factor =
      READ_IF_EXISTS(pSettings, r_float, sound_perceive_section, "world", 1.f);
}
...
IC void CSoundMemoryManager::update_sound_threshold() {
  ...
  m_sound_threshold =
      _max(m_self_sound_factor * m_sound_threshold *
  ...
  if (sound_power >= m_sound_threshold) {
    ...
    if (!m_stalker || !m_stalker->memory().enemy().selected())
      ...
      if (object) {
        ...
        //<= COMBAT_SOUND_PERCEIVE_RADIUS_SQR; 				if
        //(is_shooting || is_colliding
        //|| very_close)
        add(object, sound_type, position, sound_power);
      }
    }
...
void CSoundMemoryManager::add(const CSoundObject &sound_object,
  ...
  if (check_for_existance) {
    if (m_sounds->end() != std::find(m_sounds->begin(), m_sounds->end(),
  ...
  if (m_max_sound_count <= m_sounds->size()) {
    xr_vector<CSoundObject>::iterator I = std::min_element(
        m_sounds->begin(), m_sounds->end(), SLevelTimePredicate<CGameObject>());
...
void CSoundMemoryManager::add(const CObject *object, int sound_type,
  ...
  if (object && object->H_Parent() &&
      (me->tfGetRelationType(smart_cast<const CEntityAlive *>(
           object->H_Parent())) == ALife::eRelationTypeFriend))
    return;
#endif
  ...
  if (entity_alive && me &&
      (me->tfGetRelationType(entity_alive) == ALife::eRelationTypeFriend))
    return;
#endif
...
struct CRemoveSoundObjectPredicate {
  ...
  CRemoveSoundObjectPredicate(const MemorySpace::CSoundObject *object)
      : m_object(object) {}
...
void CSoundMemoryManager::remove(
    const MemorySpace::CSoundObject *sound_object) {
  SOUNDS::iterator I = std::find_if(m_sounds->begin(), m_sounds->end(),
                                    CRemoveSoundObjectPredicate(sound_object));
  if (I != m_sounds->end())
    m_sounds->erase(I);
}
