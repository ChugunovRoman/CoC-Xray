...
void HUD_SOUND_ITEM::LoadSound(LPCSTR section, LPCSTR line,
  ...
  // if (!bLoaded)
  // Msg("[WARNING] HUD_SOUND_ITEM::LoadSound section=%s line=%s does not
  // exist!", section, line);
}

void HUD_SOUND_ITEM::LoadSound(LPCSTR section, LPCSTR line, ref_sound &snd,
                               int type, float *volume, float *delay) {
...
void HUD_SOUND_ITEM::PlaySound(HUD_SOUND_ITEM &hud_snd, const Fvector &position,
                               const CObject *parent, bool b_hud_mode,
                               bool looped, u8 index) {
  ...
  if (hud_snd.m_b_exclusive) {
    hud_snd.m_activeSnd->snd.play_at_pos(const_cast<CObject *>(parent),
                                         flags & sm_2D ? Fvector().set(0, 0, 0)
                                                       : position,
                                         flags, hud_snd.m_activeSnd->delay);
  } else {
  ...
  // * b_hud_mode?psHUDSoundVolume:1.0f);
  hud_snd.m_activeSnd->snd.set_volume(hud_snd.m_activeSnd->volume *
                                      (b_hud_mode ? psHUDSoundVolume : 1.0f));
}
...
void HUD_SOUND_COLLECTION::PlaySound(LPCSTR alias, const Fvector &position,
                                     const CObject *parent, bool hud_mode,
                                     bool looped, u8 index) {
  xr_vector<HUD_SOUND_ITEM>::iterator it = m_sound_items.begin();
  xr_vector<HUD_SOUND_ITEM>::iterator it_e = m_sound_items.end();
  for (; it != it_e; ++it) {
  ...
  if (snd_item)
    HUD_SOUND_ITEM::PlaySound(*snd_item, position, parent, hud_mode, looped,
                              index);
}
...
void HUD_SOUND_COLLECTION::LoadSound(LPCSTR section, LPCSTR line, LPCSTR alias,
...
void HUD_SOUND_COLLECTION_LAYERED::LoadSound(LPCSTR section, LPCSTR line,
                                             LPCSTR alias, bool exclusive,
                                             int type) {
  LPCSTR str = pSettings->r_string(section, line);
  string256 buf_str;
