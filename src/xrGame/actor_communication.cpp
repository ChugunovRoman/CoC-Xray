...
void CActor::UpdateAvailableDialogs(CPhraseDialogManager *partner) {
  ...
  for (u32 i = 0; i < pInvOwnerPartner->CharacterInfo().ActorDialogs().size();
       i++)
    AddAvailableDialog(pInvOwnerPartner->CharacterInfo().ActorDialogs()[i],
                       partner);
  ...
  if (GO) {
    ...
    for (int t = ALife::eRelationTypeFriend; t < ALife::eRelationTypeLast;
      ...
      Level().MapManager().RemoveMapLocation(
          RELATION_REGISTRY().GetSpotName(tt), GO->ID());
    }
