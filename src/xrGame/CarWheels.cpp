...
void CCar::SWheel::WheellCollisionCallback(bool &do_colide, bool bo1,
  ...
  if (ini->section_exist(section)) {
    collision_params.damping_factor =
        READ_IF_EXISTS(ini, r_float, section, "damping_factor",
                       collision_params.damping_factor);
    collision_params.spring_factor = READ_IF_EXISTS(
        ini, r_float, section, "spring_factor", collision_params.spring_factor);
    collision_params.mu_factor = READ_IF_EXISTS(
        ini, r_float, section, "friction_factor", collision_params.mu_factor);
...
void CCar::SWheelBreak::Load(LPCSTR section) {
  ...
  if (ini->section_exist(section)) {
    break_torque =
        READ_IF_EXISTS(ini, r_float, section, "break_torque", break_torque);
    hand_break_torque = READ_IF_EXISTS(ini, r_float, section,
                                       "hand_break_torque", hand_break_torque);
  }
}
