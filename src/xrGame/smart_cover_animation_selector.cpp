...
animation_selector::animation_selector(CAI_Stalker *object)
    : m_object(object), m_callback_called(false), m_first_time(true),
...
MotionID
animation_selector::select_animation(bool &animation_movement_controller) {
  ...
  if (m_callback_called) {
    if (m_planner->initialized()) {
      ...
      if (!m_planner->initialized()) {
        //				Msg				("%6d no
        //planner update, planner is not initialized, exiting",
        // Device.dwTimeGlobal);
