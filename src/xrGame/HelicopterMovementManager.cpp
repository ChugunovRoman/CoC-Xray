...
void SHeliMovementState::CreateRoundPoints(Fvector center, float radius,
                                           float start_h, float end_h,
...
void SHeliMovementState::goByRoundPath(Fvector center_, float radius_,
                                       bool clockwise_) {
  if (type == eMovRoundPath)
  ...
  for (; it != it_e; ++it, ++pt_idx) {
    ...
    CPatrolPoint pt =
        CPatrolPoint((CLevelGraph *)0, (CGameLevelCrossTable *)0,
                     (CGameGraph *)0, pp, (*it).point, u32(-1), 0, pt_name);
...
void CHelicopter::OnRender() {
  /*
     ...
     m_movement.currPatrolPath->vertices().end(); for ( ; b != e; ++b) { Fvector
     p = (*b).second->data().position(); Level().debug_renderer().draw_aabb
