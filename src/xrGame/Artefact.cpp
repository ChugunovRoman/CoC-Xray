...
#define FASTMODE_DISTANCE                                                      \
  (50.f) // distance to camera from sphere, when zone switches to fast update
         // sequence
...
void CArtefact::Load(LPCSTR section) {
  ...
  if (pSettings->section_exist(hit_sect)) {
    m_ArtefactHitImmunities[ALife::eHitTypeBurn] =
        READ_IF_EXISTS(pSettings, r_float, hit_sect, "burn_immunity", 0.f);
    m_ArtefactHitImmunities[ALife::eHitTypeStrike] =
        READ_IF_EXISTS(pSettings, r_float, hit_sect, "strike_immunity", 0.f);
    m_ArtefactHitImmunities[ALife::eHitTypeShock] =
        READ_IF_EXISTS(pSettings, r_float, hit_sect, "shock_immunity", 0.f);
    m_ArtefactHitImmunities[ALife::eHitTypeWound] =
        READ_IF_EXISTS(pSettings, r_float, hit_sect, "wound_immunity", 0.f);
    m_ArtefactHitImmunities[ALife::eHitTypeRadiation] =
        READ_IF_EXISTS(pSettings, r_float, hit_sect, "radiation_immunity", 0.f);
    m_ArtefactHitImmunities[ALife::eHitTypeTelepatic] = READ_IF_EXISTS(
        pSettings, r_float, hit_sect, "telepathic_immunity", 0.f);
    m_ArtefactHitImmunities[ALife::eHitTypeChemicalBurn] = READ_IF_EXISTS(
        pSettings, r_float, hit_sect, "chemical_burn_immunity", 0.f);
    m_ArtefactHitImmunities[ALife::eHitTypeExplosion] =
        READ_IF_EXISTS(pSettings, r_float, hit_sect, "explosion_immunity", 0.f);
    m_ArtefactHitImmunities[ALife::eHitTypeFireWound] = READ_IF_EXISTS(
        pSettings, r_float, hit_sect, "fire_wound_immunity", 0.f);
    m_ArtefactHitImmunities[ALife::eHitTypeLightBurn] =
        m_ArtefactHitImmunities[ALife::eHitTypeBurn];
  }
...
void CArtefact::StartLights() {
  ...
  bool const b_light_shadow =
      !!pSettings->r_bool(cNameSect(), "idle_light_shadow");
...
void CArtefact::FollowByPath(LPCSTR path_name, int start_idx,
                             Fvector magic_force) {
  if (m_detectorObj)
...
SArtefactDetectorsSupport::SArtefactDetectorsSupport(CArtefact *A)
    : m_parent(A), m_currPatrolPath(NULL), m_currPatrolVertex(NULL),
      m_switchVisTime(0) {}
...
void SArtefactDetectorsSupport::SetVisible(bool b) {
  ...
  if (b) {
    LPCSTR curr =
        pSettings->r_string(m_parent->cNameSect().c_str(),
...
void SArtefactDetectorsSupport::Blink() {
  LPCSTR curr =
      pSettings->r_string(m_parent->cNameSect().c_str(), "det_show_particles");
...
void SArtefactDetectorsSupport::FollowByPath(LPCSTR path_name, int start_idx,
                                             Fvector force) {
