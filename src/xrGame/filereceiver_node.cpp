...
filereceiver_node::filereceiver_node(shared_str const &file_name,
    ...
    : m_file_name(file_name), m_is_writer_memory(false),
...
filereceiver_node::filereceiver_node(CMemoryWriter *mem_writer,
                                     receiving_state_callback_t const &callback)
    : m_is_writer_memory(true), m_process_callback(callback),
      m_last_read_time(0) {
...
void split_received_to_buffers(u8 *data_ptr, u32 data_size,
