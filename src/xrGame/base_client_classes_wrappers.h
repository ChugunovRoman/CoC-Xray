...
template <typename _1, typename _2> struct heritage {
  ...
  typedef
      typename Loki::GenLinearHierarchy<pure_tl, linear_registrator>::LinBase
          result;
};
...
public:
  ...
  /*	static float			shedule_Scale_static	(base *self)
          {
                  ai().script_engine().script_log(eLuaMessageTypeError,"You are
     trying to call a pure virtual function ISheduled::shedule_Scale!\nReturning
     default value 1000.0"); return	(1000.f);
          }
  */
  ...
  /*	static  void			shedule_Update_static	(base *self, u32
     dt)
          {
...
public:
  ...
  /*
          ...
          static	void	renderable_Render_static
     (IRenderable *self)
          {
                  ai().script_engine().script_log(eLuaMessageTypeError,"You are
     trying to call a pure virtual function IRenderable::renderable_Render!");
          }

          virtual	BOOL	renderable_ShadowGenerate		()
          {
                  return
     ((BOOL)call_member<bool>(this,"renderable_ShadowGenerate"));
          }

          static	bool	renderable_ShadowGenerate_static(IRenderable
     *self)
          {
                  return		(!!
     self->IRenderable::renderable_ShadowGenerate());
          }

          virtual	BOOL	renderable_ShadowReceive		()
          {
                  return
     ((BOOL)call_member<bool>(this,"renderable_ShadowReceive"));
          }

          static	bool	renderable_ShadowReceive_static (IRenderable
     *self)
          {
                  return		(!!
     self->IRenderable::renderable_ShadowReceive());
          }
  */
};
...
// BoundingBox			()
// const; 	virtual void			Load (LPCSTR section); 	virtual
// void UpdateCL			();
...
//(NET_Packet&
// P); 	virtual	void			net_ImportInput		(NET_Packet& P);
...
// OnH_B_Chield		(); 	virtual void OnH_B_Independent	(bool
// just_before_destroy); 	virtual void OnH_A_Chield		();
...
public:
  ...
  static void HitSignal_static(CEntity *self, float P, Fvector &local_dir,
  ...
  static void HitImpulse_static(float P, Fvector &vWorldDir,
                                Fvector &vLocalDir) {
    ai().script_engine().script_log(
        eLuaMessageTypeError,
