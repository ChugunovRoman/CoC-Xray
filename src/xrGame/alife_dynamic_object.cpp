...
bool CSE_ALifeDynamicObject::synchronize_location() {
  ...
  if (tGraphID != m_tGraphID) {
    if (!m_bOnline) {
      ...
      if (ai().level_graph().inside(ai().level_graph().vertex(level_vertex_id),
                                    position)) {
...
void CSE_ALifeInventoryBox::add_offline(
  ...
  for (u32 i = 0, n = saved_children.size(); i < n; ++i) {
    ...
    Msg("[LSS][%d] Going offline [%d][%s][%d] with parent [%d][%s] on '%s'",
        Device.dwFrame, Device.dwTimeGlobal,
        inventory_item->base()->name_replace(), inventory_item->base()->ID, ID,
        name_replace(), "*SERVER*");
#endif
...
void CSE_ALifeDynamicObject::clear_client_data() {
  ...
  if (!client_data.empty())
    Msg("CSE_ALifeDynamicObject::switch_offline: client_data is cleared for "
        "[%d][%s]",
        ID, name_replace());
#endif // DEBUG
