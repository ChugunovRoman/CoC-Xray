...
void CStalkerActionReachWounded::initialize() {
  ...
  object().set_goal(eObjectActionIdle, weapon_to_kill(&object()), MIN_QUEUE,
...
void CStalkerActionKillWounded::execute() {
  ...
  const CEntityAlive *enemy = object().memory().enemy().selected();
  object().sight().setup(CSightAction(enemy, true));
  object().set_goal(eObjectActionFire1, weapon_to_kill(&object()), MIN_QUEUE,
                    MAX_QUEUE, MIN_INTERVAL, MAX_INTERVAL);
  ...
  HS.power = 1.f;
  HS.boneID =
      smart_cast<IKinematics *>((const_cast<CEntityAlive *>(enemy))->Visual())
          ->LL_GetBoneRoot();
