...
** GAMES **
**********/
static void piSBGamesListCallback(SBServerListPtr serverlist,
...
static void piSBGamesEngineCallback(SBQueryEnginePtr engine,
...
PEERBool piSBStartListingGames(PEER peer, const unsigned char *fields,
  ...
  for (i = 0; i < numFields; i++) {
    ...
    // Add the key.
    ///////////////
    listLen += sprintf(fullFields + listLen, "\\%s",
                       qr2_registered_key_list[fields[i]]);
  ...
  ///////////////////////////
  if (SBServerListConnectAndQuery(&connection->gameList, fullFields,
                                  smartSpyFilter, PUSH_UPDATES,
                                  0) != sbe_noerror) {
    piSBStopListingGames(peer);
    return PEERFalse;
  }
...
** GROUPS **
***********/
static void piSBGroupsListCallback(SBServerListPtr serverlist,
                                   SBListCallbackReason reason, SBServer server,
                                   void *instance) {
  ...
  case slc_serveradded: {
    ...
    //#else
    //		const unsigned short * name = SBServerGetStringValue(server,
    // L"hostname", L"(No Name)"); 		int numWaiting =
    // SBServerGetIntValue(server, L"numwaiting", 0); 		int maxWaiting
    // = SBServerGetIntValue(server, L"maxwaiting", 0); 		int numGames =
    // SBServerGetIntValue(server, L"numservers", 0); 		int numPlaying
    // = SBServerGetIntValue(server, L"numplayers", 0); #endif
  ...
  case slc_initiallistcomplete:
    ...
    piAddListGroupRoomsCallback(peer, PEERTrue, 0, NULL, NULL, 0, 0, 0, 0,
                                (peerListGroupRoomsCallback)operation->callback,
                                operation->callbackParam, operation->ID);
...
PEERBool piSBStartListingGroups(PEER peer, const char *fields) {
  ...
  // Start updating the list.
  ///////////////////////////
  if (SBServerListConnectAndQuery(&connection->groupList, fullFields, "",
                                  SEND_GROUPS, 0) != sbe_noerror) {
    piSBStopListingGroups(peer);
    return PEERFalse;
  }
...
static PEERBool piIsLocalServer(PEER peer, SBServer server) {
  ...
  if (SBServerHasPrivateAddress(server)) {
    ...
    if (SBServerGetPrivateQueryPort(server) !=
        connection->autoMatchOperation->port)
      return PEERFalse;
  }
  // check the public port
  else {
    if (SBServerGetPublicQueryPort(server) !=
        connection->autoMatchOperation->port)
      return PEERFalse;
  }
...
static int piSBAutoMatchGetServerRating(PEER peer, SBServer server) {
  ...
  if (SBServerGetIntValue(server, "numplayers", 0) >=
    ...
    return 0;
#else
  if (SBServerGetIntValue(server, L"numplayers", 0) >=
      SBServerGetIntValue(server, L"maxplayers", 0))
    return 0;
#endif
...
static void piSBAutoMatchCheckPushedServer(PEER peer, SBServer server) {
  ...
  if (connection->autoMatchStatus != PEERWaiting &&
      connection->autoMatchStatus != PEERSearching)
    return;
  ...
  // rate each of them and join the one with the highest rating.
    ...
    if (!piJoinAutoMatchRoom(peer,
                             SBServerListNth(&connection->autoMatchList, 0))) {
      // it's bad news if it failed to start the op
      piSetAutoMatchStatus(peer, PEERFailed);
    }
  }
...
static void piSBAutoMatchListCallback(SBServerListPtr serverlist,
  ...
  case slc_serveradded:
    ...
    else // send normal update
    {
      ...
      if (serverlist->backendgameflags & QR2_USE_QUERY_CHALLENGE)
        usequerychallenge = SBTrue;
      SBQueryEngineUpdateServer(&connection->autoMatchEngine, server, 0,
                                QTYPE_FULL, usequerychallenge);
    }
    break;
  ...
  case slc_queryerror:
    ...
    if (connection->autoMatchStatus == PEERSearching)
      piSetAutoMatchStatus(peer, connection->autoMatchQRFailed ? PEERFailed
                                                               : PEERWaiting);
    break;
...
static void piSBAutoMatchEngineCallback(SBQueryEnginePtr engine,
  ...
  case qe_engineidle:
    ...
    if (connection->inRoom[StagingRoom] ||
        connection->enteringRoom[StagingRoom])
      return;
...
PEERBool piSBStartListingAutoMatches(PEER peer) {
  ...
  // start updating the list.
  if (SBServerListConnectAndQuery(&connection->autoMatchList, NULL,
                                  connection->autoMatchFilter, PUSH_UPDATES,
                                  0) != sbe_noerror) {
    piSBStopListingAutoMatches(peer);
    connection->autoMatchSBFailed = PEERTrue;
    return PEERFalse;
...
PEERBool piSBInit(PEER peer) {
  ...
  SBServerListInit(&connection->gameList, connection->title, connection->sbName,
                   connection->sbSecretKey, connection->sbGameVersion, SBFalse,
                   piSBGamesListCallback, peer);
  SBQueryEngineInit(&connection->gameEngine, connection->sbMaxUpdates,
                    piSBQueryVersion, SBFalse, piSBGamesEngineCallback, peer);

  // init the group list
  memset(&connection->groupList, 0, sizeof(connection->groupList));
  SBServerListInit(&connection->groupList, connection->title,
                   connection->sbName, connection->sbSecretKey,
                   connection->sbGameVersion, SBFalse, piSBGroupsListCallback,
                   peer);
  ...
  SBServerListInit(&connection->autoMatchList, autoMatchTitle,
                   connection->sbName, connection->sbSecretKey,
                   connection->sbGameVersion, SBFalse,
                   piSBAutoMatchListCallback, peer);
  SBQueryEngineInit(&connection->autoMatchEngine, connection->sbMaxUpdates,
                    piSBQueryVersion, SBFalse, piSBAutoMatchEngineCallback,
                    peer);
...
void piSBUpdateGame(PEER peer, SBServer server, PEERBool fullUpdate,
  ...
  if (forceUpdateByMaster) {
    ...
    SBGetServerRulesFromMaster(&connection->gameList, server->publicip,
                               server->publicport);
    return;
  }
  ...
  if (icmpEcho && (server->flags & ICMP_IP_FLAG)) {
    ...
    SBQueryEngineUpdateServer(&connection->gameEngine, server, 1, QTYPE_ICMP,
                              usequerychallenge);
    return;
  }
  ...
  if (server->flags & UNSOLICITED_UDP_FLAG) {
    ...
    // query the server
    SBQueryEngineUpdateServer(&connection->gameEngine, server, 1,
...
void piSendNatNegotiateCookie(PEER peer, unsigned int ip, unsigned short port,
  ...
  if (peerIsAutoMatching(peer)) {
    SBSendNatNegotiateCookieToServer(&connection->autoMatchList, ip,
                                     htons(port), cookie);
  } else
    SBSendNatNegotiateCookieToServer(&connection->gameList, ip, htons(port),
                                     cookie);
}
