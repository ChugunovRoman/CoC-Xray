...
static void piQRServerKeyCallback(int key, qr2_buffer_t buffer, PEER peer) {
  ...
  if (connection->inRoom[StagingRoom] &&
      (!connection->playing ||
       (connection->reportingOptions & PEER_REPORT_INFO))) {
  ...
  if (connection->callbacks.qrServerKey)
    connection->callbacks.qrServerKey(peer, key, buffer,
                                      connection->callbacks.param);
}

static void piQRPlayerKeyCallback(int key, int index, qr2_buffer_t buffer,
                                  PEER peer) {
  piPlayer *player;
  PEER_CONNECTION;
  ...
  if (connection->inRoom[StagingRoom] &&
      (!connection->playing ||
       (connection->reportingOptions & PEER_REPORT_PLAYERS))) {
  ...
  if (connection->callbacks.qrPlayerKey)
    connection->callbacks.qrPlayerKey(peer, key, index, buffer,
                                      connection->callbacks.param);
}

static void piQRTeamKeyCallback(int key, int index, qr2_buffer_t buffer,
                                PEER peer) {
  PEER_CONNECTION;

  // pass it to the app
  if (connection->callbacks.qrTeamKey)
    connection->callbacks.qrTeamKey(peer, key, index, buffer,
                                    connection->callbacks.param);
}

static void piQRKeyListCallback(qr2_key_type type, qr2_keybuffer_t keyBuffer,
                                PEER peer) {
  PEER_CONNECTION;
  ...
  if (connection->callbacks.qrKeyList)
    connection->callbacks.qrKeyList(peer, type, keyBuffer,
                                    connection->callbacks.param);
}
...
static int piQRCountCallback(qr2_key_type type, PEER peer) {
  ...
  if (connection->callbacks.qrCount)
    return connection->callbacks.qrCount(peer, type,
                                         connection->callbacks.param);
...
static void piQRAddErrorCallbackA(qr2_error_t error, char *errorString,
                                  PEER peer) {
  PEER_CONNECTION;
  ...
  if (peerIsAutoMatching(peer)) {
    ...
    if (connection->autoMatchSBFailed &&
        (connection->autoMatchStatus != PEERStaging))
      status = PEERFailed;
    else
...
#ifndef GSI_UNICODE
    connection->callbacks.qrAddError(peer, error, errorString,
                                     connection->callbacks.param);
#else
    unsigned short *errorString_W = UTF8ToUCS2StringAlloc(errorString);
    connection->callbacks.qrAddError(peer, error, errorString_W,
                                     connection->callbacks.param);
...
#ifdef GSI_UNICODE
static void piQRAddErrorCallbackW(qr2_error_t error,
                                  unsigned short *errorString, PEER peer) {
...
static void piQRNatNegotiateCallback(int cookie, PEER peer) {
  ...
  if (connection->callbacks.qrNatNegotiateCallback)
    connection->callbacks.qrNatNegotiateCallback(peer, cookie,
                                                 connection->callbacks.param);
}

static void piQRPublicAddressCallback(unsigned int ip, unsigned short port,
                                      PEER peer) {
  PEER_CONNECTION;

  // pass it to the app
  if (connection->callbacks.qrPublicAddressCallback)
    connection->callbacks.qrPublicAddressCallback(peer, ip, port,
                                                  connection->callbacks.param);
}
...
PEERBool piStartReporting(PEER peer, SOCKET socket, unsigned short port) {
  ...
  if (socket == INVALID_SOCKET) {
    rcode = qr2_initA(
        &connection->queryReporting, NULL, PI_QUERYPORT, connection->title,
        connection->qrSecretKey, 1, connection->natNegotiate,
        piQRServerKeyCallback, piQRPlayerKeyCallback, piQRTeamKeyCallback,
        piQRKeyListCallback, piQRCountCallback, piQRAddErrorCallback, peer);
  } else {
    rcode = qr2_init_socketA(
        &connection->queryReporting, socket, port, connection->title,
        connection->qrSecretKey, 1, connection->natNegotiate,
        piQRServerKeyCallback, piQRPlayerKeyCallback, piQRTeamKeyCallback,
        piQRKeyListCallback, piQRCountCallback, piQRAddErrorCallback, peer);
  }
  ...
  ////////////////////////////////////
  qr2_register_natneg_callback(connection->queryReporting,
                               piQRNatNegotiateCallback);

  // Set the public address callback.
  ///////////////////////////////////
  qr2_register_publicaddress_callback(connection->queryReporting,
...
PEERBool piStartAutoMatchReporting(PEER peer) {
  ...
  if (operation->socket == INVALID_SOCKET) {
    rcode = qr2_initA(
        &connection->autoMatchReporting, NULL, PI_QUERYPORT, autoMatchTitle,
        connection->qrSecretKey, 1, connection->natNegotiate,
        piQRServerKeyCallback, piQRPlayerKeyCallback, piQRTeamKeyCallback,
        piQRKeyListCallback, piQRCountCallback, piQRAddErrorCallback, peer);
  } else {
    rcode = qr2_init_socketA(
        &connection->autoMatchReporting, operation->socket, operation->port,
        autoMatchTitle, connection->qrSecretKey, 1, connection->natNegotiate,
        piQRServerKeyCallback, piQRPlayerKeyCallback, piQRTeamKeyCallback,
        piQRKeyListCallback, piQRCountCallback, piQRAddErrorCallback, peer);
  ...
  // Setup the nat-negotiate callback.
  ////////////////////////////////////
  qr2_register_natneg_callback(connection->autoMatchReporting,
                               piQRNatNegotiateCallback);

  // Set the public address callback.
  ///////////////////////////////////
  qr2_register_publicaddress_callback(connection->autoMatchReporting,
                                      piQRPublicAddressCallback);
