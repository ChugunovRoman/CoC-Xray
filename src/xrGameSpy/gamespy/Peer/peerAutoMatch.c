...
void piSetAutoMatchStatus(PEER peer, PEERAutoMatchStatus status) {
  ...
  if ((status == PEERWaiting) && !connection->inRoom[StagingRoom]) {
    if (!piCreateAutoMatchRoom(peer))
      piSetAutoMatchStatus(peer, connection->autoMatchSBFailed ? PEERFailed
                                                               : PEERSearching);
    return;
  }
  ...
  case PEERWaiting:
    ...
    if (!connection->autoMatchReporting) {
      if (!piStartAutoMatchReporting(peer)) {
        piSetAutoMatchStatus(
            peer, connection->autoMatchSBFailed ? PEERFailed : PEERSearching);
        return;
      }
    }
    break;
...
static void piJoinAutoMatchRoomCallback(PEER peer, PEERBool success,
                                        PEERJoinResult result,
                                        RoomType roomType, void *param) {
  PEER_CONNECTION;

  // if we're the only one, or if there's no host, leave
  if (success && ((connection->numPlayers[StagingRoom] <= 1) ||
                  !piCountRoomOps(peer, StagingRoom, connection->nick))) {
    piLeaveRoom(peer, StagingRoom, "");
    success = PEERFalse;
  }
...
PEERBool piJoinAutoMatchRoom(PEER peer, SBServer server) {
  ...
  // start the operation.
  if (!piNewJoinRoomOperation(peer, StagingRoom, room, NULL,
                              piJoinAutoMatchRoomCallback, NULL,
                              piGetNextID(peer)))
    return PEERFalse;
...
static void piAutoMatchGetChannelCallback(CHAT chat, CHATBool success,
  ...
  if (success) {
    ...
    piSetAutoMatchStatus((PEER *)&connection, connection->autoMatchSBFailed
                                                  ? PEERFailed
                                                  : PEERSearching);
  }
}

static void piCreateAutoMatchRoomCallback(PEER peer, PEERBool success,
                                          PEERJoinResult result,
                                          RoomType roomType, void *param) {
  ...
  if (success) {
    ...
    if (connection->numPlayers[StagingRoom] <= 1) {
      ...
      chatGetChannelMode(
          connection->chat, peerGetRoomChannel(peer, StagingRoom),
          piAutoMatchGetChannelCallback, (void *)connection, CHATFalse);
...
static PEERBool piCreateAutoMatchRoom(PEER peer) {
  ...
  if (!piNewCreateStagingRoomOperation(
          peer, connection->nick, "", connection->maxPlayers, operation->socket,
          operation->port, piCreateAutoMatchRoomCallback, NULL,
          piGetNextID(peer))) {
    connection->autoMatchQRFailed = PEERTrue;
    return PEERFalse;
  }
