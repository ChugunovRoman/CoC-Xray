...
** FUNCTIONS **
**************/
#ifndef GSI_UNICODE // NOT GSI_UNICODE, these have to coincide with the chatsdk
                    // build
...
static int GS_STATIC_CALLBACK WatchKeysCompare(const void *elem1,
                                               const void *elem2) {
...
static int GS_STATIC_CALLBACK WatchCacheCompare(const void *elem1,
                                                const void *elem2) {
  int rcode;
...
PEERBool piKeysInit(PEER peer) {
  ...
  connection->globalWatchCache =
      TableNew(sizeof(piCacheKey), PI_WATCH_CACHE_NUM_BUCKETS, WatchCacheHash,
               WatchCacheCompare, WatchCacheFree);
  if (!connection->globalWatchCache)
    return PEERFalse;
  for (roomType = 0; roomType < NumRooms; roomType++) {
    connection->globalWatchKeys[roomType] =
        TableNew(sizeof(piWatchKey), PI_WATCH_KEYS_NUM_BUCKETS, WatchKeysHash,
                 WatchKeysCompare, WatchKeysFree);
...
static const unsigned short *piGetWatchKeyW(const unsigned short *nick,
...
const unsigned short *piGetGlobalWatchKeyW(PEER peer,
...
const unsigned short *piGetRoomWatchKeyW(PEER peer, RoomType roomType,
                                         const unsigned short *nick,
                                         const unsigned short *key) {
  PEER_CONNECTION;
...
static void piGetRoomKeysCallbackA(CHAT chat, CHATBool success,
...
#ifdef GSI_UNICODE
static void piGetRoomKeysCallbackW(CHAT chat, CHATBool success,
                                   const unsigned short *channel,
                                   const unsigned short *user, int num,
                                   const unsigned short **keys,
                                   const unsigned short **values, void *param) {
  char *channel_A = UCS2ToUTF8StringAlloc(channel);
  char *user_A = UCS2ToUTF8StringAlloc(user);
  char **keys_A = UCS2ToUTF8StringArrayAlloc(keys, num);
  char **values_A = UCS2ToUTF8StringArrayAlloc(values, num);
  int i;
  piGetRoomKeysCallbackA(chat, success, channel_A, user_A, num,
                         (const char **)keys_A, (const char **)values_A, param);
  gsifree(channel_A);
  gsifree(user_A);
  for (i = 0; i < num; i++) {
    if (keys_A != NULL)
...
void piSetGlobalWatchKeys(PEER peer, RoomType roomType, int num,
  ...
  if (ENTERING_ROOM || IN_ROOM)
    chatGetGlobalKeysA(connection->chat, ROOM, num, keys,
                       piGetGlobalKeysCallback, peer, CHATFalse);
}
...
void piSetRoomWatchKeys(PEER peer, RoomType roomType, int num,
  ...
  ///////////////////
  if (ENTERING_ROOM || IN_ROOM)
    chatGetChannelKeysA(connection->chat, ROOM, "*", num, keys,
                        piGetRoomKeysCallback, peer, CHATFalse);
}

static PEERBool piKeyChanged(PEER peer, const char *nick, const char *key,
                             const char *value, HashTable watchKeys,
                             HashTable watchCache, PEERBool inRoom,
                             RoomType roomType) {
  piWatchKey watchKeyTemp;
  piCacheKey cacheKey;
...
void piGlobalKeyChanged(PEER peer, const char *nick, const char *key,
  ...
  if (piKeyChanged(peer, nick, key, value,
                   connection->globalWatchKeys[TitleRoom],
                   connection->globalWatchCache, PEERFalse, (RoomType)0) ||
      piKeyChanged(peer, nick, key, value,
...
void piRoomKeyChanged(PEER peer, RoomType roomType, const char *nick,
  ...
  if (piKeyChanged(peer, nick, key, value, connection->roomWatchKeys[roomType],
                   connection->roomWatchCache[roomType], PEERTrue, roomType))
...
static void piKeyCacheRefresh(PEER peer, RoomType roomType, const char *nick) {
  ...
  if (!nick)
    getIP =
        (connection->pingRoom[roomType] || connection->alwaysRequestPlayerInfo)
            ? PEERTrue
            : PEERFalse;
  else
    getIP = PEERFalse;
  ...
  if (getFlags) {
    ...
    getFlags = (PEERBool)(
        !(TableLookup(connection->globalWatchKeys[roomType], &watchKey) ||
          TableLookup(connection->roomWatchKeys[roomType], &watchKey)));
  }
  ...
  if (num) {
    ...
    chatGetGlobalKeysA(connection->chat, nick ? nick : ROOM, num,
                       (const char **)data.keys, piGetGlobalKeysCallback, peer,
                       CHATFalse);
    gsifree(data.keys);
  }
  ...
  if (!nick) {
    ...
    if (num) {
      ...
      assert(data.next == num);
      chatGetChannelKeysA(connection->chat, ROOM, nick ? nick : "*", num,
                          (const char **)data.keys, piGetRoomKeysCallback, peer,
                          CHATFalse);
      gsifree(data.keys);
    }
  }
