...
HCURSOR CPeerTestDlg::OnQueryDragIcon() { return (HCURSOR)m_hIcon; }

static const gsi_char *RoomToString(RoomType roomType) {
...
static void ClearRoomsList(CListBox &list) { list.ResetContent(); }
...
static void DisconnectedCallback(PEER peer, const gsi_char *reason,
                                 void *param) {
...
static void RoomMessageCallback(PEER peer, RoomType roomType,
                                const gsi_char *nick, const gsi_char *message,
  ...
  if (messageType == NormalMessage) {
    _stprintf(buffer, _T("%c]%s: %s"), c, nick, message);
...
static void RoomUTMCallback(PEER peer, RoomType roomType, const gsi_char *nick,
                            const gsi_char *command, const gsi_char *parameters,
                            PEERBool authenticated, void *param) {
#if VERBOSE
  gsi_char buffer[256];
  _stprintf(buffer, _T(" UTM %s| %s | %s | %s | %s\n"),
            authenticated ? "(authenticated) " : "", RoomToString(roomType),
...
const gsi_char *ModeToString(CHATChannelMode *mode) {
...
static void RoomModeChangedCallback(PEER peer, RoomType roomType,
...
static void PlayerMessageCallback(PEER peer, const gsi_char *nick,
                                  const gsi_char *message,
                                  MessageType messageType, void *param) {
  gsi_char buffer[2048];
  if (messageType == NormalMessage) {
    _stprintf(buffer, _T("%s: %s"), nick, message);
  } else if (messageType == ActionMessage) {
    _stprintf(buffer, _T("%s %s"), nick, message);
  } else {
    _stprintf(buffer, _T("*%s*: %s"), nick, message);
...
static void PlayerUTMCallback(PEER peer, const gsi_char *nick,
  ...
  _stprintf(buffer, _T(" UTM %s| %s | %s | %s\n"),
            authenticated ? "(authenticated) " : "", nick, command, parameters);
  OutputDebugString(buffer);
#endif
...
static void ReadyChangedCallback(PEER peer, const gsi_char *nick,
...
static void GameStartedCallback(PEER peer, SBServer server,
                                const gsi_char *message, void *param) {
  gsi_char buffer[256];
  _stprintf(buffer, _T("The game is starting: %s:%d %s"),
            SBServerGetPublicAddress(server),
            SBServerGetIntValue(server, _T("hostport"), 0), message);
...
static void PlayerJoinedCallback(PEER peer, RoomType roomType,
                                 const gsi_char *nick, void *param) {
...
static void PlayerLeftCallback(PEER peer, RoomType roomType,
                               const gsi_char *nick, const gsi_char *reason,
                               void *param) {
  dlg->FillPlayerList(roomType);
...
static void KickedCallback(PEER peer, RoomType roomType, const gsi_char *nick,
                           const gsi_char *reason, void *param) {
  CString str;
  str.Format(_T("Kicked from %s by %s: \"%s\""), RoomToString(roomType), nick,
             reason);
  dlg->MessageBox(str);
...
static void PlayerInfoCallback(PEER peer, RoomType roomType,
                               const gsi_char *nick, unsigned int IP,
  ...
  _stprintf(buffer, _T(" INFO | %s | %s | %s | %d\n"), RoomToString(roomType),
            nick ? nick : "(END)", inet_ntoa(addr), profileID);
  OutputDebugString(buffer);
#endif
...
static void PlayerFlagsChangedCallback(PEER peer, RoomType roomType,
                                       const gsi_char *nick, int oldFlags,
...
static int FindPingPlayersNick(const gsi_char *nick) {
...
static void PingCallback(PEER peer, const gsi_char *nick, int ping,
                         void *param) {
#if VERBOSE
  gsi_char buffer[256];
...
static void CrossPingCallback(PEER peer, const gsi_char *nick1,
                              const gsi_char *nick2, int crossPing,
                              void *param) {
#if VERBOSE
  gsi_char buffer[256];
...
static void GlobalKeyChangedCallback(PEER peer, const gsi_char *nick,
                                     const gsi_char *key, const gsi_char *value,
                                     void *param) {
#if VERBOSE
  gsi_char buffer[256];
...
static void RoomKeyChangedCallback(PEER peer, RoomType roomType,
                                   const gsi_char *nick, const gsi_char *key,
                                   const gsi_char *value, void *param) {
#if VERBOSE
  gsi_char buffer[256];
  _stprintf(buffer, _T("KEY | %s | %s | %s = %s\n"), nick,
            RoomToString(roomType), key, value);
  OutputDebugString(buffer);
#endif
...
static void QRAddErrorCallback(PEER peer, qr2_error_t error,
                               gsi_char *errorString, void *param) {
#if VERBOSE
  gsi_char verbose[256];
  _stprintf(verbose, "QR_ADD_ERROR | %s | %s\n", ErrorTypeToString(error),
            errorString);
  OutputDebugString(verbose);
#endif

  CString str;
  str.Format(_T("Reporting error: %s (%s)"), ErrorTypeToString(error),
             errorString);
  dlg->MessageBox(str);
...
static void QRPublicAddressCallback(PEER peer, unsigned int ip,
...
void CPeerTestDlg::OnButton7() {
  if (!m_peer) {
    ...
    if (result == GSIACUnavailable)
      ...
      MessageBox("This game's backend services are temporarily "
...
static void ConnectCallback(PEER peer, PEERBool success, int failureReason,
                            void *param) {
  CString str;
  if (success)
...
static void NickErrorCallback(PEER peer, int type, const gsi_char *nick,
                              int numSuggestedNicks,
                              const char **suggestedNicks, void *param) {
  CString str;
  ...
  if (type == PEER_UNIQUENICK_EXPIRED) {
    ...
    str.Format(_T("This account's unique nick has expired.\nRegister '%s'?"),
               dlg->m_nick);
    rcode = dlg->MessageBox(str, NULL, MB_YESNO);
    peerRegisterUniqueNick(peer, atoi(dlg->m_namespace),
                           (rcode == IDYES) ? ((LPCSTR)dlg->m_nick) : NULL,
                           NULL);
    return;
  }
  ...
  if (type == PEER_INVALID_UNIQUENICK) {
    ...
    for (int i = 0; i < numSuggestedNicks; i++) {
      str.Format(
          _T("The unique nick you attempted to register is either invalid or ")
      ...
      if (rcode == IDCANCEL)
        break;
      if (rcode == IDYES) {
        peerRegisterUniqueNick(peer, atoi(dlg->m_namespace), suggestedNicks[i],
                               NULL);
        return;
      }
    }
...
void CPeerTestDlg::OnButton1() {
  if (m_peer) {
    ...
    peerConnect(m_peer, m_nick, 0, NickErrorCallback, ConnectCallback, this,
                (PEERBool)m_blocking);
  }
}
...
void CPeerTestDlg::OnButton36() {
  if (m_peer) {
    ...
    peerConnectLogin(m_peer, atoi(m_namespace), m_email, m_nick, NULL,
                     m_loginPassword, NickErrorCallback, ConnectCallback, this,
                     (PEERBool)m_blocking);
  }
}
...
void CPeerTestDlg::OnButton50() {
  if (m_peer) {
    ...
    peerConnectLogin(m_peer, atoi(m_namespace), NULL, NULL, m_nick,
                     m_loginPassword, NickErrorCallback, ConnectCallback, this,
                     (PEERBool)m_blocking);
  }
}
...
void CPeerTestDlg::OnButton49() {
  if (m_peer) {
    ...
    peerConnectPreAuth(m_peer, m_authToken, m_partnerChallenge,
                       NickErrorCallback, ConnectCallback, this,
                       (PEERBool)m_blocking);
  }
}
...
void EnumPlayersCallback(PEER peer, PEERBool success, RoomType roomType,
                         int index, const gsi_char *nick, int flags,
                         void *param) {
  if (!success) {
...
void CPeerTestDlg::FillPlayerList(RoomType roomType) {
  UpdateData();
  ClearPlayersList(m_players(roomType));
  m_count = 0;
...
void CPeerTestDlg::OnButton2() {
  if (m_peer) {
    UpdateData();
    peerCreateStagingRoom(m_peer, m_name, 6, m_password, JoinRoomCallback, NULL,
                          (PEERBool)m_blocking);
  }
}
...
void CPeerTestDlg::OnButton3() {
  if (m_peer) {
    ...
    if (server) {
      if (SBServerGetIntValue(server, _T("password"), 0) &&
          !m_password.GetLength())
        dlg->MessageBox(_T("This server requires a password"));
      peerJoinStagingRoom(m_peer, server, m_password, JoinRoomCallback, NULL,
                          (PEERBool)m_blocking);
    }
  }
}
...
void CPeerTestDlg::OnButton17() {
  if (m_peer) {
    ...
    if (server) {
      ...
      if (groupID != 0)
        peerJoinGroupRoom(m_peer, groupID, JoinRoomCallback, NULL,
                          (PEERBool)m_blocking);
    }
  }
}
...
void CPeerTestDlg::OnButton22() {
  if (m_peer) {
    peerJoinTitleRoom(m_peer, NULL, JoinRoomCallback, NULL,
                      (PEERBool)m_blocking);
  }
}
...
void ListGroupRoomsCallback(PEER peer, PEERBool success, int groupID,
                            SBServer server, const gsi_char *name,
...
void CPeerTestDlg::OnButton6() {
  if (m_peer) {
    ...
    m_count = 0;
    peerListGroupRooms(m_peer, _T(""), ListGroupRoomsCallback, NULL,
                       (PEERBool)m_blocking);
  }
}
...
void GetPlayerIPCallback(PEER peer, PEERBool success, const gsi_char *nick,
                         unsigned int IP, void *param) {
  gsi_char buffer[128];
...
void CPeerTestDlg::OnButton26() {
  if (m_peer) {
    ...
    if (!name || !channel)
      ...
      gsi_char buffer[256];
      _stprintf(buffer, _T("Name: %s\nChannel: %s\nUsers: %d"), name, channel,
                chatGetChannelNumUsers(peerGetChat(m_peer), channel));
      dlg->MessageBox(buffer);
    }
  }
}
...
BOOL CPeerTestDlg::DestroyWindow() {
  ...
  if (file) {
    _ftprintf(file, _T("%s\n%s\n%s\n%s\n%s\n%s\n"), m_title, m_secretKey,
              m_nick, m_email, m_loginPassword, m_namespace);
    fclose(file);
  }
...
void ListingGamesCallback(PEER peer, PEERBool success, const gsi_char *name,
                          SBServer server, PEERBool staging, int msg,
                          int progress, void *param) {
  int nIndex;
...
void CPeerTestDlg::OnButton29() {
  if (m_peer) {
    ...
    const unsigned char keys[] = {NUMPLAYERS_KEY, MAXPLAYERS_KEY, HOSTPORT_KEY,
                                  MAPNAME_KEY, PASSWORD_KEY};
    peerStartListingGames(m_peer, keys, sizeof(keys), m_filter,
                          ListingGamesCallback, NULL);
  }
}
...
void CPeerTestDlg::OnButton24() {
  if (m_peer) {
    ...
    PEERBool result =
        peerSetTitle(m_peer, m_title, m_secretKey, m_title, m_secretKey, 0, 30,
                     PEERTrue, pingRooms, crossPingRooms);
...
static void ChangeNickCallback(PEER peer, PEERBool success,
                               const gsi_char *oldNick, const gsi_char *newNick,
                               void *param) {
  gsi_char buffer[128];
...
void CPeerTestDlg::OnChangeNick() {
  if (m_peer) {
    UpdateData();
    if (m_nick) {
      peerChangeNick(m_peer, m_nick, ChangeNickCallback, NULL,
                     (PEERBool)m_blocking);
    }
  }
}
...
static void GetGlobalKeysCallback(PEER peer, PEERBool success,
...
static void GetRoomKeysCallback(PEER peer, PEERBool success, RoomType roomType,
                                const gsi_char *nick, int num, gsi_char **keys,
                                gsi_char **values, void *param) {
  int i;
  if (success) {
    if (nick)
      OutputDebugString(nick);
...
void CPeerTestDlg::OnButton38() {
  ...
  if (m_peer) {
    ...
    if (m_keyType == 0) {
      ...
      if (m_player.IsEmpty())
        peerGetRoomGlobalKeys(m_peer, (RoomType)m_keyRoom, 1, &key,
                              GetGlobalKeysCallback, NULL,
                              (PEERBool)m_blocking);
      else
        peerGetPlayerGlobalKeys(m_peer, m_player, 1, &key,
                                GetGlobalKeysCallback, NULL,
                                (PEERBool)m_blocking);
    } else {
      peerGetRoomKeys(m_peer, (RoomType)m_keyRoom, m_player, key[0] ? 1 : 0,
                      &key, GetRoomKeysCallback, NULL, (PEERBool)m_blocking);
    }
  }
}
...
void CPeerTestDlg::OnButton40() {
  if (m_peer) {
    ...
    MessageBox(
        _T("Will stay in the title room when the title is cleared/changed"));
  }
}
...
void CPeerTestDlg::OnButton11() { m_pingPlayers.ResetContent(); }
...
void AuthenticateCDKeyCallback(PEER peer, int result, const gsi_char *message,
                               void *param) {
  CString str;
...
void CPeerTestDlg::OnButton46() {
  if (m_peer) {
    UpdateData();
    peerAuthenticateCDKey(m_peer, m_cdKey, AuthenticateCDKeyCallback, NULL,
                          (PEERBool)m_blocking);
  }
}
...
void CPeerTestDlg::OnButton47() {
  if (m_peer) {
    peerKickPlayer(m_peer, m_selectedRoom, m_selectedNick,
                   _T("You've been kicked"));
  }
}
...
void AutoMatchStatusCallback(PEER peer, PEERAutoMatchStatus status,
                             void *param) {
  const gsi_char *StatusNames[] = {_T("Failed"),  _T("Searching"),
                                   _T("Waiting"), _T("Staging"),
                                   _T("Ready"),   _T("Complete")};
...
int AutoMatchRateCallback(PEER peer, SBServer match, void *param) {
  dlg->UpdateData();
...
void CPeerTestDlg::OnStartAutoMatch() {
  if (m_peer) {
    ...
    peerStartAutoMatch(m_peer, m_maxPlayers, _T(""), AutoMatchStatusCallback,
                       AutoMatchRateCallback, NULL, (PEERBool)m_blocking);
  }
}
