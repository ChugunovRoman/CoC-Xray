...
#define PEER_INVALID_UNIQUENICK                                                \
  5 // the uniquenick to associate with the account is invalid or in use
#define PEER_NICK_TOO_LONG 6 // the nick was too long

// Possible values for the failureReason passed to the peerConnectCallback.
///////////////////////////////////////////////////////////////////////////
#define PEER_DISCONNECTED                                                      \
  0 // disconnected from, or unable to connect to, the server
#define PEER_NICK_ERROR 1 // a nick error was either ignored or not handled
#define PEER_LOGIN_FAILED                                                      \
  2 // the login info passed to peerConnectLogin was not valid
...
#define PEER_STOP_REPORTING                                                    \
  1 // Stop reporting.  Cannot be used with other options.
#define PEER_REPORT_INFO                                                       \
  2 // Continue reporting server keys (as if it were not playing).
#define PEER_REPORT_PLAYERS                                                    \
  4 // Continue reporting player keys (as if it were not playing).
...
// Boolean.
///////////
typedef enum { PEERFalse, PEERTrue } PEERBool;

// Types of rooms.
//////////////////
typedef enum {
  TitleRoom,   // The main room for a game.
  GroupRoom,   // A room which is, in general, for a particular type of gameplay
               // (team, dm, etc.).
...
// PANTS-01.08.01
/////////////////////////////////////////
typedef enum { NormalMessage, ActionMessage, NoticeMessage } MessageType;
...
typedef enum {
  ...
  PEERBadPassword, // An incorrect password (or none) was given for a passworded
                   // room.

  PEERAlreadyInRoom, // The local user is already in or entering a room of the
                     // same type.
  PEERNoTitleSet,    // Can't join a room if no title is set.
  PEERNoConnection,  // Can't join a room if there's no chat connection.
  PEERAutoMatching,  // The user can't join a staging room during an auto match
                     // attempt.
...
typedef enum {
  ...
  PEERStaging,   // In a staging room with at least one other player, possibly
                 // waiting for more.
  PEERReady,     // All players are in the staging room, the game is ready to be
                 // launched.
...
typedef void (*peerQRKeyListCallback)(
    PEER peer,                 // The peer object.
    qr2_key_type type,         // The type of keys being asked for (key_server,
...
typedef void (*peerConnectCallback)(
    ...
    int failureReason, // If failure, the reason for it (PEER_DISCONNECTED,
                       // etc.)
    void *param        // User-data.
);
...
/////////////////////////////////////////////////////////////////////////
void peerRegisterUniqueNick(PEER peer, int namespaceID,
...
PEERBool peerSetTitle(
    ...
    int sbMaxUpdates,      // The maximum number of concurent updates (10-15 for
                           // modem users, 20-30 for high-bandwidth).
    PEERBool natNegotiate, // PEERTrue if the title supports GameSpy's
                           // NAT-negotiation technology (or another similar
                           // technology).
    PEERBool pingRooms[NumRooms], // To do pings int a room, set it to PEERTrue.
    PEERBool crossPingRooms[NumRooms] // To do cross-pings in a room, set it to
                                      // PEERTrue.
);
...
///////////////////////////////////////////////////////////
const gsi_char *
peerTranslateNick(gsi_char *nick,           // The nick to translate
...
void peerSendNatNegotiateCookie(
    PEER peer, // The peer object.
    unsigned int
        ip, // IP (network byte order) of the server to send the cookie to.
    unsigned short
        port,  // Port (host byte order) of the server to send the cookie to.
...
void peerJoinTitleRoom(
    PEER peer, // The peer object.
    const gsi_char
        password[PEER_PASSWORD_LEN], // An optional password, normally NULL.
...
void peerJoinStagingRoom(
    ...
    const gsi_char
        password[PEER_PASSWORD_LEN], // The password of the room being joined.
                                     // Can be NULL or "".
    peerJoinRoomCallback callback,   // Called when finished.
    void *param,                     // Passed to the callback.
    PEERBool blocking // If PEERTrue, don't return until finished.
...
void peerCreateStagingRoom(
    ...
    const gsi_char password[PEER_PASSWORD_LEN], // An optional password for the
                                                // staging room
    peerJoinRoomCallback callback,              // Called when finished.
    void *param,                                // Passed to the callback.
    PEERBool blocking // If PEERTrue, don't return until finished.
...
void peerLeaveRoom(PEER peer,         // The peer object.
                   RoomType roomType, // The room you want to leave (TitleRoom,
                                      // GroupRoom, or StagingRoom).
                   const gsi_char *reason // The reason the player is leaving
                                          // (can be NULL).  PANTS|03.13.01
);
...
// If staging==PEERTrue, the game hasn't started yet, it's still in the staging
// room
//   use peerJoinStagingRoom() to join the staging room, or if
//   staging==peerfalse use the server object to get the game's IP and port to
//   join with.
// During the _intial_ listing of games, progress is the percentage (0-100) of
// the
...
// passworded.  This can be checked with ServerGetIntValue(server, "password",
// 0).
/////////////////////////////////////////////////////////////////////////////////////
typedef void (*peerListingGamesCallback)(
    ...
    PEERBool
        staging, // If PEERTrue, this is a staging room and not a running game.
    int msg,     // The type of message this is.
                 // PEER_CLEAR:
                 //   Clear the list.  This has the same effect as if this was
                 //   called with PEER_REMOVE for every server listed.
                 // PEER_ADD:
                 //   This is a new server. Add it to the list.
                 // PEER_UPDATE:
                 //   This server is already on the list, and its been updated.
                 // PEER_REMOVE:
    //   Remove this server from the list.  The server object for this
    //   server should not be used again after this callback returns.
...
void peerStartListingGames(
    PEER peer, // The peer object.
    const unsigned char
        *fields,   // An array of registered QR2 keys to request from servers.
...
///////////////////////////////////////////
const gsi_char *
peerGetRoomName(PEER peer,        // The peer object.
...
///////////////////////////////////////////////////
const gsi_char *
peerGetRoomChannel(PEER peer,        // The peer object.
...
void peerUTMPlayer(
    ...
    const gsi_char *command,    // The command.
    const gsi_char *parameters, // The UTM's parameters.
    PEERBool authenticate // If true, the server will authenticate this UTM
...
/////////////////////////////////////////////////////////////////////////////
PEERBool
peerGetPlayerPing(PEER peer,            // The peer object.
...
/////////////////////////////////////////////////
PEERBool peerGetPlayerInfoNoWait(PEER peer, const gsi_char *nick,
...
typedef void (*peerGetPlayerIPCallback)(
    ...
    PEERBool success,     // PEERTrue if success, PEERFalse if failure.
    const gsi_char *nick, // The player's nick.
    unsigned int IP, // The player's IP, in network byte order.  PANTS|09.11.00
                     // - was unsigned long
    void *param      // User-data.
);
...
/////////////////////////////////////////////////////////
PEERBool peerGetPlayerFlags(PEER peer, const gsi_char *nick, RoomType roomType,
                            int *flags);
...
///////////////////////////////
PEERBool
peerGetReady(PEER peer,            // The peer object.
             const gsi_char *nick, // The player's nick.
...
void peerSetRoomKeys(PEER peer,              // The peer object.
                     ...
                     const gsi_char *nick,   // The player to set the keys on
                                             // (NULL or "" for the room).
                     int num,                // The number of keys.
                     const gsi_char **keys,  // The keys to set.
...
void peerSetGlobalWatchKeys(
    ...
    PEERBool addKeys // If PEERTrue, add these keys to the existing global watch
                     // keys for this room.
);
...
void peerSetRoomWatchKeys(
    ...
    int num,               // The number of keys.
    const gsi_char **keys, // The keys to watch for.
    PEERBool addKeys // If PEERTrue, add these keys to the existing room watch
                     // keys for this room.
);
...
///////////////////////////////////////////////////////////////
const gsi_char *
peerGetGlobalWatchKey(PEER peer,            // The peer object.
...
///////////////////////////////////////////////////////////////
const gsi_char *
peerGetRoomWatchKey(PEER peer,            // The peer object.
...
void peerStartAutoMatch(
    PEER peer,      // The peer object.
    int maxPlayers, // The total number of players to match (including the local
                    // player).
    const gsi_char *filter, // Hard criteria - filters out servers.
    peerAutoMatchStatusCallback
        statusCallback, // Called as the attempt status changes.
