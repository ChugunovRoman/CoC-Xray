...
static void RoomMessageCallback(PEER peer, RoomType roomType, const char *nick,
  ...
  if (roomType == StagingRoom) {
    ...
    StagingPage->m_chatWindow.SetTopIndex(StagingPage->m_chatWindow.GetCount() -
                                          1);
...
///////////////////////////////////////////////////////////
static void ReadyChangedCallback(PEER peer, const char *nick, PEERBool ready,
                                 void *param) {
...
////////////////////////////////////////////////////////////////
static void GameStartedCallback(PEER peer, SBServer server, const char *message,
                                void *param) {
...
static void PingCallback(PEER peer, const char *nick, int ping, void *param) {
  ...
  if (Wizard->m_groupRooms && TitlePage->m_hWnd &&
      TitlePage->FindPlayer(nick) != -1)
...
////////////////////////////////////////////
static void CrossPingCallback(PEER peer, const char *nick1, const char *nick2,
...
////////////////////////////////////
static const char *KeyToString(int key) { return qr2_registered_key_list[key]; }
...
static void QRAddErrorCallback(PEER peer, qr2_error_t error, char *errorString,
  ...
  CString str;
  str.Format("Peer: Server Reporting error: %s (%s)", ErrorTypeToString(error),
             errorString);
...
static void NickErrorCallback(PEER peer, int type, const char *nick,
  ...
  /////////////////////
  if (type == PEER_IN_USE)
    ConnectPage->MessageBox(
        "That nickname is already taken, please choose another one.");
  else
    ConnectPage->MessageBox("That nickname contains at least 1 invalid "
                            "character, please choose another one.");
...
LRESULT CConnectPage::OnWizardNext() {
  ...
  // Set the title.
  /////////////////
  if (!peerSetTitle(Wizard->m_peer, m_title, m_key, m_title, m_key, 0, 30,
                    PEERFalse, pingRooms, crossPingRooms)) {
  ...
  peerConnect(Wizard->m_peer, m_nick, 0, NickErrorCallback, ConnectCallback,
              NULL, PEERTrue);
