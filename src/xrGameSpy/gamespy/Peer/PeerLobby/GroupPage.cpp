...
static void ListingGamesCallback(PEER peer, PEERBool success, const char *name,
  ...
  if (msg == PEER_CLEAR) {
    ...
    nIndex = games->InsertItem(
        LVIF_TEXT | LVIF_PARAM | LVIF_IMAGE, 0, (LPCTSTR)name, 0, 0,
...
BOOL CGroupPage::OnSetActive() {
  ...
  peerStartListingGames(Wizard->m_peer, fields, sizeof(fields), NULL,
...
void CGroupPage::SendMessage() {
  ...
  // Send it.
  ///////////
  peerMessageRoom(Wizard->m_peer, Wizard->m_groupRooms ? GroupRoom : TitleRoom,
                  m_message, NormalMessage);
  // PEERSTOP
