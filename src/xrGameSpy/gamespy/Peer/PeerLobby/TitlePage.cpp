...
//////////////////////////////////////////////
static void EnumTitlePlayersCallback(PEER peer, PEERBool success,
...
static void JoinTitleRoomCallback(PEER peer, PEERBool success,
...
static void ListGroupRoomsCallback(PEER peer, PEERBool success, int groupID,
  ...
  int nIndex = groups->InsertItem(LVIF_TEXT | LVIF_PARAM, 0, name, 0, 0, 0,
                                  (LPARAM)groupID);
  if (nIndex == -1)
    return;
...
BOOL CTitlePage::OnSetActive() {
  ...
  peerJoinTitleRoom(Wizard->m_peer, NULL, JoinTitleRoomCallback, NULL,
                    PEERTrue);
  ...
  /////////////////////////////
  peerListGroupRooms(Wizard->m_peer, "", ListGroupRoomsCallback, NULL,
                     PEERFalse);
  // PEERSTOP
...
// Called when the join is complete (successful or unsuccessful).
/////////////////////////////////////////////////////////////////
static void JoinGroupRoomCallback(PEER peer, PEERBool success,
                                  PEERJoinResult result, RoomType roomType,
                                  void *param) {
  joinSuccess = success;
  GSI_UNUSED(roomType);
...
void CTitlePage::JoinGroup(int nIndex) {
  ...
  ///////////////////////
  Wizard->StartHourglass();
  peerJoinGroupRoom(Wizard->m_peer, groupID, JoinGroupRoomCallback, NULL,
                    PEERTrue);
  Wizard->StopHourglass();
  if (!joinSuccess) {
