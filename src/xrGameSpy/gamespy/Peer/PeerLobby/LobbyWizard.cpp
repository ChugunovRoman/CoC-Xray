...
CLobbyWizard::CLobbyWizard(LPCTSTR pszCaption, CWnd *pParentWnd,
                           UINT iSelectPage)
...
BOOL CLobbyWizard::OnInitDialog() {
  ...
  if (!m_greenSmileyIcon || !m_yellowSmileyIcon || !m_redSmileyIcon ||
      !m_stagingRoomIcon || !m_runningGameIcon)
    return FALSE;
  ...
  if ((m_yellowSmileyIndex == -1) || (m_redSmileyIndex == -1) ||
      (m_greenSmileyIndex == -1) || (m_runningGameIndex == -1) ||
      (m_stagingRoomIndex == -1))
    return FALSE;
