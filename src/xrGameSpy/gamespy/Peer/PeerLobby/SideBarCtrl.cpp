...
void CSideBarCtrl::OnPaint() {
  ...
  HFONT oldFont =
      (HFONT)dc.SelectObject((HFONT)::GetStockObject(ANSI_VAR_FONT));
 ...
 *
 * DESCRIPTION:
 *  Found this as part of an overly complicated gradient progress bar on
 *codeguru So, I simplified it for my own uses :)   -Lumberjack
 *
 *******************************************************************************/
void CSideBarCtrl::DrawGradient(CDC *pDC, const CRect &rcGrad,
  ...
  float nSpacePerStep = bVertical ? (float)rcGrad.Height() / nSteps
                                  : (float)rcGrad.Width() / nSteps;
  CRect rcFill(rcGrad);
  CBrush br;
  ...
  for (int i = 0; i < nSteps; i++) {
    ...
    COLORREF clrFill =
        RGB(r + (int)(i * rStep), g + (int)(i * gStep), b + (int)(i * bStep));
