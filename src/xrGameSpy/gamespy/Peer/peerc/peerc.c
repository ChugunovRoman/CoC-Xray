...
static void CreateStagingRoomCallback(PEER peer, PEERBool success,
...
static void RoomMessageCallback(PEER peer, RoomType roomType,
  ...
  //	stop = PEERTrue;
  // else if((strlen(message) > 5) && (strncasecmp(message, _T("echo"), 4) ==
  // 0)) 	peerMessageRoom(peer, roomType, message + 5, messageType);
        ...
        if(success)
        {
                ...
                case PEER_ADD:
                        ...
                        if (SBServerHasBasicKeys(server))
                                _tprintf(_T("  firewall server name: %s
numplayers: %d ping: %d firewall: %s\n"),
SBServerGetStringValue(server,_T("hostname"),_T("UNKNOWN")),
SBServerGetIntValue(server,_T("numplayers"),0), SBServerGetPing(server),
SBServerDirectConnect(server)==SBTrue?_T("NO"):_T("YES")); if
(SBServerHasFullKeys(server)) _tprintf(_T("  server gametype: %s \n"),
SBServerGetStringValue(server, _T("gametype"), _T("unknown"))); break; case
PEER_UPDATE: msgname = _T("PEER_UPDATE"); if(server)
                        {
                                _tprintf(_T("  update server name: %s
numplayers: %d ping: %d firewall: %s\n"),
SBServerGetStringValue(server,_T("hostname"),_T("UNKNOWN")),
SBServerGetIntValue(server,_T("numplayers"),0), SBServerGetPing(server),
SBServerDirectConnect(server)==SBTrue?_T("NO"):_T("YES")); _tprintf(_T("  server
gametype: %s \n"), SBServerGetStringValue(server, _T("gametype"),
_T("unknown")));
                        }
                        break;
                case PEER_REMOVE:
...
static void ListGroupRoomsCallback(PEER peer, PEERBool success, int groupID,
  ...
  if (success && (groupID > 0)) {
    ...
    groupRoomCallbackDone = PEERTrue; // if groupID is set to 0 it means all
                                      // group rooms have been listed
...
static void NickErrorCallback(PEER peer, int type, const gsi_char *badNick,
...
/*
static void AutoMatchStatusCallback(PEER thePeer, PEERAutoMatchStatus theStatus,
void* theParam)
{
...
static int AutoMatchRateCallback(PEER thePeer, SBServer theServer, void*
theParam)
{
        GSI_UNUSED(thePeer);
...
*/
static void RoomKeyChangedCallback(PEER thePeer, RoomType theType,
                                   const gsi_char *theNick,
                                   const gsi_char *theKey,
                                   const gsi_char *theValue, void *theParam) {
  GSI_UNUSED(thePeer);
  GSI_UNUSED(theType);
  GSI_UNUSED(theNick);
  GSI_UNUSED(theKey);
...
int test_main(int argc, char **argv) {
  ...
  int profileID =
      0; // we will not be using gp accounts, so this will be ignored
  ...
  // set our nickname to a random string so that we'll have different nicks when
  // running multiple instances of peerc
  _tsnprintf(nick, NICK_SIZE, _T("peerc%u"), (unsigned int)current_time());
  nick[NICK_SIZE - 1] = '\0';
  ...
  // set the title
  if (!peerSetTitle(peer, TITLE, secret_key, TITLE, secret_key, 0, maxUpdates,
                    natNeg, pingRooms, crossPingRooms)) {
    peerShutdown(peer);
  ...
  peerConnect(peer, nick, profileID, NickErrorCallback, ConnectCallback,
              userData, blocking);
  if (!connectSuccess) {
    peerShutdown(peer);
  ...
  peerListGroupRooms(peer, _T(""), ListGroupRoomsCallback, userData,
                     non_blocking);
  ...
  peerCreateStagingRoom(peer, serverName, maxPlayers, noPassword,
                        CreateStagingRoomCallback, userData, blocking);
  ...
  // peerStartAutoMatch(peer, 3, _T(""), AutoMatchStatusCallback,
  // AutoMatchRateCallback, NULL, PEERFalse);
  ...
  // disconnect local client from chat server (peerShutdown must still be
  // called)
  peerDisconnect(peer);
