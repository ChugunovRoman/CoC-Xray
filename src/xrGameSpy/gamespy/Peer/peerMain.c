...
void peerStartAutoMatchWithSocketA(PEER peer, int maxPlayers,
                                   const char *filter, SOCKET socket,
                                   unsigned short port,
...
static PEERBool piCheckBlockingID(PEER peer, int ID) {
  return (PEERBool)(piIsOperationFinished(peer, ID) &&
                    piIsCallbackFinished(peer, ID));
}
...
void peerConnectA(PEER peer, const char *nick, int profileID,
                  ...
                  peerConnectCallback connectCallback, void *param,
                  PEERBool blocking) {
  ...
  if (success) {
    ...
    // Start connecting.
    ////////////////////
    if (!piNewConnectOperation(peer, PI_CONNECT, nick, 0, NULL, NULL, NULL,
                               NULL, NULL, NULL, connectCallback, param,
                               opID)) {
      success = PEERFalse;
  ...
  if (!success)
    piAddConnectCallback(peer, PEERFalse, PEER_DISCONNECTED, connectCallback,
                         param, opID);

  PI_DO_BLOCKING;
}
#ifdef GSI_UNICODE
void peerConnectW(PEER peer, const unsigned short *nick, int profileID,
                  peerNickErrorCallback nickErrorCallback,
                  peerConnectCallback connectCallback, void *param,
                  PEERBool blocking) {
  char *nick_A = UCS2ToUTF8StringAlloc(nick);
  peerConnectA(peer, nick_A, profileID, nickErrorCallback, connectCallback,
               param, blocking);
  gsifree(nick_A);
}
#endif
...
void peerConnectLoginA(PEER peer, int namespaceID, const char *email,
                       const char *profilenick, const char *uniquenick,
                       const char *password,
                       peerNickErrorCallback nickErrorCallback,
                       peerConnectCallback connectCallback, void *param,
                       PEERBool blocking) {
  ...
  if (success) {
    ...
    if (!piNewConnectOperation(
            ...
            NULL, namespaceID, email, profilenick, uniquenick, password, NULL,
            NULL, connectCallback, param, opID)) {
      success = PEERFalse;
      piDisconnectCleanup(peer);
    }
  }
...
#ifdef GSI_UNICODE
void peerConnectLoginW(PEER peer, int namespaceID, const unsigned short *email,
  ...
  peerConnectLoginA(peer, namespaceID, email_A, profilenick_A, uniquenick_A,
                    password_A, nickErrorCallback, connectCallback, param,
                    blocking);
  gsifree(email_A);
...
void peerConnectPreAuthA(PEER peer, const char *authtoken,
  ...
  if (success) {
    ...
    // Start connecting.
    ////////////////////
    if (!piNewConnectOperation(peer, PI_CONNECT_PREAUTH, NULL, 0, NULL, NULL,
                               NULL, NULL, authtoken, partnerchallenge,
...
#ifdef GSI_UNICODE
void peerConnectPreAuthW(PEER peer, const unsigned short *authtoken,
  ...
  peerConnectPreAuthA(peer, authtoken_A, partnerchallenge_A, nickErrorCallback,
                      connectCallback, param, blocking);
  gsifree(authtoken_A);
...
void peerRegisterUniqueNickA(PEER peer, int namespaceID, const char *uniquenick,
                             const char *cdkey) {
  PEER_CONNECTION;
...
#ifdef GSI_UNICODE
void peerRegisterUniqueNickW(PEER peer, int namespaceID,
                             const unsigned short *uniquenick,
...
PEERBool peerSetTitleA(PEER peer, const char *title, const char *qrSecretKey,
                       const char *sbName, const char *sbSecretKey,
                       int sbGameVersion, int sbMaxUpdates,
                       PEERBool natNegotiate, PEERBool pingRooms[NumRooms],
...
#ifdef GSI_UNICODE
PEERBool peerSetTitleW(PEER peer, const unsigned short *title,
                       const unsigned short *qrSecretKey,
                       const unsigned short *sbName,
                       const unsigned short *sbSecretKey, int sbGameVersion,
                       int sbMaxUpdates, PEERBool natNegotiate,
  ...
  PEERBool result = peerSetTitleA(peer, title_A, qrSecretKey_A, sbName_A,
                                  sbSecretKey_A, sbGameVersion, sbMaxUpdates,
                                  natNegotiate, pingRooms, crossPingRooms);
  gsifree(title_A);
...
#ifdef _DEBUG
static void piNumPlayersConsistencyCheckMap(piPlayer *player,
                                            int count[NumRooms]) {
  int i;
...
static void piThink(PEER peer, int opID) {
  ...
  if (connection->players) {
    ...
    ////////////////////////////////////////////////////
    {
      ...
      //////////////////////////////////////////////
      TableMap(connection->players, (TableMapFn)piNumPlayersConsistencyCheckMap,
               count);
  ...
  // If we got disconnected from chat, do the cleanup before calling the
  // callback.
  ////////////////////////////////////////////////////////////////////////////////
...
const unsigned short *peerTranslateNickW(unsigned short *nick,
...
void peerChangeNickA(PEER peer, const char *newNick,
  ...
  /////////////////////
  if (!success)
    piAddChangeNickCallback(peer, PEERFalse, connection->nick, newNick,
                            callback, param, opID);

  PI_DO_BLOCKING;
}
#ifdef GSI_UNICODE
void peerChangeNickW(PEER peer, const unsigned short *newNick,
                     peerChangeNickCallback callback, void *param,
                     PEERBool blocking) {
...
#endif
*/
void peerAuthenticateCDKeyA(PEER peer, const char *cdkey,
                            peerAuthenticateCDKeyCallback callback, void *param,
                            PEERBool blocking) {
  PEERBool success = PEERTrue;
  ...
  /////////////////////
  if (!success)
    piAddAuthenticateCDKeyCallback(peer, 0, "Error starting CD Key check",
                                   callback, param, opID);

  PI_DO_BLOCKING;
}
#ifdef GSI_UNICODE
void peerAuthenticateCDKeyW(PEER peer, const unsigned short *cdkey,
                            peerAuthenticateCDKeyCallback callback, void *param,
                            PEERBool blocking) {
  char *cdkey_A = UCS2ToUTF8StringAlloc(cdkey);
...
void peerSendNatNegotiateCookie(PEER peer, unsigned int ip, unsigned short port,
                                int cookie) {
  piSendNatNegotiateCookie(peer, ip, port, cookie);
}

void peerSendMessageToServer(PEER peer, unsigned int ip, unsigned short port,
...
** ROOMS **
**********/
void peerJoinTitleRoomA(PEER peer, const char password[PEER_PASSWORD_LEN],
                        peerJoinRoomCallback callback, void *param,
                        PEERBool blocking) {
  PEERBool success = PEERTrue;
  ...
  ////////////////////////////////////
  assert(!connection->enteringRoom[TitleRoom] &&
         !connection->inRoom[TitleRoom]);
  if ((success && connection->enteringRoom[TitleRoom]) ||
      connection->inRoom[TitleRoom]) {
    success = PEERFalse;
  ...
  // Start the operation.
  ///////////////////////
  if (success && !piNewJoinRoomOperation(peer, TitleRoom, buffer, password,
                                         callback, param, opID))
    success = PEERFalse;
  ...
  /////////////////////
  if (!success)
    piAddJoinRoomCallback(peer, PEERFalse, result, TitleRoom, callback, param,
                          opID);
...
void peerJoinGroupRoom(PEER peer, int groupID, peerJoinRoomCallback callback,
  ...
  //////////////////////////////////
  if (success &&
      (connection->enteringRoom[GroupRoom] || connection->inRoom[GroupRoom])) {
    success = PEERFalse;
    result = PEERAlreadyInRoom;
  }
  ...
  // Start the operation.
  ///////////////////////
  if (success && !piNewJoinRoomOperation(peer, GroupRoom, room, NULL, callback,
                                         param, opID))
    success = PEERFalse;
  ...
  /////////////////////
  if (!success)
    piAddJoinRoomCallback(peer, PEERFalse, result, GroupRoom, callback, param,
                          opID);
...
static void piJoinStagingRoom(PEER peer, SBServer server, const char *channel,
                              const char password[PEER_PASSWORD_LEN],
                              peerJoinRoomCallback callback, void *param,
                              PEERBool blocking) {
  ...
  ////////////////////////////////////
  if (success && (connection->enteringRoom[StagingRoom] ||
  ...
  /////////////////////////////////////////////
  if (success && server)
    piMangleStagingRoom(room, connection->title, publicIP, privateIP,
                        privatePort);

  // Start the operation.
  ///////////////////////
  if (success &&
      !piNewJoinRoomOperation(peer, StagingRoom, server ? room : channel,
                              password, callback, param, opID))
    success = PEERFalse;
  ...
  /////////////////////
  if (!success)
    piAddJoinRoomCallback(peer, PEERFalse, result, StagingRoom, callback, param,
                          opID);
...
#ifdef GSI_UNICODE
void peerJoinStagingRoomW(PEER peer, SBServer server,
                          const unsigned short password[PEER_PASSWORD_LEN],
                          peerJoinRoomCallback callback, void *param,
                          PEERBool blocking) {
  char *password_A = UCS2ToUTF8StringAlloc(password);
...
void peerJoinStagingRoomByChannelA(PEER peer, const char *channel,
                                   const char password[PEER_PASSWORD_LEN],
                                   peerJoinRoomCallback callback, void *param,
                                   PEERBool blocking) {
...
void peerJoinStagingRoomByChannelW(
    PEER peer, const unsigned short *channel,
    const unsigned short password[PEER_PASSWORD_LEN],
    peerJoinRoomCallback callback, void *param, PEERBool blocking) {
  char *channel_A = UCS2ToUTF8StringAlloc(channel);
  char *password_A = UCS2ToUTF8StringAlloc(password);
  peerJoinStagingRoomByChannelA(peer, channel_A, password_A, callback, param,
                                blocking);
  gsifree(password_A);
  gsifree(channel_A);
}
...
void peerCreateStagingRoomA(PEER peer, const char *name, int maxPlayers,
                            const char password[PEER_PASSWORD_LEN],
                            peerJoinRoomCallback callback, void *param,
                            PEERBool blocking) {
  peerCreateStagingRoomWithSocketA(peer, name, maxPlayers, password,
                                   INVALID_SOCKET, 0, callback, param,
                                   blocking);
}
#ifdef GSI_UNICODE
void peerCreateStagingRoomW(PEER peer, const unsigned short *name,
                            int maxPlayers,
                            const unsigned short password[PEER_PASSWORD_LEN],
                            peerJoinRoomCallback callback, void *param,
                            PEERBool blocking) {
  ...
  char *password_A = UCS2ToUTF8StringAlloc(password);
  peerCreateStagingRoomA(peer, name_A, maxPlayers, password_A, callback, param,
                         blocking);
  gsifree(password_A);
  gsifree(name_A);
}
...
void peerCreateStagingRoomWithSocketA(PEER peer, const char *name,
  ...
  ///////////////////////
  if (success &&
      !piNewCreateStagingRoomOperation(peer, name, password, maxPlayers, socket,
                                       port, callback, param, opID))
    success = PEERFalse;
...
void peerCreateStagingRoomWithSocketW(
    PEER peer, const unsigned short *name, int maxPlayers,
    const unsigned short password[PEER_PASSWORD_LEN], SOCKET socket,
    unsigned short port, peerJoinRoomCallback callback, void *param,
    PEERBool blocking) {
  char *name_A = UCS2ToUTF8StringAlloc(name);
  char *password_A = UCS2ToUTF8StringAlloc(password);
  peerCreateStagingRoomWithSocketA(peer, name_A, maxPlayers, password_A, socket,
                                   port, callback, param, blocking);
  gsifree(password_A);
  gsifree(name_A);
}
#endif
...
#ifdef GSI_UNICODE
void peerLeaveRoomW(PEER peer, RoomType roomType,
...
void peerListGroupRoomsA(PEER peer, const char *fields,
                         peerListGroupRoomsCallback callback, void *param,
                         PEERBool blocking) {
  PEERBool success = PEERTrue;
  PI_OP_ID;
  PEER_CONNECTION;
  ...
  ///////////////////////////////
  if (!success)
    piAddListGroupRoomsCallback(peer, PEERFalse, 0, NULL, NULL, 0, 0, 0, 0,
                                callback, param, opID);

  PI_DO_BLOCKING;
}
#ifdef GSI_UNICODE
void peerListGroupRoomsW(PEER peer, const unsigned short *fields,
                         peerListGroupRoomsCallback callback, void *param,
                         PEERBool blocking) {
...
void peerStartListingGamesW(PEER peer, const unsigned char *fields,
                            int numFields, const unsigned short *filter,
                            peerListingGamesCallback callback, void *param) {
...
void peerMessageRoomA(PEER peer, RoomType roomType, const char *message,
...
#ifdef GSI_UNICODE
void peerMessageRoomW(PEER peer, RoomType roomType,
                      const unsigned short *message, MessageType messageType) {
...
void peerUTMRoomA(PEER peer, RoomType roomType, const char *command,
...
#ifdef GSI_UNICODE
void peerUTMRoomW(PEER peer, RoomType roomType, const unsigned short *command,
                  const unsigned short *parameters, PEERBool authenticate) {
...
void peerSetPasswordA(PEER peer, RoomType roomType,
                      const char password[PEER_PASSWORD_LEN]) {
  PEER_CONNECTION;
  PEER_CONNECTED;
...
#ifdef GSI_UNICODE
void peerSetPasswordW(PEER peer, RoomType roomType,
                      const unsigned short password[PEER_PASSWORD_LEN]) {
  char *password_A = UCS2ToUTF8StringAlloc(password);
...
#ifdef GSI_UNICODE
void peerSetRoomNameW(PEER peer, RoomType roomType,
...
static void piEnumPlayersEnumRoomPlayersCallback(PEER peer, RoomType roomType,
                                                 piPlayer *player, int index,
                                                 void *param) {
...
void peerEnumPlayers(PEER peer, RoomType roomType,
  ...
  data.param = param;
  piEnumRoomPlayers(peer, roomType, piEnumPlayersEnumRoomPlayersCallback,
                    &data);
}
...
#ifdef GSI_UNICODE
void peerMessagePlayerW(PEER peer, const unsigned short *nick,
...
#ifdef GSI_UNICODE
void peerUTMPlayerW(PEER peer, const unsigned short *nick,
...
void peerKickPlayerA(PEER peer, RoomType roomType, const char *nick,
                     const char *reason) {
  PEER_CONNECTION;
  PEER_CONNECTED;
...
#ifdef GSI_UNICODE
void peerKickPlayerW(PEER peer, RoomType roomType, const unsigned short *nick,
                     const unsigned short *reason) {
  char *nick_A = UCS2ToUTF8StringAlloc(nick);
  char *reason_A = UCS2ToUTF8StringAlloc(reason);
...
#ifdef GSI_UNICODE
PEERBool peerGetPlayersCrossPingW(PEER peer, const unsigned short *nick1,
                                  const unsigned short *nick2, int *crossPing) {
...
PEERBool peerGetPlayerInfoNoWaitA(PEER peer, const char *nick, unsigned int *IP,
                                  int *profileID) {
  piPlayer *player;
...
#ifdef GSI_UNICODE
PEERBool peerGetPlayerInfoNoWaitW(PEER peer, const unsigned short *nick,
...
void peerGetPlayerInfoA(PEER peer, const char *nick,
  ...
  /////////////////////////////
  if (player && player->gotIPAndProfileID) {
    piAddGetPlayerInfoCallback(peer, PEERTrue, nick, player->IP,
  ...
  ///////////////////////////////
  if (!success)
    piAddGetPlayerInfoCallback(peer, PEERFalse, nick, 0, 0, callback, param,
                               opID);

  PI_DO_BLOCKING;
}
#ifdef GSI_UNICODE
void peerGetPlayerInfoW(PEER peer, const unsigned short *nick,
                        peerGetPlayerInfoCallback callback, void *param,
                        PEERBool blocking) {
  char *nick_A = UCS2ToUTF8StringAlloc(nick);
...
void peerGetPlayerProfileIDA(PEER peer, const char *nick,
  ...
  /////////////////////////////
  if (player && player->gotIPAndProfileID) {
    piAddGetPlayerProfileIDCallback(peer, PEERTrue, nick, player->profileID,
                                    callback, param, opID);
  } else {
    // Start an op to get it.
    /////////////////////////
  ...
  ///////////////////////////////
  if (!success)
    piAddGetPlayerProfileIDCallback(peer, PEERFalse, nick, 0, callback, param,
                                    opID);

  PI_DO_BLOCKING;
}
#ifdef GSI_UNICODE
void peerGetPlayerProfileIDW(PEER peer, const unsigned short *nick,
                             peerGetPlayerProfileIDCallback callback,
                             void *param, PEERBool blocking) {
  char *nick_A = UCS2ToUTF8StringAlloc(nick);
...
void peerGetPlayerIPA(PEER peer, const char *nick,
                      peerGetPlayerIPCallback callback, void *param,
                      PEERBool blocking) {
  PEERBool success = PEERTrue;
  piPlayer *player;
  PI_OP_ID;
  ...
  ///////////////////////////////
  if (player && player->gotIPAndProfileID) {
    piAddGetPlayerIPCallback(peer, PEERTrue, nick, player->IP, callback, param,
                             opID);
  } else {
    // Start an op to get it.
    /////////////////////////
...
#ifdef GSI_UNICODE
void peerGetPlayerIPW(PEER peer, const unsigned short *nick,
                      peerGetPlayerIPCallback callback, void *param,
                      PEERBool blocking) {
  char *nick_A = UCS2ToUTF8StringAlloc(nick);
...
#ifdef GSI_UNICODE
PEERBool peerIsPlayerHostW(PEER peer, const unsigned short *nick,
                           RoomType roomType) {
  char *nick_A = UCS2ToUTF8StringAlloc(nick);
...
PEERBool peerGetPlayerFlagsA(PEER peer, const char *nick, RoomType roomType,
                             int *flags) {
  piPlayer *player;
...
#ifdef GSI_UNICODE
PEERBool peerGetPlayerFlagsW(PEER peer, const unsigned short *nick,
...
void peerSetReady(PEER peer, PEERBool ready) {
  ...
  // THIS IS ONLY NEEDED FOR BACKWARDS COMPATIBILITY AND SHOULD BE REMOVED AT
...
static void piAreAllReadyEnumRoomPlayersCallback(PEER peer, RoomType roomType,
                                                 piPlayer *player, int index,
                                                 void *param) {
  if (player) {
...
PEERBool peerAreAllReady(PEER peer) {
  ...
  ///////////////////////////////////////
  allReady = PEERTrue;
  piEnumRoomPlayers(peer, StagingRoom, piAreAllReadyEnumRoomPlayersCallback,
                    &allReady);
...
void peerStartGameA(PEER peer, const char *message, int reportingOptions) {
  ...
  // Send the launch UTM.
  ///////////////////////
  piSendChannelUTM(peer, connection->rooms[StagingRoom], PI_UTM_LAUNCH, message,
                   PEERFalse);
...
#ifdef GSI_UNICODE
void peerStartGameW(PEER peer, const unsigned short *message,
...
PEERBool peerStartReportingWithSocket(PEER peer, SOCKET socket,
                                      unsigned short port) {
  PEER_CONNECTION;
...
void piSendChannelUTM(PEER peer, const char *channel, const char *command,
  ...
  chatSendChannelMessageA(connection->chat, channel, buffer,
                          authenticate ? CHAT_ATM : CHAT_UTM);
}
...
#ifdef GSI_UNICODE
void peerSetGlobalKeysW(PEER peer, int num, const unsigned short **keys,
...
void peerGetPlayerGlobalKeysA(PEER peer, const char *nick, int num,
  ...
  /////////////////////
  if (!success)
    piAddGetGlobalKeysCallback(peer, PEERFalse, nick, 0, NULL, NULL, callback,
                               param, opID);

  PI_DO_BLOCKING;
}
#ifdef GSI_UNICODE
void peerGetPlayerGlobalKeysW(PEER peer, const unsigned short *nick, int num,
  ...
  char **keys_A = UCS2ToUTF8StringArrayAlloc(keys, num);
  int i;
  peerGetPlayerGlobalKeysA(peer, nick_A, num, (const char **)keys_A, callback,
                           param, blocking);
  gsifree(nick_A);
  for (i = 0; i < num; i++)
    gsifree(keys_A[i]);
...
void peerGetRoomGlobalKeysA(PEER peer, RoomType roomType, int num,
                            const char **keys,
                            peerGetGlobalKeysCallback callback, void *param,
                            PEERBool blocking) {
  PEERBool success = PEERTrue;
  ...
  // Start the operation.
  ///////////////////////
  if (!piNewGetGlobalKeysOperation(peer, ROOM, num, keys, callback, param,
                                   opID))
    success = PEERFalse;
  ...
  /////////////////////
  if (!success)
    piAddGetGlobalKeysCallback(peer, PEERFalse, "", 0, NULL, NULL, callback,
                               param, opID);

  PI_DO_BLOCKING;
}
#ifdef GSI_UNICODE
void peerGetRoomGlobalKeysW(PEER peer, RoomType roomType, int num,
                            const unsigned short **keys,
                            peerGetGlobalKeysCallback callback, void *param,
                            PEERBool blocking) {
  char **keys_A = UCS2ToUTF8StringArrayAlloc(keys, num);
  int i;
  peerGetRoomGlobalKeysA(peer, roomType, num, (const char **)keys_A, callback,
                         param, blocking);
  for (i = 0; i < num; i++)
    gsifree(keys_A[i]);
  gsifree(keys_A);
...
void peerSetRoomKeysA(PEER peer, RoomType roomType, const char *nick, int num,
...
#ifdef GSI_UNICODE
void peerSetRoomKeysW(PEER peer, RoomType roomType, const unsigned short *nick,
                      int num, const unsigned short **keys,
                      const unsigned short **values) {
  char *nick_A = UCS2ToUTF8StringAlloc(nick);
  char **keys_A = UCS2ToUTF8StringArrayAlloc(keys, num);
  char **values_A = UCS2ToUTF8StringArrayAlloc(values, num);
  int i;
  peerSetRoomKeysA(peer, roomType, nick_A, num, (const char **)keys_A,
                   (const char **)values_A);
  gsifree(nick_A);
  for (i = 0; i < num; i++) {
    gsifree(keys_A[i]);
...
void peerGetRoomKeysA(PEER peer, RoomType roomType, const char *nick, int num,
                      const char **keys, peerGetRoomKeysCallback callback,
                      void *param, PEERBool blocking) {
  PEERBool success = PEERTrue;
  ...
  /////////////////////
  if (!success)
    piAddGetRoomKeysCallback(peer, PEERFalse, roomType, nick, 0, NULL, NULL,
                             callback, param, opID);

  PI_DO_BLOCKING;
}
#ifdef GSI_UNICODE
void peerGetRoomKeysW(PEER peer, RoomType roomType, const unsigned short *nick,
                      int num, const unsigned short **keys,
  ...
  char **keys_A = UCS2ToUTF8StringArrayAlloc(keys, num);
  int i;
  peerGetRoomKeysA(peer, roomType, nick_A, num, (const char **)keys_A, callback,
                   param, blocking);
  gsifree(nick_A);
  for (i = 0; i < num; i++)
    gsifree(keys_A[i]);
...
#ifdef GSI_UNICODE
void peerSetGlobalWatchKeysW(PEER peer, RoomType roomType, int num,
                             const unsigned short **keys, PEERBool addKeys) {
  char **keys_A = UCS2ToUTF8StringArrayAlloc(keys, num);
  int i;
...
#ifdef GSI_UNICODE
void peerSetRoomWatchKeysW(PEER peer, RoomType roomType, int num,
                           const unsigned short **keys, PEERBool addKeys) {
  char **keys_A = UCS2ToUTF8StringArrayAlloc(keys, num);
  int i;
  peerSetGlobalWatchKeysA(peer, roomType, num, (const char **)keys_A, addKeys);
...
#ifdef GSI_UNICODE
const unsigned short *peerGetGlobalWatchKeyW(PEER peer,
...
const char *peerGetRoomWatchKeyA(PEER peer, RoomType roomType, const char *nick,
                                 const char *key) {
  return piGetRoomWatchKeyA(peer, roomType, nick, key);
}
#ifdef GSI_UNICODE
const unsigned short *peerGetRoomWatchKeyW(PEER peer, RoomType roomType,
                                           const unsigned short *nick,
                                           const unsigned short *key) {
...
void peerStartAutoMatchA(PEER peer, int maxPlayers, const char *filter,
                         peerAutoMatchStatusCallback statusCallback,
                         peerAutoMatchRateCallback rateCallback, void *param,
                         PEERBool blocking) {
  peerStartAutoMatchWithSocketA(peer, maxPlayers, filter, INVALID_SOCKET, 0,
                                statusCallback, rateCallback, param, blocking);
}
#ifdef GSI_UNICODE
void peerStartAutoMatchW(PEER peer, int maxPlayers,
                         const unsigned short *filter,
                         peerAutoMatchStatusCallback statusCallback,
                         peerAutoMatchRateCallback rateCallback, void *param,
                         PEERBool blocking) {
  char *filter_A = UCS2ToUTF8StringAlloc(filter);
  peerStartAutoMatchA(peer, maxPlayers, filter_A, statusCallback, rateCallback,
                      param, blocking);
  gsifree(filter_A);
}
#endif
...
void peerStartAutoMatchWithSocketA(PEER peer, int maxPlayers,
  ...
  // Start the AutoMatch.
  ///////////////////////
  if (!piNewAutoMatchOperation(peer, socket, port, statusCallback, rateCallback,
                               param, opID)) {
...
#ifdef GSI_UNICODE
void peerStartAutoMatchWithSocketW(PEER peer, int maxPlayers,
                                   const unsigned short *filter, SOCKET socket,
                                   unsigned short port,
                                   peerAutoMatchStatusCallback statusCallback,
                                   peerAutoMatchRateCallback rateCallback,
                                   void *param, PEERBool blocking) {
  char *filter_A = UCS2ToUTF8StringAlloc(filter);
  peerStartAutoMatchWithSocketA(peer, maxPlayers, filter_A, socket, port,
                                statusCallback, rateCallback, param, blocking);
  gsifree(filter_A);
}
#endif
...
void peerSetStagingRoomMaxPlayers(PEER peer, int maxPlayers) {
  ...
  if (connection->inRoom[StagingRoom]) {
    ...
    chatSetChannelLimitA(connection->chat, connection->rooms[StagingRoom],
                         maxPlayers);
  }
}
