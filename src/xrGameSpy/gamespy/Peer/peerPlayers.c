...
static int GS_STATIC_CALLBACK piPlayersTableCompareFn(const void *elem1,
                                                      const void *elem2) {
...
PEERBool piPlayersInit(PEER peer) {
  ...
  //////////////////////////
  connection->players =
      TableNew(sizeof(piPlayer), PI_PLAYERS_NUM_BUCKETS, piPlayersTableHashFn,
               piPlayersTableCompareFn, piPlayersTableElementFreeFn);
...
static void piEnumRoomPlayersMap(void *elem, void *clientdata) {
  ...
  if (player->inRoom[data->roomType]) {
    // Call the callback.
    /////////////////////
    data->callback(data->peer, data->roomType, player, data->count,
                   data->param);
...
static void piSetNewPlayerFlags(PEER peer, const char *nick, RoomType roomType,
                                int flags) {
  piPlayer *player;
  int oldFlags;
...
void piSetPlayerModeFlags(PEER peer, const char *nick, RoomType roomType,
                          int mode) {
  piPlayer *player;
  int nFlags;
...
PEERBool piIsPlayerVIP(piPlayer *player, RoomType roomType) {
  ...
  return (PEERBool)(
      (player->flags[roomType] & (PEER_FLAG_OP | PEER_FLAG_VOICE)) != 0);
}
...
PEERBool piIsPlayerHost(piPlayer *player) {
  ...
  return (PEERBool)(
      (player->flags[StagingRoom] & (PEER_FLAG_OP | PEER_FLAG_HOST)) ==
