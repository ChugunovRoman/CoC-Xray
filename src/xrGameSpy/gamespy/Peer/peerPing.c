...
static int GS_STATIC_CALLBACK piXpingTableCompareFn(const void *param1,
                                                    const void *param2) {
...
static void piPinged(unsigned int IP, unsigned short port, int ping,
...
PEERBool piPingInit(PEER peer) {
  ...
  ////////////////////////
  connection->xpings =
      TableNew(sizeof(piXping), PI_XPING_NUM_BUCKETS, piXpingTableHashFn,
               piXpingTableCompareFn, piXpingTableElementFreeFn);
...
static void piPingerReply(unsigned int IP, unsigned short port, int ping,
...
static void piPingPlayer(PEER peer, piPlayer *player) {
  ...
  // Do the ping.
  ///////////////
  pingerPing(player->IP, PI_PINGER_PORT, piPingerReply, peer, PINGERFalse,
             PI_PING_TIMEOUT);
...
static void piPickPingPlayersMap(void *elem, void *clientData) {
  ...
  if ((player->inRoom[TitleRoom] &&
       (player->flags[TitleRoom] & PI_DONT_PING_FLAGS)) ||
      (player->inRoom[GroupRoom] &&
       (player->flags[GroupRoom] & PI_DONT_PING_FLAGS)) ||
      (player->inRoom[StagingRoom] &&
       (player->flags[StagingRoom] & PI_DONT_PING_FLAGS)))
    return;
  ...
  for (i = (data->max - 1); i >= 0; i--) {
    ...
    if ((!other->numPings && player->numPings) || // Not pinged.
        (other->inRoom[StagingRoom] &&
         !player->inRoom[StagingRoom]) || // In staging room.
        (piIsPlayerVIP(other, StagingRoom) &&
         !piIsPlayerVIP(
             player, StagingRoom)) || // Op or voice in the same staging room.
...
static piPlayer **piPickPingPlayers(PEER peer, int *numPings) {
  ...
  ////////////////////////
  if (!connection->players || !(*numPings) ||
      !TableCount(connection->players)) {
    *numPings = 0;
    return NULL;
  }
