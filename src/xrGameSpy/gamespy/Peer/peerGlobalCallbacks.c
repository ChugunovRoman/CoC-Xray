...
static void piProcessUTM(PEER peer, piPlayer *player, PEERBool inRoom,
                         RoomType roomType) {
...
void piChatPrivateMessageA(CHAT chat, const char *user, const char *message,
  ...
  if ((type == CHAT_UTM) || (type == CHAT_ATM)) {
    if (piParseUTM(message)) {
      ...
      // Pass it along.
      /////////////////
      piAddPlayerUTMCallback(peer, user, piUTMCommand, piUTMParameters,
                             (PEERBool)(type == CHAT_ATM));
    }
...
static void piChannelMessageA(CHAT chat, const char *channel, const char *user,
                              const char *message, int type, PEER peer) {
  piPlayer *player;
  RoomType roomType;
  ...
  if ((type == CHAT_UTM) || (type == CHAT_ATM)) {
    if (piParseUTM(message)) {
      ...
      // Pass it along.
      /////////////////
      piAddRoomUTMCallback(peer, roomType, user, piUTMCommand, piUTMParameters,
                           (type == CHAT_ATM) ? PEERTrue : PEERFalse);
    }
...
static void piChannelKickedA(CHAT chat, const char *channel, const char *user,
                             const char *reason, PEER peer) {
  RoomType roomType;
...
static void piChannelKickedW(CHAT chat, const unsigned short *channel,
                             const unsigned short *user,
                             const unsigned short *reason, PEER peer) {
  char *channel_A = UCS2ToUTF8StringAlloc(channel);
  char *user_A = UCS2ToUTF8StringAlloc(user);
  char *reason_A = UCS2ToUTF8StringAlloc(reason);
...
static void piChannelUserJoinedA(CHAT chat, const char *channel,
                                 const char *user, int mode, PEER peer) {
  RoomType roomType;
  piPlayer *player;
  ...
  if (!player->gotIPAndProfileID) {
    ...
    if (chatGetBasicUserInfoNoWaitA(connection->chat, user, &info, NULL) &&
...
#ifdef GSI_UNICODE
static void piChannelUserJoinedW(CHAT chat, const unsigned short *channel,
...
static void piChannelUserPartedA(CHAT chat, const char *channel,
                                 const char *user, int why, const char *reason,
                                 const char *kicker, PEER peer) {
  RoomType roomType;
  ...
  // Check if this is an AutoMatch room.
  //////////////////////////////////////
  autoMatchRoom = ((roomType == StagingRoom) && peerIsAutoMatching(peer))
                      ? PEERTrue
                      : PEERFalse;
  if (autoMatchRoom) {
    piPlayer *player;
...
static void piChannelUserPartedW(CHAT chat, const unsigned short *channel,
                                 const unsigned short *user, int why,
                                 const unsigned short *reason,
                                 const unsigned short *kicker, PEER peer) {
  char *channel_A = UCS2ToUTF8StringAlloc(channel);
  char *user_A = UCS2ToUTF8StringAlloc(user);
  char *reason_A = UCS2ToUTF8StringAlloc(reason);
...
static void piChannelUserChangedNickA(CHAT chat, const char *channel,
...
static void piChannelTopicChangedA(CHAT chat, const char *channel,
                                   const char *topic, PEER peer) {
  RoomType roomType;
...
#ifdef GSI_UNICODE
static void piChannelTopicChangedW(CHAT chat, const unsigned short *channel,
                                   const unsigned short *topic, PEER peer) {
  char *channel_A = UCS2ToUTF8StringAlloc(channel);
...
static void piChannelNewUserListA(CHAT chat, const char *channel, int num,
                                  const char **users, int *modes, PEER peer) {
  int i;
  RoomType roomType;
...
static void piChannelNewUserListW(CHAT chat, const unsigned short *channel,
                                  int num, const unsigned short **users,
                                  int *modes, PEER peer) {
  char *channel_A = UCS2ToUTF8StringAlloc(channel);
  char **users_A = UCS2ToUTF8StringArrayAlloc(users, num);
  int i;
  piChannelNewUserListA(chat, channel_A, num, (const char **)users_A, modes,
                        peer);
  gsifree(channel_A);
  for (i = 0; i < num; i++)
    gsifree(users_A[i]);
...
static void piBroadcastKeyChangedA(CHAT chat, const char *channel,
                                   const char *user, const char *key,
                                   const char *value, PEER peer) {
  RoomType roomType;
...
#ifdef GSI_UNICODE
static void piBroadcastKeyChangedW(CHAT chat, const unsigned short *channel,
                                   const unsigned short *user,
                                   const unsigned short *key,
                                   const unsigned short *value, PEER peer) {
  char *channel_A = UCS2ToUTF8StringAlloc(channel);
  char *user_A = UCS2ToUTF8StringAlloc(user);
...
static void piUserModeChangedA(CHAT chat, const char *channel, const char *user,
                               int mode, PEER peer) {
  RoomType roomType;
...
#ifdef GSI_UNICODE
static void piUserModeChangedW(CHAT chat, const unsigned short *channel,
                               const unsigned short *user, int mode,
                               PEER peer) {
  char *channel_A = UCS2ToUTF8StringAlloc(channel);
  char *user_A = UCS2ToUTF8StringAlloc(user);
...
static void piChannelModeChangedA(CHAT chat, const char *channel,
                                  CHATChannelMode *mode, PEER peer) {
  RoomType roomType;
...
#ifdef GSI_UNICODE
static void piChannelModeChangedW(CHAT chat, const unsigned short *channel,
                                  CHATChannelMode *mode, PEER peer) {
  char *channel_A = UCS2ToUTF8StringAlloc(channel);
