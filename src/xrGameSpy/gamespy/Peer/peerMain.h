...
#define ASSERT_ROOMTYPE(type)                                                  \
  assert((type == TitleRoom) || (type == GroupRoom) || (type == StagingRoom))
#define ASSERT_MESSAGETYPE(type)                                               \
  assert((type == NormalMessage) || (type == ActionMessage) ||                 \
         (type == NoticeMessage))
...
#define strzcat(dest, src, len)                                                \
  {                                                                            \
    strncat(dest, src, (len)-strlen(dest));                                    \
    (dest)[(len)-1] = '\0';                                                    \
  }

#if defined(_PS3)
#define PEERCBType                                                             \
  void * // Note: ANSI function pointers should int rather than void*
#else
#define PEERCBType                                                             \
  int // Note: ANSI function pointers should int rather than void*
#endif
...
typedef struct piConnection {
  ...
  int reportingGroupID; // might be different than groupID if left group room
                        // after started reporting
