...
#define piCreateStagingRoomEnumUsersCallback                                   \
  piCreateStagingRoomEnumUsersCallbackA
#define piCreateStagingRoomEnterChannelCallback                                \
  piCreateStagingRoomEnterChannelCallbackA
...
#define piCreateStagingRoomEnumUsersCallback                                   \
  piCreateStagingRoomEnumUsersCallbackW
#define piCreateStagingRoomEnterChannelCallback                                \
  piCreateStagingRoomEnterChannelCallbackW
...
PEERBool piOperationsInit(PEER peer) {
  ...
  // Init the list.
  /////////////////
  connection->operationList =
      ArrayNew(sizeof(piOperationContainer), 0, piOperationsListFree);
...
void piCancelJoinOperation(PEER peer, RoomType roomType) {
  ...
  for (i = 0; i < count; i++) {
    ...
    //////////////////////////
    if ((operation->type == PI_JOIN_ROOM_OPERATION) ||
        (operation->type == PI_CREATE_ROOM_OPERATION)) {
      // Is it the same room?
      ///////////////////////
...
static piOperation *piAddOperation(PEER peer, piOperationType type, void *data,
                                   PEERCBType callback, void *callbackParam,
                                   int opID) {
  piOperation *operation;
  piOperationContainer container;
...
/* Connect.
**********/
static void piConnectConnectCallback(CHAT chat, CHATBool success,
  ...
  // Add the callback.
  ////////////////////
  piAddConnectCallback(peer, (PEERBool)success, failureReason,
...
static void piConnectNickErrorCallbackA(CHAT chat, int type, const char *nick,
  ...
  /////////////////////////
  piAddNickErrorCallback(peer, type, nick, numSuggestedNicks, suggestedNicks,
                         operation->callbackParam, operation->ID);

  GSI_UNUSED(chat);
}
#ifdef GSI_UNICODE
static void piConnectNickErrorCallbackW(CHAT chat, int type,
                                        const unsigned short *nick,
                                        int numSuggestedNicks,
                                        const unsigned short **suggestedNicks,
                                        void *param) {
  char *nick_A = UCS2ToUTF8StringAlloc(nick);
  char **suggestedNicks_A =
      UCS2ToUTF8StringArrayAlloc(suggestedNicks, numSuggestedNicks);
  int i;
  piConnectNickErrorCallbackA(chat, type, nick_A, numSuggestedNicks,
                              (const char **)suggestedNicks_A, param);
  gsifree(nick_A);
  if (suggestedNicks_A) {
...
static void piConnectFillInUserCallbackA(CHAT chat, unsigned int IP,
                                         char user[128], void *param) {
  int pid;

  PEER_CONNECTION_OP;

  // 2004.Sep.17.JED - For reasons unknown, in the span of one month Arcade's
  // automated
  //   crash handling system reported +15 null pointer crashes when publicIP was
  //   stored.
  //////////////////////////////////////////////////////////////////////////////////////
  if (!connection)
    return;
...
#ifdef GSI_UNICODE
static void
piConnectFillInUserCallbackW(CHAT chat, unsigned int IP,
...
PEERBool piNewConnectOperation(
    PEER peer, piConnectType connectType, const char *nick, int namespaceID,
    ...
    peerConnectCallback callback, void *callbackParam, int opID) {
  piOperation *operation;
  ...
  // Add an operation.
  ////////////////////
  operation = piAddOperation(peer, PI_CONNECT_OPERATION, NULL,
                             (PEERCBType)callback, callbackParam, opID);
  if (!operation)
    return PEERFalse;
  ...
  if (connectType == PI_CONNECT) {
    connection->chat = chatConnectSecureA(
        PI_CHAT_SERVER_ADDRESS, PI_CHAT_SERVER_PORT, nick, encodedUniqueID,
        connection->sbName, connection->sbSecretKey, &globalCallbacks,
        nickErrorCallback, piConnectFillInUserCallback,
        piConnectConnectCallback, operation, CHATFalse);
  } else if ((connectType == PI_CONNECT_UNIQUENICK_LOGIN) ||
             (connectType == PI_CONNECT_PROFILENICK_LOGIN)) {
    connection->chat = chatConnectLoginA(
        PI_CHAT_SERVER_ADDRESS, PI_CHAT_SERVER_PORT, namespaceID, email,
        profilenick, uniquenick, password, encodedUniqueID, connection->sbName,
        connection->sbSecretKey, &globalCallbacks, nickErrorCallback,
        piConnectFillInUserCallback, piConnectConnectCallback, operation,
        CHATFalse);
  } else if (connectType == PI_CONNECT_PREAUTH) {
    connection->chat = chatConnectPreAuthA(
        PI_CHAT_SERVER_ADDRESS, PI_CHAT_SERVER_PORT, authtoken,
...
static void piCreateStagingRoomEnumUsersCallbackA(CHAT chat, CHATBool success,
                                                  const char *channel,
                                                  int numUsers,
                                                  const char **users,
  ...
  // Add the callback.
  ////////////////////
  piAddJoinRoomCallback(peer, (PEERBool)success,
                        success ? PEERJoinSuccess : PEERJoinFailed, StagingRoom,
...
#ifdef GSI_UNICODE
static void piCreateStagingRoomEnumUsersCallbackW(CHAT chat, CHATBool success,
  ...
  int i;
  piCreateStagingRoomEnumUsersCallbackA(chat, success, channel_A, numUsers,
                                        (const char **)users_A, modes, param);
  gsifree(channel_A);
  for (i = 0; i < numUsers; i++)
    gsifree(users_A[i]);
  gsifree(users_A);
...
static void piCreateStagingRoomEnterChannelCallbackA(CHAT chat,
  ...
  if (success) {
    ...
    if (operation->password)
      chatSetChannelPasswordA(connection->chat, channel, CHATTrue,
                              operation->password);

    // get the channel modes so we can set the limits on our staging channel
    chatGetChannelMode(connection->chat, peerGetRoomChannel(peer, StagingRoom),
                       piCreateStagingRoomGetChannelModeCallback,
                       (void *)connection, CHATFalse);

    // How many users?
    //////////////////
    chatEnumUsersA(chat, channel, piCreateStagingRoomEnumUsersCallback,
                   operation, CHATFalse);
  } else {
    ...
    // Add the callback.
    ////////////////////
    piAddJoinRoomCallback(peer, PEERFalse, joinResult, StagingRoom,
                          (peerJoinRoomCallback)operation->callback,
                          operation->callbackParam, operation->ID);
...
static void piCreateStagingRoomEnterChannelCallbackW(
    CHAT chat, CHATBool success, CHATEnterResult result,
    const unsigned short *channel, void *param) {
  char *channel_A = UCS2ToUTF8StringAlloc(channel);
  piCreateStagingRoomEnterChannelCallbackA(chat, success, result, channel_A,
                                           param);
  gsifree(channel_A);
}
#endif
...
PEERBool piNewCreateStagingRoomOperation(PEER peer, const char *name,
                                         const char *password, int maxPlayers,
  ...
  if (socket == INVALID_SOCKET) {
    ...
    addr.s_addr = 0; // crt -- don't bind to privateIP (for clients that have
                     // both public and private)  connection->privateIP;
    if (port)
      privatePort = port;
    else
  ...
  // Add the operation.
  /////////////////////
  operation = piAddOperation(peer, PI_CREATE_ROOM_OPERATION, NULL,
                             (PEERCBType)callback, callbackParam, opID);
  if (!operation)
    return PEERFalse;
  ...
  chatEnterChannelA(connection->chat, room, NULL, &channelCallbacks,
                    piCreateStagingRoomEnterChannelCallback, operation,
                    CHATFalse);
...
/* Join Room.
************/
static void piJoinRoomEnumUsersCallbackA(CHAT chat, CHATBool success,
                                         const char *channel, int numUsers,
                                         const char **users, int *modes,
                                         void *param) {
  PEER_CONNECTION_OP;
  ...
  // Add the callback.
  ////////////////////
  piAddJoinRoomCallback(
      peer, (PEERBool)success, success ? PEERJoinSuccess : PEERJoinFailed,
      operation->roomType, (peerJoinRoomCallback)operation->callback,
      operation->callbackParam, operation->ID);
...
#ifdef GSI_UNICODE
static void piJoinRoomEnumUsersCallbackW(CHAT chat, CHATBool success,
                                         const unsigned short *channel,
                                         int numUsers,
                                         const unsigned short **users,
                                         int *modes, void *param) {
  ...
  char **users_A = UCS2ToUTF8StringArrayAlloc(users, numUsers);
  int i;
  piJoinRoomEnumUsersCallbackA(chat, success, channel_A, numUsers,
                               (const char **)users_A, modes, param);
  gsifree(channel_A);
  for (i = 0; i < numUsers; i++)
    gsifree(users_A[i]);
...
static void piJoinRoomEnterChannelCallbackA(CHAT chat, CHATBool success,
                                            CHATEnterResult result,
  ...
  if (success) {
    // How many users?
    //////////////////
    chatEnumUsersA(chat, channel, piJoinRoomEnumUsersCallback, operation,
                   CHATFalse);
  } else {
    PEERJoinResult joinResult;
    ...
    // Add the callback.
    ////////////////////
    piAddJoinRoomCallback(peer, PEERFalse, joinResult, operation->roomType,
                          (peerJoinRoomCallback)operation->callback,
                          operation->callbackParam, operation->ID);
...
#ifdef GSI_UNICODE
static void piJoinRoomEnterChannelCallbackW(CHAT chat, CHATBool success,
                                            CHATEnterResult result,
                                            const unsigned short *channel,
                                            void *param) {
  char *channel_A = UCS2ToUTF8StringAlloc(channel);
...
PEERBool piNewJoinRoomOperation(PEER peer, RoomType roomType,
  ...
  // Add the operation.
  /////////////////////
  operation = piAddOperation(peer, PI_JOIN_ROOM_OPERATION, NULL,
                             (PEERCBType)callback, callbackParam, opID);
  if (!operation)
    return PEERFalse;
  ...
  chatEnterChannelA(connection->chat, channel, password, &channelCallbacks,
                    piJoinRoomEnterChannelCallback, operation, CHATFalse);
...
PEERBool piNewListGroupRoomsOperation(PEER peer, const char *fields,
  ...
  operation = connection->listingGroupsOperation =
      piAddOperation(peer, PI_LIST_GROUP_ROOMS_OPERATION, NULL,
                     (PEERCBType)callback, param, opID);
  if (!operation)
    return PEERFalse;
...
/* Get Player Info.
*******************/
static void piGetPlayerInfoCallbackA(CHAT chat, CHATBool success,
  ...
  if (operation->callback) {
    if (operation->type == PI_GET_PLAYER_INFO_OPERATION)
      piAddGetPlayerInfoCallback(peer, (PEERBool)success, nick, IP, profileID,
                                 (peerGetPlayerInfoCallback)operation->callback,
                                 operation->callbackParam, operation->ID);
    else if (operation->type == PI_GET_PROFILE_ID_OPERATION)
      piAddGetPlayerProfileIDCallback(
          peer, (PEERBool)success, nick, profileID,
          (peerGetPlayerProfileIDCallback)operation->callback,
          operation->callbackParam, operation->ID);
    else if (operation->type == PI_GET_IP_OPERATION)
      piAddGetPlayerIPCallback(peer, (PEERBool)success, nick, IP,
...
#ifdef GSI_UNICODE
static void piGetPlayerInfoCallbackW(CHAT chat, CHATBool success,
                                     const unsigned short *nick,
...
PEERBool piNewGetPlayerInfoOperation(PEER peer, const char *nick,
  ...
  // Add the operation.
  /////////////////////
  operation = piAddOperation(peer, PI_GET_PLAYER_INFO_OPERATION, NULL,
                             (PEERCBType)callback, param, opID);
  if (!operation)
    return PEERFalse;
  ...
  // Get the user's info.
  ///////////////////////
  chatGetBasicUserInfoA(connection->chat, nick, piGetPlayerInfoCallback,
                        operation, CHATFalse);
...
PEERBool piNewGetProfileIDOperation(PEER peer, const char *nick,
  ...
  // Add the operation.
  /////////////////////
  operation = piAddOperation(peer, PI_GET_PROFILE_ID_OPERATION, NULL,
                             (PEERCBType)callback, param, opID);
  if (!operation)
    return PEERFalse;
...
PEERBool piNewGetIPOperation(PEER peer, const char *nick,
  ...
  // Add the operation.
  /////////////////////
  operation = piAddOperation(peer, PI_GET_IP_OPERATION, NULL,
                             (PEERCBType)callback, param, opID);
  if (!operation)
    return PEERFalse;
...
/* Change Nick.
**************/
static void piChangeNickCallbackA(CHAT chat, CHATBool success,
  ...
  ////////////////////
  if (operation->callback)
    piAddChangeNickCallback(peer, (PEERBool)success, oldNick, newNick,
...
#ifdef GSI_UNICODE
static void piChangeNickCallbackW(CHAT chat, CHATBool success,
                                  const unsigned short *oldNick,
                                  const unsigned short *newNick, void *param) {
...
PEERBool piNewChangeNickOperation(PEER peer, const char *newNick,
  ...
  // Add the operation.
  /////////////////////
  operation = piAddOperation(peer, PI_CHANGE_NICK_OPERATION, NULL,
                             (PEERCBType)callback, param, opID);
  if (!operation)
    return PEERFalse;
  ...
  // Change the nick.
  ///////////////////
  chatChangeNickA(connection->chat, newNick, piChangeNickCallback, operation,
                  CHATFalse);
...
/* Get Global Keys.
******************/
static void piGetGlobalKeysCallbackA(CHAT chat, CHATBool success,
                                     const char *user, int num,
  ...
  ////////////////////
  if (operation->callback)
    piAddGetGlobalKeysCallback(peer, (PEERBool)success, user, num, keys, values,
...
#ifdef GSI_UNICODE
static void piGetGlobalKeysCallbackW(CHAT chat, CHATBool success,
  ...
  int i;
  piGetGlobalKeysCallbackA(chat, success, user_A, num, (const char **)keys_A,
                           (const char **)values_A, param);
  gsifree(user_A);
  for (i = 0; i < num; i++) {
    gsifree(keys_A[i]);
...
PEERBool piNewGetGlobalKeysOperation(PEER peer, const char *target, int num,
  ...
  // Add the operation.
  /////////////////////
  operation = piAddOperation(peer, PI_GET_GLOBAL_KEYS_OPERATION, NULL,
                             (PEERCBType)callback, param, opID);
  if (!operation)
    return PEERFalse;
  ...
  chatGetGlobalKeysA(connection->chat, target, num, keys,
                     piGetGlobalKeysCallback, operation, CHATFalse);
...
/* Get Room Keys.
****************/
static void piGetChannelKeysCallbackA(CHAT chat, CHATBool success,
  ...
  ////////////////////
  if (operation->callback)
    piAddGetRoomKeysCallback(peer, (PEERBool)success, operation->roomType, user,
                             num, keys, values,
...
#ifdef GSI_UNICODE
static void piGetChannelKeysCallbackW(CHAT chat, CHATBool success,
                                      const unsigned short *channel,
                                      const unsigned short *user, int num,
                                      const unsigned short **keys,
                                      const unsigned short **values,
  ...
  char **values_A = UCS2ToUTF8StringArrayAlloc(values, num);
  int i;
  piGetChannelKeysCallbackA(chat, success, channel_A, user_A, num,
                            (const char **)keys_A, (const char **)values_A,
                            param);
  gsifree(channel_A);
  gsifree(user_A);
  for (i = 0; i < num; i++) {
...
PEERBool piNewGetRoomKeysOperation(PEER peer, RoomType roomType,
  ...
  // Add the operation.
  /////////////////////
  operation = piAddOperation(peer, PI_GET_ROOM_KEYS_OPERATION, NULL,
                             (PEERCBType)callback, param, opID);
  if (!operation)
    return PEERFalse;
  operation->roomType = roomType;
  ...
  // Get the keys.
  ////////////////
  chatGetChannelKeysA(connection->chat, ROOM, nick, num, keys,
                      piGetChannelKeysCallback, operation, CHATFalse);
...
/* Authenticate CD Key
*********************/
static void piAuthenticateCDKeyCallbackA(CHAT chat, int result,
  ...
  ////////////////////
  if (operation->callback)
    piAddAuthenticateCDKeyCallback(
        peer, result, message,
...
#ifdef GSI_UNICODE
static void piAuthenticateCDKeyCallbackW(CHAT chat, int result,
...
PEERBool piNewAuthenticateCDKeyOperation(PEER peer, const char *cdkey,
  ...
  // Add the operation.
  /////////////////////
  operation = piAddOperation(peer, PI_AUTHENTICATE_CDKEY_OPERATION, NULL,
                             (PEERCBType)callback, param, opID);
  if (!operation)
    return PEERFalse;
  ...
  // Check the CD key.
  ////////////////////
  chatAuthenticateCDKeyA(connection->chat, cdkey, piAuthenticateCDKeyCallback,
                         operation, CHATFalse);
...
/* AutoMatch
***********/
PEERBool piNewAutoMatchOperation(PEER peer, SOCKET socket, unsigned short port,
  ...
  // Add the operation.
  /////////////////////
  operation = piAddOperation(peer, PI_AUTO_MATCH_OPERATION, NULL,
                             (PEERCBType)statusCallback, param, opID);
  if (!operation)
    return PEERFalse;
  ...
  /*	if(connection->inRoom[StagingRoom])
          {
                  ...
                  else if(connection->numPlayers[StagingRoom] >=
