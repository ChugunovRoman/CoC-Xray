...
**
** Staging Room
**   #GSP!<gamename>!X<encoded public IP><encoded public port><encoded private
*IP><encoded private port>X
**
** User
...
static const char *piStagingRoomHash(unsigned int publicIP,
                                     unsigned int privateIP,
                                     unsigned short port, char *buffer) {
  unsigned int result;

  publicIP = ntohl(publicIP);
  privateIP = ntohl(privateIP);

  result = (((privateIP >> 24) & 0xFF) | ((privateIP >> 8) & 0xFF00) |
            ((privateIP << 8) & 0xFF0000) | ((privateIP << 24) & 0xFF000000));
  result ^= publicIP;
  result ^= (port | (port << 16));
...
void piMangleStagingRoom(char buffer[PI_ROOM_MAX_LEN], const char *title,
  ...
  sprintf(buffer, "#GSP" PI_SEPERATOR "%s" PI_SEPERATOR "%c%s%c", title,
          borderChar, encodeBuffer, borderChar);
}

void piMangleUser(char buffer[PI_USER_MAX_LEN], unsigned int IP,
                  int profileID) {
  assert(buffer);
  assert(IP != 0);
  assert(profileID >= 0);
