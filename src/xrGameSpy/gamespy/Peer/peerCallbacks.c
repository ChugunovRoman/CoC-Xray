...
typedef struct piCallbackData {
  ...
  PEERCBType
      callback; // the function callback -- int type for ANSI compatability
  void *callbackParam; // user-data for the callback
  void *params;        // extra callback params
  int ID;              // unique ID for this callback
  PEERBool inCall; // set to true immediately before callback is called, then to
                   // false
} piCallbackData;
...
** CALLBACKS **
**************/
static int piAddCallback(PEER peer, PEERBool success, PEERCBType callback,
                         void *param, piCallbackType type, void *paramsIn,
                         size_t paramsSize, int opID);
...
static void piConnectCall(PEER peer, piCallbackData *data) {
  ...
  params = data->params;
  ((peerConnectCallback)data->callback)(
      peer, data->success, params->failureReason, data->callbackParam);
}
void piAddConnectCallback(PEER peer, PEERBool success, int failureReason,
                          peerConnectCallback callback, void *param, int opID) {
  piConnectParams params;

  params.failureReason = failureReason;

  piAddCallback(peer, success, (PEERCBType)callback, param, PI_CONNECT_CALLBACK,
                &params, sizeof(piConnectParams), opID);
}
...
static void piJoinRoomCall(PEER peer, piCallbackData *data) {
  ...
  ((peerJoinRoomCallback)(PEERCBType)data->callback)(
      peer, data->success, params->result, params->roomType,
      data->callbackParam);
}
void piAddJoinRoomCallback(PEER peer, PEERBool success, PEERJoinResult result,
                           RoomType roomType, peerJoinRoomCallback callback,
                           void *param, int opID) {
  piJoinRoomParams params;

  // if this was called as part of an AutoMatch attempt, call the callback
  // directly this is safe because we're only calling into Peer code
  ...
  piAddCallback(peer, success, (PEERCBType)callback, param,
                PI_JOIN_ROOM_CALLBACK, &params, sizeof(piJoinRoomParams), opID);
}
...
#ifndef GSI_UNICODE
  ((peerListGroupRoomsCallback)(PEERCBType)data->callback)(
      peer, data->success, params->groupID, params->server, params->name,
      params->numWaiting, params->maxWaiting, params->numGames,
      params->numPlaying, data->callbackParam);
#else
  {
    unsigned short *name_W = NULL;
    if (params->name != NULL)
      name_W = UTF8ToUCS2StringAlloc(params->name);
    ((peerListGroupRoomsCallback)(int)data->callback)(
        peer, data->success, params->groupID, params->server, name_W,
        params->numWaiting, params->maxWaiting, params->numGames,
        params->numPlaying, data->callbackParam);
    gsifree(name_W);
  }
...
void piAddListGroupRoomsCallback(PEER peer, PEERBool success, int groupID,
  ...
  piAddCallback(peer, success, (PEERCBType)callback, param,
                PI_LIST_GROUP_ROOMS_CALLBACK, &params,
                sizeof(piListGroupRoomsParams), opID);
}
...
static void piListingGamesCall(PEER peer, piCallbackData *data) {
  ...
  if (params->progress == 100) {
    ...
    for (i = 0; i < len; i++) {
      ...
      piCallbackData *anotherData =
          (piCallbackData *)ArrayNth(connection->callbackList, i);
      if (anotherData != data &&
          anotherData->type == PI_LISTING_GAMES_CALLBACK) {
        piListingGamesParams *anotherParams =
            (piListingGamesParams *)anotherData->params;
...
#ifndef GSI_UNICODE
  ((peerListingGamesCallback)(PEERCBType)data->callback)(
      peer, data->success, params->name, params->server, params->staging,
      params->msg, params->progress, data->callbackParam);
#else
  {
    unsigned short *name_W = NULL;
    if (params->name != NULL)
      name_W = UTF8ToUCS2StringAlloc(params->name);
    ((peerListingGamesCallback)(int)data->callback)(
        peer, data->success, name_W, params->server, params->staging,
        params->msg, params->progress, data->callbackParam);
    gsifree(name_W);
  }
#endif
...
void piAddListingGamesCallback(PEER peer, PEERBool success, SBServer server,
                               int msg) {
  piListingGamesParams params;
  const char *name;
  const char *gamemode;
  ...
  if (connection->initialGameList) {
    ...
    if (count) {
      pendingCount = (connection->gameEngine.querylist.count +
                      connection->gameEngine.pendinglist.count);
  ...
  piAddCallback(peer, success, (PEERCBType)connection->gameListCallback,
                connection->gameListParam, PI_LISTING_GAMES_CALLBACK, &params,
                sizeof(piListingGamesParams), -1);
}
...
static void piNickErrorCall(PEER peer, piCallbackData *data) {
  ...
  params = data->params;
#ifndef GSI_UNICODE
  ((peerNickErrorCallback)(PEERCBType)data->callback)(
      peer, params->type, params->nick, params->numSuggestedNicks,
      (const char **)params->suggestedNicks, data->callbackParam);
#else
  {
    unsigned short *nick_W = UTF8ToUCS2StringAlloc(params->nick);
    unsigned short **suggestedNicks_W = UTF8ToUCS2StringArrayAlloc(
        (const UTF8String *)params->suggestedNicks, params->numSuggestedNicks);
    int i;
    ((peerNickErrorCallback)data->callback)(
        peer, params->type, nick_W, params->numSuggestedNicks,
        (const unsigned short **)suggestedNicks_W, data->callbackParam);
    gsifree(nick_W);
    for (i = 0; i < params->numSuggestedNicks; i++) {
...
void piAddNickErrorCallback(PEER peer, int type, const char *nick,
                            int numSuggestedNicks, const char **suggestedNicks,
                            void *param, int opID) {
  piNickErrorParams params;
  ...
  piAddCallback(peer, PEERFalse, (PEERCBType)connection->nickErrorCallback,
                param, PI_NICK_ERROR_CALLBACK, &params,
                sizeof(piNickErrorParams), opID);
}
...
static void piEnumPlayersCall(PEER peer, piCallbackData *data) {
  ...
  ((peerEnumPlayersCallback)data->callback)(
      peer, data->success, params->roomType, params->index, params->nick,
      params->flags, data->callbackParam);
#else
  {
    unsigned short *nick_W = UTF8ToUCS2StringAlloc(params->nick);
    ((peerEnumPlayersCallback)data->callback)(
        peer, data->success, params->roomType, params->index, nick_W,
        params->flags, data->callbackParam);
    gsifree(nick_W);
  }
#endif
}
void piAddEnumPlayersCallback(PEER peer, PEERBool success, RoomType roomType,
  ...
  piAddCallback(peer, success, (PEERCBType)callback, param,
                PI_ENUM_PLAYERS_CALLBACK, &params, sizeof(piEnumPlayersParams),
                opID);
}
...
static void piGetPlayerInfoCall(PEER peer, piCallbackData *data) {
  ...
  params = data->params;
#ifndef GSI_UNICODE
  ((peerGetPlayerInfoCallback)data->callback)(peer, data->success, params->nick,
                                              params->IP, params->profileID,
                                              data->callbackParam);
#else
  {
    unsigned short *nick_W = UTF8ToUCS2StringAlloc(params->nick);
    ((peerGetPlayerInfoCallback)data->callback)(peer, data->success, nick_W,
                                                params->IP, params->profileID,
                                                data->callbackParam);
    gsifree(nick_W);
  }
...
void piAddGetPlayerInfoCallback(PEER peer, PEERBool success, const char *nick,
                                unsigned int IP, int profileID,
                                peerGetPlayerInfoCallback callback, void *param,
                                int opID) {
  piGetPlayerInfoParams params;
  params.nick = (char *)nick;
  params.IP = IP;
  ...
  piAddCallback(peer, success, (PEERCBType)callback, param,
                PI_GET_PLAYER_INFO_CALLBACK, &params,
                sizeof(piGetPlayerInfoParams), opID);
}
...
static PEERBool piGetPlayerProfileIDCopy(void *paramsOut_, void *paramsIn_) {
  piGetPlayerProfileIDParams *paramsOut =
      (piGetPlayerProfileIDParams *)paramsOut_;
  piGetPlayerProfileIDParams *paramsIn =
      (piGetPlayerProfileIDParams *)paramsIn_;
...
static void piGetPlayerProfileIDCall(PEER peer, piCallbackData *data) {
  ...
  ((peerGetPlayerProfileIDCallback)data->callback)(
      peer, data->success, params->nick, params->profileID,
      data->callbackParam);
#else
  {
    unsigned short *nick_W = UTF8ToUCS2StringAlloc(params->nick);
...
void piAddGetPlayerProfileIDCallback(PEER peer, PEERBool success,
  ...
  piAddCallback(peer, success, (PEERCBType)callback, param,
                PI_GET_PLAYER_PROFILE_ID_CALLBACK, &params,
                sizeof(piGetPlayerProfileIDParams), opID);
}
...
static void piGetPlayerIPCall(PEER peer, piCallbackData *data) {
  ...
  params = data->params;
#ifndef GSI_UNICODE
  ((peerGetPlayerIPCallback)data->callback)(peer, data->success, params->nick,
                                            params->IP, data->callbackParam);
#else
  {
    unsigned short *nick_W = UTF8ToUCS2StringAlloc(params->nick);
    ((peerGetPlayerIPCallback)data->callback)(peer, data->success, nick_W,
                                              params->IP, data->callbackParam);
    gsifree(nick_W);
  }
#endif
}
void piAddGetPlayerIPCallback(PEER peer, PEERBool success, const char *nick,
                              unsigned int IP, peerGetPlayerIPCallback callback,
                              void *param, int opID) {
  piGetPlayerIPParams params;
  params.nick = (char *)nick;
  params.IP = IP;
  ...
  piAddCallback(peer, success, (PEERCBType)callback, param,
                PI_GET_PLAYER_IP_CALLBACK, &params, sizeof(piGetPlayerIPParams),
                opID);
}
...
static void piRoomMessageCall(PEER peer, piCallbackData *data) {
  ...
  ((peerRoomMessageCallback)data->callback)(
      peer, params->roomType, params->nick, params->message,
      params->messageType, data->callbackParam);
#else
  {
    unsigned short *nick_W = UTF8ToUCS2StringAlloc(params->nick);
    unsigned short *message_W = UTF8ToUCS2StringAlloc(params->message);

    ((peerRoomMessageCallback)data->callback)(peer, params->roomType, nick_W,
                                              message_W, params->messageType,
                                              data->callbackParam);
    gsifree(nick_W);
    gsifree(message_W);
  }
...
void piAddRoomMessageCallback(PEER peer, RoomType roomType, const char *nick,
  ...
  if (callbacks->roomMessage) {
    ...
    piAddCallback(peer, PEERTrue, (PEERCBType)callbacks->roomMessage,
                  callbacks->param, PI_ROOM_MESSAGE_CALLBACK, &params,
                  sizeof(piRoomMessageParams), -1);
  }
}
...
static void piRoomUTMCall(PEER peer, piCallbackData *data) {
  ...
  ((peerRoomUTMCallback)data->callback)(
      peer, params->roomType, params->nick, params->command, params->parameters,
      params->authenticated, data->callbackParam);
#else
  {
    unsigned short *nick_W = UTF8ToUCS2StringAlloc(params->nick);
    ...
    ((peerRoomUTMCallback)data->callback)(
        peer, params->roomType, nick_W, command_W, parameters_W,
        params->authenticated, data->callbackParam);
    gsifree(nick_W);
    gsifree(command_W);
...
void piAddRoomUTMCallback(PEER peer, RoomType roomType, const char *nick,
  ...
  if (callbacks->roomUTM) {
    ...
    piAddCallback(peer, PEERTrue, (PEERCBType)callbacks->roomUTM,
                  callbacks->param, PI_ROOM_UTM_CALLBACK, &params,
                  sizeof(piRoomUTMParams), -1);
  }
}
...
static void piRoomNameChangedCall(PEER peer, piCallbackData *data) {
  ...
  params = data->params;
  ((peerRoomNameChangedCallback)data->callback)(peer, params->roomType,
                                                data->callbackParam);
}
void piAddRoomNameChangedCallback(PEER peer, RoomType roomType) {
  ...
  if (callbacks->roomNameChanged) {
    ...
    piAddCallback(peer, PEERTrue, (PEERCBType)callbacks->roomNameChanged,
                  callbacks->param, PI_ROOM_NAME_CHANGED_CALLBACK, &params,
                  sizeof(piRoomNameChangedParams), -1);
  }
}
...
static void piRoomModeChangedCall(PEER peer, piCallbackData *data) {
  ...
  params = data->params;
  ((peerRoomModeChangedCallback)data->callback)(
      peer, params->roomType, &params->mode, data->callbackParam);
}
void piAddRoomModeChangedCallback(PEER peer, RoomType roomType,
                                  CHATChannelMode *mode) {
  PEERCallbacks *callbacks;
  ...
  if (callbacks->roomModeChanged) {
    ...
    piAddCallback(peer, PEERTrue, (PEERCBType)callbacks->roomModeChanged,
                  callbacks->param, PI_ROOM_MODE_CHANGED_CALLBACK, &params,
                  sizeof(piRoomModeChangedParams), -1);
  }
}
...
static void piPlayerMessageCall(PEER peer, piCallbackData *data) {
  ...
  ((peerPlayerMessageCallback)data->callback)(
      peer, params->nick, params->message, params->messageType,
      data->callbackParam);
#else
  {
    unsigned short *nick_W = UTF8ToUCS2StringAlloc(params->nick);
    unsigned short *message_W = UTF8ToUCS2StringAlloc(params->message);
    ((peerPlayerMessageCallback)data->callback)(
        peer, nick_W, message_W, params->messageType, data->callbackParam);
    gsifree(nick_W);
    gsifree(message_W);
  }
#endif
}
void piAddPlayerMessageCallback(PEER peer, const char *nick,
                                const char *message, MessageType messageType) {
  PEERCallbacks *callbacks;
  ...
  if (callbacks->playerMessage) {
    ...
    piAddCallback(peer, PEERTrue, (PEERCBType)callbacks->playerMessage,
                  callbacks->param, PI_PLAYER_MESSAGE_CALLBACK, &params,
                  sizeof(piPlayerMessageParams), -1);
  }
}
...
static void piPlayerUTMCall(PEER peer, piCallbackData *data) {
  ...
  ((peerPlayerUTMCallback)data->callback)(
      peer, params->nick, params->command, params->parameters,
      params->authenticated, data->callbackParam);
#else
  {
    unsigned short *nick_W = UTF8ToUCS2StringAlloc(params->nick);
    unsigned short *command_W = UTF8ToUCS2StringAlloc(params->command);
    unsigned short *parameters_W = UTF8ToUCS2StringAlloc(params->parameters);
    ((peerPlayerUTMCallback)data->callback)(peer, nick_W, command_W,
                                            parameters_W, params->authenticated,
                                            data->callbackParam);
    gsifree(nick_W);
    gsifree(command_W);
    gsifree(parameters_W);
...
void piAddPlayerUTMCallback(PEER peer, const char *nick, const char *command,
  ...
  if (callbacks->playerUTM) {
    ...
    piAddCallback(peer, PEERTrue, (PEERCBType)callbacks->playerUTM,
                  callbacks->param, PI_PLAYER_UTM_CALLBACK, &params,
                  sizeof(piPlayerUTMParams), -1);
  }
}
...
static void piReadyChangedCall(PEER peer, piCallbackData *data) {
  ...
  params = data->params;
#ifndef GSI_UNICODE
  ((peerReadyChangedCallback)data->callback)(peer, params->nick, params->ready,
                                             data->callbackParam);
#else
  {
    unsigned short *nick_W = UTF8ToUCS2StringAlloc(params->nick);
    ((peerReadyChangedCallback)data->callback)(peer, nick_W, params->ready,
                                               data->callbackParam);
    gsifree(nick_W);
  }
#endif
...
void piAddReadyChangedCallback(PEER peer, const char *nick, PEERBool ready) {
  ...
  if (callbacks->readyChanged) {
    ...
    piAddCallback(peer, PEERTrue, (PEERCBType)callbacks->readyChanged,
                  callbacks->param, PI_READY_CHANGED_CALLBACK, &params,
                  sizeof(piReadyChangedParams), -1);
  }
}
...
static void piGameStartedCall(PEER peer, piCallbackData *data) {
  ...
  params = data->params;
#ifndef GSI_UNICODE
  ((peerGameStartedCallback)data->callback)(
      peer, params->server, params->message, data->callbackParam);
#else
  {
    unsigned short *message_W = UTF8ToUCS2StringAlloc(params->message);
    ((peerGameStartedCallback)data->callback)(peer, params->server, message_W,
                                              data->callbackParam);
    gsifree(message_W);
  }
#endif
...
void piAddGameStartedCallback(PEER peer, SBServer server, const char *message) {
  ...
  if (callbacks->gameStarted) {
    ...
    piAddCallback(peer, PEERTrue, (PEERCBType)callbacks->gameStarted,
                  callbacks->param, PI_GAME_STARTED_CALLBACK, &params,
                  sizeof(piGameStartedParams), -1);
  }
}
...
static void piPlayerJoinedCall(PEER peer, piCallbackData *data) {
  ...
  params = data->params;
#ifndef GSI_UNICODE
  ((peerPlayerJoinedCallback)data->callback)(peer, params->roomType,
                                             params->nick, data->callbackParam);
#else
  {
    unsigned short *nick_W = UTF8ToUCS2StringAlloc(params->nick);
    ((peerPlayerJoinedCallback)data->callback)(peer, params->roomType, nick_W,
                                               data->callbackParam);
    gsifree(nick_W);
  }
#endif
...
void piAddPlayerJoinedCallback(PEER peer, RoomType roomType, const char *nick) {
  ...
  if (callbacks->playerJoined) {
    ...
    piAddCallback(peer, PEERTrue, (PEERCBType)callbacks->playerJoined,
                  callbacks->param, PI_PLAYER_JOINED_CALLBACK, &params,
                  sizeof(piPlayerJoinedParams), -1);
  }
}
...
static void piPlayerLeftCall(PEER peer, piCallbackData *data) {
  ...
  params = data->params;
#ifndef GSI_UNICODE
  ((peerPlayerLeftCallback)data->callback)(peer, params->roomType, params->nick,
                                           params->reason, data->callbackParam);
#else
  {
    unsigned short *nick_W = UTF8ToUCS2StringAlloc(params->nick);
    unsigned short *reason_W = UTF8ToUCS2StringAlloc(params->reason);
    ((peerPlayerLeftCallback)data->callback)(peer, params->roomType, nick_W,
                                             reason_W, data->callbackParam);
    gsifree(nick_W);
    gsifree(reason_W);
  }
#endif
}
void piAddPlayerLeftCallback(PEER peer, RoomType roomType, const char *nick,
                             const char *reason) {
  PEERCallbacks *callbacks;
  ...
  if (callbacks->playerLeft) {
    ...
    piAddCallback(peer, PEERTrue, (PEERCBType)callbacks->playerLeft,
                  callbacks->param, PI_PLAYER_LEFT_CALLBACK, &params,
                  sizeof(piPlayerLeftParams), -1);
  }
}
...
static void piKickedCall(PEER peer, piCallbackData *data) {
  ...
  params = data->params;
#ifndef GSI_UNICODE
  ((peerKickedCallback)data->callback)(peer, params->roomType, params->nick,
                                       params->reason, data->callbackParam);
#else
  {
    unsigned short *nick_W = UTF8ToUCS2StringAlloc(params->nick);
    unsigned short *reason_W = UTF8ToUCS2StringAlloc(params->reason);
    ((peerKickedCallback)data->callback)(peer, params->roomType, nick_W,
                                         reason_W, data->callbackParam);
    gsifree(nick_W);
    gsifree(reason_W);
  }
...
void piAddKickedCallback(PEER peer, RoomType roomType, const char *nick,
                         const char *reason) {
  PEERCallbacks *callbacks;
  ...
  if (callbacks->kicked) {
    ...
    piAddCallback(peer, PEERTrue, (PEERCBType)callbacks->kicked,
                  callbacks->param, PI_KICKED_CALLBACK, &params,
                  sizeof(piKickedParams), -1);
  }
}
...
static void piNewPlayerListCall(PEER peer, piCallbackData *data) {
  ...
  params = data->params;
  ((peerNewPlayerListCallback)data->callback)(peer, params->roomType,
                                              data->callbackParam);
}
void piAddNewPlayerListCallback(PEER peer, RoomType roomType) {
  ...
  if (callbacks->newPlayerList) {
    ...
    piAddCallback(peer, PEERTrue, (PEERCBType)callbacks->newPlayerList,
                  callbacks->param, PI_NEW_PLAYER_LIST_CALLBACK, &params,
                  sizeof(piNewPlayerListParams), -1);
  }
}
...
static PEERBool piPlayerChangedNickCopy(void *paramsOut_, void *paramsIn_) {
  piPlayerChangedNickParams *paramsOut =
      (piPlayerChangedNickParams *)paramsOut_;
...
static void piPlayerChangedNickCall(PEER peer, piCallbackData *data) {
  ...
  ((peerPlayerChangedNickCallback)data->callback)(
      peer, params->roomType, params->oldNick, params->newNick,
      data->callbackParam);
#else
  {
...
void piAddPlayerChangedNickCallback(PEER peer, RoomType roomType,
  ...
  if (callbacks->playerChangedNick) {
    ...
    piAddCallback(peer, PEERTrue, (PEERCBType)callbacks->playerChangedNick,
                  callbacks->param, PI_PLAYER_CHANGED_NICK_CALLBACK, &params,
                  sizeof(piPlayerChangedNickParams), -1);
  }
}
...
#ifndef GSI_UNICODE
  ((peerPlayerInfoCallback)data->callback)(peer, params->roomType, params->nick,
                                           params->IP, params->profileID,
                                           data->callbackParam);
#else
  {
    unsigned short *nick_W = UTF8ToUCS2StringAlloc(params->nick);
    ((peerPlayerInfoCallback)data->callback)(peer, params->roomType, nick_W,
                                             params->IP, params->profileID,
                                             data->callbackParam);
    gsifree(nick_W);
  }
...
void piAddPlayerInfoCallback(PEER peer, RoomType roomType, const char *nick,
  ...
  if (callbacks->playerInfo) {
    ...
    piAddCallback(peer, PEERTrue, (PEERCBType)callbacks->playerInfo,
                  callbacks->param, PI_PLAYER_INFO_CALLBACK, &params,
                  sizeof(piPlayerInfoParams), -1);
  }
}
...
static void piDisconnectedCall(PEER peer, piCallbackData *data) {
  ...
  params = data->params;
#ifndef GSI_UNICODE
  ((peerDisconnectedCallback)data->callback)(peer, params->reason,
                                             data->callbackParam);
#else
  {
    unsigned short *reason_W = UTF8ToUCS2StringAlloc(params->reason);
    ((peerDisconnectedCallback)data->callback)(peer, reason_W,
                                               data->callbackParam);
    gsifree(reason_W);
  }
#endif
...
void piAddDisconnectedCallback(PEER peer, const char *reason) {
  ...
  if (callbacks->disconnected) {
    ...
    piAddCallback(peer, PEERTrue, (PEERCBType)callbacks->disconnected,
                  callbacks->param, PI_DISCONNECTED_CALLBACK, &params,
                  sizeof(piDisconnectedParams), -1);
  }
}
...
static void piPingCall(PEER peer, piCallbackData *data) {
  ...
  params = data->params;
#ifndef GSI_UNICODE
  ((peerPingCallback)data->callback)(peer, params->nick, params->ping,
                                     data->callbackParam);
#else
  {
    unsigned short *nick_W = UTF8ToUCS2StringAlloc(params->nick);
    ((peerPingCallback)data->callback)(peer, nick_W, params->ping,
                                       data->callbackParam);
    gsifree(nick_W);
  }
#endif
...
void piAddPingCallback(PEER peer, const char *nick, int ping) {
  ...
  if (callbacks->ping) {
    ...
    piAddCallback(peer, PEERTrue, (PEERCBType)callbacks->ping, callbacks->param,
                  PI_PING_CALLBACK, &params, sizeof(piPingParams), -1);
  }
}
...
static void piCrossPingCall(PEER peer, piCallbackData *data) {
  ...
  params = data->params;
#ifndef GSI_UNICODE
  ((peerCrossPingCallback)data->callback)(peer, params->nick1, params->nick2,
                                          params->crossPing,
                                          data->callbackParam);
#else
  {
    ...
    ((peerCrossPingCallback)data->callback)(
        peer, nick1_W, nick2_W, params->crossPing, data->callbackParam);
    gsifree(nick1_W);
    gsifree(nick2_W);
  }
#endif
}
void piAddCrossPingCallback(PEER peer, const char *nick1, const char *nick2,
                            int crossPing) {
  PEERCallbacks *callbacks;
  ...
  if (callbacks->crossPing) {
    ...
    piAddCallback(peer, PEERTrue, (PEERCBType)callbacks->crossPing,
                  callbacks->param, PI_CROSS_PING_CALLBACK, &params,
                  sizeof(piCrossPingParams), -1);
  }
}
...
static void piChangeNickCall(PEER peer, piCallbackData *data) {
  ...
  params = data->params;
#ifndef GSI_UNICODE
  ((peerChangeNickCallback)data->callback)(peer, data->success, params->oldNick,
                                           params->newNick,
                                           data->callbackParam);
#else
  {
    unsigned short *oldNick_W = UTF8ToUCS2StringAlloc(params->oldNick);
    unsigned short *newNick_W = UTF8ToUCS2StringAlloc(params->newNick);
    ((peerChangeNickCallback)data->callback)(peer, data->success, oldNick_W,
                                             newNick_W, data->callbackParam);
    gsifree(oldNick_W);
    gsifree(newNick_W);
  }
#endif
}
void piAddChangeNickCallback(PEER peer, PEERBool success, const char *oldNick,
                             const char *newNick,
                             peerChangeNickCallback callback, void *param,
                             int opID) {
  piChangeNickParams params;
  params.newNick = (char *)newNick;
  params.oldNick = (char *)oldNick;
  ...
  piAddCallback(peer, success, (PEERCBType)callback, param,
                PI_CHANGE_NICK_CALLBACK, &params, sizeof(piChangeNickParams),
                opID);
}
...
static void piGlobalKeyChangedCall(PEER peer, piCallbackData *data) {
  ...
  params = data->params;
#ifndef GSI_UNICODE
  ((peerGlobalKeyChangedCallback)data->callback)(
      peer, params->nick, params->key, params->value, data->callbackParam);
#else
  {
    unsigned short *nick_W = UTF8ToUCS2StringAlloc(params->nick);
    unsigned short *key_W = UTF8ToUCS2StringAlloc(params->key);
    unsigned short *value_W = UTF8ToUCS2StringAlloc(params->value);
    ((peerGlobalKeyChangedCallback)data->callback)(peer, nick_W, key_W, value_W,
                                                   data->callbackParam);
    gsifree(nick_W);
    gsifree(key_W);
    gsifree(value_W);
...
void piAddGlobalKeyChangedCallback(PEER peer, const char *nick, const char *key,
  ...
  if (callbacks->globalKeyChanged) {
    ...
    piAddCallback(peer, PEERTrue, (PEERCBType)callbacks->globalKeyChanged,
                  callbacks->param, PI_GLOBAL_KEY_CHANGED_CALLBACK, &params,
                  sizeof(piGlobalKeyChangedParams), -1);
  }
}
...
static void piRoomKeyChangedCall(PEER peer, piCallbackData *data) {
  ...
  ((peerRoomKeyChangedCallback)data->callback)(
      peer, params->roomType, params->nick, params->key, params->value,
      data->callbackParam);
#else
  {
    unsigned short *nick_W = UTF8ToUCS2StringAlloc(params->nick);
...
void piAddRoomKeyChangedCallback(PEER peer, RoomType roomType, const char *nick,
  ...
  if (callbacks->roomKeyChanged) {
    ...
    piAddCallback(peer, PEERTrue, (PEERCBType)callbacks->roomKeyChanged,
                  callbacks->param, PI_ROOM_KEY_CHANGED_CALLBACK, &params,
                  sizeof(piRoomKeyChangedParams), -1);
  }
}
...
static void piGetGlobalKeysCall(PEER peer, piCallbackData *data) {
  ...
  params = data->params;
#ifndef GSI_UNICODE
  ((peerGetGlobalKeysCallback)data->callback)(
      peer, data->success, params->nick, params->num,
      (const char **)params->keys, (const char **)params->values,
      data->callbackParam);
#else
  {
    unsigned short *nick_W = UTF8ToUCS2StringAlloc(params->nick);
    unsigned short **keys_W = UTF8ToUCS2StringArrayAlloc(
        (const UTF8String *)params->keys, params->num);
    unsigned short **values_W = UTF8ToUCS2StringArrayAlloc(
        (const UTF8String *)params->values, params->num);
    int i;
    ((peerGetGlobalKeysCallback)data->callback)(
        peer, data->success, nick_W, params->num,
        (const unsigned short **)keys_W, (const unsigned short **)values_W,
        data->callbackParam);
    gsifree(nick_W);
    for (i = 0; i < params->num; i++) {
      gsifree(keys_W[i]);
...
void piAddGetGlobalKeysCallback(PEER peer, PEERBool success, const char *nick,
                                int num, const char **keys, const char **values,
                                peerGetGlobalKeysCallback callback, void *param,
                                int opID) {
  piGetGlobalKeysParams params;
  params.nick = (char *)nick;
  params.num = num;
  ...
  piAddCallback(peer, success, (PEERCBType)callback, param,
                PI_GET_GLOBAL_KEYS_CALLBACK, &params,
                sizeof(piGetGlobalKeysParams), opID);
}
...
static void piGetRoomKeysCall(PEER peer, piCallbackData *data) {
  ...
  ((peerGetRoomKeysCallback)data->callback)(
      peer, data->success, params->roomType, params->nick, params->num,
      params->keys, params->values, data->callbackParam);
#else
  {
    unsigned short *nick_W = UTF8ToUCS2StringAlloc(params->nick);
    unsigned short **keys_W = UTF8ToUCS2StringArrayAlloc(
    ...
    int i;
    ((peerGetRoomKeysCallback)data->callback)(
        peer, data->success, params->roomType, nick_W, params->num, keys_W,
        values_W, data->callbackParam);
    gsifree(nick_W);
    for (i = 0; i < params->num; i++) {
      gsifree(keys_W[i]);
      if (values_W != NULL) // may be a NULL when getting keys for "*"
...
void piAddGetRoomKeysCallback(PEER peer, PEERBool success, RoomType roomType,
                              const char *nick, int num, const char **keys,
                              const char **values,
                              peerGetRoomKeysCallback callback, void *param,
                              int opID) {
  piGetRoomKeysParams params;
  params.roomType = roomType;
  params.nick = (char *)nick;
  ...
  piAddCallback(peer, success, (PEERCBType)callback, param,
                PI_GET_ROOM_KEYS_CALLBACK, &params, sizeof(piGetRoomKeysParams),
                opID);
}
...
static PEERBool piPlayerFlagsChangedCopy(void *paramsOut_, void *paramsIn_) {
  piPlayerFlagsChangedParams *paramsOut =
      (piPlayerFlagsChangedParams *)paramsOut_;
  piPlayerFlagsChangedParams *paramsIn =
      (piPlayerFlagsChangedParams *)paramsIn_;
...
static void piPlayerFlagsChangedCall(PEER peer, piCallbackData *data) {
  ...
  ((peerPlayerFlagsChangedCallback)data->callback)(
      peer, params->roomType, params->nick, params->oldFlags, params->newFlags,
      data->callbackParam);
#else
  {
    unsigned short *nick_W = UTF8ToUCS2StringAlloc(params->nick);
    ((peerPlayerFlagsChangedCallback)data->callback)(
        peer, params->roomType, nick_W, params->oldFlags, params->newFlags,
        data->callbackParam);
    gsifree(nick_W);
  }
#endif
...
void piAddPlayerFlagsChangedCallback(PEER peer, RoomType roomType,
  ...
  if (callbacks->playerFlagsChanged) {
    ...
    piAddCallback(peer, PEERTrue, (PEERCBType)callbacks->playerFlagsChanged,
                  callbacks->param, PI_PLAYER_FLAGS_CHANGED_CALLBACK, &params,
                  sizeof(piPlayerFlagsChangedParams), -1);
  }
}
...
static PEERBool piAuthenticateCDKeyCopy(void *paramsOut_, void *paramsIn_) {
  piAuthenticateCDKeyParams *paramsOut =
      (piAuthenticateCDKeyParams *)paramsOut_;
...
static void piAuthenticateCDKeyCall(PEER peer, piCallbackData *data) {
  ...
  params = data->params;
#ifndef GSI_UNICODE
  ((peerAuthenticateCDKeyCallback)data->callback)(
      peer, params->result, params->message, data->callbackParam);
#else
  {
    unsigned short *message_W = UTF8ToUCS2StringAlloc(params->message);
    ((peerAuthenticateCDKeyCallback)data->callback)(
        peer, params->result, message_W, data->callbackParam);
    gsifree(message_W);
  }
#endif
}
void piAddAuthenticateCDKeyCallback(PEER peer, int result, const char *message,
  ...
  piAddCallback(peer, PEERTrue, (PEERCBType)callback, param,
                PI_AUTHENTICATE_CDKEY_CALLBACK, &params,
                sizeof(piAuthenticateCDKeyParams), opID);
}
...
static void piAutoMatchStatusCall(PEER peer, piCallbackData *data) {
  ...
  params = data->params;
  ((peerAutoMatchStatusCallback)data->callback)(peer, params->status,
                                                data->callbackParam);
}
void piAddAutoMatchStatusCallback(PEER peer) {
  ...
  piAddCallback(peer, PEERTrue, operation->callback, operation->callbackParam,
                PI_AUTO_MATCH_STATUS_CALLBACK, &params,
                sizeof(piAutoMatchStatusParams), operation->ID);
}
...
static const piCallbackFuncs callbackFuncs[] = {
    ...
    {PI_LIST_GROUP_ROOMS_CALLBACK, piListGroupRoomsCopy, piListGroupRoomsFree,
     piListGroupRoomsCall},
    {PI_LISTING_GAMES_CALLBACK, piListingGamesCopy, piListingGamesFree,
     piListingGamesCall},
    {PI_NICK_ERROR_CALLBACK, piNickErrorCopy, piNickErrorFree, piNickErrorCall},
    {PI_ENUM_PLAYERS_CALLBACK, piEnumPlayersCopy, piEnumPlayersFree,
     piEnumPlayersCall},
    {PI_GET_PLAYER_INFO_CALLBACK, piGetPlayerInfoCopy, piGetPlayerInfoFree,
     piGetPlayerInfoCall},
    {PI_GET_PLAYER_PROFILE_ID_CALLBACK, piGetPlayerProfileIDCopy,
     piGetPlayerProfileIDFree, piGetPlayerProfileIDCall},
    {PI_GET_PLAYER_IP_CALLBACK, piGetPlayerIPCopy, piGetPlayerIPFree,
     piGetPlayerIPCall},
    {PI_ROOM_MESSAGE_CALLBACK, piRoomMessageCopy, piRoomMessageFree,
     piRoomMessageCall},
    {PI_ROOM_UTM_CALLBACK, piRoomUTMCopy, piRoomUTMFree, piRoomUTMCall},
    {PI_ROOM_NAME_CHANGED_CALLBACK, piRoomNameChangedCopy,
     piRoomNameChangedFree, piRoomNameChangedCall},
    {PI_ROOM_MODE_CHANGED_CALLBACK, piRoomModeChangedCopy,
     piRoomModeChangedFree, piRoomModeChangedCall},
    {PI_PLAYER_MESSAGE_CALLBACK, piPlayerMessageCopy, piPlayerMessageFree,
     piPlayerMessageCall},
    {PI_PLAYER_UTM_CALLBACK, piPlayerUTMCopy, piPlayerUTMFree, piPlayerUTMCall},
    {PI_READY_CHANGED_CALLBACK, piReadyChangedCopy, piReadyChangedFree,
     piReadyChangedCall},
    {PI_GAME_STARTED_CALLBACK, piGameStartedCopy, piGameStartedFree,
     piGameStartedCall},
    {PI_PLAYER_JOINED_CALLBACK, piPlayerJoinedCopy, piPlayerJoinedFree,
     piPlayerJoinedCall},
    {PI_PLAYER_LEFT_CALLBACK, piPlayerLeftCopy, piPlayerLeftFree,
     piPlayerLeftCall},
    {PI_KICKED_CALLBACK, piKickedCopy, piKickedFree, piKickedCall},
    {PI_NEW_PLAYER_LIST_CALLBACK, piNewPlayerListCopy, piNewPlayerListFree,
     piNewPlayerListCall},
    {PI_PLAYER_CHANGED_NICK_CALLBACK, piPlayerChangedNickCopy,
     piPlayerChangedNickFree, piPlayerChangedNickCall},
    {PI_PLAYER_INFO_CALLBACK, piPlayerInfoCopy, piPlayerInfoFree,
     piPlayerInfoCall},
    {PI_DISCONNECTED_CALLBACK, piDisconnectedCopy, piDisconnectedFree,
     piDisconnectedCall},
    {PI_PING_CALLBACK, piPingCopy, piPingFree, piPingCall},
    {PI_CROSS_PING_CALLBACK, piCrossPingCopy, piCrossPingFree, piCrossPingCall},
    {PI_CHANGE_NICK_CALLBACK, piChangeNickCopy, piChangeNickFree,
     piChangeNickCall},
    {PI_GLOBAL_KEY_CHANGED_CALLBACK, piGlobalKeyChangedCopy,
     piGlobalKeyChangedFree, piGlobalKeyChangedCall},
    {PI_ROOM_KEY_CHANGED_CALLBACK, piRoomKeyChangedCopy, piRoomKeyChangedFree,
     piRoomKeyChangedCall},
    {PI_GET_GLOBAL_KEYS_CALLBACK, piGetGlobalKeysCopy, piGetGlobalKeysFree,
     piGetGlobalKeysCall},
    {PI_GET_ROOM_KEYS_CALLBACK, piGetRoomKeysCopy, piGetRoomKeysFree,
     piGetRoomKeysCall},
    {PI_PLAYER_FLAGS_CHANGED_CALLBACK, piPlayerFlagsChangedCopy,
     piPlayerFlagsChangedFree, piPlayerFlagsChangedCall},
    {PI_AUTHENTICATE_CDKEY_CALLBACK, piAuthenticateCDKeyCopy,
     piAuthenticateCDKeyFree, piAuthenticateCDKeyCall},
    {PI_AUTO_MATCH_STATUS_CALLBACK, piAutoMatchStatusCopy,
     piAutoMatchStatusFree, piAutoMatchStatusCall},
...
PEERBool piCallbacksInit(PEER peer) {
  ...
  // Init the list.
  /////////////////
  connection->callbackList =
      ArrayNew(sizeof(piCallbackData), 0, piCallbackListFree);
...
void piCallbacksThink(PEER peer, int blockingID) {
  ...
  if (blockingID != -1) {
    ...
    for (index = 0; index < len; index++) {
      ...
      ////////////////////////////////////////////////
      if ((data->ID == blockingID) ||
          (data->type == PI_DISCONNECTED_CALLBACK)) {
        // Call it.
        ///////////
...
static int GS_STATIC_CALLBACK
piIsCallbackFinishedCompareCallback(const void *elem1, const void *elem2) {
...
PEERBool piIsCallbackFinished(PEER peer, int opID) {
  ...
  /////////////////
  data.ID = opID;
  index = ArraySearch(connection->callbackList, &data,
                      piIsCallbackFinishedCompareCallback, 0, 0);
