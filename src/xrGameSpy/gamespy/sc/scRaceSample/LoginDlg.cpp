...
void LoginCallback(GHTTPResult httpResult, WSLoginResponse *theResponse,
                   void *theUserData) {
  CString debugStr;
  ...
  if (httpResult != GHTTPSuccess) {
    ...
    debugStr.Format("[LoginCallback] Login Result Error: %d",
...
void CLoginDlg::OnOK() {
  ...
  // Initialize GP
  ////////////////
  if (gpInitialize(&connection, SCRACE_PRODUCTID,
                   WSLogin_NAMESPACE_SHARED_NONUNIQUE,
                   GP_PARTNERID_GAMESPY) != GP_NO_ERROR) {
  ...
  //////////////////////////////////
  result = gpCheckUser(&connection, m_nick, m_email, m_password, GP_BLOCKING,
  ...
  //////////////////////////////////////
  loginResult = wsLoginProfile(WSLogin_NAMESPACE_SHARED_NONUNIQUE,
                               WSLogin_PARTNERCODE_GAMESPY, m_nick, m_email,
                               m_password, "", LoginCallback, NULL);
  if (loginResult != WSLogin_Success) {
    if (loginResult == WSLogin_InvalidParameters)
      MessageBox(
          "Login Failed - Invalid Parameters."); // this should not be reached
