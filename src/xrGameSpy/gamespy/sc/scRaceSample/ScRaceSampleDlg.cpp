...
void createSessionCallback(const SCInterfacePtr theInterface,
  ...
  if (theHttpResult == GHTTPSuccess && theResult == SCResult_NO_ERROR) {
    ...
    memcpy(gPlayerData.mSessionId, scGetSessionId(theInterface),
           SC_SESSION_GUID_SIZE);
    memcpy(gPlayerData.mConnectionId, scGetConnectionId(theInterface),
...
///////////////////////////////////////////////////////////////////////////////
// GT2 Callbacks
void ConnectedCallback(GT2Connection connection, GT2Result result,
...
void ReceivedCallback(GT2Connection connection, GT2Byte *message, int len,
                      GT2Bool reliable) {
  if (!message || !len)
    return;
  ...
  if (type == MSG_TO_CLIENT_CERT_TYPE) {
    ...
    if (gtDecode(MSG_TO_CLIENT_CERT, (char *)message, len, &pid, remoteCert,
                 &remoteCertLen) == -1) {
      Dlg->m_state = JOIN_ERROR;
      return;
    }
    ...
    wsLoginCertWriteBinary(&gPlayerData.mCertificate, cert, sizeof(cert),
                           &certLen);

    rcode = gtEncode(MSG_TO_HOST_CERT_TYPE, MSG_TO_HOST_CERT, buffer,
                     sizeof(buffer), gPlayerData.mProfileId, cert, certLen);
    ASSERT(rcode != -1);
    ...
    if (gtDecode(MSG_TO_HOST_CERT, (char *)message, len, &pid, remoteCert,
                 &remoteCertLen) == -1) {
    ...
    if (gtDecode(MSG_TO_CLIENT_KEYS, (char *)message, len, recvMsg,
                 &recvMsgLen) == -1) {
      Dlg->m_state = JOIN_ERROR;
      return;
    }
    ...
    /////////////////////////////////////////////////////////
    scPeerCipherParseKeyExchangeMsg(&gPlayerData.mCertificate,
                                    &gPlayerData.mPrivateData, recvMsg,
                                    &gPlayerData.mPeerSendCipher);
    ...
    scPeerCipherCreateKeyExchangeMsg(&Dlg->m_remoteCertificate,
                                     &gPlayerData.mPeerRecvCipher, exchangeMsg);

    // Now send the key to the other player
    /////////////////////////////////////////////////////////
    rcode = gtEncode(MSG_TO_HOST_KEYS_TYPE, MSG_TO_HOST_KEYS, buffer,
                     sizeof(buffer), exchangeMsg, GS_CRYPT_RSA_BYTE_SIZE);
    ASSERT(rcode != -1);
    gt2Send(connection, (const unsigned char *)buffer, rcode, GT2True);
    ...
    if (gtDecode(MSG_TO_HOST_KEYS, (char *)message, len, exchangeMsg,
                 &exchangeMsgLen) == -1) {
      Dlg->m_state = HOST_ERROR;
      return;
    }
    ...
    /////////////////////////////////////////////////////////
    scPeerCipherParseKeyExchangeMsg(&gPlayerData.mCertificate,
                                    &gPlayerData.mPrivateData, exchangeMsg,
                                    &gPlayerData.mPeerSendCipher);
    ...
    if (gtDecode(MSG_COUNTDOWN, (char *)message, len, &Dlg->m_countdown) ==
        -1) {
      Dlg->m_state = JOIN_ERROR;
      return;
    }
    ...
    /////////////////////////////////////////////////////////
    if (gtDecode(MSG_SESSION_ID, (char *)message, len, sessionCrypt, &sidLen,
                 connCrypt, &ccidLen) == -1) {
      Dlg->m_state = JOIN_ERROR;
      return;
    }
    ...
    /////////////////////////////////////////////////////////
    scPeerCipherDecryptBufferIV(&gPlayerData.mPeerRecvCipher, 1,
                                (gsi_u8 *)sessionCrypt, SC_SESSION_GUID_SIZE);
    memcpy(gPlayerData.mSessionId, sessionCrypt, SC_SESSION_GUID_SIZE);

    scPeerCipherDecryptBufferIV(&gPlayerData.mPeerRecvCipher, 2,
                                (gsi_u8 *)connCrypt, SC_CONNECTION_GUID_SIZE);
    ...
    scSetReportIntention(gPlayerData.mStatsInterface, gPlayerData.mConnectionId,
                         SCRACE_AUTHORITATIVE, &gPlayerData.mCertificate,
                         &gPlayerData.mPrivateData, setReportIntentionCallback,
                         SCRACE_TIMEOUT_MS, NULL);
    ...
    /////////////////////////////////////////////////////////
    if (gtDecode(MSG_CONNECTION_ID, (char *)message, len, connCrypt,
                 &ccidLen) == -1) {
      Dlg->m_state = HOST_ERROR;
      return;
    }
...
void ConnectAttemptCallback(GT2Socket listener, GT2Connection connection,
...
BOOL CScRaceSampleDlg::SetupHosting() {
  ...
  GT2Result aResult = gt2CreateSocket(&m_GT2Socket, HOST_PORT_STRING, 0, 0,
                                      SocketErrorCallback);
...
BOOL CScRaceSampleDlg::SetupJoining() {
  ...
  /////////////////////////
  char buffer[64];
  rcode = gtEncodeNoType(MSG_CONNECT, buffer, sizeof(buffer),
                         gPlayerData.mProfileId, m_loginDlg.m_nick);
  ASSERT(rcode != -1);
  ...
  // Create the socket
  GT2Result aResult = gt2CreateSocket(&m_GT2Socket, CLIENT_PORT_STRING, 0, 0,
                                      SocketErrorCallback);
  ...
  GT2Result result;
  result = gt2Connect(m_GT2Socket, &m_GT2Connection, remoteAddress,
                      (const GT2Byte *)buffer, sizeof(buffer), -1, &callbacks,
                      GT2False);
  if (!m_GT2Connection)
    return FALSE;
...
BOOL CScRaceSampleDlg::OnInitDialog() {
  ...
  if (aResult == GSIACTemporarilyUnavailable) {
    MessageBox(
        "Online service for ScRaceSample is temporarily down for maintenance.");
    return FALSE;
  }

  // Initialize SDK core object - used for both the AuthService and the
  // Competition SDK
  /////////////////////////////////
  gsCoreInitialize();
...
HCURSOR CScRaceSampleDlg::OnQueryDragIcon() { return (HCURSOR)m_hIcon; }
  ...
  if (nIDEvent == TIMER_THINK) {
    ...
    if (!thinking) {
      ...
      if (m_state == HOST_EXCHANGE_CERT) {
        ...
        /////////////////////////////////////////////
        rcode = gtEncode(MSG_TO_CLIENT_CERT_TYPE, MSG_TO_CLIENT_CERT, buffer,
                         sizeof(buffer), gPlayerData.mProfileId, cert, certLen);
        ASSERT(rcode != -1);
        ...
        /////////////////////////////////////////////
        Dlg->m_remoteCertificate.mIsValid =
            wsLoginCertIsValid(&Dlg->m_remoteCertificate);
        if (gsi_is_false(Dlg->m_remoteCertificate.mIsValid)) {
          MessageBox(
              "Remote player has an invalid certificate, cancelling game.");
          m_waitingDlg.EndDialog(IDCANCEL);
          Logout();
        } else
        ...
        //     channel
        /////////////////////////////////////////////
        scPeerCipherInit(&gPlayerData.mCertificate,
                         &gPlayerData.mPeerRecvCipher);

        // Create a key exchange message for transmitting the key to the other
        // player using the remote player's certificate to encrypt the cipher
        /////////////////////////////////////////////
        scPeerCipherCreateKeyExchangeMsg(
            &m_remoteCertificate, &gPlayerData.mPeerRecvCipher, exchangeMsg);

        // Now send the key to the other player
        /////////////////////////////////////////////
        rcode = gtEncode(MSG_TO_CLIENT_KEYS_TYPE, MSG_TO_CLIENT_KEYS, buffer,
                         sizeof(buffer), exchangeMsg, GS_CRYPT_RSA_BYTE_SIZE);
        ASSERT(rcode != -1);
        gt2Send(m_GT2Connection, (const unsigned char *)buffer, rcode, GT2True);
        ...
        if (sessionCreated) {
          ...
          scPeerCipherEncryptBufferIV(&gPlayerData.mPeerSendCipher, 1,
                                      (gsi_u8 *)sessionCrypt,
                                      SC_SESSION_GUID_SIZE);

          memcpy(connCrypt, gPlayerData.mConnectionId, SC_CONNECTION_GUID_SIZE);
          scPeerCipherEncryptBufferIV(&gPlayerData.mPeerSendCipher, 2,
                                      (gsi_u8 *)connCrypt,
                                      SC_CONNECTION_GUID_SIZE);
          ...
          /////////////////////////////////////////////
          rcode = gtEncode(MSG_SESSION_ID_TYPE, MSG_SESSION_ID, buffer,
                           sizeof(buffer), sessionCrypt, SC_SESSION_GUID_SIZE,
                           connCrypt, SC_CONNECTION_GUID_SIZE);
          ASSERT(rcode != -1);
          gt2Send(m_GT2Connection, (const unsigned char *)buffer, rcode,
                  GT2True);
          ...
          scSetReportIntention(
              m_interface, gPlayerData.mConnectionId, SCRACE_AUTHORITATIVE,
              &gPlayerData.mCertificate, &gPlayerData.mPrivateData,
      ...
      // ********************************************************** //
        ...
        if (connIdSet) {
          ...
          /////////////////////////////////////////////
          rcode = gtEncode(MSG_CONNECTION_ID_TYPE, MSG_CONNECTION_ID, buffer,
                           sizeof(buffer), connCrypt, SC_CONNECTION_GUID_SIZE);
          ASSERT(rcode != -1);
          gt2Send(m_GT2Connection, (const unsigned char *)buffer, rcode,
                  GT2True);
      ...
      if (m_localTime) {
        ...
        if (m_remoteTime) {
          ...
          if (m_localTime < m_remoteTime) {
            message.Format("You won!\n%0.3fs to %0.3fs", m_localTime / 1000.0,
                           m_remoteTime / 1000.0);
            m_win = gsi_true;
        ...
        ////////////////////////////////////////
        rcode = gtEncode(MSG_PROGRESS_TYPE, MSG_PROGRESS, buffer,
                         sizeof(buffer), m_numSteps);
        ASSERT(rcode != -1);
        gt2Send(m_GT2Connection, (const unsigned char *)buffer, rcode,
                GT2False);
      }
...
void CScRaceSampleDlg::Logout() { OnLogout(); }
...
void CScRaceSampleDlg::Countdown() {
  ...
  if (m_hosting) {
    ...
    rcode = gtEncode(MSG_COUNTDOWN_TYPE, MSG_COUNTDOWN, message,
                     sizeof(message), m_countdown);
    ASSERT(rcode != -1);
...
void CScRaceSampleDlg::OnStart() {
  ...
  // During this loading phase - the Host will create the game session and
  // notify the other players of the session ID. All players will set their
  // report intentions.
  /////////////////////////////////////////////
  if (m_hosting) {
    scCreateSession(m_interface, &gPlayerData.mCertificate,
                    &gPlayerData.mPrivateData, createSessionCallback,
                    SCRACE_TIMEOUT_MS, NULL);
      ...
      if ((pMsg->lParam & 0xFFFF) == 1) {
        ...
        if (stepped && m_racing) {
          ...
          if (m_numSteps == m_totalSteps) {
            ...
            char buffer[32];
            int rcode;
            rcode = gtEncode(MSG_END_RACE_TYPE, MSG_END_RACE, buffer,
                             sizeof(buffer), m_localTime);
            ASSERT(rcode != -1);
            gt2Send(m_GT2Connection, (const unsigned char *)buffer, rcode,
                    GT2True);
          }
...
void CScRaceSampleDlg::StartRace() {
  if (m_hosting) {
    ...
    char buffer[32];
    rcode =
        gtEncode(MSG_START_RACE_TYPE, MSG_START_RACE, buffer, sizeof(buffer));
    ASSERT(rcode != -1);
    gt2Send(m_GT2Connection, (const unsigned char *)buffer, rcode, GT2True);
  }
...
void CScRaceSampleDlg::ReportStats() {
  ...
  /////////////////////////////////////////////
  aResult = scCreateReport(m_interface, ATLAS_RULE_SET_VERSION, numPlayers,
                           numTeams, &aReport);
  ...
  scReportSetPlayerData(aReport, 0, gPlayerData.mConnectionId, myTeam,
                        myGameResult, gPlayerData.mProfileId,
                        &gPlayerData.mCertificate, gPlayerData.mStatsAuthdata);
  if (!m_disconnect)
  ...
  scReportSetPlayerData(aReport, 1, m_remoteConnId, opponentTeam,
                        opponentGameResult, m_remoteProfile,
                        &m_remoteCertificate, gPlayerData.mStatsAuthdata);
  if (!m_disconnect)
  ...
  // Submit the Report
  /////////////////////////////////////////////
  if (SCResult_NO_ERROR !=
      scSubmitReport(m_interface, aReport, SCRACE_AUTHORITATIVE,
                     &gPlayerData.mCertificate, &gPlayerData.mPrivateData,
                     submitReportCallback, SCRACE_TIMEOUT_MS, NULL)) {
    MessageBox("Failed to submit Stats Report - Out of memory");
    return;
  }

  // To keep the logic clean, wait for submission to finish and then cleanup the
  // report buffer
  /////////////////////////////////////////////
  waitForScCallbacks(m_interface, 1);

  m_reportSent = gsi_true; // mark that we've submitted a report for this
                           // session
