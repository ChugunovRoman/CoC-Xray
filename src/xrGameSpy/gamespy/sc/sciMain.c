...
///////////////////////////////////////////////////////////////////////////////
SCResult SC_CALL
scSetSessionId(const SCInterfacePtr theInterface,
...
SCResult SC_CALL scCreateSession(SCInterfacePtr theInterface,
  ...
  GS_ASSERT(theCertificate != NULL ||
            (theCertificate == NULL && thePrivateData == NULL));

  if (!wsLoginCertIsValid(theCertificate))
    return SCResult_INVALID_PARAMETERS;

  return sciWsCreateSession(&anInterface->mWebServices, anInterface->mGameId,
                            theCertificate, thePrivateData, theCallback,
...
SCResult SC_CALL scCreateMatchlessSession(
    SCInterfacePtr theInterface, const GSLoginCertificate *theCertificate,
    const GSLoginPrivateData *thePrivateData,
    SCCreateSessionCallback theCallback, gsi_time theTimeoutMs,
    void *theUserData) {
  SCInterface *anInterface = (SCInterface *)theInterface;
  GS_ASSERT(theInterface != NULL);
  GS_ASSERT(theCertificate != NULL ||
  ...
  return sciWsCreateMatchlessSession(
      &anInterface->mWebServices, anInterface->mGameId, theCertificate,
      thePrivateData, theCallback, theTimeoutMs, theUserData);
}
...
SCResult SC_CALL scJoinSession(SCInterfacePtr     theInterface,
                                                           GSLoginCertificate *
theCertificate, GSLoginPrivateData * thePrivateData, gsi_u8
theSessionId[SC_SESSION_GUID_SIZE], SCSetReportIntentionCallback theCallback,
                                                           gsi_time
theTimeoutMs)
{
        SCInterface * anInterface = (SCInterface*)theInterface;
...
SCResult SC_CALL scSetReportIntention(
    ...
    gsi_bool isAuthoritative, const GSLoginCertificate *theCertificate,
    const GSLoginPrivateData *thePrivateData,
    SCSetReportIntentionCallback theCallback, gsi_time theTimeoutMs,
    void *theUserData) {
  SCInterface *anInterface = (SCInterface *)theInterface;
  ...
  return sciWsSetReportIntention(
      &anInterface->mWebServices, anInterface->mGameId,
      (char *)anInterface->mSessionId, (char *)anInterface->mConnectionId,
      isAuthoritative, theCertificate, thePrivateData, theCallback,
      theTimeoutMs, theUserData);
}
...
SCResult SC_CALL scSubmitReport(const SCInterfacePtr theInterface,
  ...
  // Prepare the report hash
  {
    ...
    MD5Init(&md5);
    MD5Update(&md5, (unsigned char *)aReport->mBuffer.mData,
              aReport->mBuffer.mPos);
  ...
  return sciWsSubmitReport(&anInterface->mWebServices, anInterface->mGameId,
                           ...
                           (char *)anInterface->mConnectionId, aReport,
                           isAuthoritative, theCertificate, thePrivateData,
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
SCResult SC_CALL scReportEnd(SCReportPtr theReport, gsi_bool isAuth,
...
SCResult SC_CALL scReportSetPlayerData(
    SCReportPtr theReport, gsi_u32 thePlayerIndex,
    const gsi_u8 thePlayerConnectionId[SC_CONNECTION_GUID_SIZE],
    gsi_u32 thePlayerTeamId, SCGameResult theResult, gsi_u32 theProfileId,
  ...
  if (aResult == SCResult_NO_ERROR)
    aResult = sciReportSetPlayerConnectionId(aReport, thePlayerIndex,
                                             thePlayerConnectionId);
  if (aResult == SCResult_NO_ERROR)
    aResult =
        sciReportSetPlayerTeamIndex(aReport, thePlayerIndex, thePlayerTeamId);
  if (aResult == SCResult_NO_ERROR)
    aResult = sciReportSetPlayerGameResult(aReport, thePlayerIndex, theResult);
  if (aResult == SCResult_NO_ERROR)
    aResult = sciReportSetPlayerAuthInfo(aReport, thePlayerIndex,
                                         theCertificate, theAuthHash);
...
SCResult SC_CALL scReportAddIntValue(const SCReportPtr theReport,
                                     gsi_u16 theKeyId, gsi_i32 theValue) {
  SCIReport *aReport = NULL;
  GS_ASSERT(theReport != NULL);
  aReport = (SCIReport *)theReport;
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
SCResult SC_CALL scReportAddInt64Value(SCReportPtr theReport, gsi_u16 theKeyId,
                                       gsi_i64 theValue) {
  SCIReport *aReport = NULL;
  GS_ASSERT(theReport != NULL);
  aReport = (SCIReport *)theReport;
...
SCResult SC_CALL scReportAddShortValue(const SCReportPtr theReport,
                                       gsi_u16 theKeyId, gsi_i16 theValue) {
  SCIReport *aReport = NULL;
  GS_ASSERT(theReport != NULL);
  aReport = (SCIReport *)theReport;
...
SCResult SC_CALL scReportAddByteValue(const SCReportPtr theReport,
                                      gsi_u16 theKeyId, gsi_i8 theValue) {
  SCIReport *aReport = NULL;
  GS_ASSERT(theReport != NULL);
  aReport = (SCIReport *)theReport;
...
SCResult SC_CALL scReportAddFloatValue(const SCReportPtr theReport,
                                       gsi_u16 theKeyId, float theValue) {
  SCIReport *aReport = NULL;
  GS_ASSERT(theReport != NULL);
  aReport = (SCIReport *)theReport;
...
SCResult SC_CALL scPeerCipherInit(const GSLoginCertificate *theLocalCert,
  ...
  //     randomness) 3) Repeat 1-2 for every 16 bytes of key block needed
  ...
  // GS_ASSERT(GS_CRYPT_RSA_BYTE_SIZE >= 32); // crypt lib must support 128-bit
  // keys.
  ...
  MD5Update(&md5, (unsigned char *)theLocalCert->mPeerPublicKey.modulus.mData,
            16); // first 16 bytes of the key
  ...
  MD5Update(&md5,
            (unsigned char *)&theLocalCert->mPeerPublicKey.modulus.mData[16],
            16); // last 16 bytes of the key
  ...
  gsDebugFormat(GSIDebugCat_App, GSIDebugType_Misc, GSIDebugLevel_Notice,
                "Created PeerCipher key block\r\n");
  gsDebugBinary(GSIDebugCat_App, GSIDebugType_Misc, GSIDebugLevel_Notice,
                (char *)theCipher->mKey, (gsi_i32)theCipher->mKeyLen);
...
SCResult SC_CALL scPeerCipherCreateKeyExchangeMsg(
    const GSLoginCertificate *theRemoteCert, const SCPeerCipher *theCipher,
    SCPeerKeyExchangeMsg theMsgOut) {
  // Encrypt the key with the recipients public key, this makes it safe to
  // transmit
  if (0 == gsCryptRSAEncryptBuffer(&theRemoteCert->mPeerPublicKey,
                                   theCipher->mKey, theCipher->mKeyLen,
                                   (unsigned char *)theMsgOut))
    return SCResult_NO_ERROR;
  else
...
SCResult SC_CALL scPeerCipherParseKeyExchangeMsg(
    ...
    const SCPeerKeyExchangeMsg theMsg, SCPeerCipher *theCipherOut) {
  GSI_UNUSED(theLocalCert);
  // Decrypt the key with the local player's private key
  if (0 == gsCryptRSADecryptBuffer(&theCertPrivateData->mPeerPrivateKey,
                                   (unsigned char *)theMsg, theCipherOut->mKey,
                                   &theCipherOut->mKeyLen)) {
    gsDebugFormat(GSIDebugCat_App, GSIDebugType_Misc, GSIDebugLevel_Notice,
                  "Decrypted PeerCipher key block\r\n");
    gsDebugBinary(GSIDebugCat_App, GSIDebugType_Misc, GSIDebugLevel_Notice,
                  (char *)theCipherOut->mKey, (gsi_i32)theCipherOut->mKeyLen);
    RC4Init(&theCipherOut->mRC4, theCipherOut->mKey,
            (int)theCipherOut->mKeyLen);
...
SCResult SC_CALL scPeerCipherEncryptBuffer(SCPeerCipher *theCipher,
  ...
  GS_ASSERT(gsi_is_true(theCipher->mInitialized));
  RC4Encrypt(&theCipher->mRC4, (const unsigned char *)theData, theData,
             (int)theLen);
  return SCResult_NO_ERROR;
}
