...
static gsi_bool CreateReportAndSubmit() {
  ...
  for (i = 0; i < SCTEST_NUM_PLAYERS; i++) {
    aResult = scCreateReport(gPlayerData[i].mStatsInterface,
                             SCTEST_KEY_HEADER_VERSION, SCTEST_NUM_PLAYERS,
                             SCTEST_NUM_TEAMS, &gPlayerData[i].mReport);
    ...
    scReportSetTeamData(gPlayerData[i].mReport, SCTEST_KEY_TEAM_ARENAID_HEROES,
                        SCGameResult_WIN);
    ...
    scReportEnd(gPlayerData[i].mReport, (i == 0) ? gsi_true : gsi_false,
                SCGameStatus_COMPLETE);

    // Submit the report
    aResult = scSubmitReport(
        gPlayerData[i].mStatsInterface, gPlayerData[i].mReport,
        (i == 0) ? gsi_true : gsi_false, &gPlayerData[i].mCertificate,
        &gPlayerData[i].mPrivateData, submitReportCallback, TIMEOUT_MS, NULL);
...
static gsi_bool RunTests(gsi_u8 loginMethod, gsi_bool isLoginOnly) {
  ...
  // strcpy(scServiceURL,
  // "http://*****/CompetitionService/CompetitionService.asmx");
  ...
  if (!isLoginOnly) {
    ...
    scCreateSession(gPlayerData[0].mStatsInterface,
                    ...
                    createSessionCallback, TIMEOUT_MS, NULL);
    ...
    scSetReportIntention(
        gPlayerData[0].mStatsInterface, NULL, gsi_true,
        &gPlayerData[0].mCertificate, &gPlayerData[0].mPrivateData,
