...
#define SCTEST_TOKEN_1                                                         \
  _T("GMTy13lsJmiY7L19ojyN3XTM08ll0C4EWWijwmJyq3ttiZmoDUQJ0OSnar9nQCu5MpOGvi") \
  _T("4Z0EcC2uNaS4yKrUA+h+tTDDoJHF7ZjoWKOTj00yNOEdzWyG08cKdVQwFRkF+h8oG/")     \
  _T("Jd+Ik3sWviXq/+5bhZQ7iXxTbbDwNL6Lagp/pLZ9czLnYPhY7VEcoQlx9oO")
#define SCTEST_CHALLENGE_1 _T("LH8c.DLe")
#define SCTEST_TOKEN_2                                                         \
  _T("GMTSc9Hl0VvG1FjqkCLkanABNB/")                                            \
  _T("u9jhNrQnXng6cQva0JtsJgEkwt6znjLDABAm3MmyEHDEcj6yZWwZmvPl1R8uEcJ0RW1Opl") \
  _T("ewifmRhfU3dglMq3OP1VOgX6+")                                              \
  _T("lG0SoPwp9BLmka8O4JGiSTSq8jGuDyHnCBLVozMMpeYPJ3wGAuxrx2OlWdUe3PTNhFK/")   \
  _T("LeNBl")
...
#define SCTEST_KEY_ROUND_TIME                                                  \
  17 // [TYPE: int] [DESC: The amount of time spent in one session]
#define SCTEST_KEY_TEAM_ID 18   // [TYPE: int]
#define SCTEST_KEY_TEAM_NAME 19 // [TYPE: string]
#define SCTEST_KEY_TEAM_RANK 20 // [TYPE: byte]
#define SCTEST_KEY_WINNINGTEAM                                                 \
  21 // [TYPE: int] [DESC: the team that won the round]
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
static void myLoginCallback(GHTTPResult httpResult,
                            WSLoginResponse *theResponse, void *theUserData) {
  if (httpResult != GHTTPSuccess) {
    ...
    memcpy(&newPlayer->mCertificate, &theResponse->mCertificate,
           sizeof(GSLoginCertificate));
    memcpy(&newPlayer->mPrivateData, &theResponse->mPrivateData,
           sizeof(GSLoginPrivateData));
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
/*static void myPs3LoginCallback(GHTTPResult httpResult, WSLoginPs3CertResponse
* theResponse, void * theUserData)
{
        if (httpResult != GHTTPSuccess)
        {
                gsDebugFormat(GSIDebugCat_App, GSIDebugType_Misc,
GSIDebugLevel_HotError, "Failed on player login, HTTP error: %d\r\n",
httpResult); getc(stdin); exit(0);
        }
        else if (theResponse->mLoginResult != WSLogin_Success)
        {

                gsDebugFormat(GSIDebugCat_App, GSIDebugType_Misc,
GSIDebugLevel_HotError, "Failed on player login, Login result: %d\r\n",
theResponse->mLoginResult); getc(stdin); exit(0);
        }
        else
        {
                SamplePlayerData * newPlayer = NULL;
...
static void myPlayerLogin(gsi_u8 logintype, const gsi_char *nick,
                          const gsi_char *password, int localPlayerNumber) {
  gsi_u32 result;

  if (logintype == SCTEST_LOGIN_PROFILE) {
    if (0 != wsLoginProfile(SCTEST_LOGIN_PARTNERCODE, SCTEST_LOGIN_NAMESPACE,
                            nick, SCTEST_EMAIL, password, _T(""),
                            myLoginCallback, (void *)localPlayerNumber)) {
      gsDebugFormat(GSIDebugCat_App, GSIDebugType_Misc, GSIDebugLevel_HotError,
    ...
    result = wsLoginUnique(SCTEST_LOGIN_PARTNERCODE, SCTEST_LOGIN_NAMESPACE,
                           nick, password, _T(""), myLoginCallback,
                           (void *)localPlayerNumber);
    ...
    result = wsLoginRemoteAuth(SCTEST_REMOTE_PARTNERCODE,
                               SCTEST_REMOTE_NAMESPACEID, nick, password,
                               myLoginCallback, (void *)localPlayerNumber);
    if (result != WSLogin_Success) {
      gsDebugFormat(GSIDebugCat_App, GSIDebugType_Misc, GSIDebugLevel_HotError,
                    "Failed on wsLoginUnique. Result: %d\r\n", result);
  ...
  {
          gsDebugFormat(GSIDebugCat_App, GSIDebugType_Misc,
  GSIDebugLevel_HotError, "Failed on wsLoginPs3Cert. Result: %d\r\n", result);
          getc(stdin);
          exit(0);
  }*/
...
static void myRecordPlayerJoined(gsi_u32 theProfileId, gsi_u32 theTeamId) {
  ...
  //   TODO: dervice obfuscation seed in a way that prevents copying between
  //   variables
  memcpy(newPlayer->mHiddenFrags, gServerData.mObfuscationSeed,
         sizeof(newPlayer->mHiddenDeaths));
  memcpy(newPlayer->mHiddenDeaths, gServerData.mObfuscationSeed,
         sizeof(newPlayer->mHiddenDeaths));
  memcpy(newPlayer->mHiddenScore, gServerData.mObfuscationSeed,
         sizeof(newPlayer->mHiddenDeaths));
  memcpy(newPlayer->mHiddenShots, gServerData.mObfuscationSeed,
         sizeof(newPlayer->mHiddenDeaths));
...
void createSessionCallback(SCInterfacePtr theInterface,
                           GHTTPResult theHttpResult, SCResult theResult,
                           void *theUserData) {
...
void setReportIntentionCallback(SCInterfacePtr theInterface,
  ...
  gsDebugFormat(
      GSIDebugCat_App, GSIDebugType_Misc, GSIDebugLevel_Notice,
...
static gsi_bool CreateReportAndSubmit() {
  ...
  ////////////////////////////////////////
  // Submit report
  //    This is a little hard to demonstrate since we are simulating four
  //    players. In a live application you would need to determine which players
  //    would submit data and when, depending on game specifics.

  //    It's not as complicated as it may sound.
  //    - For late entry games, this will likely occur after each map rotation
  //    - For coordinated entry, this will likely occur once for each player
  //    eliminated
  //
  // Build the report
  ...
  for (i = 0; i < SCTEST_NUM_PLAYERS; i++) {
    ...
    scReportAddStringValue(gPlayerData[i].mReport, SCTEST_KEY_MAPNAME,
                           gServerData.mMapName);
    scReportAddIntValue(gPlayerData[i].mReport, SCTEST_KEY_GAMETYPE,
                        gServerData.mGameType);
    scReportAddStringValue(gPlayerData[i].mReport, SCTEST_KEY_GAMEVER,
                           gServerData.mVersion);
    scReportAddStringValue(gPlayerData[i].mReport, SCTEST_KEY_HOSTNAME,
                           gServerData.mHostName);
    scReportAddIntValue(gPlayerData[i].mReport, SCTEST_KEY_WINNINGTEAM,
                        SCTEST_KEY_TEAM_ARENAID_HEROES);

    // test float and byte values
    scReportAddFloatValue(gPlayerData[i].mReport, SCTEST_KEY_ROUND_TIME,
                          gServerData.mRoundTime);
    scReportAddByteValue(gPlayerData[i].mReport, SCTEST_KEY_CUSTOM_MAP,
                         gServerData.mCustomMap);
    ...
    scReportSetPlayerData(
        gPlayerData[i].mReport, (gsi_u32)0, gPlayerData[i].mConnectionId,
        gPlayerData[i].mTeam, playersGameResult, gPlayerData[i].mProfileId,
        &gPlayerData[i].mCertificate, gPlayerData[i].mStatsAuthdata);
    scReportAddStringValue(gPlayerData[i].mReport, SCTEST_KEY_PLAYER_NICK,
                           gPlayerData[i].mCertificate.mUniqueNick);
    scReportAddIntValue(gPlayerData[i].mReport, SCTEST_KEY_PLAYER_FRAGS,
                        gPlayerData[i].mFrags);
    scReportAddIntValue(gPlayerData[i].mReport, SCTEST_KEY_PLAYER_SCORE,
                        gPlayerData[i].mScore);
    scReportAddShortValue(gPlayerData[i].mReport, SCTEST_KEY_PLAYER_SHOTS,
                          gPlayerData[i].mShots);
    scReportAddShortValue(gPlayerData[i].mReport, SCTEST_KEY_PLAYER_DEATHS,
                          gPlayerData[i].mDeaths);
    scReportAddIntValue(gPlayerData[i].mReport, SCTEST_KEY_PLAYER_TEAM,
                        (gsi_i32)gPlayerData[i].mTeam);
    ...
    if ((i & 1) == 0) {
      ...
      scReportSetTeamData(gPlayerData[i].mReport,
                          SCTEST_KEY_TEAM_ARENAID_HEROES, SCGameResult_WIN);
      scReportAddIntValue(gPlayerData[i].mReport, SCTEST_KEY_TEAM_ID,
                          SCTEST_KEY_TEAM_ARENAID_HEROES);
      scReportAddByteValue(gPlayerData[i].mReport, SCTEST_KEY_TEAM_RANK,
                           SCTEST_TEAMRANK_HEROES);
      scReportAddStringValue(gPlayerData[i].mReport, SCTEST_KEY_TEAM_NAME,
                             SCTEST_KEY_TEAM_NAME_HEROES);
    } else {
      scReportBeginNewTeam(gPlayerData[i].mReport);
      scReportSetTeamData(gPlayerData[i].mReport,
                          SCTEST_KEY_TEAM_ARENAID_VILLAINS, SCGameResult_LOSS);
      scReportAddIntValue(gPlayerData[i].mReport, SCTEST_KEY_TEAM_ID,
                          SCTEST_KEY_TEAM_ARENAID_VILLAINS);
      scReportAddByteValue(gPlayerData[i].mReport, SCTEST_KEY_TEAM_RANK,
                           SCTEST_TEAMRANK_VILLAINS);
      scReportAddStringValue(gPlayerData[i].mReport, SCTEST_KEY_TEAM_NAME,
                             SCTEST_KEY_TEAM_NAME_VILLAINS);
    }
    ...
    // The setting of an authoritative report in matchless sessions should
    // be ignored.  It can be set to either gsi_true or gsi_false
    aResult = scSubmitReport(
        gPlayerData[i].mStatsInterface, gPlayerData[i].mReport, gsi_false,
        &gPlayerData[i].mCertificate, &gPlayerData[i].mPrivateData,
  ...
  // The host would then send all other player's the Session ID generated when
  // the session was created so that they can set their intentions, and retrieve
  // the connection ID for report submissions
  // scSetSessionId(gPlayerData[1].mStatsInterface, (const unsigned char
  // *)scGetSessionId(gPlayerData[0].mStatsInterface));

  scSetReportIntention(
      gPlayerData[1].mStatsInterface,
      (const gsi_u8 *)scGetConnectionId(gPlayerData[1].mStatsInterface),
      gsi_false, &gPlayerData[1].mCertificate, &gPlayerData[1].mPrivateData,
      setReportIntentionCallback, TIMEOUT_MS, &gPlayerData[1]);

  // store each connection id for the other players
  // strcpy((char *)gPlayerData[j].mConnectionId,
  // scGetConnectionId(gPlayerData[j].mStatsInterface));

  // *NOTE* this is simply meant to show HOW this process works, in truth the
  // connection id here is overwritten because it's using the same interface so
  // it will report 4 of the same player information. We need to upgrade the
  // sample to communicate with other instances in order to report individual
  // player data
...
static gsi_bool RunTests(gsi_u8 loginMethod, gsi_bool isLoginOnly) {
  ...
  // strcpy(wsAuthServiceURL,
  // "http://mwstage.gamespy.com/AuthService/AuthService.asmx");
  // strcpy(scServiceURL,
  // "http://mwstage.gamespy.com/CompetitionService/CompetitionService.asmx");
  ...
  if (loginMethod == SCTEST_LOGIN_REMOTEAUTH) {
    myPlayerLogin(SCTEST_LOGIN_REMOTEAUTH, SCTEST_TOKEN_1, SCTEST_CHALLENGE_1,
                  0);
    myPlayerLogin(SCTEST_LOGIN_REMOTEAUTH, SCTEST_TOKEN_2, SCTEST_CHALLENGE_2,
                  1);
  ...
  if (!isLoginOnly) {
    ...
    for (i = 0; i < SCTEST_NUM_PLAYERS; i++) {
      ...
      if (aResult != SCResult_NO_ERROR) {
        gsDebugFormat(GSIDebugCat_App, GSIDebugType_Misc,
                      GSIDebugLevel_HotError, "scInitialize returned %s\r\n",
                      SCResultStr[aResult]);
    ...
    //    Certificates may also be used to exchange keys for encrypting game
    //    traffic.
    //
    //    Step1 - Trade certificates using unsecured game socket. (The sample
    //    doesn't do this) Step2 - Verify certificate is authentic using
    //    wsLoginCertIsValid Step3 - Verify that the guy who gave you the
    //    certificate actually owns it

    // Step1 - skipped.
    //       - The usual scenario is to have the host verify the client
    //       certificate when the player joins.
    ...
    for (i = 0; i < SCTEST_NUM_PLAYERS; i++) {
      for (k = i + 1; k < SCTEST_NUM_PLAYERS; k++) {
        ...
        // Each player should create a key for receiving data from the remote
        // player
        //     For extra security, we use a different encryption key for each
        //     channel
        scPeerCipherInit(
        ...
        // Create a key exchange message for transmitting the key to the other
        // player
        scPeerCipherCreateKeyExchangeMsg(
            &gPlayerData[k].mCertificate, &gPlayerData[i].mPeerRecvCipher[k],
            exchangeMsg1); // from 'i', encrypted with 'k' public key
        scPeerCipherCreateKeyExchangeMsg(
            &gPlayerData[i].mCertificate, &gPlayerData[k].mPeerRecvCipher[i],
        ...
        scPeerCipherParseKeyExchangeMsg(
            &gPlayerData[k].mCertificate, &gPlayerData[k].mPrivateData,
            exchangeMsg1,
        ...
        // Now we can send secure data by using the (fast) encryption and
        // decryption functions
        //   - Encrypts and Decrypts in place
        strcpy(cipherMsg, plainTextMsg);
        scPeerCipherEncryptBufferIV(&gPlayerData[i].mPeerSendCipher[k], 1,
                                    (gsi_u8 *)cipherMsg,
                                    (gsi_u32)msgLen); // 'i' sending to 'k'
        scPeerCipherDecryptBufferIV(&gPlayerData[k].mPeerRecvCipher[i], 1,
                                    (gsi_u8 *)cipherMsg,
    ...
    // REQUIRED FOR MATCHLESS GAMES
    // For matchless games this function needs to be called so that the
    // Competition System is aware of the type of reports coming in.
    scCreateMatchlessSession(
        gPlayerData[0].mStatsInterface, &gPlayerData[0].mCertificate,
        &gPlayerData[0].mPrivateData, createSessionCallback, TIMEOUT_MS, NULL);

    myWaitForCallbacks(gPlayerData[0].mStatsInterface, 1);
    // End creating the session

    // The host can now set his connection ID
    strcpy((char *)gPlayerData[0].mConnectionId,
           scGetConnectionId(gPlayerData[0].mStatsInterface));
    ...
    // system will not need to wait for all players to submit before processing
    // stats.
    scSetReportIntention(
        gPlayerData[0].mStatsInterface, NULL, gsi_false,
        &gPlayerData[0].mCertificate, &gPlayerData[0].mPrivateData,
        setReportIntentionCallback, TIMEOUT_MS, &gPlayerData[0]);

    myWaitForCallbacks(gPlayerData[0].mStatsInterface, 1);

    for (j = 1; j < SCTEST_NUM_PLAYERS; j++) {
      // The host would then send all other player's the Session ID generated
      // when the session was created so that they can set their intentions, and
      // retrieve the connection ID for report submissions
      scSetSessionId(gPlayerData[j].mStatsInterface,
                     (const unsigned char *)scGetSessionId(
                         gPlayerData[0].mStatsInterface));

      scSetReportIntention(
          gPlayerData[j].mStatsInterface, NULL, gsi_false,
          &gPlayerData[j].mCertificate, &gPlayerData[j].mPrivateData,
          setReportIntentionCallback, TIMEOUT_MS, &gPlayerData[j]);

      // store each connection id for the other players
      // strcpy((char *)gPlayerData[j].mConnectionId,
      // scGetConnectionId(gPlayerData[j].mStatsInterface));

      // *NOTE* this is simply meant to show HOW this process works, in truth
      // the connection id here is overwritten because it's using the same
      // interface so it will report 4 of the same player information. We need
      // to upgrade the sample to communicate with other instances in order to
      // report individual player data
    ...
    memset(gServerData.mObfuscationSeed, 0,
           sizeof(gServerData.mObfuscationSeed));

    // Pretend everyone is joining now
    for (i = 0; i < SCTEST_NUM_PLAYERS; i++) {
      if ((i & 1) == 0) // even players on hero team, odd players on villans
      {
        myRecordPlayerJoined(gPlayerData[i].mProfileId,
                             (gsi_u32)((i & 1) == 0
                                           ? SCTEST_KEY_TEAM_ARENAID_HEROES
                                           : SCTEST_KEY_TEAM_ARENAID_VILLAINS));
      } else {
    ...
    for (i = 0; i < 100; i++) {
      ...
      gsi_u32 aShooteeId =
          myGetRandomPlayerId(aShooterId); // prevent shooter from suiciding
    ...
    for (i = 0; i < SCTEST_NUM_PLAYERS; i++)
      myRecordPlayerScored(gPlayerData[i].mProfileId,
                           (gsi_u32)myGetRandomInt(100));
