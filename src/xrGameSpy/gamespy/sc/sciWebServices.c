...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
#define SC_CREATEMATCHLESSSESSION_SOAPACTION                                   \
  "SOAPAction: \"http://gamespy.net/competition/CreateMatchlessSession\""
#define SC_CREATESESSION_SOAPACTION                                            \
  "SOAPAction: \"http://gamespy.net/competition/CreateSession\""
#define SC_SUBMITREPORT_SOAPACTION                                             \
  "SOAPAction: \"http://gamespy.net/competition/SubmitReport\""
#define SC_SETINTENTION_SOAPACTION                                             \
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
SCResult sciWsCreateSession(SCWebServices *theWebServices, gsi_u32 theGameId,
  ...
  aRequest = gsXmlCreateStreamWriter(SC_SERVICE_NAMESPACES,
                                     SC_SERVICE_NAMESPACE_COUNT);
  if (aRequest == NULL)
  ...
  if (gsi_is_false(gsXmlWriteOpenTag(aRequest, "gsc", "CreateSession")) ||
      ...
      gsi_is_false(gsXmlWriteHexBinaryElement(
          aRequest, "gsc", "proof", (const gsi_u8 *)thePrivateData->mKeyHash,
          GS_CRYPT_MD5_HASHSIZE)) ||
      gsi_is_false(gsXmlWriteIntElement(aRequest, "gsc", "gameid",
                                        (gsi_u32)theGameId)) ||
  ...
  // Execute soap call
  gsiExecuteSoap(scServiceURL, SC_CREATESESSION_SOAPACTION, aRequest,
                 sciWsCreateSessionCallback, theWebServices);
...
SCResult sciWsCreateMatchlessSession(SCWebServices *theWebServices,
  ...
  // Fill in the request data
  if (gsi_is_false(
          gsXmlWriteOpenTag(aRequest, "gsc", "CreateMatchlessSession")) ||
      gsi_is_false(gsXmlWriteOpenTag(aRequest, "gsc", "certificate")) ||
      gsi_is_false(wsLoginCertWriteXML(theCertificate, "gsc", aRequest)) ||
      gsi_is_false(gsXmlWriteCloseTag(aRequest, "gsc", "certificate")) ||
      gsi_is_false(gsXmlWriteHexBinaryElement(
      ...
      gsi_is_false(gsXmlWriteIntElement(aRequest, "gsc", "gameid",
                                        (gsi_u32)theGameId)) ||
      gsi_is_false(
          gsXmlWriteCloseTag(aRequest, "gsc", "CreateMatchlessSession")) ||
      gsi_is_false(gsXmlCloseWriter(aRequest))) {
    gsXmlFreeWriter(aRequest);
    return SCResult_HTTP_ERROR;
  }
  ...
  // Execute soap call
  gsiExecuteSoap(scServiceURL, SC_CREATEMATCHLESSSESSION_SOAPACTION, aRequest,
                 sciWsCreateSessionCallback, theWebServices);
  GSI_UNUSED(theTimeoutMs);
  return SCResult_NO_ERROR;
}
...
void sciWsCreateSessionCallback(GHTTPResult theHttpResult,
  ...
  if (theHttpResult == GHTTPSuccess) {
    ...
    // Parse through in a way that will work for either type of CreateSession
    // response.
    if (gsi_is_false(gsXmlMoveToStart(theResponseData))) {
      aTranslatedResult = SCResult_RESPONSE_INVALID;
    } else if (gsi_is_false(
                   gsXmlMoveToNext(theResponseData, "CreateSessionResponse"))) {
      if (gsi_is_false(gsXmlMoveToNext(theResponseData,
                                       "CreateMatchlessSessionResponse"))) {
        aTranslatedResult = SCResult_RESPONSE_INVALID;
      }
    }
    ...
    if (gsi_is_false(gsXmlMoveToNext(theResponseData, "CreateSessionResult"))) {
      if (gsi_is_false(gsXmlMoveToNext(theResponseData,
                                       "CreateMatchlessSessionResult"))) {
        aTranslatedResult = SCResult_RESPONSE_INVALID;
      }
    }

    if (gsi_is_false(
            gsXmlReadChildAsInt(theResponseData, "result", &createResult))) {
      aTranslatedResult = SCResult_RESPONSE_INVALID;
      ...
      if (createResult == SCWsResult_NO_ERROR) {
        ...
        if (gsi_is_false(gsXmlReadChildAsStringNT(theResponseData, "csid", csid,
                                                  csidLen)) ||
            gsi_is_false(gsXmlReadChildAsStringNT(theResponseData, "ccid", ccid,
                                                  ccidLen))) {
          aTranslatedResult = SCResult_RESPONSE_INVALID;
        } else {
          sciInterfaceSetSessionId((SCInterface *)aWebServices->mInterface,
                                   csid);
          sciInterfaceSetConnectionId((SCInterface *)aWebServices->mInterface,
                                      ccid);
  ...
  if (aWebServices->mCreateSessionCallback != NULL) {
    aWebServices->mCreateSessionCallback(aWebServices->mInterface,
                                         theHttpResult, aTranslatedResult,
...
SCResult sciWsSetReportIntention(SCWebServices *theWebServices,
                                 gsi_u32 theGameId, const char *theSessionId,
  ...
  if (gsi_is_false(gsXmlWriteOpenTag(aRequest, "gsc", "SetReportIntention")) ||
      ...
      gsi_is_false(gsXmlWriteHexBinaryElement(
          ...
          GS_CRYPT_MD5_HASHSIZE)) ||
      gsi_is_false(
          gsXmlWriteStringElement(aRequest, "gsc", "csid", theSessionId)) ||
      gsi_is_false(
          gsXmlWriteStringElement(aRequest, "gsc", "ccid", theConnectionId)) ||
      gsi_is_false(gsXmlWriteIntElement(aRequest, "gsc", "gameid",
                                        (gsi_u32)theGameId)) ||
      gsi_is_false(gsXmlWriteIntElement(
          aRequest, "gsc", "authoritative",
          (gsi_u32)(gsi_is_true(isAuthoritative) ? 1 : 0))) ||
  ...
  // Execute soap call
  gsiExecuteSoap(scServiceURL, SC_SETINTENTION_SOAPACTION, aRequest,
                 sciWsSetReportIntentionCallback, theWebServices);
...
void sciWsSetReportIntentionCallback(GHTTPResult theHttpResult,
  ...
  if (theHttpResult == GHTTPSuccess) {
    ...
    if (gsi_is_false(gsXmlMoveToStart(theResponseData)) ||
        gsi_is_false(
            gsXmlMoveToNext(theResponseData, "SetReportIntentionResponse")) ||
        gsi_is_false(
            gsXmlMoveToNext(theResponseData, "SetReportIntentionResult")) ||
        gsi_is_false(
            gsXmlReadChildAsInt(theResponseData, "result", &intentionResult)) ||
        gsi_is_false(
            gsXmlReadChildAsStringNT(theResponseData, "ccid", ccid, ccidLen))) {
      aTranslatedResult = SCResult_RESPONSE_INVALID;
    } else {
  ...
  if (aWebServices->mSetReportIntentionCallback != NULL) {
    aWebServices->mSetReportIntentionCallback(
        aWebServices->mInterface, theHttpResult, aTranslatedResult,
...
extern GHTTPBool ghiPostAddFileFromMemory(GHTTPPost post, const char *name,
...
static void sciWsSubmitReportCustom(GHTTPPost thePost, void *theUserData) {
  ...
  ghiPostAddFileFromMemory(
      thePost, "report", (char *)aWebServices->mSubmitReportData,
      (gsi_i32)aWebServices->mSubmitReportLength, "report", "application/bin");
}

///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
SCResult sciWsSubmitReport(SCWebServices *theWebServices, gsi_u32 theGameId,
                           const char *theSessionId,
                           const char *theConnectionId,
                           const SCIReport *theReport, gsi_bool isAuthoritative,
                           const GSLoginCertificate *theCertificate,
                           const GSLoginPrivateData *thePrivateData,
  ...
  // Execute soap call
  gsiExecuteSoapCustom(scServiceURL, SC_SUBMITREPORT_SOAPACTION, aRequest,
                       sciWsSubmitReportCallback, sciWsSubmitReportCustom,
                       theWebServices);
...
void sciWsSubmitReportCallback(GHTTPResult theHttpResult,
  ...
  if (theHttpResult == GHTTPSuccess) {
    ...
    if (gsi_is_false(gsXmlMoveToStart(theResponseData)) ||
        gsi_is_false(
            gsXmlMoveToNext(theResponseData, "SubmitReportResponse")) ||
        gsi_is_false(gsXmlMoveToNext(theResponseData, "SubmitReportResult")) ||
        gsi_is_false(
            gsXmlReadChildAsInt(theResponseData, "result", &submitResult))) {
      aTranslatedResult = SCResult_RESPONSE_INVALID;
    } else {
      switch (submitResult) {
