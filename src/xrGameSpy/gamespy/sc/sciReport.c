...
SCResult SC_CALL sciCreateReport(gsi_u8 theSessionGuid[SC_SESSION_GUID_SIZE],
                                 ...
                                 gsi_u32 thePlayerCount, gsi_u32 theTeamCount,
  ...
  theReportHeader->mAuthSectionLength =
      SC_REPORT_AUTHDATA_LENGTH * thePlayerCount;
  theReportHeader->mResultsSectionLength =
      SC_REPORT_ENTITYRESULT_LENGTH * (thePlayerCount + theTeamCount);
...
SCResult SC_CALL sciReportSetPlayerConnectionId(
  ...
  GS_ASSERT((rosterDataOffset + SC_GUID_BINARY_SIZE) <
            theReport->mBuffer.mCapacity);

  sciSerializeGUID((gsi_u8 *)&theReport->mBuffer.mData[rosterDataOffset],
                   theConnectionId);
  // memcpy(&theReport->mBuffer.mData[rosterDataOffset], theConnectionId,
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
SCResult SC_CALL sciReportSetPlayerTeamIndex(SCIReport *theReport,
  ...
  GS_ASSERT((rosterDataOffset + sizeof(gsi_i32)) <
            theReport->mBuffer.mCapacity);
  sciSerializeInt32((gsi_u8 *)&theReport->mBuffer.mData[rosterDataOffset],
                    (gsi_i32)theTeamIndex);
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
SCResult SC_CALL sciReportSetPlayerGameResult(SCIReport *theReport,
                                              gsi_u32 thePlayerIndex,
                                              SCGameResult theGameResult) {
  SCIReportHeader *aHeader = (SCIReportHeader *)theReport->mBuffer.mData;
  gsi_u32 resultsDataOffset = 0;

  // Results section is just after the auth section
  resultsDataOffset = sizeof(SCIReportHeader) + aHeader->mRosterSectionLength +
                      aHeader->mAuthSectionLength;
  resultsDataOffset +=
      sizeof(gsi_u32) * thePlayerIndex; // 4 byte game result per player

  // copy the game result in network byte order
  GS_ASSERT((resultsDataOffset + sizeof(gsi_u32)) <
            theReport->mBuffer.mCapacity);
  sciSerializeInt32((gsi_u8 *)&theReport->mBuffer.mData[resultsDataOffset],
                    theGameResult);
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
SCResult SC_CALL sciReportSetTeamGameResult(SCIReport *theReport,
                                            gsi_u32 theTeamIndex,
                                            SCGameResult theGameResult) {
  SCIReportHeader *aHeader = (SCIReportHeader *)theReport->mBuffer.mData;
  gsi_u32 resultsDataOffset = 0;
  ...
  resultsDataOffset = sizeof(SCIReportHeader) + aHeader->mRosterSectionLength +
                      aHeader->mAuthSectionLength;
  resultsDataOffset +=
      sizeof(gsi_u32) * aHeader->mPlayerCount; // 4 byte game result per player
  resultsDataOffset +=
      sizeof(gsi_u32) * theTeamIndex; // 4 byte game result per player

  // copy the game result in network byte order
  GS_ASSERT((resultsDataOffset + sizeof(gsi_i32)) <
            theReport->mBuffer.mCapacity);
  sciSerializeInt32((gsi_u8 *)&theReport->mBuffer.mData[resultsDataOffset],
                    theGameResult);
...
SCResult SC_CALL sciReportPlayerAuth(SCIReport * theReport,
                                                                         gsi_u32
thePlayerIndex, gsi_u8      thePlayerConnectionId[SC_CONNECTION_GUID_SIZE],
                                                                         gsi_u32
thePlayerTeamIndex, gsi_u32     theProfileId, GSLoginCertificate  *
theCertificate, gsi_u8      theAuthHash[16])
{
        SCIReportHeader * aHeader = (SCIReportHeader*)theReport->mBuffer.mData;
        //unsigned int len = 0;
        ...
        // Auth section is just after the roster
        authDataOffset = sizeof(SCIReportHeader) +
aHeader->mRosterSectionLength;

        // Check size and length
        GS_ASSERT((authDataOffset + aHeader->mAuthSectionLength) <
SC_REPORT_BUFFER_BYTES);

        // Record player index and team in roster section
        //     The report contains an array of players, so we offset the write
position
        //     based on the number of players previously reported
        rosterDataOffset += aHeader->mPlayerCount * (SC_GUID_BINARY_SIZE +
SC_REPORT_ENTITYINDEX_LENGTH);

        // write the connection id GUID
        GS_ASSERT(rosterDataOffset < authDataOffset);
        sciSerializeGUID((gsi_u8 *)&theReport->mBuffer.mData[rosterDataOffset],
thePlayerConnectionId); rosterDataOffset += SC_GUID_BINARY_SIZE;


//	GS_ASSERT(rosterDataOffset < authDataOffset);
//	memcpy(&theReport->mBuffer.mData[rosterDataOffset],
thePlayerConnectionId, SC_GUID_BINARY_SIZE);
//	rosterDataOffset += SC_CONNECTION_GUID_SIZE;


        GS_ASSERT(rosterDataOffset < authDataOffset);
        sciSerializeInt16((gsi_u8 *)&theReport->mBuffer.mData[rosterDataOffset],
(gsi_i16)thePlayerTeamIndex); rosterDataOffset += 2;  // we just wrote an Int16,
so bump by two.

        GS_ASSERT(rosterDataOffset < authDataOffset); // check overrun

        // Write auth data for this player
        //     The report contains an array of auth data, so we offset the write
position
        //     based on the number of players previously reported
        authDataOffset += aHeader->mPlayerCount * (SC_GUID_BINARY_SIZE +
SC_REPORT_ENTITYINDEX_LENGTH);

        GS_ASSERT((authDataOffset + SC_REPORT_AUTHDATA_LENGTH) <
theReport->mBuffer.mCapacity); // check buffer space

        // Write auth data
        //result = wsLoginCertWriteBinary(theCertificate,
&theReport->mBuffer.mData[theReport->mBuffer.mPos], (SC_REPORT_BUFFER_BYTES -
theReport->mBuffer.mPos), &len);
        //if (gsi_is_false(result))
        //	return SCResult_OUT_OF_MEMORY;
        //memcpy(&theReport->mBuffer.mData[theReport->mBuffer.mPos],
theAuthHash, 16);
        //theReport->mBuffer.mPos += 16;

        //theReport->mNumPlayersReported++; // remember how many player's we've
written data for GSI_UNUSED(theAuthHash); GSI_UNUSED(theCertificate);
...
SCResult SC_CALL sciReportPlayerResult(SCIReport * theReport,
                                                                           gsi_u32
thePlayerIndex, SCGameResult theResult)
{
        SCIReportHeader * aHeader = (SCIReportHeader*)theReport->mBuffer.mData;
        //unsigned int len = 0;
        gsi_u32 resultsOffset = 0;

        resultsOffset = sizeof(SCIReportHeader) + aHeader->mRosterSectionLength
+ aHeader->mAuthSectionLength; GS_ASSERT(resultsOffset <
SC_REPORT_BUFFER_BYTES);

        resultsOffset += SC_REPORT_ENTITYRESULT_LENGTH *
theReport->mNumResultsReported; sciSerializeInt32((gsi_u8
*)&theReport->mBuffer.mData[resultsOffset], theResult);
...
SCResult SC_CALL sciReportTeamResult(SCIReport * theReport,
                                                                         gsi_u32
theTeamIndex, SCGameResult theResult)
{
        SCIReportHeader * aHeader = (SCIReportHeader*)theReport->mBuffer.mData;
        //unsigned int len = 0;
        gsi_u32 resultsOffset = 0;
        ...
        resultsOffset = sizeof(SCIReportHeader) + aHeader->mRosterSectionLength
+ aHeader->mAuthSectionLength; resultsOffset += SC_REPORT_ENTITYRESULT_LENGTH *
theReport->mNumResultsReported; GS_ASSERT(resultsOffset <
SC_REPORT_BUFFER_BYTES);
...
SCResult SC_CALL sciReportBeginNewTeam(SCIReport *theReport) {
  ...
  sciSerializeInt16(
      (gsi_u8 *)&theReport->mBuffer.mData[theReport->mCurEntityStartPos],
      (gsi_i16)theReport->mCurEntityKeyCount);

  aHeader->mTeamSectionLength += sizeof(gsi_u16);

  // Mark the start of the new entity
  // theReport->mStatus.mCurEntityStart =
  // &theReport->mBuffer.mData[theReport->mBuffer.mPos];
  // theReport->mStatus.mCurEntityType = SC_REPORT_ENTITY_TEAM;
  // theReport->mBuffer.mPos += 4; // skip 4 byte for length, it will be filled
  // in later aHeader->mTeamCount++;
...
SCResult SC_CALL sciReportBeginNewPlayer(SCIReport *theReport) {
  ...
  // theReport->mBuffer.mPos += 4; // skip 4 byte for length, it will be filled
  // in later
  // sciSerializeInt32(&theReport->mBuffer.mData[theReport->mBuffer.mPos],
  // thePlayerIndex); theReport->mBuffer.mPos += 4; // 4 byte index
...
SCResult SC_CALL sciReportBeginGlobalData(SCIReport *theReport) {
  ...
  theReport->mBuffer.mPos =
      sizeof(SCIReportHeader) + aHeader->mRosterSectionLength +
...
SCResult SC_CALL sciReportEndEntity(SCIReport *theReport) {
  ...
  /*if (theReport->mStatus.mCurEntityStart != NULL)
  {
          ...
          sciSerializeDataLength(theReport->mStatus.mCurEntityStart,
  anEntityLen); theReport->mStatus.mCurEntityStart = NULL;
  }*/
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
SCResult SC_CALL sciReportEnd(SCIReport *theReport, gsi_bool isAuth,
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
SCResult SC_CALL sciReportAddIntValue(SCIReport *theReport, gsi_u16 theKeyId,
                                      gsi_i32 theValue) {
  SCIReportHeader *aHeader = (SCIReportHeader *)theReport->mBuffer.mData;
  int writtenLen = 0;
  ...
  writtenLen +=
      sizeof(gsi_u16) + sizeof(gsi_u16); // 2 byte key ID, 2 byte key type;
  writtenLen += sizeof(gsi_u32);         // 4 bytes of data

  // Check room in buffer
  GS_ASSERT((theReport->mBuffer.mPos + writtenLen) <
            theReport->mBuffer.mCapacity);
  ...
  // Write the data
  sciSerializeInt16(
      (gsi_u8 *)&theReport->mBuffer.mData[theReport->mBuffer.mPos],
      (gsi_i16)theKeyId);
  theReport->mBuffer.mPos += sizeof(gsi_u16);
  sciSerializeInt16(
      (gsi_u8 *)&theReport->mBuffer.mData[theReport->mBuffer.mPos], theKeyType);
  theReport->mBuffer.mPos += sizeof(gsi_u16);
  sciSerializeInt32(
      (gsi_u8 *)&theReport->mBuffer.mData[theReport->mBuffer.mPos], theValue);
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
SCResult SC_CALL sciReportAddInt64Value(SCIReport *theReport, gsi_u16 theKeyId,
                                        gsi_i64 theValue) {
  SCIReportHeader *aHeader = (SCIReportHeader *)theReport->mBuffer.mData;
  int writtenLen = 0;
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
SCResult SC_CALL sciReportAddShortValue(SCIReport *theReport, gsi_u16 theKeyId,
                                        gsi_i16 theValue) {
  SCIReportHeader *aHeader = (SCIReportHeader *)theReport->mBuffer.mData;
  int writtenLen = 0;
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
SCResult SC_CALL sciReportAddByteValue(SCIReport *theReport, gsi_u16 theKeyId,
                                       gsi_i8 theValue) {
  SCIReportHeader *aHeader = (SCIReportHeader *)theReport->mBuffer.mData;
  int writtenLen = 0;
  ...
  sciSerializeInt16(
      (gsi_u8 *)&theReport->mBuffer.mData[theReport->mBuffer.mPos], theKeyType);
  theReport->mBuffer.mPos += sizeof(gsi_u16);
  sciSerializeInt8((gsi_u8 *)&theReport->mBuffer.mData[theReport->mBuffer.mPos],
                   theValue);
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
SCResult SC_CALL sciReportAddFloatValue(SCIReport *theReport, gsi_u16 theKeyId,
                                        float theValue) {
  SCIReportHeader *aHeader = (SCIReportHeader *)theReport->mBuffer.mData;
  int writtenLen = 0;
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
SCResult SC_CALL sciReportAddStringValue(SCIReport *theReport, gsi_u16 theKeyId,
                                         const gsi_char *theValue) {
  SCIReportHeader *aHeader = (SCIReportHeader *)theReport->mBuffer.mData;
  int writtenLen = 0, numLenBytes = 0, asciiLen = 0;
  gsi_i16 theKeyType = SCIKeyType_STRING;

  // for unicode we need to store the ascii-converted string first in the buffer
  // so as not to dynamically allocate space for it. Then once we have the
  // length of it, we can write over the buffer

#if defined(GSI_UNICODE)
  // First check room in buffer
  GS_ASSERT(((theReport->mBuffer.mPos + (_tcslen(theValue)) +
              (sizeof(gsi_u16) * 2)) < theReport->mBuffer.mCapacity));

  // as long as we have enough room - add to the buffer and store length of the
  // Ascii string need to remove 1 for the appended null char \0 which we want
  // to remove
  asciiLen = UCS2ToAsciiString(
                 theValue, &theReport->mBuffer.mData[theReport->mBuffer.mPos]) -
             1;
  ...
  numLenBytes = (gsi_i32)(strlen(theValue) / 127) +
                1; // finds out number of bytes necessary to represent len
  ...
  // now prior to writing the string we need to prepend the length of the string
  // to follow the .NET format of (string length)(String)
  memset(&theReport->mBuffer.mData[theReport->mBuffer.mPos], asciiLen,
         (gsi_u32)numLenBytes);
  theReport->mBuffer.mPos += numLenBytes;

#if defined(GSI_UNICODE)
  // Now strip to Ascii and write the length - subtract 1 to get rid of appended
  // null character
  theReport->mBuffer.mPos +=
      UCS2ToAsciiString(theValue,
                        &theReport->mBuffer.mData[theReport->mBuffer.mPos]) -
      1;
#else
  strncpy(&theReport->mBuffer.mData[theReport->mBuffer.mPos], theValue,
          strlen(theValue));
