...
#define SC_GUID_BINARY_SIZE                                                    \
  16 // convert the 40 byte string guid into an int, 2 shorts and 8 bytes
...
typedef void (*SCCreateSessionCallback)(const SCInterfacePtr theInterface,
                                        ...
                                        SCResult theResult, void *theUserData);
...
typedef void (*SCKeyCallback) (SCReportPtr theReport,
                                                           SCReportKeyType
theKeyType, gsi_u32 theIndex, // how many times this has been called void*
theUserParam);
*/
...
// "matchless" means incoming data will be scrutinized less, and applied to
// stats immediately instead of when the match is over.
...
SCResult SC_CALL scReportSetPlayerData(
    ...
    const GSLoginCertificate *theCertificate, const gsi_u8 theAuthData[16]);

//     - Write team info and key/values
SCResult SC_CALL scReportBeginNewTeam(SCReportPtr theReportData);
SCResult SC_CALL scReportSetTeamData(SCReportPtr theReport, gsi_u32 theTeamId,
...
// This is needed if the report is being submitted to a "matchless" game
// session.
...
SCResult SC_CALL scReportAddInt64Value(SCReportPtr theReportData,
                                       gsi_u16 theKeyId, gsi_i64 theValue);
...
// Encrypt/Decrypt in place, also the RC4 context is modified everytime
// encryption/decryption take place
