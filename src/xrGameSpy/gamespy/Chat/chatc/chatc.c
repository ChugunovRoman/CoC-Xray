...
int WideCaseNCompare(const unsigned short *s1, const unsigned short *s2,
                     size_t count);
...
static void ChangedNickCallback(CHAT chat, CHATBool success,
...
static void PrivateMessage(CHAT chat, const gsi_char *user,
                           const gsi_char *message, int type, void *param) {
...
static void Invited(CHAT chat, const gsi_char *channel, const gsi_char *user,
                    void *param) {
...
static void ChannelMessage(CHAT chat, const gsi_char *channel,
                           const gsi_char *user, const gsi_char *message,
                           int type, void *param) {
  ...
  if (message[0] == '!') {
    message++;
    if (!_tstrcasecmp(message, _T("quit")) ||
        !_tstrcasecmp(message, _T("exit")))
      quit = CHATTrue;
    return;
  }
...
static void Kicked(CHAT chat, const gsi_char *channel, const gsi_char *user,
...
static void UserJoined(CHAT chat, const gsi_char *channel, const gsi_char *user,
                       int mode, void *param) {
...
static void UserParted(CHAT chat, const gsi_char *channel, const gsi_char *user,
                       int why, const gsi_char *reason, const gsi_char *kicker,
                       void *param) {
  if (why == CHAT_LEFT)
    ...
    _tprintf(_T("%s was kicked from %s by %s: %s"), user, channel, kicker,
             reason);
...
static void UserChangedNick(CHAT chat, const gsi_char *channel,
...
static void UserModeChanged(CHAT chat, const gsi_char *channel,
                            const gsi_char *user, int mode, void *param) {
...
static void TopicChanged(CHAT chat, const gsi_char *channel,
...
static void ChannelModeChanged(CHAT chat, const gsi_char *channel,
...
static void ConnectCallback(CHAT chat, CHATBool success, int failureReason,
                            void *param) {
...
static void FillInUserCallback(CHAT chat, unsigned int IP, gsi_char user[128],
                               void *param) {
...
static void NickErrorCallback(CHAT chat, int type, const gsi_char *nick,
                              int numSuggestedNicks,
                              const gsi_char **suggestedNicks, void *param) {
  if (type == CHAT_IN_USE) {
    _tprintf(_T("The nick %s is already being used.\n"), nick);
    _tsnprintf(chatNick, CHAT_NICK_SIZE, _T("ChatC%lu"),
    ...
    chatNick[CHAT_NICK_SIZE - 1] = '\0';
    chatRetryWithNick(chat, chatNick);
  } else if ((type == CHAT_UNIQUENICK_EXPIRED) ||
             (type == CHAT_NO_UNIQUENICK)) {
...
static void EnterChannelCallback(CHAT chat, CHATBool success,
...
static void GetUserInfoCallback(CHAT chat, CHATBool success,
                                const gsi_char *nick, const gsi_char *user,
                                const gsi_char *name, const gsi_char *address,
                                int numChannels, const gsi_char **channels,
                                void *param) {
  int i;
...
/*
static void EnumChannelsAllCallback(CHAT chat, CHATBool success, int
numChannels, const gsi_char ** channel, const gsi_char ** topic, int* numUsers,
void * param)
{
        GSI_UNUSED(chat);
        GSI_UNUSED(success);
...
*/
static void EnumUsersCallback(CHAT chat, CHATBool success,
                              const gsi_char *channel, int numUsers,
                              const gsi_char **users, int *modes, void *param) {
  int i;
...
int test_main(int argc, char **argv) {
  ...
  // Connect.
  ///////////
  chat = chatConnectSecure(serverAddress[0] ? serverAddress : NULL, port,
                           chatNick, chatName, gamename, secretKey,
                           &globalCallbacks, NickErrorCallback,
                           FillInUserCallback, ConnectCallback, NULL, CHATTrue);
  if (!chat) {
  ...
  // Join.
  ////////
  chatEnterChannel(chat, chatChannel, NULL, &channelCallbacks,
                   EnterChannelCallback, NULL, CHATTrue);
