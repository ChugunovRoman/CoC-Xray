...
static const char *ciGetTime(void) {
  ...
  if (now) // fixes the date > 2060 crash (23apr03/bgw)
  {
    if (now->tm_year > 99)
      now->tm_year -= 100;
    sprintf(buffer, "%02d.%02d.%02d %02d:%02d.%02d", now->tm_mon + 1,
            now->tm_mday, now->tm_year, now->tm_hour, now->tm_min, now->tm_sec);
  } else
...
static void ciBufferFree(ciBuffer *buffer) { gsifree(buffer->buffer); }
...
#if !defined(INSOCK) && !defined(_NITRO) && !defined(_REVOLUTION)
  keepalive = 1;
  rcode = setsockopt(sock->sock, SOL_SOCKET, SO_KEEPALIVE, (char *)&keepalive,
                     sizeof(int));
...
static void ciSocketSelect(SOCKET sock, CHATBool *readFlag, CHATBool *writeFlag,
                           CHATBool *exceptFlag) {
  int aReadFlag = 0;
  int aWriteFlag = 0;
  int aExceptFlag = 0;
...
static CHATBool ciAddParam(const char *param, ciServerMessage *message) {
  ...
  //////////////////////////////////
  tempPtr =
      gsirealloc(message->params, sizeof(char *) * (message->numParams + 1));
  if (tempPtr == NULL)
    return CHATFalse; // ERRCON
