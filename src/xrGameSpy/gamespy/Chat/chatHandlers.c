...
int numServerMessageTypes =
    (sizeof(serverMessageTypes) / sizeof(ciServerMessageType));
...
static ciServerMessageFilter *ciFindFilter(CHAT chat, int numMatches,
  ...
  for (filter = connection->filterList; filter != NULL;
       filter = filter->pnext) {
    for (i = 0; i < numMatches; i++) {
      ...
      if (filter->type == matches[i].type) {
        ...
        if ((!name && !filter->name) ||
            (name && filter->name && (strcasecmp(name, filter->name) == 0))) {
          if ((!name2 && !filter->name2) ||
              (name2 && filter->name2 &&
               (strcasecmp(name2, filter->name2) == 0))) {
...
static void ciFinishFilter(CHAT chat, ciServerMessageFilter *filter,
  ...
  if (filter->type == TYPE_LIST) {
    ...
    if (filter->callback2)
      ciAddCallback_(chat, CALLBACK_ENUM_CHANNELS_ALL, filter->callback2,
    ...
    if (filter->callback != NULL)
      ciAddCallback_(chat, CALLBACK_ENTER_CHANNEL, filter->callback, params,
    ...
    if (filter->callback)
      ciAddCallback_(chat, CALLBACK_GET_CHANNEL_TOPIC, filter->callback, params,
                     filter->param, filter->ID, channel,
    ...
    if (filter->callback)
      ciAddCallback_(chat, CALLBACK_GET_USER_INFO, filter->callback, params,
                     filter->param, filter->ID, NULL,
    ...
    if (filter->callback)
      ciAddCallback_(chat, CALLBACK_GET_BASIC_USER_INFO, filter->callback,
                     params, filter->param, filter->ID, NULL,
    ...
    if (filter->callback)
      ciAddCallback_(chat, CALLBACK_GET_CHANNEL_MODE, filter->callback, params,
                     filter->param, filter->ID, NULL,
                     sizeof(ciCallbackGetChannelModeParams));
  } else if (filter->type == TYPE_UMODE) {
    if (filter->callback)
      ciAddCallback_(chat, CALLBACK_GET_USER_MODE, filter->callback, params,
                     filter->param, filter->ID, NULL,
                     sizeof(ciCallbackGetUserModeParams));
  } else if (filter->type == TYPE_GETUDPRELAY) {
    if (filter->callback)
      ciAddCallback_(chat, CALLBACK_GET_UDPRELAY, filter->callback, params,
                     filter->param, filter->ID, NULL,
    ...
    if (filter->callback)
      ciAddCallback_(chat, CALLBACK_ENUM_CHANNEL_BANS, filter->callback, params,
                     filter->param, filter->ID, NULL,
    ...
    if (filter->callback)
      ciAddCallback_(chat, CALLBACK_CHANGE_NICK, filter->callback, params,
                     filter->param, filter->ID, NULL,
    ...
    if (filter->callback)
      ciAddCallback_(chat, CALLBACK_GET_GLOBAL_KEYS, filter->callback, params,
                     filter->param, filter->ID, NULL,
    ...
    if (filter->callback)
      ciAddCallback_(chat, CALLBACK_GET_CHANNEL_KEYS, filter->callback, params,
                     filter->param, filter->ID, NULL,
    ...
    if (filter->callback)
      ciAddCallback_(chat, CALLBACK_NEW_USER_LIST, filter->callback, params,
                     filter->param, filter->ID, NULL,
    ...
    if (filter->callback)
      ciAddCallback_(chat, CALLBACK_AUTHENTICATE_CDKEY, filter->callback,
                     params, filter->param, filter->ID, NULL,
...
void ciCleanupFilters(CHAT chat) {
  ...
  while (connection->filterList)
    ciFilterTimedout(
        chat,
        connection->filterList); // PANTS|03.13.01 - changed from ciRemoveFilter
}

static int ciAddFilter(CHAT chat, int type, const char *name, const char *name2,
                       void *callback, void *callback2, void *param,
                       void *data) {
  ciServerMessageFilter *filter;
  CONNECTION;
...
int ciAddLISTFilter(CHAT chat, chatEnumChannelsCallbackEach callbackEach,
  ...
  return ciAddFilter(chat, TYPE_LIST, NULL, NULL, (void *)callbackEach,
...
int ciAddJOINFilter(CHAT chat, const char *channel,
  ...
  rcode = ciAddFilter(chat, TYPE_JOIN, channel, NULL, (void *)callback, NULL,
                      param, data);
  if (rcode == 0)
    gsifree(data);
...
int ciAddTOPICFilter(CHAT chat, const char *channel,
  ...
  return ciAddFilter(chat, TYPE_TOPIC, channel, NULL, (void *)callback, NULL,
                     param, NULL);
}
...
int ciAddNAMESFilter(CHAT chat, const char *channel,
  ...
  return ciAddFilter(chat, TYPE_NAMES, channel, NULL, (void *)callback, NULL,
                     param, data);
}
...
int ciAddWHOISFilter(CHAT chat, const char *user,
  ...
  rcode = ciAddFilter(chat, TYPE_WHOIS, user, NULL, (void *)callback, NULL,
                      param, data);
  if (rcode == 0)
    gsifree(data);
  return rcode;
...
int ciAddWHOFilter(CHAT chat, const char *user,
  ...
  return ciAddFilter(chat, TYPE_WHO, user, NULL, (void *)callback, NULL, param,
                     NULL);
}

int ciAddCWHOFilter(CHAT chat, const char *channel,
                    chatGetChannelBasicUserInfoCallback callback, void *param) {
  return ciAddFilter(chat, TYPE_CWHO, channel, NULL, (void *)callback, NULL,
                     param, NULL);
}

int ciAddCMODEFilter(CHAT chat, const char *channel,
                     chatGetChannelModeCallback callback, void *param) {
  return ciAddFilter(chat, TYPE_CMODE, channel, NULL, (void *)callback, NULL,
                     param, NULL);
}

int ciAddUMODEFilter(CHAT chat, const char *user, const char *channel,
                     chatGetUserModeCallback callback, void *param) {
  return ciAddFilter(chat, TYPE_UMODE, user, channel, (void *)callback, NULL,
                     param, NULL);
}

int ciAddGETUDPRELAYFilter(CHAT chat, const char *channel,
                           chatGetUdpRelayCallback callback, void *param) {
  return ciAddFilter(chat, TYPE_GETUDPRELAY, channel, NULL, (void *)callback,
                     NULL, param, NULL);
}
...
int ciAddGETBANFilter(CHAT chat, const char *channel,
  ...
  return ciAddFilter(chat, TYPE_GETBAN, channel, NULL, (void *)callback, NULL,
                     param, data);
}

int ciAddNICKFilter(CHAT chat, const char *oldNick, const char *newNick,
                    chatChangeNickCallback callback, void *param) {
  return ciAddFilter(chat, TYPE_NICK, oldNick, newNick, (void *)callback, NULL,
                     param, NULL);
}

int ciAddGETKEYFilter(CHAT chat, const char *cookie, int num, const char **keys,
  ...
  return ciAddFilter(chat, TYPE_GETKEY, cookie, NULL, (void *)callback, NULL,
                     param, data);
}

int ciAddGETCKEYFilter(CHAT chat, const char *cookie, int num,
                       const char **keys, CHATBool channel,
  ...
  return ciAddFilter(chat, TYPE_GETCKEY, cookie, NULL, (void *)callback, NULL,
                     param, data);
}
...
int ciAddGETCHANKEYFilter(CHAT chat, const char *cookie, int num,
  ...
  return ciAddFilter(chat, TYPE_GETCHANKEY, cookie, NULL, (void *)callback,
                     NULL, param, data);
}
...
int ciAddUNQUIETFilter(CHAT chat, const char *channel) {
  ...
  return ciAddFilter(chat, TYPE_UNQUIET, channel, NULL,
                     (void *)callbacks->newUserList, NULL, callbacks->param,
                     data);
}

int ciAddCDKEYFilter(CHAT chat, chatAuthenticateCDKeyCallback callback,
                     void *param) {
  return ciAddFilter(chat, TYPE_CDKEY, NULL, NULL, (void *)callback, NULL,
                     param, NULL);
}
...
void ciPrivmsgHandler(CHAT chat, const ciServerMessage *message) {
  ...
  if (strcasecmp(target, connection->nick) == 0) {
    if (connection->globalCallbacks.privateMessage != NULL) {
      ...
      ciAddCallback(chat, CALLBACK_PRIVATE_MESSAGE,
                    (void *)connection->globalCallbacks.privateMessage, &params,
    ...
    if ((callbacks != NULL) && (callbacks->channelMessage != NULL)) {
      ...
      ciAddCallback(chat, CALLBACK_CHANNEL_MESSAGE,
                    (void *)callbacks->channelMessage, &params,
                    callbacks->param, 0, target);
    }
  }
}
...
void ciJoinHandler(CHAT chat, const ciServerMessage *message) {
  ...
  ////////////////////////////////
  if (ciWasJoinCallbackCalled(
          chat,
          channel)) // PANTS - 03.01.00 - check if the join callback was called
  {
    ...
    if (callbacks != NULL) {
      if (callbacks->userJoined != NULL) {
        ...
        params.user = nick;
        params.mode = mode;
        ciAddCallback(chat, CALLBACK_USER_JOINED, (void *)callbacks->userJoined,
                      &params, callbacks->param, 0, channel);
      }
  ...
  if (callbacks != NULL) {
    ...
    if (strcasecmp(kickee, connection->nick) == 0) {
      ...
      if (callbacks->kicked != NULL) {
        ...
        params.user = kicker;
        params.reason = reason;
        ciAddCallback(chat, CALLBACK_KICKED, (void *)callbacks->kicked, &params,
...
static void ciQuitEnumChannelsCallback(CHAT chat, const char *user,
  ...
  if (ciWasJoinCallbackCalled(chat, channel)) {
    ...
    if (callbacks != NULL) {
      ...
      if (callbacks->userParted != NULL) {
        ...
        params.kicker = NULL;
        ciAddCallback(chat, CALLBACK_USER_PARTED, (void *)callbacks->userParted,
                      &params, callbacks->param, 0, channel);
      }
...
static void ciKillEnumChannelsCallback(CHAT chat, const char *user,
...
void ciTopicHandler(CHAT chat, const ciServerMessage *message) {
  ...
  if ((callbacks != NULL) && (callbacks->topicChanged != NULL)) {
    ...
    params.channel = channel;
    params.topic = topic;
    ciAddCallback(chat, CALLBACK_TOPIC_CHANGED, (void *)callbacks->topicChanged,
                  &params, callbacks->param, 0, channel);
  }
}
...
void ciModeHandler(CHAT chat, const ciServerMessage *message) {
  ...
  for (change = changes; change->mode != MODE_END; change++) {
    ...
    case MODE_OP:
      if (change->param)
        ciUserChangedMode(chat, change->param, channel, CHAT_OP,
                          change->enable);
      break;

    case MODE_VOICE:
      if (change->param)
        ciUserChangedMode(chat, change->param, channel, CHAT_VOICE,
                          change->enable);
      break;
    case MODE_OPS_OBEY_CHANNEL_LIMIT:
      break;
...
void ciInviteHandler(CHAT chat, const ciServerMessage *message) {
  ...
  if (connection->globalCallbacks.invited != NULL) {
    ...
    ciAddCallback(chat, CALLBACK_INVITED,
                  (void *)connection->globalCallbacks.invited, &params,
                  connection->globalCallbacks.param, 0, NULL);
  }
}
  ...
  while (nick != NULL) {
    ...
    if (filter->type != TYPE_JOIN) {
      ...
      if (data->numUsers == data->len) {
        tempPtr = (char **)gsirealloc(
            data->users, sizeof(char *) * (data->len + NAMES_ARRAY_INC));
        if (tempPtr == NULL) {
          assert(0);
          return; // ERRCON
        }
        ...
        tempPtr = (char **)gsirealloc(
            data->modes, sizeof(int) * (data->len + NAMES_ARRAY_INC));
        if (tempPtr == NULL) {
          assert(0);
          return; // ERRCON
        }
  ...
  if (filter != NULL) {
    ...
    if ((callbacks != NULL) && (callbacks->topicChanged != NULL)) {
      ...
      ciAddCallback(chat, CALLBACK_TOPIC_CHANGED,
                    (void *)callbacks->topicChanged, &params, callbacks->param,
                    0, channel);
    }
  }
}
...
void ciRplWhoReplyHandler(CHAT chat, const ciServerMessage *message) {
  ...
  if (filter->type == TYPE_WHO) {
    ...
    ciAddCallback(chat, CALLBACK_GET_BASIC_USER_INFO, filter->callback, &params,
                  filter->param, filter->ID, NULL);

    // We want to wait until we get the end, but we've already called the
    // callback.
    ///////////////////////////////////////////////////////////////////////////////
  ...
  if ((filter->type == TYPE_CWHO) && filter->callback) {
    ...
    ciAddCallback(chat, CALLBACK_GET_CHANNEL_BASIC_USER_INFO, filter->callback,
                  &params, filter->param, filter->ID, NULL);
...
void ciRplGetKeyHandler(CHAT chat, const ciServerMessage *message) {
  ...
  if (filter) {
    ...
    if (!data->channel) {
      ...
      ciAddCallback(chat, CALLBACK_GET_GLOBAL_KEYS, filter->callback, &params,
                    filter->param, filter->ID, NULL);
    }
...
void ciRplGetCKeyHandler(CHAT chat, const ciServerMessage *message) {
  ...
  if (filter) {
    ...
    if (!data->channel) {
      ...
      ciAddCallback(chat, CALLBACK_GET_CHANNEL_KEYS, filter->callback, &params,
                    filter->param, filter->ID, NULL);
    }
      ...
      if (data->allBroadcastKeys) {
        while ((key = ciParseValue(flags, &len)) != NULL) {
          ...
          if (value) {
            ...
            //////////////////////////////////////
            tempPtr =
                (char **)gsirealloc(data->keys, sizeof(char *) * (num + 1));
            if (tempPtr) {
              data->keys = tempPtr;
              tempPtr = (char **)gsirealloc(values, sizeof(char *) * (num + 1));
              if (tempPtr) {
...
void ciRplUserIPHandler(CHAT chat, const ciServerMessage *message) {
  ...
  if (IP) {
    ...
    if (connection->fillInUserCallback) {
      ...
      UCS2ToAsciiString(
          user, connection->user); // convert the unicode user to ascii
                                   // (nicknames cannot currently be UTF-8!!)
#endif
    }
  }
...
void ciRplListHandler(CHAT chat, const ciServerMessage *message) {
  ...
  if (filter != NULL) {
    if (filter->callback != NULL) {
      ...
      ciAddCallback(chat, CALLBACK_ENUM_CHANNELS_EACH, filter->callback,
                    &params, filter->param, filter->ID, NULL);
      ...
      ///////////////////
      tempPtr =
          gsirealloc(data->channels, sizeof(char *) * (data->numChannels + 1));
      if (tempPtr == NULL) {
        gsifree(channel);
        gsifree(topic);
        return; // ERRCON
      ...
      ////////////////////
      tempPtr =
          gsirealloc(data->numUsers, sizeof(int) * (data->numChannels + 1));
      if (tempPtr == NULL) {
        gsifree(channel);
        gsifree(topic);
        return; // ERRCON
      ...
      /////////////////
      tempPtr =
          gsirealloc(data->topics, sizeof(char *) * (data->numChannels + 1));
      if (tempPtr == NULL) {
        gsifree(channel);
        gsifree(topic);
        return; // ERRCON
...
void ciRplChannelModeIsHandler(CHAT chat, const ciServerMessage *message) {
  ...
  if (!ciGetChannelMode(chat, channel, &dummyMode)) {
    ...
    if ((callbacks != NULL) && (callbacks->channelModeChanged != NULL)) {
      ...
      params.channel = channel;
      params.mode = &mode;
      ciAddCallback(chat, CALLBACK_CHANNEL_MODE_CHANGED,
                    (void *)callbacks->channelModeChanged, &params,
                    callbacks->param, 0, channel);
    }
  }
...
void ciRplWhoisUserHandler(CHAT chat, const ciServerMessage *message) {
  ...
  if (filter->type == TYPE_BAN) {
    ...
    // Ban this guy.
    ////////////////
    ciSocketSendf(&connection->chatSocket, "MODE %s +b *!*@%s", data->channel,
                  host);
...
void ciRplWhoisChannelsHandler(CHAT chat, const ciServerMessage *message) {
  ...
  if (filter != NULL) {
    ...
    while (str != NULL) {
      ...
      if (channel == NULL)
        return; // ERRCON
      tempPtr = (char **)gsirealloc(data->channels,
...
void ciRplEndOfWhoisHandler(CHAT chat, const ciServerMessage *message) {
  ...
  if (filter != NULL) {
    WHOISData *data = (WHOISData *)filter->data;
    ciCallbackGetUserInfoParams params;
    params.success = (CHATBool)(
        data->user != NULL); // PANTS|08.21.00 - false if nothing found
    params.nick = nick;
...
void ciRplSecureKeyHandler(CHAT chat, const ciServerMessage *message) {
  ...
  // Take the random keys and the secret key to create the encoding/decoding
  // keys.
  ////////////////////////////////////////////////////////////////////////////////
  ...
  gs_prepare_key((unsigned char *)outKeyRand, outKeyLen,
                 &connection->chatSocket.outKey);
  gs_prepare_key((unsigned char *)inKeyRand, inKeyLen,
                 &connection->chatSocket.inKey);
...
void ciErrLoginFailedHandler(CHAT chat, const ciServerMessage *message) {
  ...
  if (connection->connecting) {
    ...
    ////////////////////////////////////
    if (connection->connectCallback != NULL)
      connection->connectCallback(chat, CHATFalse, CHAT_LOGIN_FAILED,
                                  connection->connectParam);
  }
...
void ciErrRegisterNickFailedHandler(CHAT chat, const ciServerMessage *message) {
  ...
  ////////////////////////////////
  ciNickError(chat, CHAT_INVALID_UNIQUENICK, connection->uniquenick, num,
              nickArray);
