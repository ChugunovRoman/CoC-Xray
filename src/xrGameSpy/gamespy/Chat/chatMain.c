...
** GLOBALS **
************/
// This can be overridden using an extern to pass a different versionID number
// to the chat server as part of the crypt negotiation
int ciVersionID = 1;
...
#define ASSERT_TYPE(type)                                                      \
  assert((type == CHAT_MESSAGE) || (type == CHAT_ACTION) ||                    \
         (type == CHAT_NOTICE) || (type == CHAT_UTM) || (type == CHAT_ATM));
...
** FUNCTIONS **
**************/
static CHATBool ciProcessServerMessage(CHAT chat,
...
void ciHandleDisconnect(CHAT chat, const char *reason) {
  ...
  if (connection->connecting) {
    ...
    if (connection->connectCallback != NULL)
      connection->connectCallback(chat, CHATFalse, CHAT_DISCONNECTED,
  ...
  //////////////////////////////////////
    ...
    ciAddCallback(chat, CALLBACK_DISCONNECTED,
                  (void *)connection->globalCallbacks.disconnected, &params,
  ...
  if (connection->chatSocket.connectState == ciConnected) {
    ...
    while ((message = ciSocketRecv(&connection->chatSocket)) != NULL) {
      ...
      if (connection->globalCallbacks.raw != NULL) {
        ...
        ciAddCallback(chat, CALLBACK_RAW,
                      (void *)connection->globalCallbacks.raw, &params,
                      connection->globalCallbacks.param, 0, NULL);
      }
...
void ciSendUser(CHAT chat) {
  ...
  // Send the user.
  /////////////////
  ciSocketSendf(&connection->chatSocket, "USER %s %s %s :%s", connection->user,
                "127.0.0.1", connection->server, connection->name);
}
...
void ciSendLogin(CHAT chat) {
  ...
  if (connection->loginType == CIPreAuthLogin) {
    ciSocketSendf(&connection->chatSocket, "LOGINPREAUTH %s %s",
                  connection->authtoken, connection->partnerchallenge);
  ...
  /////////////////////////////////////////////////////////////////
  MD5Digest((unsigned char *)connection->password, strlen(connection->password),
            passwordHash);
  ...
  if (connection->loginType == CIUniqueNickLogin) {
    ciSocketSendf(&connection->chatSocket, "LOGIN %d %s %s",
                  connection->namespaceID, connection->uniquenick,
                  passwordHash);
  } else if (connection->loginType == CIProfileLogin) {
    ciSocketSendf(&connection->chatSocket, "LOGIN %d * %s :%s@%s",
                  connection->namespaceID, passwordHash,
                  connection->profilenick, connection->email);
  } else {
    // If we get here, the login type is invalid or isn't being handled
    // properly.
    /////////////////////////////////////////////////////////////////////////////
    assert(0);
  }
...
static CHAT chatConnectDoit(
    CILoginType loginType, const char *serverAddress, int port,
    const char *nick, const char *user, const char *name, int namespaceID,
    ...
    const char *gamename, const char *secretKey, chatGlobalCallbacks *callbacks,
    chatNickErrorCallback nickErrorCallback,
    chatFillInUserCallback fillInUserCallback,
    chatConnectCallback connectCallback, void *param, CHATBool blocking) {
  ...
  if (partnerchallenge)
    strzcpy(connection->partnerchallenge, partnerchallenge,
            MAX_PARTNERCHALLENGE);
  strzcpy(connection->server,
          serverAddress ? serverAddress : CI_DEFAULT_SERVER_ADDRESS,
          MAX_SERVER);
  ...
  // Connect the socket.
  //////////////////////
  if (!ciSocketConnect(&connection->chatSocket, connection->server,
                       connection->port)) {
  ...
  if (gamename && gamename[0] && secretKey && secretKey[0]) {
    ...
    // Get the random keys.
    ///////////////////////
    ciSocketSendf(&connection->chatSocket, "CRYPT des %d %s", ciVersionID,
                  gamename);
...
CHAT chatConnectA(const char *serverAddress, int port, const char *nick,
                  const char *user, const char *name,
                  chatGlobalCallbacks *callbacks,
                  chatNickErrorCallback nickErrorCallback,
                  chatConnectCallback connectCallback, void *param,
                  CHATBool blocking) {
  return chatConnectDoit(CINoLogin, serverAddress, port, nick, user, name, 0,
                         NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                         callbacks, nickErrorCallback, NULL, connectCallback,
                         param, blocking);
}
#ifdef GSI_UNICODE
CHAT chatConnectW(const unsigned short *serverAddress, int port,
                  const unsigned short *nick, const unsigned short *user,
                  const unsigned short *name, chatGlobalCallbacks *callbacks,
                  chatNickErrorCallback nickErrorCallback,
                  chatConnectCallback connectCallback, void *param,
                  CHATBool blocking) {
  ...
  CHAT aChat =
      chatConnectA(serverAddress_A, port, nick_A, user_A, name_A, callbacks,
                   nickErrorCallback, connectCallback, param, blocking);
...
CHAT chatConnectSpecialA(const char *serverAddress, int port, const char *nick,
                         const char *name, chatGlobalCallbacks *callbacks,
                         chatNickErrorCallback nickErrorCallback,
                         chatFillInUserCallback fillInUserCallback,
                         chatConnectCallback connectCallback, void *param,
                         CHATBool blocking) {
  return chatConnectDoit(CINoLogin, serverAddress, port, nick, NULL, name, 0,
                         NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                         callbacks, nickErrorCallback, fillInUserCallback,
                         connectCallback, param, blocking);
}
#ifdef GSI_UNICODE
CHAT chatConnectSpecialW(const unsigned short *serverAddress, int port,
                         const unsigned short *nick, const unsigned short *name,
                         chatGlobalCallbacks *callbacks,
                         chatNickErrorCallback nickErrorCallback,
                         chatFillInUserCallback fillInUserCallback,
                         chatConnectCallback connectCallback, void *param,
  ...
  char *nick_A = (char *)UCS2ToUTF8StringAlloc(nick);
  char *name_A = (char *)UCS2ToUTF8StringAlloc(name);
  CHAT aChat = chatConnectSpecialA(
      serverAddress_A, port, nick_A, name_A, callbacks, nickErrorCallback,
      fillInUserCallback, connectCallback, param, blocking);
  gsifree(serverAddress_A);
  gsifree(nick_A);
  gsifree(name_A);
...
CHAT chatConnectSecureA(const char *serverAddress, int port, const char *nick,
                        const char *name, const char *gamename,
                        const char *secretKey, chatGlobalCallbacks *callbacks,
                        chatNickErrorCallback nickErrorCallback,
                        chatFillInUserCallback fillInUserCallback,
                        chatConnectCallback connectCallback, void *param,
                        CHATBool blocking) {
  return chatConnectDoit(CINoLogin, serverAddress, port, nick, NULL, name, 0,
                         NULL, NULL, NULL, NULL, NULL, NULL, gamename,
                         secretKey, callbacks, nickErrorCallback,
                         fillInUserCallback, connectCallback, param, blocking);
}
#ifdef GSI_UNICODE
CHAT chatConnectSecureW(const unsigned short *serverAddress, int port,
                        const unsigned short *nick, const unsigned short *name,
  ...
  CHAT aChat = chatConnectSecureA(
      serverAddress_A, port, nick_A, name_A, gamename_A, secretKey_A, callbacks,
      nickErrorCallback, fillInUserCallback, connectCallback, param, blocking);
  gsifree(serverAddress_A);
  gsifree(nick_A);
  gsifree(name_A);
  gsifree(gamename_A);
...
CHAT chatConnectLoginA(const char *serverAddress, int port, int namespaceID,
                       const char *email, const char *profilenick,
                       const char *uniquenick, const char *password,
                       const char *name, const char *gamename,
                       const char *secretKey, chatGlobalCallbacks *callbacks,
                       chatNickErrorCallback nickErrorCallback,
                       chatFillInUserCallback fillInUserCallback,
                       chatConnectCallback connectCallback, void *param,
                       CHATBool blocking) {
  return chatConnectDoit(
      (uniquenick && uniquenick[0]) ? CIUniqueNickLogin : CIProfileLogin,
      serverAddress, port, NULL, NULL, name, namespaceID, email, profilenick,
      uniquenick, password, NULL, NULL, gamename, secretKey, callbacks,
      nickErrorCallback, fillInUserCallback, connectCallback, param, blocking);
}
#ifdef GSI_UNICODE
CHAT chatConnectLoginW(
    const unsigned short *serverAddress, int port, int namespaceID,
    const unsigned short *email, const unsigned short *profilenick,
    const unsigned short *uniquenick, const unsigned short *password,
    const unsigned short *name, const unsigned short *gamename,
    const unsigned short *secretKey, chatGlobalCallbacks *callbacks,
    chatNickErrorCallback nickErrorCallback,
    chatFillInUserCallback fillInUserCallback,
    chatConnectCallback connectCallback, void *param, CHATBool blocking) {
  char *serverAddress_A = (char *)UCS2ToUTF8StringAlloc(serverAddress);
  ...
  char *gamename_A = (char *)UCS2ToUTF8StringAlloc(gamename);
  char *secretKey_A = (char *)UCS2ToUTF8StringAlloc(secretKey);
  CHAT aChat = chatConnectLoginA(
      serverAddress_A, port, namespaceID, email_A, profilenick_A, uniquenick_A,
      password_A, name_A, gamename_A, secretKey_A, callbacks, nickErrorCallback,
      fillInUserCallback, connectCallback, param, blocking);
  gsifree(serverAddress_A);
  gsifree(email_A);
...
CHAT chatConnectPreAuthA(const char *serverAddress, int port,
                         ...
                         chatConnectCallback connectCallback, void *param,
                         CHATBool blocking) {
  return chatConnectDoit(CIPreAuthLogin, serverAddress, port, NULL, NULL, name,
                         0, NULL, NULL, NULL, NULL, authtoken, partnerchallenge,
                         gamename, secretKey, callbacks, nickErrorCallback,
                         fillInUserCallback, connectCallback, param, blocking);
}
#ifdef GSI_UNICODE
CHAT chatConnectPreAuthW(
    const unsigned short *serverAddress, int port,
    const unsigned short *authtoken, const unsigned short *partnerchallenge,
    const unsigned short *name, const unsigned short *gamename,
    const unsigned short *secretKey, chatGlobalCallbacks *callbacks,
    chatNickErrorCallback nickErrorCallback,
    chatFillInUserCallback fillInUserCallback,
  ...
  char *gamename_A = (char *)UCS2ToUTF8StringAlloc(gamename);
  char *secretKey_A = (char *)UCS2ToUTF8StringAlloc(secretKey);
  CHAT aChat = chatConnectPreAuthA(
      serverAddress_A, port, authtoken_A, partnerchallenge_A, name_A,
      gamename_A, secretKey_A, callbacks, nickErrorCallback, fillInUserCallback,
      connectCallback, param, blocking);
  gsifree(serverAddress_A);
  gsifree(authtoken_A);
...
void chatRetryWithNickA(CHAT chat, const char *nick) {
  ...
  if (nick == NULL) {
    ...
    /////////////////////
    if (connection->connectCallback != NULL)
      connection->connectCallback(chat, CHATFalse, CHAT_NICK_ERROR,
                                  connection->connectParam);
...
void chatRegisterUniqueNickA(CHAT chat, int namespaceID, const char *uniquenick,
                             const char *cdkey) {
  CONNECTION;
  ...
  // Send the message.
  ////////////////////
  ciSocketSendf(&connection->chatSocket, "REGISTERNICK %d %s %s", namespaceID,
                uniquenick, cdkey);
...
#ifdef GSI_UNICODE
void chatRegisterUniqueNickW(CHAT chat, int namespaceID,
...
#ifdef GSI_UNICODE
    connection->globalCallbacks.disconnected(chat, L"",
                                             connection->globalCallbacks.param);
#else
    connection->globalCallbacks.disconnected(chat, "",
                                             connection->globalCallbacks.param);
#endif
...
void chatThink(CHAT chat) { ciThink(chat, 0); }
...
void chatChangeNickA(CHAT chat, const char *newNick,
  ...
  if (!success) {
    if (callback) {
      ...
      ID = ciGetNextID(chat);
      ciAddCallback(chat, CALLBACK_CHANGE_NICK, (void *)callback, &params,
                    param, ID, NULL);
...
#ifdef GSI_UNICODE
void chatChangeNickW(CHAT chat, const unsigned short *newNick,
                     chatChangeNickCallback callback, void *param,
                     CHATBool blocking) {
...
static void ciSetQuietModeEnumJoinedChannelsA(CHAT chat, int index,
                                              const char *channel,
                                              void *param) {
  // Setup a filter.
  //////////////////
...
#ifdef GSI_UNICODE
static void ciSetQuietModeEnumJoinedChannelsW(CHAT chat, int index,
...
void chatAuthenticateCDKeyA(CHAT chat, const char *cdkey,
                            chatAuthenticateCDKeyCallback callback, void *param,
                            CHATBool blocking) {
  int ID;
  CHATBool success = CHATTrue;
  ...
  if (!success) {
    if (callback) {
      ...
      params.message = "";
      ID = ciGetNextID(chat);
      ciAddCallback(chat, CALLBACK_AUTHENTICATE_CDKEY, (void *)callback,
...
#ifdef GSI_UNICODE
void chatAuthenticateCDKeyW(CHAT chat, const unsigned short *cdkey,
                            chatAuthenticateCDKeyCallback callback, void *param,
                            CHATBool blocking) {
  char *cdkey_A = (char *)UCS2ToUTF8StringAlloc(cdkey);
...
** CHANNELS **
*************/
void chatEnumChannelsA(CHAT chat, const char *filter,
...
#ifdef GSI_UNICODE
void chatEnumChannelsW(CHAT chat, const unsigned short *filter,
                       chatEnumChannelsCallbackEach callbackEach,
                       chatEnumChannelsCallbackAll callbackAll, void *param,
                       CHATBool blocking) {
...
void chatEnterChannelW(CHAT chat, const unsigned short *channel,
  ...
  char *channel_A = (char *)UCS2ToUTF8StringAlloc(channel);
  char *password_A = (char *)UCS2ToUTF8StringAlloc(password);
  chatEnterChannelA(chat, channel_A, password_A, callbacks, callback, param,
                    blocking);
  gsifree(channel_A);
  gsifree(password_A);
}
...
void chatSendChannelMessageA(CHAT chat, const char *channel,
  ...
  if (type == CHAT_MESSAGE)
    ...
    ciSocketSendf(&connection->chatSocket, "PRIVMSG %s :\001ACTION %s\001",
                  channel, message);
  ...
  if (callbacks != NULL) {
    ...
    params.type = type;
    ciAddCallback(chat, CALLBACK_CHANNEL_MESSAGE,
                  (void *)callbacks->channelMessage, &params, callbacks->param,
                  0, channel);
  }
}
#ifdef GSI_UNICODE
void chatSendChannelMessageW(CHAT chat, const unsigned short *channel,
                             const unsigned short *message, int type) {
  char *channel_A = (char *)UCS2ToUTF8StringAlloc(channel);
...
void chatGetChannelTopicA(CHAT chat, const char *channel,
  ...
  if (topic) {
    ...
    ciAddCallback(chat, CALLBACK_GET_CHANNEL_TOPIC, (void *)callback, &params,
                  param, ID, channel);
  } else {
...
void chatGetChannelModeA(CHAT chat, const char *channel,
  ...
  if (ciInChannel(chat, channel)) {
    ...
    if (ciGetChannelMode(chat, channel, &mode)) {
      ...
      params.channel = (char *)channel;
      params.mode = &mode;
      ciAddCallback(chat, CALLBACK_GET_CHANNEL_MODE, (void *)callback, &params,
                    param, ID, NULL);
...
void chatSetChannelPasswordW(CHAT chat, const unsigned short *channel,
                             CHATBool enable, const unsigned short *password) {
  char *channel_A = (char *)UCS2ToUTF8StringAlloc(channel);
  char *password_A = (char *)UCS2ToUTF8StringAlloc(password);
...
void chatGetChannelPasswordA(CHAT chat, const char *channel,
  ...
  ciAddCallback(chat, CALLBACK_GET_CHANNEL_PASSWORD, (void *)callback, &params,
                param, ID, NULL);
...
void chatEnumChannelBansA(CHAT chat, const char *channel,
...
static void ciEnumUsersCallback(CHAT chat, const char *channel, int numUsers,
...
#ifdef GSI_UNICODE
  {
    ...
    data->callback(chat, CHATTrue, channel_W, numUsers,
                   (const unsigned short **)users_W, modes, data->param);
    gsifree(channel_W);
    while (numUsers-- > 0)
...
void chatEnumUsersA(CHAT chat, const char *channel,
...
//////////////////////////////////////////////////////
void chatEnumJoinedChannels(CHAT chat, chatEnumJoinedChannelsCallback callback,
                            void *param) {
  ciEnumJoinedChannels(chat, callback, param);
}

void chatSendUserMessageA(CHAT chat, const char *user, const char *message,
                          int type) {
  CONNECTION;
  CONNECTED;
...
#ifdef GSI_UNICODE
void chatSendUserMessageW(CHAT chat, const unsigned short *user,
                          const unsigned short *message, int type) {
  char *user_A = (char *)UCS2ToUTF8StringAlloc(user);
  char *message_A = (char *)UCS2ToUTF8StringAlloc(message);
...
void chatGetUserInfoA(CHAT chat, const char *user,
...
#ifdef GSI_UNICODE
void chatGetUserInfoW(CHAT chat, const unsigned short *user,
                      chatGetUserInfoCallback callback, void *param,
                      CHATBool blocking) {
  char *user_A = (char *)UCS2ToUTF8StringAlloc(user);
...
void chatGetBasicUserInfoA(CHAT chat, const char *nick,
  ...
  if (ciGetUserBasicInfoA(chat, nick, &user, &address)) {
    ...
    ciAddCallback(chat, CALLBACK_GET_BASIC_USER_INFO, (void *)callback, &params,
                  param, ID, NULL);
  } else {
...
#ifdef GSI_UNICODE
void chatGetBasicUserInfoW(CHAT chat, const unsigned short *nick,
                           chatGetBasicUserInfoCallback callback, void *param,
                           CHATBool blocking) {
  char *nick_A = (char *)UCS2ToUTF8StringAlloc(nick);
...
CHATBool chatGetBasicUserInfoNoWaitA(CHAT chat, const char *nick,
...
#ifdef GSI_UNICODE
CHATBool chatGetBasicUserInfoNoWaitW(CHAT chat, const unsigned short *nick,
...
void chatKickUserA(CHAT chat, const char *channel, const char *user,
                   const char *reason) {
  CONNECTION;
  CONNECTED;
  ...
  ciSocketSendf(&connection->chatSocket, "KICK %s %s :%s", channel, user,
                reason);
}
#ifdef GSI_UNICODE
void chatKickUserW(CHAT chat, const unsigned short *channel,
                   const unsigned short *user, const unsigned short *reason) {
  char *channel_A = (char *)UCS2ToUTF8StringAlloc(channel);
  char *user_A = (char *)UCS2ToUTF8StringAlloc(user);
  char *reason_A = (char *)UCS2ToUTF8StringAlloc(reason);
...
void chatSetUserModeA(CHAT chat, const char *channel, const char *user,
                      int mode) {
  int sign;
...
void chatSetUserModeW(CHAT chat, const unsigned short *channel,
                      const unsigned short *user, int mode) {
  char *channel_A = (char *)UCS2ToUTF8StringAlloc(channel);
  char *user_A = (char *)UCS2ToUTF8StringAlloc(user);
...
void chatGetUserModeA(CHAT chat, const char *channel, const char *user,
  ...
  if (mode != -1) {
    ...
    ID = ciGetNextID(chat);
    ciAddCallback(chat, CALLBACK_GET_USER_MODE, (void *)callback, &params,
                  param, ID, NULL);
...
CHATBool chatGetUserModeNoWaitA(CHAT chat, const char *channel,
...
void chatGetUdpRelayA(CHAT chat, const char *channel,
                      chatGetUdpRelayCallback callback, void *param,
                      CHATBool blocking) {
  int ID;
  CONNECTION;
  CONNECTED;
...
** KEYS **
*********/
void chatSetGlobalKeysA(CHAT chat, int num, const char **keys,
                        const char **values) {
  char buffer[512];
  const char *key;
  const char *value;
...
#ifdef GSI_UNICODE
void chatSetGlobalKeysW(CHAT chat, int num, const unsigned short **keys,
...
static void ciSendGetKey(CHAT chat, const char *target, const char *cookie,
                         int num, const char **keys) {
  char buffer[512];
  int len;
  int i;
  int j;
...
void chatGetGlobalKeysA(CHAT chat, const char *target, int num,
                        const char **keys, chatGetGlobalKeysCallback callback,
                        void *param, CHATBool blocking) {
  char *cookie;
  const char *channel;
  int ID;
...
#ifdef GSI_UNICODE
void chatGetGlobalKeysW(CHAT chat, const unsigned short *target, int num,
  ...
  chatGetGlobalKeysA(chat, target_A, num, (const char **)keys_A, callback,
                     param, blocking);
  gsifree(target_A);
...
void chatSetChannelKeysA(CHAT chat, const char *channel, const char *user,
...
#ifdef GSI_UNICODE
void chatSetChannelKeysW(CHAT chat, const unsigned short *channel,
                         const unsigned short *user, int num,
                         const unsigned short **keys,
                         const unsigned short **values) {
  char *channel_A;
  char *user_A;
  ...
  chatSetChannelKeysA(chat, channel_A, user_A, num, (const char **)keys_A,
                      (const char **)values_A);
...
static CHATBool ciSendGetChannelKey(CHAT chat, const char *channel,
                                    const char *nick, const char *cookie,
                                    int num, const char **keys) {
  char buffer[512];
  int len;
  int i;
...
void chatGetChannelKeysA(CHAT chat, const char *channel, const char *user,
  ...
  ////////////////////
  getBroadcastKeys =
      ciSendGetChannelKey(chat, channel, user, cookie, num, keys);

  if (!user || !user[0])
    ID = ciAddGETCHANKEYFilter(chat, cookie, num, keys, getBroadcastKeys,
                               callback, param);
  else
    ID = ciAddGETCKEYFilter(chat, cookie, num, keys,
                            (CHATBool)(strcmp(user, "*") == 0),
...
void chatGetChannelKeysW(CHAT chat, const unsigned short *channel,
  ...
  chatGetChannelKeysA(chat, channel_A, user_A, num, (const char **)keys_A,
                      callback, param, blocking);
...
** NICK ERRORS **
****************/
void ciNickError(CHAT chat, int type, const char *nick, int numSuggestedNicks,
                 char **suggestedNicks) {
  CONNECTION;
  ...
  if (connection->nickErrorCallback) {
    ...
    ciAddCallback(chat, CALLBACK_NICK_ERROR,
                  (void *)connection->nickErrorCallback, &params,
