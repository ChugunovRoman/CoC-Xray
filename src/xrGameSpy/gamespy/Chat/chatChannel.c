...
static ciChatChannel *ciGetChannel(ciConnection *connection,
  ...
  chatChannel =
      (ciChatChannel *)TableLookup(connection->channelTable, &channelTemp);
...
static int GS_STATIC_CALLBACK ciChannelTableCompareFn(const void *elem1,
                                                      const void *elem2) {
  const char *str1;
  const char *str2;
...
static int GS_STATIC_CALLBACK ciUserTableCompareFn(const void *elem1,
                                                   const void *elem2) {
  const char *str1;
  const char *str2;
...
CHATBool ciInitChannels(ciConnection *connection) {
  connection->channelTable = TableNew2(
      sizeof(ciChatChannel), CHANNEL_HASH_BUCKETS, 2, ciChannelTableHashFn,
      ciChannelTableCompareFn, ciChannelTableElementFreeFn);
...
void ciChannelEntered(CHAT chat, const char *channel,
  ...
  strzcpy(chatChannel.name, channel, MAX_CHANNEL);
  chatChannel.users =
      TableNew2(sizeof(ciChatUser), USER_HASH_BUCKETS, 2, ciUserTableHashFn,
                ciUserTableCompareFn, ciUserTableElementFreeFn);
...
void ciChannelListUsers(CHAT chat, const char *channel,
                        ciChannelListUsersCallback callback, void *param) {
  ciChatChannel *chatChannel;
  ciChannelListUsersData data;
  CONNECTION;
  ...
  // Call the callback.
  /////////////////////
  callback(chat, channel, data.numUsers, (const char **)data.users, data.modes,
           param);
...
** USER FUNCTIONS **
*******************/
void ciUserEnteredChannel(CHAT chat, const char *name, const char *channel,
                          int mode, const char *user, const char *address) {
  ciChatUser chatUser;
  ciChatChannel *chatChannel;
  CONNECTION;
...
void ciUserEnumChannels(CHAT chat, const char *user,
                        ciUserEnumChannelsCallback callback, void *param) {
  ciChatUser chatUser;
  ciUserEnumChannelsData data;
  CONNECTION;
...
static void ciUserChangeNickMap(void *elem, void *clientData) {
  ...
  if (user != NULL) {
    ...
    if (ciWasJoinCallbackCalled(data->chat,
                                channel->name)) // PANTS - 03.01.00 - check if
                                                // the join callback was called
    {
      // Add the callback.
      ////////////////////
      if (channel->callbacks.userChangedNick != NULL) {
        ...
        ciAddCallback(data->chat, CALLBACK_USER_CHANGED_NICK,
                      (void *)channel->callbacks.userChangedNick, &params,
                      channel->callbacks.param, 0, channel->name);
      }
    }
  }
  GSI_UNUSED(rcode);
...
void ciUserChangedMode(CHAT chat, const char *user, const char *channel,
  ...
  if (chatChannel->callbacks.userModeChanged != NULL) {
    ...
    ciAddCallback(chat, CALLBACK_USER_MODE_CHANGED,
                  (void *)chatChannel->callbacks.userModeChanged, &params,
                  chatChannel->callbacks.param, 0, channel);
  }
}
...
//////////////////////////////////////////////////////
void ciEnumJoinedChannels(CHAT chat, chatEnumJoinedChannelsCallback callback,
                          void *param) {
  ciEnumJoinedChannelsData data;
  CONNECTION;
  CONNECTED;
...
CHATBool ciGetUserBasicInfoA(CHAT chat, const char *nick, const char **user,
                             const char **address) {
  ciChatUser chatUser;
  ciGetUserBasicInfoData data;
  CONNECTION;
...
CHATBool ciGetUserBasicInfoW(CHAT chat, const char *nick,
...
static void ciClearAllUsersChannelMap(void *elem, void *clientData) {
  ...
  if (channel->callbacks.userListUpdated != NULL) {
    ...
    ciAddCallback(chat, CALLBACK_USER_LIST_UPDATED,
                  (void *)channel->callbacks.userListUpdated, &params,
                  channel->callbacks.param, 0, channel->name);
  }
}
