...
#define DOC                                                                    \
  ...
  ASSERT(pDoc != NULL);                                                        \
  if (!pDoc->m_inChannel) {                                                    \
    OutputDebugString("Not in channel anymore!\n");                            \
    return;                                                                    \
  }
#define ADD(s)                                                                 \
  *str += " | ";                                                               \
...
#define ADD_INT(i)                                                             \
  ...
  *str += "--";                                                                \
  itoa(i, __i__, 10);                                                          \
  *str += __i__;
void ChannelMessage(CHAT chat, const char *channel, const char *user,
                    const char *message, int type, void *param) {
...
void Kicked(CHAT chat, const char *channel, const char *user,
...
void UserJoined(CHAT chat, const char *channel, const char *user, int mode,
                void *param) {
...
void UserParted(CHAT chat, const char *channel, const char *user, int why,
                const char *reason, const char *kicker, void *param) {
...
void UserChangedNick(CHAT chat, const char *channel, const char *oldNick,
...
void UserModeChanged(CHAT chat, const char *channel, const char *user, int mode,
                     void *param) {
...
void TopicChanged(CHAT chat, const char *channel, const char *topic,
                  void *param) {
...
void ChannelModeChanged(CHAT chat, const char *channel, CHATChannelMode *mode,
                        void *param) {
...
void EnumUsersCallback(CHAT chat, CHATBool success, const char *channel,
                       int numUsers, const char **users, int *modes,
                       void *param) {
...
void EnterChannelCallback(CHAT chat, CHATBool success, CHATEnterResult result,
  ...
  if (success) {
    ...
    sprintf(buffer, "%dms to enum %d users in %s\n", after - before,
            pDoc->m_numUsers, pDoc->m_channelName);
...
BOOL CChattyDoc::OnNewDocument() {
  ...
  chatEnterChannel(theApp.m_chat, m_channelName, dlg.m_password, &callbacks,
                   EnterChannelCallback, this, CHATTrue);
...
#ifdef _DEBUG
void CChattyDoc::AssertValid() const { CDocument::AssertValid(); }

void CChattyDoc::Dump(CDumpContext &dc) const { CDocument::Dump(dc); }
#endif //_DEBUG
...
void GetChannelModeCallback(CHAT chat, CHATBool success, const char *channel,
...
void CChattyDoc::OnChannelMode() {
  if (theApp.m_chat != NULL) {
    ...
    ////////////////////////
    chatGetChannelMode(theApp.m_chat, m_channelName, GetChannelModeCallback,
                       &mode, CHATTrue);
...
void GetChannelPasswordCallback(CHAT chat, CHATBool success,
                                const char *channel, CHATBool enabled,
...
void CChattyDoc::OnChannelPassword() {
  if (theApp.m_chat != NULL) {
    CSetPasswordDlg dlg;
    chatGetChannelPassword(theApp.m_chat, m_channelName,
                           GetChannelPasswordCallback, &dlg, CHATTrue);
    if (dlg.DoModal() == IDOK) {
      chatSetChannelPassword(theApp.m_chat, m_channelName,
                             (CHATBool)dlg.m_enable, dlg.m_password);
    }
  }
}

void EnumChannelBansCallback(CHAT chat, CHATBool success, const char *channel,
                             int numBans, const char **bans, void *param) {
  DOC;
  int i;
...
void CChattyDoc::OnChannelGetbanlist() {
  if (theApp.m_chat != NULL) {
    chatEnumChannelBans(theApp.m_chat, m_channelName, EnumChannelBansCallback,
                        this, CHATFalse);
  }
}
