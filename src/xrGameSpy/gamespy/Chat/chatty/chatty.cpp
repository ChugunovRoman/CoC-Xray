...
BOOL CChattyApp::InitInstance() {
  ...
  shared DLL #else Enable3dControlsStatic();	// Call this when linking to MFC
  statically #endif
  */
  ...
  pDocTemplate = new CMultiDocTemplate(
      IDR_CHATTYTYPE, RUNTIME_CLASS(CChattyDoc),
...
void PrivateMessage(CHAT chat, const char *user, const char *message, int type,
                    void *param) {
...
void NickErrorCallback(CHAT chat, int type, const char *nick,
...
void ConnectCallback(CHAT chat, CHATBool success, int failureReason,
                     void *param) {
...
void CChattyApp::OnFileConnect() {
  if (m_chat == NULL) {
    ...
    if (dlg.DoModal() == IDOK) {
      ...
      m_chat = chatConnect(dlg.m_address, dlg.m_port, dlg.m_nick, "pants",
                           "pants", &callbacks, NickErrorCallback,
                           ConnectCallback, this, CHATTrue);
