...
CChattyView::CChattyView() : CFormView(CChattyView::IDD) {
...
#ifdef _DEBUG
void CChattyView::AssertValid() const { CFormView::AssertValid(); }

void CChattyView::Dump(CDumpContext &dc) const { CFormView::Dump(dc); }
...
void CChattyView::OnUserKick() {
  if (theApp.m_chat != NULL) {
    ...
    if (index != LB_ERR) {
      ...
      CKickReasonDlg dlg;
      if (dlg.DoModal() == IDOK) {
        chatKickUser(theApp.m_chat, GetDocument()->m_channelName, user,
                     dlg.m_reason);
      }
    }
  }
...
void CChattyView::OnUserModeOp() {
  if (theApp.m_chat != NULL) {
    ...
    if (index != LB_ERR) {
      ...
      chatSetUserMode(theApp.m_chat, GetDocument()->m_channelName, user,
                      CHAT_OP);
    }
  }
}
...
void CChattyView::OnSendbutt() {
  ...
  if (theApp.m_chat != NULL) {
    UpdateData();
    chatSendChannelMessage(theApp.m_chat, GetDocument()->m_channelName,
                           (LPCSTR)m_edit, CHAT_MESSAGE);
    m_edit.Empty();
    UpdateData(FALSE);
  }
}
