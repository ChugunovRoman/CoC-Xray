...
unsigned char NNMagicData[] = {NN_MAGIC_0, NN_MAGIC_1, NN_MAGIC_2,
...
static void SendPacket(SOCKET sock, unsigned int toaddr, unsigned short toport,
                       void *data, int len) {
...
static unsigned int GetLocalIP() {
  ...
  return localip; // else a specific private address wasn't found - return what
                  // we've got
}
...
static void SendReportPacket(NATNegotiator neg) {
  ...
  p.Packet.Report.negResult =
      (unsigned char)(neg->result == nr_success ? gsi_true : gsi_false);
  p.Packet.Report.natType = natType;
  p.Packet.Report.natMappingScheme = natMappingScheme;

  if (strlen(__GSIACGamename) > 0)
    memcpy(&p.Packet.Report.gamename, __GSIACGamename,
           sizeof(p.Packet.Report.gamename));
  ...
  SendPacket(neg->negotiateSock, matchup1ip, MATCHUP_PORT1, &p,
             REPORTPACKET_SIZE);
}

static void StartReport(NATNegotiator neg, NegotiateResult result,
                        SOCKET socket, struct sockaddr_in *remoteaddr) {
  neg->result = result;
  ...
  if (result == nr_inittimeout || result == nr_deadbeatpartner) {
    ...
    neg->completedCallback(neg->result, neg->connectedSocket,
                           (struct sockaddr_in *)&neg->remoteaddr,
                           neg->userdata);

    // Close the socket here - no need to keep it open, connected Socket is
    // INVALID
...
static void SendInitPackets(NATNegotiator neg) {
  ...
  p->Packet.Init.usegameport =
      (unsigned char)((neg->gameSock == INVALID_SOCKET) ? 0 : 1);
  ...
  if (p->Packet.Init.usegameport && !neg->initAckRecv[NN_PT_GP]) {
    ...
    gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network,
  ...
  if (!neg->initAckRecv[NN_PT_NN1]) {
    ...
    gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network,
                  GSIDebugLevel_Notice, "Sending INIT (NN1) to %s:%d...\n",
                  inet_ntoa(*(struct in_addr *)&matchup1ip), MATCHUP_PORT1);
  ...
  localport = ntohs(GetLocalPort(
      (p->Packet.Init.usegameport) ? neg->gameSock : neg->negotiateSock));
  ...
  if (!neg->initAckRecv[NN_PT_NN2]) {
    ...
    gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network,
                  GSIDebugLevel_Notice, "Sending INIT (NN2) to %s:%d...\n",
  ...
  if (!neg->initAckRecv[NN_PT_NN3]) {
    ...
    gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network,
                  GSIDebugLevel_Notice, "Sending INIT (NN3) to %s:%d...\n",
...
static void SendPingPacket(NATNegotiator neg) {
  ...
  p.Packet.Connect.finished =
      (unsigned char)((neg->state == ns_connectping) ? 0 : 1);

  //////////////
  // playing with a way to re-sync with the NAT's port mappings in the case the
  // guess is off:
  ...
  gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice,
                ...
                neg->gotRemoteData);
  SendPacket((neg->gameSock != INVALID_SOCKET) ? neg->gameSock
                                               : neg->negotiateSock,
             neg->guessedIP, neg->guessedPort, &p, CONNECTPACKET_SIZE);
...
NegotiateError NNBeginNegotiation(int cookie, int clientindex,
                                  ...
                                  NegotiateCompletedFunc completedcallback,
                                  void *userdata) {
  return NNBeginNegotiationWithSocket(INVALID_SOCKET, cookie, clientindex,
...
static unsigned int ResolveServer(const char *overrideHostname,
                                  const char *defaultHostname) {
  const char *hostname;
  char hostnameBuffer[64];

  if (overrideHostname == NULL) {
    snprintf(hostnameBuffer, sizeof(hostnameBuffer), "%s.%s", __GSIACGamename,
             defaultHostname);
...
NegotiateError NNStartNatDetection(NatDetectionResultsFunc resultscallback) {
  ...
  DiscoverMapping(mappingSock, matchup1ip, MATCHUP_PORT1, NN_PT_NN1,
                  packet_map1a);
  DiscoverMapping(mappingSock, matchup1ip, MATCHUP_PORT1, NN_PT_NN1,
                  packet_map1b);
  DiscoverMapping(mappingSock, matchup2ip, MATCHUP_PORT2, NN_PT_NN2,
                  packet_map2);
  DiscoverMapping(mappingSock, matchup3ip, MATCHUP_PORT3, NN_PT_NN3,
                  packet_map3);
...
static void NegotiateThink(NATNegotiator neg) {
  ...
  if (neg->negotiateSock != INVALID_SOCKET) {
    ...
    while (CanReceiveOnSocket(neg->negotiateSock)) {
      error = recvfrom(neg->negotiateSock, indata, NNINBUF_LEN, 0,
  ...
  if (neg->state == ns_initsent ||
      neg->state == ns_connectping) // see if we need to resend init packets
  {
  ...
  if (neg->state == ns_finished &&
      current_time() > neg->retryTime) // check if it is ready to be removed
  {
    struct sockaddr_in saddr;
    saddr.sin_family = AF_INET;
    ...
    if (neg->gameSock == INVALID_SOCKET)
      StartReport(neg, nr_success, neg->negotiateSock,
                  (struct sockaddr_in *)&saddr);
    else
      StartReport(neg, nr_success, neg->gameSock, (struct sockaddr_in *)&saddr);
  }

  if (neg->state == ns_initack &&
      current_time() > neg->retryTime) // see if the partner has timed out
  {
  ...
  if (neg->state == ns_reportsent && current_time() > neg->retryTime) {
    if (neg->retryCount > neg->maxRetryCount) {
      ...
      if (neg->gameSock == INVALID_SOCKET)
        neg->negotiateSock = INVALID_SOCKET; // no gameSock, so don't let
                                             // NNCancel close this socket
...
static void SendConnectAck(NATNegotiator neg, struct sockaddr_in *toaddr) {
  ...
  gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice,
                "Sending connect ack...\n");
  SendPacket(neg->negotiateSock, toaddr->sin_addr.s_addr,
             ntohs(toaddr->sin_port), &p, INITPACKET_SIZE);
}

static void ProcessConnectPacket(NATNegotiator neg, NatNegPacket *p,
  ...
  // send an ack..
  if (p->Packet.Connect.finished ==
      FINISHED_NOERROR) // don't need to ack any errors
  ...
  if (p->Packet.Connect.finished !=
      FINISHED_NOERROR) // call the completed callback with the error code
  {
...
static void ProcessPingPacket(NATNegotiator neg, NatNegPacket *p,
                              struct sockaddr_in *fromaddr) {
  if (neg->state < ns_connectping)
    return;
  ...
  gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice,
                ...
                p->Packet.Connect.gotyourdata, p->Packet.Connect.finished);
  ...
  if (!p->Packet.Connect
           .gotyourdata) // send another packet until they have our data
    SendPingPacket(neg);
...
static void ProcessInitPacket(NATNegotiator neg, NatNegPacket *p,
                              struct sockaddr_in *fromaddr) {
  switch (p->packettype) {
  case NN_INITACK:
    ...
    {
      if (neg->initAckRecv[NN_PT_NN1] != 0 &&
          neg->initAckRecv[NN_PT_NN2] != 0 &&
          neg->initAckRecv[NN_PT_NN3] != 0 &&
          (neg->gameSock == INVALID_SOCKET ||
           neg->initAckRecv[NN_PT_GP] != 0)) {
  ...
  case NN_ERTTEST:
    ...
    SendPacket(neg->negotiateSock, fromaddr->sin_addr.s_addr,
               ntohs(fromaddr->sin_port), p, INITPACKET_SIZE);
    break;

  case NN_REPORT_ACK:
    gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network,
                  GSIDebugLevel_Notice, "Got REPORT ACK\n");
    neg->state = ns_reportack;
    ...
    if (neg->gameSock == INVALID_SOCKET)
      neg->negotiateSock = INVALID_SOCKET; // no gameSock, so don't let NNCancel
                                           // close this socket
...
void NNProcessData(char *data, int len, struct sockaddr_in *fromaddr) {
  ...
  if (ptype == NN_CONNECT || ptype == NN_CONNECT_PING) { // it's a connect
                                                         // packet
