...
const char *AddressToString(unsigned int ip, unsigned short port,
                            char string[22]);
...
int DiscoverReachability(SOCKET sock, unsigned int ip, unsigned short port,
  ...
  success = sendto(sock, (const char *)&p, sizeof(p), 0, (SOCKADDR *)&addr,
                   sizeof(addr));
...
void OutputMapping(const AddressMapping *theMap) {
  if (theMap == NULL) {
    gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network,
                  GSIDebugLevel_Comment, "ERROR: Port mapping not available.");
  } else {
    gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network,
                  GSIDebugLevel_Comment, "Discovered map: [%s] -> [%s]\n",
...
static int Think(SOCKET sock, NAT *nat) {
  ...
  if (gotERT1 && gotERT2 && gotERT3 && gotADDRESS1a && gotADDRESS1b &&
      gotADDRESS2 && gotADDRESS3) {
  ...
  if (sock != INVALID_SOCKET) {
    ...
    while (CanReceiveOnSocket(sock)) {
      length = recvfrom(sock, data, NNINBUF_LEN, 0, (struct sockaddr *)&saddr,
                        &saddrlen);
      ...
      if (ptype == NN_ERTTEST) {
        ...
        case NN_PT_NN1:
          ...
          gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network,
                        ...
                        AddressToString(saddr.sin_addr.s_addr,
        ...
        gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network,
                      GSIDebugLevel_Comment, "Got ADDRESS REPLY %d from: %s\n",
                      p.cookie,
                      AddressToString(saddr.sin_addr.s_addr,
                                      ntohs(saddr.sin_port), NULL));
...
gsi_bool DetermineNatType(NAT *nat) {
  ...
  if (!nat->portRestricted && !nat->ipRestricted &&
      (nat->mappings[packet_map1a].publicIp ==
       nat->mappings[packet_map1a].privateIp)) {
    nat->natType = no_nat;
  } else if (nat->mappings[packet_map1a].publicIp ==
             nat->mappings[packet_map1a].privateIp) {
    nat->natType = firewall_only;
  } else {
    // What type of NAT is it?
    if (!nat->ipRestricted && !nat->portRestricted &&
        (abs(nat->mappings[packet_map3].publicPort -
             nat->mappings[packet_map2].publicPort) >= 1)) {
  ...
  if (nat->mappings[packet_map1a].publicPort ==
          nat->mappings[packet_map1a].privatePort &&
      nat->mappings[packet_map2].publicPort ==
          nat->mappings[packet_map2].privatePort &&
      nat->mappings[packet_map3].publicPort ==
          nat->mappings[packet_map3].privatePort)
    // Using private port as the public port.
    nat->mappingScheme = private_as_public;
  else if (nat->mappings[packet_map1a].publicPort ==
               nat->mappings[packet_map2].publicPort &&
           nat->mappings[packet_map2].publicPort ==
               nat->mappings[packet_map3].publicPort)
  ...
  else if (nat->mappings[packet_map3].publicPort -
               nat->mappings[packet_map2].publicPort ==
           1)
  ...
  if (nat->mappings[packet_map1b].publicPort != 0 &&
      nat->mappings[packet_map1a].publicPort !=
          nat->mappings[packet_map1b].publicPort) {
