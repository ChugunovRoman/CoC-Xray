...
static void tryread(SOCKET s) {
  ...
  while (CanReceiveOnSocket(s)) {
    len = recvfrom(s, buf, sizeof(buf) - 1, 0, (struct sockaddr *)&saddr,
                   &saddrlen);
...
static void pc(NegotiateState state, void *userdata) {
  if (aString != NULL) {
    ...
    switch (state) {
    case ns_initack:
      sprintf(aString + charsWritten,
              "ns_initack, Init Packets Acknowledged.\n");
      break;
    case ns_connectping:
      sprintf(
          aString + charsWritten,
          "ns_connectping, Starting connection and pinging other machine.\n");
      break;
    default:
      break;
    }
...
static void cc(NegotiateResult result, SOCKET gamesocket,
  ...
  if (result != nr_success) {
    ...
    case nr_deadbeatpartner:
      sprintf(
          aString,
          "Result: nr_deadbeatpartner, The other machine isn't responding.\n");
      break;
    case nr_inittimeout:
      sprintf(
          aString,
          "Result: nr_inittimeout, The NAT server could not be contacted.\n");
      break;
    case nr_pingtimeout:
      sprintf(aString, "Result: nr_pingtimeout, The other machine could not be "
                       "contacted.\n");
      break;
    default:
      break;
...
static void nr(gsi_bool success, NAT nat) {
  ...
  case firewall_only:
    charsWritten =
        sprintf(aString, "No NAT detected, but firewall may be present.");
    break;
  case full_cone:
  ...
  if (nat.natType != no_nat && nat.natType != firewall_only)
    ...
    case private_as_public:
      charsWritten +=
          sprintf(aString + charsWritten,
                  "and is using the private port as the public port.");
      break;
    case consistent_port:
      charsWritten += sprintf(aString + charsWritten,
                              "and is using the same public port for all "
                              "requests from the same private port.");
      break;
    case incremental:
      charsWritten +=
          sprintf(aString + charsWritten,
                  "and is using an incremental port mapping scheme.");
      break;
    case mixed:
      charsWritten += sprintf(aString + charsWritten,
                              "and is using a mixed port mapping scheme.");
      break;
    case unrecognized:
    default:
      charsWritten +=
          sprintf(aString + charsWritten,
                  "and is using an unrecognized port mapping scheme.");
      break;
    }
...
#define MEMPOOL_SIZE (8 * 1024 * 1024)
  ...
  error = NNBeginNegotiationWithSocket(sock, 666, (argc == 1) ? 0 : 1, pc, cc,
                                       NULL);
#else
  ...
  while ((current_time() - startTime) < 60000) {
    ...
    if (connected) {
      if (current_time() - lastsendtime > 2000) {
        int ret = sendto(sock, "woohoo!!", 8, 0, (struct sockaddr *)&otheraddr,
