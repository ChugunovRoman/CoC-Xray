...
** DEFINES **
************/
#define PTA_DEFAULT_VERCHECK_URL                                               \
  "http://motd." GSI_DOMAIN_NAME "/motd/vercheck.asp"
...
static void ptaCallPatchCallback(ptaPatchData *data, PTBool available,
                                 PTBool mandatory, const char *versionName,
                                 int fileID, const char *downloadURL) {
  if (data->callback) {
#ifndef GSI_UNICODE
    data->callback(available, mandatory, versionName, fileID, downloadURL,
                   data->param);
#else
    ...
    data->callback(available, mandatory, versionName_W, fileID, downloadURL_W,
                   data->param);
#endif
  }
...
static void ptaCallFilePlanetInfoCallback(ptaFilePlanetInfoData *data,
                                          PTBool found, const char *description,
                                          const char *size, int numMirrors,
                                          char **mirrorNames,
                                          char **mirrorURLs) {
  int i;
  ...
  if (data->callback) {
    if (!found)
      data->callback(data->fileID, PTFalse, NULL, NULL, 0, NULL, NULL,
                     data->param);
    else {
#ifndef GSI_UNICODE
      data->callback(data->fileID, found, description, size, numMirrors,
                     (const char **)mirrorNames, (const char **)mirrorURLs,
                     data->param);
#else
      ...
      mirrorNames_W = UTF8ToUCS2StringArrayAlloc(
          (const UTF8String *)mirrorNames, numMirrors);
      mirrorURLs_W = UTF8ToUCS2StringArrayAlloc((const UTF8String *)mirrorURLs,
                                                numMirrors);
...
static GHTTPBool ptaPatchCompletedCallback(GHTTPRequest request,
  ...
  // Call the callback.
  /////////////////////
  ptaCallPatchCallback(data, PTTrue, mandatory, versionName, fileID,
                       downloadURL);

  return GHTTPTrue;
}

PTBool ptCheckForPatchA(int productID, const char *versionUniqueID,
                        int distributionID, ptPatchCallback callback,
  ...
  // Send the request.
  ////////////////////
  if ((ghttpGetFileA(aURL, (GHTTPBool)blocking, ptaPatchCompletedCallback,
...
#ifdef GSI_UNICODE
PTBool ptCheckForPatchW(int productID, const unsigned short *versionUniqueID,
                        int distributionID, ptPatchCallback callback,
                        PTBool blocking, void *param) {
  char versionUniqueID_A[255];
  UCS2ToUTF8String(versionUniqueID, versionUniqueID_A);
  return ptCheckForPatchA(productID, versionUniqueID_A, distributionID,
                          callback, blocking, param);
}
#endif

PTBool ptTrackUsageA(int userID, int productID, const char *versionUniqueID,
                     int distributionID, PTBool blocking) {
  int charsWritten;
  char aURL[PTA_MAX_STRING_SIZE];
  ...
  charsWritten = snprintf(aURL, PTA_MAX_STRING_SIZE,
                          "%s?userid=%d&productid=%d&versionuniqueid=%s&distid="
                          "%d&uniqueid=%s&gamename=%s",
                          gPTAMOTDURL, userID, productID, versionUniqueID,
                          distributionID, GOAGetUniqueID(), __GSIACGamename);
  assert(charsWritten >= 0);
  if (charsWritten < 0)
    return PTFalse;
  // Send the info.
...
#ifdef GSI_UNICODE
PTBool ptTrackUsageW(int userID, int productID,
                     const unsigned short *versionUniqueID, int distributionID,
                     PTBool blocking) {
  char versionUniqueID_A[255];
  UCS2ToUTF8String(versionUniqueID, versionUniqueID_A);
  return ptTrackUsageA(userID, productID, versionUniqueID_A, distributionID,
                       blocking);
}
#endif

PTBool ptCreateCheckPatchTrackUsageReqA(
    int userID, int productID, const char *versionUniqueID, int distributionID,
  ...
  charsWritten = snprintf(aURL, PTA_MAX_STRING_SIZE,
                          "%s?userid=%d&productid=%d&versionuniqueid=%s&distid="
                          "%d&uniqueid=%s&gamename=%s",
                          gPTAVercheckURL, userID, productID, versionUniqueID,
                          distributionID, GOAGetUniqueID(), __GSIACGamename);
  ...
  ////////////////////
  *request =
      ghttpGetFileA(aURL, (GHTTPBool)blocking, ptaPatchCompletedCallback, data);
  if (*request == -1)
    return PTFalse;
  return PTTrue;
}
...
#ifdef GSI_UNICODE
PTBool ptCreateCheckPatchTrackUsageReqW(int userID, int productID,
                                        const unsigned short *versionUniqueID,
                                        int distributionID,
                                        ptPatchCallback callback,
                                        PTBool blocking, int *request,
                                        void *param) {
  char versionUniqueID_A[255];
  UCS2ToUTF8String(versionUniqueID, versionUniqueID_A);
  return ptCreateCheckPatchTrackUsageReqA(userID, productID, versionUniqueID_A,
                                          distributionID, callback, blocking,
                                          request, param);
}
#endif

PTBool ptCheckForPatchAndTrackUsageA(int userID, int productID,
                                     const char *versionUniqueID,
                                     int distributionID,
                                     ptPatchCallback callback, PTBool blocking,
                                     void *param) {
  GHTTPRequest aRequest;

  // create the request and send it.
  ///////////////////////////////////
  return ptCreateCheckPatchTrackUsageReqA(userID, productID, versionUniqueID,
                                          distributionID, callback, blocking,
                                          &aRequest, param);
}

#ifdef GSI_UNICODE
PTBool ptCheckForPatchAndTrackUsageW(int userID, int productID,
                                     const unsigned short *versionUniqueID,
                                     int distributionID,
                                     ptPatchCallback callback, PTBool blocking,
                                     void *param) {
  char versionUniqueID_A[255];
  UCS2ToUTF8String(versionUniqueID, versionUniqueID_A);
  return ptCheckForPatchAndTrackUsageA(userID, productID, versionUniqueID_A,
                                       distributionID, callback, blocking,
                                       param);
}
#endif
...
static GHTTPBool ptaFilePlanetCompletedCallback(GHTTPRequest request,
  ...
  // Call the callback.
  /////////////////////
  ptaCallFilePlanetInfoCallback(data, PTTrue, description, size, i, mirrorNames,
                                mirrorURLs);
...
//
PTBool ptLookupFilePlanetInfo(int fileID, ptFilePlanetInfoCallback callback,
                              PTBool blocking, void *param) {
  char aURL[PTA_MAX_STRING_SIZE];
  ...
  ///////////////////////////////////////////////
  snprintf(aURL, PTA_MAX_STRING_SIZE, "%s?file=%d&gamename=%s",
           gPTAFilePlanetURL, fileID, __GSIACGamename);
