...
static void PatchCallback(PTBool available, PTBool mandatory,
                          const gsi_char *versionName, int fileID,
                          const gsi_char *downloadURL, void *param) {
...
int test_main(int argc, char **argv) {
  ...
  if (!ptCheckForPatch(GMTEST_PID, GMTEST_VID_1, 0, PatchCallback, PTFalse,
                       NULL))
