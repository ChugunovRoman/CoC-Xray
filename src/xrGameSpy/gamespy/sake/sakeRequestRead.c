...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
static SAKEStartRequestResult SAKE_CALL
sakeiValidateRequestFieldNames(char **fields, int numFields) {
  int i;
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
static void SAKE_CALL sakeiFillSoapRequestFieldNames(SAKERequest request,
                                                     char **names,
                                                     int numFields) {
  int i;

  gsXmlWriteOpenTag(request->mSoapRequest, GSI_SAKE_SERVICE_NAMESPACE,
                    "fields");

  for (i = 0; i < numFields; i++) {
    gsXmlWriteStringElement(request->mSoapRequest, GSI_SAKE_SERVICE_NAMESPACE,
                            "string", names[i]);
  }

  gsXmlWriteCloseTag(request->mSoapRequest, GSI_SAKE_SERVICE_NAMESPACE,
                     "fields");
}

///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
static void SAKE_CALL sakeiFillSoapRequestRecordIds(SAKERequest request,
                                                    int *recordIds,
                                                    int numRecordIds) {
  int i;
  ...
  for (i = 0; i < numRecordIds; i++) {
    gsXmlWriteIntElement(request->mSoapRequest, GSI_SAKE_SERVICE_NAMESPACE,
                         "int", (gsi_u32)recordIds[i]);
  }
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
static void SAKE_CALL sakeiFillSoapRequestOwnerIds(SAKERequest request,
                                                   int *ownerIds,
                                                   int numOwnerIds) {
  int i;
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
static SAKERequestResult SAKE_CALL
sakeiReadOutputRecords(SAKERequest request, SAKEField ***outputRecordsPtr,
                       int *numRecords, int numFields, char **fieldNames)

{
  ...
  *numRecords =
      gsXmlCountChildren(request->mSoapResponse, "ArrayOfRecordValue");
  ...
  *outputRecordsPtr = outputRecords;
  ...
  for (recordIndex = 0; recordIndex < *numRecords; recordIndex++) {
    ...
    if (gsi_is_false(
            gsXmlMoveToNext(request->mSoapResponse, "ArrayOfRecordValue")))
      return SAKERequestResult_MALFORMED_RESPONSE;
    ...
    for (fieldIndex = 0; fieldIndex < numFields; fieldIndex++) {
      ...
      if (gsXmlMoveToChild(request->mSoapResponse, "byteValue")) {
        int value;
        if (gsi_is_false(
                gsXmlReadChildAsInt(request->mSoapResponse, "value", &value)))
          return SAKERequestResult_MALFORMED_RESPONSE;
        ...
        gsi_i64 value;
        if (gsi_is_false(
                gsXmlReadChildAsInt64(request->mSoapResponse, "value", &value)))
          return SAKERequestResult_MALFORMED_RESPONSE;
        ...
        float value;
        if (gsi_is_false(
                gsXmlReadChildAsFloat(request->mSoapResponse, "value", &value)))
          return SAKERequestResult_MALFORMED_RESPONSE;
        ...
        char *value;
        int len;
        if (gsi_is_false(gsXmlReadChildAsString(request->mSoapResponse, "value",
      ...
      } else if (gsXmlMoveToChild(request->mSoapResponse,
                                  "unicodeStringValue")) {
        char *value;
        ...
        time_t value;
        if (gsi_is_false(gsXmlReadChildAsDateTimeElement(request->mSoapResponse,
                                                         "value", &value)))
          return SAKERequestResult_MALFORMED_RESPONSE;
        ...
        gsi_u8 *value;
        int len;
        if (gsi_is_false(gsXmlReadChildAsBase64Binary(request->mSoapResponse,
                                                      "value", NULL, &len)))
          return SAKERequestResult_MALFORMED_RESPONSE;
        if (len > 0) {
          ...
          if (!value)
            return SAKERequestResult_OUT_OF_MEMORY;
          if (gsi_is_false(gsXmlReadChildAsBase64Binary(
                  request->mSoapResponse, "value", value, &len))) {
            gsifree(value);
            return SAKERequestResult_MALFORMED_RESPONSE;
          }
        } else {
...
static void SAKE_CALL sakeiFreeOutputRecord(int numFields, SAKEField *record) {
  ...
  for (i = 0; i < numFields; i++) {
    if (record[i].mType == SAKEFieldType_BINARY_DATA &&
        record[i].mValue.mBinaryData.mValue != NULL)
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
static void SAKE_CALL sakeiFreeOutputRecords(int numFields, int numRecords,
                                             SAKEField **records) {
  int i, j;
  ...
  for (i = 0; i < numRecords; i++) {
    ...
    for (j = 0; j < numFields; j++) {
      if (records[i][j].mType == SAKEFieldType_BINARY_DATA &&
...
static SAKEStartRequestResult SAKE_CALL
sakeiSearchForRecordsValidateInput(SAKERequest request) {
  SAKESearchForRecordsInput *input =
      (SAKESearchForRecordsInput *)request->mInput;
...
static SAKEStartRequestResult SAKE_CALL
sakeiSearchForRecordsFillSoapRequest(SAKERequest request) {
  SAKESearchForRecordsInput *input =
      (SAKESearchForRecordsInput *)request->mInput;

  // write the table id
  gsXmlWriteStringElement(request->mSoapRequest, GSI_SAKE_SERVICE_NAMESPACE,
                          "tableid", input->mTableId);

  // write the filter
  if (input->mFilter != NULL)
    gsXmlWriteTStringElement(request->mSoapRequest, GSI_SAKE_SERVICE_NAMESPACE,
                             "filter", input->mFilter);
  ...
  // write the offset
  gsXmlWriteIntElement(request->mSoapRequest, GSI_SAKE_SERVICE_NAMESPACE,
                       "offset", (gsi_u32)input->mOffset);

  // write the max
  gsXmlWriteIntElement(request->mSoapRequest, GSI_SAKE_SERVICE_NAMESPACE, "max",
                       (gsi_u32)input->mMaxRecords);

  // write the target record filter
  if (input->mTargetRecordFilter != NULL)
    gsXmlWriteTStringElement(request->mSoapRequest, GSI_SAKE_SERVICE_NAMESPACE,
                             "targetfilter", input->mTargetRecordFilter);

  // write the surrounding record count
  gsXmlWriteIntElement(request->mSoapRequest, GSI_SAKE_SERVICE_NAMESPACE,
                       "surrounding", (gsi_u32)input->mSurroundingRecordsCount);
  ...
  gsXmlWriteIntElement(request->mSoapRequest, GSI_SAKE_SERVICE_NAMESPACE,
                       "cacheFlag", (gsi_u32)input->mCacheFlag);

  // fill in the field names
  sakeiFillSoapRequestFieldNames(request, input->mFieldNames,
                                 input->mNumFields);

  return SAKEStartRequestResult_SUCCESS;
}

static SAKERequestResult
sakeiSearchForRecordsProcessSoapResponse(SAKERequest request) {
  SAKESearchForRecordsInput *input =
      (SAKESearchForRecordsInput *)request->mInput;
  SAKESearchForRecordsOutput *output =
      (SAKESearchForRecordsOutput *)request->mOutput;

  // fill the output records
  return sakeiReadOutputRecords(request, &output->mRecords,
                                &output->mNumRecords, input->mNumFields,
                                input->mFieldNames);
}
...
static void sakeiSearchForRecordsFreeData(SAKERequest request) {
  ...
  if (output)
    sakeiFreeOutputRecords(input->mNumFields, output->mNumRecords,
                           output->mRecords);
}
...
static SAKEStartRequestResult SAKE_CALL
sakeiGetMyRecordsValidateInput(SAKERequest request) {
...
static SAKEStartRequestResult SAKE_CALL
sakeiGetMyRecordsFillSoapRequest(SAKERequest request) {
  SAKEGetMyRecordsInput *input = (SAKEGetMyRecordsInput *)request->mInput;
...
static SAKERequestResult
sakeiGetMyRecordsProcessSoapResponse(SAKERequest request) {
  SAKEGetMyRecordsInput *input = (SAKEGetMyRecordsInput *)request->mInput;
...
static SAKEStartRequestResult SAKE_CALL
sakeiGetSpecificRecordsValidateInput(SAKERequest request) {
  SAKEGetSpecificRecordsInput *input =
      (SAKEGetSpecificRecordsInput *)request->mInput;
...
static SAKEStartRequestResult SAKE_CALL
sakeiGetSpecificRecordsFillSoapRequest(SAKERequest request) {
  SAKEGetSpecificRecordsInput *input =
      (SAKEGetSpecificRecordsInput *)request->mInput;
  ...
  sakeiFillSoapRequestRecordIds(request, input->mRecordIds,
                                input->mNumRecordIds);
...
static SAKERequestResult
sakeiGetSpecificRecordsProcessSoapResponse(SAKERequest request) {
  SAKEGetSpecificRecordsInput *input =
      (SAKEGetSpecificRecordsInput *)request->mInput;
  SAKEGetSpecificRecordsOutput *output =
      (SAKEGetSpecificRecordsOutput *)request->mOutput;
...
static SAKEStartRequestResult SAKE_CALL
sakeiGetRandomRecordValidateInput(SAKERequest request) {
...
static SAKEStartRequestResult SAKE_CALL
sakeiGetRandomRecordFillSoapRequest(SAKERequest request) {
  SAKEGetRandomRecordInput *input = (SAKEGetRandomRecordInput *)request->mInput;
...
static SAKERequestResult
sakeiGetRandomRecordProcessSoapResponse(SAKERequest request) {
  SAKEGetRandomRecordInput *input = (SAKEGetRandomRecordInput *)request->mInput;
  SAKEGetRandomRecordOutput *output =
      (SAKEGetRandomRecordOutput *)request->mOutput;
  SAKEField **records;
  int numRecords;

  // fill the output record
  SAKERequestResult result = sakeiReadOutputRecords(
      request, &records, &numRecords, input->mNumFields, input->mFieldNames);
