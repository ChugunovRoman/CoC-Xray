...
SAKERequest SAKE_CALL sakeiInitRequest(SAKE sake, SAKEIRequestType type,
  ...
  if (gsi_is_false(sake->mIsGameAuthenticated) ||
      gsi_is_false(sake->mIsProfileAuthenticated)) {
...
sakeiCheckHttpResult(GHTTPResult httpResult) {
  switch (httpResult) {
  case GHTTPSuccess:
...
static SAKERequestResult SAKE_CALL
sakeiCheckSakeResult(const char *sakeResult) {
...
static void SAKE_CALL sakeiSoapCallback(GHTTPResult httpResult,
  ...
  if (result == SAKERequestResult_SUCCESS) {
    if (gsi_is_false(gsXmlMoveToStart(responseData)) ||
        gsi_is_false(
            gsXmlMoveToNext(responseData, request->mInfo->mResponseTag)) ||
        gsi_is_false(
            gsXmlReadChildAsStringNT(responseData, request->mInfo->mResultTag,
  ...
  if (request->mCallback)
    request->mCallback(request->mSake, request, result, request->mInput, output,
                       request->mUserData);
...
static SAKEStartRequestResult SAKE_CALL sakeiSetupRequest(SAKERequest request) {
  ...
  request->mSoapRequest = gsXmlCreateStreamWriter(
...
static void SAKE_CALL sakeiExecuteRequest(SAKERequest request) {
  if (sakeiSoapUrl[0] == '\0') {
    int rcode;
    rcode = snprintf(sakeiSoapUrl, SAKE_MAX_URL_LENGTH, SAKEI_SOAP_URL_FORMAT,
                     request->mSake->mGameName);
    GS_ASSERT(rcode >= 0);
    GSI_UNUSED(rcode);
  }
  gsiExecuteSoap(sakeiSoapUrl, request->mInfo->mSoapAction,
                 request->mSoapRequest, sakeiSoapCallback, request);
}
