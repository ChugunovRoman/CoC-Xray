...
static void RecvBuddyRequest(GPConnection *pconnection,
...
static void RecvBuddyStatus(GPConnection *pconnection,
                            GPRecvBuddyMessageArg *arg, void *param) {
  GSI_UNUSED(arg);
  GSI_UNUSED(pconnection);
  GSI_UNUSED(param);
...
static void RecvBuddyMessage(GPConnection *pconnection,
                             GPRecvBuddyMessageArg *arg, void *param) {
  GSI_UNUSED(arg);
  GSI_UNUSED(pconnection);
  GSI_UNUSED(param);
...
static void RecvGameInvite(GPConnection *pconnection,
                           GPRecvBuddyMessageArg *arg, void *param) {
  GSI_UNUSED(arg);
  GSI_UNUSED(pconnection);
  GSI_UNUSED(param);
...
static void TransferCallback(GPConnection *pconnection,
                             GPRecvBuddyMessageArg *arg, void *param) {
  GSI_UNUSED(arg);
  GSI_UNUSED(pconnection);
  GSI_UNUSED(param);
...
static void RecvBuddyAuth(GPConnection *pconnection, GPRecvBuddyMessageArg *arg,
                          void *param) {
  GSI_UNUSED(arg);
  GSI_UNUSED(pconnection);
  GSI_UNUSED(param);
...
static void RecvBuddyRevoke(GPConnection *pconnection,
                            GPRecvBuddyMessageArg *arg, void *param) {
  GSI_UNUSED(arg);
  GSI_UNUSED(pconnection);
  GSI_UNUSED(param);
...
static void PrintSeperator(void) { printf("*******************\n"); }
...
static gsi_bool HandleRequestResult(SAKERequestResult requestResult,
                                    const char *requestType) {
  if (requestResult != SAKERequestResult_SUCCESS) {
    printf("%s - Error - %s\n", requestType,
           RequestResultToString(requestResult));
    return gsi_false;
  }
...
static void DisplayReadResults(SAKEField **records, int numRecords,
                               int numFields) {
  SAKEField *field;
  int recordIndex;
  int fieldIndex;
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
static void CreateRecordCallback(SAKE sake, SAKERequest request,
...
static void UpdateRecordCallback(SAKE sake, SAKERequest request,
                                 SAKERequestResult result, void *inputData,
                                 void *outputData, void *userData) {
  NumOperations--;
...
static void DeleteRecordCallback(SAKE sake, SAKERequest request,
                                 SAKERequestResult result, void *inputData,
                                 void *outputData, void *userData) {
  NumOperations--;
...
static void SearchForRecordsCallback(SAKE sake, SAKERequest request,
                                     SAKERequestResult result, void *inputData,
                                     void *outputData, void *userData) {
...
static void GetMyRecordsCallback(SAKE sake, SAKERequest request,
                                 SAKERequestResult result, void *inputData,
                                 void *outputData, void *userData) {
...
static void GetSpecificRecordsCallback(SAKE sake, SAKERequest request,
  ...
  SAKEGetSpecificRecordsOutput *output =
      (SAKEGetSpecificRecordsOutput *)outputData;
...
static void GetRandomRecordCallback(SAKE sake, SAKERequest request,
                                    SAKERequestResult result, void *inputData,
                                    void *outputData, void *userData) {
...
static void RateRecordCallback(SAKE sake, SAKERequest request,
                               SAKERequestResult result, void *inputData,
                               void *outputData, void *userData) {
...
static void GetRecordLimitCallback(SAKE sake, SAKERequest request,
                                   SAKERequestResult result, void *inputData,
                                   void *outputData, void *userData) {
...
static GHTTPBool UploadCompletedCallback(GHTTPRequest request,
...
static GHTTPBool DownloadCompletedCallback(GHTTPRequest request,
                                           GHTTPResult result, char *buffer,
                                           GHTTPByteCount bufferLen,
                                           void *param) {
  SAKEFileResult fileResult;
...
static void postCallback(GHTTPRequest request, int bytesPosted, int totalBytes,
...
static void RunTests(SAKE sake) {
...
#if 1
  {
    ...
    request =
        sakeSearchForRecords(sake, &input, SearchForRecordsCallback, NULL);
    if (!request) {
      startRequestResult = sakeGetStartRequestResult(sake);
      printf("Failed to start request: %d\n", startRequestResult);
    } else {
...
#if 1
  {
    ...
    static char *fieldNames[] = {
        "recordid",    "ownerid",       "MyByte",        "MyShort",
        "MyInt",       "MyFloat",       "MyAsciiString", "MyUnicodeString",
        "MyBoolean",   "MyDateAndTime", "MyBinaryData",  "MyFileID",
...
#if 1
  {
    ...
    request =
        sakeGetSpecificRecords(sake, &input, GetSpecificRecordsCallback, NULL);
    if (!request) {
      startRequestResult = sakeGetStartRequestResult(sake);
      printf("Failed to start request: %d\n", startRequestResult);
    } else {
...
#if 1
  {
    ...
    // ghttpPostAddFileFromMemory(post, "memory.file", memFile,
    // (int)strlen(memFile), "memory.file", NULL);
    ghttpPostAddFileFromMemory(post, _T("memory.file"), (const char *)&data,
                               SAKE_UPLOAD_AMOUNT, _T("memory.file"), NULL);
    request = ghttpPostEx(url, NULL, post, GHTTPFalse, GHTTPTrue, NULL,
    ...
    if (gUploadResult) {
      ...
      // download a file
      request = ghttpSave(url, _T("downloaded.file"), GHTTPTrue,
...
static void LoginAndAuthenticate(SAKE sake) {
  ...
  CHECK_GP_RESULT(
      gpInitialize(pconn, PRODUCTID, NAMESPACEID, GP_PARTNERID_GAMESPY),
      "gpInitialize failed");

  // setup callbacks
  CHECK_GP_RESULT(gpSetCallback(pconn, GP_ERROR, (GPCallback)Error, NULL),
                  "gpSetCallback failed");
  CHECK_GP_RESULT(gpSetCallback(pconn, GP_RECV_BUDDY_REQUEST,
                                (GPCallback)RecvBuddyRequest, NULL),
                  "gpSetCallback failed");
  CHECK_GP_RESULT(gpSetCallback(pconn, GP_RECV_BUDDY_STATUS,
                                (GPCallback)RecvBuddyStatus, NULL),
                  "gpSetCallback failed");
  CHECK_GP_RESULT(gpSetCallback(pconn, GP_RECV_BUDDY_MESSAGE,
                                (GPCallback)RecvBuddyMessage, NULL),
                  "gpSetCallback failed");
  CHECK_GP_RESULT(gpSetCallback(pconn, GP_RECV_GAME_INVITE,
                                (GPCallback)RecvGameInvite, NULL),
                  "gpSetCallback failed");
  CHECK_GP_RESULT(gpSetCallback(pconn, GP_TRANSFER_CALLBACK,
                                (GPCallback)TransferCallback, NULL),
                  "gpSetCallback failed");
  CHECK_GP_RESULT(
      gpSetCallback(pconn, GP_RECV_BUDDY_AUTH, (GPCallback)RecvBuddyAuth, NULL),
      "gpSetCallback failed");
  CHECK_GP_RESULT(gpSetCallback(pconn, GP_RECV_BUDDY_REVOKE,
                                (GPCallback)RecvBuddyRevoke, NULL),
                  "gpSetCallback failed");

  // connect to GP
  printf("Connecting to GP...\n");
  CHECK_GP_RESULT(gpConnect(pconn, NICKNAME, EMAIL, PASSWORD, GP_NO_FIREWALL,
                            GP_BLOCKING, (GPCallback)ConnectResponse, NULL),
                  "gpConnect failed");

  // retrieve the login ticket
  CHECK_GP_RESULT(gpGetLoginTicket(pconn, loginTicket),
                  "gpGetLoginTicket failed");

  // process
  CHECK_GP_RESULT(gpProcess(pconn), "gpProcess failed");

  // authenticate Sake with the login ticket and the profileid returned from
  // login
