...
int noComLineArgs;
int namespaceIds[GP_MAX_NAMESPACEIDS] = {1, 2,  3,  4,  5,  6,  7,  8,
...
static void Whois(GPConnection *pconnection, GPGetInfoResponseArg *arg,
                  void *param) {
...
static void ConnectResponse(GPConnection *pconnection,
...
static void ProfileSearchResponse(GPConnection *pconnection,
  ...
  if (arg->result == GP_NO_ERROR) {
    if (arg->numMatches > 0) {
      for (i = 0; i < arg->numMatches; i++) {
        result = gpGetInfo(pconn, arg->matches[i].profile, GP_DONT_CHECK_CACHE,
                           GP_BLOCKING, (GPCallback)Whois, NULL);
...
int msgCount = 0;
static void RecvBuddyMessage(GPConnection *pconnection, void *theArg,
                             void *param) {
  ...
  result = gpGetInfo(pconn, arg->profile, GP_DONT_CHECK_CACHE, GP_BLOCKING,
                     (GPCallback)Whois, NULL);
  if (result != GP_NO_ERROR)
    printf(" gpGetInfo failed\n");
  else
...
void RecvBuddyStatus(GPConnection *connection, void *arg_, void *param) {
  ...
  static char *statusToString[6] = {"GP_OFFLINE", "GP_ONLINE",   "GP_PLAYING",
                                    "GP_STAGING", "GP_CHATTING", "GP_AWAY"};
  ...
  if (arg->profile == other) {
    ...
    if (status.status == 1 && _tcscmp(status.statusString, "Ready") &&
        _tcscmp(status.statusString, "BlockTime")) {
      if (appState < 1) {
        appState = 1;
        // printf("Buddy is online!\n");
        CHECK_GP_RESULT(
            gpSetStatus(pconn, (GPEnum)GP_ONLINE, _T("Ready"), _T("gptestc")),
            "gpSetStatus failed");
  ...
  printf(" ");
  gpGetInfo(connection, arg->profile, GP_DONT_CHECK_CACHE, GP_BLOCKING,
            (GPCallback)GetInfoResponse, NULL);

  gpGetBuddyStatus(connection, arg->index, &status);

  printf("  Status: %s, Status String: %s, Location String: %s, IP: %d, Port: "
         "%d\n",
         statusToString[status.status], status.statusString,
         status.locationString, status.ip, status.port);
...
void RecvBuddyRequest(GPConnection *connection, void *arg_, void *param) {
  ...
  gpGetInfo(connection, arg->profile, GP_CHECK_CACHE, GP_BLOCKING,
            (GPCallback)Whois, NULL);
...
int test_main(int argc, char **argv) {
  ...
  // gsSetDebugLevel(GSIDebugCat_QR2, GSIDebugType_Network,
  // GSIDebugLevel_Verbose);   // Show Detailed data on network traffic
  // gsSetDebugLevel(GSIDebugCat_App, GSIDebugType_All, GSIDebugLevel_Hardcore);
  // // Show All app comment
#endif
  ...
  // INITIALIZE
  ////////////
  CHECK_GP_RESULT(gpInitialize(pconn, GPTC_PRODUCTID, 0, GP_PARTNERID_GAMESPY),
                  "gpInitialize failed");
  gpEnable(pconn, GP_INFO_CACHING_BUDDY_AND_BLOCK_ONLY);
  printf("\nInitialized\n");
  CHECK_GP_RESULT(gpSetCallback(pconn, GP_ERROR, (GPCallback)Error, NULL),
                  "gpSetCallback failed");
  CHECK_GP_RESULT(
      gpSetCallback(pconn, GP_RECV_BUDDY_MESSAGE, &RecvBuddyMessage, NULL),
      "gpSetCallback failed");
  CHECK_GP_RESULT(
      gpSetCallback(pconn, GP_RECV_BUDDY_STATUS, &RecvBuddyStatus, NULL),
      "gpSetCallback failed");
  CHECK_GP_RESULT(
      gpSetCallback(pconn, GP_RECV_BUDDY_REQUEST, &RecvBuddyRequest, NULL),
      "gpSetCallback failed");
  CHECK_GP_RESULT(
      gpSetCallback(pconn, GP_RECV_BUDDY_REVOKE, &RecvBuddyRevoke, NULL),
      "gpSetCallback failed");

  // CONNECT
  /////////
  printf("\nConnecting...\n");
  CHECK_GP_RESULT(gpConnect(pconn, nick, email, password, GPTC_FIREWALL_OPTION,
                            GP_BLOCKING, (GPCallback)ConnectResponse, NULL),
                  "gpConnect failed");
  ...
  CHECK_GP_RESULT(gpProfileSearch(pconn, _T(""), _T(""), _T("dan@gamespy.com"),
                                  _T(""), _T(""), 0, GP_BLOCKING,
                                  (GPCallback)ProfileSearchResponse, NULL),
                  "gpProfileSearch failed");
  printf(" Search on nick ('crt')\n");
  CHECK_GP_RESULT(gpProfileSearch(pconn, _T("crt"), _T(""), _T(""), _T(""),
                                  _T(""), 0, GP_BLOCKING,
                                  (GPCallback)ProfileSearchResponse, NULL),
                  "gpProfileSearch failed");
  printf(" Search on nick and email ('gptestc1', gptestc@gptestc.com)\n");
  CHECK_GP_RESULT(gpProfileSearch(pconn, GPTC_NICK1, _T(""), GPTC_EMAIL1,
                                  _T(""), _T(""), 0, GP_BLOCKING,
                                  (GPCallback)ProfileSearchResponse, NULL),
                  "gpProfileSearch failed");
  printf(" Search on unique nick for different namespaces...\n");
  CHECK_GP_RESULT(gpProfileSearchUniquenick(
                      pconn, GPTC_NICK1, namespaceIds, GP_MAX_NAMESPACEIDS,
                      GP_BLOCKING, (GPCallback)ProfileSearchResponse, NULL),
                  "gpProfileSearchUniquenick failed");

  if (noComLineArgs) {
    CHECK_GP_RESULT(gpProfileFromID(pconn, &other, GPTC_PID2),
                    "gpProfileFromID failed");
  } else {
    CHECK_GP_RESULT(gpProfileFromID(pconn, &other, GPTC_PID1),
                    "gpProfileFromID failed");
  }
  ...
  //////////
  printf("\nGetting Info\n");
  CHECK_GP_RESULT(gpGetInfo(pconn, GPTC_PID1, GP_DONT_CHECK_CACHE, GP_BLOCKING,
                            (GPCallback)GetInfoResponse, NULL),
                  "gpGetInfo failed");
  ...
  while (appState < 2) {
    ...
    if (totalTime > 10000) {
      ...
      {
        if (noComLineArgs)
          printf(" %s is not on our buddy list! Sending him a buddy request "
                 "and waiting for a response...\n",
                 GPTC_NICK2);
        else
          printf("%s is not on our buddy list! Sending him a buddy request and "
      ...
      } else // buddy is on our buddy list but did not come online and set
             // status to "Ready"
      {
        if (noComLineArgs)
          printf("\n%s never showed up =(\n", GPTC_NICK2);
  ...
  if (!gotStoodUp) {
    ...
    while (messagesSent < 5) {
      ...
      if (totalTime % 1000 == 0) {
#ifdef GSI_UNICODE
        _stprintf(messageToSend, sizeof(messageToSend), _T("%d_Hello!"),
                  (int)(totalTime / 1000));
#else
        _stprintf(messageToSend, _T("%d_Hello!"), (int)(totalTime / 1000));
#endif
        CHECK_GP_RESULT(gpSendBuddyMessage(pconn, other, messageToSend),
                        "gpSendBuddyMessage failed");
        messagesSent++;
      }
    }
    ...
    CHECK_GP_RESULT(
        gpSetStatus(pconn, (GPEnum)GP_ONLINE, _T("BlockTime"), _T("gptestc")),
        "gpSetStatus failed");
  ...
  if (noComLineArgs) {
    // block buddy
    if (gotStoodUp) {
      CHECK_GP_RESULT(gpProfileFromID(pconn, &other, GPTC_PID3),
                      "gpProfileFromID failed");
      printf("\nBlocking %s\n", GPTC_NICK3);
    } else
      printf("\nBlocking %s\n", GPTC_NICK2);

    CHECK_GP_RESULT(gpAddToBlockedList(pconn, other),
                    "gpAddToBlockedList failed");
    CHECK_GP_RESULT(gpProcess(pconn), "gpProcess failed");
    printBlockedList();
    ...
    if (!gotStoodUp)
      ...
      printf("\nUnblocking %s\n", GPTC_NICK3);
    CHECK_GP_RESULT(gpRemoveFromBlockedList(pconn, other),
                    "gpRemoveFromBlockedList");
    CHECK_GP_RESULT(gpProcess(pconn), "gpProcess failed");
    printBlockedList();

    CHECK_GP_RESULT(gpSetStatus(pconn, (GPEnum)GP_ONLINE,
                                _T("DoneBlockTesting"), _T("gptestc")),
                    "gpSetStatus failed");
    ...
    if (!gotStoodUp) {
      ...
      printf("We have been blocked. Trying to send buddy message to %s\n",
             GPTC_NICK1);
      CHECK_GP_RESULT(
          gpSendBuddyMessage(pconn, other, _T("Why did you block me?")),
          "gpSendBuddyMessage failed");
