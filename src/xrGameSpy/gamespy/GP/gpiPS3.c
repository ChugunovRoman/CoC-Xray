...
GPResult gpiInitializeNpBasic(GPConnection *connection) {
  ...
  if (ret == SCE_NP_ERROR_ALREADY_INITIALIZED) {
    ...
    gsDebugFormat(GSIDebugCat_GP, GSIDebugType_Misc, GSIDebugLevel_HotError,
                  "gpiInitializeNpBasic: sceNpInit() failed, NP-functionality "
                  "disabled. ret = 0x%x\n",
                  ret);
    return GP_MISC_ERROR;
  } else
    iconnection->npBasicGameInitialized =
        gsi_false; // GP initialized, so destroy after complete
...
GPResult gpiCheckNpStatus(GPConnection *connection) {
  ...
  if (status != SCE_NP_MANAGER_STATUS_ONLINE &&
      (current_time() - iconnection->loginTime > GPI_NP_STATUS_TIMEOUT)) {
    gsDebugFormat(GSIDebugCat_GP, GSIDebugType_Misc, GSIDebugLevel_HotError,
  ...
  if (status == SCE_NP_MANAGER_STATUS_ONLINE) {
    ...
    // Note - we ignore error messages here - if something fails we really don't
    // care
    /////////////////////////////////////////////////////////////////////////////////
    if (!iconnection->npBasicGameInitialized) {
      ...
      ret = sceNpBasicRegisterHandler(&gpi_communication_id, gpiNpBasicCallback,
                                      NULL);
      if (ret < 0) {
        gsDebugFormat(GSIDebugCat_GP, GSIDebugType_Misc, GSIDebugLevel_HotError,
                      "gpiCheckNpStatus: sceNpBasicRegisterHandler() failed. "
                      "ret = 0x%x\n",
                      ret);
      }
    }
    ...
    if (ret == SCE_NP_COMMUNITY_ERROR_ALREADY_INITIALIZED) {
      // If already initialized - DO NOT terminate after GP destroy (game might
      // need it)
      //////////////////////////////////////////////////////////////////////////////////
...
GPResult gpiSyncNpBuddies(GPConnection *connection) {
  ...
  for (i = 0; i < count; i++) {
    ...
    gpiProfileSearchUniquenick(connection, npId.handle.data,
                               &iconnection->namespaceID, 1, GP_NON_BLOCKING,
  ...
  if (arg->result == GP_NO_ERROR) {
    if (arg->numMatches == 1) {
      ...
      if (!gpIsBuddy(pconnection, arg->matches[0].profile)) {
        gsDebugFormat(GSIDebugCat_GP, GSIDebugType_Misc, GSIDebugLevel_Comment,
                      "PS3BuddySync: NP Buddy \"%s\" found in namespace %d. "
                      "Sending Request.\n",
                      arg->matches[0].uniquenick, arg->matches[0].namespaceID);

        // Send the add request
        ////////////////////////
        gpSendBuddyRequest(pconnection, arg->matches[0].profile,
                           _T("PS3 Buddy Sync"));
      } else
        gsDebugFormat(GSIDebugCat_GP, GSIDebugType_Misc, GSIDebugLevel_Comment,
...
void gpiSyncNpBlockListCallback(GPConnection *pconnection,
  ...
  if (arg->result == GP_NO_ERROR) {
    if (arg->numMatches == 1) {
      ...
      ////////////////////////////
      if (!gpiGetProfile(pconnection, arg->matches[0].profile, &pProfile) ||
          !pProfile->blocked) {
        gsDebugFormat(GSIDebugCat_GP, GSIDebugType_Misc, GSIDebugLevel_Comment,
                      "PS3BlockSync: NP Block Entry \"%s\" found in namespace "
                      "%d. Adding to BlockedList.\n",
                      arg->matches[0].uniquenick, arg->matches[0].namespaceID);

        // Add to GP Blocked List - set lock to make sure we dont try to add to
        // NP list
        ///////////////////////////////////////////////////////////////////////////////
...
GPResult gpiAddToNpBlockList(GPConnection *connection, int profileid) {
  GPIConnection *iconnection = (GPIConnection *)*connection;
  // TODO: consider developer method for cache input in order to check HDD
  // cache?
...
void gpiAddToNpBlockListInfoCallback(GPConnection *pconnection,
  ...
  if (arg->result == GP_NO_ERROR) {
    ...
    if (_tcslen(arg->uniquenick) != 0) {
      ...
      if (ArrayLength(iconnection->npTransactionList) <
          GPI_NP_NUM_TRANSACTIONS) {
        ret = sceNpLookupCreateTransactionCtx(iconnection->npLookupTitleCtxId);
        if (ret < 0) {
          gsDebugFormat(
              GSIDebugCat_GP, GSIDebugType_Misc, GSIDebugLevel_HotError,
              "PS3AddToNpBlockList: sceNpLookupCreateTransactionCtx() failed. "
              "ret = 0x%x\n",
              ret);
        } else {
          ...
          if (transaction.npIdForAdd == NULL) {
            ...
            gsDebugFormat(GSIDebugCat_GP, GSIDebugType_Misc,
          ...
          if (ret < 0) {
            gsDebugFormat(GSIDebugCat_GP, GSIDebugType_Misc,
                          GSIDebugLevel_HotError,
                          "PS3AddToNpBlockList: sceNpLookupNpIdAsync() failed. "
                          "ret = 0x%x\n",
                          ret);
          }
        }
      } else {
        // Can only have a max of 32 simultaneous transactions (based on PS3
        // lib)
        /////////////////////////////////////////////////////////////////////////
        gsDebugFormat(
      ...
      gsDebugFormat(GSIDebugCat_GP, GSIDebugType_Misc, GSIDebugLevel_HotError,
                    "PS3AddToNpBlockList: Profile [%d] does not have a "
                    "uniquenick in namespace %d!\n",
...
GPResult gpiProcessNp(GPConnection *connection) {
  ...
  for (i = 0; i < ArrayLength(iconnection->npTransactionList); i++) {
    ...
    /////////////////////////////////////
    transaction =
        (npIdLookupTrans *)ArrayNth(iconnection->npTransactionList, i);
    ...
    if (!transaction->npLookupDone) {
      ...
      if (ret < 0) {
        ...
        if (ret == (int)SCE_NP_BASIC_ERROR_BUSY) {
          ...
          gsDebugFormat(GSIDebugCat_GP, GSIDebugType_Misc,
                        ...
                        "PS3AddToNpBlockList: SCE_NP_BASIC_ERROR_BUSY. "
                        "continue trying to add to NP\n");
          return GP_NO_ERROR;
        } else if (ret < 0) {
          gsDebugFormat(GSIDebugCat_GP, GSIDebugType_Misc,
                        GSIDebugLevel_HotError,
                        "PS3AddToNpBlockList: sceNpBasicAddBlockListEntry() "
                        "failed. ret = 0x%x\n",
                        ret);
        } else {
          gsDebugFormat(
      ...
      if (ret < 0) {
        gsDebugFormat(GSIDebugCat_GP, GSIDebugType_Misc, GSIDebugLevel_HotError,
                      "PS3AddToNpBlockList: sceNpLookupDestroyTransactionCtx() "
                      "failed. ret = 0x%x\n",
                      ret);
      }
