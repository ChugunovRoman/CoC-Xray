...
GPResult gpConnectW(GPConnection *connection,
                    ...
                    GPEnum firewall, GPEnum blocking, GPCallback callback,
                    void *param) {
  ...
  return gpConnectA(connection, nick_A, email_A, password_A, firewall, blocking,
                    callback, param);
}
#endif
...
GPResult gpConnectNewUserW(GPConnection *connection,
  ...
  return gpConnectNewUserA(connection, nick_A, uniquenick_A, email_A,
                           password_A, cdkey_A, firewall, blocking, callback,
                           param);
}
#endif
...
#ifdef GSI_UNICODE
GPResult
gpConnectUniqueNickW(GPConnection *connection,
                     const unsigned short uniquenick[GP_UNIQUENICK_LEN],
                     const unsigned short password[GP_PASSWORD_LEN],
                     GPEnum firewall, GPEnum blocking, GPCallback callback,
                     void *param) {
...
GPResult gpConnectPreAuthenticatedW(
    GPConnection *connection, const unsigned short authtoken[GP_AUTHTOKEN_LEN],
    const unsigned short partnerchallenge[GP_PARTNERCHALLENGE_LEN],
    GPEnum firewall, GPEnum blocking, GPCallback callback, void *param) {
  ...
  return gpConnectPreAuthenticatedA(connection, authtoken_A, partnerchallenge_A,
                                    firewall, blocking, callback, param);
}
#endif
...
GPResult gpCheckUserW(GPConnection *connection,
  ...
  return gpCheckUserA(connection, nick_A, email_A, password_A, blocking,
                      callback, param);
}
#endif
...
GPResult gpNewUserW(GPConnection *connection,
                    ...
                    const unsigned short email[GP_EMAIL_LEN],
                    const unsigned short password[GP_PASSWORD_LEN],
                    const unsigned short cdkey[GP_CDKEY_LEN], GPEnum blocking,
                    GPCallback callback, void *param) {
  char nick_A[GP_NICK_LEN];
  char uniquenick_A[GP_UNIQUENICK_LEN];
  char email_A[GP_EMAIL_LEN];
  ...
  return gpNewUserA(connection, nick_A, uniquenick_A, email_A, password_A,
                    cdkey_A, blocking, callback, param);
}
#endif
...
GPResult gpSuggestUniqueNickW(GPConnection *connection,
  ...
  return gpSuggestUniqueNickA(connection, desirednick_A, blocking, callback,
                              param);
}
#endif
...
#ifdef GSI_UNICODE
GPResult
gpRegisterUniqueNickW(GPConnection *connection,
                      const unsigned short uniquenick[GP_UNIQUENICK_LEN],
                      const unsigned short cdkey[GP_CDKEY_LEN], GPEnum blocking,
                      GPCallback callback, void *param) {
  char uniquenick_A[GP_UNIQUENICK_LEN];
  ...
  return gpRegisterUniqueNickA(connection, uniquenick_A, cdkey_A, blocking,
                               callback, param);
}
#endif
...
#ifdef GSI_UNICODE
GPResult gpGetErrorStringW(GPConnection *connection,
...
GPResult gpNewProfileW(GPConnection *connection,
                       const unsigned short nick[GP_NICK_LEN], GPEnum replace,
                       GPEnum blocking, GPCallback callback, void *param) {
  char nick_A[GP_NICK_LEN];

  // Since we don't currently allow UNICODE nicknames, make sure the first byte
  // is empty (We make this check as an early alert to devlopers that the
...
GPResult gpGetBuddyStatus(GPConnection *connection, int index,
  ...
  if (profile->buddyStatus->locationString)
    UTF8ToUCS2String(profile->buddyStatus->locationString,
...
GPResult gpGetBuddyStatusInfo(GPConnection *connection, int index,
  ...
  if (buddyStatus) {
    ...
    if (buddyStatus->statusString)
      strzcpy(statusInfo->richStatus, buddyStatus->statusString,
              GP_RICH_STATUS_LEN);
    else
...
#ifndef GSI_UNICODE
    strzcpy(statusInfo->richStatus, buddyStatusInfo->richStatus,
            GP_RICH_STATUS_LEN);
    strzcpy(statusInfo->gameType, buddyStatusInfo->gameType,
            GP_STATUS_BASIC_STR_LEN);
    strzcpy(statusInfo->gameVariant, buddyStatusInfo->gameVariant,
            GP_STATUS_BASIC_STR_LEN);
    strzcpy(statusInfo->gameMapName, buddyStatusInfo->gameMapName,
            GP_STATUS_BASIC_STR_LEN);
#else
...
GPResult gpGetBlockedProfile(GPConnection *connection, int index,
                             GPProfile *profile) {
  GPIConnection *iconnection;
  int num;
  GPIProfile *pProfile;
