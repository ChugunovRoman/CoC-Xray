...
static void DebugCallback(GSIDebugCategory theCat, GSIDebugType theType,
  ...
  vsDebugOut(NULL, "[%s][%s] ", gGSIDebugCatStrings[theCat],
...
CGptestDlg::CGptestDlg(CWnd *pParent /*=NULL*/)
    : CDialog(CGptestDlg::IDD, pParent), m_StatusState(0), m_QueryPort(0),
      m_HostPort(0), m_RichStatus(_T("")), m_GameType(_T("")),
      m_GameVariant(_T("")), m_GameMapname(_T("")), m_KeyName(_T("")),
      m_KeyValue(_T("")), m_HostIp(_T("")), m_HostPrivateIp(_T("")),
      m_cdkey(_T("")) {
...
HCURSOR CGptestDlg::OnQueryDragIcon() { return (HCURSOR)m_hIcon; }
...
void ConnectResponse(GPConnection *connection, void *arg_, void *param) {
  ...
  if (arg->result == GP_NO_ERROR) {
    ...
    gpSetStatus(connection, (GPEnum)dlg->m_status, dlg->m_statusString,
                dlg->m_locationString);
    // gpSetStatusInfo(connection, (GPEnum)dlg->m_StatusState, 0, 0, 0, 0, 0,
    // dlg->m_RichStatus, dlg->m_RichStatus.GetLength(), "", 0, "", 0, "", 0);
    CString text;
...
void GetInfoResponse(GPConnection *connection, void *arg_, void *param) {
  ...
  if (arg->result == GP_NO_ERROR) {
    ...
    dlg->m_isex = (arg->sex == GP_MALE)
                      ? "Male"
                      : ((arg->sex == GP_FEMALE) ? "Female" : "Pat");
    ...
    dlg->m_ipmcountrycode =
        (arg->publicmask & GP_MASK_COUNTRYCODE) ? TRUE : FALSE;
...
void Error(GPConnection *connection, void *arg_, void *param) {
  ...
  if (arg->fatal == GP_FATAL) {
    MessageBox(NULL, arg->errorString, "Fatal Error",
               MB_OK | MB_ICONERROR | MB_APPLMODAL);
  } else {
    MessageBox(NULL, arg->errorString, "Error",
               MB_OK | MB_ICONERROR | MB_APPLMODAL);
  }
  GSI_UNUSED(connection);
  GSI_UNUSED(param);
...
void RecvBuddyRevoke(GPConnection *connection, void *arg_, void *param) {
  ...
  if (gpIsBuddy(connection, arg->profile)) {
    ...
    CString message;
    message.Format("User %d has revoked their buddy authorization.",
                   arg->profile);
...
void ProfileSearchResponse(GPConnection *connection, void *arg_, void *param) {
  ...
  if (arg->result == GP_NO_ERROR) {
    ...
    ////////////////////////////
    for (int i = 0; (i < arg->numMatches) &&
                    (i < (sizeof(searchMatches) / sizeof(searchMatches[0])));
         i++) {
      // Add it to the list.
      //////////////////////
    ...
    if (arg->more == GP_MORE) {
      if (dlg->MessageBox("More search results?", "Profile Search",
...
void RecvBuddyAuth(GPConnection *connection, void *arg_, void *param) {
  ...
  MessageBox(dlg->m_hWnd, msg_for_requester, "Received Buddy Auth",
             MB_ICONINFORMATION);
  GSI_UNUSED(connection);
  GSI_UNUSED(param);
}
...
void SuggestUniqueNickResponse(GPConnection *connection, void *arg_,
                               void *param) {
...
void GetReverseBuddiesResponse(GPConnection *connection, void *arg_,
                               void *param) {
  ...
  if (arg->result == GP_NO_ERROR) {
    ...
    for (i = 0; i < arg->numProfiles; i++) {
      ...
      str.Format("Revoke %s [%s] (%s %s, %s, %d)?", match->nick,
                 match->uniquenick, match->firstname, match->lastname,
                 match->email, match->profile);
...
void RecvGameInvite(GPConnection *connection, void *arg_, void *param) {
  ...
  char msg[4096];
  sprintf(msg, "%s has invited you to play the game with product ID %d", whois,
          arg->productID);
...
void GetInfoForBuddyKeys(GPConnection *theConnection, void *theArg,
                         void *theParam) {
...
void GetBuddyKeysCallback(GPConnection *theConnection, void *theArg,
                          void *theParam) {
  ...
  gpGetInfo(theConnection, anArg->profile, GP_CHECK_CACHE, GP_BLOCKING,
            GetInfoForBuddyKeys, (void *)&anInfoArg);
...
void TransferCallback(GPConnection *connection, void *arg_, void *param) {
  ...
  CString str;
  str.Format("%s - %d - %d - %s\n", TypeToString(arg->type), arg->index,
             arg->num, arg->message);
...
void CGptestDlg::CodeToString(CString &string) {
  ...
  switch (code) {
    CtoS(GP_GENERAL) CtoS(GP_PARSE) CtoS(GP_NOT_LOGGED_IN) CtoS(GP_BAD_SESSKEY) CtoS(
        GP_DATABASE) CtoS(GP_NETWORK) CtoS(GP_FORCED_DISCONNECT) CtoS(GP_CONNECTION_CLOSED)
        CtoS(GP_LOGIN) CtoS(GP_LOGIN_TIMEOUT) CtoS(GP_LOGIN_BAD_NICK) CtoS(
            GP_LOGIN_BAD_EMAIL) CtoS(GP_LOGIN_BAD_PASSWORD) CtoS(GP_LOGIN_BAD_PROFILE)
            CtoS(GP_LOGIN_PROFILE_DELETED) CtoS(GP_LOGIN_CONNECTION_FAILED) CtoS(
                GP_LOGIN_SERVER_AUTH_FAILED) CtoS(GP_LOGIN_BAD_UNIQUENICK)
                CtoS(GP_LOGIN_BAD_PREAUTH) CtoS(GP_NEWUSER) CtoS(GP_NEWUSER_BAD_NICK) CtoS(
                    GP_NEWUSER_BAD_PASSWORD) CtoS(GP_NEWUSER_UNIQUENICK_INVALID)
                    CtoS(GP_NEWUSER_UNIQUENICK_INUSE) CtoS(GP_UPDATEUI) CtoS(
                        GP_UPDATEUI_BAD_EMAIL) CtoS(GP_NEWPROFILE) CtoS(GP_NEWPROFILE_BAD_NICK)
                        CtoS(GP_NEWPROFILE_BAD_OLD_NICK) CtoS(GP_UPDATEPRO) CtoS(
                            GP_UPDATEPRO_BAD_NICK) CtoS(GP_ADDBUDDY)
                            CtoS(GP_ADDBUDDY_BAD_FROM) CtoS(GP_ADDBUDDY_BAD_NEW) CtoS(
                                GP_ADDBUDDY_ALREADY_BUDDY) CtoS(GP_AUTHADD) CtoS(GP_AUTHADD_BAD_FROM)
                                CtoS(GP_AUTHADD_BAD_SIG) CtoS(GP_STATUS) CtoS(
                                    GP_BM) CtoS(GP_BM_NOT_BUDDY) CtoS(GP_GETPROFILE)
                                    CtoS(GP_GETPROFILE_BAD_PROFILE) CtoS(
                                        GP_DELBUDDY) CtoS(GP_DELBUDDY_NOT_BUDDY)
                                        CtoS(GP_DELPROFILE) CtoS(
                                            GP_DELPROFILE_LAST_PROFILE) CtoS(GP_SEARCH)
                                            CtoS(GP_SEARCH_CONNECTION_FAILED) CtoS(
                                                GP_CHECK) CtoS(GP_CHECK_BAD_EMAIL)
                                                CtoS(GP_CHECK_BAD_NICK) CtoS(
                                                    GP_CHECK_BAD_PASSWORD)
                                                    CtoS(GP_REGISTERUNIQUENICK) CtoS(
...
void CGptestDlg::OnInitialize() {
  ...
  CHECK(
      gpInitialize(&m_connection, m_productid, atoi(m_namespace), m_partnerid));
  CHECK(gpSetCallback(&m_connection, GP_RECV_BUDDY_STATUS, RecvBuddyStatus,
                      NULL));
  ...
  CHECK(gpSetCallback(&m_connection, GP_RECV_BUDDY_MESSAGE, RecvBuddyMessage,
                      NULL));
  CHECK(gpSetCallback(&m_connection, GP_RECV_BUDDY_REQUEST, RecvBuddyRequest,
                      NULL));
  CHECK(
      gpSetCallback(&m_connection, GP_RECV_GAME_INVITE, RecvGameInvite, NULL));
  CHECK(gpSetCallback(&m_connection, GP_TRANSFER_CALLBACK, TransferCallback,
                      NULL));
  CHECK(gpSetCallback(&m_connection, GP_RECV_BUDDY_UTM, RecvBuddyUTM, NULL));
  CHECK(gpSetCallback(&m_connection, GP_RECV_BUDDY_REVOKE, RecvBuddyRevoke,
                      NULL));
}
...
void CGptestDlg::OnConnect() {
  ...
  ///////////////////////////
  SetHost();
  CHECK(gpConnect(&m_connection, m_nick, m_email, m_password,
                  (GPEnum)m_firewall, (GPEnum)m_blocking, ConnectResponse,
                  NULL));
}
...
void CGptestDlg::OnConnectunique() {
  ...
  CHECK(gpConnectUniqueNick(&m_connection, m_uniquenick, m_password,
                            (GPEnum)m_firewall, (GPEnum)m_blocking,
...
void CGptestDlg::OnConnectpreauth() {
  ...
  CHECK(gpConnectPreAuthenticated(&m_connection, m_authtoken,
                                  m_partnerchallenge, (GPEnum)m_firewall,
                                  (GPEnum)m_blocking, ConnectResponse, NULL));
}
...
void CGptestDlg::OnCheck() {
  ...
  UpdateData();
  CHECK(gpCheckUser(&m_connection, m_nick, m_email, m_password,
                    (GPEnum)m_blocking, CheckResponse, NULL));
}
...
void CGptestDlg::OnNewuser() {
  ...
  UpdateData();
  SetHost();
  CHECK(gpNewUser(&m_connection, m_nick, m_uniquenick, m_email, m_password,
                  NULL, (GPEnum)m_blocking, NewUserResponse, NULL));
  //	CHECK(gpConnectNewUser(&m_connection, m_nick, m_uniquenick, m_email,
  // m_password, NULL, (GPEnum)m_firewall, (GPEnum)m_blocking, ConnectResponse,
  // NULL));
}
...
void CGptestDlg::OnSelchangeBuddies() {
  ...
  if (index != LB_ERR) {
    ...
    //////////////////////////////
    if (profile != -1)
      CHECK(gpGetInfo(&m_connection, profile, GP_CHECK_CACHE,
                      (GPEnum)m_blocking, GetInfoResponse, NULL));
  }
}
...
void CGptestDlg::OnSet() {
  ...
  CHECK(gpSetStatus(&m_connection, (GPEnum)m_status, m_statusString,
                    m_locationString));
}
...
void CGptestDlg::OnRefresh() {
  ...
  if (index != LB_ERR) {
    ...
    // Get the info on this buddy.
    //////////////////////////////
    CHECK(gpGetInfo(&m_connection, profile, GP_DONT_CHECK_CACHE,
                    (GPEnum)m_blocking, GetInfoResponse, NULL));
  }
}
...
void CGptestDlg::OnSearch() {
  ...
  CHECK(gpProfileSearch(&m_connection, m_snick, m_suniquenick, m_semail,
                        m_sfirstname, m_slastname, m_sicquin,
                        (GPEnum)m_blocking, ProfileSearchResponse, NULL));
}
...
void CGptestDlg::OnValidate() {
  ...
  UpdateData();
  CHECK(gpIsValidEmail(&m_connection, m_semail, (GPEnum)m_blocking,
                       IsValidEmailResponse, NULL));
}
...
void CGptestDlg::OnNicks() {
  ...
  dlg->m_results.ResetContent();
  CHECK(gpGetUserNicks(&m_connection, m_semail, m_password, (GPEnum)m_blocking,
                       GetUserNicksResponse, NULL));
}
...
void CGptestDlg::OnSuggest() {
  ...
  dlg->m_results.ResetContent();
  CHECK(gpSuggestUniqueNick(&m_connection, m_suniquenick, (GPEnum)m_blocking,
                            SuggestUniqueNickResponse, NULL));
}
...
void CGptestDlg::OnSetinfo() {
  ...
  gpSetInfod(&m_connection, GP_BIRTHDAY, m_ibirthday, m_ibirthmonth,
             m_ibirthyear);
...
static void DeleteResponseCallback(GPConnection *connection, void *_arg,
                                   void *param) {
...
void CGptestDlg::OnNewpro() {
  ...
  UpdateData();
  CHECK(gpNewProfile(&m_connection, m_newnick,
                     m_replace ? GP_REPLACE : GP_DONT_REPLACE,
                     (GPEnum)m_blocking, NewProfileResponse, NULL));
}
...
void SendFilesCallback(GPConnection *connection, int index, const char **path,
...
void CGptestDlg::OnSendFiles() {
  ...
  if (index != LB_ERR) {
    ...
    ///////////////
    UpdateData();
    CHECK(gpSendFiles(&m_connection, &transfer, profile, "Want this file?",
                      SendFilesCallback, NULL));
  }
}
...
void CGptestDlg::OnRevoke() {
  ...
  CHECK(gpGetReverseBuddies(&m_connection, (GPEnum)m_blocking,
                            GetReverseBuddiesResponse, NULL));
}
...
void CGptestDlg::OnSetstatusinfo() {
  ...
  m_SessionFlags.GetSelItems(m_SessionFlags.GetSelCount(),
  aSessionFlagsSelected);
  ...
  gpSetStatusInfo(&m_connection, aStatusState, aHostIp, aHostPrivateIp,
  aQueryPort, aHostPort, sessFlags, (LPCSTR)m_RichStatus,
          m_RichStatus.GetLength(), (LPCSTR)m_GameType, m_GameType.GetLength(),
  (LPCSTR)m_GameVariant, m_GameVariant.GetLength(), (LPCSTR)m_GameMapname,
  m_GameMapname.GetLength()); delete[] aSessionFlagsSelected;
  */
}
...
void CGptestDlg::OnAddSetKey() {
  ...
  if (keyValue)
          gpSetStatusInfoKey(&m_connection, (LPCSTR)m_KeyName,
  (LPCSTR)m_KeyValue); else gpAddStatusInfoKey(&m_connection, (LPCSTR)m_KeyName,
  (LPCSTR)m_KeyValue);
  */
}
...
void CGptestDlg::OnGetBuddyKeys() {
  ...
  if(index != LB_ERR)
  {
          ...
          CHECK(gpGetBuddyStatusInfoKeys(&m_connection, bIndex,
  GetBuddyKeysCallback, NULL));
  }
  */
}
...
void CGptestDlg::OnRegisterCdKey() {
  ...
  // Register CDkey
  CHECK(gpRegisterCdKey(&m_connection, (LPCSTR)m_cdkey, GP_BLOCKING,
                        RegisterCdKeyCallback, NULL));
}
