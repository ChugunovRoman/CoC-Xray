    ...
    while ((next = strstr(iconnection->socketBuffer.buffer, "\\final\\")) !=
      ...
      //////////////////////////////////
        ...
        if (gpiCheckForError(connection, iconnection->inputBuffer, GPITrue)) {
          ...
          // Process the buddy list - retrieved upon login before final login
          // response
          // * Note: this only gets the list of your buddies so at least you'll
          // know who is a buddy while the status of each is asynchronously
          // updated.
          //////////////////////////////////////////////////////////////////////////////
          CHECK_RESULT(
              gpiProcessRecvBuddyList(connection, iconnection->inputBuffer));
        } else if (strncmp(iconnection->inputBuffer, "\\blk\\", 5) == 0) {
          // Process the block list - retrieved upon login before final login
          // response
          //////////////////////////////////////////////////////////////////////////////
          CHECK_RESULT(
              gpiProcessRecvBlockedList(connection, iconnection->inputBuffer));
        } else {
...
GPResult gpiProcess(GPConnection *connection, int blockingOperationID) {
  ...
  */
  ...
  if ((iconnection->connectState == GPI_CONNECTED) ||
      ...
      (iconnection->connectState == GPI_PROFILE_DELETING)) {
#ifdef _PS3
    // initialize NP during the sync delay, if initialized wait for status ==
    // online
    ////////////////////////////////////////////////////////////////////////////////
...
// 2); 		if(profile->buddyStatus->locationString_W) 			temp
