...
static GPResult gpiOpenDiskProfiles(GPConnection *connection, GPIBool write,
                                    GPIBool *failed) {
  FILE *fp = NULL;
  GPIConnection *iconnection = (GPIConnection *)*connection;
...
static GPResult gpiReadDiskKeyValue(GPConnection *connection, GPIBool *failed,
                                    char key[512], char value[512]) {
  int c;
  FILE *fp;
  GPIConnection *iconnection = (GPIConnection *)*connection;
  int i;
...
static GPIBool gpiSaveDiskProfile(GPConnection *connection, GPIProfile *profile,
                                  void *data) {
  FILE *fp;
  GPIConnection *iconnection = (GPIConnection *)*connection;
...
GPIBool gpiCanFreeProfile(GPIProfile *profile) {
  return ((profile && !profile->cache && !profile->buddyStatus &&
           !profile->buddyStatusInfo && !profile->peerSig &&
...
GPResult gpiAddToBlockedList(GPConnection *connection, int profileid) {
...
#ifndef _PS2
      gpiProfileMap(connection, gpiFixBuddyIndices,
...
#ifdef _PS3
    // Only perform if profile isn't already locally blocked and the sync isnt
    // taking place
    ///////////////////////////////////////////////////////////////////////////////////////
  ...
  // NOTE: There is no callback for this function simply in order to remain
  // consistent with the existing GP structure. If an error occurs, it gets
  // passed to the error callback else its considered the request was processed
  // correctly.

  // Add to outgoing buffer - have server handle error check if block already
  // exists (consistency)
  /////////////////////////////////////////////////////////////////////////////////////////////////
  gpiAppendStringToBuffer(connection, &iconnection->outputBuffer,
                          "\\addblock\\\\sesskey\\");
...
static GPIBool gpiFixBlockIndices(GPConnection *connection, GPIProfile *profile,
                                  void *data) {
#ifndef _PS2
...
static GPIBool gpiCheckForBlock(GPConnection *connection, GPIProfile *profile,
                                void *udata) {
  GPIFindProfileData *data = (GPIFindProfileData *)udata;
...
GPResult gpiProcessRecvBlockedList(GPConnection *connection,
  ...
  if (!gpiValueForKeyWithIndex(input, "\\blk\\", &index, buffer,
                               sizeof(buffer)))
    CallbackFatalError(connection, GP_NETWORK_ERROR, GP_PARSE,
                       "Unexpected data was received from the server.");
  num = atoi(buffer);
