...
void ReceivedCallback(GT2Connection connection, GT2Byte *message, int len,
  ...
  if (type == MSG_CHALLENGE_TYPE) {
    ...
    if (gtDecode(MSG_COUNTDOWN, (char *)message, len, &Dlg->m_event,
                 &Dlg->m_countdown) == -1) {
      Dlg->m_state = JOIN_ERROR;
      return;
    }
...
BOOL CMultiTrackDlg::SetupHosting() {
  ...
  GT2Result aResult =
      gt2CreateSocket(&m_socket, HOST_PORT_STRING, 0, 0, SocketErrorCallback);
...
BOOL CMultiTrackDlg::SetupJoining() {
  ...
  // Create the socket
  GT2Result aResult =
      gt2CreateSocket(&m_socket, CLIENT_PORT_STRING, 0, 0, SocketErrorCallback);
  ...
  aResult = gt2Connect(m_socket, &m_connection, remoteAddress,
...
GHTTPBool PlayerRatingsPageCompleted(GHTTPRequest request, GHTTPResult result,
  ...
  if (result == GHTTPSuccess) {
    ...
    // Scan out ratings.
    ////////////////////
    rcode = sscanf(buffer, "%d %d %d %d", &ratings[0], &ratings[1], &ratings[2],
                   &ratings[3]);
    if (rcode == 4) {
      if (localPlayer)
...
BOOL CMultiTrackDlg::SetupMatch() {
  ...
  CString url;
  url.Format(
      "http://sdkdev.gamespy.com/games/st_rank/web/playerratings.asp?pid=%d",
...
HCURSOR CMultiTrackDlg::OnQueryDragIcon() { return (HCURSOR)m_hIcon; }
...
int CalculateHandicap(int rating1, int rating2) {
  static int htable[] = {12,  34,  56,  78,  101, 126, 151, 177,
                         206, 239, 273, 315, 366, 446, 471, 715};
  int diff;
  int i;
...
void CMultiTrackDlg::UpdateStats() {
  ...
  handicap =
      CalculateHandicap(m_localRatings[m_event], m_remoteRatings[m_event]);
  if (handicap) {
...
void CMultiTrackDlg::Logout() { OnLogout(); }
...
void CMultiTrackDlg::Countdown() {
  if (m_hosting) {
    ...
    rcode = gtEncode(MSG_COUNTDOWN_TYPE, MSG_COUNTDOWN, message,
                     sizeof(message), m_event, m_countdown);
    ASSERT(rcode != -1);
