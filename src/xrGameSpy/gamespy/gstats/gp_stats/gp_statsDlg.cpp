...
BOOL CGp_statsDlg::CreateAccount() {
  ...
  // Create the account.
  //////////////////////
  GPResult result = gpNewUser(&m_gp, m_nick, NULL, m_email, m_password, NULL,
                              GP_BLOCKING, CreateAccountCallback, &pid);
...
BOOL CGp_statsDlg::CheckAccount() {
  ...
  /////////////////////////
  GPResult result = gpCheckUser(&m_gp, m_nick, m_email, m_password, GP_BLOCKING,
                                CheckAccountCallback, &pid);
  if (result != GP_NO_ERROR) {
  ...
  if (errorCode == GP_CHECK_BAD_EMAIL) {
    ...
    //////////////////////////////////////////
    rcode = MessageBox("This account does not exist.\n"
    ...
    rcode = MessageBox("There are no profiles under this account with the nick "
                       "you have entered\n"
...
void StatsAuthenticationCallback(int localid, int profileid, int authenticated,
  ...
  *result = authenticated;
  ...
  if (authenticated != 1)
    MessageBox(NULL, errmsg, "Error authenticating with the stats backend",
               MB_OK);
...
BOOL CGp_statsDlg::StatsAuthentication() {
  ...
  PreAuthenticatePlayerPM(
      ...
      StatsAuthenticationCallback, &result);
...
void GetDataCallback(int localid, int profileid, persisttype_t type, int index,
                     int success, time_t modified, char *data, int len,
                     void *instance) {
...
void CGp_statsDlg::OnGet() {
  ...
  GetPersistDataValues(0, m_profileID, TypeConversion(m_type), 0, "",
...
void SetDataCallback(int localid, int profileid, persisttype_t type, int index,
                     int success, time_t modified, void *instance) {
...
void CGp_statsDlg::OnSet() {
  ...
  SetPersistDataValues(0, m_profileID, TypeConversion(m_type), 0,
                       (char *)(LPCSTR)data, SetDataCallback, this);
