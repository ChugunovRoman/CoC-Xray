...
static gsi_char *CreateSnapShotFromData(gsi_char *snapshot,
                                        serverinfo_t *psinfo,
...
int test_main(int argc, char **argv) {
  ...
  ******************/
  ...
  The first example will not be bucket based, we'll generate the snapshot
  ourselves.
  **********/
  ...
  ********/
  ...
  _tcscpy(players[sinfo.numplayers].auth,
  ...
  ********/
  ...
  BucketStringOp(NULL, "auth", bo_set, "7cca8e60a13781eebc820a50754f57cd",
                 bl_player, 0);

  NewPlayer(NULL, 1, _T("Joey!"));
  BucketIntOp(NULL, "pid", bo_set, 643423, bl_player, 1);
  BucketStringOp(NULL, "auth", bo_set, "19ea14d9d92a7fcc635cf5716944d9bc",
                 bl_player, 1);
  ...
  *********/
  ...
  BucketStringOp(g1, "auth", bo_set, "7cca8e60a13781eebc820a50754f57cd",
                 bl_player, 0);

  NewPlayer(g1, 1, _T("Joey"));
  BucketIntOp(g1, "pid", bo_set, 643423, bl_player, 1);
  BucketStringOp(g1, "auth", bo_set, "19ea14d9d92a7fcc635cf5716944d9bc",
                 bl_player, 1);
  ...
  ********/
  ...
  BucketStringOp(g1, "auth", bo_set, "6a7d3fee261eb3db7f94f0f02c2c756b",
                 bl_player, 2);
  ...
  BucketStringOp(g1, "auth", bo_set, "9e67d812bde48a6a29ee425c03b26894",
                 bl_player, 0);
  BucketIntOp(g1, "team", bo_set, GetTeamIndex(g1, 0), bl_player, 0);
  ...
  NewPlayer(g1, 1,
            _T("Killer")); /* you can have two players with the same name */
  BucketIntOp(g1, "pid", bo_set, 32323, bl_player, 1);
  BucketStringOp(g1, "auth", bo_set, "dd975866a2c346814433e523bf8eda0e",
                 bl_player, 1);
  ...
  ***********/
  ...
  BucketStringOp(g2, "auth", bo_set, "eea4c47e3d4a2f68dffab1b084e382b7",
                 bl_player, 0);
  ...
  BucketStringOp(g2, "auth", bo_set, "d837be1371484bcbde9885dfc384b861",
                 bl_player, 1);
  ...
  BucketStringOp(g2, "auth", bo_set, "417f3e186ca5ae0fa32665db3ddf54c8",
                 bl_player, 2);
  ...
  BucketIntOp(g2, va("kills%d", GetPlayerIndex(g2, 2)), bo_add, 1, bl_player,
              0);
  BucketIntOp(g2, "score", bo_add, 1, bl_player, 2);
  BucketIntOp(g2, "deaths", bo_add, 1, bl_player, 1);
  BucketIntOp(g2, va("kills%d", GetPlayerIndex(g2, 1)), bo_add, 1, bl_player,
              2);
  ...
  ***********/
  ...
  BucketIntOp(g2, va("kills%d", GetPlayerIndex(g2, 0)), bo_add, 1, bl_player,
              3);
  BucketIntOp(g2, "score", bo_add, 1, bl_player, 3);
  BucketIntOp(g2, "deaths", bo_add, 1, bl_player, 0);
  BucketIntOp(g2, va("kills%d", GetPlayerIndex(g2, 0)), bo_add, 1, bl_player,
...
static gsi_char *CreateSnapShotFromData(gsi_char *snapshot,
  ...
  _tsnprintf(snapshot, SNAPSHOT_LEN,
             _T("\\hostname\\%s\\mapname\\%s\\gametype\\%s\\gamever\\%f"),
             psinfo->hostname, psinfo->mapname, psinfo->gametype,
             psinfo->version);
  for (i = 0; i < psinfo->numplayers; i++) {
    len = _tcslen(snapshot);
    _tsnprintf(snapshot + len, SNAPSHOT_LEN - len,
               _T("\\player_%d\\%s\\points_%d\\%d\\deaths_%d\\%d\\pid_%d\\%")
               _T("d\\auth_%d\\%s"),
               i, players[i].name, i, players[i].points, i, players[i].deaths,
               i, players[i].profileid, i, players[i].auth);
  }
  return snapshot;
}
