...
void ReceivedCallback(GT2Connection connection, GT2Byte *message, int len,
  ...
  if (type == MSG_CHALLENGE_TYPE) {
    ...
    GenerateAuth(challenge, (char *)(LPCSTR)Dlg->m_loginDlg.m_password,
                 response);
    ...
    char message[64];
    int rcode;
    rcode = gtEncode(MSG_RESPONSE_TYPE, MSG_RESPONSE, message, sizeof(message),
                     response);
    ASSERT(rcode != -1);
...
BOOL CLadderTrackDlg::SetupJoining() {
  ...
  rcode = gtEncodeNoType(MSG_CONNECT, buffer, sizeof(buffer),
                         m_loginDlg.m_profile, m_loginDlg.m_nick);
  ASSERT(rcode != -1);
...
GHTTPBool PlayerPositionPageCompleted(GHTTPRequest request, GHTTPResult result,
                                      char *buffer, __int64 bufferLen,
                                      void *param) {
...
void CLadderTrackDlg::UpdatePlayerPositions() {
  CString url;
  url.Format(
      "http://sdkdev.gamespy.com/games/st_ladder/web/playerposition.asp?pid=%d",
      m_loginDlg.m_profile);
...
HCURSOR CLadderTrackDlg::OnQueryDragIcon() { return (HCURSOR)m_hIcon; }
    ...
    if (!thinking) {
      ...
      if (m_state == HOST_CHALLENGING) {
        if (!m_challenged) {
          ...
          /////////////////////////////////////////////
          rcode = gtEncode(MSG_CHALLENGE_TYPE, MSG_CHALLENGE, buffer,
                           sizeof(buffer), m_loginDlg.m_profile,
                           (LPCSTR)m_challenge);
          ASSERT(rcode != -1);
...
void CLadderTrackDlg::Logout() { OnLogout(); }
...
void CLadderTrackDlg::ReportStats() {
  ...
  ////////
  NewGame(1);
  BucketStringOp(NULL, "hostname", bo_set, (char *)(LPCSTR)m_loginDlg.m_nick,
                 bl_server, 0);
  ...
  GenerateAuth((char *)(LPCSTR)m_challenge,
               (char *)(LPCSTR)m_loginDlg.m_password, response);
  ...
  BucketStringOp(NULL, "auth", bo_set, (char *)(LPCSTR)m_remoteResponse,
                 bl_player, 1);
...
FakeProfile FakeProfiles[] = {{4933555, "mrpants", "ABCDEF", 4500, 6000},
...
void CLadderTrackDlg::FakeStats() {
  ...
  for (i = 0; i < total; i++) {
    ...
    m_localTime =
        (DWORD)(FakeProfiles[p1].minTime +
                ((rand() / (float)RAND_MAX) *
                 (FakeProfiles[p1].maxTime - FakeProfiles[p1].minTime)));

    m_remoteNick = FakeProfiles[p2].nick;
    m_remoteProfile = FakeProfiles[p2].pid;
    GenerateAuth((char *)(LPCSTR)m_challenge, FakeProfiles[p2].password,
                 response);
    m_remoteResponse = response;
    m_remoteTime =
        (DWORD)(FakeProfiles[p2].minTime +
                ((rand() / (float)RAND_MAX) *
                 (FakeProfiles[p2].maxTime - FakeProfiles[p2].minTime)));
...
void CLadderTrackDlg::OnUpdatePositions() { UpdatePlayerPositions(); }
