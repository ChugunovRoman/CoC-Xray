...
GHTTPBool PlayerBestTimesPageCompleted(GHTTPRequest request, GHTTPResult result,
  ...
  if (result == GHTTPSuccess) {
    ...
    ///////////////////////////
    rcode =
        sscanf(buffer, "%d %d %d", &bestTimes[0], &bestTimes[1], &bestTimes[2]);
    if (rcode == 3) {
      Dlg->UpdateData();
...
GHTTPBool TopTimePageCompleted(GHTTPRequest request, GHTTPResult result,
                               char *buffer, GHTTPByteCount bufferLen,
                               void *param) {
  if (result == GHTTPSuccess) {
...
void CTrackDlg::SetupUser() {
  CString title;
  title.Format("track - best time (%s - %s - %d)", m_loginDlg.m_email,
               m_loginDlg.m_nick, m_loginDlg.m_profile);
  SetWindowText(title);

  CString url;
  int event;
  url.Format(
      "http://sdkdev.gamespy.com/games/st_highscore/web/playertimes.asp?pid=%d",
      m_loginDlg.m_profile);
  ghttpGet(url, GHTTPFalse, PlayerBestTimesPageCompleted, NULL);
  for (event = 1; event <= 3; event++) {
    url.Format("http://sdkdev.gamespy.com/games/st_highscore/web/top_%d.txt",
               event);
...
HCURSOR CTrackDlg::OnQueryDragIcon() { return (HCURSOR)m_hIcon; }
...
void CTrackDlg::ReportStats(DWORD time) {
  ...
  GenerateAuth(GetChallenge(NULL), (char *)(LPCSTR)m_loginDlg.m_password,
               response);
