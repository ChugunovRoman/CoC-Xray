...
static int ghiSslCertValidator(struct sslCertInfo *theCertInfo,
                               void *theUserData) {
...
GHIEncryptionResult ghiEncryptorSslInitFunc(struct GHIConnection *connection,
  ...
  matrixSslSetCertValidator((ssl_t *)theEncryptor->mInterface,
                            ghiSslCertValidator, NULL);
...
GHIEncryptionResult ghiEncryptorSslInitFunc(struct GHIConnection *connection,
  ...
  ecodeResult = matrixSslEncodeClientHello((ssl_t *)theEncryptor->mInterface,
                                           &helloWrapper, 0); // 0 = cipher
  if (ecodeResult != 0)
...
GHIEncryptionResult ghiEncryptorSslEncryptFunc(
  ...
  encodeResult = matrixSslEncode(connection->encryptor.mInterface,
                                 (unsigned char *)thePlainTextBuffer,
  ...
  if (encodeResult == SSL_ERROR)
    ...
    //*thePlainTextLength = *thePlainTextLength; // we always use the entire
    // buffer
...
GHIEncryptionResult ghiEncryptorSslDecryptFunc(
  ...
  while (decryptMore != GHTTPFalse && ((inBuf.end - inBuf.start) > 0)) {
    ...
    decodeResult =
        matrixSslDecode(theEncryptor->mInterface, &inBuf, &decryptedBuf, &error,
    ...
    case SSL_SEND_RESPONSE: {
      ...
      // Remove the bytes from the decrypted buffer (we don't want to return
      // them to the app)
...
GHIEncryptionResult ghiEncryptorSslInitFunc(struct GHIConnection *connection,
...

  {
    ...
    rcode =
        SSLSetBuiltinClientCert(sslInterface->mId, SSL_CLIENTCERT_NINTENDO_0);
    if (rcode != SSL_ENONE) {
      SSLShutdown(sslInterface->mId);
      return GHIEncryptionResult_Error;
    }
...
static GHTTPBool ghiEncryptorReadNBOLength(GHIBuffer *data, int *value,
  ...
  if (size > sizeof(*value))
    return GHTTPFalse;
  if (GHTTPFalse == ghiReadDataFromBufferFixed(
                        data, ((char *)value) + (sizeof(int) - size), size))
    return GHTTPFalse;
...
static void ghiEncryptorGenerateEncryptionKeys(gsSSL *sslInterface) {
  ...
  MD5Final(
      (unsigned char *)&sslInterface->mastersecret[0 * GS_CRYPT_MD5_HASHSIZE],
      &md5);
  ...
  MD5Update(&md5, temp, GS_CRYPT_SHA1_HASHSIZE);
  MD5Final(
      (unsigned char *)&sslInterface->mastersecret[1 * GS_CRYPT_MD5_HASHSIZE],
      &md5);
  ...
  MD5Update(&md5, temp, GS_CRYPT_SHA1_HASHSIZE);
  MD5Final(
      (unsigned char *)&sslInterface->mastersecret[2 * GS_CRYPT_MD5_HASHSIZE],
      &md5);
...
GHIEncryptionResult ghiEncryptorSslInitFunc(struct GHIConnection *connection,
  ...
  // Make sure the send buffer is large enough for the SSL handshake (handshake
  // is <1k)
...
ghiEncryptorSslStartFunc(struct GHIConnection *connection,
  ...
  ghiEncryptorWriteNBOLength(
      helloMsg.header.lengthNBO,
      sizeof(gsSSLClientHelloMsg) - sizeof(gsSSLRecordHeaderMsg), 2);
  ...
  ghiEncryptorWriteNBOLength(
      helloMsg.lengthNBO,
      sizeof(gsSSLClientHelloMsg) - sizeof(gsSSLRecordHeaderMsg) - 4, 3);
  // ghttpEncryptorSetNBOBytesFromHBOInt(helloMsg.time, (gsi_u32)current_time(),
  // 4); // 4 byte time value (for randomness)
...
#pragma message(                                                               \
    "!!!WARNING: SSL Random disable for debug purposes.  SSL not secured!!!")
  ...
  if (theEncryptor != NULL) {
    ...
    if (sslInterface == NULL ||
      ...
      *theEncryptedLength += thePlainTextLength; // number of bytes written
      ...
      MD5Update(&md5, (unsigned char *)"\x17",
                1); // content type application data
...
GHIEncryptionResult ghiEncryptorSslDecryptFunc(
  ...
  while (readPos < *theEncryptedLength) {
    ...
    //    BAD!! -> length = ntohs((header->lengthNBO[0] | (header->lengthNBO[1]
    //    << 8)));
    // length = (unsigned short)(header->lengthNBO[0] | (header->lengthNBO[1] <<
    // 8));
...
ghiEncryptorProcessSSLHandshake(struct GHIConnection *connection,
  ...
  while (data->pos < data->len) {
    ...
    if (messageType == GS_SSL_HANDSHAKE_SERVERHELLO) {
      ...
      int msgLength =
          0; // combined length of the message (size in SSL message header)
      ...
      while (data->pos < certListEndPos) {
        ...
        // if (certCopy == NULL)
        //{
        //	gsDebugFormat(GSIDebugCat_HTTP, GSIDebugType_Memory,
        // GSIDebugLevel_WarmError, 		"SSL failed to allocate
        // certificate #%d (out of memory)\r\n", certCount); 	return
        ...
        if (certCount == 1) {
          ...
          //    0x30 marks the start of a sequence.  next byte is a length field
          //    size 0x82 is a length tag, meaning the next two bytes contain
          //    the length 0x81 is the same thing, only the next one byte
          //    contains the length The other values usually denote required
          //    types
          ...
          temp =
              (unsigned char)data
                  ->data[data->pos++]; // remaining data size (check or ignore)
          ...
          if ((data->data[data->pos] & 0x80) ==
              0x80) // ASN1 variable length field
          {
          ...
          sslInterface->serverpub.modulus.mLength =
              (unsigned int)((temp - 1) /
                             GS_LARGEINT_DIGIT_SIZE_BYTES); //-1 to subtract the
                                                            // previous 0x00
                                                            // byte
...
#pragma message(                                                               \
    "!!!WARNING: SSL Random disable for debug purposes.  SSL not secured!!!")
      ...
      clientKeyExchange = (gsSSLClientKeyExchangeMsg *)&connection->sendBuffer
      ...
      changeCipherSpec = (gsSSLRecordHeaderMsg *)&connection->sendBuffer
                             .data[connection->sendBuffer.len];
      ...
      finalHandshake = (gsSSLRecordHeaderMsg *)&connection->sendBuffer
                           .data[connection->sendBuffer.len];
      ...
      ghiEncryptorWriteNBOLength((unsigned char *)&connection->sendBuffer
                                     .data[connection->sendBuffer.len],
                                 36, 3);
      connection->sendBuffer.len += 3;

      // MD5(master_secret + pad2 +
      // MD5(handshake_messages+"CLNT"+master_secret+pad1)) SHA1(master_secret +
      // pad2 + SHA1(handshake_messages+"CLNT"+master_secret+pad1)) prepare the
      // final hashes (inner hashes)
      MD5Update(&sslInterface->finishHashMD5,
      ...
      // hash(MAC_write_secret+pad_1+seq_num+length+content))); Re-using the
      // finishHashMD5 since it has already been allocated
      MD5Init(&sslInterface->finishHashMD5);
      MD5Update(&sslInterface->finishHashMD5,
      ...
      temp[0] = 0x16;
      temp[1] = (unsigned char)((GS_CRYPT_MD5_HASHSIZE +
                                 GS_CRYPT_SHA1_HASHSIZE + 4) >>
                                8);
      temp[2] =
          (unsigned char)((GS_CRYPT_MD5_HASHSIZE + GS_CRYPT_SHA1_HASHSIZE + 4));
      // temp[1] = (unsigned
      // char)(htons(GS_CRYPT_MD5_HASHSIZE+GS_CRYPT_SHA1_HASHSIZE)); temp[2] =
      // (unsigned
      // char)(htons(GS_CRYPT_MD5_HASHSIZE+GS_CRYPT_SHA1_HASHSIZE+4)>>8);
