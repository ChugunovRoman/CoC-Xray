...
void gviSpursSpeexEncoderInitialize(SpursSpeexTaskOutput *spuOutput) {
  ...
  if (sampleRate == GVRate_8KHz)
    gviSpeexEncoderState =
        speex_encoder_init(&speex_nb_mode, gviSpursSpeexStateBuffer);
  else if (sampleRate == GVRate_16KHz)
    gviSpeexEncoderState =
        speex_encoder_init(&speex_wb_mode, gviSpursSpeexStateBuffer);
  else {
  ...
  speex_encoder_ctl(gviSpeexEncoderState, SPEEX_GET_FRAME_SIZE,
                    &spuOutput->mSpeexSamplesPerFrame);

  // set the quality
  speex_encoder_ctl(gviSpeexEncoderState, SPEEX_SET_QUALITY, &quality);

  // (re)initialize the bits struct
  speex_bits_init_buffer(&gviSpursSpeexBits, gviSpursSpeexBitsBuffer,
                         sizeof(gviSpursSpeexBitsBuffer));
  ...
  // convert to bytes per frame and store, round up to allocate more space than
  // needed.
...
void gviSpursSpeexEncode(SpursSpeexTaskOutput *spuTaskOut) {
  ...
  speexBuffer = (float *)memalign(16, gviSpursSpeexTaskDesc.mInputBufferSize *
                                          sizeof(float));
  inBuffer = (short *)memalign(16, gviSpursSpeexTaskDesc.mInputBufferSize *
                                       sizeof(short));
  outBuffer = (char *)memalign(16, gviSpursSpeexTaskDesc.mOutputBufferSize);

  memset(speexBuffer, 0,
         gviSpursSpeexTaskDesc.mInputBufferSize * sizeof(float));
  ...
  cellDmaGet(inBuffer, (uint64_t)gviSpursSpeexTaskDesc.mInputBuffer,
             gviSpursSpeexTaskDesc.mInputBufferSize * sizeof(short), DMA_TAG(1),
             0, 0);
  ...
  spuTaskOut->mSpeexOutBufferSize =
      speex_bits_write(&gviSpursSpeexBits, (char *)outBuffer,
                       gviSpursSpeexTaskDesc.mEncodedFrameSize);
  // spuDebugPrintf("[Speex][SPU] transferring data back, output size should be:
  // %d\n",
  // gviSpursSpeexTaskDesc.mOutputBufferSize>16?gviSpursSpeexTaskDesc.mOutputBufferSize:16);
  cellDmaPut(outBuffer, (uint64_t)gviSpursSpeexTaskDesc.mOutputBuffer,
             gviSpursSpeexTaskDesc.mOutputBufferSize, DMA_TAG(1), 0, 0);
  cellDmaWaitTagStatusAll(DMA_MASK(1));
...
void gviSpursSpeexDecodeAdd(SpursSpeexTaskOutput *spuTaskOut) {
  ...
  speexBuffer = (float *)memalign(16, gviSpursSpeexTaskDesc.mOutputBufferSize *
                                          sizeof(float));
  outBuffer = (short *)memalign(16, gviSpursSpeexTaskDesc.mOutputBufferSize *
                                        sizeof(short));
  inBuffer = (char *)memalign(16, gviSpursSpeexTaskDesc.mInputBufferSize);

  memset(speexBuffer, 0,
         gviSpursSpeexTaskDesc.mOutputBufferSize * sizeof(float));
  memset(outBuffer, 0, gviSpursSpeexTaskDesc.mOutputBufferSize);
  memset(inBuffer, 0, gviSpursSpeexTaskDesc.mInputBufferSize * sizeof(short));

  // spuDebugPrintf("[Speex][SPU] done allocating, getting input data, inbuffer
  // size: %d\n", gSpuSampleTaskDesc.mInputBufferSize);
  cellDmaGet(inBuffer, (uint64_t)gviSpursSpeexTaskDesc.mInputBuffer,
             gviSpursSpeexTaskDesc.mInputBufferSize, DMA_TAG(1), 0, 0);
  cellDmaWaitTagStatusAll(DMA_MASK(1));
  // spuDebugPrintf("[Speex][SPU] done getting input data, preparing for speex
  // to decode\n"); read the data into the bits (re)initialize the bits struct
  speex_bits_init_buffer(&gviSpursSpeexBits, gviSpursSpeexBitsBuffer,
                         sizeof(gviSpursSpeexBitsBuffer));

  speex_bits_read_from(&gviSpursSpeexBits, (char *)inBuffer,
                       gviSpursSpeexTaskDesc.mEncodedFrameSize);

  // decode it
  rcode = speex_decode((void *)gviSpursSpeexStateBuffer, &gviSpursSpeexBits,
                       speexBuffer);
  assert(rcode == 0);
  ...
  cellDmaPut(outBuffer, (uint64_t)gviSpursSpeexTaskDesc.mOutputBuffer,
             gviSpursSpeexTaskDesc.mOutputBufferSize * sizeof(short),
             DMA_TAG(1), 0, 0);
  cellDmaWaitTagStatusAll(DMA_MASK(1));
  // spuDebugPrintf("[Speex][SPU] done transferring data back\n");
  free(speexBuffer);
...
void procesEncodeInit(unsigned int uiPtr) {
  ...
  cellDmaGet(&gviSpursSpeexTaskDesc, uiPtr, sizeof(SpursSpeexTaskDesc),
             DMA_TAG(1), 0, 0);
  cellDmaWaitTagStatusAll(DMA_MASK(1));
  ...
  // spuDebugPrintf("[Speex][SPU] done with initializing things for speex, now
  // returning data via DMA put\n");

  // printGlobalTaskDescData();

  cellDmaPut(&spuOutput, (uint64_t)gviSpursSpeexTaskDesc.mSpeexTaskOutput,
             sizeof(SpursSpeexTaskOutput), DMA_TAG(1), 0, 0);
  cellDmaWaitTagStatusAll(DMA_MASK(1));

  // spuDebugPrintf("[Speex][SPU] task dma done\n");

  cellDmaLargePut(gviSpursSpeexStateBuffer,
                  (uint64_t)gviSpursSpeexTaskDesc.mSpeexStateBuffer,
                  SPEEX_ENCODER_STATE_BUFFER_SIZE, DMA_TAG(1), 0, 0);
  cellDmaWaitTagStatusAll(DMA_MASK(1));
...
void processDecodeInit(unsigned int uiPtr) {
  ...
  cellDmaLargePut(gviSpursSpeexStateBuffer,
                  (uint64_t)gviSpursSpeexTaskDesc.mSpeexStateBuffer,
                  gviSpursSpeexTaskDesc.mSpeexStateBufferSize, DMA_TAG(1), 0,
                  0);
  cellDmaWaitTagStatusAll(DMA_MASK(1));
...
void processEncode(unsigned int uiPtr) {
  ...
  cellDmaLargeGet(gviSpursSpeexStateBuffer,
                  (uint64_t)gviSpursSpeexTaskDesc.mSpeexStateBuffer,
                  SPEEX_ENCODER_STATE_BUFFER_SIZE, DMA_TAG(1), 0, 0);
  cellDmaWaitTagStatusAll(DMA_MASK(1));
...
void processDecodeAdd(unsigned int uiPtr) {
  ...
  cellDmaLargePut(gviSpursSpeexStateBuffer,
                  (uint64_t)gviSpursSpeexTaskDesc.mSpeexStateBuffer,
                  SPEEX_DECODER_STATE_BUFFER_SIZE, DMA_TAG(1), 0, 0);
  cellDmaWaitTagStatusAll(DMA_MASK(1));
...
void cellSpursMain(qword argTask, uint64_t argTaskset) {
  ...
  default: {
    // spuDebugPrintf("SPURS Sample:unknown case in switch uiCommand: %x uiArg1
