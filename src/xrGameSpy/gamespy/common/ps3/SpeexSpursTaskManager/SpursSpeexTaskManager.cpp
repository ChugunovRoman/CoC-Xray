...
SpursSpeexTaskManager::SpursSpeexTaskManager(
    spursThreadSupportInterface *threadInterface,
...
int SpursSpeexTaskManager::issueEncodeInitTask(
    int theQuality, int theGviSpeexSamplesPerSecond,
    SpursSpeexTaskOutput *taskOutput, char *userAllocatedSpeexBuffer,
...
int SpursSpeexTaskManager::issueEncodeTask(int16_t *inBuffer, int inBufferSize,
...
int SpursSpeexTaskManager::issueDecodeAddTask(
    char *decoderStateBuffer, int decoderStateBufferSize, char *inBuffer,
    int inBufferSize, int encodedFrameSize, short *outBuffer, int outBufferSize,
...
int SpursSpeexTaskManager::issueDecodeSetTask(
    char *decoderStateBuffer, int decoderStateBufferSize, char *inBuffer,
    int inBufferSize, int encodedFrameSize, short *outBuffer, int outBufferSize,
    struct SpursSpeexTaskOutput *taskOutput) {
  m_taskBusy[m_currentTask] = true;
...
int SpursSpeexTaskManager::issueDecodeInitTask(
    char *decoderStateBuffer, int decoderStateBufferSize, int sampleRate,
    struct SpursSpeexTaskOutput *taskOutput) {
  m_taskBusy[m_currentTask] = true;
  m_numBusyTasks++;
...
int SpursSpeexTaskManager::issueTask(SpursSpeexTaskDesc &taskDesc,
                                     uint32_t uiCommand) {
#ifdef DEBUG_SPU_TASK_SCHEDULING
  printf("SpuSampleTaskProcess::issueTask (m_currentTask= %d\)n",
         m_currentTask);
#endif // DEBUG_SPU_TASK_SCHEDULING
  ...
  if (m_threadInterface->sendRequest(uiCommand, (uint32_t)&taskDesc,
                                     m_currentTask) != 0) {
