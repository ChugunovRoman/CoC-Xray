...
static MenuScreenConfiguration msCheckingBackendAvailability = {
    "Checking Backend Availability",  {{mscCancel}},
    CheckingBackendAvailabilityInit,  CheckingBackendAvailabilityChose,
    CheckingBackendAvailabilityThink, SCREEN_OPTION_ANIMATED};

static void BackendUnavailableChose(const char *choice);

static MenuScreenConfiguration msBackendUnavailable = {
    "Backend Unavailable", {{mscExit}}, NULL, BackendUnavailableChose};
...
static MenuScreenConfiguration msHostingServer = {
    "Hosting Server",   {{mscStopHosting}}, HostingServerInit,
    HostingServerChose, HostingServerThink, SCREEN_OPTION_EXTRAS_CENTERED};
...
static MenuScreenConfiguration msViewHostInfo = {
    "View Host Info",  {{mscBack}}, ViewHostInfoInit,
    ViewHostInfoChose, NULL,        SCREEN_OPTION_EXTRAS_CENTERED};
...
static char FailureReason[MAX_EXTRA_TEXT_STRINGS]
                         [MAX_EXTRA_TEXT_STRING_LEN + 1];
...
static void DebugCallback(GSIDebugCategory category, GSIDebugType type,
                          ...
                          va_list params) {
  // Output line prefix
  Printf("[%s][%s][%s] ", gGSIDebugCatStrings[category],
         gGSIDebugTypeStrings[type], gGSIDebugLevelStrings[DebugLog2(level)]);
...
static void KeyListCallback(PEER peer, qr2_key_type type,
                            qr2_keybuffer_t buffer, void *param) {
  switch (type) {
  case key_server:
...
static void ServerKeyCallback(PEER peer, int key, qr2_buffer_t buffer,
                              void *param) {
  switch (key) {
  case HOSTNAME_KEY:
...
static void AddErrorCallback(PEER peer, qr2_error_t error,
...
static void ListingGamesCallback(PEER peer, PEERBool success,
                                 const gsi_char *name, SBServer server,
                                 PEERBool staging, int msg, int progress,
                                 void *param) {
  int num;
  int i;
...
static void CheckingBackendAvailabilityThink(void) {
  ...
  if (!peerSetTitle(Peer, GAMENAME, SECRETKEY, GAMENAME, SECRETKEY, 0, 10,
                    PEERTrue, NULL, NULL)) {
    peerShutdown(Peer);
    SetNextMenuScreen(&msFailedToIntializePeerSDK);
    return;
...
static void HostingServerInit(void) {
  ...
  snprintf(screen->extraText[line++], MAX_EXTRA_TEXT_STRING_LEN + 1,
           "Mapname: %s", Mapnames[Mapname]);
  snprintf(screen->extraText[line++], MAX_EXTRA_TEXT_STRING_LEN + 1,
           "Gametype: %s", Gametypes[Gametype]);
}
static void HostingServerChose(const char *choice) {
...
static void ListingHostsInit(void) {
  ...
  peerStartListingGames(Peer, keys, sizeof(keys), NULL, ListingGamesCallback,
                        NULL);
}
static void ListingHostsChose(const char *choice) {
  if (choice == mscViewHostInfo) {
    ...
    SelectedServer =
        SBServerListNth(&connection->gameList, screen->listSelection);
...
static void ViewHostInfoInit(void) {
  ...
  snprintf(screen->extraText[line++], MAX_EXTRA_TEXT_STRING_LEN, "  %s",
           SBServerGetStringValue(SelectedServer, "hostname", "N/A"));

  // public address
  strcpy(screen->extraText[line++], "[public address]");
  snprintf(screen->extraText[line++], MAX_EXTRA_TEXT_STRING_LEN, "  %s:%d",
