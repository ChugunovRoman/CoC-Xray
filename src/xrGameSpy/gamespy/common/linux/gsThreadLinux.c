...
//
// NOTE: when implementing this make sure the "-lpthread" compiler option is
// used
//
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
...
gsi_u32 gsiInterlockedIncrement(gsi_u32 *value) {
  GS_ASSERT_STR(gsi_false, "gsiInterlockIncrement is unsupported for LINUX in "
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
int gsiStartThread(GSThreadFunc func, gsi_u32 theStackSize, void *arg,
                   GSIThreadID *id) {
  pthread_attr_init(&id->attr);
  pthread_attr_setstacksize(&id->attr, theStackSize);

  if (pthread_create(&id->thread, &id->attr, (void *)func, arg) !=
      PTHREAD_NO_ERROR) {
    gsDebugFormat(GSIDebugCat_Common, GSIDebugType_Misc,
                  GSIDebugLevel_WarmError, "Failed to create thread\r\n");
    return -1;
  }
...
void gsiCancelThread(GSIThreadID id) {
  ...
  if (pthread_cancel(id.thread) != PTHREAD_NO_ERROR) {
    ...
    gsDebugFormat(GSIDebugCat_Common, GSIDebugType_Misc,
                  GSIDebugLevel_WarmError, "Failed to cancel thread\r\n");
  }
...
void gsiDeleteCriticalSection(GSICriticalSection *theCrit) {
  if (pthread_mutex_destroy(theCrit) != PTHREAD_NO_ERROR) {
    gsDebugFormat(GSIDebugCat_Common, GSIDebugType_Misc,
                  GSIDebugLevel_WarmError, "Failed to destroy mutex\r\n");
  }
  theCrit = NULL;
}
...
GSISemaphoreID gsiCreateSemaphore(gsi_i32 theInitialCount, gsi_i32 theMaxCount,
  ...
  if (result != PTHREAD_NO_ERROR) {
    gsDebugFormat(GSIDebugCat_Common, GSIDebugType_Misc,
                  GSIDebugLevel_WarmError, "Failed to create semaphore\r\n");
  }
...
gsi_u32 gsiWaitForSemaphore(GSISemaphoreID theSemaphore, gsi_u32 theTimeoutMs) {
  ...
  } while (gsi_is_true(infinite) ||
           ((current_time() - startTime) < theTimeoutMs));
...
void gsiReleaseSemaphore(GSISemaphoreID theSemaphore, gsi_i32 theReleaseCount) {
  if (pthread_mutex_trylock(&theSemaphore.mLock) != PTHREAD_NO_ERROR) {
    gsDebugFormat(GSIDebugCat_Common, GSIDebugType_Misc,
                  GSIDebugLevel_WarmError, "Failed to lock semaphore\r\n");
...
void gsiCloseSemaphore(GSISemaphoreID theSemaphore) {
  if (pthread_mutex_destroy(&theSemaphore.mLock) != PTHREAD_NO_ERROR) {
    gsDebugFormat(GSIDebugCat_Common, GSIDebugType_Misc,
                  GSIDebugLevel_WarmError, "Failed to destroy semaphore\r\n");
