...
int SetSockBlocking(SOCKET sock, int isblocking) {
  ...
  rcode = setsockopt(sock, SCE_NET_INET_SOL_SOCKET, SCE_NET_INET_SO_NBIO, &argp,
                     sizeof(argp));
...
#define GetHostByNameBufferSize 1024
    char *buf =
        gsimalloc(GetHostByNameBufferSize); //[GetHostByNameBufferSize]; // PSP
                                            // documentation recommends 1024
    struct in_addr ip;
    ...
    result = sceNetResolverStartNtoA(resolverID, name, &ip,
                                     GSI_RESOLVER_TIMEOUT, GSI_RESOLVER_RETRY);
    sceNetResolverDelete(
        resolverID); // delete right away, result is stored in ip
    if (result < 0) {
      gsifree(buf);
      return NULL;
    }
    ...
    {
      const char *out =
          sceNetInetInetNtop(SCE_NET_INET_AF_INET, &ip, buf, sizeof(buf));
      gsDebugFormat(GSIDebugCat_Common, GSIDebugType_Network,
                    GSIDebugLevel_Notice, "gsSocketGetHostByName: %s = %s\r\n",
                    name, out ? out : "void");
    }
    gsifree(buf);
    return &ahostent;
...
int _NetworkAdHocSocketCreate(gsi_u16 port) {
  ...
  printf("Create Adhoc Socket: MAC:%x,%x,%x,%x,%x,%x\n",
         (gsi_u32)tmp_local.addr[0], (gsi_u32)tmp_local.addr[1],
         (gsi_u32)tmp_local.addr[2], (gsi_u32)tmp_local.addr[3],
         (gsi_u32)tmp_local.addr[4], (gsi_u32)tmp_local.addr[5]);
#endif
...
int _NetworkAdHocSocketBroadcast(int socketid, const void *data, int len,
                                 int flags, gsi_u16 dest_port) {
  const static gsi_u8 broadcast_addr[SCE_NET_ETHER_ADDR_LEN] = {0xff};
  int ret =
      sceNetAdhocPdpSend(socketid,       //	id Socket ID
...
int _NetworkAdHocSocketSendTo(int socketid, const void *data, int len,
                              int flags, const char *dest_addr,
                              gsi_u16 dest_port) {
#if (0) // test
  const static gsi_u8 broadcast_addr[SCE_NET_ETHER_ADDR_LEN] = {0xff};
  int ret =
  ...
  printf("AdHoc SendTo: MAC:%x,%x,%x,%x,%x,%x port=%d Len=%d\n",
         (gsi_u32)dest_addr[0], (gsi_u32)dest_addr[1], (gsi_u32)dest_addr[2],
         (gsi_u32)dest_addr[3], (gsi_u32)dest_addr[4], (gsi_u32)dest_addr[5],
         dest_port, len);
#endif
  // todo: translate return value to GSI specific
  return ret;
  ...
  */
  ...
  if (ret < 0) {
    ...
    if (ret == SCE_ERROR_NET_ADHOC_INVALID_ARG) {
      ...
      printf("sceNetAdhocGetPdpStat() failed. "
             "SCE_ERROR_NET_ADHOC_NOT_INITIALIZED ret = 0x%x\n",
             ret);
    } else {
...
int _NetworkAdHocSocketRecv(int socket_id, char *buf, int bufferlen, int flags,
  ...
  int ret = sceNetAdhocPdpRecv(
      ...
      &bufferlen, // len		Receive buffer size (IN), receive data
                  // length (OUT)
  ...
  if (ret >= 0)
    printf("AdHoc Recv From: MAC:%x,%x,%x,%x,%x,%x: port=%d len:%d\n", saddr[0],
           saddr[1], saddr[2], saddr[3], saddr[4], saddr[5], *sport, bufferlen);
#endif
