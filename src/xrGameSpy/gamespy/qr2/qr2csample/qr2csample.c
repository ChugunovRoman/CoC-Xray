...
******
 ...
 See the ReadMe file for qrcsample info, and consult the GameSpy Query &
Reporting 2 SDK documentation for more information on implementing the qr2 SDK.
...
// define our additional keys, making sure not to overwrite the reserved
// standard key ids standard keys use 0-NUM_RESERVED_KEYS (defined in
// qr2regkeys.h)
...
void playerkey_callback(int keyid, int index, qr2_buffer_t outbuf,
                        void *userdata);
void teamkey_callback(int keyid, int index, qr2_buffer_t outbuf,
                      void *userdata);
void keylist_callback(qr2_key_type keytype, qr2_keybuffer_t keybuffer,
                      void *userdata);
...
void cc_callback(SOCKET gamesocket, struct sockaddr_in *remoteaddr,
                 void *userdata);
...
// called when a client sends a message to the server through qr2 (not commonly
// used)
void cm_callback(gsi_char *data, int len, void *userdata) {
...
void DoGameStuff(gsi_time totalTime) {
  ...
  // qr2_send_statechanged This should only be called after major changes such
  // as mapname or gametype, and cannot be applied more than once every 10
  // seconds (subsequent calls will be delayed when necessary)
...
int test_main(int argc, char **argp) {
  ...
  const int isNatNegSupported =
      1;                   // set to '0' if you don't support Nat Negotiation
  gsi_time aStartTime = 0; // for sample, so we don't run forever
  void *userData =
      NULL; // optional data that will be passed to the callback functions
  ...
  // register our custom keys (you do not have to register the reserved standard
  // keys)
  ...
  qr2_register_key(AVGPING_T_KEY,
                   _T("avgping_t")); // team keys always end with '_t'
  ...
  AppDebug("Initializing SDK; server should show up on the master list within "
           "6-10 sec.\n");
  // Call qr_init with the query port number and gamename, default IP address,
  // and no user data Pass NULL for the qrec parameter (first parameter) as long
  // as you're running a single game server instance per process Reference
  // gt2nat sample for qr2_init_socket implementation
  if (qr2_init(NULL, argc > 1 ? ip : NULL, BASE_PORT, GAME_NAME, secret_key,
               isPublic, isNatNegSupported, serverkey_callback,
               playerkey_callback, teamkey_callback, keylist_callback,
               count_callback, adderror_callback, userData) != e_qrnoerror) {
  ...
  while ((current_time() - aStartTime) < 60000) {
    gsi_time totalTime =
        current_time() -
        aStartTime; // used to change the game state after 30 seconds

    // An actual game would do something between "thinks"
    DoGameStuff(totalTime);

    // check for / process incoming queries
    // should be called every 10-100 ms; quicker calls produce more accurate
    // ping measurements
    qr2_think(NULL);
  }

  AppDebug(
      "Shutting down - server will be removed from the master server list\n");
