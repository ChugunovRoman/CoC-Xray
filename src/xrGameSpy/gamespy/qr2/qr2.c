...
qr2_error_t qr2_init_socketA(/*[out]*/ qr2_t *qrec, SOCKET s, int boundport,
  ...
  if (ispublic) {
    ...
    ret = get_sockaddrin(override ? qr2_hostname : hostname, MASTER_PORT,
                         &(cr->hbaddr), NULL);
...
void qr2_check_queries(qr2_t qrec) {
  ...
  while (CanReceiveOnSocket(qrec->hbsock)) {
    // else we have data
    error = (int)recvfrom(qrec->hbsock, indata, (INBUF_LEN - 1), 0,
...
void qr2_check_send_heartbeat(qr2_t qrec) {
  ...
  if (qrec->listed_state > 0 &&
    ...
    if (qrec->listed_state >= MAX_FIRST_COUNT) {
      ...
      qrec->adderror_callback(
          e_qrnochallengeerror,
          L"No challenge value was received from the master server.",
          qrec->udata);
#endif
      return;
    } else {
...
static void NatNegCompletedCallback(NegotiateResult result, SOCKET gamesocket,
...
static void qr_process_client_message(qr2_t qrec, char *buf, int len) {
  ...
  if (isnatneg) {
    ...
    if (qrec->cc_callback) {
      ...
      // do the negotiation
      error = NNBeginNegotiationWithSocket(qrec->hbsock, cookie, 1,
                                           NatNegProgressCallback,
                                           NatNegCompletedCallback, qrec);
      if (error != ne_noerror) {
...
void qr2_parse_queryA(qr2_t qrec, char *query, int len,
  ...
  /*if (query[0] == '\\') //it's a QR1-style query
  {
          ...
          sendto(qrec->hbsock, buf.buffer, buf.len, 0, sender, sizeof(struct
  sockaddr_in)); gsDebugFormat(GSIDebugCat_QR2, GSIDebugType_Network,
  GSIDebugLevel_Comment, "Sent %d bytes as QR1 query response\r\n", buf.len);
          gsDebugBinary(GSIDebugCat_QR2, GSIDebugType_Network,
  GSIDebugLevel_RawDump, buf.buffer, buf.len);

          return;
  }*/
  if ((len >= 6) &&
      (memcmp(query, NNMagicData, NATNEG_MAGIC_LEN) == 0)) /* a natneg query */
  {
    gsDebugFormat(GSIDebugCat_QR2, GSIDebugType_Network, GSIDebugLevel_Notice,
...
//#define SIMULATE_FIREWALL
  ...
  if (qrec->denyresp2_ip_callback) {
    ...
    unsigned int senderAddr = ((SOCKADDR_IN *)sender)->sin_addr.s_addr; // in
                                                                        // hbo
    if ((aMasterAddr & 0xffff) !=
        (senderAddr &
         0xffff)) // if sender ip is not in gamespy subnet /16 (fake)
    {
      int deny_result = 0;
...
#if defined(GSI_UNICODE)
void qr2_parse_queryW(qr2_t qrec, unsigned short *query, int len, struct
sockaddr *sender)
{
        char query_A[4096];
