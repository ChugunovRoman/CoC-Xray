...
// typedef char GSLoginCertificateKeyHash[GS_CRYPT_MD5_HASHSIZE]; // Hash of
// private key, for simple auth
...
typedef struct WSLoginResponse {
  ...
  GSLoginCertificate
      mCertificate; // Show this to others (prooves: "Bill is a valid user")
  GSLoginPrivateData mPrivateData; // Keep this secret!   (prooves: "I am Bill")
  void *mUserData;
} WSLoginResponse;

typedef void (*WSLoginCallback)(GHTTPResult httpResult,
                                WSLoginResponse *response, void *userData);
...
typedef struct WSLoginPs3CertResponse {
  ...
  char mPartnerChallenge[WS_LOGIN_PARTNERCHALLENGE_LEN]; // keep this secret!
                                                         // (It's a "password"
                                                         // for the token.)
  void *mUserData;
} WSLoginPs3CertResponse;

typedef void (*WSLoginPs3CertCallback)(GHTTPResult httpResult,
...
gsi_u32 wsLoginProfile(int partnerCode, int namespaceId,
                       ...
                       const gsi_char *password, const gsi_char *cdkeyhash,
