...
//////////////////////////////////////////////////////
const char *soundFiles[] = {"sounds\\explosion.wav", "sounds\\mine.wav",
                            "sounds\\die.wav",       "sounds\\rocket.wav",
                            "sounds\\pickup.wav",    NULL};

// Adapted from a Microsoft DirectSound sample.
///////////////////////////////////////////////
static HRESULT ReadMMIO(HMMIO hmmioIn, MMCKINFO *pckInRIFF,
...
// Adapted from a Microsoft DirectSound sample.
///////////////////////////////////////////////
static HRESULT WaveOpenFile(CHAR *strFileName, HMMIO *phmmioIn,
...
////////////////////////////////////////////
static GT2Bool LoadSound(int sound, const char *filename,
  ...
  /////////////////
  rcode =
      WaveOpenFile((char *)filename, &IOHandle, &waveFormat, &parentChunkInfo);
  if (rcode)
    return GT2False;
  ...
  if (!restoreBuffer) {
    ...
    // Create the buffer.
    /////////////////////
    result = IDirectSound_CreateSoundBuffer(DirectSound, &bufferDescriptor,
                                            &soundEffect.SoundBuffers[0], NULL);
    if (FAILED(result)) {
  ...
  // Lock the buffer.
  ///////////////////
  result = IDirectSoundBuffer_Lock(soundBuffer, 0, 0, &lockedBuffer,
                                   &lockedBufferLen, NULL, NULL,
  ...
  /////////////////////////////////
  rcode = WaveReadFile(IOHandle, lockedBufferLen, (BYTE *)lockedBuffer,
                       &dataChunkInfo, &numBytes);
  if (rcode) {
    mmioClose(IOHandle, 0);
    return GT2False;
  }
  ...
  // Unlock the buffer.
  /////////////////////
  IDirectSoundBuffer_Unlock(soundBuffer, lockedBuffer, lockedBufferLen, NULL,
                            0);
...
void ToggleSound(void) { playSounds = !playSounds; }
...
void PlaySoundEffect(int sound) {
  ...
  for (i = 0; i < soundEffect->numDuplicates; i++) {
    ...
    //////////////////////////
    result =
        IDirectSoundBuffer_GetStatus(soundEffect->SoundBuffers[i], &status);
    if (FAILED(result))
      return;
  ...
  if (i == soundEffect->numDuplicates) {
    ...
    if (i == MAX_DUPLICATES) {
      ...
      ////////////////////////
      result = IDirectSound_DuplicateSoundBuffer(DirectSound,
