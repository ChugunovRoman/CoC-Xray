...
static void DrawString(const char *string, int x, int y, const V3b color,
                       float scale) {
  scale *= TEXT_SCALE;
  glColor3ubv(color);
  glPushMatrix();
...
static void Predict(V2f newPosition, float *rotation, Player *player) {
  ...
  if (DrawWithPredictionOption && !player->dead && player->motion) {
    ...
    ////////////////////////
    ComputeNewPosition(newPosition, player->position, player->motion,
                       player->rotation, Diff, PLAYER_SPEED, GT2True);

    // Predict rotation if not local.
    /////////////////////////////////
    if (!local)
      *rotation = ComputeNewRotation(player->rotation, player->turning, Diff,
                                     PLAYER_TURN_SPEED);
  } else {
...
static void DrawRocket(CObject *rocket) {
  ...
  if (DrawWithPredictionOption) {
    // Predict new position.
    ////////////////////////
    ComputeNewPosition(position, rocket->position, FORWARD, rocket->rotation,
                       Diff, ROCKET_SPEED, GT2False);
  } else {
    // Just use the actual position.
    ////////////////////////////////
  ...
  glBindTexture(GL_TEXTURE_2D,
                rocketTextures[RandomInt(0, NUM_ROCKET_TEXTURES - 1)]);
...
static void DrawMine(CObject *mine) {
  ...
  glBindTexture(GL_TEXTURE_2D,
                mineTextures[(int)mine->position[0] % NUM_MINE_TEXTURES]);
...
static void DrawAsteroid(CObject *asteroid) {
  ...
  // glBindTexture(GL_TEXTURE_2D, asteroidTextures[(int)asteroid->position[0] %
  // NUM_ASTEROID_TEXTURES]);
...
static GT2Bool LoadTexture(const char *filename, GLuint *texture,
                           GT2Bool useAlpha) {
  int width;
  int height;
  GLubyte *bytes;
...
static void DrawStars(void) {
  ...
  for (i = 0; i < NUM_STARS; i++) {
    ...
    glColor4ub(stars[i].color[0], stars[i].color[1], stars[i].color[2],
               (byte)alpha);
...
static void DrawRadar(void) {
  ...
  //////////////////////////////////
  glTranslatef(WINDOW_MAX - (RADAR_SIZE * RadarScaleOption),
               WINDOW_MAX - (RADAR_SIZE * RadarScaleOption), 0);

  // Scale so we can draw in map space.
  /////////////////////////////////////
  glScalef(RADAR_SIZE / MAP_MAX * RadarScaleOption,
           RADAR_SIZE / MAP_MAX * RadarScaleOption, 1);
...
static void SetMapView(void) {
  ...
  gluOrtho2D(corner[0], corner[0] + FULL_VIEW, corner[1],
             corner[1] + FULL_VIEW);
}
