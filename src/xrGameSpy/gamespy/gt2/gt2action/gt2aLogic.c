...
static void RocketThink(SObject *rocket) {
  ...
  // Move the rocket.
  ///////////////////
  ComputeNewPosition(rocket->position, rocket->position, FORWARD,
                     rocket->rotation, Diff, ROCKET_SPEED, GT2False);
...
static void MineThink(SObject *mine) {
  ...
  if ((Now - mine->startTime) > MINE_TIME) {
    ...
    ///////////////////
    mine->rotation =
        ComputeNewRotation(mine->rotation, RIGHT, Diff, MINE_TURN_SPEED);
...
static void AsteroidThink(SObject *asteroid) {
  ...
  ///////////////////////
  asteroid->rotation =
      ComputeNewRotation(asteroid->rotation, LEFT, Diff, ASTEROID_TURN_SPEED);

  // Move the asteroid.
  /////////////////////
  ComputeNewPosition(asteroid->position, asteroid->position, FORWARD,
                     asteroid->heading, Diff, asteroid->speed, GT2False);
...
static void InitAsteroid(SObject *asteroid) {
  ...
  asteroid->speed =
      RandomFloat(ASTEROID_SPEED_MIN, ASTEROID_SPEED_MAX, GT2False);
...
void ObjectThink(SObject *object) {
  ...
  if (object->touchObject) {
    for (i = 0; i < MAX_OBJECTS; i++) {
      ...
      // Check for a touch.
      /////////////////////
      if (DistanceV2f(object->position, other->position) <=
          (object->radius + other->radius)) {
        // Touched.
        ///////////
