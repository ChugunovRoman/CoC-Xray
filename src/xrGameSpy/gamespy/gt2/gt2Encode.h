...
// Used to identify the type of message so you can look up the correct format
// string and pass the correct parameters You should use 1 msgType for each
// unique format string/parameter combination
////////////////////////////////////////////////////////
typedef unsigned short GTMessageType;

// Encode a message into outBuffer
// Returns the length of the encoded message, or -1 to indicate insufficient
// space You must make sure the number of arguments match the fmtString list
////////////////////////////////////////////////////////
int gtEncode(GTMessageType msgType, const char *fmtString, char *outBuffer,
             int outLength, ...);
int gtEncodeV(GTMessageType msgType, const char *fmtString, char *outBuffer,
...
automatically. The NUL character is restored in decode. You simply pass the char
* string as the arguement for Encode. Note that in the pointer passed to Decode
must have enough memory allocated to it for the max string that will be encoded
- otherwise the destination may get trashed. If you cannot guarantee them max
length of the string, you should use GT_RAW (see below).

GT_DBSTR: Same as a GT_CSTR, except with 2-byte characters instead of single
byte. String must be terminated with a double NUL character.
...
For Decode, you must initialize the length pointer to the max length of the
buffer. If the decoded data exceeds this length, the Decode function will return
-1, and the length pointer will be set to the actual length. You can then call
Decode again with a buffer that is at least the required length. Example:
...
e.g.: "ziz" will use 6 bytes (2 for the bits, 4 for the int), whereas "zzi"
would use only 5 bytes (1 for the bits, 4 for the int) The argument type for
bits is char for Encode and char * for Decode - If the char is 0, the bit will
not be set, if it's non-zero, the bit will be set. Note that in Decode, the set
bit will always be returned as 1 (not the non-zero value you set)
*/
