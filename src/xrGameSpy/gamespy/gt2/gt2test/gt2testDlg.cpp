...
BOOL CGt2testDlg::OnInitDialog() {
  ...
  // we initialize sockets ourself because it might be needed for the address
  // functions this function is declared in nonport.h
  SocketStartUp();
...
// MFC stuff
HCURSOR CGt2testDlg::OnQueryDragIcon() { return (HCURSOR)m_hIcon; }
...
static void DelayCheck(DArray delayedMessages, unsigned long now, BOOL send) {
  ...
  for (i = (num - 1); i >= 0; i--) {
    ...
    // check if the time passed since the start time is greater than the delay
    // time
    if ((now - message->startTime) > message->delayTime) {
      if (send)
        gt2FilteredSend(message->connection, message->filterID,
                        message->message, message->len, message->reliable);
      else
        gt2FilteredReceive(message->connection, message->filterID,
                           message->message, message->len, message->reliable);
...
static void RemoveDelayedMessages(DArray delayedMessages,
...
void CGt2testDlg::OnTimer(UINT nIDEvent) {
  ...
  if (nIDEvent == 100) {
    ...
    // thinking, think the check for already thinking is because this can get
    // called from inside a dialog box brought up inside a callback - this would
    // be bad (recursive thinking).
    m_alwaysThink =
        (((CButton *)GetDlgItem(IDC_ALWAYS_THINK))->GetCheck() == BST_CHECKED);
    ...
    // find the active connection, and check what the connection's state should
    // be
    ...
    if (newState != m_connectionState) {
      ...
      if (newState != -1)
        ((CButton *)GetDlgItem(IDC_CONNECTING + newState))
            ->SetCheck(BST_CHECKED);

      // if there's a previous state, uncheck it
      if (m_connectionState != -1)
        ((CButton *)GetDlgItem(IDC_CONNECTING + m_connectionState))
...
// used to enable or disable socket controls when a socket is created or
// destroyed
...
// used to enable or disable the listen controls, depending on if the socket is
// listening or not
...
// used to enable or disable the connection controls, depending on if there's a
// connection or not
...
int CGt2testDlg::GetConnectionIndex(GT2Connection connection) {
  ...
  for (nIndex = 0; nIndex < num; nIndex++) {
    if (((ConnectionInfo *)m_connections.GetItemDataPtr(nIndex))->connection ==
        connection)
      return nIndex;
  }
...
void CGt2testDlg::AddConnection(GT2Connection connection) {
  ...
  CString address = gt2AddressToString(gt2GetRemoteIP(connection),
                                       gt2GetRemotePort(connection), NULL);
...
// sets a connection as active (or, if connection is NULL and nIndex is -1, sets
// the list to no active connectoin)
...
void CGt2testDlg::AddMessageString(GT2Connection connection,
                                   const char *string) {
...
static void SendDumpCallback(GT2Socket socket, GT2Connection connection,
                             unsigned int ip, unsigned short port,
                             GT2Bool reset, const GT2Byte *message, int len) {
  CString str;
  str.Format("SEND %p %p %s %d\n", socket, connection,
             gt2AddressToString(ip, port, NULL), len);
...
static void ReceiveDumpCallback(GT2Socket socket, GT2Connection connection,
                                unsigned int ip, unsigned short port,
                                GT2Bool reset, const GT2Byte *message,
                                int len) {
  CString str;
  if (reset)
...
void CGt2testDlg::OnCreateSocket() {
  ...
  GT2Result result =
      gt2CreateSocket(&m_socket, m_localAddress, atoi(m_outBufferSize),
                      atoi(m_inBufferSize), SocketErrorCallback);
  ...
  m_localAddress = gt2AddressToString(gt2GetLocalIP(m_socket),
                                      gt2GetLocalPort(m_socket), NULL);
...
static void ConnectAttemptCallback(GT2Socket socket, GT2Connection connection,
                                   unsigned int ip, unsigned short port,
  ...
  if (dlg->m_acceptMode == 0)
    ...
    str.Format("Accept a connection from %s (latency: %dms)?\n%s",
               (LPCSTR)address, latency, message);
...
void CGt2testDlg::OnConnect() {
  ...
  GT2Connection connection;
  GT2Result result = gt2Connect(m_socket, &connection, m_remoteAddress,
                                (GT2Byte *)(LPCSTR)m_connectMessage, -1,
                                timeout, &callbacks, m_blocking);
  if (result != GT2Success) {
    CString str;
...
void PingCallback(GT2Connection connection, int latency) {
...
// ROT13 changes every letter to it's "opposite" in the alphabet by adding 13 to
// it. a->m, b->n, c->o, d->p, m->a, n->b, o->c, p->d, etc. performing ROT13
// twice results in the original string, allowing the same filter to be applied
// to both sending and receiving to end up with the original string.  cheap
// encryption.
...
static void SendFilterCallbackROT13(GT2Connection connection, int filterID,
...
static void ReceiveFilterCallbackROT13(GT2Connection connection, int filterID,
                                       GT2Byte *message, int len,
                                       GT2Bool reliable) {
  // ROT13 the string
...
static BOOL CheckDrop(int dropValue) { return ((rand() % 100) < dropValue); }

static void SendFilterCallbackDrop(GT2Connection connection, int filterID,
                                   const GT2Byte *message, int len,
                                   GT2Bool reliable) {
...
static void ReceiveFilterCallbackDrop(GT2Connection connection, int filterID,
                                      GT2Byte *message, int len,
                                      GT2Bool reliable) {
  // if it's unreliable, check if it should be dropped
...
static void AddDelayedMessage(DArray array, int delayValue,
                              GT2Connection connection, int filterID,
                              const GT2Byte *message, int len,
                              GT2Bool reliable) {
...
static void SendFilterCallbackDelay(GT2Connection connection, int filterID,
                                    const GT2Byte *message, int len,
                                    GT2Bool reliable) {
  // add it to the delayed sends list
  AddDelayedMessage(dlg->m_delayedSends, atoi(dlg->m_sendDelayValue),
                    connection, filterID, message, len, reliable);
}

static void ReceiveFilterCallbackDelay(GT2Connection connection, int filterID,
                                       GT2Byte *message, int len,
                                       GT2Bool reliable) {
  // add it to the delayed receives list
  AddDelayedMessage(dlg->m_delayedReceives, atoi(dlg->m_receiveDelayValue),
                    connection, filterID, message, len, reliable);
}
...
void CGt2testDlg::OnChangeReceiveDropValue() {
  ...
  // save the setting with the connection's info
  GetConnectionInfo(GetActiveConnection())->receiveDropValue =
      m_receiveDropValue;
}
...
void CGt2testDlg::OnChangeReceiveDelayValue() {
  ...
  // save the setting with the connection's info
  GetConnectionInfo(GetActiveConnection())->receiveDelayValue =
      m_receiveDelayValue;
}
...
void CGt2testDlg::HandleHostInfo(const char *hostname, char **aliases,
                                 unsigned int **ips) {
  int count;
