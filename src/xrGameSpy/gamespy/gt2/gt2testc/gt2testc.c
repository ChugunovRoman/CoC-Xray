...
Message messageList[] = {
    ...
    {GT2True, "12345678901234567890123456789012345678901234567890", 50,
     GT2True}, // 50 bytes
...
Message hardcoreMessage = {
    GT2True, "12345678901234567890123456789012345678901234567890", 50, GT2True};

static void ConnectedCallback(GT2Connection connection, GT2Result result,
...
static void ReceivedCallback(GT2Connection connection, GT2Byte *message,
  ...
  if (server) {
    ...
    if ((pcurr->reliable != reliable) || (pcurr->len != len) ||
        (memcmp(pcurr->data, message, (size_t)len) != 0)) {
      printf("Message check failed (%d, %d)\n", pcurr->len, len);
    } else {
      if (!hardcore || !(messageCount % 100)) {
        printf("Message %d validated (%d bytes, %s)", messageCount + 1, len,
               reliable ? "reliable" : "unreliable");
        if (server)
          printf("\n");
        else
...
static void ConnectAttemptCallback(GT2Socket socket, GT2Connection connection,
  ...
  if ((len == ((int)strlen(initialMessage.data) + 1)) &&
      (memcmp(message, initialMessage.data, (size_t)len) == 0)) {
...
static void EncodeTest() {
  const char encodestr[] = {GT_INT,  GT_UINT,  GT_SHORT, GT_USHORT,
                            GT_CHAR, GT_UCHAR, GT_FLOAT, GT_DOUBLE,
                            GT_CSTR, GT_DBSTR, GT_RAW,   0};
  const char encodestr2[] = {GT_CSTR_PTR, GT_DBSTR_PTR, GT_RAW_PTR, 0};
  const char bitstr[] = {GT_BIT, GT_BIT, GT_BIT, GT_BIT, GT_BIT, GT_BIT,
                         GT_BIT, GT_BIT, GT_BIT, GT_BIT, 0};
  int i = -30, i2;
  ...
  char mystr[] = "this is a test string, this is only a test", mystr2[255],
       *mystr3;
  ...
  retlen = gtEncode(0, encodestr, outBuff, 1024, i, uint, sh, ush, ch, uch, f,
                    d, mystr, dbstr, rawstr, rawdatalen);
  retlen2 = gtDecode(encodestr, outBuff, retlen, &i2, &uint2, &sh2, &ush2, &ch2,
                     &uch2, &f2, &d2, mystr2, dbstr2, rawstr2, &rawdatalen2);
  printf("i=%d,i2=%d\nuint=%u,uint2=%u\nsh=%d,sh2=%d\nush=%u,ush2=%u\nch=%d,"
         "ch2=%d\nuch=%u,uch2=%u\nf=%f,f2=%f\nd=%f,d2=%f\n",
         i, i2, uint, uint2, sh, sh2, ush, ush2, ch, ch2, uch, uch2, f, f2, d,
         d2);
  printf("mystr =%s\nmystr2=%s\n", mystr, mystr2);
  for (temp = 0; dbstr[temp] != 0; temp++)
    printf("dbstr[%d]=%d,dbstr2[%d]=%d  ", temp, dbstr[temp], temp,
           dbstr2[temp]);
  printf("\nrawdatalen=%d,rawdatalen2=%d\n", rawdatalen, rawdatalen2);
  for (temp = 0; temp < rawdatalen; temp++)
    printf("rawstr[%d]=%d,rawstr2[%d]=%d  ", temp, rawstr[temp], temp,
           rawstr2[temp]);
  printf("\nretlen=%d,retlen2=%d\n", retlen, retlen2);
  retlen =
      gtEncode(0, bitstr, outBuff, 1024, bits[0], bits[1], bits[2], bits[3],
               bits[4], bits[5], bits[6], bits[7], bits[8], bits[9]);
  retlen2 = gtDecode(bitstr, outBuff, retlen, bits2, bits2 + 1, bits2 + 2,
  ...
  printf("\nretlen=%d,retlen2=%d\n", retlen, retlen2);
  retlen =
      gtEncode(0, encodestr2, outBuff, 1024, mystr, dbstr, rawstr, rawdatalen);
  retlen2 = gtDecode(encodestr2, outBuff, retlen, &mystr3, &dbstr3, &rawstr3,
                     &rawdatalen2);
  printf("mystr =%s\nmystr3=%s\n", mystr, mystr3);
#ifdef ALIGNED_COPY
  for (temp = 0; dbstr[temp] != 0; temp++) {
...
#else
  for (temp = 0; dbstr[temp] != 0; temp++)
    printf("dbstr[%d]=%d,dbstr3[%d]=%d  ", temp, dbstr[temp], temp,
           dbstr3[temp]);
#endif
  printf("\nrawdatalen=%d,rawdatalen2=%d\n", rawdatalen, rawdatalen2);
  for (temp = 0; temp < rawdatalen; temp++)
    printf("rawstr[%d]=%d,rawstr3[%d]=%d  ", temp, rawstr[temp], temp,
           rawstr3[temp]);
  printf("\nretlen=%d,retlen2=%d\n", retlen, retlen2);
}
#endif
...
int test_main(int argc, char **argv) {
  ...
  do {
    ...
    result = gt2Connect(socket, &Connection, remoteAddress,
                        (GT2Byte *)initialMessage.data, initialMessage.len,
                        10000, &connectionCallbacks, GT2True);
    if (result != GT2Success) {
    ...
    while (!quit && msgloopcount < 2) {
      ...
      gt2Send(Connection, (GT2Byte *)message->data, message->len,
              message->reliable);
      if (hardcore) {
        do {
          gt2Think(socket);
        ...
        if (sendTime != 0) {
          printf("Sending %d bytes %s failed!!\n", message->len,
                 message->reliable ? "reliable" : "unreliable");
        }
        messageCount++;
      }
    }
