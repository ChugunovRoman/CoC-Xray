...
static GT2Bool gti2ConnectionError(GT2Connection connection, GT2Result result,
...
static GT2Bool gti2ConnectionCommunicationError(GT2Connection connection) {
  return gti2ConnectionError(connection, GT2NegotiationError,
...
static GT2Bool gti2HandleESN(GT2Connection connection, unsigned short ESN) {
  ...
  message = (GTI2OutgoingBufferMessage *)ArrayNth(
      connection->outgoingBufferMessages, 0);
...
static GT2Bool gti2HandleAppUnreliable(GT2Connection connection,
...
static GT2Bool gti2HandleAppReliable(GT2Connection connection, GT2Byte *message,
                                     int len) {
  // check the state
  if ((connection->state != GTI2Connected) &&
      (connection->state != GTI2Closing)) {
...
static GT2Bool gti2HandleClientChallenge(GT2Connection connection,
                                         GT2Byte *message, int len) {
...
static GT2Bool gti2HandleServerChallenge(GT2Connection connection,
                                         GT2Byte *message, int len) {
  char response[GTI2_RESPONSE_LEN];
  ...
  if (!gti2SendClientResponse(connection, response, connection->initialMessage,
                              connection->initialMessageLen))
    return GT2False;
...
static GT2Bool gti2HandleClientResponse(GT2Connection connection,
                                        GT2Byte *message, int len) {
  int latency;
  ...
  if (!gti2ConnectAttemptCallback(
          connection->socket, connection, connection->ip, connection->port,
          latency, message + GTI2_RESPONSE_LEN, len - GTI2_RESPONSE_LEN))
    return GT2False;
...
static GT2Bool gti2HandleReject(GT2Connection connection, GT2Byte *message,
                                int len) {
  // check the state
  if (connection->state != GTI2AwaitingAcceptance) {
    if (!gti2ConnectionCommunicationError(connection))
...
static GT2Bool gti2HandleClose(GT2Connection connection) {
  ...
  if (!gti2ConnectionError(connection, GT2Rejected,
                           localClose ? GT2LocalClose : GT2RemoteClose))
    return GT2False;

  return GT2True;
}

static GT2Bool gti2DeliverReliableMessage(GT2Connection connection,
...
#ifdef WIN32
static int __cdecl gti2IncomingBufferMessageCompare(const void *elem1,
                                                    const void *elem2)
#else
static int gti2IncomingBufferMessageCompare(const void *elem1,
                                            const void *elem2)
#endif
{
  const GTI2IncomingBufferMessage *message1 =
      (GTI2IncomingBufferMessage *)elem1;
  const GTI2IncomingBufferMessage *message2 =
      (GTI2IncomingBufferMessage *)elem2;
...
static GT2Bool gti2BufferIncomingMessage(GT2Connection connection,
                                         GTI2MessageType type,
                                         unsigned short SN, GT2Byte *message,
                                         int len, GT2Bool *overflow) {
  ...
  for (i = 0; i < num; i++) {
    // get the message
    bufferedMessage = (GTI2IncomingBufferMessage *)ArrayNth(
        connection->incomingBufferMessages, i);
  ...
  // add it to the list
  ArrayInsertSorted(connection->incomingBufferMessages, &messageInfo,
  ...
  // we want to send one when we think a message or messages were probably
  // dropped
  if (num == 0) {
    // if we're the only message in the hold, send a nack
    if (!gti2SendNack(connection, connection->expectedSerialNumber,
                      (unsigned short)(SN - 1)))
      return GT2False;
  } else {
    ...
    msg = (GTI2IncomingBufferMessage *)ArrayNth(
        connection->incomingBufferMessages, num);
    if (msg->serialNumber == SN) {
      GTI2IncomingBufferMessage *prev;
      unsigned short diff;

      // if we're not right after the second-highest SN, the ones in between
      // were probably dropped
      prev = (GTI2IncomingBufferMessage *)ArrayNth(
          connection->incomingBufferMessages, num - 1);
      diff = (unsigned short)gti2SNDiff(SN, prev->serialNumber);
      if (diff > 1) {
        if (!gti2SendNack(connection, (unsigned short)(prev->serialNumber + 1),
                          (unsigned short)(SN - 1)))
          return GT2False;
      }
    }
...
static void gti2RemoveHoldMessage(GT2Connection connection,
  ...
  for (i = 0; i < num; i++) {
    ...
    message = (GTI2IncomingBufferMessage *)ArrayNth(
        connection->incomingBufferMessages, i);
...
restart:
  ...
  // loop through the buffered messages, checking if there are any that can now
  // be delivered loop through backwards to ease removal
  num = ArrayLength(connection->incomingBufferMessages);
  for (i = (num - 1); i >= 0; i--) {
    message = (GTI2IncomingBufferMessage *)ArrayNth(
        connection->incomingBufferMessages, i);
...
static GT2Bool gti2HandleReliableMessage(GT2Connection connection,
                                         GTI2MessageType type, GT2Byte *message,
                                         int len) {
  unsigned short SN;
  unsigned short ESN;
  const int headerLength =
      connection->socket->protocolOffset + GTI2_MAGIC_STRING_LEN + 1 + 2 + 2;
  GT2Bool overflow;

  // check if it's long enough
  if (len < (connection->socket->protocolOffset + GTI2_MAGIC_STRING_LEN + 1 +
             2 + 2)) // magic string + type + SN + ESN
  {
    if (!gti2ConnectionCommunicationError(connection))
      return GT2False;
  ...
  // get the SN
  SN = gti2UShortFromBuffer(message + connection->socket->protocolOffset,
                            GTI2_MAGIC_STRING_LEN + 1);

  // get the ESN
  ESN = gti2UShortFromBuffer(message + connection->socket->protocolOffset,
                             GTI2_MAGIC_STRING_LEN + 3);

  // update the message and length to point to the actual message
  if (connection->socket->protocolType == GTI2VdpProtocol &&
      type == GTI2MsgAppReliable) {
    message[connection->socket->protocolOffset + GTI2_MAGIC_STRING_LEN + 3] =
        message[0];
    message[connection->socket->protocolOffset + GTI2_MAGIC_STRING_LEN + 4] =
        message[1];
...
static GT2Bool gti2HandleAck(GT2Connection connection, const GT2Byte *message,
                             int len) {
  unsigned short ESN;
...
static GT2Bool gti2HandleNack(GT2Connection connection, const GT2Byte *message,
                              int len) {
  ...
  for (i = 0; i < num; i++) {
    messageInfo = (GTI2OutgoingBufferMessage *)ArrayNth(
        connection->outgoingBufferMessages, i);
...
static GT2Bool gti2HandlePing(GT2Connection connection, GT2Byte *message,
                              int len) {
  // send it right back
  return gti2SendPong(connection, message, len);
}

static GT2Bool gti2HandlePong(GT2Connection connection, const GT2Byte *message,
                              int len) {
  gsi_time startTime;
...
static GT2Bool gti2HandleUnreliableMessage(GT2Connection connection,
                                           GTI2MessageType type,
                                           GT2Byte *message, int len) {
  int headerLength;
  GT2Byte *dataStart;
  ...
  headerLength =
      (connection->socket->protocolOffset + GTI2_MAGIC_STRING_LEN + 1);
...
static GT2Bool gti2HandleMessage(GT2Socket socket, GT2Byte *message, int len,
  ...
  if (socket->receiveDumpCallback) {
    if (!gti2DumpCallback(socket, connection, ip, port, GT2False, message, len,
                          GT2False))
      return GT2False;
  }
  ...
  if (!connection) {
    // if we don't know who this is from, let the unrecognized message callback
    // have first crack at it
    if (!gti2UnrecognizedMessageCallback(socket, ip, port, message, len,
                                         &handled))
      return GT2False;
    ...
    if (!magicString ||
        (actualMessage[GTI2_MAGIC_STRING_LEN] != GTI2MsgClientChallenge)) {
      // if this is a closed message, don't send one back (to avoid recursion)
      if (!magicString ||
          (actualMessage[GTI2_MAGIC_STRING_LEN] != GTI2MsgClosed)) {
    ...
    if (result != GT2Success) {
      // as long as this wasn't a duplicate address error, tell them we're
      // closed in the case of duplicates, we don't want to close the existing
      // connection
  ...
  if (magicString &&
      ((actualLength >= (GTI2_MAGIC_STRING_LEN * 2)) &&
       (memcmp(actualMessage + GTI2_MAGIC_STRING_LEN, GTI2_MAGIC_STRING,
               GTI2_MAGIC_STRING_LEN) == 0))) {
    message[3] = message[1];
    message[2] = message[0];
  ...
  if (!magicString) {
    // First determine if the connection found has gone throught the internal
    // challenge response
    if (connection->state < GTI2Connected) {
...
GT2Bool gti2HandleConnectionReset(GT2Socket socket, unsigned int ip,
  ...
  if (socket->receiveDumpCallback) {
    if (!gti2DumpCallback(socket, connection, ip, port, GT2True, NULL, 0,
                          GT2False))
      return GT2False;
  }
  ...
  if (connection->state == GTI2AwaitingServerChallenge) {
    ...
    if (!connection->timeout ||
        ((current_time() - connection->startTime) < connection->timeout))
      return GT2True;
...
// <=0 on error
gsi_bool
_NetworkAdHocSocketRecv(int socket_id, char *buf, int bufferlen, int flags,
...
GT2Bool gti2ReceiveAdHocMessages(GT2Socket socket, char *buffer,
                                 int buffersize) {
  int rcode;
  SOCKADDR_IN address;
  ...
  while (1) {
      ...
      break; // no data
    {
      ...
      rcode = _NetworkAdHocSocketRecv(socket->socket, buffer, buffersize, 0,
                                      mac, &port);
      if (rcode < 0) // fatal socket error
      {
        ...
        {
          // handle the reset
          if (!gti2HandleConnectionReset(socket, address.sin_addr.s_addr,
                                         ntohs(address.sin_port)))
            return GT2False;
        } else if (rcode == WSAEHOSTUNREACH) {
          if (!gti2HandleHostUnreachable(socket, address.sin_addr.s_addr,
                                         ntohs(address.sin_port), GT2False))
            return GT2False;
        } else
#endif
        {
      ...
      gti2LogMessage(address.sin_addr.s_addr, ntohs(address.sin_port),
                     socket->ip, socket->port, buffer, rcode);
#endif
      // handle the message
      if (!gti2HandleMessage(socket, (GT2Byte *)buffer, rcode,
...
// avoid overflowing stack
  ...
  if (socket->protocolType == GTI2AdHocProtocol) {
    return gti2ReceiveAdHocMessages(socket, buffer,
                                    GTI2_STACK_RECV_BUFFER_SIZE);
  }
#endif

  // check for messages
  while (CanReceiveOnSocket(socket->socket)) {
    // mj todo: get this plat specific stuff out of here.  Belongs in play
    // specific layer. abstract recvfrom

    // receive the message
    addressLen = sizeof(address);

    rcode = recvfrom(socket->socket, buffer, sizeof(buffer), 0,
...
static GT2Bool gti2StoreOutgoingReliableMessageInfo(GT2Connection connection,
                                                    unsigned short SN,
                                                    int len) {
  GTI2OutgoingBufferMessage messageInfo;
  int num;
...
static GT2Bool gti2BeginReliableMessage(GT2Connection connection,
                                        GTI2MessageType type, int len,
                                        GT2Bool *overflow) {
  int freeSpace;

  // VDP data length needed in the front of every packet
  unsigned short vdpDataLength =
      (unsigned short)(len - connection->socket->protocolOffset);
  ...
  if (!gti2StoreOutgoingReliableMessageInfo(connection,
                                            connection->serialNumber, len)) {
    if (!gti2ConnectionMemoryError(connection))
      return GT2False;
  ...
  if (connection->socket->protocolType == GTI2VdpProtocol)
    gti2BufferWriteData(&connection->outgoingBuffer,
                        (const GT2Byte *)&vdpDataLength,
                        connection->socket->protocolOffset);
  gti2BufferWriteData(&connection->outgoingBuffer,
                      (const GT2Byte *)GTI2_MAGIC_STRING,
                      GTI2_MAGIC_STRING_LEN);
  gti2BufferWriteByte(&connection->outgoingBuffer, (GT2Byte)type);
  gti2BufferWriteUShort(&connection->outgoingBuffer,
...
static GT2Bool gti2EndReliableMessage(GT2Connection connection) {
  ...
  assert(len > 0);
  message = (GTI2OutgoingBufferMessage *)ArrayNth(
      connection->outgoingBufferMessages, len - 1);

  // send it
  if (!gti2ConnectionSendData(
          connection, connection->outgoingBuffer.buffer + message->start,
          message->len))
    return GT2False;
...
GT2Bool gti2SendAppReliable(GT2Connection connection, const GT2Byte *message,
  ...
  // begin the message
  if (!gti2BeginReliableMessage(connection, GTI2MsgAppReliable, totalLen,
                                &overflow))
    return GT2False;
  if (overflow)
    return GT2True;
...
GT2Bool gti2SendClientChallenge(GT2Connection connection,
  ...
  // begin the message
  if (!gti2BeginReliableMessage(connection, GTI2MsgClientChallenge,
                                totalLen + connection->socket->protocolOffset,
                                &overflow))
    return GT2False;
  ...
  // write the challenge
  gti2BufferWriteData(&connection->outgoingBuffer, (const GT2Byte *)challenge,
                      GTI2_CHALLENGE_LEN);
...
GT2Bool gti2SendServerChallenge(GT2Connection connection,
  ...
  int totalLen = (GTI2_MAGIC_STRING_LEN + 1 + 2 + 2 + GTI2_RESPONSE_LEN +
                  GTI2_CHALLENGE_LEN);
  GT2Bool overflow;

  // begin the message
  if (!gti2BeginReliableMessage(connection, GTI2MsgServerChallenge,
                                totalLen + connection->socket->protocolOffset,
                                &overflow))
    return GT2False;
  ...
  // write the response
  gti2BufferWriteData(&connection->outgoingBuffer, (const GT2Byte *)response,
                      GTI2_RESPONSE_LEN);
...
GT2Bool gti2SendClientResponse(GT2Connection connection,
  ...
  // begin the message
  if (!gti2BeginReliableMessage(connection, GTI2MsgClientResponse,
                                totalLen + connection->socket->protocolOffset,
                                &overflow))
    return GT2False;
  ...
  // write the message
  gti2BufferWriteData(&connection->outgoingBuffer, (const GT2Byte *)message,
                      len);
...
GT2Bool gti2SendAccept(GT2Connection connection) {
  ...
  // begin the message
  if (!gti2BeginReliableMessage(connection, GTI2MsgAccept,
                                totalLen + connection->socket->protocolOffset,
                                &overflow))
    return GT2False;
...
GT2Bool gti2SendReject(GT2Connection connection, const GT2Byte *message,
  ...
  // begin the message
  if (!gti2BeginReliableMessage(connection, GTI2MsgReject,
                                totalLen + connection->socket->protocolOffset,
                                &overflow))
    return GT2False;
...
GT2Bool gti2SendClose(GT2Connection connection) {
  ...
  // begin the message
  if (!gti2BeginReliableMessage(connection, GTI2MsgClose,
                                totalLen + connection->socket->protocolOffset,
                                &overflow))
    return GT2False;
...
GT2Bool gti2SendKeepAlive(GT2Connection connection) {
  ...
  // begin the message
  if (!gti2BeginReliableMessage(connection, GTI2MsgKeepAlive,
                                totalLen + connection->socket->protocolOffset,
                                &overflow))
    return GT2False;
...
GT2Bool gti2SendAppUnreliable(GT2Connection connection, const GT2Byte *message,
  ...
  // check if we can send it right away (unreliable that doesn't start with the
  // magic string)
  if ((len < GTI2_MAGIC_STRING_LEN) ||
      (memcmp(message + connection->socket->protocolOffset, GTI2_MAGIC_STRING,
              GTI2_MAGIC_STRING_LEN) != 0)) {
...
GT2Bool gti2SendPing(GT2Connection connection) {
  ...
  // part of the buffer may not be used but more efficience to be on stack
  char buffer[MAX_PROTOCOL_OFFSET + GTI2_MAGIC_STRING_LEN + 1 + 4 +
              sizeof(gsi_time)];
  int pos = 0;
  gsi_time now;
...
GT2Bool gti2SendClosed(GT2Connection connection) {
  // normal close
  return gti2SendClosedOnSocket(connection->socket, connection->ip,
                                connection->port);
}
...
GT2Bool gti2ResendMessage(GT2Connection connection,
  ...
  pos = (message->start + connection->socket->protocolOffset +
         GTI2_MAGIC_STRING_LEN + 1 + 2);

  gti2UShortToBuffer(connection->outgoingBuffer.buffer, pos,
                     connection->expectedSerialNumber);
  ...
  type = (GTI2MessageType)connection->outgoingBuffer
             .buffer[message->start + connection->socket->protocolOffset +
                     GTI2_MAGIC_STRING_LEN];
...
GT2Bool gti2WasMessageIDConfirmed(const GT2Connection connection,
  ...
  // if there are no reliable messages waiting confirmation, then this has
  ...
  // if the message id we are looking for is older than the first one waiting
  // confirmation, then it has already been confirmed
