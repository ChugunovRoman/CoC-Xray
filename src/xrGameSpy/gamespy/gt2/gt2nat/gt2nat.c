...
char *constnames[QR2_MAX_PLAYERS] = {
    "Joe Player", "L33t 0n3", "Raptor",  "Gr81",  "Flubber",
    "Sarge",      "Void",     "runaway", "Ph3ar", "wh00t",
    "gr1nder",    "Mace",     "stacy",   "lamby", "Thrush"};
gamedata_t gamedata;
...
********/
void DoGameStuff(void) { msleep(10); }

GT2Bool UnrecognizedMessageCallback(GT2Socket socket, unsigned int ip,
  ...
  if (!len || !message ||
      ((message[0] != QR_MAGIC_1) && (message[1] != QR_MAGIC_2) &&
       (message[0] != '\\')))
    return GT2False;
...
void ConnectAttemptCallback(GT2Socket socket, GT2Connection connection,
...
#endif
{
  ...
  GT2Result result;
  int natNegotiate = 1;
  result = gt2CreateSocket(&socket, gt2AddressToString(0, QR2_BASE_PORT, NULL),
                           0, 0, NULL);
  ...
  if (qr_init_socket(NULL,gt2GetSocketSOCKET(socket), QR2_GAME_NAME, secret_key,
  basic_callback, info_callback, rules_callback, players_callback, NULL) != 0)
  {
  ...
  */
  ...
  if (qr2_init_socket(NULL, gt2GetSocketSOCKET(socket), QR2_BASE_PORT,
                      QR2_GAME_NAME, secret_key, 1, natNegotiate,
