...
static int GS_STATIC_CALLBACK gti2SendFiltersCompare(const void *elem1,
                                                     const void *elem2) {
...
static int GS_STATIC_CALLBACK gti2ReceiveFiltersCompare(const void *elem1,
                                                        const void *elem2) {
...
GT2Bool gti2AddSendFilter(GT2Connection connection,
  ...
  return (ArraySearch(connection->sendFilters, &callback,
                      gti2SendFiltersCompare, 0, 0) != NOT_FOUND);
}
...
GT2Bool gti2AddReceiveFilter(GT2Connection connection,
  ...
  // Return GT2True if it was added.
  return (ArraySearch(connection->receiveFilters, &callback,
                      gti2ReceiveFiltersCompare, 0, 0) != NOT_FOUND);
}
...
void gti2RemoveSendFilter(GT2Connection connection,
  ...
  // Find it.
  index = ArraySearch(connection->sendFilters, &callback,
                      gti2SendFiltersCompare, 0, 0);
  if (index == NOT_FOUND)
    return;
...
void gti2RemoveReceiveFilter(GT2Connection connection,
  ...
  // Find it.
  index = ArraySearch(connection->receiveFilters, &callback,
                      gti2ReceiveFiltersCompare, 0, 0);
  if (index == NOT_FOUND)
    return;
...
GT2Bool gti2FilteredReceive(GT2Connection connection, int filterID,
  ...
  if (filterID == (num - 1)) {
    ...
    // Filter it.
    if (!gti2ReceiveFilterCallback(connection, ++filterID, message, len,
                                   reliable))
      return GT2False;
  }
