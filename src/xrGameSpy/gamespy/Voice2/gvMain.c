...
** CODEC **
**********/
GVBool gvSetCodec(GVCodec codec) { return gviSetCodec(codec); }

void gvSetCustomCodec(GVCustomCodecInfo *info) { gviSetCustomCodec(info); }

void gvGetCodecInfo(int *samplesPerFrame, int *encodedFrameSize,
                    int *bitsPerSecond) {
  if (samplesPerFrame)
  ...
  if (bitsPerSecond)
    *bitsPerSecond =
        (int)(8 * GVIBytesPerSecond * GVIEncodedFrameSize / GVIBytesPerFrame);
}

/****************
** Sample Rate **
****************/
void gvSetSampleRate(GVRate sampleRate) { gviSetSampleRate(sampleRate); }

GVRate gvGetSampleRate(void) { return gviGetSampleRate(); }
...
void gvFreeDevice(GVDevice device) { device->m_methods.m_freeDevice(device); }
...
void gvSetUnpluggedCallback(GVDevice device,
...
** CAPTURE **
************/
GVBool gvCapturePacket(GVDevice device, GVByte *packet, int *len,
                       GVFrameStamp *frameStamp, GVScalar *volume) {
  return device->m_methods.m_capturePacket(device, packet, len, frameStamp,
                                           volume);
}
...
void gvSetCaptureMode(GVDevice device, GVCaptureMode captureMode) {
  ...
  if ((device->m_captureMode == GVCaptureModeThreshold) &&
      (captureMode == GVCaptureModePushToTalk)) {
  ...
  if ((device->m_captureMode == GVCaptureModePushToTalk) &&
      (captureMode == GVCaptureModeThreshold)) {
...
** PLAYBACK **
*************/
void gvPlayPacket(GVDevice device, const GVByte *data, int len, GVSource source,
...
void gvSetGlobalMute(GVBool mute) { gviSetGlobalMute(mute); }

GVBool gvGetGlobalMute(void) { return gviGetGlobalMute(); }
...
GVBool gvGetCustomPlaybackAudio(GVDevice device, GVSample *audio,
                                int numSamples) {
  return gviGetCustomPlaybackAudio(device, audio, numSamples);
}

GVBool gvSetCustomCaptureAudio(GVDevice device, const GVSample *audio,
                               int numSamples, GVByte *packet, int *packetLen,
                               GVFrameStamp *frameStamp, GVScalar *volume) {
  return gviSetCustomCaptureAudio(device, audio, numSamples, packet, packetLen,
                                  frameStamp, volume);
}
...
void gvGetChannelName(GVDevice device, GVDeviceType type, int channel,
