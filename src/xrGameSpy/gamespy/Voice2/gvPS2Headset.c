...
static GVBool gviPlaybackDeviceThink(GVIDevice *device) {
  ...
  newBytes =
      (gviMultiplyByBytesPerMillisecond(GVI_PLAYBACK_STAYAHEAD_MILLISECONDS) -
       remainingBytes);
  ...
  for (i = 0; i < numFrames; i++) {
    ...
    result = lgAudWrite(data->m_handle, LGAUD_BLOCKMODE_BLOCKING,
                        (u_char *)data->m_playbackBuffer, &size);
    if (LGAUD_FAILED(result))
      return GVFalse;
...
int gviPS2HeadsetListDevices(GVDeviceInfo devices[], int maxDevices,
                             GVDeviceType types) {
...
enumerate:
  for (index = 0; numDevices < maxDevices; index++) {
    ...
    if (gviDoesSupportOurFormat(headsetDesc.RecordingFormats,
                                headsetDesc.RecordingFormatsCount))
      supportedTypes |= GV_CAPTURE;
    if (gviDoesSupportOurFormat(headsetDesc.PlaybackFormats,
                                headsetDesc.PlaybackFormatsCount))
    ...
    if (supportedTypes & types) {
      ...
      devices[numDevices].m_defaultDevice =
          (GVDeviceType)0; // ps2 doesn't support default devices
...
static GVBool gviPS2HeadsetIsDeviceStarted(GVIDevice *device,
                                           GVDeviceType type) {
  GVIPS2HeadsetData *data = (GVIPS2HeadsetData *)device->m_data;
...
static void gviPS2HeadsetSetDeviceVolume(GVIDevice *device, GVDeviceType type,
                                         GVScalar volume) {
  GVIPS2HeadsetData *data = (GVIPS2HeadsetData *)device->m_data;

  if (type & GV_PLAYBACK) {
    lgAudSetPlaybackVolume(data->m_handle, LGAUD_CH_BOTH,
                           (u_char)(volume * 100));
    data->m_playbackVolume = volume;
  }
  if (type & GV_CAPTURE) {
    lgAudSetRecordingVolume(data->m_handle, LGAUD_CH_BOTH,
                            (u_char)(volume * 100));
    data->m_captureVolume = volume;
  }
}

static GVScalar gviPS2HeadsetGetDeviceVolume(GVIDevice *device,
                                             GVDeviceType type) {
  GVIPS2HeadsetData *data = (GVIPS2HeadsetData *)device->m_data;
...
static void gviPS2HeadsetSetCaptureThreshold(GVIDevice *device,
                                             GVScalar threshold) {
  GVIPS2HeadsetData *data = (GVIPS2HeadsetData *)device->m_data;
...
static void gviProcessCapturedFrame(GVDevice device, GVByte *frameOut,
                                    GVSample *frameIn, GVScalar *volume,
                                    GVBool *threshold) {
  GVIPS2HeadsetData *data = (GVIPS2HeadsetData *)device->m_data;
...
static GVBool gviPS2HeadsetCapturePacket(GVDevice device, GVByte *packet,
  ...
  *frameStamp = data->m_captureClock;
  ...
  for (numFrames = 0; numFrames < framesAvailable; numFrames++) {
    ...
    result = lgAudRead(data->m_handle, LGAUD_BLOCKMODE_NOT_BLOCKING,
                       ((u_char *)data->m_captureBuffer) +
...
static void gviPS2HeadsetPlayPacket(GVIDevice *device, const GVByte *packet,
...
static int gviPS2HeadsetListTalkingSources(GVDevice device, GVSource sources[],
                                           int maxSources) {
  GVIPS2HeadsetData *data = (GVIPS2HeadsetData *)device->m_data;
...
static GVBool gviPS2HeadsetInitDevice(GVIDevice *device, int deviceIndex,
                                      GVDeviceType type) {
  ...
  if (type & GV_CAPTURE) {
    ...
    openParam.RecordingFormat.BufferMilliseconds =
        GVI_CAPTURE_BUFFER_MILLISECONDS;
  }
  if (type & GV_PLAYBACK) {
  ...
  if (type & GV_PLAYBACK) {
    ...
    data->m_playbackBuffer =
        (GVSample *)gsimalloc((unsigned int)GVIBytesPerFrame);
...
GVDevice gviPS2HeadsetNewDevice(GVDeviceID deviceID, GVDeviceType type) {
  ...
  // create a new device
  device = gviNewDevice(deviceID, GVHardwarePS2Headset, type,
                        sizeof(GVIPS2HeadsetData));
  if (!device)
    return NULL;
  ...
  device->m_methods.m_getAvailableCaptureBytes =
      gviPS2HeadsetGetAvailableCaptureBytes;
