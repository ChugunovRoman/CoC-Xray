...
#define GVI_PLAYBACK_SAMPLE_FACTOR                                             \
  (GVI_PLAYBACK_SAMPLE_RATE / gviGetSampleRate())
...
static GVBool gviPlaybackDeviceThink(GVIDevice *device) {
  ...
  // check the queue
  result =
      sys_event_queue_receive(data->m_playbackQueue, &playbackQueueEvent, 1);
  if (result == ETIMEDOUT) {
    return GVTrue;
  }
  if (result != CELL_OK) {
  ...
  currentBlock = (unsigned int)*(
      uint64_t *)(data->m_playbackCellAudioConfig.readIndexAddr);
  playbackReadPos = (currentBlock * GVI_PLAYBACK_BLOCK_SAMPLES);

  if (data->m_playbackPortPos == -1) {
    unsigned int nextBlock =
        (currentBlock + 1) %
        GVI_PLAYBACK_NUM_BLOCKS; // write target is next block
    data->m_playbackPortPos = (nextBlock * GVI_PLAYBACK_BLOCK_SAMPLES);
  }

  remainingSamples =
      (((playbackReadPos + totalSamples) - data->m_playbackPortPos) %
       totalSamples);

  // figure out the number of frames that we can write
  numFrames =
      ((remainingSamples / GVI_PLAYBACK_SAMPLE_FACTOR) / GVISamplesPerFrame);
  ...
  for (i = 0; i < numFrames; i++) {
    ...
    // converting from 8khz or 16KHz to 48khz and mono to stereo (write each
    // sample 6-12 times depending on sample rate)
    for (j = 0; j < GVISamplesPerFrame; j++) {
      ...
      for (k = 0; k < GVI_PLAYBACK_SAMPLE_FACTOR; k++) {
        float *dest = (float *)(data->m_playbackCellAudioConfig.portAddr +
                                (data->m_playbackPortPos * GVI_NUM_CHANNELS *
                                 sizeof(float)));
        *dest++ = sample;
        *dest = sample;
...
static GVBool gviPS3HeadsetInitHeadphone(GVIPS3HeadsetData *data) {
  ...
  // get the config for the audio port so we can use it to write data to the
  // audio port ring buffer
  result = cellAudioGetPortConfig(data->m_playbackCellAudioPortNum,
                                  &data->m_playbackCellAudioConfig);
  if (result != CELL_OK) {
    return GVFalse;
  }
  ...
  while (aCount < 10) {
    result =
        sys_event_queue_create(&data->m_playbackQueue, &aQueueAttr,
...
GVBool gviPS3HeadsetInitMic(GVIPS3HeadsetData *data) {
  ...
  while ((aCount++) < 100) {
    result = sys_event_queue_create(&data->m_captureCallbackQueue, &equeue_attr,
                                    data->m_captureEventQueueKey, 32);
    if (result == CELL_OK)
      break;
    data->m_captureEventQueueKey =
        GVI_CAPTURE_QUEUE_KEY_BASE | (rand() & 0xffff);
  }
  if (result != CELL_OK) {
    return GVFalse;
  }

  // install "MicIn" system-callback(with devnum == -1) to recv attach/detach
  // event
  ...
  if (aMsg == CELLMIC_ATTACH && aDevNum == data->m_deviceNum) {
    ...
    result = cellMicOpenEx(data->m_deviceNum, GVI_PLAYBACK_SAMPLE_RATE, 1,
                           GV_SAMPLES_PER_SECOND, GVI_PS3_MIC_BUFFER_MS,
                           CELLMIC_SIGTYPE_DSP);
...
static GVBool gviPS3HeadsetIsDeviceStarted(GVIDevice *device,
                                           GVDeviceType type) {
...
static void gviPS3HeadsetSetDeviceVolume(GVIDevice *device, GVDeviceType type,
                                         GVScalar volume) {
  GVIPS3HeadsetData *data = (GVIPS3HeadsetData *)device->m_data;
  ...
  if (type & GV_CAPTURE) {
    cellMicSetDeviceAttr(data->m_deviceNum, CELLMIC_DEVATTR_VOLUME,
                         (int)(volume * GVI_CAPTURE_VOLUME_MAX), 0);
...
static GVScalar gviPS3HeadsetGetDeviceVolume(GVIDevice *device,
                                             GVDeviceType type) {
  GVIPS3HeadsetData *data = (GVIPS3HeadsetData *)device->m_data;
...
static void gviPS3HeadsetSetCaptureThreshold(GVIDevice *device,
                                             GVScalar threshold) {
  GVIPS3HeadsetData *data = (GVIPS3HeadsetData *)device->m_data;
...
static void gviProcessCapturedFrame(GVDevice device, GVSample *frameIn,
                                    GVByte *frameOut, GVScalar *volume,
                                    GVBool *threshold) {
  GVIPS3HeadsetData *data = (GVIPS3HeadsetData *)device->m_data;
  ...
  if (threshold && !*threshold) {
    if (volume) {
      ...
      *threshold = gviIsOverThreshold(frameIn, GVISamplesPerFrame,
...
///////////////////////////////////////////////////////////////////////////////
// captures a packet
static GVBool gviPS3HeadsetCapturePacket(GVDevice device, GVByte *packet,
  ...
  *frameStamp = data->m_captureClock;
  ...
  for (numFrames = 0; numFrames < framesAvailable; numFrames++) {
    ...
    if (aCaptureQueueMsg == CELLMIC_ATTACH) {
      ...
      readSize = ((GVISamplesPerFrame - data->m_capturePreConvertBufferLen) *
                  sizeof(float));
      frameIn =
          data->m_capturePreConvertBuffer + data->m_capturePreConvertBufferLen;
      ...
      for (sample = 0; sample < GVISamplesPerFrame; sample++) {
        data->m_captureBuffer[sample] =
            (GVSample)((SHRT_MAX)*data->m_capturePreConvertBuffer[sample]);
      }

      // process the frame
      gviProcessCapturedFrame(device, data->m_captureBuffer, frameOut, volume,
                              &overThreshold);
  ...
  if (overThreshold) {
    ...
    overThreshold =
        ((GVFrameStamp)(*frameStamp - data->m_captureLastCrossedThresholdTime) <
...
static void gviPS3HeadsetPlayPacket(GVIDevice *device, const GVByte *packet,
...
static int gviPS3HeadsetListTalkingSources(GVDevice device, GVSource sources[],
                                           int maxSources) {
  GVIPS3HeadsetData *data = (GVIPS3HeadsetData *)device->m_data;
...
static GVBool gviPS3HeadsetInitDevice(GVIDevice *device, int deviceIndex,
                                      GVDeviceType type) {
  ...
  if (type & GV_PLAYBACK) {
    ...
    data->m_playbackBuffer =
        (GVSample *)gsimemalign(16, (unsigned int)(GVIBytesPerFrame));
  ...
  if (type & GV_CAPTURE) {
    ...
    data->m_captureLastCrossedThresholdTime =
        (GVFrameStamp)(data->m_captureClock - GVI_HOLD_THRESHOLD_FRAMES - 1);

    // allocate the buffer
    data->m_captureBuffer =
        (GVSample *)gsimemalign(16, (unsigned int)(GVIBytesPerFrame));
    ...
    data->m_capturePreConvertBuffer = (float *)gsimemalign(
        16, (unsigned int)(GVISamplesPerFrame * sizeof(float)));
...
GVDevice gviPS3HeadsetNewDevice(GVDeviceID deviceID, GVDeviceType type) {
  ...
  // create a new device
  device = gviNewDevice(deviceID, GVHardwarePS3Headset, type,
                        sizeof(GVIPS3HeadsetData));
  if (!device)
    return NULL;
  ...
  device->m_methods.m_getAvailableCaptureBytes =
      gviPS3HeadsetGetAvailableCaptureBytes;
