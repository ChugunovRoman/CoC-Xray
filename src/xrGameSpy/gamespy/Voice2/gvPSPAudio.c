...
static GVBool gviHardwareCapturePacket(GVDevice device, GVByte *packet,
  ...
  *len = 0;
  ...
  if (volume) {
    ...
    overThreshold =
        gviIsOverThreshold(readPtr, numSamples, data->m_captureThreshold);
  }
    ...
    for (i = 0; i < numFrames; i++) {
      ...
      if (device->m_captureFilterCallback)
        device->m_captureFilterCallback(
            device, readPtr, (GVFrameStamp)(data->m_captureClock + i));
...
static GVBool gviPSPAudioInitDevice(GVIDevice *device) {
  ...
  // create a semaphore
  data->m_captureBufferSemaphore =
      sceKernelCreateSema("capture buffer", SCE_KERNEL_SA_THFIFO, 1, 1, NULL);
  ...
  data->m_captureThreadID = sceKernelCreateThread(
      "capture", gviPSPCaptureThread, SCE_KERNEL_USER_HIGHEST_PRIORITY,
      GVI_CAPTURE_THREAD_STACK_SIZE, 0, NULL);
  ...
  rcode = sceKernelStartThread(data->m_captureThreadID, sizeof(GVIDevice *),
                               &device);
  if (rcode < 0) {
...
GVDevice gviHardwareNewDevice(GVDeviceID deviceID, GVDeviceType type) {
  ...
  // create the device
  device = gviNewDevice(deviceID, GVHardwarePSPHeadset, type,
