...
typedef enum { GVCaptureModeThreshold, GVCaptureModePushToTalk } GVCaptureMode;
...
typedef struct {
  ...
  void (*m_decodeAddCallback)(GVSample *out, const GVByte *in,
                              GVDecoderData data); // adds to output (required)
  void (*m_decodeSetCallback)(GVSample *out, const GVByte *in,
                              GVDecoderData data); // sets output (optional)
} GVCustomCodecInfo;
...
GVBool gvAreDevicesSetup(GVDeviceID captureDeviceID,
                         GVDeviceID playbackDeviceID);
#endif
...
typedef void (*gvFilterCallback)(GVDevice device, GVSample *audio,
...
// PLAYBACK
//////////
void gvPlayPacket(GVDevice device, const GVByte *packet, int len,
                  GVSource source, GVFrameStamp frameStamp, GVBool mute);
...
// samplesPerFrame this ensures that no data needs to be buffered by the SDK
