...
static GVISource *gviFindSourceInList(GVISourceList sourceList,
                                      GVSource source) {
  GVISource *gviSource;
  int i;
...
GVBool gviGetGlobalMute(void) { return GVIGlobalMute; }

static GVISource *gviAddSourceToList(GVISourceList sourceList,
                                     GVSource source) {
  GVBool result;
...
static void gviAddPacketToSource(GVISource *source, GVFrameStamp frameStamp,
                                 const GVByte *packet, int len, GVBool mute) {
  ...
  if (mute || GVIGlobalMute) {
    // Flag that they are currently talking because we skip this step in the
    // hardware.
...
void gviAddPacketToSourceList(GVISourceList sourceList, const GVByte *packet,
  ...
  if (!gviSource) {
    ...
    gviSource->m_finishedTalkingTime =
        (GVFrameStamp)(frameStamp + gviSource->m_clockOffset);
  } else {
    // unwrap the clock offset if needed
    if ((clockOffset < gviSource->m_clockOffsetAverage) &&
        gviIsFrameStampGT(clockOffset,
                          (GVFrameStamp)gviSource->m_clockOffsetAverage)) {
      unwrappedClockOffset = (clockOffset + GVI_FRAMESTAMP_MAX);
    } else if ((gviSource->m_clockOffsetAverage < clockOffset) &&
               gviIsFrameStampGT((GVFrameStamp)gviSource->m_clockOffsetAverage,
                                 clockOffset)) {
      unwrappedClockOffset = (clockOffset - GVI_FRAMESTAMP_MAX);
    } else {
    ...
    gviSource->m_clockOffsetAverage =
        (float)((gviSource->m_clockOffsetAverage *
                 GVI_CLOCK_OFFSET_AVERAGING_FACTOR) +
                (unwrappedClockOffset *
                 (1.0 - GVI_CLOCK_OFFSET_AVERAGING_FACTOR)));
    ...
    divergence = (GVFrameStamp)(gviSource->m_clockOffset -
                                (GVFrameStamp)gviSource->m_clockOffsetAverage);
  ...
  for (i = 0; i < GVI_MAX_SOURCES; i++) {
    ...
    while (source->m_frameHead.m_next) {
      ...
      if (gviIsFrameStampGTE(frame->m_frameStamp, startTime)) {
        ...
        GVSample *writePtr =
            (sampleBuffer + ((GVFrameStamp)(frame->m_frameStamp - startTime) *
                             GVISamplesPerFrame));
#if GVI_PRE_DECODE
        int j;
    ...
    if (!source->m_frameHead.m_next &&
        gviIsFrameStampGTE(timeSliceEnd, source->m_finishedTalkingTime))
