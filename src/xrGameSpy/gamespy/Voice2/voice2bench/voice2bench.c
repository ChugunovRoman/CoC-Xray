...
static void Cleanup(void) { gviLGCodecCleanup(); }

static int GetSamplesPerFrame(void) { return gviLGCodecGetSamplesPerFrame(); }
...
static void Cleanup(void) { gsm_destroy(EncoderState); }

static int GetSamplesPerFrame(void) { return 160; }
...
static void FreeDecoder(GVDecoderData data) { gsm_destroy((gsm)data); }
...
static void Cleanup(void) { gviSpeexCleanup(); }

static int GetSamplesPerFrame(void) { return gviSpeexGetSamplesPerFrame(); }

static GVBool NewDecoder(GVDecoderData *data) {
  return gviSpeexNewDecoder(data);
}

static void FreeDecoder(GVDecoderData data) { gviSpeexFreeDecoder(data); }

static void Encode(GVByte *out, const GVSample *in) { gviSpeexEncode(out, in); }
...
static void TestCodec(GVCodec codec, GVRate sampleRate, const char *name) {
  ...
  sampleTime =
      (unsigned long)((double)sizeof(voice_sample) /
                      ((int)(sampleRate)*GV_BYTES_PER_SAMPLE) * 1000000);
