...
GVBool gviLGCodecInitialize(const char *name) {
  ...
  GVILGCodecDecodeBuffer =
      (GVSample *)gsimalloc((unsigned int)codecDesc->bytes_per_pcm_frame);
  ...
  GVILGCodecSamplesPerFrame =
      (int)(GVILGCodecBytesPerFrame / GV_BYTES_PER_SAMPLE);
...
int gviLGCodecGetSamplesPerFrame(void) { return GVILGCodecSamplesPerFrame; }

int gviLGCodecGetEncodedFrameSize(void) { return GVILGCodecEncodedFrameSize; }
...
void gviLGCodecDecodeAdd(GVSample *out, const GVByte *in, GVDecoderData data) {
  ...
  lgCodecDecode(GVILGCodecHandle, in, GVILGCodecEncodedFrameSize,
                GVILGCodecDecodeBuffer, &destSize);
