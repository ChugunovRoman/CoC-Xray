...
static void CustomDecodeAddCallback(GVSample *out, const GVByte *in,
                                    GVDecoderData data) {
...
static void CustomDecodeSetCallback(GVSample *out, const GVByte *in,
                                    GVDecoderData data) {
  const GVSample *sampleIn = (const GVSample *)in;
  int i;
...
static void HandleCapturedPacket(const GVByte *packet, int len,
...
#if SEND_PACKETS
  sendto(sock, (char *)packet, len, 0, (SOCKADDR *)&sendAddr,
         sizeof(SOCKADDR_IN));
#endif
...
static GVBool Initialize(const char *remoteIP) {
  ...
  iReturn = cellSpursInitialize(myCellSpurs, iNumSpus, spuThreadPriority,
                                ppuThreadPriority, exitIfNoWork);
  if (iReturn != CELL_OK) {
  ...
  customCodecInfo.m_encodedFrameSize =
      (CUSTOM_CODEC_SAMPLES_PER_FRAME * GV_BYTES_PER_SAMPLE);
...
static int PSPPlaybackThread(SceSize args, void *argp) {
  ...
  while (1) {
    ...
    // write it
    rcode = sceWaveAudioWriteBlocking(PSP_CHANNEL, PSP_VOLUME, PSP_VOLUME,
                                      PSPBuffer[PSPBufferIndex]);
    if (rcode < 0) {
...
static GVBool PSPStartup(void) {
  ...
  PSPPlaybackThreadID =
      sceKernelCreateThread("playback", PSPPlaybackThread,
...
static void ShowInfo(void) {
  ...
  if ((now - lastPrintTime) > 1000) {
    ...
    // see who's talking
    num = gvListTalkingSources(playDevice, sources,
                               sizeof(sources) / sizeof(sources[0]));
    printf("Talking sources:");
    for (i = 0; i < num; i++)
      printf(" %s:%d", inet_ntoa(sources[i].sin_addr),
             ntohs(sources[i].sin_port));
    printf("\n");
  }
#endif
}
...
static void Think(void) {
  ...
  do {
    ...
    rcode = recvfrom(sock, (char *)packet, sizeof(packet), 0,
  ...
  if (gvIsDeviceStarted(capDevice, GV_CAPTURE)) {
    do {
      ...
      memset(packet, 0, sizeof(packet));
      result =
          gvCapturePacket(capDevice, packet, &packetLen, &frameStamp, &volume);
      if (result) {
        // handle the capture packet
        frameStamp = htons(frameStamp);
        memcpy(packet + packetLen, &frameStamp, sizeof(GVFrameStamp));
        HandleCapturedPacket(packet, packetLen + sizeof(GVFrameStamp),
                             frameStamp, volume);
      }
    } while (result);
  }
...
static GVBool CheckInput(void) {
  ...
  if (PadData.len > 0) {
    ...
    if (PadData.button[CELL_PAD_BTN_OFFSET_DIGITAL2] & CELL_PAD_CTRL_SQUARE) {
      if ((capDevice && playDevice) &&
          !gvIsDeviceStarted(capDevice, GV_CAPTURE_AND_PLAYBACK)) {
