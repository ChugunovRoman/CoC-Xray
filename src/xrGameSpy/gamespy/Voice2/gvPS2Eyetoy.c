...
enumerate:
  for (index = 0; numDevices < maxDevices; index++) {
    ...
    for (modeIndex = 0; modeIndex < eyetoyDesc.SupportedModesCount;
         modeIndex++) {
      if (eyetoyDesc.SupportedModes[modeIndex].OperatingMode.AudioRate ==
          LGVID_AUD_8000) {
...
#else
      strncpy(devices[numDevices].m_name, eyetoyDesc.FriendlyName,
              GV_DEVICE_NAME_LEN);
...
static GVBool gviPS2EyetoyIsDeviceStarted(GVIDevice *device,
                                          GVDeviceType type) {
  GVIPS2EyetoyData *data = (GVIPS2EyetoyData *)device->m_data;
...
static void gviPS2EyetoySetDeviceVolume(GVIDevice *device, GVDeviceType type,
                                        GVScalar volume) {
  GVIPS2EyetoyData *data = (GVIPS2EyetoyData *)device->m_data;
...
static GVScalar gviPS2EyetoyGetDeviceVolume(GVIDevice *device,
                                            GVDeviceType type) {
  GVIPS2EyetoyData *data = (GVIPS2EyetoyData *)device->m_data;
  lgVidCameraControl cameraControl;
  int result;
...
static void gviPS2EyetoySetCaptureThreshold(GVIDevice *device,
                                            GVScalar threshold) {
  GVIPS2EyetoyData *data = (GVIPS2EyetoyData *)device->m_data;
...
static GVBool gviPS2EyetoyCapturePacket(GVDevice device, GVByte *packet,
  ...
  *frameStamp = data->m_captureClock;
  ...
  frameOut = packet;
  for (numFrames = 0;
       (numFrames < framesAvailable) && (numFrames < framesCaptured);
       numFrames++) {
    // process the frame
  ...
  if (numFrames < framesAvailable) {
    ...
    if (audioDesc.TimeStamp != -1) {
      ...
      for (; numFrames < framesAvailable; numFrames++) {
        ...
        // process the frame
        gviProcessCapturedFrame(device, frameOut, frameIn, volume,
                                &overThreshold);
      ...
      if (data->m_captureBufferBytes)
        memmove(data->m_captureBuffer, frameIn,
                (unsigned)data->m_captureBufferBytes);
    }
  }
...
static GVBool gviPS2EyetoyInitDevice(GVIDevice *device, int deviceIndex,
                                     GVDeviceType type) {
  lgVidDeviceDesc eyetoyDesc;
  ...
  // TODO: make this better at choosing the video mode that uses the least CPU
  // and memory (ideally none at all)
  ...
  memcpy(&openParam.OperatingMode, &eyetoyDesc.SupportedModes[i].OperatingMode,
         sizeof(lgVidOperatingMode));
...
GVDevice gviPS2EyetoyNewDevice(GVDeviceID deviceID, GVDeviceType type) {
  ...
  // create a new device
  device = gviNewDevice(deviceID, GVHardwarePS2Eyetoy, type,
                        sizeof(GVIPS2EyetoyData));
  if (!device)
    return NULL;

  // init the device
  result = gviPS2EyetoyInitDevice(
      device, (int)(deviceID & ~GVI_EYETOY_DEVICEID_BIT), type);
  if (result == GVFalse) {
    gviFreeDevice(device);
    return NULL;
  }
