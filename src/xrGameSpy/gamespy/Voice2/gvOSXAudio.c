...
static void gviPushFirstFrame(GVICapturedFrame *frameHead,
                              GVICapturedFrame *frame) {
  frame->m_next = frameHead->m_next;
  frameHead->m_next = frame;
}

static void gviPushLastFrame(GVICapturedFrame *frameHead,
                             GVICapturedFrame *frame) {
...
GVBool gviHardwareStartup(void) {
  ...
  GVIVoiceFormat.mFormatFlags = kAudioFormatFlagsNativeEndian |
...
static int gviCountChannels(AudioDeviceID deviceID, bool input) {
  ...
  result = AudioDeviceGetPropertyInfo(
      deviceID, 0, input, kAudioDevicePropertyStreamConfiguration, &size, NULL);
  if (result != noErr)
    return 0;
  ...
  result = AudioDeviceGetProperty(
      deviceID, 0, input, kAudioDevicePropertyStreamConfiguration, &size, list);
  if (result != noErr) {
    gsifree(list);
    return 0;
  }
...
static GVBool gviFillDeviceInfo(AudioDeviceID deviceID, GVDeviceInfo *device,
                                GVDeviceType types) {
  OSStatus result;
  UInt32 size;
  UInt32 isAlive;
  ...
  size = sizeof(UInt32);
  result = AudioDeviceGetProperty(
      deviceID, 0, true, kAudioDevicePropertyDeviceIsAlive, &size, &isAlive);
  ...
  size = sizeof(nameRef);
  result = AudioDeviceGetProperty(
      deviceID, 0, (supportedTypes & GV_CAPTURE) ? true : false,
      kAudioDevicePropertyDeviceNameCFString, &size, &nameRef);
  if (result != noErr)
    return GVFalse;
...
int gviHardwareListDevices(GVDeviceInfo devices[], int maxDevices,
  ...
  // get the device ids
  result =
      AudioHardwareGetProperty(kAudioHardwarePropertyDevices, &size, deviceIDs);
  if (result != noErr) {
    gsifree(deviceIDs);
    return 0;
  }
  ...
  for (i = 0; (i < maxDevices) && (deviceCount < numDevices); i++) {
    ...
    GVBool addedDevice =
        gviFillDeviceInfo(deviceIDs[i], &devices[deviceCount], types);
...
static OSStatus gviAudioConverterCaptureProc(
  ...
  GVICaptureConverterData *converterData =
      (GVICaptureConverterData *)inUserData;
  ...
  ioData->mBuffers[0].mNumberChannels =
      bufferList->mBuffers[data->m_captureChannel].mNumberChannels;
  ioData->mBuffers[0].mData =
      bufferList->mBuffers[data->m_captureChannel].mData;
  ioData->mBuffers[0].mDataByteSize =
      bufferList->mBuffers[data->m_captureChannel].mDataByteSize;

  // fill in the number of samples we provided
  *ioNumberDataPackets = (ioData->mBuffers[0].mDataByteSize /
                          data->m_captureStreamDescriptor.mBytesPerPacket);
  ...
  if (data->m_capturing) {
    ...
    do {
      ...
      result = AudioConverterFillComplexBuffer(
          data->m_captureConverter, gviAudioConverterCaptureProc,
          &converterData, &size, &bufferList, NULL);
      ...
      if (result == noErr) {
        ...
        // if there aren't any available frames, repurpose the oldest captured
        // frame
        if (!frame) {
...
static OSStatus gviAudioConverterPlaybackProc(
    AudioConverterRef inAudioConverter, UInt32 *ioNumberDataPackets,
    AudioBufferList *ioData,
    AudioStreamPacketDescription **outDataPacketDescription, void *inUserData) {
  ...
  if (data->m_playing) {
    ...
    wroteToBuffer =
        gviWriteSourcesToBuffer(data->m_playbackSources, data->m_playbackClock,
                                data->m_playbackBuffer, 1);
  }
  ...
  // check if we need to adjust the volume
    ...
    for (i = 0; i < GVISamplesPerFrame; i++)
      data->m_playbackBuffer[i] =
          (GVSample)(data->m_playbackBuffer[i] * data->m_playbackVolume);
  }

  // filter
  if (device->m_playbackFilterCallback)
    device->m_playbackFilterCallback(device, data->m_playbackBuffer,
...
static OSStatus gviHardwarePlaybackIOProc(AudioDeviceID inDevice,
  ...
  size = (outOutputData->mBuffers[0].mDataByteSize /
          data->m_playbackStreamDescriptor.mBytesPerFrame);
  ...
  result = AudioConverterFillComplexBuffer(data->m_playbackConverter,
                                           gviAudioConverterPlaybackProc,
                                           device, &size, &bufferList, NULL);
  if (result != noErr)
    return (OSStatus)1;
...
static GVBool gviHardwareStartPlayback(GVDevice device) {
  ...
  // add the IO proc
  result = AudioDeviceAddIOProc((AudioDeviceID)device->m_deviceID,
                                gviHardwarePlaybackIOProc, device);
  if (result != noErr) {
...
static void gviHardwareStopDevice(GVDevice device, GVDeviceType type) {
  ...
  if ((type & GV_CAPTURE) && data->m_capturing) {
    ...
    // stop io
    AudioDeviceStop((AudioDeviceID)device->m_deviceID,
                    gviHardwareCaptureIOProc);
    ...
    // remove the IO proc
    AudioDeviceRemoveIOProc((AudioDeviceID)device->m_deviceID,
                            gviHardwareCaptureIOProc);
...
static void gviSetDeviceVolume(GVDevice device, GVBool isInput,
                               GVScalar volume) {
  GVIHardwareData *data = (GVIHardwareData *)device->m_data;
...
static void gviSetDeviceVolume(GVDevice device, GVBool isInput,
  ...
  for (channel = 0; channel < 3; channel++)
    AudioDeviceSetProperty(device->m_deviceID, NULL, channel, isInput,
                           kAudioDevicePropertyVolumeScalar, sizeof(Float32),
                           &vol);
}

static GVBool gviGetChannelVolume(GVDevice device, GVBool isInput, int channel,
                                  Float32 *volume) {
  OSStatus result;
  ...
  result =
      AudioDeviceGetProperty(device->m_deviceID, channel, isInput,
...
static void gviHardwareSetDeviceVolume(GVDevice device, GVDeviceType type,
                                       GVScalar volume) {
  GVIHardwareData *data = (GVIHardwareData *)device->m_data;
...
static void gviHardwareSetCaptureThreshold(GVDevice device,
                                           GVScalar threshold) {
  GVIHardwareData *data = (GVIHardwareData *)device->m_data;
...
static GVBool gviHardwareCapturePacket(GVDevice device, GVByte *packet,
                                       int *len, GVFrameStamp *frameStamp,
                                       GVScalar *volume) {
  GVIHardwareData *data = (GVIHardwareData *)device->m_data;
  int numFrames;
  GVICapturedFrame *frame;
  ...
  *len = 0;
  ...
  do {
    ...
    if (overThreshold) {
      ...
      overThreshold = ((GVFrameStamp)(frame->m_frameStamp -
    ...
    if (overThreshold) {
      ...
      if (data->m_captureVolume < 1.0) {
        ...
        for (j = 0; j < GVISamplesPerFrame; j++)
          frame->m_frame[j] =
              (GVSample)(frame->m_frame[j] * data->m_captureVolume);
      }

      // filter
      if (device->m_captureFilterCallback)
        device->m_captureFilterCallback(device, frame->m_frame,
                                        frame->m_frameStamp);
...
static void gviHardwarePlayPacket(GVDevice device, const GVByte *packet,
                                  int len, GVSource source,
                                  GVFrameStamp frameStamp, GVBool mute) {
  GVIHardwareData *data = (GVIHardwareData *)device->m_data;
  ...
  if (data->m_playing) {
    // add it
    gviAddPacketToSourceList(data->m_playbackSources, packet, len, source,
                             frameStamp, mute, data->m_playbackClock);
  }
...
static int gviHardwareListTalkingSources(GVDevice device, GVSource sources[],
                                         int maxSources) {
  GVIHardwareData *data = (GVIHardwareData *)device->m_data;
  int numTalking = 0;
  ...
  if (data->m_playing) {
    // list them
    numTalking =
        gviListTalkingSources(data->m_playbackSources, sources, maxSources);
  }
...
static int gviHardwareGetNumChannels(GVDevice device, GVDeviceType type) {
  ...
  result = AudioDeviceGetPropertyInfo(device->m_deviceID, 0,
                                      (type & GV_CAPTURE) ? true : false,
...
static void gviHardwareGetChannelName(GVDevice device, GVDeviceType type,
                                      int index,
  ...
  // get the stream IDs
  result = AudioDeviceGetProperty(
      device->m_deviceID, 0, (type & GV_CAPTURE) ? true : false,
      kAudioDevicePropertyStreams, &size, streamIDs);
  if (result == noErr) {
    // get the CFString for this stream
    size = sizeof(CFStringRef);
    result = AudioStreamGetProperty(streamIDs[index], 0,
...
static void gviHardwareSetChannel(GVDevice device, GVDeviceType type,
                                  int index) {
  GVIHardwareData *data = (GVIHardwareData *)device->m_data;
  int num;
...
OSStatus gviPropertyListener(AudioDeviceID inDevice, UInt32 inChannel,
                             Boolean isInput,
  ...
  result =
      AudioDeviceGetProperty(device->m_deviceID, 0, isInput,
                             kAudioDevicePropertyDeviceIsAlive, &len, &value);
...
static GVBool gviHardwareInitCapture(GVIDevice *device) {
  ...
  result = AudioDeviceGetProperty(device->m_deviceID, 0, true,
                                  kAudioDevicePropertyStreamFormat, &size,
                                  &data->m_captureStreamDescriptor);
  if (result != noErr)
    return GVFalse;

  // create a converter from the capture format to the GV format
  result = AudioConverterNew(&data->m_captureStreamDescriptor, &GVIVoiceFormat,
                             &data->m_captureConverter);
  if (result != noErr)
    return GVFalse;
  ...
  numCaptureBufferBytes =
      gviMultiplyByBytesPerMillisecond(GVI_CAPTURE_BUFFER_MILLISECONDS);
  numCaptureBufferBytes =
      gviRoundUpToNearestMultiple(numCaptureBufferBytes, GVIBytesPerFrame);
  numCaptureBufferFrames = (numCaptureBufferBytes / GVIBytesPerFrame);
  for (i = 0; i < numCaptureBufferFrames; i++) {
    frame = (GVICapturedFrame *)gsimalloc(sizeof(GVICapturedFrame) +
                                          GVIBytesPerFrame - sizeof(GVSample));
    if (!frame) {
      gviFreeCapturedFrames(&data->m_captureAvailableFrames);
      gsifree(data->m_captureBuffer);
      AudioConverterDispose(data->m_captureConverter);
  ...
  data->m_captureLastCrossedThresholdTime =
      (data->m_captureClock - GVI_HOLD_THRESHOLD_FRAMES - 1);

  // add property listener
  AudioDeviceAddPropertyListener(device->m_deviceID, 0, true,
                                 kAudioDevicePropertyDeviceIsAlive,
                                 gviPropertyListener, device);
...
static GVBool gviHardwareInitPlayback(GVIDevice *device) {
  ...
  size = sizeof(AudioStreamBasicDescription);
  result = AudioDeviceGetProperty(device->m_deviceID, 0, false,
                                  kAudioDevicePropertyStreamFormat, &size,
                                  &data->m_playbackStreamDescriptor);
  if (result != noErr) {
    gviFreeSourceList(data->m_playbackSources);
    return GVFalse;
  }
  ...
  result = AudioConverterNew(&GVIVoiceFormat, &data->m_playbackStreamDescriptor,
                             &data->m_playbackConverter);
  if (result != noErr) {
    gviFreeSourceList(data->m_playbackSources);
    return GVFalse;
  }
  ...
  result = AudioConverterSetProperty(data->m_playbackConverter,
                                     kAudioConverterPrimeMethod, sizeof(UInt32),
                                     &primeMethod);
  if (result != noErr) {
    AudioConverterDispose(data->m_playbackConverter);
    gviFreeSourceList(data->m_playbackSources);
  ...
  result = AudioConverterGetPropertyInfo(
      data->m_playbackConverter, kAudioConverterChannelMap, &size, NULL);
  if (result == noErr) {
    result =
        AudioConverterGetProperty(data->m_playbackConverter,
                                  kAudioConverterChannelMap, &size, channelMap);
    if (result == noErr) {
      for (i = 0; i < (size / sizeof(SInt32)); i++)
        channelMap[i] = 0;

      AudioConverterSetProperty(data->m_playbackConverter,
                                kAudioConverterChannelMap, size, channelMap);
    }
  }
  ...
  // add property listener
  AudioDeviceAddPropertyListener(device->m_deviceID, 0, false,
                                 kAudioDevicePropertyDeviceIsAlive,
                                 gviPropertyListener, device);
...
GVDevice gviHardwareNewDevice(GVDeviceID deviceID, GVDeviceType type) {
  ...
  // create a new device
  device =
      gviNewDevice(deviceID, GVHardwareMacOSX, type, sizeof(GVIHardwareData));
  if (!device)
    return NULL;
