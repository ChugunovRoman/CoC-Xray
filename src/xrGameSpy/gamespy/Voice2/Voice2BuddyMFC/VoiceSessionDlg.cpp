...
void CVoiceSessionDlg::OnTimer(UINT nIDEvent) {
  ...
  bool isLocalSpeaking =
      (GVTrue == gvIsSourceTalking(m_SetupInfo.m_PlaybackDevice, 0));
  bool isRemoteSpeaking =
      (GVTrue == gvIsSourceTalking(m_SetupInfo.m_PlaybackDevice, m_RemoteAddr));
  ...
  if (aBytesAvailable > 0) {
    ...
    GVBool gotPacket = gvCapturePacket(
        m_SetupInfo.m_CaptureDevice, aVoicePacket.mBuffer,
        &aVoicePacket.mDataLength, &aVoicePacket.mFrameStamp, &aVolume);
    if (gotPacket == GVTrue) {
      // Playback for local echo
      // gvPlayPacket(m_SetupInfo.m_PlaybackDevice, aVoicePacket.mBuffer,
      // aVoicePacket.mDataLength, 0, aVoicePacket.mFrameStamp);

      // Contruct a message for the buddy
      int aTotalPacketSize = gVoicePacketHeaderSize + aVoicePacket.mDataLength;
      gt2Send(m_Connection, (GT2Byte *)&aVoicePacket, aTotalPacketSize,
              GT2False);
    }
  }
...
void GT2ConnectedCallback(GT2Connection theConnection, GT2Result theResult,
                          unsigned char *theMessage, int theLength) {
...
void GT2ReceivedCallback(GT2Connection theConnection, unsigned char *theBuffer,
                         int theLength, int wasReliable) {
  // Get the ptr to the dialog
  CVoiceSessionDlg *aDlg = (CVoiceSessionDlg *)gt2GetSocketData(
      gt2GetConnectionSocket(theConnection));
  ...
  gvPlayPacket(aDlg->m_SetupInfo.m_PlaybackDevice, aVoicePacket->mBuffer,
               aVoicePacket->mDataLength, aSource, aVoicePacket->mFrameStamp,
               0);
...
GT2Bool GT2UnrecognizedMessageCallback(GT2Socket theSocket, unsigned int theIp,
                                       unsigned short thePort, GT2Byte *theData,
                                       int theLength) {
...
void GT2ConnectAttemptCallback(GT2Socket theSocket, GT2Connection theConnection,
                               unsigned int theIp, unsigned short thePort,
                               int theLatency, GT2Byte *theMessage,
                               int theLength) {
  CVoiceSessionDlg *aDialog = (CVoiceSessionDlg *)gt2GetSocketData(theSocket);
...
void NNCompletedCallback(NegotiateResult theResult, SOCKET theSocket,
                         sockaddr_in *theAddr, void *theParam) {
  CVoiceSessionDlg *aDialog = (CVoiceSessionDlg *)theParam;
  ...
  if (theResult == nr_success) {
    ...
    CString aAddrString;
    aAddrString.Format("%s:%d", inet_ntoa(theAddr->sin_addr),
                       ntohs(theAddr->sin_port));
    ...
    // ...otherwise connect to the host
      ...
      int aTimeout = 0;
      GT2Result aResult =
          gt2Connect(aDialog->m_Socket, &aDialog->m_Connection, aAddrString,
                     NULL, 0, aTimeout, &aCallbackList, GT2False);
...
BOOL CVoiceSessionDlg::OnInitDialog() {
  ...
  GT2Result aResult =
      gt2CreateSocket(&m_Socket, NULL, 0, 0, GT2SocketErrorCallback);
  if (aResult != GT2Success) {
  ...
  NNBeginNegotiationWithSocket(aWinSocket, m_NNCookie, m_IsHost,
                               NNProgressCallback, NNCompletedCallback, this);
