...
void CLoginDlg::OnOK() {
  ...
  HKEY aRegKey = NULL;
  LONG aResult = RegCreateKeyEx(
      HKEY_CURRENT_USER, "Software\\GameSpy\\Voice2BuddyMFC", 0, NULL,
      REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &aRegKey, NULL);
  if (aResult == ERROR_SUCCESS) {
    RegSetValue(aRegKey, "LastLoginNickname", REG_SZ, m_Nickname,
                m_Nickname.GetLength());
    RegSetValue(aRegKey, "LastLoginEmail", REG_SZ, m_Email,
                m_Email.GetLength());
    RegCloseKey(aRegKey);
  }
...
void CLoginDlg::OnCancel() { CDialog::OnCancel(); }
...
BOOL CLoginDlg::OnInitDialog() {
  ...
  if (aResult == ERROR_SUCCESS) {
    ...
    aLongBuf = 255;
    aResult =
        RegQueryValue(aRegKey, "LastLoginNickname", aStringBuf, &aLongBuf);
    if (aResult == ERROR_SUCCESS)
