...
enum SetupMessage { V2B_STARTDEVICES = 0 };
...
BOOL CSetupDlg::OnInitDialog() {
  ...
  int aNumDevices = gvListDevices(m_SetupInfo->m_DeviceInfoArray, MAX_DEVICES,
                                  GV_CAPTURE_AND_PLAYBACK);
  ...
  for (int aDeviceNum = 0; aDeviceNum < aNumDevices; aDeviceNum++) {
    ...
    if (GV_CAPTURE & m_SetupInfo->m_DeviceInfoArray[aDeviceNum].m_deviceType) {
      // Set the item data to the device number (this may differ from the row
      // due to sorting)
      m_CaptureCombo.InsertString(
          0, m_SetupInfo->m_DeviceInfoArray[aDeviceNum].m_name);
      m_CaptureCombo.SetItemData(0, aDeviceNum);
      if (GV_CAPTURE &
          m_SetupInfo->m_DeviceInfoArray[aDeviceNum].m_defaultDevice)
...
void CSetupDlg::OnSelChangeCaptureCombo() { StartSelCaptureDevice(); }

// Switch the active device
void CSetupDlg::OnSelChangePlaybackCombo() { StartSelPlaybackDevice(); }
...
void CSetupDlg::StartSelCaptureDevice() {
  ...
  m_SetupInfo->m_CaptureDevice =
      gvNewDevice(m_SetupInfo->m_DeviceInfoArray[aDeviceNum].m_id, GV_CAPTURE);
  if (m_SetupInfo->m_CaptureDevice != NULL)
...
void CSetupDlg::StartSelPlaybackDevice() {
  ...
  // Create and start the new device
  m_SetupInfo->m_PlaybackDevice =
      gvNewDevice(m_SetupInfo->m_DeviceInfoArray[aDeviceNum].m_id, GV_PLAYBACK);
  if (m_SetupInfo->m_PlaybackDevice != NULL)
...
void CSetupDlg::OnTimer(UINT nIDEvent) {
  ...
  int aBytesAvailable =
      gvGetAvailableCaptureBytes(m_SetupInfo->m_CaptureDevice);
  if (aBytesAvailable > 0) {
    ...
    GVBool gotPacket = gvCapturePacket(m_SetupInfo->m_CaptureDevice, aBuffer,
                                       &aLength, &aFrameStamp, &aVolume);
    if (gotPacket == GVTrue) {
      gvPlayPacket(m_SetupInfo->m_PlaybackDevice, aBuffer, aLength, 0,
                   aFrameStamp, gMute);
