...
static int gEEStreanIntr =
    0; // flag for when an endpoint is reached by the audio play head.
static int gEEDataSampleCount =
    0; // how many samples do we have buffered up? ( 0 - 256 )
...
static void EEStreamClearSPUBuffer() {
  ...
  sceSdRemote(1, rSdVoiceTrans, AUTODMA_CH,
              SD_TRANS_MODE_WRITE | SD_TRANS_BY_DMA, IOPBuffer, CORE_INPUT_L,
              0x800);
  sceSdRemote(1, rSdVoiceTrans, AUTODMA_CH,
              SD_TRANS_MODE_WRITE | SD_TRANS_BY_DMA, IOPBuffer + IOP_BUFF_SIZE,
              CORE_INPUT_R, 0x800);
...
static void AudioUpSample(gsi_i16 *Dest48Khz, const gsi_i16 *Src8Khz,
                          int num8Khzsamples)
...
// numsamples = Src samples (8khz )
{
  ...
  Dest48Khz[1] =
      (gsi_u16)(a * -0.058f + b * 0.938f + c * 0.132f + d * -0.012f); // t= 1/6;
  Dest48Khz[2] =
      (gsi_u16)(a * -0.074f + b * 0.778f + c * 0.333f + d * -0.037f); // t= 2/6;
  Dest48Khz[3] =
      (gsi_u16)(a * -0.063f + b * 0.563f + c * 0.563f + d * -0.063f); // t= 3/6;
  Dest48Khz[4] =
      (gsi_u16)(a * -0.037f + b * 0.333f + c * 0.778f + d * -0.074f); // t= 4/6;
  Dest48Khz[5] =
      (gsi_u16)(a * -0.012f + b * 0.132f + c * 0.938f + d * -0.058f); // t= 5/6;
...
// copies and converts.  returns numsamples read
{
  ...
  AudioUpSample((gsi_i16 *)&EEBuffer[gEEDataSampleCount], (const gsi_i16 *)data,
                num8Khzsamples);
...
static void EEStreamConvertAndTransfer(GVIDevice *device, int which) {
  ...
  do {
    ...
    // push it
    samplesPushed =
        EEStreamDataPush(data->m_playbackBuffer, GVISamplesPerFrame);
  ...
  // Do a memcpy from EE to IOP memory using DMA, respecting 256 sample
  // interleave
...
void gviPS2Spu2Think(void) {
  ...
  // Check if an interrupt has occured.  IF this were a separate thread, it
  // could sleep until this happens.
  if (!gEEStreanIntr)
    return;
...
static void gviPS2Spu2SetDeviceVolume(GVIDevice *device, GVDeviceType type,
                                      GVScalar volume) {
  GVIPS2Spu2Data *data = (GVIPS2Spu2Data *)device->m_data;
  gsi_i16 sVol;
...
static GVScalar gviPS2Spu2GetDeviceVolume(GVIDevice *device,
                                          GVDeviceType type) {
  GVIPS2Spu2Data *data = (GVIPS2Spu2Data *)device->m_data;
...
static void gviPS2Spu2PlayPacket(GVIDevice *device, const GVByte *packet,
...
static int gviPS2Spu2ListTalkingSources(GVDevice device, GVSource sources[],
                                        int maxSources) {
  GVIPS2Spu2Data *data = (GVIPS2Spu2Data *)device->m_data;
...
static GVBool gviPS2Spu2InitDevice(GVIDevice *device, int deviceIndex,
                                   GVDeviceType type) {
  GVIPS2Spu2Data *data = (GVIPS2Spu2Data *)device->m_data;
  ...
  // they are double buffered, with one half being read while the other is
  // dma'ed into from the EE
  ...
  // clear interrupt flag.  After this each interrupt, this flag will be set
  // to 1.
  gEEStreanIntr = 0;
...
GVDevice gviPS2Spu2NewDevice(GVDeviceID deviceID, GVDeviceType type) {
  ...
  // create a new device
  device =
      gviNewDevice(deviceID, GVHardwarePS2Spu2, type, sizeof(GVIPS2Spu2Data));
  if (!device)
    return NULL;
