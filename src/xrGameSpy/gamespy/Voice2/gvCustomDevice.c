...
static void gviCustomSetDeviceVolume(GVIDevice *device, GVDeviceType type,
                                     GVScalar volume) {
  GVICustomData *data = (GVICustomData *)device->m_data;
...
static void gviCustomSetCaptureThreshold(GVIDevice *device,
                                         GVScalar threshold) {
  GVICustomData *data = (GVICustomData *)device->m_data;
...
static GVBool gviCustomCapturePacket(GVDevice device, GVByte *packet, int *len,
...
static void gviCustomPlayPacket(GVIDevice *device, const GVByte *packet,
...
static int gviCustomListTalkingSources(GVDevice device, GVSource sources[],
                                       int maxSources) {
  GVICustomData *data = (GVICustomData *)device->m_data;
...
GVDevice gviCustomNewDevice(GVDeviceType type) {
  ...
  // create a new device
  device = gviNewDevice(GVICustomDeviceID, GVHardwareCustom, type,
                        sizeof(GVICustomData));
  if (!device)
    return NULL;
  ...
  device->m_methods.m_getAvailableCaptureBytes =
      gviCustomGetAvailableCaptureBytes;
...
GVBool gviGetCustomPlaybackAudio(GVIDevice *device, GVSample *audio,
  ...
  // fill it
  wroteToBuffer = gviWriteSourcesToBuffer(
      data->m_playbackSources, data->m_playbackClock, audio, numFrames);
  ...
  if (device->m_playbackFilterCallback) {
    for (i = 0; i < numFrames; i++)
      device->m_playbackFilterCallback(
          device, audio + (GVISamplesPerFrame * i),
  ...
  *frameStamp = data->m_captureClock;
  ...
  if (audio) {
    ...
    if (volume) {
      ...
      overThreshold =
          gviIsOverThreshold(audio, numSamples, data->m_captureThreshold);
    }
      ...
      for (i = 0; i < numFrames; i++) {
        ...
        if (device->m_captureFilterCallback)
          device->m_captureFilterCallback(
              device, (GVSample *)audio + (i * GVISamplesPerFrame),
              (GVFrameStamp)(data->m_captureClock + i));

        // encode the buffer into the packet
        gviEncode(packet + (i * GVIEncodedFrameSize),
                  audio + (i * GVISamplesPerFrame));
      }
    }
