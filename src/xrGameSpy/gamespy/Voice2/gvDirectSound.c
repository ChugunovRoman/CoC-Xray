...
const GUID GVDefaultPlaybackDeviceID = {
    0xdef00002, 0x9c6d, 0x47ed, 0xaa, 0xf1, 0x4d, 0xda, 0x8f, 0x2b, 0x5c, 0x03};
const GUID GVDefaultCaptureDeviceID = {
...
static GVBool gviPlaybackDeviceThink(GVIDevice *device) {
  ...
  result = IDirectSoundBuffer_GetCurrentPosition(data->m_playbackBuffer,
                                                 &rawPlayCursor, &writeCursor);
  if (FAILED(result))
    return GVFalse;

  // we only want to deal with whole frames
  playCursor = (DWORD)gviRoundDownToNearestMultiple((int)rawPlayCursor,
                                                    GVIBytesPerFrame);

  // get the number of new bytes
  newBytes = (((playCursor + data->m_playbackBufferSize) -
               data->m_playbackBufferPosition) %
              data->m_playbackBufferSize);
  ...
  if (diff >=
      gviDivideByBytesPerMillisecond((int)data->m_playbackBufferHalfSize)) {
    int numMissedLoops;
    int numMissedFrames;
    int msPerBuffer =
        gviDivideByBytesPerMillisecond((int)data->m_playbackBufferSize);
    ...
    // convert to frames
    numMissedFrames =
        (numMissedLoops * (data->m_playbackBufferSize / GVIBytesPerFrame));

    // adjust the clock
    // Expanded to remove warnings in VS2K5
    data->m_playbackClock =
        data->m_playbackClock + (GVFrameStamp)(numMissedFrames);
  }
  ...
  result = IDirectSoundBuffer_Lock(data->m_playbackBuffer, lockPosition,
                                   newBytes, &audioPtr1, &audioLen1, &audioPtr2,
                                   &audioLen2, 0);
  if (FAILED(result))
    return GVFalse;
  ...
  if (device->m_playbackFilterCallback) {
    for (i = 0; i < numFrames; i++)
      device->m_playbackFilterCallback(
          device, (GVSample *)audioPtr1 + (GVISamplesPerFrame * i),
          (GVFrameStamp)(data->m_playbackClock + i));
  }
  ...
  if (audioPtr2) {
    ...
    if (device->m_playbackFilterCallback) {
      for (i = 0; i < numFrames; i++)
        device->m_playbackFilterCallback(
            device, (GVSample *)audioPtr2 + (GVISamplesPerFrame * i),
            (GVFrameStamp)(data->m_playbackClock + i));
    }
  ...
  // unlock the buffer
  result = IDirectSoundBuffer_Unlock(data->m_playbackBuffer, audioPtr1,
                                     audioLen1, audioPtr2, audioLen2);
  if (FAILED(result))
    return GVFalse;
...
static BOOL CALLBACK gviEnumDevicesCallback(LPGUID lpGuid, LPCSTR desc,
                                            LPCSTR module, LPVOID lpContext)
#else
static BOOL CALLBACK gviEnumDevicesCallback(LPGUID lpGuid, LPCWSTR desc,
                                            LPCWSTR module, LPVOID lpContext)
#endif
{
  if (lpGuid != NULL) {
...
static GVBool gviStartPlaybackDevice(GVIHardwareData *data) {
  ...
  // lock the buffer
  result = IDirectSoundBuffer_Lock(data->m_playbackBuffer, 0, 0, &audioPtr1,
                                   &audioLen1, &audioPtr2, &audioLen2,
  ...
  // start the buffer
  result =
      IDirectSoundBuffer_Play(data->m_playbackBuffer, 0, 0, DSBPLAY_LOOPING);
  if (FAILED(result))
    return GVFalse;
...
static void gviHardwareSetDeviceVolume(GVIDevice *device, GVDeviceType type,
                                       GVScalar volume) {
  GVIHardwareData *data = (GVIHardwareData *)device->m_data;
...
static GVScalar gviHardwareGetDeviceVolume(GVIDevice *device,
                                           GVDeviceType type) {
  GVIHardwareData *data = (GVIHardwareData *)device->m_data;
...
static void gviHardwareSetCaptureThreshold(GVIDevice *device,
                                           GVScalar threshold) {
  GVIHardwareData *data = (GVIHardwareData *)device->m_data;
...
static int gviHardwareGetAvailableCaptureBytes(GVDevice device) {
  ...
  // get the current position
  result = IDirectSoundCaptureBuffer_GetCurrentPosition(data->m_captureBuffer,
                                                        NULL, &readPosition);
  if (FAILED(result)) {
    gviDeviceUnplugged(device);
    return 0;
  ...
  // get the number of new bytes
  newBytes = (((readPosition + data->m_captureBufferSize) -
               data->m_captureBufferPosition) %
              data->m_captureBufferSize);
...
static GVBool gviHardwareCapturePacket(GVDevice device, GVByte *packet,
  ...
  *len = 0;
  ...
  if (data->m_capturing) {
    // lock the buffer
    result = IDirectSoundCaptureBuffer_Lock(
        data->m_captureBuffer, data->m_captureBufferPosition, newBytes,
        &audioPtr1, &audioLen1, &audioPtr2, &audioLen2, 0);
    if (FAILED(result)) {
      gviDeviceUnplugged(device);
      return GVFalse;
    }
    ...
    if (volume) {
      GVScalar vol1 =
          gviGetSamplesVolume(audioPtr1, audioLen1 / GV_BYTES_PER_SAMPLE);
      GVScalar vol2 =
          gviGetSamplesVolume(audioPtr2, audioLen2 / GV_BYTES_PER_SAMPLE);
    ...
    if (volume) {
      ...
      overThreshold = gviIsOverThreshold(
          audioPtr1, audioLen1 / GV_BYTES_PER_SAMPLE, data->m_captureThreshold);

      // if not over threshold, and there is a second portion of audio, check it
      if (!overThreshold && audioPtr2)
        overThreshold =
            gviIsOverThreshold(audioPtr2, audioLen2 / GV_BYTES_PER_SAMPLE,
                               data->m_captureThreshold);
    }
    ...
    if (overThreshold) {
      ...
      overThreshold = ((GVFrameStamp)(data->m_captureClock -
      ...
      for (i = 0; i < numFrames; i++) {
        ...
        if (device->m_captureFilterCallback)
          device->m_captureFilterCallback(
              device, audioPtr, (GVFrameStamp)(data->m_captureClock + i));
    ...
    // unlock the buffer
    result = IDirectSoundCaptureBuffer_Unlock(data->m_captureBuffer, audioPtr1,
                                              audioLen1, audioPtr2, audioLen2);
    if (FAILED(result)) {
      gviDeviceUnplugged(device);
      return GVFalse;
...
static void gviHardwarePlayPacket(GVIDevice *device, const GVByte *packet,
...
static GVBool gviInitPlaybackDevice(GVIHardwareData *data) {
  ...
  result =
      IDirectSound_SetCooperativeLevel(data->m_playback, GVIHwnd, DSSCL_NORMAL);
  if (FAILED(result))
    return GVFalse;

  // setup the buffer size
  data->m_playbackBufferSize =
      gviMultiplyByBytesPerMillisecond(GVI_PLAYBACK_BUFFER_MILLISECONDS);

  // make sure it is a multiple of twice the raw frame size
  // it needs to be twice the size because it will be split in two
  data->m_playbackBufferSize = gviRoundUpToNearestMultiple(
      data->m_playbackBufferSize, GVIBytesPerFrame * 2);
  ...
  // create the buffer
  result = IDirectSound_CreateSoundBuffer(data->m_playback, &bufferDescriptor,
                                          &data->m_playbackBuffer, NULL);
  if (FAILED(result))
    return GVFalse;
...
static GVBool gviInitCaptureDevice(GVIHardwareData *data) {
  ...
  data->m_captureBufferSize =
      gviRoundUpToNearestMultiple(data->m_captureBufferSize, GVIBytesPerFrame);
  ...
  result = IDirectSoundCapture_CreateCaptureBuffer(
      data->m_capture, &bufferDescriptor, &data->m_captureBuffer, NULL);
  if (FAILED(result))
    return GVFalse;

  // start the buffer
  result =
      IDirectSoundCaptureBuffer_Start(data->m_captureBuffer, DSCBSTART_LOOPING);
  if (FAILED(result)) {
    IDirectSoundCaptureBuffer_Release(data->m_captureBuffer);
...
GVDevice gviHardwareNewDevice(GVDeviceID deviceID, GVDeviceType type) {
  ...
  GVIWaveFormat.nBlockAlign =
      ((GVIWaveFormat.nChannels * GVIWaveFormat.wBitsPerSample) / 8);
  GVIWaveFormat.nAvgBytesPerSec =
      (GVIWaveFormat.nSamplesPerSec * GVIWaveFormat.nBlockAlign);

  // create a new device
  device = gviNewDevice(deviceID, GVHardwareDirectSound, type,
                        sizeof(GVIHardwareData));
  if (!device)
    return NULL;
...
static LPDIRECTPLAYVOICETEST GetVoiceTestInstance(void) {
  ...
  result =
      CoCreateInstance(&CLSID_DirectPlayVoiceTest, NULL, CLSCTX_INPROC_SERVER,
                       &IID_IDirectPlayVoiceTest, (LPVOID *)&voiceTest);
...
GVBool gvRunSetupWizard(GVDeviceID captureDeviceID,
  ...
  // run the dialog
  result = IDirectPlayVoiceTest_CheckAudioSetup(voiceTest, &playbackDeviceID,
                                                &captureDeviceID, GVIHwnd, 0);

  // releate the instance
  IDirectPlayVoiceTest_Release(voiceTest);

  // check for success
  if ((result == DV_OK) || (result == DV_FULLDUPLEX) ||
      (result == DV_HALFDUPLEX))
    return GVTrue;
...
GVBool gvAreDevicesSetup(GVDeviceID captureDevice, GVDeviceID playbackDevice) {
  ...
  // run the dialog
  result = IDirectPlayVoiceTest_CheckAudioSetup(
      voiceTest, &playbackDevice, &captureDevice, GVIHwnd, DVFLAGS_QUERYONLY);
