...
// amount of time to keep capturing even after voice drops below the capture
// threshold makes sure that speech that trails off, or slightly dips in volume,
// is still captured
...
typedef struct {
  ...
  void (*m_setDeviceVolume)(GVDevice device, GVDeviceType type,
                            GVScalar volume);
...
GVIDevice *gviNewDevice(GVDeviceID deviceID, GVHardwareType hardwareType,
                        GVDeviceType types, int dataSize);
