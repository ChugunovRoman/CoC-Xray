...
typedef struct piActivePing {
  ...
  pingerGotPing reply; // call this callback if we're the originator (otherwise
                       // call pingerPinged)
...
static PINGERBool piBytesToPing(unsigned char *buffer, piUDPPing *udpPing,
                                char *data) {
  assert(buffer != NULL);
  assert(udpPing != NULL);
  assert(data != NULL);
...
static PINGERBool piSendPing(SOCKADDR_IN *to, uint16 trip, uint16 ID_A,
                             uint16 ID_B, const char *data) {
  ...
  //////////////////////////
  rcode = sendto(piSocket, (char *)buffer, PINGER_UDP_PING_SIZE, 0,
...
static PINGERBool piSocketInit(const char *localAddress,
  ...
  rcode = setsockopt(piSocket, SOL_SOCKET, SO_REUSEADDR, (const char *)&bFlag,
                     sizeof(bFlag));
#endif
...
static int GS_STATIC_CALLBACK piFindActivePingCompareFn(const void *elem1,
                                                        const void *elem2) {
...
static void piProcessTrip1(piUDPPing *udpPing, const char *data,
                           SOCKADDR_IN *from, gsi_time recvTime) {
...
static void piProcessTrip2(piUDPPing *udpPing, const char *data,
                           SOCKADDR_IN *from, gsi_time recvTime) {
  char dataOut[PI_DATA_MAX_LEN];
  int index;
  piActivePing *activePing;
  ...
  if (activePing->reply != NULL) {
    ...
    // Add the callback.
    ////////////////////
    piQueueCallback(from->sin_addr.s_addr, from->sin_port, ping, data,
                    PI_DATA_MAX_LEN, activePing->replyParam, activePing->reply);
  }
...
static void piProcessTrip3(piUDPPing *udpPing, const char *data,
                           SOCKADDR_IN *from, gsi_time recvTime) {
  int index;
  piActivePing *activePing;
...
static void piProcessPing(piUDPPing *udpPing, const char *data,
                          SOCKADDR_IN *from, gsi_time recvTime) {
  assert(udpPing != NULL);
  assert(data != NULL);
  assert(from != NULL);
...
static void piProcessIncoming(void) {
  ...
  while (piInitialized && CanReceiveOnSocket(piSocket)) {
    ...
    rcode = recvfrom(piSocket, (char *)buffer, PINGER_UDP_PING_SIZE, 0,
                     (SOCKADDR *)&from, &len);
      ...
      if (activePing->timeout != 0) {
        ...
        if (activePing->timeout <= now) {
          ...
          if ((activePing->originator) && (activePing->reply != NULL)) {
            // Queue the callback.
            //////////////////////
            piQueueCallback(activePing->remoteIP, activePing->remotePort,
                            PINGER_TIMEOUT, NULL, 0, activePing->replyParam,
                            activePing->reply);
          }
