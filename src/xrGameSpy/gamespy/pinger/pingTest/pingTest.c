...
void pinged(unsigned int IP, unsigned short port, int ping, const char *data,
            int len, void *param) {
  IN_ADDR addr;
  addr.S_un.S_addr = IP;
  ...
  printf("PINGED!\n"
         "%s:%d %dms",
         inet_ntoa(addr), ntohs(port), ping);
...
void reply(unsigned int IP, unsigned short port, int ping, const char *data,
           int len, void *param) {
  IN_ADDR addr;
  addr.S_un.S_addr = IP;
...
void setData(unsigned int IP, unsigned short port, char *data, int len,
             void *param) {
  char buffer[32];
