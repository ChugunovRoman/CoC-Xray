...
This program is provided as a diagnostic tool for the Query and Reporting 2 SDK,
but is also provided here as a sample to demonstrate how to query a single
server for full rules using the Server Browisng SDK.
...
void PrintResultsCallback(ServerBrowser sb, SBCallbackReason reason,
  ...
  if (reason == sbc_serverupdated) {
    printf("Got server data: %s:%d [%d]\nKeys/Values:\n",
           ...
           ntohs(server->publicport), SBServerGetPing(server));
...
int test_main(int argc, char **argp) {
  ...
  sb = ServerBrowserNew("gmtest", "gmtest", "HA6zkS", 0, 10, QVERSION_QR2,
                        SBFalse, PrintResultsCallback, NULL);
  // port 11111 for qr2csample
  ServerBrowserAuxUpdateIP(sb, argp[1], (unsigned short)atoi(argp[2]), SBFalse,
                           SBTrue, SBTrue);
