...
GLOBAL VARS
********/
static gsi_bool UpdateFinished =
    gsi_false; // used to track status of server browser updates
...
static void DebugCallback(GSIDebugCategory theCat, GSIDebugType theType,
  ...
  GSI_UNUSED(theLevel);
  printf("[%s][%s] ", gGSIDebugCatStrings[theCat],
...
static void AppDebug(const unsigned short *format, ...) {
  ...
  gsDebugFormat(GSIDebugCat_App, GSIDebugType_Misc, GSIDebugLevel_Notice, "%s",
                tmp);
}
#else
...
static void SBCallback(ServerBrowser sb, SBCallbackReason reason,
                       SBServer server, void *instance) {
  int i; // for-loop ctr
  gsi_char *defaultString =
      _T(""); // default string for SBServerGet functions - returns if specified
              // string key is not found
  int defaultInt = 0; // default int value for SBServerGet functions - returns
                      // if specified int key is not found
  ...
  case sbc_serveradded: // new SBServer added to the server browser list
    // output the server's IP and port (the rest of the server's basic keys may
    // not yet be available)
    AppDebug(_T("Server Added: %s:%d\n"), anAddress,
             SBServerGetPublicQueryPort(server));
    break;
  case sbc_serverchallengereceived: // received ip verification challenge from
                                    // server
    // informational, no action required
    break;
  case sbc_serverupdated: // either basic or full information is now available
                          // for this server
    // retrieve and print the basic server fields (specified as a parameter in
    // ServerBrowserUpdate)
    ...
    if (SBServerHasFullKeys(server)) {
           ...
           i++) // loop through all players on the server
      {
        ...
        AppDebug(_T("    %s\n"), SBServerGetPlayerStringValue(
                                     server, i, _T("player"), defaultString));
        AppDebug(_T("      Score: %d\n"),
  ...
  case sbc_serverupdatefailed:
    AppDebug(_T("Update Failed: %s:%d\n"), anAddress,
             SBServerGetPublicQueryPort(server));
    break;
  case sbc_updatecomplete: // update is complete; server query engine is now
                           // idle (not called upon AuxUpdate completion)
    AppDebug(_T("Server Browser Update Complete\r\n"));
    UpdateFinished =
        gsi_true; // this will let us know to stop calling ServerBrowserThink
    break;
  case sbc_queryerror: // the update returned an error
    AppDebug(_T("Query Error: %s\n"), ServerBrowserListQueryError(sb));
    UpdateFinished =
        gsi_true; // set to true here since we won't get an updatecomplete call
    break;
  default:
    break;
  }
...
int test_main(int argc, char **argp) {
  ...
  int version = 0; // ServerBrowserNew parameter; set to 0 unless otherwise
                   // directed by GameSpy
  int maxConcUpdates = 20; // max number of queries the ServerBrowsing SDK will
                           // send out at one time
  SBBool lanBrowse = SBFalse; // set true for LAN only browsing
  void *userData = NULL; // optional data that will be passed to the SBCallback
                         // function after updates

  /* ServerBrowserUpdate parameters */
  SBBool async = SBTrue; // we will run the updates asynchronously
  SBBool discOnComplete =
      SBTrue; // disconnect from the master server after completing update
              // (future updates will automatically re-connect)
  // these will be the only keys retrieved on server browser updates
  unsigned char basicFields[] = {HOSTNAME_KEY, GAMETYPE_KEY, MAPNAME_KEY,
                                 NUMPLAYERS_KEY, MAXPLAYERS_KEY};
  int numFields = sizeof(basicFields) / sizeof(basicFields[0]);
  gsi_char serverFilter[100] = {
      '\0'}; // filter string for server browser updates
  ...
  SBCompareMode compareMode =
      sbcm_int; // we are sorting integers (as opposed to floats or strings)
  ...
  SBServer
      server; // used to hold each server when iterating through the server list
  int totalServers; // keep track of the total number of servers in our server
                    // list
  gsi_char *defaultString = _T(""); // default string for SBServerGet functions
                                    // - returns if specified string key is not
                                    // found

  // for debug output on these platforms
#if defined(_PS3) || defined(_PS2) || defined(_PSP) || defined(_NITRO)
#ifdef GSI_COMMON_DEBUG Define GSI_COMMON_DEBUG if you want to view the SDK debug output
  ...
  sb = ServerBrowserNew(GAME_NAME, GAME_NAME, secret_key, version,
                        maxConcUpdates, QVERSION_QR2, lanBrowse, SBCallback,
                        userData);
  ...
  ServerBrowserUpdate(sb, async, discOnComplete, basicFields, numFields,
                      serverFilter);

  // think while the update is in progress
  while ((ServerBrowserThink(sb) == sbe_noerror) &&
         (UpdateFinished == gsi_false))
    msleep(10); // think should be called every 10-100ms; quicker calls produce
                // more accurate ping measurements
                /** End Update **/
  ...
  if (totalServers == 0)
    ...
    for (i = 0; i < totalServers; i++) {
      server = ServerBrowserGetServer(
          sb, i); // get the SBServer object at index 'i' in the server list
  ...
  ServerBrowserClear(
      sb); // need to clear first so we don't end up with duplicates

  AppDebug(_T("US servers with more than 5 players, or servers with a ")
           _T("hostname containing 'GameSpy'\n\n"));
  // filter in US servers that have more than 5 players, or any server
  // containing 'GameSpy' in the hostname
  _tcscpy(
      serverFilter,
      _T("(country = 'US' and numplayers > 5) or hostname like '%GameSpy%'"));
  // note that filtering by "ping" is not possible, since ping is determined by
  // the client - not the master server

  // begin the update (async)
  ServerBrowserUpdate(sb, async, discOnComplete, basicFields, numFields,
                      serverFilter);

  UpdateFinished = gsi_false; // this was set to true from the last update, so
                              // we set it back until the new update completes
  ...
  /** If the qr2 sample server is running, we will do an AuxUpdate to retrieve
   * its full keys **/
  ...
  if (totalServers == 0)
    ...
    for (i = 0; i < totalServers; i++) {
      ...
      if (!(_tcscmp(
              SBServerGetStringValue(server, _T("hostname"), defaultString),
              _T("GameSpy QR2 Sample")))) {

        AppDebug(_T("Found it!\n\nRunning AuxUpdate to get more specific ")
                 _T("server info:\n\n"));
        // update the qr2 sample server object to contain its full keys
        ServerBrowserAuxUpdateServer(sb, server, async, fullUpdate);
        // Note: Only call this on a server currently in the server list;
        // otherwise call ServerBrowserAuxUpdateIP

        // think once again while the update is in progress; done once the
        // server object has full keys
        while ((ServerBrowserThink(sb) == sbe_noerror) &&
               !(SBServerHasFullKeys(server)))
          msleep(10);

        serverFound = 1;
        break; // already found the qr2 sample server, no need to loop through
               // the rest
      }
    }
    if (serverFound == 0)
