...
static void NatNegCompletedCallback(NegotiateResult result, SOCKET gamesocket,
  ...
  if (result == nr_success) {
    sb->ConnectCallback(sb, sbcs_succeeded, gamesocket, remoteaddr,
                        sb->instance);
  } else {
...
SBError ServerBrowserConnectToServer(ServerBrowser sb, SBServer server,
  ...
  sbError = ServerBrowserSendNatNegotiateCookieToServerA(
      sb, SBServerGetPublicAddress(server), SBServerGetPublicQueryPort(server),
      cookie);
  if (sbError != sbe_noerror)
    return sbError;

  // start the nn
  nnError = NNBeginNegotiation(cookie, 0, NatNegProgressCallback,
                               NatNegCompletedCallback, sb);
