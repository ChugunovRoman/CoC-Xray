...
BOOL CSbmfcsampleDlg::OnInitDialog() {
  ...
  m_serverList.InsertColumn(COL_SERVERNAME, "Server Name", LVCFMT_LEFT, 150,
                            -1);
...
void CSbmfcsampleDlg::OnRefresh() {
  ...
  unsigned char fields[] = {HOSTNAME_KEY, NUMPLAYERS_KEY, MAXPLAYERS_KEY,
                            MAPNAME_KEY, GAMETYPE_KEY};
  ...
  SBError error;
  if (internet)
    error = ServerBrowserUpdate(m_serverBrowser, SBTrue, SBFalse, fields,
                                numFields, (char *)(const char *)m_filter);
  else
    error = ServerBrowserLANUpdate(m_serverBrowser, SBTrue,
...
void CSbmfcsampleDlg::OnClickServerlist(NMHDR *pNMHDR, LRESULT *pResult) {
  ...
  for (int i = 0; i < count; i++) {
    m_playerList.InsertItem(
        0, SBServerGetPlayerStringValue(server, i, "player", "(NO NAME)"));
    m_playerList.SetItem(0, COL_PPING, LVIF_TEXT,
                         SBServerGetPlayerStringValue(server, i, "ping", "0"),
                         -1, 0, 0, 0);
    m_playerList.SetItem(0, COL_PSCORE, LVIF_TEXT,
                         SBServerGetPlayerStringValue(server, i, "score", "0"),
                         -1, 0, 0, 0);
  }
...
void CSbmfcsampleDlg::OnDblclkServerlist(NMHDR *pNMHDR, LRESULT *pResult) {
  ...
  // MessageBox("If this were a real server browser, you would be launched
  // now!","Go!");
  ...
  SBServer server = (SBServer)item.lParam;
  SBError error =
      ServerBrowserConnectToServer(m_serverBrowser, server, SBConnectCallback);
...
void CSbmfcsampleDlg::OnColumnclickServerlist(NMHDR *pNMHDR, LRESULT *pResult) {
  ...
  case COL_SERVERNAME:
    ServerBrowserSort(m_serverBrowser, (SBBool)ascending, "hostname",
                      sbcm_stricase);
    break;
  case COL_PING:
    ...
    break;
  case COL_PLAYERS:
    ServerBrowserSort(m_serverBrowser, (SBBool)ascending, "numplayers",
                      sbcm_int);
    break;
  case COL_MAPNAME:
    ServerBrowserSort(m_serverBrowser, (SBBool)ascending, "mapname",
                      sbcm_stricase);
    break;
  case COL_GAMETYPE:
    ServerBrowserSort(m_serverBrowser, (SBBool)ascending, "gametype",
                      sbcm_stricase);
    break;
  }
...
void CSbmfcsampleDlg::SBCallback(ServerBrowser serverBrowser,
                                 SBCallbackReason reason, SBServer server,
                                 void *instance) {
  CString str;
  ...
  CString address;
  if (server)
    address.Format("%s:%d", SBServerGetPublicAddress(server),
...
void CSbmfcsampleDlg::SBConnectCallback(ServerBrowser serverBrowser,
...
BOOL CSbmfcsampleDlg::CreateServerList() {
  ...
  if (!m_serverBrowser) {
    ...
    // create it
    m_serverBrowser = ServerBrowserNew(
        m_gamename, "gmtest", "HA6zkS", 0, MAX_UPDATES,
        (IsDlgButtonChecked(IDC_GOA) == BST_CHECKED) ? QVERSION_GOA
                                                     : QVERSION_QR2,
        lanBrowsing, SBCallback, this);
    if (!m_serverBrowser) {
...
void CSbmfcsampleDlg::AddServer(SBServer server, BOOL checkForReplace) {
  ...
  if (ServerBrowserCount(m_serverBrowser) > 0)
    m_progress.SetPos((ServerBrowserCount(m_serverBrowser) -
                       ServerBrowserPendingQueryCount(m_serverBrowser)) *
                      100 / ServerBrowserCount(m_serverBrowser));
  ...
  const char *hostname =
      SBServerGetStringValue(server, "hostname", "(NO NAME)");
  if (replace) {
    m_serverList.SetItem(index, COL_SERVERNAME, LVIF_TEXT, hostname, -1, 0, 0,
                         NULL);
  } else {
    m_serverList.InsertItem(index, hostname);
    m_serverList.SetItem(index, COL_SERVERNAME, LVIF_PARAM, NULL, -1, 0, 0,
                         (LPARAM)server);
  }
  ...
  if (SBServerHasValidPing(server))
    ping.Format("%d%s", SBServerGetPing(server),
                SBServerDirectConnect(server) ? "" : "i");
  else
    ping = "Unknown";
  players.Format("%d/%d", numplayers,
  ...
  m_serverList.SetItem(index, COL_MAPNAME, LVIF_TEXT,
                       SBServerGetStringValue(server, "mapname", "(NO MAP)"),
                       -1, 0, 0, 0);
  m_serverList.SetItem(index, COL_GAMETYPE, LVIF_TEXT,
                       SBServerGetStringValue(server, "gametype", ""), -1, 0, 0,
                       0);
