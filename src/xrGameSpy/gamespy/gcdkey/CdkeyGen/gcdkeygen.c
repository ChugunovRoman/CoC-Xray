...
a mathematically correct key, without knowing the algorithm, is 1 in 65
thousand. You could make that 1 in 4 billion by using a 32 bit check value.
However, even if a key is mathematically correct, it is not necessarily valid.
...
non-random algorithm (for example, every 1000000'th seed) then you can figure
out all the valid CD Keys from 1 valid one.
...
void DoGenerateKeys() {
  ...
  while (keyct != 0) {
    seed =
        base + Util_RandInt(
                   0, interval); // pick a number between this base and the next
    ...
    check = ((short)(seed % 65535)) ^
            0x9249; // these are "secret" check calculation values

    sprintf(&key[11], "%.8x",
            check); // add the check as the last 4 chars (but it prints 8 chars)
