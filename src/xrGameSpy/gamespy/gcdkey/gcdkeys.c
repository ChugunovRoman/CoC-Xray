...
PROTOTYPES
********/
static void send_auth_req(gsproduct_t *prod, gsclient_t *client,
...
static void send_uon(int skey, const char *ignored, const char *proof,
                     struct sockaddr_in *fromaddr);
static void free_client_node(gsnode_t *node);

static int get_sockaddrin(char *host, int port, struct sockaddr_in *saddr,
...
int gcd_init_qr2(qr2_t qrec, int gameid) {
  ...
  localport =
      (unsigned short)-1; /* we don't process any incoming data ourselves - it
                             gets passed from the QR SDK */
...
void gcd_authenticate_user(int gameid, int localid, unsigned int userip,
                           const char *challenge, const char *response,
                           AuthCallBackFn authfn,
  ...
  char hkey[33];
  char *errmsg = NULL;
  char badcdkey_t[] = {'B', 'a', 'd', ' ', 'C', 'D',
                       ' ', 'K', 'e', 'y', '\0'}; // Bad CD Key
  char keyinuse_t[] = {'C', 'D', ' ', 'K', 'e', 'y', ' ',
                       'i', 'n', ' ', 'u', 's', 'e', '\0'}; // CD Key in use
...
void gcd_process_reauth(int gameid, int localid, int skey,
                        const char *response) {
  ...
  if (node) {
    ...
    if (client) {
      ...
      while (client->reauthq.next != NULL) {
        gsnode_t *authNode =
            remove_from_queue(client->reauthq.next, &client->reauthq);
...
void gcd_think(void) {
  ...
  if (localport != (unsigned short)-1) /* don't check if we are getting data
                                          from the QR SDK instead */
  {
    FD_ZERO(&set);
    FD_SET(sock, &set);
    while (1) {
      ...
      if (gsiSocketIsError(error) || 0 == error)
        break;
      /* else we have data */
      error = recvfrom(sock, indata, INBUF_LEN - 1, 0,
          ...
          while (authnode->next != NULL) {
            ...
            if ((now - authdata->starttime) > REAUTH_LIFESPAN) {
              gsDebugFormat(
                  ...
                  "Removing timed out reauth request [localid: %d, from: "
                  "%s\r\n",
...
static gsclient_t *find_client(char *keyhash, int sesskey, int *productid) {
  ...
  for (i = 0; i < numproducts; i++) {
    ...
    while ((node = node->next) != NULL) {
      gsclient_t *client = (gsclient_t *)node->object;
      if (strcmp(keyhash, client->hkey) == 0 &&
          (sesskey == -1 || client->sesskey == sesskey)) {
        if (productid != NULL)
...
static void process_oks(char *buf, int isok) {
  ...
  \unok\\cd\fe6667736f0c8ed7ff5cd9c0e74f\skey\23423\errmsg\Already playing on
  xyz server */
...
static void process_ucount(char *buf, struct sockaddr_in *fromaddr) {
  ...
  sendto(sock, outbuf, len, 0, (struct sockaddr *)fromaddr,
...
static void send_uon(int skey, const char *ignored, const char *proof,
  ...
  const char uonformat[] = {
      '\\', 'u',       'o',  'n', '\\', '\\',        's',  'k', 'e',
      'y',  '\\',      '%',  'd', '\\', IGNORED_TXT, '\\', '%', 's',
      '\\', PROOF_TXT, '\\', '%', 's',  '\0'}; //\\uon\\\\skey\\%d\\seed\\%s\\proof\\%s
  /* \ison\\cd\fe6667736f0c8ed7ff5cd9c0e74f\skey\32423 */
  /* \uon\\skey\32423\seed\\proof\ OR
   * \un\skey\32423\proof\fe6667736f0c8ed7ff5cd9c0e74f OR \uoff\\skey\32423 */
...
static void process_ison(char *buf, struct sockaddr_in *fromaddr) {
  ...
  if ((client = find_client(value_for_key(buf, cd_t), -1, &productid)) !=
          NULL &&
      (client->state == cs_done)) /* If they are connected, return on */
  {
    // check the queue size to prevent memory growth (from malicious reauth
    // requests)
    ...
    if (count < MAXPENDING_REAUTH) {
      ...
      client->refreshauthfn(productid, client->localid, sesskey, proofchallenge,
                            client->instance);
    }
  } else {
...
static void send_auth_req(gsproduct_t *prod, gsclient_t *client,
  ...
  int len;
  const char
      authformat[] = {'\\', 'a',  'u',  't',  'h',  '\\', '\\', 'p',
                      'i',  'd',  '\\', '%',  'd',  '\\', 'c',  'h',
                      '\\', '%',  's',  '\\', 'r',  'e',  's',  'p',
                      '\\', '%',  's',  '\\', 'i',  'p',  '\\', '%',
                      'd',  '\\', 's',  'k',  'e',  'y',  '\\', '%',
                      'd',  '\\', 'r',  'e',  'q',  'p',  'r',  'o',
                      'o',  'f',  '\\', '1',  '\\', '\0'}; //\\auth\\\\pid\\%d\\ch\\%s\\resp\\%s\\ip\\%d\\skey\\%d\\reqproof\\1
   ...
   */
  len = snprintf(buf, BUFSIZE, authformat, prod->pid, challenge, response,
                 client->ip, client->sesskey);
...
static void resend_auth_req(gsclient_t *client) {
  client->sttime = current_time();
  client->ntries++;
  sendto(sock, client->reqstr, client->reqlen, 0, (struct sockaddr *)&valaddr,
         sizeof(valaddr));
}
...
static void send_keep_alive() {
  ...
  if (current_time() > lastKeepAliveSent + MAX_KEEP_ALIVE_INTERVAL) {
    ...
    sendto(sock, buf, strlen(keepAlive), 0, (struct sockaddr *)&valaddr,
           sizeof(struct sockaddr_in));
    lastKeepAliveSent = current_time();
  }
}
...
static int get_sockaddrin(char *host, int port, struct sockaddr_in *saddr,
  ...
  char broadcast_t[] = {'2', '5', '5', '.', '2', '5', '5', '.', '2',
                        '5', '5', '.', '2', '5', '5', '\0'}; // 255.255.255.255
