...
******
  ...
  demonstrates support for Query & Reporting 2 SDK integration, as detailed in
the documentation.
...
static void ClientAuthorize(int gameid, int localid, int authenticated,
...
static void ClientRefreshAuthorize(int gameid, int localid, int hint,
                                   char *challenge, void *instance) {
  client_t *clients = (client_t *)instance;
  char outbuf[512];

  // Some one else is trying to login using this key, so we're asking the client
  // to re-validate The hint is sent along as a pass through value.  The client
...
static void HostAuthorize(int gameid, int localid, int authenticated,
                          char *errmsg, void *instance) {
  if (!authenticated) // doh.. bad!
...
static void HostRefreshAuthorize(int gameid, int localid, int hint,
                                 char *challenge, void *instance) {
  char response[73];
  printf("REAUTHENTICATING host\n");
  gcd_compute_response(DEFAULT_KEY, challenge, response,
...
void PublicAddressCallback(unsigned int ip, unsigned short port,
                           void *userdata) {
  ...
  gcd_authenticate_user(GAMEID, HOST_ID, ip, challenge, response, HostAuthorize,
                        HostRefreshAuthorize, NULL);
...
int test_main(int argc, char **argv) {
  ...
  /**
  First we set up the Query & Reporting SDK - this is mostly taken from the
  "qrcsample" sample
  **/
  ...
  // call qr2_init with the query port number and gamename, default IP address,
  // and no user data
  if (qr2_init(NULL, NULL, 26900, GAME_NAME, secret_key, 1, 1,
               serverkey_callback, playerkey_callback, teamkey_callback,
               keylist_callback, count_callback, adderror_callback,
               NULL) != 0) {
  ...
  /* Once the QR SDK is initialized, you can initialize the CDKey SDK with the
  special QR integration function. Pass in the GAMEID you were assigned. */
  ...
  while (!quit) { // main loop
    ...
    automatically passes any network traffic for the CDKey SDK to the SDK
    directly */
    qr2_think(NULL);
    gcd_think();
    ...
    if (FD_ISSET(ssock, &set)) {
      for (i = 0; i < MAXCLIENTS; i++)
        if (clients[i].sock == INVALID_SOCKET) {
          clients[i].sock =
              accept(ssock, (struct sockaddr *)&(clients[i].saddr), &saddrlen);
          listen(ssock, SOMAXCONN);
    ...
    for (i = 0; i < MAXCLIENTS; i++)
      if (clients[i].sock != INVALID_SOCKET &&
        ...
        buf[len] = 0;
        if (buf[0] == 'r' && buf[1] == ':' &&
            clients[i].auth == 0) // challenge response
        {
          printf("Client %d said %s\n", i, buf);
          clients[i].auth = 1;
          gcd_authenticate_user(GAMEID, i, clients[i].saddr.sin_addr.s_addr,
                                clients[i].challenge, buf + 2, ClientAuthorize,
                                ClientRefreshAuthorize, clients);
        } else if (buf[0] == 'p' && buf[1] == ':' &&
                   clients[i].auth == 1) // ison proof response
        {
          // gcd_send_reauth_response needs to know the sesskey (so keymaster
          // can find the task) 	and fromaddr (so gcdkey knows which keymaster
          // to
          // respond to)
          if (len > 11) {
            char hintstr[9];
