...
XRGAMESPY_API gsi_u32 xrGS_wsLoginProfile(const gsi_char *profileNick,
                                          ...
                                          void *userData) {
  return wsLoginProfile(GP_PARTNERID_GAMESPY, GAMESPY_GP_NAMESPACE_ID,
                        profileNick, email, password, cdkeyhash, callback,
                        userData);
}
...
XRGAMESPY_API SCResult xrGS_scCreateSession(
    ...
    void *theUserData) {
  return scCreateSession(theInterface, theCertificate, thePrivateData,
                         theCallback, theTimeoutMs, theUserData);
}

XRGAMESPY_API const char *
xrGS_scGetSessionId(const SCInterfacePtr theInterface) {
  return scGetSessionId(theInterface);
}

XRGAMESPY_API SCResult
xrGS_scSetSessionId(const SCInterfacePtr theInterface,
...
XRGAMESPY_API SCResult xrGS_scSetReportIntention(
    ...
    void *theUserData) {
  return scSetReportIntention(theInterface, theConnectionId, isAuthoritative,
                              theCertificate, thePrivateData, theCallback,
                              theTimeoutMs, theUserData);
}

XRGAMESPY_API const char *
xrGS_scGetConnectionId(const SCInterfacePtr theInterface) {
...
XRGAMESPY_API SCResult xrGS_scCreateReport(const SCInterfacePtr theInterface,
  ...
  return scCreateReport(theInterface, theHeaderVersion, thePlayerCount,
                        theTeamCount, theReportOut);
}
...
XRGAMESPY_API SCResult xrGS_scReportSetPlayerData(
  ...
  return scReportSetPlayerData(theReport, thePlayerIndex, thePlayerConnectionId,
...
XRGAMESPY_API SCResult xrGS_scReportEnd(SCReportPtr theReport, gsi_bool isAuth,
                                        SCGameStatus theStatus) {
  return scReportEnd(theReport, isAuth, theStatus);
}

XRGAMESPY_API SCResult xrGS_scSubmitReport(
    const SCInterfacePtr theInterface, const SCReportPtr theReport,
    gsi_bool isAuthoritative, const GSLoginCertificate *theCertificate,
    const GSLoginPrivateData *thePrivateData,
    SCSubmitReportCallback theCallback, gsi_time theTimeoutMs,
    void *theUserData) {
  return scSubmitReport(theInterface, theReport, isAuthoritative,
                        theCertificate, thePrivateData, theCallback,
                        theTimeoutMs, theUserData);
}