...
XRGAMESPY_API void xrGS_gcd_compute_response(char *cdkey, char *challenge,
                                             char *response, bool Reauth) {
  gcd_compute_response(cdkey, challenge, response,
                       Reauth ? CDResponseMethod_REAUTH
...
XRGAMESPY_API void xrGS_gcd_authenticate_user(int localid, unsigned int userip,
  ...
  gcd_authenticate_user(GAMESPY_GAMEID, localid, userip, challenge, response,
                        authfn, refreshfn, instance);
}

XRGAMESPY_API void xrGS_gcd_reauthenticate_user(int localid, int hint,
                                                const char *response) {
