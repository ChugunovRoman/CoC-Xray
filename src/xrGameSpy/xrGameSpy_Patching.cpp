...
XRGAMESPY_API bool xrGS_ptCheckForPatchA(
  ...
  //	return ptCheckForPatch(productID, versionUniqueID, distributionID,
  ...
  return ptCheckForPatchA(GAMESPY_PRODUCTID,
                          ...
                          GetGameDistribution(), callback, blocking,
                          instance) != PTFalse;
};

XRGAMESPY_API bool xrGS_ptTrackUsageA(int userID) {
  return ptTrackUsageA(userID, GAMESPY_PRODUCTID, GetGameVersion(),
                       GetGameDistribution(), PTFalse) != PTFalse;
}