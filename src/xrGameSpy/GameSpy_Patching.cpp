...
void CGameSpy_Patching::LoadGameSpy() {
  ...
  if (0 == hGameSpyDLL)
    R_CHK(GetLastError());
  R_ASSERT2(hGameSpyDLL, "GameSpy DLL raised exception during loading or there "
                         "is no game DLL at all");

  GAMESPY_LOAD_FN(xrGS_ptCheckForPatch);
}

void __cdecl GS_ptPatchCallback(PTBool available, PTBool mandatory,
                                const char *versionName, int fileID,
...
void CGameSpy_Patching::CheckForPatch() {
  ...
  bool res = xrGS_ptCheckForPatch(10953, "test_version_1", 0,
                                  GS_ptPatchCallback, PTTrue, this);
  int x = 0;
  x = x;
};