...
int LoadShaderLCList(LPSTRVec &lst) {
  ...
  for (Shader_xrLCIt l_it = LIB.Library().begin(); l_it != LIB.Library().end();
       l_it++, s_it++)
...
int LoadGameMtlList(LPSTRVec &lst) {
  ...
  LPSTRIt s_it = lst.begin();
  for (GameMtlIt gm_it = GMLib.FirstMaterial(); gm_it != GMLib.LastMaterial();
       gm_it++, s_it++)
