...
public:
  ...
  TexmapSlot();
  RGBA Eval(ShadeContext &sc) { return amount * map->EvalColor(sc); }
...
public:
  ...
  BOOL InvisibleProperty() {
    return TRUE;
  } // maps are made visible in scripter by pb_maps paramblock in the material
    // so don't expose them as a subanim
  ...
  SvGraphNodeReference SvTraverseAnimGraph(IGraphObjectManager *gom,
                                           Animatable *owner, int id,
                                           DWORD flags);
};
