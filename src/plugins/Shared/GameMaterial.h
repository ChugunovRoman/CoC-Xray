...
#define NEWSTDMTL_ROLLUP_FLAGS                                                 \
  (NEWSTDMTL_ROLLUP0_OPEN | NEWSTDMTL_ROLLUP1_OPEN | NEWSTDMTL_ROLLUP2_OPEN |  \
   NEWSTDMTL_ROLLUP3_OPEN | NEWSTDMTL_ROLLUP4_OPEN | NEWSTDMTL_ROLLUP5_OPEN |  \
   NEWSTDMTL_ROLLUP6_OPEN)
...
enum {
  std2_ssampler,
  std2_ssampler_qual,
  std2_ssampler_enable,
  std2_ssampler_adapt_on,
  std2_ssampler_adapt_threshold,
  std2_ssampler_advanced,
  std2_ssampler_subsample_tex_on,
  std2_ssampler_by_name,
  std2_ssampler_param0,
  std2_ssampler_param1,
};
// std_maps param IDs
enum {
  std2_map_enables,
...
enum {
  std2_bounce,
  std2_static_friction,
  std2_sliding_friction,
};
...
public:
  ...
  BOOL KeyAtTime(int id, TimeValue t) {
    return (id == OPACITY_PARAM) ? pb_extended->KeyFrameAtTime(std2_opacity, t)
                                 : FALSE;
  }
  ...
  void SetWire(BOOL onoff) {
    pb_shader->SetValue(std2_wire, 0, (onoff != 0));
  } // SetFlag(NEWSTDMTL_WIRE,onoff); }
  ...
  BOOL IsShaderInUI() {
    return pb_shader && pb_shader->GetMap() && pShader &&
           pShader->GetParamDlg();
  }
  ...
  BOOL GetFalloffOut() {
    return GetFlag(NEWSTDMTL_FALLOFF_OUT);
  } // 1: out, 0: in
  ...
  int GetTransparencyType() {
    return (flags & NEWSTDMTL_FILT_TRANSP) ? TRANSP_FILTER
           : flags & NEWSTDMTL_ADD_TRANSP  ? TRANSP_ADDITIVE
                                           : TRANSP_SUBTRACTIVE;
  }
  ...
  void UpdateReshadeRequirements(RefTargetHandle hTarget,
                                 PartID partID); // mjm - 06.02.00
  ReshadeRequirements GetReshadeRequirements() {
    return mReshadeRQ;
  } // mjm - 06.02.00
  void PreShade(ShadeContext &sc, IReshadeFragment *pFrag);
  void PostShade(ShadeContext &sc, IReshadeFragment *pFrag, int &nextTexIndex,
                 IllumParams *ip);
