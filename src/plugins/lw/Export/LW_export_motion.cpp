...
void SelectedCount(LWItemType type, int &sel_obj_count,
                   LWItemID &last_sel_obj) {
  LWItemID object;
...
void __cdecl SaveObjectMotion(GlobalFunc *global) {
  ...
  if (buf[0]) {
    ...
    if (sel_obj_cnt == 1) {
      ...
      m_Motion->SetParam(g_intinfo->previewStart, g_intinfo->previewEnd,
                         (float)g_lwsi->framesPerSecond);
