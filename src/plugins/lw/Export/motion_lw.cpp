...
void CSMotion::ParseBoneMotion(LWItemID bone) {
  ...
  while (chan) {
    ...
    if (t != ctUnsupported) {
      if (!bGoalOrient ||
          (bGoalOrient &&
           ((t == ctPositionX) || (t == ctPositionY) || (t == ctPositionZ)))) {
          ...
          if (bParent) {
            ...
            while (chan_goal_parent) {
              if (t == GetChannelType(chan_goal_parent))
                break;
              chan_goal_parent =
                  g_chinfo->nextChannel(group_goal_parent, chan_goal_parent);
            }
          }

          CEnvelope *env =
              CreateEnvelope(chan_goal, bParent ? &chan_goal_parent : 0);
          bm.envs[t] = env;
        }
      }
...
CEnvelope *CCustomMotion::CreateEnvelope(LWChannelID chan,
  ...
  while (lwkey = g_envf->nextKey(lwenv, lwkey)) {
    ...
    key->value = (float)val + val_parent;
    g_envf->keyGet(lwenv, lwkey, LWKEY_TIME, &val);
    key->time = (float)val;
    g_envf->keyGet(lwenv, lwkey, LWKEY_TENSION, &val);
    key->tension = (float)val;
    g_envf->keyGet(lwenv, lwkey, LWKEY_CONTINUITY, &val);
    key->continuity = (float)val;
    g_envf->keyGet(lwenv, lwkey, LWKEY_BIAS, &val);
    key->bias = (float)val;
    g_envf->keyGet(lwenv, lwkey, LWKEY_PARAM_0, &val);
    key->param[0] = (float)val;
    g_envf->keyGet(lwenv, lwkey, LWKEY_PARAM_1, &val);
    key->param[1] = (float)val;
    g_envf->keyGet(lwenv, lwkey, LWKEY_PARAM_2, &val);
    key->param[2] = (float)val;
    g_envf->keyGet(lwenv, lwkey, LWKEY_PARAM_3, &val);
