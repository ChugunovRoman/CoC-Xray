      ...
      // surfaces
      {
        for (int i = 0; i < I->nsurfaces; i++) {
          ...
          if (!Osf->_Texture() || !Osf->_Texture()[0]) {
            g_msg->error(
                "Can't create shader. Textures empty. Invalid surface:. ",
                Osf->_Name());
            bResult = false;
            break;
          }
          ...
          Osf->SetFVF(D3DFVF_XYZ | D3DFVF_NORMAL |
                      (Isf->tex_cnt << D3DFVF_TEXCOUNT_SHIFT));
        }
      }
      if (bResult) {
        // mesh
        do {
          ...
          for (int vm_i = 0; vm_i < I->nvertmaps; vm_i++) {
            ...
            case LWVMAP_TXUV: {
              ...
              MESH->m_VMaps.push_back(
                  xr_new<st_VMap>(Ivmap->name, vmtUV, *Ivmap->vdpol >= 0));
            ...
            case LWVMAP_PICK:
              g_msg->error("Found 'PICK' VMAP. Import failed.", 0);
              bResult = false;
              break;
            case LWVMAP_MNVW:
              g_msg->error("Found 'MNVW' VMAP. Import failed.", 0);
              bResult = false;
              break;
            case LWVMAP_MORF:
              g_msg->error("Found 'MORF' VMAP. Import failed.", 0);
              bResult = false;
              break;
            case LWVMAP_SPOT:
              g_msg->error("Found 'SPOT' VMAP. Import failed.", 0);
              bResult = false;
              break;
            case LWVMAP_RGB:
              g_msg->error("Found 'RGB' VMAP. Import failed.", 0);
              bResult = false;
              break;
            case LWVMAP_RGBA:
              g_msg->error("Found 'RGBA' VMAP. Import failed.", 0);
              bResult = false;
              break;
            }
            if (!bResult)
          ...
          // points
          {
            ...
            for (int i = 0; i < I->npoints; i++) {
              ...
              Mpt.set(Ipt.pos[0]);
              copy(Ipt.pol, Ipt.pol + Ipt.npols,
                   inserter(a_lst, a_lst.begin()));
          ...
          if (!bResult)
            break;
          int p_idx = 0;
          for (FaceIt pl_it = MESH->m_Faces.begin();
               pl_it != MESH->m_Faces.end(); pl_it++) {
          ...
          if (!m_Bones.empty()) {
            for (BoneIt b_it = m_Bones.begin(); b_it != m_Bones.end(); b_it++) {
              if ((*b_it)->WMap()[0] &&
                  (-1 == MESH->FindVMapByName(MESH->m_VMaps, (*b_it)->WMap(),
                                              vmtWeight, FALSE))) {
