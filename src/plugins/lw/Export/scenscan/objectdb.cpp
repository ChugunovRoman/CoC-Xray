...
ObjectDB *getObjectDB(LWItemID id, GlobalFunc *global) {
  ...
  if (mesh->scanPoints(
          mesh, (int(__cdecl *)(void *, struct st_GCoreVertex *))pntScan, odb))
    goto Finish;
  ...
  if (mesh->scanPolys(
          mesh, (int(__cdecl *)(void *, struct st_GCorePolygon *))polScan, odb))
    goto Finish;
...
int printObjectDB(ObjectDB *odb, FILE *fp, int c) {
  ...
  fprintf(fp, "Points (%d)\n\n", odb->npoints);
  for (i = 0; i < odb->npoints; i++) {
    fprintf(fp, "%08.8x  pos (%g, %g, %g)  npols %d:", odb->pt[i].id,
            odb->pt[i].pos[c][0], odb->pt[i].pos[c][1], odb->pt[i].pos[c][2],
            odb->pt[i].npols);
  ...
  for (i = 0; i < odb->npolygons; i++) {
    fprintf(fp, "%08.8x  surf %d  norm (%g, %g, %g)  nverts %d:\n",
            odb->pol[i].id, odb->pol[i].sindex, odb->pol[i].norm[c][0],
            odb->pol[i].norm[c][1], odb->pol[i].norm[c][2], odb->pol[i].nverts);
    for (j = 0; j < odb->pol[i].nverts; j++) {
      fprintf(fp, "  vert %d  vnorm (%g, %g, %g)\n", odb->pol[i].v[j].index,
              odb->pol[i].v[j].norm[c][0], odb->pol[i].v[j].norm[c][1],
  ...
  for (i = 0; i < odb->nsurfaces; i++) {
    ...
    fprintf(fp, "  " SURF_COLR "  %g %g %g\n", odb->surf[i].colr[0],
            odb->surf[i].colr[1], odb->surf[i].colr[2]);
    ...
    fprintf(fp, "  " SURF_LCOL "  %g %g %g\n", odb->surf[i].lcol[0],
            odb->surf[i].lcol[1], odb->surf[i].lcol[2]);
