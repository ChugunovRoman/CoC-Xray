...
int getObjectSurfs(ObjectDB *odb, LWMeshInfo *mesh, GlobalFunc *global) {
  ...
  if (!imglist)
    return 0;
  txfunc = (st_LWTextureFuncs *)global(
      LWTEXTUREFUNCS_GLOBAL, GFUSE_TRANSIENT); /* get the surface ID array */
  if (!txfunc)
    return 0;
  ...
  for (i = 0; i < odb->nsurfaces; i++) {
    ...
    for (LWTLayerID tlid = txfunc->firstLayer(tid); tlid;
         tlid = txfunc->nextLayer(tid, tlid)) {
      DWORD imid;
