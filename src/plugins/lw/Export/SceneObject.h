...
public:
  ...
  IC bool CheckVersion() {
    return m_pReference ? (m_Version == m_pReference->Version()) : false;
  }
  ...
  IC bool RefCompare(LPCSTR ref) {
    return ref && m_pReference ? (strcmp(ref, m_pReference->GetName()) == 0)
                               : false;
  }
  ...
  virtual bool RayPick(float &dist, const Fvector &S, const Fvector &D,
                       SRayPickInfo *pinf = 0);
