...
extern "C" __declspec(dllimport) lwObject *LWO_ImportObject(char *filename,
                                                            lwObject *new_obj);
...
bool CEditableObject::Import_LWO(const char *fn, bool bNeedOptimize) {
  ...
  if (I) {
      ...
      st_lwSurface *Isf = 0;
      {
        ...
        for (Isf = I->surf; Isf; Isf = Isf->next) {
          ...
          if (Isf->name && Isf->name[0])
            Osf->SetName(Isf->name);
          else
            Osf->SetName("Default");
          Osf->m_Flags.set(CSurface::sf2Sided,
                           (Isf->sideflags == 3) ? TRUE : FALSE);
          AnsiString en_name = "default", lc_name = "default",
                     gm_name = "default";
          XRShader *sh_info = 0;
          if (Isf->nshaders &&
              (stricmp(Isf->shader->name, SH_PLUGIN_NAME) == 0)) {
            ...
            ELog.Msg(mtError,
                     "CEditableObject: Shader not found on surface '%s'.",
                     Osf->_Name());
#ifdef _EDITOR
          if (!Device.Resources->_FindBlender(en_name.c_str())) {
            ELog.Msg(mtError,
                     "CEditableObject: Render shader '%s' - can't find in "
                     "library.\nUsing 'default' shader on surface '%s'.",
                     en_name.c_str(), Osf->_Name());
            en_name = "default";
          }
          if (!Device.ShaderXRLC.Get(lc_name.c_str())) {
            ELog.Msg(mtError,
                     "CEditableObject: Compiler shader '%s' - can't find in "
                     "library.\nUsing 'default' shader on surface '%s'.",
          ...
          if (!GMLib.GetMaterial(gm_name.c_str())) {
            ELog.Msg(mtError,
                     "CEditableObject: Game material '%s' - can't find in "
          ...
          for (st_lwTexture *Itx = Isf->color.tex; Itx; Itx = Itx->next) {
            ...
            if (Itx->type == ID_IMAP)
              ...
              ELog.DlgMsg(
                  mtError,
                  "Import LWO (Surface '%s'): 'Texture' is not Image Map!",
                  Osf->_Name());
              bResult = false;
              break;
            }
            if (cidx != -1) {
              ...
              if (tname[0] == 0) {
                ELog.DlgMsg(mtError,
                            "Import LWO (Surface '%s'): 'Texture' name is "
                            "empty or non 'STIL' type!",
                            Osf->_Name());
                bResult = false;
                break;
              }
          ...
          if (!Osf->_VMap() || !Osf->_VMap()[0]) {
            ELog.DlgMsg(mtError, "Invalid surface '%s'. VMap empty.",
                        Osf->_Name());
            bResult = false;
            break;
          }
          if (!Osf->_Texture() || !Osf->_Texture()[0]) {
            ELog.DlgMsg(
                mtError,
                "Can't create shader. Invalid surface '%s'. Textures empty.",
                Osf->_Name());
            bResult = false;
            break;
          }
          if (en_name.c_str() == 0) {
            ELog.DlgMsg(
                mtError,
                "Can't create shader. Invalid surface '%s'. Shader empty.",
                Osf->_Name());
            bResult = false;
            break;
          }
          ...
          Osf->SetFVF(D3DFVF_XYZ | D3DFVF_NORMAL |
                      (dwNumTextures << D3DFVF_TEXCOUNT_SHIFT));
          i++;
        }
      }
      if (bResult) {
        ...
        for (Ilr = I->layer; Ilr; Ilr = Ilr->next) {
          ...
          if (Ilr->name)
            MESH->SetName(Ilr->name);
          else
            MESH->SetName("");
          MESH->m_Box.set(Ilr->bbox[0], Ilr->bbox[1], Ilr->bbox[2],
                          Ilr->bbox[3], Ilr->bbox[4], Ilr->bbox[5]);
          ...
          for (Ivmap = Ilr->vmap; Ivmap; Ivmap = Ivmap->next) {
            ...
            case ID_TXUV: {
              if (Ivmap->dim != 2) {
                ELog.DlgMsg(mtError,
                            "Import LWO: 'UV Map' must contain 2 value!");
                bResult = false;
                break;
              }
              MESH->m_VMaps.push_back(
                  xr_new<st_VMap>(Ivmap->name, vmtUV, Ivmap->perpoly));
            ...
            case ID_WGHT: {
              if (Ivmap->dim != 1) {
                ELog.DlgMsg(mtError,
                            "Import LWO: 'Weight' must contain 1 value!");
                bResult = false;
                break;
              }
              MESH->m_VMaps.push_back(
                  xr_new<st_VMap>(Ivmap->name, vmtWeight, FALSE));
              st_VMap *Mvmap = MESH->m_VMaps.back();
              int vcnt = Ivmap->nverts;
              // VMap
              Mvmap->copyfrom(*Ivmap->val, vcnt);
            ...
            case ID_PICK:
              ELog.Msg(mtError, "Found 'PICK' VMAP. Import failed.");
              bResult = false;
              break;
            case ID_MNVW:
              ELog.Msg(mtError, "Found 'MNVW' VMAP. Import failed.");
              bResult = false;
              break;
            case ID_MORF:
              ELog.Msg(mtError, "Found 'MORF' VMAP. Import failed.");
              bResult = false;
              break;
            case ID_SPOT:
              ELog.Msg(mtError, "Found 'SPOT' VMAP. Import failed.");
              bResult = false;
              break;
            case ID_RGB:
              ELog.Msg(mtError, "Found 'RGB' VMAP. Import failed.");
              bResult = false;
              break;
            case ID_RGBA:
              ELog.Msg(mtError, "Found 'RGBA' VMAP. Import failed.");
              bResult = false;
              break;
            }
            if (!bResult)
          ...
          if (!bResult)
            break;
          // points
          //					UI->ProgressStart(Ilr->point.count,"Fill
          // points:");
          {
          ...
          if (!bResult)
            break;
          // polygons
          //					UI->ProgressStart(Ilr->polygon.count,"Fill
          // polygons:");
          ...
          for (int i = 0; i < Ilr->polygon.count; i++) {
            ...
            if (Ipol.nverts != 3) {
              ELog.DlgMsg(mtError,
                          "Import LWO: Face must contain only 3 vertices!");
              bResult = false;
              break;
            }
            for (int pv_i = 0; pv_i < 3; pv_i++) {
              ...
              if (vmpl_cnt) {
                ...
                for (int vm_i = 0; vm_i < vmpl_cnt; vm_i++) {
                  ...
                  pt.vmap_index =
                      VMIndices[Ipv.vm[vm_i].vmap]; // номер моей VMap
              ...
              if (vmpt_cnt) {
                ...
                for (int vm_i = 0; vm_i < vmpt_cnt; vm_i++) {
                  ...
                  if (std::find(names.begin(), names.end(),
                                Ipt.vm[vm_i].vmap->name) != names.end())
                    continue;
                  vm_lst.push_back(st_VMapPt());
                  st_VMapPt &pt = vm_lst.back();
                  pt.vmap_index =
                      VMIndices[Ipt.vm[vm_i].vmap]; // номер моей VMap
              ...
              for (int vm_i = 0; vm_i < vm_cnt; vm_i++) {
                if (Ipt.vm[vm_i].vmap->type != ID_WGHT)
                  continue;
                vm_lst.push_back(st_VMapPt());
                st_VMapPt &pt = vm_lst.back();
              ...
              Memory.mem_copy(m_vm_lst.pts, &*vm_lst.begin(),
                              m_vm_lst.count * sizeof(st_VMapPt));
            }
            if (!bResult)
              break;
    ...
    if (bResult)
      m_LoadName = (strext(fname)) ? strcpy(strext(fname), ".object")
                                   : strcat(strext(fname), ".object");
    return bResult;
  } else
