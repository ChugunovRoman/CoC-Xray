...
static int get_globals(GlobalFunc *global) {
  ...
  return (g_chinfo && g_envf && g_iteminfo && g_lwsi && g_intinfo && g_msg &&
          g_boneinfo && g_objfunc && g_objinfo && g_surff);
}
...
XCALL_(int)
SkeletonMotionExport(long version, GlobalFunc *global, LWLayoutGeneric *local,
                     void *serverData) {
...
XCALL_(int)
ObjectExport(long version, GlobalFunc *global, LWLayoutGeneric *local,
             void *serverData) {
  if (version != LWLAYOUTGENERIC_VERSION)
    return AFUNC_BADVERSION;
  if (!get_globals(global))
...
XCALL_(int)
ObjectMotionExport(long version, GlobalFunc *global, LWLayoutGeneric *local,
                   void *serverData) {
  if (version != LWLAYOUTGENERIC_VERSION)
    return AFUNC_BADVERSION;
  if (!get_globals(global))
...
ServerRecord ServerDesc[] = {
    {LWLAYOUTGENERIC_CLASS, "XRay_Skeleton_Motion_Export",
     SkeletonMotionExport},
