...
//				   object (for example: file texture), or a
// constant 				   color (ex: bright red).
// 2. Target identity: either the color, or the target object itself. Note
//					   that the caller is responsible to first
//check the type 					   before calling
// color() or object().
// 3. Directness: Whether or not the connection was direct (ie: there was
//				  no shading switch in between the source and
//the
// target) 			      or indirect (ie: there were shading switches in
// between.)
//
...
public:
  enum TYPE { CONSTANT_COLOR, TEXTURE };

  ShadingConnection(MObject shaderObj, MString shapeName,
                    MString attribute = "");
