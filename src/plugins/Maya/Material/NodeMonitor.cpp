...
bool NodeMonitor::attachCallbacks() {
  ...
  fDirtyCallbackId = MNodeMessage::addNodeDirtyCallback(
      node, watchedObjectDirtyCallback, this, &stat);
  if (stat) {
    fRenamedCallbackId = MNodeMessage::addNameChangedCallback(
        node, watchedObjectRenamedCallback, this, &stat);
  }
...
// Callback functions. Those are called, respectively, when a node is dirty (has
// changed substantially), or when a node is renamed.
...
/*static*/ void NodeMonitor::watchedObjectRenamedCallback(MObject &node,
                                                          void *clientData) {
  NodeMonitor *pMon = (NodeMonitor *)clientData;
