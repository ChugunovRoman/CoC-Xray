...
private:
  MTexture *m_texture;
  unsigned int lastAccessedTimestamp; // can be used to track when the texture
                                      // was last used.
...
public:
  ...
  MTexture *texture(MObject textureObj, MTexture::Type type = MTexture::RGBA,
                    bool mipmapped = true, GLenum target = GL_TEXTURE_2D);

  // Returns true if the texture was found and bound; returns false otherwise.
  bool bind(MObject textureObj, MTexture::Type type = MTexture::RGBA,
            bool mipmapped = true, GLenum target = GL_TEXTURE_2D);
...
private:
  ...
  typedef xr_map<std::string, MTextureCacheElement *>
      string_to_cacheElement_map;
