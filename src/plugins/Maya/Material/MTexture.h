...
public:
  ...
  void parameter(GLenum pname, GLint param) {
    ...
    case GL_TEXTURE_MIN_FILTER:
      fMinFilterParam = param;
      break;
    case GL_TEXTURE_MAG_FILTER:
      fMagFilterParam = param;
      break;
...
public:
  ...
  bool set(MImage &image, Type type, bool mipmapped = true,
           GLenum target = GL_TEXTURE_2D);

  // This function assumes that the file texture is square, and
  // that its dimensions are exponents of 2.
  bool load(MString filename, Type type, bool mipmapped = true,
            GLenum target = GL_TEXTURE_2D);
  ...
  // Width is always >= 1, to prevent non-square textures from having zero-sized
  // levels.
...
protected:
  inline unsigned char *internalFetch(unsigned int s, unsigned int t,
                                      unsigned int level) {
