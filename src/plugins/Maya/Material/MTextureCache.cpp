...
//				texture cache to plug-ins to improve memory
...
MTexture *MTextureCache::texture(MObject textureObj,
  ...
  // Check if we already have a texCacheElement assigned to the given texture
  // name.
  ...
  if (textureDirty || newTexture) {
    ...
    if (texCacheElement->m_texture->load(textureFilename, type, mipmapped,
                                         target) == false) {
...
void MTextureCache::onNodeRenamed(MObject &node, MString oldName,
                                  MString newName) {
