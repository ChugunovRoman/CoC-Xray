...
public:
  ...
  //
  bool convertToNormalMap(unsigned char *inImagePtr, unsigned int width,
                          unsigned int height, OutFormatType outputPixelFormat,
                          float bumpScale = 1.0,
                          unsigned char *outImagePtr = NULL);

  // Convert the normal map texture to its corresponding heightfield texture
  //
  bool convertToHeightMap(unsigned char *inImagePtr, unsigned int width,
                          unsigned int height, OutFormatType outputPixelFormat,
...
protected:
  ...
  //
  bool convertToNormalMap_InPlace(unsigned char *inImagePtr, unsigned int width,
                                  unsigned int height,
