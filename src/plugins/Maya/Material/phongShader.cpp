...
MStatus CXRayMtl::initialize() {
  ...
  */
  aTranslucenceCoeff =
      nAttr.create("translucenceCoeff", "tc", MFnNumericData::kFloat);
  MAKE_INPUT(nAttr);

  aDiffuseReflectivity =
      nAttr.create("diffuseReflectivity", "drfl", MFnNumericData::kFloat);
  MAKE_INPUT(nAttr);
  ...
  aLightSpecular =
      nAttr.create("lightSpecular", "ls", MFnNumericData::kBoolean);
  MAKE_INPUT(nAttr);
  CHECK_MSTATUS(nAttr.setHidden(true));

  aLightShadowFraction =
      nAttr.create("lightShadowFraction", "lsf", MFnNumericData::kFloat);
  MAKE_INPUT(nAttr);
  CHECK_MSTATUS(nAttr.setHidden(true));

  aPreShadowIntensity =
      nAttr.create("preShadowIntensity", "psi", MFnNumericData::kFloat);
  MAKE_INPUT(nAttr);
  CHECK_MSTATUS(nAttr.setHidden(true));

  aLightBlindData =
      nAttr.create("lightBlindData", "lbld", MFnNumericData::kLong);
  MAKE_INPUT(nAttr);
  CHECK_MSTATUS(nAttr.setHidden(true));

  aLightData =
      lAttr.create("lightDataArray", "ltd", aLightDirection, aLightIntensity,
...
MStatus CXRayMtl::compute(const MPlug &plug, MDataBlock &block) {
  ...
  // ).asFloat(); User-defined Reflection Color Gain
  ...
  for (int count = 1; count <= numLights; count++) {
    ...
    if (blindData == 0) {
      ...
      if (currentLight.child(aLightDiffuse).asBool()) {
        ...
        {
          float RV =
              (((2 * surfaceNormal) * cosln) - lightDirection) * cameraPosition;
          if (RV > 0.0)
            RV = 0.0;
          if (RV < 0.0)
            RV = -RV;
      ...
      float cosln = MRenderUtil::diffuseReflectance(blindData, lightDirection,
                                                    point, surfaceNormal, true);
      if (cosln > 0.0f) // calculate only if facing light
      {
        diffuseR += lightIntensity[0] * (cosln * diffuseReflectivity);
        diffuseG += lightIntensity[1] * (cosln * diffuseReflectivity);
      ...
      if (currentLight.child(aLightSpecular).asBool()) {
        ...
        specLightDirection = MRenderUtil::maximumSpecularReflection(
            blindData, lightDirection, point, surfaceNormal, direction);
        lightAttenuation = MRenderUtil::lightAttenuation(blindData, point,
                                                         surfaceNormal, false);
        ...
        if (specLightDirection * surfaceNormal > 0.0f) {
          float power = block.inputValue(aPower).asFloat();
          MFloatVector rv =
              2 * surfaceNormal * (surfaceNormal * direction) - direction;
