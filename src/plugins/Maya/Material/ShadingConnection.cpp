...
//				 a shader's attribute, including what's
//connected
// upstream of it. 				 It also automatically passes through
// shader switches.
//
...
ShadingConnection::ShadingConnection(MObject shaderObj, MString shapeName,
                                     MString attribute /* = "" */) {
...
ShadingConnection::TYPE
ShadingConnection::traverseAttribute(MString attributeName) {
...
ShadingConnection::TYPE
ShadingConnection::traverseTripleShadingSwitch(MObject connectedObject) {
  ...
  // This assumption should always be safe, unless some drastic changes occured
  // in the implementation of the shading switches... in which case this code
  // won't work, so we would throw an exception.
  ...
  for (unsigned int index = 0; index < numElements; index++) {
    ...
    for (unsigned int shapeNum = 0; shapeNum < numCorrespondingShapes;
         shapeNum++) {
  ...
  // If there was no special case in the switch for our shape, check the
  // default.
