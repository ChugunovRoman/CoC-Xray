// MTexture.cpp
//
// Copyright (C) 2000-2003 Alias|Wavefront, a division of Silicon Graphics
// Limited.
//
...
bool MTexture::load(MString filename, MTexture::Type type,
                    bool mipmapped /* = true */,
...
bool MTexture::set(MImage &image, Type type, bool mipmapped /* = true */,
                   GLenum target /* = GL_TEXTURE_2D) */) {
  ...
  // If that's the case we resize the picture to the closest larger valid
  // rectangle.
  ...
  // The number of mipmap levels cannot be greater than the exponent of width or
  // height. The number of mipmap levels is 1 for a non-mipmapped texture. For
  // mipmapped textures, m_numLevels = max level + 1.
  ...
  // multiply the target s, t coordinates by the width and height ratio
  // respectively.
  for (unsigned int current_level = 1; current_level < m_numLevels;
       current_level++) {
    ...
    for (unsigned int target_t = 0; target_t < height(current_level);
         target_t++) {
      for (unsigned int target_s = 0; target_s < width(current_level);
           target_s++) {
        ...
        unsigned char *destination =
            internalFetch(target_s, target_t, current_level);
        unsigned char *source1 =
            internalFetch(source_s, source_t, previous_level);
        unsigned char *source2 =
            internalFetch(source_s2, source_t, previous_level);
        unsigned char *source3 =
            internalFetch(source_s, source_t2, previous_level);
        unsigned char *source4 =
            internalFetch(source_s2, source_t2, previous_level);

        // Average byte per byte.
        unsigned int average1 =
            (*source1++ + *source2++ + *source3++ + *source4++) / 4;
        *destination++ = average1;

        unsigned int average2 =
            (*source1++ + *source2++ + *source3++ + *source4++) / 4;
  ...
  if (type == NMAP) {
    ...
    for (unsigned int i = 0; i < m_numLevels; i++) {
      mapConverter.convertToNormalMap(m_levels[i], width(i), height(i),
                                      MNormalMapConverter::RGBA, 2.0f);
    }
  }
...
bool MTexture::specify(GLenum target /* = GL_TEXTURE_2D */) {
  ...
  for (unsigned int i = 0; i < m_numLevels; i++) {
    glTexImage2D(target, i, m_internalFormat, width(i), height(i), 0, m_format,
                 m_componentFormat, m_levels[i]);
