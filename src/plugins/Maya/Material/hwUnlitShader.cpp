...
//				supports decal texturing using standard OpenGL
// commands.
//
...
//     right-click on the cube and select Material->Create new
//     Material->CXRayMtl.
...
//     standard maya shader. Try it, press '6' to go in textured mode and see
//     the texture.
...
void CXRayMtl::attachSceneCallbacks() {
  ...
  fBeforeOpenCB = MSceneMessage::addCallback(MSceneMessage::kBeforeOpen,
                                             releaseCallback, this);
  fBeforeRemoveReferenceCB = MSceneMessage::addCallback(
      MSceneMessage::kBeforeRemoveReference, releaseCallback, this);
...
MStatus initializePlugin(MObject obj) {
  ...
  status = plugin.registerNode("XRayMtl", CXRayMtl::id, CXRayMtl::creator,
                               CXRayMtl::initialize, MPxNode::kHwShaderNode,
                               &UserClassify);
  if (!status) {
...
MStatus CXRayMtl::initialize() {
  ...
  nAttr.setStorable(true);
  nAttr.setKeyable(true);
  nAttr.setDefault(1.0f, 0.5f,
                   0.5f); // ugly pink-salmon color. You can't miss it.
  ...
  transparency = nAttr.create("transparency", "it", transparencyR,
                              transparencyG, transparencyB);
  nAttr.setStorable(true);
  nAttr.setKeyable(true);
...
MStatus CXRayMtl::compute(const MPlug &plug, MDataBlock &block) {
  if ((plug != outColor) && (plug != outColorR) && (plug != outColorG) &&
      (plug != outColorB))
...
/* virtual */
MStatus CXRayMtl::bind(const MDrawRequest &request, M3dView &view) {
  MStatus status;
  ...
  // If the color attribute is ultimately connected to a file texture, find its
  // filename. otherwise use the default color texture.
  if (colorConnection.type() == ShadingConnection::TEXTURE &&
    ...
    MPlug filenamePlug(colorConnection.texture(),
...
/* virtual */
MStatus CXRayMtl::unbind(const MDrawRequest &request, M3dView &view) {
  view.beginGL();
  glPopClientAttrib();
  glPopAttrib();
...
/* virtual */
MStatus CXRayMtl::geometry(const MDrawRequest &request, M3dView &view, int prim,
                           unsigned int writable, int indexCount,
                           const unsigned int *indexArray, int vertexCount,
                           const int *vertexIDs, const float *vertexArray,
