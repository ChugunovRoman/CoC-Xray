...
public:
  ...
  // virtual MString			defaultExtension		()
  // const;
  MFileKind identifyFile(const MFileObject &fileName, const char *buffer,
                         short size) const;

private:
  // these functions are all used to iterate across geometry, filling data
  // structures
  MStatus gotoBindPose(void);
  MStatus getBones(void); // loads geometry from scene
  MStatus getBoneData(
      const MMatrix &locator); // loads bone position per animation frame
  MStatus
  setUpBoneMap(MObject *pSkinObject); // creates data structure to store data
  void parseMesh(MObject &mesh, MDagPath path);
  MStatus parseBoneGeometry();
  MStatus parseShape(MDagPath path);
  MStatus parsePolySet(MItMeshPolygon &meshPoly, MObjectArray &rgShaders,
                       MIntArray texMap, int weight);
  MStatus CalculateTriangleVertex(int vt, MVector &pt, float &u, float &v,
                                  WBVec &weights, MItMeshPolygon &meshPoly,
                                  PtLookupMap &ptMap);
  void clearData(void); // call inbetween function calls
  //	MStatus			GetShaderFileName		(MString &filename,
  //MObject &set);
...
private:
  ...
  bool m_bReferenceFile; // true for a reference file, false for an animation
                         // file
  bool m_fSkinCluster;   // true if we should only export the cluster named by
                         // "m_strSkinCluster"
  MString m_strSkinCluster; // name of skin cluster selected by user to be
                            // exported. if empty, use first cluster in list
};
