...
{
        std::cerr <<"Dbg : MString CXRaySkinExport::defaultExtension ()
const\n"; return m_bReferenceFile?"object":"skl";
}
*/
MPxFileTranslator::MFileKind
CXRaySkinExport::identifyFile(const MFileObject &fileName, const char *buffer,
                              short size) const {
...
MStatus CXRaySkinExport::writer(const MFileObject &file, const MString &options,
                                FileAccessMode mode) {
  MStatus status;
    ...
    for (; !iter.isDone(); iter.next()) {
      ...
      if (stat == MS::kSuccess && inMeshPlug.isConnected()) {
        ...
        MItDependencyGraph dgIt(inMeshPlug, MFn::kInvalid,
                                ...
                                MItDependencyGraph::kPlugLevel, &stat);
  ...
  // first look through the scene for skin cluster objects. Find any bones used
  // in these clusters and load and store their weights in table by vertex.
  status = getBones();
  if (!status)
    Msg("!problem loading bone weights");

  // now look through the scene for any geometry directly attached to bones
  // this is most commonly found when creating mechanical models vs. organic
  // models
  status = parseBoneGeometry();
  if (!status)
    Msg("!problem loading skin information");

  // now load the geometry information for any skin cluster objects detected
  // earlier
  ...
  if ((mode == MPxFileTranslator::kExportAccessMode) ||
    ...
    status = (m_bReferenceFile) ? exportObject(fname, b_ogf)
                                : exportMotion(fname, Mlocator);
...
MStatus CXRaySkinExport::reader(const MFileObject &file, const MString &options,
                                FileAccessMode mode) {
...
MStatus CXRaySkinExport::gotoBindPose(void) {
  MStatus status;
  if (m_bReferenceFile) {
    for (SmdBoneIt itBones = m_boneList.begin(); itBones != m_boneList.end();
         itBones++) {
...
MStatus CXRaySkinExport::exportObject(LPCSTR fn, bool b_ogf) {
  ...
  // Write out the vertex table
  {
    ...
    for (SmdVertIt it = m_vertList.begin(); it != m_vertList.end();
         it++, pt_it++) {
      ...
      pt_it->set((float)dst_x.asMeters(), (float)dst_y.asMeters(),
                 (float)dst_z.asMeters());
  ...
  // faces
  {
    ...
    for (SmdTriIt it = m_triList.begin(); it != m_triList.end(); it++, f_id++) {
      ...
      for (int k = 0; k < 3; k++) {
        ...
        for (WBIt vd_it = V->influence.begin(); vd_it != V->influence.end();
             vd_it++) {
  ...
  for (SmdBoneIt boneIt = m_boneList.begin(); boneIt != m_boneList.end();
       ++boneIt) {
    float length = 0.5f;
    ...
    BONE->SetParentName((*boneIt)->parentId > -1
                            ? m_boneList[(*boneIt)->parentId]->name
                            : 0); //. need convert space
...
MStatus CXRaySkinExport::exportMotion(LPCSTR fn, const MMatrix &locator) {
  ...
  // cout << "Exporting animation frames from " << frameFirst << " to " <<
  // frameLast << "\n";
  {
...
int CXRaySkinExport::AppendVertex(MPoint pt, float u, float v,
                                  const WBVec &wb) {
...
// look up these values using the vertices that come from the "get triangles"
// function
//
...
//
// if presetBone is set to anything other than -1 then it will force vertices to
// be set to it otherwise vertex influences are pulled from the mesh directly
////////////////////////////////////////////////////

MStatus CXRaySkinExport::parsePolySet(MItMeshPolygon &meshPoly,
                                      MObjectArray &rgShaders, MIntArray texMap,
                                      int presetBone = -1) {
  SmdTriangle *tri;
  MPointArray rgpt;
  MIntArray rgint;
  ...
  for (; !meshPoly.isDone(); meshPoly.next()) {
    ...
    */
    ...
    // now iterate through each triangle on this polygon and create a triangle
    // object in our list
...
MStatus CXRaySkinExport::CalculateTriangleVertex(int vt, MVector &pt, float &u,
                                                 float &v, WBVec &weights,
                                                 MItMeshPolygon &meshPoly,
                                                 PtLookupMap &ptMap) {
  MStatus status;
  int vtLocal;
  int vtUV;
  ...
  /*
          ...
          if (!status) {
                  Msg("!error getting normal for local vertex '%d' and object
     vertex '%d'",vtLocal,vt); return status;
          }
  */

  status = meshPoly.getUVIndex(vtLocal, vtUV, u, v);
  if (!status) {
    Msg("!error getting UV Index for local vertex '%d' and object vertex '%d'",
        vtLocal, vt);
    return status;
  }
  ...
  //.
          ...
          if (!m_rgWeights.empty()) {
                  ...
                  if (mapIt2 != m_jointMap.end()) {
                          ...
                          Msg("!can't find weight bone with index '%d' and name
  '%s' in joint map.",weight,name); return MS::kFailure;
                  }
          } else {
                  weight = 0;
          }
...
MStatus CXRaySkinExport::getBones(void) {
  ...
  for (; !iter.isDone(); iter.next()) {
    ...
    // get the list of influence objects. store all influence objects in the
    // array "m_rgInfs" only used if the user decides not to export all bones
    ...
    for (size_t ii = 0; ii < nGeoms; ++ii) {
      ...
      // cout << "found skin: '" << m_skinPath.partialPathName().asChar() << "'
      // with " << gIter.count() << " vertices " << "and " << nInfs << "
      // influences\n";
          ...
          if (!fis_zero(wts[iInf])) {
            int bone_idx = -1;
            {
              ...
              if (mapIt2 != m_jointMap.end()) {
                ...
                // error - bone not found!
                Msg("!can't find weight bone with name '%s' in joint map.",
                    name);
                return MS::kFailure;
              }
            }
  ...
  if (0 == count) {
    Msg("!No skinned meshes found in this scene. Is your mesh bound to a "
        "skeleton?");
    return MStatus::kFailure;
...
static IC void ParseMatrix(MTransformationMatrix &mat, Fvector &t, Fvector &r,
                           bool bRoot) {
  MEulerRotation rot;
  ...
  t.set((float)dst_x.asMeters(), (float)dst_y.asMeters(),
        -(float)dst_z.asMeters());
}
...
// note: the "m_rgInfs" array must already be set up with the DAG path to each
// bone
MStatus CXRaySkinExport::getBoneData(const MMatrix &locator) {
  ...
  if (status == MS::kSuccess) {
    for (SmdBoneIt itBones = m_boneList.begin(); itBones != m_boneList.end();
      ...
      ParseMatrix(mat, (*itBones)->trans, (*itBones)->orient,
                  (*itBones)->parentId == -1);
    }
  }
...
void CXRaySkinExport::CreateSMGFacegroups(MFnMesh &fnMesh) {
  ...
  polySmoothingGroups =
      xr_alloc<int>(numPolygons); //(int*)malloc( sizeof(int) *  numPolygons );
...
//
{
  ...
  if (NULL == edgeTable[v1]) {
    edgeTable[v1] = xr_alloc<SXREdgeInfo>(
        1); //(EdgeInfoPtr)malloc( sizeof(struct EdgeInfo) );
    ...
    element->next = xr_alloc<SXREdgeInfo>(
        1); //(EdgeInfoPtr)malloc( sizeof(struct EdgeInfo) );
    element = element->next;
  }
