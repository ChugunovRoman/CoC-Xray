...
MStatus initializePlugin(MObject obj) {
  ...
  status = plugin.registerFileTranslator(
      "XRay_Object_Export", "none", CXRayObjectExport::creator,
      (char *)xrayObjectOptionScript, (char *)xrayObjectDefaultOptions);
  if (status != MS::kSuccess)
    return status;
  status = plugin.registerFileTranslator(
      "XRay_Skin_Export", "none", CXRaySkinExport::creator_skin,
      (char *)xraySkinOptionScript, (char *)xraySkinDefaultOptions);
  if (status != MS::kSuccess)
    return status;
  status = plugin.registerFileTranslator(
      "XRay_Skin_Motion_Export", "none", CXRaySkinExport::creator_skin_motion,
      (char *)xraySkinMotionOptionScript, (char *)xraySkinMotionDefaultOptions);
  if (status != MS::kSuccess)
    return status;
  status = plugin.registerFileTranslator(
      "XRay_Camera_Motion_Export", "none", CXRayCameraExport::creator,
      (char *)xraySkinMotionOptionScript,
      (char *)xrayCameraMotionDefaultOptions);
  if (status != MS::kSuccess)
    return status;
