...
MStatus CXRayObjectExport::reader(const MFileObject &file,
                                  const MString &options, FileAccessMode mode) {
  fprintf(stderr, "CXRayObjectExport::reader called in error\n");
  return MS::kFailure;
}
//////////////////////////////////////////////////////////////

MStatus CXRayObjectExport::writer(const MFileObject &file,
                                  const MString &options, FileAccessMode mode) {
  ...
  if (MS::kSuccess == status) {
    ...
    Msg("%d vertices, %d faces", OBJECT->GetVertexCount(),
        OBJECT->GetFaceCount(true, false));
    if (export_mode) {
      mname = file.fullName() + ".ogf";
      Log("Export OGF object: ", fname);
      fname = mname.asChar();

      OBJECT->m_objectFlags.set(CEditableObject::eoProgressive,
                                (export_mode == 1));
...
{
        std::cerr <<"Dbg : MString CXRayObjectExport::defaultExtension ()
const\n"; return "object";
}
*/
//////////////////////////////////////////////////////////////
...
CXRayObjectExport::identifyFile(const MFileObject &fileName, const char *buffer,
                                short size) const {
...
//
{
  ...
  if (exportAll) {
    ...
    for (; !iter.isDone(); iter.next()) {
      ...
      status = dagIterator.reset(objectPath.node(), MItDag::kDepthFirst,
                                 MFn::kInvalid);
  ...
  if (objectCount > 0) {
    ...
    for (i = 0; i < length; i++) {
      ...
      recFindTransformDAGNodes(objectNodeNamesArray[i],
                               transformNodeNameIndicesArray);
    }

    if (transformNodeNameArray.length() > 0) {
      objectGroupsTablePtr = xr_alloc<bool *>(
          objectCount); // (bool**) malloc( sizeof(bool*)*objectCount );
      length = transformNodeNameArray.length();
      for (i = 0; i < objectCount; i++) {
        //				objectGroupsTablePtr[i] = (bool*)calloc(
        // length, sizeof(bool) );
  ...
  if (objectCount > 0) {
    ...
    vertexTablePtr = xr_alloc<bool *>(
        objectCount); //(bool**) malloc( sizeof(bool*)*objectCount );
    polygonTablePtr = xr_alloc<bool *>(
        objectCount); //(bool**) malloc( sizeof(bool*)*objectCount );

    for (i = 0; i < objectCount; i++) {
      //			vertexTablePtr[i] = (bool*)calloc(
      // vertexCounts[i]*numSets, sizeof(bool) );
      ...
      //			polygonTablePtr[i] = (bool*)calloc(
      // polygonCounts[i]*numSets, sizeof(bool) );
              ...
              for (o = 0; o < numObjectNames; o++) {
                if ((*objectNames)[o] == name) {
                  ...
                  if (compIdx >= polygonCounts[o]) {
                    Msg("!Bad polygon index '%d' found. Polygon skipped",
                        compIdx);
                    break;
                  }
...
void CXRayObjectExport::CreateSMGFacegroups(MFnMesh &fnMesh) {
  ...
  /*
          for ( int idx=0; idx<numPolygons; ++idx )
          {
                  string128			buff;
                  sprintf				(buff,"[%d]
     smg=%d",idx,polySmoothingGroups[idx]); Msg
     (buff);
          }
  */
}
...
void CXRayObjectExport::CreateSMGEdgeAttrs(MFnMesh &fnMesh) {
  ...
  for (int pid = 0; pid < numPolygons; pid++) {
    ...
    if (vcount != 3)
      Msg("poly vertex count not equel 3(is not a tri) vertex count = %d",
          vcount);
...
void CXRayObjectExport::recFindTransformDAGNodes(
    MString &nodeName, MIntArray &transformNodeIndicesArray) {
