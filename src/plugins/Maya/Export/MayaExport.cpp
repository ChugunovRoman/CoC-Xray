...
BOOL CEditableObject::ParseMAMaterial(CSurface *dest, SXRShaderData &d) {
  ...
  dest->SetShaderXRLC(
      _ChangeSymbol(strcpy(tmp, d.comp_name.asChar()), '/', '\\'));
      ...
      for (; !dagIterator.isDone(); dagIterator.next()) {
        ...
        if (status) {
          ...
          if (dagPath.hasFn(MFn::kNurbsSurface)) {
            ...
            if (status != MS::kSuccess) {
              fprintf(stderr,
                      "Error: exporting geom failed, check your selection.\n");
              freeLookupTables();
              destroyEdgeTable(); // Free up the edge table
              return MS::kFailure;
            }
...
int AppendVertex(FvectorVec &_points, MPoint &_pt) {
  ...
  pt.set((float)dst_x.asMeters(), (float)dst_y.asMeters(),
...
MStatus CXRayObjectExport::set_smoth_flags(
    u32 &flags,
    const MIntArray &tri_vert_indeces) //  const MFnMesh &fnMesh, const
                                       //  MItMeshPolygon &meshPoly,
{
  return t_set_smoth_flags(*this, flags, tri_vert_indeces);
}

MStatus CXRayObjectExport::ExportPart(CEditableObject *O, MDagPath &mdagPath,
                                      MObject &mComponent) {
  ...
  // write faces
  {
    ...
    for (; !meshPoly.isDone(); meshPoly.next()) {
      ...
      /*			if (meshPoly.zeroUVArea()){
                                      status = MS::kFailure;
                                      Log("!polygon have zero UV
         area:",meshPoly.index()); return status;
                              }
      */
      ...
      for (int i = 0; i < cTri; i++) {
        ...
        for (int vtx = 0; vtx < 3; vtx++) {
          ...
          if (!status) {
            Msg("!error getting UV Index for local vertex '%d' and object "
                "vertex '%d'",
                vtLocal, vt);
            return status;
          }
        ...
        if (iTexture < 0)
          Debug.fatal(DEBUG_INFO, "Can't find material for polygon: %d",
                      meshPoly.index());
        SXRShaderData &D = xr_data[iTexture];

        int compIdx = meshPoly.index();
        surf = MESH->Parent()->CreateSurface(
            getMaterialName(mdagPath, compIdx, objectIdx), D);
        if (!surf)
          return MStatus::kFailure;
    ...
    }
    {
      ...
      Memory.mem_copy(MESH->m_Vertices, &*_points.begin(),
                      MESH->m_VertCount * sizeof(Fvector));
      MESH->m_Faces = xr_alloc<st_Face>(MESH->m_FaceCount);
      Memory.mem_copy(MESH->m_Faces, &*_faces.begin(),
                      MESH->m_FaceCount * sizeof(st_Face));
      MESH->m_SmoothGroups = xr_alloc<u32>(MESH->m_FaceCount);
      Memory.mem_copy(MESH->m_SmoothGroups, &*_sgs.begin(),
                      MESH->m_FaceCount * sizeof(u32));
...
//-----------------------------------------------------------------------------------------
LPCSTR CXRayObjectExport::getMaterialName(MDagPath &mdagPath, int cid,
                                          int objectIdx) {
  MStatus stat;
  ...
  if (!materialsEqual) {
    ...
    if (mLength == 0)
      Debug.fatal(DEBUG_INFO, "Object '%s' has polygon '%d' without material.",
                  0, cid);
    if (mLength > 1) {
      Debug.fatal(DEBUG_INFO,
                  "Object '%s' has polygon '%d' with more than one material.",
                  0, cid);
    }
  } else {
