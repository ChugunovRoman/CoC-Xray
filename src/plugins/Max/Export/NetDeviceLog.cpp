...
BOOL CALLBACK ConsoleDialogProc(HWND hw, UINT msg, WPARAM wp, LPARAM lp) {
  ...
  case WM_INITDIALOG:
    ...
    if (!EConsole.m_Messages.empty()) {
      ...
      for (; _F != _E; _F++) {
        int k = SendDlgItemMessage(hw, IDC_MESSAGES, LB_ADDSTRING, 0,
                                   (LPARAM)_F->buf);
...
DWORD WINAPI ConsoleThreadProc(LPVOID) {
  DialogBox(EConsole.m_hInstance, MAKEINTRESOURCE(IDD_CONSOLE),
...
void CExportConsole::StayOnTop(BOOL flag) {
  if (flag) {
    ...
    SetWindowPos(m_hWindow, HWND_NOTOPMOST, 0, 0, 0, 0,
...
void CExportConsole::print(TMsgDlgType mt, const char *buf) {
  ...
  int k =
      SendDlgItemMessage(m_hWindow, IDC_MESSAGES, LB_ADDSTRING, 0, (LPARAM)buf);
