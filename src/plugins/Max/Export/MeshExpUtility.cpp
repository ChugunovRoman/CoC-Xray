...
static BOOL CALLBACK DefaultDlgProc(HWND hWnd, UINT msg, WPARAM wParam,
                                    LPARAM lParam) {
  switch (msg) {
...
void MeshExpUtility::BeginEditParams(Interface *ip, IUtil *iu) {
  ...
  hPanel = ip->AddRollupPage(hInstance, MAKEINTRESOURCE(IDD_MWND),
                             DefaultDlgProc, "S.T.A.L.K.E.R. Export", 0);
}
...
BOOL MeshExpUtility::BuildObject(CEditableObject *&exp_obj,
  ...
  char fname[256];
  _splitpath(m_ExportName, 0, 0, fname, 0);
  ...
  for (; it != m_Items.end(); it++) {
    ...
    if (submesh->Convert(it->pNode)) {
      // transform
      Matrix3 mMatrix;
      mMatrix = it->pNode->GetNodeTM(0) *
                Inverse(it->pNode->GetParentNode()->GetNodeTM(0));
      ...
      // flip faces
      Fvector v;
      v.crossproduct(m.j, m.k);
      if (v.dotproduct(m.i) < 0.f)
        submesh->FlipFaces();
      if (m_ObjectFlipFaces)
        submesh->FlipFaces();
  ...
  if (bResult) {
    ...
    ELog.Msg(mtInformation, "Object '%s' contains: %d points, %d faces",
             exp_obj->GetName(), exp_obj->GetVertexCount(),
             exp_obj->GetFaceCount());
  } else {
    xr_delete(exp_obj);
  }
  //-------------------------------------------------------------------
...
BOOL MeshExpUtility::SaveAsObject(const char *m_ExportName) {
  ...
  if (bResult = BuildObject(exp_obj, m_ExportName)) {
    ...
    for (SurfaceIt s_it = exp_obj->FirstSurface();
         s_it != exp_obj->LastSurface(); s_it++) {
...
//-------------------------------------------------------------------
//     registry access
const char *g_rOptionsKey =
    "SOFTWARE\\NerGoSoft\\S.T.A.L.K.E.R. Map Editor\\2.0\\Options";
...
void MeshExpUtility::ExportObject() {
  ...
  if (strext(m_ExportName))
    *strext(m_ExportName) = 0;
  strcat(m_ExportName, ".object");
  bResult = SaveAsObject(m_ExportName);

  ELog.Msg(mtInformation,
           bResult ? ". Export succesfully completed." : "! Export failed.");
  ELog.Msg(mtInformation,
           "-------------------------------------------------------");
