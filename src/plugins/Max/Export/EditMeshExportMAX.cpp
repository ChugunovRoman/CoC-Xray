...
bool CEditableMesh::Convert(CExporter *E) {
  ...
  for (DWORD b_i = 0; b_i < E->m_Bones.size(); b_i++)
    m_VMaps[b_i] =
        xr_new<st_VMap>(E->m_Bones[b_i]->name.c_str(), vmtWeight, false);
  ;
  // UV map
  ...
  // points
  {
    ...
    for (ExpVertIt ev_it = E->m_ExpVertices.begin();
         ev_it != E->m_ExpVertices.end(); ev_it++, p_it++) {
      p_it->set((*ev_it)->P);
  ...
  // faces
  {
    ...
    int f_id = 0;
    for (ExpFaceIt ef_it = E->m_ExpFaces.begin(); ef_it != E->m_ExpFaces.end();
         ef_it++, f_id++) {
      // FACES
