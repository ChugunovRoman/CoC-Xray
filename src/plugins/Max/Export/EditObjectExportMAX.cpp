...
BOOL CEditableObject::ParseStdMaterial(StdMat *src, CSurface *dest) {
  R_ASSERT(src);
  R_ASSERT((src->ClassID() == Class_ID(DMTL_CLASS_ID, 0)) ||
           (src->ClassID() == XRAYMTL_CLASS_ID));
...
BOOL CEditableObject::ParseMultiMaterial(MultiMtl *src, u32 mid,
                                         CSurface *dest) {
  R_ASSERT(src);
  ...
  if (M) {
    ...
    ELog.Msg(mtError, "'%s' -> can't extract multi-material items.",
             src->GetName());
    return FALSE;
  }
}
...
BOOL CEditableObject::ParseXRayMaterial(XRayMtl *src, u32 mid, CSurface *dest) {
  ...
  if (e_shader && c_shader && g_mtl) {
    ELog.Msg(mtInformation,
             " -Found S.T.A.L.K.E.R. shaders [E:'%s', C:'%s', M:'%s']",
             dest->_ShaderName(), dest->_ShaderXRLCName(),
             dest->_GameMtlName());
  } else {
    if (!e_shader)
...
CSurface *CEditableObject::CreateSurface(Mtl *mtl, u32 mid) {
  ...
  BOOL bRes = FALSE;
  if (cls_id == Class_ID(MULTI_CLASS_ID, 0))
    bRes = ParseMultiMaterial((MultiMtl *)mtl, mid, S);
  else if (cls_id == Class_ID(DMTL_CLASS_ID, 0))
    bRes = ParseStdMaterial((StdMat *)mtl, S);
  else if (cls_id == XRAYMTL_CLASS_ID)
    bRes = ParseXRayMaterial((XRayMtl *)mtl, mid, S);
  else {
...
bool CEditableObject::ImportMAXSkeleton(CExporter *E) {
  ...
  for (int B = 0; B != E->m_Bones.size(); B++) {
    ...
    Fmatrix m;
    if (parent_bone)
      m.mul(parent_bone->matOffset, bone->matInit);
    else
      m.set(bone->matInit);
    ...
    BONE->SetParentName(
        Helper::ConvertSpace(string(bone->pBone->GetParentNode()->GetName()))
            .c_str());
  ...
  if ((0 == GetVertexCount()) || (0 == GetFaceCount())) {
    ...
    ELog.Msg(mtInformation,
             "Model '%s' contains: %d points, %d faces, %d bones",
             E->m_MeshNode->GetName(), GetVertexCount(), GetFaceCount(),
             Bones().size());
  }
