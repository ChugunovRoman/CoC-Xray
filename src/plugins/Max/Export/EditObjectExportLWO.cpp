...
bool CEditableObject::ExportLWO(LPCSTR fname) {
  ...
  for (s_it = m_Surfaces.begin(); s_it != m_Surfaces.end(); s_it++) {
    ...
    F->Wsurface(S->_Name(), S->m_Flags.is(CSurface::sf2Sided), (u16)im_idx,
                (vm_name && vm_name[0]) ? vm_name : "Texture", S->_ShaderName(),
                S->_ShaderXRLCName());
  }
  // meshes/layers
  for (EditMeshIt mesh_it = FirstMesh(); mesh_it != LastMesh(); mesh_it++) {
    ...
    for (u32 f_id = 0; f_id < MESH->GetFCount(); f_id++)
      F->w_face3(MESH->GetFaces()[f_id].pv[0].pindex,
                 MESH->GetFaces()[f_id].pv[1].pindex,
                 MESH->GetFaces()[f_id].pv[2].pindex);
    F->close_chunk();
    // surf<->face
    F->open_chunk(ID_PTAG);
    F->w_u32(ID_SURF);
    for (SurfFacesPairIt sf_it = MESH->m_SurfFaces.begin();
         sf_it != MESH->m_SurfFaces.end(); sf_it++) {
    ...
    F->close_chunk();
    // vmap&vmad
    for (VMapIt vm_it = MESH->m_VMaps.begin(); vm_it != MESH->m_VMaps.end();
         vm_it++) {
      st_VMap *vmap = *vm_it;
      F->begin_vmap(vmap->polymap, (vmap->type == vmtUV) ? ID_TXUV : ID_WGHT,
                    vmap->dim, vmap->name.c_str());
      if (vmap->polymap)
        for (int k = 0; k < vmap->size(); k++)
          F->w_vmad(vmap->vindices[k], vmap->pindices[k], vmap->dim,
                    vmap->getVMdata(k));
      else
        for (int k = 0; k < vmap->size(); k++)
