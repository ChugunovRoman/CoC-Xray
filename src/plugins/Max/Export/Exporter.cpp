...
CExporter::~CExporter() {
  ...
  for (VertexDefIt v_it = m_Vertices.begin(); v_it != m_Vertices.end();
       v_it++) {
    xr_delete(*v_it);
  }
  for (ExpVertIt ev_it = m_ExpVertices.begin(); ev_it != m_ExpVertices.end();
       ev_it++) {
    xr_delete(*ev_it);
  }
  for (ExpFaceIt ef_it = m_ExpFaces.begin(); ef_it != m_ExpFaces.end();
       ef_it++) {
    xr_delete(*ef_it);
  }
}
//-----------------------------------------------------------------------------
int CExporter::AddBone(INode *pNode, Matrix3 &matMesh,
...
void CExporter::ScanMesh(INode *pNode) {
  ...
  if (!Helper::IsMesh(pNode)) {
    ...
    for (int iChild = 0; iChild < cChildren; iChild++) {
      //			LPCSTR nm1	=
      // pNode->GetChildNode(iChild)->GetName();
  ...
  if (eExportSkin == m_Style) {
    ...
    for (int iVertex = 0; iVertex < numVertices; iVertex++) {
      IPhyVertexExport *pVertexExport =
          (IPhyVertexExport *)pContext->GetVertexInterface(iVertex);
      ...
      IPhyRigidVertex *pRigidVertex =
          (IPhyRigidVertex *)pContext->GetVertexInterface(iVertex);
      ...
      case RIGID_TYPE: {
        ...
        if (BONE_NONE == boneId) {
          ...
          pVertex->Append(boneId, 1.f);
      } break;
      case RIGID_BLENDED_TYPE: {
        IPhyBlendedRigidVertex *pBlendedRigidVertex =
            (IPhyBlendedRigidVertex *)pRigidVertex;
        ...
        for (int i = 0; i < cnt; i++) {
          ...
          if (BONE_NONE == boneId) {
            ...
            pVertex->Append(boneId, pBlendedRigidVertex->GetWeight(i));
        }
      } break;
      }
    ...
    // our Faces and Vertices
    {
      for (int i = 0; i < M.getNumFaces(); i++) {
        ...
        for (int VVV = 0; VVV < 3; VVV++) {
          ...
          v.SetUV(UV.x, 1 - UV.y);
          //					v.sm_group	=
          // U.m_SkinSuppressSmoothGroup?0:gF->getSmGroup(); // smooth group
          nF->v[VVV] = AddVert(v);
        }
