...
int SceneEnumProc::callback(INode *node) {
  ...
  if (node->IsTarget()) {
    ...
    if (ln) {
      ...
      case LIGHT_CLASS_ID:
        Append(node, obj, OBTYPE_LTARGET);
        break;
      case CAMERA_CLASS_ID:
        Append(node, obj, OBTYPE_CTARGET);
        break;
      }
    }
    return TREE_CONTINUE;
...
ObjectList::ObjectList(SceneEnumProc &scene) {
  head = tail = NULL;
  count = 0;
  // Zip thru the object list and record all unique objects (Some may be used by
  // more than one node)
...
bool MeshExpUtility::SaveSkinKeys(const char *n) {
  ...
  sprintf(S, "%d %d %d", FirstTick / TicksPerFrame, LastTick / TicksPerFrame,
          FramesPerSecond);
  F.Wstring(S);
      ...
      for (n = 0; n < NumNT; n++) {
        ...
        if (pNT->ClassID() == Class_ID(NOTETRACK_CLASS_ID, 0)) {
          ...
          for (i = 0; i < j; i++) {
            pNK = pDNT->keys[i];
            if ((pNK->time >= FirstTick) && (pNK->time <= LastTick)) {
              sprintf(S, "%d: %s", (pNK->time - FirstTick) / TicksPerFrame,
                      pNK->note);
              F.Wstring(S);
            }
          }
...
void MeshExpUtility::ExportSkinKeys() {
  ...
  // Ask the scene to enumerate all its nodes so we can determine if there are
  // any we can use
  ...
  ELog.Msg(mtInformation,
           "-------------------------------------------------------");
  if (bResult)
    ELog.Msg(mtInformation, "Export completed");
  else
