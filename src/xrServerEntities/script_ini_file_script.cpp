...
CScriptIniFile *create_ini_file(LPCSTR ini_string, LPCSTR path) {
  return ((CScriptIniFile *)xr_new<CInifile>(
      &IReader((void *)ini_string, xr_strlen(ini_string)), path));
}
#pragma warning(pop)
...
void CScriptIniFile::script_register(lua_State *L) {
  module(
      L)[class_<CScriptIniFile>("ini_file")
...
#endif // XRGAME_EXPORTS
         def("create_ini_file",
             (CScriptIniFile * (*)(LPCSTR))(&create_ini_file), adopt(result)),
         def("create_ini_file",
             (CScriptIniFile * (*)(LPCSTR, LPCSTR))(&create_ini_file),
             adopt(result))];
}