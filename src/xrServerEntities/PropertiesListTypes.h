...
template <class T> IC void set_value(T &val, const T &_val) { val = _val; };
...
public:
  PropItem(EPropType _type)
      : type(_type), prop_color(0), val_color(0), item(0), key(0),
        OnClickEvent(0), OnDrawTextEvent(0), OnItemFocused(0), m_Owner(0) {
    m_Flags.zero();
  }
  virtual ~PropItem() {
...
public:
  typedef fastdelegate::FastDelegate3<CanvasValue *, CanvasValue *, bool &>
      TOnTestEqual;
  typedef fastdelegate::FastDelegate3<CanvasValue *, void * /* TCanvas* */,
                                      const Irect &>
      TOnDrawCanvasEvent;
...
public:
  RStringVec value;
  int btn_num;
  typedef fastdelegate::FastDelegate3<ButtonValue *, bool &, bool &>
      TOnBtnClick;
...
public:
  typedef fastdelegate::FastDelegate3<ShortcutValue *, const xr_shortcut &,
                                      bool &>
      TOnValidateResult;
...
public:
  typedef fastdelegate::FastDelegate2<PropValue *, xr_string &>
      TOnBeforeEditEvent;
  typedef fastdelegate::FastDelegate2<PropValue *, xr_string &, bool>
      TOnAfterEditEvent;
  TOnBeforeEditEvent OnBeforeEditEvent;
  TOnAfterEditEvent OnAfterEditEvent;
...
public:
  ChooseValue(shared_str *val, u32 cid, LPCSTR path, void *param,
              u32 sub_item_count, u32 choose_flags)
      : RTextValue(val), m_ChooseID(cid), m_StartPath(path),
        subitem(sub_item_count), m_Items(0), m_FillParam(param),
        OnChooseFillEvent(0), OnDrawThumbnailEvent(0),
