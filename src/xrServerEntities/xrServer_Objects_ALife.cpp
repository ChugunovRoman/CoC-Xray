...
struct logical_string_predicate {
  static HRESULT AnsiToUnicode(LPCSTR pszA, LPVOID buffer,
    ...
    if (MultiByteToWideChar(CP_ACP, 0, pszA, cCharacters, (LPOLESTR)buffer,
                            cCharacters))
      return (NOERROR);
...
void SFillPropData::load() {
  ...
  for (k = 0; Ini->r_line("graph_points_draw_color_palette", k, &N, &V); ++k) {
    ...
    if (1 == sscanf(V, "%x", &color)) {
      ...
      Msg("! invalid record format in [graph_points_draw_color_palette] %s=%s",
          N, V);
  }
  ...
  // story names
  {
    ...
    story_names.insert(story_names.begin(),
...
#ifdef XRSE_FACTORY_EXPORTS
  PHelper().CreateRToken8(items, PrepareKey(pref, *s_name, "Location\\1"),
                          &m_tLocations[0], &*fp_data.locations[0].begin(),
                          fp_data.locations[0].size());
  PHelper().CreateRToken8(items, PrepareKey(pref, *s_name, "Location\\2"),
                          &m_tLocations[1], &*fp_data.locations[1].begin(),
                          fp_data.locations[1].size());
  PHelper().CreateRToken8(items, PrepareKey(pref, *s_name, "Location\\3"),
                          &m_tLocations[2], &*fp_data.locations[2].begin(),
                          fp_data.locations[2].size());
  PHelper().CreateRToken8(items, PrepareKey(pref, *s_name, "Location\\4"),
                          &m_tLocations[3], &*fp_data.locations[3].begin(),
                          fp_data.locations[3].size());
  PHelper().CreateRList(items,
                        PrepareKey(pref, *s_name, "Connection\\Level name"),
                        &m_caConnectionLevelName, &*fp_data.level_ids.begin(),
                        fp_data.level_ids.size());
  PHelper().CreateRText(items,
                        PrepareKey(pref, *s_name, "Connection\\Point name"),
                        &m_caConnectionPointName);
#endif // #ifdef XRSE_FACTORY_EXPORTS
}

void CSE_ALifeGraphPoint::on_render(CDUInterface *du,
                                    ISE_AbstractLEOwner *owner, bool bSelected,
                                    const Fmatrix &parent, int priority,
                                    bool strictB2F) {
#ifdef XRSE_FACTORY_EXPORTS
  ...
  u32 cc;
  string16 buff;
  xr_sprintf(buff, "%03d_%03d_%03d_%03d", m_tLocations[0], m_tLocations[1],
             m_tLocations[2], m_tLocations[3]);
  ...
  du->DrawIndexedPrimitive(2 /*D3DPT_LINELIST*/, 8, parent.c, PT, 6, IL, 16,
                           C.get());
  C.mul_rgba(0.75f);
  du->DrawIndexedPrimitive(4 /*D3DPT_TRIANGLELIST*/, 4, parent.c, PT, 6, IT, 12,
                           C.get());
...
void CSE_ALifeObject::FillProps(LPCSTR pref, PropItemVec &items) {
  ...
  if (m_flags.is(flUseSwitches)) {
    PHelper().CreateFlag32(
        items, PrepareKey(pref, *s_name, "ALife\\Can switch online"), &m_flags,
        flSwitchOnline);
    PHelper().CreateFlag32(
        items, PrepareKey(pref, *s_name, "ALife\\Can switch offline"), &m_flags,
        flSwitchOffline);
  }
  PHelper().CreateFlag32(items, PrepareKey(pref, *s_name, "ALife\\Interactive"),
                         &m_flags, flInteractive);
  PHelper().CreateFlag32(items,
                         PrepareKey(pref, *s_name, "ALife\\Used AI locations"),
                         &m_flags, flUsedAI_Locations);
  PHelper().CreateRToken32(items, PrepareKey(pref, *s_name, "ALife\\Story ID"),
                           &m_story_id, &*fp_data.story_names.begin(),
...
u32 CSE_ALifeObject::ef_main_weapon_type() const {
  ...
  CLSID2TEXT(m_tClassID, temp);
  R_ASSERT3(false,
            "Invalid alife main weapon type request, virtual function is not "
            "properly overloaded!",
            temp);
  return (u32(-1));
  //	return		(5);
...
void CSE_ALifeGroupAbstract::FillProps(LPCSTR pref, PropItemVec &items) {
  PHelper().CreateU16(items, PrepareKey(pref, "ALife\\Count"), &m_wCount, 0,
                      0xff);
};
#endif // #ifndef XRGAME_EXPORTS
...
void CSE_ALifeLevelChanger::FillProps(LPCSTR pref, PropItemVec &items) {
  ...
  PHelper().CreateRList(items, PrepareKey(pref, *s_name, "Level to change"),
...
void CSE_ALifeObjectPhysic::FillProps(LPCSTR pref, PropItemVec &values) {
  ...
  PHelper().CreateToken32(values, PrepareKey(pref, *s_name, "Type"), &type,
                          po_types);
  PHelper().CreateFloat(values, PrepareKey(pref, *s_name, "Mass"), &mass, 0.1f,
                        10000.f);
  PHelper().CreateFlag8(values, PrepareKey(pref, *s_name, "Active"), &_flags,
                        flActive);
...
void CSE_ALifeObjectHangingLamp::FillProps(LPCSTR pref, PropItemVec &values) {
  ...
  PropValue *P = 0;
  PHelper().CreateFlag16(values, PrepareKey(pref, *s_name, "Flags\\Physic"),
                         &flags, flPhysic);
  PHelper().CreateFlag16(values,
                         PrepareKey(pref, *s_name, "Flags\\Cast Shadow"),
                         &flags, flCastShadow);
  PHelper().CreateFlag16(values, PrepareKey(pref, *s_name, "Flags\\Allow R1"),
                         &flags, flR1);
  PHelper().CreateFlag16(values, PrepareKey(pref, *s_name, "Flags\\Allow R2"),
                         &flags, flR2);
  P = PHelper().CreateFlag16(values,
                             PrepareKey(pref, *s_name, "Flags\\Allow Ambient"),
                             &flags, flPointAmbient);
  P->OnChangeEvent.bind(this, &CSE_ALifeObjectHangingLamp::OnChangeFlag);
  //
  P = PHelper().CreateFlag16(values, PrepareKey(pref, *s_name, "Light\\Type"),
                             &flags, flTypeSpot, "Point", "Spot");
  P->OnChangeEvent.bind(this, &CSE_ALifeObjectHangingLamp::OnChangeFlag);
  PHelper().CreateColor(values, PrepareKey(pref, *s_name, "Light\\Main\\Color"),
                        &color);
  PHelper().CreateFloat(values,
                        PrepareKey(pref, *s_name, "Light\\Main\\Brightness"),
                        &brightness, 0.1f, 5.f);
  PHelper().CreateChoose(
      values, PrepareKey(pref, *s_name, "Light\\Main\\Color Animator"),
      &color_animator, smLAnim);
  PHelper().CreateFloat(values, PrepareKey(pref, *s_name, "Light\\Main\\Range"),
                        &range, 0.1f, 1000.f);
  PHelper().CreateFloat(values,
                        PrepareKey(pref, *s_name, "Light\\Main\\Virtual Size"),
  ...
  PHelper().CreateChoose(values, PrepareKey(pref, *s_name, "Light\\Main\\Bone"),
                         &light_main_bone, smSkeletonBones, 0,
                         (void *)visual()->get_visual());
  if (flags.is(flTypeSpot)) {
    PHelper().CreateAngle(values,
                          PrepareKey(pref, *s_name, "Light\\Main\\Cone Angle"),
                          &spot_cone_angle, deg2rad(1.f), deg2rad(120.f));
    //		PHelper().CreateFlag16	(values,
    // PrepareKey(pref,*s_name,"Light\\Main\\Volumetric"),	&flags,
    // flVolumetric);
    P = PHelper().CreateFlag16(values,
                               PrepareKey(pref, *s_name, "Flags\\Volumetric"),
                               &flags, flVolumetric);
    P->OnChangeEvent.bind(this, &CSE_ALifeObjectHangingLamp::OnChangeFlag);
  }
  ...
  if (flags.is(flPointAmbient)) {
    PHelper().CreateFloat(values,
                          PrepareKey(pref, *s_name, "Light\\Ambient\\Radius"),
                          &m_ambient_radius, 0.f, 1000.f);
    PHelper().CreateFloat(values,
                          PrepareKey(pref, *s_name, "Light\\Ambient\\Power"),
                          &m_ambient_power);
    PHelper().CreateChoose(values,
                           PrepareKey(pref, *s_name, "Light\\Ambient\\Texture"),
                           &m_ambient_texture, smTexture, "lights");
    PHelper().CreateChoose(values,
                           PrepareKey(pref, *s_name, "Light\\Ambient\\Bone"),
  ...
  PHelper().CreateChoose(values, PrepareKey(pref, *s_name, "Glow\\Texture"),
...
void CSE_ALifeObjectHangingLamp::on_render(CDUInterface *du,
  ...
  if ((1 == priority) && (false == strictB2F)) {
    ...
    if (bSelected) {
      if (flags.is(flTypeSpot)) {
        du->DrawSpotLight(main_xform.c, main_xform.k, range, spot_cone_angle,
                          clr);
      } else {
...
#ifdef XRGAME_EXPORTS
  return ((flags.test(flR1) &&
           (::Render->get_generation() == IRender_interface::GENERATION_R1)) ||
          (flags.test(flR2) &&
           (::Render->get_generation() == IRender_interface::GENERATION_R2)));
#else
  return (true);
#endif
}
...
CSE_ALifeHelicopter::CSE_ALifeHelicopter(LPCSTR caSection)
    : CSE_ALifeDynamicObjectVisual(caSection), CSE_Motion(),
...
void CSE_ALifeCar::FillProps(LPCSTR pref, PropItemVec &values) {
  inherited1::FillProps(pref, values);
  inherited2::FillProps(pref, values);
  PHelper().CreateFloat(values, PrepareKey(pref, *s_name, "Health"), &health,
                        0.f, 1.0f);
}
#endif // #ifndef XRGAME_EXPORTS
...
void CSE_ALifeObjectBreakable::FillProps(LPCSTR pref, PropItemVec &values) {
  inherited::FillProps(pref, values);
  PHelper().CreateFloat(values, PrepareKey(pref, *s_name, "Health"), &m_health,
                        0.f, 100.f);
}
#endif // #ifndef XRGAME_EXPORTS
...
void CSE_ALifeObjectClimable::FillProps(LPCSTR pref, PropItemVec &values) {
  ...
  inherited2::FillProps(pref, values);
  // PHelper().CreateFloat		(values,
  // PrepareKey(pref,*s_name,"Health"), &m_health,			0.f,
  // 100.f);
}
