...
template <class M, typename P> struct CLoader {
  ...
  template <typename T> struct CHelper {
    ...
    template <> IC static void load_data<true>(T &data, M &stream, const P &p) {
      CLoader<M, P>::load_data(
          *(data = xr_new<object_type_traits::remove_pointer<T>::type>()),
          stream, p);
    }
  };
  ...
  template <typename T> struct CHelper4 {
    template <bool a> IC static void load_data(T &data, M &stream, const P &p) {
      CHelper<T>::load_data<object_type_traits::is_pointer<T>::value>(
          data, stream, p);
    }
  ...
  IC static void load_data(std::pair<T1, T2> &data, M &stream, const P &p) {
    if (p(data,
          const_cast<object_type_traits::remove_const<T1>::type &>(data.first),
          true)) {
  ...
  IC static void load_data(std::priority_queue<T1, T2, T3> &data, M &stream,
                           const P &p) {
    load_data(data, stream, p, true);
  }
  ...
  template <typename T>
  IC static void load_data(T &data, M &stream, const P &p) {
    CHelper4<T>::load_data<object_type_traits::is_stl_container<T>::value>(
        data, stream, p);
  }
};
