...
void CDbgLuaHelper::DrawGlobalVariables() {
  ...
  while (lua_next(L, -2)) {
    ...
    //		xr_sprintf(var, "%s-%s",	lua_typename(L, lua_type(L,
    //-2)), lua_typename(L, lua_type(L, -1)) );
    //		CScriptDebugger::GetDebugger()->AddLocalVariable(var, "global",
    //"_g_");
...
void CDbgLuaHelper::DrawVariable(lua_State *l, const char *name,
                                 bool bOpenTable) {
  Variable var;
  ...
  case LUA_TTABLE:
    ...
    /*	case LUA_TUSERDATA:{
       ...
       obj->get_lua_table(); lua_State * ls = NULL; r.get(ls); DrawTable(ls,
       name); return; }break;*/
