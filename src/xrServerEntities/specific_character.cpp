...
void CSpecificCharacter::load_shared(LPCSTR) {
  ...
  R_ASSERT3(!(data()->m_bNoRandom && data()->m_bDefaultForCommunity),
            "cannot set 'no_random' and 'team_default' flags simultaneously, "
            "profile id",
  ...
  R_ASSERT3(team != NULL,
            "'community' field not fulfiled for specific character", *m_OwnId);
  ...
  if (data()->m_Community.index() == NO_COMMUNITY_INDEX)
    Debug.fatal(DEBUG_INFO, "wrong 'community' '%s' in specific character %s ",
                team, *m_OwnId);
  ...
  if (min_rank != NO_RANK && max_rank != NO_RANK) {
    ...
    R_ASSERT3(rank != NO_RANK,
              "'rank' field not fulfiled for specific character", *m_OwnId);
    RankDef().min = rank;
    RankDef().max = rank;
  }
