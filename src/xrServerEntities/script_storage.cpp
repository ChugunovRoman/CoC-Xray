...
static void put_function(lua_State *state, u8 const *buffer,
                         u32 const buffer_size, LPCSTR package_id) {
...
#ifndef USE_DEBUGGER
  STATIC_CHECK(false,
               Do_Not_Define_USE_LUA_STUDIO_macro_without_USE_DEBUGGER_macro);
#endif //! USE_DEBUGGER
...
void CScriptStorage::reinit() {
  ...
  struct luajit {
    static void open_lib(lua_State *L, pcstr module_name,
...
void CScriptStorage::print_stack() {
  ...
  for (int i = 0; lua_getstack(L, i, &l_tDebugInfo); ++i) {
    ...
    if (!l_tDebugInfo.name) {
      ...
      // script_log(ScriptStorage::eLuaMessageTypeError, "%2d : [%s] %s(%d) :
      // %s", i, l_tDebugInfo.what, l_tDebugInfo.short_src,
      // l_tDebugInfo.currentline, "");
    } else {
      if (!xr_strcmp(l_tDebugInfo.what, "C")) {
        script_log_no_stack(ScriptStorage::eLuaMessageTypeError,
                            "%2d : [C  ] %s", i, l_tDebugInfo.name);
        // script_log(ScriptStorage::eLuaMessageTypeError, "%2d : [C  ] %s", i,
        // l_tDebugInfo.name);
      } else {
        script_log_no_stack(ScriptStorage::eLuaMessageTypeError,
                            "%2d : [%s] %s(%d) : %s", i, l_tDebugInfo.what,
...
int __cdecl CScriptStorage::script_log_no_stack(
    ScriptStorage::ELuaMessageType tLuaMessageType, LPCSTR caFormat, ...) {
  va_list marker;
...
bool CScriptStorage::parse_namespace(LPCSTR caNamespaceName, LPSTR b,
                                     u32 const b_size, LPSTR c,
                                     u32 const c_size) {
  *b = 0;
  *c = 0;
  LPSTR S2;
...
bool CScriptStorage::load_buffer(lua_State *L, LPCSTR caBuffer, size_t tSize,
  ...
  if (caNameSpaceName && xr_strcmp("_G", caNameSpaceName)) {
    ...
    __try {
      ...
      R_ASSERT2(
          errcode,
          "Could not reset the stack after \"Stack overflow\" exception!");
#ifdef DEBUG
      script = (LPSTR)Memory.mem_alloc(total_size,
                                       "lua script file (after exception)");
#else  //#ifdef DEBUG
      script = (LPSTR)Memory.mem_alloc(total_size);
#endif //#ifdef DEBUG
      dynamic_allocation = true;
...
bool CScriptStorage::do_file(LPCSTR caScriptName, LPCSTR caNameSpaceName) {
  ...
  // because that's the first and the only call of the main chunk - there is no
  // point to compile it
  //	luaJIT_setmode	(lua(),0,LUAJIT_MODE_ENGINE|LUAJIT_MODE_OFF);
  //// Oles
  int l_iErrorCode =
      lua_pcall(lua(), 0, 0, (-1 == errFuncId) ? 0 : errFuncId); // new_Andy
...
bool CScriptStorage::namespace_loaded(LPCSTR N, bool remove_from_stack) {
  ...
  for (;;) {
    ...
    if (lua_isnil(lua(), -1)) {
      ...
      lua_pop(lua(), 1);
      VERIFY(start == lua_gettop(lua()));
      FATAL(" Error : the namespace name is already being used by the "
            "non-table object!\n");
      return (false);
    }
    lua_remove(lua(), -2);
...
bool CScriptStorage::object(LPCSTR namespace_name, LPCSTR identifier,
                            int type) {
  int start = lua_gettop(lua());
  ...
  ~raii_guard() {
...
#endif //-DEBUG
    {
      ...
      static bool const break_on_assert =
          !!strstr(Core.Params, "-break_on_assert");
#else  //! DEBUG
      static bool const break_on_assert =
          false; // Alundaio: Can't get a proper stack trace with this enabled
#endif //-DEBUG
      if (!m_error_code)
        return;
...
bool CScriptStorage::print_output(lua_State *L, LPCSTR caScriptFileName,
                                  int iErorCode) {
  if (iErorCode)
...
void CScriptStorage::flush_log() {
  ...
  strconcat(sizeof(log_file_name), log_file_name, Core.ApplicationName, "_",
            Core.UserName, "_lua.log");
