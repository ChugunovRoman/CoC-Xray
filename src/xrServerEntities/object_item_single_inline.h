...
ObjectFactory::CLIENT_BASE_CLASS *CSObjectItemSingle::client_object() const {
  FATAL("Cannot instantiate client object, because client class is not "
        "declared!");
  return (0);
}
#endif
...
CSObjectItemSingle::server_object(LPCSTR section) const {
...
CObjectItemSingle<_unknown_type, true>::server_object(LPCSTR section) const {
  FATAL("Cannot instantiate server object, because server class is not "
        "declared!");
  return (0);
}
#endif
