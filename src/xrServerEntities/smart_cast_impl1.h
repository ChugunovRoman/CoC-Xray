...
template <typename List1, typename List2> struct merge {
  template <typename T> struct iterator {
    typedef Loki::Typelist<typename T::Head,
                           typename iterator<typename T::Tail>::result>
        result;
  };

  template <> struct iterator<Loki::NullType> { typedef List2 result; };
...
template <typename Base, typename Source> struct has_conversion {
  template <typename T> struct search_base {
    ...
    template <bool> struct selector { typedef typename Head result; };

    template <> struct selector<false> {
      typedef typename search_base<Tail>::result result;
    };

    typedef typename selector<is_type<Base, typename Head::Head>::value>::result
        result;
  };
  ...
  template <typename T> struct search_conversion {
    ...
    enum {
      value = selector<
          object_type_traits::is_same<typename T::Head, Source>::value>::value
    };
  };
...
template <typename Target, typename Source> struct CMatcher {
  ...
  template <typename HeadTail, typename T> struct CMatchHelper2 {
    template <typename P> struct CMatchHelper3 {
      ...
      template <bool> struct selector {
        ...
        typedef typename _selector<object_type_traits::is_same<
            Head, typename PrevHead::Head>::value>::result result;
      };
      ...
      typedef
          typename selector<has_conversion<Head, Target>::value>::result result;
    };
  ...
  template <typename T> struct CMatchHelper {
    ...
    typedef typename selector<
        object_type_traits::is_base_and_derived<typename Head::Head,
                                                Source>::value ||
        object_type_traits::is_same<typename Head::Head, Source>::value>::result
        result;
  };
...
template <typename Target, typename Source, int max_length,
          bool can_use_heritage>
struct conversion_sequence {
  ...
  template <typename T> struct list_iterator {
    ...
    template <typename T, int length, bool use_heritage> struct helper {
      typedef
          typename conversion_sequence<Target, T, length,
                                       use_heritage /**,new_visited/**/>::result
              search_result;
      ...
      template <> struct selector<false> { typedef search_result result; };
    ...
    template <bool> struct selector {
      typedef typename helper<typename Head::Head, max_length, false>::result
          helper_result;

      template <bool> struct _selector { typedef helper_result result; };
      ...
      typedef typename _selector<
          !is_type<Loki::NullType, helper_result>::value>::result result;
    };

    template <> struct selector<false> {
      template <bool> struct _selector {
        typedef
            typename helper<typename Head::Head, max_length - 1, true>::result
                helper_result;
      ...
      typedef typename _selector<
          has_conversion<Source, typename Head::Head>::value>::result result;
    };
  ...
  template <int length> struct selector {
    ...
    template <bool> struct _selector { typedef nearest result; };

    template <> struct _selector<false> {
      typedef typename list_iterator<cast_type_list>::result result;
    };

    typedef typename _selector<!is_type<Loki::NullType, nearest>::value>::result
        result;
  };
  ...
  template <> struct selector<0> { typedef Loki::NullType result; };
...
template <typename T1, typename T2> struct get_conversion_sequence {
  typedef
      typename conversion_sequence<T1, T2, MAX_SEQUENCE_LENGTH, true>::result
          result;
};
...
template <typename T, typename Target> struct CSmartCaster {
  ...
  template <typename P> struct CHelper {
    IC static Target *smart_cast(Head *p) {
      return (CSmartCaster<Tail, Target>::smart_cast(
          SmartDynamicCast::smart_cast<NextHead>(p)));
    }
  };
...
template <typename T1, typename T2> struct CSmartMatcher {
  template <typename T3> IC static T1 *smart_cast(T2 *p) {
    return (
        CSmartCaster<T3, T1>::smart_cast(static_cast<typename T3::Head *>(p)));
  }
...
template <typename T1, typename T2> struct CHelper1 {
  template <bool base> IC static T1 *smart_cast(T2 *p) {
    return (CSmartMatcher<T1, T2>::smart_cast<
            get_conversion_sequence<T1, T2>::result>(p));
  }
...
template <typename T1, typename T2> IC T1 *smart_cast(T2 *p) {
  return (CHelper1<T1, T2>::smart_cast <
...
template <typename T2> struct CHelper2 {
  template <typename T1> IC static T1 *smart_cast(T2 *p) {
    STATIC_CHECK(!object_type_traits::is_const<T2>::value ||
                     object_type_traits::is_const<T1>::value,
    ...
    VERIFY2(
        temp == test,
        make_string("SmartCast<%s*>(%s*) FAILED (result differs from the "
                    "dynamic_cast) or object is CORRUPTED (0x%08x -> 0x%08x)!",
                    typeid(T1).name(), typeid(T2).name(), *(u32 *)&test,
                    *(u32 *)&temp));
    return (temp);
#else
...
template <typename T1, typename T2> IC T1 smart_cast(T2 *p) {
  ...
  STATIC_CHECK(object_type_traits::is_pointer<T1>::value,
               Invalid_target_type_for_Dynamic_Cast);
  STATIC_CHECK(
      object_type_traits::is_void<
          object_type_traits::remove_pointer<T1>::type>::value ||
          is_polymorphic<object_type_traits::remove_pointer<T1>::type>::result,
      Invalid_target_type_for_Dynamic_Cast);
  STATIC_CHECK(is_polymorphic<T2>::result,
               Invalid_source_type_for_Dynamic_Cast);
#endif
#ifdef SMART_CAST_STATS_ALL
  ...
  return (SmartDynamicCast::CHelper2<T2>::smart_cast<
          object_type_traits::remove_pointer<T1>::type>(p));
}

template <typename T1, typename T2> IC T1 smart_cast(T2 &p) {
#ifdef PURE_DYNAMIC_CAST_COMPATIBILITY_CHECK
  STATIC_CHECK(object_type_traits::is_reference<T1>::value,
               Invalid_target_type_for_Dynamic_Cast);
  STATIC_CHECK(
      is_polymorphic<object_type_traits::remove_reference<T1>::type>::result,
      Invalid_target_type_for_Dynamic_Cast);
  STATIC_CHECK(is_polymorphic<T2>::result,
               Invalid_source_type_for_Dynamic_Cast);
#endif
#ifdef SMART_CAST_STATS_ALL
  add_smart_cast_stats_all(
      typeid(T2 *).name(),
      typeid(object_type_traits::remove_reference<T1>::type *).name());
#endif
  return (*SmartDynamicCast::CHelper2<T2>::smart_cast<
          object_type_traits::remove_reference<T1>::type>(&p));
}

#ifdef XRGAME_EXPORTS
template <>
extern CGameObject *
SmartDynamicCast::smart_cast<CGameObject, CObject>(CObject *p);
#endif
