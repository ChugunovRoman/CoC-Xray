...
template <class M, typename P> struct CSaver {
  ...
  template <typename T> struct CHelper4 {
    template <bool a>
    IC static void save_data(const T &data, M &stream, const P &p) {
      CHelper<T>::save_data<object_type_traits::is_pointer<T>::value>(
          data, stream, p);
    }
  ...
  IC static void save_data(const std::pair<T1, T2> &data, M &stream,
                           const P &p) {
  ...
  IC static void save_data(const svector<T, size> &data, M &stream,
                           const P &p) {
    stream.w_u32((u32)data.size());
  ...
  template <typename T1, typename T2>
  IC static void save_data(const std::queue<T1, T2> &data, M &stream,
                           const P &p) {
  ...
  template <typename T1, typename T2>
  IC static void save_data(const xr_stack<T1, T2> &data, M &stream,
                           const P &p) {
  ...
  template <typename T>
  IC static void save_data(const T &data, M &stream, const P &p) {
    CHelper4<T>::save_data<object_type_traits::is_stl_container<T>::value>(
        data, stream, p);
  }
};
