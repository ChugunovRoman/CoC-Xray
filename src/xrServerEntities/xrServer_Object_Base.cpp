  ...
  if (pSettings->line_exist(caSection, "custom_data")) {
    ...
    if (config) {
      ...
      /* 			if (Core.ParamFlags.is(Core.dbgdev))
                              {
                                      Msg("%s try loading custom data from %s
         [%d]", caSection, raw_file_name, size);
                              } */
...
#pragma warning(disable : 4238)
    m_ini_file = xr_new<CInifile>(
        &IReader((void *)(*m_ini_string), xr_strlen(m_ini_string)),
...
void CSE_Abstract::Spawn_Write(NET_Packet &tNetPacket, BOOL bLocal) {
  ...
  if (bLocal)
    ...
    tNetPacket.w_u16(
        u16(s_flags.flags & ~(M_SPAWN_OBJECT_LOCAL | M_SPAWN_OBJECT_ASPLAYER)));
...
static enum EGameTypes {
  ...
  GAME_DEATHMATCH = 2,
  //	GAME_CTF							= 3,
  //	GAME_ASSAULT						= 4,	// Team1 - assaulting,
  //Team0
  //- Defending
  GAME_CS = 5,
...
BOOL CSE_Abstract::Spawn_Read(NET_Packet &tNetPacket) {
  ...
  if (m_wVersion > 70) {
    u16 client_data_size =
        (m_wVersion > 93) ? tNetPacket.r_u16()
                          : tNetPacket.r_u8(); //не может быть больше 256 байт
    if (client_data_size > 0) {
      //			Msg
      //("SERVER:loading:load:%d bytes:%d:%s",client_data_size,ID,s_name_replace
      //? s_name_replace : "");
