...
int engine::lua_sethook(lua_State *L, lua_Hook func, lua_mask_type mask,
                        int count) {
...
void engine::type_convert_class(char *const buffer, unsigned int const size,
...
bool engine::type_convert_instance(char *buffer, unsigned int const size,
                                   lua_State *state, int index) {
...
void engine::type_convert_userdata(char *buffer, unsigned int const size,
                                   lua_State *state, int index) {
...
bool engine::type_to_string(char *const buffer, unsigned int const size,
                            lua_State *const state, int const index,
...
void engine::fill_class_info(cs::lua_studio::backend &backend,
                             char *const buffer, unsigned int const size,
                             luabind::detail::object_rep *object,
                             luabind::detail::class_rep *class_rep,
                             lua_State *state) {
  pstr stream = buffer;
  ...
  for (u32 i = 0; I != E; ++I) {
    ...
    if (use_in_description)
      stream += xr_sprintf(stream, size - (stream - buffer), "%s[%s]=%s ",
                           (*I).first, type, value);
    else
...
void engine::value_convert_class(cs::lua_studio::backend &backend, char *buffer,
                                 unsigned int size,
                                 luabind::detail::class_rep *class_rep,
                                 lua_State *state, int index,
                                 cs::lua_studio::icon_type &icon_type,
...
bool engine::value_convert_instance(cs::lua_studio::backend &backend,
  ...
  for (i = 0; lua_next(state, -2); ++i) {
    ...
    if (use_in_description)
      stream += xr_sprintf(stream, size - (stream - buffer), "%s[%s]=%s ", name,
                           type, value);
    else
      stream +=
          xr_sprintf(stream, size - (stream - buffer), "%s=%s ", name, value);
...
bool engine::value_convert_instance(cs::lua_studio::backend &backend,
  ...
  if (full_description &&
    ...
    value_convert_class(backend, buffer, size, object->crep(), state, index,
...
bool engine::value_to_string(cs::lua_studio::backend &backend,
  ...
  case engine::lua_type_user_data: {
    if (!luabind::detail::is_class_object(state, index)) {
      ...
      VERIFY(class_rep);
      value_convert_class(backend, buffer, size, class_rep, state, index,
                          icon_type, full_description);
      return (true);
    }
...
void engine::fill_class_data(cs::lua_studio::backend &backend,
...

  {
    ...
    for (; i != e; ++i)
      value_to_expand.add_value((*i).base->name(),
