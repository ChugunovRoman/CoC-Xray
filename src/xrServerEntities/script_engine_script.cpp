...
#ifndef MASTER_GOLD
  ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeMessage, "%s",
                                  caMessage);
#else
  Log(caMessage);
...
struct profile_timer_script {
  ...
  IC profile_timer_script &
  operator=(const profile_timer_script &profile_timer) {
  ...
  IC float time() const {
    FPU::m64r();
    float result = (float(double(m_accumulator) / double(CPU::clk_per_second)) *
                    1000000.f);
    FPU::m24r();
    return (result);
  }
...
void CScriptEngine::script_register(lua_State *L) {
  module(L)[
      ...
      def("log", &LuaLog), def("print_stack", &PrintStack),
