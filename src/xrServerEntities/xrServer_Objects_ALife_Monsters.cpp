...
// Reutskiy and Dmitriy Iassenev 	Description : Server objects monsters for
// ALife simulator
////////////////////////////////////////////////////////////////////////////
...
void setup_location_types_section(GameGraph::TERRAIN_VECTOR &m_vertex_types,
                                  CInifile const *ini, LPCSTR section) {
...
void setup_location_types(GameGraph::TERRAIN_VECTOR &m_vertex_types,
                          CInifile const *ini, LPCSTR string) {
      ...
      if (!char_info.data()->m_Class.size() || class_found) {
        ...
        if (char_info.data()->m_Rank == NO_RANK ||
          ...
          if (char_info.data()->m_Reputation == NO_REPUTATION ||
            ...
            if (ai().get_alife())
              count =
                  ai().alife().registry(specific_characters).object(id, true);
...
void CSE_ALifeTraderAbstract::set_specific_character(shared_str new_spec_char) {
  ...
  if (m_SpecificCharacter.size()) {
    if (ai().get_alife())
      ai().alife()
          .registry(specific_characters)
          .remove(m_SpecificCharacter, true);
  }
#endif
  ...
  if (ai().get_alife()) {
    ...
    ai().alife()
        .registry(specific_characters)
        .add(m_SpecificCharacter, a, true);
  }
#endif
...
CSE_ALifeCustomZone::CSE_ALifeCustomZone(LPCSTR caSection)
  ...
  if (pSettings->line_exist(caSection, "hit_type"))
    m_tHitType =
        ALife::g_tfString2HitType(pSettings->r_string(caSection, "hit_type"));
  else
...
void CSE_ALifeAnomalousZone::FillProps(LPCSTR pref, PropItemVec &items) {
  ...
  PHelper().CreateFloat(items,
                        PrepareKey(pref, *s_name, "offline interactive radius"),
                        &m_offline_interactive_radius, 0.f, 100.f);
  PHelper().CreateU16(
      items, PrepareKey(pref, *s_name, "ALife\\Artefact spawn places count"),
      &m_artefact_spawn_count, 32, 256);
  PHelper().CreateFlag32(items,
                         PrepareKey(pref, *s_name, "ALife\\Visible for AI"),
                         &m_flags, flVisibleForAI);
}
#endif // #ifndef XRGAME_EXPORTS
...
CSE_ALifeCreatureAbstract::CSE_ALifeCreatureAbstract(LPCSTR caSection)
  ...
  m_ef_weapon_type =
      READ_IF_EXISTS(pSettings, r_u32, caSection, "ef_weapon_type", u32(-1));
  m_ef_detector_type =
      READ_IF_EXISTS(pSettings, r_u32, caSection, "ef_detector_type", u32(-1));
...
void CSE_ALifeCreatureAbstract::FillProps(LPCSTR pref, PropItemVec &items) {
  inherited::FillProps(pref, items);
  PHelper().CreateU8(items, PrepareKey(pref, *s_name, "Team"), &s_team, 0, 64,
                     1);
  PHelper().CreateU8(items, PrepareKey(pref, *s_name, "Squad"), &s_squad, 0, 64,
                     1);
  PHelper().CreateU8(items, PrepareKey(pref, *s_name, "Group"), &s_group, 0, 64,
                     1);
  PHelper().CreateFloat(items, PrepareKey(pref, *s_name, "Personal", "Health"),
                        &fHealth, 0, 2, 5);
}
#endif // #ifndef XRGAME_EXPORTS
...
IC void
CSE_ALifeCreatureAbstract::set_killer_id(ALife::_OBJECT_ID const killer_id) {
...
CSE_ALifeMonsterAbstract::CSE_ALifeMonsterAbstract(LPCSTR caSection)
  ...
  m_fCurrentLevelGoingSpeed =
      READ_IF_EXISTS(pSettings, r_float, caSection, "current_level_going_speed",
                     m_fGoingSpeed);
  ...
  m_stay_after_death_time_interval = generate_time(
      1, 1, 1,
      pSettings->r_u32("monsters_common", "stay_after_death_time_interval"), 0,
      0);
#endif // XRGAME_EXPORTS
}
...
void CSE_ALifeMonsterAbstract::FillProps(LPCSTR pref, PropItemVec &items) {
  ...
  PHelper().CreateFlag32(items,
                         PrepareKey(pref, *s_name, "ALife\\No move in offline"),
                         &m_flags, flOfflineNoMove);
  PHelper().CreateFlag32(items,
                         PrepareKey(pref, *s_name, "Use smart terrain tasks"),
                         &m_flags, flUseSmartTerrains);
...
void CSE_ALifeMonsterRat::FillProps(LPCSTR pref, PropItemVec &items) {
  ...
  PHelper().CreateFloat(items,
                        PrepareKey(pref, *s_name, "Personal", "Field of view"),
                        &fEyeFov, 0, 170, 10);
  PHelper().CreateFloat(items,
                        PrepareKey(pref, *s_name, "Personal", "Eye range"),
                        &fEyeRange, 0, 300, 10);
  PHelper().CreateFloat(items,
                        PrepareKey(pref, *s_name, "Personal", "Minimum speed"),
                        &fMinSpeed, 0, 10, 0.1f);
  PHelper().CreateFloat(items,
                        PrepareKey(pref, *s_name, "Personal", "Maximum speed"),
                        &fMaxSpeed, 0, 10, 0.1f);
  PHelper().CreateFloat(items,
                        PrepareKey(pref, *s_name, "Personal", "Attack speed"),
                        &fAttackSpeed, 0, 10, 0.1f);
  PHelper().CreateFloat(
      items, PrepareKey(pref, *s_name, "Personal", "Pursuit distance"),
      &fMaxPursuitRadius, 0, 300, 10);
  PHelper().CreateFloat(items,
                        PrepareKey(pref, *s_name, "Personal", "Home distance"),
  ...
  PHelper().CreateFloat(items,
                        PrepareKey(pref, *s_name, "Morale", "Restore quant"),
  ...
  PHelper().CreateFloat(items,
                        PrepareKey(pref, *s_name, "Morale", "Minimum value"),
                        &fMoraleMinValue, -100, 100, 5);
  PHelper().CreateFloat(items,
                        PrepareKey(pref, *s_name, "Morale", "Maximum value"),
                        &fMoraleMaxValue, -100, 100, 5);
  PHelper().CreateFloat(items,
                        PrepareKey(pref, *s_name, "Morale", "Normal value"),
                        &fMoraleNormalValue, -100, 100, 5);
  // attack
  PHelper().CreateFloat(items, PrepareKey(pref, *s_name, "Attack", "Hit power"),
                        &fHitPower, 0, 200, 5);
  PHelper().CreateU16(items,
                      PrepareKey(pref, *s_name, "Attack", "Hit interval"),
                      &u16HitInterval, 0, 65535, 500);
  PHelper().CreateFloat(items, PrepareKey(pref, *s_name, "Attack", "Distance"),
                        &fAttackDistance, 0, 300, 10);
  PHelper().CreateFloat(items,
                        PrepareKey(pref, *s_name, "Attack", "Maximum angle"),
...
void CSE_ALifeHumanAbstract::FillProps(LPCSTR pref, PropItemVec &items) {
  inherited1::FillProps(pref, items);
  inherited2::FillProps(pref, items);
  PHelper().CreateFlag32(items, PrepareKey(pref, *s_name, "Group behaviour"),
