...
#endif // #ifndef USE_LUA_STUDIO
  //	lua_sethook							(lua(),
  //lua_hook_call, LUA_MASKLINE|LUA_MASKCALL|LUA_MASKRET,	0);
...
void CScriptEngine::process_file_if_exists(LPCSTR file_name,
  ...
  if (it != xray_scripts.end()) {
    ...
    load_file_into_namespace(it->second.c_str(),
                             strcmp(file_name, "_g") == 0 ? "_G" : file_name);
    return;
  }

  if (warn_if_not_exist)
    Msg("Variable %s not found; No script by this name exists, either.",
        file_name);
...
bool CScriptEngine::function_object(LPCSTR function_to_call,
                                    luabind::object &object, int type) {
