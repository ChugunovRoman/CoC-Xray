...
CObjectItemScript::server_object(LPCSTR section) const {
  ...
  try {
    ...
    try {
      instance =
          xr_new<luabind::object>((luabind::object)(m_server_creator(section)));
    } catch (const std::exception &e) {
      Msg("Exception [%s] raised while creating server object from section "
          "[%s]",
          e.what(), section);
      return (0);
    } catch (...) {
      Msg("Exception raised while creating server object from section [%s]",
          section);
      return (0);
    }
    ...
    Msg("Exception [%s] raised while casting and adopting script server object "
        "from section [%s]",
        e.what(), section);
    return (0);
  } catch (...) {
    Msg("Exception raised while creating script server object from section "
        "[%s]",
        section);
    return (0);
  }
