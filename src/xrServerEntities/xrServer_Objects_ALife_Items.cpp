...
void CSE_ALifeInventoryItem::UPDATE_Read(NET_Packet &tNetPacket) {
...

  {
    ...
    base()->o_Position.set(
        State.position); // this is very important because many functions use
                         // this o_Position..
...
void CSE_ALifeInventoryItem::FillProps(LPCSTR pref, PropItemVec &values) {
  PHelper().CreateFloat(values,
                        PrepareKey(pref, *base()->s_name, "Item condition"),
...
void CSE_ALifeItemWeapon::STATE_Read(NET_Packet &tNetPacket, u16 size) {
  ...
  if (a_current_addon.data ==
      90) // Alun: Hack because all.spawn ammo_current all set to 90. Since this
          // unlikely anyway (launcher=26, silencer=1, scope=0), this hack is
          // okay.
