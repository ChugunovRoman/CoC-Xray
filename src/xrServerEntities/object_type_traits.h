...
template <bool expression, typename T1, typename T2> struct _if {
  template <bool> struct selector { typedef T2 result; };

  template <> struct selector<true> { typedef T1 result; };
...
template <typename T> struct type { typedef T result; };
...
struct is_base_and_derived_or_same_for_template_template_1_1 {
  template <typename P>
  static
      typename _if<is_base_and_derived<P, T3>::value || is_same<P, T3>::value,
                   detail::yes, detail::no>::result
      select(T1<P> *);
  static detail::no select(...);
