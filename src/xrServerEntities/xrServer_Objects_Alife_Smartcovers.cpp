...
BOOL is_combat_cover(shared_str const &table_id) {
  ...
  VERIFY2(result, make_string("bad or missing description in smart_cover [%s]",
                              table_id.c_str()));
  ...
  if (value.type() == LUA_TNIL) {
    Msg("! is_combat_cover flag not found for smart_cover [%s], forcing to "
        "\"true\"",
        table_id.c_str());
    return (TRUE);
  }
...
void CSE_SmartCover::FillProps(LPCSTR pref, PropItemVec &items) {
  ...
  PHelper().CreateFloat(items,
                        PrepareKey(pref, *s_name, "enter min enemy distance"),
                        &m_enter_min_enemy_distance, 0.f, 100.f);
  PHelper().CreateFloat(items,
                        PrepareKey(pref, *s_name, "exit min enemy distance"),
...
void parse_table(luabind::object const &table, LPCSTR identifier,
...
shared_str parse_vertex(luabind::object const &table, LPCSTR identifier,
                        bool const &in) {
...
void CSE_SmartCover::check_enterable_loopholes(shared_str const &description) {
  ...
  VERIFY2(result,
          make_string("bad or missing transitions table in smart_cover [%s]",
                      temp));
...
void draw_frustum(CDUInterface *du, float FOV, float _FAR, float A, Fvector &P,
                  Fvector &D, Fvector &U, u32 const &CL) {
...
void CSE_SmartCover::load_draw_data() {
  ...
  if (!result) {
    Msg("no or invalid smart cover description (bad or missing loopholes table "
        "in smart_cover [%s])",
        temp);
    return;
    //		VERIFY2					(result, make_string("bad or missing loopholes
    //table in smart_cover [%s]", temp));
  }
  ...
  for (; I != E; ++I) {
    ...
    /*		luabind::object	transitions;
       ...
       == LUA_TTABLE, "invalid loophole description passed"); shared_str
       action_from = smart_cover::parse_vertex(transition, "action_from", true);
                            if (action_from != "idle")
                                    continue;
                            shared_str			action_to =
       smart_cover::parse_vertex(transition, "action_to", false); if (action_to
       != "fire") continue;
    ...
    */
    ...
    H.animation_id =
        make_string("loophole_%s_visual", H.string_identifier.c_str()).c_str();
  }
...
void CSE_SmartCover::on_render(CDUInterface *du, ISE_AbstractLEOwner *owner,
                               bool bSelected, const Fmatrix &parent,
  ...
  for (; it != it_e; ++it) {
    ...
    // du->DrawFlag(H.point_position, 0, 1.0f, 1, 1, color_rgba(0,255,0,255),
    // FALSE); du->DrawCylinder(Fidentity, pos, Fvector().set(0.f, 1.f,
    // 0.f), 1.f, .05f, color_rgba(0,255,0,255), color_rgba(0,255,0,255), TRUE,
    // FALSE);
