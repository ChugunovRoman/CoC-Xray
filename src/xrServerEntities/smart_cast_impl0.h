  ...
  template <typename T1> struct add {
    ...
    template <bool a> struct selector {
      typedef Loki::Typelist<
          Loki::Typelist<typename Head::Head,
                         Loki::Typelist<P, typename Head::Tail>>,
    ...
    typedef typename selector<is_type<T, typename Head::Head>::value>::result
        result;
  };

  template <> struct add<Loki::NullType> {
    typedef Loki::Typelist<Loki::Typelist<T, Loki::Typelist<P, Loki::NullType>>,
                           List>
        result;
  };
...
#define add_to_cast_list(B, A)                                                 \
  typedef SmartDynamicCast::CTypeHelper<cast_type_list, A, B>::result          \
      TypeList_##A##B
