...
public:
  ...
  IC void play_at_pos(CObject *O, const Fvector &pos, u32 flags = 0,
                      float delay = 0.f);
  IC void play_no_feedback(CObject *O, u32 flags = 0, float delay = 0.f,
...
IC ref_sound_data::ref_sound_data(LPCSTR fName, esound_type sound_type,
                                  int game_type) {
...
IC void ref_sound::clone(const ref_sound &from, esound_type sound_type,
                         int game_type) {
  VERIFY(!::Sound->i_locked());
...
IC void ref_sound::play_at_pos(CObject *O, const Fvector &pos, u32 flags,
                               float d) {
  VERIFY(!::Sound->i_locked());
  ::Sound->play_at_pos(*this, O, pos, flags, d);
}
IC void ref_sound::play_no_feedback(CObject *O, u32 flags, float d,
