...
void CSoundStream::Update() {
  ...
  if (bNeedUpdate) {
    ...
    pBuffer->SetVolume(LONG((1 - fRealVolume * psSoundVMusic * fBaseVolume) *
                            float(DSBVOLUME_MIN)));
    bNeedUpdate = false;
  }
}
    ...
    u32 dwSoundBytes)    // size of block to copy
{
  ...
  if (DS_OK == pBuffer->Lock(dwOffset, dwSoundBytes, &lpvPtr1, &dwBytes1,
                             &lpvPtr2, &dwBytes2, 0)) {
    // Write to pointers.
...
//--------------------------------------------------------------------------------------------------
BOOL ADPCMCreateSoundBuffer(IDirectSound8 *lpDS, IDirectSoundBuffer **pDSB,
                            WAVEFORMATEX *fmt) {
  DSBUFFERDESC dsBD;
  ...
  dsBD.dwFlags =
      DSBCAPS_CTRLVOLUME | DSBCAPS_GETCURRENTPOSITION2 | DSBCAPS_LOCSOFTWARE;
...
void CSoundStream::LoadADPCM() {
  ...
  VERIFY(DataPos);
  // dest format
  CHK_DX(acmFormatSuggest(NULL, psrc, pwfx, dwFMT_Size,
