...
Comments:
  ...
  Todo
  -	Performance improvements, right now for each vertex I am building the
list of it's neighbors, when we could do a single pass and build the full
adjacency map in the beginning.  Note: I tried this and didn't see a real perf
improvement.
...
public:
  virtual bool CanSmooth(MeshMender::Triangle *t1, MeshMender::Triangle *t2,
    ...
    if (D3DXVec3Dot(&tmp1, &tmp2) >= minCreaseAngle) {
      return true;
    } else if ((tmp1 == D3DXVECTOR3(0, 0, 0)) &&
...
void MeshMender::ProcessNormals(TriangleList &possibleNeighbors,
  ...
  for (i = 0; i < neighborGroups.size(); ++i) {
    ...
    for (size_t t = 0; t < curGroup.size();
  ...
  for (i = 0; i < neighborGroups.size(); ++i) {
      ...
      for (size_t indx = 0; indx < 3; ++indx) // for each vert in that tri
      {
        ...
        if (theVerts[m_Triangles[tID].indices[indx]].pos == workingPosition) {
          ...
          if (otherGroupsIndices.find(m_Triangles[tID].indices[indx]) !=
            ...
            theVerts[m_Triangles[tID].indices[indx]].normal =
                groupNormalVectors[i];
          }
    ...
    for (xr_set<size_t>::iterator it = thisGroupIndices.begin();
         it != thisGroupIndices.end(); ++it) {
      ...
      for (size_t indx = 0; indx < 3; indx++) // for each vert in that tri
      {
        ...
        if (theVerts[m_Triangles[tID].indices[indx]].pos == workingPosition) {
          ...
          if (otherGroupsIndices.find(m_Triangles[tID].indices[indx]) !=
            ...
            // otherwise, just update it with the new vector
            theVerts[m_Triangles[tID].indices[indx]].tangent =
                groupTangentVectors[i];
          }
      ...
      for (size_t indx = 0; indx < 3; ++indx) // for each vert in that tri
      {
        ...
        if (theVerts[m_Triangles[tID].indices[indx]].pos == workingPosition) {
          ...
          if (otherGroupsIndices.find(m_Triangles[tID].indices[indx]) !=
            ...
            // otherwise, just update it with the new vector
            theVerts[m_Triangles[tID].indices[indx]].binormal =
                groupBinormalVectors[i];
          }
...
bool MeshMender::Mend(xr_vector<Vertex> &theVerts,
  ...
  for (VertexChildrenMap::iterator vert = m_VertexChildrenMap.begin();
    ...
    if (computeNormals == CALCULATE_NORMALS) {
      ProcessNormals(possibleNeighbors, theVerts, mappingNewToOldVert,
                     workingPosition);
    }
    ProcessTangents(possibleNeighbors, theVerts, mappingNewToOldVert,
                    workingPosition);
    ProcessBinormals(possibleNeighbors, theVerts, mappingNewToOldVert,
                     workingPosition);
  }
...
void MeshMender::BuildGroups(
  ...
  BuildGroups(neighbor1, possibleNeighbors, neighborGroups, theVerts,
              smoothChecker, minCreaseAngle);
  BuildGroups(neighbor2, possibleNeighbors, neighborGroups, theVerts,
              smoothChecker, minCreaseAngle);
}

void MeshMender::FindNeighbors(Triangle *tri, TriangleList &possibleNeighbors,
...
bool MeshMender::TriHasEdge(const size_t &p0, const size_t &p1,
                            const size_t &triA, const size_t &triB,
                            const size_t &triC) {
...
bool MeshMender::TriHasEdge(const D3DXVECTOR3 &p0, const D3DXVECTOR3 &p1,
...
bool MeshMender::SharesEdge(Triangle *triA, Triangle *triB,
                            xr_vector<Vertex> &theVerts) {
...
void MeshMender::SetUpData(xr_vector<Vertex> &theVerts,
                           const xr_vector<unsigned int> &theIndices,
                           xr_vector<unsigned int> &mappingNewToOldVert,
                           const NormalCalcOption computeNormals) {
  assert(((theIndices.size() % 3) == 0) &&
         "expected the indices to be a multiple of 3");
  unsigned int i;
...
void MeshMender::SetUpFaceVectors(Triangle &t, const xr_vector<Vertex> &verts,
  ...
  GetGradients(verts[t.indices[0]], verts[t.indices[1]], verts[t.indices[2]],
...
void MeshMender::OrthogonalizeTangentsAndBinormals(
  ...
  for (size_t i = 0; i < len; ++i) {
    ...
    // of the T, B,N matrix
    // we can use the Gram-Schmidt algorithm to find the newTangent and the
    // newBinormal newT = T - (N dot T)N newB = B - (N dot B)N - (newT dot
    // B)newT
    ...
    if ((lenTan <= 0.001f) || (lenBin <= 0.001f)) // should be approx 1.0f
    {
      ...
      if (lenTan > 0.5f) {
        ...
        // I'm checking two possible axis, because the normal could be one of
        // them, and we want to chose a different one to start making our valid
        // basis. I can find out which is further away from it by checking the
        // dot product
        D3DXVECTOR3 startAxis;

        if (D3DXVec3Dot(&xAxis, &(theVerts[i].normal)) <
            D3DXVec3Dot(&yAxis, &(theVerts[i].normal))) {
          // the xAxis is more different than the yAxis when compared to the
          // normal
          startAxis = xAxis;
        } else {
          // the yAxis is more different than the xAxis when compared to the
          // normal
          startAxis = yAxis;
        }

        D3DXVec3Cross(&(theVerts[i].tangent), &(theVerts[i].normal),
                      &startAxis);
        D3DXVec3Cross(&(theVerts[i].binormal), &(theVerts[i].normal),
                      &(theVerts[i].tangent));
      }
    } else {
      // one final sanity check, make sure that they tangent and binormal are
      // different enough
      if (D3DXVec3Dot(&(theVerts[i].binormal), &(theVerts[i].tangent)) >
          0.999f) {
...
void MeshMender::GetGradients(const MeshMender::Vertex &v0,
  ...
  // this is a linear system with six unknowns and six equatinos, for TxTyTz
  // BxByBz [px,py,pz] = [s1,t1] * [Tx,Ty,Tz]
...
void MeshMender::UpdateTheIndicesWithFinalIndices(
    xr_vector<unsigned int> &theIndices) {
...
void MeshMender::FixCylindricalWrapping(
  ...
  // to fix this, we can duplicate a vertex and add 1.0 to the wrapped texture
  // coordinate we need to do this for both the S and the T directions.
  ...
  for (index = 0; index < theIndices.size(); index += 3) {
    ...
    for (unsigned int begin = 0; begin < 3; ++begin) {
      ...
      if (sBegin <= 1.0f && sEnd <= 1.0f && sBegin >= 0.0f && sEnd >= 0.0f) {
        // we only handle coordinates between 0 and 1 for the cylindrical
        // wrappign fix
...
void MeshMender::AppendToMapping(const size_t oldIndex,
  ...
  if (oldIndex >= originalNumVerts) {
    // then this is a newer vertex we are mapping to another vertex we created
    // in meshmender. we need to find the original old vertex index to map to.
