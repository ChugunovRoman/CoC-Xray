...
Comments:
  ...
  per pixel lighting.  Given a set of vertices and normal map texture
coordinates, MeshMender will return a set of normals, binormals, and tangents
taking into account texture mirroring.
  ...
  so that each unsmoothable neighbor has it's own set of vectors in a unique
vertex.
...
need to fill those in.
                //however, if you do not have meshmender compute the normals,
you _must_ pass in valid
    ...
    //then update your mesh with the data provided in the Vertex vector
        ...
        //to help associate any special per vertex data you had in your original
mesh
...
public:
  ...
  //			  mesh mender is done with it, the number of vertices
  //may grow 			  and it will be filled with normals, tangents
  // and binormals
  //
  // theIndices - should be initialized with your mesh indices
  //				will contain the new indices..we are not adding
  // triangles, 				  so the number of indices passed back should be the
  // same as the 				  number of indices passed in, but they may point to new vertices
  // now.
  //
  // mappingNewToOldVert - this should be passed in as an empty vector. after
  // mending 				it will contain a mapping of newvertexindex ->
  // oldvertexindex 				so it
  // could be used to map any per vertex data you had in your original
  ...
  //
  //				where myData is some custom vertex data in your
  //original mesh.
  //
  // minNormalsCreaseCosAngle - the minimum cosine of the angle between normals
  //							 so that they are allowed to be
  //smoothed
  // together 							 ranges between -1.0 and +1.0
  // this is ignored if computeNormals is set to DONT_CALCULATE_NORMALS
  //
  //
  // minTangentsCreaseCosAngle - the minimum cosine of the angle between
  // tangents 							  so that they are allowed to be smoothed
  // together 							  ranges between
  //-1.0 and +1.0
  //
  // minBinormalsCreaseCosAngle - the minimum cosine of the angle between
  // binormals 							   so that they are allowed to be smoothed
  // together 		 					   ranges
  // between
  //-1.0 and +1.0
  //
  // weightNormalsByArea - an ammount to blend the normalized face normal, and
  // the 						unnormalized face normal together.  Thus weighting
  // the 						normal by the
  // face area by a given ammount 						ranges between 0.0 and +1.0
  // 0.0 means use the normalized face normals (not weighted by
  // area) 						1.0 means use the
  // unnormalized face normal(weighted by area) 						this is
  // ignored if computeNormals is set to DONT_CALCULATE_NORMALS
  //
  // computeNormals - should mesh mender calculate normals? If this is set to
  // DONT_CALCULATE_NORMALS 					then the vertex normals after mesh
  // mender is called
  // will be the 					same ones you pass in.  If you are automatically
  // calculating normals yourself, 					you may
  // find that meshmender provides greater control over how normals are smoothed
  // together. I've been able to get better results using the Crease angle with
  // meshmender's smoothing groups
  //
  // respectExistingSplits - DONT_RESPECT_SPLITS means that neighboring
  // triangles for smoothing will be determined 						  based on
  // position and not on
  // indices. 						  RESPECT_SPLITS means that neighboring
  // triangles will be determined based on the indices of the
  // triangle and not the positions of the vertices. 						  you can usually get better
  //smoothing by not
  // respecting existing splits 						  only respect them if you
  // know they should be respected.
  //
  // fixCylindricalWrapping - DONT_FIX_CYLINDRICAL means take the texture
  // coordinates as they come 						   FIX_CYLINDRICAL means we
  // might need to split the
  // verts 						   at that point and generate the proper texture
  // coordinate. 						   for instance,
  // if we have tex coords   0.9 -> 0.0-> 0.2 we would need to add
  // a
  // new vert so that we have       0.9 -> 1.0  0.0-> 0.2
  ...
  bool Mend(
      xr_vector<Vertex> &theVerts, xr_vector<unsigned int> &theIndices,
...
protected:
  ...
  void FindNeighbors(Triangle *tri, TriangleList &possibleNeighbors,
                     Triangle **neighbor1, Triangle **neighbor2,
  ...
  bool TriHasEdge(const D3DXVECTOR3 &p0, const D3DXVECTOR3 &p1,
                  const D3DXVECTOR3 &triA, const D3DXVECTOR3 &triB,
                  const D3DXVECTOR3 &triC);

  bool TriHasEdge(const size_t &p0, const size_t &p1, const size_t &triA,
  ...
  void UpdateIndices(const size_t oldIndex, const size_t newIndex,
                     TriangleList &curGroup);

  // adds a new mapping entry,
  // takes into account that we may be mapping a new vertex to another new
  // vertex, and uses the original old vertex index....is that confusing?
