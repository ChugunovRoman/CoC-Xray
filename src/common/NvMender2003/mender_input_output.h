...
template <typename type_face>
IC void set_face(type_face &iF, unsigned int v0, unsigned int v1,
                 unsigned int v2) {
  face_vertex(iF, 0) = v0;
  face_vertex(iF, 1) = v1;
  face_vertex(iF, 2) = v2;
...
static void fill_mender_input(const xr_vector<type_vertex> &vertices,
  ...
  // fill inputs ( indices )
  for (xr_vector<type_face>::const_iterator face_it = faces.begin();
...
static void retrive_data_from_mender_otput(
    xr_vector<type_vertex> &vertices, // in-out
  ...
  xr_vector<type_vertex> old_vertices;
  {
    old_vertices.clear();
    old_vertices =
        vertices; // save old vertices to retrive through mappingNewToOldVert
                  // data that missing in MeshMender::Vertex
    // retriving data
    const u32 face_count = faces.size();
    for (u32 i = 0; i < face_count; ++i)
      set_face(faces[i], theIndices[3 * i + 0], theIndices[3 * i + 1],
               theIndices[3 * i + 2]);
  }
  {
    ...
    for (u32 i = 0; i < vertex_count; i++)
      set_vertex(vertices[i], old_vertices[mappingNewToOldVert[i]],
                 theVerts[i]);
  }
}
