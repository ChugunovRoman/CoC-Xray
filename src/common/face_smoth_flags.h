...
IC bool do_connect_faces_by_faces_edge_flags(u32 start_face_flags,
                                             u32 test_face_flags,
  ...
  bool start_edge_smooth = is_soft_edge(
      start_face_flags,
      start_common_edge_idx); //!( start.sm_group & (1<<start_common_face) );
  bool test_edge_smooth = is_soft_edge(
      test_face_flags,
      test_common_edge_idx); //!( test.sm_group  & (1<<test_common_face) );
