  ...
  g_pGamePersistent->LoadTitle();
  {
    ...
    for (u32 i = 0; i < count; i++) // skip first shader as "reserved" one
    {
      ...
      *delim = 0;
      ...
      // Alundaio: Fix Static Renderer issue with 'default' shader! Force change
      // to 'def_shaders\def_vertex'
      if (!xr_strcmp(n_sh, "default") && xr_strcmp(n_tlist, ","))
        ...
        Shaders[i] =
            dxRenderDeviceRender::Instance().Resources->Create(n_sh, n_tlist);
    }
    chunk->close();
  }
  ...
  if (base_fs->find_chunk(fsL_VB)) {
    ...
    for (u32 i = 0; i < count; i++) {
      ...
      R_CHK(HW.pDevice->CreateVertexBuffer(vCount * vSize, dwUsage, 0,
                                           D3DPOOL_MANAGED, &VB[i], 0));
      HW.stats_manager.increment_stats(
          vCount * vSize, enum_stats_buffer_type_vertex, D3DPOOL_MANAGED);
      R_CHK(VB[i]->Lock(0, 0, (void **)&pData, 0));
      fs->r(pData, vCount * vSize);
      //			CopyMemory
      //(pData,fs->pointer(),vCount*vSize);	//.???? copy while skip T&B
      VB[i]->Unlock();
  ...
  if (count) {
    ...
    for (i = 0; i < count; i++) {
      ...
      for (u32 j = 2; j < P.vertices.size(); j++)
        CL.add_face_packed_D(P.vertices[0], P.vertices[j - 1], P.vertices[j],
                             u32(i));
    }
    if (CL.getTS() < 2) {
      Fvector v1, v2, v3;
