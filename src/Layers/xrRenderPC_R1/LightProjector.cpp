...
void CLightProjector::setup(int id) {
  ...
  float factor =
      _sqr(dist / clipD(Rd)) * (1 - ps_r1_lmodel_lerp) + ps_r1_lmodel_lerp;
...
void CLightProjector::calculate() {
  ...
  CHK_DX(HW.pDevice->Clear(
      0, 0, D3DCLEAR_ZBUFFER | (HW.Caps.bStencil ? D3DCLEAR_STENCIL : 0), 0, 1,
      0));
  ...
  for (u32 c_it = 0; c_it < cache.size(); c_it++) {
    ...
    CHK_DX(HW.pDevice->Clear(
        0, 0, D3DCLEAR_TARGET,
        color_rgba_f(cap.x, cap.y, cap.z, (cap.x + cap.y + cap.z) / 4.f), 1,
        0));
    ...
    if (spatial) {
      ...
      if (spatial->spatial.sector)
        RImplementation.r_dsgraph_render_R1_box(spatial->spatial.sector, BB,
                                                SE_R1_LMODELS);
    }
...
void CLightProjector::render() {
  ...
  p1.y);	pv++;
          pv->set(0,			0,			.0001f,.9999f,
  C, p0.x, p0.y);	pv++;
          pv->set(float(_w),	float(_h),	.0001f,.9999f, C, p1.x, p1.y);
  pv++; pv->set(float(_w),	0,			.0001f,.9999f, C, p1.x,
  p0.y);	pv++; geom_Screen->Unlock			(4);
