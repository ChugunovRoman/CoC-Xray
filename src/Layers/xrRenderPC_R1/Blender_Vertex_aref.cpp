...
void CBlender_Vertex_aref::Compile(CBlender_Compile &C) {
  ...
  if (C.bEditor) {
    C.PassBegin();
    {
      ...
      if (oBlend.value)
        C.PassSET_Blend(TRUE, D3DBLEND_SRCALPHA, D3DBLEND_INVSRCALPHA, TRUE,
                        oAREF.value);
      else
    ...
    case SE_R1_NORMAL_HQ:
      // Level view
      {
        ...
        if (oBlend.value)
          ...
          C.r_Pass(sname, sname, TRUE, TRUE, TRUE, TRUE, D3DBLEND_ONE,
                   D3DBLEND_ZERO, TRUE, oAREF.value);
    ...
    case SE_R1_LPOINT:
      ...
      break;
    case SE_R1_LSPOT:
      C.r_Pass("vert_spot", "add_spot", FALSE, TRUE, FALSE, TRUE, D3DBLEND_ONE,
