...
BOOL CRenderTarget::Create() {
  ...
  if ((rtHeight != Device.dwHeight) || (rtWidth != Device.dwWidth)) {
    R_CHK(HW.pDevice->CreateDepthStencilSurface(
        rtWidth, rtHeight, HW.Caps.fDepth, D3DMULTISAMPLE_NONE, 0, TRUE, &ZB,
        NULL));
  } else {
    ZB = HW.pBaseZB;
    ZB->AddRef();
  ...
  R_CHK(HW.pDevice->CreateDepthStencilSurface(
      512, 512, HW.Caps.fDepth, D3DMULTISAMPLE_NONE, 0, TRUE, &pTempZB, NULL));
...
void CRenderTarget::calc_tc_noise(Fvector2 &p0, Fvector2 &p1) {
  ...
  //= iCeil(float(T->get_Width
  //())*param_noise_scale+EPS_S); .	u32			th
  //= iCeil(float(T->get_Height ())*param_noise_scale+EPS_S);
...
void CRenderTarget::calc_tc_duality_ss(Fvector2 &r0, Fvector2 &r1, Fvector2 &l0,
                                       Fvector2 &l1) {
...
BOOL CRenderTarget::Perform() {
  return Available() &&
         (((BOOL)RImplementation.m_bMakeAsyncSS) || NeedPostProcess() ||
...
struct TL_2c3uv {
  ...
  u32 color1;
  Fvector2 uv[3];
  IC void set(float x, float y, u32 c0, u32 c1, float u0, float v0, float u1,
...
void CRenderTarget::End() {
  ...
  // color_get_B(p_color)); Msg				("gray: %d,%d,%d",
  // color_get_R(p_gray), color_get_G(p_gray),		color_get_B(p_gray));
  // Msg				("add:  %d,%d,%d",
  // color_get_R(p_brightness), color_get_G(p_brightness),
  ...
  pv->set(du + 0, dv + float(_h), p_color, p_gray, r0.x, r1.y, l0.x, l1.y, n0.x,
          n1.y);
  pv++;
  pv->set(du + 0, dv + 0, p_color, p_gray, r0.x, r0.y, l0.x, l0.y, n0.x, n0.y);
  pv++;
  pv->set(du + float(_w), dv + float(_h), p_color, p_gray, r1.x, r1.y, l1.x,
          l1.y, n1.x, n1.y);
  pv++;
  pv->set(du + float(_w), dv + 0, p_color, p_gray, r1.x, r0.y, l1.x, l0.y, n1.x,
          n0.y);
  pv++;
