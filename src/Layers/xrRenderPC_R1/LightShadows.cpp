  ...
  for (u32 o_it = 0; o_it < casters.size(); o_it++) {
    ...
    for (u32 l_it = 0; (l_it < lights.size()) && (slot_id < slot_max); l_it++) {
      ...
      if (!bRTS) {
        ...
        HW.pDevice->Clear(0, 0, D3DCLEAR_TARGET, D3DCOLOR_XRGB(255, 255, 255),
                          1, 0);
      }
      ...
      // float		p_nearR	= C.C.distance_to(L.source->position) +
      // p_R*0.85f + eps; 			p_nearR =	p_near;
...
IC int PLC_calc(Fvector &P, Fvector &N, light *L, float energy, Fvector &O) {
  ...
  float C1 =
      clampr(Device.vCameraPosition.distance_to_sqr(P) / S_distance2, 0.f, 1.f);
...
void CLightShadows::render() {
  ...
  int batch = 0;
  u32 Offset = 0;
  FVF::LIT *pv = (FVF::LIT *)RCache.Vertex.Lock(batch_size * 3,
                                                geom_World->vb_stride, Offset);
  for (u32 s_it = 0; s_it < shadows.size(); s_it++) {
    ...
    if (CI_ptr == cache.end()) { // empty ?
      ...
      if (CI_ptr->O != CI_what.O ||
          CI_ptr->L != CI_what.L) { // we found something different
        CI_ptr = cache.insert(CI_ptr, CI_what);
        CI = &*CI_ptr;
        bValid = FALSE;
      } else {
    ...
    for (u32 tid = 0; tid < CI->tcnt; tid++) {
      ...
      */
      ...
      if (batch == batch_size) {
        ...
        pv = (FVF::LIT *)RCache.Vertex.Lock(batch_size * 3,
                                            geom_World->vb_stride, Offset);
        batch = 0;
      }
    }
