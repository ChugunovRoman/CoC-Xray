...
public:
  ...
  //if (C) C->i_Texture	(stage,texture); return *this;	} adopt_sampler&
  // _projective (bool _b)				{ if (C)
  //C->i_Projective(stage,_b); return *this;	} 	adopt_sampler&
  // _clamp
  //()						{ if (C) C->i_Address
  //(stage,D3DTADDRESS_CLAMP);									return
  //*this;
  //}
  //	adopt_sampler&			_wrap			()
  ...
  //()						{ if (C) C->i_Address
  //(stage,D3DTADDRESS_MIRROR);									return
  //*this;
  //}
  ...
  //{ if (C) C->i_Filter	(stage,D3DTEXF_LINEAR,D3DTEXF_POINT,
  // D3DTEXF_LINEAR); return *this;	} 	adopt_sampler&
  // _f_linear
  //()						{ if (C) C->i_Filter
  ...
  // _fmin_point
  //()						{ if (C)
  //C->i_Filter_Min(stage,D3DTEXF_POINT); return *this;	} 	adopt_sampler&
  //_fmin_linear	()						{ if (C)
  //C->i_Filter_Min(stage,D3DTEXF_LINEAR); return *this;	}
  // adopt_sampler&			_fmin_aniso
  //()						{ if (C)
  // C->i_Filter_Min(stage,D3DTEXF_ANISOTROPIC);
  // return *this;	} 	adopt_sampler&			_fmip_none
  //()						{ if (C)
  //C->i_Filter_Mip(stage,D3DTEXF_NONE); return *this;	} adopt_sampler&
  // _fmip_point
  //()						{ if (C)
  //C->i_Filter_Mip(stage,D3DTEXF_POINT); return *this;	} 	adopt_sampler&
  //_fmip_linear	()						{ if (C)
  //C->i_Filter_Mip(stage,D3DTEXF_LINEAR); return *this;	}
  // adopt_sampler&			_fmag_none
  //()						{ if (C)
  //C->i_Filter_Mag(stage,D3DTEXF_NONE); return *this;	} adopt_sampler&
  // _fmag_point
  //()						{ if (C)
  //C->i_Filter_Mag(stage,D3DTEXF_POINT); return *this;	} 	adopt_sampler&
  //_fmag_linear	()						{ if (C)
  //C->i_Filter_Mag(stage,D3DTEXF_LINEAR); return *this;	}
};
/*
...
public:
        adopt_dx10texture	(CBlender_Compile*	C, u32 TextureIndex)
...
public:
  ...
  adopt_compiler &_dx10Stencil(bool Enable, u32 Func, u32 Mask, u32 WriteMask,
...
doug_lea_allocator g_render_lua_allocator(s_fake_array, s_arena_size,
                                          "render:lua");
...
void CResourceManager::LS_Load() {
  ...
  module(LSVM)
      [class_<adopt_dx10options>("_dx10options")
       ...
       //&adopt_sampler::_projective ,return_reference_to(_1))
       //.def("clamp",
       ...
       class_<adopt_cmp_func>("cmp_func")
           .enum_("cmp_func")[value("never", int(D3DCMP_NEVER)),
  ...
  // load shaders
  xr_vector<char *> *folder = FS.file_list_open(
      "$game_shaders$", ::Render->getShaderPath(), FS_ListFiles | FS_RootOnly);
  VERIFY(folder);
...
Shader *CResourceManager::_lua_Create(LPCSTR d_shader, LPCSTR s_textures) {
  ...
  if (Script::bfIsObjectPresent(LSVM, s_shader, "normal_hq", LUA_TFUNCTION)) {
    ...
    C.bDetail = dxRenderDeviceRender::Instance()
