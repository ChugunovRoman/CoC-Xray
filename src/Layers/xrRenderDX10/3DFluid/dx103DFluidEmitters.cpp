...
dx103DFluidEmitters::dx103DFluidEmitters(int gridWidth, int gridHeight,
                                         int gridDepth, dx103DFluidGrid *pGrid)
    : m_pGrid(pGrid) {
...
void dx103DFluidEmitters::ApplyDensity(const CEmitter &Emitter) {
  ...
  case ET_SimpleDraught:
    // fRadius += (0.1f - fRadius) * (1.0f + 0.5f * _sin( (1.0f/30.0f) * t *
    // (2.0f * float(PI))) ); float fFactor = 1.0f + 0.5f * _sin(t * (2.0f *
    // float(PI)) / 30 ); FlowVelocity.mul( fFactor );
    break;
  }
...
void dx103DFluidEmitters::ApplyVelocity(const CEmitter &Emitter) {
  ...
  case ET_SimpleDraught:
    ...
    if (fPeriod < 0.0001f)
      fPeriod = 0.0001f;
    float fFactor = 1.0f + Emitter.m_DraughtParams.m_fAmp *
