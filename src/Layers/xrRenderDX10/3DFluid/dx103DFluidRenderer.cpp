...
void dx103DFluidRenderer::Initialize(int gridWidth, int gridHeight,
                                     int gridDepth) {
  Destroy();
  ...
  // Initialize the grid offset matrix
  {
    ...
    D3DXMatrixScaling(&scaleM, m_vGridDim[0] / m_fMaxDim,
                      m_vGridDim[1] / m_fMaxDim, m_vGridDim[2] / m_fMaxDim);
    ...
    // m_gridMatrix.scale(m_vGridDim[0] / m_fMaxDim, m_vGridDim[1] / m_fMaxDim,
    // m_vGridDim[2] / m_fMaxDim); m_gridMatrix.translate_over(-0.5, -0.5,
    // -0.5);
  }
...
void dx103DFluidRenderer::CreateGridBox() {
  ...
  CHK_DX(dx10BufferUtils::CreateVertexBuffer(&m_pGridBoxVertexBuffer, vertices,
                                             sizeof(vertices)));
  ...
  // D3Dxx_INPUT_PER_VERTEX_DATA, 0 },
  //};
  // UINT numElements = sizeof(layout)/sizeof(layout[0]);
  static D3DVERTEXELEMENT9 layout[] = {{0, 0, D3DDECLTYPE_FLOAT3,
  ...
  // V_RETURN( m_pD3DDevice->CreateInputLayout( layout, numElements,
  // PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &pGridBoxLayout
  // )
  // );
...
void dx103DFluidRenderer::CreateScreenQuad() {
  ...
  static D3DVERTEXELEMENT9 quadlayout[] = {{0, 0, D3DDECLTYPE_FLOAT3,
                                            D3DDECLMETHOD_DEFAULT,
                                            D3DDECLUSAGE_POSITION, 0},
                                           D3DDECL_END()};
  ...
  // V_RETURN( m_pD3DDevice->CreateBuffer( &vbdesc, &InitData,
  // &pQuadVertexBuffer ) );
...
void dx103DFluidRenderer::CreateJitterTexture() {
  ...
  // V( m_pD3DDevice->CreateShaderResourceView( NoiseTexture, &descSRV,
  // &JitterTextureSRV ) );
  // pEffect->GetVariableByName("jitterTex")->AsShaderResource() -> SetResource
  // (JitterTextureSRV);

  m_JitterTexture = dxRenderDeviceRender::Instance().Resources->_CreateTexture(
      "$user$NVjitterTex");
...
void dx103DFluidRenderer::CreateRayDataResources(int width, int height) {
  ...
  /*
          ...
          V_RETURN( m_pD3DDevice->CreateRenderTargetView(pRayDataTex2D, &DescRT,
     &pRayDataRTV)); V_RETURN(
     m_pD3DDevice->CreateRenderTargetView(pRayDataSmallTex2D, &DescRT,
     &pRayDataSmallRTV)); V_RETURN(
     m_pD3DDevice->CreateRenderTargetView(pRayCastTex2D, &DescRT,
     ...
     m_pD3DDevice->CreateRenderTargetView(pEdgeTex2D, &DescRT, &pEdgeRTV));
          ...
          V_RETURN(m_pD3DDevice->CreateShaderResourceView(pRayDataTex2D,
     &SRVDesc, &pRayDataSRV));
          V_RETURN(m_pD3DDevice->CreateShaderResourceView(pRayDataSmallTex2D,
     &SRVDesc, &pRayDataSmallSRV));
          V_RETURN(m_pD3DDevice->CreateShaderResourceView(pRayCastTex2D,
     &SRVDesc, &pRayCastSRV)); SRVDesc.Format = DXGI_FORMAT_R32_FLOAT;
          V_RETURN(m_pD3DDevice->CreateShaderResourceView(pEdgeTex2D, &SRVDesc,
     &pEdgeSRV));


          pRayDataVar =
     pEffect->GetVariableByName("rayDataTex")->AsShaderResource();
          pRayDataSmallVar =
     pEffect->GetVariableByName("rayDataTexSmall")->AsShaderResource();
          pRayCastVar =
     pEffect->GetVariableByName("rayCastTex")->AsShaderResource(); pEdgeVar    =
     pEffect->GetVariableByName("edgeTex")->AsShaderResource();
          */
  ...
  for (int i = 1; i < RRT_NumRT; ++i) {
    RT[i] = 0;
    RT[i].create(m_pRTNames[i], m_iRenderTextureWidth, m_iRenderTextureHeight,
                 RTFormats[i]);
  }
}
...
void dx103DFluidRenderer::Draw(const dx103DFluidData &FluidData) {
  ...
  //  in view space. This is used to convert the length of a ray from view space
  //  to grid space.
  // D3DXVECTOR3 worldXaxis = D3DXVECTOR3(worldView._11, worldView._12,
  // worldView._13);
  ...
  D3DXMatrixInverse((D3DXMATRIX *)&WorldViewInv, NULL,
...
void dx103DFluidRenderer::DrawScreenQuad() {
  ...
  // m_pD3DDevice->IASetVertexBuffers( 0, 1, &pQuadVertexBuffer, &strides,
  // &offsets ); m_pD3DDevice->IASetPrimitiveTopology(
  // D3Dxx_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP ); m_pD3DDevice->Draw( 4, 0 );
...
void dx103DFluidRenderer::DrawBox() {
  ...
  //	UINT offset = 0;
  //	m_pD3DDevice->IASetVertexBuffers( 0, 1, &pGridBoxVertexBuffer, &stride,
  //&offset ); 	m_pD3DDevice->IASetIndexBuffer( pGridBoxIndexBuffer,
  // DXGI_FORMAT_R32_UINT, 0 ); 	m_pD3DDevice->IASetPrimitiveTopology(
  // D3Dxx_PRIMITIVE_TOPOLOGY_TRIANGLELIST );
