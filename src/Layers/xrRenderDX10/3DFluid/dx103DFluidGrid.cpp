...
inline void ComputeRowColsForFlat3DTexture(int depth, int *outCols,
                                           int *outRows) {
  // Compute # of m_iRows and m_iCols for a "flat 3D-texture" configuration
  // (in this configuration all the slices in the volume are spread in a single
  // 2D texture)
...
void dx103DFluidGrid::CreateVertexBuffers() {
  ...
  static D3DVERTEXELEMENT9 layoutDesc[] = {
      {0, 0, D3DDECLTYPE_FLOAT3, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_POSITION,
       0},
      {0, 12, D3DDECLTYPE_FLOAT3, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_TEXCOORD,
       0},
      D3DDECL_END()};
  ...
  // 3D-texture as a flat 3D-texture (used to draw all the individual slices at
  // once to the screen buffer)
  index = 0;
...
void dx103DFluidGrid::InitScreenSlice(VS_INPUT_FLUIDSIM_STRUCT **vertices,
  ...
  tempVertex1.Pos =
      D3DXVECTOR3(px * 2.0f / Width - 1.0f, -(py * 2.0f / Height) + 1.0f, 0.0f);
  ...
  tempVertex3.Pos = D3DXVECTOR3((px + w) * 2.0f / Width - 1.0f,
                                -((py + h) * 2.0f / Height) + 1.0f, 0.0f);
  tempVertex3.Tex = D3DXVECTOR3(w, h, float(z));

  tempVertex4.Pos = D3DXVECTOR3((px)*2.0f / Width - 1.0f,
                                -((py + h) * 2.0f / Height) + 1.0f, 0.0f);
...
void dx103DFluidGrid::InitSlice(int z, VS_INPUT_FLUIDSIM_STRUCT **vertices,
                                int &index) {
  VS_INPUT_FLUIDSIM_STRUCT tempVertex1;
  VS_INPUT_FLUIDSIM_STRUCT tempVertex2;
  VS_INPUT_FLUIDSIM_STRUCT tempVertex3;
  ...
  tempVertex2.Pos =
      D3DXVECTOR3((w - 1.0f) * 2.0f / w - 1.0f, -1 * 2.0f / h + 1.0f, 0.0f);
  ...
  tempVertex4.Pos =
      D3DXVECTOR3(1 * 2.0f / w - 1.0f, -(h - 1.0f) * 2.0f / h + 1.0f, 0.0f);
