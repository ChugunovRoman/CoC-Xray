...
-1)); // 0
        matrix.transform_tiny			(aabb[1],Fvector().set( -1, +1,
-1)); // 1
        matrix.transform_tiny			(aabb[2],Fvector().set( +1, +1,
-1)); // 2
        matrix.transform_tiny			(aabb[3],Fvector().set( +1, -1,
-1)); // 3
        matrix.transform_tiny			(aabb[4],Fvector().set( -1, -1,
+1)); // 4
        matrix.transform_tiny			(aabb[5],Fvector().set( -1, +1,
+1)); // 5
        matrix.transform_tiny			(aabb[6],Fvector().set( +1, +1,
+1)); // 6
        matrix.transform_tiny			(aabb[7],Fvector().set( +1, -1,
+1)); // 7
...
dx103DFluidObstacles::dx103DFluidObstacles(int gridWidth, int gridHeight,
                                           int gridDepth,
...
void dx103DFluidObstacles::ProcessObstacles(const dx103DFluidData &FluidData,
  ...
  Fmatrix WorldToFluid;
  {
    ...
    // (float)m_iTextureDepth-1); Translate.translate(0.5, 0.5, 0.5); It seems
    // that y axis is inverted in fluid simulation, so shange maths a bit
...
void dx103DFluidObstacles::ProcessStaticObstacles(
    const dx103DFluidData &FluidData, const Fmatrix &WorldToFluid) {
...
void dx103DFluidObstacles::ProcessDynamicObstacles(
  ...
  for (u32 i = 0; i < m_lstRenderables.size(); ++i) {
    ...
    // if	(0==sector)										continue;	// disassociated from
    // S/P structure if	(PortalTraverser.i_marker != sector->r_marker) continue;
