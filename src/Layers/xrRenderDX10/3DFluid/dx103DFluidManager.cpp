...
LPCSTR dx103DFluidManager::m_pEngineTextureNames[NUM_RENDER_TARGETS] = {
    ...
    "$user$Texture_velocity0",    //	RENDER_TARGET_VELOCITY0 =
                                  // NUM_OWN_RENDER_TARGETS,	//	For
...
dx103DFluidManager::dx103DFluidManager()
    : m_bInited(false),
      ...
      m_fSaturation(0.78f), m_bAddDensity(true), m_fImpulseSize(0.15f),
      m_fConfinementScale(0.0f), m_fDecay(1.0f), m_pGrid(0), m_pRenderer(0),
...
void dx103DFluidManager::CreateRTTextureAndViews(int rtIndex,
  ...
  CHK_DX(HW.pDevice->CreateRenderTargetView(pRT, &DescRT,
                                            &pRenderTargetViews[rtIndex]));
...
void dx103DFluidManager::AdvectColorBFECC(float timestep, bool bTeperature) {
  ...
  // RENDER_TARGET_COLOR0 with RENDER_TARGET_TEMPVECTOR 	Find texture index
  // and patch texture manually using DirecX call!
