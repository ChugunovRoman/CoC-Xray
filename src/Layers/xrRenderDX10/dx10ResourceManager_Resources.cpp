...
SVS *CResourceManager::_CreateVS(LPCSTR _name) {
  ...
  if (I != m_vs.end())
    ...
    string_path cname;
    strconcat(sizeof(cname), cname, ::Render->getShaderPath(), /*_name*/ shName,
              ".vs");
    ...
    if (!file) {
      ...
      xr_sprintf(tmp, "DX10: %s is missing. Replace with stub_default.vs",
                 cname);
      Msg(tmp);
      strconcat(sizeof(cname), cname, ::Render->getShaderPath(), "stub_default",
                ".vs");
      FS.update_path(cname, "$game_shaders$", cname);
    ...
    CHECK_OR_EXIT(!FAILED(_hr),
                  make_string("Your video card doesn't meet game "
...
SGS *CResourceManager::_CreateGS(LPCSTR name) {
  ...
  if (I != m_gs.end())
    ...
    HRESULT const _hr = ::Render->shader_compile(
        name, (DWORD const *)file->pointer(), file->length(), c_entry, c_target,
        D3D10_SHADER_PACK_MATRIX_ROW_MAJOR, (void *&)_gs);
...
/*	//	DX10 cut
//--------------------------------------------------------------------------------------------------------------
CRTC*	CResourceManager::_CreateRTC		(LPCSTR Name, u32 size,
D3DFORMAT f)
{
        ...
        else
        {
                CRTC *RT				=	xr_new<CRTC>();
                RT->dwFlags				|=
xr_resource_flagged::RF_REGISTERED; m_rtargets_c.insert
(mk_pair(RT->set_name(Name),RT)); if (Device.b_is_Ready)	RT->create
(Name,size,f); return					RT;
        }
}
...
SGeometry *CResourceManager::CreateGeom(u32 FVF, ID3DVertexBuffer *vb,
                                        ID3DIndexBuffer *ib) {
