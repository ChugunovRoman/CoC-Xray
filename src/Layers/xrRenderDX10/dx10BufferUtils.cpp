...
HRESULT IC CreateBuffer(ID3DBuffer **ppBuffer, const void *pData, UINT DataSize,
                        bool bImmutable, bool bIndexBuffer) {
...
HRESULT CreateVertexBuffer(ID3DVertexBuffer **ppBuffer, const void *pData,
                           UINT DataSize, bool bImmutable) {
  return CreateBuffer(ppBuffer, pData, DataSize, bImmutable, false);
}

HRESULT CreateIndexBuffer(ID3DIndexBuffer **ppBuffer, const void *pData,
                          UINT DataSize, bool bImmutable) {
...
VertexFormatPairs VertexFormatList[] = {
    ...
    {D3DDECLTYPE_D3DCOLOR,
     DXGI_FORMAT_R8G8B8A8_UNORM}, // Warning. Explicit RGB component swizzling
                                  // is nesessary	//	Not available
    {D3DDECLTYPE_UBYTE4,
     DXGI_FORMAT_R8G8B8A8_UINT}, // Note: Shader gets UINT values, but if
                                 // Direct3D 9 style integral floats are needed
                                 // (0.0f, 1.0f... 255.f), UINT can just be
                                 // converted to float32 in shader.
    {D3DDECLTYPE_SHORT2,
     DXGI_FORMAT_R16G16_SINT}, // Note: Shader gets SINT values, but if Direct3D
                               // 9 style integral floats are needed, SINT can
