...
#endif
{
  ...
  if (SampleCount <= 1)
    ...
    desc.BindFlags =
        (bUseAsDepth ? D3D_BIND_DEPTH_STENCIL
  ...
  // OK
#ifdef DEBUG
  Msg("* created RT(%s), %dx%d, format = %d samples = %d", Name, w, h, dx10FMT,
      SampleCount);
#endif // DEBUG
...
f, D3DPOOL_DEFAULT, &pSurface,NULL); if (FAILED(_hr) || (0==pSurface))	return;
        ...
        for (u32 face=0; face<6; face++)
                R_CHK	(pSurface->GetCubeMapSurface
((D3DCUBEMAP_FACES)face, 0, pRT+face)); pTexture	= DEV->_CreateTexture
(Name); pTexture->surface_set (pSurface);
}
