...
BOOL R_constant_table::parseConstants(
    ID3DShaderReflectionConstantBuffer *pTable, u32 destination) {
  // VERIFY(_desc);
  ...
  for (u32 i = 0; i < TableDesc.Variables; ++i) {
    ...
    case D3D10_SVC_VECTOR: {
      ...
      case 2:
        ...
        break;
      default:
        fatal("Vector: 1 components is scalar - there is special case for "
              "this!!!!!");
        break;
      }
    } break;
    ...
    case D3D10_SVC_OBJECT: {
      ...
      created ref_constant	C		=	get	(name); if (!C)
      { C					=
      xr_new<R_constant>();//.g_constant_allocator.create(); C->name
      =	name; C->destination		=	RC_dest_sampler; C->type
      =	RC_sampler; R_constant_load& L	=	C->samp; L.index
      =	u16(r_index	+ ( (destination&1)? 0 : D3DVERTEXTEXTURESAMPLER0 ));
      L.cls				=	RC_sampler	;
      table.push_back		(C);
      } else {
      ...
      break;
      default:
      fatal		("Pclass D3DXPC_OBJECT - object isn't of 'sampler'
      type"); break;
      }
      */
    }
    ...
    if (!C) {
      ...
      R_constant_load &L =
          C->get_load(destination); /*((destination&RC_dest_pixel)
...
? RC_dest_vertex_cb_index_shift : RC_dest_geometry_cb_index_shift);*/
        ...
        uiBufferIndex |=
            dest_to_cbuf_type(destination); /*(destination&RC_dest_pixel)
    ...
    ? CB_BufferVertexShader : CB_BufferGeometryShader;*/
        ...
        ref_cbuffer tempBuffer =
            dxRenderDeviceRender::Instance().Resources->_CreateConstantBuffer(
                pTable);
