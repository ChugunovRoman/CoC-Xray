...
#ifdef USE_DX11
IC void CBackend::Compute(UINT ThreadGroupCountX, UINT ThreadGroupCountY,
...
IC void CBackend::Render(D3DPRIMITIVETYPE T, u32 baseV, u32 startV, u32 countV,
                         u32 startI, u32 PC) {
  // VERIFY(vs);
...
IC void CBackend::set_Stencil(u32 _enable, u32 _func, u32 _ref, u32 _mask,
                              u32 _writemask, u32 _fail, u32 _pass,
                              u32 _zfail) {
  StateManager.SetStencil(_enable, _func, _ref, _mask, _writemask, _fail, _pass,
                          _zfail);
  // Simple filter
  // if (stencil_enable		!= _enable)		{
  // stencil_enable=_enable; CHK_DX(HW.pDevice->SetRenderState	(
  // D3DRS_STENCILENABLE,		_enable
  // )); } if (!stencil_enable)					return; if
  // (stencil_func		!= _func)		{ stencil_func=_func;
  // CHK_DX(HW.pDevice->SetRenderState	( D3DRS_STENCILFUNC,		_func
  // )); } if (stencil_ref			!= _ref)		{
  // stencil_ref=_ref;				CHK_DX(HW.pDevice->SetRenderState
  // (
  // D3DRS_STENCILREF,			_ref				)); } if
  // (stencil_mask		!= _mask)		{ stencil_mask=_mask;
  // CHK_DX(HW.pDevice->SetRenderState	( D3DRS_STENCILMASK,		_mask
  // )); } if (stencil_writemask	!= _writemask)	{
  // stencil_writemask=_writemask; CHK_DX(HW.pDevice->SetRenderState	(
  ...
  // stencil_zfail=_zfail;			CHK_DX(HW.pDevice->SetRenderState
...
IC void CBackend::ApplyVertexLayout() {
  ...
  if (it == decl->vs_to_layout.end()) {
    ...
    it = decl->vs_to_layout
             .insert(std::pair<ID3DBlob *, ID3DInputLayout *>(m_pInputSignature,
                                                              pLayout))
             .first;
  }
...
IC bool CBackend::CBuffersNeedUpdate(ref_cbuffer buf1[MaxCBuffers],
                                     ref_cbuffer buf2[MaxCBuffers], u32 &uiMin,
                                     u32 &uiMax) {
  bool bRes = false;
  int i = 0;
...
IC void CBackend::set_Constants(R_constant_table *C) {
  ...
  //	Setup constant tables
  {
    ...
    if (CBuffersNeedUpdate(m_aGeometryConstants, aGeometryConstants, uiMin,
                           uiMax)) {
      ++uiMax;
...
IC void CBackend::get_ConstantDirect(shared_str &n, u32 DataSize, void **pVData,
                                     void **pGData, void **pPData) {
