...
void CHW::CreateD3D() {
  ...
  */
  ...
  install latest version of DirectX before running this program"); typedef
  IDirect3D9 * WINAPI _Direct3DCreate9(UINT SDKVersion); _Direct3DCreate9*
  createD3D	= (_Direct3DCreate9*)GetProcAddress(hD3D,"Direct3DCreate9");
  R_ASSERT(createD3D);
  this->pD3D 					= createD3D( D3D_SDK_VERSION );
  R_ASSERT2					(this->pD3D,"Please install
  DirectX 9.0c");
  */
}
...
void CHW::CreateDevice(HWND m_hWnd, bool move_window) {
  ...
  */
  ...
  m_DriverType =
      Caps.bForceGPU_REF ? D3D_DRIVER_TYPE_REFERENCE : D3D_DRIVER_TYPE_HARDWARE;
  ...
  */
  ...
  if ((D3DFMT_UNKNOWN==fTarget) || (D3DFMT_UNKNOWN==fTarget))	{
          Msg					("Failed to initialize graphics
  hardware.\nPlease try to restart the game."); FlushLog
  ();
          MessageBox			(NULL,"Failed to initialize graphics
  ...
  */
  ...
  R = D3D11CreateDeviceAndSwapChain(
      0, // m_pAdapter,//What wrong with adapter??? We should use another
         // version of DXGI?????
      m_DriverType, NULL, createDeviceFlags, pFeatureLevels,
      sizeof(pFeatureLevels) / sizeof(pFeatureLevels[0]), D3D11_SDK_VERSION,
      &sd, &m_pSwapChain, &pDevice, &FeatureLevel, &pContext);
#else
  ...
  */
  ...
  //(D3DSBT_ALL,&dwDebugSB)); #endif 	Create render target and depth-stencil
  // views here
  UpdateViews();
...
void CHW::Reset(HWND hwnd) {
  ...
  CHK_DX(m_pSwapChain->ResizeBuffers(cd.BufferCount, desc.Width, desc.Height,
                                     desc.Format,
  ...
  /*
          // Windoze
          DevPP.SwapEffect			=
  bWindowed?D3DSWAPEFFECT_COPY:D3DSWAPEFFECT_DISCARD;
  ...
  DevPP.FullScreen_RefreshRateInHz = D3DPRESENT_RATE_DEFAULT; #endif
          ...
          while	(TRUE)	{
                  HRESULT _hr = HW.pDevice->Reset	(&DevPP); if
  (SUCCEEDED(_hr)) break; Msg		("! ERROR: [%dx%d]:
  %s",DevPP.BackBufferWidth,DevPP.BackBufferHeight,Debug.error2string(_hr));
                  Sleep	(100);
          }
          R_CHK				(pDevice->GetRenderTarget
  (0,&pBaseRT)); R_CHK (pDevice->GetDepthStencilSurface	(&pBaseZB));
  */
...
void CHW::updateWindowProps(HWND m_hWnd) {
  //	BOOL	bWindowed				= strstr(Core.Params,"-dedicated")
  //? TRUE : !psDeviceFlags.is	(rsFullscreen);
  BOOL bWindowed = !psDeviceFlags.is(rsFullscreen);
  ...
  if (bWindowed) {
    if (m_move_window) {
      ...
      SetWindowPos(m_hWnd, HWND_NOTOPMOST, m_rcWindowBounds.left,
