...
void CTexture::surface_set(ID3DBaseTexture *surf) {
  ...
  if (pSurface) {
    ...
    if (D3D_RESOURCE_DIMENSION_TEXTURE2D == type) {
      ...
      if ((desc.SampleDesc.Count <= 1) ||
          (ViewDesc.Format != DXGI_FORMAT_R24_UNORM_X8_TYPELESS))
        CHK_DX(HW.pDevice->CreateShaderResourceView(pSurface, &ViewDesc,
                                                    &m_pSRView));
      else
        m_pSRView = 0;
    } else
...
void CTexture::Apply(u32 dwStage) {
  ...
  //   VERIFY( !((!pSurface)^(!m_pSRView)) );	//	Both present or both
  //   missing
  // else
  //{
...
void CTexture::apply_theora(u32 dwStage) {
  if (pTheora->Update(m_play_time != 0xFFFFFFFF ? m_play_time
...
void CTexture::video_Play(BOOL looped, u32 _time) {
  if (pTheora)
    pTheora->Play(looped, (_time != 0xFFFFFFFF) ? (m_play_time = _time)
                                                : Device.dwTimeContinual);
}
