...
void CRender::create() {
  ...
  */
  ...
  //.	o.sunstatic			= (strstr(Core.Params,"-sunstatic"))?
  //TRUE :FALSE	;
  ...
  o.ssao_hbao = !o.ssao_hdao && ps_r2_ls_flags_ext.test(R2FLAGEXT_SSAO_HBAO) &&
                (ps_r_ssao != 0);
  ...
  // (HW.pDevice->CreateQuery(&qdesc,&q_sync_point[1])); 	Prevent error on
  // first
  // get data q_sync_point[0]->End(); q_sync_point[1]->End(); R_CHK
...
// XXX nitrocaster: workaround to eliminate conflict between different GUIDs
// from DXSDK/Windows SDK 0a233719-3960-4578-9d7c-203b8b1d9cc1
...
static HRESULT create_shader(LPCSTR const pTarget, DWORD const *buffer,
  ...
  HRESULT const _hr = D3DReflect(buffer, buffer_size, guidShaderReflection,
                                 (void **)&pReflection);
...
static HRESULT create_shader(LPCSTR const pTarget, DWORD const *buffer,
...
#ifdef USE_DX11
    _result =
        HW.pDevice->CreatePixelShader(buffer, buffer_size, 0, &sps_result->ps);
#else  // #ifdef USE_DX11
    _result =
        HW.pDevice->CreatePixelShader(buffer, buffer_size, &sps_result->ps);
...
#ifdef USE_DX11
    _result = D3DReflect(buffer, buffer_size, guidShaderReflection,
                         (void **)&pReflection);
#else
...
#ifdef USE_DX11
    _result =
        HW.pDevice->CreateVertexShader(buffer, buffer_size, 0, &svs_result->vs);
#else  // #ifdef USE_DX11
    _result =
        HW.pDevice->CreateVertexShader(buffer, buffer_size, &svs_result->vs);
#endif // #ifdef USE_DX11
...
#else  // #ifdef USE_DX11
    _result =
        HW.pDevice->CreateGeometryShader(buffer, buffer_size, &sgs_result->gs);
#endif // #ifdef USE_DX11
    if (!SUCCEEDED(_result)) {
      Log("! GS: ", file_name);
  ...
  // #else // #ifdef USE_DX11 		_result			=
  //HW.pDevice->CreateComputeShader(buffer, buffer_size, &scs_result->sh);
  // #endif // #ifdef USE_DX11 		if (
  //! SUCCEEDED(_result) ) { 			Log			("! CS: ",
  //! file_name);
  ...
  if (disasm) {
    ...
    // D3DXDisassembleShader		(LPDWORD(code->GetBufferPointer()), FALSE,
    // 0, &disasm );
    string_path dname;
...
public:
  HRESULT __stdcall Open(D3D10_INCLUDE_TYPE IncludeType, LPCSTR pFileName,
...
HRESULT CRender::shader_compile(LPCSTR name, DWORD const *pSrcData,
  ...
  if (FS.exist(file_name)) {
    ...
    if (file->length() > 4) {
      ...
      processor.process_block(file->pointer(),
                              ((char *)file->pointer()) + file->elapsed());
  ...
  if (FAILED(_result)) {
    ...
    if (SUCCEEDED(_result)) {
      ...
      processor.process_block(pShaderBuf->GetBufferPointer(),
                              ((char *)pShaderBuf->GetBufferPointer()) +
...
static inline bool match_shader(LPCSTR const debug_shader_id,
                                LPCSTR const full_shader_id, LPCSTR const mask,
