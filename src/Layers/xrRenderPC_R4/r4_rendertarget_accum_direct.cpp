...
static Fvector3 corners[8] = {{-1, -1, 0.7}, {-1, -1, +1}, {-1, +1, +1},
...
void CRenderTarget::accum_direct(u32 sub_phase) {
  ...
  // if( 0 )
  {
    ...
    if (!RImplementation.o.dx10_msaa) {
      ...
      if (RImplementation.o.dx10_msaa_opt) {
        ...
        RCache.set_CullMode(CULL_NONE);
        RCache.set_Stencil(TRUE, D3DCMP_EQUAL, dwLightMarkerID | 0x80, 0x81,
                           0x7f, D3DSTENCILOP_KEEP, D3DSTENCILOP_REPLACE,
                           D3DSTENCILOP_KEEP);
        RCache.Render(D3DPT_TRIANGLELIST, Offset, 0, 4, 0, 2);
      } else {
  ...
  // Perform lighting
  {
    ...
    // 0.0f,
    //	0.0f,				0.0f,				fRange,
    // 0.0f, 	0.5f + fTexelOffs,	0.5f + fTexelOffs,
    // fBias,			1.0f
    //};
    ...
    // float	scale_Y				= float(Device.dwHeight)/
    // float(TEX_jitter);
    ...
    // Fill vertex buffer
    FVF::TL2uv *pv =
        (FVF::TL2uv *)RCache.Vertex.Lock(4, g_combine_2UV->vb_stride, Offset);
    // pv->set						(EPS,			float(_h+EPS),	d_Z,	d_W, C, p0.x, p1.y, j0.x,
    // j1.y); pv++;
    // pv->set						(EPS,			EPS,			d_Z,	d_W, C, p0.x, p0.y, j0.x,
    // j0.y); pv++;
    // pv->set						(float(_w+EPS),	float(_h+EPS),	d_Z,	d_W, C, p1.x,
    // p1.y, j1.x, j1.y);	pv++;
    // pv->set						(float(_w+EPS),	EPS,			d_Z,	d_W, C, p1.x, p0.y, j1.x,
    // j0.y); pv++;
    ...
    if (!RImplementation.o.dx10_msaa) {
      ...
      if (RImplementation.o.dx10_msaa_opt) {
        ...
        RCache.set_Stencil(TRUE, D3DCMP_EQUAL, dwLightMarkerID | 0x80, 0xff,
                           0x00);
        RCache.set_CullMode(CULL_NONE);
        RCache.Render(D3DPT_TRIANGLELIST, Offset, 0, 4, 0, 2);
      } else {
...
void CRenderTarget::accum_direct_cascade(u32 sub_phase, Fmatrix &xform,
                                         Fmatrix &xform_prev, float fBias) {
  // Choose normal code-path or filtered
  phase_accumulator();
  if (RImplementation.o.sunfilter) {
    accum_direct_f(sub_phase);
  ...
  // Perform lighting
  {
    ...
    u32 i_offset;
    {
      ...
      if (/*ps_r2_ls_flags_ext.is(R2FLAGEXT_SUN_ZCULLING) &&*/ sub_phase ==
          SE_SUN_FAR)
...
void CRenderTarget::accum_direct_blend() {
  ...
  if (!RImplementation.o.fp16_blend) {
    ...
    FVF::TL2uv *pv =
        (FVF::TL2uv *)RCache.Vertex.Lock(4, g_combine_2UV->vb_stride, Offset);
    // pv->set						(EPS,			float(_h+EPS),	d_Z,	d_W, C, p0.x, p1.y, p0.x,
    // p1.y); pv++;
    // pv->set						(EPS,			EPS,			d_Z,	d_W, C, p0.x, p0.y, p0.x,
    // p0.y); pv++;
    // pv->set						(float(_w+EPS),	float(_h+EPS),	d_Z,	d_W, C, p1.x,
    // p1.y, p1.x, p1.y);	pv++;
    // pv->set						(float(_w+EPS),	EPS,			d_Z,	d_W, C, p1.x, p0.y, p1.x,
    // p0.y); pv++;
  ...
  // Perform lighting
  {
    ...
    if (!RImplementation.o.dx10_msaa) {
      ...
      if (RImplementation.o.dx10_msaa_opt) {
        ...
        RCache.set_CullMode(CULL_NONE);
        RCache.set_Stencil(TRUE, D3DCMP_LESS, dwLightMarkerID | 0x80, 0xff,
                           0x00);
        RCache.Render(D3DPT_TRIANGLELIST, Offset, 0, 4, 0, 2);
      } else {
        for (u32 i = 0; i < RImplementation.o.dx10_msaa_samples; ++i) {
