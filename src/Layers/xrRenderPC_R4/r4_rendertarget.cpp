...
void CRenderTarget::u_setrt(const ref_rt &_1, const ref_rt &_2,
                            const ref_rt &_3, ID3DDepthStencilView *zb) {
  VERIFY(_1 || zb);
  if (_1) {
    dwWidth = _1->dwWidth;
...
void CRenderTarget::u_setrt(u32 W, u32 H, ID3DRenderTargetView *_1,
                            ID3DRenderTargetView *_2, ID3DRenderTargetView *_3,
                            ID3DDepthStencilView *zb) {
  // VERIFY (_1);
  dwWidth = W;
  dwHeight = H;
...
CRenderTarget::CRenderTarget() {
  ...
  if (RImplementation.o.HW_smap) {
    ...
    if (RImplementation.o.dx10_minmax_sm) {
      rt_smap_depth_minmax.create(r2_RT_smap_depth_minmax, size / 4, size / 4,
                                  D3DFMT_R32F);
    ...
    // (r2_RT_smap_surf, size,size,D3DFMT_R32F); rt_smap_depth
    // = NULL; R_CHK
  ...
  //D3DFMT_G16R16F,
  // SampleCount); 	s_ssao.create				(b_ssao,
  // "r2\\ssao");
  ...
  // COMBINE
  {
    static D3DVERTEXELEMENT9 dwDecl[] = {{0, 0, D3DDECLTYPE_FLOAT4,
    ...
    if (1) {
      ...
      for (u32 y = 0; y < TEX_jitter; y++) {
        for (u32 x = 0; x < TEX_jitter; x++) {
          ...
          for (u32 it = 0; it < TEX_jitter_count - 1; it++) {
            u32 *p = (u32 *)(LPBYTE(subData[it].pSysMem) +
      ...
      for (u32 y = 0; y < TEX_jitter; y++) {
        for (u32 x = 0; x < TEX_jitter; x++) {
          ...
          float *p =
              (float *)(LPBYTE(subData[it].pSysMem) +
                        y * subData[it].SysMemPitch + x * 4 * sizeof(float));
          *p = (float)(_cos(angle));
      ...
      //	Create noise mipped
      {
        ...
        HW.pContext->CopySubresourceRegion(t_noise_surf_mipped, 0, 0, 0, 0,
                                           t_noise_surf[0], 0, 0);
...
bool CRenderTarget::use_minmax_sm_this_frame() {
  ...
  case CRender::MMSM_AUTODETECT: {
    u32 dwScreenArea =
        HW.m_ChainDesc.BufferDesc.Width * HW.m_ChainDesc.BufferDesc.Height;
