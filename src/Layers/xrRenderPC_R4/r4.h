...
public:
  struct _options {
    ...
    u32 disasm : 1;
    u32 advancedpp : 1; //	advanced post process (DOF, SSAO, volumetrics,
                        // etc.)
    u32 volumetricfog : 1;

    u32 dx10_msaa : 1;           //	DX10.0 path
    u32 dx10_msaa_hybrid : 1;    //	DX10.0 main path with DX10.1 A-test msaa
                                 // allowed
...
public:
  ...
  ID3DBaseTexture *texture_load(LPCSTR fname, u32 &msize,
                                bool bStaging = false);
  virtual HRESULT shader_compile(LPCSTR name, DWORD const *pSrcData,
  ...
  virtual void
  add_Occluder(Fbox2 &bb_screenspace); // mask screen region as oclluded
  virtual void add_Visual(
      IRenderVisual *V); // add visual leaf	(no culling performed at all)
  virtual void
  add_Geometry(IRenderVisual *V); // add visual(s)	(all culling performed)
  ...
  virtual void add_SkeletonWallmark(const Fmatrix *xf, CKinematics *obj,
