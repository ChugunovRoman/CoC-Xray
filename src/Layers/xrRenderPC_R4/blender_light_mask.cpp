...
void CBlender_accum_direct_mask::Compile(CBlender_Compile &C) {
  ...
  case SE_MASK_DIRECT: // stencil mask for directional light
    //	FVF::F_TL
    // C.r_Pass			("null",			"accum_sun_mask",
    // false, FALSE,FALSE,TRUE,D3DBLEND_ZERO,D3DBLEND_ONE,TRUE,1); C.r_Pass
    // ("stub_notransform","accum_sun_mask",	false,
    // FALSE,FALSE,TRUE,D3DBLEND_ZERO,D3DBLEND_ONE,TRUE,1);
    C.r_Pass("stub_notransform_t", "accum_sun_mask_nomsaa", false, FALSE, FALSE,
             TRUE, D3DBLEND_ZERO, D3DBLEND_ONE, TRUE, 1);
  ...
  case SE_MASK_ACCUM_VOL: // copy accumulator (temp -> real), volumetric
    ...
    C.r_End();
    break;
  case SE_MASK_ACCUM_2D: // copy accumulator (temp -> real), 2D (usually after
                         // sun-blend)
    //	FVF::F_TL2uv but only uv0 is used
    // C.r_Pass			("null",			"copy",
    // false, FALSE,FALSE); C.r_Pass
    // ("stub_notransform","copy", false,	FALSE,FALSE);
...
void CBlender_accum_direct_mask_msaa::Compile(CBlender_Compile &C) {
  ...
  case SE_MASK_DIRECT: // stencil mask for directional light
    ...
    // ("stub_notransform","accum_sun_mask",	false,
    // FALSE,FALSE,TRUE,D3DBLEND_ZERO,D3DBLEND_ONE,TRUE,1);
    C.r_Pass("stub_notransform_t", "accum_sun_mask_msaa", false, FALSE, FALSE,
             TRUE, D3DBLEND_ZERO, D3DBLEND_ONE, TRUE, 1);
    // C.r_Sampler_rtf		("s_normal",		r2_RT_N);
    C.r_dx10Texture("s_normal", r2_RT_N);
    C.r_dx10Texture("s_position", r2_RT_P);
