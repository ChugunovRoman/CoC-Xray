...
void CRender::render_lights(light_Package &LP) {
  ...
  // 2. refactor - infact we could go from the backside and sort in ascending
  // order
  {
    xr_vector<light *> &source = LP.v_shadowed;
  ...
  while (LP.v_shadowed.size()) {
    ...
    while (true) {
      ...
      if (RImplementation.o.Tshadows)
        ...
        r_pmask(true, false);
      L->svis.begin();
      r_dsgraph_render_subspace(L->spatial.sector, L->X.S.combine, L->position,
                                TRUE);
      bool bNormal =
          mapNormalPasses[0][0].size() || mapMatrixPasses[0][0].size();
