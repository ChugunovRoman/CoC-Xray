...
void CRenderTarget::u_setrt(const ref_rt &_1, const ref_rt &_2,
                            const ref_rt &_3, IDirect3DSurface9 *zb) {
  VERIFY(_1);
  dwWidth = _1->dwWidth;
...
void CRenderTarget::u_setrt(u32 W, u32 H, IDirect3DSurface9 *_1,
                            IDirect3DSurface9 *_2, IDirect3DSurface9 *_3,
...
CRenderTarget::CRenderTarget() {
  ...
  //	NORMAL
  {
    ...
    // rt_Generic_2.create
    // (r2_RT_generic2,w,h,D3DFMT_A8R8G8B8
  ...
  if (RImplementation.o.HW_smap) {
    ...
    rt_smap_depth = NULL;
    R_CHK(HW.pDevice->CreateDepthStencilSurface(size, size, D3DFMT_D24X8,
  ...
  // TONEMAP
  {
    ...
    for (u32 it = 0; it < HW.Caps.iGPUNum * 2; it++) {
      ...
      CHK_DX(
          HW.pDevice->Clear(0L, NULL, D3DCLEAR_TARGET, 0x7f7f7f7f, 1.0f, 0L));
    }
  ...
  // Build textures
  {
    ...
    {
      // Surface
      R_CHK(D3DXCreateVolumeTexture(HW.pDevice, TEX_material_LdotN,
                                    TEX_material_LdotH, 4, 1, 0, D3DFMT_A8L8,
                                    D3DPOOL_MANAGED, &t_material_surf));
      t_material = dxRenderDeviceRender::Instance().Resources->_CreateTexture(
          r2_material);
      ...
      for (u32 slice = 0; slice < 4; slice++) {
        for (u32 y = 0; y < TEX_material_LdotH; y++) {
          for (u32 x = 0; x < TEX_material_LdotN; x++) {
            u16 *p = (u16 *)(LPBYTE(R.pBits) + slice * R.SlicePitch +
                             y * R.RowPitch + x * 2);
    ...
    if (1) {
      ...
      for (int it1 = 0; it1 < TEX_jitter_count - 1; it1++) {
        ...
        R_CHK(D3DXCreateTexture(HW.pDevice, TEX_jitter, TEX_jitter, 1, 0,
                                D3DFMT_Q8W8V8U8, D3DPOOL_MANAGED,
                                &t_noise_surf[it1]));
        t_noise[it1] =
            dxRenderDeviceRender::Instance().Resources->_CreateTexture(name);
      ...
      for (u32 y = 0; y < TEX_jitter; y++) {
        for (u32 x = 0; x < TEX_jitter; x++) {
          ...
          float *p = (float *)(LPBYTE(R[it].pBits) + y * R[it].Pitch +
                               x * 4 * sizeof(float));
          *p = (float)(_cos(angle));
  ...
  g_postprocess.create(D3DFVF_XYZRHW | D3DFVF_DIFFUSE | D3DFVF_SPECULAR |
                           D3DFVF_TEX3,
  ...
  HW.pDevice->CreateOffscreenPlainSurface(Device.dwWidth, Device.dwHeight,
                                          desc.Format, D3DPOOL_SYSTEMMEM, &pFB,
                                          NULL);
  ...
  if (bResetStencil) {
    ...
    /*
       ...
       float(Device.dwHeight);
                    u32		C					=
       color_rgba (255,255,255,255); float	eps
       = 0; float	_dw					= 0.5f; float
       _dh					= 0.5f; FVF::TL* pv
       ...
       (_w-_dw,	_h-_dh,		eps,	1.f, C, 0, 0);	pv++; pv->set
