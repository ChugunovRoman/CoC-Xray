...
void CRender::create() {
  ...
  */
  ...
  o.fp16_filter =
      HW.support(D3DFMT_A16B16G16R16F, D3DRTYPE_TEXTURE, D3DUSAGE_QUERY_FILTER);
  ...
  if ((HW.Caps.id_vendor == 0x10DE) && (HW.Caps.id_device >= 0x40)) {
    ...
    o.nvstencil = (S_OK == HW.pD3D->CheckDeviceFormat(
                               ...
                               D3DFMT_X8R8G8B8, 0, D3DRTYPE_TEXTURE,
...
static HRESULT create_shader(LPCSTR const pTarget, DWORD const *buffer,
  ...
  if (pTarget[0] == 'p') {
    ...
    LPCVOID data = NULL;
    _result = D3DXFindShaderComment(buffer, MAKEFOURCC('C', 'T', 'A', 'B'),
                                    &data, NULL);
...
HRESULT CRender::shader_compile(LPCSTR name, DWORD const *pSrcData,
  ...
  if (FAILED(_result)) {
    ...
    if (0 == xr_strcmp(pFunctionName, "main")) {
      if ('v' == pTarget[0])
        pTarget =
            D3DXGetVertexShaderProfile(HW.pDevice); // vertex	"vs_2_a"; //
      else
        pTarget =
            D3DXGetPixelShaderProfile(HW.pDevice); // pixel	"ps_2_a"; //
    }
    ...
    _result = D3DXCompileShader((LPCSTR)pSrcData, SrcDataLen, defines, pInclude,
    ...
    if (SUCCEEDED(_result)) {
      ...
      _result = create_shader(pTarget, (DWORD *)pShaderBuf->GetBufferPointer(),
                              pShaderBuf->GetBufferSize(), file_name, result,
                              o.disasm);
    } else {
      //			Msg
