...
public:
  ...
  ref_rt rt_Generic_0; // 32bit		(r,g,b,a) // post-process, intermidiate
                       // results, etc.
  ref_rt rt_Generic_1; // 32bit		(r,g,b,a) // post-process, intermidiate
                       // results, etc.
  //	Igor: for volumetric lights
  ref_rt rt_Generic_2; // 32bit		(r,g,b,a) // post-process, intermidiate
                       // results, etc.
  ...
  ref_rt rt_LUM_pool[CHWCaps::MAX_GPUS *
                     2];  // 1xfp32,1x1,		exp-result -> scaler
...
public:
  ...
  void u_setrt(const ref_rt &_1, const ref_rt &_2, const ref_rt &_3,
               IDirect3DSurface9 *zb);
  void u_setrt(u32 W, u32 H, IDirect3DSurface9 *_1, IDirect3DSurface9 *_2,
               IDirect3DSurface9 *_3, IDirect3DSurface9 *zb);
  void u_calc_tc_noise(Fvector2 &p0, Fvector2 &p1);
  void u_calc_tc_duality_ss(Fvector2 &r0, Fvector2 &r1, Fvector2 &l0,
                            Fvector2 &l1);
  BOOL u_need_PP();
  bool u_need_CM();
  ...
  void accum_direct_cascade(u32 sub_phase, Fmatrix &xform, Fmatrix &xform_prev,
                            float fBias);
  void accum_direct_f(u32 sub_phase);
  void accum_direct_lum();
  void accum_direct_blend();
  void accum_direct_volumetric(u32 sub_phase, const u32 Offset,
  ...
  IC void dbg_addbox(const Fvector &c, float rx, float ry, float rz,
                     u32 color) {
