...
void CKinematics::BuildBoneMatrix(const CBoneData *bd, CBoneInstance &bi,
                                  const Fmatrix *parent,
                                  u8 channel_mask /* = (1<<0)*/) {
  bi.mTransform.mul_43(*parent, bd->bind_transform);
}

void CKinematics::CLBone(const CBoneData *bd, CBoneInstance &bi,
    ...
    if (bi.callback_overwrite()) {
      ...
      if (bi.callback()) {
        ...
        R_ASSERT2(_valid(bi.mTransform),
                  make_string("callback kils bone matrix bone: %s ",
                              bd->name.c_str()));
#endif // #ifndef MASTER_GOLD
      }
    }
...
void CKinematics::Bone_GetAnimPos(Fmatrix &pos, u16 id, u8 mask_channel,
...
void CKinematics::BoneChain_Calculate(const CBoneData *bd, CBoneInstance &bi,
                                      u8 mask_channel, bool ignore_callbacks) {
  u16 SelfID = bd->GetSelfID();
