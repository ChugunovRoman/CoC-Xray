...
void R_dsgraph_structure::r_dsgraph_insert_dynamic(dxRender_Visual *pVisual,
                                                   Fvector &Center) {
...
void R_dsgraph_structure::r_dsgraph_insert_static(dxRender_Visual *pVisual) {
  ...
  for (u32 iPass = 0; iPass < sh->passes.size(); ++iPass) {
    ...
    //(pass.vs); 	mapNormalPS::TNode*			Nps =
    // Nvs->val.insert	(pass.ps); #else #if defined(USE_DX10) ||
    // defined(USE_DX11) 	mapNormalVS::TNode*			Nvs
    //= map.insert		(&*pass.vs); #else	//	USE_DX10
    //	mapNormalVS::TNode*			Nvs		= map.insert
    //(pass.vs->vs); #endif	//	USE_DX10 	mapNormalPS::TNode*
    // Nps		= Nvs->val.insert	(pass.ps->ps); #endif
...
void CRender::add_leafs_Dynamic(dxRender_Visual *pVisual) {
  ...
  xr_vector<dxRender_Visual *>::iterator I,
      E; // it may be useful for 'hierrarhy' visual
  ...
  case MT_PARTICLE_GROUP: {
    ...
    for (PS::CParticleGroup::SItemVecIt i_it = pG->items.begin();
      ...
      for (xr_vector<dxRender_Visual *>::iterator pit =
               I._children_related.begin();
           pit != I._children_related.end(); ++pit)
        add_leafs_Dynamic(*pit);
      for (xr_vector<dxRender_Visual *>::iterator pit =
               I._children_free.begin();
  ...
  case MT_SKELETON_RIGID: {
    ...
    if (pV->m_lod) {
      ...
      float ssa = CalcSSA(D, Tpos,
                          pV->vis.sphere.R /
                              2.f); // assume dynamics never consume full sphere
      if (ssa < r_ssaLOD_A)
        _use_lod = TRUE;
    }
    if (_use_lod) {
