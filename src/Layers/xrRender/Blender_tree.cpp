  ...
  if (C.bEditor) {
    ...
    case SE_R1_NORMAL_HQ:
      if (oNotAnTree.value) {
        ...
        if (oBlend.value)
          ...
          C.r_Pass(tsv, tsp, TRUE, TRUE, TRUE, TRUE, D3DBLEND_ONE,
        ...
        if (C.bDetail_Diffuse) {
          if (oBlend.value)
            C.r_Pass("tree_w_dt", "vert_dt", TRUE, TRUE, TRUE, TRUE,
    ...
    case SE_R1_NORMAL_LQ:
      ...
      if (oBlend.value)
        ...
        C.r_Pass("tree_s", "vert", TRUE, TRUE, TRUE, TRUE, D3DBLEND_ONE,
                 D3DBLEND_ZERO, TRUE, tree_aref);
      C.r_Sampler("s_base", C.L_textures[0]);
      C.r_End();
      break;
    case SE_R1_LPOINT:
      C.r_Pass((oNotAnTree.value) ? "tree_s_point" : "tree_w_point",
      ...
      break;
    case SE_R1_LSPOT:
      C.r_Pass((oNotAnTree.value) ? "tree_s_spot" : "tree_w_spot", "add_spot",
               FALSE, TRUE, FALSE, TRUE, D3DBLEND_ONE, D3DBLEND_ONE, TRUE, 0);
      C.r_Sampler("s_base", C.L_textures[0]);
...
void CBlender_Tree::Compile(CBlender_Compile &C) {
  ...
  case SE_R2_SHADOW: // smap-spot
    //	TODO: DX10: Use dumb shader for shadowmap since shadows are drawn using
    // hardware PCF
    if (oBlend.value)
      C.r_Pass(tvs_s, "shadow_direct_base_aref", FALSE, TRUE, TRUE, TRUE,
