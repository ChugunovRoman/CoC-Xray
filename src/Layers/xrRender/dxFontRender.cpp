...
void dxFontRender::OnRender(CGameFont &owner) {
  ...
  for (u32 i = 0; i < owner.strings.size();) {
    ...
    u32 vOffset;
    FVF::TL *v =
        (FVF::TL *)RCache.Vertex.Lock(length * 4, pGeom.stride(), vOffset);
    FVF::TL *start = v;
    ...
    for (; i < last; i++) {
      ...
      if (len) {
...
#if defined(USE_DX10) || defined(USE_DX11) //	Vertex shader will cancel a DX9
                                           // correction, so make fake offset
        X -= 0.5f;
        Y -= 0.5f;
        Y2 -= 0.5f;
#endif //	USE_DX10
        ...
        for (int j = 0; j < len; j++) {
          ...
          if (!fis_zero(l.z)) {
            //						tu			= ( l.x / owner.vTS.x ) + ( 0.5f /
            //owner.vTS.x
            //); 						tv			= ( l.y / owner.vTS.y ) + ( 0.5f
            /// owner.vTS.y );
            tu = (l.x / owner.vTS.x);
            tv = (l.y / owner.vTS.y);
#ifndef USE_DX10
