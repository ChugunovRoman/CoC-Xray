...
ICF bool pred_dot(const std::pair<float, u32> &_1,
...
void R_dsgraph_structure::r_dsgraph_render_lods(bool _setup_zb, bool _clear) {
  ...
  for (u32 i = 0; i < lstLODs.size(); i++) {
    ...
    FLOD::_hw *V = (FLOD::_hw *)RCache.Vertex.Lock(
        iBatchSize * uiVertexPerImposter, firstV->geom->vb_stride, vOffset);
