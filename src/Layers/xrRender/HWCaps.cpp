...
u32 GetNVGpuNum() {
  ...
  for (u32 i = 0; i < logicalGPUCount; i++) {
    status = NvAPI_GetPhysicalGPUsFromLogicalGPU(logicalGPUs[i], physicalGPUs,
                                                 &physicalGPUCount);
    if (status == NVAPI_OK)
...
void CHWCaps::Update() {
  ...
  if ((!(dwStencilCaps & D3DSTENCILCAPS_INCR) &&
    ...
    // enough stencil bits
    soInc = (dwStencilCaps & D3DSTENCILCAPS_INCRSAT) ? D3DSTENCILOP_INCRSAT
                                                     : D3DSTENCILOP_INCR;
    soDec = (dwStencilCaps & D3DSTENCILCAPS_DECRSAT) ? D3DSTENCILOP_DECRSAT
                                                     : D3DSTENCILOP_DECR;
