...
void dxDebugRender::Render() {
  ...
  RCache.dbg_Draw(D3DPT_LINELIST, &*m_line_vertices.begin(),
                  m_line_vertices.size(), &*m_line_indices.begin(),
                  m_line_indices.size() / 2);
  m_line_vertices.resize(0);
  m_line_indices.resize(0);
}

void dxDebugRender::try_render(u32 const &vertex_count,
...
void _add_lines(xr_vector<FVF::L> &vertices, xr_vector<u16> &indices,
                Fvector const *pvertices, u32 const &vertex_count,
...
void dxDebugRender::add_lines(Fvector const *vertices, u32 const &vertex_count,
                              u16 const *pairs, u32 const &pair_count,
                              u32 const &color) {
  try_render(vertex_count, pair_count);
  _add_lines(m_line_vertices, m_line_indices, vertices, vertex_count, pairs,
             pair_count, color);
}
...
void dxDebugRender::dbg_DrawTRI(Fmatrix &T, Fvector &p1, Fvector &p2,
                                Fvector &p3, u32 C) {
...
public:
  ...
  virtual void add_lines(Fvector const *vertices, u32 const &vertex_count,
                         u16 const *pairs, u32 const &pair_count,
                         u32 const &color) {
    _line_indices.resize(0);
    _line_vertices.resize(0);
    _add_lines(_line_vertices, _line_indices, vertices, vertex_count, pairs,
               pair_count, color);
  }
} rdebug_render_impl;
