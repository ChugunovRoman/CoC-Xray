...
void dxEnvironmentRender::OnFrame(CEnvironment &env) {
  ...
  CHK_DX(HW.pDevice->SetRenderState(
      D3DRS_FOGCOLOR, color_rgba_f(fog_color.x, fog_color.y, fog_color.z, 0)));
  CHK_DX(HW.pDevice->SetRenderState(D3DRS_FOGSTART,
                                    *(u32 *)(&env.CurrentEnv->fog_near)));
  CHK_DX(HW.pDevice->SetRenderState(D3DRS_FOGEND,
                                    *(u32 *)(&env.CurrentEnv->fog_far)));
#endif //	USE_DX10
}
...
void dxEnvironmentRender::RenderSky(CEnvironment &env) {
  ...
  // SetRenderState() function before and the RCache flag will remain unchanged
  // to it's old value.
  //
  RCache.set_Z(FALSE);
  RCache.set_Z(TRUE);
...
void dxEnvironmentRender::RenderClouds(CEnvironment &env) {
  ...
  // Fill vertex buffer
  v_clouds *pv = (v_clouds *)RCache.Vertex.Lock(env.CloudsVerts.size(),
                                                clouds_geom.stride(), v_offset);
