...
void CWallmarksEngine::clear() {
  {
    for (WMSlotVecIt p_it = marks.begin(); p_it != marks.end(); p_it++) {
      for (StaticWMVecIt m_it = (*p_it)->static_items.begin();
           m_it != (*p_it)->static_items.end(); m_it++)
...
//--------------------------------------------------------------------------------
void CWallmarksEngine::RecurseTri(u32 t, Fmatrix &mView,
...
void CWallmarksEngine::BuildMatrix(Fmatrix &mView, float invsz,
                                   const Fvector &from) {
  // build projection
  Fmatrix mScale;
...
void CWallmarksEngine::AddSkeletonWallmark(const Fmatrix *xf, CKinematics *obj,
...
ICF void BeginStream(ref_geom hGeom, u32 &w_offset, FVF::LIT *&w_verts,
                     FVF::LIT *&w_start) {
  w_offset = 0;
  w_verts =
      (FVF::LIT *)RCache.Vertex.Lock(MAX_TRIS * 3, hGeom->vb_stride, w_offset);
  w_start = w_verts;
}

ICF void FlushStream(ref_geom hGeom, ref_shader shader, u32 &w_offset,
                     FVF::LIT *&w_verts, FVF::LIT *&w_start,
                     BOOL bSuppressCull) {
...
void CWallmarksEngine::Render() {
  ...
  for (WMSlotVecIt slot_it = marks.begin(); slot_it != marks.end(); slot_it++) {
    ...
    // static wallmarks
    for (StaticWMVecIt w_it = slot->static_items.begin();
         w_it != slot->static_items.end();) {
      ...
      if (RImplementation.ViewBase.testSphere_dirty(W->bounds.P, W->bounds.R)) {
        ...
        W->ttl -=
            0.1f * Device.fTimeDelta; // visible wallmarks fade much slower
      } else {
