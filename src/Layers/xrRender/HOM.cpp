...
void CHOM::Load() {
  ...
  tbb::parallel_for(
      tbb::blocked_range<u32>(0, CL.getTS()),
...
ICF BOOL xform_b0(Fvector2 &min, Fvector2 &max, float &minz, Fmatrix &X,
...
ICF BOOL xform_b1(Fvector2 &min, Fvector2 &max, float &minz, Fmatrix &X,
                  float _x, float _y, float _z) {
  float t;
  float z = _x * X._13 + _y * X._23 + _z * X._33 + X._43;
  if (z < EPS)
...
BOOL CHOM::visible(vis_data &vis) {
  ...
  // 2. New object slides into view								- delay
  // test| frame-old, tested-old, hom_res = ???;
