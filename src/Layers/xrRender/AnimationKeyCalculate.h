...
IC void MixInterlerp(CKey &Result, const CKey *R,
  ...
  case 2: {
    ...
    if (fis_zero(ws))
      ...
      //(!_valid(w))){
      //.						Debug.fatal
      //(DEBUG_INFO,"TO ALEXMX VERY IMPORTANT: (TOTAL: %f) w: %f, w0: %f, w1:
  ...
  default: {
    ...
    for (int cnt = 1; cnt < b_count; cnt++) {
      ...
      //.							Debug.fatal
      //(DEBUG_INFO,"TO ALEXMX VERY IMPORTANT: (TOTAL: %f) w: %f, total: %f,
      // count: %d, real count:
...
IC void q_scale_vs_basem(Fmatrix &m_res, const Fquaternion &q,
                         const Fquaternion &base, float scale_factor) {
  Fmatrix mb, imb;
  mb.rotation(base);
  imb.invert(mb);
...
IC void q_add_scaled_basem(Fquaternion &q, const Fquaternion &base,
                           const Fquaternion &q0, const Fquaternion &q1,
                           float v1) {
  // VERIFY(0.f =< v && 1.f >= v );
  Fmatrix m0;
  m0.rotation(q0);
...
IC void process_single_channel(CKey &Result, const animation::channel_def &ch,
                               const CKey *R,
...
IC void MixChannels(CKey &Result, const CKey *R,
                    const animation::channel_def *BA, int b_count) {
  VERIFY(b_count > 0);
  Result = R[0];
