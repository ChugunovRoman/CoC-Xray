...
xr_token qsun_shafts_token[] = {{"st_opt_off", 0},
                                {"st_opt_low", 1},
                                {"st_opt_medium", 2},
                                {"st_opt_high", 3},
                                {0, 0}};
...
xr_token sunshafts_mode_token[] = {
    {"volumetric", 0}, {"screen_space", 1}, {0, 0}};
  ...
  virtual void Execute(LPCSTR args) {
    if (0 == xr_strcmp(args, "on")) {
      ...
      if (ps_r2_ls_flags.test(R2FLAG_GLOBALMATERIAL)) {
        ...
        Msg("* material set to [%s]-[%s], with lerp of [%f]", name[m0],
            name[m1], frc);
      }
    }
  }
...
public:
  ...
  virtual void Execute(LPCSTR args) {
    ...
    dxRenderDeviceRender::Instance().ResourcesGetMemoryUsage(m_base, c_base,
                                                             m_lmaps, c_lmaps);
    ...
    float vb_video =
        (float)
            HW.stats_manager.memory_usage_summary[enum_stats_buffer_type_vertex]
                                                 [D3DPOOL_DEFAULT] /
        1024 / 1024;
    float vb_managed =
        (float)
    ...
    float ib_video =
        (float)
            HW.stats_manager.memory_usage_summary[enum_stats_buffer_type_index]
                                                 [D3DPOOL_DEFAULT] /
        1024 / 1024;
    float ib_managed =
        (float)
    ...
    float rt_video = (float)HW.stats_manager
                         .memory_usage_summary[enum_stats_buffer_type_rtarget]
                                              [D3DPOOL_DEFAULT] /
                     1024 / 1024;
    float rt_managed = (float)HW.stats_manager
                           .memory_usage_summary[enum_stats_buffer_type_rtarget]
                                                [D3DPOOL_MANAGED] /
                       1024 / 1024;
    float rt_system = (float)HW.stats_manager
                          .memory_usage_summary[enum_stats_buffer_type_rtarget]
                                               [D3DPOOL_SYSTEMMEM] /
                      1024 / 1024;
...
public:
  ...
  virtual void Info(TInfo &I) {
    xr_sprintf(I, "vector3 in range [%f,%f,%f]-[%f,%f,%f]", min.x, min.y, min.z,
               max.x, max.y, max.z);
  }
};
...
void xrRender_initconsole() {
  ...
  CMD4(CCC_Float, "r__detail_density",
       &ps_current_detail_density /*&ps_r__Detail_density*/, 0.15f, 1.0f);
  ...
  CMD4(CCC_Float, "r2_sun_depth_far_bias", &ps_r2_sun_depth_far_bias, -0.5,
       +0.5);
  CMD4(CCC_Float, "r2_sun_depth_near_scale", &ps_r2_sun_depth_near_scale, 0.5,
       1.5);
  ...
  // "r3_msaa_opt", &ps_r2_ls_flags,			R3FLAG_MSAA_OPT);
  ...
  CMD4(CCC_Integer, "r__clear_models_on_unload", &ps_clear_models_on_unload, 0,
       1); // Alundaio
  CMD4(CCC_Integer, "r__no_scale_on_fade", &ps_no_scale_on_fade, 0,
       1); // Alundaio
#endif
  ...
  CMD3(CCC_Mask, "r3_dynamic_wet_surfaces", &ps_r2_ls_flags,
       R3FLAG_DYN_WET_SURF);
  CMD4(CCC_Float, "r3_dynamic_wet_surfaces_near", &ps_r3_dyn_wet_surf_near, 10,
       70);
