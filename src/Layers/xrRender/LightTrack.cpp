...
inline void CROS_impl::accum_hemi(float *hemi_cube, Fvector3 &dir,
                                  float scale) {
  if (dir.x > 0)
...
void CROS_impl::update(IRenderable *O) {
  ...
  if (MODE & IRender_ObjectSpecific::TRACE_LIGHTS) {
    ...
    for (u32 lit = 0; lit < lights.size(); lit++) {
      ...
      // multiply intensity on attenuation and accumulate result in hemi cube
      // face
      float koef =
          (lights[lit].color.r + lights[lit].color.g + lights[lit].color.b) /
...
void CROS_impl::calc_sun_value(Fvector &position, CObject *_object) {
  ...
  if (MODE & IRender_ObjectSpecific::TRACE_SUN) {
    if (--result_sun < 0) {
      ...
      sun_value = !(g_pGameLevel->ObjectSpace.RayTest(position, direction,
                                                      500.f, collide::rqtBoth,
                                                      &cache_sun, _object))
                      ? 1.f
                      : 0.f;
    }
...
void CROS_impl::calc_sky_hemi_value(Fvector &position, CObject *_object) {
  ...
  if (MODE & IRender_ObjectSpecific::TRACE_HEMI) {
    ...
    for (u32 it = 0; it < (u32)ps_r2_dhemi_count;
         it++) { // five samples per one frame
      u32 sample = 0;
      if (result_count < lt_hemisamples) {
        sample = result_count;
        result_count++;
      ...
      // take sample
      Fvector direction;
      direction.set(hdir[sample][0], hdir[sample][1], hdir[sample][2])
          .normalize();
      //.			result[sample]	=
  ...
  for (int it = 0; it < result_count; it++) {
    if (result[it]) {
      accum_hemi(hemi_cube,
                 Fvector3().set(hdir[it][0], hdir[it][1], hdir[it][2]),
...
void CROS_impl::prepare_lights(Fvector &position, IRenderable *O) {
  ...
  if (bTraceLights) {
...
#if RENDER != R_R1
    g_SpatialSpace->q_box(RImplementation.lstSpatial, 0, STYPE_LIGHTSOURCEHEMI,
                          position, bb_size);
#else
    g_SpatialSpace->q_box(RImplementation.lstSpatial, 0, STYPE_LIGHTSOURCE,
                          position, bb_size);
#endif
    ...
    for (s32 id = 0; id < s32(track.size()); id++) {
      ...
      if (g_pGameLevel->ObjectSpace.RayTest(LP, D.div(f), f, collide::rqtStatic,
                                            &I->cache, _object))
        amount -= lt_dec;
      else
        amount += lt_inc;
