...
#else  //	USE_DX10
  CHK_DX(HW.pDevice->Clear(0, 0, D3DCLEAR_TARGET, D3DCOLOR_ARGB(0, 0, 0, 0), 1,
                           0));
#endif //	USE_DX10
...
// D3D_CLEAR_DEPTH|D3D_CLEAR_STENCIL, 1.0f, 0);
  ...
  for (u32 idx = 0; idx < v_cnt + 1; ++idx) {
    clr =
        calc_progress_color(idx, v_cnt, owner.load_stage, owner.max_load_stage);
    pv->set(back_coords.lt.x + pos_delta * idx + offs, back_coords.rb.y + offs,
            0 + EPS_S, 1, clr, back_tex_coords.lt.x + tc_delta * idx,
            back_tex_coords.rb.y);
    pv++;
    pv->set(back_coords.lt.x + pos_delta * idx + offs, back_coords.lt.y + offs,
            0 + EPS_S, 1, clr, back_tex_coords.lt.x + tc_delta * idx,
...
void dxApplicationRender::draw_face(ref_shader &sh, Frect &coords,
                                    Frect &tex_coords, const Fvector2 &tsz) {
  u32 Offset;
  u32 C = 0xffffffff;
  FVF::TL *pv = NULL;
...
void draw_multiline_text(CGameFont *F, float fTargetWidth, LPCSTR pszText) {
  ...
  while (*ch) {
    ...
    if (curr_len + curr_word_len > fTargetWidth) {
      ...
      strncpy_s(buff + xr_strlen(buff), sizeof(buff) - xr_strlen(buff), ch,
                next_word - ch);
      ch = next_word;
    }
