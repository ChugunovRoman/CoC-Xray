...
void STextureParams::Load(IReader &F) {
  ...
  if (F.find_chunk(THM_CHUNK_BUMP)) {
    ...
    if (bump_mode < STextureParams::tbmNone) {
      bump_mode =
          STextureParams::tbmNone; //.. временно (до полного убирания Autogen)
    }
...
void STextureParams::FillProp(LPCSTR base_name, PropItemVec &items,
  ...
  case ttCubeMap:
    ...
    PHelper().CreateFlag32(items, "MipMaps\\Enabled", &flags,
                           flGenerateMipMaps);
    PHelper().CreateToken32(items, "MipMaps\\Filter", (u32 *)&mip_filter,
                            tparam_token);

    P = PHelper().CreateToken32(items, "Bump\\Mode", (u32 *)&bump_mode,
                                tbmode_token);
    P->OnChangeEvent.bind(this, &STextureParams::OnTypeChange);
    ...
    PHelper().CreateFloat(items, "Details\\Scale", &detail_scale, 0.1f, 10000.f,
                          0.1f, 2);
    ...
    PHelper().CreateU32(items, "Fade\\% of color to fade in", &fade_amount, 0,
                        100, 0);
  ...
  case ttBumpMap:
    PHelper().CreateChoose(items, "Bump\\Special NormalMap",
                           &ext_normal_map_name, smTexture, base_name);
  ...
  case ttTerrain:
    ...
    P = PHelper().CreateFlag32(items, "Flags\\Implicit Lighted", &flags,
                               flImplicitLighted);
    P->Owner()->Enable(false);
    break;
  }
...
BOOL STextureParams::similar(STextureParams &tp1,
  ...
  for (; it != it_e; ++it) {
    ...
    if (par_name == "Type") {
      ...
      res =
          (flags.test(flGenerateMipMaps) == tp1.flags.test(flGenerateMipMaps));
      ...
      res =
          (flags.test(flImplicitLighted) == tp1.flags.test(flImplicitLighted));
