...
void dxThunderboltRender::Render(CEffect_Thunderbolt &owner) {
  ...
  u32 VS_Offset;
  FVF::LIT *pv =
      (FVF::LIT *)RCache.Vertex.Lock(8, hGeom_gradient.stride(), VS_Offset);
  // top
  {
  ...
  // center
  {
    u32 c_val = iFloor(owner.current->m_GradientTop->fOpacity *
                       owner.lightning_phase * 255.f);
    u32 c = color_rgba(c_val, c_val, c_val, c_val);
    vecSx.mul(Device.vCameraRight, owner.current->m_GradientCenter->fRadius.x *
                                       owner.lightning_size);
    vecSy.mul(Device.vCameraTop, -owner.current->m_GradientCenter->fRadius.y *
                                     owner.lightning_size);
  ...
  RCache.set_Shader(
      ((dxFlareRender *)&*owner.current->m_GradientTop->m_pFlare)->hShader);
  ...
  RCache.set_Shader(
      ((dxFlareRender *)&*owner.current->m_GradientCenter->m_pFlare)->hShader);
