...
void CDetailManager::hw_Load_Geom() {
  ...
  R_CHK(HW.pDevice->CreateIndexBuffer(dwIndices * 2, dwUsage, D3DFMT_INDEX16,
                                      D3DPOOL_MANAGED, &hw_IB, 0));
...
#if defined(USE_DX10) || defined(USE_DX11)
    R_CHK(
        dx10BufferUtils::CreateIndexBuffer(&hw_IB, pIOriginal, dwIndices * 2));
    HW.stats_manager.increment_stats_ib(hw_IB);
    xr_free(pIOriginal);
#else  //	USE_DX10
    R_CHK(hw_IB->Unlock());
...
void CDetailManager::hw_Render() {
  ...
  // float		tm_rot1		=
  // (PI_MUL_2*RDEVICE.fTimeGlobal/swing_current.rot1);
  // float		tm_rot2		=
...
void CDetailManager::hw_Render_dump(ref_constant x_array, u32 var_id,
                                    u32 lod_id, u32 c_offset) {
