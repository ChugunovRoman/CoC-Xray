...
struct vertHW_4W {
  ...
  u32 _i;
  void set(Fvector3 &P, Fvector3 N, Fvector3 T, Fvector3 B, Fvector2 &tc,
           int index0, int index1, int index2, int index3, float w0, float w1,
           float w2) {
    N.normalize_safe();
    T.normalize_safe();
    B.normalize_safe();
...
void CSkeletonX_ext::_Load_hw(Fvisual &V, void *_verts_) {
  ...
  //|*/ (bSoft?D3DUSAGE_SOFTWAREPROCESSING:0);	// VB may be read by wallmarks
  // code
  switch (RenderMode) {
  ...
  case RM_SKINNING_1B: {
    ...
    // R_CHK
    // (V.p_rm_Vertices->Lock(0,0,(void**)&bytes,0));
    ...
    // R_CHK
    // (HW.pDevice->CreateVertexBuffer(V.vCount*vStride,dwUsage,0,D3DPOOL_MANAGED,&V.p_rm_Vertices,0));
    R_CHK(dx10BufferUtils::CreateVertexBuffer(&V.p_rm_Vertices, dstOriginal,
                                              V.vCount * vStride));
  ...
  case RM_SKINNING_3B: {
    ...
    for (u32 it = 0; it < V.vCount; it++) {
      ...
      dst->set(src->P, src->N, src->T, src->B, uv, int(src->m[0]) * 3,
               int(src->m[1]) * 3, int(src->m[2]) * 3, src->w[0], src->w[1]);
      dst++;
      src++;
    }
    R_CHK(dx10BufferUtils::CreateVertexBuffer(&V.p_rm_Vertices, dstOriginal,
  ...
  case RM_SKINNING_4B: {
    ...
    for (u32 it = 0; it < V.vCount; it++) {
      ...
      uv.set(src->u, src->v);
      dst->set(src->P, src->N, src->T, src->B, uv, int(src->m[0]) * 3,
               int(src->m[1]) * 3, int(src->m[2]) * 3, int(src->m[3]) * 3,
...
void CSkeletonX_ext::_Load_hw(Fvisual &V, void *_verts_) {
  ...
  case RM_SKINNING_1B: {
    ...
    BYTE *bytes = 0;
    VERIFY(NULL == V.p_rm_Vertices);
    R_CHK(HW.pDevice->CreateVertexBuffer(V.vCount * vStride, dwUsage, 0,
                                         D3DPOOL_MANAGED, &V.p_rm_Vertices, 0));
    HW.stats_manager.increment_stats_vb(V.p_rm_Vertices);
...
static void verify_vertex(const vertex_type &v, const Fvisual *V,
                          const CKinematics *Parent, u32 iBase, u32 iCount,
  ...
  for (u8 i = 0; i < vertex_type::bones_count; ++i)
    if (v.get_bone_id(i) >= Parent->LL_BoneCount()) {
      ...
      Msg(" iBase: %d, iCount: %d, V->iBase %d, V->iCount %d, V->vBase: %d,  "
          "V->vCount  %d, vertex_idx: %d, idx: %d",
          iBase, iCount, V->iBase, V->iCount, V->vBase, V->vCount, vertex_idx,
          idx);
      Msg(" v.P: %s , v.N: %s, v.T: %s, v.B: %s", get_string(v.P).c_str(),
          get_string(v.N).c_str(), get_string(v.T).c_str(),
          get_string(v.B).c_str());
...
void CSkeletonX_ext::_CollectBoneFaces(Fvisual *V, u32 iBase, u32 iCount) {
...
#if !defined(USE_DX10) && !defined(USE_DX11) //	Don't use hardware buffers in
                                             // DX10 since we can't read them
  switch (RenderMode) {
  case RM_SKINNING_SOFT:
#endif //	USE_DX10
  {
  ...
  case RM_SKINNING_1B: {
    ...
    R_CHK(V->p_rm_Vertices->Lock(V->vBase, V->vCount, (void **)&vertices,
                                 D3DLOCK_READONLY));
    for (u32 idx = 0; idx < iCount; idx++) {
...
BOOL CSkeletonX_ext::_PickBoneHW1W(IKinematics::pick_result &r, float dist,
                                   const Fvector &S, const Fvector &D,
                                   Fvisual *V, u16 *indices,
                                   CBoneData::FacesVec &faces) {
  return pick_bone<vertHW_1W>(Parent, r, dist, S, D, V, indices, faces);
}
BOOL CSkeletonX_ext::_PickBoneHW2W(IKinematics::pick_result &r, float dist,
                                   const Fvector &S, const Fvector &D,
                                   Fvisual *V, u16 *indices,
                                   CBoneData::FacesVec &faces) {
...
BOOL CSkeletonX_ext::_PickBoneHW3W(IKinematics::pick_result &r, float dist,
                                   const Fvector &S, const Fvector &D,
                                   Fvisual *V, u16 *indices,
                                   CBoneData::FacesVec &faces) {
  return pick_bone<vertHW_3W>(Parent, r, dist, S, D, V, indices, faces);
}
BOOL CSkeletonX_ext::_PickBoneHW4W(IKinematics::pick_result &r, float dist,
                                   const Fvector &S, const Fvector &D,
                                   Fvisual *V, u16 *indices,
                                   CBoneData::FacesVec &faces) {
...
BOOL CSkeletonX_ext::_PickBone(IKinematics::pick_result &r, float dist,
...
BOOL CSkeletonX_ST::PickBone(IKinematics::pick_result &r, float dist,
                             const Fvector &start, const Fvector &dir,
                             u16 bone_id) {
  return inherited2::_PickBone(r, dist, start, dir, this, bone_id, iBase,
                               iCount);
}
BOOL CSkeletonX_PM::PickBone(IKinematics::pick_result &r, float dist,
                             const Fvector &start, const Fvector &dir,
                             u16 bone_id) {
...
void CSkeletonX_ext::_FillVerticesHW1W(const Fmatrix &view,
                                       ...
                                       const Fvector &normal, float size,
                                       Fvisual *V, u16 *indices,
                                       CBoneData::FacesVec &faces) {
  ...
  void CSkeletonX_ext::_FillVerticesHW1W(
    ...
    for (CBoneData::FacesVecIt it = faces.begin(); it != faces.end(); it++) {
      ...
      for (u32 k = 0; k < 3; k++) {
        ...
        const Fmatrix &xform =
            Parent->LL_GetBoneInstance(F.bone_id[k][0]).mRenderTransform;
    ...
    for (CBoneData::FacesVecIt it = faces.begin(); it != faces.end(); ++it) {
      ...
      for (u32 k = 0; k < 3; k++) {
        ...
        Fmatrix &xform0 =
            Parent->LL_GetBoneInstance(F.bone_id[k][0]).mRenderTransform;
        Fmatrix &xform1 =
            Parent->LL_GetBoneInstance(F.bone_id[k][1]).mRenderTransform;
        vert.get_pos(F.vert[k]);
...
void CSkeletonX_ext::_FillVertices(const Fmatrix &view, CSkeletonWallmark &wm,
                                   const Fvector &normal, float size,
                                   Fvisual *V, u16 bone_id, u32 iBase,
                                   u32 iCount) {
  VERIFY(Parent && (ChildIDX != u16(-1)));
  CBoneData &BD = Parent->LL_GetData(bone_id);
  CBoneData::FacesVec *faces = &BD.child_faces[ChildIDX];
...
void CSkeletonX_ST::FillVertices(const Fmatrix &view, CSkeletonWallmark &wm,
                                 const Fvector &normal, float size,
                                 u16 bone_id) {
  inherited2::_FillVertices(view, wm, normal, size, this, bone_id, iBase,
                            iCount);
}
void CSkeletonX_PM::FillVertices(const Fmatrix &view, CSkeletonWallmark &wm,
                                 const Fvector &normal, float size,
                                 u16 bone_id) {
  FSlideWindow &SW = nSWI.sw[0];
...
IC void TEnumBoneVertices(vertex_buffer_type vertices, u16 *indices,
                          CBoneData::FacesVec &faces,
...
void CSkeletonX_ext::_EnumBoneVertices(SEnumVerticesCallback &C, Fvisual *V,
                                       u16 bone_id, u32 iBase,
                                       u32 iCount) const {
