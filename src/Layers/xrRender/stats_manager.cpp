...
void stats_manager::increment_stats(u32 size, enum_stats_buffer_type type,
                                    _D3DPOOL location) {
...
void stats_manager::increment_stats(u32 size, enum_stats_buffer_type type,
                                    _D3DPOOL location, void *buff_ptr) {
  if (g_dedicated_server)
    return;
...
void stats_manager::increment_stats_ib(ID3DIndexBuffer *buff) {
  ...
  D3D_BUFFER_DESC desc;
  buff->GetDesc(&desc);
  increment_stats(desc.ByteWidth, enum_stats_buffer_type_index, D3DPOOL_MANAGED,
                  buff);
#else
...
void stats_manager::decrement_stats_ib(ID3DIndexBuffer *buff) {
  ...
  D3D_BUFFER_DESC desc;
  buff->GetDesc(&desc);
  decrement_stats(desc.ByteWidth, enum_stats_buffer_type_index, D3DPOOL_MANAGED,
                  buff);
#else
  D3DINDEXBUFFER_DESC desc;
  buff->GetDesc(&desc);
...
void stats_manager::decrement_stats(u32 size, enum_stats_buffer_type type,
                                    _D3DPOOL location) {
  if (g_dedicated_server)
    return;
...
void stats_manager::decrement_stats(u32 size, enum_stats_buffer_type type,
  ...
  for (; it != en; ++it) {
    if (it->buff_ptr == buff_ptr) {
      ...
      // normally.
      // R_ASSERT ( it->type == type && it->location == location && it->size ==
      // size );
  ...
  R_ASSERT(find); //  "Specified buffer not fount in the buffers list.
                  //	The buffer may not incremented to stats or it already
                  // was removed"
#endif            // DEBUG
