...
r_aabb_ssa r_pixel_calculator::calculate(dxRender_Visual *V) {
  ...
  for (u32 face = 0; face < 6; face++) {
    ...
    mView.build_camera_dir(vFrom.invert(cmDir[face]).mul(100.f), cmDir[face],
                           cmNorm[face]);
    aabb.xform(V->vis.box, mView);
    D3DXMatrixOrthoOffCenterLH((D3DXMATRIX *)&mProject, aabb.min.x, aabb.max.x,
                               aabb.min.y, aabb.max.y, aabb.min.z, aabb.max.z);
