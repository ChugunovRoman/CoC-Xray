...
CHW::CHW()
    : hD3D(NULL), pD3D(NULL), pDevice(NULL), pBaseRT(NULL), pBaseZB(NULL),
      m_move_window(true), maxRefreshRate(200) // ECO_RENDER
{
  ;
}
...
#pragma todo("R2 need to specify depth format")
  ...
  for (int it = 0; it < fDS_Cnt; it++) {
    if (SUCCEEDED(pD3D->CheckDeviceFormat(DevAdapter, DevT, fTarget,
                                          D3DUSAGE_DEPTHSTENCIL,
                                          D3DRTYPE_SURFACE, fDS_Try[it]))) {
      if (SUCCEEDED(pD3D->CheckDepthStencilMatch(DevAdapter, DevT, fTarget,
                                                 fTarget, fDS_Try[it]))) {
        return fDS_Try[it];
      }
    }
...
void CHW::CreateDevice(HWND m_hWnd, bool move_window) {
  ...
  if (bWindowed) {
    ...
    case 32:
      ...
      if (SUCCEEDED(
              pD3D->CheckDeviceType(DevAdapter, DevT, fTarget, fTarget, FALSE)))
        break;
...
#else
        if (Mode.RefreshRate <= (UINT)maxRefreshRate &&
...
void CHW::updateWindowProps(HWND m_hWnd) {
  ...
  //? TRUE : !psDeviceFlags.is	(rsFullscreen); #ifndef DEDICATED_SERVER
  // BOOL bWindowed				= !psDeviceFlags.is
  // (rsFullscreen); #else 	BOOL	bWindowed				= TRUE;
  //#endif
