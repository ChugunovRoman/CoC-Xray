...
struct ECORE_API resptrcode_geom : public resptr_base<SGeometry> {
  void create(D3DVERTEXELEMENT9 *decl, ID3DVertexBuffer *vb,
              ID3DIndexBuffer *ib);
...
struct ECORE_API SPass : public xr_resource_flagged {
  ...
  ref_vs vs; // may be NULL = FFP, in that case "state" must contain RS setup,
             // *and* FVF-compatible declaration must be used
...
public:
  ref_selement E[6]; // R1 - 0=norm_lod0(det),	1=norm_lod1(normal),
                     // 2=L_point,		3=L_spot,	4=L_for_models,
                     // R2 - 0=deffer,
