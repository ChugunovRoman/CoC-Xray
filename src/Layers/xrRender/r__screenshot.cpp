...
void CRender::ScreenshotImpl(ScreenshotMode mode, LPCSTR name,
...
#else
    HRESULT hr =
        D3DX10SaveTextureToMemory(pSrcSmallTexture, D3DX10_IFF_DDS, &saved, 0);
    // HRESULT hr					=
  ...
  case IRender_interface::SM_NORMAL: {
    string64 t_stemp;
    string_path buf;
    xr_sprintf(buf, sizeof(buf), "ss_%s_%s_(%s).jpg", Core.UserName,
               timestamp(t_stemp),
               (g_pGameLevel) ? g_pGameLevel->name().c_str() : "mainmenu");
    ID3DBlob *saved = 0;
#ifdef USE_DX11
    CHK_DX(D3DX11SaveTextureToMemory(HW.pContext, pSrcTexture, D3DX11_IFF_JPG,
                                     &saved, 0));
#else
    ...
    if (strstr(Core.Params, "-ss_tga")) { // hq
      xr_sprintf(buf, sizeof(buf), "ssq_%s_%s_(%s).tga", Core.UserName,
                 timestamp(t_stemp),
                 (g_pGameLevel) ? g_pGameLevel->name().c_str() : "mainmenu");
      ID3DBlob *saved = 0;
#ifdef USE_DX11
      CHK_DX(D3DX11SaveTextureToMemory(HW.pContext, pSrcTexture, D3DX11_IFF_BMP,
                                       &saved, 0));
#else
      CHK_DX(D3DX10SaveTextureToMemory(pSrcTexture, D3DX10_IFF_BMP, &saved, 0));
      //		CHK_DX
      //(D3DXSaveSurfaceToFileInMemory (&saved,D3DXIFF_TGA,pFB,0,0));
#endif
      IWriter *fs = FS.w_open("$screenshots$", buf);
      R_ASSERT(fs);
      fs->w(saved->GetBufferPointer(), (u32)saved->GetBufferSize());
  ...
  case IRender_interface::SM_FOR_CUBEMAP: {
    VERIFY(!"CRender::Screenshot. This screenshot type is not supported for "
            "DX10.");
    /*
    string64			t_stemp;
    string_path			buf;
    VERIFY				(name);
    strconcat
    (sizeof(buf),buf,"ss_",Core.UserName,"_",timestamp(t_stemp),"_#",name);
    xr_strcat				(buf,".tga");
    IWriter*		fs	= FS.w_open	("$screenshots$",buf);
    R_ASSERT(fs); TGAdesc				p; p.format
    = IMG_24B;
    ...
    // save
    u32* data			=
    (u32*)xr_malloc(Device.dwHeight*Device.dwHeight*4); imf_Process
...
void CRender::ScreenshotImpl(ScreenshotMode mode, LPCSTR name,
  ...
  D3DLOCKED_RECT D;
  HRESULT hr;
  hr = HW.pDevice->CreateOffscreenPlainSurface(Device.dwWidth, Device.dwHeight,
                                               D3DFMT_A8R8G8B8,
  ...
  */
  ...
  case IRender_interface::SM_FOR_GAMESAVE: {
    ...
    hr = D3DXCreateTexture(HW.pDevice, GAMESAVE_SIZE, GAMESAVE_SIZE, 1, 0,
                           D3DFMT_DXT1, D3DPOOL_SCRATCH, &texture);
    if (hr != D3D_OK)
      goto _end_;
    if (NULL == texture)
      goto _end_;
  ...
  case IRender_interface::SM_FOR_MPSENDING: {
    // texture
    ID3DTexture2D *texture = NULL;
    hr = D3DXCreateTexture(HW.pDevice, SM_FOR_SEND_WIDTH, SM_FOR_SEND_HEIGHT, 1,
                           0, D3DFMT_R8G8B8, D3DPOOL_SCRATCH, &texture);
    if (hr != D3D_OK)
      goto _end_;
    if (NULL == texture)
      goto _end_;
  ...
  case IRender_interface::SM_FOR_CUBEMAP: {
    ...
    imf_Process(data, Device.dwHeight, Device.dwHeight, (u32 *)D.pBits,
                Device.dwWidth, Device.dwHeight, imf_lanczos3);
    p.scanlenght = Device.dwHeight * 4;
    p.width = Device.dwHeight;
    p.height = Device.dwHeight;
    p.data = data;
  ...
  if (Target->rt_Color->fmt == D3DFMT_A16B16G16R16F) {
    ...
    while (pPixel != pEnd) {
      ...
      for (int i = 0; i < iProcessPixels; ++i) {
        *pPixel = color_argb_f(1.0f, tmpArray[i * 4], tmpArray[i * 4 + 1],
                               tmpArray[i * 4 + 2]);
