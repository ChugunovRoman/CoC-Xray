...
IC bool InterpolateAndDither(float *alpha255, u32 x, u32 y, u32 sx, u32 sy,
                             u32 size, int dither[16][16]) {
  ...
  for (u32 z = 0; z <= d_size; z++) {
    for (u32 x = 0; x <= d_size; x++) {
      ...
      if ((DS.id0 != DetailSlot::ID_Empty) &&
          InterpolateAndDither(alpha255[0], x, z, shift_x, shift_z, d_size,
                               dither))
        selected.push_back(0);
      if ((DS.id1 != DetailSlot::ID_Empty) &&
          InterpolateAndDither(alpha255[1], x, z, shift_x, shift_z, d_size,
                               dither))
        selected.push_back(1);
      if ((DS.id2 != DetailSlot::ID_Empty) &&
          InterpolateAndDither(alpha255[2], x, z, shift_x, shift_z, d_size,
                               dither))
        selected.push_back(2);
      if ((DS.id3 != DetailSlot::ID_Empty) &&
          InterpolateAndDither(alpha255[3], x, z, shift_x, shift_z, d_size,
                               dither))
      ...
      for (u32 tid = 0; tid < triCount; tid++) {
        ...
        for (int k = 0; k < (int)I.inf.size(); k++) {
          ...
          I.e_obj->GetFaceWorld(I.s_obj->_Transform(), I.e_mesh, I.inf[k].id,
                                verts);
...
#ifndef DBG_SWITCHOFF_RANDOMIZE
      Item.scale =
          r_scale.randF(Dobj->m_fMinScale * 0.5f, Dobj->m_fMaxScale * 0.9f);
#else
      ...
      if (det_render_debug)
        ...
        float gray255	[4];
        gray255[0]						=
        255.f*float(c_pal->a0)/15.f;
        gray255[1]						=
        255.f*float(c_pal->a1)/15.f;
        gray255[2]						=
        255.f*float(c_pal->a2)/15.f;
        gray255[3]						=
        255.f*float(c_pal->a3)/15.f;
        */
        // float c_f						=	1.f;
        // //Interpolate (gray255,x,z,d_size)+.5f; int c_dw
        // =	255;	//iFloor (c_f); clamp (c_dw,0,255); Item.C_dw
