...
void dxStatGraphRender::RenderBack(CStatGraph &owner) {
  ...
  int PNum_H_LinesUp =
      int((base_y - float(owner.lt.y)) / (owner.grid_step.y * elem_factor));
  int PNum_H_LinesDwn =
      u32((float(owner.rb.y) - base_y) / (owner.grid_step.y * elem_factor));
  int Num_H_LinesUp =
  ...
  for (int g_y = 1; g_y <= Num_H_LinesDwn; g_y++) {
    pv->set(owner.lt.x, int(base_y + g_y * owner.grid_step.y * elem_factor),
            owner.grid_color);
    pv++;
    pv->set(owner.rb.x, int(base_y + g_y * owner.grid_step.y * elem_factor),
            owner.grid_color);
    pv++;
  };
