...
void CDetailManager::UpdateVisibleM() {
  ...
  for (u32 _mz = 0; _mz < dm_cache1_line; _mz++) {
    for (u32 _mx = 0; _mx < dm_cache1_line; _mx++) {
      ...
      u32 mask = 0xff;
      u32 res = View.testSAABB(MS.vis.sphere.P, MS.vis.sphere.R,
                               MS.vis.box.data(), mask);
      if (fcvNone == res) {
      ...
      for (u32 _i = 0; _i < dwCC; _i++) {
        ...
        if (fcvPartial == res) {
          u32 _mask = mask;
          u32 _res = View.testSAABB(S.vis.sphere.P, S.vis.sphere.R,
                                    S.vis.box.data(), _mask);
          if (fcvNone == _res) {
            continue; // invisible-view frustum
          }
        }
