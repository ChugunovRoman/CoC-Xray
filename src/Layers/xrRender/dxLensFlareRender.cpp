...
void dxLensFlareRender::Render(CLensFlare &owner, BOOL bSun, BOOL bFlares,
                               BOOL bGradient) {
  Fcolor dwLight;
  Fcolor color;
  ...
  u32 VS_Offset;
  FVF::LIT *pv =
      (FVF::LIT *)RCache.Vertex.Lock(MAX_Flares * 4, hGeom.stride(), VS_Offset);
  ...
  if (bSun) {
    if (owner.m_Current->m_Flags.is(CLensFlareDescriptor::flSource)) {
      ...
      pv++;
      _2render.push_back(
          ((dxFlareRender *)&*owner.m_Current->m_Source.m_pRender)->hShader);
    }
  }
  if (owner.fBlend >= EPS_L) {
    ...
    if (bGradient && (owner.fGradientValue >= EPS_L)) {
      if (owner.m_Current->m_Flags.is(CLensFlareDescriptor::flGradient)) {
        ...
        pv++;
        _2render.push_back(
            ((dxFlareRender *)&*owner.m_Current->m_Gradient.m_pRender)
                ->hShader);
      }
    }
  }
