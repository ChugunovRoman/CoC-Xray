...
*/
    ...
    // (S.v1,(D3DTEXTURESTAGESTATETYPE)S.v2,S.v3));
    //	TODO: DX10: Enable
    //	VERIFY(!"DirectX 10 doesn't support texture stage states. Implement
    // shader instead!"); 	break;
  }
}
  ...
  for (u32 it = 0; it < States.size(); it++) {
    ...
    if (S.type == 0) {
      ...
      case D3DRS_ZWRITEENABLE:
        desc.DepthWriteMask =
            S.v2 ? D3D_DEPTH_WRITE_MASK_ALL : D3D_DEPTH_WRITE_MASK_ZERO;
        break;
      ...
      case D3DRS_STENCILFUNC:
        desc.FrontFace.StencilFunc =
            dx10StateUtils::ConvertCmpFunction((D3DCMPFUNC)S.v2);
        break;
...
void SimulatorStates::UpdateDesc(
  ...
  for (u32 it = 0; it < States.size(); it++) {
    ...
    if (S.type == 2) {
      ...
      case D3DSAMP_MIPFILTER: /* D3DTEXTUREFILTER filter to use between mipmaps
                                 during minification */
        switch (S.v3) {
        case D3DTEXF_NONE:
        case D3DTEXF_POINT:
      ...
      case D3DSAMP_ADDRESSU: /* D3DTEXTUREADDRESS for U coordinate */
        desc.AddressU =
            dx10StateUtils::ConvertTextureAddressMode(D3DTEXTUREADDRESS(S.v3));
        break;
