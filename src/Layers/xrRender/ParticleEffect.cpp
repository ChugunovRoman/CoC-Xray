...
void CParticleEffect::SetBirthDeadCB(PAPI::OnBirthParticleCB bc,
                                     PAPI::OnDeadParticleCB dc, void *owner,
                                     u32 p) {
...
#ifndef _EDITOR
//----------------------------------------------------
IC void FillSprite_fpu(FVF::LIT *&pv, const Fvector &T, const Fvector &R,
                       const Fvector &pos, const Fvector2 &lt,
                       const Fvector2 &rb, float r1, float r2, u32 clr,
                       float angle) {
  float sa = _sin(angle);
  float ca = _cos(angle);
...
IC void FillSprite(FVF::LIT *&pv, const Fvector &T, const Fvector &R,
  ...
  Vr = _mm_mul_ps(_mm_set1_ps(r1),
                  _mm_add_ps(_mm_mul_ps(_T, _sa), _mm_mul_ps(_R, _ca)));
  Vt = _mm_mul_ps(_mm_set1_ps(r2),
                  _mm_sub_ps(_mm_mul_ps(_T, _ca), _mm_mul_ps(_R, _sa)));
...
IC void FillSprite(FVF::LIT *&pv, const Fvector &pos, const Fvector &dir,
                   const Fvector2 &lt, const Fvector2 &rb, float r1, float r2,
...
__forceinline void magnitude_sse(Fvector &vec, float &res) {
  ...
  tu =
      _mm_shuffle_ps(tu, tu, _MM_SHUFFLE(1, 1, 1, 1)); // tu = zz | zz | zz | zz
    ...
    if (pPE.m_Def->m_Flags.is(CPEDef::dfAlignToPath)) {
      ...
      if ((speed < EPS_S) && pPE.m_Def->m_Flags.is(CPEDef::dfWorldAlign)) {
        ...
        if (pPE.m_RT_Flags.is(CParticleEffect::flRT_XFORM)) {
          ...
          FillSprite(pv, M.k, M.i, m.pos, lt, rb, r_x, r_y, m.color, sina,
                     cosa);
        }
      } else if ((speed >= EPS_S) &&
        ...
        if (pPE.m_RT_Flags.is(CParticleEffect::flRT_XFORM)) {
          ...
          FillSprite(pv, M.j, M.i, m.pos, lt, rb, r_x, r_y, m.color, sina,
                     cosa);
        }
      } else {
        Fvector dir;
      ...
      if (pPE.m_RT_Flags.is(CParticleEffect::flRT_XFORM)) {
        ...
        FillSprite(pv, RDEVICE.vCameraTop, RDEVICE.vCameraRight, p, lt, rb, r_x,
                   r_y, m.color, sina, cosa);
      } else {
        FillSprite(pv, RDEVICE.vCameraTop, RDEVICE.vCameraRight, m.pos, lt, rb,
                   r_x, r_y, m.color, sina, cosa);
      }
    }
  }
}
  ...
  if (p_cnt > 0) {
    if (m_Def && m_Def->m_Flags.is(CPEDef::dfSprite)) {
      ...
      PRS_PARAMS *prsParams =
          (PRS_PARAMS *)_alloca(sizeof(PRS_PARAMS) * nWorkers);
      ...
      for (u32 i = 0; i < nWorkers; ++i) {
        ...
        prsParams[i].p_to =
            (i == (nWorkers - 1)) ? p_cnt : (prsParams[i].p_from + nStep);
        ...
        if (m_Def->m_Flags.is(CPEDef::dfAlignToPath)) {
          ...
          if ((speed < EPS_S) && m_Def->m_Flags.is(CPEDef::dfWorldAlign)) {
            ...
            if (m_RT_Flags.is(flRT_XFORM)) {
              ...
              FillSprite(pv, M.k, M.i, m.pos, lt, rb, r_x, r_y, m.color,
                         m.rot.x);
            }
          } else if ((speed >= EPS_S) &&
            ...
            if (m_RT_Flags.is(flRT_XFORM)) {
              ...
              FillSprite(pv, M.j, M.i, m.pos, lt, rb, r_x, r_y, m.color,
                         m.rot.x);
            }
          } else {
            Fvector dir;
          ...
          if (m_RT_Flags.is(flRT_XFORM)) {
            Fvector p;
            m_XFORM.transform_tiny(p, m.pos);
            FillSprite(pv, RDEVICE.vCameraTop, RDEVICE.vCameraRight, p, lt, rb,
                       r_x, r_y, m.color, m.rot.x);
          } else {
            FillSprite(pv, RDEVICE.vCameraTop, RDEVICE.vCameraRight, m.pos, lt,
                       rb, r_x, r_y, m.color, m.rot.x);
          }
        }
      }
