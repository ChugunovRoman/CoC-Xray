...
struct TL0uv {
  ...
  IC void transform(const Fvector &v, const Fmatrix &matSet) {
    ...
    p.x =
        (matSet._11 * v.x + matSet._21 * v.y + matSet._31 * v.z + matSet._41) /
        p.w;
    p.y =
        -(matSet._12 * v.x + matSet._22 * v.y + matSet._32 * v.z + matSet._42) /
        p.w;
    p.z =
        (matSet._13 * v.x + matSet._23 * v.y + matSet._33 * v.z + matSet._43) /
        p.w;
  };
};
...
struct TL2uv {
  ...
  IC void set(float x, float y, float z, float w, u32 c, Fvector2 &t0,
              Fvector2 &t1) {
