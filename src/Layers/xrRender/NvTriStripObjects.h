...
public:
  ...
  NvStripInfo(const NvStripStartInfo &startInfo, int stripId,
              int experimentId = -1)
  ...
  // returns true if the face is "unique", i.e. has a vertex which doesn't exist
  // in the faceVec
...
public:
  ...
  // the target vertex cache size, the structure to place the strips in, and the
  // input indices
  void Stripify(const WordVec &in_indices, const int in_cacheSize,
                const int in_minStripLength, NvStripInfoVec &allStrips,
...
protected:
  ...
  void RemoveSmallStrips(NvStripInfoVec &allStrips,
                         NvStripInfoVec &allBigStrips, NvFaceInfoVec &faceList);

  bool FindTraversal(NvFaceInfoVec &faceInfos, NvEdgeInfoVec &edgeInfos,
