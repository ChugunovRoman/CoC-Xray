...
void CKinematics::IBoneInstances_Create() {
  // VERIFY2				(bones->size() < 64, "More than 64 bones is a
  // crazy thing!");
  u32 size = bones->size();
...
void CKinematics::Load(const char *N, IReader *data, u32 dwFlags) {
  ...
  pUserData =
      UD ? xr_new<CInifile>(UD, FS.get_path("$game_config$")->m_Path) : 0;
  if (UD)
    UD->close();
#endif
...
#pragma todo("container is created in stack!")
  ...
  // Msg				("!!! %d bones",dwCount);
  // if (dwCount >= 64)	Msg			("!!! More than 64 bones is a crazy
  // thing!
  // (%d), %s",dwCount,N);
...
IC static void RecursiveBindTransform(CKinematics *K,
                                      xr_vector<Fmatrix> &matrices, u16 bone_id,
...
void BuildMatrix(Fmatrix &mView, float invsz, const Fvector norm,
                 const Fvector &from) {
  // build projection
  Fmatrix mScale;
...
bool CKinematics::PickBone(const Fmatrix &parent_xform,
                           IKinematics::pick_result &r, float dist,
...
void CKinematics::AddWallmark(const Fmatrix *parent_xform,
                              const Fvector3 &start, const Fvector3 &dir,
...
struct zero_wm_pred
    : public std::unary_function<intrusive_ptr<CSkeletonWallmark>, bool> {
...
void CKinematics::CalculateWallmarks() {
  if (!wallmarks.empty() && (wm_frame != RDEVICE.dwFrame)) {
    ...
    if (need_remove) {
      SkeletonWMVecIt new_end =
          std::remove_if(wallmarks.begin(), wallmarks.end(), zero_wm_pred());
