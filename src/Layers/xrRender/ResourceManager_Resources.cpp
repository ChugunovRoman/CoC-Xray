...
(mk_pair(RT->set_name(Name),RT)); if (RDEVICE.b_is_Ready)	RT->create
...
SVS *CResourceManager::_CreateVS(LPCSTR _name) {
  ...
  if (I != m_vs.end())
    ...
    _hr = ::Render->shader_compile(
        name, LPCSTR(fs->pointer()), fs->length(), NULL, &Includer, c_entry,
        c_target,
        D3DXSHADER_DEBUG |
            D3DXSHADER_PACKMATRIX_ROWMAJOR /*| D3DXSHADER_PREFER_FLOW_CONTROL*/,
        &pShaderBuf, &pErrorBuf, NULL);
    //		_hr = D3DXCompileShader		(LPCSTR(fs->pointer()),fs->length(),
    //NULL, &Includer, "main", target, D3DXSHADER_DEBUG |
...
SPS *CResourceManager::_CreatePS(LPCSTR name) {
  ...
  if (I != m_ps.end())
    ...
    _hr = ::Render->shader_compile(
        ...
        &pErrorBuf, NULL);
    //_hr = D3DXCompileShader		(text,text_size, NULL, &Includer,
    //c_entry, c_target, D3DXSHADER_DEBUG | D3DXSHADER_PACKMATRIX_ROWMAJOR,
    // &pShaderBuf, &pErrorBuf, NULL);
    xr_free(data);
