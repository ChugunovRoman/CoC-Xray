...
void dxRenderDeviceRender::SetupStates() {
  ...
  CHK_DX(
      HW.pDevice->SetRenderState(D3DRS_DIFFUSEMATERIALSOURCE, D3DMCS_MATERIAL));
  CHK_DX(HW.pDevice->SetRenderState(D3DRS_SPECULARMATERIALSOURCE,
                                    D3DMCS_MATERIAL));
  CHK_DX(
      HW.pDevice->SetRenderState(D3DRS_AMBIENTMATERIALSOURCE, D3DMCS_MATERIAL));
  CHK_DX(
      HW.pDevice->SetRenderState(D3DRS_EMISSIVEMATERIALSOURCE, D3DMCS_COLOR1));
...
void dxRenderDeviceRender::Create(HWND hWnd, u32 &dwWidth, u32 &dwHeight,
                                  float &fWidth_2, float &fHeight_2,
                                  bool move_window) {
...
void dxRenderDeviceRender::SetupGPU(BOOL bForceGPU_SW, BOOL bForceGPU_NonPure,
                                    BOOL bForceGPU_REF) {
...
void dxRenderDeviceRender::overdrawEnd() {
  ...
  CHK_DX(
      HW.pDevice->Clear(0, 0, D3DCLEAR_TARGET, D3DCOLOR_XRGB(255, 0, 0), 0, 0));
...
void dxRenderDeviceRender::ResourcesGetMemoryUsage(u32 &m_base, u32 &c_base,
...
void dxRenderDeviceRender::End() {
  ...
  // if				(D3DERR_DEVICELOST==_hr)	return;			// we will
  // handle this later
}
