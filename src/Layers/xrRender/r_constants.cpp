...
ref_constant R_constant_table::get(LPCSTR S) {
  ...
  c_table::iterator I =
      std::lower_bound(table.begin(), table.end(), S, p_search);
      ...
      case 4:
        ...
        case 3:
          ...
          case 3:
            ...
            break;
          default:
            Msg("Invalid matrix dimension:%dx%d in constant %s",
                it->RegisterCount, T->Columns, name);
  ...
  for (u32 it = 0; it < T->table.size(); it++) {
    ...
    if (!C) {
      ...
      VERIFY2(!(C->destination & src->destination & RC_dest_sampler),
              "Can't have samplers or textures with the same name for PS, VS "
              "and GS.");
      C->destination |= src->destination;
