...
void CBlender_Model_EbB::Compile(CBlender_Compile &C) {
  ...
  if (C.bEditor) {
    ...
    case SE_R1_LPOINT:
      ...
      psname = "add_point";
      C.r_Pass(vsname, psname, FALSE, TRUE, FALSE, TRUE, D3DBLEND_ONE,
               D3DBLEND_ONE, TRUE);
      C.r_Sampler("s_base", C.L_textures[0]);
    ...
    case SE_R1_LMODELS:
      ...
      psname = "model_shadow";
      C.r_Pass(vsname, psname, FALSE, FALSE, FALSE, TRUE, D3DBLEND_ZERO,
               D3DBLEND_SRCCOLOR, FALSE, 0);
      C.r_End();
      break;
    }
  }
...
void CBlender_Model_EbB::Compile(CBlender_Compile &C) {
  ...
  if (oBlend.value) {
    ...
    case SE_R2_SHADOW: // smap
      ...
      // ("shadow_direct_model","dumb",	FALSE,TRUE,TRUE,FALSE); else
      // C.r_Pass	("shadow_direct_model","shadow_direct_base",FALSE);
      C.r_Pass("shadow_direct_model", "dumb", FALSE, TRUE, TRUE, FALSE);
      // C.r_Sampler		("s_base",C.L_textures[0]);
      C.r_dx10Texture("s_base", C.L_textures[0]);
