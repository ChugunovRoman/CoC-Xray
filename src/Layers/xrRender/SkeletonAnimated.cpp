...
CKinematicsAnimated::LL_MotionDefName_dbg(MotionID ID) {
...
static void dump_blend(CKinematicsAnimated *K, CBlend &B, u32 index) {
  ...
  Msg("time total: %f, speed: %f , power: %f ", B.timeTotal, B.speed,
      B.blendPower);
  Msg("ammount: %f, time current: %f, frame %d ", B.blendAmount, B.timeCurrent,
      B.dwFrame);
...
void CKinematicsAnimated::IBlendSetup(CBlend &B, u16 part, u8 channel,
                                      MotionID motion_ID, BOOL bMixing,
                                      float blendAccrue, float blendFalloff,
                                      float Speed, BOOL noloop,
...
void CKinematicsAnimated::IFXBlendSetup(CBlend &B, MotionID motion_ID,
                                        float blendAccrue, float blendFalloff,
...
CBlend *CKinematicsAnimated::LL_PlayCycle(u16 part, MotionID motion_ID,
                                          BOOL bMixing, float blendAccrue,
                                          float blendFalloff, float Speed,
                                          BOOL noloop, PlayCallback Callback,
                                          LPVOID CallbackParam,
                                          u8 channel /*=0*/) {
  // validate and unroll
  if (!motion_ID.valid())
  ...
  if (BI_NONE == part) {
    for (u16 i = 0; i < MAX_PARTS; i++)
      LL_PlayCycle(i, motion_ID, bMixing, blendAccrue, blendFalloff, Speed,
                   noloop, Callback, CallbackParam, channel);
    return 0;
  }
  if (part >= MAX_PARTS)
    return 0;
  ...
  _DBG_SINGLE_USE_MARKER;
  IBlendSetup(*B, part, channel, motion_ID, bMixing, blendAccrue, blendFalloff,
              Speed, noloop, Callback, CallbackParam);
  for (u32 i = 0; i < P.bones.size(); i++)
...
CBlend *CKinematicsAnimated::LL_PlayCycle(u16 part, MotionID motion_ID,
                                          BOOL bMixIn, PlayCallback Callback,
                                          LPVOID CallbackParam,
                                          u8 channel /*=0*/) {
...
CBlend *CKinematicsAnimated::PlayCycle(LPCSTR N, BOOL bMixIn,
                                       PlayCallback Callback,
                                       LPVOID CallbackParam,
                                       u8 channel /*= 0*/) {
...
CBlend *CKinematicsAnimated::PlayCycle(MotionID motion_ID, BOOL bMixIn,
                                       PlayCallback Callback,
                                       LPVOID CallbackParam,
                                       u8 channel /*= 0*/) {
  VERIFY(motion_ID.valid());
  CMotionDef *m_def =
      m_Motions[motion_ID.slot].motions.motion_def(motion_ID.idx);
  VERIFY(m_def);
  return LL_PlayCycle(m_def->bone_or_part, motion_ID, bMixIn, m_def->Accrue(),
                      m_def->Falloff(), m_def->Speed(), m_def->StopAtEnd(),
                      Callback, CallbackParam, channel);
}

CBlend *CKinematicsAnimated::PlayCycle(u16 partition, MotionID motion_ID,
                                       BOOL bMixIn, PlayCallback Callback,
                                       LPVOID CallbackParam,
                                       u8 channel /*= 0*/) {
  VERIFY(motion_ID.valid());
...
CBlend *CKinematicsAnimated::LL_PlayFX(u16 bone, MotionID motion_ID,
                                       float blendAccrue, float blendFalloff,
...
void CKinematicsAnimated::LL_UpdateTracks(float dt, bool b_force,
                                          bool leave_blends) {
  BlendSVecIt I, E;
  // Cycles
  for (u16 part = 0; part < MAX_PARTS; part++) {
    ...
    for (; I != E; I++) {
      ...
      // float timeCurrent						=
      // B.timeCurrent; 	xr_vector<motion_marks>::iterator it	=
      //m_def->marks.begin(); 	xr_vector<motion_marks>::iterator it_e	=
      //m_def->marks.end(); 	for(;it!=it_e; ++it)
      //	{
...
void CKinematicsAnimated::UpdateTracks() {
  ...
  if (GetUpdateTracksCalback()) {
    if ((*GetUpdateTracksCalback())(
            float(RDEVICE.dwTimeGlobal - Update_LastTime) / 1000.f, *this))
...
CKinematicsAnimated::CKinematicsAnimated()
    : CKinematics(), IKinematicsAnimated(), blend_instances(NULL),
      m_Partition(NULL), m_blend_destroy_callback(0),
...
void CKinematicsAnimated::Load(const char *N, IReader *data, u32 dwFlags) {
  ...
  //.		Msg("* WARNING: model '%s' has only one motion. Candidate for
  // SkeletonRigid???",N);
}
...
void CKinematicsAnimated::LL_BoneMatrixBuild(CBoneInstance &bi,
  ...
  for (u16 j = 0; MAX_CHANNELS > j; ++j) {
    ...
    process_single_channel(channel_keys[ch_count], BC[ch_count], keys.keys[j],
                           keys.blends[j], keys.chanel_blend_conts[j]);
    ++ch_count;
  }
  CKey Result;
  // Mix channels
...
void CKinematicsAnimated::BuildBoneMatrix(const CBoneData *bd,
  ...
  // CKey				R						[MAX_CHANNELS][MAX_BLENDED];
  // //all keys float				BA [MAX_CHANNELS][MAX_BLENDED];
  // //all factors int				b_counts
  // [MAX_CHANNELS]	= {0,0,0,0}; //channel counts
  SKeyTable keys;
  ...
  *LL_GetMotion(B->motionID,SelfID); float			time	=
  ...
  iFloor(time); u32				count	=	M.get_count();
