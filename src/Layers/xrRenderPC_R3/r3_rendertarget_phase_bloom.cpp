...
void CRenderTarget::phase_bloom() {
  ...
  // Transfer into Bloom1
  {
    ...
    // Fill vertex buffer
    v_build *pv =
        (v_build *)RCache.Vertex.Lock(4, g_bloom_build->vb_stride, Offset);
    ...
    // Perform combine (all scalers must account for 4 samples + final diffuse
    // multiply);
    float s = ps_r2_ls_bloom_threshold; // scale
    f_bloom_factor = .9f * f_bloom_factor +
                     .1f * ps_r2_ls_bloom_speed * Device.fTimeDelta; // speed
  ...
  if (ps_r2_ls_flags.test(R2FLAG_FASTBLOOM)) {
    ...
    // Transfer into Bloom2, horizontal filter
    {
      ...
      // Fill vertex buffer
      v_filter *pv =
          (v_filter *)RCache.Vertex.Lock(4, g_bloom_filter->vb_stride, Offset);
  ...
  // for FP16-BLEND capable HW we can blend flares into smaller target, because
  // they are smooth
