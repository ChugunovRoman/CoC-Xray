...
void CRender::render_main(Fmatrix &m_ViewProjection, bool _fportals) {
  ...
  if (pLastSector) {
    //!!!
    //!!! BECAUSE OF PARALLEL HOM RENDERING TRY TO DELAY ACCESS TO HOM AS MUCH
    //! AS POSSIBLE
    //!!!
    {
      ...
      if (phase == PHASE_NORMAL) {
        ...
        if (lstRenderables.size()) {
          IRenderable *renderable =
              lstRenderables[uID_LTRACK]->dcast_Renderable();
          if (renderable) {
    ...
    PortalTraverser.traverse(
        ...
        //. disabled scissoring
        //(HW.Caps.bScissor?CPortalTraverser::VQ_SCISSOR:0)	// generate
        // scissoring info
    );
    ...
    for (u32 o_it = 0; o_it < lstRenderables.size(); o_it++) {
      ...
      for (u32 v_it = 0; v_it < sector->r_frustums.size(); v_it++) {
        ...
        if (spatial->spatial.type & STYPE_RENDERABLE) {
          ...
          vis_data &v_orig =
              ((dxRender_Visual *)renderable->renderable.visual)->vis;
...
void CRender::Render() {
  ...
  if (1) {
    ...
    while ((hr = GetData(q_sync_point[q_sync_count], &result,
                         sizeof(result))) == S_FALSE) {
      if (!SwitchToThread())
  ...
  if (split_the_scene_to_minimize_wait) {
    ...
    if (0) {
      ...
      if (!RImplementation.o.dx10_msaa)
        Target->u_setrt(Target->rt_Generic_0, Target->rt_Generic_1, 0,
                        HW.pBaseZB);
      else
