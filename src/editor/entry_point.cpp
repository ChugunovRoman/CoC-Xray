...
public:
  window_ide_final(editor::ide *&ide, editor::engine *engine)
    ...
    m_ide = ide;
    Application::Idle +=
        gcnew System::EventHandler(this, &window_ide_final::on_idle);
  }
...
protected:
  virtual void WndProc(Message % m) override {
    LRESULT result;
    if (m_engine &&
        m_engine->on_message((HWND)m.HWnd.ToInt32(), m.Msg, m.WParam.ToInt32(),
                             m.LParam.ToInt32(), result))
      return;
...
extern "C" __declspec(dllexport) void initialize(editor::ide *&ide,
