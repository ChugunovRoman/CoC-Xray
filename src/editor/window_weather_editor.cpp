...
void window_weather_editor::fill_weathers() {
  TimeFactorNumericUpDown->Value =
      System::Decimal(m_engine.weather_time_factor());
...
void window_weather_editor::fill_frames(LPCSTR current_weather_id) {
  ...
  int index = FramesComboBox->Items->IndexOf(
      to_string(m_engine.current_weather_frame()));
...
Void window_weather_editor::window_weather_editor_Enter(Object ^ sender,
                                                        EventArgs ^ e) {
...
void window_weather_editor::on_load_finished() {
  ...
  ::property_holder *properties =
      dynamic_cast<::property_holder *>(m_engine.blend_frame_property_holder());
  VERIFY(properties);
...
Void window_weather_editor::WeathersComboBox_SelectedIndexChanged(
  ...
  WeathersComboBox->SelectedIndex =
      WeathersComboBox->Items->IndexOf(to_string(m_engine.weather()));
  fill_frames(m_engine.weather());
}

Void window_weather_editor::FramesComboBox_SelectedIndexChanged(Object ^ sender,
                                                                EventArgs ^ e) {
  ...
  String ^ new_weather_frame =
      safe_cast<String ^>(FramesComboBox->SelectedItem);
...
Void window_weather_editor::PreviousFrameButton_Click(Object ^ sender,
                                                      EventArgs ^ e) {
  if (FramesComboBox->SelectedIndex == -1)
    return;
...
Void window_weather_editor::NextFrameButton_Click(Object ^ sender,
                                                  EventArgs ^ e) {
  if (FramesComboBox->SelectedIndex == -1)
    return;
...
void window_weather_editor::update_frame() {
  ...
  properties = dynamic_cast<::property_holder *>(
      m_engine.current_frame_property_holder());
  if (properties) {
    if (safe_cast<property_container ^>(current->SelectedObject) !=
  ...
  properties = dynamic_cast<::property_holder *>(
      m_engine.target_frame_property_holder());
  if (properties) {
    if (safe_cast<property_container ^>(target->SelectedObject) !=
        properties->container()) {
...
Void window_weather_editor::FramesComboBox_DropDown(Object ^ sender,
                                                    EventArgs ^ e) {
  m_update_enabled = false;
}

Void window_weather_editor::FramesComboBox_DropDownClosed(Object ^ sender,
                                                          EventArgs ^ e) {
...
Void window_weather_editor::TimeFactorNumericUpDown_ValueChanged(
    Object ^ sender, EventArgs ^ e) {
...
Void window_weather_editor::PasteCurrentButton_Click(Object ^ sender,
                                                     EventArgs ^ e) {
  char *buffer = to_string(Clipboard::GetText());
  if (buffer)
    m_engine.paste_current_time_frame(buffer,
...
Void window_weather_editor::PasteTargetButton_Click(Object ^ sender,
                                                    EventArgs ^ e) {
  char *buffer = to_string(Clipboard::GetText());
  if (buffer)
    m_engine.paste_target_time_frame(buffer,
                                     (strlen(buffer) + 1) * sizeof(char));
  free(buffer);
...
Void window_weather_editor::CreateFromButton_Click(Object ^ sender,
                                                   EventArgs ^ e) {
  char buffer[4096];
  if (!m_engine.save_time_frame(buffer, sizeof(buffer)))
    return;
...
Void window_weather_editor::window_weather_editor_SizeChanged(Object ^ sender,
                                                              EventArgs ^ e) {
  panel12->Width = int(Width * .4f);
  panel14->Width = int(Width * .4f);
}

Void window_weather_editor::CurrentTimeTrackBar_ValueChanged(Object ^ sender,
                                                             EventArgs ^ e) {
...
Void window_weather_editor::CurrentTimeTrackBar_MouseDown(Object ^ sender,
                                                          MouseEventArgs ^ e) {
...
Void window_weather_editor::CurrentTimeTrackBar_MouseUp(Object ^ sender,
                                                        MouseEventArgs ^ e) {
  m_engine.weather_paused(PauseButton->ImageIndex ? true : false);
  blend->Refresh();
  m_mouse_down = false;
...
Void window_weather_editor::WeatherTrackBar_ValueChanged(Object ^ sender,
                                                         EventArgs ^ e) {
...
Void window_weather_editor::WeatherTrackBar_MouseDown(Object ^ sender,
                                                      MouseEventArgs ^ e) {
  m_engine.weather_paused(true);
...
Void window_weather_editor::WeatherTrackBar_MouseUp(Object ^ sender,
                                                    MouseEventArgs ^ e) {
  m_engine.weather_paused(PauseButton->ImageIndex ? true : false);
  blend->Refresh();
  m_mouse_down = false;
...
Void window_weather_editor::CurrentTimeTextBox_TextChanged(Object ^ sender,
                                                           EventArgs ^ e) {
...
Void window_weather_editor::ReloadCurrentButton_Click(Object ^ sender,
                                                      EventArgs ^ e) {
  m_engine.reload_current_time_frame();
  current->Refresh();
}

Void window_weather_editor::ReloadTargetButton_Click(Object ^ sender,
                                                     EventArgs ^ e) {
