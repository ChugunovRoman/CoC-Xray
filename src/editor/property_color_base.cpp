...
property_color_base::property_color_base(editor::color const % color,
  ...
  PropertySpec ^ spec;
  spec = gcnew PropertySpec("red", float ::typeid, "components",
                            "red component", color.r, (String ^) nullptr,
                            property_converter_float::typeid);
  spec->Attributes = gcnew array<System::Attribute ^>{
      gcnew System::ComponentModel::NotifyParentPropertyAttribute(true)};
  m_container->add_property(
      spec, gcnew property_float_limited(getter, setter, .0025f, 0.f, 1.f));

  getter.bind(m_components, &color_components::green_getter);
  setter.bind(m_components, &color_components::green_setter);
  spec = gcnew PropertySpec("green", float ::typeid, "components",
                            "green component", color.g, (String ^) nullptr,
                            property_converter_float::typeid);
  ...
  spec = gcnew PropertySpec("blue", float ::typeid, "components",
                            "blue component", color.b, (String ^) nullptr,
                            property_converter_float::typeid);
...
Object ^ property_color_base::get_value() { return (m_container); }
...
void property_color_base::on_double_click(editor::controls::property_grid ^
                                          property_grid) {
#if 0
