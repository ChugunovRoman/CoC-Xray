...
Object ^ property_collection_editor::CreateInstance(Type ^ type) {
  ...
  property_collection ^ collection =
      safe_cast<property_collection ^>(raw_value);
...
property_collection_editor::CollectionForm ^
    property_collection_editor::CreateCollectionForm() {
  //	VERIFY						(!m_collection_form);
  m_collection_form = inherited::CreateCollectionForm();
  m_collection_form->Move +=
      gcnew System::EventHandler(this, &property_collection_editor::on_move);
  return (m_collection_form);
}

Object ^ property_collection_editor::EditValue(ITypeDescriptorContext ^ context,
                                               IServiceProvider ^ provider,
                                               Object ^ value) {
  if (!m_collection_form || !m_collection_form->Visible)
    return (inherited::EditValue(context, provider, value));

  property_collection_editor ^ editor =
      gcnew property_collection_editor(CollectionType);
