...
PropertyDescriptorCollection ^ property_container_converter::GetProperties(
  ...
  property_container ^ container =
      dynamic_cast<property_container ^>(context->Instance);
  if (container) {
    ...
    return (current->Sort(
        reinterpret_cast<array<String ^> ^>(names->ToArray(String::typeid))));
  }
...
bool property_container_converter::CanConvertTo(ITypeDescriptorContext ^
                                                    context,
...
Object ^
    property_container_converter::ConvertTo(ITypeDescriptorContext ^ context,
                                            CultureInfo ^ culture,
                                            Object ^ value,
                                            Type ^ destination_type) {
