...
PropertyDescriptorCollection ^
    property_converter_vec3f::GetProperties(ITypeDescriptorContext ^ context,
                                            Object ^ value,
                                            array<Attribute ^> ^ attributes) {
  PropertyDescriptorCollection ^ current =
...
bool property_converter_vec3f::GetPropertiesSupported(ITypeDescriptorContext ^
                                                      context) {
  return (true);
}

bool property_converter_vec3f::CanConvertTo(ITypeDescriptorContext ^ context,
...
Object ^ property_converter_vec3f::ConvertTo(ITypeDescriptorContext ^ context,
                                             CultureInfo ^ culture,
                                             Object ^ value,
                                             Type ^ destination_type) {
  if (destination_type == String::typeid) {
    ...
    editor::vec3f vec3f =
        safe_cast<property_vec3f_base %>(container->container_holder())
            .get_value_raw();
    return ("" +
            property_converter_float().ConvertTo(context, culture, vec3f.x,
                                                 String::typeid) +
            " " +
            property_converter_float().ConvertTo(context, culture, vec3f.y,
                                                 String::typeid) +
            " " +
            property_converter_float().ConvertTo(context, culture, vec3f.z,
                                                 String::typeid) +
            "");
  }
...
bool property_converter_vec3f::CanConvertFrom(ITypeDescriptorContext ^ context,
                                              Type ^ source_type) {
...
Object ^ property_converter_vec3f::ConvertFrom(ITypeDescriptorContext ^ context,
                                               CultureInfo ^ culture,
                                               Object ^ value) {
