...
WeifenLuo::WinFormsUI::IDockContent ^
    window_ide::reload_content(System::String ^ persist_string) {
...
void window_ide::load_on_create() {
  ...
  if (windows) {
    ...
    if (temp) {
      System::Array ^ object =
          safe_cast<System::Array ^>(windows->GetValue("editor"));
      ...
      using System::IO::MemoryStream;
      MemoryStream ^ stream = gcnew MemoryStream();
      stream->Write(safe_cast<array<unsigned char, 1> ^>(object), 0,
                    object->Length);
      stream->Seek(0, System::IO::SeekOrigin::Begin);
      Editor->LoadFromXml(stream,
                          gcnew WeifenLuo::WinFormsUI::DeserializeDockContent(
                              this, &window_ide::reload_content));
      delete (stream);
      return;
    }
