...
property_container::property_container(property_holder *holder,
                                       property_container_holder ^
                                           container_holder)
    : m_holder(holder), m_container_holder(container_holder),
      m_properties(gcnew Hashtable()), m_ordered_properties(gcnew ArrayList()) {
  GetValue += gcnew PropertySpecEventHandler(
      this, &property_container::get_value_handler);
  SetValue += gcnew PropertySpecEventHandler(
      this, &property_container::set_value_handler);
}
...
bool property_container::equal_category(String ^ new_category,
                                        String ^ old_category) {
...
void property_container::try_update_name(PropertySpec ^ description,
                                         String ^ name) {
  VERIFY(!!name->Length);
  VERIFY((name[0] != '\t'));
...
void property_container::add_property(PropertySpec ^ description,
...
void property_container::get_value_handler(Object ^ sender,
                                           PropertySpecEventArgs ^ e) {
  property_value ^ value =
      safe_cast<property_value ^>(m_properties[e->Property]);
  VERIFY(value);
  e->Value = value->get_value();
}

void property_container::set_value_handler(Object ^ sender,
                                           PropertySpecEventArgs ^ e) {
  property_value ^ value =
      safe_cast<property_value ^>(m_properties[e->Property]);
  VERIFY(value);
