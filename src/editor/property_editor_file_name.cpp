...
property_editor_file_name::GetEditStyle(ITypeDescriptorContext ^ context) {
  if (context)
...
Object ^ property_editor_file_name::EditValue(ITypeDescriptorContext ^ context,
                                              IServiceProvider ^ provider,
                                              Object ^ value) {
  if (!context || !provider)
    return (inherited::EditValue(context, provider, value));

  typedef System::Windows::Forms::Design::IWindowsFormsEditorService
  ...
  property_file_name_value_base ^ real_value =
      safe_cast<property_file_name_value_base ^>(raw_value);
  ...
  //	m_dialog->InitialDirectory				=
  // real_value->initial_directory();
  m_dialog->Title = real_value->title();
  String ^ initial_directory =
      System::IO::Path::GetFullPath(real_value->initial_directory()->ToLower());
  ...
  CustomControls::FormOpenFileDialog ^ dialog =
      gcnew CustomControls::FormOpenFileDialog();
  ...
  dialog->OpenDialog->InitialDirectory =
      System::IO::Path::GetFullPath(real_value->initial_directory());
  dialog->OpenDialog->Title = real_value->title();
  dialog->OpenDialog->FileName =
      dialog->OpenDialog->InitialDirectory + safe_cast<System::String ^>(value);
  dialog->OpenDialog->InitialDirectory = "";
  dialog->ShowDialog();
  delete (dialog);
