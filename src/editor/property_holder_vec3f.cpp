...
editor::property_value *property_holder::add_property(
    ...
    editor::vec3f const &default_value, vec3f_getter_type const &getter,
    vec3f_setter_type const &setter, readonly_enum const &read_only,
  ...
  m_container->add_property(
      gcnew PropertySpec(
          to_string(identifier), Vec3f::typeid, to_string(category),
...
editor::property_value *property_holder::add_property(
    LPCSTR identifier, LPCSTR category, LPCSTR description,
    editor::vec3f const &default_value, editor::vec3f &value,
