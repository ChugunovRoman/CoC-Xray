...
bool property_editor_color::GetPaintValueSupported(ITypeDescriptorContext ^
                                                   context) {
  return (true);
}
...
void property_editor_color::PaintValue(PaintValueEventArgs ^ arguments) {
  ...
  property_container ^ container =
      safe_cast<property_container ^>(arguments->Value);
  editor::color color =
      safe_cast<property_color_base %>(container->container_holder())
          .get_value_raw();
  Graphics ^ graphics = arguments->Graphics;

  SolidBrush ^ brush = gcnew SolidBrush(System::Drawing::Color::FromArgb(
      255, convert_color(color.r), convert_color(color.g),
...
property_editor_color::GetEditStyle(ITypeDescriptorContext ^ context) {
  if (context)
...
Object ^ property_editor_color::EditValue(ITypeDescriptorContext ^ context,
  ...
  property_color_base ^ real_value =
      safe_cast<property_color_base ^>(raw_value);
  ...
  dialog->Color = System::Drawing::Color::FromArgb(
      255, int(255.f * color.r), int(255.f * color.g), int(255.f * color.b));
  if (dialog->ShowDialog() != System::Windows::Forms::DialogResult::Cancel)
    real_value->set_value(::Color(dialog->Color.R / 255.f,
                                  dialog->Color.G / 255.f,
                                  dialog->Color.B / 255.f));
