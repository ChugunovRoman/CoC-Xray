...
property_editor_tree_values::GetEditStyle(ITypeDescriptorContext ^ context) {
  if (context)
...
Object ^
    property_editor_tree_values::EditValue(ITypeDescriptorContext ^ context,
  ...
  property_string_values_value_base ^ real_value =
      safe_cast<property_string_values_value_base ^>(raw_value);
  m_dialog->values(real_value->values(),
                   safe_cast<String ^>(raw_value->get_value()));
