...
void compute_moment(moment &M, Fvector &p, Fvector &q, Fvector &r) {
  ...
  M.s.m[0][0] =
      M.A * (9 * M.m[0] * M.m[0] + p[0] * p[0] + q[0] * q[0] + r[0] * r[0]) /
      12;
  M.s.m[0][1] =
      M.A * (9 * M.m[0] * M.m[1] + p[0] * p[1] + q[0] * q[1] + r[0] * r[1]) /
      12;
  M.s.m[1][1] =
      M.A * (9 * M.m[1] * M.m[1] + p[1] * p[1] + q[1] * q[1] + r[1] * r[1]) /
      12;
  M.s.m[0][2] =
      M.A * (9 * M.m[0] * M.m[2] + p[0] * p[2] + q[0] * q[2] + r[0] * r[2]) /
      12;
  M.s.m[1][2] =
      M.A * (9 * M.m[1] * M.m[2] + p[1] * p[2] + q[1] * q[2] + r[1] * r[2]) /
      12;
  M.s.m[2][2] =
      M.A * (9 * M.m[2] * M.m[2] + p[2] * p[2] + q[2] * q[2] + r[2] * r[2]) /
      12;
  M.s.m[2][1] = M.s.m[1][2];
  M.s.m[1][0] = M.s.m[0][1];
  M.s.m[2][0] = M.s.m[0][2];
...
void compute_moments(moment *M, Fvector *vertices, int num_tris) {
  ...
  for (i = 0; i < num_tris; i++) {
    compute_moment(M[i], vertices[i * 3 + 0], vertices[i * 3 + 1],
                   vertices[i * 3 + 2]);
    if (M[i].A == 0.0f) {
      zero = 1;
    } else {
...
BOOL RAPIDMinBox(Fobb &B, Fvector *vertices, u32 v_count) {
  ...
  B.m_translate.x = c[0] * B.m_rotate.m[0][0] + c[1] * B.m_rotate.m[0][1] +
                    c[2] * B.m_rotate.m[0][2];
  B.m_translate.y = c[0] * B.m_rotate.m[1][0] + c[1] * B.m_rotate.m[1][1] +
                    c[2] * B.m_rotate.m[1][2];
  B.m_translate.z = c[0] * B.m_rotate.m[2][0] + c[1] * B.m_rotate.m[2][1] +
                    c[2] * B.m_rotate.m[2][2];
