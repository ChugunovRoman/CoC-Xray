...
mappings, causing stretching that can ruin per-pixel lighting.
  ...
  Say you have positions & indices & normals, and want textures, and tangents,
and binormals. and assume that positions, indices, and normals are in STL
arrays: vpos, triIndices and vnor respectively.
...
triangle's size
  ...
  vpos = outputAtts[0].floatVector_; // Note that there may be more vertices
than you sent in. vnor = outputAtts[2].floatVector_; xr_vector<float> texCoords
= outputAtts[3].floatVector_; // texcoords xr_vector<float> vtgt =
outputAtts[4].floatVector_;		 // tgts triIndices =
outputAtts[1].intVector_;					 // new indices.
...
public:
  ...
  bool NVMeshMender::Munge(
      const NVMeshMender::VAVector &input, NVMeshMender::VAVector &output,
      const float bSmoothCreaseAngleRadians = 3.141592654f / 3.0f,
      const float *pTextureMatrix = 0, const Option _FixTangents = FixTangents,
