...
template <class _T>
inline const vec3t<_T> operator^(const vec3t<_T> &u, const vec3t<_T> &v) {
  return vec3t<_T>(u.y * v.z - u.z * v.y, u.z * v.x - u.x * v.z,
                   u.x * v.y - u.y * v.x);
}
template <class _T>
...
struct DECLSPEC_NV_MATH mat3 {
  ...
  mat3(const nv_scalar &f0, const nv_scalar &f1, const nv_scalar &f2,
      ...
      : a00(f0), a10(f1), a20(f2), a01(f3), a11(f4), a21(f5), a02(f6), a12(f7),
        a22(f8) {}
...
struct DECLSPEC_NV_MATH mat4 {
  ...
  mat4(const nv_scalar &f0, const nv_scalar &f1, const nv_scalar &f2,
       const nv_scalar &f3, const nv_scalar &f4, const nv_scalar &f5,
       const nv_scalar &f6, const nv_scalar &f7, const nv_scalar &f8,
       const nv_scalar &f9, const nv_scalar &f10, const nv_scalar &f11,
       const nv_scalar &f12, const nv_scalar &f13, const nv_scalar &f14,
       const nv_scalar &f15)
      : a00(f0), a10(f1), a20(f2), a30(f3), a01(f4), a11(f5), a21(f6), a31(f7),
        a02(f8), a12(f9), a22(f10), a32(f11), a03(f12), a13(f13), a23(f14),
        a33(f15) {}

  const vec4 col(const int i) const { return vec4(&mat_array[i * 4]); }

  const vec4 operator[](const int &i) const {
    return vec4(mat_array[i], mat_array[i + 4], mat_array[i + 8],
                mat_array[i + 12]);
  }
...
const nv_scalar array16_id[] = {
    nv_one,  nv_zero, nv_zero, nv_zero, nv_zero, nv_one,  nv_zero, nv_zero,
...
const nv_scalar array16_scale_bias[] = {
    nv_zero_5, nv_zero,   nv_zero,   nv_zero, nv_zero,   nv_zero_5,
...
extern mat4 &look_at(mat4 &M, const vec3 &eye, const vec3 &center,
                     const vec3 &up);
extern mat4 &frustum(mat4 &M, const nv_scalar l, const nv_scalar r,
                     const nv_scalar b, const nv_scalar t, const nv_scalar n,
                     const nv_scalar f);

extern mat4 &perspective(mat4 &M, const nv_scalar fovy, const nv_scalar aspect,
...
// surface properties
extern mat3 &tangent_basis(mat3 &basis, const vec3 &v0, const vec3 &v1,
                           const vec3 &v2, const vec2 &t0, const vec2 &t1,
...
inline nv_scalar nv_clamp(nv_scalar u, const nv_scalar min,
                          const nv_scalar max) {
  u = (u < min) ? min : u;
  u = (u > max) ? max : u;
  return u;
...
extern nv_scalar nv_find_in_circle(vec3 &center, const vec3 &v1, const vec3 &v2,
                                   const vec3 &v3);
// find the circumscribed circle
extern nv_scalar nv_find_circ_circle(vec3 &center, const vec3 &v1,
