...
{
        u32 v_count_reserve			=
3*iFloor(float(m_Verts.size())*1.33f);
    ...
    int i_id=-1;
        input[++i_id].Name_= "position";	xr_vector<float>&
i_position	= input[i_id].floatVector_;	i_position.reserve
(v_count_reserve);
        input[++i_id].Name_= "normal";		xr_vector<float>& i_normal
= input[i_id].floatVector_;	i_normal.reserve	(v_count_reserve);
        input[++i_id].Name_= "tex0";		xr_vector<float>& i_tc =
input[i_id].floatVector_;	i_tc.reserve		(v_count_reserve);
        input[++i_id].Name_= "c_w0_b0";  	xr_vector<float>&
i_c_w0_b0	= input[i_id].floatVector_;	i_c_w0_b0.reserve
(v_count_reserve); input[++i_id].Name_= "w1_w2_w3";	xr_vector<float>&
i_w1_w2_w3	= input[i_id].floatVector_;	i_w1_w2_w3.reserve
(v_count_reserve); input[++i_id].Name_= "b1_b2_b3";	xr_vector<float>&
i_b1_b2_b3	= input[i_id].floatVector_;	i_b1_b2_b3.reserve
(v_count_reserve); input[++i_id].Name_= "indices";		xr_vector<int>&
i_indices	= input[i_id].intVector_;	i_indices.reserve
(i_count_reserve);

        // Declare outputs
        xr_vector<NVMeshMender::VertexAttribute> 			output;
        output.push_back(NVMeshMender::VertexAttribute());	// position,
needed for mender output.push_back(NVMeshMender::VertexAttribute());	//
normal output.push_back(NVMeshMender::VertexAttribute());	// tangent
    ...
    int o_id=-1;
        output[++o_id].Name_= "position";	xr_vector<float>&
o_position	= output[o_id].floatVector_; output[++o_id].Name_= "normal";
xr_vector<float>&	o_normal	= output[o_id].floatVector_;
        output[++o_id].Name_= "tangent";	xr_vector<float>&
o_tangent	= output[o_id].floatVector_; output[++o_id].Name_= "binormal";
xr_vector<float>&	o_binormal	= output[o_id].floatVector_;
        output[++o_id].Name_= "tex0";		xr_vector<float>& o_tc =
output[o_id].floatVector_; output[++o_id].Name_= "c_w0_b0";
xr_vector<float>&	o_c_w0_b0	= output[o_id].floatVector_;
        output[++o_id].Name_= "w1_w2_w3";	xr_vector<float>&
o_w1_w2_w3	= output[o_id].floatVector_; output[++o_id].Name_= "b1_b2_b3";
xr_vector<float>&	o_b1_b2_b3	= output[o_id].floatVector_;
        output[++o_id].Name_= "indices";	xr_vector<int>& o_indices
= output[o_id].intVector_;

    // fill inputs (verts&indices)
    for (SkelVertIt vert_it=m_Verts.begin(); vert_it!=m_Verts.end(); vert_it++){
        SSkelVert	&iV = *vert_it;
        i_position.push_back(iV.offs.x);
i_position.push_back(iV.offs.y);
i_position.push_back(iV.offs.z); i_normal.push_back	(iV.norm.x);
i_normal.push_back	(iV.norm.y); i_normal.push_back	(iV.norm.z);
        i_tc.push_back		(iV.uv.x);
i_tc.push_back		(iV.uv.y); i_tc.push_back		(0); u32 sz
= iV.bones.size(); i_c_w0_b0.push_back	(*(float*)&sz);
i_c_w0_b0.push_back(iV.bones[0].w); i_c_w0_b0.push_back
(*(float*)&iV.bones[0].id); u32 pt_w			= i_w1_w2_w3.size(); u32
pt_b			= i_b1_b2_b3.size(); i_w1_w2_w3.push_back(0.f);
i_w1_w2_w3.push_back(0.f);	i_w1_w2_w3.push_back(0.f);
        i_b1_b2_b3.push_back(0.f);	i_b1_b2_b3.push_back(0.f);
i_b1_b2_b3.push_back(0.f); if (iV.bones.size()>1){ i_w1_w2_w3[pt_w+0]
= iV.bones[1].w; i_b1_b2_b3[pt_b+0]			=
(*(float*)&iV.bones[1].id); if (iV.bones.size()>2){
...
// outputs attributes
                deg2rad(75.f),
// tangent space smooth angle
                0,
// no texture matrix applied to my texture coordinates
                NVMeshMender::FixTangents,
// fix degenerate bases & texture mirroring
                NVMeshMender::DontFixCylindricalTexGen,		// handle
cylindrically mapped textures via vertex duplication
                NVMeshMender::DontWeightNormalsByFaceSize	// weigh vertex
normals by the triangle's size
                ))
        {
                Debug.fatal	(DEBUG_INFO,"NVMeshMender failed
(%s)",mender.GetLastError().c_str());
        }
    ...
    for (u32 v_idx=0; v_idx!=v_cnt; v_idx++){
        ...
        oV.offs.set	(o_position[v_idx*3+0],	o_position[v_idx*3+1],
o_position[v_idx*3+2]); oV.norm.set	(o_normal[v_idx*3+0],
o_normal[v_idx*3+1],	o_normal[v_idx*3+2]); oV.tang.set
(o_tangent[v_idx*3+0],	o_tangent[v_idx*3+1],	o_tangent[v_idx*3+2]);
        oV.binorm.set(o_binormal[v_idx*3+0],o_binormal[v_idx*3+1],
o_binormal[v_idx*3+2]); oV.uv.set	(o_tc[v_idx*3+0],
o_tc[v_idx*3+1]); oV.bones.resize	(*(u32*)&o_c_w0_b0 [v_idx*3+0]);

        oV.bones[0].w	= o_c_w0_b0 [v_idx*3+1];oV.bones[0].id =
(u16)(*(u32*)&o_c_w0_b0 	[v_idx*3+2]); if (oV.bones.size()>1){
            ...
            if (oV.bones.size()>2){
                ...
                if (oV.bones.size()>3){
                    ...
                    oV.bones[3].id	= (u16)(*(u32*)&o_b1_b2_b3
[v_idx*3+2]);
                }
            }
        }
...
void CExportSkeleton::SSplit::OptimizeTextureCoordinates() {
  ...
  if ((Tsize.x > 32) || (Tsize.y > 32))
    Msg("#!Surface [T:'%s', S:'%s'] has UV tiled more than 32 times.",
        *m_Texture, *m_Shader);
  {
    // 2. Recalc UV mapping
    for (u32 v_idx = 0; v_idx != v_cnt; v_idx++) {
...
IC void set_vertex(SSkelVert &out_vertex, const SSkelVert &in_old_vertex,
...
void CExportSkeleton::SSplit::CalculateTB() {
  ...
  fill_mender_input(m_Verts, m_Faces, mender_in_out_verts,
                    mender_in_out_indices);

  MeshMender mender;
  if (!mender.Mend(mender_in_out_verts, mender_in_out_indices,
                   mender_mapping_out_to_in_vert, 1, 0.5, 0.5, 0.0f,
  ...
  retrive_data_from_mender_otput(m_Verts, m_Faces, mender_in_out_verts,
                                 mender_in_out_indices,
