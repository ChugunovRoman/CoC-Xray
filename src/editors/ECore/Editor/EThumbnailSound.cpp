...
//------------------------------------------------------------------------------
ESoundThumbnail::ESoundThumbnail(LPCSTR src_name, bool bLoad)
...
bool ESoundThumbnail::Load(LPCSTR src_name, LPCSTR path) {
  ...
  strcpy(
      fn,
      EFS.ChangeFileExt(src_name ? src_name : m_Name.c_str(), ".thm").c_str());
  if (path)
...
void ESoundThumbnail::OnMaxDistChange(PropValue *sender) {
  ...
  VERIFY(S);
  bool bChanged = false;
  for (PropItem::PropValueIt it = S->Values().begin(); S->Values().end() != it;
       ++it) {
    ...
    if (*CV->value > CV->lim_mx) {
      ELog.DlgMsg(
          mtInformation,
          "'Max AI Dist' <= 'Max Dist'. 'Max AI Dist' will be clamped.");
      bChanged = true;
      *CV->value = CV->lim_mx;
    }
...
void ESoundThumbnail::FillInfo(PropItemVec &items) {
  PHelper().CreateCaption(items, "Quality",
                          AnsiString().sprintf("%3.2f", m_fQuality).c_str());
  PHelper().CreateCaption(items, "Min Dist",
                          AnsiString().sprintf("%3.2f", m_fMinDist).c_str());
  PHelper().CreateCaption(items, "Max Dist",
                          AnsiString().sprintf("%3.2f", m_fMaxDist).c_str());
  PHelper().CreateCaption(items, "Max AI Dist",
                          AnsiString().sprintf("%3.2f", m_fMaxAIDist).c_str());
  PHelper().CreateCaption(items, "Base Volume",
                          AnsiString().sprintf("%3.2f", m_fBaseVolume).c_str());
  PHelper().CreateCaption(items, "Game Type",
