...
void CEditorRenderDevice::_Create(IReader *F) {
  ...
  //	pSystemFont					=
  // xr_new<CGameFont>("hud_font_medium");
}
...
void __fastcall CEditorRenderDevice::Resize(int w, int h) {
  ...
  mProject.build_projection(deg2rad(fFOV), fASPECT, m_Camera.m_Znear,
                            m_Camera.m_Zfar);
...
void CEditorRenderDevice::FrameMove() {
  ...
  fTimeDelta = 0.1f * fTimeDelta +
               0.9f * fPreviousFrameTime; // smooth random system activity -
                                          // worst case ~7% error
  if (fTimeDelta > .1f)
  ...
  dwTimeGlobal =
      TimerGlobal
          .GetElapsed_ms(); // u32((qTime*u64(1000))/CPU::cycles_per_second);
...
void CEditorRenderDevice::DP(D3DPRIMITIVETYPE pt, ref_geom geom, u32 vBase,
                             u32 pc) {
...
void CEditorRenderDevice::DIP(D3DPRIMITIVETYPE pt, ref_geom geom, u32 baseV,
                              u32 startV, u32 countV, u32 startI, u32 PC) {
  ref_shader S = m_CurrentShader ? m_CurrentShader : m_WireShader;
  u32 dwRequired = S->E[0]->passes.size();
  RCache.set_Geometry(geom);
  for (u32 dwPass = 0; dwPass < dwRequired; dwPass++) {
