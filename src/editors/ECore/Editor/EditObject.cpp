...
void CSurface::CreateImageData() {
  ...
  ImageLib.LoadTextureData(*m_ImageData->name, m_ImageData->layers.back(),
                           m_ImageData->w, m_ImageData->h);
}
void CSurface::RemoveImageData() { xr_delete(m_ImageData); }
#endif

CEditableObject::CEditableObject(LPCSTR name)
    : m_physics_shell(0), m_object_xform(0) {
  m_LibName = name;
...
CEditableMesh *CEditableObject::FindMeshByName(const char *name,
                                               CEditableMesh *Ignore) {
...
void CEditableObject::PrepareOGFDesc(ogf_desc &desc) {
  ...
  desc.build_name =
      strconcat(sizeof(tmp), tmp, "\\\\", Core.CompName, "\\", Core.UserName);
...
void CEditableObject::GetFaceWorld(const Fmatrix &parent, CEditableMesh *M,
                                   int idx, Fvector *verts) {
  const Fvector *PT[3];
  M->GetFacePT(idx, PT);
...
bool CEditableObject::Validate() {
  ...
  for (SurfaceIt s_it = m_Surfaces.begin(); s_it != m_Surfaces.end(); s_it++)
    if (false == (*s_it)->Validate()) {
      Msg("!Invalid surface found: Object [%s], Surface [%s].", GetName(),
          (*s_it)->_Name());
      bRes = false;
    }
  for (EditMeshIt m_def = m_Meshes.begin(); m_def != m_Meshes.end(); m_def++)
    if (false == (*m_def)->Validate()) {
      Msg("!Invalid mesh found: Object [%s], Mesh [%s].", m_LibName.c_str(),
          (*m_def)->Name().c_str());
      bRes = false;
    }
  return bRes;
