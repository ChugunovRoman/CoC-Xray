...
void CEditableMesh::RebuildVMaps() {
  ...
  // refs copy to new
  {
    ...
    for (VMRefsIt o_it = m_VMRefs.begin(), n_it = nVMRefs.begin();
         o_it != m_VMRefs.end(); o_it++, n_it++) {
      ...
      for (u32 pt_id = 0; pt_id < pts_cnt; pt_id++) {
        ...
        case vmtUV: {
          ...
          if (-1 == pm) { // point map
            pm = F.pv[k].vmref;
            int vm_idx =
                FindVMapByName(nVMaps, vmap->name.c_str(), vmap->type, false);
            if (-1 == vm_idx) {
              nVMaps.push_back(
                  xr_new<st_VMap>(vmap->name.c_str(), vmap->type, false));
              vm_idx = nVMaps.size() - 1;
            }
            st_VMap *nVMap = nVMaps[vm_idx];

            //						int uv_idx =
            // FindSimilarUV(nVMap,vmap->getUV(pt_it->index)); 						if
            // (uv_idx==-1){ 							uv_idx	= nVMap->size();
            ...
            int vm_idx =
                FindVMapByName(nVMaps, vmap->name.c_str(), vmap->type, true);
            if (-1 == vm_idx) {
              nVMaps.push_back(
                  xr_new<st_VMap>(vmap->name.c_str(), vmap->type, true));
              vm_idx = nVMaps.size() - 1;
            }
            st_VMap *nVMapPM = nVMaps[vm_idx];

            //						int uv_idx =
            // FindSimilarUV(nVMapPM,vmap->getUV(pt_it->index)); 						if
            // (uv_idx==-1){ 							uv_idx	= nVMapPM->size();
...
bool CEditableMesh::OptimizeFace(st_Face &face) {
  ...
  if ((mface[0] == mface[1]) || (mface[1] == mface[2]) ||
      (mface[0] == mface[2])) {
...
void CEditableMesh::OptimizeMesh(BOOL NoOpt) {
  if (!NoOpt) {
    ...
    for (int x = 0; x < MX + 1; x++)
      for (int y = 0; y < MY + 1; y++)
        for (int z = 0; z < MZ + 1; z++)
          VM[x][y][z].clear();
    VMscale.set(m_Box.max.x - m_Box.min.x + EPS_S,
                m_Box.max.y - m_Box.min.y + EPS_S,
                m_Box.max.z - m_Box.min.z + EPS_S);
    ...
    Memory.mem_copy(m_Vertices, &*m_NewPoints.begin(),
                    m_NewPoints.size() * sizeof(Fvector));
    ...
    if (i_del_face) {
      ...
      for (u32 dk = 0; dk < m_FaceCount; ++dk) {
        if (faces_mark[dk]) {
          for (SurfFacesPairIt plp_it = m_SurfFaces.begin();
               plp_it != m_SurfFaces.end(); ++plp_it) {
