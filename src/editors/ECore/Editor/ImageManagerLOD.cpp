...
/*
IC void SetCamera(float angle, const Fvector& C, float height, float radius,
float dist)
{
    Fvector 	D;
    Fvector 	hpb;
...
IC void CopyLODImage(U32Vec& src, U32Vec& dest, u32 src_w, u32 src_h, int id,
int pitch)
{
...
void CImageManager::CreateLODTexture(const Fbox& bb, U32Vec& tgt_data, u32
tgt_w, u32 tgt_h, int samples)
{
        U32Vec pixels;
    ...
    u32 cc						=
EPrefs->scene_clear_color; EPrefs.scene_clear_color 	= 	0x0000000;
    psDeviceFlags.zero			();
    ...
    for (int frame=0; frame<samples; frame++){
        ...
        // copy LOD to final
                ...
                CopyMemory
(tgt_data.begin()+y*pitch+frame*tgt_w,pixels.begin()+y*tgt_w,tgt_w*sizeof(u32));
    }
...
BOOL GetPointColor(SPickQuery::SResult *R, u32 &alpha, u32 &color) {
  ...
  /*
  //	float
  filter_core[3][3]={{0.0125,0.0125,0.0125},{0.0125,0.9,0.0125},{0.0125,0.0125,0.0125}};
  //	float
  filter_core[3][3]={{0.0625,0.0625,0.0625},{0.0625,0.5,0.0625},{0.0625,0.0625,0.0625}};
      ...
      for (int y=-1; y<=1; y++){
          ...
          if (v<0||v>=(int)surf->m_ImageData->h) continue;
              for (int x=-1; x<=1; x++){
                  ...
                  Fvector4 	c;
              color		=
  surf->m_ImageData->layers.back()[v*surf->m_ImageData->w+u]; float f 	=
  filter_core[x+1][y+1]; c.set
  (f*color_get_R(color),f*color_get_G(color),f*color_get_B(color),f*color_get_A(color));
              C.add		(c);
              cnt++;
          }
      }
...
ICF static void simple_hemi_callback(float x, float y, float z, float E,
                                     LPVOID P) {
  BLVec *dst = (BLVec *)P;
  SBuildLight T;
  T.energy = E;
...
void CreateLODSamples(const Fbox &bbox, U32Vec &tgt_data, u32 tgt_w,
                      u32 tgt_h) {
  ...
  for (int frame = 0; frame < LOD_SAMPLE_COUNT; frame++) {
    ...
    EDevice.MakeScreenshot(s_pixels, tgt_w * lod_ss_quality,
                           tgt_h * lod_ss_quality);
    d_pixels.resize(tgt_w * tgt_h);
    imf_Process(d_pixels.begin(), tgt_w, tgt_h, s_pixels.begin(),
                tgt_w * lod_ss_quality, tgt_h * lod_ss_quality, imf_box);
    // copy LOD to final
    for (u32 y = 0; y < tgt_h; y++)
      CopyMemory(tgt_data.begin() + y * pitch + frame * tgt_w,
                 d_pixels.begin() + y * tgt_w, tgt_w * sizeof(u32));
  }
...
void CImageManager::CreateLODTexture(CEditableObject *OBJECT,
                                     U32Vec &lod_pixels, U32Vec &nm_pixels,
                                     u32 tgt_w, u32 tgt_h, int _samples,
                                     int quality) {
  // build hemi light
  BLVec simple_hemi;
  // fill simple hemi
  ...
  SPBItem *PB =
      UI->ProgressStart(LOD_SAMPLE_COUNT * LOD_IMAGE_SIZE, OBJECT->GetName());
  ...
  // preload textures
  for (SurfaceIt surf_it = OBJECT->Surfaces().begin();
       surf_it != OBJECT->Surfaces().end(); surf_it++) {
  ...
  for (u32 sample_idx = 0; sample_idx < LOD_SAMPLE_COUNT; sample_idx++) {
    ...
    for (s32 iH = 0; iH < LOD_IMAGE_SIZE; iH++) {
      ...
      for (s32 iW = 0; iW < LOD_IMAGE_SIZE; iW++) {
        ...
        u32 pixel =
            (LOD_IMAGE_SIZE - iH - 1) * LOD_SAMPLE_COUNT * LOD_IMAGE_SIZE +
            LOD_IMAGE_SIZE * sample_idx + iW;
        ...
        TT.Start();
        SPickQuery PQ;
        for (s32 iiH = -LOD_CALC_SAMPLES_LIM; iiH <= LOD_CALC_SAMPLES_LIM;
             iiH++) {
          float dY = iiH * (dH / LOD_CALC_SAMPLES);
          for (s32 iiW = -LOD_CALC_SAMPLES_LIM; iiW <= LOD_CALC_SAMPLES_LIM;
               iiW++) {
            ...
            if (PQ.r_count()) {
              ...
              for (s32 k = PQ.r_count() - 1; k >= 0; k--) {
                ...
                ///(uC);
                ///.							if
                ///(Cn.a>=(200.f/255.f)) C.lerp(C,Cn,Cn.a);
              }
              float n_mag = N.magnitude();
              if (!fis_zero(n_mag, EPS))
                n_vec.push_back(N.div(n_mag));
              ///.		                c_vec.push_back
              ///(Fvector4().set(C.r,C.g,C.b,C.a));
            }
          }
        }
        tN += TT.GetElapsed_sec();
        ///.			Fvector4 cC			= {0,0,0,0};
        ///.			for (Fvector4It c_it=c_vec.begin();
        /// c_it!=c_vec.end(); c_it++) .				cC.add
        ///(*c_it); .			cC.div
        ///(c_vec.size()); .			cC.mul
        ///(255.f); .			tgt_c				=
        ...
        N.add(0.5f);
        N.mul(255.f);
        tgt_n = color_rgba(iFloor(N.x), iFloor(N.y), iFloor(N.z),
                           color_get_A(tgt_c));
        ...
        for (Fvector4It pt_it = sample_pt_vec.begin();
             pt_it != sample_pt_vec.end(); pt_it++) {
          float avg_transp = 0.f;
          for (BLIt it = simple_hemi.begin(); it != simple_hemi.end(); it++) {
            TT1.Start();
            Fvector start;
            start.mad(Fvector().set(pt_it->x, pt_it->y, pt_it->z),
                      it->light.direction, -dR);
  ...
  ImageLib.ApplyBorders(lod_pixels, LOD_IMAGE_SIZE * LOD_SAMPLE_COUNT,
                        LOD_IMAGE_SIZE);
  ImageLib.ApplyBorders(nm_pixels, LOD_IMAGE_SIZE * LOD_SAMPLE_COUNT,
                        LOD_IMAGE_SIZE);
  ImageLib.ApplyBorders(hemi_tmp, LOD_IMAGE_SIZE * LOD_SAMPLE_COUNT,
                        LOD_IMAGE_SIZE);
  // fill alpha to N-channel (HEMI)
  for (int px_idx = 0; px_idx < int(nm_pixels.size()); px_idx++)
    nm_pixels[px_idx] =
        subst_alpha(nm_pixels[px_idx], color_get_R(hemi_tmp[px_idx]));
...
//
//------------------------------------------------------------------------------
void CImageManager::CreateLODTexture(CEditableObject *OBJECT, LPCSTR tex_name,
                                     u32 tgt_w, u32 tgt_h, int samples, int age,
                                     int quality) {
