...
class ECORE_API EDetail : public CDetail {
  ...
  struct EVertexIn : public fvfVertexIn {
    ...
    void remapUV(const fvfVertexIn &src, const Fvector2 &offs,
  ...
  /*
          struct fvfVertexIn{
                  Fvector 		P;
                  float			u,v;
                                          fvfVertexIn	(const Fvector& _P,
     float _u, float _v){P.set(_P); u=_u; v=_v;}; void			set
     (fvfVertexIn& src){P.set(src.P); u=src.u; v=src.v;}; void
     set			(const Fvector& _P, float _u, float _v){P.set(_P);
     u=_u; v=_v;}; BOOL			similar		(fvfVertexIn& V)
          {
...
public:
  ...
  bool Load(IReader &);
  void Save(IWriter &);
  void Export(IWriter &, LPCSTR tex_name, const Fvector2 &offs,
              const Fvector2 &scale, bool rot);
