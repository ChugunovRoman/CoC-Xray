...
extern "C" __declspec(dllimport) int DXTCompress(LPCSTR out_name, u8 *raw_data,
                                                 u8 *ext_data, u32 w, u32 h,
                                                 u32 pitch,
...
bool Surface_Load(LPCSTR full_name, U32Vec &data, u32 &w, u32 &h, u32 &a) {
  ...
  if (ext == ".tga") {
    ...
    if (!IsValidSize(w, h))
      ELog.Msg(mtError, "Texture (%s) - invalid size: [%d, %d]", full_name, w,
               h);
    return true;
  } else {
    ...
    if (bm) {
      ...
      data.resize(w * h);
      for (int y = h - 1; y >= 0; y--)
        CopyMemory(data.begin() + (h - y - 1) * w, FreeImage_GetScanLine(bm, y),
                   w4);
...
void CImageManager::MakeThumbnailImage(ETextureThumbnail *THM, u32 *data, u32 w,
  ...
  imf_Process(THM->m_Pixels.begin(), THUMB_WIDTH, THUMB_HEIGHT, data,
              THM->_Width(), THM->_Height(), imf_box);
  THM->VFlip();
}
...
void CImageManager::CreateTextureThumbnail(ETextureThumbnail *THM,
                                           const AnsiString &src_name,
                                           LPCSTR initial, bool bSetDefParam) {
  ...
  if (bSetDefParam) {
    ...
    THM->m_TexParams.fmt =
        (a) ? STextureParams::tfDXT3 : STextureParams::tfDXT1;
    if ((h * 6) == w) {
...
//------------------------------------------------------------------------------
void CImageManager::CreateGameTexture(LPCSTR src_name,
                                      ETextureThumbnail *thumb) {
...
//------------------------------------------------------------------------------
bool CImageManager::MakeGameTexture(LPCSTR game_name, u32 *data,
                                    const STextureParams &tp) {
  VerifyPath(game_name);
  // fill texture params
  // compress
  u32 w4 = tp.width * 4;
  int res = DXTCompress(game_name, (u8 *)data, 0, tp.width, tp.height, w4,
                        (STextureParams *)&tp, 4);
  if (1 != res) {
...
bool CImageManager::MakeGameTexture(ETextureThumbnail *THM, LPCSTR game_name,
    ...
    if (NM_THM->_Format().type == STextureParams::ttNormalMap) {
      if (NM_THM->_Format().fmt == STextureParams::tfRGBA) {
        ...
        if (!LoadTextureData(e_name, ext_data, _w, _h)) {
          ELog.DlgMsg(mtError, "Can't load special normal map texture '%s'.",
                      e_name);
          e_res = false;
        } else if ((_w != w) || (_h != h)) {
          ELog.DlgMsg(
        ...
        ELog.DlgMsg(mtError,
                    "Invalid special normal map format '%s'. It should be '32 "
                    "bit (8:8:8:8)'",
                    e_name);
        e_res = false;
      }
  ...
  int res = DXTCompress(game_name, (u8 *)load_data,
                        (u8 *)(ext_data.empty() ? 0 : ext_data.begin()), w, h,
                        w4, &THM->m_TexParams, 4);
  if (1 != res) {
...
bool CImageManager::LoadTextureData(LPCSTR src_name, U32Vec &data, u32 &w,
  ...
  string_path fn;
  //.	FS.update_path
  //(fn,_textures_,ChangeFileExt(src_name,".tga").c_str());
...
void CImageManager::SynchronizeTextures(
    bool sync_thm, bool sync_game, bool bForceGame, FS_FileSet *source_list,
  ...
  for (; it != _E; it++) {
    ...
    FS.update_path(fn, _textures_,
                   EFS.ChangeFileExt(base_name, ".tga").c_str());
    if (!FS.exist(fn))
      continue;

    FS_FileSetIt th = M_THUM.find(base_name);
    bool bThm = ((th == M_THUM.end()) ||
    ...
    if (bProgress)
      pb->Inc(bUpdated ? xr_string(base_name +
                                   (bFailed ? " - FAILED" : " - UPDATED."))
                             .c_str()
                       : base_name.c_str(),
              bUpdated);
    ...
    if (bUpdated) {
      ...
      FS.update_path(tga_fn, _textures_,
                     EFS.ChangeFileExt(base_name, ".tga").c_str());
      FS.update_path(thm_fn, _game_textures_,
                     EFS.ChangeFileExt(base_name, ".thm").c_str());
      FS.update_path(dds_fn, _game_textures_,
                     EFS.ChangeFileExt(base_name, ".dds").c_str());
      if (bForceBaseAge) {
...
void CImageManager::ChangeFileAgeTo(FS_FileSet* tgt_map, int age)
{
    ...
    // change
        SPBItem* pb=0;
    if (M_BASE->size()>1) pb	= UI->ProgressStart(M_BASE->size(),"Change
textures age..."); FS_FileSetIt it			= M_BASE->begin();
        FS_FileSetIt _E 			= M_BASE->end();
        for (; it!=_E; it++){
        xr_string base_name	= EFS.ChangeFileExt(it->name,"");
xr_strlwr(base_name); xr_string	tga_fn,thm_fn,dds_fn; FS.update_path
(tga_fn,_textures_,		EFS.ChangeFileExt(base_name,".tga").c_str());
        FS.update_path			(thm_fn,_textures_,
EFS.ChangeFileExt(base_name,".thm").c_str()); FS.update_path
(dds_fn,_game_textures_,EFS.ChangeFileExt(base_name,".dds").c_str());
...
BOOL CImageManager::CreateOBJThumbnail(LPCSTR tex_name, CEditableObject *obj,
  ...
  //	u32 cc						=
  // EPrefs.scene_clear_color; 	EPrefs.scene_clear_color 	=
  // 0x00333333;
...
BOOL CImageManager::CreateSmallerCubeMap(LPCSTR src_name, LPCSTR dst_name) {
  ...
  if (Surface_Load(full_name, data, wf, h, a)) {
    ...
    if (!btwIsPow2(h) || (h * 6 != wf) || (wf < sm_wf) || (h < sm_h)) {
      ELog.Msg(mtError, "Texture '%s' - invalid size: [%d, %d]", src_name, wf,
               h);
      return FALSE;
    }
    // generate smaller
    ...
    CTimer T;
    T.Start();
    ETOOLS::SimplifyCubeMap(data.begin(), w, h, sm_data.begin(), sm_w, sm_h,
                            16.f, pb_callback, PB);
