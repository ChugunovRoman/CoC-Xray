  ...
  if (EFS.GetOpenName("$app_root$", fn, false, NULL, 0)) {
    if (Surface_Load(fn.c_str(), image_data, image_w, image_h, image_a)) {
      ...
      for (int y = 0; y < image_h / 2; y++, y2--) {
        ...
        CopyMemory(image_data.begin() + y2 * image_w,
                   image_data.begin() + y * image_w, sz_ln);
      ...
      if (fn.find(_mark) != fn.npos) {
        ...
        int cnt = sscanf(_str, "_[%f, %f]-[%f, %f]", &map_bb.min.x,
                         &map_bb.min.y, &map_bb.max.x, &map_bb.max.y);
        //                "ss_andy_05-08-07_15-24-11_#ai_test_[-150.000,
        //                -100.000]-[52.927, 50.000].tga"
        if (cnt != 4) {
          map_bb.min.x = 0.0f;
          map_bb.min.y = 0.0f;
          map_bb.max.x = 100.0f;
...
void TTMinimapEditor::ApplyPoints(bool to_controls) {
  ...
  string512 buff;
  sprintf(buff, "bound_rect = %3.3f, %3.3f, %3.3f, %3.3f", map_bb.min.x,
          map_bb.min.y, map_bb.max.x, map_bb.max.y);
...
void DrawThumbnail(TCanvas *pCanvas, TRect &r, U32Vec &data,
                   bool bDrawWithAlpha, int _w, int _h) {
...
void __fastcall TTMinimapEditor::imgPanelPaint(TObject *Sender) {
  ...
  //.    FHelper.DrawThumbnail(imgPanel->Canvas->Handle, R,
  //&*image_data.begin(), image_w, image_h);
  ...
  */
  float kw =
      (map_bb_loaded.max.x - map_bb_loaded.min.x) / (float)image_draw_size.x;
  float kh =
      (map_bb_loaded.max.y - map_bb_loaded.min.y) / (float)image_draw_size.y;
  ...
  FHelper.DrawThumbnail(imgPanel->Canvas->Handle, R, &*image_data.begin(),
                        image_w, image_h);
...
void __fastcall TTMinimapEditor::imgPanelResize(TObject *Sender) {
  ...
  } else { // align to height
    image_draw_size.set(iFloor(imgPanel->Height * _k_img), imgPanel->Height);
  }
  VERIFY(image_draw_size.x <= imgPanel->Width &&
         image_draw_size.y <= imgPanel->Height);
}
//---------------------------------------------------------------------------
