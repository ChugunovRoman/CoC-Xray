...
bool TUI::CreateMailslot() {
  AnsiString slot_name =
      AnsiString("\\\\.\\mailslot\\") + AnsiString(UI->EditorName());
  hMailSlot =
      ::CreateMailslot(slot_name.c_str(),
...
void TUI::CheckMailslot() {
  ...
  while (cMessage != 0) // retrieve all messages
  {
    ...
    lpszBuffer[0] = '\0';
    fResult =
        ReadFile(hMailSlot, lpszBuffer, cbMessage, &cbRead, (LPOVERLAPPED)NULL);
    if (!fResult) {
...
void TUI::OnReceiveMail(LPCSTR msg) {
  ...
  if (cnt) {
    ...
    if (cnt > 1)
      _GetItems(msg, 1, cnt, p[1], ' ');
    if (p[0] == "exit") {
      ELog.DlgMsg(mtInformation, "'%s EDITOR': Critical update!",
                  AnsiString(UI->EditorName()).UpperCase().c_str());
      while (1) {
      ...
      ELog.Msg(mtInformation, "'%s EDITOR': Super critical update!",
               AnsiString(UI->EditorName()).UpperCase().c_str());
      ExecCommand(COMMAND_SAVE_BACKUP);
      ExecCommand(COMMAND_QUIT);
    } else if (p[0] == "info") {
      if (cnt > 1)
        ELog.DlgMsg(mtInformation, "'%s EDITOR': %s",
                    AnsiString(UI->EditorName()).UpperCase().c_str(),
                    p[1].c_str());
    } else if (p[0] == "error") {
      if (cnt > 1)
        ELog.DlgMsg(mtError, "'%s EDITOR': %s",
                    AnsiString(UI->EditorName()).UpperCase().c_str(),
                    p[1].c_str());
    }
  }
