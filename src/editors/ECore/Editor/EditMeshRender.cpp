  ...
  */
  ...
  for (SurfFacesPairIt sp_it = m_SurfFaces.begin(); sp_it != m_SurfFaces.end();
    ...
    do {
      ...
      R_CHK(HW.pDevice->CreateVertexBuffer(buf_size, D3DUSAGE_WRITEONLY, 0,
      ...
      v_cnt -= V_LIM;
      start_face += (_S->m_Flags.is(CSurface::sf2Sided)) ? rb.dwNumVertex / 6
                                                         : rb.dwNumVertex / 3;
    } while (v_cnt > 0);
    if (num_verts > 0)
...
void CEditableMesh::UnloadRenderBuffers() {
  if (m_RenderBuffers) {
    for (RBMapPairIt rbmp_it = m_RenderBuffers->begin();
         rbmp_it != m_RenderBuffers->end(); rbmp_it++) {
      for (RBVecIt rb_it = rbmp_it->second.begin();
           rb_it != rbmp_it->second.end(); rb_it++)
        if (rb_it->pGeom) {
...
void CEditableMesh::FillRenderBuffer(IntVec &face_lst, int start_face,
                                     int num_face, const CSurface *surf,
                                     LPBYTE &src_data) {
  ...
  for (int fl_i = start_face; fl_i < start_face + num_face; fl_i++) {
    ...
    for (int k = 0; k < 3; k++) {
      ...
      for (int t = 0; t < (int)dwTexCnt; t++) {
        VERIFY2((t + offs) < (int)m_VMRefs[fv.vmref].count,
        ...
        VERIFY2(vm_pt.vmap_index < int(m_VMaps.size()),
                "- VMap index out of range");
        ...
        data += sz;
        //                Msg("%3.2f,
        //                %3.2f",vmap->getUV(vm_pt.index).x,vmap->getUV(vm_pt.index).y);
      }
    }
      ...
      for (int k = 2; k >= 0; k--) {
        ...
        for (int t = 0; t < (int)dwTexCnt; t++) {
          ...
          VERIFY2(vm_pt.vmap_index < int(m_VMaps.size()),
                  "- VMap index out of range");
          st_VMap *vmap = m_VMaps[vm_pt.vmap_index];
          VERIFY2(vm_pt.index < vmap->size(),
                  "- VMap point index out of range");
          CopyMemory(data, &vmap->getUV(vm_pt.index), sz);
          data += sz;
...
void CEditableMesh::RenderList(const Fmatrix &parent, u32 color, bool bEdge,
                               IntVec &fl) {
  //	if (!m_Visible) return;
  ...
  for (IntIt dw_it = fl.begin(); dw_it != fl.end(); ++dw_it) {
    ...
    if (RB_cnt == MAX_VERT_COUNT) {
      DU_impl.DrawPrimitiveL(D3DPT_TRIANGLELIST, RB_cnt / 3, RB, RB_cnt, color,
                             true, false);
      RB_cnt = 0;
    }
  }
...
void CEditableMesh::RenderSelection(const Fmatrix &parent, CSurface *s,
                                    u32 color) {
  if (0 == m_RenderBuffers)
    GenerateRenderBuffers();
  //	if (!m_Visible) return;
  ...
  if (s) {
    ...
    for (RBMapPairIt p_it = m_RenderBuffers->begin();
         p_it != m_RenderBuffers->end(); p_it++) {
...
void CEditableMesh::RenderSkeleton(const Fmatrix &, CSurface *S) {
  ...
  svertRender *pv = (svertRender *)Stream->Lock(
      SKEL_MAX_FACE_COUNT * 3, m_Parent->vs_SkeletonGeom->vb_stride, vBase);
  Fvector P0, N0, P1, N1;
  ...
  for (IntIt i_it = face_lst.begin(); i_it != face_lst.end(); i_it++) {
    for (int k = 0; k < 3; k++, pv++) {
      ...
      for (u8 cnt = 1; cnt < (u8)SV.bones.size(); cnt++) {
        ...
        const Fmatrix &M =
            m_Parent->m_Bones[SV.bones[cnt].id]->_RenderTransform();
