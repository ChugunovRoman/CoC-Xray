...
static Fvector LOD_pos[4] = {{-1.0f + offs_x, 1.0f - offs_y, 0.0f},
...
bool CEditableObject::RayPick(float &dist, const Fvector &S, const Fvector &D,
                              const Fmatrix &inv_parent, SRayPickInfo *pinf) {
  bool picked = false;
...
void CEditableObject::RayQuery(const Fmatrix &parent, const Fmatrix &inv_parent,
                               SPickQuery &pinf) {
  for (EditMeshIt m = m_Meshes.begin(); m != m_Meshes.end(); m++)
    (*m)->RayQuery(parent, inv_parent, pinf);
}

void CEditableObject::BoxQuery(const Fmatrix &parent, const Fmatrix &inv_parent,
                               SPickQuery &pinf) {
  for (EditMeshIt m = m_Meshes.begin(); m != m_Meshes.end(); m++)
...
#ifdef _EDITOR
bool CEditableObject::FrustumPick(const CFrustum &frustum,
...
bool CEditableObject::BoxPick(CCustomObject *obj, const Fbox &box,
...
void CEditableObject::Render(const Fmatrix &parent, int priority,
                             bool strictB2F) {
  ...
  if (EPrefs->object_flags.is(epoDrawLOD) &&
    ...
    if (m_objectFlags.is(eoHOM)) {
      ...
      if (psDeviceFlags.is(rsEdgedFaces) && (1 == priority) &&
          (false == strictB2F))
        RenderEdge(parent);

      for (SurfaceIt s_it = m_Surfaces.begin(); s_it != m_Surfaces.end();
           s_it++) {
...
void CEditableObject::RenderEdge(const Fmatrix &parent, CEditableMesh *mesh,
                                 CSurface *surf, u32 color) {
  if (!(m_LoadState.is(LS_RBUFFERS)))
    DefferedLoadRP();
...
void CEditableObject::RenderSelection(const Fmatrix &parent,
                                      CEditableMesh *mesh, CSurface *surf,
                                      u32 color) {
  if (!(m_LoadState.is(LS_RBUFFERS)))
    DefferedLoadRP();
...
IC static void CalculateLODTC(int frame, int w_cnt, int h_cnt, Fvector2 &lt,
                              Fvector2 &rb) {
  Fvector2 ts;
...
void CEditableObject::GetLODFrame(int frame, Fvector p[4], Fvector2 t[4],
                                  const Fmatrix *parent) {
...
void CEditableObject::RenderLOD(const Fmatrix &parent) {
  ...
  for (int frame = 0; frame < LOD_SAMPLE_COUNT; frame++) {
    float angle =
        angle_normalize(frame * (PI_MUL_2 / float(LOD_SAMPLE_COUNT)) + HPB.x);
...
void CEditableObject::DefferedLoadRP() {
  ...
  // skeleton
  if (IsSkeleton())
    vs_SkeletonGeom.create(FVF_SV, RCache.Vertex.Buffer(),
                           RCache.Index.Buffer());
...
// bool CEditableObject::PrepareOGF(IWriter& F, u8 infl, bool gen_tb,
// CEditableMesh* mesh)
//{
//	return
// IsSkeleton()?PrepareSkeletonOGF(F,infl):PrepareRigidOGF(F,gen_tb,mesh);
//}
//
// bool CEditableObject::PrepareRigidOGF(IWriter& F, bool gen_tb, CEditableMesh*
// mesh)
//{
...
bool CEditableObject::CheckShaderCompatible() {
  ...
  for (SurfaceIt s_it = m_Surfaces.begin(); s_it != m_Surfaces.end(); s_it++) {
    ...
    if (!B || !C) {
      ELog.Msg(mtError,
               "Object '%s': invalid or missing shader [E:'%s', C:'%s']",
               GetName(), (*s_it)->_ShaderName(), (*s_it)->_ShaderXRLCName());
      bRes = false;
    } else {
      if (!BE(B->canBeLMAPped(), !C->flags.bLIGHT_Vertex)) {
        ELog.Msg(mtError,
                 "Object '%s': engine shader '%s' non compatible with compiler "
                 "shader '%s'",
                 GetName(), (*s_it)->_ShaderName(), (*s_it)->_ShaderXRLCName());
        bRes = false;
      }
...
void CEditableObject::RenameBone(CBone *bone, LPCSTR new_name) {
  ...
  for (SMotionIt s_it = m_SMotions.begin(); s_it != m_SMotions.end(); ++s_it) {
    CSMotion *M = *s_it;
    for (BoneMotionIt bm_it = M->BoneMotions().begin();
         bm_it != M->BoneMotions().end(); ++bm_it) {
