...
extern "C" int ogg_enc(const char *in_fn, const char *out_fn, float quality,
                       void *comment, int size);
//------------------------------------------------------------------------------
...
int CSoundManager::GetSounds(FS_FileSet &files, BOOL bFolders) {
  return FS.file_list(
      files, _sounds_,
      (bFolders ? FS_ListFolders : 0) | FS_ListFiles | FS_ClampExt, "*.wav");
}

int CSoundManager::GetGameSounds(FS_FileSet &files) {
  return FS.file_list(files, _game_sounds_, FS_ListFiles | FS_ClampExt,
                      "*.ogg");
}
...
int CSoundManager::GetSoundEnvs(AStringVec &items) {
  /*
      ...
      if (Lib){
              for (SoundEnvironment_LIB::SE_IT it=Lib->Library().begin();
     it!=Lib->Library().end(); it++) items.push_back(*(*it)->name);
      }
          return items.size();
  */
  return 0;
...
void CSoundManager::OnFrame() {
  ...
  Sound->update(EDevice.m_Camera.GetPosition(), EDevice.m_Camera.GetDirection(),
                EDevice.m_Camera.GetNormal());
}
...
int CSoundManager::GetLocalNewSounds(FS_FileSet &files) {
  return FS.file_list(files, _import_, FS_ListFiles | FS_RootOnly | FS_ClampExt,
                      "*.wav");
}
...
void CSoundManager::SafeCopyLocalToServer(FS_FileSet& files)
{
    ...
    FS_FileSetIt it	= files.begin();
        ...
        // copy sources
                fn 				=
EFS.ChangeFileExt(it->first,".wav"); src_name 		= p_import	+ fn;
                UpdateFileName	(fn);
                dest_name 		= p_sounds	+ fn;
        ...
        FS.set_file_age
(dest_name.c_str(),FS.get_file_age(src_name.c_str())); EFS.WriteAccessLog
(dest_name.c_str(),"Replace"); EFS.MarkFile		(src_name.c_str(),true);
    }
}
*/
//------------------------------------------------------------------------------
// создает тхм
//------------------------------------------------------------------------------
void CSoundManager::CreateSoundThumbnail(ESoundThumbnail *THM,
...
void CSoundManager::MakeGameSound(ESoundThumbnail *THM, LPCSTR src_name,
                                  LPCSTR game_name) {
...
//------------------------------------------------------------------------------
void CSoundManager::SynchronizeSounds(bool sync_thm, bool sync_game,
                                      bool bForceGame, FS_FileSet *source_list,
  ...
  for (; it != it_e; ++it) {
    ...
    if (bProgress)
      pb->Inc(bUpdated ? xr_string(base_name + " - UPDATED.").c_str() : "",
              bUpdated);

    if (bUpdated) {
      string_path wav_fn, thm_fn, ogg_fn;
      FS.update_path(wav_fn, _sounds_,
                     EFS.ChangeFileExt(base_name, ".wav").c_str());
      FS.update_path(thm_fn, _sounds_,
                     EFS.ChangeFileExt(base_name, ".thm").c_str());
      FS.update_path(ogg_fn, _game_sounds_,
                     EFS.ChangeFileExt(base_name, ".ogg").c_str());
...
void CSoundManager::CleanupSounds() {
  ...
  SPBItem *pb = UI->ProgressStart(M_GAME_DEL.size() + M_THM_DEL.size(),
                                  "Cleanup sounds...");
  ...
  for (; it != it_e; ++it) {
    ...
    xr_strlwr(base_name);
    string_path fn;
    FS.update_path(fn, _game_sounds_,
                   EFS.ChangeFileExt(base_name, ".ogg").c_str());
...
sounds age..."); FS_FileSetIt it			= M_BASE->begin();
        FS_FileSetIt _E 			= M_BASE->end();
        for (; it!=_E; it++){
        xr_string base_name	= EFS.ChangeFileExt(it->first,"");
xr_strlwr(base_name); xr_string	wav_fn,thm_fn,ogg_fn; FS.update_path
(wav_fn,_sounds_,		EFS.ChangeFileExt(base_name,".wav").c_str());
        FS.update_path			(thm_fn,_sounds_,
EFS.ChangeFileExt(base_name,".thm").c_str()); FS.update_path
(ogg_fn,_game_sounds_,	EFS.ChangeFileExt(base_name,".ogg").c_str());
