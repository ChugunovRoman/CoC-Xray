...
bool CEditableObject::LoadBoneData(IReader &F) {
  ...
  if (!load_bones.empty()) {
    for (BoneIt b_it = m_Bones.begin(); b_it != m_Bones.end(); b_it++) {
      CBone *B = *b_it;
      BoneIt n_it = std::find_if(load_bones.begin(), load_bones.end(),
                                 fBoneNameEQ(B->Name()));
  ...
  if (F.find_chunk(EOBJ_CHUNK_BONEPARTS2)) {
    ...
    for (BPIt bp_it = m_BoneParts.begin(); bp_it != m_BoneParts.end();
         bp_it++) {
      F.r_stringZ(buf);
      bp_it->alias = buf;
      bp_it->bones.resize(F.r_u32());
      for (RStringVecIt s_it = bp_it->bones.begin(); s_it != bp_it->bones.end();
           s_it++)
        F.r_stringZ(*s_it);
    }
...
void CEditableObject::RenderBones(const Fmatrix &parent) {
  if (IsSkeleton()) {
    ...
    for (BoneIt b_it = lst.begin(); b_it != lst.end(); b_it++) {
      ...
      Fvector p1 = M.c;
      u32 c_joint = (*b_it)->flags.is(CBone::flSelected)
                        ? color_bone_sel_color
      ...
      */
      ...
      if (EPrefs->object_flags.is(epoDrawBoneAxis)) {
        Fmatrix mat;
        mat.mul(parent, M);
        DU_impl.DrawObjectAxis(mat, 0.03f,
                               (*b_it)->flags.is(CBone::flSelected));
      }
      ...
      if (EPrefs->object_flags.is(epoDrawBoneShapes)) {
        ...
        if ((*b_it)->shape.Valid()) {
          ...
          case SBoneShape::stCylinder:
            DU_impl.DrawCylinder(mat, (*b_it)->shape.cylinder.m_center,
                                 (*b_it)->shape.cylinder.m_direction,
                                 (*b_it)->shape.cylinder.m_height,
                                 (*b_it)->shape.cylinder.m_radius, c, c, TRUE,
                                 TRUE);
            break;
          }
        }
...
CBone *CEditableObject::PickBone(const Fvector &S, const Fvector &D,
...
BOOL SphereValid(FvectorVec &geom, Fsphere &test) {
  if (!f_valid(test.P.x) || !f_valid(test.R)) {
    Msg("*** Attention ***: invalid sphere: %f,%f,%f - %f", test.P.x, test.P.y,
        test.P.z, test.R);
  }
...
void ComputeCylinder(Fcylinder &C, Fobb &B, FvectorVec &V) {
  ...
  //    vI					=
  //    pow(2*M_PI*max_rI*hI+2*M_PI*_sqr(max_rI),3/2)/vI; vJ
  //    = pow(2*M_PI*max_rJ*hJ+2*M_PI*_sqr(max_rJ),3/2)/vJ; vK
  //    = pow(2*M_PI*max_rK*hK+2*M_PI*_sqr(max_rK),3/2)/vK;
  // pow(area,(3/2))/volume
  // 2*Pi*R*H+2*Pi*R*R
  ...
  /*
      ...
      Mgc::Cylinder CYL	= Mgc::ContCylinder(V.size(), (const Mgc::Vector3*)
     V.begin()); B.m_center.set		(CYL.Center()); B.m_direction.set
     (CYL.Direction()); B.m_height			= CYL.Height();
    ...
    for (u32 f_id = 0; f_id != MESH->GetFCount(); f_id++) {
      for (int k = 0; k < 3; k++) {
        ...
        if (!bFound)
          P.push_back(p);
        //		        if (sv.bone1!=BI_NONE)
        // bone_points[sv.bone1].push_back(sv.offs1);
      }
    }
