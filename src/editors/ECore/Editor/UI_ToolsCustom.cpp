...
CToolCustom::CToolCustom() {
  m_bReady = false;
  m_Action = etaSelect;
  m_Settings.assign(etfNormalAlign | etfGSnap | etfOSnap | etfMTSnap |
                    etfVSnap | etfASnap | etfMSnap);
  m_Axis = etAxisZX;
  fFogness = 0.9f;
...
bool __fastcall CToolCustom::MouseStart(TShiftState Shift) {
  ...
  if (m_Action == etaMove && m_pAxisMoveObject) {
    ...
    dist = 10000;
    m_pAxisMoveObject->RayPick(dist, start_point, start_dir, inv_parent,
                               &pinfo);
    if (pinfo.e_mesh) {
...
void CToolCustom::Render() {
  ...
  AnsiString temp;
  int cnt = 0;
  for (SDebugDraw::PointIt vit = m_DebugDraw.m_Points.begin();
       vit != m_DebugDraw.m_Points.end(); ++vit) {
    LPCSTR s = NULL;
    if (vit->i) {
  ...
  EDevice.SetShader(EDevice.m_SelectionShader);
  cnt = 0;
  for (SDebugDraw::LineIt eit = m_DebugDraw.m_Lines.begin();
       eit != m_DebugDraw.m_Lines.end(); eit++) {
    if (eit->i)
      temp.sprintf("L: %d", cnt++);
    DU_impl.dbgDrawEdge(eit->p[0], eit->p[1], eit->c,
                        eit->i ? temp.c_str() : "");
  }
  EDevice.SetShader(EDevice.m_SelectionShader);
  cnt = 0;
  for (SDebugDraw::FaceIt fwit = m_DebugDraw.m_WireFaces.begin();
       fwit != m_DebugDraw.m_WireFaces.end(); fwit++) {
    if (fwit->i)
      temp.sprintf("F: %d", cnt++);
    DU_impl.dbgDrawFace(fwit->p[0], fwit->p[1], fwit->p[2], fwit->c,
                        fwit->i ? temp.c_str() : "");
  }
  cnt = 0;
  if (!m_DebugDraw.m_SolidFaces.empty()) {
    ...
    for (SDebugDraw::FaceIt fsit = m_DebugDraw.m_SolidFaces.begin();
         fsit != m_DebugDraw.m_SolidFaces.end(); fsit++)
      DU_impl.DD_DrawFace_push(fsit->p[0], fsit->p[1], fsit->p[2], fsit->c);
    DU_impl.DD_DrawFace_end();
  }
  EDevice.SetShader(EDevice.m_SelectionShader);
  cnt = 0;
  for (SDebugDraw::OBBVecIt oit = m_DebugDraw.m_OBB.begin();
       oit != m_DebugDraw.m_OBB.end(); oit++) {
  ...
  if (m_pAxisMoveObject && GetSelectionPosition(m_axis_xform)) {
    for (SurfaceIt s_it = m_pAxisMoveObject->Surfaces().begin();
         s_it != m_pAxisMoveObject->Surfaces().end(); ++s_it) {
