...
u16 CSkeletonCollectorPacked::VPack(SSkelVert &V) {
  ...
  if (0xffffffff == P) {
    ...
    u32 ixE, iyE, izE;
    ixE =
        iFloor(float(V.offs.x + m_VMeps.x - m_VMmin.x) / m_VMscale.x * clpSMX);
    iyE =
        iFloor(float(V.offs.y + m_VMeps.y - m_VMmin.y) / m_VMscale.y * clpSMY);
    izE =
        iFloor(float(V.offs.z + m_VMeps.z - m_VMmin.z) / m_VMscale.z * clpSMZ);
...
CSkeletonCollectorPacked::CSkeletonCollectorPacked(const Fbox &_bb,
                                                   int apx_vertices,
                                                   int apx_faces) {
  Fbox bb;
  bb.set(_bb);
  bb.grow(EPS_L);
  // Params
  m_VMscale.set(bb.max.x - bb.min.x + EPS, bb.max.y - bb.min.y + EPS,
                bb.max.z - bb.min.z + EPS);
  m_VMmin.set(bb.min).sub(EPS);
  m_VMeps.set(m_VMscale.x / clpSMX / 2, m_VMscale.y / clpSMY / 2,
              m_VMscale.z / clpSMZ / 2);
...
CExportSkeleton::SSplit::SSplit(CSurface *surf, const Fbox &bb, u16 part)
    : CSkeletonCollectorPacked(bb) {
  //.	m_b2Link	= FALSE;
...
void CExportSkeleton::SSplit::Save(IWriter &F) {
  ...
  /*
          ...
          for (SkelFaceIt f_it=m_Faces.begin(); f_it!=m_Faces.end(); f_it++){
              ...
              AnsiString 		tmp;
              tmp.sprintf		("f %d %d
     %d",pF.v[0]+1,pF.v[2]+1,pF.v[1]+1); W->w_string		(tmp.c_str());
          }
          // normals
...
void CExportSkeleton::SSplit::MakeProgressive() {
  VIPM_Init();
  for (SkelVertIt vert_it = m_Verts.begin(); vert_it != m_Verts.end();
       vert_it++)
...
void CExportSkeleton::SSplit::MakeStripify() {
  //	int ccc 	= xrSimulate
  //((u16*)&m_Faces.front(),m_Faces.size()*3,24); 	Log("SRC:",ccc);
  // alternative stripification - faces
  {
    DWORD *remap = xr_alloc<DWORD>(m_Faces.size());
    HRESULT rhr = D3DXOptimizeFaces(&m_Faces.front(), m_Faces.size(),
                                    m_Verts.size(), FALSE, remap);
    R_CHK(rhr);
    ...
    //	    int ccc 	= xrSimulate
    //((u16*)&m_Faces.front(),m_Faces.size()*3,24); 		Log("X:",ccc);
  }
  // alternative stripification - vertices
  {
    DWORD *remap = xr_alloc<DWORD>(m_Verts.size());
    HRESULT rhr = D3DXOptimizeVertices(&m_Faces.front(), m_Faces.size(),
                                       m_Verts.size(), FALSE, remap);
    R_CHK(rhr);
    ...
    //	    int ccc 	= xrSimulate
    //((u16*)&m_Faces.front(),m_Faces.size()*3,24); 		Log("Y:",ccc);
  }
}
...
void ComputeOBB_WML(Fobb &B, FvectorVec &V) {
  ...
  {
    Wml::Box3<float> BOX;
    Wml::MinBox3<float> mb(V.size(), (const Wml::Vector3<float> *)&V.front(),
                           BOX);
...
int CExportSkeletonCustom::FindSplit(shared_str shader, shared_str texture,
                                     u16 part_id) {
  for (SplitIt it = m_Splits.begin(); it != m_Splits.end(); it++)
    if (it->m_Shader.equal(shader) && it->m_Texture.equal(texture) &&
        (it->m_PartID == part_id))
...
IC void BuildGroups(CBone *B, U16Vec &tgt, u16 id, u16 &last_id) {
  if (B->IK_data.ik_flags.is(SJointIKData::flBreakable))
    id = ++last_id;
  tgt[B->SelfID] = id;
  for (BoneIt bone_it = B->children.begin(); bone_it != B->children.end();
       bone_it++)
...
bool CExportSkeleton::PrepareGeometry(u8 influence) {
...
#ifdef _EDITOR
  SPBItem *pb = UI->ProgressStart(5 + m_Source->MeshCount() * 2 +
                                      m_Source->SurfaceCount(),
  ...
  CBone *root = 0;
  for (BoneIt bone_it = m_Source->FirstBone(); bone_it != m_Source->LastBone();
       bone_it++) {
    CBone *B = *bone_it;
    if (B->IK_data.ik_flags.is(SJointIKData::flBreakable))
      bBreakable = true;
  ...
  for (U16It uit = bone_brk_parts.begin(); uit != bone_brk_parts.end(); uit++) {
    Msg("Bone: %s - Part: %d",
        *m_Source->GetBone(uit - bone_brk_parts.begin())->Name(), *uit);
  }
  ...
  for (EditMeshIt mesh_it = m_Source->FirstMesh();
       mesh_it != m_Source->LastMesh(); mesh_it++) {
    if (!bRes)
      break;
    ...
    for (SurfFacesPairIt sp_it = MESH->m_SurfFaces.begin();
      ...
      for (IntIt f_it = face_lst.begin(); f_it != face_lst.end(); f_it++) {
...

        {
          ...
          u32 link_type = _max(MESH->m_SVertices[f_idx * 3 + 0].bones.size(),
                               MESH->m_SVertices[f_idx * 3 + 1].bones.size());
          link_type =
              _max(link_type, MESH->m_SVertices[f_idx * 3 + 2].bones.size());
          ...
          for (int k = 0; k < 3; k++) {
            ...
            if (link_type == 1) {
              ...
              {
                v[k].set(sv.offs, sv.norm, sv.uv, (u8)sv.bones.size(),
                         sv.bones.begin());
          ...
          if (bBreakable) {
            ...
            for (; tit != tmp_bone_lst.end(); tit++)
              if (bone_brk_part != bone_brk_parts[*tit]) {
                ELog.Msg(mtError, "Can't export object as breakable. Object "
                                  "have N-Link face(s).");
                bRes = false;
              }
          }
          // find split
          int mtl_idx =
              FindSplit(surf->m_ShaderName, surf->m_Texture, bone_brk_part);
          if (mtl_idx < 0) {
          ...
          cur_split.m_SkeletonLinkType =
              _max(link_type, cur_split.m_SkeletonLinkType);

          cur_split.m_UsedBones.insert(cur_split.m_UsedBones.end(),
                                       tmp_bone_lst.begin(),
                                       tmp_bone_lst.end());
  ...
  for (int k = 0; k < (int)m_Splits.size(); k++) {
    ...
    if (bRes) {
      if (!m_Splits[k].valid()) {
        ELog.Msg(mtError, "Empty split found (Shader/Texture: %s/%s). Removed.",
        ...
        U16It ne =
            std::unique(split.m_UsedBones.begin(), split.m_UsedBones.end());
        split.m_UsedBones.erase(ne, split.m_UsedBones.end());
        Msg(" - Split %d: [Bones: %d, Links: %d, Faces: %d, Verts: %d, BrPart: "
            "%d, Shader/Texture: '%s'/'%s']",
            k, split.m_UsedBones.size(), split.m_SkeletonLinkType,
            split.getTS(), split.getVS(), split.m_PartID, *m_Splits[k].m_Shader,
...
bool CExportSkeleton::ExportGeometry(IWriter &F, u8 infl) {
...
#ifdef _EDITOR
  SPBItem *pb =
      UI->ProgressStart(3 + m_Splits.size(), "..Export skeleton geometry");
  ...
  for (SplitIt split_it = m_Splits.begin(); split_it != m_Splits.end();
    ...
    for (SkelVertIt sv_it = lst.begin(); sv_it != lst.end(); sv_it++) {
      ...
      bones[sv_it->bones[0].id]->_RITransform().transform_tiny(
          bone_points[sv_it->bones[0].id].back());
    }
#ifdef _EDITOR
    pb->Inc();
#endif
  ...
  for (bone_it = m_Source->FirstBone(); bone_it != m_Source->LastBone();
       ++bone_it, ++bone_idx)
...
bool CExportSkeleton::ExportMotionKeys(IWriter &F) {
...
#ifdef _EDITOR
  SPBItem *pb = UI->ProgressStart(1 + m_Source->SMotionCount(),
  ...
  mRotate.setHPB(m_Source->a_vRotate.y, m_Source->a_vRotate.x,
                 m_Source->a_vRotate.z);
  mTranslate.translate(m_Source->a_vPosition);
  mGT.mul(mTranslate, mRotate);

  for (SMotionIt motion_it = m_Source->FirstSMotion();
       motion_it != m_Source->LastSMotion(); motion_it++, smot++) {
    CSMotion *cur_motion = *motion_it;

    if (cur_motion->m_Flags.test(esmRootMover) &&
        !m_Source->AnimateRootObject(cur_motion)) {
      Msg("! %s has moveXform flag - but skeleton root has more than one child "
          "or has mesh! add special root bone please!",
          cur_motion->Name());
      return false;
    }
    //		if (motion->m_Flags.is(esmStopAtEnd)) Msg("%s -
    //%d",motion->Name(),motion->m_Flags.is(esmStopAtEnd));
    ...
    for (int frame = cur_motion->FrameStart(); frame <= cur_motion->FrameEnd();
         ++frame) {

      float t = (float)frame / cur_motion->FPS();
      int bone_id = 0;

      for (BoneIt b_it = b_lst.begin(); b_it != b_lst.end();
           ++b_it, ++bone_id) {
        Fvector T, R;
        ...
        if (bone_id == 0 && frame == (cur_motion->FrameEnd())) {
          Msg("motion [%s] end frame %f,%f,%f", cur_motion->Name(), T.x, T.y,
              T.z);
        }
      }
    ...
    for (itm_idx = 0; itm_idx < b_lst.size(); ++itm_idx) {
      ...
      if (g_force16BitTransformQuant || St.magnitude() > 1.5f) {
        ...
        Msg("animation [%s] is 16bit-transform (%f)m", cur_motion->Name(),
            St.magnitude());
      }
      ...
      // save
      F.w_u8(u8((t_present ? flTKeyPresent : 0) |
                (r_present ? 0 : flRKeyAbsent) |
...
bool CExportSkeleton::ExportMotionDefs(IWriter &F) {
  ...
  if (m_Source->m_SMotionRefs.size()) {
    ...
    for (SMotionIt motion_it = m_Source->FirstSMotion();
         motion_it != m_Source->LastSMotion(); ++motion_it) {
      CSMotion *motion = *motion_it;
      // verify
      if (!motion->m_Flags.is(esmFX)) {
        if (!((motion->m_BoneOrPart == BI_NONE) ||
              (motion->m_BoneOrPart < (int)bp_lst.size()))) {
          ELog.Msg(mtError, "Invalid Bone Part of motion: '%s'.",
                   motion->Name());
          bRes = false;
          continue;
        }
