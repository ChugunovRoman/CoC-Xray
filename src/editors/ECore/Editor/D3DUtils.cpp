...
#define DU_DRAW_SH_C(sh, c)                                                    \
  {                                                                            \
    ...
    RCache.set_c("tfactor", float(color_get_R(c)) / 255.f,                     \
                 float(color_get_G(c)) / 255.f, float(color_get_B(c)) / 255.f, \
...
void SPrimitiveBuffer::CreateFromData(D3DPRIMITIVETYPE _pt, u32 _p_cnt, u32 FVF,
                                      LPVOID vertices, u32 _v_cnt, u16 *indices,
                                      u32 _i_cnt) {
  ...
  R_CHK(HW.pDevice->CreateVertexBuffer(v_cnt * stride, D3DUSAGE_WRITEONLY, 0,
                                       D3DPOOL_MANAGED, &pVB, 0));
  u8 *bytes;
  ...
  R_CHK(pVB->Unlock());
  if (i_cnt) {
    R_CHK(HW.pDevice->CreateIndexBuffer(i_cnt * sizeof(u16), D3DUSAGE_WRITEONLY,
...
void CDrawUtilities::UpdateGrid(int number_of_cell, float square_size,
                                int subdiv) {
  m_GridPoints.clear();
  // grid
  int m_GridSubDiv[2];
...
void CDrawUtilities::OnDeviceCreate() {
  ...
  m_SolidCone.CreateFromData(
      D3DPT_TRIANGLELIST, DU_CONE_NUMFACES, D3DFVF_XYZ | D3DFVF_DIFFUSE,
      du_cone_vertices, DU_CONE_NUMVERTEX, du_cone_faces, DU_CONE_NUMFACES * 3);
  m_SolidSphere.CreateFromData(D3DPT_TRIANGLELIST, DU_SPHERE_NUMFACES,
                               D3DFVF_XYZ | D3DFVF_DIFFUSE, du_sphere_vertices,
                               DU_SPHERE_NUMVERTEX, du_sphere_faces,
                               DU_SPHERE_NUMFACES * 3);
  m_SolidSpherePart.CreateFromData(
      D3DPT_TRIANGLELIST, DU_SPHERE_PART_NUMFACES, D3DFVF_XYZ | D3DFVF_DIFFUSE,
      du_sphere_part_vertices, DU_SPHERE_PART_NUMVERTEX, du_sphere_part_faces,
      DU_SPHERE_PART_NUMFACES * 3);
  m_SolidCylinder.CreateFromData(D3DPT_TRIANGLELIST, DU_CYLINDER_NUMFACES,
                                 D3DFVF_XYZ | D3DFVF_DIFFUSE,
                                 du_cylinder_vertices, DU_CYLINDER_NUMVERTEX,
                                 du_cylinder_faces, DU_CYLINDER_NUMFACES * 3);
  m_WireBox.CreateFromData(D3DPT_LINELIST, DU_BOX_NUMLINES,
                           D3DFVF_XYZ | D3DFVF_DIFFUSE, du_box_vertices,
                           DU_BOX_NUMVERTEX, du_box_lines, DU_BOX_NUMLINES * 2);
  m_WireCone.CreateFromData(
      D3DPT_LINELIST, DU_CONE_NUMLINES, D3DFVF_XYZ | D3DFVF_DIFFUSE,
      du_cone_vertices, DU_CONE_NUMVERTEX, du_cone_lines, DU_CONE_NUMLINES * 2);
  m_WireSphere.CreateFromData(D3DPT_LINELIST, DU_SPHERE_NUMLINES,
                              D3DFVF_XYZ | D3DFVF_DIFFUSE, du_sphere_verticesl,
                              DU_SPHERE_NUMVERTEXL, du_sphere_lines,
  ...
  m_WireCylinder.CreateFromData(D3DPT_LINELIST, DU_CYLINDER_NUMLINES,
                                D3DFVF_XYZ | D3DFVF_DIFFUSE,
                                du_cylinder_vertices, DU_CYLINDER_NUMVERTEX,
...
void CDrawUtilities::DrawSpotLight(const Fvector &p, const Fvector &d,
                                   float range, float phi, u32 clr) {
  Fmatrix T;
  Fvector p1;
  float H, P;
  ...
  u32 vBase;
  FVF::L *pv =
      (FVF::L *)Stream->Lock(LINE_DIVISION * 2 + 2, vs_L->vb_stride, vBase);
...
void CDrawUtilities::DrawDirectionalLight(const Fvector &p, const Fvector &d,
                                          float radius, float range, u32 c) {
...
void CDrawUtilities::DrawFlag(const Fvector &p, float heading, float height,
                              float sz, float sz_fl, u32 clr,
                              BOOL bDrawEntity) {
  // fill VB
  _VertexStream *Stream = &RCache.Vertex;
  u32 vBase;
...
void CDrawUtilities::DrawRomboid(const Fvector &p, float r, u32 c) {
  static const u32 IL[24] = {0, 2, 2, 5, 0, 5, 3, 5, 3, 0, 4, 3,
                             4, 0, 4, 2, 1, 2, 1, 5, 1, 3, 1, 4};
  static const u32 IT[24] = {2, 4, 0, 4, 3, 0, 3, 5, 0, 5, 2, 0,
...
//------------------------------------------------------------------------------
void CDrawUtilities::DrawIdentCone(BOOL bSolid, BOOL bWire, u32 clr_s,
                                   u32 clr_w) {
  if (bWire) {
  ...
  if (bSolid) {
    DU_DRAW_SH_C(color_get_A(clr_s) >= 254 ? EDevice.m_WireShader
                                           : EDevice.m_SelectionShader,
                 clr_s);
    m_SolidCone.Render();
  }
...
void CDrawUtilities::DrawIdentSphere(BOOL bSolid, BOOL bWire, u32 clr_s,
                                     u32 clr_w) {
  if (bWire) {
    DU_DRAW_SH_C(EDevice.m_WireShader, clr_w);
    m_WireSphere.Render();
...
void CDrawUtilities::DrawIdentSpherePart(BOOL bSolid, BOOL bWire, u32 clr_s,
                                         u32 clr_w) {
  if (bWire) {
    DU_DRAW_SH_C(EDevice.m_WireShader, clr_w);
...
void CDrawUtilities::DrawIdentCylinder(BOOL bSolid, BOOL bWire, u32 clr_s,
                                       u32 clr_w) {
  if (bWire) {
    DU_DRAW_SH_C(EDevice.m_WireShader, clr_w);
...
void CDrawUtilities::DrawIdentBox(BOOL bSolid, BOOL bWire, u32 clr_s,
                                  u32 clr_w) {
  if (bWire) {
    DU_DRAW_SH_C(EDevice.m_WireShader, clr_w);
    m_WireBox.Render();
...
void CDrawUtilities::DrawLineSphere(const Fvector &p, float radius, u32 c,
                                    BOOL bCross) {
  // fill VB
  _VertexStream *Stream = &RCache.Vertex;
  u32 vBase;
...
//----------------------------------------------------
#ifdef _EDITOR
IC float _x2real(float x) { return (x + 1) * EDevice.m_RenderWidth_2; }
IC float _y2real(float y) { return (y + 1) * EDevice.m_RenderHeight_2; }
#else
IC float _x2real(float x) { return (x + 1) * EDevice.dwWidth * 0.5f; }
IC float _y2real(float y) { return (y + 1) * EDevice.dwHeight * 0.5f; }
#endif

void CDrawUtilities::dbgDrawPlacement(const Fvector &p, int sz, u32 clr,
                                      LPCSTR caption, u32 clr_font) {
  VERIFY(EDevice.b_is_Ready);
  Fvector c;
  float w = p.x * EDevice.mFullTransform._14 +
            p.y * EDevice.mFullTransform._24 +
...
void CDrawUtilities::dbgDrawEdge(const Fvector &p0, const Fvector &p1, u32 clr,
                                 LPCSTR caption) {
  dbgDrawPlacement(p0, 1, clr, caption);
  DrawCross(p0, 0.01f, 0.01f, 0.01f, 0.01f, 0.01f, 0.01f, clr, false);
...
void CDrawUtilities::dbgDrawFace(const Fvector &p0, const Fvector &p1,
                                 const Fvector &p2, u32 clr, LPCSTR caption) {
  dbgDrawPlacement(p0, 1, clr, caption);
  DrawCross(p0, 0.01f, 0.01f, 0.01f, 0.01f, 0.01f, 0.01f, clr, false);
  DrawCross(p1, 0.01f, 0.01f, 0.01f, 0.01f, 0.01f, 0.01f, clr, false);
...
//----------------------------------------------------
void CDrawUtilities::DrawSelectionBox(const Fvector &C, const Fvector &S,
                                      u32 *clr) {
...
void CDrawUtilities::DrawBox(const Fvector &offs, const Fvector &Size,
                             BOOL bSolid, BOOL bWire, u32 clr_s, u32 clr_w) {
  _VertexStream *Stream = &RCache.Vertex;
  if (bWire) {
    u32 vBase;
    FVF::L *pv =
        (FVF::L *)Stream->Lock(identboxwirecount, vs_L->vb_stride, vBase);
  ...
  if (bSolid) {
    u32 vBase;
    FVF::L *pv =
        (FVF::L *)Stream->Lock(DU_BOX_NUMVERTEX2, vs_L->vb_stride, vBase);
...
void CDrawUtilities::DrawOBB(const Fmatrix &parent, const Fobb &box, u32 clr_s,
                             u32 clr_w) {
  Fmatrix R, S, X;
  box.xform_get(R);
...
void CDrawUtilities::DrawAABB(const Fmatrix &parent, const Fvector &center,
                              const Fvector &size, u32 clr_s, u32 clr_w,
...
void CDrawUtilities::DrawAABB(const Fvector &p0, const Fvector &p1, u32 clr_s,
...
void CDrawUtilities::DrawSphere(const Fmatrix &parent, const Fvector &center,
                                float radius, u32 clr_s, u32 clr_w, BOOL bSolid,
                                BOOL bWire) {
  Fmatrix B;
...
void CDrawUtilities::DrawFace(const Fvector &p0, const Fvector &p1,
                              const Fvector &p2, u32 clr_s, u32 clr_w,
                              BOOL bSolid, BOOL bWire) {
  _VertexStream *Stream = &RCache.Vertex;
...
void CDrawUtilities::DD_DrawFace_begin(BOOL bWire) {
  ...
  m_DD_wire = bWire;
  m_DD_pv_start =
      (FVF::L *)RCache.Vertex.Lock(MAX_VERT_COUNT, vs_L->vb_stride, m_DD_base);
...
void CDrawUtilities::DD_DrawFace_push(const Fvector &p0, const Fvector &p1,
                                      const Fvector &p2, u32 clr) {
  m_DD_pv->set(p0, clr);
  m_DD_pv++;
  m_DD_pv->set(p1, clr);
  m_DD_pv++;
...
void CDrawUtilities::DrawCylinder(const Fmatrix &parent, const Fvector &center,
                                  const Fvector &dir, float height,
                                  float radius, u32 clr_s, u32 clr_w,
                                  BOOL bSolid, BOOL bWire) {
  Fmatrix mScale;
...
void CDrawUtilities::DrawCone(const Fmatrix &parent, const Fvector &apex,
                              const Fvector &dir, float height, float radius,
...
void CDrawUtilities::DrawPlane(const Fvector &p, const Fvector &n,
                               const Fvector2 &scale, u32 clr_s, u32 clr_w,
...
void CDrawUtilities::DrawPlane(const Fvector &center, const Fvector2 &scale,
                               const Fvector &rotate, u32 clr_s, u32 clr_w,
                               BOOL bCull, BOOL bSolid, BOOL bWire) {
  Fmatrix M;
...
void CDrawUtilities::DrawRectangle(const Fvector &o, const Fvector &u,
                                   const Fvector &v, u32 clr_s, u32 clr_w,
                                   BOOL bSolid, BOOL bWire) {
  _VertexStream *Stream = &RCache.Vertex;
...
void CDrawUtilities::DrawCross(const Fvector &p, float szx1, float szy1,
                               float szz1, float szx2, float szy2, float szz2,
                               u32 clr, BOOL bRot45) {
  _VertexStream *Stream = &RCache.Vertex;
  // actual rendering
  u32 vBase;
...
void CDrawUtilities::DrawAxis(const Fmatrix &T) {
  /*
      ...
      // colors
      c[0]=c[2]=c[4]=0x00222222; c[1]=0x00FF0000; c[3]=0x0000FF00;
     c[5]=0x000000FF;
...
void CDrawUtilities::DrawObjectAxis(const Fmatrix &T, float sz, BOOL sel) {
  ...
  _VertexStream *Stream = &RCache.Vertex;
  Fvector c, r, n, d;
  float w = T.c.x * EDevice.mFullTransform._14 +
            T.c.y * EDevice.mFullTransform._24 +
...
void CDrawUtilities::DrawPrimitiveL(D3DPRIMITIVETYPE pt, u32 pc,
                                    Fvector *vertices, int vc, u32 color,
...
void CDrawUtilities::DrawIndexedPrimitive(int pt, u32 pc, const Fvector &pos,
                                          const Fvector *vb, const u32 &vb_size,
                                          const u32 *ib, const u32 &ib_size,
                                          const u32 &clr_argb, float scale) {
  _VertexStream *Stream = &RCache.Vertex;
  _IndexStream *StreamI = &RCache.Index;
...
void CDrawUtilities::DrawPrimitiveTL(D3DPRIMITIVETYPE pt, u32 pc,
                                     FVF::TL *vertices, int vc, BOOL bCull,
                                     BOOL bCycle) {
  // fill VB
  _VertexStream *Stream = &RCache.Vertex;
  u32 vBase, dwNeed = (bCycle) ? vc + 1 : vc;
...
void CDrawUtilities::DrawPrimitiveLIT(D3DPRIMITIVETYPE pt, u32 pc,
                                      FVF::LIT *vertices, int vc, BOOL bCull,
                                      BOOL bCycle) {
  // fill VB
  _VertexStream *Stream = &RCache.Vertex;
  u32 vBase, dwNeed = (bCycle) ? vc + 1 : vc;
...
void CDrawUtilities::DrawLink(const Fvector &p0, const Fvector &p1, float sz,
                              u32 clr) {
  DrawLine(p1, p0, clr);
...
void CDrawUtilities::OutText(const Fvector &pos, LPCSTR text, u32 color,
                             u32 shadow_color) {
  Fvector p;
  float w = pos.x * EDevice.mFullTransform._14 +
            pos.y * EDevice.mFullTransform._24 +
