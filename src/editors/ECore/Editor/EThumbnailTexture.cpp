...
//------------------------------------------------------------------------------
ETextureThumbnail::ETextureThumbnail(LPCSTR src_name, bool bLoad)
...
int ETextureThumbnail::MemoryUsage() {
  ...
  FS.update_path(fn, _game_textures_,
                 EFS.ChangeFileExt(m_Name.c_str(), ".seq").c_str());
  if (FS.exist(fn)) {
    string128 buffer;
...
void ETextureThumbnail::Save(int age, LPCSTR path) {
  ...
  /*
          F.w_chunk		(THM_CHUNK_DATA |
     CFS_CompressMark,m_Pixels.begin(),m_Pixels.size()*sizeof(u32));
  */
...
void ETextureThumbnail::FillInfo(PropItemVec &items) {
  ...
  PHelper().CreateCaption(items, "Height",
                          shared_str().printf("%d", _Height()));
  PHelper().CreateCaption(items, "Alpha", _Alpha() ? "on" : "off");
}

BOOL ETextureThumbnail::similar(ETextureThumbnail *thm1,
...
void ETextureThumbnail::Draw(HDC hdc, const Irect &R) {
  if (0 == m_Pixels.size()) {
    ...
    if (!FS.exist(fn)) {
      ...
      if (!FS.exist(fn)) {
        ELog.Msg(mtError, "Can't make preview for texture '%s'.",
                 m_Name.c_str());
        return;
      }
    }
  ...
  if (Valid()) {
    ...
    if (_Width() > _Height()) {
      r.y2 -=
          r.height() - iFloor(r.height() * float(_Height()) / float(_Width()));
    } else {
      r.x2 -=
          r.width() - iFloor(r.width() * float(_Width()) / float(_Height()));
    }
