...
CCommandVar ExecCommand(u32 cmd, CCommandVar p1, CCommandVar p2) {
  ...
  if (bAllowLogCommands) {
    ...
    if (p1.IsString())
      sp1 = ((sp1.find("\n") == sp1.npos) && (sp1.find("\r") == sp1.npos))
                ? sp1
                : xr_string("...");
    if (p2.IsString())
      sp2 = ((sp2.find("\n") == sp2.npos) && (sp2.find("\r") == sp2.npos))
                ? sp2
                : xr_string("...");
...
void RegisterCommand(u32 cmd, SECommand *cmd_impl) {
  ...
  SECommand *&CMD = ECommands[cmd];
  if (CMD) {
    Msg("RegisterCommand: command '%s' overridden by command '%s'.", *CMD->desc,
        *cmd_impl->desc);
    xr_delete(CMD);
  }
  CMD = cmd_impl;
}
void RegisterSubCommand(SECommand *cmd_impl, LPCSTR desc, CCommandVar p0,
                        CCommandVar p1) {
  VERIFY(cmd_impl);
    ...
    if (CMD && CMD->editable) {
      for (u32 sub_cmd_idx = 0; sub_cmd_idx < CMD->sub_commands.size();
        ...
        if (SUB->p0.IsString() && SUB->p1.IsString())
          sprintf(tmp, "%d, \"%s\",\"%s\"", SUB->shortcut.hotkey,
                  xr_string(SUB->p0).c_str(), xr_string(SUB->p1).c_str());
        else if (SUB->p0.IsInteger() && SUB->p1.IsInteger())
          sprintf(tmp, "%d, %d,%d", SUB->shortcut.hotkey, u32(SUB->p0),
                  u32(SUB->p1));
        else if (SUB->p0.IsInteger() && SUB->p1.IsString())
          sprintf(tmp, "%d, %d,\"%s\"", SUB->shortcut.hotkey, u32(SUB->p0),
                  xr_string(SUB->p1).c_str());
        else if (SUB->p0.IsString() && SUB->p1.IsInteger())
          sprintf(tmp, "%d, \"%s\",%d", SUB->shortcut.hotkey,
                  xr_string(SUB->p0).c_str(), u32(SUB->p1));
...
CCommandVar TUI::CommandBreakLastOperation(CCommandVar p1, CCommandVar p2) {
  if (mrYes == ELog.DlgMsg(mtConfirmation, TMsgDlgButtons() << mbYes << mbNo,
                           "Are you sure to break current action?")) {
    NeedBreak();
    ...
    if (Tools->OnCreate()) {
      ...
      /*
                  if(bWeather && EPrefs->sWeather.size() )
                  {
                      ...
                      g_pGamePersistent->Environment().SetWeather(EPrefs->sWeather,
         true);
...
CCommandVar CommandSyncSounds(CCommandVar p1, CCommandVar p2) {
  if (ELog.DlgMsg(mtConfirmation, TMsgDlgButtons() << mbYes << mbNo,
                  "Are you sure to synchronize sounds?") == mrYes)
...
CCommandVar CommandRefreshTextures(CCommandVar p1, CCommandVar p2) {
  if (ELog.DlgMsg(mtConfirmation, TMsgDlgButtons() << mbYes << mbNo,
                  "Are you sure to synchronize textures?") == mrYes)
    ...
    if (!line.empty()) {
      ...
      if (CMD) {
        ...
        if (!sub_cmd_name.empty() && !SUB) {
          ELog.DlgMsg(mtError, "Can't find sub-command: '%s'",
                      sub_cmd_name.c_str());
          res = FALSE;
          break;
        }
...
CCommandVar CommandEditCommandList(CCommandVar _p1, CCommandVar _p2) {
  if (NULL == frmEditCommandList) {
    frmEditCommandList = TfrmText::CreateForm(
        sCommandListText, "Execute command list", 0, 0, "Run",
        OnRunExecuteListClick, OnCloseCommandListEditor);
    return TRUE;
  }
  return FALSE;
}
...
void TUI::RegisterCommands() {
  ...
  REGISTER_CMD_SE(COMMAND_USE_SIMULATE_POSITIONS, "Use Simulate Positions",
                  CommandUseSimulatePositions, true);

  REGISTER_SUB_CMD_SE(COMMAND_CHANGE_ACTION, "Change Action",
                      CommandChangeAction, false);
  ...
  REGISTER_CMD_SE(COMMAND_SHOW_PROPERTIES, "Show Properties",
                  CommandShowProperties, false);
  ...
  REGISTER_SUB_CMD_END;
  REGISTER_CMD_SE(COMMAND_MOVE_CAMERA_TO, "Scene\\Move Camera To",
                  CommandMoveCameraTo, false);
  REGISTER_CMD_SE(COMMAND_TOGGLE_RENDER_WIRE, "Toggle Wireframe",
                  CommandToggleRenderWire, false);
  REGISTER_CMD_C(COMMAND_RENDER_FOCUS, this, TUI::CommandRenderFocus);
  REGISTER_CMD_CE(COMMAND_BREAK_LAST_OPERATION, "Break Last Operation", this,
                  TUI::CommandBreakLastOperation, false);
  REGISTER_CMD_SE(COMMAND_TOGGLE_SAFE_RECT, "Toggle Safe Rect",
                  CommandToggleSafeRect, false);
  ...
  REGISTER_SUB_CMD_SE(COMMAND_GRID_SLOT_SIZE, "Change Grid Size",
                      CommandGridSlotSize, false);
