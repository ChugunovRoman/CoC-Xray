...
public:
  itterate_adjacents_params_dynamic(
      ...
      Fvector &_normal, const u32 *_SmoothGroups, const Fvector *_FaceNormals,
      const IntVec &_a_lst, const type_face *_Faces, u32 _FaceCount)
      :

        normal(_normal), m_SmoothGroups(_SmoothGroups),
        m_FaceNormals(_FaceNormals), a_lst(_a_lst), m_Faces(_Faces),
  ...
  IC const u32 current_adjacents_size() const { return u32(a_lst.size()); }

private:
  IC static bool has_same_edge(const type_face *F1, const type_face *F2,
                               u16 &F1_edge_index, u16 &F2_edge_index) {
    F1_edge_index = u16(-1);
    F2_edge_index = u16(-1);
  ...
  IC bool do_connect_faces(u32 start_face_idx, u32 test_face_idx,
    ...
    return do_connect_faces_by_faces_edge_flags(
        m_SmoothGroups[start_face_idx], m_SmoothGroups[test_face_idx],
        start_common_edge_idx, test_common_edge_idx);
  }
  ...
  IC bool add_adjacents(u32 start_face_adj_idx, u32 test_face_adj_idx) {
    ...
    if (has_same_edge(start_face, test_face, StartFace_common_edge_index,
      ...
      if ((start_face_idx == test_face_idx) ||
          do_connect_faces(start_face_idx, test_face_idx,
                           StartFace_common_edge_index,
                           TestFace_common_edge_index)) {
