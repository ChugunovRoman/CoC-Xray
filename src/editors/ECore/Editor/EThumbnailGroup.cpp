...
//------------------------------------------------------------------------------
EGroupThumbnail::EGroupThumbnail(LPCSTR src_name, bool bLoad)
    : EImageThumbnail(src_name, ETObject) {
  if (bLoad)
    Load();
}
//------------------------------------------------------------------------------
...
void EGroupThumbnail::CreateFromData(u32 *p, u32 w, u32 h,
                                     const SStringVec &lst) {
...
void EGroupThumbnail::Save(int age, LPCSTR path) {
  ...
  F.w_chunk(THM_CHUNK_DATA | CFS_CompressMark, m_Pixels.begin(),
            m_Pixels.size() * sizeof(u32));
...
void EGroupThumbnail::FillProp(PropItemVec &items) {
  PHelper().CreateCaption(items, "Objects\\Count",
                          AnsiString(objects.size()).c_str());
  for (SStringVecIt it = objects.begin(); it != objects.end(); it++)
    PHelper().CreateCaption(
        items,
        AnsiString().sprintf("Objects\\#%d", it - objects.begin()).c_str(),
        it->c_str());
}
//------------------------------------------------------------------------------
