...
void __fastcall TfrmSoundLib::FormCreate(TObject *Sender) {
  ...
  m_ItemProps->SetModifiedEvent(
      fastdelegate::bind<TOnModifiedEvent>(this, &TfrmSoundLib::OnModified));
  m_ItemList = TItemList::CreateForm(
      "Items", paItems, alClient,
      TItemList::
          ilMultiSelect /*|TItemList::ilEditMenu|TItemList::ilDragAllowed*/);
  m_ItemList->SetOnItemsFocusedEvent(fastdelegate::bind<TOnILItemsFocused>(
      this, &TfrmSoundLib::OnItemsFocused));
  TOnItemRemove on_remove;
  on_remove.bind(this, &TfrmSoundLib::RemoveSound);
  TOnItemRename on_rename;
  on_rename.bind(this, &TfrmSoundLib::RenameSound);
...
void TfrmSoundLib::OnModified() { m_ItemProps->RefreshForm(); }
//---------------------------------------------------------------------------
...
void __fastcall TfrmSoundLib::RegisterModifiedTHM() {
  if (m_ItemProps->IsModified()) {
    for (THMIt t_it = m_THM_Current.begin(); t_it != m_THM_Current.end();
         t_it++) {
      //.            (*t_it)->Save	(0,0);
...
void __fastcall TfrmSoundLib::OnControlClick(ButtonValue *V, bool &bModif,
                                             bool &bSafe) {
  switch (V->btn_num) {
  case 0:
    m_Snd.play(0, sm_2D);
  ...
  case 2: {
    bAutoPlay = !bAutoPlay;
    V->value[V->btn_num] =
        shared_str().printf("Auto (%s)", bAutoPlay ? "on" : "off");
  } break;
  }
  bModif = false;
}
...
void __fastcall TfrmSoundLib::OnControl2Click(ButtonValue *V, bool &bModif,
                                              bool &bSafe) {
  switch (V->btn_num) {
  case 0: {
    bAutoPlay = !bAutoPlay;
...
#define X_GRID 10
#define Y_GRID 5
void TfrmSoundLib::OnAttenuationDraw(CanvasValue *sender, void *_canvas,
                                     const Irect &_rect) {
  ...
  canvas->MoveTo(x0 + 0,
                 y0 + iFloor(float(h) - float(h) * 0.01f)); // snd cull = 0.01f
  ...
  for (int d = 1; d < w; d++) {
    ...
    //		float b =
    // m_Brightness/(m_Attenuation0+m_Attenuation1*R+m_Attenuation2*R*R);
    float bb = h - (h * b);
    int y = iFloor(y0 + bb);
...
void __stdcall TfrmSoundLib::OnAttClick(ButtonValue *V, bool &bModif,
                                        bool &bSafe) {
  bModif = true;
  ESoundThumbnail *thm = m_THM_Current.back();
  switch (V->btn_num) {
  ...
  case 1: {
    float dist = psSoundRolloff *
                 (thm->MaxDist() - (0.1f / 1.1f) * thm->MaxDist()) * 0.01f;
    thm->SetMinDist(dist);
  } break;
  }
}
...
void __fastcall TfrmSoundLib::OnItemsFocused(ListItemsVec &items) {
  ...
  if (m_THM_Current.size() == 1) {
    ...
    //		C->OnTestEqual.bind
    //(this,&TfrmSoundLib::OnPointDataTestEqual);
    B = PHelper().CreateButton(props, "Auto Att", "By Min,By Max",
                               ButtonValue::flFirstOnly);
    B->OnBtnClickEvent.bind(this, &TfrmSoundLib::OnAttClick);

    PHelper().CreateCaption(
        props, "File Length",
        shared_str().printf("%.2f Kb", float(size) / 1024.f));
    PHelper().CreateCaption(
        props, "Total Time",
        shared_str().printf("%.2f sec", float(time) / 1000.f));
    if (!m_Flags.is(flReadOnly)) {
      B = PHelper().CreateButton(props, "Control", "Play,Stop",
                                 ButtonValue::flFirstOnly);
      B->OnBtnClickEvent.bind(this, &TfrmSoundLib::OnControlClick);
    }
  }
  if (!m_Flags.is(flReadOnly)) {
    B = PHelper().CreateButton(props, "Auto Play", bAutoPlay ? "on" : "off",
                               ButtonValue::flFirstOnly);
    B->OnBtnClickEvent.bind(this, &TfrmSoundLib::OnControl2Click);
  }

  if (!m_Flags.is(flReadOnly) && m_THM_Current.size()) {
    B = PHelper().CreateButton(props, "MANAGE", "SyncCurrent",
                               ButtonValue::flFirstOnly);
...
void __fastcall TfrmSoundLib::OnSyncCurrentClick(ButtonValue *V, bool &bModif,
                                                 bool &bSafe) {
  //.
