...
public:
  ...
  IC BOOL IsStatic() {
    return !m_objectFlags.is(eoSoundOccluder) && !m_objectFlags.is(eoDynamic) &&
           !m_objectFlags.is(eoHOM) && !m_objectFlags.is(eoMultipleUsage);
  }
  IC BOOL IsMUStatic() {
    return !m_objectFlags.is(eoSoundOccluder) && !m_objectFlags.is(eoDynamic) &&
           !m_objectFlags.is(eoHOM) && m_objectFlags.is(eoMultipleUsage);
...
public:
  ...
  void GetLODFrame(int frame, Fvector p[4], Fvector2 t[4],
                   const Fmatrix *parent = 0);
  ...
  void GetBoneWorldTransform(u32 bone_idx, float t, CSMotion *motion,
                             Fmatrix &matrix);
  ...
  // statistics methods
  void GetFaceWorld(const Fmatrix &parent, CEditableMesh *M, int idx,
                    Fvector *verts);
  ...
  void RenderSelection(const Fmatrix &parent, CEditableMesh *m = 0,
                       CSurface *s = 0, u32 c = 0x40E64646);
  void RenderEdge(const Fmatrix &parent, CEditableMesh *m = 0, CSurface *s = 0,
                  u32 c = 0xFFC0C0C0);
  ...
  // pick methods
  bool RayPick(float &dist, const Fvector &S, const Fvector &D,
               const Fmatrix &inv_parent, SRayPickInfo *pinf = 0);
#ifdef _EDITOR
  ...
  void RayQuery(const Fmatrix &parent, const Fmatrix &inv_parent,
                SPickQuery &pinf);
  void BoxQuery(const Fmatrix &parent, const Fmatrix &inv_parent,
                SPickQuery &pinf);
  bool BoxPick(CCustomObject *obj, const Fbox &box, const Fmatrix &inv_parent,
...
private:
  ...
  virtual void Bone_GetAnimPos(Fmatrix &pos, u16 id, u8 channel_mask,
  ...
  // Main functionality
  virtual void CalculateBones(BOOL bForceExact = FALSE) {
