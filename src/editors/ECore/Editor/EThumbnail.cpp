...
/*
void DrawThumbnail(TCanvas* pCanvas, TRect& r, U32Vec& data, bool
bDrawWithAlpha, int _w = THUMB_WIDTH, int _h = THUMB_HEIGHT)
{
...
back.mul_rgb(255.f); for (int y = 0; y < pBitmap->Height; y++)
        {
            ...
            for (int x = 0; x < pBitmap->Width; x++){
                ...
                float inv_a	= 1.f-a;;
                u32 r		=
iFloor(float(color_get_R(src))*a+back.r*inv_a); u32 g		=
iFloor(float(color_get_G(src))*a+back.g*inv_a); u32 b		=
iFloor(float(color_get_B(src))*a+back.b*inv_a); ptr[x] 		=
color_rgba(r,g,b,0);
            }
        }
    }else{
...
void EImageThumbnail::VFlip() {
  ...
  for (int y = 0; y < THUMB_HEIGHT / 2; y++, y2--) {
    ...
    CopyMemory(m_Pixels.begin() + y2 * THUMB_WIDTH,
               m_Pixels.begin() + y * THUMB_WIDTH, sz_ln);
    CopyMemory(m_Pixels.begin() + y * THUMB_WIDTH, line, sz_ln);
  }
}

void EImageThumbnail::CreatePixels(u32 *p, u32 w, u32 h) {
  //	imf_filter	imf_box  imf_triangle  imf_bell  imf_b_spline
  // imf_lanczos3  imf_mitchell
  R_ASSERT(p && (w > 0) && (h > 0));
