...
void CEStats::Show(CGameFont *font) {
  ...
  if (EDevice.fTimeDelta > EPS_S) {
    ...
    if (RenderTOTAL.result > EPS_S) {
      fTPS =
          fInv * fTPS + fOne * float(DPS.polys) / (RenderTOTAL.result * 1000.f);
  ...
  if (psDeviceFlags.is(rsStatistic)) {
    ...
    F.OutNext("SH/T/M/C:     %d/%d/%d/%d", dwShader_Codes, dwShader_Textures,
              dwShader_Matrices, dwShader_Constants);
    ...
    //        F.OutNext	(" Sel Faces:   %d",
    //        dwLevelSelFaceCount); F.OutNext	(" Sel Verts:   %d",
  ...
  dwShader_Codes = dwShader_Textures = dwShader_Matrices = dwShader_Constants =
      0;
