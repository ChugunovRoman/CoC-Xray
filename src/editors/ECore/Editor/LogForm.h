...
class ECORE_API TfrmLog : public TForm {
  ...
  void __fastcall lbLogDrawItem(TWinControl *Control, int Index, TRect &Rect,
  ...
  void __fastcall lbLogKeyDown(TObject *Sender, WORD &Key, TShiftState Shift);
...
public: // User declarations
  ...
  static void __fastcall AddMessage(const AnsiString &msg) {
    AddMessage(mtCustom, msg);
  }
  static void __fastcall AddDlgMessage(TMsgDlgType mt, const AnsiString &msg);
  static void __fastcall AddDlgMessage(const AnsiString &msg) {
    AddDlgMessage(mtCustom, msg);
  }
  ...
  static void __fastcall ChangeVisible() {
    if (form->Visible)
      HideLog();
    else
      ShowLog();
