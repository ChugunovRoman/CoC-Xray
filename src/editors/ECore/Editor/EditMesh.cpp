...
void CEditableMesh::Clear() {
  ...
  VERIFY(m_FNormalsRefs == 0 && m_VNormalsRefs == 0 && m_AdjsRefs == 0 &&
         m_SVertRefs == 0);
...
void CEditableMesh::GenerateVNormals(const Fmatrix *parent_xform) {
  ...
  for (u32 f_i = 0; f_i < m_FaceCount; f_i++) {
    ...
    for (int k = 0; k < 3; k++) {
      ...
      typedef itterate_adjacents<itterate_adjacents_params_dynamic<st_FaceVert>>
          iterate_adj;
      iterate_adj::recurse_tri_params p(N, m_SmoothGroups, m_FaceNormals, a_lst,
                                        m_Faces, m_FaceCount);
      ...
      if (len > EPS_S) {
        ...
        //.                	Msg		("!Invalid smooth group found
        //(Maya type). Object: '%s'. Vertex: [%3.2f, %3.2f,
        //%3.2f]",m_Parent->GetName(),VPUSH(m_Vertices[m_Faces[f_i].pv[k].pindex]));

#ifdef _EDITOR
        if (parent_xform) {
          Fvector p0;
          parent_xform->transform_tiny(p0,
                                       m_Vertices[m_Faces[f_i].pv[k].pindex]);
          Tools->m_DebugDraw.AppendPoint(p0, 0xffff0000, true, true,
                                         "invalid vNORMAL");
        }
#endif
...
void CEditableMesh::GenerateVNormals(const Fmatrix* parent_xform)
{
    ...
    if (m_VertexNormals)		return;
        m_VertexNormals				=
xr_alloc<Fvector>(m_FaceCount*3);
        ...
        if (m_Flags.is(flSGMask))
        {
                for (u32 f_i=0; f_i<m_FaceCount; f_i++ )
                {
                        ...
                        for (int k=0; k<3; k++)
                        {
                                Fvector& N 	= m_VertexNormals[f_i*3+k];
                                IntVec&
a_lst=(*m_Adjs)[m_Faces[f_i].pv[k].pindex];
//
                                typedef itterate_adjacents<
itterate_adjacents_params_dynamic<st_FaceVert> > iterate_adj ;
                                iterate_adj::recurse_tri_params p( N,
m_SmoothGroups, m_FaceNormals, a_lst, m_Faces, m_FaceCount );
...
//
                                if (sg)
                                {
                                        ...
                                        VERIFY(a_lst.size());
                                        for (IntIt i_it=a_lst.begin();
i_it!=a_lst.end(); i_it++) if (sg&m_SmoothGroups[*i_it]) N.add
(m_FaceNormals[*i_it]);
                    ...
                    if (len>EPS_S)
                            ...
                            N.div	(len);
                                        ...
                                        {
//.                    	Msg		("!Invalid smooth group found (MAX
type). Object: '%s'. Vertex: [%3.2f, %3.2f,
%3.2f]",m_Parent->GetName(),VPUSH(m_Vertices[m_Faces[f_i].pv[k].pindex]));
#ifdef _EDITOR
                                                Fvector p0;
                        p0 = m_Vertices[m_Faces[f_i].pv[k].pindex];
                        Tools->m_DebugDraw.AppendPoint(p0, 0xffff0000, true,
true, "invalid vNORMAL"); #endif N.set	(m_FaceNormals[a_lst.front()]);
                    }
                                }else
                                {
                                        N.set		(FN);
...
i_it!=a_lst.end(); i_it++)
                    ...
                    if (len>EPS_S)
                            ...
                            N.div	(len);
                                        ...
                                        {
//.                    	Msg		("!Invalid smooth group found (Maya
type). Object: '%s'. Vertex: [%3.2f, %3.2f,
%3.2f]",m_Parent->GetName(),VPUSH(m_Vertices[m_Faces[f_i].pv[k].pindex]));
                                                ...
                                                if(parent_xform)
                        {
                                                Fvector p0;
                        parent_xform->transform_tiny(p0,
m_Vertices[m_Faces[f_i].pv[k].pindex]); Tools->m_DebugDraw.AppendPoint(p0,
0xffff0000, true, true, "invalid vNORMAL");
                        }
#endif
...
i_it!=a_lst.end(); i_it++)
     ...
     //               if (len>EPS_S){
            //                N.div	(len);
     //               }else{
     //               	Msg		("!Invalid smooth group found (No
smooth). Object: '%s'. Vertex: [%3.2f, %3.2f,
    ...
    for (int k = 0; k < 3; ++k) {
      ...
      for (u8 vmpt_id = 0; vmpt_id != vmpt_lst.count; ++vmpt_id) {
        ...
        if (VM.type == vmtWeight) {
          wb.push_back(st_WB(m_Parent->GetBoneIndexByWMap(VM.name.c_str()),
                             VM.getW(vmpt_lst.pts[vmpt_id].index)));

          if (wb.back().bone == BI_NONE) {
            ELog.DlgMsg(mtError, "Can't find bone assigned to weight map %s",
                        *VM.name);
...
CSurface *CEditableMesh::GetSurfaceByFaceID(u32 fid) {
  ...
  for (SurfFacesPairIt sp_it = m_SurfFaces.begin(); sp_it != m_SurfFaces.end();
       sp_it++) {
    ...
    if ((f_it != face_lst.end()) && (*f_it == (int)fid))
      return sp_it->first;
    //.		if
    //(std::find(face_lst.begin(),face_lst.end(),fid)!=face_lst.end()) return
    // sp_it->first;
  }
  return 0;
}
...
float CEditableMesh::CalculateSurfacePixelArea(CSurface *surf,
                                               bool bMatch2Sided) {
  SurfFacesPairIt sp_it = m_SurfFaces.find(surf);
  if (sp_it == m_SurfFaces.end())
    return 0;
  ...
  for (int k = 0; k < int(pol_lst.size()); k++) {
    ...
    e01.sub(Fvector().set(tc[1]->x, tc[1]->y, 0),
            Fvector().set(tc[0]->x, tc[0]->y, 0));
    e02.sub(Fvector().set(tc[2]->x, tc[2]->y, 0),
            Fvector().set(tc[0]->x, tc[0]->y, 0));
    area += c.crossproduct(e01, e02).magnitude() / 2.f;
  }
  if (bMatch2Sided && sp_it->first->m_Flags.is(CSurface::sf2Sided))
...
int CEditableMesh::FindVMapByName(VMapVec &vmaps, const char *name, u8 t,
                                  bool polymap) {
  for (VMapIt vm_it = vmaps.begin(); vm_it != vmaps.end(); vm_it++) {
    if (((*vm_it)->type == t) && (stricmp((*vm_it)->name.c_str(), name) == 0) &&
        (polymap == (*vm_it)->polymap))
