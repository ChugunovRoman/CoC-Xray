...
void CUI_Camera::Update(float dt) {
  if (m_bMoving) {
    ...
    if ((m_Style == csFreeFly) && (bLeftDn || bRightDn) &&
        !(bLeftDn && bRightDn)) {
      Fvector vmove;
      ...
      if (m_Shift.Contains(ssCtrl)) {
        float dist = UI->ZFar();
        if (Tools->RayPick(m_Position, down_dir,
                           dist)) // UI->R PickGround(pos,m_Position,dir,-1))
    ...
    if (dx || dy) {
      ...
      case csPlaneMove:
        if (m_Shift.Contains(ssLeft) && m_Shift.Contains(ssRight))
          Rotate(dx, dy);
        else if (m_Shift.Contains(ssLeft))
          Pan(dx, dy);
        else if (m_Shift.Contains(ssRight))
          Scale(dy);
        break;
      case csFreeFly:
        if (m_Shift.Contains(ssLeft) || m_Shift.Contains(ssRight))
...
void CUI_Camera::MouseRayFromPoint(Fvector &start, Fvector &direction,
                                   const Ivector2 &point) {
