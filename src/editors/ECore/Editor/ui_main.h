...
public:
  SPBItem(LPCSTR txt, LPCSTR inf, float mx)
      : text(txt), info(inf), max(mx), progress(0.f) {}
...
public:
  ...
  bool __fastcall IsMouseCaptured() {
  ...
  void EndEState(EEditorState st) {
    VERIFY(std::find(m_EditorState.begin(), m_EditorState.end(), st) !=
           m_EditorState.end());
    for (EStateIt it = m_EditorState.end() - 1; it >= m_EditorState.begin();
         it--)
      if (*it == st) {
  ...
  bool ContainEState(EEditorState st) {
    return std::find(m_EditorState.begin(), m_EditorState.end(), st) !=
           m_EditorState.end();
  }
