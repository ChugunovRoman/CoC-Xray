...
struct ECORE_API SSkelVert : public st_SVert {
  ...
  void set(const Fvector &_o, const Fvector &_n, const Fvector2 &_uv, u8 _w_cnt,
           const st_SVert::bone *b) {
    offs.set(_o);
    norm.set(_n);
    uv.set(_uv);
...
public:
  CSkeletonCollectorPacked(const Fbox &bb, int apx_vertices = 5000,
                           int apx_faces = 5000);
  bool check(SSkelFace &F) {
    ...
    for (SkelFaceIt f_it = m_Faces.begin(); f_it != m_Faces.end(); f_it++) {
      ...
      if ((f_it->v[0] == F.v[0]) && (f_it->v[1] == F.v[1]) &&
          (f_it->v[2] == F.v[2]))
        return false;
      if ((f_it->v[0] == F.v[0]) && (f_it->v[2] == F.v[1]) &&
          (f_it->v[1] == F.v[2]))
        return false;
      if ((f_it->v[2] == F.v[0]) && (f_it->v[0] == F.v[1]) &&
          (f_it->v[1] == F.v[2]))
        return false;
      if ((f_it->v[2] == F.v[0]) && (f_it->v[1] == F.v[1]) &&
          (f_it->v[0] == F.v[2]))
        return false;
      if ((f_it->v[1] == F.v[0]) && (f_it->v[0] == F.v[1]) &&
          (f_it->v[2] == F.v[2]))
        return false;
      if ((f_it->v[1] == F.v[0]) && (f_it->v[2] == F.v[1]) &&
          (f_it->v[0] == F.v[2]))
        return false;
    }
    return true;
  ...
  bool add_face(SSkelVert &v0, SSkelVert &v1, SSkelVert &v2) {
    if (v0.offs.similar(v1.offs, EPS) || v0.offs.similar(v2.offs, EPS) ||
        v1.offs.similar(v2.offs, EPS)) {
  ...
  //    SSkelFace&		getFace(int id)	{ return
  //    VERIFY(id<m_Faces.size()); m_Faces[id];	}
