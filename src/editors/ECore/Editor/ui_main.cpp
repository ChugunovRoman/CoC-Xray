  ...
  if (!EDevice.m_Camera.MoveStart(m_ShiftState)) {
    ...
    if (!m_MouseCaptured) {
      if (Tools->HiddenMode()) {
        ...
        EDevice.m_Camera.MouseRayFromPoint(m_CurrentRStart, m_CurrentRDir,
                                           m_CurrentCp);
      }
...
bool TUI::ShowHint(const AStringVec &SS) {
  ...
  if (SS.size()) {
    ...
    rect.Left += m_HintPoint.x;
    rect.Top += m_HintPoint.y;
    rect.Right += m_HintPoint.x;
    rect.Bottom += m_HintPoint.y;
...
void TUI::PrepareRedraw() {
  ...
  /*
     ...
     (color_get_R(fog_color),color_get_G(fog_color),color_get_B(fog_color));
          g_pGamePersistent->Environment().CurrentEnv->fog_far		=
     fog_end;
          g_pGamePersistent->Environment().CurrentEnv->fog_near		=
     fog_start;
      }
  */
  ...
  try {
    ...
    if (EDevice.Begin()) {
      ...
      try {
        ...
        ELog.DlgMsg(mtError, "Please notify AlexMX!!! Critical error has "
                             "occured in render routine!!! [Type B]");
      }
      ...
      try {
        ...
        ELog.DlgMsg(mtError, "Please notify AlexMX!!! Critical error has "
                             "occured in render routine!!! [Type C]");
      }
    }
    EDevice.Statistic->RenderDUMP_RT.End();
  } catch (...) {
    ELog.DlgMsg(mtError, "Please notify AlexMX!!! Critical error has occured "
                         "in render routine!!! [Type A]");
    //		_clear87();
    //		FPU::m24r();
    //    	ELog.DlgMsg(mtError, "Critical error has occured in render
