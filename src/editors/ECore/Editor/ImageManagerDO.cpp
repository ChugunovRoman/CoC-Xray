...
IC void blit(u32 *dest, u32 ds_x, u32 ds_y, u32 *src, u32 ss_x, u32 ss_y,
             u32 px, u32 py) {
...
IC void blit_r(u32 *dest, u32 ds_x, u32 ds_y, u32 *src, u32 ss_x, u32 ss_y,
               u32 px, u32 py) {
...
IC void _rect_register(U8Vec &mask, int dest_width, int dest_height, Irect &R) {
  ...
  for (u32 y = 0; y < s_y; y++) {
    u8 *P = mask.begin() + (y + R.lt.y) * dest_width +
            R.lt.x; // destination scan-line
...
IC bool _rect_place(U8Vec &mask, int dest_width, int dest_height, Irect &r,
                    BOOL &bRotated) {
  Irect R;
...
bool item_area_sort_pred(const SSimpleImage &item0, const SSimpleImage &item1) {
  return ((item0.Area() > item1.Area()) ||
          (item0.LongestEdge() > item1.LongestEdge()));
}
extern bool Surface_Load(LPCSTR full_name, U32Vec &data, u32 &w, u32 &h,
                         u32 &a);

int CImageManager::CreateMergedTexture(
    u32 layer_cnt, SSimpleImageVec &src_images, SSimpleImage &dst_image,
    int dest_width, int dest_height, Fvector2Vec &dest_offset,
  ...
  for (SSimpleImage::DATAIt layer_it = dest_layers.begin();
       layer_it != dest_layers.end(); layer_it++)
  ...
  for (SSimpleImageVecIt s_it = src_images.begin(); s_it != src_images.end();
       s_it++) {
  ...
  for (s_it = src_images.begin(); s_it != src_images.end(); s_it++) {
    ...
    for (u32 k = 0; k < layer_cnt; k++) {
      if (!bRotated)
        blit(dest_layers[k].begin(), dest_width, dest_height,
             s_it->layers[k].begin(), s_it->w, s_it->h, R.lt.x, R.lt.y);
      else
        blit_r(dest_layers[k].begin(), dest_width, dest_height,
               s_it->layers[k].begin(), s_it->w, s_it->h, R.lt.x, R.lt.y);
    }
  }
...
int CImageManager::CreateMergedTexture(
    u32 layer_cnt, SSimpleImageVec &src_images, SSimpleImage &dst_image,
    int dest_width_min, int dest_width_max, int dest_height_min,
    int dest_height_max, Fvector2Vec &dest_offset, Fvector2Vec &dest_scale,
    boolVec &dest_rotate, U32Vec &dest_remap) {
  int res = 0;
  int w = dest_width_min;
  int h = dest_height_min;
  do {
    res = ImageLib.CreateMergedTexture(layer_cnt, src_images, dst_image, w, h,
                                       dest_offset, dest_scale, dest_rotate,
                                       dest_remap);
    if (0 == res) {
...
int CImageManager::CreateMergedTexture(
    const RStringVec &_names, LPCSTR dest_name, STextureParams::ETFormat fmt,
    int dest_width, int dest_height, Fvector2Vec &dest_offset,
    Fvector2Vec &dest_scale, boolVec &dest_rotate, U32Vec &dest_remap) {
  if (_names.empty())
    return -1;
  ...
  for (RStringVecIt n_it = src_names.begin(); n_it != src_names.end();
       n_it++, s_it++) {
    s_it->layers.resize(1);
    string_path t_name;
    //.        FS.update_path	(t_name,_textures_,**n_it);
    FS.update_path(t_name, _game_textures_, **n_it);
    if (!Surface_Load(EFS.ChangeFileExt(t_name, ".dds" /*".tga"*/).c_str(),
                      s_it->layers.back(), s_it->w, s_it->h, s_it->a)) {
      ELog.DlgMsg(mtError, "Can't load texture '%s'. Check file existence.",
                  **n_it);
      return -1;
    }
    calc_area += (s_it->w * s_it->h);
  ...
  for (s_it = src_items.begin(); s_it != src_items.end(); s_it++) {
    ...
    // Perform BLIT
    if (!bRotated)
      blit(dest_pixels.begin(), dest_width, dest_height,
           s_it->layers.back().begin(), s_it->w, s_it->h, R.lt.x, R.lt.y);
    else
      blit_r(dest_pixels.begin(), dest_width, dest_height,
             s_it->layers.back().begin(), s_it->w, s_it->h, R.lt.x, R.lt.y);
  }
...
int CImageManager::CreateMergedTexture(
    const RStringVec &src_names, LPCSTR dest_name, STextureParams::ETFormat fmt,
    int dest_width_min, int dest_width_max, int dest_height_min,
    int dest_height_max, Fvector2Vec &dest_offset, Fvector2Vec &dest_scale,
    boolVec &dest_rotate, U32Vec &remap) {
  int res = 0;
  int w = dest_width_min;
  int h = dest_height_min;
  do {
    res = ImageLib.CreateMergedTexture(src_names, dest_name, fmt, w, h,
                                       dest_offset, dest_scale, dest_rotate,
                                       remap);
    if (0 == res) {
      if ((w <= h) && (w < dest_width_max))
...
void CImageManager::MergedTextureRemapUV(float &dest_u, float &dest_v,
                                         float src_u, float src_v,
                                         const Fvector2 &offs,
                                         const Fvector2 &scale, bool bRotate) {
  if (bRotate) {
