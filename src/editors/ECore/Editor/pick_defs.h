...
public:
  ...
  public:
    ...
    SResult(const Fmatrix &parent, CDB::RESULT *r, CEditableObject *obj,
            CEditableMesh *mesh) {
...
public:
  ...
  IC void append_mtx(const Fmatrix &parent, CDB::RESULT *R,
                     CEditableObject *obj, CEditableMesh *mesh) {
  ...
  IC SResult *r_begin() { return results.begin(); }
  IC SResult *r_end() { return results.end(); }
  IC void r_clear() { results.clear(); }
  IC void r_sort() { std::sort(results.begin(), results.end()); }
};
