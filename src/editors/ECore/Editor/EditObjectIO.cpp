...
void CEditableObject::Save(IWriter &F) {
  ...
  F.w_chunk(EOBJ_CHUNK_FLAGS, &m_objectFlags.flags,
            sizeof(m_objectFlags.flags));
  ...
  for (SurfaceIt sf_it = m_Surfaces.begin(); sf_it != m_Surfaces.end();
       ++sf_it) {
...
bool CEditableObject::Load(IReader &F) {
  ...
  do {
    ...
    if (version != EOBJ_CURRENT_VERSION) {
      ELog.DlgMsg(
          mtError,
          "CEditableObject: unsupported file version. Object can't load.");
      bRes = false;
      break;
    }
    ...
    if (F.find_chunk(EOBJ_CHUNK_SURFACES3)) {
      ...
      for (SurfaceIt s_it = m_Surfaces.begin(); s_it != m_Surfaces.end();
        ...
        *s_it = xr_new<CSurface>();
        ...
        cnt = F.r_u32();
        if (cnt > 1)
          ELog.DlgMsg(mtError, "Object surface '%s' has more than one TC's.",
                      buf.c_str());
        R_ASSERT(1 <= cnt);
      }
    ...
    if (OBJ) {
      ...
      for (int count = 1; M; count++) {
        ...
        if (mesh->LoadMesh(*M))
          ...
          ELog.DlgMsg(mtError, "CEditableObject: Can't load mesh '%s'!",
                      *mesh->m_Name);
          xr_delete(mesh);
          bRes = false;
        }
    ...
    if (bRes) {
      ...
      if (F.find_chunk(EOBJ_CHUNK_SMOTIONS)) {
        ...
        for (SMotionIt s_it = m_SMotions.begin(); s_it != m_SMotions.end();
             s_it++) {
    ...
    if (bRes) {
      if (F.find_chunk(EOBJ_CHUNK_BONEPARTS)) {
        ...
        for (BPIt bp_it = m_BoneParts.begin(); bp_it != m_BoneParts.end();
          ...
          for (RStringVecIt s_it = bp_it->bones.begin();
               s_it != bp_it->bones.end(); s_it++) {
            int idx = F.r_u32();
        ...
        if (!m_BoneParts.empty() && !VerifyBoneParts())
          Log("!Invalid bone parts. Found duplicate bones in object '%s'.",
              GetName());
