...
public:
  VCPacked(const Fbox &bb, float eps = EPS, u32 clpSX = 24, u32 clpSY = 16,
           u32 clpSZ = 24, int apx_vertices = 5000);
...
class ECORE_API GCPacked : public VCPacked {
  ...
  void validate(GCFace &F) {
    if ((F.verts[0] == F.verts[1]) || (F.verts[0] == F.verts[2]) ||
        (F.verts[1] == F.verts[2]))
      F.valid = false;
    else
      F.valid = true;
...
public:
  GCPacked(const Fbox &bb, float eps = EPS, u32 clpMX = 24, u32 clpMY = 16,
           u32 clpMZ = 24, int apx_vertices = 5000, int apx_faces = 5000)
