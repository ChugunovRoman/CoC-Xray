...
void CEditableObject::OnFrame() {
  if (IsSkeleton()) {
    ...
    if (IsSMotionActive()) {
      ...
      m_SMParam.Update(EDevice.fTimeDelta, m_ActiveSMotion->fSpeed,
                       !m_ActiveSMotion->m_Flags.is(esmStopAtEnd));
    } else {
...
static void CalculateAnim(CBone *bone, CSMotion *motion, Fmatrix &parent) {
  ...
  // Calculate children
  for (BoneIt b_it = bone->children.begin(); b_it != bone->children.end();
       b_it++)
...
static void SetBoneTransform(CBone &bone, const Fmatrix &T,
...
void CEditableObject::CalculateRootObjectAnimation(const Fmatrix &anchor) {
  ...
  //  for (BoneIt b_it=root_bone .children.begin(); b_it!=root_bone
  //  .children.end(); b_it++) 	SetBoneTransform(   *(*b_it),
  //  (*(*b_it))._LTransform(), root_transform );
}
  ...
  if (0 == stricmp(ext, ".skl")) {
    ...
    if (bRes) {
      ...
      for (int k = 0; k < cnt; k++) {
        CSMotion *M = xr_new<CSMotion>();
        if (!M->Load(*F)) {
          ELog.Msg(mtError, "Motion '%s' has different version. Load failed.",
                   M->Name());
          xr_delete(M);
          bRes = false;
          break;
...
bool CEditableObject::RenameSMotion(const char *old_name,
                                    const char *new_name) {
...
CSMotion *CEditableObject::FindSMotionByName(const char *name,
                                             const CSMotion *Ignore) {
  if (name && name[0]) {
    SMotionVec &lst = m_SMotions;
    for (SMotionIt m = lst.begin(); m != lst.end(); m++)
...
void CEditableObject::GenerateSMotionName(char *buffer, const char *start_name,
                                          const CSMotion *M) {
...
ICF void fill_bones_by_parent(BoneVec &bones, CBone *start) {
  ...
  for (BoneIt b_it = start->children.begin(); b_it != start->children.end();
       b_it++)
...
void CEditableObject::PrepareBones() {
  ...
  for (b_it = m_Bones.begin(); b_it != m_Bones.end(); b_it++) {
    (*b_it)->children.clear();
    (*b_it)->parent = NULL;
    BoneIt parent = std::find_if(m_Bones.begin(), m_Bones.end(),
                                 fBoneNameEQ((*b_it)->ParentName()));
  ...
  /*
      for (b_it=m_Bones.begin(); b_it!=m_Bones.end(); b_it++)
          Msg("%20s -
     %20s",(*b_it)->Name().c_str(),(*b_it)->ParentName().c_str());
  */
  CalculateBindPose();
}
...
u16 CEditableObject::GetBoneIndexByWMap(const char *wm_name) {
  BoneIt bone =
      std::find_if(m_Bones.begin(), m_Bones.end(), fBoneWMNameEQ(wm_name));
  return (bone == m_Bones.end()) ? BI_NONE : (u16)(bone - m_Bones.begin());
}

void CEditableObject::GetBoneWorldTransform(u32 bone_idx, float t,
                                            CSMotion *motion, Fmatrix &matrix) {
  ...
  do {
    lst.push_back(idx);
  } while ((idx = m_Bones[idx]->Parent() ? m_Bones[idx]->Parent()->SelfID
                                         : -1) > -1);
...
bool CEditableObject::CheckBoneCompliance(CSMotion *M) {
  ...
  /*
     ...
     Motion has '%d' bones.",m_Bones.size(),lst.size()); return false;
      }
      for(BoneMotionIt bm_it=lst.begin(); bm_it!=lst.end(); bm_it++)
          if (!FindBoneByName(*bm_it->name)){
                  Msg		("!Can't find bone '%s' in
     object.",bm_it->name); return false;
          }
  */
  for (BoneIt b_it = m_Bones.begin(); b_it != m_Bones.end(); b_it++)
    if (!M->FindBoneMotion((*b_it)->Name())) {
      //        	Msg		("!Can't find bone '%s' in
      //        motion.",*(*b_it)->Name()); 	return false;
