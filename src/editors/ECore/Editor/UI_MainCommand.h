...
public:
  ...
  //	IC operator 	LPCSTR 			()
  //{VERIFY(type==tpStr);return s.c_str();}
};

typedef fastdelegate::FastDelegate2<CCommandVar, CCommandVar, CCommandVar>
    TECommandEvent;
...
public:
  SECommand(LPCSTR n, LPCSTR d, bool edit, bool multi, TECommandEvent cmd,
            u32 i, bool _gs)
  ...
  ~SECommand() {
    xr_free(name);
    xr_free(desc);
    for (ESubCommandVecIt it = sub_commands.begin(); it != sub_commands.end();
         it++)
      xr_delete(*it);
  }
...
ECORE_API CCommandVar ExecCommand(u32 cmd, CCommandVar p1 = u32(0),
                                  CCommandVar p2 = u32(0));
ECORE_API CCommandVar ExecCommand(const xr_shortcut &val);
ECORE_API void RegisterCommand(u32 cmd, SECommand *cmd_impl);
ECORE_API void RegisterSubCommand(SECommand *cmd_impl, LPCSTR desc,
                                  CCommandVar p0, CCommandVar p1);
...
#define REGISTER_CMD_S(id, cmd)                                                \
  RegisterCommand(id, xr_new<SECommand>(#id, "", false, false,                 \
                                        BIND_CMD_EVENT_S(cmd), id, false));
#define REGISTER_CMD_C(id, owner, cmd)                                         \
  RegisterCommand(id,                                                          \
                  xr_new<SECommand>(#id, "", false, false,                     \
                                    BIND_CMD_EVENT_C(owner, cmd), id, false));
#define REGISTER_CMD_SE(id, desc, cmd, gs)                                     \
  RegisterCommand(id, xr_new<SECommand>(#id, desc, true, false,                \
                                        BIND_CMD_EVENT_S(cmd), id, gs));
#define REGISTER_CMD_CE(id, desc, owner, cmd, gs)                              \
  RegisterCommand(id,                                                          \
                  xr_new<SECommand>(#id, desc, true, false,                    \
                                    BIND_CMD_EVENT_C(owner, cmd), id, gs));
#define REGISTER_SUB_CMD_SE(id, desc, cmd, gs)                                 \
  {                                                                            \
    SECommand *SUB_CMD_HOLDER;                                                 \
    RegisterCommand(id, SUB_CMD_HOLDER =                                       \
                            xr_new<SECommand>(#id, desc, true, true,           \
                                              BIND_CMD_EVENT_S(cmd), id, gs));
#define REGISTER_SUB_CMD_CE(id, desc, owner, cmd, gs)                          \
  {                                                                            \
    SECommand *SUB_CMD_HOLDER;                                                 \
    RegisterCommand(                                                           \
        id, SUB_CMD_HOLDER = xr_new<SECommand>(                                \
                #id, desc, true, true, BIND_CMD_EVENT_C(owner, cmd), id, gs));
#define APPEND_SUB_CMD(desc, p0, p1)                                           \
  RegisterSubCommand(SUB_CMD_HOLDER, desc, p0, p1);
