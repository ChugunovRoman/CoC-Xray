...
CObjectOGFCollectorPacked::CObjectOGFCollectorPacked(const Fbox &bb,
                                                     int apx_vertices,
                                                     int apx_faces) {
  // Params
  ...
  m_VMeps.set(m_VMscale.x / clpOGFMX / 2, m_VMscale.y / clpOGFMY / 2,
              m_VMscale.z / clpOGFMZ / 2);
...
void CExportObjectOGF::SSplit::AppendPart(int apx_vertices, int apx_faces) {
  m_Parts.push_back(
      xr_new<CObjectOGFCollectorPacked>(apx_box, apx_vertices, apx_faces));
  m_CurrentPart = m_Parts.back();
}

void CExportObjectOGF::SSplit::SavePart(IWriter &F,
                                        CObjectOGFCollectorPacked *part) {
  // Header
  ...
  for (OGFVertIt v_it = part->m_Verts.begin(); v_it != part->m_Verts.end();
       ++v_it) {
...
bool CExportObjectOGF::PrepareMESH(CEditableMesh *MESH) {
  ...
  for (SurfFacesPairIt sp_it = MESH->m_SurfFaces.begin();
    ...
    int elapsed_faces = surf->m_Flags.is(CSurface::sf2Sided)
                            ? face_lst.size() * 2
                            : face_lst.size();
      ...
      for (IntIt f_it = face_lst.begin(); f_it != face_lst.end(); ++f_it) {
        ...
        st_Face &face = MESH->m_Faces[*f_it];
        {
          ...
          for (int k = 0; k < 3; ++k) {
            ...
            R_ASSERT2(norm_id < MESH->GetFCount() * 3,
                      "Normal index out of range.");
            v[k].set(MESH->m_Vertices[fv.pindex],
                     MESH->m_VertexNormals[norm_id], *uv);
          }
          --elapsed_faces;
  ...
  for (split_it = m_Splits.begin(); split_it != m_Splits.end(); ++split_it) {
    ...
    for (COGFCPIt it = (*split_it)->m_Parts.begin();
      ...
      for (f_it = FACES.begin(); f_it != FACES.end(); ++f_it) {
        sprintf(tmp, "f %d/%d/%d %d/%d/%d %d/%d/%d", v_offs + f_it->v[2] + 1,
                v_offs + f_it->v[2] + 1, v_offs + f_it->v[2] + 1,
                v_offs + f_it->v[1] + 1, v_offs + f_it->v[1] + 1,
                v_offs + f_it->v[1] + 1, v_offs + f_it->v[0] + 1,
                v_offs + f_it->v[0] + 1, v_offs + f_it->v[0] + 1);
        F.w_string(tmp);
      }
      v_offs += VERTS.size();
    }
...
bool CEditableObject::PrepareOGF(IWriter &F, u8 infl, bool gen_tb,
                                 CEditableMesh *mesh) {
  return IsSkeleton() ? PrepareSkeletonOGF(F, infl)
                      : PrepareRigidOGF(F, gen_tb, mesh);
}
...
bool CEditableObject::PrepareRigidOGF(IWriter &F, bool gen_tb,
                                      CEditableMesh *mesh) {
