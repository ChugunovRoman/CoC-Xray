...
//------------------------------------------------------------------------------
void EDetail::EVertexIn::remapUV(const fvfVertexIn &src, const Fvector2 &offs,
...
u16 EDetail::_AddVert(const Fvector &p, float u, float v) {
  ...
  for (u16 k = 0; k < (u16)number_vertices; k++)
    if (V.similar((EVertexIn &)vertices[k]))
      return k;
  number_vertices++;
  vertices = (fvfVertexIn *)xr_realloc(vertices,
                                       number_vertices * sizeof(fvfVertexIn));
...
void EDetail::Export(IWriter &F, LPCSTR tex_name, const Fvector2 &offs,
