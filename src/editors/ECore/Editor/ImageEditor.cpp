...
//---------------------------------------------------------------------------
__fastcall TfrmImageLib::TfrmImageLib(TComponent *Owner) : TForm(Owner) {
...
void __fastcall TfrmImageLib::FormCreate(TObject *) {
  ...
  m_ItemList->SetOnItemsFocusedEvent(fastdelegate::bind<TOnILItemsFocused>(
      this, &TfrmImageLib::OnItemsFocused));
  m_ItemList->SetOnItemRemoveEvent(fastdelegate::bind<TOnItemRemove>(
      &ImageLib, &CImageManager::RemoveTexture));
...
void TfrmImageLib::InitItemsList() {
  ...
  /*
          FS_FileSet				flist;
          FS.file_list
     (flist,"$game_textures$",FS_ListFiles|FS_ClampExt,"*.thm"); Msg
     ("TfrmImageLib::InitItemsList count of .thm files=%d", flist.size());
    ...
    for (ListItemsIt it = items.begin(); it != items.end(); it++) {
      ...
      if (prop) {
        ...
        if (bImportMode)
        {
            ...
            if (!thm)
            {
                m_THM_Used.push_back
        (thm=xr_new<ETextureThumbnail>(prop->Key(),false)); xr_string fn =
        prop->Key(); ImageLib.UpdateFileName (fn);

                    if (!thm->Load(prop->Key(),_import_))
                {
                    bool bLoad                      =
        thm->Load(fn.c_str(),_game_textures_); ImageLib.CreateTextureThumbnail
        (thm, prop->Key(), _import_, !bLoad);
                }
            }
        }else
        {
        ...
        */
        ...
        // fill prop
        thm->FillProp(props,
                      PropValue::TOnChange(this, &TfrmImageLib::OnTypeChange));

        if (thm->_Format().type == STextureParams::ttCubeMap) {
          ButtonValue *B =
              PHelper().CreateButton(props, "CubeMap\\Edit", "Make Small", 0);
...
void __fastcall TfrmImageLib::btFilterClick(TObject *Sender) {
  ...
  for (TElTreeItem *node = m_ItemProps->tvProperties->Items->GetFirstNode();
...
void TfrmImageLib::SortList(ETextureThumbnail *thm0,
