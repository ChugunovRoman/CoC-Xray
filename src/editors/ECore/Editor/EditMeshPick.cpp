...
inside[v_it-m_Points.begin()]=false; break; }
    }
    for(FaceIt f_it=m_Faces.begin();f_it!=m_Faces.end();f_it++,i++)
        if
(inside[f_it->pv[0].pindex]&&inside[f_it->pv[1].pindex]&&inside[f_it->pv[2].pindex])
fl.push_back(i);
}
*/
//----------------------------------------------------
...
void CEditableMesh::GenerateCFModel() {
  ...
  for (SurfFacesPairIt sp_it = m_SurfFaces.begin(); sp_it != m_SurfFaces.end();
    ...
    for (IntIt it = face_lst.begin(); it != face_lst.end(); it++) {
      ...
      ETOOLS::collector_add_face_d(CL, m_Vertices[F.pv[0].pindex],
                                   m_Vertices[F.pv[1].pindex],
                                   m_Vertices[F.pv[2].pindex], *it);
      if (sp_it->first->m_Flags.is(CSurface::sf2Sided))
        ETOOLS::collector_add_face_d(CL, m_Vertices[F.pv[2].pindex],
                                     m_Vertices[F.pv[1].pindex],
                                     m_Vertices[F.pv[0].pindex], *it);
    }
  }
...
void CEditableMesh::RayQuery(SPickQuery &pinf) {
  ...
  /*
          XRC.ray_query	(m_CFModel, pinf.m_Start, pinf.m_Direction,
  pinf.m_Dist); for (int r=0; r<XRC.r_count(); r++) pinf.append
  (XRC.r_begin()+r,m_Parent,this);
  //*/
}

void CEditableMesh::RayQuery(const Fmatrix &parent, const Fmatrix &inv_parent,
                             SPickQuery &pinf) {
  if (!m_CFModel)
    GenerateCFModel();
  //*
  ETOOLS::ray_query_m(inv_parent, m_CFModel, pinf.m_Start, pinf.m_Direction,
                      pinf.m_Dist);
  for (int r = 0; r < ETOOLS::r_count(); r++)
    pinf.append_mtx(parent, ETOOLS::r_begin() + r, m_Parent, this);
  /*
          XRC.ray_query	(inv_parent, m_CFModel, pinf.m_Start, pinf.m_Direction,
  pinf.m_Dist); for (int r=0; r<XRC.r_count(); r++)
          pinf.append_mtx(parent,XRC.r_begin()+r,m_Parent,this);
  //*/
}

void CEditableMesh::BoxQuery(const Fmatrix &parent, const Fmatrix &inv_parent,
                             SPickQuery &pinf) {
  if (!m_CFModel)
    GenerateCFModel();
...
bool CEditableMesh::RayPick(float &distance, const Fvector &start,
                            const Fvector &direction, const Fmatrix &inv_parent,
                            SRayPickInfo *pinf) {
  ...
  if (!m_CFModel)
    GenerateCFModel();
  //.	float m_r 		= pinf?pinf->inf.range+EPS_L:UI->ZFar();// (bugs:
  //не всегда выбирает) //S ????
...
bool CEditableMesh::CHullPickMesh(PlaneVec &pl, const Fmatrix &parent) {
  ...
  for (u32 f_id = 0; f_id < m_FaceCount; f_id++)
    if (inside[m_Faces[f_id].pv[0].pindex] &&
        inside[m_Faces[f_id].pv[1].pindex] &&
...
void CEditableMesh::RecurseTri(int id) {
  ...
  if (std::find(sml_processed.begin(), sml_processed.end(), id) !=
      sml_processed.end())
    return;
...
void CEditableMesh::GetTiesFaces(int start_id, U32Vec &fl, float fSoftAngle,
                                 bool bRecursive) {
...
bool CEditableMesh::BoxPick(const Fbox &box, const Fmatrix &inv_parent,
...
bool CEditableMesh::FrustumPick(const CFrustum &frustum,
...
void CEditableMesh::FrustumPickFaces(const CFrustum &frustum,
                                     const Fmatrix &parent, U32Vec &fl) {
  if (!m_Flags.is(flVisible))
    return;
