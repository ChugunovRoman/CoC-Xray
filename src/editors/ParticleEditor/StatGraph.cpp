...
void CStatGraph::RenderBack() {
  ...
  int PNum_H_LinesUp =
      int((base_y - float(lt.y)) / (grid_step.y * elem_factor));
  int PNum_H_LinesDwn =
      u32((float(rb.y) - base_y) / (grid_step.y * elem_factor));
  ...
  //	    pv->set
  //(lt.x,iFloor(g_y*grid_offs_y+lt.y),grid_color); pv++; 	    pv->set
  //(rb.x,iFloor(g_y*grid_offs_y+lt.y),grid_color); pv++;
  //	}
...
void CStatGraph::RenderLines(FVF::TL0uv **ppv, ElementsDeq *pelements) {
  ...
  for (ElementsDeqIt it = pelements->begin() + 1;
...
void CStatGraph::RenderBarLines(FVF::TL0uv **ppv, ElementsDeq *pelements) {
  ...
  for (ElementsDeqIt it = pelements->begin() + 1;
    ...
    ElementsDeqIt it_prev = it - 1;
    float X0 =
        float(it_prev - pelements->begin()) * elem_offs + lt.x + elem_offs;
    float Y0 = base_y - it_prev->data * elem_factor;
    (*ppv)->set(X0, Y0, it->color);
    (*ppv)++;
    float X1 = float(it - pelements->begin()) * elem_offs + lt.x;
...
void CStatGraph::RenderMarkers(FVF::TL0uv **ppv, MarkersDeq *pmarkers) {
  ...
  for (MarkersDeqIt it = pmarkers->begin();
