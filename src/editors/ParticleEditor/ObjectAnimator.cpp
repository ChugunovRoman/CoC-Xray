...
COMotion *CObjectAnimator::Play(bool loop, LPCSTR name) {
  if (name && name[0]) {
    MotionIt it = std::lower_bound(m_Motions.begin(), m_Motions.end(), name,
                                   motion_find_pred);
...
void CObjectAnimator::DrawPath() {
  ...
  if (m_Current) {
    ...
    if (!path_points.empty())
      DU.DrawPrimitiveL(D3DPT_LINESTRIP, path_points.size() - 1,
    ...
    for (KeyIt k_it = E->keys.begin(); k_it != E->keys.end(); k_it++) {
      ...
      if (EDevice.m_Camera.GetPosition().distance_to_sqr(T) < 50.f * 50.f) {
        ...
        DU.OutText(T, AnsiString().sprintf("K: %3.3f", (*k_it)->time).c_str(),
