...
void CParticleTool::OnParticleItemRename(LPCSTR old_name, LPCSTR new_name,
                                         EItemType type) {
...
void CParticleTool::OnControlClick(ButtonValue *sender, bool &bDataModified,
                                   bool &bSafe) {
...
void CParticleTool::OnParticleItemFocused(ListItemsVec &items) {
  ...
  ButtonValue *B;
  B = PHelper().CreateButton(props, "Transform\\Edit", "Reset",
                             ButtonValue::flFirstOnly);
  B->OnBtnClickEvent.bind(this, &CParticleTool::OnControlClick);
  PHelper().CreateFlag32(props, "Transform\\Type", &m_Flags, flSetXFORM,
                         "Update", "Set");
...
void CParticleTool::RealUpdateProperties() {
  ...
  ListItemsVec items;
  {
    ...
    for (; Pe != Ee; Pe++) {
      ListItem *I =
          LHelper().CreateItem(items, *(*Pe)->m_Name, emEffect, 0, *Pe);
      I->SetIcon(1);
    }
  }
  {
    ...
    for (; Pg != Eg; Pg++) {
      ListItem *I =
          LHelper().CreateItem(items, *(*Pg)->m_Name, emGroup, 0, *Pg);
      I->SetIcon(2);
    }
  }
