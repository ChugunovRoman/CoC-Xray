...
void __fastcall TfrmMain::FormShow(TObject *Sender) {
  ...
  // special case :(
  frmMain->WindowState = (TWindowState)fsStorage->ReadInteger(
      "window_state", frmMain->WindowState);
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmMain::UpdateCaption() {
  AnsiString name;
  name.sprintf("%s - [%s%s]", UI->EditorDesc(), UI->GetCaption(),
               UI->IsModified() ? "*" : "");
  Caption = name;
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmMain::D3DWindowChangeFocus(TObject *Sender) {
  ...
  if (D3DWindow->Focused()) {
    ...
    UI->OnAppDeactivate();
    UI->IR_Release();
    //        paRender->Color=paRender->Color; // чтобы не было  internal code
    //        gen error
  }
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmMain::D3DWindowMouseUp(TObject *Sender, TMouseButton Button,
                                           TShiftState Shift, int X, int Y) {
...
void __fastcall TfrmMain::D3DWindowMouseMove(TObject *Sender, TShiftState Shift,
                                             int X, int Y) {
