...
void pEAvoid(float magnitude, float epsilon, float look_ahead,
             PDomainEnum dtype, float a0, float a1, float a2, float a3,
...
void pEBounce(float friction, float resilience, float cutoff, PDomainEnum dtype,
              float a0, float a1, float a2, float a3, float a4, float a5,
              float a6, float a7, float a8, BOOL allow_translate,
              BOOL allow_rotate) {
...
void pECopyVertexB(BOOL copy_pos) {
  EPACopyVertexB *S =
      dynamic_cast<EPACopyVertexB *>(pCreateEAction(PACopyVertexBID));
...
void pEDamping(float damping_x, float damping_y, float damping_z, float vlow,
               float vhigh) {
...
void pEGravity(float dir_x, float dir_y, float dir_z, BOOL allow_translate,
               BOOL allow_rotate) {
...
#define parse_xd(xd)                                                           \
  xd.type, xd.f[0], xd.f[1], xd.f[2], xd.f[3], xd.f[4], xd.f[5], xd.f[6],      \
      xd.f[7], xd.f[8]

void pEJet(float center_x, float center_y, float center_z, float magnitude,
           float epsilon, float max_radius, BOOL allow_translate,
           BOOL allow_rotate) {
...
void pEMatchVelocity(float magnitude, float epsilon, float max_radius) {
  EPAMatchVelocity *S =
      dynamic_cast<EPAMatchVelocity *>(pCreateEAction(PAMatchVelocityID));
...
void pEOrbitLine(float p_x, float p_y, float p_z, float axis_x, float axis_y,
                 float axis_z, float magnitude, float epsilon, float max_radius,
                 BOOL allow_translate, BOOL allow_rotate) {
...
void pEOrbitPoint(float center_x, float center_y, float center_z,
                  ...
                  BOOL allow_translate, BOOL allow_rotate) {
  EPAOrbitPoint *S =
      dynamic_cast<EPAOrbitPoint *>(pCreateEAction(PAOrbitPointID));
...
void pERandomAccel(PDomainEnum dtype, float a0, float a1, float a2, float a3,
                   float a4, float a5, float a6, float a7, float a8,
                   BOOL allow_translate, BOOL allow_rotate) {
  EPARandomAccel *S =
      dynamic_cast<EPARandomAccel *>(pCreateEAction(PARandomAccelID));
...
void pERandomDisplace(PDomainEnum dtype, float a0, float a1, float a2, float a3,
                      float a4, float a5, float a6, float a7, float a8,
                      BOOL allow_translate, BOOL allow_rotate) {
  EPARandomDisplace *S =
      dynamic_cast<EPARandomDisplace *>(pCreateEAction(PARandomDisplaceID));
...
void pERandomVelocity(PDomainEnum dtype, float a0, float a1, float a2, float a3,
                      float a4, float a5, float a6, float a7, float a8,
                      BOOL allow_translate, BOOL allow_rotate) {
  EPARandomVelocity *S =
      dynamic_cast<EPARandomVelocity *>(pCreateEAction(PARandomVelocityID));
...
void pESink(BOOL kill_inside, PDomainEnum dtype, float a0, float a1, float a2,
...
void pESinkVelocity(BOOL kill_inside, PDomainEnum dtype, float a0, float a1,
                    float a2, float a3, float a4, float a5, float a6, float a7,
                    float a8, BOOL allow_translate, BOOL allow_rotate) {
  EPASinkVelocity *S =
      dynamic_cast<EPASinkVelocity *>(pCreateEAction(PASinkVelocityID));
...
void pESource(float particle_rate, PDomainEnum dtype, float a0, float a1,
              float a2, float a3, float a4, float a5, float a6, float a7,
              float a8, BOOL allow_translate, BOOL allow_rotate) {
  _ParticleState &_ps = _GetPState();
...
void pESpeedLimit(float min_speed, float max_speed) {
  EPASpeedLimit *S =
      dynamic_cast<EPASpeedLimit *>(pCreateEAction(PASpeedLimitID));
...
void pETargetColor(float color_x, float color_y, float color_z, float alpha,
                   float scale) {
  EPATargetColor *S =
      dynamic_cast<EPATargetColor *>(pCreateEAction(PATargetColorID));
...
void pETargetSize(float size_x, float size_y, float size_z, float scale_x,
                  float scale_y, float scale_z) {
  EPATargetSize *S =
      dynamic_cast<EPATargetSize *>(pCreateEAction(PATargetSizeID));
...
void pETargetRotate(float rot_x, float rot_y, float rot_z, float scale) {
  EPATargetRotate *S =
      dynamic_cast<EPATargetRotate *>(pCreateEAction(PATargetRotateID));
...
void pETargetRotateD(float scale, PDomainEnum dtype, float a0, float a1,
                     float a2, float a3, float a4, float a5, float a6, float a7,
                     float a8) {
...
void pETargetVelocity(float vel_x, float vel_y, float vel_z, float scale,
                      BOOL allow_translate, BOOL allow_rotate) {
  EPATargetVelocity *S =
      dynamic_cast<EPATargetVelocity *>(pCreateEAction(PATargetVelocityID));
...
void pETargetVelocityD(float scale, PDomainEnum dtype, float a0, float a1,
                       float a2, float a3, float a4, float a5, float a6,
                       float a7, float a8, BOOL allow_translate,
                       BOOL allow_rotate) {
...
void pEVortex(float center_x, float center_y, float center_z, float axis_x,
              float axis_y, float axis_z, float magnitude, float epsilon,
              float max_radius, BOOL allow_translate, BOOL allow_rotate) {
...
void ErrMsg(LPCSTR ps, int c, int l, int v, LPCSTR line) {
  switch (c) {
  case BAD_COMMAND:
    ELog.Msg(mtError, "[Error] '%s'(%d): Unrecognized command: '%s'", ps, l + 1,
             line);
    break;
  case FEW_PARAMS:
    ELog.Msg(mtError, "[Error] '%s': Too few parameters in call to '%s'", ps,
             line);
    break;
  case MANY_PARAMS:
    ELog.Msg(mtError, "[Error] '%s': Extra parameters in call to '%s'", ps,
             line);
    break;
  case BAD_PARAM:
    ELog.Msg(mtError, "[Error] '%s'(%d): Bad parameter '%d' - '%s'", ps, l + 1,
             v, line);
    break;
  case STA_MISSING:
    ELog.Msg(mtError, "[Error] '%s'(%d): Statement missing ';'", ps, l + 1,
             line);
    break;
  }
}
...
public:
  ...
  bool LoadTemplate(LPCSTR line) {
    ...
    for (int k = 0; k < p_cnt; k++) {
      ...
      if (!P.s_data.IsEmpty()) {
        switch (P.type) {
        case ptDomain:
          bRes = get_domain_type(P.s_data.c_str(), P.data);
          break;
        case ptBOOL:
          bRes = get_bool(P.s_data.c_str(), P.data);
          break;
        case ptFloat:
          bRes = get_float(P.s_data.c_str(), P.data);
          break;
        case ptInt:
        case ptWORD:
        case ptDWORD:
          bRes = get_int(P.s_data.c_str(), P.data);
          break;
        case ptString:
          bRes = get_string(P.s_data.c_str(), P.data);
          break;
        }
      } else {
        req_params = k + 1;
  ...
  bool Parse(int l, LPCSTR line) {
    ...
    for (int k = 0; k < p_cnt; k++) {
      ...
      switch (P.type) {
      case ptDomain:
        bRes = get_domain_type(pm.c_str(), P.data);
        break;
      case ptBOOL:
        bRes = get_bool(pm.c_str(), P.data);
        break;
      case ptFloat:
        bRes = get_float(pm.c_str(), P.data);
        break;
      case ptInt:
      case ptWORD:
      case ptDWORD:
        bRes = get_int(pm.c_str(), P.data);
        break;
      case ptString:
        bRes = get_string(pm.c_str(), P.data);
        break;
      }
      if (!bRes) {
...
#define P15 P14, p(14)
    ...
    if (command == "pColor")
      ...
      pSetMaxParticles(P1);
    //        else if (command=="pObject")       		parent->pObject
    //        (P1);
    //		else if (command=="pAlignToPath")
    // parent->pAlignToPath(P3); 		else if
    // (command=="pVelocityScale") parent->pVelocityScale(P3); 		else if
    // (command=="pCollision") parent->pCollision(P4); 		else if
    // (command=="pSprite") parent->pSprite (P2); 		else if
    //(command=="pFrame")		  		parent->pFrame (P6);
    //else if (command=="pTimeLimit") parent->pTimeLimit(P1); 		else if
    // (command=="pParentMotion") pParentMotion	(P1);
    // actions
    //		else if (command=="pAnimate")
    // parent->pAnimate(P2);
...
static LPCSTR PStateCommands[] = {
    ...
    "pColorD(float alpha, PDomainEnum dtype, float a0=0.0f, float a1=0.0f, "
    "float a2=0.0f, float a3=0.0f, float a4=0.0f, float a5=0.0f, float "
    "a6=0.0f, float a7=0.0f, float a8=0.0f);",
    "pRotate(float rot_x, float rot_y=0.f, float rot_z=0.f);",
    "pRotateD(PDomainEnum dtype, float a0=0.0f, float a1=0.0f, float a2=0.0f, "
    ...
    "pSizeD(PDomainEnum dtype, float a0=0.0f, float a1=0.0f, float a2=0.0f, "
    "float a3=0.0f, float a4=0.0f, float a5=0.0f, float a6=0.0f, float "
    "a7=0.0f, float a8=0.0f, BOOL single_size=TRUE);",
    "pStartingAge(float age, float sigma=1.0f);",
    "pVelocity(float x, float y, float z);",
    "pVelocityD(PDomainEnum dtype, float a0=0.0f, float a1=0.0f, float "
    "a2=0.0f, float a3=0.0f, float a4=0.0f, float a5=0.0f, float a6=0.0f, "
    "float a7=0.0f, float a8=0.0f);",
    "pVertexB(float x, float y, float z);",
    "pVertexBD(PDomainEnum dtype, float a0=0.0f, float a1=0.0f, float a2=0.0f, "
    "float a3=0.0f, float a4=0.0f, float a5=0.0f, float a6=0.0f, float "
    "a7=0.0f, float a8=0.0f);",
    ...
    "pCollision(float friction, float resilience, float cutoff, BOOL "
    "destroy_on_contact=FALSE);",
    //    "pObject(LPCSTR obj_name);",
    "pSprite(LPCSTR sh_name, LPCSTR tex_name);",
    "pFrame(BOOL random_frame=TRUE, u32 frame_count=16, u32 texture_width=128, "
    "u32 texture_height=128, u32 frame_width=32, u32 frame_height=32);",
    "pTimeLimit(float time_limit);", 0};

static LPCSTR PActionCommands[] = {
    "pAvoid(float magnitude, float epsilon, float look_ahead,  PDomainEnum "
    "dtype, float a0=0.0f, float a1=0.0f, float a2=0.0f, float a3=0.0f, float "
    "a4=0.0f, float a5=0.0f, float a6=0.0f, float a7=0.0f, float a8=0.0f, BOOL "
    "allow_translate=TRUE, BOOL allow_rotate=TRUE);",
    "pBounce(float friction, float resilience, float cutoff, PDomainEnum "
    "dtype, float a0=0.0f, float a1=0.0f, float a2=0.0f, float a3=0.0f, float "
    "a4=0.0f, float a5=0.0f, float a6=0.0f, float a7=0.0f, float a8=0.0f, BOOL "
    "allow_translate=TRUE, BOOL allow_rotate=TRUE);",
    "pCopyVertexB(BOOL copy_pos=TRUE);",
    "pDamping(float damping_x, float damping_y, float damping_z, float "
    "vlow=0.0f, float vhigh=P_MAXFLOAT);",
    "pExplosion(float center_x, float center_y, float center_z, float "
    "velocity, float magnitude, float stdev, float epsilon=EPS_L, float "
    "age=0.0f, BOOL allow_translate=TRUE, BOOL allow_rotate=TRUE);",
    "pFollow(float magnitude=1.0f, float epsilon=EPS_L, float "
    "max_radius=P_MAXFLOAT);",
    "pGravitate(float magnitude=1.0f, float epsilon=EPS_L, float "
    "max_radius=P_MAXFLOAT);",
    "pGravity(float dir_x, float dir_y, float dir_z, BOOL "
    "allow_translate=TRUE, BOOL allow_rotate=TRUE);",
    "pJet(float center_x, float center_y, float center_z, float "
    "magnitude=1.0f, float epsilon=EPS_L, float max_radius=P_MAXFLOAT, BOOL "
    "allow_translate=TRUE, BOOL allow_rotate=TRUE);",
    "pKillOld(float age_limit, BOOL kill_less_than=FALSE);",
    "pMatchVelocity(float magnitude=1.0f, float epsilon=EPS_L, float "
    "max_radius=P_MAXFLOAT);",
    "pMove();",
    "pOrbitLine(float p_x, float p_y, float p_z, float axis_x, float axis_y, "
    "float axis_z, float magnitude=1.0f, float epsilon=EPS_L, float "
    "max_radius=P_MAXFLOAT, BOOL allow_translate=TRUE, BOOL "
    "allow_rotate=TRUE);",
    "pOrbitPoint(float center_x, float center_y, float center_z, float "
    "magnitude=1.0f, float epsilon=EPS_L, float max_radius=P_MAXFLOAT, BOOL "
    "allow_translate=TRUE, BOOL allow_rotate=TRUE);",
    "pRandomAccel(PDomainEnum dtype, float a0=0.0f, float a1=0.0f, float "
    "a2=0.0f, float a3=0.0f, float a4=0.0f, float a5=0.0f, float a6=0.0f, "
    "float a7=0.0f, float a8=0.0f, BOOL allow_translate=TRUE, BOOL "
    "allow_rotate=TRUE);",
    "pRandomDisplace(PDomainEnum dtype, float a0=0.0f, float a1=0.0f, float "
    "a2=0.0f, float a3=0.0f, float a4=0.0f, float a5=0.0f, float a6=0.0f, "
    "float a7=0.0f, float a8=0.0f, BOOL allow_translate=TRUE, BOOL "
    "allow_rotate=TRUE);",
    "pRandomVelocity(PDomainEnum dtype, float a0=0.0f, float a1=0.0f, float "
    "a2=0.0f, float a3=0.0f, float a4=0.0f, float a5=0.0f, float a6=0.0f, "
    "float a7=0.0f, float a8=0.0f, BOOL allow_translate=TRUE, BOOL "
    "allow_rotate=TRUE);",
    "pRestore(float time);",
    "pSink(BOOL kill_inside, PDomainEnum dtype, float a0=0.0f, float a1=0.0f, "
    "float a2=0.0f, float a3=0.0f, float a4=0.0f, float a5=0.0f, float "
    "a6=0.0f, float a7=0.0f, float a8=0.0f, BOOL allow_translate=TRUE, BOOL "
    "allow_rotate=TRUE);",
    "pSinkVelocity(BOOL kill_inside, PDomainEnum dtype, float a0=0.0f, float "
    "a1=0.0f, float a2=0.0f, float a3=0.0f, float a4=0.0f, float a5=0.0f, "
    "float a6=0.0f, float a7=0.0f, float a8=0.0f, BOOL allow_translate=TRUE, "
    "BOOL allow_rotate=TRUE);",
    "pSource(float particle_rate, PDomainEnum dtype, float a0=0.0f, float "
    "a1=0.0f, float a2=0.0f, float a3=0.0f, float a4=0.0f, float a5=0.0f, "
    "float a6=0.0f, float a7=0.0f, float a8=0.0f, BOOL allow_translate=TRUE, "
    "BOOL allow_rotate=TRUE);",
    "pSpeedLimit(float min_speed, float max_speed=P_MAXFLOAT);",
    "pTargetColor(float color_x, float color_y, float color_z, float alpha, "
    "float scale);",
    "pTargetSize(float size_x, float size_y, float size_z, float scale_x=0.0f, "
    "float scale_y=0.0f, float scale_z=0.0f);",
    "pTargetRotate(float rot_x, float rot_y, float rot_z, float scale=0.0f);",
    "pTargetRotateD(float scale, PDomainEnum dtype, float a0=0.0f, float "
    "a1=0.0f, float a2=0.0f, float a3=0.0f, float a4=0.0f, float a5=0.0f, "
    "float a6=0.0f, float a7=0.0f, float a8=0.0f);",
    "pTargetVelocity(float vel_x, float vel_y, float vel_z, float scale, BOOL "
    "allow_translate=TRUE, BOOL allow_rotate=TRUE);",
    "pTargetVelocityD(float scale, PDomainEnum dtype, float a0=0.0f, float "
    "a1=0.0f, float a2=0.0f, float a3=0.0f, float a4=0.0f, float a5=0.0f, "
    "float a6=0.0f, float a7=0.0f, float a8=0.0f, BOOL allow_translate=TRUE, "
    "BOOL allow_rotate=TRUE);",
    "pVertex(float x, float y, float z);",
    "pVortex(float center_x, float center_y, float center_z, float axis_x, "
...
const AnsiString GetFunctionTemplate(const AnsiString &command) {
  ...
  if (F) {
    ...
    if (!F->params.empty()) {
      for (PFunction::ParamIt it = F->params.begin(); it != F->params.end();
           it++) {
        text += it->s_data;
        text += ", ";
      }
...
void FillStateMenu(TMenuItem *root, TNotifyEvent on_click) {
  ...
  for (PFuncPairIt b = CommandTemplates.begin(); b != CommandTemplates.end();
       b++) {
...
void CPEDef::Compile() {
  ...
  */
  ...
  try {
    ...
    // save action list
    //        ParticleActions* pa		=
    //        _GetListPtr(action_list_handle); R_ASSERT(pa); m_Actions.clear
    //        (); m_Actions.w_u32			(pa->size()); for
    //        (PAVecIt it=pa->begin(); it!=pa->end(); it++){ 	m_Actions.w_u32
    //        ((*it)->type);
    //            (*it)->Save			(m_Actions);
    //        }
...
\n \r
    ...
    // save effect data
        ParticleEffect *pe 		= _GetEffectPtr(effect_handle);
R_ASSERT(pe); m_MaxParticles			= pe->max_particles;

    // save action list
        PAPI::PAHeader *pa		= _GetListPtr(action_list_handle);
R_ASSERT(pa); m_ActionCount	 		= pa->count-1; PAPI::PAHeader
*action	= pa+1; xr_free					(m_ActionList);
    m_ActionList			=
xr_alloc<PAPI::PAHeader>(m_ActionCount); Memory.mem_copy
(m_ActionList,action,m_ActionCount*sizeof(PAPI::PAHeader));
