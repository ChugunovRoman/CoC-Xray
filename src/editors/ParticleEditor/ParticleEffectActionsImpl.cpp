...
void EParticleAction::FillProp(PropItemVec &items, LPCSTR pref, u32 clr) {
  ...
  for (OrderVecIt o_it = orders.begin(); o_it != orders.end(); o_it++) {
    ...
    case tpVector: {
      ...
      switch (vect.type) {
      case PVector::vNum:
        V = PHelper().CreateVector(items, PrepareKey(pref, name).c_str(),
                                   &vect.val, vect.mn, vect.mx, 0.001f, 3);
        break;
      case PVector::vAngle:
        V = PHelper().CreateAngle3(items, PrepareKey(pref, name).c_str(),
                                   &vect.val, vect.mn, vect.mx, 0.001f, 3);
        break;
      case PVector::vColor:
        V = PHelper().CreateVColor(items, PrepareKey(pref, name).c_str(),
                                   &vect.val);
        break;
      }
    } break;
    case tpFloat: {
      ...
      V = PHelper().CreateFloat(items, PrepareKey(pref, name).c_str(), &flt.val,
                                flt.mn, flt.mx, 0.001f, 3);
    } break;
    case tpInt: {
      PInt &el = ints[o_it->name];
      V = PHelper().CreateS32(items, PrepareKey(pref, name).c_str(), &el.val,
                              el.mn, el.mx);
    } break;
    case tpBool:
      V = PHelper().CreateBOOL(items, PrepareKey(pref, name).c_str(),
                               &bools[o_it->name].val);
      break;
    }
    if (V)
  ...
  V = PHelper().CreateFlag32(items, PrepareKey(pref, "Draw").c_str(), &flags,
                             flDraw);
  V->Owner()->prop_color = clr;
  V = PHelper().CreateFlag32(items, PrepareKey(pref, "Enabled").c_str(), &flags,
                             flEnabled);
  V->Owner()->prop_color = clr;
}
...
void EParticleAction::appendVector(LPCSTR name, PVector::EType type, float vx,
                                   float vy, float vz, float mn, float mx) {
...
//---------------------------------------------------------------------------
void pAvoid(IWriter &F, float magnitude, float epsilon, float look_ahead,
            pDomain D, BOOL allow_rotate) {
  PAAvoid S;
  S.type = PAAvoidID;
...
void pBounce(IWriter &F, float friction, float resilience, float cutoff,
             pDomain D, BOOL allow_rotate) {
  PABounce S;
  S.type = PABounceID;
...
void pJet(IWriter &F, pDomain acc, const Fvector &center, float magnitude,
          float epsilon, float max_radius, BOOL allow_rotate) {
  PAJet S;
  S.type = PAJetID;
...
void pMatchVelocity(IWriter &F, float magnitude, float epsilon,
                    float max_radius) {
  PAMatchVelocity S;
...
void pOrbitPoint(IWriter &F, const Fvector &center, float magnitude,
                 float epsilon, float max_radius, BOOL allow_rotate) {
  PAOrbitPoint S;
...
void pScatter(IWriter &F, const Fvector &center, float magnitude, float epsilon,
              float max_radius, BOOL allow_rotate) {
  PAScatter S;
  S.type = PAScatterID;
...
void pSource(IWriter &F, float particle_rate, pDomain pos, pDomain vel,
             pDomain rot, pDomain size, BOOL single_size, pDomain color,
             float alpha, float age, float age_sigma, float parent_motion,
             BOOL allow_rotate) {
  PASource S;
  S.type = PASourceID;
...
void pTargetVelocity(IWriter &F, const Fvector &vel, float scale,
                     BOOL allow_rotate) {
  PATargetVelocity S;
...
void pTurbulence(IWriter &F, float freq, int octaves, float magnitude,
                 float epsilon, const Fvector &offs) {
  PATurbulence S;
...
#define EXPAND_DOMAIN(D)                                                       \
  D.type, D.f[0], D.f[1], D.f[2], D.f[3], D.f[4], D.f[5], D.f[6], D.f[7], D.f[8]
...
void EPAAvoid::Compile(IWriter &F) {
  pAvoid(F, _float("Magnitude").val, _float("Epsilon").val,
         _float("Look Ahead").val, pDomain(EXPAND_DOMAIN(_domain("Position"))),
...
void EPABounce::Compile(IWriter &F) {
  pBounce(F, _float("Friction").val, _float("Resilience").val,
          _float("Cutoff").val, pDomain(EXPAND_DOMAIN(_domain("Position"))),
          _bool("Allow Rotate").val);
}
...
void EPAJet::Compile(IWriter &F) {
  pJet(F, pDomain(EXPAND_DOMAIN(_domain("Accelerate"))), _vector("Center").val,
       _float("Magnitude").val, _float("Epsilon").val, _float("Max Radius").val,
       _bool("Allow Rotate").val);
}
void EPAJet::Render(const Fmatrix &parent) {
  ...
  DU_impl.DrawCross(_vector("Center").val, 0.05f, 0.05f, 0.05f, 0.05f, 0.05f,
                    0.05f, 0x600000ff);
}
...
void EPASink::Compile(IWriter &F) {
  pSink(F, _bool("Kill Inside").val, pDomain(EXPAND_DOMAIN(_domain("Domain"))),
        _bool("Allow Rotate").val);
}
...
EPASource::EPASource() : EParticleAction(PAPI::PASourceID) {
  ...
  appendDomain("Color",
               PDomain(PDomain::vColor, FALSE, 0x00000000, PAPI::PDPoint, 1.f,
...
void EPASource::Compile(IWriter &F) {
  pSource(F, _float("Rate").val, pDomain(EXPAND_DOMAIN(_domain("Domain"))),
          ...
          _float("Starting Age").val, _float("Age Sigma").val,
          _float("Parent Motion").val, _bool("Allow Rotate").val);
}
    ...
    for (i = 0; i < detail; i++) {
      for (j = 0; j < detail; j++) {
        for (k = 0; k < detail; k++) {
          ...
          vec[0] = (((float)i / (float)detail) - 0.5) * 2.0 * (float)draw_area +
                   offs.x * age;
          vec[1] = (((float)j / (float)detail) - 0.5) * 2.0 * (float)draw_area +
                   offs.y * age;
          vec[2] = (((float)k / (float)detail) - 0.5) * 2.0 * (float)draw_area +
                   offs.z * age;
          nval[i][j][k] =
              fractalsum3(vec, _float("Frequency").val, _int("Octaves").val);
        }
      }
    }
  }
