...
CParticleTool::~CParticleTool() { xr_delete(fmItemProp); }
//---------------------------------------------------------------------------
...
bool CParticleTool::OnCreate() {
  ...
  m_ItemProps = TProperties::CreateForm(
      "Item Props", fmItemProp, alClient,
      fastdelegate::bind<TOnModifiedEvent>(this,
                                           &CParticleTool::OnItemModified),
      0, 0,
      TProperties::plFolderStore |
          TProperties::plFullExpand /*| TProperties::plItemFolders*/ |
          TProperties::plIFTop);
  m_ItemProps->Visible = true;
  // item list
  m_PList =
      TItemList::CreateForm("Items", fraLeftBar->paPSList, alClient,
                            TItemList::ilEditMenu | TItemList::ilDragAllowed);

  m_PList->SetOnItemsFocusedEvent(fastdelegate::bind<TOnILItemsFocused>(
      this, &CParticleTool::OnParticleItemFocused));
  m_PList->SetOnItemRenameEvent(fastdelegate::bind<TOnItemRename>(
      this, &CParticleTool::OnParticleItemRename));
  m_PList->SetOnItemRemoveEvent(fastdelegate::bind<TOnItemRemove>(
      this, &CParticleTool::OnParticleItemRemove));
...
bool CParticleTool::IfModified() {
  if (m_bModified) {
    int mr = ELog.DlgMsg(
        mtConfirmation,
        "The particles has been modified.\nDo you want to save your changes?");
    switch (mr) {
    case mrYes:
...
void CParticleTool::Render() {
  ...
  case emGroup: {
    if (m_EditPG) {
      ...
      for (int k = 0; k < cnt; k++) {
        PS::CParticleEffect *E =
            (PS::CParticleEffect *)m_EditPG->items[k]._effect;
        if (E && E->GetDefinition() &&
            m_LibPGD->m_Effects[k]->m_Flags.is(PS::CPGDef::SEffect::flEnabled))
...
void CParticleTool::OnFrame() {
  ...
  case emEffect:
    if (m_EditPE->IsPlaying())
      UI->SetStatus(
          AnsiString()
              .sprintf(" PE Playing...[%d]", m_EditPE->ParticlesCount())
              .c_str(),
          false);
    else
  ...
  case emGroup:
    if (m_EditPG->IsPlaying())
      UI->SetStatus(
          AnsiString()
              .sprintf(" PE Playing...[%d]", m_EditPG->ParticlesCount())
              .c_str(),
          false);
    else
      UI->SetStatus(" Stopped.", false);
...
void CParticleTool::SelectPreviewObject(int p) {
  LPCSTR fn;
  if (!TfrmChoseItem::SelectItem(smObject, fn, 1,
                                 m_EditObject ? m_EditObject->GetName() : 0))
    return;
  Lib.RemoveEditObject(m_EditObject);
...
bool CParticleTool::Save(bool bAsXR) {
  ...
  if (!Validate(true)) {
    ELog.DlgMsg(mtError,
                "Invalid particle's found. Validate library and try again.");
    return false;
  }
  bool bRes = false;
  if (bAsXR) {
...
void CheckEffect(const xr_string &group_path, const shared_str &eff_full_name,
                 xr_string &res_name, bool bRenameOnly) {
  res_name =
      group_path + "effects\\" + EFS.ExtractFileName(eff_full_name.c_str());
  ...
  if (0 != stricmp(res_name.c_str(), eff_full_name.c_str())) {
    ...
    if (!new_ped) {
      new_ped =
          (bRenameOnly) ? old_ped : ::Render->PSLibrary.AppendPED(old_ped);
      new_ped->m_Name = res_name.c_str();
      if (bRenameOnly)
        Msg("rename effect [%s]->[%s]", eff_full_name.c_str(),
            res_name.c_str());
      else
...
CCommandVar CParticleTool::CreateGroupFromSelected(CCommandVar p1,
                                                   CCommandVar p2) {
...
void CParticleTool::RemoveCurrent() { m_PList->RemoveSelItems(); }
...
void CParticleTool::FillReferenceList() {
  ...
  if (m_EditMode == emGroup) {
    if (m_EditPG->GetDefinition()) {
      xr_vector<PS::CPGDef::SEffect *>::const_iterator pe_it =
          m_EditPG->GetDefinition()->m_Effects.begin();
      xr_vector<PS::CPGDef::SEffect *>::const_iterator pe_it_e =
          m_EditPG->GetDefinition()->m_Effects.end();
      for (; pe_it != pe_it_e; ++pe_it) {
        fraLeftBar->refLB->AddItem((*pe_it)->m_EffectName.c_str(), NULL);
      }
      if (m_EditPG->GetDefinition()->m_Flags.test(
              PS::CPGDef::SEffect::flOnPlayChild))
        fraLeftBar->refLB->AddItem((*pe_it)->m_OnPlayChildName.c_str(), NULL);
      if (m_EditPG->GetDefinition()->m_Flags.test(
              PS::CPGDef::SEffect::flOnBirthChild))
        fraLeftBar->refLB->AddItem((*pe_it)->m_OnBirthChildName.c_str(), NULL);
      if (m_EditPG->GetDefinition()->m_Flags.test(
              PS::CPGDef::SEffect::flOnDeadChild))
...
void CParticleTool::CommandJumpToItem() {
  for (int i = 0; i < fraLeftBar->refLB->Count; ++i) {
    if (fraLeftBar->refLB->Selected[i]) {
      m_PList->SelectItem((fraLeftBar->refLB->Items->Strings[i]).c_str(), true,
                          false, true);
      break;
    }
  }
...
void CParticleTool::PlayCurrent(int idx) {
  ...
  case emGroup:
    if (idx > -1) {
      ...
      m_LibPED = ((PS::CParticleEffect *)m_EditPG->items[idx]._effect)
                     ->GetDefinition();
      m_EditPE->Compile(m_LibPED);
      m_EditPE->Play();
    } else {
  ...
  case etaMove: {
    if (Shift.Contains(ssCtrl)) {
      if (m_EditObject) {
        float dist = UI->ZFar();
        SRayPickInfo pinf;
        if (m_EditObject->RayPick(dist, UI->m_CurrentRStart, UI->m_CurrentRDir,
                                  Fidentity, &pinf))
...
void CParticleTool::RealRemoveAction() {
  if (m_LibPED) {
    ...
    m_LibPED->m_EActionList.erase(m_LibPED->m_EActionList.begin() +
                                  remove_action_num);
  }
  m_Flags.set(flRemoveAction, FALSE);
}

LPCSTR CParticleTool::GetInfo() { return 0; }
//------------------------------------------------------------------------------

void CParticleTool::SelectListItem(LPCSTR pref, LPCSTR name, bool bVal,
...
PS::CPGDef *CParticleTool::AppendPG(PS::CPGDef *src) {
  ...
  if (src) {
    ...
    strconcat(sizeof(pref), pref, folder_name.c_str(), "pg");
  AnsiString new_name =
      FHelper.GenerateName(pref, 2,
                           fastdelegate::bind<TFindObjectByName>(
                               &(::Render->PSLibrary), &CPSLibrary::FindByName),
...
bool CParticleTool::RayPick(const Fvector &start, const Fvector &dir,
                            float &dist, Fvector *pt, Fvector *n) {
  if (m_EditObject) {
    SRayPickInfo pinf;
...
void CParticleTool::EditPreviewPrefs() {
  ...
  PropValue *V;
  V = PHelper().CreateFlag32(items, "Parent\\Allow Animated", &m_Flags,
                             flAnimatedParent);
  ...
  V->OnChangeEvent.bind(this, &CParticleTool::OnChangeMotion);
  PHelper().CreateFloat(items, "Parent\\Motion Speed",
                        &m_ParentAnimator->Speed(), 0.f, 10000.f);
