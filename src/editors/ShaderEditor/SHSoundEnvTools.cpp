...
void CSHSoundEnvTools::OnChangeWAV	(PropValue* prop)
{
        ...
        if (m_SoundName.size()){
        m_PreviewSnd.create
(*m_SoundName,st_Effect,sg_Undefined); CSoundRender_Source* src=
(CSoundRender_Source*)m_PreviewSnd._handle(); m_Params.min_distance	=
src->m_fMinDist; m_Params.max_distance	= src->m_fMaxDist;
    }
...
void CSHSoundEnvTools::OnActivate()
{
        ...
        PropValue* V;
        V							=
PHelper().CreateChoose	(items,"Source\\WAVE name",	&m_SoundName,
smSoundSource); V->OnChangeEvent.bind
(this,&CSHSoundEnvTools::OnChangeWAV);
        ButtonValue* B				= PHelper().CreateButton
(items,"Source\\Controls", "Play,Stop",0); B->OnBtnClickEvent.bind
(this,&CSHSoundEnvTools::OnControlClick);
    ...
    // fill items
    FillItemList		();
    Ext.m_Items->SetOnModifiedEvent
(fastdelegate::bind<TOnModifiedEvent>(this,&CSHSoundEnvTools::Modified));
    Ext.m_Items->SetOnItemRenameEvent
(fastdelegate::bind<TOnItemRename>(this,&CSHSoundEnvTools::OnRenameItem));
    Ext.m_Items->SetOnItemRemoveEvent
(fastdelegate::bind<TOnItemRemove>(this,&CSHSoundEnvTools::OnRemoveItem));
...
void CSHSoundEnvTools::OnRender()
{
        ...
        DU_impl.DrawLineSphere	(Fvector().set(0,0,0), m_Params.max_distance,
clr1, true); DU_impl.DrawLineSphere	(Fvector().set(0,0,0),
m_Params.min_distance, clr0, false);
    }
}
//---------------------------------------------------------------------------
...
LPCSTR CSHSoundEnvTools::AppendItem(LPCSTR folder_name, LPCSTR parent_name)
{
    ...
    AnsiString pref		=
parent_name?AnsiString(parent_name):AnsiString(folder_name)+"env";
    m_LastSelection		=
FHelper.GenerateName(pref.c_str(),2,fastdelegate::bind<TFindObjectByName>(this,&CSHSoundEnvTools::ItemExist),false,true);
...
void CSHSoundEnvTools::OnRenameItem(LPCSTR old_full_name, LPCSTR new_full_name,
EItemType type)
{
        if (type==TYPE_OBJECT){
        ApplyChanges	();
...
void __fastcall CSHSoundEnvTools::OnRevResetClick(ButtonValue* V, bool& bModif,
bool& bSafe)
{
    switch (V->btn_num){
...
void CSHSoundEnvTools::RealUpdateProperties()
{
        ...
        ButtonValue* B			= 0;
        PHelper().CreateName	(items, "Name",
&S.name,  				m_CurrentItem);
        B=PHelper().CreateButton(items, "Environment\\Set",
"Identity,Reset", 	ButtonValue::flFirstOnly); B->OnBtnClickEvent.bind
(this,&CSHSoundEnvTools::OnRevResetClick); PropValue* V=0;
        V=PHelper().CreateToken32(items,"Environment\\Preset",
&S.Environment	       ,eax_environment); V->OnChangeEvent.bind
(this,&CSHSoundEnvTools::OnEnvChange); V=PHelper().CreateFloat	(items,
"Environment\\Size",					&S.EnvironmentSize
,EAXLISTENER_MINENVIRONMENTSIZE, 	EAXLISTENER_MAXENVIRONMENTSIZE
,0.01f,	3); V->OnChangeEvent.bind
(this,&CSHSoundEnvTools::OnEnvSizeChange); PHelper().CreateFloat	(items,
"Environment\\Diffusion",
&S.EnvironmentDiffusion,EAXLISTENER_MINENVIRONMENTDIFFUSION,EAXLISTENER_MAXENVIRONMENTDIFFUSION
,0.01f,	3); PHelper().CreateFloat	(items, "Room\\Room",
&S.Room                ,(float)EAXLISTENER_MINROOM,
(float)EAXLISTENER_MAXROOM				,1.f,	0);
        PHelper().CreateFloat	(items, "Room\\RoomHF",
&S.RoomHF              ,(float)EAXLISTENER_MINROOMHF,
(float)EAXLISTENER_MAXROOMHF			,1.f,	0);
        PHelper().CreateFloat	(items, "Distance Effects\\RoomRolloffFactor",
&S.RoomRolloffFactor   ,EAXLISTENER_MINROOMROLLOFFFACTOR,
EAXLISTENER_MAXROOMROLLOFFFACTOR		,0.01f,	3);
        PHelper().CreateFloat	(items, "Distance Effects\\AirAbsorptionHF",
&S.AirAbsorptionHF     ,EAXLISTENER_MINAIRABSORPTIONHF,
EAXLISTENER_MAXAIRABSORPTIONHF			,0.01f,	3);
        PHelper().CreateFloat	(items, "Reflections\\Reflections",
&S.Reflections         ,(float)EAXLISTENER_MINREFLECTIONS,
(float)EAXLISTENER_MAXREFLECTIONS		,1.f,	0);
        PHelper().CreateFloat	(items, "Reflections\\ReflectionsDelay",
&S.ReflectionsDelay    ,EAXLISTENER_MINREFLECTIONSDELAY,
EAXLISTENER_MAXREFLECTIONSDELAY			,0.01f,	3);
        PHelper().CreateFloat	(items, "Reverb\\Reverb",
&S.Reverb              ,(float)EAXLISTENER_MINREVERB,
(float)EAXLISTENER_MAXREVERB			,1.f,	0);
        PHelper().CreateFloat	(items, "Reverb\\ReverbDelay",
&S.ReverbDelay         ,EAXLISTENER_MINREVERBDELAY,
EAXLISTENER_MAXREVERBDELAY				,0.01f,	3);
        PHelper().CreateFloat	(items, "Decay\\DecayTime",
&S.DecayTime           ,EAXLISTENER_MINDECAYTIME,
EAXLISTENER_MAXDECAYTIME				,0.01f,	3);
        PHelper().CreateFloat	(items, "Decay\\DecayHFRatio",
&S.DecayHFRatio        ,EAXLISTENER_MINDECAYHFRATIO,
EAXLISTENER_MAXDECAYHFRATIO				,0.01f,	3);
    }
    Ext.m_ItemProps->AssignItems		(items);
    Ext.m_ItemProps->SetModifiedEvent
(fastdelegate::bind<TOnModifiedEvent>(this,&CSHSoundEnvTools::Modified));
}
//---------------------------------------------------------------------------
