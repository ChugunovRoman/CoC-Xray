...
public:
  virtual void Parse(CSHEngineTools *owner, DWORD type, LPCSTR key,
                     LPVOID data) {
    switch (type) {
    case xrPID_MATRIX:
...
CSHEngineTools::~CSHEngineTools() { MCString.clear(); }
//---------------------------------------------------------------------------
...
void CSHEngineTools::OnDestroy() {
  ...
  // free palette
  for (TemplateIt it = m_TemplatePalette.begin(); it != m_TemplatePalette.end();
       it++)
    xr_delete(*it);
...
xr_token preview_obj_token[] = {
    {"None", pvoNone}, {"-", pvoNone},           {"Plane", pvoPlane},
    {"Box", pvoBox},   {"Sphere", pvoSphere},    {"Teapot", pvoTeapot},
    {"-", pvoNone},    {"Custom...", pvoCustom}, {0, 0}};
...
bool CSHEngineTools::OnPreviewObjectRefChange(PropValue *sender, u32 &new_val) {
  ...
  case pvoTeapot:
    ...
    break;
  case pvoCustom:
    fn = m_PreviewObject ? m_PreviewObject->GetName() : "";
    if (!TfrmChoseItem::SelectItem(smObject, fn))
      return false;
    m_bCustomEditObject = true;
    break;
  }
  if (AnsiString(fn).LowerCase() !=
      AnsiString(m_PreviewObject ? m_PreviewObject->GetName() : "")
          .LowerCase()) {
    Lib.RemoveEditObject(m_PreviewObject);
...
void CSHEngineTools::OnActivate() {
  PropItemVec items;
  TokenValue<u32> *V =
      PHelper().CreateToken32(items, PrepareKey("Object", "Reference"),
                              &m_PreviewObjectType, preview_obj_token);
  ...
  Ext.m_Items->SetOnModifiedEvent(
      fastdelegate::bind<TOnModifiedEvent>(this, &CSHEngineTools::Modified));
  Ext.m_Items->SetOnItemRenameEvent(
      fastdelegate::bind<TOnItemRename>(this, &CSHEngineTools::OnRenameItem));
  Ext.m_Items->SetOnItemRemoveEvent(
      fastdelegate::bind<TOnItemRemove>(this, &CSHEngineTools::OnRemoveItem));
  inherited::OnActivate();
}
//---------------------------------------------------------------------------

void CSHEngineTools::OnDeactivate() { inherited::OnDeactivate(); }
    ...
    // Load blenders
    {
      ...
      while ((chunk = fs->open_chunk(chunk_id)) != NULL) {
        ...
        if (B) {
          ...
          std::pair<BlenderPairIt, bool> I =
              m_Blenders.insert(mk_pair(blender_name, B));
...
LPCSTR CSHEngineTools::AppendItem(LPCSTR folder_name, LPCSTR parent_name) {
  ...
  if (!parent) {
    ...
    if (!TfrmChoseItem::SelectItem(
            ...
            fastdelegate::bind<TOnChooseFillItems>(
                this, &CSHEngineTools::FillChooseTemplate)) ||
        !M)
      return 0;
    for (TemplateIt it = m_TemplatePalette.begin();
         it != m_TemplatePalette.end(); it++)
  ...
  while (!data.eof()) {
    ...
    case xrPID_TOKEN:
      sz = sizeof(xrP_TOKEN) +
           sizeof(xrP_TOKEN::Item) * (((xrP_TOKEN *)data.pointer())->Count);
      break;
    default:
      THROW2("xrPID_????");
    }
  ...
  // set name
  AnsiString pref = parent_name ? AnsiString(parent_name)
                                : AnsiString(folder_name) + "shader";
  m_LastSelection = FHelper.GenerateName(
      pref.c_str(), 2,
      fastdelegate::bind<TFindObjectByName>(this, &CSHEngineTools::ItemExist),
      false, true);
  B->getDescription().Setup(m_LastSelection.c_str());
  // insert blender
  std::pair<BlenderPairIt, bool> I =
      m_Blenders.insert(mk_pair(xr_strdup(m_LastSelection.c_str()), B));
  R_ASSERT2(I.second, "shader.xr - found duplicate name!!!");
  // insert to TreeView
  ExecCommand(COMMAND_UPDATE_LIST);
...
void CSHEngineTools::RealRenameItem(LPCSTR old_full_name,
                                    LPCSTR new_full_name) {
  ApplyChanges();
  ...
  std::pair<BlenderPairIt, bool> RES =
      m_Blenders.insert(mk_pair(xr_strdup(new_full_name), B));
...
LPCSTR CSHEngineTools::AppendConstant(CConstant *src, CConstant **dest) {
  ...
  C->dwReference = 1;
  char name[128];
  std::pair<ConstantPairIt, bool> I =
      m_Constants.insert(mk_pair(xr_strdup(GenerateConstantName(name)), C));
  VERIFY(I.second);
  if (dest)
    *dest = C;
...
LPCSTR CSHEngineTools::AppendMatrix(CMatrix *src, CMatrix **dest) {
  ...
  M->dwReference = 1;
  char name[128];
  std::pair<MatrixPairIt, bool> I =
      m_Matrices.insert(mk_pair(xr_strdup(GenerateMatrixName(name)), M));
  VERIFY(I.second);
  if (dest)
    *dest = M;
  return I.first->first;
...
void CSHEngineTools::OnRenameItem(LPCSTR old_full_name, LPCSTR new_full_name,
                                  EItemType type) {
  if (type == TYPE_OBJECT)
...
void CSHEngineTools::CollapseConstant(LPSTR name) {
  ...
  VERIFY(C);
  C->dwReference--;
  for (ConstantPairIt c = m_OptConstants.begin(); c != m_OptConstants.end();
       c++) {
...
void CSHEngineTools::OnDeviceCreate() { ResetShaders(); }
...
void CSHEngineTools::OnShowHint(AStringVec &ss) {
  if (m_PreviewObject) {
    float dist = UI->ZFar();
    SRayPickInfo pinf;
    if (m_PreviewObject->RayPick(dist, UI->m_CurrentRStart, UI->m_CurrentRDir,
                                 Fidentity, &pinf)) {
      ...
      ss.push_back(AnsiString("LC Shader: ") +
                   AnsiString(surf->_ShaderXRLCName()));
      ss.push_back(AnsiString("Game Mtl: ") + AnsiString(surf->_GameMtlName()));
      ss.push_back(
          AnsiString("2 Sided: ") +
          AnsiString(surf->m_Flags.is(CSurface::sf2Sided) ? "on" : "off"));
    }
  }
}
