...
void SGameMtl::FillProp(PropItemVec &items, ListItem *owner) {
  ...
  PHelper().CreateFlag32(items, "Flags\\Suppress Shadows", &Flags,
                         flSuppressShadows);
  PHelper().CreateFlag32(items, "Flags\\Suppress Wallmarks", &Flags,
                         flSuppressWallmarks);
  PHelper().CreateFlag32(items, "Flags\\Actor Obstacle", &Flags,
                         flActorObstacle);
  PHelper().CreateFlag32(items, "Flags\\Bullet No Ricoshet", &Flags,
                         flNoRicoshet);
  // physics part
  PHelper().CreateFloat(items, "Physics\\Friction", &fPHFriction, 0.f, 100.f,
                        0.001f, 3);
  PHelper().CreateFloat(items, "Physics\\Damping", &fPHDamping, 0.001f, 100.f,
                        0.001f, 3);
  PHelper().CreateFloat(items, "Physics\\Spring", &fPHSpring, 0.001f, 100.f,
                        0.001f, 3);
  PHelper().CreateFloat(items, "Physics\\Bounce start vel",
                        &fPHBounceStartVelocity, 0.f, 100.f, 0.01f, 2);
  PHelper().CreateFloat(items, "Physics\\Bouncing", &fPHBouncing, 0.f, 1.f,
                        0.001f, 3);
  // factors
  PHelper().CreateFloat(items, "Factors\\Bounce Damage", &fBounceDamageFactor,
                        0.f, 100.f, 0.1f, 1);
  PHelper().CreateFloat(items, "Factors\\Injurious", &fInjuriousSpeed, 0.f,
                        10000.f);
  PHelper().CreateFloat(items, "Factors\\Shooting (1-went through)",
                        &fShootFactor);
  PHelper().CreateFloat(items, "Factors\\Shooting MP (1-went through)",
                        &fShootFactorMP);
  PHelper().CreateFloat(items, "Factors\\Transparency (1-full transp)",
                        &fVisTransparencyFactor);
  PHelper().CreateFloat(items, "Factors\\Sound occlusion (1-full hear)",
                        &fSndOcclusionFactor);
  PHelper().CreateFloat(items, "Factors\\Flotation (1-full passable)",
                        &fFlotationFactor);

  PHelper().CreateFloat(items, "Factors\\Density Factor", &fDensityFactor, 0.0f,
                        1000.0f, 1.0f, 1);
}
...
BOOL CGameMtlLibrary::UpdateMtlPairs(SGameMtl *src) {
  ...
  for (GameMtlIt m1_it = materials.begin(); m1_it != materials.end(); ++m1_it) {
    ...
    if ((!M0->Flags.is(SGameMtl::flDynamic)) &&
        (!M1->Flags.is(SGameMtl::flDynamic))) {
...
IC u32 SetMask(u32 mask, Flags32 flags, u32 flag) {
  return mask ? (mask | (flags.is(flag) ? PropItem::flCBChecked
                                        : PropItem::flDisabled))
              : 0;
}
...
IC SGameMtlPair *GetLastParentValue(SGameMtlPair *who, u32 flag) {
  ...
  if ((GAMEMTL_NONE_ID == who->GetParent()) || (who->OwnProps.is(flag)))
    ...
    return GetLastParentValue(who->m_Owner->GetMaterialPair(who->GetParent()),
                              flag);
}
...
IC BOOL ValidateParent(SGameMtlPair *who, SGameMtlPair *parent) {
  ...
  if (who == parent)
    ...
    return ValidateParent(
        who, parent->m_Owner->GetMaterialPair(parent->GetParent()));
}
...
BOOL SGameMtlPair::SetParent(int parent) {
  ...
  for (GameMtlPairIt it = m_Owner->FirstMaterialPair();
       it != m_Owner->LastMaterialPair(); it++) {
...
void __fastcall SGameMtlPair::FillChooseMtl(ChooseItemVec &items, void *param) {
  for (GameMtlIt m0_it = m_Owner->FirstMaterial();
       m0_it != m_Owner->LastMaterial(); m0_it++) {
    SGameMtl *M0 = *m0_it;
    for (GameMtlIt m1_it = m_Owner->FirstMaterial();
         m1_it != m_Owner->LastMaterial(); m1_it++) {
      SGameMtl *M1 = *m1_it;
      GameMtlPairIt p_it = GMLib.GetMaterialPairIt(M0->GetID(), M1->GetID());
      if (p_it != GMLib.LastMaterialPair())
        items.push_back(
            SChooseItem(GMLib.MtlPairToName(M0->GetID(), M1->GetID()), ""));
    }
  }
}

void __fastcall SGameMtlPair::OnParentClick(ButtonValue *V, bool &bModif,
                                            bool &bSafe) {
  bModif = false;
  switch (V->btn_num) {
  case 0: {
    ...
    AnsiString nm =
        P ? m_Owner->MtlPairToName(P->GetMtl0(), P->GetMtl1()) : NONE_CAPTION;

    if (TfrmChoseItem::SelectItem(smCustom, MP, 1,
                                  (nm == NONE_CAPTION) ? 0 : nm.c_str(),
                                  fastdelegate::bind<TOnChooseFillItems>(
                                      this, &SGameMtlPair::FillChooseMtl))) {
      if (MP) {
        int m0, m1;
...
void __fastcall SGameMtlPair::OnCommandClick(ButtonValue *V, bool &bModif,
                                             bool &bSafe) {
  bModif = false;
  switch (V->btn_num) {
  case 0: {
    ...
    AnsiString nm =
        P ? m_Owner->MtlPairToName(P->GetMtl0(), P->GetMtl1()) : NONE_CAPTION;
    if (TfrmChoseItem::SelectItem(smCustom, MP, 128, 0,
                                  fastdelegate::bind<TOnChooseFillItems>(
                                      this, &SGameMtlPair::FillChooseMtl))) {
      if (MP) {
        AStringVec lst;
...
void SGameMtlPair::FillProp(PropItemVec &items) {
  ...
  B = PHelper().CreateButton(
      items, "Parent",
      P ? m_Owner->MtlPairToName(P->GetMtl0(), P->GetMtl1()) : NONE_CAPTION, 0);
  B->OnBtnClickEvent.bind(this, &SGameMtlPair::OnParentClick);

  propBreakingSounds =
      PHelper().CreateChoose(items, "Breaking Sounds", &BreakingSounds,
                             smSoundSource, 0, 0, GAMEMTL_SUBITEM_COUNT);
  propStepSounds =
      PHelper().CreateChoose(items, "Step Sounds", &StepSounds, smSoundSource,
                             0, 0, GAMEMTL_SUBITEM_COUNT + 2);
  propCollideSounds =
      PHelper().CreateChoose(items, "Collide Sounds", &CollideSounds,
                             smSoundSource, 0, 0, GAMEMTL_SUBITEM_COUNT);
  propCollideParticles =
      PHelper().CreateChoose(items, "Collide Particles", &CollideParticles,
                             smParticles, 0, 0, GAMEMTL_SUBITEM_COUNT);
  propCollideMarks =
      PHelper().CreateChoose(items, "Collide Marks", &CollideMarks, smTexture,
                             0, 0, GAMEMTL_SUBITEM_COUNT);

  propBreakingSounds->Owner()->m_Flags.assign(
      SetMask(show_CB, OwnProps, flBreakingSounds));
  propStepSounds->Owner()->m_Flags.assign(
      SetMask(show_CB, OwnProps, flStepSounds));
  propCollideSounds->Owner()->m_Flags.assign(
      SetMask(show_CB, OwnProps, flCollideSounds));
  propCollideParticles->Owner()->m_Flags.assign(
      SetMask(show_CB, OwnProps, flCollideParticles));
  propCollideMarks->Owner()->m_Flags.assign(
      SetMask(show_CB, OwnProps, flCollideMarks));
...
SGameMtlPair *CGameMtlLibrary::CreateMaterialPair(int m0, int m1,
                                                  SGameMtlPair *parent) {
...
SGameMtlPair *CGameMtlLibrary::AppendMaterialPair(int m0, int m1,
                                                  SGameMtlPair *parent) {
...
void CGameMtlLibrary::RemoveMaterialPair(GameMtlPairIt rem_it) {
  ...
  for (GameMtlPairIt it = material_pairs.begin(); it != material_pairs.end();
       it++)
...
void SGameMtlPair::Save(IWriter &fs) {
  ...
  if (ID_parent != GAMEMTL_NONE_ID) {
    ...
    if ((0 != (P = GetLastParentValue(this, flCollideParticles))) &&
        (P != this))
  ...
  /*
          ...
          OwnProps.zero();
          if (!BreakingSounds.IsEmpty())
     OwnProps.set(flBreakingSounds,TRUE); if (!StepSounds.IsEmpty())
     OwnProps.set(flStepSounds,TRUE); if (!CollideSounds.IsEmpty())
     OwnProps.set(flCollideSounds,TRUE); if
     (!CollideParticles.IsEmpty())OwnProps.set(flCollideParticles,TRUE); if
     (!CollideMarks.IsEmpty())	OwnProps.set(flCollideMarks,TRUE);
      }
  */
  // save
