...
void CSHGameMtlTools::OnActivate() {
  ...
  Ext.m_Items->SetOnModifiedEvent(
      fastdelegate::bind<TOnModifiedEvent>(this, &CSHGameMtlTools::Modified));
  Ext.m_Items->SetOnItemRenameEvent(
      fastdelegate::bind<TOnItemRename>(this, &CSHGameMtlTools::OnRenameItem));
  Ext.m_Items->SetOnItemRemoveEvent(
      fastdelegate::bind<TOnItemRemove>(this, &CSHGameMtlTools::OnRemoveItem));
  inherited::OnActivate();
}

void CSHGameMtlTools::OnDeactivate() { inherited::OnDeactivate(); }

void CSHGameMtlTools::OnFrame() { inherited::OnFrame(); }
//---------------------------------------------------------------------------
...
void CSHGameMtlTools::OnDestroy() { m_bModified = FALSE; }
...
void CSHGameMtlTools::FillItemList() {
  ...
  // fill items
  ListItemsVec items;
  for (GameMtlIt m_it = GMLib.FirstMaterial(); m_it != GMLib.LastMaterial();
       m_it++)
  ...
  if (!parent) {
    if (!TfrmChoseItem::SelectItem(
            ...
            fastdelegate::bind<TOnChooseFillItems>(
                this, &CSHGameMtlTools::FillChooseMtlType)) ||
        !M)
      return 0;
  }
  AnsiString pref =
      parent_name ? AnsiString(parent_name) : AnsiString(folder_name) + M;
  m_LastSelection = FHelper.GenerateName(
      pref.c_str(), 2,
      fastdelegate::bind<TFindObjectByName>(this, &CSHGameMtlTools::ItemExist),
      false, true);
...
void CSHGameMtlTools::OnRenameItem(LPCSTR old_full_name, LPCSTR new_full_name,
                                   EItemType type) {
  if (type == TYPE_OBJECT) {
...
void CSHGameMtlTools::ResetCurrentItem() { m_Mtl = 0; }
//---------------------------------------------------------------------------

void CSHGameMtlTools::RealUpdateList() { FillItemList(); }
//------------------------------------------------------------------------------
