...
//---------------------------------------------------------------------------
xr_token mode_token[] = {{"Programmable", CMatrix::modeProgrammable},
                         {"TCM", CMatrix::modeTCM},
                         {"Spherical Reflection", CMatrix::modeS_refl},
                         {"Cube Reflection", CMatrix::modeC_refl},
                         {0, 0}};
//---------------------------------------------------------------------------
void __fastcall CSHEngineTools::FillMatrixProps(PropItemVec &items, LPCSTR pref,
                                                LPSTR name) {
  CMatrix *M = AppendMatrix(name);
  R_ASSERT(M);

  PHelper().CreateToken32(items, PrepareKey(pref, "Mode"), &M->dwMode,
                          mode_token);

  if (M->dwMode == CMatrix::modeTCM) {
    PHelper().CreateFlag32(items, PrepareKey(pref, "Scale enabled"),
                           &M->tcm_flags, CMatrix::tcmScale);
    PHelper().CreateWave(items, PrepareKey(pref, "Scale U"), &M->scaleU);
    PHelper().CreateWave(items, PrepareKey(pref, "Scale V"), &M->scaleV);
    PHelper().CreateFlag32(items, PrepareKey(pref, "Rotate enabled"),
                           &M->tcm_flags, CMatrix::tcmRotate);
    PHelper().CreateWave(items, PrepareKey(pref, "Rotate"), &M->rotate);
    PHelper().CreateFlag32(items, PrepareKey(pref, "Scroll enabled"),
                           &M->tcm_flags, CMatrix::tcmScroll);
...
void __fastcall CSHEngineTools::MCOnDraw(PropValue *sender,
                                         xr_string &draw_val) {
  if (draw_val[0] != '$')
    draw_val = "Custom";
}
...
void __fastcall CSHEngineTools::FillConstProps(PropItemVec &items, LPCSTR pref,
                                               LPSTR name) {
...
bool CSHEngineTools::NameOnAfterEdit(PropValue *sender, xr_string &new_name) {
  ...
  if (FHelper.NameAfterEdit((TElTreeItem *)m_CurrentItem->Item(), V->GetValue(),
                            nn)) {
    new_name = nn.c_str();
...
void CSHEngineTools::RealUpdateProperties() {
  ...
  if (m_CurrentBlender) { // fill Tree
    ...
    CTextValue *V =
        PHelper().CreateCText(items, "Name", desc->cName, sizeof(desc->cName));
    ...
    while (!data.eof()) {
      ...
      case xrPID_TOKEN: {
        ...
        PHelper().CreateTokenSH(
            ...
            (TokenValueSH::Item *)(LPBYTE(data.pointer()) + sizeof(xrP_TOKEN)),
            V->Count);
      } break;
      case xrPID_MATRIX: {
        sz = sizeof(string64);
        ...
        CListValue *P =
            PHelper().CreateCList(items, PrepareKey(marker_text.c_str(), key),
                                  V, sz, &*MCString.begin(), MCString.size());
        AnsiString pref =
            AnsiString(PrepareKey(marker_text.c_str(), "Custom ").c_str()) +
            key;
        if (V && V[0] && (*V != '$'))
      ...
      case xrPID_TEXTURE:
        sz = sizeof(string64);
        PHelper().CreateTexture(items, PrepareKey(marker_text.c_str(), key),
                                (LPSTR)data.pointer(), sz);
        break;
      case xrPID_INTEGER: {
        sz = sizeof(xrP_Integer);
        xrP_Integer *V = (xrP_Integer *)data.pointer();
        PHelper().CreateS32(items, PrepareKey(marker_text.c_str(), key),
                            &V->value, V->min, V->max, 1);
      } break;
      case xrPID_FLOAT: {
        sz = sizeof(xrP_Float);
        xrP_Float *V = (xrP_Float *)data.pointer();
        PHelper().CreateFloat(items, PrepareKey(marker_text.c_str(), key),
                              &V->value, V->min, V->max, 0.01f, 2);
      } break;
      case xrPID_BOOL: {
        sz = sizeof(xrP_BOOL);
        xrP_BOOL *V = (xrP_BOOL *)data.pointer();
        PHelper().CreateBOOL(items, PrepareKey(marker_text.c_str(), key),
                             &V->value);
      } break;
      default:
