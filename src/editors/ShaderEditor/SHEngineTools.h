...
class CSHEngineTools : public ISHTools {
  ...
  CMemoryWriter m_BlenderStream; // пользоваться функциями обновления стрима для
                                 // синхронизации
  bool m_bUpdateCurrent;         // если менялся объект непосредственно
                                 // Update____From___()
...
public:
  ...
  virtual void __stdcall OnRenameItem(LPCSTR old_full_name,
                                      LPCSTR new_full_name, EItemType type);
