...
void CShaderTool::Modified() { Current()->Modified(); }
//---------------------------------------------------------------------------
...
bool CShaderTool::OnCreate() {
  ...
  // create props
  m_Items =
      TItemList::CreateForm("Items", fraLeftBar->paItemList, alClient,
                            TItemList::ilEditMenu | TItemList::ilDragAllowed |
                                TItemList::ilFolderStore);
  m_Items->SetOnItemsFocusedEvent(
      fastdelegate::bind<TOnILItemsFocused>(this, &CShaderTool::OnItemFocused));
  m_ItemProps = TProperties::CreateForm("Item Properties",
                                        fraLeftBar->paShaderProps, alClient);
  m_PreviewProps = TProperties::CreateForm(
      "Preview Properties", fraLeftBar->paPreviewProps, alClient);
...
void CShaderTool::ZoomObject(BOOL bOnlySel) { Current()->ZoomObject(bOnlySel); }
...
void CShaderTool::OnShowHint(AStringVec &ss) { Current()->OnShowHint(ss); }

void CShaderTool::ApplyChanges() { Current()->ApplyChanges(); }
...
LPCSTR CShaderTool::GetInfo() { return 0; }
...
bool CShaderTool::Load(LPCSTR name) { return true; }
...
void CShaderTool::RegisterTools() {
  for (int k = aeFirstTool; k < aeMaxTools; k++) {
    ...
    switch (k) {
    case aeEngine:
      tools = xr_new<CSHEngineTools>(ISHInit(EToolsID(k), m_Items,
                                             fraLeftBar->tsEngine, m_ItemProps,
                                             m_PreviewProps));
      break;
    case aeCompiler:
      tools = xr_new<CSHCompilerTools>(ISHInit(EToolsID(k), m_Items,
                                               fraLeftBar->tsCompiler,
                                               m_ItemProps, m_PreviewProps));
      break;
    case aeMtl:
      tools = xr_new<CSHGameMtlTools>(ISHInit(EToolsID(k), m_Items,
                                              fraLeftBar->tsMaterial,
                                              m_ItemProps, m_PreviewProps));
      break;
    case aeMtlPair:
      tools = xr_new<CSHGameMtlPairTools>(ISHInit(EToolsID(k), m_Items,
                                                  fraLeftBar->tsMaterialPair,
                                                  m_ItemProps, m_PreviewProps));
      break;
      //    	case aeSoundEnv:	tools = xr_new<CSHSoundEnvTools>
      //    (ISHInit( EToolsID(k),	m_Items,	fraLeftBar->tsSoundEnv,
      //    m_ItemProps,	m_PreviewProps));	break;
    }
    R_ASSERT(tools);
...
bool CShaderTool::RayPick(const Fvector &start, const Fvector &dir, float &dist,
