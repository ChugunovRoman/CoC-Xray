...
void CSHCompilerTools::OnActivate() {
  ...
  Ext.m_Items->SetOnModifiedEvent(
      fastdelegate::bind<TOnModifiedEvent>(this, &CSHCompilerTools::Modified));
  Ext.m_Items->SetOnItemRenameEvent(
      fastdelegate::bind<TOnItemRename>(this, &CSHCompilerTools::OnRenameItem));
  Ext.m_Items->SetOnItemRemoveEvent(
      fastdelegate::bind<TOnItemRemove>(this, &CSHCompilerTools::OnRemoveItem));

  inherited::OnActivate();
}
void CSHCompilerTools::OnDeactivate() { inherited::OnDeactivate(); }

void CSHCompilerTools::OnFrame() { inherited::OnFrame(); }
//---------------------------------------------------------------------------
...
void CSHCompilerTools::OnDestroy() { m_bModified = FALSE; }
...
LPCSTR CSHCompilerTools::AppendItem(LPCSTR folder_name, LPCSTR parent_name) {
  ...
  m_LastSelection = FHelper.GenerateName(
      pref.c_str(), 2,
      fastdelegate::bind<TFindObjectByName>(this, &CSHCompilerTools::ItemExist),
      false, true);
...
void CSHCompilerTools::OnRenameItem(LPCSTR old_full_name, LPCSTR new_full_name,
                                    EItemType type) {
  if (type == TYPE_OBJECT) {
    ApplyChanges();
...
void CSHCompilerTools::ResetCurrentItem() { m_Shader = 0; }
