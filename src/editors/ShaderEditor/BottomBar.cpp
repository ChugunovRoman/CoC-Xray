...
void __fastcall TfraBottomBar::ClickOptionsMenuItem(TObject *Sender) {
  ...
  if (mi) {
    ...
    else if (mi == miRenderWithTextures)
      psDeviceFlags.set(rsRenderTextures, mi->Checked);
    else if (mi == miMuteSounds)
      psDeviceFlags.set(rsMuteSounds, mi->Checked);
    else if (mi == miLightScene)
      psDeviceFlags.set(rsLighting, mi->Checked);
    else if (mi == miRenderLinearFilter)
      psDeviceFlags.set(rsFilterLinear, mi->Checked);
    else if (mi == miRenderEdgedFaces)
      psDeviceFlags.set(rsEdgedFaces, mi->Checked);
    else if (mi == miFog)
      psDeviceFlags.set(rsFog, mi->Checked);
    else if (mi == miRealTime)
      psDeviceFlags.set(rsRenderRealTime, mi->Checked);
    else if (mi == miDrawSafeRect)
      ExecCommand(COMMAND_TOGGLE_SAFE_RECT);
    else if (mi == miRenderFillPoint)
      EDevice.dwFillMode = D3DFILL_POINT;
    else if (mi == miRenderFillWireframe)
      EDevice.dwFillMode = D3DFILL_WIREFRAME;
    else if (mi == miRenderFillSolid)
      EDevice.dwFillMode = D3DFILL_SOLID;
    else if (mi == miRenderShadeFlat)
      EDevice.dwShadeMode = D3DSHADE_FLAT;
    else if (mi == miRenderShadeGouraud)
      EDevice.dwShadeMode = D3DSHADE_GOURAUD;
    else if (mi == miRenderHWTransform) {
      HW.Caps.bForceGPU_SW = !mi->Checked;
      UI->Resize();
    }
  }
  UI->RedrawScene();
...
void __fastcall TfraBottomBar::fsStorageRestorePlacement(TObject *Sender) {
  // fill mode
  if (miRenderFillPoint->Checked)
    EDevice.dwFillMode = D3DFILL_POINT;
  else if (miRenderFillWireframe->Checked)
    EDevice.dwFillMode = D3DFILL_WIREFRAME;
  else if (miRenderFillSolid->Checked)
    EDevice.dwFillMode = D3DFILL_SOLID;
  // shade mode
  if (miRenderShadeFlat->Checked)
    EDevice.dwShadeMode = D3DSHADE_FLAT;
  else if (miRenderShadeGouraud->Checked)
    EDevice.dwShadeMode = D3DSHADE_GOURAUD;
  // hw transform
  ...
  if (N200->Checked)
    QualityClick(N200);
  else if (N150->Checked)
    QualityClick(N150);
  else if (N125->Checked)
    QualityClick(N125);
  else if (N100->Checked)
    QualityClick(N100);
  else if (N75->Checked)
    QualityClick(N75);
  else if (N50->Checked)
  ...
  /*
      // append weathers
      CEnvironment::EnvsMapIt
     _I=g_pGamePersistent->Environment().WeatherCycles.begin();
      CEnvironment::EnvsMapIt
     _E=g_pGamePersistent->Environment().WeatherCycles.end(); for (; _I!=_E;
     _I++){ mi				= xr_new<TMenuItem>((TComponent*)0);
          mi->Caption 	= *_I->first;
          mi->OnClick 	= miWeatherClick;
              mi->RadioItem	= true;
          miWeather->Add	(mi);
...
void __fastcall TfraBottomBar::pmOptionsPopup(TObject *Sender) {
  ...
  for (int i = 0; i < miWeather->Count; ++i) {
    ...
    BOOL bch;
    bch = ((EPrefs->sWeather.size()) &&
           (0 == stricmp(mi->Caption.c_str(), EPrefs->sWeather.c_str()))) ||
  ...
  /*
      ...
      if (mi){
          if (mi->Tag==0){
              ...
              TProperties* P 		= TProperties::CreateModalForm("Weather
     properties"); CEnvironment& env	= g_pGamePersistent->Environment();
              PropItemVec items;
              ...
              PHelper().CreateFloat	(items,"Speed",
     &sp, 		1.f,10000.f,1.f,1);
...
void __fastcall TfraBottomBar::ebMacroMouseDown(TObject *Sender,
  ...
  for (u32 k = 0; k < CMD->sub_commands.size(); ++k) {
    ...
    mi = xr_new<TMenuItem>((TComponent *)0);
    mi->Caption = AnsiString().sprintf(
        "%d: %s", k + 1, bValid ? xr_string(SUB->p0).c_str() : "<empty>");
