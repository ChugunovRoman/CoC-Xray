...
class XR_EPROPS_API TProperties : public TForm {
  ...
  void __fastcall tvPropertiesItemDraw(TObject *Sender, TElTreeItem *Item,
                                       ...
                                       int SectionIndex);
  void __fastcall tvPropertiesMouseDown(TObject *Sender, TMouseButton Button,
  ...
  void __fastcall tvPropertiesMouseUp(TObject *Sender, TMouseButton Button,
                                      TShiftState Shift, int X, int Y);
  void __fastcall tvPropertiesMouseMove(TObject *Sender, TShiftState Shift,
                                        int X, int Y);
  void __fastcall ExpandSelected1Click(TObject *Sender);
  void __fastcall CollapseSelected1Click(TObject *Sender);
  void __fastcall ebOKClick(TObject *Sender);
  void __fastcall ebCancelClick(TObject *Sender);
  void __fastcall tvPropertiesShowLineHint(TObject *Sender, TElTreeItem *Item,
                                           TElHeaderSection *Section,
                                           TElFString &Text,
                                           THintWindow *HintWindow,
                                           TPoint &MousePos, bool &DoShowHint);
  void __fastcall tvPropertiesCompareItems(TObject *Sender, TElTreeItem *Item1,
...
private: // User declarations
  ...
  void OutText(LPCSTR text, TCanvas *Surface, TRect &R, bool bEnable,
               TGraphic *g = 0, bool bArrow = false);
...
public: // User declarations
  ...
  static TProperties *CreateForm(const AnsiString &title,
                                 TWinControl *parent = 0, TAlign align = alNone,
                                 TOnModifiedEvent modif = 0,
                                 TOnItemFocused focused = 0,
                                 TOnCloseEvent close = 0,
                                 u32 flags = plFolderStore | plFullExpand);
  static TProperties *CreateModalForm(const AnsiString &title,
                                      bool bShowButtonsBar = true,
                                      TOnModifiedEvent modif = 0,
                                      TOnItemFocused focused = 0,
                                      TOnCloseEvent close = 0,
                                      u32 flags = plFolderStore | plFullExpand);
  static void DestroyForm(TProperties *&props);
  static int EditPropertiesModal(PropItemVec &values, LPCSTR title,
                                 bool bShowButtonsBar = true,
                                 TOnModifiedEvent modif = 0,
                                 TOnItemFocused focused = 0,
                                 TOnCloseEvent close = 0,
  ...
  bool __fastcall IsFocused() {
    return tvProperties->Focused() || seNumber->Focused() || edText->Focused();
  }
  void __fastcall SetModifiedEvent(TOnModifiedEvent modif = 0) {
  ...
  void __fastcall SaveParams(TFormStorage *fs) {
    fs->WriteInteger(AnsiString().sprintf("%s_column0_width", Caption.c_str()),
                     tvProperties->HeaderSections->Item[0]->Width);
    fs->WriteInteger(AnsiString().sprintf("%s_column1_width", Caption.c_str()),
                     tvProperties->HeaderSections->Item[1]->Width);
    fs->WriteInteger(AnsiString().sprintf("%s_draw_thm", Caption.c_str()),
                     miDrawThumbnails->Checked);
    fs->WriteInteger(AnsiString().sprintf("%s_auto_expand", Caption.c_str()),
                     miAutoExpand->Checked);
    fs->WriteInteger(AnsiString().sprintf("%s_fp_width", Caption.c_str()),
                     paFolders->Width);
    fs->WriteInteger(AnsiString().sprintf("%s_fp_height", Caption.c_str()),
                     paFolders->Height);
  }
  void __fastcall RestoreParams(TFormStorage *fs) {
    tvProperties->HeaderSections->Item[0]->Width = fs->ReadInteger(
        AnsiString().sprintf("%s_column0_width", Caption.c_str()),
        tvProperties->HeaderSections->Item[0]->Width);
    tvProperties->HeaderSections->Item[1]->Width = fs->ReadInteger(
        AnsiString().sprintf("%s_column1_width", Caption.c_str()),
        tvProperties->HeaderSections->Item[1]->Width);
    miDrawThumbnails->Checked = fs->ReadInteger(
        AnsiString().sprintf("%s_draw_thm", Caption.c_str()), false);
    miAutoExpand->Checked = fs->ReadInteger(
        AnsiString().sprintf("%s_auto_expand", Caption.c_str()), false);
    paFolders->Width = fs->ReadInteger(
        AnsiString().sprintf("%s_fp_width", Caption.c_str()), paFolders->Width);
    paFolders->Height =
        fs->ReadInteger(AnsiString().sprintf("%s_fp_height", Caption.c_str()),
                        paFolders->Height);
    RefreshForm();
  }
  ...
  static IC bool IsItemType(TElTreeItem *item, EPropType type) {
    return item->Tag && (((PropItem *)item->Tag)->Type() == type);
  }
  static IC EPropType GetItemType(TElTreeItem *item) {
    return item->Tag ? ((PropItem *)item->Tag)->Type() : (EPropType)0;
  }
