...
//---------------------------------------------------------------------------
__fastcall TfrmText::TfrmText(TComponent *Owner) : TForm(Owner) {
  m_Text = 0;
  OnApplyClick = 0;
  OnCloseClick = 0;
  OnCodeInsight = 0;
...
//---------------------------------------------------------------------------
void __fastcall TfrmText::FormKeyDown(TObject *Sender, WORD &Key,
                                      TShiftState Shift) {
  if (Key == VK_ESCAPE)
    ebCancel->Click();
...
TfrmText *__fastcall TfrmText::CreateForm(AnsiString &txt, LPCSTR caption,
                                          u32 flags, int lim, LPCSTR apply_name,
                                          TOnApplyClick on_apply,
                                          TOnCloseClick on_close,
...
bool __fastcall TfrmText::RunEditor(AnsiString &txt, LPCSTR caption, u32 flags,
                                    int lim, LPCSTR apply_name,
                                    TOnApplyClick on_apply,
                                    TOnCloseClick on_close,
                                    TOnCodeInsight on_insight) {
  TfrmText *form = xr_new<TfrmText>((TComponent *)0);
...
void TfrmText::OutLineNumber() {
  sbStatusPanel->Panels->Items[0]->Text = AnsiString().sprintf(
      "%3d:%3d", mmText->CaretPos.y + 1, mmText->CaretPos.x + 1);
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmText::mmTextKeyUp(TObject *Sender, WORD &Key,
  ...
  if ((Key == VK_SPACE) && (Shift.Contains(ssCtrl)) &&
      (Shift.Contains(ssShift)) && !OnCodeInsight.empty()) {
...
void TfrmText::InsertTextCP(const AnsiString &line, bool bCommas) {
  AnsiString txt =
      (bCommas) ? AnsiString("\"") + line + AnsiString("\"") : line;
  if (!txt.IsEmpty()) {
