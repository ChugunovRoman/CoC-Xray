...
void __fastcall TfrmItemDialog::FormKeyDown(TObject *Sender, WORD &Key,
                                            TShiftState Shift) {
  if (Key == VK_ESCAPE)
    ModalResult = -1;
}
...
int __fastcall TfrmItemDialog::Run(LPCSTR caption, LPCSTR msg, LPCSTR buttons) {
  ...
  int cnt = _GetItemCount(buttons);
  R_ASSERT(cnt < 9);
  AnsiString nm;
  for (int k = 0; k < cnt; k++) {
    _GetItem(buttons, k, nm);
    TExtBtn *E = dynamic_cast<TExtBtn *>(
        form->FindComponent(AnsiString().sprintf("eb%d", k)));
    VERIFY(E);
    E->Caption = nm;
    E->Visible = true;
    E->Tag = k;
