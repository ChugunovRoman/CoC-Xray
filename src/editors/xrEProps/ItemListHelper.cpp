...
ListItem *CListHelper::CreateItem(ListItemsVec &items, LPCSTR key, int type,
                                  u32 item_flags, void *object) {
...
bool CListHelper::NameAfterEdit(ListItem *sender, LPCSTR value, shared_str &N) {
  ...
  VERIFY(sender);
  TElTreeItem *node = (TElTreeItem *)sender->Item();
  VERIFY(node);
  ...
  node->Text = N.c_str();
  AnsiString tmp;
  _ReplaceItem(*sender->key, _GetItemCount(*sender->key, '\\') - 1, N.c_str(),
               tmp, '\\');
