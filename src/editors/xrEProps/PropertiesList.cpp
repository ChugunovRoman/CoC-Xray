...
const LPSTR TEXTUREString[TSTRING_COUNT] = {"Custom...", "-", "$null",
                                            "$base0"};
//---------------------------------------------------------------------------
  ...
  if (node) {
    ...
    //s
            for (TElTreeItem* item=node; item; item=item->GetNext()){
                ...
                if (V){
                        PropValuePairIt
    it=std::find(m_Values.begin(),m_Values.end(),V); VERIFY(it!=m_Values.end());
                    if (it){
                                            m_Values.erase(it);
                                            xr_delete(V);
                    }
...
void __fastcall TProperties::SelectItem(const AnsiString &full_name) {
  m_FirstClickItem = FHelper.RestoreSelection(
      tvProperties, FHelper.ExpandItem(tvProperties, full_name), false);
}
//---------------------------------------------------------------------------
...
TProperties *TProperties::CreateForm(const AnsiString &title,
                                     TWinControl *parent, TAlign align,
                                     TOnModifiedEvent modif,
                                     TOnItemFocused focused,
                                     TOnCloseEvent on_close, u32 flags) {
  ...
  if (props->m_Flags.is_any(plItemFolders)) {
    ...
    props->m_Folders = TItemList::CreateForm(
        "Folders", props->paFolders, alClient,
        TItemList::ilSuppressIcon | TItemList::ilFolderStore |
            TItemList::ilSuppressStatus |
            (props->m_Flags.is(plMultiSelect) ? TItemList::ilMultiSelect : 0));
    props->m_Folders->OnItemFocusedEvent.bind(props,
                                              &TProperties::OnFolderFocused);
  } else {
...
TProperties *TProperties::CreateModalForm(const AnsiString &title,
                                          bool bShowButtonsBar,
                                          TOnModifiedEvent modif,
                                          TOnItemFocused focused,
                                          TOnCloseEvent on_close, u32 flags) {
...
int __fastcall TProperties::EditPropertiesModal(
    PropItemVec &values, LPCSTR title, bool bShowButtonsBar,
    TOnModifiedEvent modif, TOnItemFocused focused, TOnCloseEvent close,
    u32 flags) {
  TProperties *P =
      CreateModalForm(title, bShowButtonsBar, modif, focused, close, flags);
...
void TProperties::FillElItems(PropItemVec &items, LPCSTR startup_pref) {
  ...
  for (PropItemIt it = items.begin(); it != items.end(); it++) {
    ...
    prop->Item()->CheckBoxState =
        (TCheckBoxState)prop->m_Flags.is(PropItem::flCBChecked);
    // if thumbnail draw
...
void __fastcall TProperties::AssignItems(PropItemVec &items) {
  ...
  // create EL items
  if (m_Flags.is(plItemFolders))
    m_Folders->AssignItems(folder_items, m_Flags.is(plFullExpand),
                           m_Flags.is(plFullSort));
  // fill
...
void TProperties::FolderStore() {
  if (m_Flags.is(plFolderStore) && tvProperties->Items->Count) {
    for (TElTreeItem *item = tvProperties->Items->GetFirstNode(); item;
         item = item->GetNext()) {
...
void TProperties::OutBOOL(BOOL val, TCanvas *Surface, TRect &R, bool bEnable) {
  if (bEnable) {
    ...
    if (val)
      ...
      Surface->Draw(R.Left, R.Top + 3, m_BMDot);
  } else {
    DrawText(Surface->Handle, val ? "on" : "off", -1, &R,
             DT_LEFT | DT_SINGLELINE);
  }
}

void TProperties::OutText(LPCSTR text, TCanvas *Surface, TRect &R, bool bEnable,
                          TGraphic *g, bool bArrow) {
  if (bEnable && (g || bArrow)) {
    R.Right -= g->Width + 2;
    R.Left += 1;
  } else {
...
void DrawButton(TRect R, TCanvas *Surface, LPCSTR caption, bool bDown,
                bool bSelected) {
  ...
  Surface->FillRect(R);
  DrawText(Surface->Handle, caption, -1, &R,
           DT_LEFT | DT_SINGLELINE | DT_VCENTER | DT_END_ELLIPSIS);
}
void DrawButtons(TRect R, TCanvas *Surface, RStringVec &lst, int down_btn,
                 bool bSelected) {
  if (!lst.empty()) {
    TRect r = R;
...
void __fastcall TProperties::tvPropertiesItemDraw(TObject *Sender,
  ...
  if (SectionIndex == 0) {
    ...
    DrawText(Surface->Handle, AnsiString(Item->Text).c_str(), -1, &R,
    ...
    if (prop->m_Flags.is(PropItem::flMixed)) {
      ...
      Surface->Brush->Style = bsSolid;
      Surface->Brush->Color =
          Item->Selected ? (TColor)0x00A0A0A0 : (TColor)0x008E8E8E;
      TRect r = R;
      r.Left -= 1;
      r.Right += 1;
      r.Bottom += 2;
      ...
      R.Right -= 1;
      R.Left += 1;
      DrawText(Surface->Handle, prop->GetDrawText().c_str(), &R,
               DT_LEFT | DT_SINGLELINE | DT_VCENTER);
    } else {
      TRect src_rect = R;
      ...
      case PROP_CANVAS: {
        ...
        R.Right -= 1;
        R.Left += 1;
        CanvasValue *val = dynamic_cast<CanvasValue *>(prop->GetFrontValue());
        R_ASSERT(val);
        if (!val->OnDrawCanvasEvent.empty())
          val->OnDrawCanvasEvent(val, Surface,
                                 Irect().set(R.left, R.top, R.right, R.bottom));
        DrawText(Surface->Handle, prop->GetDrawText().c_str(), &R,
                 DT_LEFT | DT_SINGLELINE | DT_VCENTER);
      } break;
      case PROP_FCOLOR: {
        ...
        R.Top += 1;
        R.Bottom -= 1;
        ColorValue *V = dynamic_cast<ColorValue *>(prop->GetFrontValue());
        R_ASSERT(V);
      ...
      case PROP_VCOLOR: {
        ...
        R.Top += 1;
        R.Bottom -= 1;
        VectorValue *V = dynamic_cast<VectorValue *>(prop->GetFrontValue());
        R_ASSERT(V);
        Fcolor C;
        C.set(V->value->z, V->value->y, V->value->x, 0.f);
      ...
      case PROP_COLOR: {
        ...
        R.Top += 1;
        R.Bottom -= 1;
        U32Value *V = dynamic_cast<U32Value *>(prop->GetFrontValue());
        R_ASSERT(V);
      ...
      case PROP_FLAG: {
        FlagValueCustom *V =
            dynamic_cast<FlagValueCustom *>(prop->GetFrontValue());
        R_ASSERT(V);
        if (V->HaveCaption())
          OutText(prop->GetDrawText().c_str(), Surface, R, prop->Enabled(),
                  m_BMEllipsis);
        else
          OutBOOL(V->m_Flags.is(FlagValueCustom::flInvertedDraw)
                      ? !V->GetValueEx()
                      : V->GetValueEx(),
                  Surface, R, prop->Enabled());
      } break;
      case PROP_BOOLEAN: {
        BOOLValue *V = dynamic_cast<BOOLValue *>(prop->GetFrontValue());
        R_ASSERT(V);
      ...
      case PROP_CHOOSE: {
        ...
        if (miDrawThumbnails->Checked &&
            prop->m_Flags.is(PropItem::flDrawThumbnail)) {
          R.top += tvProperties->LineHeight - 4;
          R.left = R.Right - (R.bottom - R.top);
          if (!V->OnDrawThumbnailEvent.empty())
            V->OnDrawThumbnailEvent(
                prop->GetDrawText().c_str(), Surface->Handle,
                Irect().set(R.left, R.top, R.right, R.bottom));
        }
      } break;
      case PROP_TEXTURE2: {
        ...
        if (miDrawThumbnails->Checked) {
          ...
          if (E && !E->on_thm.empty())
            E->on_thm(prop->GetDrawText().c_str(), Surface->Handle,
                      Irect().set(R.left, R.top, R.right, R.bottom));
        }
      } break;
      case PROP_WAVE:
      ...
      case PROP_CLIST:
        ...
        break;
      case PROP_TIME: {
        FloatValue *V = dynamic_cast<FloatValue *>(prop->GetFrontValue());
        R_ASSERT(V);
        OutText(FloatTimeToStrTime(V->GetValue()).c_str(), Surface, R,
                prop->Enabled());
      } break;
      case PROP_CTEXT:
      case PROP_RTEXT:
...
template <class T> BOOL FlagOnEdit(PropItem *prop, BOOL &bRes) {
  FlagValue<_flags<T>> *V =
      dynamic_cast<FlagValue<_flags<T>> *>(prop->GetFrontValue());
  if (!V)
    return FALSE;
    ...
    if ((HC == 1) && (Button == mbLeft)) {
      ...
      if (m_FirstClickItem == item) {
        ...
        case PROP_CANVAS:
          break;
        case PROP_BUTTON: {
          ButtonValue *FV = dynamic_cast<ButtonValue *>(prop->GetFrontValue());
          R_ASSERT(FV);
          int btn_num = iFloor(
              (X - prop->draw_rect.x1) *
              (float(FV->value.size()) / float(prop->draw_rect.width())));
          for (PropItem::PropValueIt it = prop->values.begin();
        ...
        case PROP_TOKEN: {
          pmEnum->Items->Clear();
          TokenValueCustom *T =
              dynamic_cast<TokenValueCustom *>(prop->GetFrontValue());
          R_ASSERT(T);
        ...
        case PROP_RTOKEN: {
          pmEnum->Items->Clear();
          RTokenValueCustom *T =
              dynamic_cast<RTokenValueCustom *>(prop->GetFrontValue());
          R_ASSERT(T);
          TMenuItem *mi = xr_new<TMenuItem>((TComponent *)0);
          mi->Caption = "-";
          pmEnum->Items->Add(mi);
        ...
        case PROP_SH_TOKEN: {
          pmEnum->Items->Clear();
          TokenValueSH *T = dynamic_cast<TokenValueSH *>(prop->GetFrontValue());
          R_ASSERT(T);
          TMenuItem *mi = xr_new<TMenuItem>((TComponent *)0);
          mi->Caption = "-";
          pmEnum->Items->Add(mi);
        ...
        case PROP_CLIST: {
          pmEnum->Items->Clear();
          CListValue *T = dynamic_cast<CListValue *>(prop->GetFrontValue());
          R_ASSERT(T);
          TMenuItem *mi = xr_new<TMenuItem>((TComponent *)0);
          mi->Caption = "-";
          pmEnum->Items->Add(mi);
        ...
        case PROP_RLIST: {
          pmEnum->Items->Clear();
          RListValue *T = dynamic_cast<RListValue *>(prop->GetFrontValue());
          R_ASSERT(T);
          TMenuItem *mi = xr_new<TMenuItem>((TComponent *)0);
          mi->Caption = "-";
          pmEnum->Items->Add(mi);
        ...
        case PROP_STEXT:
        case PROP_RTEXT:
          if ((X - prop->draw_rect.x1) <
              (prop->draw_rect.width() - (m_BMEllipsis->Width + 2)))
            PrepareLWText(item);
          else
            ExecTextEditor(prop);
          break;
  ...
  if (tvProperties->Selected) {
    ...
    case PROP_BUTTON: {
      if (Shift.Contains(ssLeft)) {
        ...
        int btn_num = -1;
        if (((Y > prop->draw_rect.y1) && (Y < prop->draw_rect.y2)) &&
            ((X > prop->draw_rect.x1) && (Y < prop->draw_rect.x2)))
          btn_num =
              iFloor((X - prop->draw_rect.x1) *
                     (float(V->value.size()) / float(prop->draw_rect.width())));
        for (PropItem::PropValueIt it = prop->values.begin();
             it != prop->values.end(); it++) {
          ButtonValue *V = dynamic_cast<ButtonValue *>(*it);
          R_ASSERT(V);
...
//---------------------------------------------------------------------------
template <class T> BOOL TokenOnEdit(PropItem *prop, u32 _new_val, BOOL &bRes) {
...
template <class T> BOOL RTokenOnEdit(PropItem *prop, u32 _new_val, BOOL &bRes) {
...
void __fastcall TProperties::PMItemClick(TObject *Sender) {
  ...
  if (mi) {
    ...
    case PROP_SH_TOKEN: {
      TokenValueSH *V = dynamic_cast<TokenValueSH *>(prop->GetFrontValue());
      R_ASSERT(V);
    ...
    case PROP_RLIST: {
      RListValue *V = dynamic_cast<RListValue *>(prop->GetFrontValue());
      R_ASSERT(V);
    ...
    case PROP_CLIST: {
      CListValue *V = dynamic_cast<CListValue *>(prop->GetFrontValue());
      R_ASSERT(V);
    ...
    case PROP_TEXTURE2: {
      CTextValue *T = dynamic_cast<CTextValue *>(prop->GetFrontValue());
      R_ASSERT(T);
      ...
      if (mi->Tag == 0) {
        bRes =
            TfrmChoseItem::SelectItem(smTexture, new_val, 8, edit_val.c_str());
      } else if (mi->Tag >= 2) {
...
void __fastcall TProperties::WaveFormClick(TElTreeItem *item) {
  ...
  WaveValue *V = dynamic_cast<WaveValue *>(prop->GetFrontValue());
  R_ASSERT(V);
...
//---------------------------------------------------------------------------
extern "C" DLL_API bool FSColorPickerExecute(u32 *currentColor,
                                             LPDWORD originalColor,
                                             const int initialExpansionState);
...
void __fastcall TProperties::VectorClick(TElTreeItem *item) {
  ...
  Fvector edit_val = V->GetValue();
  prop->BeforeEdit<VectorValue, Fvector>(edit_val);
  if (NumericVectorRun(AnsiString(item->Text).c_str(), &edit_val, V->dec,
                       &edit_val, &V->lim_mn, &V->lim_mx)) {
    if (prop->AfterEdit<VectorValue, Fvector>(edit_val))
      if (prop->ApplyValue<VectorValue, Fvector>(edit_val)) {
        item->RedrawItem(true);
        Modified();
...
void __fastcall TProperties::ChooseClick(TElTreeItem *item) {
  ...
  //
  LPCSTR new_val = 0;
  if (TfrmChoseItem::SelectItem(V->m_ChooseID, new_val, V->subitem,
                                edit_val.c_str(), 0, V->m_FillParam, 0,
                                items.size() ? &items : 0, V->m_ChooseFlags)) {
    edit_val = new_val;
...
template <class T> BOOL NumericOnEdit(PropItem *prop, T new_val, BOOL &bRes) {
  NumericValue<T> *V = dynamic_cast<NumericValue<T> *>(prop->GetFrontValue());
  if (!V)
    return FALSE;
...
void TProperties::PrepareLWText(TElTreeItem *item) {
  ...
  switch (prop->type) {
  case PROP_CTEXT: {
    CTextValue *V = dynamic_cast<CTextValue *>(prop->GetFrontValue());
    R_ASSERT(V);
  ...
  case PROP_STEXT: {
    STextValue *V = dynamic_cast<STextValue *>(prop->GetFrontValue());
    R_ASSERT(V);
    xr_string edit_val = V->GetValue();
  ...
  case PROP_RTEXT: {
    RTextValue *V = dynamic_cast<RTextValue *>(prop->GetFrontValue());
    R_ASSERT(V);
    shared_str edit_val = V->GetValue();
...
void TProperties::ExecTextEditor(PropItem *prop) {
  if (prop) {
    ...
    case PROP_CTEXT: {
      ...
      if (TfrmText::RunEditor(tmp, AnsiString(prop->Item()->Text).c_str(),
                              false)) {
        edit_val = tmp.c_str();
        if (prop->AfterEdit<CTextValue, xr_string>(edit_val))
          if (prop->ApplyValue<CTextValue, LPCSTR>(edit_val.c_str()))
...
void TProperties::PrepareSCText(TElTreeItem *item) {
  ...
  switch (prop->type) {
  case PROP_SHORTCUT: {
    ShortcutValue *V = dynamic_cast<ShortcutValue *>(prop->GetFrontValue());
    R_ASSERT(V);
...
void __fastcall TProperties::tvPropertiesHeaderColumnResize(TObject *Sender,
                                                            int SectionIndex) {
  ApplyEditControl();
}
//---------------------------------------------------------------------------
...
void __fastcall TProperties::tvPropertiesItemChange(
...
void __fastcall TProperties::tvPropertiesShowLineHint(
    TObject *Sender, TElTreeItem *Item, TElHeaderSection *Section,
    TElFString &Text, THintWindow *HintWindow, TPoint &MousePos,
    bool &DoShowHint) {
  PropItem *prop = (PropItem *)Item->Tag;
  if (prop) {
...
void __fastcall TProperties::tvPropertiesCompareItems(TObject *Sender,
  ...
  if (type1 == type2) {
    if (Item1->Text < Item2->Text)
      ...
      res = 0;
  } else if (type1 == TYPE_FOLDER)
    res = -1;
  else if (type2 == TYPE_FOLDER)
    res = 1;
}
//---------------------------------------------------------------------------
