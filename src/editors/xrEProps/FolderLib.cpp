...
AnsiString CFolderHelper::GetFolderName(const AnsiString &full_name,
                                        AnsiString &dest) {
...
AnsiString CFolderHelper::GetObjectName(const AnsiString &full_name,
                                        AnsiString &dest) {
  for (int i = full_name.Length(); i >= 1; i--)
    if (full_name[i] == '\\') {
...
bool CFolderHelper::MakeName(TElTreeItem *begin_item, TElTreeItem *end_item,
                             AnsiString &name, bool bOnlyFolder) {
  name = "";
  if (begin_item) {
    TElTreeItem *node = (u32(begin_item->Data) == TYPE_OBJECT)
                            ? begin_item->Parent
                            : begin_item;
    while (node) {
...
bool CFolderHelper::MakeFullName(TElTreeItem *begin_item, TElTreeItem *end_item,
                                 AnsiString &name) {
  if (begin_item) {
...
TElTreeItem *CFolderHelper::FindItemInFolder(EItemType type, TElTree *tv,
                                             TElTreeItem *start_folder,
                                             const AnsiString &name) {
  if (start_folder) {
    for (TElTreeItem *node = start_folder->GetFirstChild(); node;
         node = start_folder->GetNextChild(node))
      if (type == ((EItemType)(node->Data)) && (node->Text == name))
        return node;
  } else {
    for (TElTreeItem *node = tv->Items->GetFirstNode(); node;
...
TElTreeItem *CFolderHelper::FindItemInFolder(TElTree *tv,
                                             TElTreeItem *start_folder,
                                             const AnsiString &name) {
  if (start_folder) {
    for (TElTreeItem *node = start_folder->GetFirstChild(); node;
...
TElTreeItem *CFolderHelper::FindItem(TElTree *tv, AnsiString full_name,
                                     TElTreeItem **last_valid_node,
                                     int *last_valid_idx) {
  if (last_valid_node)
    *last_valid_node = 0;
  if (last_valid_idx)
...
TElTreeItem *CFolderHelper::FindFolder(TElTree *tv, AnsiString full_name,
                                       TElTreeItem **last_valid_node,
                                       int *last_valid_idx) {
  int cnt = _GetItemCount(full_name.c_str(), '\\');
  if (cnt <= 0)
...
TElTreeItem *CFolderHelper::FindObject(TElTree *tv, AnsiString full_name,
                                       TElTreeItem **last_valid_node,
                                       int *last_valid_idx) {
  int cnt = _GetItemCount(full_name.c_str(), '\\');
  cnt--;
...
TElTreeItem *CFolderHelper::AppendObject(TElTree *tv, AnsiString full_name,
  ...
  //.
  TElTreeItem *fld_node =
      !fld.IsEmpty() ? FindItem /*FindFolder*/ (tv, fld, &last_node, &idx) : 0;
  //.
  if (!fld_node) {
    fld_node = last_node;
...
void CFolderHelper::GenerateFolderName(TElTree *tv, TElTreeItem *node,
                                       AnsiString &name, AnsiString pref,
                                       bool num_first) {
  int cnt = 0;
  if (num_first)
...
void CFolderHelper::GenerateObjectName(TElTree *tv, TElTreeItem *node,
                                       AnsiString &name, AnsiString pref,
                                       bool num_first) {
  int cnt = 0;
  if (num_first)
...
AnsiString CFolderHelper::ReplacePart(AnsiString old_name, AnsiString ren_part,
                                      int level, LPSTR dest) {
...
// Drag'n'Drop
//---------------------------------------------------------------------------
void CFolderHelper::DragDrop(TObject *Sender, TObject *Source, int X, int Y,
                             TOnItemRename after_drag) {
  R_ASSERT(after_drag);

  TElTree *tv = dynamic_cast<TElTree *>(Sender);
  VERIFY(tv);
  ...
  for (ELVecIt it = drag_items.begin(); it != drag_items.end(); it++) {
    ...
    do {
      ...
      if (pNode && IsObject(item)) {
        Msg("#!Item '%s' already exist in folder '%s'.",
            AnsiString(item->Text).c_str(),
            AnsiString(cur_folder->Text).c_str());
        item = item->GetNext();
        continue;
      }
      // если нет добавляем
      if (!pNode) {
        pNode = (type == TYPE_FOLDER)
                    ? LL_CreateFolder(tv, cur_folder, item->Text,
                                      item->ForceButtons)
                    : LL_CreateObject(tv, cur_folder, item->Text);
        if (type == TYPE_OBJECT)
          pNode->Assign(item);
      }
      if (IsFolder(item)) {
        ...
        if (parent && ((parent->GetLastChild() == item) ||
                       (0 == parent->ChildrenCount))) {
...
void CFolderHelper::DragOver(TObject *Sender, TObject *Source, int X, int Y,
  ...
  for (ELVecIt it = drag_items.begin(); it != drag_items.end(); it++) {
    ...
    if (!src)
      ...
      if (tgt) {
        if (IsFolder(src)) {
          bool b = true;
          for (TElTreeItem *itm = tgt->Parent; itm; itm = itm->Parent)
            if (itm == src) {
              b = false;
              break;
            }
          if (IsFolder(tgt)) {
            ...
            Accept = b && (src != tgt->Parent) && (tgt != src) &&
...
void CFolderHelper::StartDragNoFolder(TObject *Sender, TDragObject *&DragObject)
{
        ...
        if (tv->ItemFocused&&IsObject(tv->ItemFocused)) DragItem =
tv->ItemFocused; else
DragItem = 0;
}
*/
...
bool CFolderHelper::RenameItem(TElTree *tv, TElTreeItem *node,
                               AnsiString &new_text, TOnItemRename OnRename) {
  R_ASSERT(OnRename);
  if (new_text.IsEmpty())
    return false;
  ...
  for (TElTreeItem *item = node->GetFirstSibling(); item;
  ...
  if (IsFolder(node)) {
    ...
    for (TElTreeItem *item = node->GetFirstChild();
         item && (item->Level > node->Level); item = item->GetNext()) {
      if (IsObject(item)) {
        MakeName(item, 0, full_name, false);
        VERIFY(node->Level < _GetItemCount(full_name.c_str(), '\\'));
        AnsiString new_full_name;
        _ReplaceItem(full_name.c_str(), node->Level, new_text.c_str(),
                     new_full_name, '\\');
...
void CFolderHelper::CreateNewFolder(TElTree *tv, bool bEditAfterCreate) {
  ...
  TElTreeItem *parent =
      tv->Selected
          ? (IsFolder(tv->Selected) ? tv->Selected : tv->Selected->Parent)
          : 0;
...
BOOL CFolderHelper::RemoveItem(TElTree *tv, TElTreeItem *pNode,
                               TOnItemRemove OnRemoveItem,
                               TOnItemAfterRemove OnAfterRemoveItem) {
  bool bRes = false;
  ...
  if (pNode) {
    ...
    if (!pSelNode)
      pSelNode = pNode->GetNextSibling();
    AnsiString full_name;
    if (IsFolder(pNode)) {
      //			if (mrYes==MessageDlg("Delete selected folder?",
      // mtConfirmation, TMsgDlgButtons() << mbYes << mbNo, 0))
      {
        bRes = true;
        for (TElTreeItem *item = pNode->GetFirstChild();
             item && (item->Level > pNode->Level); item = item->GetNext()) {
          MakeName(item, 0, full_name, false);
          if (IsObject(item)) {
            bool res = true;
    ...
    if (IsObject(pNode)) {
      //			if (mrYes==MessageDlg("Delete selected item?",
      // mtConfirmation, TMsgDlgButtons() << mbYes << mbNo, 0))
      {
...
TElTreeItem *CFolderHelper::RestoreSelection(TElTree *tv, AnsiString full_name,
                                             bool bLeaveSel) {
  TElTreeItem *last_valid = 0;
  FindItem(tv, full_name, &last_valid);
...
bool CFolderHelper::NameAfterEdit(TElTreeItem *node, AnsiString value,
                                  AnsiString &N) {
  VERIFY(node);
  N = N.LowerCase();
  if (N.IsEmpty()) {
  ...
  for (TElTreeItem *itm = node->GetFirstSibling(); itm;
...
void DrawBitmap(HDC hdc, const Irect &r, u32 *data, u32 w, u32 h) {
  ...
  int err = StretchDIBits(hdc, r.x1, r.y1, (r.x2 - r.x1), (r.y2 - r.y1), 0, 0,
                          w, h, data, &bmi, DIB_RGB_COLORS, SRCCOPY);
  if (err == GDI_ERROR) {
...
bool CFolderHelper::DrawThumbnail(HDC hdc, const Irect &r, u32 *data, u32 w,
                                  u32 h) {
  Irect R = r;
...
AnsiString CFolderHelper::GenerateName(LPCSTR _pref, int dgt_cnt,
                                       TFindObjectByName cb,
                                       bool allow_pref_name, bool allow_) {
  VERIFY(!cb.empty());
  AnsiString result;
  int counter = 0;
  // test exist name
