...
bool NumericVectorRun(const char *title, Fvector *data, int decimal,
                      Fvector *reset_value, Fvector *min, Fvector *max, int *X,
                      int *Y) {
  frmNumericVector = xr_new<TfrmNumericVector>((TComponent *)0);
  bool res =
      frmNumericVector->Run(title, data, decimal, reset_value, min, max, X, Y);
...
void __fastcall TfrmNumericVector::FormKeyDown(TObject *Sender, WORD &Key,
                                               TShiftState Shift) {
  if (Key == VK_ESCAPE)
    ebCancel->Click();
  if (Key == VK_RETURN)
...
//----------------------------------------------------
bool __fastcall TfrmNumericVector::Run(const char *title, Fvector *data,
                                       int decimal, Fvector *rv, Fvector *MN,
                                       Fvector *MX, int *X, int *Y) {
