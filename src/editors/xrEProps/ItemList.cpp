...
//---------------------------------------------------------------------------
TItemList *TItemList::CreateForm(LPCSTR title, TWinControl *parent,
                                 TAlign align, u32 flags) {
...
void TItemList::DestroyForm(TItemList *&props) {
  ...
  ILVecIt it = std::find(ILForms.begin(), ILForms.end(), props);
  VERIFY(it != ILForms.end());
  // destroy forms
  props->OnDestroy();
  xr_delete(props);
  ILForms.erase(it);
...
//---------------------------------------------------------------------------
void TItemList::OnCreate(LPCSTR title, TWinControl *parent, TAlign align,
                         u32 flags) {
  ...
  if (Parent)
    tvItems->HeaderSections->Item[0]->Text = title;
  else
    Caption = title;
...
void TItemList::ClearParams(TElTreeItem *node) {
  if (node) {
    ...
    if (m_Flags.is(ilFolderStore) && tvItems->Items->Count) {
      FolderStore.clear();
      for (TElTreeItem *item = tvItems->Items->GetFirstNode(); item;
           item = item->GetNext()) {
...
//---------------------------------------------------------------------------
void TItemList::SelectItem(LPCSTR full_name, bool bVal, bool bLeaveSel,
                           bool bExpand) {
  // select item
  TElTreeItem *item;
...
void __fastcall TItemList::AssignItems(ListItemsVec &items, bool full_expand,
                                       bool full_sort) {
  // begin fill mode
  LockUpdating();
  // clear values
  //    if (tvItems->Selected)
  //    FHelper.MakeFullName(tvItems->Selected,0,last_selected_item);
  if (!m_Items.empty())
    ClearParams();
  // fill values
  m_Items = items;
  for (ListItemsIt it = m_Items.begin(); it != m_Items.end(); it++) {
    ...
    if (prop->key.size() && (prop->key[prop->key.size() - 1] == '\\')) {
      prop->item = FHelper.AppendFolder(tvItems, *prop->key,
                                        !m_Flags.is(ilSuppressIcon));
      ...
      prop->item = FHelper.AppendObject(tvItems, *prop->key, false,
                                        !m_Flags.is(ilSuppressIcon));
      if (!prop->item) {
      ...
      prop_item->CheckBoxState =
          (TCheckBoxState)prop->m_Flags.is(ListItem::flCBChecked);
  ...
  // expand sel items
  for (RStringVecIt s_it = last_selected_items.begin();
       s_it != last_selected_items.end(); s_it++)
  ...
  for (s_it = last_selected_items.begin(); s_it != last_selected_items.end();
       s_it++)
...
void __fastcall TItemList::tvItemsMouseMove(TObject *Sender, TShiftState Shift,
                                            int X, int Y) {
  //
}
//---------------------------------------------------------------------------

void __fastcall TItemList::tvItemsMouseUp(TObject *Sender, TMouseButton Button,
                                          TShiftState Shift, int X, int Y) {
  //
}
//---------------------------------------------------------------------------
...
int __fastcall TItemList::GetSelected(RStringVec &items) {
  for (TElTreeItem *item = tvItems->GetNextSelected(0); item;
       item = tvItems->GetNextSelected(item)) {
    if (item->Hidden)
      continue;
    AnsiString nm;
    FHelper.MakeFullName(item, 0, nm);
...
int __fastcall TItemList::GetSelected(LPCSTR pref, ListItemsVec &items,
                                      bool bOnlyObject) {
  for (TElTreeItem *item = tvItems->GetNextSelected(0); item;
       item = tvItems->GetNextSelected(item)) {
    ListItem *prop = (ListItem *)item->Tag;
...
void __fastcall TItemList::tvItemsItemChange(TObject *Sender, TElTreeItem *Item,
...
void __fastcall TItemList::RefreshForm() {
  ...
  for (TElTreeItem *item = tvItems->Items->GetFirstNode(); item;
    ...
    if (prop) {
      ...
      if (prop->m_Flags.is(ListItem::flHidden)) {
        ...
        item->OwnerHeight = prop->m_Flags.is(ListItem::flDrawThumbnail)
                                ? !miDrawThumbnails->Checked
                                : true;
      }
    }
  }
  ...
  //		ListItem* prop 		= *it;
  //		((TElTreeItem*)prop->item)->Hidden	=
  // prop->m_Flags.is(ListItem::flHidden);
  //	}
  UnlockUpdating();
  tvItems->Repaint();
}

void __fastcall TItemList::tvItemsItemDraw(TObject *Sender, TElTreeItem *Item,
  ...
  if (prop) {
    ...
    if (miDrawThumbnails->Checked &&
        prop->m_Flags.is(ListItem::flDrawThumbnail)) {
      R.top += tvItems->LineHeight - 4;
      R.left = R.Right - (R.bottom - R.top);
      if (!prop->OnDrawThumbnail.empty())
        prop->OnDrawThumbnail(prop->Key(), Surface->Handle,
...
void __fastcall TItemList::InplaceEditValidateResult(TObject *Sender,
                                                     bool &InputValid) {
  ...
  if (!new_text.IsEmpty()) {
    ...
    InputValid = (find_item == IE->Item) ||
                 (!find_item); //.(!find_item); нужно для того чтобы принимало
  }
}
//---------------------------------------------------------------------------
...
void __fastcall TItemList::InplaceEditAfterOperation(TObject *Sender,
  ...
  if (Accepted) {
    ...
    TElTreeInplaceAdvancedEdit *IE = InplaceEdit;
    AnsiString new_text = AnsiString(IE->Editor->Text).LowerCase();
    bool bRes = FHelper.RenameItem(tvItems, IE->Item, new_text,
                                   TOnItemRename(this, &TItemList::RenameItem));
    if (bRes) {
      if (tvItems->OnAfterSelectionChange)
        tvItems->OnAfterSelectionChange(0);
...
void __fastcall TItemList::tvItemsDragDrop(TObject *Sender, TObject *Source,
                                           int X, int Y) {
  R_ASSERT(m_Flags.is(ilEditMenu));
  FHelper.DragDrop(Sender, Source, X, Y,
                   TOnItemRename(this, &TItemList::RenameItem));
  if (tvItems->OnAfterSelectionChange)
    tvItems->OnAfterSelectionChange(0);
}
...
void TItemList::SaveSelection(TFormStorage *storage) {
  ...
  if (GetSelected(items)) {
    ...
    for (RStringVecIt l_it = items.begin(); l_it != items.end(); l_it++)
      storage->WriteString(AnsiString().sprintf("sel%d", l_it - items.begin()),
                           **l_it);
  }
  //    for (AStringIt s_it=last_selected_items.begin();
  //    s_it!=last_selected_items.end(); s_it++)
...
void __fastcall TItemList::tvItemsResize(TObject *Sender) {
  ...
  if (tvItems->VertScrollBarVisible)
    tvItems->HeaderSections->Item[0]->Width -=
        tvItems->VertScrollBarStyles->Width;
}
//---------------------------------------------------------------------------
...
void TItemList::RemoveSelItems(TOnItemRemove on_remove) {
  if (mrYes == MessageDlg("Remove selected item(s)?", mtConfirmation,
    ...
    if (GetSelected(sel_items)) {
      ...
      for (RStringVecIt it = sel_items.begin(); it != sel_items.end(); it++) {
        ...
        if (!FHelper.RemoveItem(tvItems, pNode,
                                on_remove.empty() ? OnItemRemoveEvent
                                                  : on_remove)) {
...
void TItemList::GenerateObjectName(shared_str &name, LPCSTR start_node,
                                   LPCSTR pref, bool num_first) {
  AnsiString _name;
