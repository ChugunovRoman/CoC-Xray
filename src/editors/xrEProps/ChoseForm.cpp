...
void TfrmChoseItem::AppendEvents(u32 choose_ID, LPCSTR caption,
                                 TOnChooseFillItems on_fill,
                                 TOnChooseSelectItem on_sel,
                                 TOnDrawThumbnail on_thm,
                                 TOnChooseClose on_close, u32 flags) {
  EventsMapIt it = m_Events.find(choose_ID);
  VERIFY(it == m_Events.end());
  m_Events.insert(
      std::make_pair(choose_ID, SChooseEvents(caption, on_fill, on_sel, on_thm,
                                              on_close, flags)));
}
...
int __fastcall TfrmChoseItem::SelectItem(u32 choose_ID, LPCSTR &dest,
                                         int sel_cnt, LPCSTR init_name,
                                         TOnChooseFillItems item_fill,
                                         void *fill_param,
                                         TOnChooseSelectItem item_select,
                                         ChooseItemVec *items, u32 mask) {
  VERIFY(!form);
  ...
  //.	form->paItemsCount->Caption		= AnsiString(" Items in list:
  //")+AnsiString(form->tvItems->Items->Count);
...
void __fastcall TfrmChoseItem::AppendItem(SChooseItem *item,
                                          bool b_check_duplicate) {
  TElTreeItem *node = FHelper.AppendObject(form->tvItems, item->name.c_str(),
                                           !b_check_duplicate /*false*/, true);
  node->Tag = (int)item;
...
void __fastcall TfrmChoseItem::FillItems(u32 choose_id) {
  ...
  bool b_check_duplicate =
      (choose_id != 15); // smSkeletonAnimations is already unique set !!!
  for (; it != _E; it++)
...
// implementation
//---------------------------------------------------------------------------
__fastcall TfrmChoseItem::TfrmChoseItem(TComponent *Owner) : TForm(Owner) {
...
void __fastcall TfrmChoseItem::sbSelectClick(TObject *Sender) {
  if (m_Flags.is(cfMultiSelect)) {
    select_item = "";
    AnsiString nm;
    for (TElTreeItem *node = tvMulti->Items->GetFirstNode(); node;
         node = node->GetNext()) {
  ...
  if (m_Flags.is(cfMultiSelect)) {
    ...
    for (int i = 0; i < itm_cnt; i++) {
      TElTreeItem *itm_node = FHelper.FindObject(
          tvItems,
          _GetItem(m_LastSelection.LowerCase().c_str(), i, T, ',', "", false),
          0, 0); //,bIgnoreExt);
      TElTreeItem *fld_node = 0;
      if (itm_node) {
        tvMulti->Items->AddObject(
            0, _GetItem(m_LastSelection.c_str(), i, T, ',', "", false),
            (void *)TYPE_OBJECT);
    ...
    TElTreeItem *itm_node = FHelper.FindItem(
        tvItems, m_LastSelection.LowerCase().c_str(), 0, 0); //,bIgnoreExt);
    TElTreeItem *fld_node = 0;
    if (itm_node) {
...
void __fastcall TfrmChoseItem::FormClose(TObject *Sender,
                                         TCloseAction &Action) {
  if (tvItems->Selected)
...
void __fastcall TfrmChoseItem::tvItemsKeyPress(TObject *Sender, char &Key) {
  TElTreeItem *node = tvItems->Items->LookForItemEx(
      tvItems->Selected, -1, false, false, false, &Key, LookupFunc);
  if (!node)
    node = tvItems->Items->LookForItemEx(0, -1, false, false, false, &Key,
                                         LookupFunc);
  if (node) {
...
void __fastcall TfrmChoseItem::tvItemsItemChange(
  ...
  if (Item && (ItemChangeMode == icmCheckState)) {
    ...
    TElTreeItem *node = tvMulti->Items->LookForItem(0, fn.c_str(), 0, 0, false,
...
void __fastcall TfrmChoseItem::tvMultiDragDrop(TObject *Sender, TObject *Source,
                                               int X, int Y) {
...
void __fastcall TfrmChoseItem::tvMultiDragOver(TObject *Sender, TObject *Source,
...
void __fastcall TfrmChoseItem::ebMultiRemoveClick(TObject *Sender) {
  if (tvMulti->Selected) {
    ...
    TElTreeItem *node = tvItems->Items->LookForItem(0, cur->Text, 0, 0, false,
                                                    true, false, true, true);
    if (node)
      node->Checked = false;
...
void __fastcall TfrmChoseItem::ebMultiClearClick(TObject *Sender) {
  tvMulti->Items->Clear();
  for (TElTreeItem *node = tvItems->Items->GetFirstNode(); node;
       node = node->GetNext())
    if (node->Checked)
      node->Checked = false;
}
...
void TfrmChoseItem::DrawImage() {
  ...
  if (Item && FHelper.IsObject(Item) && Item->Tag) {
    if (ebExt->Down) { //&&!E.on_sel.empty()){
      ...
      if (!E.on_thm.empty()) {
        E.on_thm(*itm->name, paImage->Canvas->Handle,
                 Irect().set(0, 0, paImage->Width, paImage->Height));
      }
    }
  }
}
