...
class XR_EPROPS_API TItemList : public TForm {
  ...
  void __fastcall tvItemsMouseUp(TObject *Sender, TMouseButton Button,
                                 TShiftState Shift, int X, int Y);
  void __fastcall tvItemsMouseMove(TObject *Sender, TShiftState Shift, int X,
                                   int Y);
  ...
  void __fastcall InplaceEditValidateResult(TObject *Sender, bool &InputValid);
  ...
  void __fastcall tvItemsKeyDown(TObject *Sender, WORD &Key, TShiftState Shift);
...
private: // User declarations
  ...
  void OutText(LPCSTR text, TCanvas *Surface, TRect R, TGraphic *g = 0,
               bool bArrow = false);
...
public: // User declarations
  ...
  void DeselectAll();
  virtual void SelectItem(LPCSTR full_name, bool bVal, bool bLeaveSel,
                          bool bExpand);
  virtual void AssignItems(ListItemsVec &values, bool full_expand,
  ...
  virtual TElTreeItem *GetSelected() {
    R_ASSERT(!tvItems->MultiSelect);
    return (tvItems->MultiSelect) ? 0 : tvItems->Selected;
  }
  virtual ListItem *FindItem(LPCSTR full_name);

  virtual void LockUpdating() {
    tvItems->IsUpdating = true;
    iLocked++;
  }
  virtual void UnlockUpdating() {
  ...
  virtual void SetImages(TImageList *image_list) {
    tvItems->Images = image_list;
  }
  ...
  virtual void SaveParams(TFormStorage *fs) {
    ...
    fs->WriteInteger(AnsiString().sprintf("%s_draw_thm", Name.c_str()),
  ...
  virtual void LoadParams(TFormStorage *fs) {
    ...
    miDrawThumbnails->Checked = fs->ReadInteger(
        AnsiString().sprintf("%s_draw_thm", Name.c_str()), false);
    RefreshForm();
  }
  ...
  virtual void OnCreate(LPCSTR title, TWinControl *parent, TAlign align,
                        u32 flags);
  virtual void OnDestroy();

  virtual void ClearList();

  virtual void GenerateObjectName(shared_str &name, LPCSTR start_node,
                                  LPCSTR pref = "object",
                                  bool num_first = false);

  virtual void SetOnItemFocusedEvent(TOnILItemFocused e) {
    OnItemFocusedEvent = e;
  }
  virtual void SetOnItemsFocusedEvent(TOnILItemsFocused e) {
...
#ifdef __BORLANDC__
  static TItemList *CreateForm(LPCSTR title, TWinControl *parent = 0,
                               TAlign align = alNone,
                               u32 flags = ilMultiSelect | ilFolderStore);
#endif
  static TItemList *CreateModalForm(LPCSTR title,
                                    u32 flags = ilMultiSelect | ilFolderStore);
