...
void __stdcall SelectVisual(SChooseItem *item, PropItemVec &info_items) {
  ...
  //.
      AnsiString fn					=
  ChangeFileExt(item->name.c_str(),".ogf");
      IRender_Visual* visual			=
  ::Render->model_Create(fn.c_str()); if (visual){ PHelper().CreateCaption
  (info_items,	"Source",
  *visual->desc.source_file?*visual->desc.source_file:"unknown");
  ...
  T",Trim(AnsiString(ctime(&visual->desc.create_time))).c_str());
          PHelper().CreateCaption	(info_items,	"Modif N",
  *visual->desc.modif_name ?*visual->desc.modif_name :"unknown");
          PHelper().CreateCaption	(info_items,	"Modif T",
  Trim(AnsiString(ctime(&visual->desc.modif_time))).c_str());
          PHelper().CreateCaption	(info_items,	"Build N",
  *visual->desc.build_name ?*visual->desc.build_name :"unknown");
...
void __stdcall SelectGameObjectMots(SChooseItem *item,
                                    PropItemVec &info_items) {}
//---------------------------------------------------------------------------
...
void __stdcall DrawLAnim(LPCSTR name, HDC hdc, const Irect &r) {
  ...
  if (item) {
    HBRUSH hbr =
        CreateSolidBrush(item->CalculateBGR(EDevice.fTimeGlobal, frame));
...
void __stdcall FillPE(ChooseItemVec &items, void *param) {
  for (PS::PEDIt E = ::Render->PSLibrary.FirstPED();
       E != ::Render->PSLibrary.LastPED(); E++)
...
void __stdcall FillParticles(ChooseItemVec &items, void *param) {
  for (PS::PEDIt E = ::Render->PSLibrary.FirstPED();
       E != ::Render->PSLibrary.LastPED(); E++)
    items.push_back(SChooseItem(*(*E)->m_Name, "EFFECT"));
  for (PS::PGDIt G = ::Render->PSLibrary.FirstPGD();
       G != ::Render->PSLibrary.LastPGD(); G++)
...
void FillChooseEvents() {
  TfrmChoseItem::AppendEvents(
      smSoundSource, "Select Sound Source", ChoseEvents::FillSoundSource,
      ChoseEvents::SelectSoundSource, 0, ChoseEvents::CloseSoundSource, 0);
  TfrmChoseItem::AppendEvents(smSoundEnv, "Select Sound Environment",
                              ChoseEvents::FillSoundEnv, 0, 0, 0, 0);
  TfrmChoseItem::AppendEvents(
      smObject, "Select Library Object", ChoseEvents::FillObject,
      ChoseEvents::SelectObject, ChoseEvents::DrawObjectTHM, 0, 0);
  TfrmChoseItem::AppendEvents(smGroup, "Select Group", ChoseEvents::FillGroup,
                              ChoseEvents::SelectGroup,
                              ChoseEvents::DrawGroupTHM, 0, 0);
  TfrmChoseItem::AppendEvents(smEShader, "Select Engine Shader",
                              ChoseEvents::FillEShader, 0, 0, 0, 0);
  TfrmChoseItem::AppendEvents(smCShader, "Select Compiler Shader",
                              ChoseEvents::FillCShader, 0, 0, 0, 0);
  TfrmChoseItem::AppendEvents(smPE, "Select Particle Effect",
                              ChoseEvents::FillPE, 0 /*ChoseEvents::SelectPE*/,
                              0, 0, 0);
  TfrmChoseItem::AppendEvents(smParticles, "Select Particle System",
                              ChoseEvents::FillParticles,
                              0 /*ChoseEvents::SelectPG*/, 0, 0, 0);
  TfrmChoseItem::AppendEvents(
      smTextureRaw, "Select Source Texture", ChoseEvents::FillTextureRaw,
      ChoseEvents::SelectTextureRaw, ChoseEvents::DrawTextureTHMRaw, 0, 0);
  TfrmChoseItem::AppendEvents(
      smTexture, "Select Texture", ChoseEvents::FillTexture,
      ChoseEvents::SelectTexture, ChoseEvents::DrawTextureTHM, 0, 0);
  TfrmChoseItem::AppendEvents(smEntityType, "Select Entity",
                              ChoseEvents::FillEntity, 0, 0, 0, 0);
  TfrmChoseItem::AppendEvents(smLAnim, "Select Light Animation",
                              ChoseEvents::FillLAnim, 0, ChoseEvents::DrawLAnim,
                              0, SChooseEvents::flAnimated);
  TfrmChoseItem::AppendEvents(smVisual, "Select Visual",
                              ChoseEvents::FillVisual,
                              ChoseEvents::SelectVisual, 0, 0, 0);
  TfrmChoseItem::AppendEvents(smSkeletonAnims, "Select Skeleton Animation",
                              ChoseEvents::FillSkeletonAnims, 0, 0, 0, 0);
  TfrmChoseItem::AppendEvents(smSkeletonBones, "Select Skeleton Bones",
                              ChoseEvents::FillSkeletonBones, 0, 0, 0, 0);
  TfrmChoseItem::AppendEvents(smSkeletonBonesInObject, "Select Skeleton Bones",
                              ChoseEvents::FillSkeletonBonesObject, 0, 0, 0, 0);
  TfrmChoseItem::AppendEvents(smGameMaterial, "Select Game Material",
                              ChoseEvents::FillGameMaterial, 0, 0, 0, 0);
  TfrmChoseItem::AppendEvents(smGameAnim, "Select Animation",
                              ChoseEvents::FillGameAnim, 0, 0, 0, 0);
  TfrmChoseItem::AppendEvents(smGameSMotions, "Select Game Object Motions",
                              ChoseEvents::FillGameObjectMots,
