...
__fastcall TfrmEditLibrary::TfrmEditLibrary(TComponent *Owner) : TForm(Owner) {
  ...
  m_Props = TfrmPropertiesEObject::CreateProperties(
      0, alNone, TOnModifiedEvent(this, &TfrmEditLibrary::OnModified));
  m_Items = TItemList::CreateForm(
      "Objects", paItems, alClient,
      TItemList::ilMultiSelect | TItemList::ilEditMenu |
          TItemList::ilDragAllowed | TItemList::ilFolderStore);
  m_Items->SetOnItemsFocusedEvent(fastdelegate::bind<TOnILItemsFocused>(
      this, &TfrmEditLibrary::OnItemsFocused));

  m_Items->SetOnItemRemoveEvent(
      fastdelegate::bind<TOnItemRemove>(&Lib, &ELibrary::RemoveObject));
  m_Items->SetOnItemRenameEvent(
      fastdelegate::bind<TOnItemRename>(&Lib, &ELibrary::RenameObject));
  bReadOnly = false;
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
CSceneObject *__fastcall TfrmEditLibrary::RayPick(const Fvector &start,
...
void __fastcall TfrmEditLibrary::FormCloseQuery(TObject *Sender,
  ...
  if (ebSave->Enabled) {
    int res =
        ELog.DlgMsg(mtConfirmation, "Library was change. Do you want save?");
    if (res == mrCancel)
      CanClose = false;
    if (res == mrYes)
      ebSaveClick(0);
...
void __fastcall TfrmEditLibrary::OnItemsFocused(ListItemsVec &items) {
  ...
  if (b_one /*&& FHelper.IsObject(items[0])*/ &&
      UI->ContainEState(esEditLibrary)) {
    // change thm
  ...
  if (m_Thm && m_Thm->Valid()) {
    lbFaces->Caption =
        m_Thm->_FaceCount() ? AnsiString(m_Thm->_FaceCount()) : AnsiString("?");
    lbVertices->Caption = m_Thm->_VertexCount()
                              ? AnsiString(m_Thm->_VertexCount())
                              : AnsiString("?");
  } else {
    lbFaces->Caption = "?";
    lbVertices->Caption = "?";
...
void __fastcall TfrmEditLibrary::ebMakeThmClick(TObject *Sender) {
  ...
  for (; it != it_e; ++it) {
    ...
    if (obj && cbPreview->Checked) {
      string_path fn;
      FS.update_path(fn, _objects_,
                     ChangeFileExt(obj->GetName(), ".thm").c_str());
      ...
      if (ImageLib.CreateOBJThumbnail(fn, obj, obj->Version())) {
        ...
        //             AnsiString full_name; FHelper.MakeFullName
        //             (node,0,full_name);
        //                m_Items->SelectItem
        //                (item->Key(),true,false,true);
      }
    } else {
  ...
  /*
          ...
          if (node&&FHelper.IsObject(node))
      {
          ...
          if (obj&&cbPreview->Checked)
          {
              string_path 			fn;
                  FS.update_path
     (fn,_objects_,ChangeFileExt(obj->GetName(),".thm").c_str());

              if (ImageLib.CreateOBJThumbnail	(fn,obj,obj->Version())){
                      ELog.Msg					(mtInformation,"Thumbnail
     successfully created."); AnsiString
     full_name; FHelper.MakeFullName		(node,0,full_name);
                  m_Items->SelectItem
     (full_name.c_str(),true,false,true);
              }
              }else{
              ELog.DlgMsg(mtError,"Can't create thumbnail. Set preview mode.");
          }
...
bool TfrmEditLibrary::GenerateLOD(ListItemsVec &props, bool bHighQuality) {
  ...
  for (; it != it_e; ++it) {
    ...
    if (O && O->IsMUStatic()) {
      ...
      ImageLib.CreateLODTexture(O, tex_name.c_str(), LOD_IMAGE_SIZE,
                                LOD_IMAGE_SIZE, LOD_SAMPLE_COUNT, O->Version(),
                                bHighQuality ? 4 /*7*/ : 1);
      O->OnDeviceDestroy();
      O->m_objectFlags.set(CEditableObject::eoUsingLOD, bLod);
      ELog.Msg(mtInformation, "LOD for object '%s' successfully created.",
               O->GetName());
    } else {
      ELog.Msg(mtError,
  ...
  /*
      ...
      xr_vector<CSceneObject*>::iterator it_e = form->m_pEditObjects.end();
      SPBItem* pb 					=
     UI->ProgressStart(form->m_pEditObjects.size(),"Making LOD"); for(
     ;it!=it_e; ++it)
      {
          CSceneObject* SO 			= *it;
          CEditableObject* O 			= SO->GetReference();
          ...
          if (O && O->IsMUStatic())
          {
              pb->Inc					(O->GetName());
              BOOL bLod 				=
     O->m_objectFlags.is(CEditableObject::eoUsingLOD); O->m_objectFlags.set
     (CEditableObject::eoUsingLOD,FALSE); xr_string
     tex_name; tex_name 				=
     EFS.ChangeFileExt(O->GetName(),"");

              string_path				tmp;
              strcpy					(tmp,tex_name.c_str());
     _ChangeSymbol(tmp,'\\','_'); tex_name 				=
     xr_string("lod_")+tmp; tex_name 				=
     ImageLib.UpdateFileName(tex_name); ImageLib.CreateLODTexture(O,
     tex_name.c_str(),LOD_IMAGE_SIZE,LOD_IMAGE_SIZE,LOD_SAMPLE_COUNT,O->Version(),bHighQuality?7:1);
              O->OnDeviceDestroy		();
              O->m_objectFlags.set	(CEditableObject::eoUsingLOD,bLod);
              ELog.Msg				(mtInformation,"LOD for object '%s'
     successfully created.",O->GetName()); }else{ ELog.Msg
     (mtError,"Can't create LOD texture from non 'Multiple Usage' object.",
     SO->Name);
          }
...
void TfrmEditLibrary::MakeLOD(bool bHighQuality) {
  ...
  /*
          ...
          if (node && cbPreview->Checked)
      {
          LockForm();
          int res 				=
  ELog.DlgMsg(mtConfirmation,TMsgDlgButtons() << mbYes << mbNo << mbCancel,"Do
  you want to select multiple objects?"); if (res!=mrCancel)
          {
              if (res==mrYes)
              {
                  LPCSTR new_val		= 0;
                  if (TfrmChoseItem::SelectItem(smObject,new_val,256))
                  {
                      ...
                      for (int k=0; k<cnt; ++k)
                      {
                          ...
                          pb->Inc			(tmp.c_str());
                          ListItem* I		=
  m_Items->FindItem(tmp.c_str()); VERIFY			(I);
                          ListItemsVec	V;
                          V.push_back		(I);
                      ...
                      ELog.DlgMsg 	(mtInformation,"'%d' LOD's succesfully
  created.",iLODcnt); UI->ProgressEnd	(pb);
                  }
              }else{
                  ...
                  {
                      ListItem* prop 		= (ListItem*)node->Tag;
  VERIFY(prop); ListItemsVec		V2; V2.push_back		(prop);
                      GenerateLOD			(V2,bHighQuality);

                  } if(FHelper.IsFolder(node))
                  {
                      if (mrYes==ELog.DlgMsg(mtConfirmation,TMsgDlgButtons() <<
  mbYes << mbNo,"Are you sure to generate LOD for all object in this folder?"))
                      {
                          int iLODcnt = 0;
                          SPBItem* pb =
  UI->ProgressStart(node->ChildrenCount,"Making LOD"); for (TElTreeItem*
  item=node->GetFirstChild(); item; item=node->GetNextChild(item))
                          {
                              ListItem* prop 		= (ListItem*)node->Tag;
  VERIFY(prop); pb->Inc				(prop->Key()); ListItemsVec
  V3; V3.push_back		(prop); if (GenerateLOD(V3,bHighQuality))
                                  iLODcnt++;

                              if (UI->NeedAbort()) break;
                          }
                          UI->ProgressEnd			(pb);
                          ELog.DlgMsg 			(mtInformation,"'%d'
  LOD's succesfully created.",iLODcnt);
                      }
                  }
              }
          }
  ...
  for (; sit != sit_e; ++sit) {
    ...
    /*
       ...
       m_pEditObject->GetReference();
        ...
        {
                    E->t_vPosition.set
       (m_pEditObject->PPosition); E->t_vScale.set
       (m_pEditObject->PScale); E->t_vRotate.set
       (m_pEditObject->PRotation);
        }
        m_pEditObject->SetReference		(new_name);
        // get old position
            E								=
       m_pEditObject->GetReference(); if (E)
        {
    ...
    if (EFS.GetSaveName(_import_, save_nm, 0, 1)) {
      ...
      if (obj) {
        if (!obj->ExportLWO(save_nm.c_str())) {
          ELog.DlgMsg(mtInformation, "Can't export object '%s'.",
                      obj->GetName());
        } else {
...
void __fastcall TfrmEditLibrary::ebImportClick(TObject *Sender) {
  ...
  if (EFS.GetOpenName(_import_, open_nm, true)) {
    ...
    for (AStringIt it = lst.begin(); it != lst.end(); ++it) {
      ...
      if (O->Load(it->c_str())) {
        save_nm = xr_string(FS.get_path(_objects_)->m_Path) + folder.c_str() +
                  EFS.ChangeFileExt(nm, ".object");

        if (FS.exist(save_nm.c_str()))
          if (mrNo == ELog.DlgMsg(mtConfirmation,
                                  TMsgDlgButtons() << mbYes << mbNo,
                                  "Object '%s' already exist. Owerwrite it?",
                                  nm.c_str())) {
            xr_delete(O);
            break;
          }
...
void __fastcall TfrmEditLibrary::ebExportOBJClick(TObject *Sender) {
  if (!cbPreview->Checked) {
    ...
    SPBItem *pb =
        UI->ProgressStart(form->m_pEditObjects.size(), "Expotring to OBJ");
    CSceneObject *SO = xr_new<CSceneObject>((LPVOID)0, (LPSTR)0);
    for (; it != it_e; ++it) {
      ListItem *item = *it;
      pb->Inc(item->Key());
