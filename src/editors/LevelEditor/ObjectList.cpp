...
//---------------------------------------------------------------------------
__fastcall TfrmObjectList::TfrmObjectList(TComponent *Owner) : TForm(Owner) {
  bLockUpdate = false;
  find_node = NULL;
}
...
void __fastcall TfrmObjectList::FormShow(TObject *Sender) {
  ...
  tvItems->FilteredVisibility =
      ((rgSO->ItemIndex == 1) || (rgSO->ItemIndex == 2));
  InitListBox();
...
TElTreeItem *TfrmObjectList::FindObjectByType(int type, void *obj) {
  for (TElTreeItem *node = tvItems->Items->GetFirstNode(); node;
       node = node->GetNext())
    if (!node->Parent && (node->Tag == type)) {
      for (TElTreeItem *chield = node->GetFirstChild(); chield;
           chield = node->GetNextChild(chield))
        if (chield->Data == obj)
          return chield;
      break;
    }
...
//---------------------------------------------------------------------------
TElTreeItem *TfrmObjectList::AddFolder(LPCSTR name, void *data,
                                       TElTreeItem *parent_node) {
  for (TElTreeItem *fnode = tvItems->Items->GetFirstNode(); fnode;
       fnode = fnode->GetNext()) {
...
TElTreeItem *TfrmObjectList::AddObject(TElTreeItem *node, LPCSTR name,
                                       void *obj, TColor color) {
...
void gen_object_name(CCustomObject *O, string1024 &str_name,
                     bool b_parent_is_group) {
...
void __fastcall TfrmObjectList::InitListBox() {
  ...
  for (SceneToolsMapPairIt it = Scene->FirstTool(); it != Scene->LastTool();
       ++it) {
    ...
    if (ot && ((m_cur_cls == OBJCLASS_DUMMY) || (it->first == m_cur_cls))) {
      ...
      if (OBJCLASS_GROUP == it->first) {
        for (ObjectIt _F = lst.begin(); _F != lst.end(); ++_F) {
          ...
          TElTreeItem *grp_node =
              AddObject(group_profile_node, str_name, (void *)(*_F), clBlack);
          ...
          for (ObjectIt _G = grp_lst.begin(); _G != grp_lst.end(); ++_G) {
            ...
            TElTreeItem *obj = AddObject(
                grp_node, str_name, (void *)(*_G),
                (*_G)->m_CO_Flags.test(CCustomObject::flObjectInGroupUnique)
                    ? clBlack
                    : TColor(0x00a9a6a0));
          }
        }
        for (TElTreeItem *chield = tool_node->GetFirstChild(); chield;
             chield = tool_node->GetNextChild(chield))
        ...
        for (ObjectIt _F = lst.begin(); _F != lst.end(); ++_F) {
          ...
          TColor clr = (fl.test(CCustomObject::flObjectInGroup) &&
                        !fl.test(CCustomObject::flObjectInGroupUnique))
                           ? TColor(0x00a9a6a0)
                           : clBlack;
...
void TfrmObjectList::UpdateState() {
  ...
  for (TElTreeItem *node = tvItems->Items->GetFirstNode(); node;
    ...
    CCustomObject *PO = (parent_node && parent_node->Data)
                            ? (CCustomObject *)parent_node->Data
                            : NULL;
...
void TfrmObjectList::UpdateSelection() {
  if (tvItems->Items->Count) {
    ...
    for (TElTreeItem *node = tvItems->GetNextSelected(0); node;
         node = tvItems->GetNextSelected(node)) {
      if (node->Data) {
        CCustomObject *O = (CCustomObject *)(node->Data);
        O->Select(true);
        ElEdit1->Text = O->Name;

        for (TElTreeItem *child = node->GetFirstChild(); child;
...
void __fastcall TfrmObjectList::sbRefreshListClick(TObject *Sender) {
  if ((Scene->ObjCount() != obj_count) ||
      (m_cur_cls != LTools->CurrentClassID()) || bForceInitListBox) {
    InitListBox();
...
void __fastcall TfrmObjectList::ElEdit1KeyDown(TObject *Sender, WORD &Key,
  ...
  if (Key == VK_F3) {
    TElEdit *edt = ElEdit1;
    AnsiString str = edt->Text;
    ProcessFindItemInList((find_node && find_node->GetNext())
                              ? find_node->GetNext()
                              : tvItems->Items->GetFirstNode(),
                          str);
  }
}
//---------------------------------------------------------------------------
