...
void __fastcall TfrmEditLightAnim::FormCreate(TObject *Sender) {
  m_Props = TProperties::CreateForm(
      "LAProps", paProps, alClient,
      TOnModifiedEvent(this, &TfrmEditLightAnim::OnModified));
  m_Items =
      TItemList::CreateForm("LA Items", paItems, alClient,
                            TItemList::ilEditMenu | TItemList::ilDragAllowed |
                                TItemList::ilFolderStore);
  m_Items->SetOnModifiedEvent(
      TOnModifiedEvent(this, &TfrmEditLightAnim::OnModified));
  m_Items->SetOnItemFocusedEvent(fastdelegate::bind<TOnILItemFocused>(
      this, &TfrmEditLightAnim::OnItemFocused));
  m_Items->SetOnItemRemoveEvent(fastdelegate::bind<TOnItemRemove>(
      &LALib, &ELightAnimLibrary::RemoveObject));
  m_Items->SetOnItemRenameEvent(fastdelegate::bind<TOnItemRename>(
      &LALib, &ELightAnimLibrary::RenameObject));
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void __fastcall TfrmEditLightAnim::FormClose(TObject *Sender,
                                             TCloseAction &Action) {
  form = 0;
  Action = caFree;
  Scene->unlock();
...
void TfrmEditLightAnim::UpdateView() {
  if (m_CurrentItem) {
    ...
    sePointer->Color = TColor(
        m_CurrentItem->IsKey(sePointer->Value) ? 0x00BFFFFF : 0x00A0A0A0);
  }
  Caption = AnsiString().sprintf("Light Animation Library%s",
                                 ebSave->Enabled ? "*" : "");
}
//------------------------------------------------------------------------------
...
void TfrmEditLightAnim::UpdateProperties() {
  ...
  if (m_CurrentItem) {
    ...
    PHelper().CreateFloat(items, "FPS", &m_CurrentItem->fFPS, 0.1f, 1000, 1.f,
                          1);
    S32Value *V;
    V = PHelper().CreateS32(items, "Frame Count", &m_CurrentItem->iFrameCount,
                            1, 100000, 1);
    V->OnAfterEditEvent.bind(this, &TfrmEditLightAnim::OnFrameCountAfterEdit);

    u32 frame = sePointer->Value;
    PHelper().CreateCaption(items, "Current\\Frame",
                            shared_str().printf("%d", frame));
...
void __fastcall TfrmEditLightAnim::ebAddAnimClick(TObject *Sender) {
  ...
  pref += "la";
  AnsiString name =
      FHelper.GenerateName(pref.c_str(), 2,
                           fastdelegate::bind<TFindObjectByName>(
                               this, &TfrmEditLightAnim::FindItemByName),
                           false, true);
...
void __fastcall TfrmEditLightAnim::ebCloneClick(TObject *Sender) {
  if (m_CurrentItem) {
    // folder name
    AnsiString name =
        FHelper.GenerateName(m_CurrentItem->cName.c_str(), 2,
                             fastdelegate::bind<TFindObjectByName>(
                                 this, &TfrmEditLightAnim::FindItemByName),
                             false, true);
...
void __fastcall TfrmEditLightAnim::pbGMouseDown(TObject *Sender,
  ...
  if (Shift.Contains(ssLeft)) {
    ...
    sePointer->Value =
        iFloor(float(m_CurrentItem->iFrameCount) * float(X) / float(R.Width()));
    pbG->Repaint();
...
void __fastcall TfrmEditLightAnim::pbGMouseMove(TObject *Sender,
  ...
  if (Shift.Contains(ssLeft))
    if (iMoveKey >= 0) {
      ...
      TPoint pt(X, Y);
      pbG->ScreenToClient(pt);
      int new_key = iFloor(float(m_CurrentItem->iFrameCount) * float(X) /
                           float(R.Width()));
...
void __fastcall TfrmEditLightAnim::pbGPaint(TObject *Sender) {
  ...
  if (m_CurrentItem) {
    ...
    it++;
    float x_prev =
        (float(prev_key->first) / float(m_CurrentItem->iFrameCount)) *
        R.Width();
    TRect cb;
    cb.Top = R.Top;
    ...
    for (; it != Keys.end(); it++) {
      ...
      for (int k = 0; k < g_cnt; k++) {
        ...
        B->Canvas->Brush->Color = TColor(
            subst_alpha(m_CurrentItem->InterpolateBGR(prev_key->first + k), 0));
    ...
    cb.Bottom = R.Bottom;
    for (it = m_CurrentItem->Keys.begin(); it != m_CurrentItem->Keys.end();
         it++) {
      int t =
          iFloor((it->first / float(m_CurrentItem->iFrameCount)) * R.Width());
      cb.Left = t - 1 + half_segment;
      cb.Right = t + 2 + half_segment;
      B->Canvas->FillRect(cb);
    }
    // draw pointer
    int t = iFloor((sePointer->Value / float(m_CurrentItem->iFrameCount)) *
                   R.Width()) +
            half_segment;
    ...
    if ((iMoveKey >= 0) && (iTgtMoveKey != iMoveKey))
      t = iFloor((iTgtMoveKey / float(m_CurrentItem->iFrameCount)) *
                 R.Width()) +
          half_segment;
    TRect rp = R;
    rp.Left = t - 2;
...
void __fastcall TfrmEditLightAnim::sePointerKeyDown(TObject *Sender, WORD &Key,
                                                    TShiftState Shift) {
  if (Key == VK_RETURN)
    pbG->Repaint();
}
...
void __fastcall TfrmEditLightAnim::sePointerChange(TObject *Sender) {
  sePointer->Color =
      TColor(m_CurrentItem->IsKey(sePointer->Value) ? 0x00BFFFFF : 0x00A0A0A0);
  UpdateProperties();
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmEditLightAnim::wnShapeKeyDown(TObject *Sender, WORD &Key,
                                                  TShiftState Shift) {
  if (m_CurrentItem) {
    if (Key == VK_LEFT) {
