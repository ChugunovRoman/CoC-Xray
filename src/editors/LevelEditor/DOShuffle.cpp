...
bool __fastcall TfrmDOShuffle::Run() {
  VERIFY(!form);
  form = xr_new<TfrmDOShuffle>(
      (TComponent *)0,
      dynamic_cast<EDetailManager *>(Scene->GetTool(OBJCLASS_DO)));
  // show
...
void __fastcall TfrmDOShuffle::FormCreate(TObject *Sender) {
  m_ObjectProps =
      TProperties::CreateForm("Objects", paObjectProps, alClient,
                              fastdelegate::bind<TOnChooseClose>(
                                  this, &TfrmDOShuffle::OnObjectPropsModified));
...
void TfrmDOShuffle::FillData() {
  ...
  tvItems->Items->Clear();
  // objects
  for (CDetailManager::DetailIt d_it = DM->objects.begin();
       d_it != DM->objects.end(); d_it++)
...
bool TfrmDOShuffle::ApplyChanges() {
  ...
  for (u32 k = 0; k < color_indices.size(); k++) {
    ...
    if (OneColor->tvDOList->Items->Count) {
      ...
      for (TElTreeItem *node = OneColor->tvDOList->Items->GetFirstNode(); node;
           node = node->GetNext())
  ...
  if (/*bNeedUpdate||*/ bColorIndModif || bObjectModif) {
    ELog.DlgMsg(mtInformation,
                "Object or object list changed. Reinitialize needed!");
    DM->InvalidateSlots();
    return true;
  }
  return false;
...
//---------------------------------------------------------------------------
TElTreeItem *TfrmDOShuffle::AddItem(TElTreeItem *node, const char *name,
                                    void *obj) {
...
void __fastcall TfrmDOShuffle::FormClose(TObject *Sender,
                                         TCloseAction &Action) {
...
void __fastcall TfrmDOShuffle::tvMultiDragDrop(TObject *Sender, TObject *Source,
                                               int X, int Y) {
...
void __fastcall TfrmDOShuffle::tvMultiDragOver(TObject *Sender, TObject *Source,
    ...
    for (AStringIt s_it = lst.begin(); s_it != lst.end(); s_it++)
      if (!FindItem(s_it->c_str())) {
        if (tvItems->Items->Count >= dm_max_objects) {
          ELog.DlgMsg(mtInformation, "Maximum detail objects in scene '%d'",
                      dm_max_objects);
          return;
        }
...
void __fastcall TfrmDOShuffle::RemoveColorIndex(TfrmOneColor *p) {
  ...
  form->color_indices.erase(
      std::find(form->color_indices.begin(), form->color_indices.end(), p));
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmDOShuffle::tvItemsDragDrop(TObject *Sender, TObject *Source,
                                               int X, int Y) {
...
void __fastcall TfrmDOShuffle::tvItemsDragOver(TObject *Sender, TObject *Source,
                                               int X, int Y, TDragState State,
                                               bool &Accept) {
  Accept = false;
  if (Source == tvItems)
