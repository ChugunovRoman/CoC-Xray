...
// static part
//------------------------------------------------------------------------------
void CCustomObject::SnapMove(Fvector &pos, Fvector &rot, const Fmatrix &rotRP,
  ...
  if (Scene->RayPickObject(pinf.inf.range, s1, dn, OBJCLASS_SCENEOBJECT, &pinf,
                           Scene->GetSnapList(false)) ||
      Scene->RayPickObject(pinf.inf.range, s2, dn, OBJCLASS_SCENEOBJECT, &pinf,
                           Scene->GetSnapList(false))) {
    pos.set(pinf.pt);
    if (Tools->GetSettings(etfNormalAlign)) {
      Fvector verts[3];
      pinf.e_obj->GetFaceWorld(pinf.s_obj->_Transform(), pinf.e_mesh,
                               pinf.inf.id, verts);
      Fvector vR, vD, vN;
...
void CCustomObject::OnAttach(CCustomObject *owner) {
  R_ASSERT(owner);
  R_ASSERT2(((!m_pOwnerObject) || (m_pOwnerObject == owner)),
...
void CCustomObject::RotatePivot(const Fmatrix &prev_inv,
                                const Fmatrix &current) {
  Fmatrix Ol, On;
...
void CCustomObject::ScalePivot(const Fmatrix &prev_inv, const Fmatrix &current,
                               Fvector &amount) {
  UI->UpdateScene();
  Fvector p = PPosition;
  Fvector s = PScale;
...
bool CCustomObject::OnObjectNameAfterEdit(PropValue *sender,
                                          shared_str &edit_val) {
...
void CCustomObject::OnNumChangeScale(PropValue *sender) { NumSetScale(PScale); }
...
void CCustomObject::FillProp(LPCSTR pref, PropItemVec &items) {
  PropValue *V;
  V = PHelper().CreateNameCB(items, PrepareKey(pref, "Name"), &FName, NULL,
                             NULL,
...
void CCustomObject::OnShowHint(AStringVec &dest) {
  //	dest.push_back(AnsiString("Class:
  //")+AnsiString(ParentTools->ClassDesc()));
