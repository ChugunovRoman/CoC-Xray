...
// add
//------------------------------------------------------------------------------
__fastcall TUI_ControlSectorAdd::TUI_ControlSectorAdd(int st, int act,
...
void TUI_ControlSectorAdd::AddMesh() {
  ...
  if (!sector)
    return;
  SRayPickInfo pinf;
  if (Scene->RayPickObject(pinf.inf.range, UI->m_CurrentRStart,
                           UI->m_CurrentRDir, OBJCLASS_SCENEOBJECT, &pinf, 0))
    sector->AddMesh(dynamic_cast<CSceneObject *>(pinf.s_obj), pinf.e_mesh);
  else if (Scene->RayPickObject(pinf.inf.range, UI->m_CurrentRStart,
                                UI->m_CurrentRDir, OBJCLASS_GROUP, &pinf, 0)) {
    ...
    for (EditMeshIt it = meshes->begin(); it != meshes->end(); it++) {
      ...
      if (_O->AddMesh(S, *it)) {
        ...
        do {
            ...
            !valid_color(clr = color_rgba(Random.randI(0, 3) * 255 / 2,
                                          Random.randI(0, 3) * 255 / 2,
    ...
    if (m_Action == saMeshBoxSelection) {
      ...
      if (LUI->SelectionFrustum(frustum)) {
        ...
        for (ObjectIt _F = lst.begin(); _F != lst.end(); _F++) {
          ...
          for (EditMeshIt m_def = O_lib->m_Meshes.begin();
               m_def != O_lib->m_Meshes.end(); m_def++) {
...
//
//------------------------------------------------------------------------------
__fastcall TUI_ControlSectorSelect::TUI_ControlSectorSelect(
    int st, int act, ESceneToolBase *parent)
    : TUI_CustomControl(st, act, parent) {
  pFrame = 0;
}
void TUI_ControlSectorSelect::OnEnter() {
  pFrame = (TfraSector *)parent_tool->pFrame;
  VERIFY(pFrame);
}
void TUI_ControlSectorSelect::OnExit() { pFrame = 0; }
