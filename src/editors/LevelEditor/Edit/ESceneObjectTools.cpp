...
bool ESceneObjectTool::Validate(bool full_test) {
  ...
  for (ObjectIt a_it = m_Objects.begin(); a_it != m_Objects.end(); a_it++) {
    ...
    if (full_test && A->IsMUStatic()) {
      ...
      int age, age_nm;
      //.          FS.update_path
      //(fn,_textures_,EFS.ChangeFileExt(l_name,".tga").c_str());
      FS.update_path(fn, _game_textures_,
                     EFS.ChangeFileExt(l_name, ".dds").c_str());
      ...
      /*
                  ...
                  {
                      Msg				("!Invalid LOD texture
         version:
         '%s'",E->GetName()); Msg             ("tex=%d obj=%d", age,
         E->Version()); Msg             ("tex=%d obj=%d", age_nm, E->Version());
                      bRes 			= false;
                  }
      */
    }
...
void ESceneObjectTool::FillAppendRandomProperties(bool bUpdateOnly) {
  ...
  V = PHelper().CreateChoose(items, "Objects", &m_AppendRandomObjectsStr,
                             smObject, 0, 0, 32);
  V->OnChangeEvent.bind(this, &ESceneObjectTool::OnChangeAppendRandomFlags);
...
void ESceneObjectTool::HighlightTexture(LPCSTR tex_name, bool allow_ratio,
          ...
          if (0 == stricmp((*it)->_Texture(), tex_name)) {
            ...
            for (EditMeshIt mesh_it = e_obj->FirstMesh();
              ...
              if (sf_it != surf_faces.end()) {
                ...
                for (IntVec::const_iterator i_it = lst.begin();
                     i_it != lst.end(); i_it++) {
                  ...
                  Tools->m_DebugDraw.AppendSolidFace(verts[0], verts[1],
                                                     verts[2], clr, false);
                  if (mark)
