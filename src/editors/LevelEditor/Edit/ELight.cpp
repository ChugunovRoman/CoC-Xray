...
void CLight::Render(int priority, bool strictB2F) {
  ...
  if ((1 == priority) && (false == strictB2F)) {
    ...
    u32 clr = Selected() ? SEL_COLOR
                         : (m_Flags.is(ELight::flAffectDynamic) ? NORM_DYN_COLOR
                                                                : NORM_COLOR);
    switch (m_Type) {
    case ELight::ltPoint:
      if (Selected())
    ...
    case ELight::ltSpot: {
      //			Fvector dir;
      //			dir.setHP
      //(PRotation.y,PRotation.x); 			DU.DrawCone
      //(Fidentity, PPosition, dir, Selected()?m_Range:VIS_RADIUS, radius2, clr,
      // true, false);
      if (Selected())
        DU_impl.DrawSpotLight(PPosition, FTransformR.k, m_Range, m_Cone, clr);
      else
        DU_impl.DrawSpotLight(PPosition, FTransformR.k, VIS_RADIUS, m_Cone,
                              clr);
    } break;
    default:
      THROW;
    ...
    if (lt->m_Flags.is(ESceneLightTool::flShowControlName)) {
      ...
      if (!Scene->RayPickObject(dist, PPosition, D, OBJCLASS_SCENEOBJECT, 0, 0))
        DU_impl.OutText(PPosition,
                        AnsiString().sprintf(" %s", GetLControlName()).c_str(),
    ...
    case ELight::ltPoint:
      if (m_Flags.is(ELight::flPointFuzzy)) {
        ...
        case CLight::SFuzzyData::fstSphere:
          DU_impl.DrawSphere(_Transform(), zero, m_FuzzyData->m_SphereRadius,
                             clr, clr, true, true);
          break;
        case CLight::SFuzzyData::fstBox:
          DU_impl.DrawAABB(_Transform(), zero, m_FuzzyData->m_BoxDimension, clr,
                           clr, true, true);
          break;
        }
      }
...
bool CLight::RayPick(float &distance, const Fvector &start,
