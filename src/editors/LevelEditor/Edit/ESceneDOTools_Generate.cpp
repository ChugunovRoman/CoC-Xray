...
void EDetailManager::FindClosestIndex(const Fcolor &C, SIndexDistVec &best) {
  ...
  for (; it != E; it++) {
    src.set(it->first);
    float d = inv_a + sqrtf((C.r - src.r) * (C.r - src.r) +
                            (C.g - src.g) * (C.g - src.g) +
                            (C.b - src.b) * (C.b - src.b));
    if (d < dist) {
      dist = d;
  ...
  if (Scene->BoxPickObjects(bbox, pinf, &m_SnapObjects)) {
    ...
    for (SBoxPickInfoIt it = pinf.begin(); it != pinf.end(); it++) {
      for (int k = 0; k < (int)it->inf.size(); k++) {
        float range;
        Fvector verts[3];
        it->e_obj->GetFaceWorld(it->s_obj->_Transform(), it->e_mesh,
                                it->inf[k].id, verts);
        sPoly sSrc(verts, 3);
        sPoly sDest;
...
void EDetailManager::CalcClosestCount(int part, const Fcolor &C,
                                      SIndexDistVec &best) {
  float dist = flt_max;
  Fcolor src;
  float inv_a = 1 - C.a;
...
u8 EDetailManager::GetObject(ColorIndexPairIt &CI, u8 id) {
  ...
  DetailIt it =
      std::find(objects.begin(), objects.end(), (CDetail *)CI->second[id]);
  VERIFY(it != objects.end());
  return u8(it - objects.begin());
}
...
bool EDetailManager::UpdateSlotObjects(int x, int z) {
  ...
  // пройдем по 4 частям слота и определим плотность заполнения (учесть
  // переворот V)
  Irect P[4];
...
bool EDetailManager::UpdateObjects(bool bUpdateTex, bool bUpdateSelectedOnly) {
  ...
  for (u32 z = 0; z < dtH.size_z; z++)
    for (u32 x = 0; x < dtH.size_x; x++) {
      if (!bUpdateSelectedOnly ||
          (bUpdateSelectedOnly && m_Selected[z * dtH.size_x + x]))
...
void EDetailManager::AppendIndexObject(u32 color, LPCSTR name,
                                       bool bTestUnique) {
  if (bTestUnique) {
