...
//
//------------------------------------------------------------------------------
__fastcall TUI_ControlObjectAdd::TUI_ControlObjectAdd(int st, int act,
                                                      ESceneToolBase *parent)
...
bool __fastcall TUI_ControlObjectAdd::Start(TShiftState Shift) {
  ...
  { // pick already executed (see top)
    ...
    if (ot->IsAppendRandomActive() && ot->m_AppendRandomObjects.size()) {
      N = ot->m_AppendRandomObjects[Random.randI(
                                        ot->m_AppendRandomObjects.size())]
              .c_str();
    } else {
    ...
    /*        if (ref->IsDynamic()){
                ELog.DlgMsg(mtError,"TUI_ControlObjectAdd:: Can't assign dynamic
       object."); xr_delete(obj); return false;
            }
    */
    if (ot->IsAppendRandomActive()) {
      ...
      if (ot->IsAppendRandomRotationActive()) {
        Fvector p;
        p.set(Random.randF(ot->m_AppendRandomMinRotation.x,
                           ot->m_AppendRandomMaxRotation.x),
              Random.randF(ot->m_AppendRandomMinRotation.y,
                           ot->m_AppendRandomMaxRotation.y),
              Random.randF(ot->m_AppendRandomMinRotation.z,
                           ot->m_AppendRandomMaxRotation.z));
        obj->PRotation = p;
      }
      if (ot->IsAppendRandomScaleActive()) {
        Fvector s;
        if (ot->IsAppendRandomScaleProportional()) {
          s.x = Random.randF(ot->m_AppendRandomMinScale.x,
                             ot->m_AppendRandomMaxScale.x);
          s.set(s.x, s.x, s.x);
        } else {
          s.set(Random.randF(ot->m_AppendRandomMinScale.x,
                             ot->m_AppendRandomMaxScale.x),
                Random.randF(ot->m_AppendRandomMinScale.y,
                             ot->m_AppendRandomMaxScale.y),
                Random.randF(ot->m_AppendRandomMinScale.z,
                             ot->m_AppendRandomMaxScale.z));
        }
        obj->PScale = s;
      }
    }
...
bool __fastcall TUI_ControlObjectAdd::End(TShiftState _Shift) { return true; }
