...
bool CSceneObject::LoadLTX(CInifile &ini, LPCSTR sect_name) {
  ...
  do {
    ...
    if (!SetReference(ref_name.c_str())) {
      ELog.Msg(mtError, "CSceneObject: '%s' not found in library",
               ref_name.c_str());
      bRes = false;
      int mr = mrNone;
      ...
      if (b_found) {
        xr_string _message;
        _message = "Object [" + ref_name + "] not found. Relace it with [" +
                   _new_name + "] or select other from library?";
      ...
      if (!bRes) {
        ...
        LPCSTR new_val = 0;
        if ((mr == mrNone || mr == mrYes) &&
            TfrmChoseItem::SelectItem(smObject, new_val, 1)) {
    ...
    //        if(!CheckVersion())
    //            ELog.Msg( mtError, "CSceneObject: '%s' different file
    //            version!", ref_name.c_str() );
...
bool CSceneObject::LoadStream(IReader &F) {
  ...
  do {
    ...
    if (!SetReference(buf)) {
      ...
      if (b_found) {
        xr_string _message;
        _message = "Object [" + xr_string(buf) +
                   "] not found. Relace it with [" + _new_name +
