...
#define RENDER_OBJECT(P, B)                                                    \
  {                                                                            \
    try {                                                                      \
      ...
      ELog.DlgMsg(mtError,                                                     \
                  "Please notify AlexMX!!! Critical error has occured in "     \
                  "render routine!!! [Type B] - Tools: '%s' Object: '%s'",     \
                  (N->val)->ParentTool->ClassName(), (N->val)->Name);          \
    }                                                                          \
  }

void __fastcall object_Normal_0(EScene::mapObject_Node *N) {
  RENDER_OBJECT(0, false);
}
void __fastcall object_Normal_1(EScene::mapObject_Node *N) {
  RENDER_OBJECT(1, false);
}
void __fastcall object_Normal_2(EScene::mapObject_Node *N) {
  RENDER_OBJECT(2, false);
}
void __fastcall object_Normal_3(EScene::mapObject_Node *N) {
  RENDER_OBJECT(3, false);
}
//------------------------------------------------------------------------------
void __fastcall object_StrictB2F_0(EScene::mapObject_Node *N) {
  RENDER_OBJECT(0, true);
}
void __fastcall object_StrictB2F_1(EScene::mapObject_Node *N) {
  RENDER_OBJECT(1, true);
}
void __fastcall object_StrictB2F_2(EScene::mapObject_Node *N) {
  RENDER_OBJECT(2, true);
}
void __fastcall object_StrictB2F_3(EScene::mapObject_Node *N) {
...
struct tools_rp_pred
    : public std::binary_function<ESceneToolBase *, ESceneToolBase *, bool> {
...
DEFINE_MSET_PRED(ESceneToolBase *, SceneMToolsSet, SceneMToolsIt,
                 tools_rp_pred);
DEFINE_MSET_PRED(ESceneCustomOTool *, SceneOToolsSet, SceneOToolsIt,
                 tools_rp_pred);
    ...
    for (; t_it != t_end; t_it++) {
      ...
      for (; o_it != o_end; o_it++) {
        if ((*o_it)->Visible() && (*o_it)->IsRender()) {
          float distSQ =
              EDevice.vCameraPosition.distance_to_sqr((*o_it)->FPosition);
