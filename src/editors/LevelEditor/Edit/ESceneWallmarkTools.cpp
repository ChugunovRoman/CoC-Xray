...
int ESceneWallmarkTool::RaySelect(int flag, float &distance,
                                  const Fvector &start,
...
struct zero_slot_pred
    : public std::unary_function<ESceneWallmarkTool::wm_slot *, bool> {
...
void ESceneWallmarkTool::RefiningSlots() {
  ...
  WMSVecIt new_end =
      std::remove_if(marks.begin(), marks.end(), zero_slot_pred());
...
void ESceneWallmarkTool::OnRender(int priority, bool strictB2F) {
  ...
  for (WMSVecIt slot_it = marks.begin(); slot_it != marks.end(); slot_it++) {
    ...
    if ((u32(priority) == slot->shader->E[0]->flags.iPriority) &&
      ...
      u32 w_offset = 0;
      FVF::LIT *w_verts = (FVF::LIT *)RCache.Vertex.Lock(
          MAX_R_VERTEX, hGeom->vb_stride, w_offset);
      FVF::LIT *w_start = w_verts;

      for (WMVecIt w_it = slot->items.begin(); w_it != slot->items.end();
           w_it++) {
        wallmark *W = *w_it;
        VERIFY3(W->verts.size() <= MAX_R_VERTEX, "ERROR: Invalid wallmark.",
                *slot->tx_name);
        if (RImplementation.ViewBase.testSphere_dirty(W->bounds.P,
                                                      W->bounds.R)) {
          ...
          if (ssa >= ssaCLIP) {
            ...
            for (int t_idx = 0; t_idx < t_cnt; t_idx++) {
              ...
              if (w_count + 3 > MAX_R_VERTEX) {
                ...
                w_verts = (FVF::LIT *)RCache.Vertex.Lock(
                    MAX_R_VERTEX, hGeom->vb_stride, w_offset);
                w_start = w_verts;
              }
              // real fill buffer
...
struct zero_item_pred
    : public std::unary_function<ESceneWallmarkTool::wallmark *, bool> {
...
void ESceneWallmarkTool::SaveLTX(CInifile &ini, int id) {
  ...
  for (WMSVecIt slot_it = marks.begin(); slot_it != marks.end();
       ++slot_it, ++i) {
    wm_slot *slot = *slot_it;
    ...
    for (WMVecIt w_it = slot->items.begin(); w_it != slot->items.end();
      ...
      //.            F.w
      //(&*W->verts.begin(),sizeof(FVF::LIT)*W->verts.size());
    }
  }
}
...
bool ESceneWallmarkTool::LoadStream(IReader &F) {
  ...
  for (WMSVecIt slot_it = marks.begin(); slot_it != marks.end(); slot_it++) {
    ...
    WMVecIt new_end = std::remove_if(slot->items.begin(), slot->items.end(),
                                     zero_item_pred());
...
void ESceneWallmarkTool::SaveSelection(IWriter &F) { SaveStream(F); }
...
void ESceneWallmarkTool::OnDeviceDestroy() { hGeom.destroy(); }
...
// destroy
void ESceneWallmarkTool::wm_destroy(wallmark *W) { pool.push_back(W); }

struct SWMSlotFindPredicate {
  shared_str sh_name;
  shared_str tx_name;
  SWMSlotFindPredicate(shared_str sh, shared_str tx)
      : sh_name(sh), tx_name(tx) {}
  bool operator()(const ESceneWallmarkTool::wm_slot *slot) const {
    return (slot->tx_name == tx_name) && (slot->sh_name == sh_name);
  }
};
ESceneWallmarkTool::wm_slot *ESceneWallmarkTool::FindSlot(shared_str sh_name,
                                                          shared_str tx_name) {
  WMSVecIt it = std::find_if(marks.begin(), marks.end(),
                             SWMSlotFindPredicate(sh_name, tx_name));
  return (it != marks.end()) ? *it : 0;
}
ESceneWallmarkTool::wm_slot *
ESceneWallmarkTool::AppendSlot(shared_str sh_name, shared_str tx_name) {
...
void ESceneWallmarkTool::BuildMatrix(Fmatrix &mView, float inv_w, float inv_h,
                                     float angle, const Fvector &from) {
  // build projection
  Fmatrix mScale, mRot;
...
BOOL ESceneWallmarkTool::AddWallmark_internal(const Fvector &start,
                                              const Fvector &dir, shared_str sh,
                                              shared_str tx, float width,
                                              float height, float rotate) {
  if (ObjectCount() >= MAX_WALLMARK_COUNT) {
    ELog.DlgMsg(mtError, "Maximum wallmark per level is reached [Max: %d].",
                MAX_WALLMARK_COUNT);
    return FALSE;
  }
  ...
  if (Scene->RayQuery(PQ, start, dir, dist,
                      CDB::OPT_ONLYNEAREST | CDB::OPT_CULL, snap_list)) {
    contact_pt.mad(PQ.m_Start, PQ.m_Direction, PQ.r_begin()->range);
    sml_normal.mknormal(PQ.r_begin()->verts[0], PQ.r_begin()->verts[1],
                        PQ.r_begin()->verts[2]);
    sml_collector.add_face_packed_D(PQ.r_begin()->verts[0],
                                    PQ.r_begin()->verts[1],
                                    PQ.r_begin()->verts[2], 0);
  } else
    return FALSE;
  ...
  Fmatrix mView;
  BuildMatrix(mView, 2 / width, 2 / height, rotate,
              contact_pt); // width/2 height/2  (BuildMatrix need radius)
  ...
  if ((W->verts.size() < 3) || (W->verts.size() > MAX_WALLMARK_VERTEX_COUNT)) {
    ELog.DlgMsg(mtError, "Invalid wallmark vertex count. [Min: %d. Max: %d].",
                3, MAX_WALLMARK_VERTEX_COUNT);
    wm_destroy(W);
    return FALSE;
  } else {
    W->bbox.invalidate();
...
BOOL ESceneWallmarkTool::AddWallmark(const Fvector &start, const Fvector &dir) {
  return AddWallmark_internal(start, dir, m_ShName, m_TxName, m_MarkWidth,
                              m_MarkHeight, m_MarkRotate);
}

BOOL ESceneWallmarkTool::MoveSelectedWallmarkTo(const Fvector &start,
                                                const Fvector &dir) {
  if (!m_Flags.is(flDrawWallmark))
    return 0;
  wallmark *wm = 0;
  ...
  if ((0 != wm) &&
      AddWallmark_internal(start, dir, wm->parent->sh_name, wm->parent->tx_name,
                           wm->w, wm->h, wm->r)) {
    // remove wm
    for (WMSVecIt p_it = marks.begin(); p_it != marks.end(); p_it++) {
      for (WMVecIt m_it = (*p_it)->items.begin();
           m_it != (*p_it)->items.end();) {
        if (*m_it == wm) {
          wm_destroy(wm);
          *m_it = (*p_it)->items.back();
...
void ESceneWallmarkTool::FillProp(LPCSTR pref, PropItemVec &items) {
  PHelper().CreateFlag32(items, PrepareKey(pref, "Common\\Draw Wallmarks"),
                         &m_Flags, flDrawWallmark);
  PHelper().CreateFlag32(items, PrepareKey(pref, "Common\\Alignment"), &m_Flags,
                         flAxisAlign, "By Camera", "By World Axis");
  PHelper().CreateFloat(items, PrepareKey(pref, "Common\\Width"), &m_MarkWidth,
                        0.01f, 10.f);
  PHelper().CreateFloat(items, PrepareKey(pref, "Common\\Height"),
                        &m_MarkHeight, 0.01f, 10.f);
  PHelper().CreateAngle(items, PrepareKey(pref, "Common\\Rotate"),
                        &m_MarkRotate);
  PHelper().CreateChoose(items, PrepareKey(pref, "Common\\Shader"), &m_ShName,
                         smEShader);
  PHelper().CreateChoose(items, PrepareKey(pref, "Common\\Texture"), &m_TxName,
                         smTexture);
}
//----------------------------------------------------
...
bool ESceneWallmarkTool::Validate(bool) {
  ...
  for (WMSVecIt slot_it = marks.begin(); slot_it != marks.end(); slot_it++) {
    ...
    if (slot->items.size()) {
      ...
      if (!B || B->canBeLMAPped()) {
        ELog.Msg(mtError, "Wallmarks: Invalid or missing shader '%s'.",
                 *slot->sh_name);
        bRes = false;
      }
    }
...
void ESceneWallmarkTool::GetStaticDesc(int &v_cnt, int &f_cnt,
                                       bool b_selected_only, bool b_cform) {
  if (b_cform)
    return;
  for (WMSVecIt slot_it = marks.begin(); slot_it != marks.end(); slot_it++) {
    wm_slot *slot = *slot_it;
...
bool ESceneWallmarkTool::ExportStatic(SceneBuilder *B, bool b_selected_only) {
  for (WMSVecIt slot_it = marks.begin(); slot_it != marks.end(); slot_it++) {
    ...
    for (WMVecIt w_it = slot->items.begin(); w_it != slot->items.end();
      ...
      int m_id = B->BuildMaterial(*slot->sh_name, COMPILER_SHADER,
                                  *slot->tx_name, 1, sect_num, false);
...
void ESceneWallmarkTool::RemoveControls() { inherited::RemoveControls(); }
//----------------------------------------------------
