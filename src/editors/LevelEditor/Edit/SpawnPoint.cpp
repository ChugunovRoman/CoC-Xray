...
#define MAX_TEAM 6
const u32 RP_COLORS[MAX_TEAM] = {0xff0000, 0x00ff00, 0x0000ff,
...
void CSpawnPoint::CLE_Visual::OnChangeVisual() {
  ...
  if (source->visual_name.size()) {
    ...
    if (NULL == visual && !g_tmp_lock) {
      ...
      int mr = ELog.DlgMsg(mtConfirmation, TMsgDlgButtons() << mbYes << mbNo,
                           _msg.c_str());
      LPCSTR _new_val = 0;
      g_tmp_lock = true;
...
void CSpawnPoint::CLE_Visual::PlayAnimationFirstFrame() {
  ...
  if (KA) {
    ...
    if (M.valid()) {
      ...
      Msg("! visual [%s] has no animation [%s]", source->visual_name.c_str(),
          source->startup_animation.c_str());
  }
  if (K)
    K->CalculateBones();
}
  ...
  if (m_Data) {
    ...
    if (m_Data->visual()) {
      ...
      m_Data->set_editor_flag(ISE_Abstract::flVisualChange |
                              ISE_Abstract::flVisualAnimationChange);
    }
    if (m_Data->motion()) {
...
bool CSpawnPoint::SSpawnData::ExportGame(SExportStreams *F,
                                         CSpawnPoint *owner) {
  // set params
  ...
  // SHAPE
  if (cform && !(owner->m_AttachedObject &&
                 (owner->m_AttachedObject->ClassID == OBJCLASS_SHAPE))) {
    ELog.DlgMsg(mtError, "Spawn Point: '%s' must contain attached shape.",
                owner->Name);
    return false;
  }
  if (cform) {
    ...
    cform->assign_shapes(&*shape->GetShapes().begin(),
                         shape->GetShapes().size());
  }
  // end
  ...
  SExportStreamItem &tgt =
      (m_flags.test(eSDTypeRespawn)) ? F->spawn_rs : F->spawn;
...
void CSpawnPoint::SSpawnData::OnAnimControlClick(ButtonValue *value,
                                                 bool &bModif, bool &bSafe) {
...
void CSpawnPoint::SSpawnData::FillProp(LPCSTR pref, PropItemVec &items) {
  ...
  if (Scene->m_LevelOp.m_mapUsage.MatchType(
    ...
    PHelper().CreateFlag8(items, PrepareKey(pref, "MP respawn"), &m_flags,
                          eSDTypeRespawn);

  if (m_Visual) {
    ButtonValue *BV = PHelper().CreateButton(
        items, PrepareKey(pref, m_Data->name(), "Model\\AnimationControl"),
        "|<<,Play,Pause,Stop,>>|", 0);
    BV->OnBtnClickEvent.bind(this,
                             &CSpawnPoint::SSpawnData::OnAnimControlClick);
  }
}

void CSpawnPoint::SSpawnData::Render(bool bSelected, const Fmatrix &parent,
                                     int priority, bool strictB2F) {
  if (m_Visual && m_Visual->visual)
    ::Render->model_Render(m_Visual->visual, parent, priority, strictB2F, 1.f);

  if (m_Motion && m_Motion->animator && bSelected && (1 == priority) &&
      (false == strictB2F))
...
//------------------------------------------------------------------------------
CSpawnPoint::CSpawnPoint(LPVOID data, LPCSTR name)
    : CCustomObject(data, name), m_SpawnData(this) {
  m_rpProfile = "";
  m_EM_Flags.one();
  Construct(data);
}
...
bool CSpawnPoint::AttachObject(CCustomObject *obj) {
  ...
  if (m_SpawnData.Valid()) {
    ...
    case OBJCLASS_SHAPE:
      ...
      //	    	bAllowed =
      //!!dynamic_cast<xrSE_Visualed*>(m_SpawnData.m_Data);
      //        break;
    }
  }
  //
...
bool CSpawnPoint::RefCompare(LPCSTR ref) {
  return ref && ref[0] && m_SpawnData.Valid()
             ? (strcmp(ref, m_SpawnData.m_Data->name()) == 0)
             : false;
}
...
void CSpawnPoint::OnFrame() {
  ...
  if (m_SpawnData.Valid()) {
    if (m_physics_shell && m_SpawnData.m_Data->m_editor_flags.is(
                               ISE_Abstract::flVisualAnimationChange)) {
      DeletePhysicsShell();
      m_SpawnData.OnFrame();
...
void CSpawnPoint::Render(int priority, bool strictB2F) {
    ...
    if (strictB2F) {
      ...
      if (m_SpawnData.Valid()) {
        ...
        case ptEnvMod: {
          ...
          if (Selected())
            DU_impl.DrawSphere(Fidentity, PPosition, m_EM_Radius, 0x30FFAE00,
                               0x00FFAE00, true, true);
        } break;
      ...
      if (st->m_Flags.is(ESceneSpawnTool::flShowSpawnType)) {
        ...
        if (m_SpawnData.Valid()) {
          ...
          case ptRPoint:
            ...
            break;
          case ptEnvMod:
            s_name.sprintf("EnvMod V:%3.2f, F:%3.2f", m_EM_ViewDist,
                           m_EM_FogDensity);
            break;
          default:
            THROW2("CSpawnPoint:: Unknown Type");
        ...
        if (!st->m_Flags.is(ESceneSpawnTool::flPickSpawnType) ||
            !Scene->RayPickObject(dist, PPosition, D, OBJCLASS_SCENEOBJECT, 0,
                                  0))
  ...
  if (m_Type == ptRPoint) {
    ...
    VERIFY(st);
    CEditableObject *v =
        st->get_draw_visual(m_RP_TeamID, m_RP_Type, m_GameType);
    if (v)
...
bool CSpawnPoint::RayPick(float &distance, const Fvector &start,
                          const Fvector &direction, SRayPickInfo *pinf) {
  bool bPick = false;
  if (m_AttachedObject) {
      ...
      if (Fbox::rpOriginOutside == bb.Pick2(start, direction, pt)) {
        ...
        if (d < distance) {
          ...
          if (pinf && m_SpawnData.m_Visual && m_SpawnData.m_Visual->visual) {
            ...
            Fvector norm;
            if (K) {
              bPick = ETOOLS::intersect(FTransformRP, *K, start, direction,
                                        b_id, distance, norm);
              if (bPick) {
                pinf->visual_inf.K = K;
...
bool CSpawnPoint::OnAppendObject(CCustomObject *object) {
  ...
  if (m_SpawnData.Valid()) {
    if (pSettings->line_exist(m_SpawnData.m_Data->name(),
                              "shape_transp_color")) {
      sh->m_DrawTranspColor =
          pSettings->r_color(m_SpawnData.m_Data->name(), "shape_transp_color");
      sh->m_DrawEdgeColor =
          pSettings->r_color(m_SpawnData.m_Data->name(), "shape_edge_color");
    }
  }
...
shared_str CSpawnPoint::SectionToEditor(shared_str nm) {
  ESceneSpawnTool *st = dynamic_cast<ESceneSpawnTool *>(ParentTool);
  VERIFY(st);
  ESceneSpawnTool::ClassSpawnMapIt cls_it =
      st->m_Classes.find(m_SpawnData.m_ClassID);
  VERIFY(cls_it != st->m_Classes.end());
  for (ESceneSpawnTool::SSVecIt ss_it = cls_it->second.begin();
       ss_it != cls_it->second.end(); ++ss_it)
...
void CSpawnPoint::OnProfileChange(PropValue *prop) {
  if (m_SpawnData.m_Profile.size() != 0) {
    ...
    if (0 != strcmp(m_SpawnData.m_Data->name(), *s_name)) {
      ...
      m_SpawnData.m_Data->set_editor_flag(
          ISE_Abstract::flVisualChange | ISE_Abstract::flVisualAnimationChange);
      destroy_entity(tmp);
    }
  } else {
...
void CSpawnPoint::FillProp(LPCSTR pref, PropItemVec &items) {
  ...
  if (m_SpawnData.Valid()) {
    ...
    ChooseValue *C = PHelper().CreateChoose(
        items, PrepareKey(pref1.c_str(), "Profile (spawn section)"),
    ...
    case ptRPoint: {
      ...
      if (m_RP_Type == rptItemSpawn) {
        ChooseValue *C = PHelper().CreateChoose(
            items, PrepareKey(pref, "Respawn Point\\Profile"), &m_rpProfile,
            smCustom, 0, 0, 10, cfMultiSelect);
        C->OnChooseFillEvent.bind(this, &CSpawnPoint::OnFillRespawnItemProfile);
      } else {
        PHelper().CreateU8(items, PrepareKey(pref, "Respawn Point\\Team"),
                           &m_RP_TeamID, 0, 7);
      }
      Token8Value *TV = PHelper().CreateToken8(
          items, PrepareKey(pref, "Respawn Point\\Spawn Type"), &m_RP_Type,
          rpoint_type);
    ...
    case ptEnvMod: {
      PHelper().CreateFloat(items,
                            PrepareKey(pref, "Environment Modificator\\Radius"),
                            &m_EM_Radius, EPS_L, 10000.f);
      PHelper().CreateFloat(items,
                            PrepareKey(pref, "Environment Modificator\\Power"),
                            &m_EM_Power, EPS, 1000.f);

      Flag16Value *FV = NULL;

      FV = PHelper().CreateFlag16(
          items, PrepareKey(pref, "Environment Modificator\\View Distance"),
          &m_EM_Flags, eViewDist);
      FV->OnChangeEvent.bind(this, &CSpawnPoint::OnEnvModFlagChange);
      if (m_EM_Flags.test(eViewDist))
        PHelper().CreateFloat(
            items,
            PrepareKey(pref, "Environment Modificator\\View Distance\\ "),
            &m_EM_ViewDist, EPS_L, 10000.f);

      FV = PHelper().CreateFlag16(
          items, PrepareKey(pref, "Environment Modificator\\Fog Color"),
          &m_EM_Flags, eFogColor);
      FV->OnChangeEvent.bind(this, &CSpawnPoint::OnEnvModFlagChange);
      ...
      FV = PHelper().CreateFlag16(
          items, PrepareKey(pref, "Environment Modificator\\Fog Density"),
          &m_EM_Flags, eFogDensity);
      FV->OnChangeEvent.bind(this, &CSpawnPoint::OnEnvModFlagChange);
      if (m_EM_Flags.test(eFogDensity))
        PHelper().CreateFloat(
            items, PrepareKey(pref, "Environment Modificator\\Fog Density\\ "),
            &m_EM_FogDensity, 0.f, 10000.f);

      FV = PHelper().CreateFlag16(
          items, PrepareKey(pref, "Environment Modificator\\Ambient Color"),
          &m_EM_Flags, eAmbientColor);
      FV->OnChangeEvent.bind(this, &CSpawnPoint::OnEnvModFlagChange);
      if (m_EM_Flags.test(eAmbientColor))
        PHelper().CreateColor(
            items,
            PrepareKey(pref, "Environment Modificator\\Ambient Color\\ "),
            &m_EM_AmbientColor);

      FV = PHelper().CreateFlag16(
          items, PrepareKey(pref, "Environment Modificator\\Sky Color"),
          &m_EM_Flags, eSkyColor);
      FV->OnChangeEvent.bind(this, &CSpawnPoint::OnEnvModFlagChange);
      ...
      FV = PHelper().CreateFlag16(
          items, PrepareKey(pref, "Environment Modificator\\Hemi Color"),
          &m_EM_Flags, eHemiColor);
      FV->OnChangeEvent.bind(this, &CSpawnPoint::OnEnvModFlagChange);
...
bool CSpawnPoint::OnChooseQuery(LPCSTR specific) {
  return (m_SpawnData.Valid() &&
          (0 == strcmp(m_SpawnData.m_Data->name(), specific)));
}
///-----------------------------------------------------------------------------
