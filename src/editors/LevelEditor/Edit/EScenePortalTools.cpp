...
void EScenePortalTool::FillProp(LPCSTR pref, PropItemVec &items) {
  PHelper().CreateFlag32(items, PrepareKey(pref, "Common\\Draw Simple Model"),
                         &m_Flags, flDrawSimpleModel);
...
struct co_plane_pred {
  ...
  co_plane_pred(CPortal *p) : m_portal(p) {
    ...
    Mgc::OrthogonalPlaneFit(m_portal->m_Vertices.size(),
                            (Mgc::Vector3 *)m_portal->m_Vertices.begin(),
  ...
  bool operator()(CCustomObject *p) {
    ...
    if (P->m_SectorFront == m_portal->m_SectorFront &&
        P->m_SectorBack == m_portal->m_SectorBack) {
      Fvector rkOffset, rkNormal;
      Fplane p_plane;
      Mgc::OrthogonalPlaneFit(
          P->m_Vertices.size(), (Mgc::Vector3 *)P->m_Vertices.begin(),
          (Mgc::Vector3 &)rkOffset, (Mgc::Vector3 &)rkNormal);
...
template <int I> bool int_greater(int val) { return val > I; }
