...
ObjectList *EScene::GetSnapList(bool bIgnoreUse) {
  ...
  ObjectList *snap_list =
      mt->GetSnapList() ? mt->GetSnapList() : &m_ESO_SnapObjects;
  return bIgnoreUse ? snap_list
                    : (fraLeftBar->ebUseSnapList->Down ? snap_list : NULL);
}
//--------------------------------------------------------------------------------------------------

bool EScene::FindObjectInSnapList(CCustomObject *O) {
  ObjectList *snap_objects = GetSnapList(true);
  return snap_objects ? std::find(snap_objects->begin(), snap_objects->end(),
                                  O) != snap_objects->end()
                      : false;
}
...
bool EScene::AddToSnapList(CCustomObject *O, bool bUpdateScene) {
  ...
  ObjectList *snap_objects = GetSnapList(true);
  if (snap_objects) {
    if (std::find(snap_objects->begin(), snap_objects->end(), O) ==
        snap_objects->end()) {
