...
void CWayPoint::Render(LPCSTR parent_name, bool bParentSelect) {
  ...
  DU_impl.DrawCross(pos, WAYPOINT_RADIUS, WAYPOINT_SIZE * 0.85f,
                    WAYPOINT_RADIUS, WAYPOINT_RADIUS, WAYPOINT_SIZE * 0.15f,
  ...
  if (bParentSelect) {
    ...
    for (WPLIt it = m_Links.begin(); it != m_Links.end(); it++) {
      ...
      p2.set(O->way_point->m_vPosition.x,
             O->way_point->m_vPosition.y + WAYPOINT_SIZE * 0.85f,
      ...
      xx.add(p1);
      DU_impl.OutText(
          xx, AnsiString().sprintf("P: %1.2f", O->probability).c_str(), c, s);
    }
...
bool CWayPoint::RayPick(float &distance, const Fvector &S, const Fvector &D) {
  ...
  if (d > 0) {
    ...
    if (((d2 * d2 - d * d) < (WAYPOINT_RADIUS * WAYPOINT_RADIUS)) &&
        (d > WAYPOINT_RADIUS)) {
      if (d < distance) {
        distance = d;
        return true;
...
CWayPoint *CWayObject::AppendWayPoint() {
  ...
  m_WayPoints.push_back(
      xr_new<CWayPoint>(FHelper
                            .GenerateName("wp", 2,
                                          fastdelegate::bind<TFindObjectByName>(
                                              this, &CWayObject::FindWPByName),
                                          false, false)
                            .c_str()));
...
bool CWayObject::RaySelect(int flag, const Fvector &start, const Fvector &dir,
                           bool bRayTest) {
  if (IsPointMode()) {
    float dist = UI->ZFar();
    CWayPoint *nearest = 0;
...
bool CWayObject::GetBox(Fbox &box) const {
  box.invalidate();
  for (WPVec::const_iterator it = m_WayPoints.begin(); it != m_WayPoints.end();
       it++)
...
bool CWayObject::RayPick(float &distance, const Fvector &S, const Fvector &D,
                         SRayPickInfo *pinf) {
  bool bPick = false;
  for (WPIt it = m_WayPoints.begin(); it != m_WayPoints.end(); it++)
...
bool CWayObject::LoadLTX(CInifile &ini, LPCSTR sect_name) {
  ...
  if (!Name) {
    ELog.DlgMsg(mtError, "Corrupted scene file.[%s] sect[%s] has empty name",
                ini.fname(), sect_name);
    return false;
  }
  ...
  for (; cit != cit_e; ++cit) {
    if (cit->first.c_str() == strstr(cit->first.c_str(), "link_wp_")) {
      ...
      int res =
          sscanf(cit->first.c_str(), "link_wp_%4d_%4d", &wp_idx, &wp_link_idx);
      R_ASSERT4(res == 2, "bad waypoint link record format", sect_name,
                cit->first.c_str());
...
void CWayObject::SaveLTX(CInifile &ini, LPCSTR sect_name) {
  ...
  for (wp_idx = 0, it = m_WayPoints.begin(); it != m_WayPoints.end();
       ++it, ++wp_idx) {
    CWayPoint *W = *it;
    u32 link_idx = 0;
    for (WPLIt l_it = W->m_Links.begin(); l_it != W->m_Links.end();
         ++l_it, ++link_idx) {
      sprintf(buff, "link_wp_%4d_%4d", wp_idx, link_idx);

      WPIt to =
          std::find(m_WayPoints.begin(), m_WayPoints.end(), (*l_it)->way_point);
...
bool CWayObject::ExportGame(SExportStreams *F) {
  F->patrolpath.stream.open_chunk(F->patrolpath.chunk++);
  {
    ...
    for (it = m_WayPoints.begin(); it != m_WayPoints.end(); it++) {
      ...
      int from = it - m_WayPoints.begin();
      for (WPLIt l_it = W->m_Links.begin(); l_it != W->m_Links.end(); l_it++) {
        WPIt to = std::find(m_WayPoints.begin(), m_WayPoints.end(),
                            (*l_it)->way_point);
        R_ASSERT(to != m_WayPoints.end());
...
bool CWayObject::OnWayPointNameAfterEdit(PropValue *sender,
                                         shared_str &edit_val) {
...
void CWayObject::FillProp(LPCSTR pref, PropItemVec &items) {
  ...
  PropValue *V;
  V = PHelper().CreateNameCB(items, PrepareKey(pref, "Way Name"), &FName, NULL,
                             NULL,
                             RTextValue::TOnAfterEditEvent(
                                 this, &CCustomObject::OnObjectNameAfterEdit));
    ...
    for (WPIt it = m_WayPoints.begin(); it != m_WayPoints.end(); ++it) {
      ...
      if ((*it)->m_bSelected) {
        PHelper().CreateNameCB(
            ...
            fastdelegate::bind<RTextValue::TOnAfterEditEvent>(
                this, &CWayObject::OnWayPointNameAfterEdit));
        PHelper().CreateVector(
            items, PrepareKey(pref, "Way Point\\Transform\\Position"),
            &W->m_vPosition, -10000, 10000, 0.01, 2);

        for (WPLIt l_it = W->m_Links.begin(); l_it != W->m_Links.end(); l_it++)
          PHelper().CreateFloat(
              items,
              PrepareKey(pref, "Way Point\\Links", *(*l_it)->way_point->m_Name),
              &(*l_it)->probability);

        for (int k = 0; k < 32; k++)
          PHelper().CreateFlag32(
              items,
              PrepareKey(pref, "Way Point\\Flags", AnsiString(k).c_str()),
              &W->m_Flags, 1 << k);
      }
    }
  }
