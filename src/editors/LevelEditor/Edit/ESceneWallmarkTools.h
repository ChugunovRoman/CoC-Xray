...
private:
  ...
  void BuildMatrix(Fmatrix &mView, float inv_w, float inv_h, float angle,
                   const Fvector &from);
  BOOL AddWallmark_internal(const Fvector &S, const Fvector &D, shared_str s,
                            shared_str t, float w, float h, float r);
...
public:
  enum { flDrawWallmark = (1 << 0), flAxisAlign = (1 << 1) };
  Flags32 m_Flags;
