...
xr_token shape_type_tok[] = {
    {"common", eShapeCommon}, {"level bound", eShapeLevelBound}, {0, 0}};
...
void CEditShape::Detach() {
  if (shapes.size() > 1) {
    ...
    for (; it != shapes.end(); it++) {
      ...
      CEditShape *shape =
          (CEditShape *)Scene->GetOTool(ClassID)->CreateObject(0, namebuffer);
      switch (it->type) {
      case cfSphere: {
        Fsphere T = it->data.sphere;
        M.transform_tiny(T.P);
...
bool CEditShape::RayPick(float &distance, const Fvector &start,
...
void CEditShape::FillProp(LPCSTR pref, PropItemVec &values) {
  ...
  PHelper().CreateCaption(values, PrepareKey(pref, "Shape usage"),
                          m_shape_type == eShapeCommon ? "common"
                                                       : "level bound");
}
...
void CEditShape::Render(int priority, bool strictB2F) {
  ...
  if (1 == priority) {
    if (strictB2F) {
      ...
      u32 clr = Selected() ? subst_alpha(m_DrawTranspColor,
                                         color_get_A(m_DrawTranspColor) * 2)
                           : m_DrawTranspColor;
