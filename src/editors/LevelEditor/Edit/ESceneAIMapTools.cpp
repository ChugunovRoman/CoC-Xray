...
ESceneAIMapTool::ESceneAIMapTool() : ESceneToolBase(OBJCLASS_AIMAP) {
  ...
  m_AIBBox.invalidate();
  //    m_Header.size_y				=
  //    m_Header.aabb.max.y-m_Header.aabb.min.y+EPS_L;
  hash_Initialize();
  m_VisRadius = 30.f;
  m_SmoothHeight = 0.5f;
  m_BrushSize = 1;
  ...
  for (AINodeIt it = m_Nodes.begin(); it != m_Nodes.end(); it++) {
    if (!((((u32)(*it)->n1 < cnt) || ((u32)(*it)->n1 == InvalidNode)) &&
      ...
      ELog.Msg(mtError,
               ...
               VPUSH((*it)->Pos), (*it)->n1, (*it)->n2, (*it)->n3, (*it)->n4);
      (*it)->n1 = 0;
      (*it)->n2 = 0;
      (*it)->n3 = 0;
      (*it)->n4 = 0;
    ...
    /*
       ...
       = ((u32)(*it)->n1==InvalidNode)?0:m_Nodes[(u32)(*it)->n1]; else
       (*it)->n1=0; if (((u32)(*it)->n2<cnt)||((u32)(*it)->n2==InvalidNode))
       (*it)->n2	=
       ((u32)(*it)->n2==InvalidNode)?0:m_Nodes[(u32)(*it)->n2]; else
       (*it)->n2=0; if (((u32)(*it)->n3<cnt)||((u32)(*it)->n3==InvalidNode))
       (*it)->n3	=
       ((u32)(*it)->n3==InvalidNode)?0:m_Nodes[(u32)(*it)->n3]; else
       (*it)->n3=0; if (((u32)(*it)->n4<cnt)||((u32)(*it)->n4==InvalidNode))
       (*it)->n4	=
       ((u32)(*it)->n4==InvalidNode)?0:m_Nodes[(u32)(*it)->n4]; else
       (*it)->n4=0;
    */
  }
}
  ...
  if (F.find_chunk(AIMAP_CHUNK_SNAP_OBJECTS)) {
    ...
    if (cnt) {
      for (int i = 0; i < cnt; i++) {
        F.r_stringZ(buf);
        CCustomObject *O =
            Scene->FindObjectByName(buf.c_str(), OBJCLASS_SCENEOBJECT);
        if (!O)
...
void ESceneAIMapTool::OnObjectRemove(CCustomObject *O, bool bDeleting) {
  if (OBJCLASS_SCENEOBJECT == O->ClassID) {
    if (find(m_SnapObjects.begin(), m_SnapObjects.end(), O) !=
        m_SnapObjects.end()) {
...
int ESceneAIMapTool::AddNode(const Fvector &pos, bool bIgnoreConstraints,
                             bool bAutoLink, int sz) {
  Fvector Pos = pos;
  if (1 == sz) {
...
void ESceneAIMapTool::RemoveSelection() {
  ...
  case estAIMapNode: {
    if (m_Nodes.size() == (u32)SelectionCount(true)) {
      ...
      // remove sel nodes
      AINodeIt result = std::remove_if(m_Nodes.begin(), m_Nodes.end(),
                                       delete_sel_node_pred());
...
void ESceneAIMapTool::FillProp(LPCSTR pref, PropItemVec &items) {
  PHelper().CreateFlag32(items, PrepareKey(pref, "Common\\Draw Nodes"),
                         &m_Flags, flHideNodes, 0, 0,
                         FlagValueCustom::flInvertedDraw);
  PHelper().CreateFlag32(items, PrepareKey(pref, "Common\\Slow Calculate Mode"),
                         &m_Flags, flSlowCalculate);
  PHelper().CreateFloat(items, PrepareKey(pref, "Common\\Visible Radius"),
                        &m_VisRadius, 10.f, 250.f);
  PHelper().CreateFloat(items, PrepareKey(pref, "Common\\Smooth Height"),
                        &m_SmoothHeight, 0.1f, 100.f);

  PHelper().CreateU32(items, PrepareKey(pref, "Params\\Brush Size"),
                      &m_BrushSize, 1, 100);
  PHelper().CreateFloat(items, PrepareKey(pref, "Params\\Can Up"),
                        &m_Params.fCanUP, 0.f, 10.f);
  PHelper().CreateFloat(items, PrepareKey(pref, "Params\\Can Down"),
                        &m_Params.fCanDOWN, 0.f, 10.f);
}
