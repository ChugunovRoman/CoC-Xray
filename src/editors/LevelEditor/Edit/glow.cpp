...
void CGlow::Render(int priority, bool strictB2F) {
  if ((1 == priority) && (true == strictB2F)) {
    ...
    if (gt->m_Flags.is(ESceneGlowTool::flTestVisibility)) {
      ...
      if (!Scene->RayPickObject(dist, PPosition, D, OBJCLASS_SCENEOBJECT, 0,
                                0)) {
        if (m_GShader) {
    ...
    if (Selected()) {
      ...
      if (gt->m_Flags.is(ESceneGlowTool::flDrawCross)) {
        Fvector sz;
        bb.getradius(sz);
        DU_impl.DrawCross(PPosition, sz.x, sz.y, sz.z, sz.x, sz.y, sz.z,
                          0xFFFFFFFF, false);
      }
    }
  }
...
bool CGlow::RayPick(float &distance, const Fvector &start,
...
void CGlow::FillProp(LPCSTR pref, PropItemVec &items) {
  ...
  PropValue *V = 0;
  V = PHelper().CreateChoose(items, PrepareKey(pref, "Texture"), &m_TexName,
                             smTexture);
  V->OnChangeEvent.bind(this, &CGlow::ShaderChange);
  V = PHelper().CreateChoose(items, PrepareKey(pref, "Shader"), &m_ShaderName,
                             smEShader);
  V->OnChangeEvent.bind(this, &CGlow::ShaderChange);
  PHelper().CreateFloat(items, PrepareKey(pref, "Radius"), &m_fRadius, 0.01f,
                        10000.f);
  //.	PHelper().CreateFlag<Flags8>(items,PHelper().PrepareKey(pref,"Fixed
  // size"),	&m_Flags, 		gfFixedSize);
}
//----------------------------------------------------
...
bool CGlow::GetSummaryInfo(SSceneSummary *inf) {
  ...
  if (m_TexName.size())
    inf->AppendTexture(ChangeFileExt(*m_TexName, "").LowerCase().c_str(),
                       SSceneSummary::sttGlow, 0, 0, "$GLOW$");
  inf->glow_cnt++;
  return true;
}
