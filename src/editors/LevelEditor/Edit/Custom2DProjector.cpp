    ...
    for (EditMeshIt m_it = O->FirstMesh(); m_it != O->LastMesh(); m_it++) {
      for (u32 f_id = 0; f_id != (*m_it)->GetFCount(); f_id++) {
        ...
        for (int k = 0; k < 3; k++) {
          T.transform_tiny(
              v.p,
              (*m_it)->GetVertices()[(*m_it)->GetFaces()[f_id].pv[k].pindex]);
...
void CCustom2DProjector::Render(bool blended) {
  ...
  for (int k = 0; k < cnt.quot; k++) {
    ...
    CopyMemory(pv, mesh.begin() + k * MAX_BUF_SIZE,
               sizeof(FVF::V) * MAX_BUF_SIZE);
  ...
  if (cnt.rem) {
    ...
    CopyMemory(pv, mesh.begin() + cnt.quot * MAX_BUF_SIZE,
               sizeof(FVF::V) * cnt.rem);
