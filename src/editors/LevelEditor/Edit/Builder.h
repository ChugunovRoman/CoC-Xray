...
public:
  ...
  BOOL BuildMesh(const Fmatrix &parent, CEditableObject *object,
                 CEditableMesh *mesh, int sector_num, b_vertex *verts,
                 int &vert_cnt, int &vert_it, b_face *faces, int &face_cnt,
                 int &face_it, u32 *smooth_groups,
  ...
  BOOL BuildPointLight(b_light *b, const Flags32 &usage,
                       svector<WORD, 16> *sectors, FvectorVec *soft_points,
  ...
  int BuildMaterial(LPCSTR esh_name, LPCSTR csh_name, LPCSTR tx_name,
                    u32 tx_cnt, int sector_num, bool allow_draft);
...
protected:
  ...
  BOOL BuildLTX();
  BOOL ParseGAME(IWriter &game, IWriter &spawn, ObjectList &lst,
                 LPCSTR prefix = 0);
  BOOL BuildGame();
