...
bool CGroupObject::GetBox(Fbox &bb) const {
  bb.invalidate();
  // update box
  for (ObjectsInGroup::const_iterator it = m_ObjectsInGroup.begin();
  ...
  for (ObjectsInGroup::iterator it = m_ObjectsInGroup.begin();
    ...
    if (it->pObject->IsRender()) {
      ...
      case OBJCLASS_SCENEOBJECT:
        ...
        break;
      default:
        EDevice.SetShader(strictB2F ? EDevice.m_SelectionShader
...
bool CGroupObject::RayPick(float &distance, const Fvector &start,
