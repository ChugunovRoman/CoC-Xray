...
bool CPortal::GetBox(Fbox &box) const {
  ...
  for (FvectorVec::const_iterator pt = m_Vertices.begin() + 1;
       pt != m_Vertices.end(); pt++)
    box.modify(*pt);
...
void CPortal::Render(int priority, bool strictB2F) {
  if ((1 == priority) && (false == strictB2F)) {
    ...
    if (m_SectorFront) {
      ...
      DU_impl.DrawPrimitiveL(D3DPT_TRIANGLEFAN, V.size() - 2, V.begin(),
                             V.size(), col.get(), true, false);
      EDevice.SetRS(D3DRS_CULLMODE, D3DCULL_CCW);
    }
    // back
    if (m_SectorBack) {
    ...
    if (!Selected())
      col.mul_rgb(0.5f);
    // render portal edges
    EScenePortalTool *lt = dynamic_cast<EScenePortalTool *>(ParentTool);
    VERIFY(lt);
    FvectorVec &src_ln = (lt->m_Flags.is(EScenePortalTool::flDrawSimpleModel))
                             ? m_SimplifyVertices
                             : m_Vertices;
    DU_impl.DrawPrimitiveL(D3DPT_LINESTRIP, src_ln.size(), src_ln.begin(),
                           src_ln.size(), col.get(), true, true);
    EDevice.ResetNearer();
...
void CPortal::Move(Fvector &amount) {
  ...
  for (v_it = m_Vertices.begin(); v_it != m_Vertices.end(); v_it++)
    v_it->add(amount);
  for (v_it = m_SimplifyVertices.begin(); v_it != m_SimplifyVertices.end();
       v_it++)
    v_it->add(amount);
  m_Center.add(amount);
}
...
bool CPortal::FrustumPick(const CFrustum &frustum) {
  if (frustum.testPolyInside(m_Vertices.begin(), m_Vertices.size()))
    return true;
  return false;
}
//------------------------------------------------------------------------------

bool CPortal::RayPick(float &distance, const Fvector &start,
  ...
  EScenePortalTool *lt = dynamic_cast<EScenePortalTool *>(ParentTool);
  VERIFY(lt);
  FvectorVec &src = (lt->m_Flags.is(EScenePortalTool::flDrawSimpleModel))
                        ? m_SimplifyVertices
                        : m_Vertices;
...
bool CPortal::Update(bool bLoadMode) {
  ...
  if (fabsf(m) <= EPS_S) {
    Tools->m_DebugDraw.AppendWireFace(m_Vertices[0], m_Vertices[1],
                                      m_Vertices[2]);
...
bool Isect2DLvs2DL(Fvector2 &P1, Fvector2 &P2, Fvector2 &P3, Fvector2 &P4,
                   Fvector2 &P) {
...
void SimplifyVertices(Fvector2Vec &vertices) {
  ...
  for (u32 I = 0; I < vertices.size(); I++) {
    // 2. Build two lines (1[L1a,L1b], 2[L2a,L2b]) and search point of
    // intersection
    int L1a = I - 1;
    if (L1a < 0)
      L1a = vertices.size() - 1;
    ...
    Fvector2 P_Test;
    if (!Isect2DLvs2DL(vertices[L1a], vertices[L1b], vertices[L2b],
                       vertices[L2a], P_Test))
      continue;

    double area =
        tri_area(vertices[L1b], P_Test,
                 vertices[L2a]); //*(vertices[L1b].dist(vertices[L2a]));
...
void CPortal::Simplify() {
  ...
  // compute plane
  Fplane P;
  Mgc::OrthogonalPlaneFit(m_Vertices.size(), (Mgc::Vector3 *)m_Vertices.begin(),
                          (Mgc::Vector3 &)rkOffset, (Mgc::Vector3 &)rkNormal);
  ...
  //    R_ASSERT2(0,"Go to ALEXMX and say: ''Test portal simplifier.
  //    Please!''");
  /*
      ...
      for (k=0; k<vertices.size(); k++){
          ...
          if (vertices[k].similar(vertices[k1])){
     vertices.erase(vertices.begin()+k); k--; continue; } Fvector2 e1,e2;
          e1.sub(vertices[k1],vertices[k]); e1.norm();
          e2.sub(vertices[k2],vertices[k]); e2.norm();
          if ((1-fabsf(e1.dot(e2)))<=EPS){ vertices.erase(vertices.begin()+k);
     k--; continue; }
      }
  */
  // simplify
...
bool CPortal::LoadLTX(CInifile &ini, LPCSTR sect_name) {
  ...
  if (cnt < 3) {
    ELog.Msg(
        mtError,
        "Portal: '%s' can't create.\nInvalid portal. (m_Vertices.size()<3)",
        Name);
    return false;
  }
...
void CPortal::SaveLTX(CInifile &ini, LPCSTR sect_name) {
  ...
  ini.w_string(sect_name, "sector_front",
               m_SectorFront ? m_SectorFront->Name : "");
  ini.w_string(sect_name, "sector_back",
               m_SectorBack ? m_SectorBack->Name : "");
