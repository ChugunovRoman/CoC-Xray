...
public:
  SBFace(CSurface *_surf, const Fvector2 *_uv[3])
      : surf(_surf), marked(false), bone_id(-1) {
    vert_id[0] = -1;
    vert_id[1] = -1;
    vert_id[2] = -1;
    uv[0] = *_uv[0];
...
struct SBBone {
  ...
  u32 f_cnt;
  float area;
  SBBone(AnsiString _nm, AnsiString _parent, AnsiString _mtl, u32 _f_cnt,
         float _area)
...
public:
  ...
  void Clear();
  // I/O routines
  void AppendFace(CSurface *surf, const Fvector *v, const Fvector *n,
                  const Fvector2 *uvs[3]);
