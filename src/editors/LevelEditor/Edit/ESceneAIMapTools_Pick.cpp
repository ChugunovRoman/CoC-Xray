...
SAINode *ESceneAIMapTool::PickNode(const Fvector &start, const Fvector &dir,
                                   float &dist) {
  SAINode *R = 0;
  CFrustum frustum;
...
bool ESceneAIMapTool::PickObjects(Fvector &dest, const Fvector &start,
                                  const Fvector &dir, float dist) {
  SPickQuery PQ;
  if (!GetSnapList()->empty()) {
    if (Scene->RayQuery(PQ, start, dir, dist,
                        CDB::OPT_ONLYNEAREST | CDB::OPT_CULL, GetSnapList())) {
...
int ESceneAIMapTool::RaySelect(int flag, float &distance, const Fvector &start,
