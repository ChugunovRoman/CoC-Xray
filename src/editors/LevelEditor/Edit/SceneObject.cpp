...
//----------------------------------------------------
CSceneObject::CSceneObject(LPVOID data, LPCSTR name)
...
CSceneObject::~CSceneObject() { Lib.RemoveEditObject(m_pReference); }
//----------------------------------------------------
...
void CSceneObject::RenderBlink() {
  if (m_iBlinkTime > 0) {
    if (m_iBlinkTime > (int)EDevice.dwTimeGlobal) {
      int alpha = iFloor(
          sqrtf(float(m_iBlinkTime - EDevice.dwTimeGlobal) / BLINK_TIME) * 64);
      m_pReference->RenderSelection(_Transform(), 0, m_BlinkSurf,
...
bool CSceneObject::RayPick(float &dist, const Fvector &S, const Fvector &D,
                           SRayPickInfo *pinf) {
  if (!m_pReference)
    return false;
  if (::Render->occ_visible(m_TBBox))
...
CEditableObject *CSceneObject::UpdateReference() {
  ...
  m_pReference =
      (m_ReferenceName.size()) ? Lib.CreateEditObject(*m_ReferenceName) : 0;
  UpdateTransform();
  return m_pReference;
}
...
bool CSceneObject::GetSummaryInfo(SSceneSummary *inf) {
  ...
  if (IsStatic() || IsMUStatic()) {
    for (SurfaceIt s_it = E->m_Surfaces.begin(); s_it != E->m_Surfaces.end();
         s_it++) {
      float area = 0.f;
      ...
      inf->AppendTexture(ChangeFileExt(AnsiString(*(*s_it)->m_Texture), "")
                             .LowerCase()
                             .c_str(),
    ...
    if (m_Flags.is(CEditableObject::eoUsingLOD)) {
      inf->AppendTexture(E->GetLODTextureName().c_str(), SSceneSummary::sttLOD,
                         0, 0, "$LOD$");
...
void CSceneObject::OnShowHint(AStringVec &dest) {
  ...
  float dist = UI->ZFar();
  SRayPickInfo pinf;
  if (m_pReference->RayPick(dist, UI->m_CurrentRStart, UI->m_CurrentRDir,
                            _ITransform(), &pinf)) {
    dest.push_back(
        AnsiString("Object Type: ") +
        get_token_name(eo_type_token, pinf.e_obj->m_objectFlags.flags));
    ...
    if (pinf.e_obj->m_objectFlags.is(CEditableObject::eoSoundOccluder)) {
      dest.push_back(AnsiString("Game Mtl: ") +
                     AnsiString(surf->_GameMtlName()));
      int gm_id = surf->_GameMtl();
      if (gm_id != GAMEMTL_NONE_ID) {
        SGameMtl *mtl = GMLib.GetMaterialByID(gm_id);
        if (mtl)
          dest.push_back(AnsiString().sprintf("Occlusion Factor: %3.2f",
      ...
      dest.push_back(AnsiString("LC Shader: ") +
