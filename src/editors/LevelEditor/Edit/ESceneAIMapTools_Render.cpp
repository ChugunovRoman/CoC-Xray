  ...
  if (1 == priority) {
    if (false == strictB2F) {
      ...
      if (Valid()) {
        ...
        FVF::LIT *pv =
            (FVF::LIT *)Stream->Lock(block_size, m_RGeom->vb_stride, vBase);
        u32 cnt = 0;
          ...
          for (int z = rect.y1; z <= rect.y2; z++) {
            ...
            if (nodes) {
              ...
              for (AINodeIt it = nodes->begin(); it != nodes->end(); it++) {
                ...
                float b = (_abs(p_denom) < EPS_S)
                              ? m_Params.fPatchSize
                              : _abs(N.Plane.classify(v) / p_denom);
                ...
                if (Render->ViewBase.testSphere_dirty(N.Pos, _max(b, st))) {
                  ...
                  if (N.flags.is(SAINode::flSelected))
                    ...
                    clr = N.flags.is(SAINode::flHLSelected) ? 0xff909090
                                                            : 0xff606060;
                  int k = 0;
                  if (N.n1)
                    k |= 1 << 0;
                  ...
                  if (cnt >= block_size - 6) {
                    ...
                    pv = (FVF::LIT *)Stream->Lock(block_size,
                                                  m_RGeom->vb_stride, vBase);
                    cnt = 0;
                  }
                }
              }
