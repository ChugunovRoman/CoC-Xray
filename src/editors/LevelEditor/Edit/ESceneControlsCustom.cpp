...
// add
//------------------------------------------------------------------------------
CCustomObject *__fastcall TUI_CustomControl::DefaultAddObject(
    TShiftState Shift, TBeforeAppendCallback before,
    TAfterAppendCallback after) {
  if (Shift == ssRBOnly) {
  ...
  if (LUI->PickGround(p, UI->m_CurrentRStart, UI->m_CurrentRDir, 1, &n)) {
    ...
    Scene->GenObjectName(parent_tool->ClassID, namebuffer,
                         P.name_prefix.c_str());
    obj =
        Scene->GetOTool(parent_tool->ClassID)->CreateObject(P.data, namebuffer);
    if (!obj->Valid()) {
      xr_delete(obj);
      return 0;
    }
...
bool __fastcall TUI_CustomControl::AddEnd(TShiftState _Shift) { return true; }

bool TUI_CustomControl::CheckSnapList(TShiftState Shift) {
  if (fraLeftBar->ebSnapListMode->Down) {
    CCustomObject *O =
        Scene->RayPickObject(UI->ZFar(), UI->m_CurrentRStart, UI->m_CurrentRDir,
                             OBJCLASS_SCENEOBJECT, 0, 0);
    if (O) {
...
bool __fastcall TUI_CustomControl::SelectStart(TShiftState Shift) {
  ...
  int cnt = Scene->RaySelect(Shift.Contains(ssCtrl)  ? -1
                             : Shift.Contains(ssAlt) ? 0
                                                     : 1,
                             parent_tool->ClassID);
  bBoxSelection =
      ((0 != cnt) && (Shift.Contains(ssCtrl) || Shift.Contains(ssAlt))) ||
      (0 == cnt);
  if (bBoxSelection) {
...
bool __fastcall TUI_CustomControl::SelectEnd(TShiftState _Shift) {
  if (bBoxSelection) {
    ...
    bBoxSelection = false;
    Scene->FrustumSelect(_Shift.Contains(ssAlt) ? 0 : 1,
                         LTools->CurrentClassID());
  }
  return true;
}
  ...
  if (Shift.Contains(ssCtrl)) {
    ...
    if (Scene->GetQueryObjects(lst, LTools->CurrentClassID(), 1, 1, 0)) {
      if (lst.size() == 1) {
        ...
        EDevice.m_Camera.MouseRayFromPoint(UI->m_CurrentRStart,
                                           UI->m_CurrentRDir, UI->m_CurrentCp);
        if (LUI->PickGround(p, UI->m_CurrentRStart, UI->m_CurrentRDir, 1, &n)) {
...
bool __fastcall TUI_CustomControl::DefaultMovingProcess(TShiftState Shift,
                                                        Fvector &amount) {
