...
void SBPart::recurse_fragment(SBFace *F, u32 &cnt, u32 bone_id, u32 max_faces,
                              float &area) {
  if (F) {
    if (!F->marked)
...
bool SBPart::prepare(SBAdjVec &adjs, u32 bone_face_min) {
  ...
  m_BBox.invalidate();
  Fmatrix M;
  M.set(m_OBB.m_rotate.i, m_OBB.m_rotate.j, m_OBB.m_rotate.k,
        m_OBB.m_translate);
  ...
  for (f_it = m_Faces.begin(); f_it != m_Faces.end(); f_it++) {
    SBFace *F = (*f_it);
    if (F->adjs.empty()) {
      ELog.Msg(mtError, "Error face found at pos: [%3.2f,%3.2f,%3.2f]",
               VPUSH(F->o[0]));
  ...
  if (m_bValid) {
    ...
    int bone_cnt_max =
        (bone_cnt_calc < 62) ? (bone_cnt_calc <= 0 ? 1 : bone_cnt_calc) : 62;
    ...
    do {
      ...
      for (SBFaceVecIt f_it = m_Faces.begin(); f_it != m_Faces.end(); f_it++) {
        if (!(*f_it)->marked) {
          F = *f_it;
          int cnt = 0;
          for (SBFaceVecIt a_it = F->adjs.begin(); a_it != F->adjs.end();
               a_it++)
            cnt += (*a_it)->marked ? 0 : 1;
          if ((cnt == 0) || (cnt >= 2))
            break;
      ...
      if (face_accum == 1) {
        ...
        m_Bones.push_back(SBBone(bone_idx, parent_bone, F->surf->_GameMtlName(),
                                 face_accum, area));
        parent_bone = "0";
        bone_idx++;
    ...
    while (face_accum_total < m_Faces.size()) {
      if (face_accum_total_saved != face_accum_total) {
        ...
        // если проход оказалс€ безрезультатным (т.е. не смогли добавить хоть
        // один фейс) - добавл€ем
        //  все неприаттаченные фейсы к 0-кости (иначе зацикливаетс€ (mike -
        //  L03_agroprom))
        for (SBFaceVecIt f_it = m_Faces.begin(); f_it != m_Faces.end();
             f_it++) {
          SBFace *F = *f_it;
          if (-1 == F->bone_id) {
            F->marked = true;
...
bool SBPart::Export(IWriter &F, u8 infl) {
  ...
  for (SBFaceVecIt pf_it = m_Faces.begin(); pf_it != m_Faces.end(); pf_it++) {
    SBFace *face = *pf_it;
    int mtl_idx =
        FindSplit(face->surf->_ShaderName(), face->surf->_Texture(), 0);
    if (mtl_idx < 0) {
  ...
  for (SplitIt split_it = m_Splits.begin(); split_it != m_Splits.end();
       split_it++) {
    if (!split_it->valid()) {
      ELog.Msg(mtError, "Degenerate part found (Texture '%s').",
               *split_it->m_Texture);
      bRes = false;
      break;
    }
    if (0 != split_it->invalid_faces) {
      ELog.Msg(mtError,
               ...
               *split_it->m_Texture, split_it->invalid_faces);
    }
    ...
    for (SkelVertIt sv_it = lst.begin(); sv_it != lst.end(); sv_it++) {
      ...
      bone_points[sv_it->bones[0].id].back().sub(
          m_Bones[sv_it->bones[0].id].offset);
    }
  }
...
//----------------------------------------------------
void CGeomPartExtractor::AppendFace(CSurface *surf, const Fvector *v,
                                    const Fvector *n, const Fvector2 *uvs[3]) {
...
void CGeomPartExtractor::Initialize(const Fbox &bb, float eps,
                                    u32 per_bone_face_count_min) {
  VERIFY(0 == m_Verts);
