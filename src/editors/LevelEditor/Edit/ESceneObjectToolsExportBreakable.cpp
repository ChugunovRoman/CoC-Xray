...
//----------------------------------------------------
IC bool build_mesh(const Fmatrix &parent, CEditableMesh *mesh,
                   CGeomPartExtractor *extractor, u32 game_mtl_mask,
                   BOOL ignore_shader) {
  bool bResult = true;
  mesh->GenerateVNormals(&parent);
  // fill faces
  for (SurfFaces::const_iterator sp_it = mesh->GetSurfFaces().begin();
       sp_it != mesh->GetSurfFaces().end(); sp_it++) {
    ...
    if (!ignore_shader) {
      ...
      if (FALSE == B) {
        ELog.Msg(mtError,
                 "Can't find engine shader '%s'. Object '%s', surface '%s'. "
                 "Export interrupted.",
                 surf->_ShaderName(), mesh->Parent()->m_LibName.c_str(),
                 surf->_Name());
        bResult = FALSE;
        break;
      }
      if (TRUE == B->canBeLMAPped()) {
        ELog.Msg(mtError,
                 "Object '%s', surface '%s' contain static engine shader - "
                 "'%s'. Export interrupted.",
                 mesh->Parent()->m_LibName.c_str(), surf->_Name(),
                 surf->_ShaderName());
        bResult = FALSE;
        break;
    ...
    VERIFY(pts);
    for (IntVec::const_iterator f_it = face_lst.begin(); f_it != face_lst.end();
         f_it++) {
...
bool ESceneObjectTool::ExportBreakableObjects(SExportStreams *F) {
  ...
  if (bResult) {
    ...
    for (SBPartVecIt p_it = parts.begin(); p_it != parts.end(); p_it++) {
      ...
      if (P->Valid()) {
        // export visual
        AnsiString sn = AnsiString().sprintf("meshes\\brkbl#%d.ogf",
                                             (p_it - parts.begin()));
        ...
        // export spawn object
        {
          ...
          ISE_Abstract *m_Data = create_entity(entity_ref.c_str());
          VERIFY(m_Data);
          ...
          if (s_draw_dbg) {
            ...
            Tools->m_DebugDraw.AppendLine(
                P->m_RefOffset, Fvector().mad(P->m_RefOffset, MX.k, 1.f),
        ...
        ELog.Msg(mtError, "Can't export invalid part #%d",
                 p_it - parts.begin());
      }
    }
    UI->ProgressEnd(pb);
  ...
  if (bResult) {
    ...
    for (SBPartVecIt p_it = parts.begin(); p_it != parts.end(); p_it++) {
      ...
      if (P->Valid()) {
        // export visual
        AnsiString sn =
            AnsiString().sprintf("clmbl#%d", (p_it - parts.begin()));

        Fvector local_normal = {0, 0, 0};

        LPCSTR mat_name = NULL;
        for (SBFaceVecIt it = P->m_Faces.begin(); it != P->m_Faces.end();
             it++) {
          for (u32 k = 0; k < 3; k++)
        ...
        // export spawn object
        {
          ...
          // orientate object
          if (!OrientToNorm(local_normal, P->m_OBB.m_rotate,
                            P->m_OBB.m_halfsize)) {
            ELog.Msg(mtError,
            ...
            Fmatrix M;
            M.set(P->m_OBB.m_rotate.i, P->m_OBB.m_rotate.j, P->m_OBB.m_rotate.k,
                  P->m_OBB.m_translate);
            ...
            if (s_draw_dbg) {
              ...
              Tools->m_DebugDraw.AppendLine(
                  P->m_RefOffset,
                  Fvector().mad(P->m_RefOffset, local_normal, 1.f));
            }
          }
          destroy_entity(m_Data);
        }
