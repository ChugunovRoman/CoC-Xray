...
ObjectList *ESceneCustomOTool::GetSnapList() { return 0; }
//----------------------------------------------------
...
bool ESceneCustomOTool::Valid() { return true; }

bool ESceneCustomOTool::IsNeedSave() { return !m_Objects.empty(); }
...
void ESceneCustomOTool::RemoveSelection() {
  ...
  while (_F != m_Objects.end()) {
    if ((*_F)->Selected() &&
        !(*_F)->m_CO_Flags.test(CCustomObject::flObjectInGroup)) {
...
void ESceneCustomOTool::ShowObjects(bool flag, bool bAllowSelectionFlag,
                                    bool bSelFlag) {
  for (ObjectIt _F = m_Objects.begin(); _F != m_Objects.end(); _F++) {
...
BOOL ESceneCustomOTool::RayPick(CCustomObject *&object, float &distance,
...
BOOL ESceneCustomOTool::SpherePick(ObjectList &lst, const Fvector &center,
                                   float radius) {
  for (ObjectIt _F = m_Objects.begin(); _F != m_Objects.end(); _F++)
...
int ESceneCustomOTool::RaySelect(int flag, float &distance,
                                 const Fvector &start, const Fvector &direction,
                                 BOOL bDistanceOnly) {
...
int ESceneCustomOTool::GetQueryObjects(ObjectList &lst, int iSel, int iVis,
                                       int iLock) {
  int count = 0;
  for (ObjectIt _F = m_Objects.begin(); _F != m_Objects.end(); _F++) {
...
CCustomObject *ESceneCustomOTool::FindObjectByName(LPCSTR name,
                                                   CCustomObject *pass) {
...
void setEditable(PropItemVec &items, u32 start_idx, bool bEditableTool,
                 bool bObjectInGroup, bool bObjectInGroupUnique) {
  PropItemIt it = items.begin() + start_idx;
  PropItemIt it_e = items.end();
  u32 idx = 0;

  bool bEditableObject =
      bEditableTool &&
      (!bObjectInGroup || (bObjectInGroup && bObjectInGroupUnique));
...
void ESceneCustomOTool::FillProp(LPCSTR pref, PropItemVec &items) {
  for (ObjectIt it = m_Objects.begin(); it != m_Objects.end(); it++) {
    if ((*it)->Selected()) {
      ...
      setEditable(items, cnt, IsEditable(),
...
const CCustomObject *ESceneCustomOTool::LastSelected() const {
  for (ObjectList::const_iterator o_it = m_Objects.begin();
       o_it != m_Objects.end(); ++o_it) {
