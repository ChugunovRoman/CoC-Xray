...
public:
  ...
  // fuzzy
  struct SFuzzyData {
    enum EShapeType { fstSphere, fstBox, fstForceU8 = u8(-1) };
  ...
  void __stdcall OnFuzzyGenerateClick(ButtonValue *value, bool &bModif,
                                      bool &bSafe);
  ...
  void __stdcall OnAttenuationDraw(CanvasValue *sender, void *canvas,
                                   const Irect &rect);
  void __stdcall OnPointDataTestEqual(CanvasValue *a, CanvasValue *b,
                                      bool &res);

protected:
  //    virtual Fvector& GetPosition	()
  //    { return m_D3D.position; 	} virtual void 	SetPosition
  //    (const Fvector& pos)	{ m_D3D.position.set(pos);
  //    UpdateTransform();}
public:
