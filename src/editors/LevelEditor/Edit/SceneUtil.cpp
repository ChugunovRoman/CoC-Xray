...
CCustomObject *EScene::FindObjectByName(LPCSTR name,
    ...
    if (mt) {
      ...
      for (ObjectIt _F = lst.begin(); _F != lst.end(); _F++)
        if (FindObjectByName((*_F)->Name, *_F)) {
          ELog.DlgMsg(mtError, "Duplicate object name already exists: '%s'",
                      (*_F)->Name);
          return true;
        }
    }
...
void EScene::GenObjectName(ObjClassID cls_id, char *buffer, const char *pref) {
  ...
  VERIFY(ot);
  AnsiString result = FHelper.GenerateName(
      pref && pref[0] ? pref : ot->ClassName(), 4,
      fastdelegate::bind<TFindObjectByName>(this, &EScene::FindObjectByNameCB),
      true, true);
