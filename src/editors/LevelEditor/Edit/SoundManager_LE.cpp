...
bool CLevelSoundManager::Validate() {
  ...
  for (ObjectIt it = snd_envs.begin(); it != snd_envs.end(); it++) {
    ...
    if (E->m_EnvInner == E->m_EnvOuter) {
      ELog.DlgMsg(mtError,
                  "SoundEnvironment: '%s' inner and outer environment must be "
                  "different.",
                  E->Name);
      return false;
    }
...
bool CLevelSoundManager::MakeEnvGeometry(CMemoryWriter& F, bool bErrMsg)
{
    ...
    if (snd_envs.empty()){
                if (bErrMsg) ELog.Msg(mtError,"Scene hasn't sound environment
geometry."); return false;
    ...
    for (ObjectIt it=snd_envs.begin(); it!=snd_envs.end(); it++){
        ESoundEnvironment* E = dynamic_cast<ESoundEnvironment*>(*it);
R_ASSERT(E); Fbox bb; R_ASSERT	(E->GetBox(bb)); aabb.merge	(bb);
        ...
        int inner = -1;
        int outer = -1;
        for (RStringVecIt e_it=env_names.begin(); e_it!=env_names.end();
e_it++){ if ((-1==inner)&&(E->m_EnvInner==*e_it)) inner =
e_it-env_names.begin(); if ((-1==outer)&&(E->m_EnvOuter==*e_it)) outer =
e_it-env_names.begin(); if ((inner>-1)&&(outer>-1)) break;
        }

        if (-1==inner){ inner=env_names.size();
env_names.push_back(E->m_EnvInner);} if (-1==outer){ outer=env_names.size();
env_names.push_back(E->m_EnvOuter);} u32 idx = (inner<<16)|(outer);
...
M.transform_tiny(bv[k],du_box_vertices[k]); for (k=0; k<DU_BOX_NUMFACES; k++)
