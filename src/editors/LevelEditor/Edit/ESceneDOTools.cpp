        ...
        if (m_Flags.is(flSlotBoxesDraw)) {
          ...
          for (u32 z = 0; z < dtH.size_z; z++) {
            ...
            for (u32 x = 0; x < dtH.size_x; x++) {
              ...
              c.x = fromSlotX(x);
              c.y = slot->r_ybase() +
                    slot->r_yheight() * 0.5f; //(slot->y_max+slot->y_min)*0.5f;
              float dist = EDevice.m_Camera.GetPosition().distance_to_sqr(c);
              if ((dist < dist_lim) &&
                  ::Render->ViewBase.testSphere_dirty(c, DETAIL_SLOT_SIZE_2)) {
                bbox.min.set(c.x - DETAIL_SLOT_SIZE_2, slot->r_ybase(),
                             c.z - DETAIL_SLOT_SIZE_2);
                bbox.max.set(c.x + DETAIL_SLOT_SIZE_2,
...
void EDetailManager::SaveLTX(CInifile &ini, int id) {
  ...
  /*
      ...
      ini.w_u32			("detail_header", "object_count",
     dtH.object_count); ini.w_ivector2		("detail_header", "offset",
     Ivector2().set(dtH.offs_x, dtH.offs_z) );
      ini.w_ivector2		("detail_header", "size",
     Ivector2().set(dtH.size_x, dtH.size_z) );
      ...
      F.w_u32				(dtH.size_x*dtH.size_z);
          F.w
     (dtSlots,dtH.size_x*dtH.size_z*sizeof(DetailSlot)); F.close_chunk
     ();
      ...
      ini.w_float			("main", "detail_density",
     ps_r__Detail_density);

          // snap objects
      for (ObjectIt o_it=m_SnapObjects.begin(); o_it!=m_SnapObjects.end();
     ++o_it) ini.w_string	("snap_objects", (*o_it)->Name, NULL);
  */
}
  ...
  if (F.find_chunk(DETMGR_CHUNK_SNAP_OBJECTS)) {
    ...
    if (snap_cnt) {
      for (int i = 0; i < snap_cnt; i++) {
        ...
        if (!O)
          ELog.Msg(mtError, "EDetailManager: Can't find snap object '%s'.",
                   buf);
        else
...
bool EDetailManager::Export(LPCSTR path) {
  ...
  u32 new_idx = 0;
  for (DetailIt d_it = objects.begin(); d_it != objects.end();
       d_it++, remap_object_it++)
    if ((*remap_object_it == 1) &&
        (textures_set.find(((EDetail *)(*d_it))->GetTextureName()) !=
         textures_set.end()))
      *remap_object_it = (u8)new_idx++;

  AnsiString do_tex_name = ChangeFileExt(fn, "_details");
  int res = ImageLib.CreateMergedTexture(
      textures, do_tex_name.c_str(), STextureParams::tfADXT1, 256, 1024, 256,
      1024, offsets, scales, rotated, remap);
  if (1 != res)
    bRes = FALSE;
    ...
    for (DetailIt it = objects.begin(); it != objects.end(); it++) {
      if (remap_object[it - objects.begin()] != u8(-1)) {
        ...
        if (!((EDetail *)(*it))->m_pRefs) {
          ELog.DlgMsg(mtError, "Bad object or object not found '%s'.",
                      ((EDetail *)(*it))->m_sRefs.c_str());
          bRes = false;
        } else {
          ...
          t_idx = remap[t_idx];
          ((EDetail *)(*it))
              ->Export(F, do_tex_name.c_str(), offsets[t_idx], scales[t_idx],
                       rotated[t_idx]);
        }
        F.close_chunk();
        if (!bRes)
...
void EDetailManager::FillProp(LPCSTR pref, PropItemVec &items) {
  PropValue *P;
  P = PHelper().CreateFloat(items, PrepareKey(pref, "Objects per square"),
                            &ps_r__Detail_density);
  P->OnChangeEvent.bind(this, &EDetailManager::OnDensityChange);
  P = PHelper().CreateChoose(items, PrepareKey(pref, "Base Texture"),
                             &m_Base.name, smTexture);
  P->OnChangeEvent.bind(this, &EDetailManager::OnBaseTextureChange);
  PHelper().CreateFlag32(items, PrepareKey(pref, "Common\\Draw objects"),
                         &m_Flags, flObjectsDraw);
  PHelper().CreateFlag32(items, PrepareKey(pref, "Common\\Draw base texture"),
                         &m_Flags, flBaseTextureDraw);
  PHelper().CreateFlag32(items,
                         PrepareKey(pref, "Common\\Base texture blended"),
                         &m_Flags, flBaseTextureBlended);
  PHelper().CreateFlag32(items, PrepareKey(pref, "Common\\Draw slot boxes"),
                         &m_Flags, flSlotBoxesDraw);
}
...
bool EDetailManager::GetSummaryInfo(SSceneSummary *inf) {
  for (DetailIt it = objects.begin(); it != objects.end(); it++) {
    ...
    VERIFY(surf);
    inf->AppendTexture(surf->_Texture(), SSceneSummary::sttDO, 0, 0,
                       "$DETAILS$");
  }
  return true;
}
