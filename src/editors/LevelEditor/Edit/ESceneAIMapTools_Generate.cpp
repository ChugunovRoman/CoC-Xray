...
BOOL ESceneAIMapTool::CreateNode(Fvector &vAt, SAINode &N, bool bIC) {
  ...
  for (DWORD i = 0; i < dwCount; i++) {
    ...
    if (R->e_obj && R->e_mesh) {
      ...
      //.			SGameMtl* mtl 		=
      // GMLib.GetMaterialByID(surf->_GameMtl()); .			if
    ...
    /*
                  if(m_CFModel)
          {
              ...
              if(std::find(m_ignored_materials.begin(),
  m_ignored_materials.end(), mtl_id) != m_ignored_materials.end() )
              {
  //.                Msg("--ignore");
                  continue;
              }
...
void ESceneAIMapTool::HashRect(const Fvector &v, float radius, Irect &result) {
  ...
  VMmin.set(bb.min);
  VMeps.set(float(VMscale.x / HDIM_X / 2.f), float(0),
            float(VMscale.z / HDIM_Z / 2.f));
...
SAINode *ESceneAIMapTool::BuildNode(Fvector &vFrom, Fvector &vAt, bool bIC,
                                    bool bSuperIC) // return node's index
{
...
int ESceneAIMapTool::BuildNodes(const Fvector &pos, int sz, bool bIC) {
  ...
  int cnt = 0;
  if (m_CFModel)
    cnt = Scene->RayQuery(PQ, Pos, Dir, 3, CDB::OPT_ONLYNEAREST | CDB::OPT_CULL,
                          m_CFModel);
  else
    cnt = Scene->RayQuery(PQ, Pos, Dir, 3, CDB::OPT_ONLYNEAREST | CDB::OPT_CULL,
                          GetSnapList());
...
void ESceneAIMapTool::BuildNodes(bool bFromSelectedOnly) {
  ...
  float estimated_nodes =
      (LevelSize.x / m_Params.fPatchSize) * (LevelSize.z / m_Params.fPatchSize);
...
bool ESceneAIMapTool::GenerateMap(bool bFromSelectedOnly) {
  ...
  if (!GetSnapList()->empty()) {
    ...
    if (!m_Flags.is(flSlowCalculate)) {
      ...
      for (ObjectIt o_it = m_SnapObjects.begin(); o_it != m_SnapObjects.end();
        ...
        for (EditMeshIt m_it = _meshes.begin(); m_it != _meshes.end(); m_it++) {
          pb->Inc(AnsiString()
                      .sprintf("%s [%s]", S->Name, (*m_it)->Name().c_str())
                      .c_str());
          const SurfFaces &_sfaces = (*m_it)->GetSurfFaces();
          for (SurfFaces::const_iterator sp_it = _sfaces.begin();
               sp_it != _sfaces.end(); sp_it++) {
            CSurface *surf = sp_it->first;
            // test passable
            //.			        SGameMtl* mtl 		=
            // GMLib.GetMaterialByID(surf->_GameMtl());
            //.					if
            //(mtl->Flags.is(SGameMtl::flPassable))continue;
            ...
            for (IntVec::const_iterator it = face_lst.begin();
                 it != face_lst.end(); it++) {
              E->GetFaceWorld(S->_Transform(), *m_it, *it, verts);

              ETOOLS::collector_add_face_d(CL, verts[0], verts[1], verts[2],
                                           surf->_GameMtl() /* *it */);
              if (surf->m_Flags.is(CSurface::sf2Sided))
                ETOOLS::collector_add_face_d(CL, verts[2], verts[1], verts[0],
                                             surf->_GameMtl() /* *it */);
            }
          }
        }
    ...
    //.        Msg("-Rate: %3.2f Count:
    //%d",(g_tm.GetElapsed_sec()/tm.GetElapsed_sec())*100.f,g_tm.count);
...
static const u8 fl[4] = {SAINode::flN1, SAINode::flN2, SAINode::flN3,
                         SAINode::flN4};
void ESceneAIMapTool::InvertLinks() {
  ...
  for (AINodeIt a_it = m_Nodes.begin(); a_it != m_Nodes.end(); a_it++)
    (*a_it)->flags.set(
        SAINode::flN1 | SAINode::flN2 | SAINode::flN3 | SAINode::flN4, FALSE);
  UpdateHLSelected();
}

SAINode *ESceneAIMapTool::FindNeighbor(SAINode *N, int side,
  ...
  if (nodes)
    if (bIgnoreConstraints) {
      ...
      for (AINodeIt I = nodes->begin(); I != nodes->end(); I++)
        if (fsimilar((*I)->Pos.x, Pos.x, EPS_L) &&
            fsimilar((*I)->Pos.z, Pos.z, EPS_L)) {
...
void ESceneAIMapTool::MakeLinks(u8 side_flag, EMode mode,
                                bool bIgnoreConstraints) {
  if (!side_flag)
    return;
  for (AINodeIt it = m_Nodes.begin(); it != m_Nodes.end(); it++) {
...
void ESceneAIMapTool::SmoothNodes() {
  ...
  for (AINodeIt it = m_Nodes.begin(); it != m_Nodes.end(); it++) {
    ...
    if (N.flags.is(SAINode::flSelected)) {
      ...
      vNorm.set(N.Plane.n);
      vOffs.set(N.Pos);
      Mgc::OrthogonalPlaneFit(4, (Mgc::Vector3 *)data,
                              *((Mgc::Vector3 *)&vOffs),
                              *((Mgc::Vector3 *)&vNorm));
      if (vNorm.y < 0)
        vNorm.invert();
