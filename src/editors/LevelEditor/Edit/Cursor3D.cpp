...
void C3DCursor::GetPickPoint(Fvector &src, Fvector &dst, Fvector *N) {
  ...
  Fvector pick_dir;
  pick_dir.set(0, -1, 0);
  if (Scene->RayPickObject(pinf.inf.range, start, pick_dir,
                           OBJCLASS_SCENEOBJECT, &pinf,
  ...
  if (m_Visible && !EDevice.m_Camera.IsMoving()) {
    ...
    if (LUI->PickGround(pinf.pt, start, dir, -1)) {
      ...
      case csLasso: {
        ...
        DU_impl.DrawPrimitiveL(D3DPT_LINESTRIP, m_RenderBuffer.size(),
                               m_RenderBuffer.begin(), m_RenderBuffer.size(),
                               dwColor, true, true);
        //                UI->D3D_ResetNearer();
      } break;
      case csPoint: {
        ...
        pt[1].set(pt[0].p.x - 1, pt[0].p.y, pt[0].p.z, pt[0].p.w, dwColor, 0,
                  0);
        pt[2].set(pt[0].p.x + 1, pt[0].p.y, pt[0].p.z, pt[0].p.w, dwColor, 0,
                  0);
        pt[3].set(pt[0].p.x, pt[0].p.y - 1, pt[0].p.z, pt[0].p.w, dwColor, 0,
                  0);
        pt[4].set(pt[0].p.x, pt[0].p.y + 1, pt[0].p.z, pt[0].p.w, dwColor, 0,
                  0);
...
bool C3DCursor::PrepareBrush() {
  ...
  bPickObject = !!Scene->RayPickObject(pinf.inf.range, brush_start, brush_dir,
                                       OBJCLASS_SCENEOBJECT, &pinf,
                                       Scene->GetSnapList(false));
  if (!bPickObject)
