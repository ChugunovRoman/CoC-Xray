...
private:
  struct STextureInfo {
    ...
    float pixel_area;
    void OnHighlightClick(ButtonValue *sender, bool &bDataModified,
                          bool &bSafe);
...
public:
  ...
  void AppendTexture(shared_str name, ESummaryTextureType type, float area,
                     float pixel_area, shared_str obj_name) {
    ...
    if (o_it == info->objects.end()) {
      std::pair<STextureInfo::objinf_map_it, bool> res = info->objects.insert(
          std::make_pair(obj_name, STextureInfo::SObjInfo(area)));
      o_it = res.first;
    }
