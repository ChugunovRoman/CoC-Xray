...
bool CCustomObject::IsRender() {
  ...
  return ::Render->occ_visible(bb) ||
         (Selected() && m_CO_Flags.is_any(flRenderAnyWayIfSelected | flMotion));
}
...
void CCustomObject::Select(int flag) {
  if (m_RT_Flags.is(flRT_Visible) && (!!m_RT_Flags.is(flRT_Selected) != flag)) {
    m_RT_Flags.set(flRT_Selected,
                   (flag == -1) ? (m_RT_Flags.is(flRT_Selected) ? FALSE : TRUE)
                                : flag);
    UI->RedrawScene();
  ...
  if (m_CO_Flags.is(flMotion)) {
    ...
    /*
            VERIFY			(m_Motion);
                    ...
                    F.close_chunk	();
      */
    //        m_MotionParams->t_current = ini.r_float		(sect_name,
    //        "motion_params_t");
  }
...
void CCustomObject::SaveLTX(CInifile &ini, LPCSTR sect_name) {
  ...
  /*
      ...
      if (m_CO_Flags.is(flMotion))
      {
          ...
          ini.w_float		(sect_name, "motion_params_t",
...
bool CCustomObject::RaySelect(int flag, const Fvector &start,
                              const Fvector &dir, bool bRayTest) {
  float dist = UI->ZFar();
...
void CCustomObject::OnSynchronize() { OnFrame(); }
