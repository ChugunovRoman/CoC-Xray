...
void EScene::OnCreate() {
  ...
  m_SummaryInfo = TProperties::CreateForm(
      "Level Summary Info", 0, alNone, 0, 0, 0,
...
void EScene::AppendObject(CCustomObject *object, bool bUndo) {
  ...
  VERIFY3(mt,
          "Can't find Object Tools:", GetTool(object->ClassID)->ClassDesc());
...
void EScene::Clear(BOOL bEditableToolsOnly) {
  ...
  for (; t_it != t_end; t_it++)
    if (t_it->second && t_it->first != OBJCLASS_DUMMY) {
      if (!bEditableToolsOnly ||
          (bEditableToolsOnly && t_it->second->IsEditable())) {
        t_it->second->Clear();
      }
    }
  ...
  m_OwnerName =
      AnsiString().sprintf("\\\\%s\\%s", Core.CompName, Core.UserName).c_str();
...
bool EScene::IsUnsaved() {
  return (m_RTFlags.is(flRT_Unsaved) &&
          (ObjCount() || !Tools->GetEditFileName().IsEmpty()));
}
...
void EScene::OnShowHint(AStringVec &dest) {
  CCustomObject *obj =
      RayPickObject(flt_max, UI->m_CurrentRStart, UI->m_CurrentRDir,
                    LTools->CurrentClassID(), 0, 0);
  if (obj)
    obj->OnShowHint(dest);
}
//------------------------------------------------------------------------------
...
bool EScene::Validate(bool bNeedOkMsg, bool bTestPortal, bool bTestHOM,
                      bool bTestGlow, bool bTestShaderCompatible,
                      bool bFullTest) {
  bool bRes = true;
  SceneToolsMapPairIt t_it = m_SceneTools.begin();
  SceneToolsMapPairIt t_end = m_SceneTools.end();
  for (; t_it != t_end; t_it++) {
    if (t_it->second) {
      if (!t_it->second->Validate(bFullTest)) {
        ELog.Msg(mtError, "ERROR: Validate '%s' failed!",
                 t_it->second->ClassDesc());
        bRes = false;
      }
    }
  }
  ...
  if (bTestHOM) {
    ...
    //.			if (mrNo==ELog.DlgMsg(mtConfirmation,TMsgDlgButtons() << mbYes
    //<< mbNo,"Level doesn't contain HOM.\nContinue anyway?")) . return false;
  }
...
void EScene::HighlightTexture(LPCSTR t_name, bool allow_ratio, u32 t_width,
                              u32 t_height, bool leave_previous) {
  if (!leave_previous)
    Tools->ClearDebugDraw();
  ...
  for (; t_it != t_end; ++t_it)
    if (t_it->second)
      t_it->second->HighlightTexture(t_name, allow_ratio, t_width, t_height,
                                     !leave_previous);
...
xr_token js_token[] = {
    {"1 - Low", 1}, {"4 - Medium", 4}, {"9 - High", 9}, {0, 0}};
...
void EScene::FillProp(LPCSTR pref, PropItemVec &items, ObjClassID cls_id) {
  PHelper().CreateCaption(items, PrepareKey(pref, "Scene\\Name"),
                          LTools->m_LastFileName.c_str());
  PHelper().CreateRText(items, PrepareKey(pref, "Scene\\Name prefix"),
                        &m_LevelOp.m_LevelPrefix);

  PropValue *V;
  PHelper().CreateRText(items,
                        PrepareKey(pref, "Scene\\Build options\\Level path"),
                        &m_LevelOp.m_FNLevelPath);
  PHelper().CreateRText(items,
                        PrepareKey(pref, "Scene\\Build options\\Custom data"),
                        &m_LevelOp.m_BOPText);
  PHelper().CreateRText(items, PrepareKey(pref, "Scene\\Map version"),
                        &m_LevelOp.m_map_version);

  m_LevelOp.m_mapUsage.FillProp("Scene\\Usage", items);

  // common
  ButtonValue *B;
  B = PHelper().CreateButton(items,
                             PrepareKey(pref, "Scene\\Build options\\Quality"),
                             "Draft,High,Custom", 0);
  ...
  V = PHelper().CreateFloat(
      items,
      PrepareKey(pref, "Scene\\Build options\\Optimizing\\Normal smooth angle"),
      &m_LevelOp.m_BuildParams.m_sm_angle, 0.f, 180.f);
  V->Owner()->Enable(enabled);
  V = PHelper().CreateFloat(
      items,
      PrepareKey(pref, "Scene\\Build options\\Optimizing\\Weld distance (m)"),
  ...
  V = PHelper().CreateFloat(
      items,
      PrepareKey(pref, "Scene\\Build options\\Lighting\\Pixel per meter"),
      &m_LevelOp.m_BuildParams.m_lm_pixels_per_meter, 0.f, 20.f);
  V->Owner()->Enable(enabled);
  V = PHelper().CreateU32(
      items,
      PrepareKey(pref, "Scene\\Build options\\Lighting\\Error (LM collapsing)"),
      &m_LevelOp.m_BuildParams.m_lm_rms, 0, 255);
  V->Owner()->Enable(enabled);
  V = PHelper().CreateU32(
      items,
      PrepareKey(pref, "Scene\\Build options\\Lighting\\Error (LM zero)"),
...
void EScene::RegisterSubstObjectName(const xr_string &_from,
                                     const xr_string &_to) {
  xr_string _tmp;
