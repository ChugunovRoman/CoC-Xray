...
bool ESceneCustomOTool::LoadSelection(IReader &F) {
  ...
  SPBItem *pb = UI->ProgressStart(
      count,
      AnsiString().sprintf("Loading %s(stream)...", ClassDesc()).c_str());
...
bool ESceneCustomOTool::LoadLTX(CInifile &ini) {
  ...
  SPBItem *pb = UI->ProgressStart(
      count, AnsiString().sprintf("Loading %s(ltx)...", ClassDesc()).c_str());
...
bool ESceneCustomOTool::LoadStream(IReader &F) {
  ...
  SPBItem *pb = UI->ProgressStart(
      count, AnsiString().sprintf("Loading %s...", ClassDesc()).c_str());
...
bool ESceneCustomOTool::GetStaticCformData(
    ...
    bool b_selected_only) // b_vertex* verts, int& vert_cnt, int&
                          // vert_it,b_face* faces, int& face_cnt, int& face_it,
{
