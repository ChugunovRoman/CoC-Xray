...
void ESceneLightTool::SelectLightsForObject(CCustomObject *obj) {
  for (u32 i = 0; i < frame_light.size(); i++) {
    ...
    //        if (obj->IsDynamic()&&!l->m_Flags.is(CLight::flAffectDynamic))
    //        continue; if
    //        (!obj->IsDynamic()&&!l->m_Flags.is(CLight::flAffectStatic))
    //        continue;
    Fbox bb;
    obj->GetBox(bb);
    Fvector C;
...
void ESceneLightTool::AppendFrameLight(CLight *src) {
  ...
  Fvector dir;
  dir.setHP(src->PRotation.y, src->PRotation.x);
  L.direction.set(dir);
  L.range = src->m_Range;
...
void ESceneLightTool::BeforeRender() {
  if (psDeviceFlags.is(rsLighting)) {
    ...
    for (ObjectIt _F = m_Objects.begin(); _F != m_Objects.end(); _F++) {
      CLight *l = (CLight *)(*_F);
      l_cnt++;
      if (l->Visible() && l->m_UseInD3D &&
          l->m_Flags.is_any(ELight::flAffectDynamic | ELight::flAffectStatic))
    ...
    if (m_Flags.is(flShowSun)) {
      ...
      //	            C			=
      // g_pGamePersistent->Environment().CurrentEnv->sun_color;
      //            }else{
      C.set(1.f, 1.f, 1.f);
      //            }
...
void ESceneLightTool::OnControlAppendClick(ButtonValue *sender,
                                           bool &bDataModified, bool &bSafe) {
...
void ESceneLightTool::OnControlRenameRemoveClick(ButtonValue *V,
                                                 bool &bDataModified,
                                                 bool &bSafe) {
  ...
  case 1:
    RemoveLightControl(item_name.c_str());
    break;
  }
  ExecCommand(COMMAND_UPDATE_PROPERTIES);
  bDataModified = true;
...
void ESceneLightTool::FillProp(LPCSTR pref, PropItemVec &items) {
  ButtonValue *B = 0;
  // hemisphere
  //.	PHelper().CreateRToken32(items,
  // PrepareKey(pref,"Common\\Hemisphere\\Light Control"),	&m_HemiControl,
  //&*lcontrols.begin(), lcontrols.size());

  // sun
  PHelper().CreateFlag32(items, PrepareKey(pref, "Common\\Sun Shadow\\Visible"),
                         &m_Flags, flShowSun);
  PHelper().CreateAngle(items, PrepareKey(pref, "Common\\Sun Shadow\\Altitude"),
                        &m_SunShadowDir.x, -PI_DIV_2, 0);
  PHelper().CreateAngle(items,
                        PrepareKey(pref, "Common\\Sun Shadow\\Longitude"),
                        &m_SunShadowDir.y, 0, PI_MUL_2);
  // light controls
  PHelper().CreateFlag32(items, PrepareKey(pref, "Common\\Controls\\Draw Name"),
                         &m_Flags, flShowControlName);
  PHelper().CreateCaption(items, PrepareKey(pref, "Common\\Controls\\Count"),
                          shared_str().printf("%d", lcontrols.size()));
  //	B=PHelper().CreateButton(items,PHelper().PrepareKey(pref,"Common\\Controls\\Edit"),
  //"Append",	ButtonValue::flFirstOnly); 	B->OnBtnClickEvent	=
  // OnControlAppendClick;
  RTokenVecIt _I = lcontrols.begin();
  RTokenVecIt _E = lcontrols.end();
  for (; _I != _E; _I++) {
    if (_I->equal(LCONTROL_HEMI) || _I->equal(LCONTROL_STATIC) ||
        _I->equal(LCONTROL_SUN)) {
      PHelper().CreateCaption(
          items, PrepareKey(pref, "Common\\Controls\\System", *_I->name), "");
    } else {
      B = PHelper().CreateButton(
          items, PrepareKey(pref, "Common\\Controls\\User", *_I->name),
          "Rename,Remove", ButtonValue::flFirstOnly);
      B->OnBtnClickEvent.bind(this,
                              &ESceneLightTool::OnControlRenameRemoveClick);
    }
  }
...
void ESceneLightTool::AppendLightControl(LPCSTR nm, u32 *idx) {
  ...
  if (FindLightControl(name.c_str()))
    return;
  lcontrols.push_back(
      xr_rtoken(name.c_str(), idx ? *idx : lcontrol_last_idx++));
}
//------------------------------------------------------------------------------
...
bool ESceneLightTool::Validate(bool full_test) {
  ...
  for (ObjectIt it = m_Objects.begin(); it != m_Objects.end(); it++) {
    ...
    if (!L->GetLControlName()) {
      bRes = false;
      ELog.Msg(mtError, "%s: '%s' - Invalid light control.", ClassDesc(),
               L->Name);
    }
  }
  return bRes;
...
//render
        case D3DLIGHT_DIRECTIONAL:
            if (Selected()) DU.DrawDirectionalLight( m_D3D.position,
m_D3D.direction, VIS_RADIUS, DIR_SELRANGE, clr ); else
DU.DrawDirectionalLight( m_D3D.position, m_D3D.direction, VIS_RADIUS, DIR_RANGE,
clr ); break; else if ((3==priority)&&(true==strictB2F)){ if
(D3DLIGHT_DIRECTIONAL==m_D3D.type) m_LensFlare.Render();
    }
// update
        if (D3DLIGHT_DIRECTIONAL==m_D3D.type){
        m_LensFlare.Update(m_D3D.direction, m_D3D.diffuse);
...
:FillSunProp(LPCSTR pref, PropItemVec& items)
{
    ...
    PropValue* prop			= 0;
    PHelper().CreateToken		(items,
PHelper().PrepareKey(pref,"Sun\\Quality"),
&m_SunQuality,			sun_quality);

    PHelper().CreateFlag32	(items,
PHelper().PrepareKey(pref,"Sun\\Source\\Enabled"),		&F.m_Flags,
CEditFlare::flSource); PHelper().CreateFloat		(items,
PHelper().PrepareKey(pref,"Sun\\Source\\Radius"),
&F.m_Source.fRadius,	0.f,10.f);
    prop 					= PHelper().CreateTexture
(items, PHelper().PrepareKey(pref,"Sun\\Source\\Texture"), F.m_Source.texture,
sizeof(F.m_Source.texture)); prop->OnChangeEvent = OnNeedUpdate;

    PHelper().CreateFlag32	(items,
PHelper().PrepareKey(pref,"Sun\\Gradient\\Enabled"),		&F.m_Flags,
CEditFlare::flGradient); PHelper().CreateFloat		(items,
PHelper().PrepareKey(pref,"Sun\\Gradient\\Radius"),
&F.m_Gradient.fRadius,	0.f,100.f); PHelper().CreateFloat		(items,
PHelper().PrepareKey(pref,"Sun\\Gradient\\Opacity"),
&F.m_Gradient.fOpacity,	0.f,1.f); prop					=
PHelper().CreateTexture	(items,
PHelper().PrepareKey(pref,"Sun\\Gradient\\Texture"), F.m_Gradient.texture,
sizeof(F.m_Gradient.texture)); prop->OnChangeEvent		= OnNeedUpdate;

    PHelper().CreateFlag32	(items,
PHelper().PrepareKey(pref,"Sun\\Flares\\Enabled"),		&F.m_Flags,
CEditFlare::flFlare); for (CEditFlare::FlareIt it=F.m_Flares.begin();
it!=F.m_Flares.end(); it++){ AnsiString nm; nm.sprintf("%s\\Sun\\Flares\\Flare
%d",pref,it-F.m_Flares.begin()); PHelper().CreateFloat	(items,
PHelper().PrepareKey(nm.c_str(),"Radius"), 	&it->fRadius,  	0.f,10.f);
        PHelper().CreateFloat	(items,
PHelper().PrepareKey(nm.c_str(),"Opacity"),	&it->fOpacity,	0.f,1.f);
        PHelper().CreateFloat	(items,
PHelper().PrepareKey(nm.c_str(),"Position"),	&it->fPosition,	-10.f,10.f);
        prop				= PHelper().CreateTexture (items,
PHelper().PrepareKey(nm.c_str(),"Texture"),	it->texture,
...
CEditFlare::CEditFlare()
{
    ...
    FlareIt it=m_Flares.begin();
        it->fRadius=0.08f; it->fOpacity=0.18f; it->fPosition=1.3f;
strcpy(it->texture,"fx\\fx_flare1"); it++; it->fRadius=0.12f;
it->fOpacity=0.12f; it->fPosition=1.0f; strcpy(it->texture,"fx\\fx_flare2");
it++; it->fRadius=0.04f; it->fOpacity=0.30f; it->fPosition=0.5f;
strcpy(it->texture,"fx\\fx_flare2"); it++; it->fRadius=0.08f;
it->fOpacity=0.24f; it->fPosition=-0.3f; strcpy(it->texture,"fx\\fx_flare2");
it++; it->fRadius=0.12f; it->fOpacity=0.12f; it->fPosition=-0.6f;
strcpy(it->texture,"fx\\fx_flare3"); it++; it->fRadius=0.30f;
it->fOpacity=0.12f; it->fPosition=-1.0f; strcpy(it->texture,"fx\\fx_flare1");
it++;
        // source
...
void CEditFlare::Load(IReader& F){
    ...
    if (F.find_chunk(FLARE_CHUNK_FLARES2)){
            ...
            u32 deFCnt	= F.r_u32(); VERIFY(deFCnt==6);
                F.r
(m_Flares.begin(),m_Flares.size()*sizeof(SFlare)); for (FlareIt
it=m_Flares.begin(); it!=m_Flares.end(); it++) it->hShader._clear();
        CreateShaders();
    }
}
//----------------------------------------------------
...
void CEditFlare::Save(IWriter& F)
{
    ...
    F.w (m_Flares.begin(),m_Flares.size()*sizeof(SFlare)); F.close_chunk
();
}
//----------------------------------------------------
...
//----------------------------------------------------
        ...
        case OBJCLASS_LIGHT:{
            ...
            if (l->m_D3D.type==D3DLIGHT_DIRECTIONAL){
                    if (suns.Length()) suns += ", ";
                suns += l->Name;
                    pIni->w_fcolor	(l->Name, "sun_color",
l->m_D3D.diffuse); pIni->w_fvector3(l->Name, "sun_dir",
l->m_D3D.direction); pIni->w_string	(l->Name, "gradient",
l->m_LensFlare.m_Flags.is(CLensFlare::flGradient)?"on":"off"); pIni->w_string
(l->Name, "source",
l->m_LensFlare.m_Flags.is(CLensFlare::flSource)?"on":"off"); pIni->w_string
(l->Name, "flares",
l->m_LensFlare.m_Flags.is(CLensFlare::flFlare)?"on":"off"); pIni->w_float
(l->Name, "gradient_opacity", 	l->m_LensFlare.m_Gradient.fOpacity);
                pIni->w_string	(l->Name, "gradient_texture",
l->m_LensFlare.m_Gradient.texture); pIni->w_float	(l->Name,
"gradient_radius", 	l->m_LensFlare.m_Gradient.fRadius); pIni->w_string
(l->Name, "source_texture", 	l->m_LensFlare.m_Source.texture); pIni->w_float
(l->Name, "source_radius", 	l->m_LensFlare.m_Source.fRadius); AnsiString
FT=""; AnsiString FR=""; AnsiString FO=""; AnsiString FP=""; AnsiString T="";
                int i=l->m_LensFlare.m_Flares.size();
                for (CEditFlare::FlareIt it = l->m_LensFlare.m_Flares.begin();
it!=l->m_LensFlare.m_Flares.end(); it++,i--){ FT += it->texture;
