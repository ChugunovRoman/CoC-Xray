...
void EScene::ShowObjects(bool flag, ObjClassID classfilter,
                         bool bAllowSelectionFlag, bool bSelFlag) {
  if (classfilter == OBJCLASS_DUMMY) {
    SceneToolsMapPairIt _I = m_SceneTools.begin();
    SceneToolsMapPairIt _E = m_SceneTools.end();
    for (; _I != _E; _I++)
...
void EScene::ZoomExtents(ObjClassID cls, BOOL bSel) {
  ...
  if (BB.is_valid())
    ...
    ELog.Msg(mtError, "Can't calculate bounding box. Nothing selected or some "
                      "object unsupported this function.");
}
//------------------------------------------------------------------------------
