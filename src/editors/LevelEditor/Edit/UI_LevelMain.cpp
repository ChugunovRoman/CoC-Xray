...
CCommandVar CLevelTool::CommandReadonlyTarget(CCommandVar p1, CCommandVar p2) {
  ...
  if (p2) {
    if (!Scene->IfModified()) {
      ...
      //.            xr_string pn	=
      // Scene->LevelPartName(LTools->m_LastFileName.c_str(),M->ClassID);
    }
  } else {
    //.        xr_string pn		=
    // Scene->LevelPartName(LTools->m_LastFileName.c_str(), M->ClassID);
...
CCommandVar CLevelTool::CommandMultiRenameObjects(CCommandVar p1,
                                                  CCommandVar p2) {
  if (!Scene->locked()) {
    ...
    if (p2 == 1) {
      ...
      if (p1.IsInteger())
        return ExecCommand(COMMAND_SAVE,
                           xr_string(LTools->m_LastFileName.c_str()), 0);
      ...
      if (temp_fn.empty()) {
        ...
        if (LUI->m_rt_object_props->section_exist(temp_fn.c_str())) {
          CInifile::Sect &S =
              LUI->m_rt_object_props->r_section(temp_fn.c_str());
          S.Data.clear();
        }
...
CCommandVar CommandShowClipEditor(CCommandVar p1, CCommandVar p2) {
  ...
  if (!g_clip_maker->Visible) {
    ...
    CKinematicsAnimated *KA =
        PKinematicsAnimated(sp->m_SpawnData.m_Visual->visual);
    R_ASSERT(KA);
...
void CLevelMain::RegisterCommands() {
  ...
  // tools
  REGISTER_SUB_CMD_CE(COMMAND_CHANGE_TARGET, "Change Target", LTools,
                      CLevelTool::CommandChangeTarget, true);
  ...
  REGISTER_CMD_C(COMMAND_ENABLE_TARGET, LTools,
                 CLevelTool::CommandEnableTarget);
  REGISTER_CMD_C(COMMAND_SHOW_TARGET, LTools, CLevelTool::CommandShowTarget);
  REGISTER_CMD_C(COMMAND_READONLY_TARGET, LTools,
                 CLevelTool::CommandReadonlyTarget);
  REGISTER_CMD_C(COMMAND_MULTI_RENAME_OBJECTS, LTools,
                 CLevelTool::CommandMultiRenameObjects);

  REGISTER_CMD_CE(COMMAND_SHOW_OBJECTLIST, "Scene\\Show Object List", LTools,
                  CLevelTool::CommandShowObjectList, false);
  // common
  ...
  REGISTER_CMD_SE(COMMAND_LOAD_FIRSTRECENT, "File\\Load First Recent",
                  CommandLoadFirstRecent, true);
  ...
  REGISTER_CMD_SE(COMMAND_MAKE_GAME, "Compile\\Make Game", CommandMakeGame,
                  false);
  REGISTER_CMD_SE(COMMAND_MAKE_AIMAP, "Compile\\Make AI Map", CommandMakeAIMap,
                  false);
  REGISTER_CMD_SE(COMMAND_MAKE_DETAILS, "Compile\\Make Details",
                  CommandMakeDetails, false);
  ...
  REGISTER_CMD_SE(COMMAND_INVERT_SELECTION_ALL, "Selection\\Invert",
                  CommandInvertSelectionAll, false);
  REGISTER_CMD_SE(COMMAND_SELECT_ALL, "Selection\\Select All", CommandSelectAll,
                  false);
  REGISTER_CMD_SE(COMMAND_DESELECT_ALL, "Selection\\Unselect All",
                  CommandDeselectAll, false);
  REGISTER_CMD_SE(COMMAND_DELETE_SELECTION, "Edit\\Delete",
                  CommandDeleteSelection, false);
  REGISTER_CMD_SE(COMMAND_HIDE_UNSEL, "Visibility\\Hide Unselected",
                  CommandHideUnsel, false);
  REGISTER_CMD_SE(COMMAND_HIDE_SEL, "Visibility\\Hide Selected", CommandHideSel,
                  false);
  REGISTER_CMD_SE(COMMAND_HIDE_ALL, "Visibility\\Hide All", CommandHideAll,
                  false);
  ...
  //	REGISTER_CMD_S	    (COMMAND_REFRESH_SOUND_ENVS,
  // CommandRefreshSoundEnvs);
  REGISTER_CMD_S(COMMAND_REFRESH_SOUND_ENV_GEOMETRY,
                 CommandRefreshSoundEnvGeometry);
  ...
  REGISTER_CMD_SE(COMMAND_TOGGLE_AIMAP_VISIBILITY, "Visibility\\Toggle AIMap",
                  CommandToggleAiMapVisibility, true);
  REGISTER_CMD_S(COMMAND_SHOW_CLIP_EDITOR, CommandShowClipEditor);
}

char *CLevelMain::GetCaption() {
  return Tools->m_LastFileName.IsEmpty() ? "noname"
                                         : Tools->m_LastFileName.c_str();
}
...
//---------------------------------------------------------------------------
void RetrieveSceneObjPointAndNormal(Fvector &hitpoint, Fvector *hitnormal,
                                    const SRayPickInfo &pinf, int bSnap) {
  if (pinf.e_mesh == 0) {
    hitpoint = pinf.pt;
  ...
  if (Tools->GetSettings(etfVSnap) && bSnap) {
    ...
    float w = 1 - (u + v);
    Fvector verts[3];
    pinf.e_obj->GetFaceWorld(pinf.s_obj->_Transform(), pinf.e_mesh, pinf.inf.id,
                             verts);
...
bool EditLibPickObjectGeometry(Fvector &hitpoint, const Fvector &start,
                               const Fvector &direction, int bSnap,
                               Fvector *hitnormal) {
  SRayPickInfo pinf;
...
bool ScenePickObjectGeometry(Fvector &hitpoint, const Fvector &start,
                             const Fvector &direction, int bSnap,
                             Fvector *hitnormal) {
  ...
  {
    SRayPickInfo l_pinf;
    bool l_bres = Scene->RayPickObject(l_pinf.inf.range, start, direction,
                                       OBJCLASS_SPAWNPOINT, &l_pinf,
                                       Scene->GetSnapList(false));
...
bool PickObjectGeometry(EEditorState est, Fvector &hitpoint,
                        const Fvector &start, const Fvector &direction,
                        int bSnap, Fvector *hitnormal) {

  switch (est) {
  case esEditLibrary:
    return EditLibPickObjectGeometry(hitpoint, start, direction, bSnap,
                                     hitnormal);
  case esEditScene:
    return ScenePickObjectGeometry(hitpoint, start, direction, bSnap,
                                   hitnormal);
  default:
    NODEFAULT;
  }
...
bool PickGrid(Fvector &hitpoint, const Fvector &start, const Fvector &direction,
              int bSnap, Fvector *hitnormal) {
...
bool CLevelMain::PickGround(Fvector &hitpoint, const Fvector &start,
                            const Fvector &direction, int bSnap,
                            Fvector *hitnormal) {
  VERIFY(m_bReady);
  ...
  if ((bSnap == -1) || (Tools->GetSettings(etfOSnap) && (bSnap == 1))) {
    bool b =
        PickObjectGeometry(est, hitpoint, start, direction, bSnap, hitnormal);
    if (b)
      return true;
  }
...
bool CLevelMain::SelectionFrustum(CFrustum &frustum) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (EPrefs->bp_lim_depth) {
      ...
      if (Scene->RayPickObject(pinf.inf.range, st, d, OBJCLASS_SCENEOBJECT,
                               &pinf, 0))
...
void CLevelMain::OutCameraPos() {
  if (m_bReady) {
    ...
    //	s.sprintf(" Cam: %3.1f°, %3.1f°,
    //%3.1f°",rad2deg(hpb.y),rad2deg(hpb.x),rad2deg(hpb.z));
...
void CLevelMain::store_rt_flags(const CCustomObject *CO) {
  if (LTools->m_LastFileName.Length() && CO->Name && xr_strlen(CO->Name)) {
    ...
    if (CO->Selected() || !CO->Visible())
      m_rt_object_props->w_u32(
          LTools->m_LastFileName.c_str(), CO->Name,
          CO->m_RT_Flags.get() &
              (CCustomObject::flRT_Selected | CCustomObject::flRT_Visible));
  }
}
...
void CLevelMain::restore_rt_flags(CCustomObject *CO) {
  if (CO->Name && LTools->m_LastFileName.Length() &&
    ...
    CO->m_RT_Flags.set(CCustomObject::flRT_Visible |
                           CCustomObject::flRT_Selected |
                           CCustomObject::flRT_SelectedLast,
                       FALSE);
    CO->m_RT_Flags.or (fl);
  }
}
