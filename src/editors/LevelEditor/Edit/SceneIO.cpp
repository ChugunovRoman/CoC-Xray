...
void st_LevelOptions::ReadLTX(CInifile &ini) {
  ...
  if (vers_op > 0x0000000A) {
    ...
    m_mapUsage.m_GameType.set(eGameIDDeathmatch,
                              ini.r_s32(section, "usage_deathmatch"));
    m_mapUsage.m_GameType.set(eGameIDTeamDeathmatch,
                              ini.r_s32(section, "usage_teamdeathmatch"));
    m_mapUsage.m_GameType.set(eGameIDArtefactHunt,
                              ini.r_s32(section, "usage_artefacthunt"));

    if (vers_op > 0x00000008) {
      m_mapUsage.m_GameType.set(eGameIDCaptureTheArtefact,
                                ini.r_s32(section, "usage_captretheartefact"));

      m_mapUsage.m_GameType.set(
          eGameIDTeamDominationZone,
          ini.r_s32(section, "usage_team_domination_zone"));
      if (vers_op == 0x00000009)
        m_mapUsage.m_GameType.set(eGameIDDominationZone,
                                  ini.r_s32(section, "domination_zone"));
      else
        m_mapUsage.m_GameType.set(eGameIDDominationZone,
                                  ini.r_s32(section, "usage_domination_zone"));
    }
  }
}
...
void EScene::SaveLTX(LPCSTR map_name, bool bForUndo, bool bForceSaveAll) {
  ...
  if (bSaveMain) {
    ...
    for (ObjectIt SO = m_ESO_SnapObjects.begin(); SO != m_ESO_SnapObjects.end();
         ++SO) {
  ...
  for (; _I != _E; ++_I) {
    if ((_I->first != OBJCLASS_DUMMY) && _I->second &&
        _I->second->IsEnabled() && _I->second->IsEditable()) {
      if (bForUndo) {
...
void EScene::Save(LPCSTR map_name, bool bUndo, bool bForceSaveAll) {
  ...
  if (bSaveMain) {
    ...
    for (ObjectIt _F = m_ESO_SnapObjects.begin(); _F != m_ESO_SnapObjects.end();
         ++_F)
      F->w_stringZ((*_F)->Name);
...
void EScene::SaveObjectsLTX(ObjectList &lst, LPCSTR sect_name_parent,
                            LPCSTR sect_name_prefix, CInifile &ini) {
  u32 i = 0;
  string256 buff;
...
bool EScene::ReadObjectsLTX(CInifile &ini, LPCSTR sect_name_parent,
                            LPCSTR sect_name_prefix, TAppendObject on_append,
                            SPBItem *pb) {
  string128 buff;
  R_ASSERT(on_append);
  sprintf(buff, "%s_count", sect_name_prefix);
  ...
  for (u32 i = 0; i < count; ++i) {
    ...
    if (ReadObjectLTX(ini, buff, obj)) {
      ...
      if (existing) {
        ...
        if (g_frmConflictLoadObject->m_result != 2 &&
            g_frmConflictLoadObject->m_result != 4 &&
...
bool EScene::LoadLTX(LPCSTR map_name, bool bUndo) {
  ...
  if (FS.exist(full_name.c_str())) {
    ...
    for (; _I != _E; ++_I) {
      if (_I->second) {
        {
          if (!bUndo && _I->second->IsEnabled() &&
              (_I->first != OBJCLASS_DUMMY)) {
    ...
    if (ini.section_exist("snap_objects")) {
      ...
      for (; Si != Se; ++Si) {
        CCustomObject *O =
            FindObjectByName(Si->first.c_str(), OBJCLASS_SCENEOBJECT);
        if (!O)
          ELog.Msg(mtError, "EScene: Can't find snap object '%s'.",
                   Si->second.c_str());
        else
    ...
    for (; _I != _E; ++_I) {
      if (_I->second) {
        ...
        if (chunk) {
          ...
          if (!bUndo && _I->second->IsEnabled() &&
              (_I->first != OBJCLASS_DUMMY)) {
            LoadLevelPart(_I->second,
                          LevelPartName(map_name, _I->first).c_str());
          }
        }
      }
    }
    ...
    if (F->find_chunk(CHUNK_SNAPOBJECTS)) {
      ...
      if (cnt) {
        for (int i = 0; i < cnt; ++i) {
          F->r_stringZ(buf);
          CCustomObject *O =
              FindObjectByName(buf.c_str(), OBJCLASS_SCENEOBJECT);
          if (!O)
            ELog.Msg(mtError, "EScene: Can't find snap object '%s'.",
                     buf.c_str());
...
bool EScene::LoadSelection(LPCSTR fname) {
  ...
  if (FS.exist(full_name.c_str())) {
    ...
    // Objects
    if (!ReadObjectsStream(*F, CHUNK_OBJECT_LIST, OnLoadSelectionAppendObject,
                           0)) {
...
void EScene::CopySelection(ObjClassID classfilter) {
  HGLOBAL hmem =
      GlobalAlloc(GMEM_MOVEABLE | GMEM_DDESHARE, sizeof(SceneClipData));
  SceneClipData *sceneclipdata = (SceneClipData *)GlobalLock(hmem);

  sceneclipdata->m_ClassFilter = classfilter;
  GetTempFileName(FS.get_path(_temp_)->m_Path, "clip", 0,
                  sceneclipdata->m_FileName);
  ...
  */
  ...
  if (F->find_chunk(10)) { // lc error (TJ)
    ...
    F->r(Tools->m_DebugDraw.m_Points.begin(),
         sizeof(CLevelTool::SDebugDraw::Point) *
             Tools->m_DebugDraw.m_Points.size());
  ...
  */
  if (F->find_chunk(12)) { // lc error (invalid faces)
    ...
    F->r(Tools->m_DebugDraw.m_WireFaces.begin(),
         sizeof(CLevelTool::SDebugDraw::Face) *
             Tools->m_DebugDraw.m_WireFaces.size());
...
void EScene::SaveCompilerError(LPCSTR fn) {
  ...
  */
  ...
  err.w(Tools->m_DebugDraw.m_Points.begin(),
        Tools->m_DebugDraw.m_Points.size() *
            sizeof(CLevelTool::SDebugDraw::Point));
  err.close_chunk();
  ...
  err.w(Tools->m_DebugDraw.m_Lines.begin(),
        Tools->m_DebugDraw.m_Lines.size() *
            sizeof(CLevelTool::SDebugDraw::Line));
  err.close_chunk();

  // invalid
  err.open_chunk(12);
  err.w_u32(Tools->m_DebugDraw.m_WireFaces.size());
  err.w(Tools->m_DebugDraw.m_WireFaces.begin(),
        Tools->m_DebugDraw.m_WireFaces.size() *
            sizeof(CLevelTool::SDebugDraw::Face));
  err.close_chunk();
