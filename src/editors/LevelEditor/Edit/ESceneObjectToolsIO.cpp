...
bool ESceneObjectTool::LoadLTX(CInifile &ini) {
  ...
  m_AppendRandomMinScale =
      ini.r_fvector3("AppendRandom", "AppendRandomMinScale");
  m_AppendRandomMaxScale =
      ini.r_fvector3("AppendRandom", "AppendRandomMaxScale");
  m_AppendRandomMinRotation =
      ini.r_fvector3("AppendRandom", "AppendRandomMinRotation");
  m_AppendRandomMaxRotation =
      ini.r_fvector3("AppendRandom", "AppendRandomMaxRotation");
...
void ESceneObjectTool::SaveLTX(CInifile &ini, int id) {
  ...
  ini.w_fvector3("AppendRandom", "AppendRandomMinScale",
                 m_AppendRandomMinScale);
  ini.w_fvector3("AppendRandom", "AppendRandomMaxScale",
                 m_AppendRandomMaxScale);
  ini.w_fvector3("AppendRandom", "AppendRandomMinRotation",
                 m_AppendRandomMinRotation);
  ini.w_fvector3("AppendRandom", "AppendRandomMaxRotation",
                 m_AppendRandomMaxRotation);
  ini.w_u32("AppendRandom", "AppendRandomObjects_size",
            m_AppendRandomObjects.size());

  if (m_AppendRandomObjects.size()) {
    u32 i = 0;
    for (RStringVecIt it = m_AppendRandomObjects.begin();
         it != m_AppendRandomObjects.end(); ++it, ++i) {
      string128 buff;
      sprintf(buff, "object_name_%d", i);
...
void ESceneObjectTool::SaveStream(IWriter &F) {
  ...
  F.w_chunk(CHUNK_VERSION, (u16 *)&OBJECT_TOOLS_VERSION,
            sizeof(OBJECT_TOOLS_VERSION));
  ...
  if (m_AppendRandomObjects.size()) {
    for (RStringVecIt it = m_AppendRandomObjects.begin();
         it != m_AppendRandomObjects.end(); ++it)
      F.w_stringZ(*it);
  }
  F.close_chunk();
}
...
void ESceneObjectTool::GetStaticDesc(int &v_cnt, int &f_cnt,
