...
bool CSectorItem::IsItem(const char *O, const char *M) {
  return (0 == stricmp(O, object->Name)) &&
         (0 == stricmp(M, mesh->Name().c_str()));
}
//------------------------------------------------------------------------------
...
void CSector::Render(int priority, bool strictB2F) {
  ESceneSectorTool *lt = dynamic_cast<ESceneSectorTool *>(ParentTool);
  VERIFY(lt);
  if (2 == priority) {
    if (true == strictB2F) {
      if (!lt->m_Flags.is(ESceneSectorTool::flDrawSolid)) {
        ...
        for (SItemIt it = sector_items.begin(); it != sector_items.end();
             ++it) {
      ...
      color.set(sector_color.r * k, sector_color.g * k, sector_color.b * k,
                1.f);
      color2.set(sector_color.r * k2, sector_color.g * k2, sector_color.b * k2,
                 1.f);
      if (lt->m_Flags.is(ESceneSectorTool::flDrawSolid)) {
...
bool CSector::RayPick(float &distance, const Fvector &start,
                      const Fvector &direction, SRayPickInfo *pinf) {
  bool bPick = false;
  for (SItemIt s_it = sector_items.begin(); s_it != sector_items.end(); s_it++)
    if (s_it->mesh->RayPick(distance, start, direction,
                            s_it->object->_ITransform(), pinf))
      bPick = true;
  return bPick;
}
//----------------------------------------------------
...
void CSector::UpdateVolume() {
  ...
  for (SItemIt s_it = sector_items.begin(); s_it != sector_items.end();
       s_it++) {
    s_it->mesh->GetBox(bb);
...
EVisible CSector::Intersect(const Fvector &center, float radius) {
  ...
  bool bInSphere = ((dist + radius) < m_SectorRadius) && (radius > R.x) &&
                   (radius > R.y) && (radius > R.z);
...
void CSector::CaptureInsideVolume() {
  ...
  ObjectList lst;
  if (Scene->SpherePick(m_SectorCenter, m_SectorRadius, OBJCLASS_SCENEOBJECT,
                        lst)) {
...
void CSector::DistributeInsideObjects() {
  /*
          ...
          // fill object list (test bounding sphere intersection)
      ObjectList lst;
          if (Scene->SpherePick(m_SectorCenter, m_SectorRadius,
     OBJCLASS_SCENEOBJECT, lst)){
      // test all object meshes
          Fmatrix matrix;
              CSceneObject *obj=NULL;
          // ignore dynamic objects
              ...
              for(EditMeshIt m_def = M->begin();m_def!=M->end();m_def++){
                  ...
                  if ((fvFully==vis)||(fvPartialInside==vis)){
                                          float dist		=
     m_SectorRadius; Fvector start,dir; bb.getcenter	(start); _f o r ()
...
void CSector::LoadSectorDef(IReader *F) {
  ...
  sitem.object =
      (CSceneObject *)Scene->FindObjectByName(o_name, OBJCLASS_SCENEOBJECT);
...
//----------------------------------------------------
xr_token level_sub_map[] = {{"default", u8(-1)}, {"#0", 0}, {"#1", 1},
                            {"#2", 2},           {"#3", 3}, {NULL, 4}};
...
void CSector::FillProp(LPCSTR pref, PropItemVec &items) {
  ...
  PHelper().CreateCaption(items, PrepareKey(pref, Name, "Contents\\Objects"),
                          AnsiString(objects).c_str());
  PHelper().CreateCaption(items, PrepareKey(pref, Name, "Contents\\Meshes"),
                          AnsiString(meshes).c_str());
  PHelper().CreateCaption(items, PrepareKey(pref, Name, "Contents\\Faces"),
                          AnsiString(faces).c_str());
  PHelper().CreateToken8(items, PrepareKey(pref, Name, "Change LevelMap to"),
                         &m_map_idx, level_sub_map);
}
//----------------------------------------------------
...
bool CSector::Validate(bool bMsg) {
  ...
  if (f_cnt <= 4) {
    if (bMsg)
      ELog.Msg(mtError, "*ERROR: Sector: '%s' - face count < 4!", Name);
    bRes = false;
  }
  // verify shader compatibility
  bool bRenderableFound = false;
  for (SItemIt it = sector_items.begin(); it != sector_items.end(); it++) {
    for (SurfFacesPairIt sf_it = it->mesh->m_SurfFaces.begin();
         sf_it != it->mesh->m_SurfFaces.end(); sf_it++) {
  ...
  if (!bRenderableFound) {
    if (bMsg)
      ELog.Msg(mtError,
               "*ERROR: Sector: '%s' - can't find any renderable face!", Name);
    bRes = false;
  }
  return bRes;
}
