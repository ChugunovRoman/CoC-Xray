...
void ESceneFogVolumeTool::SaveStream(IWriter &F) {
  ...
  F.w_chunk(TOOL_CHUNK_VERSION, (u16 *)&FOG_VOL_TOOLS_VERSION,
            sizeof(FOG_VOL_TOOLS_VERSION));
}
...
void EFogVolume::FillProp(LPCSTR pref, PropItemVec &values) {
  ...
  PropValue *P;
  P = PHelper().CreateToken8(values, PrepareKey(pref, "VolumeType"),
                             &m_volumeType, fog_vol_type);
  P->OnChangeEvent.bind(this, &EFogVolume::OnChangeEnvs);

  if (m_volumeType == fvEmitter)
    P = PHelper().CreateRText(values, PrepareKey(pref, "profile (ltx)"),
                              &m_volume_profile);
}
//----------------------------------------------------
