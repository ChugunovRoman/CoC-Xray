  ...
  bool flush(LPCSTR fn) {
    ...
    for (ix = 0; ix < bb_sx; ix++) {
      for (iz = 0; iz < bb_sz; iz++) {
        ...
        u8 v_s = (u8)iFloor(float(svertex(ix, iz)) / float(max_svert) * 255.f +
                            0.5f);
        u8 v_mu = (u8)iFloor(
            float(muvertex(ix, iz)) / float(max_muvert) * 255.f + 0.5f);
    ...
    if (F) {
      F->w_string(
          AnsiString()
              .sprintf("Map size X x Z:            [%d x %d]", bb_sx, bb_sz)
              .c_str());
      F->w_string(AnsiString()
                      .sprintf("Max static vertex per m^2: %d", max_svert)
                      .c_str());
      F->w_string(AnsiString()
                      .sprintf("Total static vertices:     %d", total_svert)
                      .c_str());
      F->w_string(AnsiString()
                      .sprintf("Max mu vertex per m^2:     %d", max_muvert)
                      .c_str());
      F->w_string(AnsiString()
                      .sprintf("Total mu vertices:         %d", total_muvert)
                      .c_str());
      FS.w_close(F);
      return true;
    }
...
void SceneBuilder::SaveBuildAsObject() {
  ...
  for (idx = 0; idx < l_face_it; ++idx) {
    ...
    sprintf(tmp, "f %d/%d %d/%d %d/%d", it.v[0] + 1, idx * 3 + 1, it.v[1] + 1,
            idx * 3 + 2, it.v[2] + 1, idx * 3 + 3);
    tmpFaces.w_string(tmp);
  }
  ...
  for (idx = 0; idx < l_mu_models.size(); ++idx) {
    ...
    for (fi = 0; fi < m.m_iFaceCount; ++fi) {
      ...
      sprintf(tmp, "f %d/%d %d/%d %d/%d", it.v[0] + 1 + total_vertices,
              fi * 3 + 1 + total_tcs, it.v[1] + 1 + total_vertices,
              fi * 3 + 2 + total_tcs, it.v[2] + 1 + total_vertices,
              fi * 3 + 3 + total_tcs);
      /*
                              sprintf				(tmp,"f %d %d
         %d", it.v[0]+1+total_vertices, it.v[1]+1+total_vertices,
  ...
  // uv
  //                sprintf			(tmp,"vt %f
  //                %f",v_it->UV.x,_abs(1.f-v_it->UV.y));
  //                F.w_string (tmp);

  // normals
  //                sprintf			(tmp,"vn %f %f
  //                %f",mV.x,mV.y,mV.z); F.w_string	(tmp);
  ...
  /*
      string512 	tmp,tex_path,tex_name;
      // write mtl
      for (SplitIt split_it=m_Splits.begin(); split_it!=m_Splits.end();
  split_it++)
          {
              _splitpath			((*split_it)->m_Surf->_Texture(), 0, 0,
  tex_name,
  0 ); sprintf				(tmp,"newmtl %s",tex_name); F.w_string
  (tmp);

              _splitpath			((*split_it)->m_Surf->_Texture(), 0,
  tex_path, tex_name, 0 ); strconcat
  (sizeof(tex_path),tex_path,tex_path,"\\",tex_name,".tga"); sprintf
  (tmp,"map_Kd %s",tex_path); F.w_string	(tmp);
      }
      ...
      for (split_it=m_Splits.begin(); split_it!=m_Splits.end(); split_it++){
              _splitpath			((*split_it)->m_Surf->_Texture(), 0, 0,
  tex_name, 0 ); sprintf				(tmp,"g
  %d",split_it-m_Splits.begin()); F.w_string	(tmp);
          sprintf				(tmp,"usemtl %s",tex_name);
  F.w_string	(tmp); Fvector 			mV; Fmatrix mZ; mZ.mirrorZ
  (); for (COGFCPIt it=(*split_it)->m_Parts.begin();
  it!=(*split_it)->m_Parts.end(); it++){ CObjectOGFCollectorPacked* part = *it;
              // vertices
  ...
  %f",mV.x,mV.y,mV.z); F.w_string	(tmp);
              ...
              for (v_it=VERTS.begin(); v_it!=VERTS.end(); v_it++){
                  sprintf			(tmp,"vt %f
  %f",v_it->UV.x,_abs(1.f-v_it->UV.y));		F.w_string	(tmp);
              }
              for (v_it=VERTS.begin(); v_it!=VERTS.end(); v_it++){
...
void SceneBuilder::SaveBuild() {
  ...
  if (F) {
    ...
    for (xr_vector<sb_light_control>::iterator lc_it = l_light_control.begin();
         lc_it != l_light_control.end(); lc_it++) {
    ...
    F->w(l_light_static.begin(),
         sizeof(b_light_static) * l_light_static.size());
    F->close_chunk();

    F->open_chunk(EB_Light_dynamic);
    F->w(l_light_dynamic.begin(),
         sizeof(b_light_dynamic) * l_light_dynamic.size());
    F->close_chunk();
...
BOOL GetStaticCformData(const Fmatrix &parent, CEditableMesh *mesh,
                        CEditableObject *object, Fvector *verts, int &vert_cnt,
                        int &vert_it, CDB::TRI *faces, int &face_cnt,
                        int &face_it) {
  ...
  for (SurfFacesPairIt sp_it = mesh->Surfaces().begin();
       sp_it != mesh->Surfaces().end(); sp_it++) {
    ...
    for (IntIt f_it = face_lst.begin(); f_it != face_lst.end(); ++f_it) {
      ...
      float _a = CalcArea(mesh->Vertices()[face.pv[0].pindex],
...
BOOL SceneBuilder::BuildMesh(const Fmatrix &parent, CEditableObject *object,
                             CEditableMesh *mesh, int sect_num, b_vertex *verts,
                             int &vert_cnt, int &vert_it, b_face *faces,
                             int &face_cnt, int &face_it, u32 *smgroups,
  ...
  // fill faces
  for (SurfFacesPairIt sp_it = mesh->m_SurfFaces.begin();
       sp_it != mesh->m_SurfFaces.end(); sp_it++) {
    IntVec &face_lst = sp_it->second;
    CSurface *surf = sp_it->first;
    if (surf->m_GameMtlName == "materials\\occ")
      continue;
    ...
    if (gm_id < 0) {
      ELog.DlgMsg(mtError, "Surface: '%s' contains bad game material.",
                  surf->_Name());
      bResult = FALSE;
      break;
    }
    ...
    if (0 == M) {
      ELog.DlgMsg(mtError, "Surface: '%s' contains undefined game material.",
                  surf->_Name());
      bResult = FALSE;
      break;
    }
    if (M->Flags.is(SGameMtl::flBreakable)) {
      ELog.Msg(mtInformation, "Surface: '%s' contains breakable game material.",
               surf->_Name());
      continue;
    }
    if (M->Flags.is(SGameMtl::flClimable)) {
      ELog.Msg(mtInformation, "Surface: '%s' contains climable game material.",
               surf->_Name());
      continue;
    }
    if (M->Flags.is(SGameMtl::flDynamic)) {
      ELog.DlgMsg(mtError, "Surface: '%s' contains non-static game material.",
                  surf->_Name());
      bResult = FALSE;
      break;
    }
    u32 dwTexCnt =
        ((surf->_FVF() & D3DFVF_TEXCOUNT_MASK) >> D3DFVF_TEXCOUNT_SHIFT);
    if (dwTexCnt != 1) {
      ELog.DlgMsg(mtError, "Surface: '%s' contains more than 1 texture refs.",
                  surf->_Name());
      bResult = FALSE;
      break;
    }
    ...
    for (IntIt f_it = face_lst.begin(); f_it != face_lst.end(); ++f_it) {
      ...
      float _a = CalcArea(mesh->m_Vertices[face.pv[0].pindex],
                          mesh->m_Vertices[face.pv[1].pindex],
    ...
    if (dwInvalidFaces)
      Msg("!Object '%s' - '%s' has %d invalid face(s). Removed.",
          object->GetName(), mesh->Name().c_str(), dwInvalidFaces);
...
BOOL SceneBuilder::BuildObject(CSceneObject *obj) {
  ...
  for (EditMeshIt M = O->FirstMesh(); M != O->LastMesh(); M++) {
    ...
    if (!BuildMesh(T, O, *M, sect_num, l_verts, l_vert_cnt, l_vert_it, l_faces,
                   l_face_cnt, l_face_it, l_smgroups, obj->_Transform()))
      return FALSE;
    // fill DI vertices
...
// BOOL GetMuStaticCformData( CSceneObject* obj,mesh_build_data &data, bool
// b_selected_only )
//{
//   CEditableObject *O = obj->GetReference();
...
// detect sector
//    CSector* S 			=
//    PortalUtils.FindSector(obj,*O->FirstMesh());
...
BOOL SceneBuilder::BuildMUObject(CSceneObject *obj) {
  ...
  if (-1 == model_idx || m_save_as_object) {
    ...
    for (EditMeshIt MESH = O->FirstMesh(); MESH != O->LastMesh(); ++MESH)
      if (!BuildMesh(T, O, *MESH, sect_num, M.m_pVertices, M.m_iVertexCount,
                     vert_it, M.m_pFaces, M.m_iFaceCount, face_it, M.m_smgroups,
                     obj->_Transform()))
        return FALSE;
...
BOOL SceneBuilder::BuildPointLight(b_light *b, const Flags32 &usage,
  ...
  if (usage.is(ELight::flAffectStatic)) {
    if (soft_points && !soft_points->empty()) {
      ...
      float sample_energy =
          (b->data.diffuse.magnitude_rgb()) / float(soft_points->size());
      color.mul_rgb(sample_energy);
...
BOOL SceneBuilder::BuildLight(CLight *e) {
  ...
  L.controller_ID = BuildLightControl(
      e->GetLControlName()); // BuildLightControl(LCONTROL_STATIC);
  ...
  switch (e->m_Type) {
  case ELight::ltPoint:
    return BuildPointLight(&L, e->m_Flags, lpSectors,
                           e->m_FuzzyData ? &e->m_FuzzyData->m_Positions : 0,
                           &e->_Transform());
  default:
...
BOOL SceneBuilder::BuildGlow(CGlow *e) {
  ...
  b.flags =
      e->m_Flags.is(CGlow::gfFixedSize) ? 0x01 : 0x00; // 0x01 - non scalable
  return TRUE;
}
//------------------------------------------------------------------------------
...
void SceneBuilder::BuildPortal(b_portal *b, CPortal *e) {
  ...
  CopyMemory(b->vertices.begin(), e->m_SimplifyVertices.begin(),
...
int SceneBuilder::BuildMaterial(CSurface *surf, int sector_num,
  ...
  return BuildMaterial(
      ...
      ((surf->_FVF() & D3DFVF_TEXCOUNT_MASK) >> D3DFVF_TEXCOUNT_SHIFT),
      sector_num, allow_draft);
}
int SceneBuilder::BuildMaterial(LPCSTR esh_name, LPCSTR csh_name,
                                LPCSTR tx_name, u32 tx_cnt, int sector_num,
                                bool allow_draft) {
  b_material mtl;
  ZeroMemory(&mtl, sizeof(mtl));
  VERIFY(sector_num >= 0);
  ...
  if ((u16(-1) == mtl.shader) || (u16(-1) == mtl.shader_xrlc) ||
      (u16(-1) == mtl.surfidx))
    return -1;
  ...
  for (ObjectIt _F = lst.begin(); _F != lst.end(); _F++) {
    ...
    case OBJCLASS_SCENEOBJECT: {
      ...
      /*        case OBJCLASS_GROUP:{
                  ...
                  bResult = ParseStaticObjects(grp_lst, group->Name,
         b_selected_only); }break;   */
    } // end switch
    if (!bResult) {
...
BOOL SceneBuilder::CompileStatic(bool b_selected_only) {
  ...
  // make hemisphere
  ESceneLightTool *lt =
      dynamic_cast<ESceneLightTool *>(Scene->GetOTool(OBJCLASS_LIGHT));
  ...
  if (bResult && !l_lods.empty()) {
    ...
    xr_string fn_color =
        ChangeFileExt(MakeLevelPath(LEVEL_LODS_TEX_NAME).c_str(), ".dds")
            .c_str();
    xr_string fn_normal =
        ChangeFileExt(MakeLevelPath(LEVEL_LODS_NRM_NAME).c_str(), ".dds")
            .c_str();
    if (1 == ImageLib.CreateMergedTexture(2, images, merged_image, 512, 2048,
                                          64, 2048, offsets, scales, rotated,
                                          remap)) {
      ...
      tp.flags.assign(STextureParams::flDitherColor |
                      STextureParams::flGenerateMipMaps);
      ImageLib.MakeGameTexture(fn_color.c_str(), merged_image.layers[0].begin(),
                               tp);
      ImageLib.MakeGameTexture(fn_normal.c_str(),
                               merged_image.layers[1].begin(), tp);
      for (k = 0; k < (int)l_lods.size(); k++) {
        ...
        for (u32 f = 0; f < 8; f++) {
          for (u32 t = 0; t < 4; t++) {
            ...
            u32 id = remap[k];
            ImageLib.MergedTextureRemapUV(uv.x, uv.y, uv.x, uv.y, offsets[id],
                                          scales[id], rotated[id]);
          }
        }
        pb->Inc();
      }
    } else {
      ELog.DlgMsg(mtError, "Failed to build merged LOD texture. Merged texture "
                           "more than [2048x2048].");
      bResult = FALSE;
    }
    UI->ProgressEnd(pb);
