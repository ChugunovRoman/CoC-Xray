...
void ESceneGroupTool::GroupObjects(bool bUndo) {
  ...
  if (group->ObjectInGroupCount()) {
    ELog.DlgMsg(mtInformation,
                ...
                group->Name, group->ObjectInGroupCount());
...
void ESceneGroupTool::AlignToObject() {
  ...
  if (!lst.empty()) {
    ...
    for (ObjectIt it = lst.begin(); it != lst.end(); ++it) {
      if ((*it)->Selected()) {
        sel_cnt++;
        if (TfrmChoseItem::SelectItem(smCustom, nm, 1, nm, FillGroupItems,
                                      *it)) {
...
void ESceneGroupTool::ReloadRefsSelectedObject() {
  ...
  if (!lst.empty()) {
    ...
    GetTempFileName(FS.get_path(_temp_)->m_Path, "tmp_sector", 0,
                    temp_file_name_sector);
    Scene->SaveToolLTX(OBJCLASS_SECTOR, temp_file_name_sector);

    GetTempFileName(FS.get_path(_temp_)->m_Path, "tmp_portal", 0,
                    temp_file_name_portal);
