...
EParticlesObject::EParticlesObject(LPVOID data, LPCSTR name)
  ...
  if (::Render->occ_visible(bb)) {
    ...
    if (1 == priority) {
      if (false == strictB2F) {
        ...
        if (!Selected())
          DU_impl.DrawCross(PPosition, 0.30f, 0.1f, 0.3f, 0.3f, 0.3f, 0.3f,
                            0xFFFFEBAA, false);
    ...
    if (m_Particles)
      ::Render->model_Render(dynamic_cast<IRenderVisual *>(m_Particles),
                             _Transform(), priority, strictB2F, 1.f);
  }
}
//----------------------------------------------------
...
bool EParticlesObject::RayPick(float &distance, const Fvector &start,
  ...
  if (d > 0) {
    ...
    if (((d2 * d2 - d * d) < (PSOBJECT_SIZE * PSOBJECT_SIZE)) &&
        (d > PSOBJECT_SIZE)) {
      if (d < distance) {
        distance = d;
        return true;
...
void EParticlesObject::OnControlClick(ButtonValue *sender, bool &bModif,
                                      bool &bSafe) {
...
void EParticlesObject::FillProp(LPCSTR pref, PropItemVec &items) {
  ...
  PropValue *V;
  V = PHelper().CreateChoose(items, PrepareKey(pref, "Reference"), &m_RefName,
                             smParticles);
  V->OnChangeEvent.bind(this, &EParticlesObject::OnRefChange);
  ButtonValue *B;
  B = PHelper().CreateButton(items, PrepareKey(pref, "Controls"), "Play,Stop",
                             0);
...
bool EParticlesObject::GetSummaryInfo(SSceneSummary *inf) {
  ...
  /*
          ...
          inf->pe_static.insert(m_PE.GetDefinition()->m_Name);
                  if
     (m_PE.GetDefinition()->m_TextureName&&m_PE.GetDefinition()->m_TextureName[0])
