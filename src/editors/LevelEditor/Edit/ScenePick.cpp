...
int EScene::FrustumPick(const CFrustum &frustum, ObjClassID classfilter,
                        ObjectList &ol) {
  int count = 0;
...
int EScene::SpherePick(const Fvector &center, float radius,
                       ObjClassID classfilter, ObjectList &ol) {
  int count = 0;
...
int EScene::RayQuery(SPickQuery &PQ, const Fvector &start, const Fvector &dir,
...
int EScene::BoxQuery(SPickQuery &PQ, const Fbox &bb, u32 flags,
...
CCustomObject *EScene::RayPickObject(float nearest_dist, const Fvector &start,
                                     const Fvector &direction,
                                     ObjClassID classfilter, SRayPickInfo *pinf,
  ...
  if (from_list) {
    for (ObjectIt _F = from_list->begin(); _F != from_list->end(); _F++)
      if ((*_F)->Visible() &&
          (*_F)->RayPick(nearest_dist, start, direction, pinf))
        nearest_object = (*_F);
  } else {
    if (classfilter == OBJCLASS_DUMMY) {
      CCustomObject *obj = 0;
...
int EScene::GetQueryObjects(ObjectList &lst, ObjClassID classfilter, int iSel,
                            int iVis, int iLock) {
  if (classfilter == OBJCLASS_DUMMY) {
    SceneToolsMapPairIt _I = m_SceneTools.begin();
    SceneToolsMapPairIt _E = m_SceneTools.end();
...
int EScene::RaySelect(int flag, ObjClassID classfilter) {
  ...
  if (classfilter == OBJCLASS_DUMMY) {
    ...
    for (; _I != _E; _I++) {
      float range = UI->ZFar();
      _I->second->RaySelect(flag, range, UI->m_CurrentRStart, UI->m_CurrentRDir,
                            TRUE);
      if (range < dist) {
        dist = range;
        mt = _I->second;
  ...
  dist = UI->ZFar();
  if (mt)
    count = mt->RaySelect(flag, dist, UI->m_CurrentRStart, UI->m_CurrentRDir,
                          FALSE);
  return count;
  /*
          CCustomObject *nearest_object =
     RayPickObject(flt_max,UI->m_CurrentRStart,UI->m_CurrentRNorm,classfilter,0,0);
...
int EScene::BoxPickObjects(const Fbox &box, SBoxPickInfoVec &pinf,
                           ObjectList *lst) {
  if (lst) {
