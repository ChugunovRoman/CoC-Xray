...
int CPortalUtils::CalculateSelectedPortals() {
  ...
  if (Validate(false)) {
    ...
    ELog.DlgMsg(mtError,
                "*ERROR: Scene has non associated face (face without sector)!");
  }
...
bool CPortalUtils::RemoveDefaultSector() {
  CCustomObject *O =
      Scene->FindObjectByName(DEFAULT_SECTOR_NAME, OBJCLASS_SECTOR);
  if (O) {
...
sectors.push_back((CSector*)(*_F));
        ...
        Scene->UndoSave();
                ...
                ELog.DlgMsg(mtError,"*ERROR: Scene has non associated face (face
without sector)!");
    }
...
bool CPortalUtils::Validate(bool bMsg) {
  ...
  if (Scene->GetBox(box, OBJCLASS_SCENEOBJECT)) {
    ...
    if (f_cnt != 0) {
      if (bMsg) {
        ELog.DlgMsg(mtError, "*ERROR: Scene has '%d' non associated face!",
                    f_cnt);
        for (SItemIt it = sector_def->sector_items.begin();
             it != sector_def->sector_items.end(); it++)
          Msg("! - scene object: '%s' [O:'%s', M:'%s']", it->object->Name,
              it->object->RefName(), it->mesh->Name().c_str());
      }
      bResult = false;
    }
    xr_delete(sector_def);
...
class sCollector {
  ...
  struct sEdge {
    ...
    static bool c_equal(const sEdge &E1, const sEdge &E2) {
      return (E1.s[0] == E2.s[0]) && (E1.s[1] == E2.s[1]) &&
...
public:
  ...
  void export_portals() {
    ...
    int ps = portals.size();
    int curr = 0;
    for (sPortalIt p_it = portals.begin(); p_it != portals.end();
         ++p_it, ++curr) {
      if (p_it->e.size() > 1) {
        ...
        ELog.Msg(mtError,
                 "Can't create Portal from 0 edge :(\nPlease check "
                 "geometry.\n'%s'<->'%s'\n",
                 p_it->s[0]->Name, p_it->s[1]->Name);
      } else {
...
int CPortalUtils::CalculateSelectedPortals(ObjectList &sectors) {
  ...
  for (ObjectIt s_it = sectors.begin(); s_it != sectors.end(); s_it++) {
    ...
    for (SItemIt s_it = S->sector_items.begin(); s_it != S->sector_items.end();
         s_it++) {
