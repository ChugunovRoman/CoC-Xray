  ...
  for (DWORD i_t = 0; i_t != clContactedT.size(); i_t++) {
    ...
    if (T.N.dotproduct(normalizedVelocity) < 0.0f) {
      ...
      if ((sIPoint.dotproduct(T.N) + T.d) < -EPS_S) {
        ...
        bInsideTri =
            ETOOLS::TestRayTri2(sIPoint, T.N, T.p, distToPlaneIntersection);

        // calculate plane intersection point
        pIPoint.mad(sIPoint, T.N, distToPlaneIntersection);
      } else {
        // shoot ray along the velocity vector
        bInsideTri = ETOOLS::TestRayTri2(sIPoint, normalizedVelocity, T.p,
...
void ESceneAIMapTool::MotionSimulate(Fvector &result, Fvector &start,
                                     Fvector &end, float _radius,
                                     float _height) {
