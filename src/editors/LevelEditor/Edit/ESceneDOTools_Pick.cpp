...
int EDetailManager::RaySelect(int flag, float &dist, const Fvector &start,
  ...
  for (u32 z = 0; z < dtH.size_z; z++) {
    ...
    for (u32 x = 0; x < dtH.size_x; x++) {
      ...
      fx = fromSlotX(x);
      bbox.min.set(fx - DETAIL_SLOT_SIZE_2, slot->r_ybase(),
                   fz - DETAIL_SLOT_SIZE_2);
      bbox.max.set(fx + DETAIL_SLOT_SIZE_2, slot->r_ybase() + slot->r_yheight(),
                   fz + DETAIL_SLOT_SIZE_2);
