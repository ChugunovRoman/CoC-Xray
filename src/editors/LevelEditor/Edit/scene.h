...
protected:
  ...
  ObjectList m_ESO_SnapObjects; // временно здесь а вообще нужно перенести в
                                // ESceneTools
protected:
...
protected:
  ...
  void __stdcall OnBuildControlClick(ButtonValue *sender, bool &bModif,
                                     bool &bSafe);
...
public:
  ...
  bool ReadObjectsStream(IReader &F, u32 chunk_id, TAppendObject on_append,
                         SPBItem *pb);
  bool ReadObjectsLTX(CInifile &ini, LPCSTR sect_name_parent,
  ...
  void SaveObjectsLTX(ObjectList &lst, LPCSTR sect_name_parent,
...
public:
  ...
  void HighlightTexture(LPCSTR t_name, bool allow_ratio, u32 t_width,
  ...
  virtual bool RemoveObject(CCustomObject *object, bool bExecUndo,
                            bool bDeleting);
  ...
  virtual CCustomObject *RayPickObject(float dist, const Fvector &start,
                                       const Fvector &dir,
                                       ObjClassID classfilter,
                                       SRayPickInfo *pinf,
                                       ObjectList *from_list);
  int BoxPickObjects(const Fbox &box, SBoxPickInfoVec &pinf,
                     ObjectList *from_list);
  int RayQuery(SPickQuery &RQ, const Fvector &start, const Fvector &dir,
               float dist, u32 flags, ObjectList *snap_list);
  int BoxQuery(SPickQuery &RQ, const Fbox &bb, u32 flags,
               ObjectList *snap_list);
  int RayQuery(SPickQuery &RQ, const Fvector &start, const Fvector &dir,
               float dist, u32 flags, CDB::MODEL *model);
  int BoxQuery(SPickQuery &RQ, const Fbox &bb, u32 flags, CDB::MODEL *model);

  int RaySelect(int flag, ObjClassID classfilter =
                              OBJCLASS_DUMMY); // flag=0,1,-1 (-1 invert)
  ...
  void ShowObjects(bool flag, ObjClassID classfilter = OBJCLASS_DUMMY,
                   bool bAllowSelectionFlag = false, bool bSelFlag = true);
  ...
  int FrustumPick(const CFrustum &frustum, ObjClassID classfilter,
                  ObjectList &ol);
  int SpherePick(const Fvector &center, float radius, ObjClassID classfilter,
                 ObjectList &ol);

  virtual void GenObjectName(ObjClassID cls_id, char *buffer,
                             const char *prefix = NULL);
  virtual CCustomObject *FindObjectByName(LPCSTR name, ObjClassID classfilter);
  virtual CCustomObject *FindObjectByName(LPCSTR name,
                                          CCustomObject *pass_object);
...
public:
  int GetQueryObjects(ObjectList &objset, ObjClassID classfilter, int iSel = 1,
                      int iVis = 1, int iLock = 0);
  template <class Predicate>
  int GetQueryObjects_if(ObjectList &dest, ObjClassID classfilter,
                         Predicate cmp) {
...
public:
  ...
  bool Validate(bool bNeedOkMsg, bool bTestPortal, bool bTestHOM,
                bool bTestGlow, bool bTestShaderCompatible, bool bFullTest);
