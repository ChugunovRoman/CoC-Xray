...
u32 ratio_colors[ratio_count][2] = {
    {0, 0x80FF0000},   {50, 0x80FFFF00},   {150, 0x8000FF00},
    {250, 0x8000FFFF}, {1000, 0x800000FF}, {1000000, 0x800000FF},
};
              ...
              for (IntIt i_it = lst.begin(); i_it != lst.end(); i_it++) {
                ...
                if (allow_ratio) {
                  ...
                  for (idx_clr = 0; idx_clr < ratio_count - 1; ++idx_clr)
                    if (ratio >= ratio_colors[idx_clr][0] &&
                        ratio < ratio_colors[idx_clr + 1][0])
                      break;
                  w = (ratio - ratio_colors[idx_clr][0]) /
                      float(ratio_colors[idx_clr + 1][0]);
                  float inv_w = 1.f - w;
                  clr = color_rgba(
                      color_get_R(ratio_colors[idx_clr + 0][1]) * inv_w +
                      ...
                      color_get_B(ratio_colors[idx_clr + 0][1]) * inv_w +
                          color_get_B(ratio_colors[idx_clr + 1][1]) * w,
                      color_get_A(ratio_colors[idx_clr + 0][1]) * inv_w +
