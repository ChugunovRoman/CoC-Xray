...
bool CLevelTool::OnCreate() {
  ...
  m_Props = TProperties::CreateForm(
      "Object Inspector", 0, alClient,
      TOnModifiedEvent(this, &CLevelTool::OnPropsModified), 0,
      TOnCloseEvent(this, &CLevelTool::OnPropsClose),
      TProperties::plItemFolders | TProperties::plFolderStore |
          TProperties::plNoClearStore | TProperties::plFullExpand);
...
bool __fastcall CLevelTool::MouseStart(TShiftState Shift) {
  ...
  if (pCurTool && pCurTool->pCurControl) {
    if ((pCurTool->pCurControl->Action() != etaSelect) &&
        (!pCurTool->IsEditable() || !pCurTool->AllowMouseStart() ||
         (pCurTool->ClassID == OBJCLASS_DUMMY)))
      return false;
...
void __fastcall CLevelTool::RealSetTarget(ObjClassID tgt, int sub_tgt,
                                          bool bForced) {
...
ObjClassID CLevelTool::CurrentClassID() { return GetTarget(); }
//---------------------------------------------------------------------------

void CLevelTool::OnShowHint(AStringVec &ss) { Scene->OnShowHint(ss); }
//---------------------------------------------------------------------------
...
bool CLevelTool::Pick(TShiftState Shift) {
  if (Scene->locked() && (esEditLibrary == UI->GetEState())) {
    ...
    EDevice.m_Camera.MouseRayFromPoint(UI->m_CurrentRStart, UI->m_CurrentRDir,
                                       UI->m_CurrentCp);
    SRayPickInfo pinf;
...
void CLevelTool::RefreshProperties() { m_Props->RefreshForm(); }
...
void CLevelTool::GetCurrentFog(u32 &fog_color, float &s_fog, float &e_fog) {
  /*
          ...
          s_fog				=
     g_pGamePersistent->Environment().CurrentEnv->fog_near; e_fog
     = g_pGamePersistent->Environment().CurrentEnv->fog_far; Fvector& f_clr
     = g_pGamePersistent->Environment().CurrentEnv->fog_color;
          fog_color 			=
     color_rgba_f(f_clr.x,f_clr.y,f_clr.z,1.f); }else{
  */
  s_fog = psDeviceFlags.is(rsFog) ? (1.0f - fFogness) * 0.85f * UI->ZFar()
                                  : 0.99f * UI->ZFar();
...
void __fastcall CLevelTool::OnFrame() {
  ...
  EEditorState est = UI->GetEState();
  if ((est == esEditScene) || (est == esEditLibrary) ||
      (est == esEditLightAnim)) {
...
bool CLevelTool::IsModified() { return Scene->IsUnsaved(); }
//---------------------------------------------------------------------------

#include "../ECore/Editor/EditMesh.h"
bool CLevelTool::RayPick(const Fvector &start, const Fvector &dir, float &dist,
                         Fvector *pt, Fvector *n) {
  if (Scene->ObjCount() && (UI->GetEState() == esEditScene)) {
    SRayPickInfo pinf;
    pinf.inf.range = dist;
    if (Scene->RayPickObject(dist, start, dir, OBJCLASS_SCENEOBJECT, &pinf,
                             0)) {
      dist = pinf.inf.range;
      if (pt)
        pt->set(pinf.pt);
...
void CLevelTool::UseSimulatePositions() { g_scene_physics.UseSimulatePoses(); }
