...
protected:
  ...
  void MotionSimulate(Fvector &result, Fvector &start, Fvector &end,
                      float _radius, float _height);

  SAINode *BuildNode(Fvector &vFrom, Fvector &vAt, bool bIgnoreConstraints,
                     bool bSuperIgnoreConstraints = false);
  ...
  void UnpackPosition(Fvector &Pdest, const NodePosition &Psrc, Fbox &bb,
                      SAIParams &params);
  u32 UnpackLink(u32 &L);
  void PackPosition(NodePosition &Dest, Fvector &Src, Fbox &bb,
                    SAIParams &params);
...
public:
  ...
  bool PickObjects(Fvector &dest, const Fvector &start, const Fvector &dir,
                   float dist);
...
public:
  ...
  virtual int RaySelect(int flag, float &distance, const Fvector &start,
                        const Fvector &direction, BOOL bDistanceOnly);
  ...
  virtual void ShowObjects(bool flag, bool bAllowSelectionFlag = false,
                           bool bSelFlag = true) {}
  ...
  // other
  int AddNode(const Fvector &pos, bool bIgnoreConstraints, bool bAutoLink,
              int cnt);
