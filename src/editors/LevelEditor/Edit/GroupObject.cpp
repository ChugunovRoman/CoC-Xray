...
CGroupObject::CGroupObject(LPVOID data, LPCSTR name)
...
u32 CGroupObject::GetObjects(ObjectList &lst) {
  ...
  for (ObjectsInGroup::iterator it = m_ObjectsInGroup.begin();
       it != m_ObjectsInGroup.end(); ++it)
...
bool CGroupObject::LL_AppendObject(CCustomObject *object) {
  ...
  if (object->GetOwner()) {
    if (mrNo == ELog.DlgMsg(mtConfirmation, TMsgDlgButtons() << mbYes << mbNo,
                            "Object '%s' already in group '%s'. Change group?",
...
bool CGroupObject::LoadLTX(CInifile &ini, LPCSTR sect_name) {
  ...
  if (version < 0x0011) {
    ELog.DlgMsg(mtError,
                "CGroupObject: unsupported file version. Object can't load.");
    return false;
  }
  ...
  if (/*IsOpened()*/ tmp_flags.test((1 << 0))) { // old opened group save format
    ...
    return false;
    /*
            u32 cnt 	= ini.r_u32			(sect_name,
       "objects_in_group_count"); shared_str 		tmp; string128
       buff;

            for (u32 k=0; k<cnt; ++k)
            {
                            m_ObjectsInGroup.resize
       (m_ObjectsInGroup.size()+1); sprintf
       (buff,"objects_in_group_%d",k);
                    m_ObjectsInGroup.back().ObjectName	=
       ini.r_string(sect_name, buff);
            }
    */
  } else {
  ...
  if (!m_ReferenceName_.size())
    ELog.Msg(mtError,
             "ERROR: group '%s' - has empty reference. Corrupted file?", Name);
...
bool CGroupObject::LoadStream(IReader &F) {
  ...
  if (tmp_flags.test(1 << 0)) { // old format, opened group
    ...
    /*
            ...
            {
                            m_ObjectsInGroup.resize
       (m_ObjectsInGroup.size()+1); F.r_stringZ
       (m_ObjectsInGroup.back().ObjectName);
            }
    */
  } else {
    Scene->ReadObjectsStream(F, GROUPOBJ_CHUNK_OBJECT_LIST, AppendObjectLoadCB,
                             0);
  }
  VERIFY(m_ObjectsInGroup.size());
...
void CGroupObject::FillProp(LPCSTR pref, PropItemVec &items) {
  ...
  PropValue *V = PHelper().CreateChoose(items, PrepareKey(pref, "Reference"),
                                        &m_ReferenceName_, smGroup);
  V->OnChangeEvent.bind(this, &CGroupObject::ReferenceChange);

  ButtonValue *B;
  B = PHelper().CreateButton(items, PrepareKey(pref, "FreezeObjects"),
                             "Unique,Reference", 0);
  ...
  for (ObjectsInGroup::iterator it = m_ObjectsInGroup.begin();
       it != m_ObjectsInGroup.end(); ++it) {
    string_path pk;
    sprintf(pk, "%s (%s)", PrepareKey(pref, it->pObject->Name).c_str(),
            it->pObject->ParentTool->ClassDesc());
    V = PHelper().CreateFlag32(items, pk, &it->pObject->m_CO_Flags,
                               flObjectInGroupUnique);
    V->OnChangeEvent.bind(this, &CCustomObject::OnChangeIngroupUnique);
  }
}

void CGroupObject::OnFreezeAllClick(ButtonValue *sender, bool &bModif,
                                    bool &bSafe) {
...
void CGroupObject::OnObjectRemove(const CCustomObject *object) {
  ObjectsInGroup::iterator it =
      std::find(m_ObjectsInGroup.begin(), m_ObjectsInGroup.end(), object);
  ...
  for (ObjectsInGroup::iterator it = m_ObjectsInGroup.begin();
    ...
    if (it->pObject == NULL) {
      ...
      /*
                      ...
                      CCustomObject* CO 	=
         Scene->FindObjectByName(it->ObjectName.c_str(), (CCustomObject*)0);

                      R_ASSERT2
         (!CO->m_CO_Flags.test(flObjectInGroup), it->ObjectName.c_str());
         it->pObject			= CO;
                      ...
                      if (CO->Name!=buf)
                      {
                              Msg("OG name changed from[%s] to[%s]",CO->Name,
         buf); CO->Name		= buf; it->ObjectName	= buf;
                      }

                      it->pObject->m_CO_Flags.set(flObjectInGroup,
         TRUE); it->pObject->m_CO_Flags.set(flObjectInGroupUnique, 	TRUE);
                      if(it->pObject==NULL)
                          ELog.Msg	(mtError,"Gr%s' has invalid roup
         'eference to object '%s'.", Name, it->ObjectName.c_str());
      */
    }
  }
  if (m_ObjectsInGroup.empty()) {
    ELog.Msg(mtInformation, "Group '%s' has no objects and will be removed.",
             Name);
    DeleteThis();
  }
}
...
bool CGroupObject::CanUngroup(bool bMsg) {
  ...
  for (ObjectsInGroup::iterator it = m_ObjectsInGroup.begin();
    ...
    if (!tool->IsEditable()) {
      if (bMsg)
        Msg("!.Can't detach object '%s'. Target '%s' in readonly mode.",
            it->pObject->Name, tool->ClassDesc());
      res = false;
    }
  }
  return res;
