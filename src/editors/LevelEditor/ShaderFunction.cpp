...
//---------------------------------------------------------------------------
xr_token function_token[] = {{"Constant", WaveForm::fCONSTANT},
                             {"Sin", WaveForm::fSIN},
                             ...
                             {0, 0}};
//---------------------------------------------------------------------------
void FillMenuFromToken(TMxPopupMenu *menu, const xr_token *token_list,
                       ClickEvent func) {
  menu->Items->Clear();
...
//---------------------------------------------------------------------------
AnsiString &GetTokenNameFromVal_EQ(u32 val, AnsiString &res,
                                   const xr_token *token_list) {
  bool bRes = false;
...
void __fastcall TfrmShaderFunction::DrawGraph() {
  ...
  C->MoveTo(2, yy + 2);
  for (int t = 1; t < w; t++) {
    tm = seScale->Value * t * t_cost /
         (fis_zero(m_CurFunc->arg[3]) ? 1.f : m_CurFunc->arg[3]);
...
//---------------------------------------------------------------------------
void __fastcall TfrmShaderFunction::FormKeyDown(TObject *Sender, WORD &Key,
                                                TShiftState Shift) {
  if (Key == VK_ESCAPE)
    ebCancel->Click();
}
...
void TfrmShaderFunction::GetFuncData() {
  ...
  AnsiString buf;
  stFunction->Caption =
      GetTokenNameFromVal_EQ(m_CurFunc->F, buf, function_token);
...
void TfrmShaderFunction::UpdateFuncData() {
  if (bLoadMode)
    return;
  m_CurFunc->F = (WaveForm::EFunction)GetTokenValFromName(
      stFunction->Caption.c_str(), function_token);
  m_CurFunc->arg[0] = seArg1->Value;
  m_CurFunc->arg[1] = seArg2->Value;
  m_CurFunc->arg[2] = seArg3->Value;
  m_CurFunc->arg[3] = seArg4->Value;
  AnsiString t;
  t.sprintf("%.2f", m_CurFunc->arg[1] + m_CurFunc->arg[0]);
  lbMax->Caption = t;
  t.sprintf("%.2f", -m_CurFunc->arg[1] + m_CurFunc->arg[0]);
  lbMin->Caption = t;
  t.sprintf("%.2f", m_CurFunc->arg[0]);
  lbCenter->Caption = t;
  float v = seScale->Value * 1000 / m_CurFunc->arg[3];
  if (v <= 1000)
    t.sprintf("%4.0f ms", v);
  else
    t.sprintf("%.2f s", v / 1000);
  lbEnd->Caption = t;
  DrawGraph();
}
...
void __fastcall TfrmShaderFunction::seArgKeyDown(TObject *Sender, WORD &Key,
                                                 TShiftState Shift) {
  if (Key == VK_RETURN)
    UpdateFuncData();
}
