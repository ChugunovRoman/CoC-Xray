...
void __fastcall TfraObject::MultiSelByRefObject(bool clear_prev) {
  ...
  if (Scene->GetQueryObjects(objlist, OBJCLASS_SCENEOBJECT, 1, 1, -1)) {
    ...
    int max_k = iFloor(
        float(sellist.size()) / 100.f * float(seSelPercent->Value) + 0.5f);
    int k = 0;
...
void TfraObject::SelByRefObject(bool flag) {
  ...
  LPCSTR N = Current();
  //    if (!TfrmChoseItem::SelectItem(TfrmChoseItem::smObject,N,1,sel_name))
  //    return;
  if (N) {
    ObjectIt _F = Scene->FirstObj(OBJCLASS_SCENEOBJECT);
    ObjectIt _E = Scene->LastObj(OBJCLASS_SCENEOBJECT);
...
void __fastcall TfraObject::ebMultiAppendClick(TObject *Sender) {
  ...
  if (TfrmChoseItem::SelectItem(smObject, N, 32, 0)) {
    ...
    for (AStringIt it = lst.begin(); it != lst.end(); it++) {
      ...
      /*            if (ref->IsDynamic()){
                      ELog.DlgMsg(mtError,"TfraObject:: Can't assign dynamic
         object."); xr_delete(obj); return;
                  }
      */
      obj->MoveTo(pos, up);
...
void __fastcall TfraObject::FormCreate(TObject *Sender) {
  ...
  m_Items->SetOnItemsFocusedEvent(
      TOnILItemsFocused(this, &TfraObject::OnItemFocused));
  // fill list
  RefreshList();
}
//---------------------------------------------------------------------------
...
void __fastcall TfraObject::ExtBtn9Click(TObject *Sender) { RefreshList(); }
//---------------------------------------------------------------------------
