...
bool CScenePhyscs ::Simulating() { return !!physics_world(); }
...
BOOL GetStaticCformData(const Fmatrix &parent, CEditableMesh *mesh,
                        ...
                        int &face_it);
BOOL GetStaticCformData(CSceneObject *obj, mesh_build_data &data,
  ...
  for (EditMeshIt M = O->FirstMesh(); M != O->LastMesh(); M++) {
    ...
    if (!::GetStaticCformData(T, *M, O, data.l_verts, data.l_vert_cnt,
                              data.l_vert_it, data.l_faces, data.l_face_cnt,
                              data.l_face_it))
      return FALSE;
  }
...
// b_selected_only );
BOOL GetStaticCformData(ObjectList &lst, mesh_build_data &data,
                        bool b_selected_only) {
...
bool CScenePhyscs::CreateObjectSpace(bool b_selected_only) {
  ...
  if (b_selected_only) {
    if (pCurrentTool)
      pCurrentTool->GetStaticDesc(build_data.l_vert_cnt, build_data.l_face_cnt,
                                  b_selected_only, true);
    ...
    for (; t_it != t_end; ++t_it) {
      ...
      if (mt)
        mt->GetStaticDesc(build_data.l_vert_cnt, build_data.l_face_cnt,
                          b_selected_only, true);
  ...
  VERIFY(!m_object_space);
  m_object_space =
      mesh_create_object_space(build_data.l_verts, build_data.l_faces, H, 0);
