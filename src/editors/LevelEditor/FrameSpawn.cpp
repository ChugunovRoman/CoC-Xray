...
//---------------------------------------------------------------------------
__fastcall TfraSpawn::TfraSpawn(TComponent *Owner) : TForm(Owner) {
...
void __fastcall TfraSpawn::FormShow(TObject *Sender) {
  ...
  for (CInifile::RootIt it = data.begin(); it != data.end(); it++) {
    ...
    if ((*it)->line_exist("$spawn", &val)) {
      ...
      if (caption.size()) {
        ListItem *I = LHelper().CreateItem(items, caption.c_str(), 0,
...
void __fastcall TfraSpawn::FormCreate(TObject *Sender) {
  ...
  m_Items->SetOnItemsFocusedEvent(
      TOnILItemsFocused(this, &TfraSpawn::OnItemFocused));
}
//---------------------------------------------------------------------------
