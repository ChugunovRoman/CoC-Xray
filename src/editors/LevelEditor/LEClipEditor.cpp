...
__fastcall TClipMaker::TClipMaker(TComponent *Owner) : TForm(Owner) {
  ...
  m_ObjectItems =
      TItemList::CreateForm("", paAnimSelect, alClient,
                            TItemList::ilDragCustom | TItemList::ilMultiSelect |
                                TItemList::ilSuppressStatus);
}

void __fastcall TClipMaker::FormCreate(TObject *Sender) {
  m_ClipProps =
      TProperties::CreateForm("Clip Properties", paClipProps, alClient);
  m_ClipList = TItemList::CreateForm("Clips", paClipList, alClient, 0);
  m_ClipList->SetOnItemsFocusedEvent(
      TOnILItemsFocused(this, &TClipMaker::OnClipItemFocused));
...
void __fastcall TClipMaker::FormCloseQuery(TObject *Sender, bool &CanClose) {
  ...
  /*	bool bAsk		= false;
      ...
      if (tgt.size()!=clips.size()) bAsk=true;
          ...
          if (bAsk){
          int res 	= ELog.DlgMsg(mtConfirmation, "Save changes before
     quit?"); switch (res){ case mrYes:{ for (ClipIt it=tgt.begin();
     it!=tgt.end(); it++) xr_delete	(*it); tgt.resize(clips.size()); ClipIt
     t_it=tgt.begin(); for (UIClipIt s_it=clips.begin(); s_it!=clips.end();
     s_it++,t_it++){ *t_it = xr_new<CClip>();
                  **t_it= *(CClip*)*s_it;
              }
              Tools.Modified();
          }break;
...
void __fastcall TClipMaker::FormClose(TObject *Sender, TCloseAction &Action) {
  Stop();
  //.	Clear();
}
//---------------------------------------------------------------------------
...
CAnimationClip *TClipMaker::FindClip(float t) {
  if (clips.empty())
    return 0;
  AnimClipIt it =
      std::upper_bound(clips.begin(), clips.end(), t, clip_pred_float);
...
//.
//---------------------------------------------------------------------------
void __fastcall TClipMaker::ClipDragOver(TObject *Sender, TObject *Source,
                                         int X, int Y, TDragState State,
                                         bool &Accept) {
  // TEMP
  /*
      Accept = false;
          TElTreeDragObject* obj 	=
     dynamic_cast<TElTreeDragObject*>(Source); if (obj)
      {
          ...
          if (A==paClips)
          {
              ...
              if (tv->SelectedCount)
              {
                  for (TElTreeItem* item = tv->GetNextSelected(0); item; item =
     tv->GetNextSelected(item))
                  {
...
void __fastcall TClipMaker::ClipDragDrop(TObject *Sender, TObject *Source,
                                         int X, int Y) {
  ...
  if (obj) {
    ...
    if (tv->SelectedCount) {
      ...
      for (TElTreeItem *item = tv->GetNextSelected(0); item;
           item = tv->GetNextSelected(item)) {
        ListItem *prop = (ListItem *)item->Tag;
        VERIFY(prop);
...
void __fastcall TClipMaker::ClipEndDrag(TObject *Sender, TObject *Target, int X,
                                        int Y) {
  drag_obj = 0xFFFF;
  RepaintClips();
}
...
void __fastcall TClipMaker::ClipMouseDown(TObject *Sender, TMouseButton Button,
...
void __fastcall TClipMaker::ClipMouseMove(TObject *Sender, TShiftState Shift,
                                          int X, int Y) {
  VERIFY(sel_clip);

  TMxPanel *P = dynamic_cast<TMxPanel *>(Sender);
  CAnimationClip *C = FindClip(X);
  VERIFY(C);
  int cX = X - C->PLeftUI();
  ...
  if (g_resizing) {
    ...
    if (!fis_zero(dx)) {
      sel_clip->length += dx;
      if (sel_clip->length < 0.01f)
        sel_clip->length = 0.01f;
      g_X_prev = sel_clip->PRightUI();
      UI->ShowHint(AnsiString().sprintf(
          "Length: %s",
          FloatTimeToStrTime(sel_clip->Length(), false, true, true, true)
              .c_str()));
    }
    UpdateClips();
  }
...
void __fastcall TClipMaker::ClipMouseUp(TObject *Sender, TMouseButton Button,
                                        TShiftState Shift, int X, int Y) {
  if (Button == mbLeft) {
    UI->HideHint();
    g_resizing = FALSE;
...
//.
//---------------------------------------------------------------------------
void __fastcall TClipMaker::BPStartDrag(TObject *Sender,
                                        TDragObject *&DragObject) {
  TMxPanel *P = dynamic_cast<TMxPanel *>(Sender);
  VERIFY(P);
  drag_obj = P->Tag;
...
void __fastcall TClipMaker::BPEndDrag(TObject *Sender, TObject *Target, int X,
                                      int Y) {
  drag_obj = 0xFFFF;
  RepaintClips();
}
...
void __fastcall TClipMaker::BPDragOver(TObject *Sender, TObject *Source, int X,
...
void __fastcall TClipMaker::BPDragDrop(TObject *Sender, TObject *Source, int X,
                                       int Y) {
...
void __fastcall TClipMaker::BPMouseDown(TObject *Sender, TMouseButton Button,
                                        TShiftState Shift, int X, int Y) {
  TMxPanel *P = dynamic_cast<TMxPanel *>(Sender);
  if (P) {
    if (Button == mbRight) {
...
void __fastcall TClipMaker::BPMouseUp(TObject *Sender, TMouseButton Button,
                                      TShiftState Shift, int X, int Y) {
  drag_state = Shift;
}
//---------------------------------------------------------------------------
...
void __fastcall TClipMaker::OnClipItemFocused(ListItemsVec &items) {
  if (!items.empty()) {
    ...
    if (sel_clip)
      sbBase->HorzScrollBar->Position = sbBase->HorzScrollBar->Range *
                                        (sel_clip->StartTime() / m_TotalLength);
  }
}
//------------------------------------------------------------------------------
...
void __fastcall TClipMaker::OnClipLengthChange(PropValue *V) { UpdateClips(); }
//------------------------------------------------------------------------------

void __fastcall TClipMaker::OnZoomChange(PropValue *V) { UpdateClips(); }
//---------------------------------------------------------------------------
...
void TClipMaker::RealUpdateProperties() {
  ...
  PHelper().CreateCaption(
      p_items, "Length",
      FloatTimeToStrTime(m_TotalLength, true, true, true, true).c_str());
  V = PHelper().CreateFloat(p_items, "Zoom", &m_Zoom, 1.f, 1000.f, 0.1f, 1);
  V->OnChangeEvent.bind(this, &TClipMaker::OnZoomChange);
  if (sel_clip) {
    ListItem *l_owner = m_ClipList->FindItem(*sel_clip->name);
    VERIFY(l_owner);
    V = PHelper().CreateName(p_items, "Current Clip\\Name", &sel_clip->name,
                             l_owner);
    V->OnChangeEvent.bind(this, &TClipMaker::OnNameChange);
    V = PHelper().CreateFloat(p_items, "Current Clip\\Length",
                              &sel_clip->length, 0.f, 10000.f, 0.1f, 2);
    ...
    /*
            for (u16 k=0; k<4; k++)
            {
                ...
                CMotionDef* MD		= m_RenderObject->FindMotionDef
       (mname.c_str(),slot); CMotion* MI			=
       m_RenderObject->FindMotionKeys	(mname.c_str(),slot);
                SBonePart* BP		=
       (k<(u16)m_CurrentObject->BoneParts().size())?&m_CurrentObject->BoneParts()[k]:0;

                shared_str tmp;
                if (MI)
                    tmp.sprintf("%s [%3.2fs,
       %s]",mname.c_str(),MI->GetLength()/MD->Speed(),MD->bone_or_part?"stop at
       end":"looped");

                if (BP)
                    PHelper().CreateCaption	(p_items,PrepareKey("Current
       Clip\\Cycles",BP->alias.c_str()), tmp);
                    }

            if (sel_clip->fx.valid())
                    PHelper().CreateFloat
       (p_items,PrepareKey("Current Clip\\FXs",*sel_clip->fx.name),
       &sel_clip->fx_power, 0.f, 1000.f);
    */
  }
...
void TClipMaker::AppendClip() {
  ...
  CAnimationClip *clip = xr_new<CAnimationClip>(*nm, this);
  clip->start_time =
      (sel_clip) ? sel_clip->StartTime() + sel_clip->Length() - EPS_L : 0;
  clips.push_back(clip);
  UpdateClips(true, false);
  SelectClip(clip);
}
...
void TClipMaker::LoadClips() {
  ...
  if (EFS.GetOpenName("$clips$", m_ClipFName)) {
    Clear();
    IReader *F = FS.r_open(m_ClipFName.c_str());
    VERIFY(F);
    m_ClipFName = EFS.ExcludeBasePath(m_ClipFName.c_str(),
                                      FS.get_path("$clips$")->m_Path);
...
void TClipMaker::SaveClips() {
  if (!clips.empty()) {
    if (EFS.GetSaveName("$clips$", m_ClipFName)) {
      IWriter *F = FS.w_open(m_ClipFName.c_str());
      VERIFY(F);
      m_ClipFName = EFS.ExcludeBasePath(m_ClipFName.c_str(),
                                        FS.get_path("$clips$")->m_Path);
      if (F) {
...
void __fastcall TClipMaker::gtClipPaint(TObject *Sender) {
  ...
  for (AnimClipIt it = clips.begin(); it != clips.end(); ++it) {
    ...
    TRect R =
        TRect((*it)->PLeftUI() + 1 - dx, 20 - dy, (*it)->PRightUI() - dx, 20);
...
void __fastcall TClipMaker::ClipPaint(TObject *Sender) {
  ...
  for (AnimClipIt it = clips.begin(); it != clips.end(); ++it) {
    TRect R =
        TRect((*it)->PLeftUI(), 1, (*it)->PRightUI() - 1, paClips->Height);
    canvas->Pen->Width = 1;
    canvas->Brush->Color =
        (*it == sel_clip)
            ? (drag_obj == P->Tag ? CLIP_ACTIVE_DRAG_COLOR : CLIP_ACTIVE_COLOR)
            : CLIP_INACTIVE_COLOR;
    canvas->Rectangle(R);
    R.Top += 1;
    R.Bottom -= 1;
...
void __fastcall TClipMaker::BPOnPaint(TObject *Sender) {
  TMxPanel *bp = dynamic_cast<TMxPanel *>(Sender);
  VERIFY(bp);
  ...
  if (-2 == bp->Tag) {
    ...
    {
            canvas->Brush->Color=
    (*it==sel_clip)?(drag_obj==bp->Tag?BP_ACTIVE_DRAG_COLOR:BP_ACTIVE_COLOR):BP_INACTIVE_COLOR;
        TRect R 			= TRect((*it)->PLeft(), 1,
    (*it)->PRight()-1, 15); AnsiString fx_name	= (*it)->FXName(); if
    (!fx_name.IsEmpty()){ canvas->Rectangle	(R);
            R.Top				+= 1;
            R.Bottom			-= 1;
            R.Left				+= 1;
            R.Right				-= 1;
    ...
    */
    ...
    AnsiString mn_prev = "";
    for (AnimClipIt it = clips.begin(); it != clips.end(); ++it) {
      std::pair<LPCSTR, LPCSTR> name_pair =
          m_RenderObject->LL_MotionDefName_dbg((*it)->animItems[bp->Tag].mid);
      AnsiString mn = name_pair.first;
      TRect R = TRect((*it)->PLeftUI(), 1, (*it)->PRightUI() - 1, 15);
      if (!mn.IsEmpty()) {
        canvas->Brush->Color =
            (*it == sel_clip)
                ? (drag_obj == bp->Tag ? BP_ACTIVE_DRAG_COLOR : BP_ACTIVE_COLOR)
                : BP_INACTIVE_COLOR;
        canvas->Rectangle(R);
        R.Top += 1;
        R.Bottom -= 1;
...
void __fastcall TClipMaker::fsStorageRestorePlacement(TObject *Sender) {
  ...
  if (idx < (int)clips.size())
    SelectClip(clips[idx]);
}
//---------------------------------------------------------------------------
...
void TClipMaker::OnFrame() {
  ...
  if (m_RTFlags.is(flRT_Playing)) {
    ...
    if (m_CurrentPlayTime >
        (clips[play_clip]->StartTime() + clips[play_clip]->Length())) {
      play_clip++;
...
void __fastcall TClipMaker::ebInsertClipClick(TObject *Sender) { InsertClip(); }
//---------------------------------------------------------------------------

void __fastcall TClipMaker::ebAppendClipClick(TObject *Sender) { AppendClip(); }
//---------------------------------------------------------------------------

void __fastcall TClipMaker::ebLoadClipsClick(TObject *Sender) { LoadClips(); }
//---------------------------------------------------------------------------

void __fastcall TClipMaker::ebSaveClipsClick(TObject *Sender) { SaveClips(); }
//---------------------------------------------------------------------------
          ...
          for (UIClipIt c_it=clips.begin(); c_it!=clips.end(); c_it++)
          {
              ...
              for (u16 k=0; k<4; k++)
              {
                  ...
                  u16 slot			= (*c_it)->CycleSlot(k);
                                  CMotion* MI			=
     ATools->m_RenderObject.FindMotionKeys	(mname.c_str(),slot);
                                  CMotionDef* MD		=
     ATools->m_RenderObject.FindMotionDef	(mname.c_str(),slot); if (MI)
                  {
                          float new_len	=
     (MD->StopAtEnd()?MI->GetLength()-SAMPLE_SPF:MI->GetLength())/MD->Speed();
                          if (len<new_len)
                          len = new_len;
                  }
              }
...
void __fastcall TClipMaker::ebPlayClick(TObject *Sender) { Play(FALSE); }
//---------------------------------------------------------------------------

void __fastcall TClipMaker::ebPlayCycleClick(TObject *Sender) { Play(TRUE); }
//---------------------------------------------------------------------------

void __fastcall TClipMaker::ebStopClick(TObject *Sender) { Stop(); }
//---------------------------------------------------------------------------
...
void __fastcall TClipMaker::ebTrashDragOver(TObject *Sender, TObject *Source,
                                            int X, int Y, TDragState State,
                                            bool &Accept) {
  Accept = false;
  TMxPanel *P = dynamic_cast<TMxPanel *>(Source);
...
void __fastcall TClipMaker::ebTrashDragDrop(TObject *Sender, TObject *Source,
                                            int X, int Y) {
  TMxPanel *P = dynamic_cast<TMxPanel *>(Source);
  if (P == paClips) {
    RemoveClip(sel_clip);
...
void __fastcall TClipMaker::ebClearClick(TObject *Sender) {
  if (mrYes == ELog.DlgMsg(mtConfirmation,
