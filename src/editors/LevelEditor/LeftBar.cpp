...
__fastcall TfraLeftBar::TfraLeftBar(TComponent *Owner) : TFrame(Owner) {
  ...
  m_TargetButtons[OBJCLASS_GROUP] =
      std::make_pair(ebTargetGroup, btEnableGroup);
  m_TargetButtons[OBJCLASS_SCENEOBJECT] =
      std::make_pair(ebTargetObject, btEnableObject);
  m_TargetButtons[OBJCLASS_LIGHT] =
      std::make_pair(ebTargetLight, btEnableLight);
  m_TargetButtons[OBJCLASS_SHAPE] =
      std::make_pair(ebTargetShape, btEnableShape);
  m_TargetButtons[OBJCLASS_SOUND_SRC] =
      std::make_pair(ebTargetSoundSrc, btEnableSoundSrc);
  m_TargetButtons[OBJCLASS_SOUND_ENV] =
      std::make_pair(ebTargetSoundEnv, btEnableSoundEnv);
  m_TargetButtons[OBJCLASS_GLOW] = std::make_pair(ebTargetGlow, btEnableGlow);
  m_TargetButtons[OBJCLASS_SPAWNPOINT] =
      std::make_pair(ebTargetSpawnPoint, btEnableSpawnPoint);
  m_TargetButtons[OBJCLASS_WAY] = std::make_pair(ebTargetWay, btEnableWay);
  m_TargetButtons[OBJCLASS_SECTOR] =
      std::make_pair(ebTargetSector, btEnableSector);
  m_TargetButtons[OBJCLASS_PORTAL] =
      std::make_pair(ebTargetPortal, btEnablePortal);
  ...
  m_TargetButtons[OBJCLASS_AIMAP] =
      std::make_pair(ebTargetAIMap, btEnableAIMap);
  m_TargetButtons[OBJCLASS_WM] =
      std::make_pair(ebTargetWallmarks, btEnableWallmarks);
  m_TargetButtons[OBJCLASS_FOG_VOL] =
      std::make_pair(ebTargetFogVolumes, btEnableFogVolumes);
...
void TfraLeftBar::OnTimer() { RedrawBar(); }
...
void __fastcall TfraLeftBar::UpdateEnvironmentGeometry1Click(TObject *Sender) {
...
void __fastcall TfraLeftBar::ebClearSnapClick(TObject *Sender) {
  if (ELog.DlgMsg(mtConfirmation, TMsgDlgButtons() << mbYes << mbNo,
                  "Are you sure to clear snap objects?") == mrYes)
...
void __fastcall TfraLeftBar::ebSnapListModeClick(TObject *Sender) {
  TExtBtn *E = dynamic_cast<TExtBtn *>(Sender);
  VERIFY(E);
...
void TfraLeftBar::RefreshBar() {
  miRecentFiles->Clear();
  u32 idx = 0;
  for (AStringIt it = EPrefs->scene_recent_list.begin();
       it != EPrefs->scene_recent_list.end(); it++) {
  ...
  for (ObjClassID k = OBJCLASS_FIRST_CLASS; k < OBJCLASS_COUNT; ++k) {
    ...
    if (!t) {
      ...
      m_TargetButtons[k].first->NormalColor =
          t->IsEditable() ? clBlack : clGray;
      m_TargetButtons[k].first->Font->Style =
          t->IsForceReadonly() ? TFontStyles() << fsStrikeOut : TFontStyles();
