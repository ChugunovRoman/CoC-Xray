...
void __fastcall TfrmOneColor::FormClose(TObject *Sender, TCloseAction &Action) {
  Action = caFree;
...
void __fastcall TfrmOneColor::tvDOListDragOver(TObject *Sender, TObject *Source,
  ...
  if (Source != m_Parent->tvItems)
    return;
  for (TElTreeItem *node = tvDOList->Items->GetFirstNode(); node;
       node = node->GetNext())
...
void __fastcall TfrmOneColor::tvDOListDragDrop(TObject *Sender, TObject *Source,
                                               int X, int Y) {
  tvDOList->Items->AddObject(0, m_Parent->FDragItem->Text,
                             m_Parent->FDragItem->Data);
  m_Parent->bColorIndModif = true;
}
//---------------------------------------------------------------------------
