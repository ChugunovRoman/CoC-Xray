...
void EngineModel::OnRender() { UpdateObjectXform(Fmatrix()); }
void PreviewModel::RestoreParams(TFormStorage *s) {
  m_Props->RestoreParams(s);
  m_LastObjectName = s->ReadString("preview_name", "");
  int val;
  val = s->ReadInteger("preview_speed", 0);
  m_fSpeed = *((float *)&val);
  val = s->ReadInteger("preview_segment", 0);
  m_fSegment = *((float *)&val);
...
void PreviewModel::OnDestroy() { TProperties::DestroyForm(m_Props); }

void PreviewModel::OnCreate() {
  m_Props = TProperties::CreateForm("Preview prefs", 0, alNone);
}

void PreviewModel::Clear() { Lib.RemoveEditObject(m_pObject); }
void PreviewModel::SelectObject() {
  LPCSTR fn;
  if (!TfrmChoseItem::SelectItem(smObject, fn, 1, m_LastObjectName.c_str()))
    return;
...
xr_token sa_token[] = {{"+Z", PreviewModel::saZp},
                       {"-Z", PreviewModel::saZn},
                       {"+X", PreviewModel::saXp},
                       {"-X", PreviewModel::saXn},
                       {0, 0}};
...
void PreviewModel::SetPreferences() {
  ...
  PHelper().CreateFloat(items, "Speed (m/c)", &m_fSpeed, -10000.f, 10000.f,
                        0.01f, 2);
  PHelper().CreateFloat(items, "Segment (m)", &m_fSegment, -10000.f, 10000.f,
                        0.01f, 2);
...
bool CActorTools::OnCreate() {
  ...
  // props
  m_ObjectItems =
      TItemList::CreateForm("", fraLeftBar->paObjectProps, alClient,
                            TItemList::ilDragCustom | TItemList::ilMultiSelect |
                                TItemList::ilSuppressStatus);
  m_ObjectItems->SetOnItemsFocusedEvent(fastdelegate::bind<TOnILItemsFocused>(
      this, &CActorTools::OnObjectItemFocused));
  m_Props = TProperties::CreateForm(
      "", fraLeftBar->paItemProps, alClient,
      fastdelegate::bind<TOnModifiedEvent>(this, &CActorTools::OnItemModified));
...
void CActorTools::SetPreviewObjectPrefs() { m_PreviewObject.SetPreferences(); }
//---------------------------------------------------------------------------
...
void CActorTools::GetStatTime(float &a, float &b, float &c) {
  if (m_RenderObject.IsRenderable() && fraLeftBar->ebRenderEngineStyle->Down &&
    ...
    if (fraLeftBar->ebRenderEditorStyle->Down && m_pEditObject &&
...
void CActorTools::Render() {
  ...
  if (m_pEditObject) {
    ...
    if (m_RenderObject.IsRenderable() &&
        fraLeftBar->ebRenderEngineStyle->Down) {
      ::Render->model_RenderSingle(m_RenderObject.m_pVisual,
                                   m_RenderObject.ObjectXFORM(),
                                   m_RenderObject.m_fLOD);
      ...
      if (EPrefs->object_flags.is(epoDrawJoints)) {
        ...
        if (K) {
          ...
          for (u16 bidx = 0; bidx < bcnt; ++bidx) {
            ...
            Fmatrix M = Fmatrix().mul(m_RenderObject.ObjectXFORM(),
                                      K->LL_GetTransform(bidx));

            Fvector p1 = M.c;
            u32 c_joint =
                /*(*b_it)->flags.is(CBone::flSelected)?color_bone_sel_color:*/
                color_bone_norm_color;
...
void CActorTools::OnFrame() {
  ...
  if (m_pEditObject) {
    ...
    mRotate.setHPB(m_pEditObject->a_vRotate.y, m_pEditObject->a_vRotate.x,
                   m_pEditObject->a_vRotate.z);
    ...
    if (!m_KeyBar->auto_ch->Checked) {
      m_pEditObject->m_SMParam.t_current =
          float(m_KeyBar->anm_track->Position) / float(1000);
    }
    // CKinematicsAnimated* KA =
    // dynamic_cast<CKinematicsAnimated*>(m_RenderObject.m_pVisual);
    if (m_RenderObject.IsRenderable() && m_pEditObject->IsSkeleton() &&
        m_RenderObject.m_pVisual) {
      ...
      VERIFY(K);
      // K->Bone_Calculate
      // (&K->LL_GetData(K->LL_GetBoneRoot()),&Fidentity);
      if (!m_KeyBar->auto_ch->Checked)
  ...
  if (fraLeftBar->ebRenderEditorStyle->Down && !m_CurrentMotion.IsEmpty() &&
      NULL == m_pEditObject->GetActiveSMotion()) {
      ...
      if (m_pEditObject->GetSelectedBones(lst)) {
        ...
        for (BoneIt b_it = lst.begin(); b_it != lst.end(); b_it++) {
          ...
          case SBoneShape::stBox:
            r = _max(_max((*b_it)->shape.box.m_halfsize.x,
                          (*b_it)->shape.box.m_halfsize.y),
          ...
          case SBoneShape::stCylinder:
            r = _max((*b_it)->shape.cylinder.m_height,
                     (*b_it)->shape.cylinder.m_radius);
...
void CActorTools::Clear() {
  ...
  m_bObjectModified = false;
  m_Flags.set(flUpdateGeometry | flUpdateMotionDefs | flUpdateMotionKeys |
                  flReadOnlyMode,
              FALSE);
  m_EditMode = emObject;
  ...
  if (m_pEditObject) {
    ...
    for (EditMeshIt m_it = meshes.begin(); m_it != meshes.end(); m_it++) {
      ...
      for (u32 f_id = 0; f_id < mesh->GetFCount(); f_id++) {
        sprintf(tmp, "\t\t%-d,\t\t%-d,\t\t%-d,", faces[f_id].pv[0].pindex,
                faces[f_id].pv[1].pindex, faces[f_id].pv[2].pindex);
        W->w_string(tmp);
      }
      W->w_string("\t}");
      W->w_string("}");
...
bool __fastcall CActorTools::MouseStart(TShiftState Shift) {
  ...
  case etaSelect:
    switch (m_EditMode) {
    case emBone: {
      CBone *B = m_pEditObject->PickBone(UI->m_CurrentRStart, UI->m_CurrentRDir,
                                         m_AVTransform);
      bool bVal = B ? Shift.Contains(ssAlt)
                          ? false
                          : (Shift.Contains(ssCtrl) ? !B->Selected() : true)
                    : false;
      SelectListItem(BONES_PREFIX, B ? MakeFullBoneName(B).c_str() : 0, bVal,
                     Shift.Contains(ssCtrl) || Shift.Contains(ssAlt), true);
    } break;
    }
    break;
  case etaAdd:
...
CSMotion *CActorTools::GetCurrentMotion() {
  return m_pEditObject
             ? m_pEditObject->FindSMotionByName(m_CurrentMotion.c_str())
             : 0;
}
...
bool CActorTools::Pick(TShiftState Shift) { return false; }

bool CActorTools::RayPick(const Fvector &start, const Fvector &dir, float &dist,
                          Fvector *pt, Fvector *n) {
  if (m_PreviewObject.m_pObject) {
    SRayPickInfo pinf;
    if (m_PreviewObject.m_pObject->RayPick(dist, start, dir, Fidentity,
                                           &pinf)) {
      if (pt)
        pt->set(pinf.pt);
      if (n) {
...
void CActorTools::ShowClipMaker() {
  //.	if
  //(CurrentObject()&&CurrentObject()->IsSkeleton()&&CurrentObject()->IsAnimated())
...
LPCSTR CActorTools::GetInfo() { return 0; }
...
LPCSTR CActorTools::ExtractMotionName(LPCSTR full_name, LPCSTR prefix) {
  ...
  //    	return full_name + xr_strlen(prefix) + 1 + xr_strlen(_templ) +
  //    1;
}
