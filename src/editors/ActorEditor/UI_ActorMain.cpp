...
CActorMain::CActorMain() { EPrefs = xr_new<CAEPreferences>(); }
//---------------------------------------------------------------------------
...
CCommandVar CActorTools::CommandLoad(CCommandVar p1, CCommandVar p2) {
  ...
  if (temp_fn.size()) {
    ...
    ELog.Msg(mtInformation,
             "Object '%s' successfully loaded. Loading time - %3.2f(s).",
  ...
  if (p2 == 1) {
    ...
    if (p1.IsInteger())
      return ExecCommand(COMMAND_SAVE,
                         xr_string(ATools->m_LastFileName.c_str()), 0);
    ...
    if (temp_fn.empty()) {
      ...
      if (Tools->Save(temp_fn.c_str())) {
        ELog.Msg(mtInformation,
                 "Object '%s' successfully saved. Saving time - %3.2f(s).",
                 m_LastFileName.c_str(), T.GetElapsed_sec());
        m_LastFileName = temp_fn.c_str();
        EPrefs->AppendRecentFile(m_LastFileName.c_str());
        ExecCommand(COMMAND_UPDATE_CAPTION);
...
CCommandVar CActorTools::CommandImport(CCommandVar p1, CCommandVar p2) {
  ...
  if (p1.IsString() || EFS.GetOpenName(_import_, temp_fn)) {
    ...
    if (temp_fn.npos != temp_fn.find(pp->m_Path)) {
      ...
      m_LastFileName = temp_fn.c_str();
      ELog.Msg(mtInformation,
               "Object '%s' successfully imported. Loading time - %3.2f(s).",
               m_LastFileName.c_str(), T.GetElapsed_sec());
...
CCommandVar CActorTools::CommandOptimizeMotions(CCommandVar p1,
                                                CCommandVar p2) {
  OptimizeMotions();
  return TRUE;
}
...
void CActorMain::RegisterCommands() {
  ...
  // tools
  REGISTER_CMD_CE(COMMAND_CLEAR, "File\\Clear Scene", ATools,
                  CActorTools::CommandClear, true);
  REGISTER_CMD_CE(COMMAND_LOAD, "File\\Load", ATools, CActorTools::CommandLoad,
                  true);
  REGISTER_CMD_C(COMMAND_SAVE_BACKUP, ATools, CActorTools::CommandSaveBackup);
  REGISTER_SUB_CMD_CE(COMMAND_SAVE, "File", ATools, CActorTools::CommandSave,
                      true);
  APPEND_SUB_CMD("Save", 0, 0);
  APPEND_SUB_CMD("Save As", 0, 1);
  REGISTER_SUB_CMD_END;
  REGISTER_CMD_CE(COMMAND_IMPORT, "File\\Import", ATools,
                  CActorTools::CommandImport, true);
  REGISTER_CMD_CE(COMMAND_EXPORT_DM, "File\\Export DM", ATools,
                  CActorTools::CommandExportDM, true);
  REGISTER_CMD_CE(COMMAND_EXPORT_OBJ, "File\\Export OBJ", ATools,
                  CActorTools::CommandExportOBJ, true);
  REGISTER_CMD_CE(COMMAND_EXPORT_OGF, "File\\Export OGF", ATools,
                  CActorTools::CommandExportOGF, true);
  REGISTER_CMD_CE(COMMAND_EXPORT_OMF, "File\\Export OMF", ATools,
                  CActorTools::CommandExportOMF, true);
  REGISTER_CMD_CE(COMMAND_EXPORT_CPP, "File\\Export CPP", ATools,
                  CActorTools::CommandExportCPP, true);
  REGISTER_CMD_CE(COMMAND_UNDO, "Edit\\Undo", ATools, CActorTools::CommandUndo,
                  false);
  REGISTER_CMD_CE(COMMAND_REDO, "Edit\\Redo", ATools, CActorTools::CommandRedo,
                  false);
  REGISTER_CMD_C(COMMAND_OPTIMIZE_MOTIONS, ATools,
                 CActorTools::CommandOptimizeMotions);
  REGISTER_CMD_CE(COMMAND_MAKE_THUMBNAIL, "Make Thumbnail", ATools,
                  CActorTools::CommandMakeThumbnail, false);
  REGISTER_CMD_CE(COMMAND_BATCH_CONVERT, "File\\Batch Convert", ATools,
                  CActorTools::CommandBatchConvert, false);
  // ui
  ...
  REGISTER_SUB_CMD_SE(COMMAND_CHANGE_TARGET, "Change Target",
                      CommandChangeTarget, true);
...
char *CActorMain::GetCaption() {
  return ATools->GetEditFileName().IsEmpty()
             ? "noname"
             : ATools->GetEditFileName().c_str();
}
...
void CAEPreferences::Load(CInifile *I) {
  ...
  bAlwaysShowKeyBar12 =
      R_BOOL_SAFE("ae_prefs", "always_show_keybar12", bAlwaysShowKeyBar12);
  bAlwaysShowKeyBar34 =
      R_BOOL_SAFE("ae_prefs", "always_show_keybar34", bAlwaysShowKeyBar34);
}
...
void CAEPreferences::FillProp(PropItemVec &props) {
  ...
  PHelper().CreateBOOL(props, "Keybar\\show footsteps 12",
                       &bAlwaysShowKeyBar12);
  PHelper().CreateBOOL(props, "Keybar\\show footsteps 34",
                       &bAlwaysShowKeyBar34);

  PHelper().CreateBOOL(props, "Tools\\MotionExport\\Force 16bit MotionT",
                       &g_force16BitTransformQuant);
}
