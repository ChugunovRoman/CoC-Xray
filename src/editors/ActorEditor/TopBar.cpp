...
void __fastcall TfraTopBar::fsStorageRestorePlacement(TObject *Sender) {
  ...
  else if (ebCameraArcBall->Down)
    EDevice.m_Camera.SetStyle(cs3DArcBall);
  else if (ebCameraFly->Down)
...
void __fastcall TfraTopBar::SimulateButtonClick(TObject *Sender) {
  ...
  if (SimulateButton->Down)
    ActorTools->PhysicsSimulate(); //->m_RenderObject.PhysicsSimulate
                                   //(ActorTools->m_AVTransform);
  else
    ActorTools
        ->PhysicsStopSimulate(); //->m_RenderObject.PhysicsStopSimulate	();
}
//---------------------------------------------------------------------------
