  ...
  if (IsRenderable() && fraLeftBar->ebRenderEngineStyle->Down) {
    ...
    if (SA) {
      for (int k = SA->m_Motions.size() - 1; k >= 0; --k) {
        ...
        LHelper().CreateItem(items, slot_pref.c_str(), modeID,
                             ListItem::flSorted);
        // cycles
        ...
        for (; I != E; ++I) {
          ...
          LHelper().CreateItem(items, tmp.c_str(), modeID, 0,
                               *(void **)&MotionID((u16)k, I->second));
        }
        // fxs
...
bool EngineModel::UpdateVisual(CEditableObject *source, bool bUpdGeom,
                               bool bUpdKeys, bool bUpdDefs) {
  bool bRes = true;
  CMemoryWriter F;
  ...
  m_pVisual =
      ::Render->model_Create(ChangeFileExt(source->GetName(), "").c_str(), &R);
  m_pBlend = 0;
  return bRes;
}
      ...
      if (mdef->flags & esmFX) {
        for (int k = 0; k < MAX_PARTS; k++) {
          if (!m_BPPlayItems[k].name.IsEmpty()) {
            MotionID D = SA->ID_Motion(m_BPPlayItems[k].name.c_str(),
                                       m_BPPlayItems[k].slot);
            if (D.valid())
      ...
      if (M&&IsRenderable()){
          if (M->flags&esmFX){
                  ...
                  if (!m_BPPlayCache[k].IsEmpty()){
                          CMotionDef* D =
     PSkeletonAnimated(m_pVisual)->ID_Cycle_Safe(m_BPPlayCache[k].c_str()); if
     (D) D->PlayCycle(PSkeletonAnimated(m_pVisual),k,false,0,0);
                  }
              }
                  m_pBlend =
     PSkeletonAnimated(m_pVisual)->PlayFX(M->Name(),1.f); }else{
                  R_ASSERT((M->m_BoneOrPart==BI_NONE)||(M->m_BoneOrPart<MAX_PARTS));
              u16 idx 		= M->m_BoneOrPart;
                  if (BI_NONE==idx)for (int k=0; k<MAX_PARTS; k++)
     m_BPPlayCache[k] = M->Name(); else			m_BPPlayCache[idx] =
     M->Name(); m_pBlend		= 0;

                          for (int k=0; k<MAX_PARTS; k++){
                  if (!m_BPPlayCache[k].IsEmpty()){
                          CMotionDef* D =
     PSkeletonAnimated(m_pVisual)->ID_Cycle_Safe(m_BPPlayCache[k].c_str());
                      CBlend* B=0;
                      if (D){
                          B =
     D->PlayCycle(PSkeletonAnimated(m_pVisual),k,(idx==k)?!(D->flags&esmNoMix):FALSE,0,0);
                                                  if (idx==k) m_pBlend = B;
                      }
                  }
              }
...
void EngineModel::RestoreParams(TFormStorage *s) {
  for (u16 k = 0; k < MAX_PARTS; k++) {
    ...
    m_BPPlayItems[k].slot =
        (u16)s->ReadInteger("bp_cache_slot_" + AnsiString(k), 0);
  }
}
