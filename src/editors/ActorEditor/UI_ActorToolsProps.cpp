...
void CActorTools::OnObjectItemFocused(ListItemsVec &items) {
  ...
  if (m_pEditObject) {
    ...
    //.	    StopMotion					();     // убрал из-за того что не
    //миксятся анимации в режиме енжине
...
//------------------------------------------------------------------------------
void CActorTools::OnExportImportRefsClick(ButtonValue *V, bool &bModif,
                                          bool &bSafe) {
  switch (V->btn_num) {
  case 0: { // export
    xr_string fname;
...
void CActorTools::OnMotionEditClick(ButtonValue *V, bool &bModif, bool &bSafe) {
  ...
  case 1: { // delete
    ...
    if (m_ObjectItems->GetSelected(MOTIONS_PREFIX, items, true)) {
      if (ELog.DlgMsg(mtConfirmation, TMsgDlgButtons() << mbYes << mbNo,
                      "Delete selected %d item(s)?", items.size()) == mrYes) {
        for (ListItemsIt it = items.begin(); it != items.end(); it++) {
          VERIFY((*it)->m_Object);
...
void CActorTools::RealUpdateProperties() {
  ...
  if (!appended_motions.empty()) {
    ...
    m_ObjectItems->LockUpdating();
    for (SMotionIt m_it = appended_motions.begin();
         m_it != appended_motions.end(); m_it++)
...
void CActorTools::OnMotionControlClick(ButtonValue *V, bool &bModif,
                                       bool &bSafe) {
  switch (V->btn_num) {
  case 0:
    PlayMotion();
...
//------------------------------------------------------------------------------
void CActorTools::OnMarksControlClick12(ButtonValue *V, bool &bModif,
                                        bool &bSafe) {
  switch (V->btn_num) {
  case 0:
...
void CActorTools::OnMarksControlClick34(ButtonValue *V, bool &bModif,
                                        bool &bSafe) {
  switch (V->btn_num) {
  case 0:
...
void CActorTools::OnBoxAxisClick(ButtonValue *V, bool &bModif, bool &bSafe) {
  ...
  Fvector::generate_orthonormal_basis_normalized(BONE->shape.box.m_rotate.k,
                                                 BONE->shape.box.m_rotate.j,
                                                 BONE->shape.box.m_rotate.i);
  ExecCommand(COMMAND_UPDATE_PROPERTIES);
}

void CActorTools::OnCylinderAxisClick(ButtonValue *V, bool &bModif,
                                      bool &bSafe) {
  CBone *BONE = (CBone *)V->tag;
  switch (V->btn_num) {
  case 0:
...
void CActorTools::FillMotionProperties(PropItemVec &items, LPCSTR pref,
                                       ListItem *sender) {
  R_ASSERT(m_pEditObject);
  CSMotion *SM =
      m_pEditObject->m_SMotionRefs.size() ? 0 : (CSMotion *)sender->m_Object;
  PropValue *V;
  ...
  PHelper().CreateCaption(items, PrepareKey(pref, "Global\\Motion count"),
                          m_cnt.c_str());
  ...
  V = PHelper().CreateChoose(
      items, PrepareKey(pref, "Global\\Motion reference"), &m_tmp_mot_refs,
      smGameSMotions, 0, 0, MAX_ANIM_SLOT);
  // m_pEditObject->m_SMotionRefs
  V->OnChangeEvent.bind(this, &CActorTools::OnMotionRefsChange);
  ButtonValue *B;

  B = PHelper().CreateButton(items, PrepareKey(pref, "Export Import"),
                             "ExportRefs,ImportRefs", ButtonValue::flFirstOnly);
  B->OnBtnClickEvent.bind(this, &CActorTools::OnExportImportRefsClick);

  if (m_pEditObject->m_SMotionRefs.size() == 0) {
    B = PHelper().CreateButton(items, PrepareKey(pref, "Global\\Edit"),
                               "Append,Delete,Save", ButtonValue::flFirstOnly);
    B->OnBtnClickEvent.bind(this, &CActorTools::OnMotionEditClick);
  }
  if (SM) {
    B = PHelper().CreateButton(items, PrepareKey(pref, "Motion\\Control"),
                               "Play,Stop,Pause", ButtonValue::flFirstOnly);
    B->OnBtnClickEvent.bind(this, &CActorTools::OnMotionControlClick);
    PHelper().CreateCaption(items, PrepareKey(pref, "Motion\\Frame\\Start"),
                            shared_str().printf("%d", SM->FrameStart()));
    PHelper().CreateCaption(items, PrepareKey(pref, "Motion\\Frame\\End"),
                            shared_str().printf("%d", SM->FrameEnd()));
    PHelper().CreateCaption(items, PrepareKey(pref, "Motion\\Frame\\Length"),
                            shared_str().printf("%d", SM->Length()));
    PropValue *P = 0;
    P = PHelper().CreateName(items, PrepareKey(pref, "Motion\\Name"), &SM->name,
                             sender);
    P->OnChangeEvent.bind(this, &CActorTools::OnMotionNameChange);
    PHelper().CreateFloat(items, PrepareKey(pref, "Motion\\Speed"), &SM->fSpeed,
                          0.f, 10.f, 0.01f, 2);
    PHelper().CreateFloat(items, PrepareKey(pref, "Motion\\Accrue"),
                          &SM->fAccrue, 0.f, 10.f, 0.01f, 2);
    PHelper().CreateFloat(items, PrepareKey(pref, "Motion\\Falloff"),
                          &SM->fFalloff, 0.f, 10.f, 0.01f, 2);

    PropValue /**C=0,*/ *TV = 0;
    TV = PHelper().CreateFlag8(items, PrepareKey(pref, "Motion\\Type FX"),
                               &SM->m_Flags, esmFX);
    TV->OnChangeEvent.bind(this, &CActorTools::OnMotionTypeChange);
    m_BoneParts.clear();
    if (SM->m_Flags.is(esmFX)) {
      for (BoneIt it = m_pEditObject->FirstBone();
           it != m_pEditObject->LastBone(); it++)
        m_BoneParts.push_back(xr_rtoken((*it)->Name().c_str(), (*it)->SelfID));
      PHelper().CreateRToken16(
          items, PrepareKey(pref, "Motion\\FX\\Start bone"),
          (u16 *)&SM->m_BoneOrPart, &*m_BoneParts.begin(), m_BoneParts.size());

      PHelper().CreateFloat(items, PrepareKey(pref, "Motion\\FX\\Power"),
                            &SM->fPower, 0.f, 10.f, 0.01f, 2);
    } else {
      m_BoneParts.push_back(xr_rtoken("--all bones--", BI_NONE));
      for (BPIt it = m_pEditObject->FirstBonePart();
           it != m_pEditObject->LastBonePart(); it++)
        m_BoneParts.push_back(
            xr_rtoken(it->alias.c_str(), it - m_pEditObject->FirstBonePart()));
      PHelper().CreateRToken16(
          items, PrepareKey(pref, "Motion\\Cycle\\Bone part"),
          &SM->m_BoneOrPart, &*m_BoneParts.begin(), m_BoneParts.size());
      PHelper().CreateFlag8(items,
                            PrepareKey(pref, "Motion\\Cycle\\Stop at end"),
                            &SM->m_Flags, esmStopAtEnd);
      PHelper().CreateFlag8(items, PrepareKey(pref, "Motion\\Cycle\\No mix"),
                            &SM->m_Flags, esmNoMix);
      PHelper().CreateFlag8(items, PrepareKey(pref, "Motion\\Cycle\\Sync part"),
                            &SM->m_Flags, esmSyncPart);

      PHelper().CreateFlag8(items,
                            PrepareKey(pref, "Motion\\Cycle\\UseFootSteps"),
                            &SM->m_Flags, esmUseFootSteps);
      PHelper().CreateFlag8(items,
                            PrepareKey(pref, "Motion\\Cycle\\Move XForm"),
                            &SM->m_Flags, esmRootMover);
      PHelper().CreateFlag8(items, PrepareKey(pref, "Motion\\Cycle\\Idle"),
                            &SM->m_Flags, esmIdle);
      PHelper().CreateFlag8(items,
                            PrepareKey(pref, "Motion\\Cycle\\UseWeaponBone"),
                            &SM->m_Flags, esmUseWeaponBone);
    }

    B = PHelper().CreateButton(items, PrepareKey(pref, "Marks\\Control-12"),
                               "Add,Remove", ButtonValue::flFirstOnly);
    B->OnBtnClickEvent.bind(this, &CActorTools::OnMarksControlClick12);

    B = PHelper().CreateButton(items, PrepareKey(pref, "Marks\\Control-34"),
                               "Add,Remove", ButtonValue::flFirstOnly);
    ...
    for (u32 i = 0; i < SM->marks.size(); ++i) {
      ...
      P = PHelper().CreateCaption(items, PrepareKey(pref, buff),
                                  SM->marks[i].name);
    }

    {
      ...
      StartMotionPoint.x = BM.envs[ctPositionX]->keys.size()
                               ? BM.envs[ctPositionX]->keys.front()->value
                               : -1;
      StartMotionPoint.y = BM.envs[ctPositionY]->keys.size()
                               ? BM.envs[ctPositionY]->keys.front()->value
                               : -1;
      StartMotionPoint.z = BM.envs[ctPositionZ]->keys.size()
                               ? BM.envs[ctPositionZ]->keys.front()->value
                               : -1;

      PHelper().CreateVector(items,
                             PrepareKey(pref, "Motion\\RootStartTransform"),
                             &StartMotionPoint, -10000, 10000, 0.001, 4);

      EndMotionPoint.x = BM.envs[ctPositionX]->keys.size()
                             ? BM.envs[ctPositionX]->keys.back()->value
                             : -1;
      EndMotionPoint.y = BM.envs[ctPositionY]->keys.size()
                             ? BM.envs[ctPositionY]->keys.back()->value
                             : -1;
      EndMotionPoint.z = BM.envs[ctPositionZ]->keys.size()
                             ? BM.envs[ctPositionZ]->keys.back()->value
                             : -1;

      PHelper().CreateVector(items,
                             PrepareKey(pref, "Motion\\RootEndTransform"),
                             &EndMotionPoint, -10000, 10000, 0.001, 4);
    }
  }
}
//------------------------------------------------------------------------------

xr_token joint_types[] = {{"Rigid", jtRigid},
                          {"Cloth", jtCloth},
                          {"Joint", jtJoint},
...
xr_token shape_types[] = {{"None", SBoneShape::stNone},
...
void CActorTools::OnBoneCreateDeleteClick(ButtonValue *V, bool &bModif,
                                          bool &bSafe) {
  R_ASSERT(m_pEditObject);
  BoneVec sel_bones;
  ...
  case 2: {
    ...
    if (TfrmChoseItem::SelectItem(smSkeletonBonesInObject, _bone_name, 1, 0, 0,
                                  m_pEditObject)) {
...
void CActorTools::FillBoneProperties(PropItemVec &items, LPCSTR pref,
                                     ListItem *sender) {
  R_ASSERT(m_pEditObject);
  CBone *BONE = (CBone *)sender->m_Object;

  PHelper().CreateCaption(
      items, PrepareKey(pref, "Global\\Bone count"),
      shared_str().printf("%d", m_pEditObject->BoneCount()));
  ButtonValue *B;
  B = PHelper().CreateButton(items, PrepareKey(pref, "Global\\File"),
                             "Load,Save", ButtonValue::flFirstOnly);
  B->OnBtnClickEvent.bind(this, &CActorTools::OnBoneFileClick);
  B = PHelper().CreateButton(items, PrepareKey(pref, "Global\\Edit"),
                             "Bind pose,Reset IK,Clamp limits",
                             ButtonValue::flFirstOnly);
  B->OnBtnClickEvent.bind(this, &CActorTools::OnBoneEditClick);
  B = PHelper().CreateButton(items, PrepareKey(pref, "Global\\Generate Shape"),
                             "All, Selected", ButtonValue::flFirstOnly);
  B->OnBtnClickEvent.bind(this, &CActorTools::OnBoneShapeClick);
  //---
  B = PHelper().CreateButton(items, PrepareKey(pref, "Global\\Bone"),
                             "Create, Delete, Orient",
                             ButtonValue::flFirstOnly);
  ...
  if (BONE) {
    PropValue *V;
    PHelper().CreateCaption(items, PrepareKey(pref, "Bone\\Name"),
                            BONE->Name());
    ...
    // PrepareKey(pref,"Bone\\Influence"),
    // shared_str().sprintf("%d vertices",0));
    PHelper().CreateChoose(items, PrepareKey(pref, "Bone\\Game Material"),
                           &BONE->game_mtl, smGameMaterial);
    PHelper().CreateFloat(items, PrepareKey(pref, "Bone\\Mass"), &BONE->mass,
                          0.f, 10000.f);
    PHelper().CreateVector(items, PrepareKey(pref, "Bone\\Center Of Mass"),
                           &BONE->center_of_mass, -10000.f, 10000.f);
    V = PHelper().CreateVector(items, PrepareKey(pref, "Bone\\Bind Position"),
                               &BONE->_RestOffset(), -10000.f, 10000.f);
    V->OnChangeEvent.bind(this, &CActorTools::OnBindTransformChange);
    V = PHelper().CreateAngle3(items, PrepareKey(pref, "Bone\\Bind Rotation"),
                               &BONE->_RestRotate());
    V->OnChangeEvent.bind(this, &CActorTools::OnBindTransformChange);
    PHelper().CreateFlag16(items,
                           PrepareKey(pref, "Bone\\Shape\\Flags\\No Pickable"),
                           &BONE->shape.flags, SBoneShape::sfNoPickable);
    PHelper().CreateFlag16(items,
                           PrepareKey(pref, "Bone\\Shape\\Flags\\No Physics"),
                           &BONE->shape.flags, SBoneShape::sfNoPhysics);
    PHelper().CreateFlag16(
        items, PrepareKey(pref, "Bone\\Shape\\Flags\\Remove After Break"),
        &BONE->shape.flags, SBoneShape::sfRemoveAfterBreak);
    PHelper().CreateFlag16(
        items, PrepareKey(pref, "Bone\\Shape\\Flags\\No Fog Collider"),
        &BONE->shape.flags, SBoneShape::sfNoFogCollider);

    V = PHelper().CreateToken16(items, PrepareKey(pref, "Bone\\Shape\\Type"),
                                &BONE->shape.type, shape_types);
    V->OnChangeEvent.bind(this, &CActorTools::OnShapeTypeChange);
    switch (BONE->shape.type) {
    case SBoneShape::stBox:
      PHelper().CreateVector(items,
                             PrepareKey(pref, "Bone\\Shape\\Box\\Center"),
                             &BONE->shape.box.m_translate, -10000.f, 10000.f);
      B = PHelper().CreateButton(
          items, PrepareKey(pref, "Bone\\Shape\\Box\\Align Axis"), "X,Y,Z", 0);
      B->OnBtnClickEvent.bind(this, &CActorTools::OnBoxAxisClick);
      B->tag = (int)BONE;
      PHelper().CreateVector(items,
                             PrepareKey(pref, "Bone\\Shape\\Box\\Half Size"),
                             &BONE->shape.box.m_halfsize, 0.f, 1000.f);
      break;
    case SBoneShape::stSphere:
      PHelper().CreateVector(items,
                             PrepareKey(pref, "Bone\\Shape\\Sphere\\Position"),
                             &BONE->shape.sphere.P, -10000.f, 10000.f);
      PHelper().CreateFloat(items,
                            PrepareKey(pref, "Bone\\Shape\\Sphere\\Radius"),
                            &BONE->shape.sphere.R, 0.f, 1000.f);
      break;
    case SBoneShape::stCylinder:
      PHelper().CreateVector(items,
                             PrepareKey(pref, "Bone\\Shape\\Cylinder\\Center"),
                             &BONE->shape.cylinder.m_center, -10000.f, 10000.f);
      B = PHelper().CreateButton(
          items, PrepareKey(pref, "Bone\\Shape\\Cylinder\\Align Axis"), "X,Y,Z",
          0);
      B->OnBtnClickEvent.bind(this, &CActorTools::OnCylinderAxisClick);
      B->tag = (int)BONE;
      PHelper().CreateFloat(items,
                            PrepareKey(pref, "Bone\\Shape\\Cylinder\\Height"),
                            &BONE->shape.cylinder.m_height, 0.f, 1000.f);
      PHelper().CreateFloat(items,
                            PrepareKey(pref, "Bone\\Shape\\Cylinder\\Radius"),
                            &BONE->shape.cylinder.m_radius, 0.f, 1000.f);
      break;
    }

    Fvector lim_rot;
    Fmatrix mLocal, mBind, mBindI;
    mBind.setXYZi(BONE->_RestRotate().x, BONE->_RestRotate().y,
                  BONE->_RestRotate().z);
    mBindI.invert(mBind);
    ...
    V = PHelper().CreateFlag32(items,
                               PrepareKey(pref, "Bone\\Joint\\Breakable"),
                               &data.ik_flags, SJointIKData::flBreakable);
    V->OnChangeEvent.bind(this, &CActorTools::OnJointTypeChange);
    if (data.ik_flags.is(SJointIKData::flBreakable)) {
      PHelper().CreateFloat(items, PrepareKey(pref, "Bone\\Joint\\Break Force"),
                            &data.break_force, 0.f, 1000000000.f);
      PHelper().CreateFloat(items,
                            PrepareKey(pref, "Bone\\Joint\\Break Torque"),
                            &data.break_torque, 0.f, 1000000000.f);
    }
    V = PHelper().CreateToken32(items, PrepareKey(pref, "Bone\\Joint\\Type"),
                                (u32 *)&data.type, joint_types);
    V->OnChangeEvent.bind(this, &CActorTools::OnJointTypeChange);
    switch (data.type) {
    case jtRigid:
      break;
    case jtCloth: {
      PHelper().CreateFloat(items, PrepareKey(pref, "Bone\\Joint\\Friction"),
                            &data.friction, 0.f, 1000000000.f);
      PHelper().CreateFloat(items,
                            PrepareKey(pref, "Bone\\Joint\\Spring Factor"),
                            &data.spring_factor, 0.f, 1000.f);
      PHelper().CreateFloat(items,
                            PrepareKey(pref, "Bone\\Joint\\Damping Factor"),
                            &data.damping_factor, 0.f, 1000.f);
    } break;
    case jtJoint: {
      PHelper().CreateFloat(items, PrepareKey(pref, "Bone\\Joint\\Friction"),
                            &data.friction, 0.f, 1000000000.f);
      ...
      for (int k = 0; k < 3; k++) {
        V = PHelper().CreateAngle(
            items, PrepareKey(pref, "Bone\\Joint\\Limits", axis[k], "Min"),
            &data.limits[k].limit.x, -M_PI, 0.f);
        V->OnChangeEvent.bind(this, &CActorTools::OnBoneLimitsChange);
        V = PHelper().CreateAngle(
            items, PrepareKey(pref, "Bone\\Joint\\Limits", axis[k], "Max"),
            &data.limits[k].limit.y, 0.f, M_PI);
        V->OnChangeEvent.bind(this, &CActorTools::OnBoneLimitsChange);
        V = PHelper().CreateFloat(
            items,
            PrepareKey(pref, "Bone\\Joint\\Limits", axis[k], "Spring Factor"),
            &data.limits[k].spring_factor, 0.f, 1000.f);
        V->OnChangeEvent.bind(this, &CActorTools::OnBoneLimitsChange);
        V = PHelper().CreateFloat(
            items,
            PrepareKey(pref, "Bone\\Joint\\Limits", axis[k], "Damping Factor"),
    ...
    case jtWheel: {
      ...
      PHelper().CreateFloat(items,
                            ...
                            &data.damping_factor, 0.f, 1000.f);
      V = PHelper().CreateAngle(
          items, PrepareKey(pref, "Bone\\Joint\\Steer\\Limits Min"),
          &data.limits[idx].limit.x, -PI_DIV_2, 0.f);
      V->OnChangeEvent.bind(this, &CActorTools::OnBoneLimitsChange);
      V = PHelper().CreateAngle(
          items, PrepareKey(pref, "Bone\\Joint\\Steer\\Limits Max"),
          &data.limits[idx].limit.y, 0, PI_DIV_2);
      V->OnChangeEvent.bind(this, &CActorTools::OnBoneLimitsChange);
    } break;
    case jtSlider: {
      PHelper().CreateFloat(items, PrepareKey(pref, "Bone\\Joint\\Friction"),
      ...
      { // slider
        V = PHelper().CreateFloat(
            items, PrepareKey(pref, "Bone\\Joint\\Slide (Axis Z)\\Limits Min"),
            &data.limits[0].limit[0], -100.f, 0.f);
        V->OnChangeEvent.bind(this, &CActorTools::OnBoneLimitsChange);
        V = PHelper().CreateFloat(
            items, PrepareKey(pref, "Bone\\Joint\\Slide (Axis Z)\\Limits Max"),
            &data.limits[0].limit[1], 0.f, 100.f);
        V->OnChangeEvent.bind(this, &CActorTools::OnBoneLimitsChange);
        V = PHelper().CreateFloat(
            items,
            PrepareKey(pref, "Bone\\Joint\\Slide (Axis Z)\\Spring Factor"),
            &data.limits[0].spring_factor, 0.f, 1000.f);
        V->OnChangeEvent.bind(this, &CActorTools::OnBoneLimitsChange);
        V = PHelper().CreateFloat(
            items,
            PrepareKey(pref, "Bone\\Joint\\Slide (Axis Z)\\Damping Factor"),
            &data.limits[0].damping_factor, 0.f, 1000.f);
        V->OnChangeEvent.bind(this, &CActorTools::OnBoneLimitsChange);
      }
      { // rotate
        V = PHelper().CreateAngle(
            items, PrepareKey(pref, "Bone\\Joint\\Rotate (Axis Z)\\Limits Min"),
            &data.limits[1].limit[0], -M_PI, 0.f);
        V->OnChangeEvent.bind(this, &CActorTools::OnBoneLimitsChange);
        V = PHelper().CreateAngle(
            items, PrepareKey(pref, "Bone\\Joint\\Rotate (Axis Z)\\Limits Max"),
            &data.limits[1].limit[1], 0.f, M_PI);
        V->OnChangeEvent.bind(this, &CActorTools::OnBoneLimitsChange);
        V = PHelper().CreateFloat(
            items,
            PrepareKey(pref, "Bone\\Joint\\Rotate (Axis Z)\\Spring Factor"),
            &data.limits[1].spring_factor, 0.f, 1000.f);
        V->OnChangeEvent.bind(this, &CActorTools::OnBoneLimitsChange);
        V = PHelper().CreateFloat(
            items,
            PrepareKey(pref, "Bone\\Joint\\Rotate (Axis Z)\\Damping Factor"),
...
void CActorTools::FillSurfaceProperties(PropItemVec &items, LPCSTR pref,
                                        ListItem *sender) {
  R_ASSERT(m_pEditObject);
  CSurface *SURF = (CSurface *)sender->m_Object;
  PHelper().CreateCaption(
      items, PrepareKey(pref, "Statistic\\Count"),
      shared_str().printf("%d", m_pEditObject->SurfaceCount()));
  if (SURF) {
    PHelper().CreateCaption(items, PrepareKey(pref, "Surface\\Name"),
                            SURF->_Name());
...
void CActorTools::FillObjectProperties(PropItemVec &items, LPCSTR pref,
                                       ListItem *sender) {
  R_ASSERT(m_pEditObject);
  PropValue *V = 0;
  PHelper().CreateFlag32(items, "Object\\Flags\\Make Progressive",
                         &m_pEditObject->m_objectFlags,
                         CEditableObject::eoProgressive);
  PHelper().CreateFlag32(items, "Object\\Flags\\HQ Geometry",
                         &m_pEditObject->m_objectFlags,
                         CEditableObject::eoHQExport);

  V = PHelper().CreateVector(items, "Object\\Transform\\Position",
                             &m_pEditObject->a_vPosition, -10000, 10000, 0.01,
                             2);
  V->OnChangeEvent.bind(this, &CActorTools::OnChangeTransform);
  V = PHelper().CreateAngle3(items, "Object\\Transform\\Rotation",
                             &m_pEditObject->a_vRotate, -10000, 10000, 0.1, 1);
  V->OnChangeEvent.bind(this, &CActorTools::OnChangeTransform);
  V = PHelper().CreateCaption(
      items, "Object\\Transform\\BBox Min",
      shared_str().printf("{%3.2f, %3.2f, %3.2f}",
                          VPUSH(m_pEditObject->GetBox().min)));
  V = PHelper().CreateCaption(
      items, "Object\\Transform\\BBox Max",
      shared_str().printf("{%3.2f, %3.2f, %3.2f}",
                          VPUSH(m_pEditObject->GetBox().max)));

  //.    PHelper().CreateChoose		 (items, "Object\\LOD\\Reference",
  //&m_pEditObject->m_LODs, smObject);
  PHelper().CreateChoose(items, "Object\\LOD\\Reference",
                         &m_pEditObject->m_LODs, smVisual);
  m_pEditObject->FillSummaryProps("Object\\Summary", items);
}
//------------------------------------------------------------------------------

void CActorTools::SelectListItem(LPCSTR pref, LPCSTR name, bool bVal,
                                 bool bLeaveSel, bool bExpand) {
  AnsiString nm = (name && name[0]) ? PrepareKey(pref, name).c_str()
                                    : AnsiString(pref).c_str();
