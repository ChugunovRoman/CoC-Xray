...
void __fastcall TfrmBonePart::FillBoneParts() {
  ...
  for (BPIt it = m_BoneParts->begin(); it != m_BoneParts->end(); it++) {
    ...
    for (RStringVecIt w_it = it->bones.begin(); w_it != it->bones.end(); w_it++)
      FHelper.AppendObject(T[it - m_BoneParts->begin()], w_it->c_str(), false,
                           true);
  }
  for (k = 0; k < 4; k++)
...
void __fastcall TfrmBonePart::tvPartDragDrop(TObject *Sender, TObject *Source,
                                             int X, int Y) {
...
void __fastcall TfrmBonePart::tvPartDragOver(TObject *Sender, TObject *Source,
...
void __fastcall TfrmBonePart::ebSaveClick(TObject *Sender) {
  ...
  for (k = 0; k < 4; k++) {
    if (T[k]->Items->Count)
      for (TElTreeItem *node = T[k]->Items->GetFirstNode(); node;
           node = node->GetNext())
        b_use[m_EditObject->FindBoneByNameIdx(
            AnsiString(node->Text).c_str())]++;
  }
  for (U8It u_it = b_use.begin(); u_it != b_use.end(); u_it++)
    if (*u_it != 1) {
      ELog.DlgMsg(mtError,
                  "Invalid bone part found (missing or duplicate bones).");
      return;
    }
...
void __fastcall TfrmBonePart::ExtBtn1Click(TObject *Sender) {
  ...
  E[0]->Text = "default";
  for (BoneIt it = m_EditObject->FirstBone(); it != m_EditObject->LastBone();
       it++)
...
{
        TExtBtn* B = dynamic_cast<TExtBtn*>(Sender); VERIFY(B); int idx =
B->Tag; T[idx]->IsUpdating = true; T[idx]->Items->Clear(); for (BoneIt
it=m_EditObject->FirstBone(); it!=m_EditObject->LastBone(); it++)
...
void __fastcall TfrmBonePart::ebSaveToClick(TObject *Sender) {
  ...
  if (EFS.GetSaveName(_import_, temp_fn)) {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      if (T[i]->Items->Count) {
        for (TElTreeItem *node = T[i]->Items->GetFirstNode(); node;
             node = node->GetNext())
