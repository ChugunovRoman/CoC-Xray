...
enum EEditMode { emObject, emMotion, emBone, emSurface, emMesh };
...
public:
  ...
  bool UpdateVisual(CEditableObject *source, bool bUpdGeom, bool bUpdKeys,
                    bool bUpdDefs);
...
private:
  //	virtual	Fmatrix&				   	_BCL
  //ObjectXFORM
  //()				 { return m_pobject_matrix; } 	virtual	Fvector&
  //_BCL	ObjectPosition						()				 {
  //return m_pobject_matrix.c; } virtual	IRenderVisual*
  // _BCL	ObjectVisual						()				 {
  // return m_pVisual;}
...
public:
  enum { pmScroll = (1 << 0), force_dword = u32(-1) };
  enum EScrollAxis { saZp, saZn, saXp, saXn, saforceDWORD = u32(-1) };
...
class CActorTools : public CToolCustom {
  ...
  void __stdcall OnCylinderAxisClick(ButtonValue *sender, bool &bModif,
                                     bool &bSafe);

  void __stdcall OnMotionEditClick(ButtonValue *sender, bool &bModif,
                                   bool &bSafe);
  void __stdcall OnMotionControlClick(ButtonValue *sender, bool &bModif,
                                      bool &bSafe);
  void __stdcall OnExportImportRefsClick(ButtonValue *sender, bool &bModif,
                                         bool &bSafe);

  void __stdcall OnMarksControlClick12(ButtonValue *sender, bool &bModif,
                                       bool &bSafe);
  void __stdcall OnMarksControlClick34(ButtonValue *sender, bool &bModif,
                                       bool &bSafe);

  void __stdcall OnObjectItemFocused(ListItemsVec &items);

  void __stdcall OnBoneShapeClick(ButtonValue *sender, bool &bModif,
                                  bool &bSafe);
  void __stdcall OnBoneCreateDeleteClick(ButtonValue *sender, bool &bModif,
                                         bool &bSafe);
  void __stdcall OnBoneEditClick(ButtonValue *sender, bool &bModif,
                                 bool &bSafe);
  void __stdcall OnBoneFileClick(ButtonValue *sender, bool &bModif,
                                 bool &bSafe);
...
public:
  ...
  virtual bool RayPick(const Fvector &start, const Fvector &dir, float &dist,
  ...
  void SelectListItem(LPCSTR pref, LPCSTR name, bool bVal, bool bLeaveSel,
                      bool bExpand);
