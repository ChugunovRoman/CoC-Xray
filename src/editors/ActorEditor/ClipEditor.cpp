...
void TClipMaker::CUIClip::SetCycle(LPCSTR name, u16 part, u16 slot) {
  if (part == BI_NONE) {
    for (int k = 0; k < 4; k++)
      cycles[k].set(name, slot);
  } else {
    cycles[part].set(name, slot);
  }
}

void TClipMaker::CUIClip::SetFX(LPCSTR name, u16 slot) { fx.set(name, slot); }
...
void TClipMaker::DestroyForm(TClipMaker *form) { xr_delete(form); }
    ...
    if (tv->SelectedCount) {
      for (TElTreeItem *item = tv->GetNextSelected(0); item;
        ...
        if (m_CurrentObject->m_SMotionRefs.size()) {
          CMotionDef *SM = ATools->m_RenderObject.FindMotionDef(m_name, m_slot);
          VERIFY(SM);
          bp = SM->bone_or_part;
          fx = SM->flags & esmFX;
        } else {
          CSMotion *SM = ATools->FindMotion(m_name);
          VERIFY(SM);
          bp = SM->m_BoneOrPart;
...
void __fastcall TClipMaker::OnClipItemFocused(ListItemsVec &items) {
  if (!items.empty()) {
    ...
    if (sel_clip)
      sbBase->HorzScrollBar->Position =
          sbBase->HorzScrollBar->Range * (sel_clip->RunTime() / m_TotalLength);
  }
}
//------------------------------------------------------------------------------
  ...
  if (sel_clip) {
    ...
    for (u16 k = 0; k < 4; k++) {
      ...
      if (mname.IsEmpty())
        continue;
      CMotionDef *MD =
          ATools->m_RenderObject.FindMotionDef(mname.c_str(), slot);
      CMotion *MI = ATools->m_RenderObject.FindMotionKeys(mname.c_str(), slot);
      SBonePart *BP = (k < (u16)m_CurrentObject->BoneParts().size())
                          ? &m_CurrentObject->BoneParts()[k]
                          : 0;
      shared_str tmp;
      if (MI)
        tmp.sprintf("%s [%3.2fs, %s]", mname.c_str(),
                    MI->GetLength() / MD->Speed(),
                    MD->bone_or_part ? "stop at end" : "looped");
      if (BP)
        PHelper().CreateCaption(
            p_items, PrepareKey("Current Clip\\Cycles", BP->alias.c_str()),
            tmp);
    }
    if (sel_clip->fx.valid())
      PHelper().CreateFloat(p_items,
                            PrepareKey("Current Clip\\FXs", *sel_clip->fx.name),
...
void TClipMaker::InsertClip() {
  ...
  CUIClip *clip =
      xr_new<CUIClip>(*nm, this, sel_clip ? sel_clip->RunTime() - EPS_L : 0);
...
void TClipMaker::AppendClip() {
  shared_str nm;
  m_ClipList->GenerateObjectName(nm, 0, "clip", true);
  CUIClip *clip = xr_new<CUIClip>(
      *nm, this,
      sel_clip ? sel_clip->RunTime() + sel_clip->Length() - EPS_L : 0);
  clips.push_back(clip);
  UpdateClips(true, false);
  SelectClip(clip);
}
...
void TClipMaker::PlayAnimation(CUIClip *clip) {
  for (u32 k = 0; k < m_CurrentObject->BoneParts().size(); k++)
    if (clip->cycles[k].valid())
      ATools->m_RenderObject.PlayCycle(*clip->cycles[k].name, k,
                                       clip->cycles[k].slot);
  if (clip->fx.valid())
    ATools->m_RenderObject.PlayFX(*clip->fx.name, clip->fx_power,
                                  clip->fx.slot);
}
//---------------------------------------------------------------------------
...
void TClipMaker::OnFrame() {
  ...
  if (m_RTFlags.is(flRT_Playing)) {
    ...
    if (m_CurrentPlayTime >
        (clips[play_clip]->RunTime() + clips[play_clip]->Length())) {
      play_clip++;
    ...
    for (UIClipIt c_it = clips.begin(); c_it != clips.end(); c_it++) {
      ...
      for (u16 k = 0; k < 4; k++) {
        ...
        CMotionDef *MD =
            ATools->m_RenderObject.FindMotionDef(mname.c_str(), slot);
        if (MI) {
          float new_len = (MD->StopAtEnd() ? MI->GetLength() - SAMPLE_SPF
                                           : MI->GetLength()) /
                          MD->Speed();
          if (len < new_len)
            len = new_len;
