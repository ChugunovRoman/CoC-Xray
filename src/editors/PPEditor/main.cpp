...
void __fastcall TMainForm::NewEffectButtonClick(TObject *Sender) {
  if (Application->MessageBox("Do you wish to create a new effect ?", "Warning",
...
void TMainForm::FillPointList() {
  PointList->Clear();
  CPostProcessParam *cparam =
      MainForm->m_Animator->GetParam(m_ActiveShowForm->GetTimeChannel());
...
void __fastcall TMainForm::LoadButtonClick(TObject *Sender) {
  /*
      if (m_Animator->GetLength() != 0.0f)
         if (Application->MessageBox ("Do you wish to save current effect ?",
     "Warning", MB_YESNO | MB_ICONSTOP) == IDYES) SaveButtonClick (Sender);
  */
...
void TMainForm::UpdateGraph() {
  ...
  canvas->FillRect(TRect(0, 0, Image->Picture->Bitmap->Width,
                         Image->Picture->Bitmap->Height));
  canvas->Pen->Color = clBlack;
  canvas->MoveTo(0, Image->Picture->Bitmap->Height / 2);
  canvas->LineTo(Image->Picture->Bitmap->Width,
                 Image->Picture->Bitmap->Height / 2);
  canvas->MoveTo(0, 0);
  ...
  float width = (float)Image->Picture->Bitmap->Width,
        height = (float)Image->Picture->Bitmap->Height * 0.5f;
  // draw marker
  ...
  for (float t = 0.0f; t < alltime; t += increment) {
    ...
    for (u32 idx = pp_base_color; idx < pp_last; ++idx) {
      ...
      switch (idx) {
      case pp_base_color:
        canvas->Pixels[x][(int)(-m_EffectorParams.color_base.r * height +
                                height)] = clRed;
        canvas->Pixels[x][(int)(-m_EffectorParams.color_base.g * height +
                                height)] = clGreen;
        canvas->Pixels[x][(int)(-m_EffectorParams.color_base.b * height +
                                height)] = clBlue;
        break;
      case pp_add_color:
        canvas->Pixels[x][(int)(-m_EffectorParams.color_add.r * height +
                                height)] = clRed;
        canvas->Pixels[x][(int)(-m_EffectorParams.color_add.g * height +
                                height)] = clGreen;
        canvas->Pixels[x][(int)(-m_EffectorParams.color_add.b * height +
                                height)] = clBlue;
        break;
      case pp_gray_color:
        canvas->Pixels[x][(int)(-m_EffectorParams.color_gray.r * height +
                                height)] = clRed;
        canvas->Pixels[x][(int)(-m_EffectorParams.color_gray.g * height +
                                height)] = clGreen;
        canvas->Pixels[x][(int)(-m_EffectorParams.color_gray.b * height +
                                height)] = clBlue;
        break;
      case pp_gray_value:
        canvas->Pixels[x][(int)(-m_EffectorParams.blur * height + height)] =
            clBlack;
        break;
      case pp_blur:
        canvas->Pixels[x][(int)(-m_EffectorParams.gray * height + height)] =
            clBlack;
        break;
      case pp_dual_h:
        canvas
            ->Pixels[x][(int)(-m_EffectorParams.duality.h * height + height)] =
            clRed;
        break;
      case pp_dual_v:
        canvas
            ->Pixels[x][(int)(-m_EffectorParams.duality.v * height + height)] =
            clGreen;
        break;
      case pp_noise_i:
        canvas->Pixels[x][(int)(-m_EffectorParams.noise.intensity * height +
                                height)] = clRed;
        break;
      case pp_noise_g:
        canvas->Pixels[x][(int)(-m_EffectorParams.noise.grain * height +
                                height)] = clGreen;
        break;
      case pp_noise_f:
        canvas
            ->Pixels[x][(int)(-m_EffectorParams.noise.fps * height + height)] =
            clBlue;
        break;
      } // switch
    }   // channel
...
void __fastcall TMainForm::PopupClick(TObject *Sender) {
  ...
  CPostProcessParam *cparam =
      MainForm->m_Animator->GetParam(m_props[itm->Tag]->GetTimeChannel());
  u32 cnt = cparam->get_keys_count();
  string256 str;
  for (u32 i = 0; i < cnt; ++i) {
    float t = cparam->get_key_time(i);
