...
__fastcall TAddFloatForm::TAddFloatForm(TComponent *Owner, pp_params p)
  ...
  if (m_pp_params == pp_dual_h) {
  ...
  } else if (m_pp_params == pp_noise_i) {
    ...
    ;
  } else if (m_pp_params == pp_cm_influence) {
...
bool TAddFloatForm::DrawChannel(_pp_params p) {
  switch (m_pp_params) {
  case pp_dual_h:
    return (p == pp_dual_h) || (p == pp_dual_v);
  case pp_noise_i:
    return (p == pp_noise_i) || (p == pp_noise_g) || (p == pp_noise_f);
  case pp_cm_influence:
    return (p == pp_cm_influence);
  default:
    R_ASSERT(0);
  }
  return false;
