...
__fastcall TAddColorForm::TAddColorForm(TComponent *Owner, _pp_params p)
  ...
  if (m_pp_params == pp_base_color) {
  ...
  } else if (m_pp_params == pp_add_color) {
    RedValue->MinValue = -1.0f;
    GreenValue->MinValue = -1.0f;
    BlueValue->MinValue = -1.0f;
    ...
    GreenValue->MaxValue = 1.0f;
    BlueValue->MaxValue = 1.0f;
  } else if (m_pp_params == pp_gray_color) {
...
/*
{
    if (m_bLocked) return; if (MainForm->PointList->ItemIndex == -1) 	return;

    CPostProcessParam* cparam 			= MainForm->m_Animator->GetParam
(GetTimeChannel());

    float time_prev						=
cparam->get_key_time(MainForm->PointList->ItemIndex); cparam->delete_value
(time_prev);
    ...
    {
        cparam 						=
MainForm->m_Animator->GetParam (pp_gray_value); cparam->delete_value
(time_prev); cparam->add_value 			(time, IntensityValue->Value,
0);
    }
...
void __fastcall TAddColorForm::TimeValueExit(TObject *Sender) {
  ...
  if (spin->Color == clLime) {
    ...
    CPostProcessParam *cparam =
        MainForm->m_Animator->GetParam(GetTimeChannel());
...
bool TAddColorForm::DrawChannel(_pp_params p) {
  switch (m_pp_params) {
  case pp_add_color:
    return (p == pp_add_color);
  case pp_base_color:
    return (p == pp_base_color);
  case pp_gray_color:
    return (p == pp_gray_color) || (p == pp_gray_value);
  default:
    R_ASSERT(0);
  }
  return false;
...
void __fastcall TAddColorForm::ColorClick(TObject *Sender) {
  ...
  ColorDialog->Color =
      color_rgba((BlueValue->Value - BlueValue->MinValue) / kl,
                 (GreenValue->Value - GreenValue->MinValue) / kl,
                 (RedValue->Value - RedValue->MinValue) / kl, 0.0f);
