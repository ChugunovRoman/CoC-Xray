...
(s_RP			);
                        Fvector o_Position, o_Angle; tNetPacket.r_vec3
(o_Position		); tNetPacket.r_vec3			(o_Angle
); u16							RespawnTime, ID;
...
void MultipacketSender::SendPacket(const void *packet_data, u32 packet_sz,
                                   u32 flags, u32 timeout) {
  _buf_cs.Enter();
...
void MultipacketSender::_FlushSendBuffer(u32 timeout, Buffer *buf) {
  ...
  if (buf->buffer.B.count) {
    ...
    comp_sz =
        Compressor.Compress(packet_data + sizeof(MultipacketHeader),
...
void MultipacketReciever::RecievePacket(const void *packet_data, u32 packet_sz,
                                        u32 param) {
  MultipacketHeader *header = (MultipacketHeader *)packet_data;
