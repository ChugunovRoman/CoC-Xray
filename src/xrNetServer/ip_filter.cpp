...
struct subnet_comparator
    : public std::binary_function<subnet_item *, subnet_item *, bool> {
...
u32 ip_filter::load() {
  ...
  for (u32 i = 0, line_count = ini.line_count(SUBNET_LIST_SECT_NAME);
    ...
    unsigned int parsed_params =
        sscanf_s(address, "%u.%u.%u.%u/%u", &parse_data[0], &parse_data[1],
    ...
    tmp_item->subnet_ip.data = parse_data[3] | (parse_data[2] << 8) |
                               (parse_data[1] << 16) | (parse_data[0] << 24);
