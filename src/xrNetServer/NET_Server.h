...
struct XRNETSERVER_API ip_address {
  ...
  bool operator==(const ip_address &other) const {
    return (m_data.data == other.m_data.data) ||
           ((m_data.a1 == other.m_data.a1) && (m_data.a2 == other.m_data.a2) &&
...
public:
  ...
  virtual void SendTo_Buf(ClientID ID, void *data, u32 size,
                          u32 dwFlags = DPNSEND_GUARANTEED, u32 dwTimeout = 0);
  virtual void Flush_Clients_Buffers();

  void SendTo(ClientID ID, NET_Packet &P, u32 dwFlags = DPNSEND_GUARANTEED,
              u32 dwTimeout = 0);
  void SendBroadcast_LL(ClientID exclude, void *data, u32 size,
  ...
  // IC u32					client_Count		()			{
  // return net_Players.size(); } IC IClient* client_Get (u32 num)	{ return
  // net_Players[num]; }

  // IC u32					disconnected_client_Count		()			{
  // return net_Players_disconnected.size(); } IC IClient*
  // disconnected_client_Get			(u32 num)	{ return
  // net_Players_disconnected[num]; }
