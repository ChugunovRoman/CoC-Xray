...
void NET_Compressor::start_encoding		( BYTE* dest, u32 header_size )
{
        dest			+=	header_size-1;
        RNGC.low		=	0;				// Full
code range RNGC.range		=	PPM_TOP_VALUE;
    RNGC.buffer		=	0;
...
void NET_Compressor::Initialize	()
{
        ...
        if( strstr(Core.Params,"-dump_traffic") )
        {
                OriginalTrafficDump     = fopen( "x:/network_out_original.dat",
"wb" ); CompressedTrafficDump   = fopen( "x:/network_out_compressed.dat", "wb"
);
        }
#endif // DEBUG
...
u16 NET_Compressor::Compress(BYTE *dest, const u32 &dest_size, BYTE *src,
                             const u32 &count) {
...
u16 NET_Compressor::Decompress(BYTE *dest, const u32 &dest_size, BYTE *src,
                               const u32 &count) {
  VERIFY(dest);
  VERIFY(src);
  VERIFY(count);
