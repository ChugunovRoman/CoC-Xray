...
void ip_address::set(LPCSTR src_string) {
  u32 buff[4];
  int cnt =
      sscanf(src_string, "%d.%d.%d.%d", &buff[0], &buff[1], &buff[2], &buff[3]);
  if (cnt == 4) {
...
xr_string ip_address::to_string() const {
  string128 res;
  xr_sprintf(res, sizeof(res), "%d.%d.%d.%d", m_data.a1, m_data.a2, m_data.a3,
             m_data.a4);
  return res;
}
...
static HRESULT WINAPI Handler(PVOID pvUserContext, DWORD dwMessageType,
                              PVOID pMessage) {
...
IPureServer::EConnect IPureServer::Connect(LPCSTR options,
  ...
  // sertanly we can use game_descr structure for determinig level_name, but for
  // backward compatibility we save next line...
  ...
  if (!psNET_direct_connect) {
    ...
    HRESULT CoCreateInstanceRes =
        CoCreateInstance(CLSID_DirectPlay8Server, NULL, CLSCTX_INPROC_SERVER,
    ...
    CHK_DX(
        NET->SetServerInfo(&dpPlayerInfo, NULL, NULL, DPNSETSERVERINFO_SYNC));

    // Set server/session description
    WCHAR SessionNameUNICODE[4096];
    CHK_DX(MultiByteToWideChar(CP_ACP, 0, session_name, -1, SessionNameUNICODE,
                               4096));
    // Set server/session description
    ...
    // Device Address
    net_Address_device = NULL;
    CHK_DX(CoCreateInstance(CLSID_DirectPlay8Address, NULL,
                            CLSCTX_INPROC_SERVER, IID_IDirectPlay8Address,
                            (LPVOID *)&net_Address_device));
    CHK_DX(net_Address_device->SetSP(
        bSimulator ? &CLSID_NETWORKSIMULATOR_DP8SP_TCPIP : &CLSID_DP8SP_TCPIP));
    ...
    while (HostSuccess != S_OK) {
      CHK_DX(net_Address_device->AddComponent(
          DPNA_KEY_PORT, &psNET_Port, sizeof(psNET_Port), DPNA_DATATYPE_DWORD));
...
HRESULT IPureServer::net_Handler(u32 dwMessageType, PVOID pMessage) {
  ...
  case DPN_MSGID_CREATE_PLAYER: {
    ...
    // string64			cname;
    // CHK_DX( WideCharToMultiByte( CP_ACP, 0, Pinfo->pwszName, -1, cname,
    // sizeof(cname) , 0, 0 ) );
  ...
  case DPN_MSGID_INDICATE_CONNECT: {
    ...
    // first connected client is SV_Client so if it is NULL then this server
...
void IPureServer::SendTo_Buf(ClientID id, void *data, u32 size, u32 dwFlags,
                             u32 dwTimeout) {
...
void IPureServer::SendTo_LL(ClientID ID /*DPNID ID*/, void *data, u32 size,
                            u32 dwFlags, u32 dwTimeout) {
  //	if (psNET_Flags.test(NETFLAG_LOG_SV_PACKETS))
...
void IPureServer::SendBroadcast_LL(ClientID exclude, void *data, u32 size,
                                   u32 dwFlags) {
...
bool IPureServer::DisconnectAddress(const ip_address &Address, LPCSTR reason) {
  ...
  struct ToDisconnectFillerFunctor {
    ...
    ToDisconnectFillerFunctor(IPureServer *owner,
        ...
        : m_owner(owner), dest(dest_disconnect),
...
bool IPureServer::GetClientAddress(IDirectPlay8Address *pClientAddress,
  ...
  if (pPort != NULL) {
    ...
    CHK_DX(pClientAddress->GetComponentByName(DPNA_KEY_PORT, &dwPort,
                                              &dwPortSize, &dwPortDataType));
    *pPort = dwPort;
  };
...
bool IPureServer::GetClientAddress(ClientID ID, ip_address &Address,
                                   DWORD *pPort) {
