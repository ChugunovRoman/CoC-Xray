...
const GUID CLSID_DP8SP_IPX = {0x53934290,
                              0x628d,
                              0x11d2,
...
BOOL IPureClient::Connect(LPCSTR options) {
  ...
  if (!psNET_direct_connect) {
    ...
    HRESULT CoCreateInstanceRes =
        CoCreateInstance(CLSID_DirectPlay8Client, NULL, CLSCTX_INPROC_SERVER,
    ...
    // Device Address
    net_Address_device = NULL;
    R_CHK(CoCreateInstance(CLSID_DirectPlay8Address, NULL, CLSCTX_INPROC_SERVER,
    ...
    R_CHK(MultiByteToWideChar(CP_ACP, 0, server_name, -1, ServerNameUNICODE,
                              256));
    ...
    R_CHK(net_Address_server->AddComponent(
        DPNA_KEY_PORT, &psSV_Port, sizeof(psSV_Port), DPNA_DATATYPE_DWORD));
    ...
    R_CHK(MultiByteToWideChar(CP_ACP, 0, user_name_str, -1, ClientNameUNICODE,
                              256));
    ...
    if (stricmp(server_name, "localhost") == 0) {
      ...
      while (res != S_OK) {
        R_CHK(net_Address_device->AddComponent(
            DPNA_KEY_PORT, &c_port, sizeof(c_port), DPNA_DATATYPE_DWORD));
      ...
      if (dpServerDesc->pwszSessionName) {
        ...
        R_CHK(WideCharToMultiByte(CP_ACP, 0, dpServerDesc->pwszSessionName, -1,
                                  dpSessionName, sizeof(dpSessionName), 0, 0));
      ...
      while (res != S_OK && c_port <= END_PORT) {
        ...
        res =
            NET->EnumHosts(&dpAppDesc, // pApplicationDesc - for unknown reason
...
HRESULT IPureClient::net_Handler(u32 dwMessageType, PVOID pMessage) {
  ...
  case DPN_MSGID_ENUM_HOSTS_RESPONSE: {
    ...
    if (!bHostRegistered) {
      ...
      // Copy the Host Address
      R_CHK(
          pEnumHostsResponseMsg->pAddressSender->Duplicate(&NODE.pHostAddress));
      ...
      if (pDesc->pwszSessionName) {
        string4096 dpSessionName;
        R_CHK(WideCharToMultiByte(CP_ACP, 0, pDesc->pwszSessionName, -1,
                                  dpSessionName, sizeof(dpSessionName), 0, 0));
        NODE.dpSessionName = (char *)(&dpSessionName[0]);
      }
...
void IPureClient::OnMessage(void *data, u32 size) {
  ...
  P->timeReceive =
      timeServer_Async(); // TimerAsync				(device_timer);
...
void IPureClient::SendTo_LL(void *data, u32 size, u32 dwFlags, u32 dwTimeout) {
  ...
  DPNHANDLE hAsync = 0;
  HRESULT hr =
      NET->Send(&desc, 1, dwTimeout, 0, &hAsync, dwFlags | DPNSEND_COALESCE);
...
bool IPureClient::GetServerAddress(ip_address &pAddress, DWORD *pPort) {
  *pPort = 0;
  ...
  CHK_DX(net_Address_server->GetComponentByName(
      DPNA_KEY_HOSTNAME, wstrHostname, &dwHostNameSize, &dwHostNameDataType));
  ...
  //*)*pHostEnt->h_addr_list).s_net;
  //.	pAddress[1]				= (char)(*(struct in_addr
  //*)*pHostEnt->h_addr_list).s_host;
  //.	pAddress[2]				= (char)(*(struct in_addr
  //*)*pHostEnt->h_addr_list).s_lh;
  //.	pAddress[3]				= (char)(*(struct in_addr
  //*)*pHostEnt->h_addr_list).s_impno;

  DWORD dwPort = 0;
  DWORD dwPortSize = sizeof(dwPort);
  DWORD dwPortDataType = DPNA_DATATYPE_DWORD;
  CHK_DX(net_Address_server->GetComponentByName(DPNA_KEY_PORT, &dwPort,
