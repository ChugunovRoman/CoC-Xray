    ...
    if (str[0] && (_Trim(str)[0] == '#') && strstr(str, "#include")) {
      ...
      if (_GetItem(str, 1, inc_name, '"')) {
        ...
        if (inc_name == strstr(inc_name, "ui\\")) {
          shared_str fn =
              xml->correct_file_name("ui", strchr(inc_name, '\\') + 1);
          string_path buff;
        ...
        if (!I) {
          string1024 str;
          xr_sprintf(
              str, "XML file[%s] parsing failed. Can't find include file:[%s]",
              path, inc_name);
          R_ASSERT2(false, str);
        }
...
XML_NODE *CXml::NavigateToNode(XML_NODE *start_node, LPCSTR path,
                               int node_index) {
...
LPCSTR CXml::Read(XML_NODE *start_node, LPCSTR path, int index,
...
int CXml::ReadInt(XML_NODE *start_node, LPCSTR path, int index,
                  int default_int_val) {
...
float CXml::ReadFlt(XML_NODE *start_node, LPCSTR path, int index,
...
LPCSTR CXml::ReadAttrib(LPCSTR path, int index, LPCSTR attrib,
                        LPCSTR default_str_val) {
  XML_NODE *node = NavigateToNode(path, index);
  LPCSTR result = ReadAttrib(node, attrib, default_str_val);
  return result;
...
int CXml::ReadAttribInt(LPCSTR path, int index, LPCSTR attrib,
                        int default_int_val) {
...
float CXml::ReadAttribFlt(LPCSTR path, int index, LPCSTR attrib,
                          float default_flt_val) {
  LPCSTR result_str = ReadAttrib(path, index, attrib, NULL);
...
float CXml::ReadAttribFlt(XML_NODE *start_node, LPCSTR path, int index,
                          LPCSTR attrib, float default_flt_val) {
  LPCSTR result_str = ReadAttrib(start_node, path, index, attrib, NULL);
...
float CXml::ReadAttribFlt(XML_NODE *node, LPCSTR attrib,
                          float default_flt_val) {
  LPCSTR result_str = ReadAttrib(node, attrib, NULL);
...
XML_NODE *CXml::SearchForAttribute(LPCSTR path, int index, LPCSTR tag_name,
                                   LPCSTR attrib, LPCSTR attrib_value_pattern) {
  XML_NODE *start_node = NavigateToNode(path, index);
  XML_NODE *result =
      SearchForAttribute(start_node, tag_name, attrib, attrib_value_pattern);
  return result;
}

XML_NODE *CXml::SearchForAttribute(XML_NODE *start_node, LPCSTR tag_name,
                                   LPCSTR attrib, LPCSTR attrib_value_pattern) {
  while (start_node) {
...
LPCSTR CXml::CheckUniqueAttrib(XML_NODE *start_node, LPCSTR tag_name,
                               LPCSTR attrib_name) {
...
BOOL APIENTRY DllMain(HANDLE hModule, u32 ul_reason_for_call,
                      LPVOID lpReserved) {
