...
void _initialize_cpu(void) {
  ...
  if (CPU::ID.feature & _CPU_FEATURE_3DNOWEXT)
...
void thread_spawn(thread_t *entry, const char *name, unsigned stack,
                  void *arglist) {
...
void spline2(float t, Fvector *p, Fvector *ret) {
  ...
  ret->x =
      (p[0].x * m[0] + p[1].x * m[1] + p[2].x * m[2] + p[3].x * m[3]) / 6.0f;
  ret->y =
      (p[0].y * m[0] + p[1].y * m[1] + p[2].y * m[2] + p[3].y * m[3]) / 6.0f;
  ret->z =
      (p[0].z * m[0] + p[1].z * m[1] + p[2].z * m[2] + p[3].z * m[3]) / 6.0f;
}
