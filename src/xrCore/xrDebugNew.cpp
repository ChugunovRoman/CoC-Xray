...
void xrDebug::gather_info(const char *expression, const char *description,
...
void xrDebug::backend(const char *expression, const char *description,
  ...
  buffer += xr_sprintf(
      buffer, sizeof(assertion_info) - u32(buffer - &assertion_info[0]),
      "Press CONTINUE to continue execution and ignore all the errors of this "
      "type%s%s",
      endline, endline);
#endif // USE_OWN_ERROR_MESSAGE_WINDOW
...
void xrDebug::error(long hr, const char *expr, const char *e2, const char *file,
...
void xrDebug::fail(const char *e1, const std::string &e2, const char *file,
                   int line, const char *function, bool &ignore_always) {
...
void xrDebug::fail(const char *e1, const char *e2, const char *e3,
                   const char *e4, const char *file, int line,
                   const char *function, bool &ignore_always) {
...
void xrDebug::soft_fail(LPCSTR e1, const std::string &e2, LPCSTR file, int line,
                        LPCSTR function) {
  Msg("! VERIFY_FAILED: %s[%d] {%s}  %s %s", file, line, function, e1,
      e2.c_str());
}
void xrDebug::soft_fail(LPCSTR e1, LPCSTR e2, LPCSTR file, int line,
                        LPCSTR function) {
  Msg("! VERIFY_FAILED: %s[%d] {%s}  %s %s", file, line, function, e1, e2);
}
void xrDebug::soft_fail(LPCSTR e1, LPCSTR e2, LPCSTR e3, LPCSTR file, int line,
                        LPCSTR function) {
  Msg("! VERIFY_FAILED: %s[%d] {%s}  %s %s %s", file, line, function, e1, e2,
      e3);
}
void xrDebug::soft_fail(LPCSTR e1, LPCSTR e2, LPCSTR e3, LPCSTR e4, LPCSTR file,
                        int line, LPCSTR function) {
  Msg("! VERIFY_FAILED: %s[%d] {%s}  %s %s %s %s", file, line, function, e1, e2,
      e3, e4);
}
void xrDebug::soft_fail(LPCSTR e1, LPCSTR e2, LPCSTR e3, LPCSTR e4, LPCSTR e5,
                        LPCSTR file, int line, LPCSTR function) {
  Msg("! VERIFY_FAILED: %s[%d] {%s}  %s %s %s %s %s", file, line, function, e1,
      e2, e3, e4, e5);
}
//-AVO

void __cdecl xrDebug::fatal(const char *file, int line, const char *function,
                            const char *F, ...) {
  string1024 buffer;
...
int out_of_memory_handler(size_t size) {
  if (g_full_memory_stats_callback)
    ...
    Msg("* [x-ray]: process heap[%u K]", process_heap / 1024,
        process_heap / 1024);
    Msg("* [x-ray]: economy: strings[%d K], smem[%d K]", eco_strings / 1024,
        eco_smem);
  }
...
void save_mini_dump(_EXCEPTION_POINTERS *pExceptionInfo) {
  ...
  if (GetModuleFileName(NULL, szDbgHelpPath, _MAX_PATH)) {
    ...
    if (pSlash) {
      xr_strcpy(pSlash + 1, sizeof(szDbgHelpPath) - (pSlash - szDbgHelpPath),
                "DBGHELP.DLL");
    ...
    if (pDump) {
      ...
      if (hFile != INVALID_HANDLE_VALUE) {
        ...
        MINIDUMP_TYPE dump_flags = MINIDUMP_TYPE(
            MiniDumpNormal | MiniDumpFilterMemory | MiniDumpScanMemory);

        BOOL bOK = pDump(GetCurrentProcess(), GetCurrentProcessId(), hFile,
                         dump_flags, &ExInfo, NULL, NULL);
        if (bOK) {
          xr_sprintf(szScratch, "Saved dump file to '%s'", szDumpPath);
          szResult = szScratch;
          // retval = EXCEPTION_EXECUTE_HANDLER;
        } else {
          xr_sprintf(szScratch, "Failed to save dump file to '%s' (error %d)",
                     szDumpPath, GetLastError());
          szResult = szScratch;
        }
        ::CloseHandle(hFile);
      } else {
        xr_sprintf(szScratch, "Failed to create dump file '%s' (error %d)",
                   szDumpPath, GetLastError());
        szResult = szScratch;
      }
    } else {
...
LONG WINAPI UnhandledFilter(_EXCEPTION_POINTERS *pExceptionInfo) {
  ...
  *pExceptionInfo->ContextRecord = save;
  ...
  MessageBox(GetTopWindow(NULL),
             "Unhandled exception occured. See log for details", "Fatal Error",
             MB_OK | MB_ICONERROR | MB_SYSTEMMODAL);
  TerminateProcess(GetCurrentProcess(), 1);
...
void _terminate() {
  ...
  LPCSTR endline = "\r\n";
  LPSTR buffer = assertion_info + xr_strlen(assertion_info);
  buffer += xr_sprintf(buffer, sizeof(buffer), "Press OK to abort execution%s",
                       endline);
...
static void invalid_parameter_handler(const wchar_t *expression,
  ...
  Debug.backend(expression_, "invalid parameter", 0, 0, file_, line, function_,
                ignore_always);
}
