...
void CLocatorAPI::_initialize(u32 flags, LPCSTR target_folder,
  ...
  if (m_Flags.is(flTargetFolderOnly)) {
    ...
    R_ASSERT3(F,
              "Can't open file:", (fs_fname && fs_fname[0]) ? fs_fname : FSLTX);
    // append all pathes
    string_path buf;
    ...
    while (!F->eof()) {
      ...
      FS_Path *P =
          xr_new<FS_Path>((p_it != pathes.end()) ? p_it->second->m_Path : root,
...
void ProcessOne(LPCSTR path, _finddata_t &F, bool root_only, TOnFind on_find_cb,
                void *data) {
  string_path N;
  strcpy(N, path);
  strcat(N, F.name);
...
IReader *CLocatorAPI::r_open(LPCSTR path, LPCSTR _fname) {
  ...
  if (R && m_Flags.is(flBuildCopy | flReady)) {
    ...
    FS_Path *P;
    if (source_name ==
            strstr(source_name, (P = get_path("$server_root$"))->m_Path) ||
        source_name ==
            strstr(source_name, (P = get_path("$server_data_root$"))->m_Path)) {
