...
public:
  ...
  struct archive {
    ...
    u32 vfs_idx;
    archive()
        : hSrcFile(NULL), hSrcMap(NULL), header(NULL), size(0),
          vfs_idx(u32(-1)) {}
    void open();
    void close();
  };
...
private:
  ...
  template <typename T>
  void file_from_cache(T *&R, LPSTR fname, const u32 &fname_size,
  ...
  bool check_for_file(LPCSTR path, LPCSTR _fname, string_path &fname,
                      const file *&desc);
...
public:
  ...
  xr_vector<LPSTR> *file_list_open(LPCSTR initial, LPCSTR folder,
  ...
  FS_Path *append_path(LPCSTR path_alias, LPCSTR root, LPCSTR add,
                       BOOL recursive);
  LPCSTR update_path(string_path &dest, LPCSTR initial, LPCSTR src);

  int file_list(FS_FileSet &dest, LPCSTR path, u32 flags = FS_ListFiles,
                LPCSTR mask = 0);
