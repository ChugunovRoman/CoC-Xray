...
const lzo_byte __lzo_copyright[] =
    ...
    "Copyright (C) 1996, 1997, 1998, 1999, 2000, 2001, 2002 Markus Franz Xaver "
    "Johannes Oberhumer\n"
    ...
    " by Watcom C " _LZO_MEXPAND(__WATCOMC__)
                                                     ...
                                                     "$Copyright: LZO (C) "
                                                     "1996, 1997, 1998, 1999, "
                                                     "2000, 2001, 2002 Markus "
                                                     "Franz Xaver Johannes "
                                                     "Oberhumer $\n";
#endif
...
LZO_PUBLIC(int)
__lzo_init2(unsigned v, int s1, int s2, int s3, int s4, int s5, int s6, int s7,
            int s8, int s9) {
  int r;
...
#define LZO_CHECK_MPOS_NON_DET(m_pos, m_off, in, ip, max_offset)               \
  (BOUNDS_CHECKING_OFF_IN_EXPR(                                                \
      (PTR_LT(m_pos, in) || (m_off = (lzo_moff_t)PTR_DIFF(ip, m_pos)) <= 0 ||  \
       m_off > max_offset)))
...
static lzo_uint do_compress(const lzo_byte *in, lzo_uint in_len, lzo_byte *out,
...
LZO_PUBLIC(int)
DO_COMPRESS(const lzo_byte *in, lzo_uint in_len, lzo_byte *out,
            lzo_uintp out_len, lzo_voidp wrkmem) {
  lzo_byte *op = out;
  lzo_uint t;
...
LZO_PUBLIC(int)
DO_DECOMPRESS(const lzo_byte *in, lzo_uint in_len, lzo_byte *out,
              lzo_uintp out_len, lzo_voidp wrkmem)
#endif
{
