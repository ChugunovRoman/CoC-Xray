...
static void __stdcall out_of_memory(mspace const space,
  ...
  Debug.fatal(DEBUG_INFO, "not enough memory for arena [%s]",
              allocator->get_arena_id());
}

doug_lea_allocator::doug_lea_allocator(void *arena, u32 arena_size,
                                       LPCSTR arena_id)
    : m_arena_id(arena_id) {
  VERIFY(m_arena_id);

  if (arena && arena_size)
    m_dl_arena = create_mspace_with_base(arena, arena_size, 0, &out_of_memory,
                                         (void *)this);
  else
