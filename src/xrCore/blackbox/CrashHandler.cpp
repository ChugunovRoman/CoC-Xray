...
LPCTSTR __stdcall GetFaultReason(EXCEPTION_POINTERS *pExPtrs) {
  ...
  __try {
    ...
    iCurr +=
        BSUGetModuleBaseName(GetCurrentProcess(), NULL, g_szBuff, BUFF_SIZE);

    iCurr += wsprintf(g_szBuff + iCurr, _T ( " caused an " ));

    dwTemp =
        (DWORD)ConvertSimpleException(pExPtrs->ExceptionRecord->ExceptionCode);
    ...
    if (NULL == dwTemp) {
      ...
      iCurr += BSUGetModuleBaseName(GetCurrentProcess(), (HINSTANCE)dwTemp,
                                    g_szBuff + iCurr, BUFF_SIZE - iCurr);
    }

#ifdef _WIN64
    iCurr += wsprintf(g_szBuff + iCurr, _T ( " at %016X" ),
    ...
    if (TRUE ==
      ...
      if ((int)dwTemp > ((BUFF_SIZE - iCurr) - (MAX_SYM_SIZE + 50))) {
        ...
        if (dwDisp > 0) {
          iCurr += wsprintf(g_szBuff + iCurr, _T ( "%s()+%04d byte(s)" ),
                            pSym->Name, dwDisp);
        } else {
    ...
    if (TRUE == InternalSymGetLineFromAddr(
                    (HANDLE)GetCurrentProcessId(),
                    (DWORD)pExPtrs->ExceptionRecord->ExceptionAddress, &dwDisp,
                    &g_stLine)) {
      iCurr += wsprintf(g_szBuff + iCurr, _T ( ", " ));
      ...
      if ((int)dwTemp > (BUFF_SIZE - iCurr - MAX_PATH - 50)) {
        ...
        if (dwDisp > 0) {
          iCurr += wsprintf(g_szBuff + iCurr, _T("%s, line %04d+%04d byte(s)"),
                            g_stLine.FileName, g_stLine.LineNumber, dwDisp);
        } else {
          iCurr += wsprintf(g_szBuff + iCurr, _T ( "%s, line %04d"),
...
BOOL __stdcall CH_ReadProcessMemory(HANDLE, LPCVOID lpBaseAddress,
...
#define CH_MACHINE IMAGE_FILE_MACHINE_I386
    ...
    BOOL bSWRet =
        StackWalk(CH_MACHINE, (HANDLE)GetCurrentProcessId(), GetCurrentThread(),
                  &g_stFrame, pExPtrs->ContextRecord,
...
#ifdef _WIN64
    iCurr +=
        wsprintf(g_szBuff + iCurr, _T ( "0x%016X" ), g_stFrame.AddrPC.Offset);
#else
    ...
    if (GSTSO_MODULE == (dwOpts & GSTSO_MODULE)) {
      ...
      ASSERT(iCurr < (BUFF_SIZE - MAX_PATH));
      iCurr += BSUGetModuleBaseName(GetCurrentProcess(), (HINSTANCE)dwModBase,
                                    g_szBuff + iCurr, BUFF_SIZE - iCurr);
    }
    ...
    if (GSTSO_SYMBOL == (dwOpts & GSTSO_SYMBOL)) {
      ...
      if (TRUE == SymGetSymFromAddr((HANDLE)GetCurrentProcessId(),
        ...
        if (dwTemp > (DWORD)(BUFF_SIZE - iCurr - (MAX_SYM_SIZE + 50))) {
          ...
          if (dwDisp > 0) {
            iCurr +=
                wsprintf(g_szBuff + iCurr, _T( "%s()"), pSym->Name, dwDisp);
          } else {
            iCurr += wsprintf(g_szBuff + iCurr, _T ( "%s" ), pSym->Name);
          }
        }
    ...
    if (GSTSO_SRCLINE == (dwOpts & GSTSO_SRCLINE)) {
      ...
      if (TRUE == InternalSymGetLineFromAddr((HANDLE)GetCurrentProcessId(),
                                             g_stFrame.AddrPC.Offset, &dwDisp,
                                             &g_stLine)) {
        iCurr += wsprintf(g_szBuff + iCurr, _T ( ", " ));
        ...
        if (dwTemp > (DWORD)(BUFF_SIZE - iCurr - (MAX_PATH + 50))) {
          ...
          if (dwDisp > 0) {
            iCurr += wsprintf(g_szBuff + iCurr, _T("%s, %d"), g_stLine.FileName,
                              g_stLine.LineNumber, dwDisp);
          } else {
            iCurr += wsprintf(g_szBuff + iCurr, _T ( "%s, %d" ),
                              g_stLine.FileName, g_stLine.LineNumber);
          }
        }
      }
