...
BOOL __stdcall DiagAssertA(DWORD dwOverrideOpts, LPCSTR szMsg, LPCSTR szFile,
                           DWORD dwLine) {
...
BOOL __stdcall DiagAssertW(DWORD dwOverrideOpts, LPCWSTR szMsg, LPCSTR szFile,
                           DWORD dwLine) {
  // First, save off the last error value.
  DWORD dwLastError = GetLastError();
...
BOOL __stdcall DiagAssertVB(DWORD dwOverrideOpts, BOOL bAllowHalts,
                            LPCSTR szMsg) {
...
BOOL __stdcall RealAssert(DWORD dwOverrideOpts, LPCSTR szMsg,
                          BOOL bAllowHalts) {
...
static DWORD __stdcall GetModBase(HANDLE hProcess, DWORD dwAddr) {
  ...
  if (g_cSym.SymGetModuleInfo(dwAddr, &stIHM)) {
    ...
    if (0 != VirtualQueryEx(hProcess, (LPCVOID)dwAddr, &stMBI, sizeof(stMBI))) {
      ...
      dwNameLen =
          GetModuleFileName((HINSTANCE)stMBI.AllocationBase, szFile, MAX_PATH);
      ...
      DWORD dwRet =
#endif
          g_cSym.SymLoadModule(hFile, (dwNameLen ? szFile : NULL), NULL,
                               (DWORD)stMBI.AllocationBase, 0);
#ifdef _DEBUG
      if (0 == dwRet) {
...
static DWORD ConvertAddress(DWORD dwAddr, LPTSTR szOutBuff) {
  ...
  if (0 != g_cSym.SymGetSymFromAddr(dwAddr, &dwDisp, pIHS)) {
    if (0 == dwDisp) {
      ...
      pCurrPos +=
          wsprintf(pCurrPos, _T ( "%s + %d bytes" ), pIHS->Name, dwDisp);
    }
...
extern BOOL __stdcall CH_ReadProcessMemory(HANDLE, LPCVOID lpBaseAddress,
