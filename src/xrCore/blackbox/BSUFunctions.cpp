...
static DWORD __stdcall Win95GetModuleBaseName(HANDLE hProcess, HMODULE hModule,
...
DWORD __stdcall BSUGetModuleBaseName(HANDLE hProcess, HMODULE hModule,
...
static DWORD __stdcall Win95GetModuleBaseName(HANDLE /*hProcess*/,
                                              HMODULE hModule, LPSTR lpBaseName,
                                              DWORD nSize) {
...
DWORD __stdcall BSUSymInitialize(DWORD dwPID, HANDLE hProcess,
  ...
  if ((TRUE == IsNT()) || (FALSE == fInvadeProcess)) {
    ...
    for (UINT uiCurr = 0; uiCurr < dwCount; uiCurr++) {
      ...
      if (FALSE ==
          GetModuleFileName(paMods[uiCurr], szModName, sizeof(szModName))) {
      ...
      if (FALSE == SymLoadModule(hProcess, hFile, szModName, NULL,
                                 (DWORD)paMods[uiCurr], 0)) {
...
DWORD __stdcall BSUGetModuleFileNameEx(DWORD dwPID, HANDLE hProcess,
                                       HMODULE hModule, LPTSTR szFilename,
                                       DWORD nSize) {
  if (TRUE == IsNT()) {
    return (NTGetModuleFileNameEx(dwPID, hProcess, hModule, szFilename, nSize));
  }
  return (
      TLHELPGetModuleFileNameEx(dwPID, hProcess, hModule, szFilename, nSize));
}
