...
typedef void(__stdcall *out_of_memory_handler_type)(
    mspace, out_of_memory_handler_parameter_type, int);
...
extern "C" {
#endif /* __cplusplus */
LONG __cdecl _InterlockedCompareExchange(LPLONG volatile Dest, LONG Exchange,
                                         LONG Comp);
...
extern void *mmap(void *ptr, intptr_t size, intptr_t prot, intptr_t type,
    ...
    if (tbase == CMFAIL) { /* Cope with partial failure */
      if (br != CMFAIL) {  /* Try to use/extend the space we did get */
        if (asize < HALF_MAX_SIZE_T &&
          ...
          size_t esize =
              granularity_align(nb + TOP_FOOT_SIZE + SIZE_T_ONE - asize);
...
unsigned char _BitScanForward(unsigned long *index, unsigned long mask) {
  ...
  *index =
      MultiplyDeBruijnBitPosition[((mask & -(long)mask) * 0x077CB531UL) >> 27];
  return (1);
}
...
unsigned char _BitScanReverse(unsigned long *index, unsigned long mask) {
  ...
  return (_BitScanForward(
      index, 1 << MultiplyDeBruijnBitPosition[(mask * 0x077CB531UL) >> 27]));
}
