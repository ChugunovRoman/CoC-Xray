...
/* Supply definitions of things glibc would normally define for malloc.c
 * beforehand */
...
/* mmap for windows */
// static
void *mmap(void *ptr, INTERNAL_INTPTR_T size, INTERNAL_INTPTR_T prot,
           INTERNAL_INTPTR_T type, INTERNAL_INTPTR_T handle,
  ...
  if ((prot & (PROT_READ | PROT_WRITE)) == (PROT_READ | PROT_WRITE))
    ...
    if (ptr) { /* prot==PROT_NONE also appears to be a euphemism for free */
      ...
      DWORD read = 0;
      for (p = ((char *)ptr) + read;
           read < (DWORD)size && VirtualQuery(p, &mbi, sizeof(mbi));
           read += mbi.RegionSize) {
        if (mbi.State & MEM_COMMIT) {
          ...
          // MEM_DECOMMIT)) 						goto
          // mmap_exit;
          if (!VirtualAlloc((LPVOID)p, mbi.RegionSize, MEM_RESERVE,
                            PAGE_NOACCESS))
            goto mmap_exit;
#ifdef TRACE
          printf("Release %p %d\n", p, mbi.RegionSize);
...
#ifdef TRACE
  printf("%s %p %d %d %d\n", (type & MAP_NORESERVE) ? "Reserve" : "Commit", ptr,
         size, prot, type);
#endif
mmap_exit:
  /* Release spin lock */
