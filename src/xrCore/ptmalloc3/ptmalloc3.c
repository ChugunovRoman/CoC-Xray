...
#define MSPACE_OFFSET                                                          \
  (((offsetof(struct malloc_arena, buf_) + CHUNK_ALIGN_MASK) &                 \
    ~CHUNK_ALIGN_MASK))
...
#define arena_for_chunk(ptr)                                                   \
  (chunk_non_main_arena(ptr)                                                   \
       ? *(struct malloc_arena **)((char *)(ptr) + chunksize(ptr) -            \
...
#define set_non_main_arena(mem, ar_ptr)                                        \
  do {                                                                         \
    ...
    size_t SZ =                                                                \
        chunksize(P) - (is_mmapped(P) ? sizeof(struct malloc_arena *)          \
                                      : (FOOTER_OVERHEAD - SIZE_T_SIZE));      \
    assert((unsigned long)((char *)(P) + SZ) %                                 \
               sizeof(struct malloc_arena *) ==                                \
           0);                                                                 \
...
static struct malloc_arena *arena_get2(struct malloc_arena *a_tsd,
                                       size_t size) {
...
int public_pMEMALIGn(void **memptr, size_t alignment, size_t size) {
  ...
  if (alignment % sizeof(void *) != 0 ||
      !my_powerof2(alignment / sizeof(void *)) != 0 || alignment == 0)
    return EINVAL;
...
void initialize_virtual_alloc_arena(
    struct virtual_alloc_arena *result, void *buffer, size_t buffer_size,
    size_t granularity, char const *arena_id,
