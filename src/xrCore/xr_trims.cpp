...
LPSTR _GetItem(LPCSTR src, int index, LPSTR dst, u32 const dst_size,
               char separator, LPCSTR def, bool trim) {
  LPCSTR ptr;
...
LPSTR _GetItems(LPCSTR src, int idx_start, int idx_end, LPSTR dst,
                char separator) {
  LPSTR n = dst;
  int level = 0;
...
LPSTR _ReplaceItems(LPCSTR src, int idx_start, int idx_end, LPCSTR new_items,
                    LPSTR dst, char separator) {
  LPSTR n = dst;
  int level = 0;
  bool bCopy = true;
  for (LPCSTR p = src; *p != 0; p++) {
...
LPSTR _ReplaceItem(LPCSTR src, int index, LPCSTR new_item, LPSTR dst,
                   char separator) {
  LPSTR n = dst;
  int level = 0;
  bool bCopy = true;
...
#ifdef M_BORLAND
AnsiString &_ReplaceItem(LPCSTR src, int index, LPCSTR new_item,
...
AnsiString &_ReplaceItems(LPCSTR src, int idx_start, int idx_end,
                          LPCSTR new_items, AnsiString &dst, char separator) {
  dst = "";
  int level = 0;
  bool bCopy = true;
  for (LPCSTR p = src; *p != 0; p++) {
...
LPCSTR _GetItems(LPCSTR src, int idx_start, int idx_end, AnsiString &dst,
                 char separator) {
  int level = 0;
  for (LPCSTR p = src; *p != 0; p++) {
    if ((level >= idx_start) && (level < idx_end))
...
LPCSTR _GetItem(LPCSTR src, int index, AnsiString &dst, char separator,
                LPCSTR def, bool trim) {
  LPCSTR ptr;
  ptr = _SetPos(src, index, separator);
  if (ptr)
    _CopyVal(ptr, dst, separator);
...
AnsiString _ListToSequence(const AStringVec &lst) {
  ...
  if (lst.size()) {
    out = lst.front();
    for (AStringVec::const_iterator s_it = lst.begin() + 1; s_it != lst.end();
         s_it++)
      out += AnsiString(",") + (*s_it);
  }
  return out;
...
xr_string _ListToSequence(const SStringVec &lst) {
  ...
  if (lst.size()) {
    out = lst.front();
    for (SStringVec::const_iterator s_it = lst.begin() + 1; s_it != lst.end();
         s_it++)
      out += xr_string(",") + (*s_it);
  }
  return out;
...
LPCSTR _GetItem(LPCSTR src, int index, xr_string &dst, char separator,
                LPCSTR def, bool trim) {
  LPCSTR ptr;
  ptr = _SetPos(src, index, separator);
  if (ptr)
...
shared_str _ListToSequence(const RStringVec &lst) {
  ...
  if (lst.size()) {
    out = *lst.front();
    for (RStringVec::const_iterator s_it = lst.begin() + 1; s_it != lst.end();
         s_it++) {
      out += ",";
      out += **s_it;
    }
