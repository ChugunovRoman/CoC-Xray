...
CFS_PathNotificator::CFS_PathNotificator() : CThread(0) {
  FMutex = CreateMutex(
      NULL, true /* initial owner - must be Release'd by this thread*/, NULL);
  if (FMutex)
...
void CFS_PathNotificator::Execute(void) {
  ...
  for (PathIt it = events.begin(); it != events.end(); it++) {
    Path &P = *it;
    P.FWaitHandle = FindFirstChangeNotification(P.FDirectory.c_str(),
                                                P.bRecurse, FNotifyOptionFlags);
    if (P.FWaitHandle == INVALID_HANDLE_VALUE)
#ifndef __BORLANDC__
  ...
  while (!Terminated) {
    ...
    int Obj = WaitForMultipleObjects(hHandles.size(), &*hHandles.begin(), false,
                                     INFINITE);
