...
struct ray_cache {
  ...
  void set(const Fvector &_start, const Fvector &_dir, const float _range,
           const BOOL _result) {
    start = _start;
    dir = _dir;
    range = _range;
...
struct ray_defs {
  ...
  u32 flags;
  rq_target tgt;
  ray_defs(const Fvector &_start, const Fvector &_dir, float _range, u32 _flags,
           rq_target _tgt) {
    start = _start;
    dir = _dir;
    range = _range;
...
typedef BOOL test_callback(const ray_defs &rd, CObject *object,
                           LPVOID user_data);
}; // namespace collide
#endif
