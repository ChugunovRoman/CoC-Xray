      ...
      while (FALSE == result) {
        ...
        if (ogg_stream_packetout(&o_stream_state, &o_packet) > 0 &&
          ...
          if (d_frame < k_frame) {
            ...
            // AVO: commented out VERIFY to get COC start in Debug - something
            // is wrong with video stream VERIFY((0 != d_frame%key_rate) || (0
            // == d_frame%key_rate) && theora_packet_iskeyframe(&o_packet));
            continue;
          }
          BOOL is_key = theora_packet_iskeyframe(&o_packet);
          // AVO: commented out VERIFY to get COC start in Debug - something is
          // wrong with video stream VERIFY((d_frame != k_frame) || ((d_frame ==
          // k_frame) && is_key));
          // real decode
