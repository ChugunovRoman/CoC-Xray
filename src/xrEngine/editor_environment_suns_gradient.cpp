...
gradient::gradient()
    : m_use(false), m_opacity(.0f), m_radius(.0f), m_shader(""), m_texture("") {
}

void gradient::load(CInifile &config, shared_str const &section) {
  m_use = !!READ_IF_EXISTS(&config, r_bool, section, "gradient", true);
  m_opacity =
      READ_IF_EXISTS(&config, r_float, section, "gradient_opacity", .7f);
  m_radius = READ_IF_EXISTS(&config, r_float, section, "gradient_radius", .9f);
  m_shader = READ_IF_EXISTS(&config, r_string, section, "gradient_shader",
                            "effects\\flare");
  m_texture = READ_IF_EXISTS(&config, r_string, section, "gradient_texture",
                             "fx\\fx_gradient.tga");
}
...
void gradient::fill(manager const &manager, property_holder *holder,
  ...
  properties->add_property(
      "use", "gradient", "this option is resposible for gradient usage", m_use,
      boolean_getter, boolean_setter, property_holder::property_read_write,
  ...
  properties->add_property("opacity", "gradient",
                           ...
                           m_opacity, m_opacity);
  properties->add_property("radius", "gradient",
                           "this option is resposible for gradient radius",
                           m_radius, m_radius);
  properties->add_property(
      "shader", "gradient", "this option is resposible for gradient shader",
  ...
  properties->add_property(
      "texture", "gradient", "this option is resposible for gradient texture",
