...
void gradient::load(CInifile &config, shared_str const &section_id,
                    LPCSTR prefix) {
  string_path temp;
  shader = config.r_string(section_id,
                           strconcat(sizeof(temp), temp, prefix, "_shader"));
  texture = config.r_string(section_id,
                            strconcat(sizeof(temp), temp, prefix, "_texture"));
  fOpacity = config.r_float(section_id,
                            strconcat(sizeof(temp), temp, prefix, "_opacity"));
  fRadius = config.r_fvector2(section_id,
                              strconcat(sizeof(temp), temp, prefix, "_radius"));
  m_pFlare->CreateShader(*shader, *texture);
}

void gradient::save(CInifile &config, shared_str const &section_id,
                    LPCSTR prefix) {
  string_path temp;
  config.w_string(section_id.c_str(),
                  strconcat(sizeof(temp), temp, prefix, "_shader"),
                  shader.c_str());
  config.w_string(section_id.c_str(),
                  strconcat(sizeof(temp), temp, prefix, "_texture"),
                  texture.c_str());
  config.w_float(section_id.c_str(),
                 strconcat(sizeof(temp), temp, prefix, "_opacity"), fOpacity);
  config.w_fvector2(section_id.c_str(),
                    strconcat(sizeof(temp), temp, prefix, "_radius"), fRadius);
}
...
void gradient::fill(::editor::environment::manager &environment, LPCSTR name,
                    LPCSTR description, ::editor::property_holder &holder) {
  ...
  m_property_holder->add_property(
      "opacity", "properties",
      "this option is resposible for thunderbolt gradient opacity", fOpacity,
      fOpacity);
  m_property_holder->add_property(
  ...
  m_property_holder->add_property(
      ...
      shader.c_str(), string_getter, string_setter,
      &*environment.shader_ids().begin(), environment.shader_ids().size(),
