...
property_collection<weather::container_type, weather>::create() {
  using ::editor::environment::weathers::time;
  time *object = xr_new<time>(&m_holder.m_manager, &m_holder,
                              m_holder.generate_unique_id().c_str());
  object->fill(this);
...
void weather::fill(editor::property_holder_collection *collection) {
  ...
  m_property_holder =
      ::ide().create_property_holder(m_id.c_str(), collection, this);
  ...
  m_property_holder->add_property(
      "id", "properties", "this option is resposible for weather identifier",
      m_id.c_str(), string_getter, string_setter);
  m_property_holder->add_property("times", "properties",
...
shared_str weather::unique_id(shared_str const &current,
...
bool weather::try_hours(u32 &hours, u32 &minutes, u32 &seconds,
                        shared_str &result) const {
...
bool weather::try_minutes(u32 &hours, u32 &minutes, u32 &seconds,
                          shared_str &result) const {
...
bool weather::save_time_frame(shared_str const &frame_id, char *buffer,
...
bool weather::paste_time_frame(shared_str const &frame_id, char const *buffer,
  ...
  for (; i != e; ++i) {
    ...
    (*i)->load_from((*temp.sections().begin())->Name, temp,
                    shared_str((*i)->id()));
    return (true);
  }
...
bool weather::add_time_frame(char const *buffer, u32 const &buffer_size) {
  ...
  m_manager.WeatherCycles[m_id].insert(
      m_manager.WeatherCycles[m_id].begin() + index, object);
  return (true);
}
