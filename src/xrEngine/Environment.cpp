...
CEnvironment::CEnvironment() : CurrentEnv(0), m_ambients_config(0) {
  ...
  m_ambients_config = xr_new<CInifile>(
      FS.update_path(file_name, "$game_config$", "environment\\ambients.ltx"),
      TRUE, TRUE, FALSE);
  ...
  m_effects_config = xr_new<CInifile>(
      FS.update_path(file_name, "$game_config$", "environment\\effects.ltx"),
      TRUE, TRUE, FALSE);
  m_suns_config = xr_new<CInifile>(
      FS.update_path(file_name, "$game_config$", "environment\\suns.ltx"), TRUE,
      TRUE, FALSE);
...
#ifndef _EDITOR
  if (m_paused) {
    g_pGameLevel->SetEnvironmentGameTimeFactor(iFloor(fGameTime * 1000.f),
                                               fTimeFactor);
    return;
  }
#endif
...
bool CEnvironment::SetWeatherFX(shared_str name) {
  ...
  if (name.size()) {
    ...
    if (Current[0]->exec_time > Current[1]->exec_time) {
      ...
      current_length =
          (DAY_LENGTH - Current[0]->exec_time) + Current[1]->exec_time;
    ...
    C0->exec_time = NormalizeTime(
        fGameTime -
        ((rewind_tm / (Current[1]->exec_time - fGameTime)) * current_length -
         rewind_tm));
    C1->copy(*Current[1]);
...
void CEnvironment::SelectEnvs(EnvVec *envs, CEnvDescriptor *&e0,
                              CEnvDescriptor *&e1, float gt) {
  EnvIt env = std::lower_bound(envs->begin(), envs->end(), gt, lb_env_pred);
  if (env == envs->end()) {
    e0 = *(envs->end() - 1);
    e1 = envs->front();
...
void CEnvironment::lerp(float &current_weight) {
  ...
  current_weight =
      TimeWeight(fGameTime, Current[0]->exec_time, Current[1]->exec_time);
  // modifiers
  CEnvModifier EM;
  EM.far_plane = 0;
...
void CEnvironment::OnFrame() {
  ...
  shared_str l_id = (current_weight < 0.5f) ? Current[0]->lens_flare_id
                                            : Current[1]->lens_flare_id;
  eff_LensFlare->OnFrame(l_id);
  shared_str t_id =
      (current_weight < 0.5f) ? Current[0]->tb_id : Current[1]->tb_id;
...
SThunderboltDesc *
CEnvironment::thunderbolt_description(CInifile &config,
...
CEnvironment::thunderbolt_collection(CInifile *pIni, CInifile *thunderbolts,
                                     LPCSTR section) {
...
CEnvironment::add_flare(xr_vector<CLensFlareDescriptor *> &collection,
                        shared_str const &id) {
