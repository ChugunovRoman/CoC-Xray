...
engine_impl::engine_impl()
    : m_input_receiver(xr_new<IInputReceiver>()), m_input_captured(false) {}
...
bool engine_impl::on_message(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,
                             LRESULT &result) {
...
void engine_impl::weather(LPCSTR value) {
  ...
  g_pGameLevel->SetEnvironmentGameTimeFactor(iFloor(game_time),
...
void engine_impl::current_weather_frame(LPCSTR frame_id) {
  ...
  for (; i != e; ++i)
    if ((*i)->m_identifier._get() == new_frame_id._get()) {
      ...
      bool set_time = true;
      if (environment.Current[0]->exec_time <
          environment.Current[1]->exec_time) {
      ...
      if (set_time) {
        ...
        g_pGameLevel->SetEnvironmentGameTimeFactor(
            iFloor((*i)->exec_time * 1000.f), environment.fTimeFactor);
      }
      break;
    }
}
...
void engine_impl::track_weather(float const &time) {
  ...
  g_pGamePersistent->Environment().SetGameTime(
      time * 24 * 60 * 60, g_pGamePersistent->Environment().fTimeFactor);
...
property_holder *engine_impl::current_frame_property_holder() {
  ...
  return (((editor::environment::weathers::time &)(*environment.Current[0]))
              .object());
}
...
property_holder *engine_impl::blend_frame_property_holder() {
  ...
  return (((editor::environment::weathers::time &)(*environment.CurrentEnv))
              .object());
}
...
property_holder *engine_impl::target_frame_property_holder() {
  ...
  return (((editor::environment::weathers::time &)(*environment.Current[1]))
              .object());
}
...
bool engine_impl::paste_current_time_frame(char const *buffer,
...
bool engine_impl::paste_target_time_frame(char const *buffer,
                                          u32 const &buffer_size) {
  CEnvironment &environment = g_pGamePersistent->Environment();
  editor::environment::manager &manager =
      dynamic_cast<editor::environment::manager &>(environment);
...
void engine_impl::weather_current_time(char const *time) {
  ...
  g_pGamePersistent->Environment().m_paused = false;
  g_pGamePersistent->Environment().SetGameTime(
      float(hours * 60 * 60 + minutes * 60 + seconds),
