...
sun::sun(manager const &manager, shared_str const &id)
    : m_manager(manager), m_id(id), m_use(false), m_ignore_color(false),
      m_radius(0.f), m_shader(""), m_texture(""), m_property_holder(0) {}
...
void sun::load(CInifile &config) {
  ...
  m_ignore_color =
      !!READ_IF_EXISTS(&config, r_bool, m_id, "sun_ignore_color", false);
  m_radius = READ_IF_EXISTS(&config, r_float, m_id, "sun_radius", .15f);
  m_shader =
      READ_IF_EXISTS(&config, r_string, m_id, "sun_shader", "effects\\sun");
  m_texture =
      READ_IF_EXISTS(&config, r_string, m_id, "sun_texture", "fx\\fx_sun.tga");
}
...
void sun::fill(editor::property_holder_collection *collection) {
  ...
  properties->add_property("id", "common",
                           "this option is resposible for sun identifier",
                           m_id.c_str(), string_getter, string_setter);
  properties->add_property(
      "use", "sun", "this option is resposible for sun usage", m_use, m_use);
  properties->add_property("ignore color", "sun",
                           "this option is resposible for sun ignore color",
                           m_ignore_color, m_ignore_color);
  properties->add_property("radius", "sun",
                           "this option is resposible for sun radius", m_radius,
                           m_radius);
  properties->add_property("shader", "sun",
                           "this option is resposible for sun shader",
                           m_shader.c_str(), m_shader,
  ...
  properties->add_property(
      "texture", "sun", "this option is resposible for sun texture",
