...
public:
  IConsole_Command(LPCSTR N BENCH_SEC_SIGN)
      : cName(N), bEnabled(TRUE), bLowerCaseArgs(TRUE),
  ...
  void InvalidSyntax() {
    ...
    g_SASH.OnConsoleInvalidSyntax(false, "~ Invalid syntax in call to '%s'",
                                  cName);
...
public:
  ...
  virtual void fill_tips(vecTips &tips, u32 mode) {
    TStatus str;
    xr_sprintf(str, sizeof(str), "%3.5f (current) [%3.3f,%3.3f]", *value, min,
               max);
    tips.push_back(str);
...
public:
  ...
  virtual void Info(TInfo &I) {
    xr_sprintf(I, sizeof(I), "vector3 in range [%e,%e,%e]-[%e,%e,%e]", min.x,
