  ...
  for (u32 it = 0; it < Registration.size(); it++) {
    ...
    if (R.OP) {
      ...
      if (!bFoundAndErased) {
        ...
        Msg("SCHEDULER: internal register [%s][%x][%s]",
            *R.Object->shedule_Name(), R.Object, R.RT ? "true" : "false");
#endif // DEBUG_SCHEDULER
        internal_Register(R.Object, R.RT);
      }
#ifdef DEBUG_SCHEDULER
      else
        Msg("SCHEDULER: internal register skipped, because unregister found "
            "[%s][%x][%s]",
...
bool CSheduler::internal_Unregister(ISheduled *O, BOOL RT,
  ...
  if (RT) {
...
#ifdef DEBUG_SCHEDULER
        Msg("SCHEDULER: internal unregister [%s][%x][%s]", "unknown", O,
            "true");
#endif // DEBUG_SCHEDULER
    ...
    for (u32 i = 0; i < Items.size(); i++) {
      if (Items[i].Object == O) {
#ifdef DEBUG_SCHEDULER
        Msg("SCHEDULER: internal unregister [%s][%x][%s]",
            *Items[i].scheduled_name, O, "false");
#endif // DEBUG_SCHEDULER
        Items[i].Object = NULL;
        return (true);
      }
...
#ifdef DEBUG_SCHEDULER
    Msg("SCHEDULER: internal unregister (self unregistering) [%x][%s]", O,
        "false");
#endif // DEBUG_SCHEDULER
...
bool CSheduler::Registered(ISheduled *object) const {
  ...
  if (!count && (m_current_step_obj == object)) {
    VERIFY2(m_processing_now, "trying to unregister self unregistering object "
                              "while not processing now");
    count = 1;
  }
...
void CSheduler::ProcessStep() {
  ...
  for (int i = 0; !Items.empty() && Top().dwTimeForExecute < dwTime; ++i) {
    ...
    if (condition) {
      // Erase element
#ifdef DEBUG_SCHEDULER
      Msg("SCHEDULER: process unregister [%s][%x][%s]", *T.scheduled_name,
          T.Object, "false");
#endif // DEBUG_SCHEDULER
       // if (T.Object)
       // Msg ("0x%08x UNREGISTERS because shedule_Needed() returned
       // false",T.Object); else Msg ("UNREGISTERS unknown object");
      Pop();
      continue;
    }
    ...
    T.Object->shedule_Update(clampr(
        Elapsed, u32(1), u32(_max(u32(T.Object->shedule.t_max), u32(1000)))));
...
#ifdef DEBUG
    // u32 execTime = eTimer.GetElapsed_ms ();
    // VERIFY3 (T.Object->dbg_update_shedule == T.Object->dbg_startframe,
    // "Broken sequence of calls to 'shedule_Update'", _obj_name );
    if (delta_ms > 3 * dwUpdate) {
      // Msg ("! xrSheduler: failed to shedule object [%s] (%dms)", _obj_name,
      // delta_ms );
    }
    // if (execTime> 15) {
...
void CSheduler::Update() {
  ...
  cycles_limit =
      CPU::qpc_freq * u64(iCeil(psShedulerCurrent)) / 1000i64 + cycles_start;
  internal_Registration();
  ...
  for (u32 it = 0; it < ItemsRT.size(); it++) {
    ...
    R_ASSERT(T.Object);
#ifdef DEBUG_SCHEDULER
    Msg("SCHEDULER: process step [%s][%x][true]", *T.Object->shedule_Name(),
        T.Object);
#endif // DEBUG_SCHEDULER
    if (!T.Object->shedule_Needed()) {
#ifdef DEBUG_SCHEDULER
      Msg("SCHEDULER: process unregister [%s][%x][%s]",
          *T.Object->shedule_Name(), T.Object, "false");
#endif // DEBUG_SCHEDULER
