...
CGameFont::CGameFont(LPCSTR section, u32 flags) {
  ...
  Initialize(pSettings->r_string(section, "shader"),
             pSettings->r_string(section, "texture"));
...
#define DI2PX(x)                                                               \
  float(iFloor((x + 1) * float(::Render->getTarget()->get_width()) * 0.5f))
#define DI2PY(y)                                                               \
  float(iFloor((y + 1) * float(::Render->getTarget()->get_height()) * 0.5f))
  ...
  if (wsStr != NULL) {
    ...
    for (u16 i = 1; i <= len; i++) {
      ...
      if (((fCurWidth + fDelta) > fTargetWidth) && // overlength
          ...
          ((i > 1) &&
           (!IsBadEndCharacter(wsStr[i - 1]))) // && // do not stop the string
                                               // on a "bad" character
          // ( ( i > 1 ) && ( ! ( ( IsAlphaCharacter( wsStr[ i - 1 ] ) ) && (
          // IsAlphaCharacter( wsStr[ i ] ) ) ) ) ) // do not split numbers or
          // words
      ) {
        fCurWidth = fDelta;
...
void CGameFont::MasterOut(BOOL bCheckDevice, BOOL bUseCoords, BOOL bScaleCoords,
                          BOOL bUseSkip, float _x, float _y, float _skip,
                          LPCSTR fmt, va_list p) {
...
#define MASTER_OUT(CHECK_DEVICE, USE_COORDS, SCALE_COORDS, USE_SKIP, X, Y,     \
  ...
  {                                                                            \
    va_list p;                                                                 \
    va_start(p, fmt);                                                          \
    MasterOut(CHECK_DEVICE, USE_COORDS, SCALE_COORDS, USE_SKIP, X, Y, SKIP,    \
              FMT, p);                                                         \
    va_end(p);                                                                 \
  }
...
float CGameFont::SizeOf_(const char cChar) {
  return (
      GetCharTC((u16)(u8)(((IsMultibyte() && cChar == ' ')) ? 0 : cChar)).z *
      vInterval.x);
}
