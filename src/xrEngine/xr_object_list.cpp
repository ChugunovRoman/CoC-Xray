...
CObject *CObjectList::FindObjectByName(shared_str name) {
  for (Objects::iterator I = objects_active.begin(); I != objects_active.end();
       I++)
...
CObject *CObjectList::FindObjectByCLS_ID(CLASS_ID cls) {
  {
    Objects::iterator O = std::find_if(objects_active.begin(),
                                       objects_active.end(), fClassEQ(cls));
    if (O != objects_active.end())
      return *O;
  }
  {
    Objects::iterator O = std::find_if(objects_sleeping.begin(),
                                       objects_sleeping.end(), fClassEQ(cls));
...
void CObjectList::SingleUpdate(CObject *O) {
  ...
  if (O->H_Parent() &&
    ...
    Msg("! ERROR: incorrect destroy sequence for object[%d:%s], section[%s], "
        "parent[%d:%s]",
...
void CObjectList::Update(bool bForce) {
  if (!Device.Paused() || bForce) {
    ...
    if (Device.fTimeDelta > EPS_S || bForce) {
      ...
      CObject **objects =
          (CObject **)_alloca(objects_count * sizeof(CObject *));
  ...
  if (!destroy_queue.empty()) {
    // Info
    for (Objects::iterator oit = objects_active.begin();
         oit != objects_active.end(); oit++)
      for (int it = destroy_queue.size() - 1; it >= 0; it--) {
        (*oit)->net_Relcase(destroy_queue[it]);
      }
    for (Objects::iterator oit = objects_sleeping.begin();
         oit != objects_sleeping.end(); oit++)
      for (int it = destroy_queue.size() - 1; it >= 0; it--)
        (*oit)->net_Relcase(destroy_queue[it]);
    ...
    for (int it = destroy_queue.size() - 1; it >= 0; it--) {
      ...
      if (debug_destroy)
        Msg("Destroying object[%x][%x] [%d][%s] frame[%d]",
            dynamic_cast<void *>(O), O, O->ID(), *O->cName(), Device.dwFrame);
#endif // DEBUG
      O->net_Destroy();
      Destroy(O);
    }
...
u32 CObjectList::net_Export(NET_Packet *_Packet, u32 start,
                            u32 max_object_size) {
...
void CObjectList::dump_list(Objects &v, LPCSTR reason) {
  ...
  for (; it != it_e; ++it)
    Msg("%x - name [%s] ID[%d] parent[%s] getDestroy()=[%s]", (*it),
