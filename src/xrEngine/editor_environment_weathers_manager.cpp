...
template <>
void property_collection<manager::weather_container_type,
...
property_collection<manager::weather_container_type, manager>::create() {
  weather *object = xr_new<weather>(
      &m_holder.m_manager, generate_unique_id("weather_unique_id_").c_str());
  object->fill(this);
...
void manager::fill(property_holder_type *holder) {
  VERIFY(holder);
  holder->add_property("weathers", "weathers",
                       "this option is resposible for weathers", m_collection);
  ...
  ::ide().weather_editor_setup(weathers_getter, weathers_size_getter,
                               frames_getter, frames_size_getter);
}
...
manager::weather_ids_type const &manager::weather_ids() const {
  ...
  std::sort(m_weather_ids.begin(), m_weather_ids.end(),
...
bool manager::paste_current_time_frame(char const *buffer,
                                       u32 const &buffer_size) {
  ...
  for (; i != e; ++i) {
    ...
    return ((*i)->paste_time_frame(m_manager.Current[0]->m_identifier, buffer,
                                   buffer_size));
  }
...
bool manager::paste_target_time_frame(char const *buffer,
                                      u32 const &buffer_size) {
  ...
  for (; i != e; ++i) {
    ...
    return ((*i)->paste_time_frame(m_manager.Current[1]->m_identifier, buffer,
                                   buffer_size));
  }
