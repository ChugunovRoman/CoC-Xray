...
manager::manager()
    : m_suns(0), m_levels(0), m_effects(0), m_sound_channels(0), m_ambients(0),
      m_thunderbolts(0), m_weathers(0) {
...
manager::shader_ids_type const &manager::shader_ids() const {
  ...
  std::sort(m_shader_ids.begin(), m_shader_ids.end(),
...
manager::particle_ids_type const &manager::particle_ids() const {
  ...
  std::sort(m_particle_ids.begin(), m_particle_ids.end(),
            logical_string_predicate());
  return (m_particle_ids);
}
...
manager::light_animator_ids_type const &manager::light_animator_ids() const {
  ...
  std::sort(m_light_animator_ids.begin(), m_light_animator_ids.end(),
            logical_string_predicate());
...
void manager::create_mixer() {
  ...
  editor::environment::weathers::time *object =
      xr_new<editor::environment::weathers::time>(
...
SThunderboltDesc *manager::thunderbolt_description(CInifile &config,
                                                   shared_str const &section) {
  return (m_thunderbolts->description(config, section));
}

SThunderboltCollection *manager::thunderbolt_collection(CInifile *pIni,
...
manager::add_flare(xr_vector<CLensFlareDescriptor *> &collection,
                   shared_str const &id) {
#if 0
