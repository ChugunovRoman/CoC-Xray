...
void SThunderboltCollection::load(CInifile *pIni, CInifile *thunderbolts,
                                  LPCSTR sect) {
  section = sect;
...
CEffect_Thunderbolt::CEffect_Thunderbolt() {
  ...
  // "environment","min_dist_factor" )); p_tilt = deg2rad (pSettings->r_float (
  // "environment","tilt" )); p_second_prop = pSettings->r_float (
  // "environment","second_propability" ); clamp (p_second_prop,0.f,1.f);
...
BOOL CEffect_Thunderbolt::RayPick(const Fvector &s, const Fvector &d,
                                  float &dist) {
  BOOL bRes = TRUE;
#ifdef _EDITOR
...
void CEffect_Thunderbolt::OnFrame(shared_str id, float period, float duration) {
  ...
  if (state == stWorking) {
    ...
    int frame;
    u32 uClr =
        current->color_anim->CalculateRGB(current_time / life_time, frame);
...
void CEffect_Thunderbolt::Render() {
  if (state == stWorking) {
    ...
    current_xform.c.y+vecSx.y-vecSy.y, current_xform.c.z+vecSx.z-vecSy.z, c, 0,
    0); pv++; pv->set (current_xform.c.x+vecSx.x+vecSy.x,
    current_xform.c.y+vecSx.y+vecSy.y, current_xform.c.z+vecSx.z+vecSy.z, c, 0,
    1); pv++; pv->set (current_xform.c.x-vecSx.x-vecSy.x,
    current_xform.c.y-vecSx.y-vecSy.y, current_xform.c.z-vecSx.z-vecSy.z, c, 1,
    0); pv++; pv->set (current_xform.c.x-vecSx.x+vecSy.x,
    current_xform.c.y-vecSx.y+vecSy.y, current_xform.c.z-vecSx.z+vecSy.z, c, 1,
    1); pv++;
    }
    ...
    (Device.vCameraTop, -current->m_GradientCenter.fRadius.y*lightning_size);
    pv->set (lightning_center.x+vecSx.x-vecSy.x,
    lightning_center.y+vecSx.y-vecSy.y, lightning_center.z+vecSx.z-vecSy.z, c,
    0, 0); pv++; pv->set (lightning_center.x+vecSx.x+vecSy.x,
    lightning_center.y+vecSx.y+vecSy.y, lightning_center.z+vecSx.z+vecSy.z, c,
    0, 1); pv++; pv->set (lightning_center.x-vecSx.x-vecSy.x,
    lightning_center.y-vecSx.y-vecSy.y, lightning_center.z-vecSx.z-vecSy.z, c,
    1, 0); pv++; pv->set (lightning_center.x-vecSx.x+vecSy.x,
    lightning_center.y-vecSx.y+vecSy.y, lightning_center.z-vecSx.z+vecSy.z, c,
    1, 1); pv++;
    }
