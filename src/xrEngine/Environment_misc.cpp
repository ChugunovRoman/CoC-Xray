...
void CEnvDescriptor::load(CEnvironment &environment, CInifile &config) {
  ...
  R_ASSERT3((tm.x >= 0) && (tm.x < 24) && (tm.y >= 0) && (tm.y < 60) &&
                (tm.z >= 0) && (tm.z < 60),
  ...
  // );
  // AVO: commented to allow COC run in debug. I belive Cromm set longtitude to
  // negative value in AF3 and that's why it is failing here VERIFY2(sun_dir.y <
  // 0, "Invalid sun direction settings while loading");
  ...
  bolt_period = (tb_id.size())
                    ? config.r_float(m_identifier.c_str(), "thunderbolt_period")
                    : 0.f;
...
void CEnvDescriptorMixer::lerp(CEnvironment *, CEnvDescriptor &A,
                               CEnvDescriptor &B, float f, CEnvModifier &Mdf,
