...
CInput::CInput(BOOL bExclusive, int deviceForInit) {
  ...
  if (!pDI)
    CHK_DX(DirectInput8Create(GetModuleHandle(NULL), DIRECTINPUT_VERSION,
  ...
  if (deviceForInit & keyboard_device_key)
    CHK_DX(CreateInputDevice(&pKeyboard, GUID_SysKeyboard, &c_dfDIKeyboard,
                             kb_input_flags, KEYBOARDBUFFERSIZE));
...
HRESULT CInput::CreateInputDevice(LPDIRECTINPUTDEVICE8 *device, GUID guidDevice,
  ...
  //(void**)device, NULL ) );
  CHK_DX(
      pDI->CreateDevice(guidDevice, /*IID_IDirectInputDevice8,*/ device, NULL));
...
void CInput::KeyUpdate() {
  ...
  hr = pKeyboard->GetDeviceData(sizeof(DIDEVICEOBJECTDATA), &od[0], &dwElements,
                                0);
...
bool CInput::get_dik_name(int dik, LPSTR dest_str, int dest_sz) {
  ...
  int cnt = WideCharToMultiByte(CP_ACP, 0, keyname.wsz, -1, dest_str, dest_sz,
                                NULL, NULL);
  if (cnt == -1) {
...
void CInput::MouseUpdate() {
  ...
  hr =
      pMouse->GetDeviceData(sizeof(DIDEVICEOBJECTDATA), &od[0], &dwElements, 0);
  if ((hr == DIERR_INPUTLOST) || (hr == DIERR_NOTACQUIRED)) {
    hr = pMouse->Acquire();
    if (hr != S_OK)
      return;
