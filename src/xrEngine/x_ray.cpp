...
extern int PASCAL IntroDSHOW_wnd(HINSTANCE hInstC, HINSTANCE hInstP,
...
hModuleHandle = CreateFile(szModuleFileName, GENERIC_READ, FILE_SHARE_READ,
NULL, OPEN_EXISTING, 0, NULL);

if (hModuleHandle == INVALID_HANDLE_VALUE)
  return pszHash;

hFileMapping = CreateFileMapping(hModuleHandle, NULL, PAGE_READONLY, 0, 0,
NULL);
...
VirtualQuery(lpvMapping, &MemoryBasicInformation,
sizeof(MEMORY_BASIC_INFORMATION));

if (MemoryBasicInformation.RegionSize)
{
  char szHash[33];
  MD5Digest((unsigned char*)lpvMapping, (unsigned
int)MemoryBasicInformation.RegionSize, szHash); MD5Digest((unsigned
char*)szHash, 32, pszHash); for (int i = 0; i < 32; ++i) pszHash[i] =
(char)toupper(pszHash[i]);
}
...
struct damn_keys_filter {
  ...
  damn_keys_filter() {
    ...
    SystemParametersInfo(SPI_GETSCREENSAVEACTIVE, 0, (PVOID)&bScreenSaverState,
                         0);
...
int APIENTRY WinMain_impl(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  if (!IsDebuggerPresent()) {
    ...
    if (heap_set_information) {
      ...
      BOOL const result =
#endif // #ifdef DEBUG
          heap_set_information(GetProcessHeap(), HeapCompatibilityInformation,
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  // "rb" ); u32 const file_size = 29544; char* buffer =
  // (char*)malloc(file_size); fread ( buffer, file_size, 1, file ); fclose (
  // file );
...
void InitLauncher() {
  ...
  if (0 == hLauncher)
    R_CHK(GetLastError());
  R_ASSERT2(hLauncher, "xrLauncher DLL raised exception during loading or "
                       "there is no xrLauncher.dll at all");
...
void CApplication::load_draw_internal() {
  ...
  pv->set (back_coords.lt.x+pos_delta*idx+offs, back_coords.rb.y+offs, 0+EPS_S,
  1, clr, back_text_coords.lt.x+tc_delta*idx, back_text_coords.rb.y); pv++;
  pv->set (back_coords.lt.x+pos_delta*idx+offs, back_coords.lt.y+offs, 0+EPS_S,
  1, clr, back_text_coords.lt.x+tc_delta*idx, back_text_coords.lt.y); pv++;
  }
