...
void CCameraManager::Update(const Fvector &P, const Fvector &D,
                            const Fvector &N, float fFOV_Dest,
...
bool CCameraManager::ProcessCameraEffector(CEffectorCam *eff) {
  ...
  // 2. Child classes with overrided ProcessCameraEffector would be surprised if
  // eff becames invalid pointer The best way - return 'false' when the effector
  // should be deleted, and delete it in ProcessCameraEffector
...
void CCameraManager::UpdateCamEffectors() {
  ...
  while (r_it != m_EffectorsCam.rend()) {
    if (ProcessCameraEffector(*r_it)) {
      ...
      // Dereferencing reverse iterator returns previous element of the list,
      // r_it.base() returns current element So, we should use base()-1 iterator
      // to delete just processed element. 'Previous' element would be
      // automatically changed after deletion, so r_it would dereferencing to
      // another value, no need to change it
...
void CCameraManager::ApplyDevice(float _viewport_near) {
  ...
  Device.mProject.build_projection(deg2rad(m_cam_info.fFov), m_cam_info.fAspect,
                                   _viewport_near, m_cam_info.fFar);
