...
property_collection<manager::effect_container_type, manager>::create() {
  effect *object =
      xr_new<effect>(m_holder, generate_unique_id("effect_unique_id_").c_str());
  object->fill(this);
...
void manager::fill(editor::property_holder *holder) {
  VERIFY(holder);
  holder->add_property("effects", "ambients",
                       "this option is resposible for effects", m_collection);
}
...
manager::effects_ids_type const &manager::effects_ids() const {
  ...
  std::sort(m_effects_ids.begin(), m_effects_ids.end(),
