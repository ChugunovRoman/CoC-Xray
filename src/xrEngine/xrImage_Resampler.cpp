...
void imf_Process(u32 *dstI, u32 dstW, u32 dstH, u32 *srcI, u32 srcW, u32 srcH,
                 EIMF_Type FILTER) {
  R_ASSERT(dstI);
  R_ASSERT(dstW > 1);
  R_ASSERT(dstH > 1);
  ...
  if (xscale < 1.0) {
    try {
      ...
      for (i = 0; i < dst.xsize; ++i) {
        contrib[i].n = 0;
        contrib[i].p =
            (CONTRIB *)xr_malloc((int)(width * 2 + 1) * sizeof(CONTRIB));
    ...
    try {
      for (i = 0; i < dst.xsize; ++i) {
        contrib[i].n = 0;
        contrib[i].p =
            (CONTRIB *)xr_malloc((int)(fwidth * 2 + 1) * sizeof(CONTRIB));
  ...
  try {
    for (k = 0; k < tmp->ysize; ++k) {
      ...
      for (i = 0; i < tmp->xsize; ++i) {
        ...
        put_pixel(tmp, i, k,
                  color_rgba(CC(w_r), CC(w_g), CC(w_b), CC(w_a + 0.5f)));
      }
    }
    xr_free(raster);
  } catch (...) {
