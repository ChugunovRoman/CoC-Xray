...
void thunderbolt::create_top_gradient(CInifile &config,
...
void thunderbolt::create_center_gradient(CInifile &config,
                                         shared_str const &section) {
  VERIFY(section == m_id);
...
void thunderbolt::fill(::editor::environment::manager &environment,
  ...
  m_property_holder->add_property(
      ...
      m_lighting_model.c_str(), m_lighting_model, ".dm",
      "Lighting model files (*.dm)|*.dm",
      detail::real_path("$game_meshes$", "").c_str(),
      "Select lighting model...", editor::property_holder::cannot_enter_text,
      editor::property_holder::keep_extension);
  m_property_holder->add_property(
      "sound", "properties", "this option is resposible for thunderbolt sound",
      m_sound.c_str(), m_sound, ".ogg", "Sound files (*.ogg)|*.ogg",
