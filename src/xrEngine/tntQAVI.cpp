...
BOOL CAviPlayerCustom::Load(char *fname) {
  ...
  MMRESULT res;
  if (MMSYSERR_NOERROR !=
      (res = mmioDescend(hmmioFile, &mmckinfoParent, NULL, MMIO_FINDRIFF))) {
  ...
  if (MMSYSERR_NOERROR !=
      (res = mmioDescend(hmmioFile, &mmckinfoParent, NULL, MMIO_FINDLIST))) {
  ...
  if (MMSYSERR_NOERROR !=
      (res = mmioDescend(hmmioFile, &mmckinfoParent, NULL, MMIO_FINDCHUNK))) {
  ...
  if (mmckinfoParent.cksize !=
      (DWORD)mmioRead(hmmioFile, (HPSTR)&strh, mmckinfoParent.cksize)) {
  ...
  m_aviIC = ICLocate(ICTYPE_VIDEO, NULL, &m_biInFormat,
                     &m_biOutFormat, // ICMODE_DECOMPRESS
  ...
  if (MMSYSERR_NOERROR != (res = mmioDescend(hmmioFile, &mmckinfoSubchunk, NULL,
                                             MMIO_FINDLIST)) ||
  ...
  if (mmckinfoSubchunk.cksize != (DWORD)mmioRead(hmmioFile, (HPSTR)m_pMovieData,
  ...
  if (mmckinfoSubchunk.cksize != (DWORD)mmioRead(hmmioFile,
                                                 (HPSTR)m_pMovieIndex,
                                                 mmckinfoSubchunk.cksize)) {
...
VOID CAviPlayerCustom::PreRoll(DWORD dwFrameNum) {
  ...
  for (i++; i < (int)dwFrameNum; i++) {
    ...
    DWORD dwFlags;
    dwFlags =
        ICDECOMPRESS_PREROLL | ICDECOMPRESS_NOTKEYFRAME | ICDECOMPRESS_HURRYUP;
    m_biInFormat.biSizeImage = pCurrFrameIndex->dwChunkLength;
    dwFlags |= (m_biInFormat.biSizeImage) ? 0 : ICDECOMPRESS_NULLFRAME;
