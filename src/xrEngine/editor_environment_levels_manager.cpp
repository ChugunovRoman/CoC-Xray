...
void manager::fill_levels(CInifile &config, LPCSTR prefix, LPCSTR category) {
  ...
  for (; i != e; ++i) {
    ...
    if (!config.line_exist((*i).first, "weathers")) {
      m_levels.insert(std::make_pair(
          (*i).first.c_str(), std::make_pair(category, s_default_weather_id)));
      continue;
    }

    LPCSTR weather_id = config.r_string((*i).first, "weathers");
    m_levels.insert(std::make_pair((*i).first.c_str(),
                                   std::make_pair(category, weather_id)));
  }
}
...
void manager::load() {
  ...
  m_config_single = CInifile::Create(
      FS.update_path(file_name, "$game_config$", "game_maps_single.ltx"),
      false);

  m_config_mp = CInifile::Create(
      FS.update_path(file_name, "$game_config$", "game_maps_mp.ltx"), false);
...
void manager::fill() {
  ...
  for (; i != e; ++i) {
    ...
    m_property_holder->add_property(
        (*i).first.c_str(), (*i).second.first, description,
        (*i).second.second.c_str(), (*i).second.second, collection_getter,
