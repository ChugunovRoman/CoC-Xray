...
property_collection<flares::flares_type, flares>::create() {
...
void flares::load(CInifile &config, shared_str const &section) {
  ...
  m_shader = READ_IF_EXISTS(&config, r_string, section, "flare_shader",
                            "effects\\flare");
  ...
  shared_str flare_opacity =
      READ_IF_EXISTS(&config, r_string, section, "flare_opacity",
                     "0.340, 0.260, 0.500, 0.420, 0.260, 0.260");
  shared_str flare_position =
      READ_IF_EXISTS(&config, r_string, section, "flare_position",
                     "1.300, 1.000, 0.500, -0.300, -0.600, -1.000");
  shared_str flare_radius =
      READ_IF_EXISTS(&config, r_string, section, "flare_radius",
  ...
  u32 min_flare_count = _min(
      _min(_min(opacity_count, position_count), radius_count), texture_count);
  u32 max_flare_count = _max(
      _max(_max(opacity_count, position_count), radius_count), texture_count);

  u32 max_string_count =
      _max(_max(_max(flare_opacity.size(), flare_position.size()),
                flare_radius.size()),
           flare_textures.size()) +
      1;

  if (min_flare_count != max_flare_count)
    Msg("! flare count for sun [%s] is setup incorrectly. only %d flares are "
        "correct",
  ...
  for (u32 i = 0; i < min_flare_count; ++i) {
    flare *object = xr_new<flare>();
    object->m_opacity =
        (float)atof(_GetItem(flare_opacity.c_str(), i, result, buffer_size));
    object->m_position =
        (float)atof(_GetItem(flare_position.c_str(), i, result, buffer_size));
    object->m_radius =
        (float)atof(_GetItem(flare_radius.c_str(), i, result, buffer_size));
    object->m_texture =
        _GetItem(flare_textures.c_str(), i, result, buffer_size);
...
void flares::fill(manager const &manager, editor::property_holder *holder,
  ...
  properties->add_property("use", "flares",
                           "this option is resposible for the flares usage",
                           m_use, m_use);
  properties->add_property(
      "shader", "flares", "this option is resposible for flares shader",
      m_shader.c_str(), m_shader, &*manager.m_environment.shader_ids().begin(),
  ...
  properties->add_property(
      "flares", "flares", "this option is resposible for flares", m_collection);
}
