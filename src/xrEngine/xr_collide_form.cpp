...
bool CCF_Skeleton::_ElementCenter(u16 elem_id, Fvector &e_center) {
  ElementVecIt it = std::lower_bound(elements.begin(), elements.end(), elem_id,
                                     pred_find_elem);
...
IC bool RAYvsOBB(const Fmatrix &IM, const Fvector &b_hsize, const Fvector &S,
                 const Fvector &D, float &R, BOOL bCull) {
...
IC bool RAYvsSPHERE(const Fsphere &s_sphere, const Fvector &S, const Fvector &D,
                    float &R, BOOL bCull) {
...
IC bool RAYvsCYLINDER(const Fcylinder &c_cylinder, const Fvector &S,
                      const Fvector &D, float &R, BOOL bCull) {
  // Actual test
  ...
  for (ElementVecIt I = elements.begin(); I != elements.end(); I++) {
    ...
    case SBoneShape::stBox: {
      ...
      if (!b) {
        ...
        Msg("! ERROR: bone_id=[%d], world_pos[%f,%f,%f]", I->elem_id,
            VPUSH(TW.c));
