...
template <>
void property_collection<manager::thunderbolt_container_type,
                         manager>::display_name(u32 const &item_index,
                                                LPSTR const &buffer,
...
property_collection<manager::thunderbolt_container_type, manager>::create() {
  thunderbolt *object = xr_new<thunderbolt>(
      &m_holder, generate_unique_id("thunderbolt_unique_id_").c_str());
  object->fill(m_holder.environment(), this);
  return (object->object());
}

template <>
void property_collection<manager::collection_container_type,
                         manager>::display_name(u32 const &item_index,
                                                LPSTR const &buffer,
                                                u32 const &buffer_size) {
  xr_strcpy(buffer, buffer_size, m_container[item_index]->id());
...
property_collection<manager::collection_container_type, manager>::create() {
  collection *object = xr_new<collection>(
      m_holder,
      generate_unique_id("thunderbolt_collection_unique_id_").c_str());
  object->fill(this);
  return (object->object());
}

manager::manager(::editor::environment::manager *environment)
    : m_thunderbolt_collection(0), m_thunderbolts_changed(true),
      m_collections_collection(0), m_collections_changed(true),
      m_property_holder(0), m_environment(*environment) {
  m_thunderbolt_collection = xr_new<thunderbolt_collection_type>(
      &m_thunderbolts, this, &m_thunderbolts_changed);
  m_collections_collection = xr_new<collection_collection_type>(
      &m_collections, this, &m_collections_changed);
}
...
void manager::save() {
  ...
  config->w_float("environment", "delta_longitude",
                  rad2deg(environment.p_var_long));
  ...
  config->w_float("environment", "second_propability",
                  environment.p_second_prop);
...
void manager::fill(editor::property_holder *holder) {
  ...
  holder->add_property("minimum distance factor", "thunderbolts",
                       "this option is resposible for thunderbolts minimum "
                       "distance factor (distance from far plane)",
                       m_environment.p_min_dist, m_environment.p_min_dist, .0f,
                       .95f);
  ...
  holder->add_property(
      "second probability", "thunderbolts",
...
manager::thunderbolts_ids_type const &manager::thunderbolts_ids() const {
  ...
  std::sort(m_thunderbolts_ids.begin(), m_thunderbolts_ids.end(),
...
manager::thunderbolts_ids_type const &manager::collections_ids() const {
  ...
  std::sort(m_collections_ids.begin(), m_collections_ids.end(),
            logical_string_predicate());
...
SThunderboltDesc *manager::description(CInifile &config,
                                       shared_str const &section) const {
  thunderbolt_container_type::const_iterator i = m_thunderbolts.begin();
  thunderbolt_container_type::const_iterator e = m_thunderbolts.end();
  for (; i != e; ++i)
    if ((*i)->id() == section)
