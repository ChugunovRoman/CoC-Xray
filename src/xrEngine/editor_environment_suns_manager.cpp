...
property_collection<manager::container_type, manager>::create() {
  sun *object =
      xr_new<sun>(m_holder, generate_unique_id("sun_unique_id_").c_str());
  object->fill(this);
...
void manager::add(CInifile &config, shared_str const &section) {
  ...
  VERIFY(std::find_if(m_suns.begin(), m_suns.end(), predicate(section)) ==
         m_suns.end());
...
CLensFlareDescriptor *manager::get_flare(shared_str const &id) const {
  // container_type::const_iterator found = std::find_if(m_suns.begin(),
  // m_suns.end(), predicate(id)); VERIFY (found != m_suns.end()); return
  // ((*found)->);
  return (0);
}
