...
property_collection<ambient::effect_container_type, ambient>::create() {
...
template <>
editor::property_holder *
property_collection<ambient::sound_container_type, ambient>::create() {
...
ambient::ambient(manager const &manager, shared_str const &id)
    : m_manager(manager), m_property_holder(0), m_effects_collection(0),
      m_sounds_collection(0) {
  m_load_section = id;
  m_effects_collection = xr_new<effect_collection_type>(&m_effects_ids, this);
  m_sounds_collection =
      xr_new<sound_collection_type>(&m_sound_channels_ids, this);
}
...
void ambient::load(CInifile &ambients_config, CInifile &sound_channels_config,
  ...
  inherited::load(ambients_config, sound_channels_config, effects_config,
                  m_load_section);

  {
    VERIFY(m_effects_ids.empty());
    LPCSTR effects_string = READ_IF_EXISTS(&ambients_config, r_string,
                                           m_load_section, "effects", "");
    for (u32 i = 0, n = _GetItemCount(effects_string); i < n; ++i) {
      string_path temp;
      effect_id *object = xr_new<effect_id>(m_manager.effects_manager(),
                                            _GetItem(effects_string, i, temp));
...

  {
    ...
    LPCSTR sounds_string = READ_IF_EXISTS(&ambients_config, r_string,
                                          m_load_section, "sound_channels", "");
    for (u32 i = 0, n = _GetItemCount(sounds_string); i < n; ++i) {
      string_path temp;
      sound_id *object = xr_new<sound_id>(m_manager.sounds_manager(),
                                          _GetItem(sounds_string, i, temp));
...
void ambient::save(CInifile &config) {
  ...
  LPSTR temp = 0;
  {
    sound_container_type::const_iterator b = m_sound_channels_ids.begin(),
                                         i = b;
  ...
  config.w_float(m_load_section.c_str(), "min_effect_period",
                 float(m_effect_period.x) / 1000.f);
  config.w_float(m_load_section.c_str(), "max_effect_period",
                 float(m_effect_period.y) / 1000.f);
...
void ambient::fill(editor::property_holder_collection *collection) {
  ...
  m_property_holder =
      ::ide().create_property_holder(m_load_section.c_str(), collection, this);
  ...
  m_property_holder->add_property(
      "id", "properties", "this option is resposible for ambient identifier",
  ...
  m_property_holder->add_property(
      "effects", "effects", "this option is resposible for maximum effects",
      m_effects_collection);
  m_property_holder->add_property(
      "sound channels", "sounds",
      "this option is resposible for sound channels", m_sounds_collection);
}

ambient::property_holder_type *ambient::object() { return (m_property_holder); }

::editor::environment::effects::manager const &
ambient::effects_manager() const {
  return (m_manager.effects_manager());
}

::editor::environment::sound_channels::manager const &
ambient::sounds_manager() const {
...
ambient::SSndChannel *ambient::create_sound_channel(CInifile &config,
                                                    LPCSTR id) {
