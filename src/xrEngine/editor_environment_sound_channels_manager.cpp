...
template <>
void property_collection<manager::channel_container_type,
...
property_collection<manager::channel_container_type, manager>::create() {
  channel *object = xr_new<channel>(
      m_holder, generate_unique_id("sound_channel_unique_id_").c_str());
  object->fill(this);
...
manager::channels_ids_type const &manager::channels_ids() const {
  ...
  std::sort(m_channels_ids.begin(), m_channels_ids.end(),
