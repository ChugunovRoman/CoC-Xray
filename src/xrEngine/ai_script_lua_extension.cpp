...
void Script::vfExportToLua(CLuaVirtualMachine *tpLuaVM) {
  ...
  lua_sethook(tpLuaVM, LuaHookCall,
              LUA_HOOKCALL | LUA_HOOKRET | LUA_HOOKLINE | LUA_HOOKTAILRET, 0);
#endif

  vfLoadStandardScripts(tpLuaVM);
}

bool Script::bfLoadFile(CLuaVirtualMachine *tpLuaVM, LPCSTR caScriptName,
                        bool bCall) {
  ...
  if (!xr_strlen(l_caNamespaceName))
    ...
    return (bfLoadFileIntoNamespace(tpLuaVM, caScriptName, l_caNamespaceName,
                                    bCall));
}
#endif
  ...
  for (;;) {
    ...
    if (lua_isnil(tpLuaVM, -1)) {
      ...
      LuaOut(Lua::eLuaMessageTypeError,
             "the namespace name %s is already being used by the non-table "
             "object!",
             caNamespaceName);
      return (false);
    }
...
bool Script::bfLoadBuffer(CLuaVirtualMachine *tpLuaVM, LPCSTR caBuffer,
                          size_t tSize, LPCSTR caScriptName,
                          LPCSTR caNameSpaceName) {
  int l_iErrorCode;
  if (caNameSpaceName) {
    string256 insert;
...
bool bfDoFile(CLuaVirtualMachine *tpLuaVM, LPCSTR caScriptName,
...
bool Script::bfIsObjectPresent(CLuaVirtualMachine *tpLuaVM, LPCSTR identifier,
                               int type) {
  lua_pushnil(tpLuaVM);
  while (lua_next(tpLuaVM, -2)) {
...
bool Script::bfIsObjectPresent(CLuaVirtualMachine *tpLuaVM,
                               LPCSTR namespace_name, LPCSTR identifier,
                               int type) {
