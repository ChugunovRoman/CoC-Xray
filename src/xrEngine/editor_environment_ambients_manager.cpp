...
template <>
void property_collection<manager::ambient_container_type,
...
property_collection<manager::ambient_container_type, manager>::create() {
  ambient *object = xr_new<ambient>(
      m_holder, generate_unique_id("ambient_unique_id_").c_str());
  object->fill(this);
...
void manager::load() {
  ...
  for (; i != e; ++i) {
    ...
    object->load(*m_manager.m_ambients_config,
...
void manager::fill(editor::property_holder *holder) {
  VERIFY(holder);
  holder->add_property("ambients", "ambients",
                       "this option is resposible for ambients", m_collection);
}

::editor::environment::effects::manager const &
manager::effects_manager() const {
  return (m_manager.effects());
}

::editor::environment::sound_channels::manager const &
manager::sounds_manager() const {
...
manager::ambients_ids_type const &manager::ambients_ids() const {
  ...
  std::sort(m_ambients_ids.begin(), m_ambients_ids.end(),
