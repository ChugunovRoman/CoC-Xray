...
static inline editor::color create_color(float const &r, float const &g,
                                         float const &b) {
  editor::color result;
  result.r = r;
  result.g = g;
...
time::time(editor::environment::manager *manager, weather const *weather,
           shared_str const &id)
    : CEnvDescriptorMixer(id), m_manager(*manager), m_weather(weather),
      m_property_holder(0), m_ambient(""), m_sun(""),
...
void time::load_from(shared_str const &id, CInifile &config,
                     shared_str const &new_id) {
  m_identifier = id;
  load(config);
  m_identifier = new_id;
}
...
void time::load(CInifile &config) {
  ...
  // R_ASSERT3
  // ((tm.x>=0)&&(tm.x<24)&&(tm.y>=0)&&(tm.y<60)&&(tm.z>=0)&&(tm.z<60),"Incorrect
  // weather time",m_identifier.c_str()); exec_time =
  ...
  // m_fSunShaftsIntensity = config.r_float(m_identifier,
  // "sun_shafts_intensity");
  m_sun = config.r_string(m_identifier, "sun");
  m_thunderbolt_collection =
      config.r_string(m_identifier, "thunderbolt_collection");
...
void time::save(CInifile &config) {
  ...
  config.w_string(m_identifier.c_str(), "clouds_texture",
  ...
  config.w_string(m_identifier.c_str(), "sky_texture",
                  sky_texture_name.c_str());
  config.w_fvector3(m_identifier.c_str(), "sun_color", sun_color);
  config.w_float(m_identifier.c_str(), "sun_shafts_intensity",
  ...
  config.w_float(m_identifier.c_str(), "wind_direction",
                 rad2deg(wind_direction));
  ...
  config.w_float(m_identifier.c_str(), "sun_longitude",
                 rad2deg(sun_dir.getP()));
...
void time::sun_setter(LPCSTR value) {
  ...
  m_sun = value;
  lens_flare_id = m_manager.eff_LensFlare->AppendDef(
      m_manager, m_manager.m_suns_config, value);
}
...
void time::thunderbolt_setter(LPCSTR value) {
  ...
  tb_id = m_manager.eff_Thunderbolt->AppendDef(
      m_manager, m_manager.m_thunderbolt_collections_config,
...
void time::fill(editor::property_holder_collection *collection) {
  ...
  m_property_holder =
      ::ide().create_property_holder(m_identifier.c_str(), collection, this);
  ...
  m_property_holder->add_property(
      "id", "properties", "this option is resposible for time interval",
      m_identifier.c_str(), string_getter, string_setter);

  m_property_holder->add_property(
      "color", "sun", "this option is resposible for sun color",
      (editor::color const &)sun_color, (editor::color &)sun_color);
  m_property_holder->add_property(
      "shafts intensity", "sun",
  ...
  m_property_holder->add_property(
      ...
      sun_altitude_getter(), sun_altitude_getter, sun_altitude_setter, -360.f,
      360.f);
  ...
  m_property_holder->add_property(
      ...
      sun_longitude_getter(), sun_longitude_getter, sun_longitude_setter,
      -360.f, 360.f);

  typedef editor::property_holder::string_collection_getter_type
      collection_getter_type;
  collection_getter_type collection_getter;

  typedef editor::property_holder::string_collection_size_getter_type
      collection_size_getter_type;
  ...
  m_property_holder->add_property(
      "sun", "sun", "this option is resposible for ambient", m_sun.c_str(),
      m_sun, collection_getter, collection_size_getter,
  ...
  m_property_holder->add_property(
      "sky color", "hemisphere", "this option is resposible for sky color",
      (editor::color const &)sky_color, (editor::color &)sky_color);

  m_property_holder->add_property(
      "hemi color", "hemisphere",
  ...
  m_property_holder->add_property("sky rotation", "hemisphere",
                                  ...
                                  sky_rotation, float_getter, float_setter,
                                  -360.0f, 360.f);
  ...
  m_property_holder->add_property(
      "color", "clouds", "this option is resposible for clouds color",
      (editor::color const &)clouds_color, (editor::color &)clouds_color);

  m_property_holder->add_property(
      "transparency", "clouds",
      "this option is resposible for clouds transparency", clouds_color.w,
      clouds_color.w, 0.f, 1.f);

  m_property_holder->add_property(
      "color", "ambient", "this option is resposible for ambient color",
  ...
  m_property_holder->add_property(
      "ambient", "ambient", "this option is resposible for ambient",
      m_ambient.c_str(), string_getter, string_setter, collection_getter,
      collection_size_getter, editor::property_holder::value_editor_combo_box,
      editor::property_holder::cannot_enter_text);

  m_property_holder->add_property(
      "color", "fog", "this option is resposible for fog density (0..1)",
      (editor::color const &)fog_color, (editor::color &)fog_color);
  m_property_holder->add_property("far plane", "fog",
                                  "this option is resposible for far plane",
                                  far_plane, far_plane);
  m_property_holder->add_property("distance", "fog",
                                  "this option is resposible for fog distance "
                                  "(shoudl be less than far plane)",
                                  fog_distance, fog_distance);
  m_property_holder->add_property(
      "density", "fog", "this option is resposible for fog density (0..1)",
      fog_density, fog_density, 0.f, 1.f);
  m_property_holder->add_property(
      "water intensity", "fog",
      "this option is resposible for water intensity (0..1)", m_fWaterIntensity,
      m_fWaterIntensity, 0.f, 1.f);

  m_property_holder->add_property(
      "rain color", "rain", "this option is resposible for rain color",
      (editor::color const &)rain_color, (editor::color &)rain_color);
  m_property_holder->add_property(
      "rain density", "rain",
      "this option is resposible for rain density (0..1)", rain_density,
      rain_density, 0.f, 1.f);
  ...
  m_property_holder->add_property(
      ...
      m_thunderbolt_collection.c_str(), m_thunderbolt_collection,
      collection_getter, collection_size_getter,
      editor::property_holder::value_editor_combo_box,
      editor::property_holder::cannot_enter_text);

  m_property_holder->add_property(
      "duration", "thunderbolts",
      "this option is resposible for thunderbolt duration", bolt_duration,
      bolt_duration);
  m_property_holder->add_property(
      "period", "thunderbolts",
  ...
  m_property_holder->add_property(
      ...
      wind_velocity, wind_velocity, 0.f, 100.f);
}

void time::lerp(CEnvironment *parent, CEnvDescriptor &A, CEnvDescriptor &B,
