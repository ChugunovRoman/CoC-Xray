...
struct accel_str_pred
    : public std::binary_function<shared_str, shared_str, bool> {
...
struct ENGINE_API motions_value {
  ...
  u32 mem_usage() {
    ...
    for (MotionDefVecIt it = m_mdefs.begin(); it != m_mdefs.end(); it++)
      sz += it->mem_usage();
    for (BoneMotionMapIt bm_it = m_motions.begin(); bm_it != m_motions.end();
         bm_it++)
...
public:
  bool
  create(shared_str key, IReader *data,
         vecBones *bones); //{ motions_value* v =
                           // g_pMotionsContainer->dock(key,data,bones); if
                           //(0!=v) v->m_dwReference++; destroy(); p_ = v; }
  bool
  create(shared_motions const &rhs); // { motions_value* v = rhs.p_; if (0!=v)
