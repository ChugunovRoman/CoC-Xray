...
xrSASH::xrSASH()
    : m_bInited(false), m_bOpenAutomate(false), m_bBenchmarkRunning(false),
...
void xrSASH::ReportNative(LPCSTR pszTestName) {
  ...
  // ("general","average", rfps_average, "average for this run" ); res.w_float
  // ("general","middle", rfps_middlepoint, "per-frame middle-point");
  float fTotal = 0;
  float fNumFrames = 0;
  for (u32 it = 0; it < m_aFrimeTimes.size(); it++) {
    string32 id;
...
void xrSASH::DescribeOption(char *pszOptionName,
                            const oaOptionDependency &Dependency) {
  oaNamedOptionStruct Option;
  oaInitOption(&Option);
...
void xrSASH::SetOption(oaNamedOption *pOption) {
  ...
  if (pMask) {
    xr_sprintf(CmdBuf, "%s %s", pOption->Name,
               (pOption->Value.Bool ? "1" : "0"));
  } else if (pToken) {
