...
public:
  ...
  void _BCL set_callback(u32 Type, BoneCallback C, void *Param,
...
struct ECORE_API SJointIKData {
  ...
  void Export(IWriter &F) {
    ...
    for (int k = 0; k < 3; k++) {
      ...
      // F.w_float (_min(-limits[k].limit.x,-limits[k].limit.y)); // min (swap
      // special for ODE) F.w_float
      // (_max(-limits[k].limit.x,-limits[k].limit.y)); // max (swap special for
      // ODE)
...
public:
  ...
  void SetRestParams(float length, const Fvector &offset,
  ...
  bool Pick(float &dist, const Fvector &S, const Fvector &D,
...
public:
  CBoneData(u16 ID)
      : SelfID(ID), ParentID(u16(-1)), game_mtl_idx(u16(-1)), mass(0) {
...
public:
  virtual u32 mem_usage() {
    ...
    for (ChildFacesVecIt c_it = child_faces.begin(); c_it != child_faces.end();
         c_it++)
