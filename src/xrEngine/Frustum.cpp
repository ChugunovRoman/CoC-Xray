...
EFC_Visible CFrustum::testSAABB(Fvector &c, float r, const float *mM,
                                u32 &test_mask) const {
  u32 bit = 1;
  for (int i = 0; i < p_count; i++, bit <<= 1) {
    if (test_mask & bit) {
...
void CFrustum::CreateFromPortal(sPoly *poly, Fvector &vPN, Fvector &vBase,
                                Fmatrix &mFullXFORM) {
  Fplane P;
...
void CFrustum::CreateOccluder(Fvector *p, int count, Fvector &vBase,
                              CFrustum &clip) {
...
BOOL CFrustum::CreateFromClipPoly(Fvector *p, int count, Fvector &vBase,
                                  CFrustum &clip) {
  VERIFY(count < FRUSTUM_MAXPLANES);
  VERIFY(count >= 3);
