...
xrIDirect3DCubeTexture9::xrIDirect3DCubeTexture9(
    IDirect3DDevice9 *pIDirect3DDevice9, UINT iWidth, UINT iHeight,
    UINT iLevels, DWORD iUsage, D3DFORMAT iFormat, D3DPOOL iPool)
    : m_refCount(0), Width(iWidth), Height(iHeight), Levels(iLevels),
      Format(iFormat)
...
HRESULT xrIDirect3DCubeTexture9::SetPrivateData(REFGUID refguid,
                                                CONST void *pData,
                                                DWORD SizeOfData, DWORD Flags) {
  APIDEBUG("xrIDirect3DCubeTexture9::SetPrivateData");
  return HRESULT_Proc(S_OK);
};
HRESULT xrIDirect3DCubeTexture9::GetPrivateData(REFGUID refguid, void *pData,
                                                DWORD *pSizeOfData) {
...
HRESULT xrIDirect3DCubeTexture9::SetAutoGenFilterType(
    D3DTEXTUREFILTERTYPE iFilterType) {
...
HRESULT xrIDirect3DCubeTexture9::GetLevelDesc(UINT Level,
...
HRESULT xrIDirect3DCubeTexture9::GetCubeMapSurface(
  ...
  *ppCubeMapSurface = NULL;
  xrIDirect3DSurface9 *I = new xrIDirect3DSurface9(
      m_pIDirect3DDevice9, Width, Height, Format, D3DMULTISAMPLE_TYPE(0), 0);
  *ppCubeMapSurface = I;

  return HRESULT_Proc(S_OK);
};
HRESULT xrIDirect3DCubeTexture9::LockRect(D3DCUBEMAP_FACES FaceType, UINT Level,
                                          D3DLOCKED_RECT *pLockedRect,
                                          CONST RECT *pRect, DWORD Flags) {
  APIDEBUG("xrIDirect3DCubeTexture9::LockRect");
  return HRESULT_Proc(S_OK);
};
HRESULT xrIDirect3DCubeTexture9::UnlockRect(D3DCUBEMAP_FACES FaceType,
                                            UINT Level) {
  APIDEBUG("xrIDirect3DCubeTexture9::UnlockRect");
  return HRESULT_Proc(S_OK);
};
HRESULT xrIDirect3DCubeTexture9::AddDirtyRect(D3DCUBEMAP_FACES FaceType,
