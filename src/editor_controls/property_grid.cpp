...
template <typename T>
inline static T registry_value(RegistryKey ^ key, String ^ value_id,
...
void property_grid::OnChildControlMouseDoubleClick(
    Object ^ object, System::Windows::Forms::MouseEventArgs ^ e) {
  if (!SelectedObject)
    return;
  ...
  property_container_interface ^ container =
      safe_cast<property_container_interface ^>(descriptor->bag);
  property_value ^ value = container->value(descriptor->item);
  property_mouse_events ^ value_mouse_events =
      dynamic_cast<property_mouse_events ^>(value);
...
void property_grid::setup_event_handlers() {
  for each (Control ^ i in Controls) {
    i->MouseDoubleClick += gcnew MouseEventHandler(
        this, &property_grid::OnChildControlMouseDoubleClick);
    i->MouseMove +=
        gcnew MouseEventHandler(this, &property_grid::OnChildControlMouseMove);
    i->MouseDown +=
        gcnew MouseEventHandler(this, &property_grid::OnChildControlMouseDown);
  }
}

int property_grid::splitter_width() {
  Type ^ grid_type = m_property_grid_view->GetType();
  FieldInfo ^ field = grid_type->GetField(
      "labelWidth", BindingFlags::NonPublic | BindingFlags::Instance);
...
void property_grid::OnChildControlMouseDown(
    Object ^ object, System::Windows::Forms::MouseEventArgs ^ e) {
...
void property_grid::OnChildControlMouseMove(
    Object ^ object, System::Windows::Forms::MouseEventArgs ^ e) {
  if (e->Button != System::Windows::Forms::MouseButtons::Middle) {
    m_previous_location = e->Location;
    return;
