...
IC void dVectorDeviation(const float *vector3_from, const float *vector3_to,
                         float *vector_dev) {
  dVectorSub(vector_dev, vector3_to, vector3_from);
}

IC void dVectorDeviationAdd(const float *vector3_from, const float *vector3_to,
                            float *vector_dev) {
...
IC void dMatrixSmallDeviation(const float *matrix33_from,
                              const float *matrix33_to, float *vector_dev) {
...
IC void twoq_2w(const Fquaternion &q1, const Fquaternion &q2, float dt,
                Fvector &w) {
  //
...
IC void prg_pos_on_axis(const Fvector &in_ax_p, const Fvector &in_ax_d,
                        Fvector &in_out_pos) {
...
IC float prg_pos_on_plane(const Fvector &in_norm, float d,
                          const Fvector &in_pos, Fvector &out_pos) {
...
IC void prg_on_normal(const Fvector &in_norm, const Fvector &in_dir,
                      Fvector &out_dir) {
...
IC void TransferenceToThrowVel(Fvector &in_transference_out_vel, float time,
                               float gravity_accel) {
...
IC u8 TransferenceAndThrowVelToThrowDir(const Fvector &transference,
                                        float throw_vel, float gravity_accel,
                                        Fvector throw_dir[2]) {
  throw_dir[0] = throw_dir[1] = transference;
  Fvector2 tgA;
  float s;
  u8 ret = TransferenceAndThrowVelToTgA(transference, throw_vel, gravity_accel,
                                        tgA, s);
  switch (ret) {
  case 0:
    return 0;
...
struct SInertVal {
  ...
  SInertVal(float inert) : inertion(inert), val(0.f) {
