...
void CPHShell::PhDataUpdate(dReal step) {
  ...
  if (disable) {
    ...
    //dbg_draw_geometry
#endif
  if (PhOutOfBoundaries(
          cast_fv(dBodyGetPosition((*elements.begin())->get_body()))))
    Disable();
}
...
void CPHShell::applyImpulseTrace(const Fvector &pos, const Fvector &dir,
                                 float val) {
  if (!isActive())
    return;
...
void CPHShell::AddElementRecursive(CPhysicsElement *root_e, u16 id,
                                   Fmatrix global_parent, u16 element_number,
                                   bool *vis_check) {
  ...
  CPhysicsJoint *J = 0;
  bool breakable =
      joint_data.ik_flags.test(SJointIKData::flBreakable) && root_e &&
  ...
  u16 num_children = bone_data.GetNumChildren();
  for (u16 i = 0; i < num_children; ++i)
    AddElementRecursive(E, bone_data.GetChild(i).GetSelfID(), fm_position,
                        element_number, arg_check);
  /////////////////////////////////////////////////////////////////////////////////////
  if (breakable) {
...
void CPHShell::setEndElementSplitter() {
  ...
  if (!elements.back()
           ->FracturesHolder()) // adding fracture for element supposed before
                                // adding splitter. Need only one splitter for
                                // an element
...
void CPHShell::AddSplitter(CPHShellSplitter::EType type, u16 element,
                           u16 joint) {
  if (!m_spliter_holder)
    m_spliter_holder = xr_new<CPHShellSplitterHolder>(this);
...
void CPHShell::setEndJointSplitter() {
  if (!joints.back()->JointDestroyInfo()) // setting joint breacable supposed
                                          // before adding splitter. Need only
...
void CPHShell::PlaceBindToElFormsRecursive(Fmatrix parent, u16 id, u16 element,
                                           Flags64 &mask) {
  ...
  for (u16 i = 0; i < num_children; ++i)
    PlaceBindToElFormsRecursive(mXFORM, ibone_data.GetChild(i).GetSelfID(),
                                element, mask);
}
...
void CPHShell::get_Extensions(const Fvector &axis, float center_prg,
