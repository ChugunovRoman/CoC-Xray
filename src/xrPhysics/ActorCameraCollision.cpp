...
static void cammera_shell_collide_callback_common(bool &do_collide, bool bo1,
                                                  dContact &c,
                                                  SGameMtl *material_1,
                                                  SGameMtl *material_2) {
  if (!do_collide)
...
static void cammera_shell_collide_callback(bool &do_collide, bool bo1,
...
static void cammera_shell_character_collide_callback(bool &do_collide, bool bo1,
                                                     dContact &c,
                                                     SGameMtl *material_1,
                                                     SGameMtl *material_2) {
  dxGeomUserData *oposite_data =
...
static void get_viewport_geom(Fvector &box, Fmatrix &form,
                              const CCameraBase &camera, float _viewport_near) {
...
void get_camera_box(Fvector &box_size, Fmatrix &xform,
                    const CCameraBase &camera, float _viewport_near) {
...
void get_old_camera_box(Fvector &old_box_size, Fmatrix &old_form,
                        const CPhysicsElement *roote, const CBoxGeom *box) {
  roote->GetGlobalTransformDynamic(&old_form);
  box->get_size(old_box_size);
}

void set_camera_collision(const Fvector &box_size, const Fmatrix &xform,
                          CPhysicsElement *roote, CBoxGeom *box) {
  Fvector bs = box_size;
  bs.z = box_size.z * 2.f;
...
void do_collide_and_move(const Fmatrix &xform, IPhysicsShellHolder *l_actor,
                         CPhysicsShell *shell, CPhysicsElement *roote) {
  ///////////////////////////////////////////////////////////////////
  VERIFY(ph_world);
...
bool do_collide_not_move(const Fmatrix &xform, IPhysicsShellHolder *l_actor,
                         CPhysicsShell *shell, CPhysicsElement *roote) {
  ///////////////////////////////////////////////////////////////////
  VERIFY(ph_world);
  VERIFY(!ph_world->Processing());
