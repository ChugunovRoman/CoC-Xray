...
void CPHFracturesHolder::PassEndFractures(u16 from, CPHElement *dest) {
  ...
  if (i_from + 1 != i_to) // insure it!!
  {
    ...
    dest_fract_holder->m_fractures.insert(dest_fract_holder->m_fractures.end(),
                                          i_from + 1, i_to);
...
bool CPHFracture::Update(CPHElement *element) {
  ...
  break_force.mul(1.f /
                  element->getMass()); // element->getMass()//body->mass.mass
...
#ifdef DBG_BREAK
  Msg("bone_id %d break_torque - %f(max %f) break_force %f (max %f) breaked %d",
