...
IC bool negative_tri_set_ignored_by_positive_tri(const Triangle &neg_tri,
  ...
  return (common0 && common1 && common2) ||
         !common0 &&
             !(dDOT(neg_tri.norm, (dReal *)&V_array[pos_tri.T->verts[0]]) >
               neg_tri.pos) ||
         !common1 &&
             !(dDOT(neg_tri.norm, (dReal *)&V_array[pos_tri.T->verts[1]]) >
               neg_tri.pos) ||
         !common2 &&
             !(dDOT(neg_tri.norm, (dReal *)&V_array[pos_tri.T->verts[2]]) >
               neg_tri.pos);
}

int SetBackTrajectoryCnt(const dReal *p, const dReal *last_pos,
                         Triangle &neg_tri, dxGeom *o1, dxGeom *o2,
...
template <class T>
IC int
dcTriListCollider::dSortTriPrimitiveCollide(T primitive, dxGeom *o1, dxGeom *o2,
                                            int flags, dContactGeom *contact,
                                            int skip, const Fvector &AABB) {
  ...
  if (no_last_pos || !last_box.contains(box)) {
    ...
    XRC.box_options(0);
    // VERIFY( g_pGameLevel );
    XRC.box_query(inl_ph_world().ObjectSpace().GetStaticModel(), cast_fv(p),
                  aabb);
    ...
    if (tri.dist < 0.f) {
      ...
      if ((!(last_pos_dist < 0.f)) || b_pushing)
        if (__aabb_tri(Point(p), Point((float *)&AABB), vertices)) {
          ...
          bool contain_pos =
              TriContainPoint(vertices[0], vertices[1], vertices[2], tri.norm,
      ...
      if (!b_pushing && (!intersect || no_last_pos))
        ret += primitive.Collide(vertices[0], vertices[1], vertices[2], &tri,
    ...
    if (neg_depth < dInfinity) {
      ...
      if (no_last_pos)
        for (i = pos_tries.begin(); pos_tries.end() != i; ++i) {
          VERIFY(neg_tri.T);
          if (TriContainPoint((dReal *)&V_array[i->T->verts[0]],
                              (dReal *)&V_array[i->T->verts[1]],
                              (dReal *)&V_array[i->T->verts[2]], i->norm,
                              i->side0, i->side1, p))
            if (negative_tri_set_ignored_by_positive_tri(neg_tri, *i,
                                                         V_array)) {
              include = false;
              break;
            }
    ...
    if (include) {
      ...
      if (ret < flags - 10)
        bret = primitive.CollidePlain(
            b_neg_tri.side0, b_neg_tri.side1, b_neg_tri.norm, b_neg_tri.T,
