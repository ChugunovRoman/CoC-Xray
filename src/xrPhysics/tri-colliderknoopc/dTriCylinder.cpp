...
bool dcTriListCollider::circleLineIntersection(const dReal *cn, const dReal *cp,
                                               dReal r, const dReal *lv,
                                               const dReal *lp, dReal sign,
                                               dVector3 point) {
...
int dcTriListCollider::dSortedTriCyl(const dReal *triSideAx0,
                                     ...
                                     // const dReal* v1,
                                     // const dReal* v2,
                                     CDB::TRI *T, dReal dist, dxGeom *o1,
                                     dxGeom *o2, int flags,
...
IC bool dcTriListCollider::cylinderCrossesLine(const dReal *p, const dReal *R,
                                               dReal hlz, const dReal *v0,
                                               const dReal *v1, const dReal *l,
                                               dVector3 pos) {
...
int dcTriListCollider::dTriCyl(const dReal *v0, const dReal *v1,
                               const dReal *v2, Triangle *T, dxGeom *o1,
  ...
  if (isPdist0 == isPdist1 &&
      isPdist1 == isPdist2) //(here and lower) check the tryangle is on one side
                            // of the cylinder
...
#undef TEST
  ...
  if (code > 3)
    outDepth =
        pointDepth; // deepest vertex axis used if its depth less than outDepth
  // else{
