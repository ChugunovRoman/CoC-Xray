...
static void computeFinalTx(dGeomID geom_transform, dReal *final_pos,
                           dReal *final_R) {
...
void GetBoxExtensions(dGeomID box, const dReal *axis, const dReal *pos,
                      const dReal *rot, float center_prg, dReal *lo_ext,
                      dReal *hi_ext) {
...
void GetCylinderExtensions(dGeomID cyl, const dReal *axis, const dReal *pos,
                           const dReal *rot, float center_prg, dReal *lo_ext,
                           dReal *hi_ext) {
...
void GetSphereExtensions(dGeomID sphere, const dReal *axis, const dReal *pos,
...
void CODEGeom::get_local_form_bt(Fmatrix &form) {
  PHDynamicData::DMXPStoFMX(dGeomGetRotation(geom()), dGeomGetPosition(geom()),
                            form);
}
...
void CODEGeom::get_final_tx_bt(const dReal *&p, const dReal *&R, dReal *bufV,
                               dReal *bufM) const {
  VERIFY(m_geom_transform);
...
void CODEGeom::get_final_tx(dGeomID g, const dReal *&p, const dReal *&R,
                            dReal *bufV, dReal *bufM) {
  if (is_transform(g)) {
...
void CBoxGeom::get_Extensions(const Fvector &axis, float center_prg,
...
void CSphereGeom::get_Extensions(const Fvector &axis, float center_prg,
                                 float &lo_ext, float &hi_ext) const {
  VERIFY(m_geom_transform);
  const dReal *rot = NULL;
  const dReal *pos = NULL;
...
void CCylinderGeom::get_Extensions(const Fvector &axis, float center_prg,
                                   float &lo_ext, float &hi_ext) const {
  VERIFY(m_geom_transform);
  const dReal *rot = NULL;
  const dReal *pos = NULL;
