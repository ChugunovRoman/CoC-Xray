...
public:
  virtual void call(IPhysicsShellHolder *obj, float min_cs, float max_cs,
                    float &cs, float &hl, ICollisionDamageInfo *di) = 0;
...
public:
  ...
  // virtual	void							SetWeaponHideState					(
  // u16 State, bool bSet )=0;
  virtual void _BCL
  HideAllWeapons(bool v) = 0; //(SetWeaponHideState(INV_STATE_BLOCK_ALL,true))
