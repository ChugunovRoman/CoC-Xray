...
CPhysicsJoint *P_create_Joint(CPhysicsJoint::enumType type,
                              CPhysicsElement *first, CPhysicsElement *second) {
...
CPhysicsShell *P_build_SimpleShell(IPhysicsShellHolder *obj, float mass,
                                   bool not_active_state) {
  CPhysicsShell *pPhysicsShell = P_create_Shell();
#ifdef DEBUG
  pPhysicsShell->dbg_obj = (obj);
...
void ApplySpawnIniToPhysicShell(CInifile const *ini,
                                CPhysicsShell *physics_shell, bool fixed) {
  if (!ini)
    return;
...
void get_box(const CPhysicsBase *shell, const Fmatrix &form, Fvector &sz,
             Fvector &c) {
...
void phys_shell_verify_model(IKinematics &K) {
  ...
  VERIFY2(has_physics_collision_shapes(K),
          make_string("Can not create physics shell for model %s because it "
                      "has no physics collision shapes set",
...
void phys_shell_verify_object_model(IPhysicsShellHolder &O) {
  ...
  // VERIFY2( V, make_string( "Can not create physics shell for object %s it has
  // no model", O.ObjectName() )/*+ make_string("\n object dump: \n") +
  ...
  VERIFY2(K, make_string("Can not create physics shell for object %s, model %s "
                         "is not skeleton",
                         O.ObjectName(), O.ObjectNameVisual()));

  VERIFY2(
      has_physics_collision_shapes(*K), make_string("Can not create physics "
                                                    "shell for object %s, "
                                                    "model %s has no physics "
                                                    "collision shapes set",
                                                    O.ObjectName(),
                                                    O
                                                        .ObjectNameVisual()) /*+ make_string("\n object dump: \n") + dbg_object_full_dump_string( &O )*/);
...
bool __stdcall can_create_phys_shell(string1024 &reason,
  ...
  IKinematics *K = O.ObjectKinematics();
  if (!K) {
    xr_strcpy(reason, make_string("Can not create physics shell for object %s, "
  ...
  if (!has_physics_collision_shapes(*K)) {
    xr_strcpy(reason,
              make_string("Can not create physics shell for object %s, model "
                          "%s has no physics collision shapes set",
                          O.ObjectName(), O.ObjectNameVisual())
                  .c_str());
    return false;
  }
...
void StaticEnvironmentCB(bool &do_colide, bool bo1, dContact &c,
