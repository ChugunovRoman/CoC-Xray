...
private:
  ...
  void InitNewElement(CPHElement *element, const Fmatrix &shift_pivot,
                      float density);
...
public:
  ...
  void PhTune(dBodyID body); // set feedback for joints called from PhTune of
                             // ShellSplitterHolder
  bool PhDataUpdate(
      CPHElement
          *element); // collect joints and external impacts in fractures Update
                     // which set m_fractured; called from PhDataUpdate of
