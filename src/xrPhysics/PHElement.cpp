...
dMass CPHElement::recursive_mass_summ(u16 start_geom, FRACTURE_I cur_fracture) {
  ...
  if (m_fratures_holder->m_fractures.end() != cur_fracture)
    cur_fracture->SetMassParts(m_mass,
                               recursive_mass_summ(start_geom, cur_fracture));
  return end_mass;
}
...
void CPHElement::TransformPosition(const Fmatrix &form,
  ...
  Fmatrix bm;
  PHDynamicData::DMXPStoFMX(dBodyGetRotation(m_body), dBodyGetPosition(m_body),
                            bm);
  Fmatrix new_bm;
  new_bm.mul(form, bm);
  dMatrix3 dBM;
...
void CPHElement::SetBoneCallback() {
  IKinematics *K = m_shell->PKinematics();
  VERIFY(K);
  K->LL_GetBoneInstance(m_SelfID).set_callback(
      bctPhysics, m_shell->GetBonesCallback(), cast_PhysicsElement(this));
}
...
void CPHElement::Activate(const Fmatrix &transform, const Fvector &lin_vel,
                          const Fvector &ang_vel, bool disable) {
  VERIFY(!isActive());
...
void CPHElement::PhDataUpdate(dReal step) {
  ...
  if (!dV_valid(angular_velocity)) {
    Msg("angular vel %f,%f,%f", angular_velocity[0], angular_velocity[1],
        angular_velocity[2]);
    ...
    Msg("quaternion  %f,%f,%f,%f", dBodyGetQuaternion(m_body)[0],
        dBodyGetQuaternion(m_body)[1], dBodyGetQuaternion(m_body)[2],
  ...
  */
  ...
  if (!fis_zero(k_w))
    dBodyAddTorque(m_body, -angular_velocity[0] * k_w,
                   -angular_velocity[1] * k_w, -angular_velocity[2] * k_w);
  ...
  if (!fis_zero(l_air))
    dBodyAddForce(m_body, -linear_velocity[0] * l_air,
                  -linear_velocity[1] * l_air, -linear_velocity[2] * l_air);
}
...
void CPHElement::applyImpulseVsMC(const Fvector &pos, const Fvector &dir,
                                  float val) {
...
#endif
  dBodyAddForceAtRelPos(m_body, impulse.x, impulse.y, impulse.z, pos.x, pos.y,
                        pos.z);
  BodyCutForce(m_body, m_l_limit, m_w_limit);
  ////////////////////////////////////////////////////////////////////////
}
void CPHElement::applyImpulseVsGF(const Fvector &pos, const Fvector &dir,
                                  float val) {
...
void CPHElement::applyImpulseTrace(const Fvector &pos, const Fvector &dir,
                                   float val, u16 id) {
  ...
  if (debug_output().ph_dbg_draw_mask().test(phHitApplicationPoints)) {
    ...
    debug_output().DBG_DrawLine(
        dbg_position, Fvector().add(dbg_position, Fvector().mul(dir, 0.4f)),
...
void CPHElement::SetBoneCallbackOverwrite(bool v) {
  VERIFY(m_shell);
  VERIFY(m_shell->PKinematics());
  m_shell->PKinematics()->LL_GetBoneInstance(m_SelfID).set_callback_overwrite(
      v);
}
...
void CPHElement::applyForce(float x, float y,
...
void CPHElement::add_Mass(const SBoneShape &shape, const Fmatrix &offset,
  ...
  case SBoneShape::stBox: {
    dMassSetBox(&m, 1.f, shape.box.m_halfsize.x * 2.f,
                shape.box.m_halfsize.y * 2.f, shape.box.m_halfsize.z * 2.f);
    dMassAdjust(&m, mass);
...
void CPHElement::get_Extensions(const Fvector &axis, float center_prg,
                                float &lo_ext, float &hi_ext) const {
...
void CPHElement::cv2bone_Xfrom(const Fquaternion &q, const Fvector &pos,
                               Fmatrix &xform) {
...
void CPHElement::cv2obj_Xfrom(const Fquaternion &q, const Fvector &pos,
                              Fmatrix &xform) {
...
void CPHElement::dbg_draw_geometry(
    float scale, u32 color, Flags32 flags /*= Flags32().assign( 0 )*/) const {
