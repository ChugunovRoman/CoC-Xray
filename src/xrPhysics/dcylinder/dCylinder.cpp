...
// radius r1
//					O3,O4 - intersection between the line and
//sphere with center cp2 radius r2 returns false if there is no intersection
// computes
// distances O1-O3, O1-O4, O2-O3, O2-O4 in "point" returns mean point between
// intersection points with smallest distance
/////////////////////////////////////////////////////////////////////////////////////////////////
inline bool circleIntersection(const dReal *n1, const dReal *cp1, dReal r1,
                               const dReal *n2, const dReal *cp2, dReal r2,
                               dVector3 point) {
...
extern "C" int dCylBox(const dVector3 p1, const dMatrix3 R1, const dReal radius,
                       const dReal lz, const dVector3 p2, const dMatrix3 R2,
                       const dVector3 side2, dVector3 normal, dReal *depth,
                       int *code, int maxc, dContactGeom *contact, int skip) {
  dVector3 p, pp, normalC;
  const dReal *normalR = 0;
  dReal B1, B2, B3, R11, R12, R13, R21, R22, R23, R31, R32, R33, Q11, Q12, Q13,
      Q21, Q22, Q23, Q31, Q32, Q33, s, s2, l, sQ21, sQ22, sQ23;
  int i, invert_normal;
...
#define TEST(expr1, expr2, norm, cc)                                           \
  ...
  *code = 0;
  ...
  // might be used to quiken the routine but then it need to check if Q21<=1.f,
  // becouse it may slightly exeed 1.f.
...
#define TEST(expr1, expr2, n1, n2, n3, cc)                                     \
  ...
  // separating axis is a normal to the cylinder axis passing across the nearest
  // box vertex
  ...
  TEST(p[0] * Ax[0] + p[1] * Ax[1] + p[2] * Ax[2], (radius + boxProj), Ax[0],
       Ax[1], Ax[2], 4);

  // next three test used to handle collisions between cylinder circles and box
  // ages
  ...
  TEST(p[0] * Ax[0] + p[1] * Ax[1] + p[2] * Ax[2],
       (_sin * radius + _cos * hlz + boxProj), Ax[0], Ax[1], Ax[2], 5);
  ...
  _sin = dSqrt(cos1 * cos1 + cos3 * cos3);
  TEST(p[0] * Ax[0] + p[1] * Ax[1] + p[2] * Ax[2],
       (_sin * radius + _cos * hlz + boxProj), Ax[0], Ax[1], Ax[2], 6);
  ...
  _sin = dSqrt(cos1 * cos1 + cos3 * cos3);
  TEST(p[0] * Ax[0] + p[1] * Ax[1] + p[2] * Ax[2],
       (_sin * radius + _cos * hlz + boxProj), Ax[0], Ax[1], Ax[2], 7);
...
#define TEST(expr1, expr2, n1, n2, n3, cc)                                     \
  ...
  TEST(pp[0] * R31 - pp[2] * R11, (radius + B2 * Q23 + B3 * Q22), R31, 0, -R11,
       8);
  TEST(pp[0] * R32 - pp[2] * R12, (radius + B1 * Q23 + B3 * Q21), R32, 0, -R12,
       9);
  TEST(pp[0] * R33 - pp[2] * R13, (radius + B1 * Q22 + B2 * Q21), R33, 0, -R13,
       10);
...
extern "C" int dCylCyl(const dVector3 p1, const dMatrix3 R1,
                       ...
                       dVector3 normal, dReal *depth, int *code, int maxc,
...
#define TEST(expr1, expr2, n1, n2, n3, cc)                                     \
  ...
  dNormalize3(Ax);
  TEST(p[0] * Ax[0] + p[1] * Ax[1] + p[2] * Ax[2], radius1 + radius2, Ax[0],
       Ax[1], Ax[2], 6);
  ...
  TEST(p[0] * Ax[0] + p[1] * Ax[1] + p[2] * Ax[2],
       radius1 + _cos * hlz2 + _sin * radius2, Ax[0], Ax[1], Ax[2], 3);

  {
    ...
    // position// (project p on cyl2 flat surface ) cos1 = dDOT14(p,R2+0); cos3
    // = dDOT14(p,R2+2) ;
  ...
  TEST(p[0] * Ax[0] + p[1] * Ax[1] + p[2] * Ax[2],
       radius2 + _cos * hlz1 + _sin * radius1, Ax[0], Ax[1], Ax[2], 4);

  ////test circl

  //@ this needed to set right normal when cylinders edges intersect
  //@ the most precise axis for this test may be found as a line between nearest
  // points of two
  ...
  dVector3 point;
  {
    ...
    TEST(p[0] * Ax[0] + p[1] * Ax[1] + p[2] * Ax[2], cyl1Pr + cyl2Pr, Ax[0],
         Ax[1], Ax[2], 5);
  }
...
int dCollideCylS(dxGeom *o1, dxGeom *o2, int flags, dContactGeom *contact,
                 int skip) {
...
#define TEST(expr1, expr2, n1, n2, n3, cc)                                     \
  ...
  cos3 = dDOT14(Ax, R + 2);
  _sin = dSqrt(cos1 * cos1 + cos3 * cos3);
  TEST(dDOT(p, Ax), sphereRadius + cylRadius * _sin + hl * _cos, Ax[0], Ax[1],
       Ax[2], 14);
...
int dCollideCylB(dxGeom *o1, dxGeom *o2, int flags, dContactGeom *contact,
                 int skip) {
  dVector3 normal;
  dReal depth;
  int code;
  ...
  int num =
      dCylBox(dGeomGetPosition(o1), dGeomGetRotation(o1), cylRadius, cylLength,
              dGeomGetPosition(o2), dGeomGetRotation(o2), boxSides, normal,
...
int dCollideCylCyl(dxGeom *o1, dxGeom *o2, int flags, dContactGeom *contact,
                   int skip) {
  dVector3 normal;
  dReal depth;
  int code;
  ...
  int num = dCylCyl(dGeomGetPosition(o1), dGeomGetRotation(o1), cylRadius1,
                    cylLength1, dGeomGetPosition(o2), dGeomGetRotation(o2),
                    cylRadius2, cylLength2, normal, &depth, &code,
...
int dCollideCylPlane(dxGeom *o1, dxGeom *o2, int flags, dContactGeom *contact,
                     int skip) {
  VERIFY(skip >= (int)sizeof(dContactGeom));
  VERIFY(dGeomGetClass(o1) == dCylinderClassUser);
...
int dCollideCylRay(dxGeom *o1, dxGeom *o2, int flags, dContactGeom *contact,
                   int skip) {
  VERIFY(skip >= (int)sizeof(dContactGeom));
  VERIFY(dGeomGetClass(o1) == dCylinderClassUser);
  ...
  if (!inside_cyl && C < 0) {
    ...
    if (k < 0) {
      ...
      if (k >= -lz2 && k <= lz2) {
        contact->normal[0] =
            nsign * (contact->pos[0] - (p[0] + k * R[0 * 4 + 1]));
        contact->normal[1] =
            nsign * (contact->pos[1] - (p[1] + k * R[1 * 4 + 1]));
        contact->normal[2] =
            nsign * (contact->pos[2] - (p[2] + k * R[2 * 4 + 1]));
