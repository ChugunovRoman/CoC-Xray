...
void CPhysicsShellAnimator::CreateJoints(LPCSTR controled) {
  ...
  for (u16 i = 0; nb > i; ++i) {
    ...
    VERIFY2(e, make_string("shell_animation - controled bone %s has no physics "
                           "collision! object: %s, model: %s",
                           n, obj->ObjectName(), obj->ObjectNameVisual()));
    CreateJoint(e);
  }
}
...
#pragma todo("reset callback?")
    ...
    Fvector mc;
    i->m_element->CPHGeometryOwner::get_mc_vs_transform(mc,
                                                        target_obj_posFmatrixS);
    dJointSetFixedQuaternionPos(i->m_anim_fixed_dJointID,
                                target_obj_quat_dQuaternionS, &mc.x);
  }
