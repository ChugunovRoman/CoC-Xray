...
class CPHElement : public CPhysicsElement,
  ...
  dReal m_l_limit; //->to shell ??		//bl
                   ...
                   ////e
  ////st 	dQuaternion m_safe_quaternion;
...
public:
  ...
  virtual void
  set_ContactCallback(ContactCallbackFun *callback); // aux (may not be)
  ...
  virtual void get_Extensions(const Fvector &axis, float center_prg,
                              float &lo_ext, float &hi_ext) const; // aux
...
public:  //
  ...
  // void						BoneGlPos								(Fmatrix &m,
  // const CBoneInstance* B)const;
  ...
  virtual void
  applyImpulseTrace(const Fvector &pos, const Fvector &dir, float val,
                    const u16 id); // called anywhere ph state influent
  virtual void set_DisableParams(const SAllDDOParams &params); //
  virtual void
  set_DynamicLimits(float l_limit = default_l_limit,
                    float w_limit = default_w_limit); // aux (may not be)
  virtual void
  set_DynamicScales(float l_scale = default_l_scale,
  ...
  virtual void
  set_LinearVel(const Fvector &velocity); // called anywhere ph state influent
  virtual void
  set_AngularVel(const Fvector &velocity); // called anywhere ph state influent
  ...
  virtual void cv2obj_Xfrom(const Fquaternion &q, const Fvector &pos,
                            Fmatrix &xform); //
  virtual void cv2bone_Xfrom(const Fquaternion &q, const Fvector &pos,
                             Fmatrix &xform); //
  virtual void _BCL
