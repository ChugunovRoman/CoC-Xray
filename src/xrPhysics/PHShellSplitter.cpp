...
void CPHShellSplitterHolder::PassEndSplitters(const CShellSplitInfo &spl_inf,
                                              CPHShell *dest, u16 jt_add_shift,
                                              u16 el_add_shift) {
...
shell_root
CPHShellSplitterHolder::ElementSingleSplit(const element_fracture &split_elem,
  ...
  // it is not right for multiple joints attached to the unsplited part becource
  // all these need to be reattached
  ...
  if ((split_elem.first)->FracturesHolder()) // if this element can be splitted
                                             // add a splitter for it
  ...
  m_pShell->PassEndJoints(split_elem.second.m_start_jt_num,
                          split_elem.second.m_end_jt_num, new_shell_last_desc);
