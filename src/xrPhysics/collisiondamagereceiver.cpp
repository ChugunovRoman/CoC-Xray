...
void DamageReceiverCollisionCallback(bool &do_colide, bool bo1, dContact &c,
                                     SGameMtl *material_1,
                                     SGameMtl *material_2) {
  ...
  // CPHCollisionDamageReceiver	*dr	= static_cast<CPhysicsShellHolder*>(
  // o_self )->PHCollisionDamageReceiver();
  ...
  pos.sub(*(Fvector *)c.geom.pos,
          *(Fvector *)dGeomGetPosition(
              bo1 ? c.geom.g1 : c.geom.g2)); // it is not true pos in bone space

  dr->CollisionHit(source_id, ud_self->bone_id,
                   E_NL(b1, b2, c.geom.normal) * damager_material_factor / dfs,
                   dir, pos);
}

void BreakableObjectCollisionCallback(bool & /**do_colide/**/, bool bo1,
  ...
  /*
     ...
     usr_data_1->ph_ref_object ); this_object2 = smart_cast<CBreakableObject*>(
     usr_data_2->ph_ref_object ); if( usr_data_1&& usr_data_1->ph_ref_object&&
                  this_object1
                  )
          {
                          body=dGeomGetBody(c.geom.g2);
