...
bool test_sides(const Fvector &center, const Fvector &side_dir,
                const Fvector &fv_dir, const Fvector &box, int tri_id) {
  Triangle tri;
  CalculateInitTriangle(inl_ph_world().ObjectSpace().GetStaticTris() + tri_id,
                        tri, inl_ph_world().ObjectSpace().GetStaticVerts());
...
void CPHSimpleCharacter::TestPathCallback(bool &do_colide, bool bo1,
                                          dContact &c,
                                          SGameMtl * /*material_1*/,
                                          SGameMtl * /*material_2*/) {
  do_colide = false;
...
void CPHSimpleCharacter::PhTune(dReal step) {
  ...
  b_lose_ground =
      !(b_good_graund || m_elevator_state.ClimbingState()) || b_lose_control;
  ...
  if (b_jumping) {
    ...
    if (amag > 0.f)
      if (dif[0] * m_acceleration.x / amag + dif[2] * m_acceleration.z / amag <
          0.3f) {
        Fvector jump_fv =
            m_acceleration; //{
                            // m_acceleration.x/amag*1000.f,0,m_acceleration.z/amag*1000.f
                            //}
...
bool CPHSimpleCharacter::ValidateWalkOnObject() {
  ...
  // decide to clamb
  if (!m_elevator_state.Active() && b_valide_wall_contact &&
                       ...
                       // dDOT(m_wall_contact_normal,m_ground_contact_normal)<.9f
  {
    ...
    if (((m_wall_contact_position[0] - m_ground_contact_position[0]) *
             m_control_force[0] +
...
bool CPHSimpleCharacter::ValidateWalkOnMesh() {
  ...
  for (CDB::RESULT *Res = R_begin; Res != R_end; ++Res) {
    ...
    if (__aabb_tri(Point((float *)&center_forbid), Point((float *)&AABB_forbid),
                   vertices)) {
...
void CPHSimpleCharacter::ApplyAcceleration() {
  ...
  if (!m_elevator_state.ClimbingState() &&
      b_clamb_jump) { //&&m_wall_contact_normal[1]<M_SQRT1_2
...
void CPHSimpleCharacter::SafeAndLimitVelocity() {
  ...
  if (dV_valid(linear_velocity)) {
    ...
    if (b_external_impulse) {
      ...
      float ll_limit =
          m_ext_imulse.dotproduct(cast_fv(linear_velocity)) * 10.f / fixed_step;
      if (sq_mag > EPS_L) {
        Fvector acc;
...
u16 CPHSimpleCharacter::RetriveContactBone() {
  ...
  if (inl_ph_world().ObjectSpace().RayQuery(
    ...
    pos_in_object.sub(
        ...
        m_phys_ref_object
            ->ObjectPosition()); // vector from object center to contact
                                 // position currently in global frame
    Fmatrix object_form;
    object_form.set(m_phys_ref_object->ObjectXFORM());
    object_form.transpose();
    object_form.transform_dir(
        pos_in_object); // project pos_in_object on object axes now it is
...
void CPHSimpleCharacter::InitContact(dContact *c, bool &do_collide,
                                     u16 material_idx_1, u16 material_idx_2) {
...
void CPHSimpleCharacter::get_spatial_params() {
  spatialParsFromDGeom((dGeomID)m_space, spatial.sphere.P, AABB,
                       spatial.sphere.R);
}
...
bool PickMaterial(u16 &res_material_idx, const Fvector &pos_,
                  const Fvector &dir_, float range_, CObject *ignore_object) {
  Fvector pos = pos_;
  pos.y += EPS_L;
  Fvector dir = dir_;
  float range = range_;
  ...
  while (inl_ph_world().ObjectSpace().RayPick(pos, dir, range, collide::rqtBoth,
                                              R, ignore_object)) {
