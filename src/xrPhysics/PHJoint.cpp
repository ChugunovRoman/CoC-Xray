...
void CPHJoint::SetAnchorVsFirstElement(const float x, const float y,
                                       const float z) {
  vs_anchor = vs_first;
  anchor.set(x, y, z);
}

void CPHJoint::SetAnchorVsSecondElement(const float x, const float y,
                                        const float z) {
  vs_anchor = vs_second;
  anchor.set(x, y, z);
}

void CPHJoint::SetAxisDir(const float x, const float y, const float z,
                          const int axis_num) {
  int ax = axis_num;
  LimitAxisNum(ax);
  // if(-1==ax) return;
...
void CPHJoint::SetAxisDirDynamic(const Fvector &orientation,
  ...
  case hinge2:
    switch (axis_num) {
    case 0:
      dJointSetHinge2Axis1(m_joint, orientation[0], orientation[1],
                           orientation[2]);
      break;
    case 1:
      dJointSetHinge2Axis2(m_joint, orientation[0], orientation[1],
                           orientation[2]);
      break;
    default:
      NODEFAULT;
  ...
  case hinge:
    ...
    break;
  case full_control:
    dJointSetAMotorAxis(m_joint1, axis_num, 1, orientation[0], orientation[1],
                        orientation[2]);
    break;
  default:
...
void CPHJoint::SetAxisDirVsFirstElement(const float x, const float y,
                                        const float z, const int axis_num) {
  int ax = axis_num;
  LimitAxisNum(ax);
  if (-1 == ax)
    return;
...
void CPHJoint::SetAxisDirVsSecondElement(const float x, const float y,
                                         const float z, const int axis_num) {
  int ax = axis_num;
  LimitAxisNum(ax);
  if (-1 == ax)
...
void CPHJoint::SetLimits(const float low, const float high,
                         const int axis_num) {
...
CPHJoint::CPHJoint(CPhysicsJoint::enumType type, CPhysicsElement *first,
...
void CPHJoint::SetLimitsVsFirstElement(const float low, const float high,
                                       const int axis_num) {}

void CPHJoint::SetLimitsVsSecondElement(const float low, const float high,
                                        const int axis_num) {}
...
void CPHJoint::SetForceAndVelocity(const float force, const float velocity,
                                   const int axis_num) {
...
void CPHJoint::GetMaxForceAndVelocity(float &force, float &velocity,
                                      int axis_num) {
...
void CPHJoint::SetAxisSDfactors(float spring_factor, float damping_factor,
                                int axis_num)
...
void CPHJoint::GetAxisSDfactors(float &spring_factor, float &damping_factor,
                                int axis_num) {
  LimitAxisNum(axis_num);
