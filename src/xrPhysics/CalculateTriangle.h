...
ICF void CalculateInitTriangle(CDB::TRI *XTri, Triangle &triangle,
...
inline bool TriContainPoint(const dReal *v0, const dReal *v1, const dReal *v2,
                            ...
                            const dReal *triSideAx2, const dReal *triAx,
...
ICF bool TriContainPoint(const dReal *v0, const dReal *v1, const dReal *v2,
                         const dReal *triAx, const dReal *triSideAx0,
                         const dReal *triSideAx1, const dReal *pos, u16 &c) {

  dVector3 triSideAx2 = {v0[0] - v2[0], v0[1] - v2[1], v0[2] - v2[2]};
  return TriContainPoint(v0, v1, v2, triSideAx0, triSideAx1, triSideAx2, triAx,
                         pos, c);
}
ICF bool TriContainPoint(Triangle *T, const float *pos, u16 &c,
  ...
  // dReal* triAx,const dReal* triSideAx0,const dReal* triSideAx1, const dReal*
  // pos) VERIFY( g_pGameLevel ); const Fvector*
  // V_array=inl_ph_world().ObjectSpace().GetStaticVerts();
  CDB::TRI *XTri = T->T;
  const float *VRT[3] = {(dReal *)&V_array[XTri->verts[0]],
                         (dReal *)&V_array[XTri->verts[1]],
                         (dReal *)&V_array[XTri->verts[2]]};
  return TriContainPoint(VRT[0], VRT[1], VRT[2], T->norm, T->side0, T->side1,
                         pos, c);
}

enum ETriDist { tdBehind, tdPlane, tdSide, tdVert };

IC float DistToFragmenton(const dReal *point, const dReal *pt1,
                          const dReal *pt2, dReal *p, dReal *to_point, u16 &c) {
...
ICF float DistToTri(Triangle *T, const float *pos, float *dir, float *p,
                    ETriDist &c, const Fvector *V_array) {
