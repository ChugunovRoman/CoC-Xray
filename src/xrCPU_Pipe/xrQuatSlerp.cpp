...
void __stdcall xrSlerp_x86(_quaternion<float> *D, _quaternion<float> *Q1,
                           _quaternion<float> *Q2, float t) {
  D->slerp(*Q1, *Q2, t);
}
...
void	__stdcall	xrSlerp_3Dnow	(_quaternion* result, _quaternion* q1,
_quaternion* q2, float t)
{
    ...
    else
    {
        ...
        if (_m_to_int (_m_pfcmpge (cosom, _m_from_float (DELTA_2))) != 0)
        {
            ...
            scale0	= _m_pfsub (_m_from_float (ONES.m64_f32[0]), r1);    //
-slerp | 1 - slerp scale1	= _m_punpckldq (r1, r1); // slerp | slerp
        }
        else
        {
            ...
            r0		= _m_sin		(omega);
            r1		= _m_pfrcp		(r0);
            isinom	= _m_pfrcpit2	(_m_pfrcpit1 (r0, r1), r1);
// isinom = 1/sin(o) | 1/sin(o)

            //; scale0 = (float)sin ((1.0f - slerp) * omega) * isinom;
            //; scale1 = (float)sin (slerp * omega) * isinom;
            //; (this code is terribly sisd, but I'm can't puzzle out a better
way)
            scale0	= _m_sin		(_m_pfmul (_m_pfsub (ONES,
_m_from_float (slerp)), omega));

            r0		= _m_sin		(_m_pfmul (_m_from_float
(slerp), omega)); scale1	= _m_pfmul		(_m_punpckldq (r0, r0),
isinom);
