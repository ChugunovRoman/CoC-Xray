...
void __stdcall xrTransfer_x86(LPVOID vDest, LPVOID vSrc, u32 vCount,
                              u32 vStride, LPWORD iDest, LPWORD iSrc,
                              u32 iCount, u32 iOffset, Fmatrix *xform) {
  // Transfer vertices
  if (xform) {
  ...
  /*
  {
          ...
          if (sit!=send)	{
                  u64		item	= (u64(iOffset)<<48) |
(u64(iOffset)<<32) | (u64(iOffset)<<16) | u64(iOffset);
                  __asm	{
                          femms
                          movq		mm0,	[item];
                          mov			eax,	[sit];
