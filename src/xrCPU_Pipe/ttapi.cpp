...
process1:
  ...
  ttapi_dwFastIter = (DWORD)((dwNumIter * liFrequency.QuadPart) /
                             ((liEnd.QuadPart - liStart.QuadPart) * 25));
...
process2:
  ...
  ttapi_dwSlowIter = (DWORD)((dwNumIter * liFrequency.QuadPart) /
                             ((liEnd.QuadPart - liStart.QuadPart) * 2));
  ...
  if ((ttapi_threads_handles =
           (LPHANDLE)malloc(sizeof(HANDLE) * ttapi_threads_count)) == NULL)
    return 0;
  ...
  for (DWORD i = 0; i < ttapi_threads_count; i++) {
    ...
    if ((ttapi_threads_handles[i] =
             CreateThread(NULL, 0, &ttapiThreadProc, &ttapi_worker_params[i], 0,
                          &dwThreadId)) == NULL)
      return 0;
