...
const int MAX_O = 8;
const int UP_FREQ = 5, INT_BITS = 7, PERIOD_BITS = 7,
  ...
  *pText++ = fs.Symbol;
  ...
  for (pc = MaxContext; pc != MinContext; pc = pc->Suffix) {
    if ((ns1 = pc->NumStats) != 0) {
      if ((ns1 & 1) != 0) {
        pc->Stats =
            (PPM_CONTEXT::STATE *)ExpandUnits(pc->Stats, (ns1 + 1) >> 1);
        if (!pc->Stats)
          goto RESTART_MODEL;
      }
    } else {
...
static void EncodeFile2(FILE *DecodedFile) {
  ...
  rewind(DecodedFile);
  MinContext = MaxContext;
  for (int ns = MinContext->NumStats, NumBytes = 0;;
    ...
    if (FoundState->Successor)
      ...
      while (MinContext->Suffix != NULL) {
        ...
        if (!MinContext->NumStats)
          ...
          for (FoundState = MinContext->Stats; FoundState->Symbol != c;
               FoundState++)
            ;
        if (FoundState->Successor) {
          MinContext = FoundState->Successor;
...
inline double GetExtraBits(double f, double e, double f1, double sf1,
                           double sf0, int ns) {
...
void _STDCALL EncodeFile(FILE *DecodedFile, int MaxOrder, int MaxSize) {
  ...
  //    MaxContext->write(0,fp1,fp2,fp3); fclose(fp1);        fclose(fp2);
  //    fclose(fp3);
}
/*
