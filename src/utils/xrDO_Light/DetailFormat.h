...
2 - slots
    ...
    m_Header.object_count=m_Objects.size();
        // header
        F.w_chunk
(DETMGR_CHUNK_HEADER,&m_Header,sizeof(DetailHeader));
    // objects
    ...
    // slots
        ...
        F.write
(m_Slots.begin(),m_Slots.size()*sizeof(DetailSlot)); F.close_chunk
();
...
public:
  IC u32 version() { return _version; }
  ...
  IC u32 slot_index(int _x, int _z) const { return _z * size_x + _x; }
...
{
  u32 y_base : 12; // 11	// 1 unit = 20 cm, low = -200m, high = 4096*20cm -
                   // 200 = 619.2m
  u32 y_height : 8; // 20	// 1 unit = 10 cm, low = 0,     high = 256*10
                    // ~= 25.6m
...
IC bool is_empty(const DetailPalette &pallete) {
  return pallete.a0 == 0 && pallete.a1 == 0 && pallete.a2 == 0 &&
         pallete.a3 == 0;
}
