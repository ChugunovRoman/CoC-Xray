...
crlf(std::basic_ostream<char, Traits> &ostr) {
  ostr << "\r\n";
  return ostr;
}
...
template <typename Traits>
inline std::basic_ostream<char, Traits> &
operator<<(std::basic_ostream<char, Traits> &ostr,
  ...
  for (int i = 0; i < at_awards_count; ++i) {
    ...
    ostr << tmp_award_name << "=" << profile_data.m_awards[i].m_count << crlf
         << tmp_award_name
         << "_rdate=" << profile_data.m_awards[i].m_last_reward_date << crlf;
  }
  for (int i = 0; i < bst_score_types_count; ++i) {
    char const *tmp_score_name = get_best_score_name(enum_best_score_type(i));
    ostr << tmp_score_name << "=" << profile_data.m_best_scores[i] << crlf;
  }
  return ostr;
}
