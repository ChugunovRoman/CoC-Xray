...
static void debug_callback(GSIDebugCategory theCat, GSIDebugType theType,
  ...
  // hope compiler will find specializations like: template <size_t size> int
  // sprintf( char (&buffer)[size],

  sprintf(tmp_prefix_buffer, "GameSpy: [%s][%s] ", gGSIDebugCatStrings[theCat],
...
GSIACResult check_gamespy_services(void) {
  ...
  case GSIACUnavailable:
    std::cerr
        << "GameSpy: Online services are unavailable\r\n"
        << "GameSpy: Please visit www.mygame.com for more information.\r\n";
    break;
  case GSIACTemporarilyUnavailable:
    std::cerr
        << "GameSpy: Online services are temporarily unavailable.\r\n"
        << "GameSpy: Please visit www.mygame.com for more information.\r\n";
    break;
  default:
    break;
...
sake_processor::gp_processor::gp_processor() : m_connection(NULL) {
  ...
  GPResult tmp_res = gpInitialize(&m_connection, game_product_id,
                                  game_namespace_id, GP_PARTNERID_GAMESPY);
  m_complete = false;
  if (tmp_res != GP_NO_ERROR) {
    std::cerr << "GameSpy: GP initialization failed, errcode: " << tmp_res
              << std::endl;
...
bool sake_processor::gp_processor::login() {
  assert(m_connection);
  GPResult tmp_res =
      gpConnect(&m_connection, gp_nick, gp_email, gp_pass, GP_NO_FIREWALL,
                GP_BLOCKING, &gp_processor::login_result_cb, this);
...
void sake_processor::gp_processor::login_result_cb(GPConnection *connection,
  ...
  if (tmp_rarg->result != GP_NO_ERROR) {
    std::cerr << "GameSpy: GP failed to connect, errcode: " << tmp_rarg->result
              << std::endl;
    return;
  }
...
sake_processor::sake_processor() {
  ...
  sakeSetProfile(m_sake_inst, m_gp_processor.m_profile_id,
                 m_gp_processor.m_login_ticket);
...
bool sake_processor::get_profile(char const *name,
...
bool sake_processor::create_request_string() {
  m_tmp_string = "";
  m_request_names.erase(std::remove_if(m_request_names.begin(),
                                       m_request_names.end(), security_treat),
                        m_request_names.end());
...
void sake_processor::fetch() {
  ...
  SAKERequest tmp_request =
      sakeSearchForRecords(m_sake_inst, &m_get_records_input,
                           &sake_processor::request_callback, this);
  m_processing_request = true;

  if (!tmp_request) {
    SAKEStartRequestResult tmp_result = sakeGetStartRequestResult(m_sake_inst);
    std::cerr << "GameSpy: SAKE request failed, errcode: " << tmp_result
              << std::endl;
    request_callback(m_sake_inst, tmp_request, SAKERequestResult_UNKNOWN_ERROR,
                     NULL, NULL, this);
  }
}
...
void sake_processor::init_request_fields() {
  ...
  for (int i = 0; i < at_awards_count; ++i) {
    ...
    m_field_names_store[ci + ap_award_id] =
...
bool sake_processor::process_record(SAKESearchForRecordsInput *in,
  ...
  for (int i = 0; i < in->mNumFields; ++i, ++last_fi) {
    ...
    enum_awards_t tmp_award_id =
        get_award_by_stat_id_name(fields[last_fi].mName);
    ...
    if (tmp_award_id < at_awards_count) {
      dest_data.m_awards[tmp_award_id].m_last_reward_date =
          fields[last_fi].mValue.mInt;
      continue;
    }
    enum_best_score_type tmp_best_score_id =
        get_best_score_type_by_sname(fields[last_fi].mName);
...
void sake_processor::request_callback(SAKE sake, SAKERequest request,
  ...
  if (tmp_out) {
    ...
    if ((tmp_out->mNumRecords == max_request_records) &&
        (static_cast<std::size_t>(me->m_current_offset) <
         me->m_request_names.size())) {
      me->fetch();
      return;
    }
  }
