...
bool profiles_cache::search(user_name_t const &profname,
  ...
  cache_t::iterator tmp_iter = std::lower_bound(
      m_cache.begin(), m_cache.end(), search_item, cache_item_less_p());
  if (tmp_iter != m_cache.end()) {
    if (strcmp(tmp_iter->m_user_name, profname))
      return false;

    if ((xray::get_clock_ms() - tmp_iter->m_creation_time) >=
        cache_expire_time) {
...
bool const
profiles_cache::cache_item_less_p::operator()(cache_item const &left,
...
bool const
profiles_cache::cache_item_expired_p::operator()(cache_item const &left) const {
...
void profiles_cache::clear_expired() {
  ...
  m_cache.erase(std::remove_if(m_cache.begin(), m_cache.end(), tmp_predicate),
                m_cache.end());
}