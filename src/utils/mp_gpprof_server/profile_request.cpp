...
inline char *decode_url(char const (&src_url)[ArraySize],
                        char (&dst_url)[ArraySize]) {
...
char *extract_username(char const *uri, char const *root_path,
                       user_name_t &dst_user) {
  ...
  if (sscanf(uri, tmp_format, tmp_uname)) {
    ...
    strncpy(dst_user, decode_url(tmp_uname, decoded_uname),
            sizeof(dst_user) - 1);
...
fetch_profile_request::fetch_profile_request(request_ptr_t fcgx_request,
    ...
    : m_fcg_request(fcgx_request), m_profile_name(profile_name),
      m_in(m_fcg_request->in), m_out(m_fcg_request->out),
      m_err(m_fcg_request->err) {}

fetch_profile_request::~fetch_profile_request(){};

void fetch_profile_request::complete_success(
    gamespy_profile::profile_data const &profdata) {
  m_out << crlf << m_profile_name << crlf << profdata << crlf;
