...
GT2Bool StartHosting(void) {
  ...
  // create the socket
  result = gt2CreateSocket(&socket, gt2AddressToString(0, PORT, NULL), 0, 0,
                           SocketErrorCallback);
  if (result != GT2Success) {
  ...
  rcode = qr2_init(NULL, NULL, QR2_BASE_PORT, "gmtest", "HA6zkS", 1, 1,
                   server_key_callback, playerkey_callback, teamkey_callback,
                   keylist_callback, count_callback, adderror_callback, NULL);
...
GT2Bool StartConnecting(const char *server) {
  ...
  printf("Connecting to %s (timeout after %d seconds)...\n", address,
         TIMEOUT / 1000);
  result = gt2Connect(socket, &connection, address, NULL, 0, TIMEOUT,
                      &connectionCallbacks, GT2True);
  if (result != GT2Success) {
