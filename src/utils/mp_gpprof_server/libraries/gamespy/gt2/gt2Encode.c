...
#if defined(_PS2) || defined(_UNIX) || defined(_PS3) || defined(_WIN64) ||     \
    defined(_X360)
...
static int gtiDecodeBits(int bitcount, char *inBuffer, int inLength,
                         va_list *args) {
  char bucket;
  int i;
...
static int gtiEncodeBits(int bitcount, char *outBuffer, int outLength,
                         va_list *args) {
  char bucket = 0;
  int i;
...
static int gtiDecodeSingle(char elemType, char *inBuffer, int inLength,
                           va_list *args) {
  switch (elemType) {
  case GT_INT:
...
static int gtiEncodeSingle(char elemType, char *outBuffer, int outLength,
                           va_list *args) {
  switch (elemType) {
  case GT_INT:
...
static int gtInternalEncodeV(int usetype, GTMessageType msgType,
                             const char *fmtString, char *outBuffer,
  ...
  while (*fmtString) {
    ...
    {
      for (bitCounter = fmtString;
           *bitCounter == GT_BIT && bitCounter - fmtString <= 8; bitCounter++) {
      };
      elemSize = gtiEncodeBits((int)(bitCounter - fmtString), outBuffer,
                               outLength, args);
...
static int gtDecodeInternalV(int usetype, const char *fmtString, char *inBuffer,
  ...
  while (*fmtString) {
    if (*fmtString == GT_BIT) // see how many
    {
      ...
      elemSize = gtiDecodeBits((int)(bitCounter - fmtString), inBuffer,
                               inLength, args);
      fmtString = bitCounter - 1;
    } else
  ...
  // If it's not 0, then the encoding and decoding strings probably did not
  // match which would generally indicate a bug PANTS - commented out because we
  // could be decoding the rest with a gtDecodeNoType
  //	assert(inLength == 0);
