...
void ClientReceivedCallback(GT2Connection connection, GT2Byte *message, int len,
                            GT2Bool reliable) {
...
GT2ConnectionCallbacks ClientConnectionCallbacks = {
    NULL, ClientReceivedCallback, ClientClosedCallback};
...
void ServerConnectedCallback(GT2Connection connection, GT2Result result,
  ...
  if (result == GT2Success) {
    ...
    if (result == GT2Rejected)
      ...
      gt2Reject(clientConnection,
                (const GT2Byte *)("Proxy failed to connect to server."), -1);
...
void ServerReceivedCallback(GT2Connection connection, GT2Byte *message, int len,
                            GT2Bool reliable) {
  GT2Connection clientConnection;
...
GT2ConnectionCallbacks ServerConnectionCallbacks = {
    ServerConnectedCallback, ServerReceivedCallback, ServerClosedCallback};
...
void ConnectAttemptCallback(GT2Socket socket, GT2Connection connection,
  ...
  if (result != GT2Success) {
    gt2Reject(connection,
              (const GT2Byte *)("Proxy failed to create a new socket."), -1);
    return;
  }

  // Connect to the real server.
  //////////////////////////////
  result = gt2Connect(serverSocket, &serverConnection, RemoteAddress, message,
                      len, 0, &ServerConnectionCallbacks, GT2False);
  if (result != GT2Success) {
    gt2Reject(connection,
              (const GT2Byte *)("Proxy failed to connect to server."), -1);
...
void DisplayStats(void) {
  ...
  printf("%d connect attempts, %d (%d%%) accepted, %d (%d%%) rejected\n",
         numConnectAttempts, numAccepted,
  ...
  printf("client: %d messages, %d (%d%%) reliable, %d (%d%%) unreliable\n",
         total, clientReliableMessagesSent,
  ...
  printf("server: %d messages, %d (%d%%) reliable, %d (%d%%) unreliable\n",
         total, serverReliableMessagesSent,
  ...
  printf("%d closes, %d (%d%%) client, %d (%d%%) server\n", total, clientCloses,
         (total) ? (clientCloses * 100) / total : 0, serverCloses,
