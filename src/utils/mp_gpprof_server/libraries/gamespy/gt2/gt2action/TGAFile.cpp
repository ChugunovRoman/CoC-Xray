...
#define PNGCREATEERR                                                           \
  16 // internal PNG lib behavior - contact smaller animals s.w.
#define IMGDLLINTERNAL                                                         \
  17 // misc unexpected behavior error - contact smaller animals s.w.
...
#define DEVOPNOSUPPORT                                                         \
  26 // device does not support an operation required by this function
...
public:
  ...
  BOOL
  Save8BitTGA(const char *fileName,    // output path
...
TGAFile::TGAFile() { m_error = IMGOK; }

////////////////////////////////////////////////////////////////////////////
//		GetTGADimns
//		Find dims of the image in a TGA file
//		Returns - TRUE on success
BOOL TGAFile::GetTGADims(const char *fileName, UINT32 *width, UINT32 *height) {
  // for safety
  *width = 0;
  *height = 0;
  FILE *fp;
 ...
 *
 *******************************************************************************/
BOOL TGAFile::IsFileTGA(const char *fileName) {
  TGAHEADER tgahd;
  FILE *fp;
  ...
  *height = 0;
  ...
  if (tgahd.ColorMapType != 0) {
    ...
    for (__int32 i = 0; i < (tgahd.CMapStart + tgahd.CMapLength); i++) {
      TGA_GetMapEntry(&CColMap[i].rgbRed, &CColMap[i].rgbGreen,
                      &CColMap[i].rgbBlue, &Alpha, fp, tgahd.CMapDepth);
    }

    // If the TGA file actually needs the color map
    // Set the mode to show this
    if ((tgahd.ImageType != TGA_RAWRGBTYPE) &&
        (tgahd.ImageType != TGA_RAWMONOTYPE) &&
        (tgahd.ImageType != TGA_RAWENCODETYPE))
      mode = mode | MAPPED;
  }
  ...
  if ((tgahd.ImageType == TGA_MAPENCODETYPE) ||
      (tgahd.ImageType == TGA_RAWENCODETYPE) ||
      (tgahd.ImageType == TGA_MONOENCODETYPE))
    mode = mode | RLENCO;
  ...
  //	pNew = (LPVOID) theApp.m_TGABuffer.GetBuffer((size_t)(tgahd.Width *
  // tgahd.Height * 4));
  ...
  *height = 0;
  ...
  if (fp == NULL) {
    ...
    if (tgahd.ColorMapType != 0) {
      // Get the color map
      for (__int32 i = 0; i < (tgahd.CMapStart + tgahd.CMapLength); i++) {
        TGA_GetMapEntry(&pal[i].rgbRed, &pal[i].rgbGreen, &pal[i].rgbBlue,
                        &Alpha, fp, tgahd.CMapDepth);
      }
    } else {
      m_error = INVALIDTGAERR;
      fclose(fp);
...
TGAFile::LoadTGAThumbnail(const char *fileName, // Name of file
  ...
  *height = 0;
  ...
  if (fp == NULL) {
    ...
    if ((stampWidth <= 0) || (stampWidth > 64) || (stampHeight <= 0) ||
        (stampHeight > 64)) {
      m_error = INVALIDTGAERR;
      fclose(fp);
      return NULL;
...
//		Return 0 on Success
BOOL TGAFile::TGA_GetMapEntry(BYTE *Red, BYTE *Green, BYTE *Blue, BYTE *Alpha,
                              FILE *fp, UINT32 Depth) {
  UINT32 j, k, l;
  BYTE i, r, g, b, a = 0;
  long lResult;
...
__int32 TGAFile::TGA_GetFileVersion(FILE *fp) {
  long result;
  fpos_t pos;
...
BOOL TGAFile::TGA_GetPixelValue(BYTE *rRed, BYTE *rGrn, BYTE *rBlu, BYTE *rAlp,
                                BYTE **ppTGAData, UINT32 PixelDepth,
                                RGBQUAD *CColMap) {
  //
...
//		Retruns - TRUE on success
BOOL TGAFile::TGA_GetPixelValue(BYTE *rRed, BYTE *rGrn, BYTE *rBlu, BYTE *rAlp,
                                FILE *fp, UINT32 PixelDepth, RGBQUAD *CColMap) {
  BYTE i, j, k;
  long lResult;
...
extern "C" unsigned char *LoadTGAFile(const char *filename, int *width,
                                      int *height) {
  TGAFile tgaFile;
  return (unsigned char *)tgaFile.LoadTGA(filename, (UINT32 *)width,
                                          (UINT32 *)height);
}