...
unsigned long
    lastServerUpdate; // The last time we received an update from the server.
...
static void ClientConnectedCallback(GT2Connection connection, GT2Result result,
...
static void ClientReceivedCallback(GT2Connection connection, GT2Byte *message,
  ...
  if (type == MSG_S_ADDCLIENT) {
    ...
    // Decode it.
    /////////////
    rcode = gtDecode(MSG_S_ADDCLIENT_STR, (char *)message, len, &newPlayerIndex,
                     nick);
    if (rcode == -1)
      return;
  ...
  /////////////////
    ...
    /////////////
    rcode =
        gtDecode(MSG_S_DELCLIENT_STR, (char *)message, len, &delPlayerIndex);
    if (rcode == -1)
      return;
  ...
  /////////////////
    ...
    // Decode the header.
    /////////////////////
    rcode = gtDecode(MSG_S_UPDATE_STR, (char *)message, len, &updateID,
                     &updatedClients, &updatedObjects);
    if (rcode == -1)
      return;
    message += rcode;
    len -= rcode;
    ...
    for (i = 0; i < updatedClients; i++) {
      ...
      rcode = gtDecodeNoType(MSG_S_UPDATE_CLIENT_STR, (char *)message, len,
                             &index, &packedPosition[0], &packedPosition[1],
                             &packedRotation, &score, &forward, &backward,
                             &right, &left, &dead);
      if (rcode == -1)
        return;
      message += rcode;
      ...
      // Copy the values in.
      //////////////////////
      SetV2f(player->position, UnsignedShortToPosition(packedPosition[0]),
    ...
    for (i = 0; i < updatedObjects; i++) {
      ...
      rcode = gtDecodeNoType(MSG_S_UPDATE_OBJECT_STR, (char *)message, len,
                             &type, &packedPosition[0], &packedPosition[1],
                             &packedRotation, &time);
      if (rcode == -1)
        return;
      message += rcode;
      ...
      //////////////////////
      object->type = type;
      SetV2f(object->position, UnsignedShortToPosition(packedPosition[0]),
             UnsignedShortToPosition(packedPosition[1]));
...
static void ClientClosedCallback(GT2Connection connection,
...
void SendUpdate(void) {
  if (connected) {
    ...
    ///////////////////////
    rcode = gtEncode(
        MSG_C_UPDATE, MSG_C_UPDATE_STR, buffer, sizeof(buffer),
        nextClientUpdateID++, RotationToUnsignedShort(localRotation),
        localMotion == FORWARD ? 1 : 0, localMotion == BACKWARD ? 1 : 0,
        localTurning == RIGHT ? 1 : 0, localTurning == LEFT ? 1 : 0);
    if (rcode != -1) {
      // Send the message.
      ////////////////////
...
void SendPress(const char *value) {
  if (connected) {
    ...
    //////////////////////
    rcode =
        gtEncode(MSG_C_PRESS, MSG_C_PRESS_STR, buffer, sizeof(buffer), value);
    if (rcode != -1) {
      // Send the message.
      ////////////////////
...
void SendChat(const char *message) {
  if (connected) {
    ...
    //////////////////////
    rcode =
        gtEncode(MSG_C_CHAT, MSG_C_CHAT_STR, buffer, sizeof(buffer), message);
...
void ClientThink(unsigned long now) {
  ...
  if ((localIndex != -1) && !players[localIndex].dead)
    localRotation = ComputeNewRotation(localRotation, localTurning, diff,
                                       PLAYER_TURN_SPEED);
...
static void ClientSendMonitor(GT2Connection connection, int filterID,
...
static void ClientReceiveMonitor(GT2Connection connection, int filterID,
...
GT2Bool InitializeClient(void) {
  ...
  // Connect.
  ///////////
  result = gt2Connect(Socket, &Connection, serverAddress, (GT2Byte *)buffer,
                      rcode, 0, &connectionCallbacks, GT2False);
  if (result != GT2Success)
    return GT2False;
