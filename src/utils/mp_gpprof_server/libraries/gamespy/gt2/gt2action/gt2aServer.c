...
static void Broadcast(const char *message, int len, int exceptIndex,
                      GT2Bool reliable) {
  int i;
...
void BroadcastText(const char *message, int exceptIndex, GT2Bool reliable) {
  ...
  //////////////////////
  rcode =
      gtEncode(MSG_S_CHAT, MSG_S_CHAT_STR, buffer, sizeof(buffer), messageCopy);
  if (rcode != -1) {
...
static void SendDelClient(int sendClient, int delClient) {
  ...
  if (rcode != -1) {
    // Send the message.
    ////////////////////
    gt2Send(clients[sendClient].connection, (const GT2Byte *)buffer, rcode,
            GT2True);
  }
}
...
void SendNumAsteroids(int total, int sendClient) {
  ...
  // Encode the message.
  /////////////////////
  rcode = gtEncode(MSG_S_NUMASTEROIDS, MSG_S_NUMASTEROIDS_STR, buffer,
                   sizeof(buffer), (byte)total);
  ;
  if (rcode != -1) {
    // Send the message.
    ////////////////////
...
static void SendUpdates(void) {
  ...
  rcode = gtEncode(MSG_S_UPDATE, MSG_S_UPDATE_STR, buffer, len,
                   nextServerUpdateID++, numClients, numObjects);
  if (rcode == -1)
    return;
  buffer += rcode;
  len -= rcode;
  ...
  for (i = 0; i < MAX_CLIENTS; i++) {
    if (clients[i].used) {
      ...
      rcode = gtEncodeNoType(MSG_S_UPDATE_CLIENT_STR, buffer, len, i,
                             ...
                             client->score, client->motion == FORWARD ? 1 : 0,
                             client->motion == BACKWARD ? 1 : 0,
                             client->turning == RIGHT ? 1 : 0,
                             client->turning == LEFT ? 1 : 0, client->dead);
      if (rcode == -1)
        return;
      buffer += rcode;
      len -= rcode;
  ...
  for (i = 0; i < MAX_OBJECTS; i++) {
    if (sObjects[i].used) {
      ...
      rcode = gtEncodeNoType(MSG_S_UPDATE_OBJECT_STR, buffer, len, object->type,
...
static void ServerReceivedCallback(GT2Connection connection, GT2Byte *message,
  ...
  if (type == MSG_C_UPDATE) {
    ...
    // Decode it.
    /////////////
    rcode = gtDecode(MSG_C_UPDATE_STR, (char *)message, len, &updateID,
                     &packedRotation, &forward, &backward, &right, &left);
    if (rcode == -1)
      return;
    ...
    if (client->lastUpdate && !client->dead && client->motion) {
      ...
      ////////////////////////////
      ComputeNewPosition(client->position, client->position, client->motion,
                         client->rotation, diff, PLAYER_SPEED, GT2True);
    }
  ...
  ////////
    ...
    ///////////////////////////////
    rcode =
        gtDecode(MSG_C_CHAT_STR, (char *)message, len, buffer + strlen(buffer));
    if (rcode == -1)
      return;
...
static void ServerClosedCallback(GT2Connection connection,
...
GT2Bool InitializeServer(void) {
  ...
  /////////////////////
  result =
      gt2CreateSocket(&Socket, PORT_STRING, 0, 0, ServerSocketErrorCallback);
