...
HCURSOR CGhttpmfcDlg::OnQueryDragIcon() { return (HCURSOR)m_hIcon; }

static GHTTPBool CompletedCallback(GHTTPRequest request, GHTTPResult result,
                                   char *buffer, GHTTPByteCount bufferLen,
                                   void *param) {
  CGhttpmfcDlg *dlg = (CGhttpmfcDlg *)param;

  static char *resultStrings[] = {
      "GHTTPSuccess",         "GHTTPOutOfMemory",      "GHTTPBufferOverflow",
      "GHTTPParseURLFailed",  "GHTTPHostLookupFailed", "GHTTPSocketFailed",
      "GHTTPConnectFailed",   "GHTTPBadResponse",      "GHTTPRequestRejected",
      "GHTTPUnauthorized",    "GHTTPForbidden",        "GHTTPFileNotFound",
      "GHTTPServerError",     "GHTTPFileWriteFailed",  "GHTTPFileReadFailed",
      "GHTTPFileIncomplete",  "GHTTPFileToBig",        "GHTTPEncryptionError",
  ...
  if (GHTTPSuccess == result) {
    ...
    if (result < 0 ||
        result >= sizeof(resultStrings) / sizeof(resultStrings[0]))
...
void ProgressCallback(GHTTPRequest request, GHTTPState state,
                      const char *buffer, GHTTPByteCount bufferLen,
                      GHTTPByteCount bytesReceived, GHTTPByteCount totalSize,
                      void *param) {
  CGhttpmfcDlg *dlg = (CGhttpmfcDlg *)param;
  ...
  // added new states for asynch DNS lookup - set all to HOST_LOOKUP for radio
  // buttons
  if (dlg->m_state == GHTTPSocketInit || dlg->m_state == GHTTPHostLookup ||
      dlg->m_state == GHTTPLookupPending)
    dlg->m_state = 0;
  ...
  if (state == GHTTPReceivingFile) {
    if (totalSize == -1)
      ...
      dlg->m_soFar.Format("%d%% (%d / %d bytes)", percent, bytesReceived,
                          totalSize);
...
void PostCallback(GHTTPRequest request, int bytesPosted, int totalBytes,
...
void CGhttpmfcDlg::OnStart() {
  ...
  if (m_type == 0) {
    m_request = ghttpGetEx(m_url, m_headers, NULL, 0, NULL,
                           (GHTTPBool)m_throttle, (GHTTPBool)m_blocking,
                           ProgressCallback, CompletedCallback, this);
  } else if (m_type == 1) {
    m_request = ghttpSaveEx(m_url, m_saveAs, m_headers, NULL,
                            (GHTTPBool)m_throttle, (GHTTPBool)m_blocking,
                            ProgressCallback, CompletedCallback, this);
  } else if (m_type == 2) {
    m_request = ghttpStreamEx(m_url, m_headers, NULL, (GHTTPBool)m_throttle,
                              (GHTTPBool)m_blocking, ProgressCallback,
                              CompletedCallback, this);
  } else if (m_type == 3) {
    m_request = ghttpHeadEx(m_url, m_headers, (GHTTPBool)m_throttle,
                            (GHTTPBool)m_blocking, ProgressCallback,
                            CompletedCallback, this);
  } else if (m_type == 4) {
    GHTTPPost post;
    ...
    if (m_postFile) {
      ...
      ghttpPostAddFileFromMemory(post, "memfile", m_memFile, memFileSize,
                                 "steve.txt", NULL);
      ghttpPostAddFileFromDisk(post, "diskfile", "../ghttpMain.c", "main.c",
                               "text/html");
    }

#if 1
    m_request = ghttpGetEx(m_url, m_headers, NULL, 0, post,
                           (GHTTPBool)m_throttle, (GHTTPBool)m_blocking,
                           ProgressCallback, CompletedCallback, this);
#else
    m_request = ghttpPostEx(m_url, m_headers, post, (GHTTPBool)m_throttle,
                            (GHTTPBool)m_blocking, ProgressCallback,
                            CompletedCallback, this);
#endif
  }
...
void CGhttpmfcDlg::OnTimer(UINT nIDEvent) {
  if (nIDEvent == 50) {
    ...
    if (m_request >= 0) {
      ...
      // radio buttons
      if (m_state == GHTTPSocketInit || m_state == GHTTPHostLookup ||
          m_state == GHTTPLookupPending)
        m_state = 0;
      UpdateData(FALSE);
    }
  }
...
void CGhttpmfcDlg::OnIeSettings() {
  ...
  ////////////////////////////////////////
  result = RegOpenKeyEx(HKEY_CURRENT_USER, REGSTR_PATH_INTERNETSETTINGS, 0,
                        KEY_READ, &key);
  if (SUCCEEDED(result)) {
    ...
    len = sizeof(DWORD);
    data = 0;
    result = RegQueryValueEx(key, REGSTR_VAL_PROXYENABLE, 0, &type,
                             (LPBYTE)&data, &len);
    if (SUCCEEDED(result) && data) {
      ...
      len = 0;
      result =
          RegQueryValueEx(key, REGSTR_VAL_PROXYSERVER, 0, &type, NULL, &len);
      result = RegQueryValueEx(key, REGSTR_VAL_PROXYSERVER, 0, &type,
                               (LPBYTE)str.GetBuffer(len), &len);
      str.ReleaseBuffer();
      if (SUCCEEDED(result) && (len > 0)) {
        ...
        // "ftp=ftp_address;gopher=gopher_address:4;http=http_address:1;https=secure_address:2;socks=socks_address:5"
        // read as: protocol=server:port;protocol=server:port
        // If only protocol is to use proxy: "socks=socks_address:5"
        // Apparently, ports are optional - I would presume that you should
        // revert to the default port when it is missing
        //
        ...
        if (nStart != -1) {
          ...
          nStart = -1;
          nEnd = 0; // won't be used; need to initialize to prevent compiler
                    // warning
        }
