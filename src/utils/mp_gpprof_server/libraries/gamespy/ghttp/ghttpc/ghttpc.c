...
static gsi_char *stateStrings[] = {
    _T("Socket Init"),       _T("Host Lookup"),      _T("Lookup Pending"),
    _T("Connecting"),        _T("Securing Session"), _T("Sending Request"),
    _T("Posting"),           _T("Waiting"),          _T("Receiving Status"),
    _T("Receiving Headers"), _T("Receiving File")};

static gsi_char *resultStrings[] = {
    _T("GHTTPSuccess"),          _T("GHTTPOutOfMemory"),
    _T("GHTTPBufferOverflow"),   _T("GHTTPParseURLFailed"),
    _T("GHTTPHostLookupFailed"), _T("GHTTPSocketFailed"),
    _T("GHTTPConnectFailed"),    _T("GHTTPBadResponse"),
    _T("GHTTPRequestRejected"),  _T("GHTTPUnauthorized"),
    _T("GHTTPForbidden"),        _T("GHTTPFileNotFound"),
    _T("GHTTPServerError"),      _T("GHTTPFileWriteFailed"),
    _T("GHTTPFileReadFailed"),   _T("GHTTPFileIncomplete"),
    _T("GHTTPFileToBig"),        _T("GHTTPEncryptionError"),
...
static void DebugCallback(GSIDebugCategory theCat, GSIDebugType theType,
...
#if defined(_WIN32) && !defined(_XBOX)
  {
    ...
    sprintf(buffer, "[%s][%s] ", gGSIDebugCatStrings[theCat],
            gGSIDebugTypeStrings[theType]);
...
static void ProgressCallback(GHTTPRequest request, GHTTPState state,
...
int test_main(int argc, char **argv) {
  ...
  request = ghttpGet(_T("http://www.gamespy.net/images/dev_serv_main.jpg"),
                     GHTTPFalse, CompletedCallback, (void *)pendingRequests);
  ...
  request =
      ghttpGetEx(_T("http://google.com/intl/en_ALL/images/logo.gif"), NULL,
                 buffer, sizeof(buffer), NULL, GHTTPFalse, GHTTPFalse,
                 ProgressCallback, CompletedCallback, (void *)pendingRequests);
  CheckRequest(request, pendingRequests);
  pendingRequests++;
  ...
  request = ghttpSave(_T("http://www.gamespy.net/images/dev_serv_logo.jpg"),
                      _T("logo.jpg"), GHTTPFalse, CompletedCallback,
  ...
  // stream a page
  request = ghttpStreamEx(_T("http://www.gamespy.net"), NULL, NULL, GHTTPFalse,
                          GHTTPFalse, ProgressCallback, CompletedCallback,
                          (void *)pendingRequests);
  CheckRequest(request, pendingRequests);
  pendingRequests++;
  ...
  // get a header
  request =
      ghttpHead(_T("http://sdkdev.gamespy.com/games/st_ladder/web/index.html"),
                GHTTPFalse, CompletedCallback, (void *)pendingRequests);
  CheckRequest(request, pendingRequests);
  pendingRequests++;
...
#endif
      NULL, NULL, GHTTPFalse, GHTTPFalse, ProgressCallback, CompletedCallback,
      (void *)pendingRequests);
  ...
  for (i = 0; i < numRequests; i++) {
    if (results[i].started == GHTTPFalse)
      ...
      _tprintf(_T("%d: %s [%dms]\n"), i, resultStrings[results[i].result],
               results[i].stopTime - results[i].startTime);
  }
