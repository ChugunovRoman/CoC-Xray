...
#define IS_INSTANT_OP(op)                                                      \
  (((op) >= FIRST_INSTANT_OP) && ((op) <= LAST_INSTANT_OP))
...
typedef struct {
  ...
  GPProfile
      others[MAX_OTHERS];    // Remote profiles that this profile knows about.
...
bool RandomBool(void) { return (bool)RandomInt(0, 1); }
...
void RandomServer(void) {
  ...
  const char *server =
      servers[RandomInt(0, (sizeof(servers) / sizeof(*servers)) - 1)];
...
void Log(const char *buffer) {
  ...
  sprintf(timeBuffer,
          "%-2.2d/%-2.2d/%-2.2d %-2.2d:%-2.2d:%-2.2d: ", ltime->tm_mon + 1,
...
** GP GLOBAL CALLBACKS **
************************/
void GetInfoCallback(GPConnection *connnection, GPGetInfoResponseArg *arg,
                     Profile *profile);
void AddOther(Profile *profile, GPProfile gpProfile);
void PrintOp(Profile *profile, const char *opString);

void ErrorCallback(GPConnection *connection, GPErrorArg *arg,
                   Profile *profile) {
...
#define ERRORCODE(x)                                                           \
  ...
  sprintf(buffer + strlen(buffer), ", RESULT: %s (%d)", resultString,
          arg->result);
  sprintf(buffer + strlen(buffer), ", ERROR CODE: %s (0x%X)", errorCodeString,
          arg->errorCode);
...
void RecvBuddyStatusCallback(GPConnection *connection,
                             GPRecvBuddyStatusArg *arg, Profile *profile) {
  ...
  //////////////////////////
  if (RandomBool())
    gpGetInfo(&profile->gp, arg->profile, GP_DONT_CHECK_CACHE, GP_NON_BLOCKING,
              GetInfoCallback, profile);
...
#if 1
void RecvBuddyRequestCallback(GPConnection *connection,
                              GPRecvBuddyRequestArg *arg, Profile *profile) {
...
void RecvBuddyMessageCallback(GPConnection *connection,
                              GPRecvBuddyMessageArg *arg, Profile *profile) {
...
void RecvGameInviteCallback(GPConnection *connection, GPRecvGameInviteArg *arg,
                            Profile *profile) {
...
const int RemoteAuthProfiles[] = {58214083, 58214074, 58214075, 58214076,
                                  58214077, 58214078, 58214079, 58214080,
                                  58214081, 58214082};

void ConnectCallback(GPConnection *connection, GPConnectResponseArg *arg,
                     Profile *profile) {
  if (arg->result == GP_NO_ERROR) {
    ...
    if (arg->profile != RemoteAuthProfiles[profile->index % 10]) {
      ...
      sprintf(buffer,
              ...
              totalConnected, arg->profile,
              RemoteAuthProfiles[profile->index % 10], (profile->index % 10),
              ms / 1000, ms % 1000);
      Log(buffer);
    }
...
void GetInfoCallback(GPConnection *connection, GPGetInfoResponseArg *arg,
                     Profile *profile) {
  if (arg->result != GP_NO_ERROR) {
...
void FindPlayersCallback(GPConnection *connection,
                         GPFindPlayersResponseArg *arg, Profile *profile) {
  int i;
  ...
  for (i = 0; i < arg->numMatches; i++)
    gpGetInfo(&profile->gp, arg->matches[i].profile, GP_DONT_CHECK_CACHE,
              GP_NON_BLOCKING, GetInfoCallback, profile);
...
void ProfileSearchCallback(GPConnection *connection,
                           GPProfileSearchResponseArg *arg, Profile *profile) {
  int i;
...
void StartOp(Profile *profile) {
  ...
  case OpConnect:
...
#else
    {
      const char *authtokens[] = {
          ...
          "GMTyI447fNIYVh1vdI6aErka4TzJcYWctTU/Dzi2GK+4J3Vf/"
          "VAqQOs43R73I7dyX+u"};
      const char *partnerchallenges[] = {
          "WI[A)IuA", "RNd$)q)0", "LQTi;4#X", "AOA)(57=", "ob=0XiWJ",
          "mRnTENJ@", "9%@A+NKA", "L=0O)Lg2", "YR6[Zz%B", "$kWZ%]~@"};
      int tokenIndex = (profile->index % 10);
      gpConnectPreAuthenticated(&profile->gp, authtokens[tokenIndex],
                                partnerchallenges[tokenIndex], GP_FIREWALL,
                                GP_NON_BLOCKING, ConnectCallback, profile);
    }
#endif
    break;
  ...
  case OpGetInfo:
    ...
    if (profile->numOthers > 0)
      gpGetInfo(&profile->gp, RandomOther(profile), GP_DONT_CHECK_CACHE,
                GP_NON_BLOCKING, GetInfoCallback, profile);
    else
      profile->activeOp = OpNull;
    break;
  ...
  case OpProfileSearch:
    ...
    num = RandomInt(0, 3);
    if (num == 0)
      gpProfileSearch(&profile->gp, "pants", "dan@gamespy.com", NULL, NULL, 0,
                      GP_NON_BLOCKING, ProfileSearchCallback, profile);
    else if (num == 1)
      gpProfileSearch(&profile->gp, "crt", NULL, NULL, NULL, 0, GP_NON_BLOCKING,
                      ProfileSearchCallback, profile);
    else
      gpProfileSearch(&profile->gp, "STRESS-TEST", NULL, NULL, NULL, 0,
                      GP_NON_BLOCKING, ProfileSearchCallback, profile);
    break;
  ...
  case OpFindPlayers:
    ...
    // Find some players.
    /////////////////////
    gpFindPlayers(&profile->gp, RandomInt(1, PRODUCT_ID_MAX), GP_NON_BLOCKING,
                  FindPlayersCallback, profile);
    break;
#endif
  ...
  case OpSetInfo:
    ...
    gpSetInfod(&profile->gp, GP_BIRTHDAY, RandomInt(1, 28), RandomInt(1, 12),
               RandomInt(1940, 1990));
    break;
  ...
  case OpSetStatus5:
    ...
    ///////////////////////////////////////
    gpSetStatus(&profile->gp, GP_ONLINE, RandomString(string1),
...
void ProcessProfile(Profile *profile) {
  ...
  if (profile->gpInitialized) {
    ...
    /////////////////////////////////////////////////////
    if ((profile->activeOp == OpConnect) &&
        ((current_time() - profile->connectTime) > (200 * 60 * 1000))) {
      char buffer[256];
      gsi_time ms = (current_time() - profile->connectTime);
    ...
    ////////////////////////
    if ((now > profile->disconnectTime) &&
        ((profile->activeOp != OpConnect) || shuttingDown)) {
...
void InitializeProfile(Profile *profile) {
  ...
  gpSetCallback(&profile->gp, GP_RECV_BUDDY_STATUS, RecvBuddyStatusCallback,
                profile);
#if 1
  gpSetCallback(&profile->gp, GP_RECV_BUDDY_REQUEST, RecvBuddyRequestCallback,
                profile);
  gpSetCallback(&profile->gp, GP_RECV_BUDDY_MESSAGE, RecvBuddyMessageCallback,
                profile);
  gpSetCallback(&profile->gp, GP_RECV_GAME_INVITE, RecvGameInviteCallback,
                profile);
#endif
  ...
  profile->disconnectTime +=
      (((rand() * (CONNECT_TIME_MAX - CONNECT_TIME_MIN)) / RAND_MAX) +
       CONNECT_TIME_MIN);
...
** VALIDATION **
***************/
void GetUserNicksCallback(GPConnection *connection,
...
void NewUserResponse(GPConnection *connection, GPNewUserResponseArg *arg,
                     void *param) {
  GSI_UNUSED(connection);
  GSI_UNUSED(arg);
  GSI_UNUSED(param);
...
bool Validate(void) {
  ...
  /*
          for(i = 41 ; i <= 1000 ; i++)
          {
                  ...
                  gpNewUser(&gp, "gpstress", email, "gpstress", GP_BLOCKING,
     NewUserResponse, NULL); printf("%d\n", i);
          }
  */
  ...
  for (i = 0; i < numProfiles; i++) {
    ...
    if (strcmp(email, profile->email) != 0) {
      ...
      /////////////////////////////////
      gpGetUserNicks(&gp, email, profile->password, GP_BLOCKING,
                     GetUserNicksCallback, &validationData);
    }
    ...
    if (profile->invalid) {
      ...
      sprintf(buffer, "%s (%d) : %04d: Invalid profile (%s@%s)\n", profilesFile,
              (profile->index + 1), profile->index, profile->nick,
              profile->email);
      printf(buffer);
      OutputDebugString(buffer);
    }
...
void StartShutdown(unsigned long now) {
  ...
  for (i = 0; i < numProfiles; i++) {
    ...
    if (profile->gpInitialized) {
      ...
      profile->disconnectTime =
          (now + ((rand() *
                   ((numConnections / SHUTDOWN_DISCONNECTS_PER_SEC) * 1000)) /
                  RAND_MAX));
    }
  }
...
void Frame(unsigned long now) {
  ...
  if (!shuttingDown && (numConnections < maxConnections) &&
    ...
    numConnects = min(numConnects, (MAX_OUTSTANDING_CONNECTIONS -
                                    (numConnections - numConnected)));
