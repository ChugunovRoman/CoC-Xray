...
// * added due to PS3 SDK 092.00x and above - all stub libraries require you to
// load the module
...
gsi_bool _NetworkInit() {
  ...
  if (r != CELL_OK) {
    printf(
        "ccNetworkInitializeNetwork: sys_net_initialize_network() failed: %i\n",
        r);
    return gsi_false;
  }
...
void _NetworkClose() { sys_net_finalize_network(); }
...
#define aMemoryPoolSize (1024 * 1024 * 8)
  ...
  if (aMemoryPool == NULL) {
    ...
    gsMemMgrContext c = gsMemMgrCreate(gsMemMgrContext_Default, "Default",
                                       aMemoryPool, aMemoryPoolSize);
    GSI_UNUSED(c);
  }
  return aMemoryPool;
#else
...
int main(int argc, char **argp) {
  ...
  /*
          // spawn IO module
          if (sys_process_spawn(&pid, "brio.elf", NULL, 0, PRIO, 0) !=
     SUCCEEDED) { printf("sys_process_spawn(brio.elf) failed\n"); return (0);
          }
  */
