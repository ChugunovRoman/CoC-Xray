...
struct SpursSpeexTaskDesc {
  SpursSpeexTaskDesc() : mDebugPause(false) {}
  int mQuality;
...
// just add your commands here, try to keep them globally unique for debugging
// purposes
...
public:
  SpursSpeexTaskManager(spursThreadSupportInterface *threadInterface,
                        unsigned int maxNumOutstandingTasks);

  ~SpursSpeexTaskManager();

  /// call initialize in the beginning of the frame, before
  /// addCollisionPairToTask
  int initialize();

  int issueEncodeTask(int16_t *inBuffer, int inBufferSize, int encodedFrameSize,
  ...
  int issueEncodeInitTask(int theQuality, int theGviSpeexSamplesPerSecond,
                          SpursSpeexTaskOutput *taskOutput,
                          char *m_userAllocatedSpeexBuffer,
                          int userAllocatedSpeexBufferSize);

  int issueDecodeAddTask(char *decoderStateBuffer, int decoderStateBufferSize,
                         char *inBuffer, int inBufferSize, int encodedFrameSize,
                         short *outBuffer, int outBufferSize,
                         struct SpursSpeexTaskOutput *taskOutput);

  int issueDecodeSetTask(char *decoderStateBuffer, int decoderStateBufferSize,
                         char *inBuffer, int inBufferSize, int encodedFrameSize,
                         short *outBuffer, int outBufferSize,
                         struct SpursSpeexTaskOutput *taskOutput);

  int issueDecodeInitTask(char *decoderStateBuffer, int decoderStateBufferSize,
                          int sampleRate,
                          struct SpursSpeexTaskOutput *taskOutput);

  /// call flush to submit potential outstanding work to SPUs and wait for all
  /// involved SPUs to be finished
  int flush();
};
