...
unsigned int
_g_uiSPURSELFSearchPathCount=sizeof(_g_apcSPURSELFSearchPaths)/sizeof(char *);
 ...
 */
// UPDATE: this function is not currently being used in the speex task or spurs
// manager
void _SpursPrintfThreadMain(uint64_t arg) {
  sys_event_t event;
  int iReturn;
  ...
  while (1) {
    iReturn =
        sys_event_queue_receive(_g_SpuPrintfEventQueue, &event, SYS_NO_TIMEOUT);
...
// Loads the SPURS .ELF files.  If we're doing an external release, they're
linked
// in, so we're ok.
int loadBulletSpursElfs() {
        ...
        for (int iELF=0; iELF<SPU_ELF_LAST; iELF++) {
                ...
                for (int iPath=0; ((unsigned
int)iPath<_g_uiSPURSELFSearchPathCount) &&
...
_g_aSPURSElfs[iELF].pcElfName); iReturn=CELL_SPURS_EINVAL; break;
                }
        }

        // If something went wrong, free up any of the .ELF files that loaded
successfuly. if (iReturn!=CELL_SPURS_OK) { unloadBulletSpursElfs();
        }
...
int initializeSpursTaskSet(CellSpurs *pSpurs, int iSPUCount,
                           uint8_t *puiPriorities) {
  ...
  if (pSpurs) {
    ...
    iReturn =
        sys_ppu_thread_get_priority(idCurrentThread, &iCurrentThreadPriority);
    if (iReturn != CELL_OK) {
  ...
  // 										(uint64_t)
  // 0, SPU_PRINTF_THREAD_PRIO,
  // SPU_PRINTF_THREAD_STACK_SIZE, 										SYS_PPU_THREAD_CREATE_JOINABLE,
  ...
  //
  // 	iReturn=cellSpursAttachLv2EventQueue(g_spursInstance,
  // _g_SpuPrintfEventQueue, 										 &uiPrintfPort, 0);
  // if (iReturn!=CELL_OK) { 		return CELL_SPURS_EMISC;
  // 	}

  iReturn = cellSpursCreateTaskset(g_spursInstance, &g_spursTaskSet, 0,
                                   pPriorities, iNumSPUs);
...
int checkSpursTaskSet() {
  if (!g_bSpursInitialized) {
    int returnCode =
        initializeSpursTaskSet(0, g_iDefaultSPUCount, 0) != CELL_SPURS_OK;
    return returnCode;
  }
  return CELL_SPURS_OK;
}
 ...
 */
int spursConfiguration_initWithSpurs(CellSpurs *pSpurs, int iSPUCount,
                                     uint8_t auiPriorities[8]) {
  if (g_bSpursInitialized) {
 ...
 * true if initialized.
 */
bool spursConfiguration_isSpursInitialized() { return g_bSpursInitialized; }
...
int SpursSupportInterface::startSPU() {
  ...
  if (!m_bQueueInitialized) {
    ...
    iReturn = cellSpursQueueInitialize(
        &g_spursTaskSet, &m_responseQueue, m_aResponseBuffer,
        sizeof(CellSPURSArgument), CELL_SPURS_RESPONSE_QUEUE_SIZE,
...
int SpursSupportInterface::sendRequest(uint32_t uiCommand, uint32_t uiArgument0,
  ...
  iReturn = cellSpursCreateTask(&g_spursTaskSet, &taskId, g_SpursTaskElfStart,
                                NULL, 0, 0, &arguments.spursArgument);
  if (iReturn != CELL_OK) {
    return -2;
  }
 ...
 */
int SpursSupportInterface::waitForResponse(unsigned int *puiArgument0,
