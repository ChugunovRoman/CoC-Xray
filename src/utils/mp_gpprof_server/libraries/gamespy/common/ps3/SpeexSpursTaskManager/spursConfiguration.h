 ...
 * This is good if you intend to use SPURS elsewhere in your code, the
 *management can be shared across all processes.
 *
 * The second way is to control the number of SPUs used by SPURS with
 * initWithSpuCount.  Software will create its own instance of SPURS when it
 *needs it using that maximum number of SPUs.
 *
 * If you do neither of the two, Software will create its own instance of SPURS
 *when it needs it using CELL_SPURS_DEFAULT_SPU_COUNT SPUs.
 *
 * terminate() can be used to either detatch from an existing SPURS or to
 * terminate the Software-created SPURS.  Note that this will only work if all
 *collision scenes have been destroyed, otherwise SPURS would still be needed.
 *
 * Creating a new scene will cause SPURS to re-initialize.
 *
 * isSpursInitialized can be used to query whether software is currently using
 *SPURS.
 *
 **/
