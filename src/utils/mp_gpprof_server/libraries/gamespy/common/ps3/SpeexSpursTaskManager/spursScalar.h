...
Processor) #if defined (__sun) || defined (__sun__) || defined (__sparc) ||
...
SIMD_FORCE_INLINE btScalar btAtan2(btScalar x, btScalar y) { return atan2f(x,
y); } SIMD_FORCE_INLINE btScalar btExp(btScalar x) { return expf(x); }
...
SIMD_FORCE_INLINE bool      btFuzzyZero(btScalar x) { return btFabs(x) <
SIMD_EPSILON; }
