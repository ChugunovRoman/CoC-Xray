...
// Init the GSI memory manager (optional - for limiting GSI mem usage)
        ...
        {
                gsMemMgrContext	c = gsMemMgrCreate(gsMemMgrContext_Default,
"Default",aMemoryPool, aMemoryPoolSize);
        }
        return aMemoryPool;
#else
        return NULL;
...
void gsiMemManagedClose(void * aMemoryPool)
{
        ...
        gsi_u32		MemAvail = 	gsMemMgrMemAvailGet
(gsMemMgrContext_Default); gsi_u32		MemUsed	 =
gsMemMgrMemUsedGet			(gsMemMgrContext_Default); gsi_u32
HwMark	 =	gsMemMgrMemHighwaterMarkGet	(gsMemMgrContext_Default);

        gsDebugFormat(GSIDebugCat_GP, GSIDebugType_Memory,
GSIDebugLevel_Comment, "MemAvail %u: MemUsed%u  MemHWMark %u\n",
MemAvail,MemUsed,HwMark); gsMemMgrDumpStats(); gsMemMgrDumpAllocations();
...
int main(int argc, char** argp)
{
        ...
        //void	*heap	= gsiMemManagedInit();
                ...
                gsSetDebugLevel(GSIDebugCat_All, GSIDebugType_All,
GSIDebugLevel_Verbose); #endif
