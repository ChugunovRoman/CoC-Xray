...
static void ShowMenuScreen(void) {
  ...
  const char keyboardRows[numKeyboardRows][32] = {
      "1 2 3 4 5 6 7 8 9 0 - + = _ . < ", " A B C D E F G H I J K L M N O  ",
  ...
  while (!done) {
    ...
    SetTopScreenLineCentered(topTextLine, SCYellow,
                             configuration->topScreenText);
    ...
    for (i = 0; i < MAX_EXTRA_TEXT_STRINGS; i++) {
      if (options & SCREEN_OPTION_EXTRAS_CENTERED)
        SetTopScreenLineCentered(extraTextLine + i, SCWhite,
                                 Screen.extraText[i]);
      else
    ...
    if (list) {
      ...
      for (i = 0; i < numListRows; i++) {
        ...
        // show the item
        SetBottomScreenLineCentered(startListLine + 2 + i, color,
                                    Screen.list[listScroll + i]);
      }

      // show the bottom line
      SetBottomScreenLineCentered(startListLine + numListRows + 2, normalColor,
                                  listBorder);
      ...
      if (listScroll < (numListItems - numListRows)) {
        ...
        SetBottomScreenLineCentered(startListLine + numListRows + 3, color,
                                    "down");
      }
    }
    ...
    if (keyboard) {
      ...
      for (i = 0; i < numKeyboardRows; i++) {
        ...
        if (touching && (touchingLine == keyboardLine)) {
          ...
          SetBottomScreenLineHighlight(keyboardLine, normalColor,
                                       keyboardRows[i], touchingPos, range,
                                       highlightColor);
        } else {
    ...
    for (choiceIndex = 0; choiceIndex < Screen.numChoices; choiceIndex++) {
      ...
      if ((choices[choiceIndex].options & CHOICE_OPTION_DISABLED) ||
          ((choices[choiceIndex].options &
            CHOICE_OPTION_NEEDS_LIST_SELECTION) &&
           (Screen.listSelection == -1))) {
        color = disabledColor;
...
void ExitMenu(void) { SetNextMenuScreen(&MenuExitScreenConfiguration); }

MenuScreen *GetMenuScreen(void) { return &Screen; }