...
static const char mscRenameSelectedConfiguration[] =
    "Rename Selected Configuration";
static const char mscDeleteSelectedConfiguration[] =
...
static MenuScreenConfiguration msInitializingWireless = {
    "Initializing Wireless",   {{mscCancel}},
    InitializingWirelessInit,  InitializingWirelessChose,
    InitializingWirelessThink, SCREEN_OPTION_ANIMATED};
...
static MenuScreenConfiguration msFailedToInitializeWireless = {
    "Failed to Initialize Wireless", {{mscOK}}, FailedToInitializeWirelessInit,
    FailedToInitializeWirelessChose, NULL,      SCREEN_OPTION_EXTRAS_CENTERED};
...
static MenuScreenConfiguration msConfigurationDeleted = {
    "Configuration Deleted", {{mscOK}}, NULL, ConfigurationDeletedChose};
...
static MenuScreenConfiguration msSearchingForSavedNetwork = {
    "Searching for Saved Network", {{mscCancel}},
    SearchingForSavedNetworkInit,  SearchingForSavedNetworkChose,
    SearchingForSavedNetworkThink, SCREEN_OPTION_ANIMATED};
...
static MenuScreenConfiguration msSearchFailed = {
    "Search Failed",   {{mscOK}}, SearchFailedInit,
    SearchFailedChose, NULL,      SCREEN_OPTION_EXTRAS_CENTERED};
...
static MenuScreenConfiguration msBadSecurityKeyEntered = {
    "Bad Security Key Entered", {{mscBack}}, BadSecurityKeyEnteredInit,
    BadSecurityKeyEnteredChose, NULL,        SCREEN_OPTION_EXTRAS_CENTERED};
...
static MenuScreenConfiguration msConnectingToNetwork = {
    "Connecting to Network",  {{mscCancel}},
    ConnectingToNetworkInit,  ConnectingToNetworkChose,
    ConnectingToNetworkThink, SCREEN_OPTION_ANIMATED};
...
static MenuScreenConfiguration msFailedToConnectToNetwork = {
    "Failed to Connect to Network", {{mscBack}}, FailedToConnectToNetworkInit,
    FailedToConnectToNetworkChose,  NULL,        SCREEN_OPTION_EXTRAS_CENTERED};
...
static MenuScreenConfiguration msConnectingToInternet = {
    "Connecting to Internet",  {{mscCancel}},
    ConnectingToInternetInit,  ConnectingToInternetChose,
    ConnectingToInternetThink, SCREEN_OPTION_ANIMATED};
...
static MenuScreenConfiguration msConfigurationSaved = {
    "Configuration Saved", {{mscContinue}}, NULL, ConfigurationSavedChose};
...
static IWConfig WirelessConfig = {3, BSSDescBuffer, IW_BSS_SIZE *BSSDESC_MAX,
                                  0};
...
static void IW_Callback(IWNotice *arg) {
  GSI_UNUSED(arg);
  OS_TPrintf("Notify: %s | Result: %s\n", IWNotifyToString(arg->notify),
             IWResultToString(arg->result));
}
...
static gsi_bool IW_CheckResult(int result, const char *funcName) {
  ...
  default:
    snprintf(FailureReason, sizeof(FailureReason), "%s result: %s", funcName,
             IWResultToString(result));
    return gsi_false;
  }
}

static gsi_bool IW_Poll(int polling, int success, const char *funcName,
                        gsi_bool *keepPolling) {
  int phase;
  ...
  snprintf(FailureReason, sizeof(FailureReason), "%s phase: %s", funcName,
           IWPhaseToString(phase));
  return gsi_false;
}
...
static void LoadSettings(void) {
  ...
  SavedSettings.numConfigs =
      (u8)min(SavedSettings.numConfigs, MAX_SAVED_CONFIGS);
}
...
void WirelessCleanup(void) {
  ...
  do {
    if (!IW_Poll(IW_PHASE_LINK_IDLE, IW_PHASE_IDLE, "IW_Disconnect",
                 &KeepPolling))
...
static void InitializingWirelessThink(void) {
  // poll the phase
  if (KeepPolling) {
    if (!IW_Poll(IW_PHASE_WAIT_IDLE, IW_PHASE_IDLE, "IW_Startup",
                 &KeepPolling)) {
      SetNextMenuScreen(&msFailedToInitializeWireless);
      return;
    }
...
static void SelectNetworkConfigurationInit(void) {
  ...
  for (i = 0; i < SavedSettings.numConfigs; i++) {
    strncpy(screen->list[i], SavedSettings.configs[i].name,
            MAX_LIST_STRING_LEN);
...
static void RenameConfigurationInit(void) {
  ...
  strncpy(screen->extraText[0], SavedSettings.configs[SelectedConfig].name,
          MAX_EXTRA_TEXT_STRING_LEN);
  screen->extraText[0][MAX_EXTRA_TEXT_STRING_LEN - 1] = '\0';

  strncpy(screen->keyboardText, SavedSettings.configs[SelectedConfig].name,
          MAX_KEYBOARD_TEXT_LEN);
...
static void RenameConfigurationChose(const char *choice) {
  ...
  if (choice == mscRename) {
    strncpy(SavedSettings.configs[SelectedConfig].name, screen->keyboardText,
            MAX_CONFIG_NAME_LEN);
...
static void ConfirmDeleteChose(const char *choice) {
  ...
  if (choice == mscYes) {
    ...
    for (i = SelectedConfig; i < SavedSettings.numConfigs; i++)
      memcpy(&SavedSettings.configs[i], &SavedSettings.configs[i + 1],
             sizeof(SavedConfig));
...
static void SearchingForNetworksInit(void) {
  ...
  result = IW_Search(IW_BSSID_ANY, IW_ESSID_ANY,
                     IW_OPT_CHANNEL_ALL | IW_OPT_SCANTYPE_PASSIVE);
...
static void SearchingForNetworksThink(void) {
  ...
  if (KeepPolling) {
    // poll the search
    if (!IW_Poll(IW_PHASE_IDLE_SCAN, IW_PHASE_SCAN, "IW_Search",
                 &KeepPolling)) {
      IW_Search(NULL, NULL, 0);
      SetNextMenuScreen(&msSearchFailed);
      return;
...
static void SearchingForSavedNetworkInit(void) {
  ...
  // search for access points
  result = IW_Search(BSSID, IW_ESSID_ANY,
                     IW_OPT_CHANNEL_ALL | IW_OPT_SCANTYPE_PASSIVE);
  if (!IW_CheckResult(result, "IW_Search")) {
    SetNextMenuScreen(&msSearchFailed);
    return;
...
static void NetworkInformationInit(void) {
  ...
  snprintf(screen->extraText[0], MAX_EXTRA_TEXT_STRING_LEN,
           "  MAC: %02X:%02X:%02X:%02X:%02X:%02X", APList[APIndex].bssid[0],
           APList[APIndex].bssid[1], APList[APIndex].bssid[2],
           APList[APIndex].bssid[3], APList[APIndex].bssid[4],
           APList[APIndex].bssid[5]);

  // channel
  snprintf(screen->extraText[1], MAX_EXTRA_TEXT_STRING_LEN, "  Channel: %d",
  ...
  if (choice == mscEnter) {
    ...
    if (len != correctLen) {
      snprintf(FailureReason, sizeof(FailureReason),
               "Bad length (%d instead of %d)", len, correctLen);
    ...
    for (i = 0; i < len; i += 2) {
      ...
      if ((val[0] == -1) || (val[1] == -1)) {
        ...
        snprintf(FailureReason, sizeof(FailureReason),
                 "Bad value ('%c' instead of hex)", badChar);
        SetNextMenuScreen(&msBadSecurityKeyEntered);
        return;
      }
...
static void ConnectingToNetworkThink(void) {
  if (KeepPolling) {
    // poll the connect
    if (!IW_Poll(IW_PHASE_IDLE_LINK, IW_PHASE_LINK, "IW_Connect",
                 &KeepPolling)) {
      IW_Disconnect();
      SetNextMenuScreen(&msFailedToConnectToNetwork);
      return;
...
static void ConnectingToInternetInit(void) {
  ...
  if (result < 0) {
    snprintf(FailureReason, sizeof(FailureReason), "SOC_Startup failed: %d",
             result);
...
#if defined(SUPPORT_SAVED_CONFIGS)
    if (BackupExists() && NewConfig &&
        (SavedSettings.numConfigs < MAX_SAVED_CONFIGS))
...
static void SaveNetworkConfigurationInit(void) {
  ...
  strncpy(screen->keyboardText, (const char *)APList[APIndex].ssid,
          MAX_KEYBOARD_TEXT_LEN);
  screen->keyboardText[MAX_KEYBOARD_TEXT_LEN - 1] = '\0';
}
...
static void SaveNetworkConfigurationChose(const char *choice) {
  ...
  if (choice == mscSave) {
    memcpy(SavedSettings.configs[SavedSettings.numConfigs].bssid, BSSID,
           IW_BSSID_SIZE);
    memcpy(SavedSettings.configs[SavedSettings.numConfigs].wep, WEPKey,
           IW_WEP_SIZE);
    strncpy(SavedSettings.configs[SavedSettings.numConfigs].name,
            screen->keyboardText, MAX_CONFIG_NAME_LEN);
    SavedSettings.configs[SavedSettings.numConfigs]
        .name[MAX_CONFIG_NAME_LEN - 1] = '\0';
    SavedSettings.numConfigs++;
    SaveSettings();
