...
static void ScrollTopScreen(void) {
  ...
  MI_CpuClearFast((void *)&gTopScreen[(SCREEN_HEIGHT - 1) * SCREEN_WIDTH],
                  SCREEN_WIDTH * sizeof(gTopScreen[0]));
}
...
void PrintChar(char c) {
  ...
  if (c == '\n') {
    ...
    gTopScreen[((SCREEN_HEIGHT - 1) * SCREEN_WIDTH) + gPos] =
        (u16)((palette << 12) | c);
    gPos++;
  }
}
...
void SetPrintMode(int mode) { gPrintMode = mode; }

static void SetScreenLine(u16 *screen, int line, int offset, ScreenColor color,
                          const char *text, int pos, int range,
  ...
  MI_CpuClearFast((void *)&screen[line * SCREEN_WIDTH],
                  SCREEN_WIDTH * sizeof(gTopScreen[0]));
...
static void SetScreenLineCentered(u16 *screen, int line, ScreenColor color,
                                  const char *text) {
  int len;
  int offset;
...
void SetTopScreenLineHighlight(int line, ScreenColor color, const char *text,
                               int pos, int range, ScreenColor posColor) {
...
void SetBottomScreenLineCentered(int line, ScreenColor color,
                                 const char *text) {
  SetScreenLineCentered(gBottomScreen, line, color, text);
}

void SetBottomScreenLineHighlight(int line, ScreenColor color, const char *text,
                                  int pos, int range, ScreenColor posColor) {
