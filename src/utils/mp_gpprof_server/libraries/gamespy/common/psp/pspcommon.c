...
//    (e.g. You should have your own network setup code which supports other
//    configurations)
...
#define SCE_APCTL_STACKSIZE                                                    \
  (SCE_NET_APCTL_LEAST_STACK_SIZE + SCE_APCTL_HANDLER_STACKSIZE)
...
int gsiPspLoadRequiredModules() {
  ...
  if (ret < 0) {
    printf("sceUtilityLoadModule(SCE_UTILITY_MODULE_NET_COMMON) failed. ret = "
           "0x%x\n",
           ret);
  ...
  if (ret < 0) {
    printf("sceUtilityLoadModule(SCE_UTILITY_MODULE_NET_INET) failed. ret = "
           "0x%x\n",
           ret);
    gsiPspUnloadRequiredModules();
    return ret;
...
int gsiPspUnloadRequiredModules() {
  ...
  if (ret < 0) {
    printf("sceUtilityUnloadModule(SCE_UTILITY_MODULE_NET_INET) failed. ret = "
           "0x%x\n",
           ret);
  }
  ...
  if (ret < 0) {
    printf("sceUtilityUnloadModule(SCE_UTILITY_MODULE_NET_COMMON) failed. ret "
           "= 0x%x\n",
           ret);
  }
  return ret;
...
void gsiPspApctlHandler(int prev_state, int new_state, int event,
...
int main(int argc, char **argp) {
  ...
  if (ret < 0) {
    printf("gsiPspLoadRequiredModules failed. See previous return value. "
           "return = 0x%x\n",
           ret);
  ...
  if (ret < 0) {
    printf("gsiPspStartNetworkModules failed See previous return value. return "
           "= 0x%x\n",
           ret);
    return ret;
  }
  ...
  if (ret < 0) {
    printf(
        "gsiPspnetDisconnect failed See previous return value. return = 0x%x\n",
        ret);
    return ret;
  }
