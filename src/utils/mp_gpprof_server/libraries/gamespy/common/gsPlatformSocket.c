...
#ifdef _PS2
#ifdef SN_SYSTEMS
  rcode = setsockopt(sock, SOL_SOCKET, (isblocking) ? SO_BIO : SO_NBIO, &argp,
                     sizeof(argp));
#endif
...
#define MAX_IPS sceLIBNET_MAX_INTERFACE
  ...
  aNumInterfaces =
      sceInetGetInterfaceList(&gGSIInsockClientData, &gGSIInsockSocketBuffer,
                              aInterfaceIdArray, MAX_IPS);
  if (aNumInterfaces < 1)
    return NULL;
  ...
  for (aInterfaceNum = 0; aInterfaceNum < aNumInterfaces; aInterfaceNum++) {
    ...
    int result =
        sceInetInterfaceControl(&gGSIInsockClientData, &gGSIInsockSocketBuffer,
...
#define MAX_IPS 5
  ...
  if ((rcode == XNET_GET_XNADDR_NONE) ||
      (rcode == XNET_GET_XNADDR_TROUBLESHOOT))
    return NULL;
