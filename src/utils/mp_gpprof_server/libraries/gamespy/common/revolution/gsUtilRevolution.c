...
static const char *GOAGetUniqueID_Internal(void) {
  ...
  // format it
  sprintf(keyval, "%02X%02X%02X%02X%02X%02X0000", aMac[0] & 0xFF,
          aMac[1] & 0xFF, aMac[2] & 0xFF, aMac[3] & 0xFF, aMac[4] & 0xFF,
          aMac[5] & 0xFF);
...
char *gsiCTime(time_t *theTime) {
  ...
  sprintf(str, "%s %s %02d %02d:%02d:%02d %d\n", GSIWeekDayNames[ptm->tm_wday],
          GSIMonthNames[ptm->tm_mon], ptm->tm_mday, ptm->tm_hour, ptm->tm_min,
