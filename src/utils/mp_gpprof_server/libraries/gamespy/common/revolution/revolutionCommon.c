...
static void SetDefaultIpConfig(NCDIpConfig *theIpConfig) {
  ...
  theIpConfig->adjust.maxTransferUnit =
      1300; // Value can be 1460 depending on network library
...
// StopNetwork
//
void StopNetwork() { (void)SOCleanup(); }
...
void main(int argc, char **argv) {
  ...
  if ((u32)arenaHi - (u32)arenaLo < SOCKET_HEAPSIZE_DEFAULT)
    OSHalt("Insufficient memory in MEM2 for socket lib. Check "
           "SOCKET_HEAPSIZE_DEFAULT");
  heapHandleSocket = MEMCreateExpHeapEx(arenaLo, SOCKET_HEAPSIZE_DEFAULT,
