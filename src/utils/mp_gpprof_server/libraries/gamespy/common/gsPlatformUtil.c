...
#if defined(_WIN32) /*|| defined(_PS2)*/ || defined(_UNIX) ||                  \
    defined(_REVOLUTION)
...
#if (defined(_WIN32) || /*defined(_PS2) ||*/ defined(_UNIX) ||                 \
     defined(_REVOLUTION)) &&                                                  \
    !defined(GSI_NO_THREADS) && !defined(GSI_NO_ASYNC_DNS)
...
#else // if * not a supported platform OR * no threads allowed OR * no async
      // lookup allowed
      ///////////////////////////////////////////////////////////////////////////////////
      // if !(_WIN32 ||_PS2 || _LINUX || _MACOSX || _REVOLUTION) ||
...
// added support for winsock2
    ...
    (defined(GSI_WINSOCK2) || defined(_X360))
...
static const char *GOAGetUniqueID_Internal(void) {
  ...
  // format it
  sprintf(keyval, "%02X%02X%02X%02X%02X%02X0000", MAC[0] & 0xFF, MAC[1] & 0xFF,
...
static const char *GetMAC(void) {
  ...
  for (i = 0; i < num; i++) {
    ...
    // get the type
    len = sizeof(type);
    if (sceEENetGetIfinfo(interface->ifn_name, sceEENET_IFINFO_IFTYPE, &type,
                          &len) != 0)
      continue;
    ...
    if (sceEENetGetIfinfo(interface->ifn_name, sceEENET_IFINFO_MACADDR,
                          &linkAddress, &len) != 0)
      continue;
...
static const char *GetMAC(void) {
  ...
  int result =
      sceInetInterfaceControl(&gGSIInsockClientData, &gGSIInsockSocketBuffer, 1,
                              sceInetCC_GetHWaddr, MAC, sizeof(MAC));
...
const char *GOAGetUniqueID_Internal(void) {
  ...
  if (RegOpenKeyExA(HKEY_CURRENT_USER, REG_KEY, 0, KEY_ALL_ACCESS, &thekey) !=
      ERROR_SUCCESS)
    docreate = 1;
  else
    docreate = 0;
  ret = RegQueryValueExA(thekey, (LPCSTR) "Crypt", 0, &thetype, (LPBYTE)keyval,
                         &len);
#else
  FILE *f;
  f = fopen("id.bin", "r");
...
#ifdef _WIN32
    if (docreate) {
      ret = RegCreateKeyExA(HKEY_CURRENT_USER, REG_KEY, 0, NULL,
                            REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL,
                            &thekey, &disp);
    }
    RegSetValueExA(thekey, (LPCSTR) "Crypt", 0, REG_SZ, (const LPBYTE)keyval,
                   strlen(keyval) + 1);
#else
    f = fopen("id.bin", "w");
    if (f) {
...
#if (!defined(_PS2) && !defined(_PS3) && !defined(_XBOX) && !defined(_PSP)) || \
    defined(UNIQUEID)
