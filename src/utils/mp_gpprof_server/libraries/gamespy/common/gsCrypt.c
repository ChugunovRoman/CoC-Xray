...
static gsi_i32
gsCryptRSAOAEPEncryptBuffer(const gsCryptRSAKey *publicKey,
...
static gsi_bool gsiCryptRSAMaskData(unsigned char *data, gsi_u32 len,
                                    const unsigned char *maskSeed,
                                    gsi_u32 maskLen);
#else
static gsi_i32
gsCryptRSAPKCS1EncryptBuffer(const gsCryptRSAKey *publicKey,
                             const unsigned char *plainText, gsi_u32 len,
                             unsigned char buffer[GS_CRYPT_RSA_BYTE_SIZE]);
...
gsi_bool gsiCryptRSAMaskData(unsigned char *data, gsi_u32 dataLen,
                             const unsigned char *seed, gsi_u32 seedLen) {
  int i = 0;
  int k = 0;
...
// Perform the hash
#if (GS_CRYPT_HASHSIZE == GS_CRYPT_MD5_HASHSIZE)
    {
      ...
      char hashHexStr[GS_CRYPT_RSA_DATABLOCKSIZE * 2 +
                      1]; // hexstr of hash "A1" rather than 0xA1
      ...
      seedPlusIter[GS_CRYPT_RSA_DATABLOCKSIZE + 3] =
          (gsi_u8)(i / GS_CRYPT_HASHSIZE);
...
gsCryptRSAOAEPEncryptBuffer(const gsCryptRSAKey *publicKey,
...
#if (GS_CRYPT_HASHSIZE == GS_CRYPT_MD5_HASHSIZE)
  const gsi_u8 lhash[GS_CRYPT_HASHSIZE] = {
      0xd4, 0x1d, 0xd4, 0x1d, 0x8c, 0xd9, 0x8f, 0x00, 0xb2,
...
#endif
  const unsigned int maxPlainTextLen =
      GS_CRYPT_RSA_BYTE_SIZE - 2 * GS_CRYPT_HASHSIZE - 2;
  ...
  memset(&packet->maskedData[GS_CRYPT_HASHSIZE], 0,
         padSize); // pad with zero bytes
  packet->maskedData[GS_CRYPT_HASHSIZE + padSize] =
      0x01; // RSA encoding format ID (EME-OAEP)
  ...
  //       f. apply it with xor
  gsiCryptRSAMaskData(packet->maskedData, GS_CRYPT_RSA_DATABLOCKSIZE,
                      packet->maskedSeed, GS_CRYPT_HASHSIZE);

  //       g. use the masked datablock to generate a mask for the seed
  //       h. apply it with xor
  gsiCryptRSAMaskData(packet->maskedSeed, GS_CRYPT_HASHSIZE, packet->maskedData,
  ...
  gsLargeIntPowerMod(&lintRSAPacket, &publicKey->exponent, &publicKey->modulus,
  &lintRSAPacket); gsLargeIntReverseBytes(&lintRSAPacket);
...
gsCryptRSAPKCS1EncryptBuffer(const gsCryptRSAKey *publicKey,
  ...
  if (len > (GS_CRYPT_RSA_BYTE_SIZE -
             11)) // 2 byte header, 8 byte pad minimum, 1 byte separator
    return -1;
  ...
  if (gsi_is_false(gsLargeIntSetFromMemoryStream(
          &lintRSAPacket, (const gsi_u8 *)buf, GS_CRYPT_RSA_BYTE_SIZE)) ||
      gsi_is_false(gsLargeIntPowerMod(&lintRSAPacket, &publicKey->exponent,
                                      &publicKey->modulus, &lintRSAPacket)) ||
...
#else
  return gsCryptRSAPKCS1DecryptBuffer(privateKey, cipherText, plainText,
                                      lenout);
#endif
}
...
// This is included here as a testing utility but should probably not be used in
// a game client.
...
static gsi_i32 gsCryptRSAPKCS1DecryptBuffer(
  ...
  if (gsi_is_false(gsLargeIntReverseBytes(
          &lintRSAPacket)) || // reverse from bytebuffer to lint format
      gsi_is_false(gsLargeIntPowerMod(&lintRSAPacket, &privateKey->exponent,
                                      &privateKey->modulus, &lintRSAPacket)) ||
      gsi_is_false(gsLargeIntReverseBytes(
          &lintRSAPacket)) // reverse back into a bytebuffer
  ) {
    return -1;
  }

  // check post exponentiation length
  if (lintRSAPacket.mLength <
      (GS_CRYPT_RSA_BYTE_SIZE / GS_LARGEINT_DIGIT_SIZE_BYTES))
    return -1;
  ...
  // the rest is the msg
  memcpy(plainText, (temp + i + 1),
         GS_CRYPT_RSA_BYTE_SIZE);                        // +1 to skip the 0x00
...
static gsi_i32 gsCryptRSAOAEPDecryptBuffer(
  ...
  gsLargeIntReverseBytes(
      &lintRSAPacket); // reverse from bytebuffer to lint format
  gsLargeIntPowerMod(&lintRSAPacket, &privateKey->exponent,
                     &privateKey->modulus, &lintRSAPacket);
  ...
  memcpy(plainText, &packet->maskedData[i],
         (size_t)(GS_CRYPT_RSA_DATABLOCKSIZE - i));
  *lenout = (gsi_u32)(GS_CRYPT_RSA_DATABLOCKSIZE -
                      i); // final length = blocksize - pad
  return 0;
}
#endif
...
gsi_i32 gsCryptRSASignData(const gsCryptRSAKey *privateKey,
  ...
  // 2) Sign
  return gsCryptRSASignHash(privateKey, hash, GS_CRYPT_MD5_HASHSIZE,
                            signedDataOut, lenOut);
}

///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
// The more usual use of RSA signatures
//     Constructs a PKCS1 signature form of type  {
//     [0x00][0x01][0xFF..FF][0x00][hash oid][hash] } = RSA key length
gsi_i32 gsCryptRSASignHash(const gsCryptRSAKey *privateKey,
  ...
  gsi_u32 aKeyByteLength = privateKey->modulus.mLength *
                           GS_LARGEINT_DIGIT_SIZE_BYTES; // key length in bytes
  ...
  gsi_u8 sha1Header[15] = {0x30, 0x21, 0x30, 0x09, 0x06, 0x05, 0x2B, 0x0E,
  ...
  // pad with 0xFF
  memset(&writeBuf[2], 0xFF,
         aKeyByteLength - hashLen - anOidLen - aReservedLength);
  ...
  // copy in the oid
  if (hashLen == GS_CRYPT_MD5_HASHSIZE)
    memcpy(&writeBuf[aKeyByteLength - hashLen - anOidLen], md5Header,
           sizeof(md5Header));
  else if (hashLen == GS_CRYPT_SHA1_HASHSIZE)
    memcpy(&writeBuf[aKeyByteLength - hashLen - anOidLen], sha1Header,
           sizeof(sha1Header));
  else
  ...
  // sign (a.k.a. encrypt)
  gsLargeIntPowerMod(&dataToSign, &privateKey->exponent, &privateKey->modulus,
                     &dataToSign);
...
gsi_i32 gsCryptRSAVerifySignedHash(const gsCryptRSAKey *publicKey,
  ...
  gsLargeIntPowerMod(&lintRSASignature, &publicKey->exponent,
                     &publicKey->modulus, &lintRSASignature);
  gsLargeIntReverseBytes(&lintRSASignature);
  ...
  if ((i + sizeof(md5Header) + GS_CRYPT_MD5_HASHSIZE) ==
