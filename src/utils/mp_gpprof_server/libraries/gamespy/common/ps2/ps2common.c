...
// up to four DNS servers can be specified, and the list must be terminated with
// an empty string
...
int ent_cnf_load_config(const char *fname, const char *usr_name) {
  ...
  ret = sceSifCallRpc(&cd, ENT_CNF_SIFRPC_LOAD_CONFIG, 0, (void *)rpc_buffer,
                      ee_rpc_size(len), (void *)rpc_buffer,
...
int ent_cnf_set_config(void) {
  ...
  ret = sceSifCallRpc(&cd, ENT_CNF_SIFRPC_SET_CONFIG, 0, NULL, 0,
...
static int do_initialisation(void) {
  ...
  // initialize eenet
  rcode =
      sceEENetInit(eenet_pool, EENET_MEMSIZE, EENET_TPL, 8192, EENET_APP_PRIO);
...
//	PreparePowerOff();
  ...
  result = sceInsockSetSifMBindRpcValue(NETBUFSIZE, sceLIBNET_STACKSIZE,
                                        sceLIBNET_PRIORITY);
  if (result < 0)
    return result;

  result = sceLibnetInitialize(&gGSIInsockClientData, NETBUFSIZE,
                               sceLIBNET_STACKSIZE, sceLIBNET_PRIORITY);
  if (result < 0)
    return result;

  result =
      sceLibnetRegisterHandler(&gGSIInsockClientData, gGSIInsockSocketBuffer);
  if (result < 0)
    return result;

  result = load_set_conf_extra(&gGSIInsockClientData, gGSIInsockSocketBuffer,
                               NET_DB, USR_CONF_NAME, sceLIBNETF_AUTO_UPIF);
  if (result < 0)
    return result;

  result = sceLibnetWaitGetAddress(
      &gGSIInsockClientData, gGSIInsockSocketBuffer, if_id,
      sceLIBNET_MAX_INTERFACE, &myaddr, sceLIBNETF_AUTO_UPIF);
  if (result < 0)
    return result;
...
void load_voice_modules(void) {
  ...
  load_module("host0:/usr/local/sce/iop/modules/lgaud.irx", sizeof(lgaudArgs),
              lgaudArgs);
