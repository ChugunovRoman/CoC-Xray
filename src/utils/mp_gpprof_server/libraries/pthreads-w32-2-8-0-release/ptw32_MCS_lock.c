 ...
 */
INLINE void ptw32_mcs_flag_set(LONG *flag) {
  HANDLE e = (HANDLE)PTW32_INTERLOCKED_COMPARE_EXCHANGE(
      (PTW32_INTERLOCKED_LPLONG)flag, (PTW32_INTERLOCKED_LONG)-1,
 ...
 */
INLINE void ptw32_mcs_flag_wait(LONG *flag) {
  if (0 == InterlockedExchangeAdd((LPLONG)flag, 0)) /* MBR fence */
  {
    ...
    if (0 == PTW32_INTERLOCKED_COMPARE_EXCHANGE((PTW32_INTERLOCKED_LPLONG)flag,
 ...
 */
INLINE void ptw32_mcs_lock_acquire(ptw32_mcs_lock_t *lock,
                                   ptw32_mcs_local_node_t *node) {
 ...
 * ACM Transactions on Computer Systems, 9(1):21-65, Feb. 1991.
 */
INLINE void ptw32_mcs_lock_release(ptw32_mcs_local_node_t *node) {
  ptw32_mcs_lock_t *lock = node->lock;
  ptw32_mcs_local_node_t *next =
      (ptw32_mcs_local_node_t *)InterlockedExchangeAdd((LPLONG)&node->next,
                                                       0); /* MBR fence */

  if (0 == next) {
    /* no known successor */

    if (node == (ptw32_mcs_local_node_t *)PTW32_INTERLOCKED_COMPARE_EXCHANGE(
                    (PTW32_INTERLOCKED_LPLONG)lock, (PTW32_INTERLOCKED_LONG)0,
    ...
    next = (ptw32_mcs_local_node_t *)InterlockedExchangeAdd((LPLONG)&node->next,
                                                            0); /* MBR fence */
  }
