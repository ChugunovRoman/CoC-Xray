...
struct ptw32_thread_t_ {
...
#endif
  DWORD thread;
  HANDLE threadH; /* Win32 thread handle - POSIX thread is invalid if threadH ==
                     0 */
  ...
  int detachState;
  pthread_mutex_t
      threadLock;     /* Used for serialised access to public thread state */
...
#define EXCEPTION_PTW32_SERVICES                                               \
  MAKE_SOFTWARE_EXCEPTION(SE_ERROR, PTW32_SERVICES_FACILITY,                   \
...
extern PTW32_INTERLOCKED_LONG(WINAPI *ptw32_interlocked_compare_exchange)(
...
/* Thread Reuse stack bottom marker. Must not be NULL or any valid pointer to
 * memory. */
...
extern BOOL
    ptw32_smp_system; /* True: SMP system, False: Uni-processor system */
...
PTW32_INTERLOCKED_LONG WINAPI ptw32_InterlockedCompareExchange(
    PTW32_INTERLOCKED_LPLONG location, PTW32_INTERLOCKED_LONG value,
    PTW32_INTERLOCKED_LONG comparand);

LONG WINAPI ptw32_InterlockedExchange(LPLONG location, LONG value);

DWORD
ptw32_RegisterCancelation(PAPCFUNC callback, HANDLE threadH,
                          DWORD callback_arg);
...
extern "C" {
#endif
_CRTIMP unsigned long __cdecl _beginthread(void(__cdecl *)(void *), unsigned,
                                           void *);
...
#define _beginthreadex(security, stack_size, start_proc, arg, flags, pid)      \
  CreateThread(security, stack_size, (LPTHREAD_START_ROUTINE)start_proc, arg,  \
               flags, pid)
