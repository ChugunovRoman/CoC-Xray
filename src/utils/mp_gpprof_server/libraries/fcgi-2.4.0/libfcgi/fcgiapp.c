...
void FCGX_ShutdownPending(void) { OS_ShutdownPending(); }
...
int FCGX_UnGetChar(int c, FCGX_Stream *stream) {
  if (c == EOF || stream->isClosed || !stream->isReader ||
 ...
 *----------------------------------------------------------------------
 */
int FCGX_GetError(FCGX_Stream *stream) { return stream->FCGI_errno; }
...
static void PutParam(ParamsPtr paramsPtr, char *nameValue) {
  ...
  *paramsPtr->cur++ = nameValue;
  ...
  if (size >= paramsPtr->length) {
    paramsPtr->length *= 2;
    paramsPtr->vec = (FCGX_ParamArray)realloc(
        paramsPtr->vec, paramsPtr->length * sizeof(char *));
 ...
 *----------------------------------------------------------------------
 */
static FCGI_EndRequestBody MakeEndRequestBody(int appStatus,
                                              int protocolStatus) {
...
static void WriteCloseRecords(struct FCGX_Stream *stream) {
   ...
   */
  if (!(data->type == FCGI_STDERR && stream->wrNext == data->buff &&
  ...
  if (data->reqDataPtr->nWriters == 1) {
    ...
    endRequestRecord.header =
        MakeHeader(FCGI_END_REQUEST, data->reqDataPtr->requestId,
                   sizeof(endRequestRecord.body), 0);
    endRequestRecord.body =
        MakeEndRequestBody(data->reqDataPtr->appStatus, FCGI_REQUEST_COMPLETE);
...
static void EmptyBuffProc(struct FCGX_Stream *stream, int doClose) {
  ...
  if (!data->rawWrite) {
    ...
    if (cLen > 0) {
      ...
      *((FCGI_Header *)data->buff) = MakeHeader(
          data->type, data->reqDataPtr->requestId, cLen, eLen - cLen);
    } else {
...
static int ProcessManagementRecord(int type, FCGX_Stream *stream) {
  ...
  if (type == FCGI_GET_VALUES) {
    ...
    *((FCGI_Header *)response) = MakeHeader(
        FCGI_GET_VALUES_RESULT, FCGI_NULL_REQUEST_ID, len, paddedLen - len);
    FreeParams(&paramsPtr);
  } else {
    paddedLen = len = sizeof(FCGI_UnknownTypeBody);
    ((FCGI_UnknownTypeRecord *)response)->header =
        MakeHeader(FCGI_UNKNOWN_TYPE, FCGI_NULL_REQUEST_ID, len, 0);
...
static int ProcessBeginRecord(int requestId, FCGX_Stream *stream) {
  ...
  if (data->reqDataPtr->isBeginProcessed) {
     ...
     */
    FCGI_EndRequestRecord endRequestRecord;
    endRequestRecord.header = MakeHeader(FCGI_END_REQUEST, requestId,
                                         sizeof(endRequestRecord.body), 0);
    endRequestRecord.body = MakeEndRequestBody(0, FCGI_CANT_MPX_CONN);
    if (write_it_all(data->reqDataPtr->ipcFd, (char *)&endRequestRecord,
...
static void FillBuffProc(FCGX_Stream *stream) {
  ...
  for (;;) {
    ...
    if (stream->rdNext == data->buffStop) {
      count =
          OS_Read(data->reqDataPtr->ipcFd, (char *)data->buff, data->bufflen);
      if (count <= 0) {
     ...
     */
    count =
        min((int)sizeof(header) - headerLen, data->buffStop - stream->rdNext);
 ...
 *----------------------------------------------------------------------
 */
FCGX_Stream *FCGX_CreateWriter(int ipcFd, int requestId, int bufflen,
                               int streamType) {
...
int FCGX_Accept_r(FCGX_Request *reqDataPtr) {
  ...
  for (;;) {
    ...
    if (reqDataPtr->ipcFd < 0) {
      ...
      reqDataPtr->ipcFd = OS_Accept(reqDataPtr->listen_sock, fail_on_intr,
...
int FCGX_StartFilterData(FCGX_Stream *stream) {
  FCGX_Stream_Data *data = (FCGX_Stream_Data *)stream->data;
  if (data->reqDataPtr->role != FCGI_FILTER || !stream->isReader ||
