...
fcgi_streambuf::fcgi_streambuf(char_type *b, streamsize bs) { init(0, b, bs); }

fcgi_streambuf::fcgi_streambuf(FCGX_Stream *fs) { init(fs, 0, 0); }
...
int fcgi_streambuf::overflow(int c) {
  if (this->bufsize) {
    ...
    if (plen) {
      if (FCGX_PutStr(pbase(), plen, this->fcgx) != plen)
        return EOF;
      pbump(-plen);
    }
  }
...
// deprecated
void fcgi_istream::attach(FCGX_Stream *fs) { fcgi_strmbuf.attach(fs); }
...
// deprecated
void fcgi_ostream::attach(FCGX_Stream *fs) { fcgi_strmbuf.attach(fs); }
