...
static const char rcsid[] =
    "$Id: cgi-fcgi.c,v 1.15 2001/09/01 01:14:28 robs Exp $";
#endif /* not lint */
 ...
 *----------------------------------------------------------------------
 */
static FCGI_Header MakeHeader(int type, int requestId, int contentLength,
                              int paddingLength) {
  FCGI_Header header;
 ...
 *----------------------------------------------------------------------
 */
static FCGI_BeginRequestBody MakeBeginRequestBody(int role,
                                                  int keepConnection) {
  ...
  while (fromAS.next != fromAS.stop) {
    ...
    if (headerLen < sizeof(header)) {
      ...
      case FCGI_STDERR:
        ...
        contentLen -= count;
        if (count > 0) {
          outFD = (header.type == FCGI_STDOUT) ? STDOUT_FILENO : STDERR_FILENO;
      ...
      case FCGI_END_REQUEST:
        ...
        count = GetPtr(&ptr, contentLen, &fromAS);
        if (count > 0) {
          memcpy(&erBody + sizeof(erBody) - contentLen, ptr, count);
          contentLen -= count;
        }
        if (contentLen == 0) {
          ...
          exitStatus = (erBody.appStatusB3 << 24) + (erBody.appStatusB2 << 16) +
                       (erBody.appStatusB1 << 8) + (erBody.appStatusB0);
          exitStatusSet = TRUE;
...
static void ScheduleIo(void) {
  ...
  if (!fcgiWritePending && appServerSock != -1 &&
    ...
    if (OS_AsyncWrite(appServerSock, 0, fromWS.next, length,
                      AppServerWriteHandler, (ClientData)appServerSock) == -1) {
      FCGIexit(OS_Errno);
    } else {
  ...
  if (!fcgiReadPending && appServerSock != -1) {
    ...
    if (OS_AsyncRead(appServerSock, 0, fromAS.next, BUFFLEN,
                     AppServerReadHandler, (ClientData)appServerSock) == -1) {
      FCGIexit(OS_Errno);
    } else {
  ...
  if ((bytesToRead > 0) && !webServerReadHandlerEOF && !wsReadPending &&
      !fcgiWritePending && fromWS.next == &fromWS.buff[0]) {
    if (OS_AsyncReadStdin(fromWS.next + sizeof(FCGI_Header),
                          BUFFLEN - sizeof(FCGI_Header), WebServerReadHandler,
                          STDIN_FILENO) == -1) {
      FCGIexit(OS_Errno);
    } else {
      wsReadPending = TRUE;
 ...
 *----------------------------------------------------------------------
 */
static void FCGIUtil_BuildNameValueHeader(int nameLen, int valueLen,
...
#define MAXARGS 16
static int ParseArgs(int argc, char *argv[], int *doBindPtr, int *doStartPtr,
                     char *connectPathPtr, char *appPathPtr, int *nServersPtr) {
  int i, x, err = 0, ac;
  char *tp1, *tp2, *av[MAXARGS];
  FILE *fp;
  char line[BUFSIZ];
  ...
  *nServersPtr = 0;
  ...
  for (i = 1; i < argc; i++) {
    if (argv[i][0] == '-') {
      if (!strcmp(argv[i], "-f")) {
        ...
        err = ParseArgs(ac, av, doBindPtr, doStartPtr, connectPathPtr,
                        appPathPtr, nServersPtr);
        for (x = 1; x < ac; x++) {
          ASSERT(av[x] != NULL);
          free(av[x]);
        }
        ...
        *doStartPtr = FALSE;
        ...
        if (++i == argc) {
          fprintf(stderr, "Missing connection name after -connect\n");
          err++;
        } else {
  ...
  if (*connectPathPtr == 0) {
     ...
     * XXX: Test to see if we can use IP connect locally...
            This hack lets me test the ability to create a local process
     listening to a TCP/IP port for connections and subsequently connect to the
     app like we do for Unix domain and named pipes.
...
int main(int argc, char **argv) {
  ...
  if (ParseArgs(argc, argv, &doBind, &doStart, (char *)&bindPath,
                (char *)&appPath, &nServers)) {
    fprintf(
        stderr,
        "Usage:\n"
        ...
        "or (if -start is given) a :portNumber specification (uses local "
        "host).\n");
    exit(1);
  }
  ...
  for (; *envp != NULL; envp++) {
    ...
    FCGIUtil_BuildNameValueHeader(equalPtr - *envp, valueLen, &headerBuff[0],
                                  &headerLen);
