...
bool mutex::try_lock() { return (pthread_mutex_trylock(&m_mutex) == 0); }
...
void condition::wait(mutex &m) { pthread_cond_wait(&m_cond, m.get()); }
void condition::signal() { pthread_cond_signal(&m_cond); }

unsigned int const get_clock_ms() { return (clock() / CLOCKS_PER_SEC) * 1000; }
