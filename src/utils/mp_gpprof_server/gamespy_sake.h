...
private:
  ...
  private:
    ...
    bool m_complete;
    static void login_result_cb(GPConnection *connection, void *arg,
                                void *param);
  };
  ...
  static unsigned int const merged_fields_count =
      (gamespy_profile::at_awards_count *
       gamespy_profile::ap_award_params_count) +
  ...
  bool process_record(SAKESearchForRecordsInput *in, SAKEField *fields,
                      gamespy_profile::profile_data &dest_data,
                      std::string &dest_name);
  static void request_callback(SAKE sake, SAKERequest request,
