...
bool Image_DXTC::LoadFromFile(LPCSTR filename) {
  ...
  if (m_CompFormat == PF_DXT1 || m_CompFormat == PF_DXT2 ||
  ...
  if (ddsd.dwHeaderFlags & DDSD_LINEARSIZE) {
    ...
    m_pCompBytes =
        (BYTE *)calloc(ddsd.dwPitchOrLinearSize * ddsd.dwHeight, sizeof(BYTE));
...
void Image_DXTC::AllocateDecompBytes() {
  ...
  m_pDecompBytes =
      (BYTE *)calloc(m_DDSD.dwWidth * m_DDSD.dwHeight * 4, sizeof(BYTE));
  if (m_pDecompBytes == NULL) {
...
inline void GetColorBlockColors(DXTColBlock *pBlock, Color8888 *col_0,
                                Color8888 *col_1, Color8888 *col_2,
                                Color8888 *col_3, WORD &wrd) {
...
inline void DecodeColorBlock(DWORD *pImPos, DXTColBlock *pColorBlock, int width,
                             DWORD *col_0, DWORD *col_1, DWORD *col_2,
                             DWORD *col_3) {
  ...
  for (r = 0; r < 4;
       r++, pImPos += width - 4) // no width*4 as DWORD ptr inc will *4
  {
...
inline void DecodeAlphaExplicit(DWORD *pImPos,
                                DXTAlphaBlockExplicit *pAlphaBlock, int width,
                                DWORD alphazero) {
  ...
  for (int row = 0; row < 4; row++, pImPos += width - 4) {
    ...
    for (int pix = 0; pix < 4; pix++) {
      ...
      *pImPos &= alphazero;
      ...
      col.a =
          col.a | (col.a << 4); // This allows max 4 bit alpha to be 0xff alpha
...
inline void DecodeAlpha3BitLinear(DWORD *pImPos,
...
void Image_DXTC::DecompressDXT1() {
  ...
  for (int j = 0; j < yblocks; j++) {
    ...
    for (int i = 0; i < xblocks; i++, pBlock++) {
      ...
      DecodeColorBlock(pImPos, pBlock, m_nWidth, (DWORD *)&col_0,
...
void Image_DXTC::DecompressDXT5() {
  ...
  for (int j = 0; j < yblocks; j++) {
    ...
    // 1 block for alpha, 1 block for color
    DXTColBlock *pBlock =
        (DXTColBlock *)((DWORD)m_pCompBytes + j * xblocks * 16);
    for (int i = 0; i < xblocks; i++, pBlock++) {
      // inline
      // Get alpha block
...
//  PixelFormatToString()
//-----------------------------------------------------------------------------
VOID Image_DXTC::DecodePixelFormat(CHAR *strPixelFormat,
                                   DDS_PIXELFORMAT *pddpf) {
...
inline void GetColorBlockColors_m2(DXTColBlock *pBlock, Color8888 *col_0,
                                   Color8888 *col_1, Color8888 *col_2,
                                   Color8888 *col_3, WORD &wrd) {
  // method 2
  // freak variable bit structure method
...
inline void GetColorBlockColors_m3(DXTColBlock *pBlock, Color8888 *col_0,
                                   Color8888 *col_1, Color8888 *col_2,
                                   Color8888 *col_3, WORD &wrd) {
  // method 3
...
inline void GetColorBlockColors_m4(DXTColBlock *pBlock, Color8888 *col_0,
                                   Color8888 *col_1, Color8888 *col_2,
                                   Color8888 *col_3, WORD &wrd) {
  // m1 color extraction from 5-6-5
  // m3 color math on DWORD before bit shift to full precision
  wrd = pBlock->col0;
  col_0->a = 0x00; // must set to 0 to avoid possible overflow & carry to next
                   // field in DWORD add
  // extract r,g,b bits
  col_0->b =
      (unsigned char)wrd & 0x1f; // 0x1f = 0001 1111  to mask out upper 3 bits
  wrd >>= 5;
  col_0->g =
      (unsigned char)wrd & 0x3f; // 0x3f = 0011 1111  to mask out upper 2 bits
  wrd >>= 6;
...
inline void GetColorBlockColors_m1(DXTColBlock *pBlock, Color8888 *col_0,
                                   Color8888 *col_1, Color8888 *col_2,
                                   Color8888 *col_3, WORD &wrd) {
  // Method 1:
  // Shifty method
