...
void RotateArrayCCW(const float *pInArray, int num_x, int num_y,
                    float *pOutArray);
...
void CalculateNormalMap(NVI_Image *pSrc, ConvolutionKernel *pKernels,
                        int num_kernels, float scale, bool wrap) {
  ...
  for (j = 0; j < size_y; j++) {
    for (i = 0; i < size_x; i++) {
      ...
      //.			AlphaAndVectorToARGB( height, -du/mag,
      //-dv/mag, 1.0f / mag, nmap_color );
...
void ConvertAlphaToNormalMap_4x(NVI_Image *pSrc, float scale, bool wrap) {
  ...
  numelem = 2;
  ConvolutionKernelElement du_elem[] = {{-1, 0, -1.0f / 2.0f},
                                        {1, 0, 1.0f / 2.0f}};

  ConvolutionKernelElement dv_elem[] = {{0, 1, 1.0f / 2.0f},
                                        {0, -1, -1.0f / 2.0f}};
...
void ConvertAlphaToNormalMap_3x3(NVI_Image *pSrc, float scale, bool wrap) {
  ...
  numelem = 6;
  ConvolutionKernelElement du_elem[] = {{-1, 1, -wt},  {1, 1, wt},
                                        {-1, 0, -wt},  {1, 0, wt},
                                        {-1, -1, -wt}, {1, -1, wt}};
  ...
  ConvolutionKernelElement dv_elem[] = {{-1, 1, wt},  {0, 1, wt},
                                        {1, 1, wt},   {-1, -1, -wt},
                                        {0, -1, -wt}, {1, -1, -wt}};
...
void ConvertAlphaToNormalMap_5x5(NVI_Image *pSrc, float scale, bool wrap) {
  ...
  */
  ...
  ConvolutionKernelElement du_elem[] = {
      {-2, 2, -wt22},  {-1, 2, -wt12},        {1, 2, wt12},
      {2, 2, wt22},    {-2, 1, -wt12},        {-1, 1, -wt11},
      {1, 1, wt11},    {2, 1, wt12},

      {-2, 0, -wt02},  {-1, 0, -1.0f / 2.0f}, {1, 0, 1.0f / 2.0f},
      {2, 0, wt02},

      {-2, -1, -wt12}, {-1, -1, -wt11},       {1, -1, wt11},
      {2, -1, wt12},   {-2, -2, -wt22},       {-1, -2, -wt12},
      {1, -2, wt12},   {2, -2, wt22}};

  ConvolutionKernelElement dv_elem[] = {
      {-2, 2, wt22},   {-1, 2, wt12},         {0, 2, 1.0f / 4.0f},
      {1, 2, wt12},    {2, 2, wt22},          {-2, 1, wt12},
      {-1, 1, wt11},   {0, 1, 1.0f / 2.0f},   {1, 1, wt11},
      {2, 1, wt12},

      {-2, -1, -wt12}, {-1, -1, -wt11},       {0, -1, -1.0f / 2.0f},
      {1, -1, -wt11},  {2, -1, -wt12},        {-2, -2, -wt22},
      {-1, -2, -wt12}, {0, -2, -1.0f / 4.0f}, {1, -2, -wt12},
      {2, -2, -wt22}};
...
void ConvertAlphaToNormalMap_7x7(NVI_Image *pSrc, float scale, bool wrap) {
  ...
  float du_f[] = {-1, -2, -3, 0,  3,  2,  1, -2, -3, -4, 0,  4,  3,
...
void ConvertAlphaToNormalMap_9x9(NVI_Image *pSrc, float scale, bool wrap) {
  ...
  float du_f[] = {-1, -2, -3, -4, 0, 4, 3, 2, 1, -2, -3, -4, -5, 0, 5, 4, 3, 2,
                  -3, -4, -5, -6, 0, 6, 5, 4, 3, -4, -5, -6, -7, 0, 7, 6, 5, 4,
                  -5, -6, -7, -8, 0, 8, 7, 6, 5, -4, -5, -6, -7, 0, 7, 6, 5, 4,
                  -3, -4, -5, -6, 0, 6, 5, 4, 3, -2, -3, -4, -5, 0, 5, 4, 3, 2,
                  -1, -2, -3, -4, 0, 4, 3, 2, 1};
...
IC u32 it_difference(u32 d, u32 orig, u32 ucomp) {
  return color_rgba(
      128 +
          2 * (int(color_get_R(orig)) - int(color_get_R(ucomp))) / 3, // R-error
      128 +
          2 * (int(color_get_G(orig)) - int(color_get_G(ucomp))) / 3, // G-error
      128 +
          2 * (int(color_get_B(orig)) - int(color_get_B(ucomp))) / 3, // B-error
      128 + 2 * (int(color_get_A(orig)) - int(color_get_A(ucomp))) /
                3); // A-error
}
...
template <class _It>
IC void TW_Iterate_1OP(u32 width, u32 height, u32 pitch, u8 *dst, u8 *src,
                       const _It pred) {
...
template <class _It>
IC void TW_Iterate_2OP(u32 width, u32 height, u32 pitch, u8 *dst, u8 *src0,
                       u8 *src1, const _It pred) {
  for (u32 y = 0; y < height; y++) {
    for (u32 x = 0; x < width; x++) {
...
extern int DXTCompressImage(LPCSTR out_name, u8 *raw_data, u32 w, u32 h,
                            u32 pitch, STextureParams *fmt, u32 depth);
...
int DXTCompressBump(LPCSTR out_name, u8 *T_height_gloss, u8 *T_normal_map,
  ...
  pitch = w * 4;
  // tga_save
  // ("x:\\0-height-gloss.tga",w,h,T_height_gloss,true);
  if (T_normal_map) {
  ...
  TW_Iterate_1OP(w, h, pitch, T_normal_1, T_height_gloss,
                 it_gloss_rev); // T_height_gloss.a (gloss) -> T_normal_1 +
                                // reverse of channels
  ...
  if (res == 1) {
    ...
    if (CI->LoadFromFile(out_name)) {
      ...
      TW_Iterate_2OP(w, h, pitch, T_normal_1D, T_normal_1, T_normal_1U,
                     it_difference);
      // tga_save		("x:\\4-normal_1D.tga",w,h,T_normal_1D,true);

      // Rescale by virtual height
      float h_scale =
          powf(fmt->bump_virtual_height / 0.05f, 0.75f); // move towards 1.0f
      if (h_scale > 1.f)
        h_scale = _sqrt(h_scale);
      {
        for (u32 y = 0; y < h; y++) {
          for (u32 x = 0; x < w; x++) {
            ...
            sh = color_rgba(h, color_get_G(sh), color_get_B(sh),
                            color_get_A(sh));
          }
        }
      }
      ...
      res |=
          DXTCompressImage(out_name1, T_normal_1D, w, h, pitch, &fmt0, depth);
...
(T_normal_1,T_height_gloss,0,0,D3DX_CHANNEL_RED,4.f));
//TW_Save
...
// Decompress (back)
fmt								=
D3DFMT_A8R8G8B8; IDirect3DTexture9*	T_normal_1U	=
...
ref_texture			t_temp		=
Device.Resources->_CreateTexture	(fnameB); t_temp->surface_set
(T_normal_2C	);
