...
void DDSWriter::beginImage(int size, int width, int height, int depth, int face,
                           int miplevel) {}
...
ENGINE_API u32 *Build32MipLevel(u32 &_w, u32 &_h, u32 &_p, u32 *pdwPixelSrc,
                                STextureParams *fmt, float blend) {
  ...
  for (u32 y = 0; y < _h; y += 2) {
    ...
    for (u32 x = 0; x < _w; x += 2) {
      ...
      if (1 == _h)
        p4 = p2;
      float c_r =
          float(u32(p1[0]) + u32(p2[0]) + u32(p3[0]) + u32(p4[0])) / 4.f;
      float c_g =
          float(u32(p1[1]) + u32(p2[1]) + u32(p3[1]) + u32(p4[1])) / 4.f;
      float c_b =
          float(u32(p1[2]) + u32(p2[2]) + u32(p3[2]) + u32(p4[2])) / 4.f;
      float c_a =
          float(u32(p1[3]) + u32(p2[3]) + u32(p3[3]) + u32(p4[3])) / 4.f;
...
void FillRect(u8 *data, u8 *new_data, u32 offs, u32 pitch, u32 h,
              u32 full_pitch) {
  for (u32 i = 0; i < h; i++) {
...
int DXTCompressImage(LPCSTR out_name, u8 *raw_data, u32 w, u32 h, u32 pitch,
  ...
  R_ASSERT(0 != w && 0 != h);
  gFileOut =
      _open(out_name, _O_WRONLY | _O_BINARY | _O_CREAT | _O_TRUNC, _S_IWRITE);
  if (gFileOut == -1) {
  ...
  auto layout = fmt->type == STextureParams::ttCubeMap ? nvtt::TextureType_Cube
                                                       : nvtt::TextureType_2D;
  ...
  compOpt.setQuantization((fmt->flags.is(STextureParams::flDitherColor) != 0),
                          false,
  ...
  if (fmt->flags.is(STextureParams::flGenerateMipMaps) &&
    ...
    float blend = fmt->flags.is_any(STextureParams::flFadeToColor |
...
extern int DXTCompressBump(LPCSTR out_name, u8 *raw_data, u8 *normal_map, u32 w,
                           u32 h, u32 pitch, STextureParams *fmt, u32 depth);

extern "C" __declspec(dllexport) int __stdcall DXTCompress(
    LPCSTR out_name, u8 *raw_data, u8 *normal_map, u32 w, u32 h, u32 pitch,
    STextureParams *fmt, u32 depth) {
  switch (fmt->type) {
  ...
  case STextureParams::ttBumpMap:
    return DXTCompressBump(out_name, raw_data, normal_map, w, h, pitch, fmt,
                           depth);
    break;
  default:
    NODEFAULT;
