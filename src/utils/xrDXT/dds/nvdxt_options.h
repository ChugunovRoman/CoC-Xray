...
public:
  ...
  // minimum value the z value can attain in the x y z  normal keeps normal
  // point "upwards"
  int minz;
  // height multiplier
  float scale;
  // kernel used to create normal maps. Done this way to be compatible with
  // plugins
...
public:
  ...
  void SetDefaultOptions() {
    ...
    // in kQualityHighest mode, rms error above which will cause a long search
    // for a better answer
    ...
    clampScaleY = 4096;
    mipMapGeneration =
        kGenerateMipMaps;  // dNoMipMaps, dUseExistingMipMaps, dGenerateMipMaps
    ...
    alphaThreshold32F =
        0.5; // When Binary Alpha is selected, below this value, alpha is zero
    bDitherColor = false; // enable dithering during 16 bit conversion
    bDitherMip0 = false;  // enable dithering during 16 bit conversion for each
                          // MIP level (after filtering)
    ...
    bOutputWrap =
        false; // wrap the values when outputting to the desired format
  ...
  void SetQuality(nvQualitySetting setting, float threshold) {
    quality = setting;
    // if setting == kQualityHighest, if the RMS error for a 4x4 block is bigger
    // than this, an extended search is performed.  In practice this has been
    // equivalent to and exhaustive search in the entire domain. aka it doesn't
  ...
  void SetCompressionWeighting(nvCompressionWeighting type,
                               float new_weight[3]) {
    weightType = type;
  ...
  ///////////////////////////  SCALING IMAGE /////////////////////////////////
  nvRescaleTypes
      rescaleImageType; // rescaling type before image before compression
  ...
  // Evenly scale down in both directions so that the given image size is not
  // exceeded
  ...
  nvBoolean bOverrideFilterWidth; // use the specified width in
                                  // FilterWidth,instead of the default
  ...
  /////////// general enables ////////////////////////////////
  nvBoolean bOutputWrap;    // wrap the values (before clamping to the format
                            // range) when outputting to the desired format
  ...
  nvBoolean
      bCreateOnePalette; // All 4x4 compression blocks share the same palette

  /////////////////// DISABLED ////////////////////////////////
  nvBoolean bDitherColor; // enable dithering during 16 bit conversion
  nvBoolean bDitherMip0;  // enable dithering during 16 bit conversion for each
                          // MIP level (after filtering)
