...
#ifdef _DEBUG
  float numbytes =
      (float)m_nSizeX * (float)m_nSizeY * (float)GetBytesPerPixel();
...
HRESULT NVI_ImageBordered::Initialize(NVI_Image **hSrcImage,
                                      const RECT *pBorder, bool wrap) {
...
void NVI_ImageBordered::CopyDataFromSource() {
  ...
  //.	ASSERT_MSG( m_nBorderXLow < sx, "Borders larger than image not
  // supported! ulow\n" ); .	ASSERT_MSG( m_nBorderYLow < sy, "Borders larger
  // than image not supported! uhigh\n" ); .	ASSERT_MSG( m_nBorderXHigh < sx,
  //"Borders larger than image not supported! vlow\n" ); .	ASSERT_MSG(
  // m_nBorderYHigh < sy, "Borders larger than image not supported! vhigh\n" );
  ...
  if (m_bWrap) {
    ...
    for (int j = 0; j < m_nSizeY; j++) {
      ...
      memcpy(&pPadArray[j * m_nSizeX - m_nBorderXLow + sx],
             &pPadArray[j * m_nSizeX - m_nBorderXLow],
    ...
    for (int j = 0; j < m_nBorderYHigh; j++) {
      ...
      memcpy(&pPadArray[(j + sy - m_nBorderYLow) * m_nSizeX],
             &pPadArray[(j - m_nBorderYLow) * m_nSizeX],
    ...
    for (int j = 0; j < m_nBorderYHigh; j++) {
      ...
      // krn_v_lowbound is negative or zero
      memcpy(&pPadArray[(j + sy - m_nBorderYLow) * m_nSizeX],
             &pPadArray[(sy - 1 - m_nBorderYLow) * m_nSizeX],
             sizeof(DWORD) * m_nSizeX);
    }
    for (int j = 0; j < -m_nBorderYLow; j++) {
  ...
  //						(DWORD) m_nSizeX, (DWORD) m_nSizeY, 32, 32,
  //0
  //);
}
