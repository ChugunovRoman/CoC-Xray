...
struct MIPMapData {
  ...
  int numFaces; // total number of faces (depth for volume textures, 6 for cube
                // maps)
};

// Write callback function.
typedef NV_ERROR_CODE (*DXTReadCallback)(void *buffer, size_t count,
                                         void *userData);
...
// Compresses an image with a user supplied callback with the data for each MIP
// level created
...
DXTLIB_API NV_ERROR_CODE nvDXTcompress(
    const nvImageContainer &imageContainer, nvCompressionOptions *options,
...
DXTLIB_API NV_ERROR_CODE nvDXTcompress(
    const unsigned char *srcImage, size_t width, size_t height,
    size_t byte_pitch, nvPixelOrder pixelOrder, nvCompressionOptions *options,
    DXTWriteCallback fileWriteRoutine, // call to .dds write routine
    const RECT *rect = 0);

// image with MIP maps
DXTLIB_API NV_ERROR_CODE nvDXTcompress(
    const RGBAMipMappedImage &srcMIPImage, nvCompressionOptions *options,
    DXTWriteCallback fileWriteRoutine, // call to .dds write routine
    const RECT *rect = 0);

DXTLIB_API NV_ERROR_CODE nvDXTcompress(
    const RGBAMipMappedCubeMap &srcMIPCubeMap, nvCompressionOptions *options,
    DXTWriteCallback fileWriteRoutine, // call to .dds write routine
    const RECT *rect = 0);

DXTLIB_API NV_ERROR_CODE
nvDXTcompress(const RGBAMipMappedVolumeMap &srcMIPVolumeMap,
...
DXTLIB_API NV_ERROR_CODE nvDXTcompress(
    const fpMipMappedVolumeMap &srcMIPVolumeMap, nvCompressionOptions *options,
...
DXTLIB_API NV_ERROR_CODE nvDXTdecompress(nvImageContainer &imageData,
                                         nvPixelFormat pf, int readMIPMapCount,
