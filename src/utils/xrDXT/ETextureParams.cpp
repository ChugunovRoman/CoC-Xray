...
xr_token tbmode_token[] = {
    {"None", STextureParams::tbmNone}, {"Use", STextureParams::tbmUse}, {0, 0}};
...
void STextureParams::FillProp(LPCSTR base_name, PropItemVec &items,
  ...
  PHelper().CreateCaption(items, "Source\\Width",
                          shared_str().sprintf("%d", width));
  PHelper().CreateCaption(items, "Source\\Height",
                          shared_str().sprintf("%d", height));
  ...
  case ttCubeMap:
    ...
    if (false) {
      PHelper().CreateFlag32(items, "Flags\\Binary Alpha", &flags,
                             flBinaryAlpha);
    }
