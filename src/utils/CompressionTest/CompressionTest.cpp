...
public:
  ...
  {
    return ((_stop_time.QuadPart - _start_time.QuadPart) * 1000.0) /
           _freq.QuadPart;
  }
...
struct BlockInfo {
  ...
  BlockInfo(unsigned sz = 0)
      : size(sz), count(0), min_time(1000.0f), max_time(0), avg_time(0),
        min_ratio(1.0f), max_ratio(0), avg_ratio(0), total_time(0),
        total_ratio(0) {}
...
static bool _ProcessFile_PPMd(const char *file_name) {
  ...
  if (file) {
    ...
    printf("PPMd1 :  %2.0f%% %1.5fms  %u->%u\n",
           100.0f * float(dst.tell()) / float(src.tell()), (float)timer.time(),
           src.tell(), dst.tell());
    ...
    for (const char *s = src_data, *u = uncomp_data; s != src_data + src_size;
         ++s, ++u) {
      if (*s != *u) {
        ok = false;
        err_b = s - src_data;
    ...
    vector<BlockInfo>::iterator i =
        find_if(_PPM_BlockInfo.begin(), _PPM_BlockInfo.end(),
                BlockInfo::EqualSize(src_size));
...
static bool _ProcessFile_LZO(const char *file_name) {
  ...
  if (file) {
    ...
    printf("LZO  :  %2.0f%% %1.5fms  %u->%u",
           100.0f * float(comp_size) / float(src_size), (float)timer.time(),
    ...
    memset(uncomp_data, 0xDD, uncomp_size);
    if (_LZO_Dict) {
      lzo1x_decompress_dict_safe(comp_data, comp_size, uncomp_data,
                                 &uncomp_size, NULL, _LZO_Dict, _LZO_Dict_Sz);
    } else {
    ...
    for (const uint8_t *s = src_data, *u = uncomp_data;
         s != src_data + src_size; ++s, ++u) {
      if (*s != *u) {
        ok = false;
        err_b = s - src_data;
        break;
    ...
    vector<BlockInfo>::iterator i =
        find_if(_LZO_BlockInfo.begin(), _LZO_BlockInfo.end(),
                BlockInfo::EqualSize(src_size));
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 1) {
    ...
    for (unsigned i = 0; i < _PPM_BlockInfo.size(); ++i) {
      _PPM_BlockInfo[i].avg_ratio =
          _PPM_BlockInfo[i].total_ratio / _PPM_BlockInfo[i].count;
      _PPM_BlockInfo[i].avg_time =
          _PPM_BlockInfo[i].total_time / _PPM_BlockInfo[i].count;
    }
    ...
    for (unsigned i = 0; i < _LZO_BlockInfo.size(); ++i) {
      _LZO_BlockInfo[i].avg_ratio =
          _LZO_BlockInfo[i].total_ratio / _LZO_BlockInfo[i].count;
      _LZO_BlockInfo[i].avg_time =
          _LZO_BlockInfo[i].total_time / _LZO_BlockInfo[i].count;
    }
