...
IC static bool clip(float fDenom, float fNumer, float &rfT0, float &rfT1,
                    bool &code0, bool &code1) {
...
IC static bool clip(float fDenom, float fNumer, float &rfT0, float &rfT1,
                    box_axis &ax0, box_axis &ax1, box_axis ax) {
...
static bool intersect(const Fvector &start, const Fvector &dir,
                      const Fvector &extent, float &rfT0, float &rfT1,
  ...
  return bNotEntirelyClipped &&
         (ax0 != a_none ||
          ax1 != a_none); //( rfT0 != fSaveT0 || rfT1 != fSaveT1 );
}

bool intersect(const Fobb &box, const Fvector &origin, const Fvector &direction,
  ...
  Fvector kOrigin;
  kOrigin.set(kDiff.dotproduct(box.m_rotate.i),
              kDiff.dotproduct(box.m_rotate.j),
              kDiff.dotproduct(box.m_rotate.k));
  Fvector kDirection;
  kDirection.set(direction.dotproduct(box.m_rotate.i),
                 direction.dotproduct(box.m_rotate.j),
...
bool intersect(const Fsphere &sphere, const Fvector &origin,
               const Fvector &direction, float &dist, Fvector &norm) {
...
bool intersect(const Fcylinder &cylinder, const Fvector &origin,
               const Fvector &direction, float &dist, Fvector &norm) {
  ...
  if (b_result) {
    ...
    if (r_code == Fcylinder::cyl_wall) {
      const Fvector r_dir = Fvector().mad(pt, cylinder.m_direction,
                                          -pt.dotproduct(cylinder.m_direction));
...
bool bone_intersect(u16 bone, const IKinematics &K, const Fvector &origin,
                    const Fvector &direction, float &dist, Fvector &norm) {
...
bool __stdcall intersect(const IKinematics &K, const Fvector &origin,
                         const Fvector &direction, u16 &bone_id, float &dist,
                         Fvector &norm) {
