...
// The table of what score increase you get for reducing a valence to a certain
// amount. Any valence over the max value gets the max value boost. Bear in mind
// that three of these get added each tri.
...
// This stuff simulates a FIFO cache. There are lots of types, but (a) this is
// the easiest to model and (b) it's actually used in lots of real hardware
// (mainly because it's also the easiest to construct).
...
const CacheTypeInfo c_ctiCacheTypeSize[c_iCacheNumTypes] = {{20, 0.95f},
                                                            {3, 0.05f}};

#endif

struct CacheTypeTriInfo {
  int iIndicesAdded;      // Were 0, 1, 2 or 3 indices added to the start of the
                          // FIFO.
...
struct CacheVert {
  BYTE bRefcount[c_iCacheNumTypes]; // How many times each vertex is referenced
                                    // by the cache.
};
...
void CacheInit(int iNumberOfVertices) {
  ...
  for (i = 0; i < c_iCacheNumTypes; i++) {
    ...
    for (int j = 0; j < c_ctiCacheTypeSize[i].iLength; j++) {
      ccsState[i].pwCacheFifoState[j] =
          0xFFFF; // "unused" value - won't cause a cache hit.
    }
...
// Set bDontActuallyAdd to TRUE to just find the number of cache misses if this
// tri was added.
float CacheAddTri(WORD wIndex1, WORD wIndex2, WORD wIndex3,
                  BOOL bDontActuallyAdd = FALSE) {
  WORD wIndex[3];
  wIndex[0] = wIndex1;
  wIndex[1] = wIndex2;
  wIndex[2] = wIndex3;
  ...
  for (int iType = 0; iType < c_iCacheNumTypes; iType++) {
    ...
    // We must scan all three indices before adding any, otherwise you get nasty
    // cases like: 0,1,2 followed by 3,0,1. If you add as you go along, the 3
    // kicks out the 0, then the 0 kicks out the 1, then the 1 kicks out the 2.
    // And you have 0 cache hits, rather than 2. Most hardware does all three
    // checks before adding as well. NOTE! If your target hardware doesn't,
    // shorten the FIFO a bit to get roughly the same behaviour.
    BOOL bFound[3];
    int iVert;
    for (iVert = 0; iVert < 3; iVert++) {
      bFound[iVert] =
          ((cvCacheStatus.item(wIndex[iVert]))->bRefcount[iType] > 0);
    ...
    if (bDontActuallyAdd) {
      ...
      for (iVert = 0; iVert < 3; iVert++) {
        if (!bFound[iVert]) {
          ...
          // And if this tri is degenerate, we need to spot that we've loaded
          // the others.
          if (iVert == 0) {
      ...
      fTotalScore += c_ctiCacheTypeSize[iType].fMissScore *
                     (ccs.iCacheCurrentMisses - iCurNumMisses);
    }
  }
...
float FindBestScoreLookahead(int iCountdown, float fCurrentScore,
                             float fInputBestSoFar, int iNumTris,
                             WORD *pwResult) {
  ...
  // Should we limit ourselves to tris that share at least one vert with the
  // previous one?
  ...
  for (i = pstTriLocal.size() - 1; i >= 0; i--) {
    ...
    if (bNoMadJumps) {
      ...
      // To make the scores comparable, add the maximum valence boost all the
      // time.
...
#endif
    {
      ...
      for (j = 0; j < 3; j++) {
        ...
        int iValence =
            (svVertex.item(pstCurTri->wIndex[j])->iCurrentValence) - 1;
        VERIFY(iValence >= 0);
        if (iValence < c_iMaxValenceBoost) {
          fTriScore += fValenceBoost[iValence];
        } else {
    ...
    if (fTriScore > fExpensiveCutoff) {
      // This tri is a lot more expensive than the average cost of the BestSoFar
      // tris. It's very unlikely to give us a good result.
      continue;
    }
    ...
    // And the vertex cost.
    fTriScore += CacheAddTri(pstCurTri->wIndex[0], pstCurTri->wIndex[1],
                             pstCurTri->wIndex[2], TRUE);
    ...
    // We -ve the score so that the first in the heap is the one with the
    // _least_ score.
  ...
  // This attempts to get early-outs very quickly and prevent too much
  // recursion.
  // WORD *pwBest = NULL;
  ...
  while (pstCur != NULL) {
    ...
    if (fScore >= fBestSoFar) {
      // We've already gone more than the best score.
      // The reast of the heap is bound to be greater, so don't bother
      // continuing.
      break;
    }
    ...
    // Add it to the cache.
    float fScoreTemp =
        CacheAddTri(pstCur->wIndex[0], pstCur->wIndex[1], pstCur->wIndex[2]);
    ...
    if (iLookahead > 1) {
      ...
      // fScore = FindBestScoreLookahead ( iLookahead - 1, fScore, fBestSoFar,
      // pwIndices + 3, iNumTris - 1, pwResult + 3 );
      float fNewScore = FindBestScoreLookahead(
          iLookahead - 1, fScore, fBestSoFar, iNumTris - 1, pwResult + 3);
  ...
  if (pstBest == NULL) {
    ...
    if (iCountdown > 1) {
      ...
      float fScore = CacheAddTri(pstBest->wIndex[0], pstBest->wIndex[1],
                                 pstBest->wIndex[2]);
      ...
      // fNewBestScore = FindBestScoreLookahead ( iCountdown - 1, fScore, 1e10,
      // pwIndices + 3, iNumTris - 1, pwResult + 3 );
      fNewBestScore = FindBestScoreLookahead(iCountdown - 1, fScore, 1e10,
...
float FindBestScore(
    int iCountdown, float fCurrentScore, float fInputBestSoFar,
    // WORD *pwIndices,
...

{
  ...
  if (iNumTris == 1) {
    ...
    // Find score after removal.
    // float fTriScore = CacheAddTri ( pwIndices[0], pwIndices[1], pwIndices[2],
    // TRUE );
    ScoreTri *pstCur = ppstCurTris[0];
    float fTriScore = CacheAddTri(pstCur->wIndex[0], pstCur->wIndex[1],
                                  pstCur->wIndex[2], TRUE);
    for (j = 0; j < 3; j++) {
      // Use the valence score after the tri has been removed.
      // int iValence = (*(iValenceCounts.item(pwIndices[j]))) - 1;
      int iValence = (svVertex.item(pstCur->wIndex[j])->iCurrentValence) - 1;
  ...
  VERIFY(ppstBest != NULL);
  {
    ...
    if (iCountdown > 1) {
      ...
      *ppstBest = *ppstCurTris;
      ...
      // fNewBestScore = FindBestScore ( iCountdown - 1, fScore, 1e10, pwIndices
      // + 3, iNumTris - 1, pwResult + 3 );
      fNewBestScore =
          FindBestScore(iCountdown - 1, fScore, 1e10, ppstCurTris + 1,
                        iNumTris - 1, pwResult + 3);
...
// coherency. *pwList is modified (but obviously not changed in size or memory
// location).
void _OptimiseVertexCoherencyTriList(WORD *pwList, int iHowManyTris,
                                     u32 optimize_mode) {
  if (iHowManyTris <= 2) {
    // Silly.
    return;
  ...
  CacheInit(iBiggestIndex + 1);
	...
	float fTemp = FindBestScore (	iHowManyTris - 1,
		...
		pwNewIndex + 3 );
#else
  float fTemp = FindBestScore(iHowManyTris, 0.0f, 1e10, (pstTri.item(0)),
                              iHowManyTris, pwNewIndex);
#endif
  ...
  // And if you definately use indices 0 to iBiggestIndex (which all this code
  // does), then this is the max possible:
