...
// location).
// void OptimiseVertexCoherencyTriList ( WORD *pwList, int iHowManyTris, u32
// mode);
void OptimiseVertexCoherencyTriList(WORD *pwList, int iHowManyTris,
  ...
  if (iHowManyTris) {
    ...
    HRESULT rhr =
        D3DXOptimizeFaces(pwList, iHowManyTris, max_idx + 1, FALSE, remap);
    R_CHK(rhr);
...
BOOL CalculateSW(Object *object, VIPM_Result *result,
  ...
  int iNumTris = 0;
  for (tri = object->CurTriRoot.ListNext(); tri != NULL;
       tri = tri->ListNext()) {
    tri->mytri.dwNewIndex =
        INVALID_INDEX; // Mark them as not being in a collapse.
    iNumTris++;
  }
  // A lot of cards have this annoying limit - see D3DCAPS8.MaxPrimitiveCount,
  // which is exactly 65535 for DX7 and previous devices. So might as well stick
  // with that number.
  R_ASSERT(iNumTris < 65535);

  // Create a place to put indices while we build up the list. Because we don't
  // know how many we need yet, we can't create the IB until the end. Then we'll
  // copy this into it.
  ...
  while (TRUE) {
    ...
    while ((object->pNextCollapse->ListNext() != NULL) &&
           (object->pNextCollapse->ListNext()->iSlidingWindowLevel ==
      ...
      if (!bJustStartedANewLevel ||
          (iCurSlidingWindowLevel == iMaxSlidingWindowLevel)) {
        ...
        if (pCollapse->TriCollapsed.size() > 0) {
          ...
          for (u32 i = 0; i < pCollapse->TriCollapsed.size(); i++) {
            ...
            R_ASSERT(tri->mytri.dwNewIndex ==
                     INVALID_INDEX); // Should not have been in a collapse this
                                     // level.
            ...
            *wTempIndices.item(i * 3 + 0) =
                (WORD)pTriInfo->ppt[0]->mypt.dwNewIndex;
            *wTempIndices.item(i * 3 + 1) =
                (WORD)pTriInfo->ppt[1]->mypt.dwNewIndex;
            *wTempIndices.item(i * 3 + 2) =
                (WORD)pTriInfo->ppt[2]->mypt.dwNewIndex;
            iCurNumTris--;
          }
          ...
          // memcpy ( result->indices.item ( iCurTriBinned * 3 ),
          // wTempIndices.ptr(), sizeof(WORD) * 3 *
          // pCollapse->TriCollapsed.size() );
        ...
        // And move the added tris pointer back, because it didn't know we
        // weren't going to store these.
      ...
      */
      ...
      if (pCollapse->TriOriginal.size() > 0) {
        ...
        // And write them to the index list.
        result->indices.resize((iCurTriAdded + pCollapse->TriOriginal.size()) *
                               3);
        result->indices.insert(iCurTriAdded * 3, wTempIndices, 0,
                               3 * pCollapse->TriOriginal.size());
        // memcpy ( result->indices.item ( iCurTriAdded * 3 ),
        // wTempIndices.ptr(), sizeof(WORD) * 3 * pCollapse->TriOriginal.size()
        // );
    ...
    for (tri = object->CurTriRoot.ListNext(); tri != NULL;
      ...
      if (tri->mytri.dwNewIndex == INVALID_INDEX) {
        ...
        *(wTempIndices.item(iTempTriNum * 3 + 0)) =
            (u16)tri->pPt1->mypt.dwNewIndex;
        *(wTempIndices.item(iTempTriNum * 3 + 1)) =
            (u16)tri->pPt2->mypt.dwNewIndex;
        *(wTempIndices.item(iTempTriNum * 3 + 2)) =
            (u16)tri->pPt3->mypt.dwNewIndex;
        iTempTriNum++;
      }
    }
    R_ASSERT(iJustCheckingNumTris == iCurNumTris);
  ...
  /*
          ...
          for ( int i = 0; i <= iNumCollapses; i++ ){
                  ...
                  for ( int j = 0; j < swr->num_tris * 3; j++ ){
                          ...
                          //			R_ASSERT (
     *(result->indices.item(j+swr->offset)) < swr->num_verts ); if
     (!(*(result->indices.item(j+swr->offset)) < swr->num_verts )){ error_found
     = i;
                          }
                  }
          }
  ...
  for (int i = 0; i <= iNumCollapses; i++) {
    ...
    for (int j = 0; j < swr->num_tris * 3; j++) {
      ...
      swr->num_verts =
          _max(swr->num_verts,
               *(result->indices.item(
                   j + swr->offset))); // fignya index ne doljen bit bolshe!!!
                                       //.			R_ASSERT (
      //*(result->indices.item(j+swr->offset)) < swr->num_verts );
