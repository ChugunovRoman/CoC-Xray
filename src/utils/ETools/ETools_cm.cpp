...
public:
  CCubeMapHelper() {
    planes[CUBE_POSITIVE_X].build(Fvector().set(+0.5f, 0, 0),
                                  Fvector().set(-1, 0, 0));
    planes[CUBE_NEGATIVE_X].build(Fvector().set(-0.5f, 0, 0),
                                  Fvector().set(+1, 0, 0));
    planes[CUBE_POSITIVE_Y].build(Fvector().set(0, +0.5f, 0),
                                  Fvector().set(0, -1, 0));
    planes[CUBE_NEGATIVE_Y].build(Fvector().set(0, -0.5f, 0),
                                  Fvector().set(0, +1, 0));
    planes[CUBE_POSITIVE_Z].build(Fvector().set(0, 0, +0.5f),
                                  Fvector().set(0, 0, -1));
    planes[CUBE_NEGATIVE_Z].build(Fvector().set(0, 0, -0.5f),
                                  Fvector().set(0, 0, +1));
  }
  void vector_from_point(Fvector3 &normal, ECubeSide side, u32 x, u32 y,
  ...
  u32 &pixel_from_side(u32 *pixels, u32 width, u32 height, u32 side, u32 x,
                       u32 y) {
  ...
  void xys_from_vector(u32 &ux, u32 &uy, u32 &t_side, const Fvector &n,
  ...
  void scale_map(u32 *src_data, u32 src_width, u32 src_height, u32 *dst_data,
                 u32 dst_width, u32 dst_height, float sample_factor,
                 ETOOLS::pb_callback cb, void *pb_data) {
    ...
    for (u32 side = CUBE_POSITIVE_X; side < CUBE_SIDE_COUNT; side++) {
      for (u32 y_dst = 0; y_dst < dst_height; y_dst++) {
        for (u32 x_dst = 0; x_dst < dst_width; x_dst++) {
          pb_cur += pb_weight;
          cb(pb_data, pb_cur);
          vector_from_point(normal, ECubeSide(side), x_dst, y_dst, dst_width,
                            dst_height);
          u32 &out = pixel_from_side(dst_data, dst_width, dst_height, side,
                                     x_dst, y_dst);
...
ETOOLS_API void __stdcall SimplifyCubeMap(u32 *src_data, u32 src_width,
                                          ...
                                          float sample_factor, pb_callback cb,
                                          void *pb_data) {
  cm.scale_map(src_data, src_width, src_height, dst_data, dst_width, dst_height,
               sample_factor, cb, pb_data);
}
} // namespace ETOOLS
}
