...
int wav_open(FILE *in, oe_enc_opt *opt, unsigned char *oldbuf, int buflen) {
  ...
  if (format.align == format.channels * samplesize &&
    ...
    if (len) {
      ...
      if (fseek(in, 0, SEEK_END) == -1) {
        ...
        opt->total_samples_per_channel =
            (ftell(in) - pos) / (format.channels * samplesize);
    ...
    fprintf(stderr, _("ERROR: Wav file is unsupported subformat (must be 8,16, "
                      "or 24 bit PCM\n"
...
long wav_read(void *in, float **buffer, int samples) {
  ...
  if (f->totalsamples &&
      f->samplesread + bytes_read / (sampbyte * f->channels) >
          f->totalsamples) {
...
long wav_ieee_read(void *in, float **buffer, int samples) {
  wavfile *f = (wavfile *)in;
  float *buf =
      (float *)alloca(samples * 4 * f->channels); /* de-interleave buffer */
  long bytes_read = fread(buf, 1, samples * 4 * f->channels, f->f);
  int i, j;
  long realsamples;

  if (f->totalsamples &&
      f->samplesread + bytes_read / (4 * f->channels) > f->totalsamples)
...
static long read_resampled(void *d, float **buffer, int samples) {
  ...
  out_samples =
      res_push(&rs->resampler, buffer, (float const **)rs->bufs, in_samples);

  if (out_samples <= 0) {
    fprintf(stderr, _("BUG: Got zero samples from resampler: your file will be "
...
int setup_resample(oe_enc_opt *opt) {
  ...
  rs->done = 0;
  if (res_init(&rs->resampler, rs->channels, opt->resamplefreq, opt->rate,
               RES_END)) {
  ...
  if (opt->total_samples_per_channel)
    opt->total_samples_per_channel =
        (int)((float)opt->total_samples_per_channel *
