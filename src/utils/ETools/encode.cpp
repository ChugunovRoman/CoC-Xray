...
int oe_encode(oe_enc_opt *opt) {
  ...
  if (opt->channels > 255) {
    fprintf(stderr, _("255 channels should be enough for anyone. (Sorry, "
                      "vorbis doesn't support more)\n"));
    return 1;
  }
  ...
  opt->start_encode(opt->infilename, opt->filename, opt->bitrate, opt->quality,
                    opt->quality_set, opt->managed, opt->min_bitrate,
                    opt->max_bitrate);
...

  {
    ...
    /* Build the packets */
    vorbis_analysis_headerout(&vd, opt->comments, &header_main,
...
cleanup:
  ...
  opt->end_encode(opt->filename, time_elapsed, opt->rate, samplesdone,
                  bytes_written);
...
void update_statistics_notime(char *fn, long total, long done, double time) {
  ...
  fprintf(stderr, _("\tEncoding [%2dm%.2ds so far] %c "), ((int)time) / 60,
          (int)(time - (double)((int)time / 60) * 60),
          spinner[spinpoint++ % 4]);
}
...
void final_statistics(char *fn, double time, int rate, long samples,
                      long bytes) {
  double speed_ratio;
  if (fn)
...
void start_encode_full(char *fn, char *outfn, int bitrate, float quality,
                       int qset, int managed, int min, int max) {
  if (bitrate > 0) {
    if (managed > 0) {
      fprintf(stderr,
              _("Encoding %s%s%s to \n         "
              ...
              outfn ? "\"" : "", outfn ? outfn : _("standard output"),
              outfn ? "\"" : "", bitrate);
