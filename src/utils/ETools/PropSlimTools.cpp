...
ETOOLS_API void __stdcall VIPM_AppendVertex(const Fvector3 &p,
                                            const Fvector2 &uv) {
...
ETOOLS_API void __stdcall VIPM_AppendFace(u16 v0, u16 v1, u16 v2) {
  xr_new<MeshTri>(g_ppTempPts[v0], g_ppTempPts[v1], g_ppTempPts[v2],
                  &g_pObject->CurTriRoot, &g_pObject->CurEdgeRoot);
}
...
void CalculateAllCollapses(Object *m_pObject, u32 max_sliding_window = u32(-1),
  ...
  while (true) {
    ...
    for (ppt = m_pObject->CurPtRoot.ListNext(); ppt != NULL;
         ppt = ppt->ListNext()) {
      if (0 == ppt->FirstEdge())
        continue;
      ...
      if (!m_pObject->CollapseAllowedForLevel(
              ppt, m_pObject->iCurSlidingWindowLevel)) {
    ...
    if (fBestError >
        (fBestErrorNL + fAverage * m_fSlidingWindowErrorTolerance)) {
...
ETOOLS_API VIPM_Result *__stdcall VIPM_Convert(u32 max_sliding_window,
