...
//		Set to 1 to enforce/assume that when adding a tri, all three
// edges 		will exist. So all tris must have all three edge entries
// filled. 		This is a speedup, and an error check, if that's what
// your app does. 		The exception is if autocreation of edges is
// done for the MeshTri creator. 		Similarly, before deleteing
// edges, make sure you have deleted all the 		tris that use them
...
public:
  ...
  MeshTri(MeshPt *pNewPt1, MeshPt *pNewPt2, MeshPt *pNewPt3,
          MeshTri *pListRoot = NULL, MeshEdge *pEdgeListRoot = NULL);
  ~MeshTri(void);
...
public:
  ...
  // Try to merge these two edges. Result is TRUE if it succeeded - note that
  // the other edge will be NOT deleted.
...
private:
  ArbitraryList<MeshEdge *>
      EdgeList; // The list of edges that use this point (in no order).
  ArbitraryList<MeshTri *>
      TriList; // The list of tris that use this point (in no order).
...
public:
  ...
  // Return the first Edge in the list. MUST be called before calling
  // NextEdge(). If a non-NULL pPt is supplied, only edges using this and pPt
...
inline MeshTri::MeshTri(MeshPt *pNewPt1, MeshPt *pNewPt2, MeshPt *pNewPt3,
                        MeshTri *pListRoot, MeshEdge *pEdgeListRoot) {
  VERIFY(pNewPt1 != NULL);
  VERIFY(pNewPt2 != NULL);
  VERIFY(pNewPt3 != NULL);
  pPt1 = pNewPt1;
...
inline void MeshTri::InternalDelete(BOOL bBinUnusedEdges) {
  ...
  if (pEdge12 != NULL) {
    ...
    if (bBinUnusedEdges && (pEdge12->pTri12 == NULL) &&
        (pEdge12->pTri21 == NULL)) {
  ...
  if (pEdge23 != NULL) {
    ...
    if (bBinUnusedEdges && (pEdge23->pTri12 == NULL) &&
        (pEdge23->pTri21 == NULL)) {
      // This edge is no longer in use.
      xr_delete(pEdge23);
    }
  ...
  if (pEdge31 != NULL) {
    ...
    if (bBinUnusedEdges && (pEdge31->pTri12 == NULL) &&
        (pEdge31->pTri21 == NULL)) {
      // This edge is no longer in use.
      xr_delete(pEdge31);
    }
...
#define FAIL_CHECK()                                                           \
  bRes = FALSE;                                                                \
  VERIFY(FALSE)
inline bool MeshTri::ConsistencyCheck(MeshPt *pPtRoot, MeshEdge *pEdgeRoot,
                                      MeshTri *pTriRoot) {
  bool bRes = TRUE;
...
inline MeshEdge::MeshEdge(MeshPt *pNewPt1, MeshPt *pNewPt2,
                          MeshEdge *pListRoot) {
  VERIFY(pNewPt1 != NULL);
  VERIFY(pNewPt2 != NULL);
  pPt1 = pNewPt1;
...
// Try to merge these two edges. Result is TRUE if it succeeded - note that the
// other edge will be NOT deleted.
...
inline MeshEdge *MeshEdge::DoProxMatch(void) {
  ...
  //			If the other pt is prox to pPt2, then we found a prox
  // edge.
  u32 i;
...
inline bool MeshEdge::ConsistencyCheck(MeshPt *pPtRoot, MeshEdge *pEdgeRoot,
                                       MeshTri *pTriRoot) {
  bool bRes = TRUE;
...
inline MeshTri *MeshPt::FindTri(MeshPt *pPt1, MeshPt *pPt2) {
  ...
  for (i = 0; i < TriList.size(); i++) {
    ...
    VERIFY(pTri != NULL);
    VERIFY((pTri->pPt1 == this) || (pTri->pPt2 == this) ||
           (pTri->pPt3 == this));
...
inline bool MeshPt::AddProx(MeshPt *pPt, bool bProxEdges) {
  ...
  //			If those proxes form and edge with pPt, the edges are
  // prox.
...
inline bool MeshPt::ConsistencyCheck(MeshPt *pPtRoot, MeshEdge *pEdgeRoot,
                                     MeshTri *pTriRoot) {
  bool bRes = TRUE;
