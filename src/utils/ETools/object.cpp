...
bool Object::Valid(void) {
  for (MeshTri *pTriTgt = CurTriRoot.ListNext(); pTriTgt != NULL;
       pTriTgt = pTriTgt->ListNext())
...
void Object::Initialize(void) {
  ...
  for (MeshPt *pPt = CurPtRoot.ListNext(); pPt != NULL; pPt = pPt->ListNext())
    iFullNumPts++;
  iFullNumTris = 0;
  for (MeshTri *pTri = CurTriRoot.ListNext(); pTri != NULL;
       pTri = pTri->ListNext())
    iFullNumTris++;
  SetNewLevel(0);
}
...
void Object::CreateEdgeCollapse(MeshPt *pptBinned, MeshPt *pptKept) {
  ...
  for (ptri = pptBinned->FirstTri(); ptri != NULL;
       ptri = pptBinned->NextTri()) {
  ...
  for (i = iNumTrisCollapsed - 1; i >= 0; i--) {
    if (pBinnedTris[i] != NULL) {
      ...
      VERIFY(iCurSlidingWindowLevel ==
             pBinnedTris[i]->mytri.iSlidingWindowLevel);
  ...
  for (i = 0; i < iNumTrisCollapsed; i++) {
    ...
    if ((pTriInfo->ppt[1] == pptKept) || (pTriInfo->ppt[2] == pptKept)) {
      ...
      MeshTri *pTri =
          xr_new<MeshTri>(pTriInfoNew->ppt[0], pTriInfoNew->ppt[1],
...
long Object::UndoCollapse(void) {
  if (pNextCollapse->ListNext() == NULL) {
    ...
    for (i = 0; i < pNextCollapse->TriOriginal.size(); i++) {
      ...
      MeshTri *pTri =
          xr_new<MeshTri>(pTriInfo->ppt[0], pTriInfo->ppt[1], pTriInfo->ppt[2],
    ...
    if (pNextCollapse->ListNext() != NULL) {
      if (pNextCollapse->ListNext()->iSlidingWindowLevel !=
          iCurSlidingWindowLevel) {
...
long Object::CollapseAllowedForLevel(MeshPt *pptBinned, int iLevel) {
  ...
  long bRes = TRUE;
  for (MeshTri *pTri = pptBinned->FirstTri(); pTri != NULL;
       pTri = pptBinned->NextTri()) {
...
void pack_to_vector(MxVector &tgt, const Fvector3 &src_p, float src_u,
                    float src_v) {
  tgt[0] = src_p.x;
  tgt[1] = src_p.y;
  tgt[2] = src_p.z;
...
void Object::compute_face_quadric(MeshTri *tri, MxQuadric &Q) {
  ...
  pack_to_vector(v1, tri->pPt1->mypt.vPos, tri->pPt1->mypt.fU,
                 tri->pPt1->mypt.fV);
  pack_to_vector(v2, tri->pPt2->mypt.vPos, tri->pPt2->mypt.fU,
                 tri->pPt2->mypt.fV);
  pack_to_vector(v3, tri->pPt3->mypt.vPos, tri->pPt3->mypt.fU,
                 tri->pPt3->mypt.fV);
...
float Object::FindCollapseError(MeshPt *pptBinned, MeshEdge *pedgeCollapse,
  ...
  /*
          ...
          if ( bTryToCacheResult && ( pptLast == pptBinned ) ){
                  ...
                  for ( MeshTri *ptri = pptBinned->FirstTri(); ptri != NULL;
     ptri = pptBinned->NextTri() ) qSum += Quad3 ( ptri->pPt1->mypt.vPos,
     ptri->pPt2->mypt.vPos, ptri->pPt3->mypt.vPos );
