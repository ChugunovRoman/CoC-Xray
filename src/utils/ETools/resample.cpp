...
static void filt_sinc(float *dest, int N, int step, double fc, double gain,
                      int width) {
  double s = fc / step;
  int mid, x;
...
static void win_kaiser(float *dest, int N, double alpha, int width) {
  ...
  while (N--) {
    *dest *=
        I_zero(alpha * sqrt(1.0 - ((double)x * (double)x) / midsq)) / I_alpha;
    x++;
    dest += width;
    if (dest >= endpoint)
      dest = ++base;
...
int res_init(res_state *state, int channels, int outfreq, int infreq,
             res_parameter op1, ...) {
  ...
  if (state == NULL || channels <= 0 || outfreq <= 0 || infreq <= 0 ||
      taps <= 0)
    return -1;
  ...
  if ((state->table = (float *)calloc(outfreq * taps, sizeof(float))) == NULL)
    return -1;
  if ((state->pool = (SAMPLE *)calloc(channels * taps, sizeof(SAMPLE))) ==
      NULL) {
    free(state->table);
    state->table = NULL;
    return -1;
...
static SAMPLE sum(float const *scale, int count, SAMPLE const *source,
                  SAMPLE const *trigger, SAMPLE const *reset, int srcstep) {
  float total = 0.0;
...
static int push(res_state const *const state, SAMPLE *pool, int *const poolfill,
                int *const offset, SAMPLE *dest, int dststep,
                SAMPLE const *source, int srcstep, size_t srclen) {
  SAMPLE *const destbase = dest, *poolhead = pool + *poolfill,
                *poolend = pool + state->taps, *newpool = pool;
  ...
  while (source < endpoint) {
    *dest = sum(state->table + *offset * state->taps, state->taps, source, base,
                poolend, srcstep);
    dest += dststep;
    *offset += state->infreq;
  ...
  /* if we didn't get enough to completely replace the pool, then shift things
   * about a bit */
...
int res_push_check(res_state const *const state, size_t srclen) {
  ...
  return (srclen * state->outfreq - state->offset + state->infreq - 1) /
         state->infreq;
}

int res_push(res_state *state, SAMPLE **dstlist, SAMPLE const **srclist,
             size_t srclen) {
  ...
  for (i = 0; i < state->channels; i++) {
    ...
    offset = state->offset;
    result = push(state, state->pool + i * state->taps, &poolfill, &offset,
                  dstlist[i], 1, srclist[i], 1, srclen);
  }
...
int res_push_interleaved(res_state *state, SAMPLE *dest, SAMPLE const *source,
                         size_t srclen) {
  int result = -1, poolfill = -1, offset = -1, i;
  ...
  for (i = 0; i < state->channels; i++) {
    ...
    offset = state->offset;
    result =
        push(state, state->pool + i * state->taps, &poolfill, &offset, dest + i,
...
int res_drain(res_state *state, SAMPLE **dstlist) {
  ...
  for (i = 0; i < state->channels; i++) {
    ...
    offset = state->offset;
    result = push(state, state->pool + i * state->taps, &poolfill, &offset,
                  dstlist[i], 1, tail, 1, state->taps / 2 - 1);
  }
...
int res_drain_interleaved(res_state *state, SAMPLE *dest) {
  ...
  for (i = 0; i < state->channels; i++) {
    ...
    offset = state->offset;
    result = push(state, state->pool + i * state->taps, &poolfill, &offset,
                  dest + i, state->channels, tail, 1, state->taps / 2 - 1);
  }
