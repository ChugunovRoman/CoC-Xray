...
extern void xrCompiler(LPCSTR name, bool draft_mode, bool pure_covers,
                       LPCSTR out_name);
...
void execute(LPSTR cmd) {
  ...
  if (strstr(cmd, "-f")) {
    ...
    xrCompiler(prjName, !!strstr(cmd, "-draft"), !!strstr(cmd, "-pure_covers"),
               output);
  } else {
    if (strstr(cmd, "-s")) {
      if (xr_strlen(name))
...
void Startup(LPSTR lpCmdLine) {
  ...
  if ((strstr(cmd, "-f") == 0) && (strstr(cmd, "-g") == 0) &&
      (strstr(cmd, "-m") == 0) && (strstr(cmd, "-s") == 0) &&
      (strstr(cmd, "-t") == 0) && (strstr(cmd, "-c") == 0) &&
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  if (0 == hFactory)
    R_CHK(GetLastError());
  R_ASSERT2(hFactory, "Factory DLL raised exception during loading or there is "
                      "no factory DLL at all");

  create_entity =
      (Factory_Create *)GetProcAddress(hFactory, "_create_entity@4");
  R_ASSERT(create_entity);
  destroy_entity =
      (Factory_Destroy *)GetProcAddress(hFactory, "_destroy_entity@4");
