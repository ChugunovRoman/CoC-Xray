...
extern LPCSTR generate_temp_file_name(LPCSTR header0, LPCSTR header1,
                                      string_path &buffer);
...
IC shared_str CGameSpawnConstructor::actor_level_name() {
  ...
  return (strconcat(
      ...
      *game_graph()
           .header()
           .level(game_graph().vertex(m_actor->m_tGraphID)->level_id())
           .name(),
      ".spawn"));
}

extern void read_levels(CInifile *ini, xr_set<CLevelInfo> &m_levels,
                        bool rebuild_graph, xr_vector<LPCSTR> *);
...
void CGameSpawnConstructor::load_spawns(LPCSTR name, bool no_separator_check) {
  ...
  for (; I != E; ++I) {
    ...
    m_level_spawns.push_back(
        xr_new<CLevelSpawnConstructor>(level, this, no_separator_check));
  }

  string256 temp;
  xr_sprintf(temp,
             "There are no valid levels (with AI-map and graph) in the section "
             "'levels' in the '%s' to build spawn file from!",
             GAME_CONFIG);
...
void CGameSpawnConstructor::verify_spawns(ALife::_SPAWN_ID spawn_id) {
  xr_vector<ALife::_SPAWN_ID>::iterator J =
      std::find(m_temp0.begin(), m_temp0.end(), spawn_id);
...
void CGameSpawnConstructor::verify_level_changers() {
  ...
  VERIFY2(m_level_changers.empty(),
...
void CGameSpawnConstructor::add_story_object(ALife::_STORY_ID id,
  ...
  if (I != m_story_objects.end()) {
    ...
    Msg("Object %s, story id %d", (*I).second->name_replace(),
        (*I).second->m_story_id);
    VERIFY3(
        I == m_story_objects.end(),
        "There are several objects which has the same unique story ID, level ",
        level_name);
  }
...
void CGameSpawnConstructor::add_object(CSE_Abstract *object) {
  ...
  spawn_graph().add_vertex(xr_new<CServerEntityWrapper>(object),
                           object->m_tSpawnID);
  ...
  if (failed) {
    ...
    for (GameGraph::_GRAPH_ID i = 0; i < n; ++i) {
      if (game_graph().vertex(i)->level_id() == level.id()) {
        float distance =
            game_graph().vertex(i)->game_point().distance_to_sqr(current);
