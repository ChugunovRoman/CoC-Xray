...
template <typename T>
IC void CLevelGraph::unpack_xz(const CLevelGraph::CVertex &vertex, T &x,
                               T &z) const {
...
ICF const Fvector CLevelGraph::vertex_position(
  ...
  dest_position.y = (float(source_position.y()) / 65535) * header().factor_y() +
                    header().box().min.y;
...
CLevelGraph::vertex_position(CLevelGraph::CPosition &dest_position,
  ...
  VERIFY(
      iFloor((source_position.z - header().box().min.z) / header().cell_size() +
             .5f) < (int)m_row_length);
  int pxz =
      iFloor(
          ((source_position.x - header().box().min.x) / header().cell_size() +
           .5f)) *
          m_row_length +
      iFloor((source_position.z - header().box().min.z) / header().cell_size() +
             .5f);
...
IC const CLevelGraph::CPosition
CLevelGraph::vertex_position(const Fvector &position) const {
  CLevelGraph::CPosition _vertex_position;
  return (vertex_position(_vertex_position, position));
}

IC bool
CLevelGraph::inside(const CLevelGraph::CVertex &vertex,
...
IC bool
CLevelGraph::inside(const CVertex *vertex,
                    const CLevelGraph::CPosition &vertex_position) const {
...
IC bool
CLevelGraph::inside(const u32 vertex_id,
                    const CLevelGraph::CPosition &vertex_position) const {
...
IC bool CLevelGraph::inside(const u32 vertex_id,
  ...
  int pxz =
      iFloor(
          ((position.x - header().box().min.x) / header().cell_size() + .5f)) *
          m_row_length +
...
IC float CLevelGraph::vertex_plane_y(const CLevelGraph::CVertex &vertex,
                                     const float X, const float Z) const {
...
IC float CLevelGraph::vertex_plane_y(const u32 vertex_id, const float X,
                                     const float Z) const {
...
ICF bool
CLevelGraph::CVertex::operator<(const CLevelGraph::CVertex &vertex) const {
...
IC void CLevelGraph::begin(const u32 vertex_id, const_iterator &_begin,
                           const_iterator &end) const {
...
IC bool CLevelGraph::create_straight_path(u32 start_vertex_id,
  ...
  if (vertex(start_vertex_id)->position().xz() == dest_xz) {
    ...
    if (bAssignY)
      tIntersectPoint.y = vertex_plane_y(vertex(cur_vertex_id),
  ...
  for (;;) {
    ...
    for (; I != E; ++I) {
      ...
      if (box.pick_exact(start, dir)) {
        ...
        u32 dwIntersect =
#endif
            intersect_no_check(start_point.x, start_point.y, finish_point.x,
                               finish_point.y, next1.x, next1.y, next2.x,
        ...
        if (tpaOutputPoints.size() > 100000) {
          ...
          FlushLog();
          R_ASSERT2(false,
                    "Loop became infinite :-( call Dima and SAVE YOUR LOG!");
        }
#endif
        break;
      }
...
IC bool CLevelGraph::valid_vertex_position(const Fvector &position) const {
  ...
  if (!(iFloor((position.z - header().box().min.z) / header().cell_size() +
               .5f) < (int)m_row_length))
    return (false);

  if (!(iFloor((position.x - header().box().min.x) / header().cell_size() +
               .5f) < (int)m_column_length))
    return (false);
