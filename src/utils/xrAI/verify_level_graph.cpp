...
void floodfill(const CLevelGraph &level_graph, xr_vector<bool> &marks,
               u32 start_vertex_id) {
  ...
  while (stack_iterator != stack_storage) {
    ...
    for (; I != E; ++I) {
      ...
      if (level_graph.valid_vertex_id(neighbour_vertex_id) &&
          !marks[neighbour_vertex_id])
        PUSH(level_graph.vertex(neighbour_vertex_id));
    }
  }
}

IC bool single_link(const CLevelGraph &level_graph, u32 i1, u32 i2,
                    u32 link_index) {
...
void verify_level_graph(LPCSTR name, bool verbose) {
  ...
  stack_storage = (CLevelGraph::CVertex **)xr_malloc(
      level_graph->header().vertex_count() * sizeof(CLevelGraph::CVertex *));
...

  {
    ...
    xr_vector<u32>::iterator I =
        std::unique(single_links.begin(), single_links.end());
