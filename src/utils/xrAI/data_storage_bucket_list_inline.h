...
IC bool CBucketList::is_opened_empty() {
  ...
  if (!m_buckets[m_min_bucket_id]) {
    if (!clear_buckets)
      ...
      for (++m_min_bucket_id;
           (m_min_bucket_id < bucket_count) && !m_buckets[m_min_bucket_id];
           ++m_min_bucket_id)
        ;
...
IC void CBucketList::verify_buckets() const {
  ...
  //				VERIFY	(indexes[j->index()].m_path_id ==
  // current_path_id()); 				VERIFY	(compute_bucket_id(*j) == i);
  // VERIFY
  //(!j->prev() || (j == j->prev()->next())); 				VERIFY
  //(!j->next() || (j == j->next()->prev())); 				VERIFY
  // (!j->next() || (j != j->next())); 				VERIFY
  //(!j->prev() || (j != j->prev()));
  //			}
  //			for ( ; k; k=k->prev(), ++count1) {
  //				VERIFY	(indexes[k->index()].m_path_id ==
  // current_path_id()); 				VERIFY	(compute_bucket_id(*k) == i);
  // VERIFY
  //(!k->prev() || (k == k->prev()->next())); 				VERIFY
  //(!k->next() || (k == k->next()->prev())); 				VERIFY
  // (!k->next() || (k != k->next())); 				VERIFY
  //(!k->prev() || (k != k->prev()));
  //			}
  //			VERIFY		(count == count1);
  //		}
