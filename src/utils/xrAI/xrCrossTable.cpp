...
void vfRecurseUpdate(u32 dwStartNodeID, u32 percent, u32 iVertexCount) {
  ...
  for (; !curr_fringe.empty();) {
    ...
    ++curr_dist;
    Progress(float(percent) / float(iVertexCount) +
             float(total_count) / (float(iVertexCount) *
                                   float(g_tMap->header().vertex_count())));
  }
}

void vfRecurseMark(const CLevelGraph &tMap, xr_vector<bool> &tMarks,
                   u32 dwStartNodeID) {
  CLevelGraph::const_iterator I, E;
...
CCrossTableBuilder::CCrossTableBuilder(LPCSTR caProjectName) {
  ...
  tMemoryStream.open_chunk(CROSS_TABLE_CHUNK_DATA);
  {
    for (int i = 0; i < iNodeCount; i++) {
      ...
      for (; I != E; I++)
        if (float((*I)[i]) * tMap.header().cell_size() <
            tCrossTableCell.fDistance) {
          tCrossTableCell.fDistance =
              float((*I)[i]) * tMap.header().cell_size();
      ...
      for (int j = 0; j < iVertexCount; j++)
        if ((tGraph.vertex(j)->level_vertex_id() == (u32)i) &&
          ...
          Msg("! Warning : graph points are too close, therefore cross table "
              "is automatically validated");
          Msg("%d : [%f][%f][%f] %d[%f] -> %d[%f]", i,
              VPUSH(tGraph.vertex(j)->level_point()),
              tCrossTableCell.tGraphIndex, tCrossTableCell.fDistance, j,
              tDistances[j][i]);
          tCrossTableCell.fDistance =
              float(tDistances[j][i]) * tMap.header().cell_size();
  ...
  strconcat(sizeof(caFileName), caFileName, caProjectName,
            CROSS_TABLE_NAME_RAW);
