...
void Compress(NodeCompressed &Dest, vertex &Src, hdrNODES &H) {
  ...
  //	Msg				("[%.3f -> %d][%.3f -> %d][%.3f -> %d][%.3f
  //-> %d]", 		Src.cover[0],Dest.cover0,
  //Src.cover[1],Dest.cover1, 		Src.cover[2],Dest.cover2, 		Src.cover[3],Dest.cover3
  //		);
...
class CNodeRenumberer {
  ...
  struct SSortNodesPredicate {
    ...
    IC bool operator()(const NodeCompressed &vertex0,
                       const NodeCompressed &vertex1) const {
      return (vertex0.p.xz() < vertex1.p.xz());
    }

    IC bool operator()(u32 vertex_id0, u32 vertex_id1) const {
      return (compressed_nodes[vertex_id0].p.xz() <
              compressed_nodes[vertex_id1].p.xz());
    }
  };
...
void xrSaveNodes(LPCSTR N, LPCSTR out_name) {
  Msg("NS: %d, CNS: %d, ratio: %f%%", sizeof(vertex),
  ...
  //		fs->w
  //(&g_covers_palette[j],sizeof(g_covers_palette[j]));
