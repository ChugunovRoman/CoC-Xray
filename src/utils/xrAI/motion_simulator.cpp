...
// -----------------------------------------------------------------------
IC float classifyPoint(const Fvector &point, const Fvector &planeO,
...
// -----------------------------------------------------------------------
IC float intersectRayPlane(const Fvector &rayOrigin,
...
IC void closestPointOnLine(Fvector &res, const Fvector &a, const Fvector &b,
                           const Fvector &p) {
...
IC void closestPointOnTriangle(Fvector &result, const Fvector *V,
                               const Fvector &p) {
  Fvector Rab;
...
IC void closestPointOnTriangle(Fvector &result, const cl_tri &T,
                               const Fvector &P) {
  Fvector Rab;
...
IC float intersectRaySphere(const Fvector &rO, const Fvector &rV,
                            const Fvector &sO, float sR) {
  Fvector Q;
  Q.sub(sO, rO);
...
void msimulator_CheckCollision(SCollisionData &cl) {
  ...
  for (u32 i_t = 0; i_t != clContactedT.size(); i_t++) {
    ...
    if (T.N.dotproduct(normalizedVelocity) < 0.0f) {
      ...
      // OLES: 'cause our radius has unit length, this point lies exactly on
      // sphere
      ...
      if ((sIPoint.dotproduct(T.N) + T.d) < -EPS_S) {
        ...
        bInsideTri =
            CDB::TestRayTri2(sIPoint, T.N, T.p, distToPlaneIntersection);

        // calculate plane intersection point
        pIPoint.mad(sIPoint, T.N, distToPlaneIntersection);
      } else {
        // shoot ray along the velocity xr_vector
        bInsideTri = CDB::TestRayTri2(sIPoint, normalizedVelocity, T.p,
                                      distToPlaneIntersection);
      ...
      if (!bInsideTri) {
        ...
        if (distToEllipsoidIntersection >= 0) {
          ...
          sIPoint.mad(polyIPoint, normalizedVelocity,
                      -distToEllipsoidIntersection);
        }
      }
...
//-----------------------------------------------------------------------------
Fvector msimulator_CollideWithWorld(SCollisionData &cl, Fvector position,
                                    Fvector velocity, WORD cnt) {
  //
  msimulator_ResolveStuck(cl, position);

  // do we need to worry ?
  //	if
  //(fsimilar(position.x,target.x,EPS_L)&&fsimilar(position.z,target.z,EPS_L))
  ...
  if (cl.bFoundCollision == FALSE && !cl.bStuck) {
    ...
    // Generate the slide xr_vector, which will become our _new velocity
    // xr_vector for the next iteration
    Fvector newVelocityVector;
    newVelocityVector.sub(newDestinationPoint,
                          cl.vNearestPolygonIntersectionPoint);

    // now we recursively call the function with the _new position and velocity
    cl.vLastSafePosition.set(position);
    return msimulator_CollideWithWorld(cl, newSourcePoint, newVelocityVector,
                                       cnt + 1);
  }
}
...
void msimulator_Simulate(Fvector &result, Fvector &start, Fvector &end,
