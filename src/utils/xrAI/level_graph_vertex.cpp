...
void CLevelGraph::choose_point(const Fvector &start_point,
                               ...
                               const SContour &_contour, int node_id,
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    dwIntersect =
        intersect(start_point.x, start_point.z, finish_point.x, finish_point.z,
                  tCheckPoint1.x, tCheckPoint1.z, tCheckPoint2.x,
                  tCheckPoint2.z, &tIntersectPoint.x, &tIntersectPoint.z);
...
float CLevelGraph::check_position_in_direction(u32 start_vertex_id,
...
float CLevelGraph::mark_nodes_in_direction(u32 start_vertex_id,
                                           const Fvector &start_position,
  ...
  return (mark_nodes_in_direction(start_vertex_id, start_position, finish_point,
                                  tpaStack, tpaMarks));
}
...
float CLevelGraph::mark_nodes_in_direction(u32 start_vertex_id,
  ...
  return (mark_nodes_in_direction(start_vertex_id, start_position,
                                  vertex_position(finish_vertex_id), tpaStack,
                                  tpaMarks));
}
...
float CLevelGraph::farthest_vertex_in_direction(
    u32 start_vertex_id, const Fvector &start_point,
...
bool CLevelGraph::create_straight_path(u32 start_vertex_id,
  ...
  return (create_straight_path(start_vertex_id, v2d(start_point),
                               v2d(finish_point), tpaOutputPoints,
                               tpaOutputNodes, bAddFirstPoint, bClearPath));
}

u32 CLevelGraph::check_position_in_direction_slow(
    u32 start_vertex_id, const Fvector2 &start_position,
  ...
  for (;;) {
    ...
    for (; I != E; ++I) {
      ...
      if (box.pick_exact(start, dir)) {
        ...
        VERIFY(_valid(next2));
        u32 dwIntersect =
            intersect(start_point.x, start_point.y, finish_point.x,
...
float CLevelGraph::cover_in_direction(float fAngle, float b1, float b0,
                                      float b3, float b2) const {
  float fResult;
