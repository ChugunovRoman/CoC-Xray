...
IC CLevelSpawnConstructor::LEVEL_CHANGER_STORAGE &
CLevelSpawnConstructor::level_changers() const {
...
void CLevelSpawnConstructor::add_graph_point(CSE_Abstract *abstract) {
  CSE_ALifeGraphPoint *graph_point =
      smart_cast<CSE_ALifeGraphPoint *>(abstract);
  R_ASSERT(graph_point);
  m_graph_points.push_back(graph_point);
}

// void CLevelSpawnConstructor::add_spawn_group
// (CSE_Abstract			*abstract)
//{
//	CSE_SpawnGroup			*spawn_group =
// smart_cast<CSE_SpawnGroup*>(abstract); 	R_ASSERT
//(spawn_group); 	m_spawn_groups.insert
//(std::make_pair(spawn_group->name_replace(),spawn_group)); 	if
//(xr_strlen(spawn_group->m_spawn_control)) 		add_group_object
//(spawn_group,spawn_group->m_spawn_control); 	add_free_object
//(abstract);
//}

void CLevelSpawnConstructor::add_story_object(
    CSE_ALifeDynamicObject *dynamic_object) {
  m_game_spawn_constructor->add_story_object(dynamic_object->m_story_id,
                                             dynamic_object, *m_level.name());
}

void CLevelSpawnConstructor::add_space_restrictor(
    CSE_ALifeDynamicObject *dynamic_object) {
  ...
  if (space_restrictor->m_space_restrictor_type ==
  ...
  m_space_restrictors.push_back(
      xr_new<CSpaceRestrictorWrapper>(space_restrictor));
}

void CLevelSpawnConstructor::add_level_changer(CSE_Abstract *abstract) {
  CSE_ALifeLevelChanger *level_changer =
      smart_cast<CSE_ALifeLevelChanger *>(abstract);
...
// void CLevelSpawnConstructor::add_group_object
// (CSE_Abstract			*abstract, shared_str group_section,
// bool)
//{
...
//		m_spawn_objects.insert
//(std::make_pair(group_section,temp));
//	}
//	else
...
//		add_group_object
//(abstract,_GetItem(*group_section,i,temp),true);
//}
...
void CLevelSpawnConstructor::load_objects() {
  ...
  for (; chunk; chunk = level_spawn->open_chunk_iterator(id, chunk)) {
    ...
    CSE_ALifeCreatureActor *actor =
        smart_cast<CSE_ALifeCreatureActor *>(alife_object);
    if (actor) {
    ...
    CSE_ALifeDynamicObject *dynamic_object =
        smart_cast<CSE_ALifeDynamicObject *>(alife_object);
    if (dynamic_object) {
...
// IC	void CLevelSpawnConstructor::normalize_probability
...
// ()
//{
//	SPAWN_GRPOUP_OBJECTS::iterator				I =
// m_spawn_objects.begin();
//	SPAWN_GRPOUP_OBJECTS::iterator				E =
// m_spawn_objects.end();
//
//	for ( ; I != E; I++) {
//		R_ASSERT
//(xr_strlen(*(*I).first)); 		R_ASSERT
//((*I).second); 		SPAWN_GROUPS::iterator
//J = m_spawn_groups.find((*I).first); 		if (J == m_spawn_groups.end())
// clMsg
//("! ERROR (spawn group not found!) : %s",*(*I).first); R_ASSERT3 (J !=
//m_spawn_groups.end(),"Specified group control not
// found!",(*(*I).second)[0]->name_replace());
//
//		GROUP_OBJECTS::iterator					i =
//(*I).second->begin(); 		GROUP_OBJECTS::iterator e =
//(*I).second->end(); 		for ( ; i != e; i++) {
...
void CLevelSpawnConstructor::correct_objects() {
  ...
  if (dwStart >= dwFinish) {
    ...
    xr_sprintf(
        S,
        "There are no graph vertices in the game graph for the level '%s' !\n",
        *m_level.name());
  ...
  for (int i = 0; i < (int)m_spawns.size(); i++) {
    ...
    if (m_spawns[i]->used_ai_locations() &&
        !level_graph().inside(level_graph().vertex(m_spawns[i]->m_tNodeID),
                              position)) {
      Fvector new_position =
          level_graph().vertex_position(m_spawns[i]->m_tNodeID);
    ...
    if (game_graph().vertex(dwBest)->level_id() != m_level.id()) {
      ...
      S += xr_sprintf(S, sizeof(S1) - (S1 - &S[0]),
                      "Corresponding graph vertex for the spawn point is "
                      "located on the ANOTHER level\n",
      ...
      S += xr_sprintf(S, sizeof(S1) - (S1 - &S[0]),
                      ...
                      *game_graph()
                           .header()
                           .level(game_graph().vertex(dwBest)->level_id())
                           .name());
      S += xr_sprintf(
          S, sizeof(S1) - (S1 - &S[0]),
    ...
    float fCurrentBestDistance =
        cross_table().vertex(m_spawns[i]->m_tNodeID).distance();
    if (dwBest == u32(-1)) {
      string4096 S1;
      char *S = S1;
      S += xr_sprintf(
...
struct remove_too_far_predicate {
  ...
  IC remove_too_far_predicate(const CLevelGraph *graph, const Fvector &position,
                              float radius) {
    VERIFY(graph);
    m_graph = graph;
    m_position = position;
  ...
  IC bool operator()(const u32 &vertex_id) const {
    return (m_graph->vertex_position(vertex_id).distance_to_sqr(m_position) >
            m_radius_sqr);
  }
};
...
public:
  IC remove_invalid_zones_predicate(
      const CLevelSpawnConstructor *level_spawn_constructor,
  ...
  IC bool operator()(CSE_ALifeObject *object) const {
    SPAWN_STORAGE::const_iterator I =
        std::find(m_zones->begin(), m_zones->end(), object);
    if (I == m_zones->end())
      return (false);
...
void CLevelSpawnConstructor::generate_artefact_spawn_positions() {
  ...
  for (; I != E; I++) {
    ...
    const CGameLevelCrossTable::CCell &cell =
        cross_table().vertex(zone->m_tNodeID);
    zone->m_tGraphID = cell.game_vertex_id();
    zone->m_fDistance = cell.distance();

    graph_engine().search(
        level_graph(), zone->m_tNodeID, zone->m_tNodeID, &l_tpaStack,
        SFlooder<float, u32, u32>(zone->m_offline_interactive_radius, u32(-1),
                                  u32(-1)));

    l_tpaStack.erase(std::remove_if(l_tpaStack.begin(), l_tpaStack.end(),
                                    remove_too_far_predicate(
                                        &level_graph(), zone->o_Position,
                                        zone->m_offline_interactive_radius)),
                     l_tpaStack.end());
    /*
                    if (zone->m_artefact_spawn_count >= l_tpaStack.size())
                    {
                            zone->m_artefact_spawn_count	=
    (u16)l_tpaStack.size(); #ifndef IGNORE_ZERO_SPAWN_POSITIONS if
    (!zone->m_artefact_spawn_count) {
                                    Msg						("!
    CANNOT GENERATE ARTEFACT SPAWN POSITIONS FOR ZONE [%s] ON LEVEL
    [%s]",zone->name_replace(),*level().name()); Msg
    ("! ZONE [%s] ON LEVEL [%s] IS REMOVED BY AI
    COMPILER",zone->name_replace(),*level().name()); R_ASSERT3
    (zone->m_story_id == INVALID_STORY_ID,"Cannot remove story
    object",zone->name_replace()); R_ASSERT3
    (!zone->m_spawn_control,"Cannot remove spawn control
    object",zone->name_replace()); zones.push_back			(zone);
                                    l_tpaStack.clear		();
                                    continue;
                            }
    #endif
    ...
    m_level_points.resize(zone->m_artefact_position_offset +
                          zone->m_artefact_spawn_count);

    //		Msg								("%s  %f [%f][%f][%f] : artefact
    //spawn
    // positions",zone->name_replace(),zone->m_fRadius,VPUSH(zone->o_Position));

    LEVEL_POINT_STORAGE::iterator I =
        m_level_points.begin() + zone->m_artefact_position_offset;
...
void CLevelSpawnConstructor::fill_level_changers() {
  for (u32 i = 0, n = (u32)level_changers().size(); i < n; ++i) {
    ...
    for (; I != E; ++I)
      if (!xr_strcmp(*level_changers()[i]->m_caLevelPointToChange,
                     (*I)->name_replace())) {
        bool ok = false;
        for (u32 ii = 0, nn = game_graph().header().vertex_count(); ii < nn;
             ++ii) {
          if ((game_graph().vertex(ii)->level_id() != m_level.id()) ||
              !game_graph().vertex(ii)->level_point().similar((*I)->o_Position,
                                                              .001f))
            continue;
        ...
        R_ASSERT3(ok,
                  "Cannot find a correspndance between graph and graph points "
                  "from level editor! Rebuild graph for the level ",
...
void CLevelSpawnConstructor::update_artefact_spawn_positions() {
  ...
  for (; I != E; ++I) {
    ...
    CSE_ALifeObject *alife_object = smart_cast<CSE_ALifeObject *>(abstract);
    //		R_ASSERT3
    //(level_graph().valid_vertex_id(alife_object->m_tNodeID),"Invalid node for
    // object ",alife_object->name_replace());
    R_ASSERT2(alife_object, "Non-ALife object!");
    VERIFY(game_graph().vertex(alife_object->m_tGraphID)->level_id() ==
           m_level.id());
    //		alife_object->m_spawn_control	= "";
    CSE_ALifeAnomalousZone *zone =
        smart_cast<CSE_ALifeAnomalousZone *>(abstract);
    if (zone) {
      zone->m_artefact_position_offset = level_point_count;
      level_point_count += zone->m_artefact_spawn_count;
      //			Msg
      //("%s  %f [%f][%f][%f] : artefact spawn
      // positions",zone->name_replace(),zone->m_fRadius,VPUSH(zone->o_Position));
      //			for (u32 i=zone->m_artefact_position_offset;
      // i<level_point_count; ++i)
      //				Msg
      //("    [%f][%f][%f] :
...
void CLevelSpawnConstructor::verify_space_restrictors() {
  Msg("Level [%s] : searching for AI map separators space restrictors",
      *m_level.name());
  ...
  for (; I != E; ++I) {
    ...
    if ((*I)->object().m_space_restrictor_type ==
        RestrictionSpace::eRestrictorTypeNone)
      continue;
