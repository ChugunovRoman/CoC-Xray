...
IC const Fvector vertex_position(const CLevelGraph::CPosition &Psrc,
                                 const Fbox &bb, const SAIParams &params) {
  Fvector Pdest;
  int x, z, row_length;
...
struct CNodePositionConverter {
  IC CNodePositionConverter(const SNodePositionOld &Psrc, hdrNODES &m_header,
                            NodePosition &np);
};

IC CNodePositionConverter::CNodePositionConverter(const SNodePositionOld &Psrc,
                                                  hdrNODES &m_header,
                                                  NodePosition &np) {
  Fvector Pdest;
      ...
      Status("Processing textures...");
      {
        ...
        for (u32 t = 0; t < tex_count; t++) {
          ...
          if (0 == xr_strcmp(N, "level_lods")) {
            ...
            // if( version!=THM_CURRENT_VERSION )	FATAL	("Unsupported version
            // of THM file.");
            ...
            // load surface if it has an alpha channel or has "implicit
            // lighting" flag
