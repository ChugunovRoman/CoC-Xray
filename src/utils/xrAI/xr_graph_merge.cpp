...
DEFINE_MAP_PRED(LPSTR, SConnectionVertex, VERTEX_MAP, VERTEX_PAIR_IT,
                CCompareVertexPredicate);
...
public:
  ...
  CLevelGameGraph(LPCSTR graph_file_name, LPCSTR raw_cross_table_file_name,
                  CGameGraph::SLevel *tLevel, LPCSTR S, u32 dwOffset,
                  u32 dwLevelID, CInifile *Ini) {
    m_tLevel = *tLevel;
    m_dwOffset = dwOffset;
    ...
    CGameLevelCrossTable *l_tpCrossTable =
        xr_new<CGameLevelCrossTable>(caFileName);

    CLevelGraph *l_tpAI_Map = xr_new<CLevelGraph>(S);

    VERIFY2(l_tpCrossTable->header().level_guid() ==
    ...
    VERIFY(m_tpGraph->header().vertex_count() ==
           l_tpCrossTable->header().game_vertex_count());

    tLevel->m_guid = l_tpAI_Map->header().guid();

    {
      for (GameGraph::_GRAPH_ID i = 0, n = m_tpGraph->header().vertex_count();
           i < n; ++i)
        if ((!l_tpAI_Map->valid_vertex_id(
                 m_tpGraph->vertex(i)->level_vertex_id()) ||
    ...
    for (; I != E; I++) {
      ...
      (*I).tGlobalPoint.add(m_tpGraph->vertex(int(I - B))->game_point(),
                            m_tLevel.offset());
      ...
      (*I).tpaEdges = (CGameGraph::CEdge *)xr_malloc((*I).tNeighbourCount *
                                                     sizeof(CGameGraph::CEdge));
      b = i;
      for (; i != e; ++i) {
      ...
      (*I).dwPointOffset = 0;
      vfGenerateDeathPoints(int(I - B), l_tpCrossTable, l_tpAI_Map,
                            (*I).tDeathPointCount);
    }
    ...
    // updating cross-table
    {
      ...
      for (int i = 0; i < (int)tpCrossTable->header().level_vertex_count();
           i++) {
      ...
      tCrossTableHeader.dwNodeCount =
          tpCrossTable->m_tCrossTableHeader.dwNodeCount;
      tCrossTableHeader.dwGraphPointCount =
          tpCrossTable->m_tCrossTableHeader.dwGraphPointCount;
      tCrossTableHeader.m_level_guid =
          tpCrossTable->m_tCrossTableHeader.m_level_guid;
    ...
    // fill vertex map
    {
      ...
      for (int i = 0; O; O = F->open_chunk_iterator(id, O)) {
        ...
        //				E->Spawn_Read
        //(P);
        CSE_ALifeGraphPoint *tpGraphPoint =
            smart_cast<CSE_ALifeGraphPoint *>(E);
        if (tpGraphPoint) {
          E->Spawn_Read(P);
          ...
          if (fMinDistance < EPS_L) {
            ...
            T.dwLevelID = dwfGetIDByLevelName(
                Ini, *tpGraphPoint->m_caConnectionLevelName);
            //						T.tGraphID
            //= (GameGraph::_GRAPH_ID)i; 						T.tOldGraphID
            //= tGraphID;
            ...
            for (; II != EE; ++II)
              if (T.tOldGraphID == (*II).second.tOldGraphID) {
                ok = false;
                Msg("Graph point %s is removed,because it has the same "
                    "position as some another graph point",
                    E->name_replace());
                break;
              }
      ...
      if (i != m_tpGraph->header().vertex_count())
        Msg("Graph for the level %s doesn't correspond to the graph points "
            "from Level Editor! (%d : %d)",
      ...
      //			VERIFY3
      //(i == m_tpGraph->header().vertex_count(), "Rebuild graph for the level
      //",m_tLevel.name());
      O->close();
      FS.r_close(F);
    }
  ...
  void vfAddEdge(u32 dwVertexNumber, CGameGraph::CEdge &tGraphEdge) {
    ...
    m_tpVertices[dwVertexNumber].tpaEdges = (CGameGraph::CEdge *)xr_realloc(
        ...
        sizeof(CGameGraph::CEdge) *
            ++m_tpVertices[dwVertexNumber].tNeighbourCount);
    m_tpVertices[dwVertexNumber]
        .tpaEdges[m_tpVertices[dwVertexNumber].tNeighbourCount - 1] =
        tGraphEdge;
  }

  void vfSaveVertices(CMemoryWriter &tMemoryStream, u32 &dwOffset,
                      u32 &dwPointOffset, LEVEL_POINT_STORAGE *tpLevelPoints) {
    GRAPH_VERTEX_IT I = m_tpVertices.begin();
    GRAPH_VERTEX_IT E = m_tpVertices.end();
    ...
    for (; I != E; I++) {
      ...
      Memory.mem_copy(tVertex.tVertexTypes, (*I).tVertexTypes,
                      GameGraph::LOCATION_TYPE_COUNT *
                          sizeof(GameGraph::_LOCATION_ID));
  ...
  void vfGenerateDeathPoints(int iGraphIndex,
                             CGameLevelCrossTable *tpCrossTable,
                             CLevelGraph *tpAI_Map, u32 &dwDeathPointCount) {
    xr_vector<u32> l_dwaNodes;
    l_dwaNodes.clear();
    {
      for (u32 i = 0, n = tpCrossTable->m_tCrossTableHeader.dwNodeCount; i < n;
           i++)
    ...
    u32 m = l_dwaNodes.size() > 10 ? _min(iFloor(.1f * l_dwaNodes.size()), 255)
                                   : l_dwaNodes.size(),
...
void read_levels(CInifile *Ini, xr_set<CLevelInfo> &levels, bool rebuild_graph,
  ...
  for (u32 k = 0; Ini->r_line("levels", k, &_N, &V); k++) {
...

    {
      ...
      for (; I != E; ++I) {
        if (!xr_strcmp((*I).m_section, N)) {
          Msg("! Duplicated line %s in section \"levels\" in the %s", N,
              GAME_CONFIG);
          ok = false;
          break;
        }
        if (!xr_strcmp((*I).m_name, S)) {
          Msg("! Duplicated level name %s in the %s, sections %s, %s", S,
              GAME_CONFIG, *(*I).m_section, N);
          ok = false;
          break;
        }
        if ((*I).m_id == id) {
          Msg("! Duplicated level id %d in the %s, section %s, level %s", id,
              GAME_CONFIG, N, S);
          ok = false;
          break;
        }
    ...
    if (!FS.exist(file_name)) {
      Msg("! There is no ai-map for the level %s! (level is not included into "
          "the game graph)",
          S);
      continue;
    }

    {
      ...
      if (header.version() != XRAI_CURRENT_VERSION) {
        Msg("! AI-map for the level %s is incompatible (version mismatch)! "
            "(level is not included into the game graph)",
            S);
        continue;
      }
    }
...
LPCSTR generate_temp_file_name(LPCSTR header0, LPCSTR header1,
                               string_path &buffer) {
  string_path path;
...
CGraphMerger::CGraphMerger(LPCSTR game_graph_id, LPCSTR name, bool rebuild) {
  ...
  for (; I != E; ++I) {
    ...
    ::CLevelGameGraph *tpLevelGraph = xr_new<::CLevelGameGraph>(
        _0, _1, &tLevel, level_folder, dwOffset, tLevel.id(), Ini);
    dwOffset += tpLevelGraph->m_tpGraph->header().vertex_count();
    R_ASSERT2(tpGraphs.find(tLevel.id()) == tpGraphs.end(),
              "Level ids _MUST_ be different!");
    ...
    for (; I != E; I++) {
      ...
      for (; i != e; i++)
        if ((*i).second.caConnectName[0]) {
          ...
          if (K == tpGraphs.end()) {
            Msg("Cannot find level with level_id %d. Connection point will not "
                "be generated!",
          ...
          if (M == (*K).second->m_tVertexMap.end()) {
            Msg("Level %s with id %d has an INVALID connection point "
                "%s,\nwhich references to graph point %s on the level %s with "
                "id %d\n",
                *(*I).second->m_tLevel.name(), (*I).second->m_tLevel.id(),
                (*i).first, tConnectionVertex.caConnectName,
                *(*K).second->m_tLevel.name(), (*K).second->m_tLevel.id());
            R_ASSERT(M != (*K).second->m_tVertexMap.end());
          }

          //					if
          //(!stricmp("l06_rostok",*(*I).second->m_tLevel.name())) {
          //						__asm int 3;
          //					}
          ...
          VERIFY(((*M).second.tGraphID + (*K).second->m_dwOffset) <
                 (u32(1) << (8 * sizeof(GameGraph::_GRAPH_ID))));
          tGraphEdge.m_vertex_id = (GameGraph::_GRAPH_ID)(
          ...
          tGraphEdge.m_path_distance =
              (*I).second->m_tpVertices[tConnectionVertex.tGraphID]
                  .tGlobalPoint.distance_to(
                      (*K).second->m_tpVertices[(*M).second.tGraphID]
                          .tGlobalPoint);
          (*I).second->vfAddEdge((*i).second.tGraphID, tGraphEdge);
          //					tGraphEdge.dwVertexNumber
          //=
          //(*i).second.tGraphID + (*I).second->m_dwOffset;
          //					(*K).second->vfAddEdge
  ...
  // counting edges
  {
    ...
    for (; I != E; I++) {
      VERIFY((u32(tGraphHeader.m_edge_count) + (*I).second->dwfGetEdgeCount()) <
             (u32(1) << (8 * sizeof(GameGraph::_GRAPH_ID))));
      tGraphHeader.m_edge_count +=
          (GameGraph::_GRAPH_ID)(*I).second->dwfGetEdgeCount();
  ...
  }
  {
    ...
    GRAPH_P_PAIR_IT E = tpGraphs.end();
    for (; I != E; I++)
      l_tpLevelPoints.insert(l_tpLevelPoints.end(),
                             (*I).second->m_tpLevelPoints.begin(),
                             (*I).second->m_tpLevelPoints.end());
  }
  R_ASSERT2(l_dwStartPointOffset == F.size() - l_dwOffset,
            "Graph file format is corrupted");
  {
