...
IC void CGameSpawnConstructor::add_level_points(
    const LEVEL_POINT_STORAGE &level_points) {
  m_level_points.insert(m_level_points.end(), level_points.begin(),
                        level_points.end());
}

IC void
CGameSpawnConstructor::add_level_changer(CSE_ALifeLevelChanger *level_changer) {
  m_critical_section.Enter();
  m_level_changers.push_back(level_changer);
  m_critical_section.Leave();
}

IC void CGameSpawnConstructor::add_edge(ALife::_SPAWN_ID id0,
                                        ALife::_SPAWN_ID id1, float weight) {
  m_critical_section.Enter();
...
IC CGameSpawnConstructor::LEVEL_CHANGER_STORAGE &
CGameSpawnConstructor::level_changers() {
...
IC void
CGameSpawnConstructor::process_spawns(xr_vector<ALife::_SPAWN_ID> &spawns) {
  std::sort(spawns.begin(), spawns.end());
  xr_vector<ALife::_SPAWN_ID>::iterator I =
      std::unique(spawns.begin(), spawns.end());
