...
void CGameGraphBuilder::load_level_graph(const float &start,
                                         const float &amount) {
  Progress(start);
...
void CGameGraphBuilder::load_graph_point(NET_Packet &net_packet) {
  ...
  // check for duplicate graph point positions
  {
    ...
    for (; I != E; ++I) {
      if ((*I).second->data().tLocalPoint.distance_to_sqr(vertex.tLocalPoint) <
          EPS_L) {
        Msg("! removing graph point [%s][%f][%f][%f] because it is too close "
            "to the another graph point",
  ...
  if (!level_graph().valid_vertex_id(vertex.tNodeID)) {
    Msg("! removing graph point [%s][%f][%f][%f] because it is outside of the "
        "AI map",
        entity->name_replace(), VPUSH(entity->o_Position));
    F_entity_Destroy(entity);
    return;
...

  {
    ...
    for (; I != E; ++I) {
      if ((*I).second->data().tNodeID == vertex.tNodeID) {
        Msg("! removing graph point [%s][%f][%f][%f] because it has the same "
            "AI node as another graph point",
            entity->name_replace(), VPUSH(entity->o_Position));
        F_entity_Destroy(entity);
        return;
      }
...
void CGameGraphBuilder::load_graph_points(const float &start,
                                          const float &amount) {
  Progress(start);
  ...
  strconcat(sizeof(spawn_file_name), spawn_file_name, *m_level_name,
            "level.spawn");
...
void CGameGraphBuilder::fill_distances(const float &start,
                                       const float &amount) {
  Progress(start);
...
void CGameGraphBuilder::recursive_update(const u32 &game_vertex_id,
  ...
  u32 level_vertex_id =
      graph().vertex(game_vertex_id)->data().level_vertex_id();
  ...
  float amount_i = amount / (float(vertex_count) *
...
void CGameGraphBuilder::iterate_distances(const float &start,
                                          const float &amount) {
  Progress(start);
  ...
  for (int i = 0, n = (int)graph().vertices().size(); i < n; ++i) {
    if (i) {
      for (int k = 0, kn = (int)level_graph().header().vertex_count(); k < kn;
           ++k)
...
void CGameGraphBuilder::save_cross_table(const float &start,
                                         const float &amount) {
  Progress(start);
  ...
  for (int i = 0, n = level_graph().header().vertex_count(); i < n; i++) {
    ...
    tCrossTableCell.fDistance =
        float(m_distances[tCrossTableCell.tGraphIndex][i]) *
        level_graph().header().cell_size();
  ...
  //	Msg						("CT:SAVE :
  //%f",timer.GetElapsed_sec()); 	Msg
  //("Flushing cross table");

  tMemoryStream.save_to(m_cross_table_name);
  //	Msg						("CT:SAVE :
  //%f",timer.GetElapsed_sec());

  //	Msg						("Freiing cross table
  // resources");
...
void CGameGraphBuilder::build_cross_table(const float &start,
                                          const float &amount) {
  Progress(start);
...
void CGameGraphBuilder::load_cross_table(const float &start,
                                         const float &amount) {
  Progress(start);
...
float CGameGraphBuilder::path_distance(const u32 &game_vertex_id0,
                                       const u32 &game_vertex_id1) {
  //	return
  ...
  CStraightLineParams parameters(vertex0.data().level_point(),
                                 vertex1.data().level_point());

  float pure_distance =
      vertex0.data().level_point().distance_to_xz(vertex1.data().level_point());
  //	float					pure_distance =
  ...
  bool successfull = m_graph_engine->search(
      level_graph(), vertex0.data().level_vertex_id(),
  ...
  Msg("Cannot build path from [%f][%f][%f] to [%f][%f][%f]",
      VPUSH(vertex0.data().level_point()), VPUSH(vertex1.data().level_point()));
...
void CGameGraphBuilder::generate_edges(const float &start,
                                       const float &amount) {
  Progress(start);
...
void CGameGraphBuilder::connectivity_check(const float &start,
                                           const float &amount) {
  Progress(start);
...
void CGameGraphBuilder::create_tripples(const float &start,
                                        const float &amount) {
  graph_type::const_vertex_iterator I = graph().vertices().begin();
  graph_type::const_vertex_iterator E = graph().vertices().end();
  for (; I != E; ++I) {
    ...
    for (; i != e; ++i) {
      if (((*i).vertex_id() < (*I).first) &&
          graph().edge((*i).vertex_id(), (*I).first))
        continue;

      const graph_type::CEdge *edge =
          graph().vertex((*i).vertex_id())->edge((*I).first);
  ...
  std::sort(m_tripples.begin(), m_tripples.end(),
            sort_predicate_greater<TRIPPLE>);
}
...
void CGameGraphBuilder::optimize_graph(const float &start,
                                       const float &amount) {
  Progress(start);
...
void CGameGraphBuilder::build_graph(LPCSTR graph_name, LPCSTR cross_table_name,
                                    LPCSTR level_name) {
