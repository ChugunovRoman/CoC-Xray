...
IC bool RayPick(CDB::COLLIDER* DB, Fvector& P, Fvector& D, float r, RayCache& C)
{
        ...
        try { DB->ray_query	(&Level,P,D,r); } catch (...) { Msg("* ERROR:
Failed to trace ray"); } if (0==DB->r_count()) { return false; } else {
                // cache polygon
...
// IC bool RayPick(CDB::COLLIDER* DB, Fvector& P, Fvector& D, float r, RayCache&
// C)	//, Face* skip)
...
public:
  ...
  void compute_cover_value(u32 const &N, vertex &BaseNode,
                           float const &cover_height, Cover &cover) {
...
bool cover(const vertex &v, u32 index0, u32 index1) {
  return (valid_vertex_id(v.n[index0]) &&
...
void compute_cover_nodes() {
  ...
  VERIFY(!g_covers);
  g_covers = xr_new<CPointQuadTree>(aabb, g_params.fPatchSize * .5f, 8 * 65536,
                                    4 * 65536);
...
bool vertex_in_direction(const u32 &start_vertex_id,
                         const u32 &target_vertex_id) {
...
void compute_non_covers() {
...

  {
    ...
    CalculateHeight(aabb);
    VERIFY(!g_covers);
    g_covers = xr_new<CPointQuadTree>(aabb, g_params.fPatchSize * .5f,
                                      8 * 65536, 4 * 65536);
