...
//	DWORDs		contains;	// while merging - contains list of
// elementar nodes 	Fplane		plane;
//	Fvector		P;			// min
//	WORD		sector;
//	BYTE		light;
//	float		cover[4];
...
IC CNodePositionCompressor::CNodePositionCompressor(NodePosition &Pdest,
                                                    Fvector &Psrc,
                                                    hdrNODES &H) {
  float sp = 1 / g_params.fPatchSize;
  int row_length =
      iFloor((H.aabb.max.z - H.aabb.min.z) / H.size + EPS_L + 1.5f);
