  ...
  for (int z = 0; z < dimZ; z++) {
    for (int x = 0; x < dimX; x++) {
      ...
      if (T.N) {
        ...
        C = RGB(iFloor(_abs(N.x) * 255), iFloor(_abs(N.y) * 255),
                iFloor(_abs(N.z) * 255));
      }
      pixel(dc, x, z, C);
    }
...
static BOOL CALLBACK disp_proc(HWND hWnd, UINT message, WPARAM wParam,
                               LPARAM lParam) {
  switch (message) {
  case WM_INITDIALOG:
    break;
...
void xrDisplay() {
  InternalRender();
  DialogBox(HINSTANCE(GetModuleHandle(0)), MAKEINTRESOURCE(IDD_NVIEW),
            logWindow, disp_proc);
  xr_free(texels);
}
