...
IC Fvector construct_position(CLevelGraph &level_graph, u32 level_vertex_id,
                              float x, float z) {
  return (
      Fvector().set(x, level_graph.vertex_plane_y(level_vertex_id, x, z), z));
}

CSpaceRestrictorWrapper::CSpaceRestrictorWrapper(
    CSE_ALifeSpaceRestrictor *object) {
  m_object = object;
  m_level_graph = 0;
  m_graph_engine = 0;
...
bool CSpaceRestrictorWrapper::inside(const Fvector &position,
                                     float radius) const {
  Fsphere sphere;
  sphere.P = position;
  sphere.R = radius;
...
struct border_merge_predicate {
  ...
  IC border_merge_predicate(CSpaceRestrictorWrapper *restriction,
...
void CSpaceRestrictorWrapper::fill_shape(const CShapeData::shape_def &shape) {
  ...
  level_graph().iterate_vertices(start, dest,
...
bool CSpaceRestrictorWrapper::inside(u32 level_vertex_id, bool partially_inside,
  ...
  if (partially_inside)
    return (inside(construct_position(level_graph(), level_vertex_id,
                                      position.x + offset, position.z + offset),
                   radius) ||
            inside(construct_position(level_graph(), level_vertex_id,
                                      position.x + offset, position.z - offset),
                   radius) ||
            inside(construct_position(level_graph(), level_vertex_id,
                                      position.x - offset, position.z + offset),
...
void CSpaceRestrictorWrapper::build_border() {
...

  {
    ...
    m_border.erase(I, m_border.end());
    std::sort(m_border.begin(), m_border.end(),
              sort_by_xz_predicate(m_level_graph));
  }

  VERIFY3(!m_border.empty(), "space restrictor has no border",
          object().name_replace());
}
...
void CSpaceRestrictorWrapper::verify_connectivity() {
  ...
  graph_engine().search(level_graph(), start_vertex_id, start_vertex_id, &nodes,
                        GraphEngineSpace::CFlooder(
                            ...
                            GraphEngineSpace::_iteration_type(-1), u32(-1)));

  level_graph().clear_mask(m_border);

  VERIFY(nodes.size() + m_internal.size() <=
         level_graph().header().vertex_count());
  if (nodes.size() + m_internal.size() == level_graph().header().vertex_count())
    return;

  Msg("! %d nodes are disconnected!", level_graph().header().vertex_count() -
                                          (nodes.size() + m_internal.size()));

  R_ASSERT3(nodes.size() + m_internal.size() ==
                level_graph().header().vertex_count(),
            "Restrictor separates AI map into several disconnected components",
            object().name_replace());
}

void CSpaceRestrictorWrapper::verify(CLevelGraph &level_graph,
