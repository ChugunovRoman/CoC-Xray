...
#define TEMPLATE_SPECIALIZATION                                                \
  template <                                                                   \
      ...
      typename _vertex_allocator, bool euclidian_heuristics,                   \
      typename _data_storage_base, template <typename _T> class _vertex,       \
  ...
  for (; i != e; ++i) {
    ...
    if (data_storage().is_visited(neighbour_index)) {
      ...
      if (data_storage().is_opened(neighbour)) {
        ...
        _dist_type f =
            best.f() + path_manager.evaluate(best.index(), neighbour_index, i);
