...
#define CLevelEvaluatorPathManager                                             \
  CPathManager<CLevelGraph, _DataStorage, CAbstractVertexEvaluator,            \
...
TEMPLATE_SPECIALIZATION
IC void CLevelEvaluatorPathManager::setup(const _Graph *_graph,
...
TEMPLATE_SPECIALIZATION
IC _dist_type
CLevelEvaluatorPathManager::estimate(const _index_type &node_index) const {
  return (_dist_type(0));
}

TEMPLATE_SPECIALIZATION
IC bool
CLevelEvaluatorPathManager::is_goal_reached(const _index_type &node_index) {
  VERIFY(m_evaluator);
...
IC void CLevelEvaluatorPathManager::finalize() {
  if (path && graph->valid_vertex_id(m_evaluator->m_dwBestNode))
    data_storage->get_node_path(
        *path, &data_storage->get_node(m_evaluator->m_dwBestNode));
}
