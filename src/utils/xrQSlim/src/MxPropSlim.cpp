...
MxPropSlim::MxPropSlim(MxStdModel *m0)
    : MxStdSlim(m0), __quadrics(m0->vert_count()),
...
void MxPropSlim::pack_prop_to_vector(MxVertexID id, MxVector &v,
                                     unsigned int target) {
  if (target == 0) {
    v[0] = m->vertex(id)[0];
    v[1] = m->vertex(id)[1];
...
void MxPropSlim::unpack_prop_from_vector(MxVertexID id, MxVector &v,
                                         unsigned int target) {
  if (target == 0) {
    m->vertex(id)[0] = (float)v[0];
    m->vertex(id)[1] = (float)v[1];
...
unsigned int MxPropSlim::check_local_validity(unsigned int v,
                                              const float *vnew) {
