...
MxFaceID MxBlockModel::add_face(unsigned int v1, unsigned int v2,
                                unsigned int v3, bool will_link) {
...
void MxBlockModel::compute_face_normal(MxFaceID f, float *n,
                                       bool will_unitize) {
...
void MxBlockModel::compute_face_normal(MxFaceID f, double *n,
                                       bool will_unitize) {
  float *v1 = vertex(face(f)[0]);
  float *v2 = vertex(face(f)[1]);
  float *v3 = vertex(face(f)[2]);
...
double MxBlockModel::compute_corner_angle(MxFaceID f, unsigned int i) {
  ...
  mxv_unitize(mxv_sub(e_prev, corner(f, (short)i_prev), corner(f, (short)i), 3),
              3);
  mxv_unitize(mxv_sub(e_next, corner(f, (short)i_next), corner(f, (short)i), 3),
              3);
