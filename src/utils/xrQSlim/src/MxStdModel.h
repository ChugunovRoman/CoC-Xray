...
public:
  MxStdModel(unsigned int nvert, unsigned int nface)
      : MxBlockModel(nvert, nface), v_data(nvert), f_data(nface),
        face_links(nvert) {}
  virtual ~MxStdModel();
  MxStdModel *clone();
  ...
  MxVertexID split_edge(MxVertexID v1, MxVertexID v2, float x, float y,
                        float z);
  ...
  void compute_contraction(MxVertexID, MxVertexID, MxPairContraction *,
                           const float *vnew = NULL);
  void apply_contraction(const MxPairContraction &);
  void apply_expansion(const MxPairExpansion &);
  void contract(MxVertexID v1, MxVertexID v2, const float *,
                MxPairContraction *);
  ...
  void contract(MxVertexID v1, const MxVertexList &rest, const float *vnew,
                MxFaceList &changed);
};
