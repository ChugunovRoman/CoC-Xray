...
unsigned int MxStdModel::split_edge(unsigned int a, unsigned int b) {
  ...
  return split_edge(a, b, (v1[0] + v2[0]) / 2.0f, (v1[1] + v2[1]) / 2.0f,
                    (v1[2] + v2[2]) / 2.0f);
}
...
unsigned int MxStdModel::split_edge(unsigned int v1, unsigned int v2, float x,
                                    float y, float z) {
  ...
  for (oldID = 0; oldID < vert_count(); oldID++) {
    if (vertex_is_valid(oldID)) {
      if (newID != oldID) {
        ...
        for (unsigned int i = 0; i < (unsigned int)neighbors(newID).length();
             i++)
