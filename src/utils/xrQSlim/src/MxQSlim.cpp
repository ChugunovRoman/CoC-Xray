...
MxEdgeQSlim::MxEdgeQSlim(MxStdModel *_m)
    : MxQSlim(_m), edge_links(_m->vert_count()) {
...
double MxEdgeQSlim::check_local_compactness(unsigned int v1,
                                            unsigned int /*v2*/,
                                            const float *vnew) {
...
double MxEdgeQSlim::check_local_inversion(unsigned int v1, unsigned int /*v2*/,
                                          const float *vnew) {
  double Nmin = 1.0;
  const MxFaceList &N1 = m->neighbors(v1);
...
unsigned int MxEdgeQSlim::check_local_validity(unsigned int v1,
                                               unsigned int /*v2*/,
                                               const float *vnew)
  ...
  for (i = 0; i < (unsigned int)N1.length(); i++) {
    if (m->face_mark(N1[i]) == 1) {
      ...
      if (mxv_dot(d_vnew, n, 3) <
          local_validity_threshold * mxv_dot(d_vx, n, 3))
        nfailed++;
    }
  }
...
unsigned int MxEdgeQSlim::check_local_degree(unsigned int v1, unsigned int v2,
                                             const float *) {
  const MxFaceList &N1 = m->neighbors(v1);
...
void MxEdgeQSlim::apply_mesh_penalties(MxQSlimEdge *info) {
  ...
  //
  unsigned int degree_excess =
      check_local_degree(info->v1, info->v2, info->vnew);
  if (degree_excess)
...
void MxEdgeQSlim::apply_contraction(const MxPairContraction &conx) {
  ...
  edges.clear();
  for (unsigned int j = 0; j < (unsigned int)star.length(); j++)
    for (unsigned int i = 0; i < (unsigned int)edge_links(star(j)).length();
         i++)
  ...
  //	u32 rr=edges.size();
  //	Msg	("%d: %d/%d - %d",(unsigned
  // int)edge_links(conx.v1).length(),r,rr,r-rr);
