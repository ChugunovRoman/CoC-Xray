...
static double internal_invert(double *_a, double *_b, const int N) {
  ...
  for (i = 0; i < (unsigned int)N;
       i++) { /* eliminate in column i, below diag */
    max = -1.;
    ...
    pivot = A(i, i);
    det *= pivot;
    for (k = i + 1; k < (unsigned int)N;
         k++) /* only do elems to right of pivot */
      A(i, k) /= pivot;
    for (k = 0; k < (unsigned int)N; k++)
      B(i, k) /= pivot;
    /* we know that A(i, i) will be set to 1, so don't bother to do it */
    ...
    for (j = i + 1; j < (unsigned int)N; j++) { /* eliminate in rows below i */
      t = A(j, i);                              /* we're gonna zero this guy */
      for (k = i + 1; k < (unsigned int)N;
           k++)                 /* subtract scaled row i from row j */
        A(j, k) -= A(i, k) * t; /* (ignore k<=i, we know they're 0) */
      for (k = 0; k < (unsigned int)N; k++)
        B(j, k) -= B(i, k) * t;
    }
