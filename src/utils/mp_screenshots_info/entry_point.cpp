...
void debug_sign_screenshot(IReader* screenshot)
{
        ...
        if (tmp_jpeg_size < 0)
                return;
        u32	tmp_buff_size			= tmp_jpeg_size +
writer::info_max_size; u8*	tmp_buff				=
static_cast<u8*>(xr_malloc(tmp_buff_size));

        screenshot->r				(tmp_buff, tmp_jpeg_size);

        writer	tmp_writer			(tmp_buff, tmp_jpeg_size,
tmp_buff_size);

        tmp_writer.set_player_name			("some_cheater");
        tmp_writer.set_player_cdkey_digest	("238940293847298374982734");
        u32 final_ss_size					=
tmp_writer.write_info();

        IWriter* result_writer		= FS.w_open("$screenshots$",
"result.jpg"); VERIFY						(result_writer);
...
int main(int argc, char **argv) {
  ...
  Core._initialize("mp_screenshots_info", xrcore_log_cb, TRUE,
                   "fsgame4mpu.ltx");
