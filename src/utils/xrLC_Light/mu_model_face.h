...
public:
  ...
  // bool				VContains			( _vertex* pV);					// Does
  // the face contains this vertex?
  // void				VReplace			( _vertex* what, _vertex* to);
  // // Replace ONE vertex by ANOTHER
  // void				VReplace_NoRemove	( _vertex* what,
  // _vertex* to); int					VIndex ( _vertex* pV);
  // void				VSet				( int idx,
  // _vertex* V); void				VSet ( _vertex *V1, _vertex *V2,
  // _vertex *V3); BOOL				isDegenerated		( );
  // BOOL				isEqual				( _face&
  // F
  // ); float				EdgeLen				( int
  // edge);
