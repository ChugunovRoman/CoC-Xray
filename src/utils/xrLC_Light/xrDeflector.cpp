...
void blit(u32 *dest, u32 ds_x, u32 ds_y, u32 *src, u32 ss_x, u32 ss_y, u32 px,
          u32 py, u32 aREF) {
...
void blit(lm_layer &dst, u32 ds_x, u32 ds_y, lm_layer &src, u32 ss_x, u32 ss_y,
...
void blit_r(u32 *dest, u32 ds_x, u32 ds_y, u32 *src, u32 ss_x, u32 ss_y, u32 px,
            u32 py, u32 aREF) {
...
void blit_r(lm_layer &dst, u32 ds_x, u32 ds_y, lm_layer &src, u32 ss_x,
...
void CDeflector::OA_Export() {
  ...
  u32 dwWidth = iCeil(
      size.x * inlc_global_data()->g_params().m_lm_pixels_per_meter * density +
      .5f);
  clamp(dwWidth, 1u, 512u - 2 * BORDER);
  u32 dwHeight = iCeil(
      size.y * inlc_global_data()->g_params().m_lm_pixels_per_meter * density +
      .5f);
...
BOOL CDeflector::OA_Place(Face *owner) {
  // It is not correct to rely solely on normal-split-angle for lmaps - imagine
  // smooth sphere
...
void CDeflector::RemapUV(xr_vector<UVtri> &dest, u32 base_u, u32 base_v,
                         u32 size_u, u32 size_v, u32 lm_u, u32 lm_v,
                         BOOL bRotate) {
  dest.clear();
...
void CDeflector::RemapUV(u32 base_u, u32 base_v, u32 size_u, u32 size_v,
...
void CDeflector::L_Calculate(CDB::COLLIDER *DB, base_lighting *LightsSelected,
                             HASH &H) {
  try {
    lm_layer &lm = layer;
