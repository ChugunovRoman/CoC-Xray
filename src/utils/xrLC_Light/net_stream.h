...
public:
  ...
  // IC	void			reset_file_size	( u32 new_size ){file_size =
  // new_size
  // ;}
  // IC	u32				get_file_size	( )				{return
  // file_size;
  // }
...
// public:
//	CMemoryReadBlock	( const u32	file_size_, u8*	&buff
//):mem_reader(file_size_, buff ){};
//	virtual	void			r				( void
//*p, int cnt )const	{ mem_reader.r(p,cnt); } 	virtual	u32 count
// ()const						{ return
// mem_reader.count(); } 	virtual	void			*pdata
//()const						{ return
//mem_reader.pdata();
//}
//
...
//	void					alloc			(u32
//_file_size)const		{ mem_reader.alloc(_file_size); } 	bool
// allocated		()const						{ return
// mem_reader.allocated(); }
//	bool					empty			()const
//{ return mem_reader.empty(); }
//};
...
public:
  CMemoryWriteBlock(u8 *_buffer, u32 _size)
      : buffer(_buffer), buffer_size(_size), position(0) {}

  //	 bool			save_to				(LPCSTR fn)
  //{ return mem_writer.save_to(fn); }
