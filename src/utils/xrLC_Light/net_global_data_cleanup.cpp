...
void __cdecl data_cleanup_callback(const char *dataDesc,
  ...
  *stream = CreateGenericStream();
  ...
  w_pod_vector(INetMemoryBuffWriter(*stream, sizeof(buff), buff),
               _cleanup.vec_cleanup);
  // w_pod_vector( INetIWriterGenStream( *stream, 512 ), _cleanup.vec_cleanup );
}

#pragma warning(disable : 4995)
void global_data_cleanup::on_net_receive(IAgent *agent, DWORD sessionId,
  ...
  u8 buff[data_cleanup_callback_read_write_buff_size];
  r_pod_vector(INetBlockReader(globalDataStream, buff, sizeof(buff)),
               vec_cleanup);
