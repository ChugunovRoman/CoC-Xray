...
template <typename T> void r_pod(INetReader &r, T &v) { r.r(&v, sizeof(T)); }

template <typename T> void w_pod(IWriter &w, const T &v) { w.w(&v, sizeof(T)); }
...
public:
  ...
  static u32 get_id(const type *f, const xr_vector<type *> &vec) {
    ...
    //	return id_none;
    // xr_vector<type*>::const_iterator F = std::find( vec.begin(), vec.end(), f
    // ); VERIFY( F != vec.end() ); return u32( F - vec.begin() );
  }
