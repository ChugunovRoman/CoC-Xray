...
void xrMU_Model::Load(IReader &F, u32 version) {
  ...
  for (u32 f_it = 0; f_it < b_faces.size(); f_it++) {
    ...
    _face *face = create_face(m_vertices[F.v[0]], m_vertices[F.v[1]],
                              m_vertices[F.v[2]], F);
  ...
  */
  //
  clMsg("* Loading model: '%s' - v(%d/%d), f(%d/%d)", *m_name,
        m_vertices.size(), b_vertices.size(), m_faces.size(), b_faces.size());
}

_face *xrMU_Model::create_face(_vertex *v0, _vertex *v1, _vertex *v2,
                               b_face &B) {
...
_face *xrMU_Model::load_create_face(Fvector &P1, Fvector &P2, Fvector &P3,
                                    b_face &B) {
  return create_face(load_create_vertex(P1), load_create_vertex(P2),
                     load_create_vertex(P3), B);
}
