...
const Shader_xrLC &base_Face::Shader() const {
  ...
  return shader(dwMaterial, inlc_global_data()->shaders(),
                inlc_global_data()->materials());
}
void base_Face::CacheOpacity() {
  ...
  if (!flags.bOpaque &&
      !(T.THM
            .HasSurface())) //(0==T.pSurface)//	pSurface was possible deleted
  {
    flags.bOpaque = true;
...
//
// const int	edge2idx	[3][2]	= { {0,1},		{1,2},
// {2,0}	}; const int	edge2idx3	[3][3]	= { {0,1,2},	{1,2,0},
// {2,0,1}	}; const int	idx2edge	[3][3]  = {
//	{-1,  0,  2},
//	{ 0, -1,  1},
//	{ 2,  1, -1}
//};
...
template <> void Face::Failure() {
  ...
  clMsg("* ERROR: Invalid face. (A=%f,e0=%f,e1=%f,e2=%f)", CalcArea(),
        v[0]->P.distance_to(v[1]->P), v[0]->P.distance_to(v[2]->P),
        v[1]->P.distance_to(v[2]->P));
  clMsg("*        v0[%f,%f,%f], v1[%f,%f,%f], v2[%f,%f,%f]", VPUSH(v[0]->P),
        VPUSH(v[1]->P), VPUSH(v[2]->P));
...
void Face::OA_Unwarp(CDeflector *D) {
  ...
  for (int i = 0; i < 3; ++i)
    for (vecFaceIt it = v[i]->m_adjacents.begin();
         it != v[i]->m_adjacents.end(); ++it) {
      affected += 1;
      (*it)->OA_Unwarp(D);
    }
}
...
BOOL DataFace::RenderEqualTo(Face *F) {
  if (F->dwMaterial != dwMaterial)
    return FALSE;
  // if (F->tc.size()	!= F->tc.size()		)	return FALSE;	//
  // redundant???
  return TRUE;
}
...
/*
   ...
   // face normal
        ...
        void*					pDeflector;
   // does the face has LM-UV map? CLightmap* lmap_layer; u32
   sm_group;
*/
...
aa2_largest:	// aa2 is largest
                        ...
                        if (aa0 <= 0) {
                                // dDEBUGMSG ("vector has zero size"); ... this
messace is annoying a[0] = 0;	// if all a's are zero, this is where we'll end
up. a[1] = 1;	// return a default unit length vector. a[2] = 0; return
FALSE;
                        }
                        a1 /= aa0;
                        a2 /= aa0;
