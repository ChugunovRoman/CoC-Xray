...
public:
  ...
  template <typename TP> struct get_type { typedef TP type; };

  template <typename TP> struct get_type<TP *> { typedef TP type; };

  typedef typename get_type<T>::type type;

  void read(INetReader &r,
            vector_serialize<t_read<type, get_id_self_index<type>>> &rd) {
    r_pod(r, bounds);
  ...
  void
  write(IWriter &w,
        vector_serialize<t_write<type, get_id_self_index<type>>> &wt) const {
    w_pod(w, bounds);
