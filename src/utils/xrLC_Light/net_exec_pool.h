...
public:
  ...
  exec_pool(task_manager *tm)
      : _task_manager(*tm), _running(false), tasks_completed(0),
        _start(u32(-1)), _end(u32(-1)) {
    R_ASSERT(tm);
    xr_strcpy(_name, "net noname task");
  };
  exec_pool(u32 start, task_manager *tm)
      : _task_manager(*tm), _running(false), tasks_completed(0), _start(start),
        _end(start) {
    R_ASSERT(tm);
  };
  bool has(u32 id);
  ...
  void send_task(IGridUser &user, IGenericStream *outStream, u8 pool_id,
                 u32 id);
  ...
  net_execution *receive_task(IAgent *agent, DWORD sessionId,
