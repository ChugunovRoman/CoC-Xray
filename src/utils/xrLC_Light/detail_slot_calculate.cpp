...
// IC	u8	u8_clr				(float a)	{ s32 _a =
// iFloor(a*255.f); clamp(_a,0,255); return u8(_a);		}; IC	u8
// u8_clr				(float a)	{ s32 _a =
// iFloor(a*255.f); clamp(_a,0,255); return u8(_a);		};
...
IC bool RayPick(CDB::COLLIDER &DB, Fvector &P, Fvector &D, float r,
                R_Light &L) {
...
float getLastRP_Scale(CDB::COLLIDER *DB, R_Light &L) //, Face* skip)
{
  ...
  //	X_TRY
  {
    for (u32 I = 0; I < tris_count; I++) {
      ...
      //			Shader_xrLCVec&	LIB =
      // gl_data.g_shaders_xrlc->Library	(); 			if
      // (M.shader_xrlc>=LIB.size())
      // return		0;		//. hack - vy gonite rebyata - eto ne
      // hack - eto sledy zamesti - shader_xrlc - index ne togo masiva !!
      //			Shader_xrLC& SH	= LIB
      //[M.shader_xrlc];
...
float rayTrace(CDB::COLLIDER *DB, R_Light &L, Fvector &P, Fvector &D,
               float R) //, Face* skip)
{
  R_ASSERT(DB);
...
void LightPoint(CDB::COLLIDER *DB, base_color &C, Fvector &P, Fvector &N,
...
bool detail_slot_calculate(u32 _x, u32 _z, DetailSlot &DS, DWORDVec &box_result,
                           CDB::COLLIDER &DB, base_lighting &Selected) {
  ///////////////////////////////////////////////////////////
