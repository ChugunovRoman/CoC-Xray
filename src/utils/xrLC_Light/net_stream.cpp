...
/*
   ...
   w(p,(u32)xr_strlen(p));w_u8(13);w_u8(10);	} IC void
   w_stringZ(const char *p)		{	w(p,(u32)xr_strlen(p)+1);
   } IC void			w_stringZ(const shared_str& p) 	{
   w(*p?*p:"",p.size());w_u8(0);		} IC void
...
INetFileBuffWriter::INetFileBuffWriter(LPCSTR _file_name, u32 block_size,
                                       bool _reopen)
    : INetBuffWriter() {
...
void CFileWriteBlock::send(IGenericStream *_stream) {
  ...
  u32 block_size = size;
  //_stream->SetLength		(position + length + ((int(length) -
  //1)/block_size
  //+ 1)*sizeof(block_size));
...
INetIWriterGenStream::INetIWriterGenStream(IGenericStream *_stream,
                                           u32 inital_size)
