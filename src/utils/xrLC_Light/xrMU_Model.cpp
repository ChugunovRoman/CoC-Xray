...
xrMU_Model::xrMU_Model()
    : m_lod_ID(u16(-1)), read_faces(0), read_vertices(0), write_faces(0),
      write_vertices(0) {}
xrMU_Model::~xrMU_Model() {
  reading_close();
  writting_close();
...
u32 xrMU_Model::find(const _vertex *v) const {
  v_vertices::const_iterator i =
      std::find(m_vertices.begin(), m_vertices.end(), v);
  if (i == m_vertices.end())
    return u32(-1);
...
void xrMU_Model::read_adjacents(INetReader &r,
                                xrMU_Model::tread_faces &read_faces,
                                _vertex &v) {
  read_faces.read_ref(r, v.m_adjacents);
}

void xrMU_Model::write_adjacents(IWriter &w,
                                 xrMU_Model::twrite_faces &write_faces,
                                 const _vertex &v) {
...
void xrMU_Model::read_face_verts(INetReader &r,
                                 xrMU_Model::tread_vertices &read_verts,
                                 _face &v) {
  // read_verts
...
void xrMU_Model::write_face_verts(IWriter &w,
