...
template <typename e>
static xr_vector<xr_vector<e_net_globals> *> &
inter_get_table(enum_table<e> &table) {
...
template <execution_types ie, e_net_globals ig> struct add_global {
  ...
  add_global() {
    xr_vector<e_net_globals> &v =
        *inter_get_table<execution_types>(exe_reg)[ie];
...
template <typename e> struct list_exec {
  ...
  // static const  e_type		ii			=	( e_type
  // )(e::value); static const  e_type	next_ii		=	( e_type )(i+1);
