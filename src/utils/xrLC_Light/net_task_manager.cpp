...
void DumpDiff() {
  ...
  xr_vector<std::pair<CDeflector *, CDeflector *>>::iterator i = diff.begin(),
                                                             b = diff.begin(),
                                                             e = diff.end();
  for (; i != e; ++i) {
    Msg("diff %d", u32(i - b));
...
void net_task_manager::create_global_data_write(LPCSTR save_path) {
...
#ifndef LOAD_GL_DATA
  // gl_data_write = xr_new<INetFileBuffWriter>( "tmp_global_data",
  // 1024*1024/2,false); inlc_global_data()->write( *gl_data_write );
  ...
  /*{
          ...
          FS.update_path		( blfile_name, "$level$",
  "btmp_global_data"
  );

          string_path			 blfile_name_z;
          FS.update_path		( blfile_name_z, "$level$",
  "btmp_global_data_z" );

          string_path			 blfile_name_uz;
          FS.update_path		( blfile_name_uz, "$level$",
  "btmp_global_data_uz" ); compress( blfile_name, blfile_name_z ); decompress(
  blfile_name_z, blfile_name_uz );
          //"btmp_global_data"
  }
  */
  ///////////////
...
#else
  // gl_data_write = xr_new<INetFileBuffWriter>( "tmp_global_data",
  // 1024*1024/2,true); gl_data_write->w_close();
...
void __cdecl GetDataCallback(const char *dataDesc, IGenericStream **stream) {
  ...
  *stream = new CGenStreamOnFile(g_net_data); // CreateGenericStream();
  ...
  // xr_delete( *stream );
  clMsg("GetDataCallback: send end time elapsed sec: %f, ",
        time.GetElapsed_sec());
  //(*stream) = new TGenericStream(20000000);

  //(*stream)->Write(globalDataStream->GetBasePointer(),
  // globalDataStream->GetLength());
}
...
void net_task_manager::receive(INetReader &r) {
  ...
  clMsg("num task complited : %d , num task left %d  (task num %d)",
        size - pool_size, pool_size, size);
  if (pool.empty()) {
