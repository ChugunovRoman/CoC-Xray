...
public:
  ...
  void calc_lighting(xr_vector<base_color> &dest, const Fmatrix &xform,
                     CDB::MODEL *M, base_lighting &lights, u32 flags);
  void calc_lighting();
  //	void					calc_ogf
...
private:
  ...
  typedef vector_serialize<t_write<_face, get_id_self_index<_face>>>
      twrite_faces;

  typedef vector_serialize<t_read<_vertex, get_id_self_index<_vertex>>>
      tread_vertices;
  typedef vector_serialize<t_write<_vertex, get_id_self_index<_vertex>>>
      twrite_vertices;
...
private:
  ...
  static void read_adjacents(INetReader &r, xrMU_Model::tread_faces &read_faces,
                             _vertex &v);
  ...
  static void read_face_verts(INetReader &r,
                              xrMU_Model::tread_vertices &read_verts, _face &v);
  static void write_face_verts(IWriter &w,
                               xrMU_Model::twrite_vertices &write_verts,
                               const _face &v);
};

void XRLC_LIGHT_API calc_normals(xrMU_Model &model);

typedef vector_serialize<t_read<xrMU_Model, get_id_standart<xrMU_Model>>>
    tread_models;
typedef vector_serialize<t_write<xrMU_Model, get_id_standart<xrMU_Model>>>
    twrite_models;
