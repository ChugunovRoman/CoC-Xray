...
void xrLC_GlobalData::clear_build_textures_surface() {
  clLog("mem usage before clear build textures surface: %u",
        Memory.mem_usage());
...
void xrLC_GlobalData::clear_build_textures_surface(
  ...
  for (; i != e; ++i) {
    xr_vector<u32>::const_iterator ff =
        std::find(exept.begin(), exept.end(), u32(i - b));
    if (ff == exept.end())
      ::clear((*i));
  }
  Memory.mem_compact();
...
void xrLC_GlobalData ::create_rcmodel(CDB::CollectorPacked &CL) {
  ...
  _cl_globs._RCAST_Model->build(CL.getV(), (int)CL.getVS(), CL.getT(),
                                (int)CL.getTS());
}
...
/*
                ...
                b_params _g_params; vecVertex
   _g_vertices; vecFace _g_faces; vecDefl
   _g_deflectors; base_lighting					_L_static;
                CDB::MODEL* _RCAST_Model; bool
   _b_nosun; bool _gl_linear;
*/

// void			xrLC_GlobalData	::
// cdb_read_create
// ()
//{
//	VERIFY(!_RCAST_Model);
//	_RCAST_Model = xr_new<CDB::MODEL> ();
//	_RCAST_Model->build( &*verts.begin(), (int)verts.size(), &*tris.begin(),
//(int)tris.size() );
//}
...
void write(IWriter &w, const CDB::TRI &tri,
           const xrLC_GlobalData &lc_global_data) {
  ::write(w, tri);
...
void read(INetReader &r, CDB::MODEL *&m, xrLC_GlobalData &lc_global_data) {
  ...
  VERIFY(!m);
  m = xr_new<CDB::MODEL>();
  m->build(&*verts.begin(), (int)verts.size(), &*tris.begin(),
           (int)tris.size());
  verts.clear();
  tris.clear();
}
...
void xrLC_GlobalData ::read_modes_color(INetReader &r) {
  xr_vector<xrMU_Model *>::iterator i = _mu_models.begin(),
                                    e = _mu_models.end();
  for (; e != i; ++i)
    (*i)->read_color(r);
}
void xrLC_GlobalData ::write_modes_color(IWriter &w) const {
  xr_vector<xrMU_Model *>::const_iterator i = _mu_models.begin(),
                                          e = _mu_models.end();
  for (; e != i; ++i)
    (*i)->write_color(w);
}
...
template <typename T>
std::pair<u32, u32> get_id(const xr_vector<xrMU_Model *> &mu_models,
                           const T *v) {
  ...
  xr_vector<xrMU_Model *>::const_iterator ii =
      std::find_if(mu_models.begin(), mu_models.end(), f);
  if (ii == mu_models.end())
    return std::pair<u32, u32>(u32(-1), u32(-1));
  return std::pair<u32, u32>(u32(ii - mu_models.begin()), face_id);
}

// std::pair<u32,u32>			xrLC_GlobalData	::		get_id
// ( const _face * v ) const
//{
