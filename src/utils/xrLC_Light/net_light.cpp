...
//				    DWORD sessionId );
bool TaskReceive(net_task &task, IAgent *agent, DWORD sessionId,
...
bool GetGlobalData(IAgent *agent, DWORD sessionId) {
  ...
  if (!inlc_global_data()) {
    ...
    /*
                      ...
                      HRESULT rz = agent->GetData(sessionId, dataDesc,
       &globalDataStream);
    ...
    */
    ...
    /*
                    ...
                    R_ASSERT(file);
                    file->w( globalDataStream->GetCurPointer(),
...
XRLC_LIGHT_API net_task_interface *g_net_task_interface =
    &g_net_task_interface_impl;
/*
