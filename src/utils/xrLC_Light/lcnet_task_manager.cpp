...
task_manager::task_manager()
    : _user(0), tasks_completed(0), current_pool(0), start(0),
      session_id(DWORD(-1)), _release(false) {
...
void task_manager::send_result(u8 pool_id, IGenericStream *outStream,
                               net_execution &e) {
  write_task_pool(outStream, pool_id);
  pools[pool_id]->send_result(outStream, e);
}

net_execution *task_manager::receive_task(u8 &pool_id, IAgent *agent,
                                          DWORD sessionId,
...
void __cdecl data_cleanup_callback(const char *dataDesc,
                                   IGenericStream **stream);
void task_manager::startup() {
  start_time.Start();
  tasks_completed = 0;
  // create_user( );
  thread_spawn(task_manager::user_thread_proc, "release-user", 1024 * 1024,
               this);
  for (;;) {
    Sleep(1);
...
void task_manager::release() {
  ...
  init_lock.Leave();
  //	thread_spawn
  //(task_manager::release_user_thread_proc,"release-user",1024*1024,this);
}
