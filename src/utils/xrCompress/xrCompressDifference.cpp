...
struct file_comparer {
  ...
  file_comparer(char *c, CLocatorAPI *fs1, CLocatorAPI *fs2, Flags32 flag) {
    ...
    const CLocatorAPI::file *f =
        m_fs_new->exist("$target_folder$", m_full_name);
    if (f)
...
int ProcessDifference() {
  ...
  if (strstr(params, "-diff /?")) {
    ...
    printf("xrCompress.exe -diff <new_data> <old_data> -out <diff_resulf> "
           "[options]\n");
    printf("<new_data>, <old_data> and <diff_resulf> values must be a folder "
           "name\n");
  ...
  sscanf(strstr(params, "-diff ") + 6 + xr_strlen(new_folder) + 1, "%[^ ] ",
         old_folder);
  ...
  for (u32 i = 0; i < total; ++i) {
    ...
    xr_sprintf(stats, "%d of %d (%3.1f%%)", i, total,
               100.0f * ((float)i / (float)total));
