...
static lzo_byte *STORE_RUN(lzo_byte *op, const lzo_byte *ii, lzo_uint t,
                           lzo_byte *out) {
  if (t < 4 && op > out)
    op[-2] |= LZO_BYTE(t);
  else if (t <= 31)
...
LZO_EXTERN(int)
lzo1f_999_compress_callback(const lzo_byte *in, lzo_uint in_len, lzo_byte *out,
  ...
  while (c->look > 0) {
    ...
    if (m_len > 0 && lazy_match_min_gain >= 0 && c->look > m_len) {
      ...
      if (c->m_len >= m_len + lazy_match_min_gain) {
        ...
        m_len = c->m_len;
        m_off = c->m_off;
        assert(lzo_memcmp(c->ip - c->look, c->ip - c->look - m_off, m_len) ==
               0);
#endif
        lit++;
        assert(ii + lit == c->ip - c->look);
...
LZO_PUBLIC(int)
lzo1f_999_compress(const lzo_byte *in, lzo_uint in_len, lzo_byte *out,
                   lzo_uintp out_len, lzo_voidp wrkmem) {
