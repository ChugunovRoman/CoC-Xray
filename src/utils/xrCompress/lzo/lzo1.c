...
/*
     Format of the marker byte
         ...
         76543210
         --------
         00000000	a long run (a 'R0' run) - there are short and long R0
   runs 000rrrrr	a short run with len r mmmooooo	a short match (len =
   2+m, o = offset low bits) 111ooooo	a long match (o = offset low bits)
*/
...
LZO_PUBLIC(int)
lzo1_decompress(const lzo_byte *in, lzo_uint in_len, lzo_byte *out,
...
#define PS *m_pos++ != *ip++
      ...
      if (PS || PS || PS || PS || PS || PS || PS || PS || PS || PS || PS ||
          PS || PS || PS)
...
LZO_PUBLIC(int)
lzo1_compress(const lzo_byte *in, lzo_uint in_len, lzo_byte *out,
              lzo_uintp out_len, lzo_voidp wrkmem) {
  int r = LZO_E_OK;
