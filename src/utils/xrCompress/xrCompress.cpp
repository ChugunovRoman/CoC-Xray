...
xrCompressor::xrCompressor()
    : fs_pack_writer(NULL), bnoCompress(true), files_list(NULL),
      folders_list(NULL), bStoreFiles(false), pPackHeader(NULL),
      config_ltx(NULL) {
  bytesSRC = 0;
  bytesDST = 0;
  filesTOTAL = 0;
...
bool xrCompressor::testSKIP(LPCSTR path) {
  ...
  if (config_ltx && config_ltx->section_exist("exclude_files")) {
    ...
    for (CInifile::SectCIt if_it = if_sect.Data.begin();
         if_it != if_sect.Data.end(); ++if_it) {
      string512 buf;
      strcpy(buf, p_dir);
      strcat(buf, p_name);
      strcat(buf, p_ext);
  ...
  for (xr_vector<shared_str>::iterator it = exclude_exts.begin();
       it != exclude_exts.end(); ++it)
...
xrCompressor::ALIAS *xrCompressor::testALIAS(IReader *base, u32 crc,
                                             u32 &a_tests) {
...
void xrCompressor::write_file_header(LPCSTR file_name, const u32 &crc,
                                     const u32 &ptr, const u32 &size_real,
  ...
  if (A) {
    ...
    if (testVFS(path)) {
      ...
      if (0 != c_size_real) {
        ...
        if (bFast) {
          R_ASSERT(LZO_E_OK == lzo1x_1_compress((u8 *)src->pointer(),
                                                c_size_real, c_data,
                                                &c_size_compressed, c_heap));
        } else {
          R_ASSERT(LZO_E_OK == lzo1x_999_compress((u8 *)src->pointer(),
                                                  c_size_real, c_data,
                                                  &c_size_compressed, c_heap));
        }
        ...
        if ((c_size_compressed + 16) >= c_size_real) {
          ...
          if (!bFast) {
            ...
            R_ASSERT(LZO_E_OK == lzo1x_optimize(c_data, c_size_compressed,
                                                c_out, &c_orig, NULL));
...
void xrCompressor::ClosePack() {
  ...
  fs_pack_writer->w_chunk(1 | CFS_CompressMark, fs_desc.pointer(),
                          fs_desc.size());
  ...
  printf("\n\nFiles total/skipped/VFS/aliased: %d/%d/%d/%d\nOveral: %dK/%dK, "
         ...
         filesTOTAL, filesSKIP, filesVFS, filesALIAS, bytesDST / 1024,
         bytesSRC / 1024, 100.f * float(bytesDST) / float(bytesSRC),
         ((dwTimeEnd - dwTimeStart) / 1000) / 60,
         ((dwTimeEnd - dwTimeStart) / 1000) % 60,
         float((float(bytesDST) / float(1024 * 1024)) /
               (t_compress.GetElapsed_sec())));
  Msg("\n\nFiles total/skipped/VFS/aliased: %d/%d/%d/%d\nOveral: %dK/%dK, "
      "%3.1f%%\nElapsed time: %d:%d\nCompression speed: %3.1f Mb/s\n\n",
      filesTOTAL, filesSKIP, filesVFS, filesALIAS, bytesDST / 1024,
      bytesSRC / 1024, 100.f * float(bytesDST) / float(bytesSRC),
...
void xrCompressor::PerformWork() {
  if (!files_list->empty() && target_name.size()) {
    ...
    for (u32 it = 0; it < files_list->size(); it++) {
      xr_sprintf(caption, "Compress files: %d/%d - %d%%", it + 1,
                 files_list->size(), ((it + 1) * 100) / (files_list->size()));
...
bool xrCompressor::IsFolderAccepted(CInifile &ltx, LPCSTR path, BOOL &recurse) {
  ...
  if (ltx.section_exist("exclude_folders")) {
    ...
    for (CInifile::SectCIt ef_it = ef_sect.Data.begin();
         ef_it != ef_sect.Data.end(); ef_it++) {
...
void xrCompressor::ProcessLTX(CInifile &ltx) {
  ...
  if (ltx.section_exist("include_folders")) {
    ...
    for (CInifile::SectCIt if_it = if_sect.Data.begin();
      ...
      string_path path;
      LPCSTR _path = 0 == xr_strcmp(if_it->first.c_str(), ".\\")
                         ? ""
                         : if_it->first.c_str();
