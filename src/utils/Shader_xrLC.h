...
public:
  void Load(LPCSTR name) {
    ...
    if (NULL == fs) {
      ...
      //			clMsg			(inf);
      //			MessageBox
      //(logWindow,inf,"Error!",MB_OK|MB_ICONERROR);
      FATAL(inf);
      return;
    };
  ...
  u32 GetID(LPCSTR name) const {
    for (Shader_xrLCVec::const_iterator it = library.begin();
         it != library.end(); it++)
...
IC void post_process_materials(const Shader_xrLC_LIB &shaders,
  ...
  for (u32 m = 0; m < materials.size(); m++) {
    ...
    if (65535 == M.shader_xrlc) {
      ...
      if (id < 0) {
        clMsg("ERROR: Shader '%s' not found in library",
              shader_compile[M.shader].name);
        R_ASSERT(id >= 0);
      }
      M.reserved = u16(id);
    }
