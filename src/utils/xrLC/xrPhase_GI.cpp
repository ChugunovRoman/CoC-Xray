...
static Fvector GetPixel_7x7(CDB::RESULT &rpinf) {
  ...
  for (int _y = -3; _y <= 3; _y++) {
    for (int _x = -3; _x <= 3; _x++) {
      ...
      P.set(float(color_get_R(pixel)), float(color_get_G(pixel)),
            float(color_get_B(pixel)));
      R.mad(P, 1.f / 255.f);
    }
  }
  R.div(49.f);
...
public:
  ...
  virtual void Execute() {
    ...
    for (;;) {
      ...
      float factor = _sqrt(
          src.range /
          gi_optimal_range); // smaller lights get smaller amount of photons
      if (factor > 1)
        factor = 1;
      if (LT_SECONDARY == src.type)
        factor /= powf(
            2.f, float(src.level)); // secondary lights get half the photons
      factor *= _sqrt(src.energy);  // 2.f is optimal energy = baseline
      // factor	= _sqrt (factor);
      // // move towards 1.0 (one)
      ...
      for (int it = 0; it < count; it++) {
        ...
        dst.energy =
            src.energy * dot * gi_reflect * (1 - R->range / src.range) * _scale;
        if (dst.energy < _clip)
          continue;
        ...
        float _r3 = src.range;
        dst.range =
            1 * ((1.f * _r1 + 3.f * _r2 + 3.f * _r3) / 7.f); // empirical
