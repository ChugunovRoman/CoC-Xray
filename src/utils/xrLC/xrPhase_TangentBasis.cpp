...
static void fill_mender_input(xr_vector<MeshMender::Vertex> &theVerts,
  ...
  std::sort(lc_global_data()->g_vertices().begin(),
...
static void
retrive_data_from_mender_otput(const xr_vector<MeshMender::Vertex> &theVerts,
...
void CBuild::xrPhase_TangentBasis() {
  ...
  Status("Declarator...");
  u32 v_count_reserve =
      iFloor(float(lc_global_data()->g_vertices().size()) * 1.33f);
