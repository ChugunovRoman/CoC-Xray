...
void CBuild::BuildCForm() {
  ...
  decimater.module(hModQuadric)
      .set_max_err(0.0001, false); // error-limit 0.0001
  ...
  decimater
      .initialize(); // let the decimater initialize the mesh and the modules

  int nf_before = int(mesh.n_faces());
  int nv_before = int(mesh.n_vertices());
  int nc =
      decimater.decimate(nv_before); // do decimation, as large, as possible
  mesh.garbage_collection();
  ...
  // Clear pDeflector (it is stored in the same memory space with
  // dwMaterialGame)
  for (vecFaceIt I = g_faces.begin(); I != g_faces.end(); I++) {
    Face *F = *I;
    F->pDeflector = NULL;
