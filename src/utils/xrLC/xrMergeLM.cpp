...
void _rect_register(_rect &R, CDeflector *D, BOOL bRotate) {
  ...
  if (!bRotate) {
    ...
    for (u32 y = 0; y < s_y; y++) {
      BYTE *P =
          surface + (y + R.a.y) * lmap_size + R.a.x; // destination scan-line
      u32 *S = lm + y * s_x;
...
void CBuild::MergeLM() {
  ...
  Status("Processing...");
  for (u32 light_layer = 0; light_layer < pBuild->lights.size();
       light_layer++) {
    ...
    while (Layer.size()) {
      ...
      if (SEL.empty()) {
        ...
        //  Analyze resuls
        clMsg("%3d / %3d - [%d,%d]", best.size(), selected.size(),
              brect.SizeX(), brect.SizeY());
        ...
        for (K = 0; K < (int)best.size(); K++) {
          ...
          vecDeflIt OLD = std::find(g_deflectors.begin(), g_deflectors.end(),
                                    SEL[iRealIndex]);
