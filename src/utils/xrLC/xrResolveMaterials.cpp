...
void __fastcall _ADJ_Split(Face *F) {
  ...
  for (int i = 0; i < 3; i++) {
    for (vecFaceIt it = F->v[i]->adjacent.begin();
         it != F->v[i]->adjacent.end(); it++)
...
void CBuild::ResolveMaterials() {
  ...
  while (!id.empty()) {
    ...
    for (X = 1; X < (int)id.size(); X++) {
      ...
      // ok, merge face lists
      BB.set(bb);
      g_XSplit[ID].insert(g_XSplit[ID].end(), g_XSplit[guid].begin(),
                          g_XSplit[guid].end());
      g_XSplit[guid].clear();
      id.erase(id.begin() + X);
      X--;
  ...
  for (X = 0; X < int(g_XSplit.size()); X++) {
    ...
    if ((size.y > g_params.c_SS_maxsize) || (size.x > g_params.c_SS_maxsize) ||
      ...
      if ((int(s1.size()) < c_SS_LowVertLimit) ||
          (int(s2.size()) < c_SS_LowVertLimit)) {
        // splitting failed
      } else {
  ...
  for (splitIt S = g_XSplit.begin(); S != g_XSplit.end(); S++) {
    Detach(S);
    Progress(.66f +
             .33f * float(S - g_XSplit.begin()) / float(g_XSplit.size()));
  }
}
