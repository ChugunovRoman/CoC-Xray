...
IC BOOL ValidateMergeLinearSize(const Fvector &merged, const Fvector &orig1,
                                const Fvector &orig2, int iAxis) {
...
IC BOOL ValidateMerge(u32 f1, const Fbox &bb_base, const Fbox &bb_base_orig,
                      u32 f2, const Fbox &bb, float &volume) {
  // Polygons
  ...
  Fvector orig2;
  bb.getsize(orig2);
  //	if (sz.x>(4*c_SS_maxsize/3))			return FALSE;	// Don't
  //exceed limits (4/3 GEOM) 	if (sz.y>(4*c_SS_maxsize/3))
  // return FALSE; 	if (sz.z>(4*c_SS_maxsize/3))			return
  // FALSE;
...
void FindBestMergeCandidate(u32 *selected, float *selected_volume, u32 split,
...
DWORD WINAPI MergeGmThreadProc(LPVOID lpParameter) {
  ...
  while (TRUE) {
    ...
    case WAIT_OBJECT_0 + 0:
      FindBestMergeCandidate(&pParams->selected, &pParams->selected_volume,
                             pParams->split, pParams->split_size,
                             pParams->subdiv, pParams->bb_base_orig,
                             pParams->bb_base);
      // Signal "ready" event
...
void InitMergeGmThreads() {
  ...
  mergegm_threads_handles =
      (LPHANDLE)xr_malloc(mergegm_threads_count * sizeof(HANDLE));
  mergegm_ready_events =
      (LPHANDLE)xr_malloc(mergegm_threads_count * sizeof(HANDLE));
  mergegm_params = (LP_MERGEGM_PARAMS)xr_malloc(mergegm_threads_count *
                                                sizeof(MERGEGM_PARAMS));
  ...
  for (u32 i = 0; i < mergegm_threads_count; i++) {
    ...
    mergegm_threads_handles[i] =
        CreateThread(NULL, 0, &MergeGmThreadProc, &mergegm_params[i], 0, NULL);
  }
...
void DoneMergeGmThreads() {
  ...
  WaitForMultipleObjects(mergegm_threads_count, mergegm_threads_handles, TRUE,
                         INFINITE);
...
void FindBestMergeCandidate_threads(u32 *selected, float *selected_volume,
                                    u32 split, u32 split_size, vecFace *subdiv,
  ...
  for (u32 i = 0; i < mergegm_threads_count; i++) {
    ...
    mergegm_params[i].split_size = (i == (mergegm_threads_count - 1))
                                       ? split_size
                                       : mergegm_params[i].split + m_range;
  ...
  // Wait for result
  WaitForMultipleObjects(mergegm_threads_count, mergegm_ready_events, TRUE,
                         INFINITE);
...
void FindBestMergeCandidate(u32 *selected, float *selected_volume, u32 split,
  ...
  for (u32 test = split; test < split_size; test++) {
    ...
    if (!NeedMerge(TEST, bb))
      continue;
    if (!ValidateMerge(subdiv->size(), *bb_base, *bb_base_orig, TEST.size(), bb,
                       volume))
      continue;
  ...
  for (u32 split = 0; split < g_XSplit.size(); split++) {
    ...
    while (NeedMerge(subdiv, bb_base)) {
      ...
      if ((g_XSplit.size() - split) < 200) { // may need adjustment
        // single thread
        FindBestMergeCandidate(&selected, &selected_volume, split + 1,
                               g_XSplit.size(), &subdiv, &bb_base_orig,
                               &bb_base);
      } else {
        // multi thread
      ...
      // **OK**. Perform merge
      subdiv.insert(subdiv.begin(), g_XSplit[selected]->begin(),
                    g_XSplit[selected]->end());
