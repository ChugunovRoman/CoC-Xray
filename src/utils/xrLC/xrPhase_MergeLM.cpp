...
void CBuild::xrPhase_MergeLM() {
  ...
  while (Layer.size()) {
    ...
    for (u32 it = 0; it < Layer.size(); it++) {
      ...
      materials()[D->GetBaseMaterial()].internal_max_area = _max(
          D->layer.Area(), materials()[D->GetBaseMaterial()].internal_max_area);
    }
    ...
    for (u32 it = 0; it < merge_count; it++) {
      ...
      if (_rect_place(rT, &L)) {
        ...
        lmap->Capture(Layer[it], rT.a.x, rT.a.y, rT.SizeX(), rT.SizeY(),
                      bRotated);
...
class	pred_remove { public: IC bool	operator() (CDeflector* D) { { if (0==D)
return TRUE;}; if (D->bMerged) {D->bMerged=FALSE; return TRUE; } else return
FALSE;  }; };
...
DWORD WINAPI MergeThreadProc( LPVOID lpParameter )
{
        ...
        while( TRUE ) {
                ...
                switch ( WaitForMultipleObjects( 2 , pParams->hEvents , FALSE ,
INFINITE ) ) { case WAIT_OBJECT_0 + 0 : MergeLM_process( pParams->m_from ,
pParams->m_to , pParams->Layer , pParams->lmap );
                                // Signal "ready" event
...
void InitMergeThreads()
{
        ...
        mergelm_threads_handles = (LPHANDLE) xr_malloc( mergelm_threads_count *
sizeof( HANDLE ) ); mergelm_ready_events = (LPHANDLE) xr_malloc(
mergelm_threads_count * sizeof( HANDLE ) ); mergelm_params = (LP_MERGELM_PARAMS)
xr_malloc( mergelm_threads_count * sizeof( MERGELM_PARAMS ) );
        ...
        for ( u32 i = 0 ; i < mergelm_threads_count ; i++ ) {
                ...
                for( u32 x = 0 ; x < 3 ; x++ )
                        mergelm_params[ i ].hEvents[ x ] = CreateEvent( NULL ,
FALSE , FALSE , NULL );

                // Duplicate ready event into array
                mergelm_ready_events[ i ] = mergelm_params[ i ].hEvents[ 2 ];

                mergelm_threads_handles[ i ] = CreateThread( NULL , 0 ,
&MergeThreadProc , &mergelm_params[ i ] , 0 , NULL );
        }
...
void DoneMergeThreads()
{
        ...
        WaitForMultipleObjects( mergelm_threads_count , mergelm_threads_handles
, TRUE , INFINITE );
        ...
        xr_free( mergelm_ready_events );		mergelm_ready_events =
NULL; xr_free( mergelm_params );				mergelm_params =
NULL;
...
void MergeLM_process_threads( u32 m_count , vecDefl* pLayer , CLightmap* lmap )
{
        ...
        for ( u32 i = 0 ; i < mergelm_threads_count ; i++ ) {
                ...
                mergelm_params[ i ].m_to = ( i == ( mergelm_threads_count - 1 )
) ? m_count : mergelm_params[ i ].m_from + m_range;

                SetEvent( mergelm_params[ i ].hEvents[ 0 ] );
        } // for

        WaitForMultipleObjects( mergelm_threads_count , mergelm_ready_events ,
TRUE , INFINITE );
}
        ...
        for ( u32 it = m_from ; it < m_to ; it++ ) {
                ...
                if ( _rect_place( rT , &L ) ) {
                        ...
                        EnterCriticalSection( &mergelm_cs );
                                lmap->Capture( (*pLayer)[ it ] , rT.a.x , rT.a.y
, rT.SizeX() , rT.SizeY() , bRotated ); LeaveCriticalSection( &mergelm_cs );
...
void CBuild::xrPhase_MergeLM()
{
        ...
        {
                CDeflector*	D		=
lc_global_data()->g_deflectors()[it]; if (D->bMerged)		continue;
                Layer.push_back		(D);
        }
...
lc_global_data()->lightmaps().size()); Phase		(phase_name);
                ...
                Status		("Selection...");
                for (u32 it=0; it<materials().size(); it++)
materials()[it].internal_max_area	= 0; for (u32 it=0; it<Layer.size();
it++)	{ CDeflector*	D		= Layer[it];
...
= _max(D->layer.Area(),materials()[D->GetBaseMaterial()].internal_max_area);
                ...
                // Select first deflectors which can fit
                Status		("Selection...");
                u32 maxarea		= c_LMAP_size*c_LMAP_size*8;
// Max up to 8 lm selected u32 curarea		= 0; u32 merge_count	= 0; for
(u32 it=0; it<(int)Layer.size(); it++)	{ int		defl_area	=
Layer[it]->layer.Area(); if (curarea + defl_area > maxarea) break; curarea
+=	defl_area; merge_count ++;
                }
                ...
                // Remove merged lightmaps
                Status			("Cleanup...");
                vecDeflIt last	= std::remove_if
(Layer.begin(),Layer.end(),pred_remove()); Layer.erase
(last,Layer.end());
