...
D3DVERTEXELEMENT9 r1_decl_lmap[] = // 12+4+4+4+4+4	= 32
    {{0, 0, D3DDECLTYPE_FLOAT3, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_POSITION,
      0},
     {0, 12, D3DDECLTYPE_D3DCOLOR, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_NORMAL,
      0},
     {0, 16, D3DDECLTYPE_D3DCOLOR, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_TANGENT,
      0},
     {0, 20, D3DDECLTYPE_D3DCOLOR, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_BINORMAL,
      0},
     {0, 24, D3DDECLTYPE_SHORT2, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_TEXCOORD,
      0},
     {0, 28, D3DDECLTYPE_SHORT2, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_TEXCOORD,
      1},
     D3DDECL_END()};
...
struct r1v_lmap {
  ...
  r1v_lmap(Fvector3 _P, Fvector _N, base_basis _T, base_basis _B,
           base_color _CC, Fvector2 tc_base, Fvector2 tc_lmap) {
    base_color_c _C;
    _CC._get(_C);
    _N.normalize();
...
struct r1v_vert {
  ...
  r1v_vert(Fvector3 _P, Fvector _N, base_basis _T, base_basis _B,
           base_color _CC, Fvector2 tc_base) {
    base_color_c _C;
    _CC._get(_C);
    _N.normalize();
    std::pair<s16, u8> tc_u = s24_tc_base(tc_base.x);
    ...
    T = u8_vec4(_T, tc_u.second);
    B = u8_vec4(_B, tc_v.second);
    C = color_rgba(u8_clr(_C.rgb.x), u8_clr(_C.rgb.y), u8_clr(_C.rgb.z),
                   u8_clr(_C.sun));
    tc0x = tc_u.first;
    tc0y = tc_v.first;
  }
...
void OGF::Save(IWriter &fs) {
  ...
  const Shader_xrLC *SH =
      pBuild->shaders().Get(pBuild->materials()[material].reserved);
...
void OGF_Reference::Save(IWriter &fs) {
  ...
  // geom_batch_average	(vertices.size(),faces.size());	// don't use
  // reference(s) as batch estimate
...
void OGF::PreSave(u32 tree_id) {
  ...
  if (fast_path_data.vertices.size() && fast_path_data.faces.size()) {
    ...
    x_D.set(x_decl_vert);
    x_VB.Begin(x_D);
    for (itXV V = fast_path_data.vertices.begin();
         V != fast_path_data.vertices.end(); V++) {
      x_vert v(V->P);
      x_VB.Add(&v, sizeof(v));
    }
    x_VB.End(&fast_path_data.vb_id, &fast_path_data.vb_start);
    x_IB.Register(LPWORD(&*fast_path_data.faces.begin()),
                  LPWORD(&*fast_path_data.faces.end()), &fast_path_data.ib_id,
  ...
  // Faces
  g_IB.Register(LPWORD(&*data.faces.begin()), LPWORD(&*data.faces.end()),
                &data.ib_id, &data.ib_start);
}
