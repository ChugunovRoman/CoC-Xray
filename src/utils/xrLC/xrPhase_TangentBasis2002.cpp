...
int find_same_vertex(const xr_vector<u32> &m, const Fvector2 Ftc,
                     const xr_vector<float> &v_tc) {
  // Search
...
u32 add_vertex(const Vertex &V, const Fvector2 &Ftc,
               xr_vector<float> &v_position, xr_vector<float> &v_normal,
...
void fill_mender_input(xr_vector<float> &v_position, xr_vector<float> &v_normal,
  ...
  for (u32 f = 0; f < g_faces.size(); f++) {
    ...
    for (u32 v = 0; v < 3; v++) {
      Vertex *V = F->v[v];
      u32 ID = u32(std::lower_bound(g_vertices.begin(), g_vertices.end(), V) -
                   g_vertices.begin());
...
void CBuild::xrPhase_TangentBasis() {
  ...
  /*
  float	teps			= .5f / 4096.f;		// half pixel from 4096
  texture (0.0001220703125) for (u32 f=0; f<g_faces.size(); f++)
  {
          Progress	(float(f)/float(g_faces.size()));
          Face*		F		= g_faces[f];
          ...
          if (e01<teps)
          {
                  ...
                  tc1.add		(r);		tc0.sub	(r);	//
  correction } else if (e12<teps) { Fvector2	d1,d2,d,r; d0.sub
  (tc0,tc2);	d0.norm	(); d1.sub		(tc1,tc2);	d1.norm	();
                  d.averageA	(d0,d1);	d.norm	();
                  r.cross		(d);		r.mul	(teps);	// right
                  tc1.add		(r);		tc0.sub	(r);	//
  correction
