...
void			xrMU_Model::_face::VReplace		(_vertex* what,
_vertex* to)
{
        if (v[0]==what) { v[0]=to; what->prep_remove(this); to->prep_add(this);
} if (v[1]==what) { v[1]=to; what->prep_remove(this); to->prep_add(this); } if
(v[2]==what) { v[2]=to; what->prep_remove(this); to->prep_add(this); }
}
void			xrMU_Model::_face::VReplace_NoRemove(_vertex* what,
_vertex* to)
{
...
void			xrMU_Model::_face::VSet			(int idx,
_vertex* V)
{
        v[idx]=V; V->prep_add(this);
}
void			xrMU_Model::_face::VSet
(_vertex *V1, _vertex *V2, _vertex *V3)
{
...
void			xrMU_Model::_face::EdgeVerts	(int e, _vertex** A,
_vertex** B)
{
        *A			= v[edge2idx[e][0]];
        *B			= v[edge2idx[e][1]];
...
BOOL			xrMU_Model::data_face::RenderEqualTo	(Face *F)
{
        ...
        if (F->tc.size()	!= F->tc.size()		)	return
FALSE;	// redundant??? return TRUE;
}
*/
...
xrMU_Model::_vertex *
xrMU_Model::_vertex::CreateCopy_NOADJ(v_vertices &vertises_storage) const {
...
template <> Tface<xrMU_Model::data_vertex>::~Tface() {}
template <> Tvertex<xrMU_Model::data_vertex>::~Tvertex() {}
