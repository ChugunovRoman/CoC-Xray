...
struct OGF : public OGF_Base {
  ...
  /*
          ...
          // Progressive
          FSlideWindowItem	m_SWI		;		// The records
     of the collapses.
          FSlideWindowItem	x_SWI		;		// The records
     of the collapses / fast-path
  */
  // for build only
  u32 dwRelevantUV;
  ...
  */
  ...
  void adjacent_select(xr_vector<u32> &dest, xr_vector<bool> &vmark,
  ...
  //	void				Save_Cached		(IWriter &fs, ogf_header& H,
  //BOOL bColors);

  void Save_Normal_PM(IWriter &fs, ogf_header &H, BOOL bColors);
  void Load_Normal_PM(IReader &fs, ogf_header &H, BOOL bColors);

  //	void				Save_Progressive(IWriter &fs, ogf_header& H,
  //BOOL bColors);

  virtual void GetGeometry(xr_vector<Fvector> &R) {
    for (xr_vector<OGF_Vertex>::iterator I = data.vertices.begin();
         I != data.vertices.end(); I++)
      R.push_back(I->P);
  }
};
