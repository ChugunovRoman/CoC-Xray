...
Comments:
    ...
    This tool is designed to help condition meshes for use in vertex & pixel
shaders.

    It can generate normals, texture coordinates, and perhaps most importantly,
texture space basis matrices.  It also can fix common texuring problems that
come up when bump mapping, including

  Texture Mirroring -  When two one halves of a character use the same part of a
texture.

  Cylindrical TexGen - When the rendering relies on cylindrical wrapping,
texture space computation won't work right.

  Stretched Basis -    When two adjacend faces use wildly different texture
mappings, causing stretching that can ruin per-pixel lighting.
  ...
  // "tex0" is the only coordinate set generated, and the texture matrix applies
only to it

  //  All unknown attribute types, including "tex1", "tex2", "random_attribute",
"weights", "bones", etc.
  // are simply passed through.  They will be duplicated as needed just like
positions, normals, etc.

  bool bSuccess = aMender.Munge( inputAtts,              // these are my
positions & indices outputAtts,             // these are the outputs I
requested, plus extra stuff generated on my behalf 3.141592654f / 2.5f,    //
tangent space smooth angle NULL,                   // no Texture matrix applied
to my tex0 coords FixTangents,            // fix degenerate bases & texture
mirroring FixCylindricalTexGen    // handle cylidrically mapped textures via
vertex duplication WeightNormalsByFaceSize // weight vertex normals by the
triangle's size
                                 );
  ...
  //  This is because in handling tangent space smoothing, and solving texture
mirroring &
  // cylindrical texture wrapping problems, we partially duplicate vertices.

  // All attributes are duplicated, even unknowns.

  //  You may also get things you didn't ask for.  For instance, if you ask for
tangent space,
  // in other words, you ask for "tangent" or "binormal",  you will get
"normal", "tex0",
  // "tangent" and "binormal".  You can then ignore things in the output vector
that you don't want.

  //   If you ask for FixCylindricalTexGen, it will fix any absolute change in
texture coordinates > 0.5
  // across a single edge.  Therefore, if you pass in a single quad or cube, it
won't work.  Any more
...
bool NVMeshMender::Munge(const NVMeshMender::VAVector &input,
                         ...
                         const float *pTextureMatrix, const Option _FixTangents,
  ...
  if (bNeedNormals || bComputeTangentSpace) {
    ...
    if (!have_normals) {
      ...
      // VertexAttribute::FloatVector& normals = output[ (*want).second
      // ].floatVector_;
      ...
      for (unsigned int t = 0; t < indices.size(); t += 3) {
        ...
        if (_WeightNormalsByFaceSize == DontWeightNormalsByFaceSize) {
          ...
          // Leave it as-is, to weight by face size naturally by the cross
          // product result
        }
      ...
      for (unsigned int v = 0;
           v < output[(*want).second].floatVector_.size() / 3; ++v)
  ...
  if (bNeedTexCoords || bComputeTangentSpace) {
    ...
    if (have_texcoords)
      ...
      const unsigned int theCount =
          static_cast<unsigned int>(positions.size() / 3.0f);
      ...
      for (unsigned int p = 0; p < theCount; ++p) {
        ...
        // perform cylindrical mapping onto object, and remap from -pi,pi to
        // -1,1
    ...
    if (_FixCylindricalTexGen == FixCylindricalTexGen) {
      ...
      VertexAttribute::FloatVector &texcoords =
          (output[(*texIter).second].floatVector_);
      ...
      for (unsigned int f = 0; f < theSize; f += 3) {
        for (int v = 0; v < 3; ++v) {
          ...
          nv_scalar dS = texcoords[indices[end] * 3 + 0] -
                         texcoords[indices[start] * 3 + 0];
          ...
          if (_abs(dS) >= 0.5f) {
            bDoS = true;
            if (texcoords[indices[start] * 3 + 0] <
                texcoords[indices[end] * 3 + 0]) {
...

        {
          ...
          for (int v = 0; v < 3; ++v) {
            ...
            nv_scalar dT = texcoords[indices[end] * 3 + 1] -
                           texcoords[indices[start] * 3 + 1];
    ...
    if (pTextureMatrix) {
      const mat4 M(pTextureMatrix[0], pTextureMatrix[1], pTextureMatrix[2],
                   pTextureMatrix[3], pTextureMatrix[4], pTextureMatrix[5],
                   pTextureMatrix[6], pTextureMatrix[7], pTextureMatrix[8],
                   pTextureMatrix[9], pTextureMatrix[10], pTextureMatrix[11],
                   pTextureMatrix[12], pTextureMatrix[13], pTextureMatrix[14],
                   pTextureMatrix[15]);
      Mapping::iterator texIter = outmap.find("tex0");
      VertexAttribute::FloatVector &texcoords =
          output[(*texIter).second].floatVector_;
  ...
  if (bComputeTangentSpace) {
    ...
    for (unsigned int f = 0; f < theSize; f += 3) {
      ...
      if (_FixTangents == FixTangents) {
        ...
        for (int e = 0; e < 3; ++e) {
          ...
          edge.v0 = (unsigned int)nv_min((nv_scalar)indices[start],
                                         (nv_scalar)indices[end]);
          edge.v1 = (unsigned int)nv_max((nv_scalar)indices[start],
                                         (nv_scalar)indices[end]);
          ...
          if (iter == Edges.end()) {
            ...
            //  if the discontinuity in s, t, or sxt is greater than some
            //  epsilon,
            //   duplicate the vertex so it won't smooth with its neighbor
            //   anymore
            ...
            // Now that the vertices are duplicated, smoothing won't occur over
            // this edge,
            //  because the two faces will sum their tangent basis vectors into
            //  separate indices
          }
        }
      } // if fixtangents
    ...
    if (_FixCylindricalTexGen == FixCylindricalTexGen) {
      for (unsigned int v = 0; v < IdenticalVertices_.size(); ++v) {
        ...
        for (xr_set<unsigned int>::iterator iter =
                 IdenticalVertices_[v].begin();
  ...
  // At this point, tex coords, normals, binormals and tangents should be
