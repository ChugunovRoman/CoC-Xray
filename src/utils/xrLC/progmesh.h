...
//			...then create all of your vertices preserving their
// order
PM_API void __cdecl PM_Init(u32 dwRelevantUV, u32 dwRelevantUVMASK,
                            u32 dwMinVertCount, float w_UV = 0.5f,
                            float w_Pos = 1.f, float w_Norm = 1.f,
                            float p_BorderHeuristic = 150.f,
                            float p_BorderHeuristicD = 0.07f,
                            float p_QualityHeuristic = 0.85f);

PM_API void __cdecl PM_CreateVertex(float x, float y, float z, int _id,
                                    P_UV *uv);

// SECOND:
//			You need to call this function passing indices (which
//will be reordered)
//			...and collaped, so initial model state has minimal
//number of vertices
