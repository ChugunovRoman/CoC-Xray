...
D3DVERTEXELEMENT9 decl[] = // 12+4+4+4+8=32
    {{0, 0, D3DDECLTYPE_FLOAT3, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_POSITION,
     ...
     {0, 20, D3DDECLTYPE_D3DCOLOR, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_BINORMAL,
      0},
     {0, 24, D3DDECLTYPE_SHORT4, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_TEXCOORD,
      0},
     D3DDECL_END()};
...
void export_geometry(xrMU_Model &mu_model) {
  ...
  // Begin building
  for (xrMU_Model::v_subdivs_it it = mu_model.m_subdivs.begin();
       it != mu_model.m_subdivs.end(); it++) {
    // Vertices
    {
      g_VB.Begin(D);
    ...
    // Indices
    g_IB.Register(LPWORD(&*it->ogf->data.faces.begin()),
                  LPWORD(&*it->ogf->data.faces.end()), &it->ib_id,
                  &it->ib_start);
