...
const float aht_max_edge =
    c_SS_maxsize / 2.5f; // 2.0f;			// 2 m
// const	float	aht_min_edge	= .2f;					//
// 20 cm const	float	aht_min_err		= 16.f/255.f; // ~10% error
...
public:
  CPrecalcBaseHemiThread(u32 ID, u32 from, u32 to)
      : CThread(ID), _from(from), _to(to) {
    R_ASSERT(from != u32(-1));
    R_ASSERT(to != u32(-1));
    R_ASSERT(from < to);
    R_ASSERT(from >= 0);
...
void CBuild::xrPhase_AdaptiveHT() {
  ...
  Status("Precalculating...");
  {
    ...
    //	V->normalFromAdj	();
    //	LightPoint			(&DB, lc_global_data()->RCAST_Model(), vC, V->P,
    //V->N, pBuild->L_static(), LP_dont_rgb+LP_dont_sun,0); 	vC.mul (0.5f);
    //V->C._set			(vC);
    //}
    ...
    u32 threads = u32(-1);
    u32 rest = u32(-1);
    get_intervals(8, lc_global_data()->g_vertices().size(), threads, stride,
                  rest);
    for (u32 thID = 0; thID < threads; thID++)
      precalc_base_hemi.start(xr_new<CPrecalcBaseHemiThread>(
          thID, thID * stride, thID * stride + stride));
    if (rest > 0)
      precalc_base_hemi.start(xr_new<CPrecalcBaseHemiThread>(
          threads, threads * stride, threads * stride + rest));
  ...
  // clear split flag from all faces + calculate normals
          ...
          u_Tesselate		(callback_edge_error,0,callback_vertex_hemi);
  // tesselate
  }
  */
  ...
  Status				("Exporting to SMF...");
  {
          string_path			fn;
          GSaveAsSMF
  (strconcat(fn,pBuild->path,"hemi_source.smf"));
  }
  */
}
void CollectProblematicFaces(const Face &F, int max_id,
                             xr_vector<Face *> &reult, Vertex **V1,
                             Vertex **V2) {
...
void tessalate_faces(xr_vector<Face *> &faces, Vertex *V1, Vertex *V2,
                     tesscb_face *cb_F, tesscb_vertex *cb_V) {
...
void CBuild::u_Tesselate(tesscb_estimator *cb_E, tesscb_face *cb_F,
  ...
  // Cleanup
  for (u32 I = 0; I < lc_global_data()->g_faces().size(); ++I)
    if (0 != lc_global_data()->g_faces()[I] &&
        lc_global_data()->g_faces()[I]->flags.bSplitted)
  ...
  lc_global_data()->g_faces().erase(
      std::remove(lc_global_data()->g_faces().begin(),
                  lc_global_data()->g_faces().end(), (Face *)0),
      lc_global_data()->g_faces().end());
  lc_global_data()->g_vertices().erase(
      std::remove(lc_global_data()->g_vertices().begin(),
                  lc_global_data()->g_vertices().end(), (Vertex *)0),
...
void CBuild::u_SmoothVertColors(int count) {
  for (int iteration = 0; iteration < count; ++iteration) {
    ...
    for (u32 it = 0; it < lc_global_data()->g_vertices().size(); ++it) {
      ...
      circle_vec.erase(std::unique(circle_vec.begin(), circle_vec.end()),
                       circle_vec.end());
