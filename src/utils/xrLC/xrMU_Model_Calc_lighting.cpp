...
//-----------------------------------------------------------------------
void xrMU_Model::calc_lighting(xr_vector<base_color> &dest, Fmatrix &xform,
                               CDB::MODEL *MDL, base_lighting &lights,
                               u32 flags) {
  // trans-map
  ...
  m_faces[I]->flags.bDisableShadowCast	= true;
  */
...
void xrMU_Model::calc_lighting() {
  ...
  calc_lighting(color, Fidentity, M, pBuild->L_static(),
                LP_dont_rgb + LP_dont_sun);
...
void o_test(int iA, int iB, int count, base_color *A, base_color *B, float &C,
            float &D) {
...
void xrMU_Reference::calc_lighting() {
  model->calc_lighting(
      color, xform, lc_global_data()->RCAST_Model(), pBuild->L_static(),
      (lc_global_data()->b_nosun() ? LP_dont_sun : 0) | LP_DEFAULT);
  ...
  // build data
  {
    ...
    for (u32 index = 0; index < 5; index++)
      o_test(4, index, color.size(), &model->color.front(), &color.front(),
             _s[index], _b[index]);
  }
}
