...
void CBuild::PreOptimize() {
  ...
  VMmin.set(bb.min);
  VMeps.set(VMscale.x / HDIM_X / 2, VMscale.y / HDIM_Y / 2,
            VMscale.z / HDIM_Z / 2);
  ...
  mem_Compact();
  clMsg("%d vertices removed. (%d left)",
        Vcount - lc_global_data()->g_vertices().size(),
        lc_global_data()->g_vertices().size());
  clMsg("%d faces removed. (%d left)",
        Fcount - lc_global_data()->g_faces().size(),
