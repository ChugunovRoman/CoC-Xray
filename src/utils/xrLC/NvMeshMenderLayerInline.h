...
IC void set_vertex(MeshMender::Vertex &out_vertex, const Vertex &in_veretex,
                   const Fvector2 Ftc) {
...
IC void set_face(Face &out_face, const MeshMender::Vertex in_vertices[3]) {
  for (u16 v = 0; v < 3; ++v) {
    ...
    out_face.basis_binormal[v].set(
        cv_vector(binormal, in_vertices[v].binormal));
  }
}

IC void set_vertex(MeshMender::Vertex &out_vertex,
                   const OGF_Vertex &in_vertex) {
