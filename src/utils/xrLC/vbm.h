...
public:
  ...
  // Methods
  bool is_empty() const {
    return vDcl.empty() && vContainers.empty() && R_DCL.empty() &&
           R_DATA.empty();
  }
  ...
  void End(u32 *dwContainerID, u32 *dwIndexStart) {
    ...
    for (u32 CID = 0; CID < vDcl.size(); CID++) {
      ...
      *dwIndexStart = vertices_already;
      vContainers[CID].insert(vContainers[CID].end(), R_DATA.begin(),
                              R_DATA.end());
      R_DCL.clear();
      R_DATA.clear();
      return;
  ...
  void Save(IWriter &fs) {
    ...
    for (u32 i = 0; i < vDcl.size(); i++) {
      ...
      fs.w(vDcl[i].begin(),
           vDcl[i].size() * sizeof(D3DVERTEXELEMENT9)); // Vertex format
