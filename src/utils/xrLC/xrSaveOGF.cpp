...
bool CBuild::IsOGFContainersEmpty() {
  return g_SWI.is_empty() && x_SWI.is_empty() && g_VB.is_empty() &&
         x_VB.is_empty() && g_IB.is_empty() && x_IB.is_empty();
}
...
void geom_batch_average(u32 verts, u32 faces) {
  // clMsg			("* batch[%d], %d verts, %d
  // faces",g_batch_count,verts,faces);
  g_batch_count++;
  g_batch_verts += verts;
  g_batch_faces += faces;
...
static void SaveGEOMs(LPCSTR fn, VBContainer &vb, IBContainer &ib,
                      SWIContainer &swi) {
...
void CBuild::SaveTREE(IWriter &fs) {
  ...
  for (xr_vector<OGF_Base *>::iterator it = g_tree.begin(); it != g_tree.end();
       it++) {
  ...
  fs.close_chunk();
  clMsg("Average: %d verts/%d faces, 50(%2.1f), 100(%2.1f), 500(%2.1f), "
        "1000(%2.1f), 5000(%2.1f)",
  ...
  for (xr_vector<LPCSTR>::iterator T = g_Shaders.begin(); T != g_Shaders.end();
       T++)
    fs.w_stringZ(*T);
  fs.close_chunk();
  // mem_Compact			();
