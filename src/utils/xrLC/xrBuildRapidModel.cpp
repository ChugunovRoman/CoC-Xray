...
void SaveUVM(LPCSTR fname, xr_vector<b_rc_face> &vm) {
  ...
  for (u32 v_idx = 0; v_idx < vm.size(); v_idx++) {
    ...
    xr_sprintf(tmp, "f %d %d [%3.2f,%3.2f]-[%3.2f,%3.2f]-[%3.2f,%3.2f]",
               rcf.dwMaterial, rcf.dwMaterialGame, rcf.t[0].x, rcf.t[0].y,
               rcf.t[1].x, rcf.t[1].y, rcf.t[2].x, rcf.t[2].y);
    W->w_string(tmp);
  }
  FS.w_close(W);
}
...
void CBuild::BuildRapid(BOOL bSaveForOtherCompilers) {
  ...
  /*
  clMsg					("Faces: original(%d), model(%d),
  ratio(%f)",
  ...
  */
  ...
  if (bSaveForOtherCompilers) {
    ...
    IWriter *MFS =
        FS.w_open(strconcat(sizeof(fn), fn, pBuild->path, "build.cform"));
    ...
    if (g_params().m_quality != ebqDraft) {
      if (keep_temp_files)
        SaveUVM(
            strconcat(sizeof(fn), fn, pBuild->path, "build_cform_source.uvm"),
            rc_faces);
    }
