...
public:
  static void calc_normals(vecVertex &vertices, vecFace &faces) {
    ...
    for (u32 I = 0; I < Vcount; I++) {
      ...
      std::sort(pTestVertex->m_adjacents.begin(),
                pTestVertex->m_adjacents.end());

      while (pTestVertex->m_adjacents.size()) {
        vecFace new_adj;
        itterate_adjacents_type::recurse_tri_params p(pTestVertex, new_adj,
                                                      sm_cos);
        itterate_adjacents_type::RecurseTri(
            0, p); // pTestVertex, new_adj, sm_cos );
    ...
    clMsg("%d vertices was duplicated 'cause of SM groups",
          vertices.size() - Vcount);
