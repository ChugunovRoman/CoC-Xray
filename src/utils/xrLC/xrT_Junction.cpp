...
IC float SqrDistance2Segment(const Fvector &P, const Fvector &A,
                             const Fvector &B) {
...
static void check(Vertex *vE1, Vertex *vE2, Vertex *vTEST) {
  if (_sqrt(SqrDistance2Segment(vTEST->P, vE1->P, vE2->P)) < 0.002f) {
    ...
    // VPUSH(vTEST->P)); else			clMsg	("ERROR. T-junction at
    // vertex [%3.1f,%3.1f,%3.1f]",	VPUSH(vTEST->P));
...
void CBuild::CorrectTJunctions() {
  ...
  clMsg("*** %d junctions and %d long edges found.", vecJunctions->size(),
        vecEdges->size());
