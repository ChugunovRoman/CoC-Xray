...
void SaveAsSMF(LPCSTR fname, CDB::CollectorPacked &CL) {
  ...
  for (u32 f_idx = 0; f_idx < CL.getTS(); f_idx++) {
    ...
    xr_sprintf(tmp, "f %d %d %d", t.verts[0] + 1, t.verts[2] + 1,
               t.verts[1] + 1);
    W->w_string(tmp);
  }
  FS.w_close(W);
...
IC u32 common_edge_idx(const MxFace &base_f, u32 base_edge_idx,
                       const MxFace &test_f) {
...
bool do_constrain(u32 base_edge_idx, u32 test_edg_idx, face_props &base_fprops,
  ...
  return (test_fprops.material != base_fprops.material) ||
         ...
         !do_connect_faces_by_faces_edge_flags(
             base_fprops.flags, test_fprops.flags, base_edge_idx, test_edg_idx);
}
...
void SimplifyCFORM(CDB::CollectorPacked &CL) {
  ...
  for (u32 f_idx = 0; f_idx < base_faces_cnt; f_idx++) {
    ...
    FPs[f_idx].set(t.material, t.sector,
                   Fvector().mknormal(*(CL.getV() + t.verts[0]),
                                      *(CL.getV() + t.verts[1]),
                                      *(CL.getV() + t.verts[2])),
                   CL.getfFlags(f_idx));
  }
  ...
  for (f_idx = 0; f_idx < mdl->face_count(); f_idx++) {
    if (mdl->face_is_valid(f_idx)) {
      ...
      CL.add_face(
          *((Fvector *)&mdl->vertex(F[0])), *((Fvector *)&mdl->vertex(F[1])),
  ...
  if (keep_temp_files) {
    string_path fn;
    SaveAsSMF(strconcat(sizeof(fn), fn, pBuild->path, "cform_optimized.smf"),
              CL);
  }
