...
u16 OGF::x_BuildVertex(x_vertex &V1) {
  for (itXV it = fast_path_data.vertices.begin();
       it != fast_path_data.vertices.end(); it++)
...
void OGF::_BuildFace(OGF_Vertex &V1, OGF_Vertex &V2, OGF_Vertex &V3,
  ...
  if (!F.Degenerate()) {
    ...
    if (data.vertices.size() > VertCount)
      data.vertices.erase(data.vertices.begin() + VertCount,
                          data.vertices.end());
  }
}
...
void OGF::adjacent_select(xr_vector<u32> &dest, xr_vector<bool> &vmark,
                          xr_vector<bool> &fmark) {
    ...
    if (dest.empty()) {
      ...
      for (u32 vid = 0; vid < 3; vid++) {
        ...
        for (u32 sid = 0; sid < dest.size(); sid++) {
          if (id == dest[sid]) {
            bConnected = TRUE; // this face shares at least one vertex with
                               // already selected faces
            break;
          }
        }
        if (bConnected)
...
void OGF::MakeProgressive(float metric_limit) {
...

  {
    ...
    for (u32 f_idx = 0; f_idx < data.faces.size(); f_idx++)
      VIPM_AppendFace(data.faces[f_idx].v[0], data.faces[f_idx].v[1],
                      data.faces[f_idx].v[2]);
    // clMsg("--- append end.");
    ...
    while (VR && VR->swr_records.size() > 0) {
      ...
      if (_metric < metric_limit) {
        progressive_clear();
        clMsg("* mesh simplified from [%4dv] to [%4dv], nf[%4d] ==> "
              "em[%0.2f]-discarded",
  ...
  //////////////////////////////////////////////////////////////////////////
  // FAST-PATH
  if (progressive_test() && fast_path_data.vertices.size() &&
    ...
    for (u32 f_idx = 0; f_idx < fast_path_data.faces.size(); f_idx++)
      VIPM_AppendFace(fast_path_data.faces[f_idx].v[0],
                      fast_path_data.faces[f_idx].v[1],
    ...
    if (0 == VR) {
      ...
      */
      ...
      fast_path_data.m_SWI.sw =
          xr_alloc<FSlideWindow>(fast_path_data.m_SWI.count);
