...
const	float	aht_max_edge	= c_SS_maxsize/2.5f;
// 2.0f;			// 2 m
...
int		callback_edge_longest	(Face* F)
{
        ...
        for (u32 e=0; e<3; e++)
        {
                Vertex					*V1,*V2;
                F->EdgeVerts			(e,&V1,&V2);
                float len				= V1->P.distance_to
(V2->P);	// len if (len<aht_max_edge)	continue; if (len>max_err)
                {
                        max_err = len;
                        max_id	= e;
                }
...
int		callback_edge_error		(Face* F)
{
        ...
        for (u32 e=0; e<3; e++)
        {
                ...
                F->EdgeVerts			(e,&V1,&V2);
                float len				= V1->P.distance_to
(V2->P);	// len if (len<aht_min_edge)	continue; if (len>max_err)
                {
                        max_err = len;
                        max_id	= e;
                }
        ...
        if (b1 && b2 && b3)		return	-1;		// don't touch
flat-shaded triangle
        else					return	max_id; // tesselate
longest edge
}
...
void	callback_vertex_hemi	(Vertex* V)
{
        ...
        DB.ray_options			(0);
        base_color_c			vC;
        LightPoint				(&DB, RCAST_Model, vC, V->P,
V->N, pBuild->L_static, LP_dont_rgb+LP_dont_sun,0); V->C._set
(vC);
}
...
{
        return 1 +
(std::lower_bound(g_vertices.begin(),g_vertices.end(),V)-g_vertices.begin());
}
...
void GSaveAsSMF					(LPCSTR fname)
{
        ...
        for (u32 f_idx=0; f_idx<g_faces.size(); f_idx++){
                ...
                xr_sprintf			(tmp,"f %d %d %d",
                        smfVertex(t->v[0]), smfVertex(t->v[2]),
smfVertex(t->v[1])
                        );
                W->w_string		(tmp);
        }
...
// clear split flag from all faces + calculate normals
                ...
                u_Tesselate		(callback_edge_longest,0,0);
// tesselate
        }
        ...
        Status			("Precalculating...");
        {
                ...
                FPU::m64r					();
                Status
("Precalculating : base hemisphere ..."); mem_Compact
(); Light_prepare				();
                ...
                for (u32 vit=0; vit<g_vertices.size(); vit++)
                {
                        ...
                        V->normalFromAdj	();
                        LightPoint			(&DB, RCAST_Model, vC,
V->P, V->N, pBuild->L_static, LP_dont_rgb+LP_dont_sun,0); V->C._set
(vC);
                }
        }
...
// clear split flag from all faces + calculate normals
                ...
                u_Tesselate
(callback_edge_error,0,callback_vertex_hemi);	// tesselate
        }
        ...
        // prepare model
        MxStdModel* mdl			=
xr_new<MxStdModel>(g_vertices.size(),g_faces.size());
        ...
        for (u32 f_idx=0; f_idx<g_faces.size(); f_idx++){
                Face*	t		= g_faces	[f_idx];
                mdl->add_face	(smfVertex(t->v[0])-1, smfVertex(t->v[1])-1,
smfVertex(t->v[2])-1);
        }
        ...
        slim->placement_policy	= MX_PLACE_OPTIMAL;//MX_PLACE_ENDPOINTS
MX_PLACE_ENDORMID MX_PLACE_OPTIMAL; slim->weighting_policy	=
MX_WEIGHT_AREA; slim->initialize		();
...
void CBuild::u_Tesselate	(tesscb_estimator* cb_E, tesscb_face* cb_F,
tesscb_vertex* cb_V)
{
        // main process
        FPU::m64r					();
...
(); u32		cnt_faces			= g_faces.size		(); for
(u32 I=0; I<g_faces.size(); I++)
        {
                Face* F					= g_faces[I];
                if (0==F)				continue;
                if (F->flags.bSplitted)	{
                        if (!F->flags.bLocked)	FacePool.destroy
(g_faces[I]); continue;
                }
                if (F->CalcArea()<EPS_L)	continue;

                Progress
(float(I)/float(g_faces.size())); int		max_id			=
cb_E	(F); if		(max_id<0)		continue;	// nothing
selected

                // now, we need to tesselate all faces which shares this
'problematic' edge
                ...
                {
                        Face* A					=
V1->adjacent[adj]; if (A->flags.bSplitted)	continue; if (A->VContains(V2))
...
(0==g_vertices[I]->adjacent.size())	VertexPool.destroy	(g_vertices[I]);
                        Status				("Working: %d verts
created, %d(now) / %d(was) ...",counter_create,g_vertices.size(),cnt_verts);
                        FlushLog			();
                }
                ...
                for (u32 af_it=0; af_it<adjacent.size(); af_it++)
                {
                        ...
                        VERIFY
(false==AF->flags.bSplitted); AF->flags.bSplitted	= true; _TCF&	atc
= AF->tc.front();
                        ...
                        {
                                // F1
                                F1->SetVertices		(AF->v[idB],
AF->v[id1],		V);
                                F1->AddChannel		(atc.uv[idB],
atc.uv[id1],	UV);
                                // F2
...
atc.uv[id1]);
                                // F2
                                F2->SetVertices		(AF->v[idB],
AF->v[id2],		V);
                                F2->AddChannel		(atc.uv[idB],
atc.uv[id2],	UV);
                        }

                        // Normals and checkpoint
                        F1->N	= AF->N;		if (cb_F)
cb_F(F1); F2->N	= AF->N;		if (cb_F)	cb_F(F2);

                        // don't destroy old face	(it can be used as
occluder during ray-trace)
...
g_faces[I]->flags.bSplitted)	FacePool.destroy	(g_faces[I]); for (u32
I=0; I<g_vertices.size(); I++)	if (0==g_vertices[I]->adjacent.size())
VertexPool.destroy	(g_vertices[I]); g_faces.erase
(std::remove(g_faces.begin(),g_faces.end(),(Face*)0),g_faces.end());
        g_vertices.erase
(std::remove(g_vertices.begin(),g_vertices.end(),(Vertex*)0),g_vertices.end());
        g_bUnregister		= true;
}
...
void CBuild::u_SmoothVertColors(int count)
{
        for (int iteration=0; iteration<count; iteration++)
        {
                ...
                for (u32 it=0; it<g_vertices.size(); it++)
                {
                        ...
                        std::sort
(circle.begin(),circle.end()); circle.erase
