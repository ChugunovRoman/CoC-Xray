...
//
NvEdgeInfo *NvStripifier::FindEdgeInfo(NvEdgeInfoVec &edgeInfos, int v0,
                                       int v1) {
...
//
NvFaceInfo *NvStripifier::FindOtherFace(NvEdgeInfoVec &edgeInfos, int v0,
                                        int v1, NvFaceInfo *faceInfo) {
...
void NvStripifier::BuildStripifyInfo(NvFaceInfoVec &faceInfos,
  ...
  for (u32 i = 0; i < numTriangles; i++) {
    ...
    // create the face info and add it to the list of faces, but only if this
    // exact face doesn't already
    //  exist in the list
    ...
    if (!AlreadyExists(faceInfo, faceInfos)) {
      ...
      if (edgeInfo01 == NULL) {
        ...
        if (edgeInfo01->m_face1 != NULL)
          ;
        // Msg("! WARNING: BuildStripifyInfo: > 2 triangles on an edge...
...
int NvStripifier::FindStartPoint(NvFaceInfoVec &faceInfos,
  ...
  for (int i = 0; i < faceInfos.size(); i++) {
    ...
    if (FindOtherFace(edgeInfos, faceInfos[i]->m_v0, faceInfos[i]->m_v1,
                      faceInfos[i]) == NULL)
      ctr++;
    if (FindOtherFace(edgeInfos, faceInfos[i]->m_v1, faceInfos[i]->m_v2,
                      faceInfos[i]) == NULL)
      ctr++;
    if (FindOtherFace(edgeInfos, faceInfos[i]->m_v2, faceInfos[i]->m_v0,
                      faceInfos[i]) == NULL)
      ctr++;
    if (ctr > 1)
      return i;
...
NvFaceInfo *NvStripifier::FindGoodResetPoint(NvFaceInfoVec &faceInfos,
  ...
  if (result == NULL) {
    ...
    if (bFirstTimeResetPoint) {
      // first time, find a face with few neighbors (look for an edge of the
      // mesh)
  ...
  for (int i = 0; i < faceVec.size(); i++) {
    if (!bv0) {
      if ((faceVec[i]->m_v0 == face->m_v0) ||
          (faceVec[i]->m_v1 == face->m_v0) || (faceVec[i]->m_v2 == face->m_v0))
        bv0 = true;
    }

    if (!bv1) {
      if ((faceVec[i]->m_v0 == face->m_v1) ||
          (faceVec[i]->m_v1 == face->m_v1) || (faceVec[i]->m_v2 == face->m_v1))
        bv1 = true;
    }

    if (!bv2) {
      if ((faceVec[i]->m_v0 == face->m_v2) ||
          (faceVec[i]->m_v1 == face->m_v2) || (faceVec[i]->m_v2 == face->m_v2))
        bv2 = true;
    }
...
void NvStripInfo::Build(NvEdgeInfoVec &edgeInfos, NvFaceInfoVec &faceInfos) {
  ...
  while (nextFace != NULL && !IsMarked(nextFace)) {
    // this tests to see if a face is "unique", meaning that its vertices aren't
    // already in the list
  ...
  // Combine the forward and backwards stripification lists and put into our own
  // face vector3
...
bool NvStripifier::FindTraversal(NvFaceInfoVec &faceInfos,
                                 NvEdgeInfoVec &edgeInfos, NvStripInfo *strip,
...
void NvStripifier::CreateStrips(const NvStripInfoVec &allStrips,
                                IntVec &stripIndices, const bool bStitchStrips,
...
void NvStripifier::Stripify(const WordVec &in_indices, const int in_cacheSize,
  ...
  minStripLength = in_minStripLength; // this is the strip size threshold below
                                      // which we dump the strip into a list
...
void NvStripifier::SplitUpStripsAndOptimize(NvStripInfoVec &allStrips,
  ...
  if (tempStrips2.size() != 0) {
    ...
    for (i = 0; i < tempStrips2.size(); i++) {
      ...
      float currCost =
          (float)numNeighbors / (float)tempStrips2[i]->m_faces.size();
      if (currCost < minCost) {
        minCost = currCost;
        firstIndex = i;
      }
...
void NvStripifier::FindAllStrips(NvStripInfoVec &allStrips,
  ...
  while (!done) {
    ...
    for (i = 0; i < numSamples; i++) {
      ...
      // otherwise, we shall now try experiments for starting on the 01,12, and
      // 20 edges
      ...
      NvStripInfo *strip01 = xr_new<NvStripInfo>(
          NvStripStartInfo(nextFace, edge01, true), stripId++, experimentId++);
      ...
      NvStripInfo *strip10 = xr_new<NvStripInfo>(
          NvStripStartInfo(nextFace, edge10, false), stripId++, experimentId++);
      ...
      NvStripInfo *strip12 = xr_new<NvStripInfo>(
          NvStripStartInfo(nextFace, edge12, true), stripId++, experimentId++);
      ...
      NvStripInfo *strip21 = xr_new<NvStripInfo>(
          NvStripStartInfo(nextFace, edge21, false), stripId++, experimentId++);
      ...
      NvStripInfo *strip20 = xr_new<NvStripInfo>(
          NvStripStartInfo(nextFace, edge20, true), stripId++, experimentId++);
      ...
      NvStripInfo *strip02 = xr_new<NvStripInfo>(
          NvStripStartInfo(nextFace, edge02, false), stripId++, experimentId++);
