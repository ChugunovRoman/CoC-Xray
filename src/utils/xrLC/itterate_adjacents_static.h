...
public:
  itterate_adjacents_params_static(const type_vertex *_pTestVertex,
                                   vecFace &_new_adj_vec, float _sm_cos)
...
private:
  ...
  IC static bool do_connect_faces(const type_face &start, const type_face &test,
                                  ...
                                  u16 test_common_edge_idx, float sm_cos) {
    if (g_using_smooth_groups) {
      if (g_smooth_groups_by_faces)
        ...
        return do_connect_faces_by_faces_edge_flags(
            start.sm_group, test.sm_group, start_common_edge_idx,
            test_common_edge_idx);
    } else {
