...
void xrStripify(xr_vector<u16> &indices, xr_vector<u16> &perturb,
  ...
  // Copy indices
  Memory.mem_copy(&*indices.begin(), xPGROUP[0].indices,
                  (u32)indices.size() * sizeof(u16));
...
try {
  ...
  // Copy faces
  CopyMemory
(&*x_faces.begin(),&*indices.begin(),(u32)indices.size()*sizeof(u16));
  ...
  */
  {
    // alternative stripification - faces
    {
      ...
      HRESULT rhr = D3DXOptimizeFaces(
          &fast_path_data.faces.front(), fast_path_data.faces.size(),
    ...
    // alternative stripification - vertices
    {
      ...
      for (u32 it = 0; it < fast_path_data.faces.size(); it++)
        for (u32 j = 0; j < 3; j++)
          fast_path_data.faces[it].v[j] =
              (u16)remap[fast_path_data.faces[it].v[j]];
      xr_free(remap);
    }
  }
  ...
  try {
    ...
    // Copy faces
    CopyMemory(&*data.faces.begin(), &*indices.begin(),
               (u32)indices.size() * sizeof(u16));
...
void OGF::DumpFaces() {
  ...
  for (u32 i = 0; i < data.faces.size(); i++)
    clMsg("face #%4d: %4d %4d %4d", i, int(data.faces[i].v[0]),
          int(data.faces[i].v[1]), int(data.faces[i].v[2]));
  clMsg("fast:");
