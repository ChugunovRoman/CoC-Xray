...
void statistics_collector::load_settings() {
  ...
  for (u32 i = 0; i != file_count; ++i) {
    LPCSTR key = NULL;
    LPCSTR value = NULL;
    if (m_wpn_collection->settings->r_line(CSV_SETTINGS, i, &key, &value) &&
        key) {
      csv_files::iterator new_file_iter =
          m_all_params
              .insert(
                  std::make_pair(shared_str(key), xr_new<params_collection>()))
              .first;
      if (value) {
...
void statistics_collector::save_files() {
  std::for_each(m_all_params.begin(), m_all_params.end(),
                std::bind1st(std::mem_fun<void, statistics_collector>(
                                 &statistics_collector::save_file),
                             this));
}
...
void statistics_collector::save_file(csv_files::value_type const &val) {
  ...
  for (xr_vector<shared_str>::const_iterator
           i = m_wpn_collection->all_weapons.begin(),
    ...
    for (params_collection::const_iterator param_i = val.second->begin(),
                                           param_ie = val.second->end();
         param_i != param_ie; ++param_i) {
      LPCSTR val = m_wpn_collection->priquel_config->r_string(i->c_str(),
                                                              param_i->c_str());
      if (!val)
        val = "";
