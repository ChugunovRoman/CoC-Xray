...
void weapon_collection::load_all_mp_weapons() {
  ...
  /*FS.update_path			(path_ltx, "$game_config$",
  "mp\\weapons_mp\\weapons_mp_for_work.ltx");
  work_mp_weapons			= xr_new<CInifileEx>(path_ltx, TRUE,
  TRUE, FALSE);

  FS.update_path			(path_ltx, "$game_config$",
  "mp\\weapons_mp\\ammo_mp_for_work.ltx");
  work_mp_ammo				= xr_new<CInifileEx>(path_ltx, TRUE,
  TRUE, FALSE);

  FS.update_path			(path_ltx, "$game_config$",
  "mp\\weapons_mp\\items_mp_for_work.ltx");
  work_mp_items			= xr_new<CInifileEx>(path_ltx, TRUE, TRUE,
  FALSE);

  FS.update_path			(path_ltx, "$game_config$",
  "mp\\weapons_mp\\outfit_mp_for_work.ltx");
  work_mp_outfits			= xr_new<CInifileEx>(path_ltx, TRUE,
  TRUE, FALSE);*/
  ...
  /*new_mp_weapons			= xr_new<CInifileEx>("new_weapons_mp.ltx",
  FALSE, FALSE, FALSE); new_mp_ammo				=
  xr_new<CInifileEx>("new_ammo_mp.ltx", FALSE, FALSE, FALSE); new_mp_items
  = xr_new<CInifileEx>("new_items_mp.ltx", FALSE, FALSE, FALSE);
  new_mp_outfits			=
  xr_new<CInifileEx>("new_outfit_mp.ltx", FALSE, FALSE, FALSE);*/

  CInifileEx::Sect &dm_base_cost =
      priquel_config->r_section("deathmatch_base_cost");
...
struct SymbolCountComparator
    : public std::binary_function<weapon_collection::tentity_extract_keys,
...
void read_arguments_to_set(xr_set<shared_str> &dest,
...
void weapon_collection::load_settings() {
  ...
  for (u32 param = 0; param < params_count; ++param) {
    ...
    new_config.insert(
        std::make_pair(shared_str(line), xr_vector<CInifileEx::Sect>()));
    if (larg) {
...
weapon_collection::textract_list::const_iterator
weapon_collection::get_extract_keys(char const *section_name) {
  for (weapon_collection::textract_list::const_iterator
           i = extract_list.begin(),
           ie = extract_list.end();
       i != ie; ++i) {
  ...
  for (xr_vector<shared_str>::const_iterator i = all_weapons.begin(); i != ie;
    ...
    for (u32 param = 0; param < params_count; ++param)
    {
            ...
            if (patch_arg)
            {
                    ...
                    {
                            std::cout << "Patch value is not equal: " <<
    patch_arg << std::endl; if (remember_yes_keys.find(shared_str(line)) !=
    remember_yes_keys.end())
                            {
                                    larg = patch_arg;
                                    std::cout << "Processing YES action...\r\n";
                            } else if (remember_no_keys.find(shared_str(line))
    != remember_no_keys.end())
                            {
                                    std::cout << "Processing NO action...\r\n";
                            } else
                            {
                                    std::cout << "Do you want to save patch
    value ? (y - single yes, Y - multiple yes, n - single no, N - multiple
    no):"; int ch = _getch(); if (ch == 'y')
                                    {
                                            larg = patch_arg;
                                    } else if (ch == 'Y')
                                    {
            ...
            std::cout << "Saving key: " << line << ", value: " << larg <<
    std::endl << std::endl; write_store->w_string(temp_section, line, larg);
    }*/
  }
}

char const *weapon_collection::try_extract_from_patch(char const *sect,
                                                      char const *line) {
  R_ASSERT(sect && line);
...
void weapon_collection::copy_params_ex(CInifileEx::Sect &dest,
                                       CInifileEx::Sect const &from,
                                       xr_set<shared_str> const &copy_keys) {
  std::cout << "Processing section: " << from.Name.c_str() << std::endl;
  for (CInifileEx::SectCIt i = from.Data.begin(), ie = from.Data.end(); i != ie;
       ++i) {
    if (copy_keys.find(i->first) != copy_keys.end()) {
      char const *larg =
          try_extract_from_patch(from.Name.c_str(), i->first.c_str());
      CInifileEx::Item temp_item;
      temp_item.first = i->first;
      temp_item.second =
          priquel_config->r_string(dest.Name.c_str(), temp_item.first.c_str());
      ...
      if (larg) {
        if (temp_item.second != larg) {
          ...
          if (remember_yes_keys.find(shared_str(temp_item.first)) !=
              remember_yes_keys.end()) {
            temp_item.second = larg;
            std::cout << "Processing YES action...\r\n";
          } else if (remember_no_keys.find(shared_str(temp_item.first)) !=
                     remember_no_keys.end()) {
            std::cout << "Processing NO action...\r\n";
          } else {
            std::cout << "Do you want to save patch value ? (y - single yes, Y "
                         "- multiple yes, n - single no, N - multiple no):";
            int ch = _getch();
            if (ch == 'y') {
              temp_item.second = larg;
            } else if (ch == 'Y') {
...
void weapon_collection::build_section(
    CInifileEx::Sect &dest, CInifileEx::Sect const &orig,
  ...
  for (xr_vector<shared_str>::const_iterator bi = orig.base_sections.begin(),
                                             bie = orig.base_sections.end();
       bi != bie; ++bi) {
    R_ASSERT2(priquel_config->section_exist(bi->c_str()),
              "base section not exist");
    CInifileEx::Sect &base_sect = priquel_config->r_section(bi->c_str());
    copy_params_ex(dest, base_sect, extract_from_base_keys);
  }

  for (CInifileEx::SectCIt i = orig.Data.begin(), ie = orig.Data.end(); i != ie;
       ++i) {
...
void weapon_collection::save_config_to_file(
    tnew_config_map::const_iterator cfg_iter) {
  ...
  for (tnew_config_map::mapped_type::const_iterator i = sect_collection.begin(),
    ...
    for (CInifileEx::SectCIt si = i->Data.begin(), sie = i->Data.end();
         si != sie; ++si) {
      sprintf_s(temp_buffer, "%4s%-32s = %-8s", " ", si->first.c_str(),
                si->second.c_str() ? si->second.c_str() : "");
      if (si->comment.c_str()) {
