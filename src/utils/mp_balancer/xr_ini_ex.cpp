...
CInifileEx::CInifileEx(LPCSTR szFileName, BOOL ReadOnly, BOOL bLoad,
                       BOOL SaveAtEnd) {
  m_file_name[0] = 0;
  m_flags.zero();
  if (szFileName)
...
void CInifileEx::Load(IReader *F, LPCSTR path) {
  ...
  while (!F->eof()) {
    ...
    } else if (str[0] && (str[0] == '[')) // new section ?
    {
      ...
      if (Current) {
        ...
        RootIt I = std::lower_bound(DATA.begin(), DATA.end(), *Current->Name,
                                    sect_pred);
        if ((I != DATA.end()) && ((*I)->Name == Current->Name))
          Debug.fatal(DEBUG_INFO, "Duplicate section '%s' found.",
                      *Current->Name);
            ...
            while (bInsideSTR) {
              ...
              R_ASSERT2(
                  xr_strlen(value_raw) + xr_strlen(str_add_raw) <
                      sizeof(value_raw),
                  make_string(
                      "Incorrect inifile format: section[%s], variable[%s]. "
  ...
  if (Current) {
    RootIt I =
        std::lower_bound(DATA.begin(), DATA.end(), *Current->Name, sect_pred);
    if ((I != DATA.end()) && ((*I)->Name == Current->Name))
...
void CInifileEx::save_as(IWriter &writer) {
  ...
  for (RootIt r_it = DATA.begin(); r_it != DATA.end(); ++r_it) {
    ...
    for (SectCIt s_it = (*r_it)->Data.begin(); s_it != (*r_it)->Data.end();
         ++s_it) {
      const Item &I = *s_it;
      if (*I.first) {
        if (*I.second) {
          ...
          if (*I.comment) {
            ...
            sprintf_s(temp, sizeof(temp), "%8s%-32s = %-32s ;%s", " ", *I.first,
                      val, *I.comment);
          } else
#endif
          {
            // only name and value
            sprintf_s(temp, sizeof(temp), "%8s%-32s = %-32s", " ", *I.first,
                      val);
          }
        } else {
#ifdef DEBUG
          if (*I.comment) {
            // name and comment
            sprintf_s(temp, sizeof(temp), "%8s%-32s = ;%s", " ", *I.first,
                      *I.comment);
          } else
#endif
          {
...
//--------------------------------------------------------------------------------------
CInifileEx::Sect &CInifileEx::r_section(const shared_str &S) {
  return r_section(*S);
}
BOOL CInifileEx::line_exist(const shared_str &S, const shared_str &L) {
  return line_exist(*S, *L);
}
u32 CInifileEx::line_count(const shared_str &S) { return line_count(*S); }
BOOL CInifileEx::section_exist(const shared_str &S) {
...
BOOL CInifileEx::r_line(const shared_str &S, int L, const char **N,
                        const char **V) {
...
#endif
  SectIt_ it =
      std::lower_bound(data.Data.begin(), data.Data.end(), *I.first, item_pred);
...
void CInifileEx::w_color(LPCSTR S, LPCSTR L, u32 V, LPCSTR comment) {
  string128 temp;
  sprintf_s(temp, sizeof(temp), "%d,%d,%d,%d", color_get_R(V), color_get_G(V),
            color_get_B(V), color_get_A(V));
  w_string(S, L, temp, comment);
}

void CInifileEx::w_ivector2(LPCSTR S, LPCSTR L, const Ivector2 &V,
                            LPCSTR comment) {
  string128 temp;
...
void CInifileEx::w_ivector3(LPCSTR S, LPCSTR L, const Ivector3 &V,
                            LPCSTR comment) {
  string128 temp;
...
void CInifileEx::w_ivector4(LPCSTR S, LPCSTR L, const Ivector4 &V,
                            LPCSTR comment) {
  string128 temp;
  sprintf_s(temp, sizeof(temp), "%d,%d,%d,%d", V.x, V.y, V.z, V.w);
  w_string(S, L, temp, comment);
}
void CInifileEx::w_fvector2(LPCSTR S, LPCSTR L, const Fvector2 &V,
                            LPCSTR comment) {
  string128 temp;
...
void CInifileEx::w_fvector3(LPCSTR S, LPCSTR L, const Fvector3 &V,
                            LPCSTR comment) {
  string128 temp;
...
void CInifileEx::w_fvector4(LPCSTR S, LPCSTR L, const Fvector4 &V,
                            LPCSTR comment) {
  string128 temp;
...
void CInifileEx::remove_line(LPCSTR S, LPCSTR L) {
  ...
  if (line_exist(S, L)) {
    ...
    SectIt_ A =
        std::lower_bound(data.Data.begin(), data.Data.end(), L, item_pred);
