...
static char const *help_msg =
    "Format: mp_configs_verifyer.exe [--file | --unpack | --io_filter | "
    "--help] [file name]\n"
    "Keys:\n"
...
void xrcore_log_cb(LPCSTR log_string) { printf("%s\n", log_string); };

void safe_verify(LPCSTR file_name, mp_anticheat::configs_verifyer &verifyer,
                 u8 *data, u32 const data_size, string256 &dst_reason) {
  __try {
...
void unpack_file(LPCSTR file_name) {
  ...
  u32 data_size =
      tmp_reader->length() - sizeof(u32); // first word is unpacket size ...
  ...
  u32 original_size =
      ppmd_decompress_mt(uncomp_data, uncomp_size, data, data_size, tmp_cb);

  if (uncomp_size != original_size) {
    printf(
        "ERROR: bad archive (original size and unpacked size are not equal)\n");
    xr_free(uncomp_data);
    xr_free(data);
    FS.r_close(tmp_reader);
    return;
