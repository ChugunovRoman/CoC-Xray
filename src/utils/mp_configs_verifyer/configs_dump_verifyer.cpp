...
bool const configs_verifyer::verify_dsign(u8 *data, u32 data_size,
  ...
  for (CInifile::SectCIt cit = sect_ptr->Data.begin(),
    ...
    if (tmp_active_param) {
      if (active_params.line_exist(sect_ptr->Name.c_str(), cit->first)) {
        real_value =
            active_params.r_string(sect_ptr->Name.c_str(), cit->first.c_str());
...
LPCSTR configs_verifyer::get_diff(CInifile &received, CInifile &active_params,
                                  string256 &dst_diff) {
  LPCSTR diff_str = NULL;
