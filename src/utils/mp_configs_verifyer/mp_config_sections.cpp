...
/*void mp_active_params::dump	(IAnticheatDumpable const * dumpable_obj, LPCSTR
sect_name_key, CInifile & dest_dumper)
{
        LPCSTR		obj_sect_name = "";
        if (dumpable_obj)
        {
                shared_str const tmp_obj_section =
dumpable_obj->GetAnticheatSectionName(); if (tmp_obj_section.size())
                {
        ...
        dest_dumper.w_string(active_params_section, sect_name_key,
obj_sect_name); if (dest_dumper.section_exist(obj_sect_name)) return;
