...
private:
  ...
  bool const verify_dsign(u8 *data, u32 data_size,
                          sha_checksum_t &sha_checksum);
  LPCSTR get_diff(CInifile &received, CInifile &active_params,
                  string256 &dst_diff);
  LPCSTR get_section_diff(CInifile::Sect *sect_ptr, CInifile &active_params,
                          string256 &dst_diff);
