...
public:
  ...
  IC void Remove(int i) {
    ...
    m = particles[ptrdiff_t(--p_count)]; // не менять правило удаления !!!
                                         // (dependence ParticleGroup)
                                         // Msg( "pDel() : %u" , p_count );
  }

  IC BOOL Add(const pVector &pos, const pVector &posB, const pVector &size,
              const pVector &rot, const pVector &vel, u32 color,
