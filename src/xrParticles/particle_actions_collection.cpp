...
void PAPI::PAAvoid::Execute(ParticleEffect *effect, const float dt,
                            float &tm_max) {
  ...
  case PDRectangle: {
    ...
    float det = 1 / (wz * u.x * v.y - wz * u.y * v.x - u.z * wx * v.y -
...
void PACopyVertexB::Execute(ParticleEffect *effect, const float dt,
                            float &tm_max) {
  u32 i;
...
void PAExplosion::Execute(ParticleEffect *effect, const float dt,
                          float &tm_max) {
  ...
  if (max_radiusSqr < P_MAXFLOAT) {
    for (u32 i = 0; i < effect->p_count - 1; i++) {
      ...
      if (tohimlenSqr < max_radiusSqr) {
        ...
        m.vel +=
            tohim * (magdt / (_sqrt(tohimlenSqr) * (tohimlenSqr + epsilon)));
      }
    }
  } else {
    for (u32 i = 0; i < effect->p_count - 1; i++) {
...
void PAGravitate::Execute(ParticleEffect *effect, const float dt,
                          float &tm_max) {
  float magdt = magnitude * dt;
  float max_radiusSqr = max_radius * max_radius;
    ...
    for (u32 i = 0; i < effect->p_count; i++) {
      ...
      for (u32 j = i + 1; j < effect->p_count; j++) {
        ...
        if (tohimlenSqr < max_radiusSqr) {
          // Compute force exerted between the two bodies
          pVector acc(tohim *
                      (magdt / (_sqrt(tohimlenSqr) * (tohimlenSqr + epsilon))));
...
void PAMatchVelocity::Execute(ParticleEffect *effect, const float dt,
                              float &tm_max) {
  float magdt = magnitude * dt;
  float max_radiusSqr = max_radius * max_radius;
...
void PAOrbitLine::Execute(ParticleEffect *effect, const float dt,
                          float &tm_max) {
  float magdt = magnitude * dt;
  float max_radiusSqr = max_radius * max_radius;
...
void PAOrbitPoint::Execute(ParticleEffect *effect, const float dt,
                           float &tm_max) {
  float magdt = magnitude * dt;
  float max_radiusSqr = max_radius * max_radius;
...
void PARandomAccel::Execute(ParticleEffect *effect, const float dt,
                            float &tm_max) {
  for (u32 i = 0; i < effect->p_count; i++) {
    Particle &m = effect->particles[i];
...
void PARandomDisplace::Execute(ParticleEffect *effect, const float dt,
                               float &tm_max) {
  for (u32 i = 0; i < effect->p_count; i++) {
    Particle &m = effect->particles[i];
...
void PARandomVelocity::Execute(ParticleEffect *effect, const float dt,
                               float &tm_max) {
  for (u32 i = 0; i < effect->p_count; i++) {
    Particle &m = effect->particles[i];
...
void PASinkVelocity::Execute(ParticleEffect *effect, const float dt,
                             float &tm_max) {
  // Must traverse list in reverse order so Remove will work
  for (int i = effect->p_count - 1; i >= 0; i--) {
    Particle &m = effect->particles[i];
...
void PASpeedLimit::Execute(ParticleEffect *effect, const float dt,
                           float &tm_max) {
...
void PATargetColor::Execute(ParticleEffect *effect, const float dt,
                            float &tm_max) {
...
void PATargetSize::Execute(ParticleEffect *effect, const float dt,
                           float &tm_max) {
...
void PATargetRotate::Execute(ParticleEffect *effect, const float dt,
                             float &tm_max) {
  float scaleFac = scale * dt;
...
void PATargetVelocity::Execute(ParticleEffect *effect, const float dt,
                               float &tm_max) {
  float scaleFac = scale * dt;
...
void PATurbulence::Execute(ParticleEffect *effect, const float dt,
                           float &tm_max) {
#ifdef _GPA_ENABLED
  ...
  for (u32 i = 0; i < nWorkers; ++i) {
    ...
    tesParams[i].p_to =
        (i == (nWorkers - 1)) ? p_cnt : (tesParams[i].p_from + nStep);
