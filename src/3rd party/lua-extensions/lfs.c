...
static int _file_lock(lua_State *L, FILE *fh, const char *mode,
  ...
  /* lkmode valid values are:
     ...
     program immediately tries again after 1 second. If, after 10 attempts, the
     bytes cannot be locked, the constant returns an error. LK_NBLCK   Locks the
     specified bytes. If the bytes cannot be locked, the constant returns an
...
static int make_dir(lua_State *L) {
  ...
  fail = mkdir(path, S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IXGRP |
                         S_IROTH | S_IXOTH);
#endif
  if (fail) {
    lua_pushnil(L);
...
static void set_info(lua_State *L) {
  ...
  lua_pushliteral(L, "LuaFileSystem is a Lua library developed to complement "
