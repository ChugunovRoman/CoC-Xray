...
xr_dsa::xr_dsa(u8 const p[public_key_length], u8 const q[private_key_length],
...
shared_str const xr_dsa::sign(private_key_t const &priv_key, u8 const *data,
                              u32 const data_size) {
...
bool xr_dsa::verify(public_key_t const &pub_key, u8 const *data,
                    u32 const data_size, shared_str const &dsign) {
...
void xr_dsa::generate_params() {
  ...
  VERIFY(DSA_verify(0, debug_bad_digest, sizeof(debug_bad_digest), sig, siglen,
                    tmp_dsa_params) == 0);
