...
static void compute_invM_JT(int m, dRealMutablePtr J, dRealMutablePtr iMJ,
                            int *jb, dxBody *const *body, dRealPtr invI) {
  int i, j;
...
static void multiply_J(int m, dRealMutablePtr J, int *jb, dRealMutablePtr in,
                       dRealMutablePtr out) {
  int i, j;
  dRealPtr J_ptr = J;
  for (i = 0; i < m; i++) {
...
static void SOR_LCP(int m, int nb, dRealMutablePtr J, int *jb,
                    dxBody *const *body, dRealPtr invI, dRealMutablePtr lambda,
                    dRealMutablePtr fc, dRealMutablePtr b, dRealMutablePtr lo,
                    dRealMutablePtr hi, dRealPtr cfm, int *findex,
                    dxQuickStepParameters *qs) {
  const int num_iterations = qs->num_iterations;
...
void dxQuickStepper(dxWorld *world, dxBody *const *body, int nb,
  ...
  for (i = 0; i < nb; i++) {
    ...
    if (!(dValid(b->tacc[0]) && dValid(b->tacc[1]) && dValid(b->tacc[2]))) {
      ...
      dDebug(d_ERR_UASSERT, " (%s:%d \n tmp %f,%f,%f \n avel %f,%f,%f)",
             __FILE__, __LINE__, tmp[0], tmp[1], tmp[2], b->avel[0], b->avel[1],
             b->avel[2]);
    }
  ...
  // unbounded variables). joints with m=0 are inactive and are removed from the
  // joints array entirely, so that the code that follows does not consider
  // them.
  ...
  if (m > 0) {
    ...
    //   l1 l1 l1 a1 a1 a1 l2 l2 l2 a2 a2 a2  }-- jacobian for joint 0, body 1
    //   and body 2 (3 rows) l1 l1 l1 a1 a1 a1 l2 l2 l2 a2 a2 a2 / l1 l1 l1 a1
    //   a1 a1 l2 l2 l2 a2 a2 a2 }--- jacobian for joint 1, body 1 and body 2 (3
    //   rows) etc...
    //
    ...
    for (i = 0; i < nb; i++) {
      ...
      for (j = 0; j < 3; j++)
        tmp1[i * 6 + j] =
            body[i]->facc[j] * body_invMass + body[i]->lvel[j] * stepsize1;
