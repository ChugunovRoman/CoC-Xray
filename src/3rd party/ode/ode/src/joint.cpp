...
static inline void setBall2(dxJoint *joint, dxJoint::Info2 *info,
                            dVector3 anchor1, dVector3 anchor2, dVector3 axis,
                            dReal erp1) {
  // anchor points in global coordinates with respect to body PORs.
  dVector3 a1, a2;
...
static void setFixedOrientation(dxJoint *joint, dxJoint::Info2 *info,
...
static void setAnchors(dxJoint *j, dReal x, dReal y, dReal z, dVector3 anchor1,
                       dVector3 anchor2) {
  if (j->node[0].body) {
    dReal q[4];
...
static void setAxes(dxJoint *j, dReal x, dReal y, dReal z, dVector3 axis1,
                    dVector3 axis2) {
  if (j->node[0].body) {
    dReal q[4];
    q[0] = x;
...
static dReal getHingeAngleFromRelativeQuat(dQuaternion qrel, dVector3 axis) {
  ...
  dReal cost2 = qrel[0];
  dReal sint2 =
      dSqrt(qrel[1] * qrel[1] + qrel[2] * qrel[2] + qrel[3] * qrel[3]);
  dReal theta =
...
int dxJointLimitMotor::addLimot(dxJoint *joint, dxJoint::Info2 *info, int row,
  ...
  if (powered || limit) {
    ...
    if (!rotational && joint->node[1].body) {
      dVector3 c;
      c[0] = REAL(0.5) *
             (joint->node[1].body->pos[0] - joint->node[0].body->pos[0]);
      c[1] = REAL(0.5) *
             (joint->node[1].body->pos[1] - joint->node[0].body->pos[1]);
      c[2] = REAL(0.5) *
             (joint->node[1].body->pos[2] - joint->node[0].body->pos[2]);
      dCROSS(ltd, =, c, ax1);
      ...
      if (!limit) {
        ...
        if (rotational) {
          dBodyAddTorque(joint->node[0].body, -fm * ax1[0], -fm * ax1[1],
                         -fm * ax1[2]);
          if (joint->node[1].body)
            dBodyAddTorque(joint->node[1].body, fm * ax1[0], fm * ax1[1],
                           fm * ax1[2]);
        } else {
          dBodyAddForce(joint->node[0].body, -fm * ax1[0], -fm * ax1[1],
                        -fm * ax1[2]);
          if (joint->node[1].body) {
            dBodyAddForce(joint->node[1].body, fm * ax1[0], fm * ax1[1],
                          fm * ax1[2]);
...
extern "C" void dJointSetBallAnchor(dxJointBall *joint, dReal x, dReal y,
                                    dReal z) {
...
static void hingeGetInfo1(dxJointHinge *j, dxJoint::Info1 *info) {
  ...
  if ((j->limot.lostop >= -M_PI || j->limot.histop <= M_PI) &&
    ...
    dReal angle =
        getHingeAngle(j->node[0].body, j->node[1].body, j->axis1, j->qrel);
...
extern "C" void dJointSetHingeAnchor(dxJointHinge *joint, dReal x, dReal y,
                                     dReal z) {
  dUASSERT(joint, "bad joint argument");
...
extern "C" void dJointSetHingeAxis(dxJointHinge *joint, dReal x, dReal y,
                                   dReal z) {
  dUASSERT(joint, "bad joint argument");
  dUASSERT(joint->vtable == &__dhinge_vtable, "joint is not a hinge");
...
extern "C" void dJointSetHingeParam(dxJointHinge *joint, int parameter,
                                    dReal value) {
  dUASSERT(joint, "bad joint argument");
  dUASSERT(joint->vtable == &__dhinge_vtable, "joint is not a hinge");
...
extern "C" void dJointSetSliderAxis(dxJointSlider *joint, dReal x, dReal y,
                                    dReal z) {
  int i;
  dUASSERT(joint, "bad joint argument");
  dUASSERT(joint->vtable == &__dslider_vtable, "joint is not a slider");
...
extern "C" void dJointSetSliderParam(dxJointSlider *joint, int parameter,
                                     dReal value) {
  dUASSERT(joint, "bad joint argument");
  dUASSERT(joint->vtable == &__dslider_vtable, "joint is not a slider");
  joint->limot.set(parameter, value);
...
#define HINGE2_GET_AXIS_INFO(axis, sin_angle, cos_angle)                       \
  ...
  dCROSS(axis, =, ax1, ax2);                                                   \
  sin_angle =                                                                  \
      dSqrt(axis[0] * axis[0] + axis[1] * axis[1] + axis[2] * axis[2]);        \
...
extern "C" void dJointSetHinge2Anchor(dxJointHinge2 *joint, dReal x, dReal y,
                                      dReal z) {
  dUASSERT(joint, "bad joint argument");
...
extern "C" void dJointSetHinge2Axis1(dxJointHinge2 *joint, dReal x, dReal y,
                                     dReal z) {
  dUASSERT(joint, "bad joint argument");
  dUASSERT(joint->vtable == &__dhinge2_vtable, "joint is not a hinge2");
  if (joint->node[0].body) {
...
extern "C" void dJointSetHinge2Axis2(dxJointHinge2 *joint, dReal x, dReal y,
                                     dReal z) {
  dUASSERT(joint, "bad joint argument");
  dUASSERT(joint->vtable == &__dhinge2_vtable, "joint is not a hinge2");
  if (joint->node[1].body) {
...
extern "C" void dJointSetHinge2Param(dxJointHinge2 *joint, int parameter,
                                     dReal value) {
  dUASSERT(joint, "bad joint argument");
  dUASSERT(joint->vtable == &__dhinge2_vtable, "joint is not a hinge2");
...
extern "C" void dJointAddHinge2Torques(dxJointHinge2 *joint, dReal torque1,
                                       dReal torque2) {
...
static void getUniversalAxes(dxJointUniversal *joint, dVector3 ax1,
                             dVector3 ax2) {
...
extern "C" void dJointSetUniversalAnchor(dxJointUniversal *joint, dReal x,
                                         dReal y, dReal z) {
  dUASSERT(joint, "bad joint argument");
...
extern "C" void dJointSetUniversalAxis1(dxJointUniversal *joint, dReal x,
                                        dReal y, dReal z) {
  dUASSERT(joint, "bad joint argument");
  dUASSERT(joint->vtable == &__duniversal_vtable, "joint is not a universal");
  if (joint->flags & dJOINT_REVERSE)
...
extern "C" void dJointSetUniversalAxis2(dxJointUniversal *joint, dReal x,
                                        dReal y, dReal z) {
  dUASSERT(joint, "bad joint argument");
  dUASSERT(joint->vtable == &__duniversal_vtable, "joint is not a universal");
  if (joint->flags & dJOINT_REVERSE)
...
extern "C" void dJointSetUniversalParam(dxJointUniversal *joint, int parameter,
                                        dReal value) {
  dUASSERT(joint, "bad joint argument");
  dUASSERT(joint->vtable == &__duniversal_vtable, "joint is not a universal");
  if ((parameter & 0xff00) == 0x100) {
...
extern "C" void dJointSetAMotorAxis(dxJointAMotor *joint, int anum, int rel,
  ...
  dUASSERT(joint->vtable == &__damotor_vtable, "joint is not an amotor");
  dUASSERT(
      !(!joint->node[1].body && (joint->flags & dJOINT_REVERSE) && rel == 1),
      "no first body, can't set axis rel=1");
  dUASSERT(
      !(!joint->node[1].body && !(joint->flags & dJOINT_REVERSE) && rel == 2),
...
extern "C" void dJointAddAMotorTorques(dxJointAMotor *joint, dReal torque1,
                                       dReal torque2, dReal torque3) {
  dVector3 axes[3];
  dAASSERT(joint);
  dUASSERT(joint->vtable == &__damotor_vtable, "joint is not an amotor");
  ...
  if (joint->node[0].body) {
    if (joint->node[1].body) {
      /* dQMultiply1
       ...
       ofs[4]; for (i=0; i<4; i++) ofs[i] = joint->node[0].body->pos[i]; for
       (i=0; i<4; i++) ofs[i] -= joint->node[1].body->pos[i]; dMULTIPLY1_331
       (joint->offset,joint->node[0].body->R,ofs);*/
    } else {
      // set joint->qrel to the transpose of the first body's q
