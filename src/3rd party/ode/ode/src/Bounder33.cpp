...
Lcp33::Lcp33(dReal *aA, int askip, dReal *ahi, dReal *alo, dReal *ab,
             dReal *ax) {
  sourceA = aA;
  hi = ahi;
  lo = alo;
...
void Lcp33::UpdateX() {
  ...
  for (int i = NBn; i < MSIZE; ++i) {
    dReal *row = RowI(i);
    x[index[i]] = w[0] * EinRowA(row, 0) + w[1] * EinRowA(row, 1) +
                  w[2] * EinRowA(row, 2);
  }
...
void Lcp33::FillA() {
  ...
  EA(1, 0) =
      EA(0, 1); //-fDetInv * ( EsA(1,0) * EsA(2,2) - EsA(1,2) * EsA(2,0) );////
  EA(1, 1) = fDetInv * (EsA(0, 0) * EsA(2, 2) - EsA(0, 2) * EsA(2, 0));
  EA(1, 2) = -fDetInv * (EsA(0, 0) * EsA(1, 2) - EsA(0, 2) * EsA(1, 0));

  EA(2, 0) =
      EA(0, 2); // fDetInv * ( EsA(1,0) * EsA(2,1) - EsA(1,1) * EsA(2,0) );//
  EA(2, 1) =
      EA(1, 2); // -fDetInv * ( EsA(0,0) * EsA(2,1) - EsA(0,1) * EsA(2,0) );//
...
void dSolveLCP33(int n, dReal *A, dReal *x, dReal *b, dReal *w, int nub,
