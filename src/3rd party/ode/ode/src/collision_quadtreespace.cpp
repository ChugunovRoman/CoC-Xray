...
public:
  ...
  void Create(const dVector3 Center, const dVector3 Extents, Block *Parent,
              int Depth, Block *&Blocks);

  void Collide(void *UserData, dNearCallback *Callback);
  void Collide(dGeomID Object, dGeomID g, void *UserData,
...
void Block::Create(const dVector3 Center, const dVector3 Extents, Block *Parent,
                   int Depth, Block *&Blocks) {
  GeomCount = 0;
  First = 0;
  ...
  if (Depth > 0) {
    ...
    for (int i = 0; i < SPLITAXIS; i++) {
      for (int j = 0; j < SPLITAXIS; j++) {
        ...
        Children[Index].Create(ChildCenter, ChildExtents, this, Depth - 1,
                               Blocks);
      }
    }
  } else
...
struct dxQuadTreeSpace : public dxSpace {
  ...
  dxQuadTreeSpace(dSpaceID _space, dVector3 Center, dVector3 Extents,
                  int Depth);
  ~dxQuadTreeSpace();
...
dxQuadTreeSpace::dxQuadTreeSpace(dSpaceID _space, dVector3 Center,
...
PARENTRECURSE:
                          for (int& i = CurrentChild[CurrentLevel]; i < SPLITS;
i++){ if (CurrentBlock->Children[i].GeomCount == 0){ continue;
                                  }
...
void dxQuadTreeSpace::collide2(void *UserData, dxGeom *g1,
                               dNearCallback *Callback) {
...
dSpaceID dQuadTreeSpaceCreate(dxSpace *space, dVector3 Center, dVector3 Extents,
                              int Depth) {
