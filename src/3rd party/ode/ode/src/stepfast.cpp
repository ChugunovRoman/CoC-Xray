...
static void MultiplyAdd2_sym_p8p(dReal *A, dReal *B, dReal *C, int p,
                                 int Askip) {
  int i, j;
  dReal sum, *aa, *ad, *bb, *cc;
  dIASSERT(p > 0 && A && B && C);
...
void dInternalStepFast(dxWorld *world, dxBody *body[2], dReal *GI[2],
                       dReal *GinvI[2], dxJoint *joint, dxJoint::Info1 info,
                       dxJoint::Info2 Jinfo, dReal stepsize) {
  int i, j, k;
#ifdef TIMING
  ...
  for (i = 0; i < 2; i++) {
    ...
    for (j = 0; j < 3; j++)
      tmp1[i * 8 + j] =
          body[i]->facc[j] * body[i]->invMass + body[i]->lvel[j] * stepsize1;
...
inline void SwapJoints(int i, int j, dxJoint **joints, dxJoint::Info1 *info,
                       dxJoint::Info2 *Jinfo) {
...
void dInternalStepIslandFast(dxWorld *world, dxBody *const *bodies, int nb,
                             dxJoint **joints, int nj, dReal stepsize,
                             int maxiterations) {
#ifdef TIMING
  ...
  for (iter = 0; iter < maxiterations; iter++) {
    ...
    for (b = 0; b < nb; b++) {
      ...
      // global frame, and compute the rotational force and add it to the torque
      ...
      if ((body->flags & dxBodyNoGravity) == 0) {
        body->facc[0] =
            saveFacc[b * 4 + 0] + body->mass.mass * world->gravity[0];
        body->facc[1] =
            saveFacc[b * 4 + 1] + body->mass.mass * world->gravity[1];
        body->facc[2] =
            saveFacc[b * 4 + 2] + body->mass.mass * world->gravity[2];
        body->facc[3] = 0;
      }
    }
    ...
    for (j = 0; j < nj; j++) {
      ...
      if (info[j].m > 0) {
        ...
        case dJointTypeContact:
          if (info[j].m == 3) {
            dInternalStepJointContact(world, bodyPair, GIPair, GinvIPair, joint,
                                      info[j], Jinfo[j], ministep);
            // dInternalStepFast (world, bodyPair, GIPair, GinvIPair, joint,
            // info[j], Jinfo[j], ministep);
        ...
        default:
          dInternalStepFast(world, bodyPair, GIPair, GinvIPair, joint, info[j],
                            Jinfo[j], ministep);
        }
      }
    }
...
// one.
static void processIslandsFast(dxWorld *world, dReal stepsize,
                               int maxiterations) {
...
static void processIslandsFast(dxWorld *world, dReal stepsize,
  ...
  quit:
    ...
    if (jcount > 3)
      dInternalStepIslandFast(world, body, bcount, joint, jcount, stepsize,
                              maxiterations);
    else
