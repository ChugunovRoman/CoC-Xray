...
static void swapRowsAndCols(ATYPE A, int n, int i1, int i2, int nskip,
  ...
  int i;
  dIASSERT(A && n > 0 && i1 >= 0 && i2 >= 0 && i1 < n && i2 < n && nskip >= n &&
           i1 < i2);
...
static void swapProblem(ATYPE A, dReal *x, dReal *b, dReal *w, dReal *lo,
                        dReal *hi, int *p, int *state, int *findex, int n,
...
struct dLCP {
  ...
  dLCP(int _n, int _nub, dReal *_Adata, dReal *_x, dReal *_b, dReal *_w,
       dReal *_lo, dReal *_hi, dReal *_L, dReal *_d, dReal *_Dell, dReal *_ell,
       dReal *_tmp, int *_state, int *_findex, int *_p, int *_C, dReal **Arows);
...
dLCP::dLCP(int _n, int _nub, dReal *_Adata, dReal *_x, dReal *_b, dReal *_w,
           dReal *_lo, dReal *_hi, dReal *_L, dReal *_d, dReal *_Dell,
           dReal *_ell, dReal *_tmp, int *_state, int *_findex, int *_p,
           int *_C, dReal **Arows) {
...
void dSolveLCPBasic(int n, dReal *A, dReal *x, dReal *b, dReal *w, int nub,
                    dReal *lo, dReal *hi) {
  ...
  dLCP lcp(n, 0, A, x, b, w, tmp, tmp, L, d, Dell, ell, tmp, dummy, dummy, p, C,
           Arows);
  nub = lcp.getNub();
...
void dSolveLCP(int n, dReal *A, dReal *x, dReal *b, dReal *w, int nub,
  ...
  dLCP lcp(n, nub, A, x, b, w, lo, hi, L, d, Dell, ell, delta_w, state, findex,
           p, C, Arows);
  nub = lcp.getNub();
      ...
      for (;;) {
        ...
        for (k = nub; k < lcp.numC(); k++) {
          if (delta_x[lcp.indexC(k)] < 0 && lo[lcp.indexC(k)] > -dInfinity) {
            dReal s2 =
                (lo[lcp.indexC(k)] - x[lcp.indexC(k)]) / delta_x[lcp.indexC(k)];
            if (s2 < s) {
              s = s2;
              cmd = 5;
              si = lcp.indexC(k);
          ...
          if (delta_x[lcp.indexC(k)] > 0 && hi[lcp.indexC(k)] < dInfinity) {
            dReal s2 =
                (hi[lcp.indexC(k)] - x[lcp.indexC(k)]) / delta_x[lcp.indexC(k)];
            if (s2 < s) {
              s = s2;
              cmd = 6;
              si = lcp.indexC(k);
    ...
    */
    ...
    for (i = 0; i < n; i++) {
      if (x[i] == lo[i] && w[i] >= 0) {
        ...
        dDebug(0, "FAILED: i=%d x=%.4e w=%.4e lo=%.4e hi=%.4e", i, x[i], w[i],
               lo[i], hi[i]);
      }
    }
