...
bool CxImageJPG::CxExifInfo::process_EXIF(unsigned char *CharBuf,
  ...
  if (FirstOffset > 8) {
    if (!ProcessExifDir(CharBuf + 14 + FirstOffset - 8, CharBuf + 6, length - 6,
                        m_exifinfo, &LastExifRefd))
      return false;
  }
...
--------------------------------------------------------------------------*/
bool CxImageJPG::CxExifInfo::ProcessExifDir(unsigned char *DirStart,
      ...
      */
      if (m_exifinfo->ApertureFNumber == 0) {
        m_exifinfo->ApertureFNumber =
            (float)exp(ConvertAnyFormat(ValuePtr, Format) * log(2.0f) * 0.5);
      }
      break;
      ...
      */
      if (m_exifinfo->ExposureTime == 0) {
        m_exifinfo->ExposureTime =
            (float)(1 / exp(ConvertAnyFormat(ValuePtr, Format) * log(2.0f)));
      }
      break;
...
void CxImageJPG::CxExifInfo::process_SOFn(const BYTE *Data, int marker) {
  ...
  // if (ShowTags) printf("JPEG image is %uw * %uh, %d color components, %d bits
  // per sample\n",
 ...
 * this will work only on a CxImageJPG object, if the image originally has valid
 EXIF data \verbatim CxImageJPG jpg; CxIOFile in,out;
