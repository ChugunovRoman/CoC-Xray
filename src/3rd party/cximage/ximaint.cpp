 ...
 *  \author ***bd*** 2.2004
 */
void CxImage::OverflowCoordinates(long &x, long &y,
 ...
 * \author ***bd*** 2.2004
 */
void CxImage::OverflowCoordinates(float &x, float &y,
                                  OverflowMethod const ofMethod) {
 ...
 * \author ***bd*** 2.2004
 */
RGBQUAD CxImage::GetPixelColorWithOverflow(long x, long y,
                                           OverflowMethod const ofMethod,
    ...
    case OM_TRANSPARENT:
      ...
      if (AlphaIsValid()) {
       ...
       // no alpha transparency
        if (GetTransIndex() >= 0) {
          color = GetTransColor(); // single color transparency enabled (return
                                   // transparent color)
        }                          // if
#if CXIMAGE_SUPPORT_ALPHA
      } // if
 ...
 *  \author ***bd*** 2.2004
 */
RGBQUAD CxImage::GetPixelColorInterpolated(float x, float y,
  ...
  case IM_NEAREST_NEIGHBOUR:
    return GetPixelColorWithOverflow((long)(x + 0.5f), (long)(y + 0.5f),
                                     ofMethod, rplColor);
  default: {
    ...
    if (xi < -1 || xi >= head.biWidth || yi < -1 ||
      ...
      case OM_TRANSPARENT:
      case OM_BACKGROUND:
        // we don't need to interpolate anything with all points outside in this
        // case
    ...
    if ((xi + 1) < head.biWidth && xi >= 0 && (yi + 1) < head.biHeight &&
        yi >= 0 && head.biClrUsed == 0) {
      // all pixels are inside RGB24 image... optimize reading (and use fixed
      // point arithmetic)
      ...
      // default (slower) way to get pixels (not RGB24 or some pixels out of
      // borders)
      ...
      color.rgbGreen = (BYTE)(a * rgb11.rgbGreen + b * rgb21.rgbGreen +
                              c * rgb12.rgbGreen + d * rgb22.rgbGreen);
      color.rgbBlue = (BYTE)(a * rgb11.rgbBlue + b * rgb21.rgbBlue +
                             c * rgb12.rgbBlue + d * rgb22.rgbBlue);
#if CXIMAGE_SUPPORT_ALPHA
      if (AlphaIsValid())
        color.rgbReserved =
            (BYTE)(a * rgb11.rgbReserved + b * rgb21.rgbReserved +
                   c * rgb12.rgbReserved + d * rgb22.rgbReserved);
      else
#endif
      { // Alpha not supported or no alpha at all
        color.rgbReserved = 0;
  ...
  case IM_POWER:
    ...
    if (((xi + 2) < 0) || ((xi - 1) >= head.biWidth) || ((yi + 2) < 0) ||
        ((yi - 1) >= head.biHeight)) { // all points are outside bounds?:
      switch (ofMethod) {
      case OM_COLOR:
      case OM_TRANSPARENT:
      case OM_BACKGROUND:
    ...
    rr = gg = bb = aa = 0;
    if (((xi + 2) < head.biWidth) && xi >= 1 && ((yi + 2) < head.biHeight) &&
        (yi >= 1) && !IsIndexed()) {
      // optimized interpolation (faster pixel reads) for RGB24 images with all
      // pixels inside bounds
      BYTE *pxptr, *pxptra;
...
#if CXIMAGE_SUPPORT_ALPHA
        if (AlphaIsValid()) {
          // alpha is supported and valid (optimized bicubic int. for image with
          // alpha)
...
#endif
        // alpha not supported or valid (optimized bicubic int. for no alpha
        // channel)
        {
          kernel = kernelyc * kernelx[0];
          bb += kernel * (*pxptr++);
      ...
      // slower more flexible interpolation for border pixels and paletted
      // images
      RGBQUAD rgbs;
      for (yii = yi - 1; yii < yi + 3; yii++) {
        kernelyc = kernely[yii - (yi - 1)];
  ...
  case IM_LANCZOS:
    ...
    if (((xi + 6) < 0) || ((xi - 5) >= head.biWidth) || ((yi + 6) < 0) ||
        ((yi - 5) >= head.biHeight)) {
    ...
    if (((xi + 6) < head.biWidth) && ((xi - 5) >= 0) &&
        ((yi + 6) < head.biHeight) && ((yi - 5) >= 0) && !IsIndexed()) {
      // optimized interpolation (faster pixel reads) for RGB24 images with all
      // pixels inside bounds
      BYTE *pxptr, *pxptra;
 ...
 */
void CxImage::AddAveragingCont(RGBQUAD const &color, float const surf,
 ...
 * used (see above) \param  ofMethod - overflow method used when retrieving
 * individual pixel colors \param  rplColor - replacement colour to use, in
 * OM_COLOR
 *
 * \author ***bd*** 2.2004
 */
RGBQUAD CxImage::GetAreaColorInterpolated(float const xc, float const yc,
  ...
  if (h <= 1 && w <= 1) {
    // both width and height are less than one... we will use interpolation of
    // center point
    ...
    if (h > 1 && w > 1) {
      // width and height of area are greater than one pixel, so we can employ
      // "ordinary" averaging
      CxRect2 intBL, intTR; // bottom left and top right intersection
      intBL =
          area.CrossSection(CxRect2(((float)xi1) - 0.5f, ((float)yi1) - 0.5f,
                                    ((float)xi1) + 0.5f, ((float)yi1) + 0.5f));
      intTR =
          area.CrossSection(CxRect2(((float)xi2) - 0.5f, ((float)yi2) - 0.5f,
      ...
      AddAveragingCont(GetPixelColorWithOverflow(xi1, yi1, ofMethod, rplColor),
                       wBL * hBL, rr, gg, bb, aa); // bottom left pixel
      AddAveragingCont(GetPixelColorWithOverflow(xi2, yi1, ofMethod, rplColor),
                       wTR * hBL, rr, gg, bb, aa); // bottom right pixel
      AddAveragingCont(GetPixelColorWithOverflow(xi1, yi2, ofMethod, rplColor),
                       wBL * hTR, rr, gg, bb, aa); // top left pixel
      AddAveragingCont(GetPixelColorWithOverflow(xi2, yi2, ofMethod, rplColor),
                       wTR * hTR, rr, gg, bb, aa); // top right pixel
      // bottom and top row
      for (x = xi1 + 1; x < xi2; x++) {
        AddAveragingCont(GetPixelColorWithOverflow(x, yi1, ofMethod, rplColor),
                         hBL, rr, gg, bb, aa); // bottom row
        AddAveragingCont(GetPixelColorWithOverflow(x, yi2, ofMethod, rplColor),
                         hTR, rr, gg, bb, aa); // top row
      }
      // leftmost and rightmost column
      for (y = yi1 + 1; y < yi2; y++) {
        AddAveragingCont(GetPixelColorWithOverflow(xi1, y, ofMethod, rplColor),
                         wBL, rr, gg, bb, aa); // left column
        AddAveragingCont(GetPixelColorWithOverflow(xi2, y, ofMethod, rplColor),
      ...
      for (y = yi1; y <= yi2; y++) {
        for (x = xi1; x <= xi2; x++) {
          intersect =
              area.CrossSection(CxRect2(((float)x) - 0.5f, ((float)y) - 0.5f,
                                        ((float)x) + 0.5f, ((float)y) + 0.5f));
          center = intersect.Center();
          color = GetPixelColorInterpolated(center.x, center.y, inMethod,
                                            ofMethod, rplColor);
