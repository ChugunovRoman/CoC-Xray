...
#pragma warning(pop)
  ...
  if (is_exif) {
    ...
    case 0: // [andy] fix for aspect ratio...
      if ((cinfo.Y_density > 0) && (cinfo.X_density > 0)) {
        SetYDPI((long)(GetXDPI() *
                       (float(cinfo.Y_density) / float(cinfo.X_density))));
      }
      break;
  ...
  while (cinfo.output_scanline < cinfo.output_height) {
    ...
    if ((cinfo.num_components == 4) && (cinfo.quantize_colors == FALSE)) {
      ...
      dst = iter.GetRow();
      src = buffer[0];
      for (long x3 = 0, x4 = 0; x3 < (long)info.dwEffWidth && x4 < row_stride;
           x3 += 3, x4 += 4) {
        k = src[x4 + 3];
...
#pragma warning(pop)
  ...
  jpeg_set_quality(&cinfo, GetJpegQuality(),
