...
public:
  ...
  typedef struct tagCxTextInfo {
    ...
    float b_opacity; ///< opacity value for background between 0.0-1.0 Default
                     ///< is 0. (opaque)
    BYTE b_outline;  ///< outline width for background (zero: no outline)
    BYTE b_round;    ///< rounding radius for background rectangle. % of the
                  ///< height, between 0-50. Default is 10. (backgr. always has
                  ///< a frame: width = 3 pixel + 10% of height by default.)
  } CXTEXTINFO;
#endif
...
public:
  ...
  CxImage(const CxImage &src, bool copypixels = true, bool copyselection = true,
          bool copyalpha = true);
  ...
  void Copy(const CxImage &src, bool copypixels = true,
            bool copyselection = true, bool copyalpha = true);
  bool Transfer(CxImage &from, bool bTransferFrames = true);
  bool CreateFromArray(BYTE *pArray, DWORD dwWidth, DWORD dwHeight,
                       DWORD dwBitsperpixel, DWORD dwBytesperline,
                       bool bFlipImage);
  bool CreateFromMatrix(BYTE **ppMatrix, DWORD dwWidth, DWORD dwHeight,
                        DWORD dwBitsperpixel, DWORD dwBytesperline,
                        bool bFlipImage);
  ...
  void DrawLine(int StartX, int EndX, int StartY, int EndY, RGBQUAD color,
                bool bSetAlpha = false);
  void DrawLine(int StartX, int EndX, int StartY, int EndY, COLORREF cr);
  void BlendPixelColor(long x, long y, RGBQUAD c, float blend,
                       bool bSetAlpha = false);
  //@}
...
public:
  ...
  RGBQUAD
  GetPixelColorWithOverflow(long x, long y,
  ...
  RGBQUAD
  GetPixelColorInterpolated(float x, float y,
                            InterpolationMethod const inMethod = IM_BILINEAR,
                            OverflowMethod const ofMethod = OM_BACKGROUND,
                            RGBQUAD *const rplColor = 0);
  RGBQUAD
  GetAreaColorInterpolated(float const xc, float const yc, float const w,
                           float const h, InterpolationMethod const inMethod,
                           OverflowMethod const ofMethod = OM_BACKGROUND,
                           RGBQUAD *const rplColor = 0);
  //@}
...
protected:
  /** \addtogroup Protected */ //@{
  void AddAveragingCont(RGBQUAD const &color, float const surf, float &rr,
                        float &gg, float &bb, float &aa);
  //@}
...
/** \addtogroup Painting */ //@{
  ...
  long Draw(HDC hdc, long x = 0, long y = 0, long cx = -1, long cy = -1,
            RECT *pClipRect = 0, bool bSmooth = false);
  ...
  long DrawString(HDC hdc, long x, long y, const TCHAR *text, RGBQUAD color,
                  const TCHAR *font, long lSize = 0, long lWeight = 400,
                  BYTE bItalic = 0, BYTE bUnderline = 0,
                  bool bSetAlpha = false);
  // <VATI> extensions
  long DrawStringEx(HDC hdc, long x, long y, CXTEXTINFO *pTextType,
                    bool bSetAlpha = false);
...
//@}
  ...
  bool Rotate2(float angle, CxImage *iDst = NULL,
  ...
  bool Resample2(long newx, long newy,
  ...
  bool CropRotatedRectangle(long topx, long topy, long width, long height,
                            float angle, CxImage *iDst = NULL);
  bool Skew(float xgain, float ygain, long xpivot = 0, long ypivot = 0,
            bool bEnableInterpolation = false);
  bool Expand(long left, long top, long right, long bottom, RGBQUAD canvascolor,
              CxImage *iDst = 0);
...
//@}
  ...
  bool HistogramRoot();
  bool HistogramLog();
  long Histogram(long *red, long *green = 0, long *blue = 0, long *gray = 0,
                 long colorspace = 0);
  bool Jitter(long radius = 2);
  bool Repair(float radius = 0.25f, long niterations = 1, long colorspace = 0);
  bool Combine(CxImage *r, CxImage *g, CxImage *b, CxImage *a,
               long colorspace = 0);
  bool FFT2(CxImage *srcReal, CxImage *srcImag, CxImage *dstReal,
            CxImage *dstImag, long direction = 1, bool bForceFFT = true,
  ...
  bool Threshold2(BYTE level, bool bDirection, RGBQUAD nBkgndColor,
                  bool bSetAlpha = false);
  ...
  void Mix(CxImage &imgsrc2, ImageOpType op, long lXOffset = 0,
           long lYOffset = 0, bool bMixAlpha = false);
  ...
  bool TextBlur(BYTE threshold = 100, BYTE decay = 2, BYTE max_depth = 5,
                bool bBlurHorizontal = true, bool bBlurVertical = true,
                CxImage *iDst = 0);
  bool SelectiveBlur(float radius = 1.0f, BYTE threshold = 25,
                     CxImage *iDst = 0);
  bool Solarize(BYTE level = 128, bool bLinkedChannels = true);
  bool FloodFill(const long xStart, const long yStart, const RGBQUAD cFillColor,
                 const BYTE tolerance = 0, BYTE nOpacity = 255,
  ...
  int OptimalThreshold(long method = 0, RECT *pBox = 0,
                       CxImage *pContrastMask = 0);
  bool AdaptiveThreshold(long method = 0, long nBoxSize = 64,
                         CxImage *pContrastMask = 0, long nBias = 0,
...
protected:
  ...
  void blur_line(float *ctable, float *cmatrix, int cmatrix_length,
                 BYTE *cur_col, BYTE *dest_col, int y, long bytes);
  void blur_text(BYTE threshold, BYTE decay, BYTE max_depth, CxImage *iSrc,
                 CxImage *iDst, BYTE bytes);
  //@}
...
public:
  ...
  long LayerDrawAll(HDC hdc, long x = 0, long y = 0, long cx = -1, long cy = -1,
                    RECT *pClipRect = 0, bool bSmooth = false);
  long LayerDrawAll(HDC hdc, const RECT &rect, RECT *pClipRect = NULL,
                    bool bSmooth = false);
//@}
...
protected:
  ...
  void Bitfield2RGB(BYTE *src, DWORD redmask, DWORD greenmask, DWORD bluemask,
                    BYTE bpp);
