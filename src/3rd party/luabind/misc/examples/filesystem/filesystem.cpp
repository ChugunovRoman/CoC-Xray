...
const boost::filesystem::path &identity(const boost::filesystem::path &x) {
  return x;
}
...
void bind_filesystem(lua_State *L) {
  ...
  module(L, "filesystem")
      [class_<fs::path>("path")
       ...
       def("remove", &fs::remove), def("remove_all", &fs::remove_all),
       def("rename", &fs::rename), def("copy_file", &fs::copy_file),

       def("initial_path", &fs::initial_path),
       def("current_path", &fs::current_path), def("complete", &fs::complete),
