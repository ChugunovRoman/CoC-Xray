...
template <class T> yes_t is_user_defined(by_value<boost::intrusive_ptr<T>>);

template <class T>
yes_t is_user_defined(by_const_reference<boost::intrusive_ptr<T>>);

// function used to destruct the object in lua
template <class T> struct decrement_ref_count {
...
convert_lua_to_cpp(lua_State *L, by_const_reference<boost::intrusive_ptr<T>>,
                   int index) {
  typename detail::default_policy::template generate_converter<
      T *, detail::lua_to_cpp>::type converter;
...
template <class T>
boost::intrusive_ptr<T>
convert_lua_to_cpp(lua_State *L, by_value<boost::intrusive_ptr<T>>, int index) {
  return convert_lua_to_cpp(L, by_const_reference<boost::intrusive_ptr<T>>(),
                            index);
}

template <class T>
int match_lua_to_cpp(lua_State *L, by_value<boost::intrusive_ptr<T>>,
                     int index) {
  typedef typename detail::default_policy::template generate_converter<
      T *, detail::lua_to_cpp>::type converter_t;
  converter_t::match(L, LUABIND_DECORATE_TYPE(T *), index);
}

template <class T>
int match_lua_to_cpp(lua_State *L, by_const_reference<boost::intrusive_ptr<T>>,
                     int index) {
  typedef typename detail::default_policy::template generate_converter<
      T *, detail::lua_to_cpp>::type converter_t;
  converter_t::match(L, LUABIND_DECORATE_TYPE(T *), index);
...
int main() {
  ...
  module(L)[class_<A>("A").def_readonly("cnt", &A::cnt),

            def("factory", &factory), def("f", &f)];
