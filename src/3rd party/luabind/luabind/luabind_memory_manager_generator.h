...
#define BOOST_PP_ITERATION_LIMITS                                              \
  (0, LUABIND_MEMORY_MANAGER_GENERATOR_MAX_ARITY)
#define BOOST_PP_FILENAME_1                                                    \
  <luabind / luabind_memory_manager_generator.h> // this file
...
#define luabind_new_detail_copy_constructor                                    \
  merge(luabind_new_detail_copy_constructor, n)
...
template <bool> struct luabind_new_detail {
  ...
  static inline T *
  initialize(T *result BOOST_PP_ENUM_TRAILING(n, PARAMETERS, BOOST_PP_EMPTY)) {
...
struct luabind_new_detail_copy_constructor {
  ...
  static inline T *
  initialize(T *result BOOST_PP_ENUM_TRAILING(n, PARAMETERS, BOOST_PP_EMPTY)) {
    return (luabind_new_detail<__is_pod(T)>::initialize(
        result BOOST_PP_ENUM_TRAILING_PARAMS(n, p)));
  }
};
