...
#ifndef __MINGW32__
#define _CRT_SECURE_NO_DEPRECATE // get rid of sprintf security warnings on
                                 // VS2005
...
BOOL GetLoadedModuleDirectory(LPCTSTR moduleName,
  ...
  numChars = GetModuleFileName(module, directoryContainingModule,
  ...
  if (list[0] == 0) { // don't re-build lists if it's already been done once...
    ...
    for (i = 0; i < numDirs; i++) {
      ...
      if (searchHandle != INVALID_HANDLE_VALUE) {
        while (TRUE) {
          //
          // if this is an OpenAL32.dll, skip it -- it's probably a router and
          // shouldn't be enumerated regardless
          //
          strcpy_s(searchName, dir[i]);
          ...
          if (strstr(cmpName, "OPENAL32.DLL") == 0) {
            // enforce search-order rules and make sure duplicate searches
            // aren't done
            boolean skipSearch = false;
            if ((i == 0) && (strcmp(dir[0], dir[3]) ==
                             0)) { // if searching router dir and router dir is
                                   // sys dir, skip search
              skipSearch = true;
            }
            if ((i == 2) && (strcmp(dir[2], dir[1]) ==
                             0)) { // if searching app dir and app dir is
                                   // current dir, skip search
              skipSearch = true;
            }
            if ((i == 3) && ((strcmp(dir[3], dir[2]) == 0) ||
                             (strcmp(dir[3], dir[1]) == 0))) {
              // if searching sys dir and sys dir is either current or app
              // directory, skip search
              skipSearch = true;
            }
            ...
            if (skipSearch == false) {
              ...
              if (dll) {
                alcOpenDeviceFxn =
                    (ALCAPI_OPEN_DEVICE)GetProcAddress(dll, "alcOpenDevice");
                alcCreateContextFxn = (ALCAPI_CREATE_CONTEXT)GetProcAddress(
                    dll, "alcCreateContext");
                alcMakeContextCurrentFxn =
                    (ALCAPI_MAKE_CONTEXT_CURRENT)GetProcAddress(
                        dll, "alcMakeContextCurrent");
                alcGetStringFxn =
                    (ALCAPI_GET_STRING)GetProcAddress(dll, "alcGetString");
                alcDestroyContextFxn = (ALCAPI_DESTROY_CONTEXT)GetProcAddress(
                    dll, "alcDestroyContext");
                alcCloseDeviceFxn =
                    (ALCAPI_CLOSE_DEVICE)GetProcAddress(dll, "alcCloseDevice");
                alcIsExtensionPresentFxn =
                    (ALCAPI_IS_EXTENSION_PRESENT)GetProcAddress(
                        dll, "alcIsExtensionPresent");
                ...
                if ((alcOpenDeviceFxn != 0) && (alcCreateContextFxn != 0) &&
                  ...
                  if (alcIsExtensionPresentFxn(NULL, "ALC_ENUMERATION_EXT")) {
                    // this DLL can enumerate devices -- so add complete list of
                    // devices
                    specifier =
                        (char *)alcGetStringFxn(0, ALC_DEVICE_SPECIFIER);
                    if ((specifier) && strlen(specifier)) {
                      do {
                        specifierSize = (ALuint)strlen(specifier);

                        if (NewSpecifierCheck(
                                specifier)) { // make sure we're not creating a
                                              // duplicate device
                    ...
                    // no enumeration ability, -- so just add default device to
                    // the list
                    ...
                    if (device != NULL) {
                      ...
                      if (context != NULL) {
                        specifier = (char *)alcGetStringFxn(
                            device, ALC_DEVICE_SPECIFIER);
                        if ((specifier) && strlen(specifier)) {
                          specifierSize = (ALuint)strlen(specifier);

                          if (NewSpecifierCheck(
                                  specifier)) { // make sure we're not creating
                                                // a duplicate device
                            strcpy_s((char *)list, list_end - list, specifier);
                            list += specifierSize + 1;
                          }
                  ...
                  {
                          // this DLL supports capture -- so add complete list
                  of capture devices specifier = (char*)alcGetStringFxn(0,
                  ALC_CAPTURE_DEVICE_SPECIFIER); if ((specifier) &&
                  strlen(specifier))
                          {
                                  do
                                  {
                                          specifierSize =
                  (ALuint)strlen(specifier); if
                  (NewCaptureSpecifierCheck(specifier)) { // make sure we're not
                  creating a duplicate device strcpy_s((char*)captureList,
                  specifier); captureList += specifierSize + 1;
                                          }
                                          specifier += strlen(specifier) + 1;
...
//    Gets rid of functionally duplicate names (DS3D is Generic Hardware, for
//    instance)
//*****************************************************************************
//
...
ALboolean FillOutAlcFunctions(ALCdevice *device) {
  ...
  alcApi->alcCreateContext =
      (ALCAPI_CREATE_CONTEXT)GetProcAddress(device->Dll, "alcCreateContext");
  alcApi->alcMakeContextCurrent = (ALCAPI_MAKE_CONTEXT_CURRENT)GetProcAddress(
      device->Dll, "alcMakeContextCurrent");
  alcApi->alcProcessContext =
      (ALCAPI_PROCESS_CONTEXT)GetProcAddress(device->Dll, "alcProcessContext");
  alcApi->alcSuspendContext =
      (ALCAPI_SUSPEND_CONTEXT)GetProcAddress(device->Dll, "alcSuspendContext");
  alcApi->alcDestroyContext =
      (ALCAPI_DESTROY_CONTEXT)GetProcAddress(device->Dll, "alcDestroyContext");
  alcApi->alcGetCurrentContext = (ALCAPI_GET_CURRENT_CONTEXT)GetProcAddress(
      device->Dll, "alcGetCurrentContext");
  alcApi->alcGetContextsDevice = (ALCAPI_GET_CONTEXTS_DEVICE)GetProcAddress(
      device->Dll, "alcGetContextsDevice");

  alcApi->alcOpenDevice =
      (ALCAPI_OPEN_DEVICE)GetProcAddress(device->Dll, "alcOpenDevice");
  alcApi->alcCloseDevice =
      (ALCAPI_CLOSE_DEVICE)GetProcAddress(device->Dll, "alcCloseDevice");

  alcApi->alcGetError =
      (ALCAPI_GET_ERROR)GetProcAddress(device->Dll, "alcGetError");

  alcApi->alcIsExtensionPresent = (ALCAPI_IS_EXTENSION_PRESENT)GetProcAddress(
      device->Dll, "alcIsExtensionPresent");
  alcApi->alcGetProcAddress =
      (ALCAPI_GET_PROC_ADDRESS)GetProcAddress(device->Dll, "alcGetProcAddress");
  alcApi->alcGetEnumValue =
      (ALCAPI_GET_ENUM_VALUE)GetProcAddress(device->Dll, "alcGetEnumValue");

  alcApi->alcGetString =
      (ALCAPI_GET_STRING)GetProcAddress(device->Dll, "alcGetString");
  alcApi->alcGetIntegerv =
      (ALCAPI_GET_INTEGERV)GetProcAddress(device->Dll, "alcGetIntegerv");

  //
  // Get the OpenAL 1.1 Entry points.
  //
  alcApi->alcCaptureOpenDevice = (ALCAPI_CAPTURE_OPEN_DEVICE)GetProcAddress(
      device->Dll, "alcCaptureOpenDevice");
  alcApi->alcCaptureCloseDevice = (ALCAPI_CAPTURE_CLOSE_DEVICE)GetProcAddress(
      device->Dll, "alcCaptureCloseDevice");
  alcApi->alcCaptureStart =
      (ALCAPI_CAPTURE_START)GetProcAddress(device->Dll, "alcCaptureStart");
  alcApi->alcCaptureStop =
      (ALCAPI_CAPTURE_STOP)GetProcAddress(device->Dll, "alcCaptureStop");
  alcApi->alcCaptureSamples =
      (ALCAPI_CAPTURE_SAMPLES)GetProcAddress(device->Dll, "alcCaptureSamples");

  // handle legacy issue with old Creative DLLs which may not have
  // alcGetProcAddress, alcIsExtensionPresent, alcGetEnumValue
  ...
  alcFxns = (alcApi->alcCreateContext && alcApi->alcMakeContextCurrent &&
             alcApi->alcProcessContext && alcApi->alcSuspendContext &&
             alcApi->alcDestroyContext && alcApi->alcGetCurrentContext &&
             alcApi->alcGetContextsDevice && alcApi->alcOpenDevice &&
             alcApi->alcCloseDevice && alcApi->alcGetError &&
             alcApi->alcIsExtensionPresent && alcApi->alcGetProcAddress &&
             alcApi->alcGetEnumValue && alcApi->alcGetString &&
             alcApi->alcGetIntegerv);
...
ALboolean FillOutAlFunctions(ALCcontext *context) {
  ...
  //
  alApi->alEnable =
      (ALAPI_ENABLE)GetProcAddress(context->Device->Dll, "alEnable");
  alApi->alDisable =
      (ALAPI_DISABLE)GetProcAddress(context->Device->Dll, "alDisable");
  alApi->alIsEnabled =
      (ALAPI_IS_ENABLED)GetProcAddress(context->Device->Dll, "alIsEnabled");

  alApi->alGetString =
      (ALAPI_GET_STRING)GetProcAddress(context->Device->Dll, "alGetString");
  alApi->alGetBooleanv =
      (ALAPI_GET_BOOLEANV)GetProcAddress(context->Device->Dll, "alGetBooleanv");
  alApi->alGetIntegerv =
      (ALAPI_GET_INTEGERV)GetProcAddress(context->Device->Dll, "alGetIntegerv");
  alApi->alGetFloatv =
      (ALAPI_GET_FLOATV)GetProcAddress(context->Device->Dll, "alGetFloatv");
  alApi->alGetDoublev =
      (ALAPI_GET_DOUBLEV)GetProcAddress(context->Device->Dll, "alGetDoublev");
  alApi->alGetBoolean =
      (ALAPI_GET_BOOLEAN)GetProcAddress(context->Device->Dll, "alGetBoolean");
  alApi->alGetInteger =
      (ALAPI_GET_INTEGER)GetProcAddress(context->Device->Dll, "alGetInteger");
  alApi->alGetFloat =
      (ALAPI_GET_FLOAT)GetProcAddress(context->Device->Dll, "alGetFloat");
  alApi->alGetDouble =
      (ALAPI_GET_DOUBLE)GetProcAddress(context->Device->Dll, "alGetDouble");
  alApi->alGetError =
      (ALAPI_GET_ERROR)GetProcAddress(context->Device->Dll, "alGetError");
  alApi->alIsExtensionPresent = (ALAPI_IS_EXTENSION_PRESENT)GetProcAddress(
      context->Device->Dll, "alIsExtensionPresent");
  alApi->alGetProcAddress = (ALAPI_GET_PROC_ADDRESS)GetProcAddress(
      context->Device->Dll, "alGetProcAddress");
  alApi->alGetEnumValue = (ALAPI_GET_ENUM_VALUE)GetProcAddress(
      context->Device->Dll, "alGetEnumValue");

  alApi->alListenerf =
      (ALAPI_LISTENERF)GetProcAddress(context->Device->Dll, "alListenerf");
  alApi->alListener3f =
      (ALAPI_LISTENER3F)GetProcAddress(context->Device->Dll, "alListener3f");
  alApi->alListenerfv =
      (ALAPI_LISTENERFV)GetProcAddress(context->Device->Dll, "alListenerfv");
  alApi->alListeneri =
      (ALAPI_LISTENERI)GetProcAddress(context->Device->Dll, "alListeneri");
  alApi->alListener3i =
      (ALAPI_LISTENER3I)GetProcAddress(context->Device->Dll, "alListener3i");
  alApi->alListeneriv =
      (ALAPI_LISTENERIV)GetProcAddress(context->Device->Dll, "alListeneriv");
  alApi->alGetListenerf = (ALAPI_GET_LISTENERF)GetProcAddress(
      context->Device->Dll, "alGetListenerf");
  alApi->alGetListener3f = (ALAPI_GET_LISTENER3F)GetProcAddress(
      context->Device->Dll, "alGetListener3f");
  alApi->alGetListenerfv = (ALAPI_GET_LISTENERFV)GetProcAddress(
      context->Device->Dll, "alGetListenerfv");
  alApi->alGetListeneri = (ALAPI_GET_LISTENERI)GetProcAddress(
      context->Device->Dll, "alGetListeneri");
  alApi->alGetListener3i = (ALAPI_GET_LISTENER3I)GetProcAddress(
      context->Device->Dll, "alGetListener3i");
  alApi->alGetListeneriv = (ALAPI_GET_LISTENERIV)GetProcAddress(
      context->Device->Dll, "alGetListeneriv");

  alApi->alGenSources =
      (ALAPI_GEN_SOURCES)GetProcAddress(context->Device->Dll, "alGenSources");
  alApi->alDeleteSources = (ALAPI_DELETE_SOURCES)GetProcAddress(
      context->Device->Dll, "alDeleteSources");
  alApi->alIsSource =
      (ALAPI_IS_SOURCE)GetProcAddress(context->Device->Dll, "alIsSource");
  alApi->alSourcef =
      (ALAPI_SOURCEF)GetProcAddress(context->Device->Dll, "alSourcef");
  alApi->alSource3f =
      (ALAPI_SOURCE3F)GetProcAddress(context->Device->Dll, "alSource3f");
  alApi->alSourcefv =
      (ALAPI_SOURCEFV)GetProcAddress(context->Device->Dll, "alSourcefv");
  alApi->alSourcei =
      (ALAPI_SOURCEI)GetProcAddress(context->Device->Dll, "alSourcei");
  alApi->alSource3i =
      (ALAPI_SOURCE3I)GetProcAddress(context->Device->Dll, "alSource3i");
  alApi->alSourceiv =
      (ALAPI_SOURCEIV)GetProcAddress(context->Device->Dll, "alSourceiv");
  alApi->alGetSourcef =
      (ALAPI_GET_SOURCEF)GetProcAddress(context->Device->Dll, "alGetSourcef");
  alApi->alGetSource3f =
      (ALAPI_GET_SOURCE3F)GetProcAddress(context->Device->Dll, "alGetSource3f");
  alApi->alGetSourcefv =
      (ALAPI_GET_SOURCEFV)GetProcAddress(context->Device->Dll, "alGetSourcefv");
  alApi->alGetSourcei =
      (ALAPI_GET_SOURCEI)GetProcAddress(context->Device->Dll, "alGetSourcei");
  alApi->alGetSource3i =
      (ALAPI_GET_SOURCE3I)GetProcAddress(context->Device->Dll, "alGetSource3i");
  alApi->alGetSourceiv =
      (ALAPI_GET_SOURCEIV)GetProcAddress(context->Device->Dll, "alGetSourceiv");
  alApi->alSourcePlayv =
      (ALAPI_SOURCE_PLAYV)GetProcAddress(context->Device->Dll, "alSourcePlayv");
  alApi->alSourceStopv =
      (ALAPI_SOURCE_STOPV)GetProcAddress(context->Device->Dll, "alSourceStopv");
  alApi->alSourceRewindv = (ALAPI_SOURCE_REWINDV)GetProcAddress(
      context->Device->Dll, "alSourceRewindv");
  alApi->alSourcePausev = (ALAPI_SOURCE_PAUSEV)GetProcAddress(
      context->Device->Dll, "alSourcePausev");
  alApi->alSourcePlay =
      (ALAPI_SOURCE_PLAY)GetProcAddress(context->Device->Dll, "alSourcePlay");
  alApi->alSourceStop =
      (ALAPI_SOURCE_STOP)GetProcAddress(context->Device->Dll, "alSourceStop");
  alApi->alSourceRewind =
      (ALAPI_SOURCE_STOP)GetProcAddress(context->Device->Dll, "alSourceRewind");
  alApi->alSourcePause =
      (ALAPI_SOURCE_PAUSE)GetProcAddress(context->Device->Dll, "alSourcePause");

  alApi->alSourceQueueBuffers = (ALAPI_SOURCE_QUEUE_BUFFERS)GetProcAddress(
      context->Device->Dll, "alSourceQueueBuffers");
  alApi->alSourceUnqueueBuffers = (ALAPI_SOURCE_UNQUEUE_BUFFERS)GetProcAddress(
      context->Device->Dll, "alSourceUnqueueBuffers");

  alApi->alGenBuffers =
      (ALAPI_GEN_BUFFERS)GetProcAddress(context->Device->Dll, "alGenBuffers");
  alApi->alDeleteBuffers = (ALAPI_DELETE_BUFFERS)GetProcAddress(
      context->Device->Dll, "alDeleteBuffers");
  alApi->alIsBuffer =
      (ALAPI_IS_BUFFER)GetProcAddress(context->Device->Dll, "alIsBuffer");
  alApi->alBufferData =
      (ALAPI_BUFFER_DATA)GetProcAddress(context->Device->Dll, "alBufferData");
  alApi->alBufferf =
      (ALAPI_BUFFERF)GetProcAddress(context->Device->Dll, "alBufferf");
  alApi->alBuffer3f =
      (ALAPI_BUFFER3F)GetProcAddress(context->Device->Dll, "alBuffer3f");
  alApi->alBufferfv =
      (ALAPI_BUFFERFV)GetProcAddress(context->Device->Dll, "alBufferfv");
  alApi->alBufferi =
      (ALAPI_BUFFERI)GetProcAddress(context->Device->Dll, "alBufferi");
  alApi->alBuffer3i =
      (ALAPI_BUFFER3I)GetProcAddress(context->Device->Dll, "alBuffer3i");
  alApi->alBufferiv =
      (ALAPI_BUFFERIV)GetProcAddress(context->Device->Dll, "alBufferiv");
  alApi->alGetBufferf =
      (ALAPI_GET_BUFFERF)GetProcAddress(context->Device->Dll, "alGetBufferf");
  alApi->alGetBuffer3f =
      (ALAPI_GET_BUFFER3F)GetProcAddress(context->Device->Dll, "alGetBuffer3f");
  alApi->alGetBufferfv =
      (ALAPI_GET_BUFFERFV)GetProcAddress(context->Device->Dll, "alGetBufferfv");
  alApi->alGetBufferi =
      (ALAPI_GET_BUFFERI)GetProcAddress(context->Device->Dll, "alGetBufferi");
  alApi->alGetBuffer3i =
      (ALAPI_GET_BUFFER3I)GetProcAddress(context->Device->Dll, "alGetBuffer3i");
  alApi->alGetBufferiv =
      (ALAPI_GET_BUFFERIV)GetProcAddress(context->Device->Dll, "alGetBufferiv");

  alApi->alDopplerFactor = (ALAPI_DOPPLER_FACTOR)GetProcAddress(
      context->Device->Dll, "alDopplerFactor");
  alApi->alDopplerVelocity = (ALAPI_DOPPLER_VELOCITY)GetProcAddress(
      context->Device->Dll, "alDopplerVelocity");
  alApi->alSpeedOfSound = (ALAPI_SPEED_OF_SOUND)GetProcAddress(
      context->Device->Dll, "alSpeedOfSound");
  alApi->alDistanceModel = (ALAPI_DISTANCE_MODEL)GetProcAddress(
      context->Device->Dll, "alDistanceModel");

  alFxns =
      (alApi->alEnable && alApi->alDisable && alApi->alIsEnabled &&

       alApi->alGetString && alApi->alGetBooleanv && alApi->alGetIntegerv &&
       alApi->alGetFloatv && alApi->alGetDoublev && alApi->alGetBoolean &&
       alApi->alGetInteger && alApi->alGetFloat && alApi->alGetDouble &&

       alApi->alGetError &&

       alApi->alIsExtensionPresent && alApi->alGetProcAddress &&
       alApi->alGetEnumValue &&

       alApi->alListenerf && alApi->alListener3f && alApi->alListenerfv &&
       alApi->alListeneri && alApi->alGetListenerf && alApi->alGetListener3f &&
       alApi->alGetListenerfv && alApi->alGetListeneri &&

       alApi->alGenSources && alApi->alDeleteSources && alApi->alIsSource &&
       alApi->alSourcef && alApi->alSource3f && alApi->alSourcefv &&
       alApi->alSourcei && alApi->alGetSourcef && alApi->alGetSource3f &&
       alApi->alGetSourcefv && alApi->alGetSourcei && alApi->alSourcePlayv &&
       alApi->alSourceStopv && alApi->alSourceRewindv &&
       alApi->alSourcePausev && alApi->alSourcePlay && alApi->alSourceStop &&
       alApi->alSourceRewind && alApi->alSourcePause &&

       alApi->alSourceQueueBuffers && alApi->alSourceUnqueueBuffers &&

       alApi->alGenBuffers && alApi->alDeleteBuffers && alApi->alIsBuffer &&
       alApi->alBufferData && alApi->alGetBufferf && alApi->alGetBufferi &&

       alApi->alDopplerFactor && alApi->alDopplerVelocity &&
...
//*****************************************************************************
//
HINSTANCE FindDllWithMatchingSpecifier(TCHAR *dllSearchPattern, char *specifier,
  ...
  //
  // Construct our search paths.  We will search the current directory, the app
  // directory, the system directory, and the directory containing OpenAL32.dll,
  // if we can find it.
  //
  if (GetLoadedModuleDirectory("OpenAL32.dll", dir[0], MAX_PATH)) {
    ++numDirs;
  ...
  for (i = (numDirs > 3) ? 0 : 1; i < numDirs && !found; i++) {
    ...
    if (searchHandle != INVALID_HANDLE_VALUE) {
      while (TRUE) {
        ...
        _strupr_s(cmpName);
        if (strstr(cmpName, "OPENAL32.DLL") == 0) {
          // enforce search-order rules and make sure duplicate searches aren't
          // done
          boolean skipSearch = false;
          if ((i == 0) && (strcmp(dir[0], dir[3]) ==
                           0)) { // if searching router dir and router dir is
          ...
          if ((i == 2) && (strcmp(dir[2], dir[1]) ==
                           0)) { // if searching app dir and app dir is current
                                 // dir, skip search
            skipSearch = true;
          }
          if ((i == 3) && ((strcmp(dir[3], dir[2]) == 0) ||
              ...
              if ((alcOpenDeviceFxn != 0) && (alcCreateContextFxn != 0) &&
                ...
                if (alcGetStringFxn) {
                  if (captureDevice == false) {
                    if (alcIsExtensionPresentFxn(
                            0, (ALCchar *)"ALC_ENUMERATION_EXT")) {
                      // have an enumeratable DLL here, so check all available
                      // devices
                      deviceSpecifier =
                          (char *)alcGetStringFxn(0, ALC_DEVICE_SPECIFIER);
                      if (deviceSpecifier) {
                        do {
                          if (deviceSpecifier != NULL) {
                            if ((partialName == false) ||
                              ...
                              found =
                                  strncmp(deviceSpecifier, specifier, 31) == 0;
                              // note -- specifier may be a non-truncated
                              // version of deviceSpecifier, which is why
                              // strncmp is being used
                            } else {
                              found = strstr(deviceSpecifier, specifier) != 0;
                              if (actualName) {
                                strcpy_s(actualName, MAX_DEVICE_STRINGS,
                                         deviceSpecifier);
                              }
                            }
                          } else {
                      ...
                      if (device != NULL) {
                        ...
                        alcMakeContextCurrentFxn((ALCcontext *)context);
                        if (context != NULL) {
                          deviceSpecifier = (char *)alcGetStringFxn(
                              device, ALC_DEFAULT_DEVICE_SPECIFIER);
                          if (deviceSpecifier != NULL) {
                            if ((partialName == false) ||
                                (strstr(deviceSpecifier, "Generic") != 0)) {
                              found =
                                  strncmp(deviceSpecifier, specifier, 31) == 0;
                              // note -- specifier may be a non-truncated
                              // version of deviceSpecifier; hence the use of
                              // strncmp
                            } else {
                              found = strstr(deviceSpecifier, specifier) != 0;
                              if (actualName) {
                    ...
                    if (alcIsExtensionPresentFxn(
                            0, (ALCchar *)"ALC_EXT_CAPTURE")) {
                      // so check all available capture devices
                      deviceSpecifier = (char *)alcGetStringFxn(
                          0, ALC_CAPTURE_DEVICE_SPECIFIER);
                      if (deviceSpecifier) {
                        do {
                          if (deviceSpecifier != NULL) {
                            if (partialName == false) {
...
ALCAPI ALCboolean ALCAPIENTRY alcCloseDevice(ALCdevice *device) {
  ...
  if (device == g_CaptureDevice)
    return g_CaptureDevice->AlcApi.alcCloseDevice(
...
ALCAPI ALCcontext *ALCAPIENTRY alcCreateContext(ALCdevice *device,
  ...
  if (device == g_CaptureDevice)
    return g_CaptureDevice->AlcApi.alcCreateContext(
        g_CaptureDevice->CaptureDevice, attrList);
  ...
  context->DllContext =
      device->AlcApi.alcCreateContext(device->DllDevice, attrList);
...
ALCAPI ALvoid ALCAPIENTRY alcDestroyContext(ALCcontext *context) {
  ...
  alListAcquireLock(alContextList);
  listData =
      (ALCcontext *)alListRemoveEntry(alContextList, &context->ListEntry);
  if (!listData) {
    alListReleaseLock(alContextList);
    return;
  }
...
//*****************************************************************************
//
ALCAPI ALenum ALCAPIENTRY alcGetEnumValue(ALCdevice *device,
                                          const ALCchar *ename) {
  //
  // Always return the router version of the ALC enum if it exists.
  //
  ALsizei i = 0;
  while (alcEnums[i].ename &&
         strcmp((char *)alcEnums[i].ename, (char *)ename)) {
    i++;
  }
  ...
  if (device) {
    ...
    if (device == g_CaptureDevice)
      return g_CaptureDevice->AlcApi.alcGetEnumValue(
          g_CaptureDevice->CaptureDevice, ename);
...
ALCAPI ALenum ALCAPIENTRY alcGetError(ALCdevice *device) {
  ...
  if (device == g_CaptureDevice)
    errorCode =
        g_CaptureDevice->AlcApi.alcGetError(g_CaptureDevice->CaptureDevice);
  else
...
//*****************************************************************************
//
ALCAPI ALvoid ALCAPIENTRY alcGetIntegerv(ALCdevice *device, ALenum param,
                                         ALsizei size, ALint *data) {
  if (device) {
    if (IsBadReadPtr(device, sizeof(ALCdevice))) {
      LastError = ALC_INVALID_DEVICE;
      return;
    ...
    if (device == g_CaptureDevice) {
      g_CaptureDevice->AlcApi.alcGetIntegerv(g_CaptureDevice->CaptureDevice,
                                             param, size, data);
      return;
    }
...
//*****************************************************************************
//
ALCAPI ALvoid *ALCAPIENTRY alcGetProcAddress(ALCdevice *device,
                                             const ALCchar *fname) {
  //
  // Always return the router version of the ALC function if it exists.
  //
  ALsizei i = 0;
  while (alcFunctions[i].fname &&
         strcmp((char *)alcFunctions[i].fname, (char *)fname)) {
    i++;
  }
  ...
  if (device) {
    ...
    if (device == g_CaptureDevice)
      return g_CaptureDevice->AlcApi.alcGetProcAddress(
          g_CaptureDevice->CaptureDevice, fname);
...
//*****************************************************************************
//
ALCAPI ALboolean ALCAPIENTRY alcIsExtensionPresent(ALCdevice *device,
                                                   const ALCchar *ename) {
  //
  ...
  //
  ALsizei i = 0;
  while (alcExtensions[i].ename &&
         _stricmp((char *)alcExtensions[i].ename, (char *)ename)) {
    i++;
  }
  ...
  if (device) {
    ...
    if (device == g_CaptureDevice)
      return g_CaptureDevice->AlcApi.alcIsExtensionPresent(
          g_CaptureDevice->CaptureDevice, ename);
...
ALCAPI ALboolean ALCAPIENTRY alcMakeContextCurrent(ALCcontext *context) {
  ...
  if (context) {
    contextSwitched =
        context->Device->AlcApi.alcMakeContextCurrent(context->DllContext);

    //
    // If this is the first time the context has been made the current context,
    // fill in the context function pointers.
    //
    if (contextSwitched && !context->AlApi.alGetProcAddress) {
      //
      // Don't fill out the functions here in case they are context specific
...
ALCAPI ALCdevice *ALCAPIENTRY alcOpenDevice(const ALCchar *deviceName) {
  ...
  if ((!deviceName) || (strcmp((char *)deviceName, "DirectSound3D") == 0)) {
    strncpy_s(newDeviceName, alcGetString(0, ALC_DEFAULT_DEVICE_SPECIFIER),
              256);
  } else {
  ...
  if ((strcmp(newDeviceName, T2A("")) == 0) &&
      ((!deviceName) || (strcmp((char *)deviceName, T2A("DirectSound3D"))))) {
  ...
  // if device name is "Generic Software" and a device wasn't found, try to find
  // a wrapper...
  if ((!dll) && (strcmp(newDeviceName, "Generic Software") == 0)) {
    dll = FindWrapper();
  }

  // if device name is "Generic Hardware" and a device wasn't found, try to find
  // a wrapper...
  if ((!dll) && (strcmp(newDeviceName, "Generic Hardware") == 0)) {
    dll = FindWrapper();
  }

  // If we still don't have a match for these default names, try the default
  // device
  if (!dll) {
    strncpy_s(newDeviceName, alcGetString(0, ALC_DEFAULT_DEVICE_SPECIFIER),
              256);
...
void getDefaultPlaybackDeviceNames(char *longName, char *shortName,
                                   unsigned int len) {
  ...
  if (bFoundOutputName == false) {
    ...
    __asm__("pusha;");
#else
    __asm pusha; // workaround for register destruction caused by these
                 // wavOutMessage calls (weird but true)
#endif
#endif // !defined(_WIN64)
    waveOutMessage((HWAVEOUT)(UINT_PTR)WAVE_MAPPER, 0x2000 + 0x0015,
                   (LPARAM)&uDeviceID, (WPARAM)&dwFlags);
...
void getDefaultCaptureDeviceNames(char *longName, char *shortName,
                                  unsigned int len) {
...
//*****************************************************************************
//
ALCAPI const ALCchar *ALCAPIENTRY alcGetString(ALCdevice *device,
                                               ALenum param) {
  const ALCchar *value = 0;

  if ((param != ALC_DEFAULT_DEVICE_SPECIFIER) &&
      (param != ALC_CAPTURE_DEFAULT_DEVICE_SPECIFIER)) {
    if (device) {
      if (device == g_CaptureDevice)
        return g_CaptureDevice->AlcApi.alcGetString(
            g_CaptureDevice->CaptureDevice, param);
  ...
  case ALC_DEFAULT_DEVICE_SPECIFIER: {
    while (TRUE) {
      //
      // See if we can find a native implementation for the user's current
      // device.
      //
      ...
      // 1) use the long device name if available (normally it would be from
      // Vista Core Audio) 2) if #1 fails to find a match, then use the short
      // device name
      //       - if the preferred device is an Audigy or an X-Fi, then the name
      //       might not match the
      //          hardware DLL, so allow a partial match in this case
      // 3) if #2 fails to find a match, try the generic names as the ultimate
      // fallback
      getDefaultPlaybackDeviceNames(longDevice, mixerDevice, 256);
      if (strlen(longDevice)) { // test long device name first
        dll = FindDllWithMatchingSpecifier(_T("*oal.dll"), longDevice, false,
                                           actualName);
        if (dll) {
      ...
      if (strlen(mixerDevice)) { // test short device name (mixerDevice) next,
                                 // with partial matches potentially...
        ...
        dll = FindDllWithMatchingSpecifier(_T("*oal.dll"), mixerDevice,
                                           acceptPartial, actualName);
        if (dll) {
      ...
      dll = FindDllWithMatchingSpecifier(_T("*oal.dll"),
                                         T2A("Generic Hardware"), true);
      if (dll) {
...
//*****************************************************************************
//
ALCAPI ALCdevice *ALCAPIENTRY alcCaptureOpenDevice(const ALCchar *deviceName,
                                                   ALCuint frequency,
                                                   ALCenum format,
                                                   ALCsizei buffersize) {
  char newDeviceName[256];
  ...
  if (!g_CaptureDevice) {
    ...
    if (g_CaptureDevice) {
      ...
      if (!deviceName) {
        strncpy_s(newDeviceName,
                  alcGetString(0, ALC_CAPTURE_DEFAULT_DEVICE_SPECIFIER), 256);
      } else {
        strncpy_s(newDeviceName, deviceName, 256);
      }

      g_CaptureDevice->Dll = FindDllWithMatchingSpecifier(
          _T("*oal.dll"), (char *)newDeviceName, false, NULL, true);

      if (g_CaptureDevice->Dll) {
        if (FillOutAlcFunctions(g_CaptureDevice)) {
          if (g_CaptureDevice->AlcApi.alcCaptureOpenDevice) {
            g_CaptureDevice->CaptureDevice =
                g_CaptureDevice->AlcApi.alcCaptureOpenDevice(
                    newDeviceName, frequency, format, buffersize);
  ...
  if (device == g_CaptureDevice) {
    if (g_CaptureDevice != NULL) {
      if (g_CaptureDevice->AlcApi.alcCaptureCloseDevice) {
        bReturn = g_CaptureDevice->AlcApi.alcCaptureCloseDevice(
            g_CaptureDevice->CaptureDevice);
...
//*****************************************************************************
//
ALCAPI ALCvoid ALCAPIENTRY alcCaptureSamples(ALCdevice *device, ALCvoid *buffer,
                                             ALCsizei samples) {
  if (device == g_CaptureDevice) {
    if (g_CaptureDevice != NULL) {
      if (g_CaptureDevice->AlcApi.alcCaptureSamples) {
        g_CaptureDevice->AlcApi.alcCaptureSamples(
            g_CaptureDevice->CaptureDevice, buffer, samples);
      } else {
        g_CaptureDevice->LastError = ALC_INVALID_DEVICE;
      }
    }
