...
LJ_FUNCA int lj_err_unwind_dwarf(int version, int actions, uint64_t uexclass,
  ...
  if ((actions & _UA_CLEANUP_PHASE)) {
    ...
    if ((actions & _UA_FORCE_UNWIND)) {
      ...
      _Unwind_SetIP(ctx, (uintptr_t)(cframe_unwind_ff(cf) ? lj_vm_unwind_ff_eh
                                                          : lj_vm_unwind_c_eh));
...
VOID RtlUnwindEx_FIXED(PVOID, PVOID, PVOID, PVOID, PVOID,
                       PVOID) asm("RtlUnwindEx");
...
lj_err_unwind_win64(EXCEPTION_RECORD *rec, void *cf, CONTEXT *ctx,
