...
static void emit_lso(ASMState *as, ARMIns ai, Reg rd, Reg rn, int32_t ofs) {
  ...
  if (*as->mcp == (ai | ARMI_LS_P | ARMI_LS_U | ARMF_D(rd ^ 1) | ARMF_N(rn) |
                   (ofs ^ 4)) &&
...
static void emit_loadi(ASMState *as, Reg r, int32_t i) {
  ...
  if (k) {
    ...
    emit_d(as, ARMI_MOVT | ((i >> 16) & 0x0fff) | (((i >> 16) & 0xf000) << 4),
           r);
    emit_d(as, ARMI_MOVW | (i & 0x0fff) | ((i & 0xf000) << 4), r);
...
static void emit_loadn(ASMState *as, Reg r, cTValue *tv) {
  ...
  if ((as->flags & JIT_F_VFPV3) && !tv->u32.lo) {
    ...
    if (!(hi & 0xffff) && (b == 0x100 || b == 0x0ff)) {
      *--as->mcp =
          ARMI_VMOVI_D | ARMF_D(r & 15) | ((tv->u32.hi >> 12) & 0x00080000) |
...
#else
  if (dst >= RID_MAX_GPR) {
    emit_dm(as, irt_isnum(ir->t) ? ARMI_VMOV_D : ARMI_VMOV_S, (dst & 15),
            (src & 15));
    return;
  }
#endif
...
static void emit_opk(ASMState *as, ARMIns ai, Reg dest, Reg src, int32_t i,
                     RegSet allow) {
