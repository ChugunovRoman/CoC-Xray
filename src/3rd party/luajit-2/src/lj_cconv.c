...
*/
void lj_cconv_ct_ct(CTState *cts, CType *d, CType *s, uint8_t *dp, uint8_t *sp,
                    CTInfo flags) {
...
#if LJ_LE
      uint8_t fill =
          (!(sinfo & CTF_UNSIGNED) && (sp[ssize - 1] & 0x80)) ? 0xff : 0;
      memcpy(dp, sp, ssize);
...
int lj_cconv_tv_ct(CTState *cts, CType *s, CTypeID sid, TValue *o,
                   uint8_t *sp) {
  CTInfo sinfo = s->info;
  if (ctype_isnum(sinfo)) {
    if (!ctype_isbool(sinfo)) {
      ...
      if (LJ_DUALNUM && ctype_isinteger(sinfo)) {
        int32_t i;
        lj_cconv_ct_ct(cts, ctype_get(cts, CTID_INT32), s, (uint8_t *)&i, sp,
                       0);
        ...
        lj_cconv_ct_ct(cts, ctype_get(cts, CTID_DOUBLE), s, (uint8_t *)&o->n,
                       sp, 0);
...
static void cconv_array_tab(CTState *cts, CType *d, uint8_t *dp, GCtab *t,
                            CTInfo flags) {
  int32_t i;
...
static void cconv_substruct_tab(CTState *cts, CType *d, uint8_t *dp, GCtab *t,
                                int32_t *ip, CTInfo flags) {
  CTypeID id = d->sib;
  while (id) {
    ...
    if (ctype_isfield(df->info) || ctype_isbitfield(df->info)) {
      ...
      cconv_substruct_tab(cts, ctype_rawchild(cts, df), dp + df->size, t, ip,
                          flags);
...
static void cconv_struct_tab(CTState *cts, CType *d, uint8_t *dp, GCtab *t,
                             CTInfo flags) {
  int32_t i = 0;
...
void lj_cconv_ct_tv(CTState *cts, CType *d, uint8_t *dp, TValue *o,
                    CTInfo flags) {
...
static void cconv_substruct_init(CTState *cts, CType *d, uint8_t *dp, TValue *o,
                                 MSize len, MSize *ip) {
  CTypeID id = d->sib;
  while (id) {
    CType *df = ctype_get(cts, id);
    ...
    if (ctype_isfield(df->info) || ctype_isbitfield(df->info)) {
      ...
      *ip = i + 1;
      ...
      cconv_substruct_init(cts, ctype_rawchild(cts, df), dp + df->size, o, len,
                           ip);
      if ((d->info & CTF_UNION))
        break;
    } /* Ignore all other entries in the chain. */
...
void lj_cconv_ct_init(CTState *cts, CType *d, CTSize sz, uint8_t *dp, TValue *o,
                      MSize len) {
  if (len == 0)
    memset(dp, 0, sz);
