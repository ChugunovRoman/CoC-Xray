...
static TRef narrow_stripov(jit_State *J, TRef tr, int lastop, IRRef mode) {
  ...
  if (op >= IR_ADDOV && op <= lastop) {
    ...
    if (bp) {
      ...
      tr = emitir(
          IRT(op - IR_ADDOV + IR_ADD, ((mode & IRCONV_DSTMASK) >> IRCONV_DSH)),
          op1, op2);
...
TRef lj_opt_narrow_arith(jit_State *J, TRef rb, TRef rc, TValue *vb, TValue *vc,
                         IROp op) {
...
TRef lj_opt_narrow_mod(jit_State *J, TRef rb, TRef rc, TValue *vb, TValue *vc) {
  ...
  rb = conv_str_tonum(J, rb, vb);
  rc = conv_str_tonum(J, rc, vc);
  if ((LJ_DUALNUM || (J->flags & JIT_F_OPT_NARROW)) && tref_isinteger(rb) &&
      tref_isinteger(rc) && (tvisint(vc) ? intV(vc) != 0 : !tviszero(vc))) {
...
IRType lj_opt_narrow_forl(jit_State *J, cTValue *tv) {
  lua_assert(tvisnumber(&tv[FORL_IDX]) && tvisnumber(&tv[FORL_STOP]) &&
