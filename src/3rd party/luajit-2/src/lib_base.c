...
LJLIB_PUSH(lastcl)
LJLIB_ASM(pairs) { return ffh_pairs(L, MM_pairs); }
...
LJLIB_PUSH(lastcl)
LJLIB_ASM(ipairs) LJLIB_REC(.) { return ffh_pairs(L, MM_ipairs); }
  ...
  if (base == 10) {
    ...
    if (tviscdata(o)) {
      ...
      if (ctype_isnum(ct->info) || ctype_iscomplex(ct->info)) {
        if (LJ_DUALNUM && ctype_isinteger_or_bool(ct->info) && ct->size <= 4 &&
