...
#if LJ_TARGET_X86
      DB(DW_CFA_offset | DW_REG_BP); DUV(2); DB(DW_CFA_offset | DW_REG_DI);
      DUV(3); DB(DW_CFA_offset | DW_REG_SI); DUV(4);
      DB(DW_CFA_offset | DW_REG_BX); DUV(5);
#elif LJ_TARGET_X64
      DB(DW_CFA_offset | DW_REG_BP); DUV(2); DB(DW_CFA_offset | DW_REG_BX);
      DUV(3); DB(DW_CFA_offset | DW_REG_15); DUV(4);
      DB(DW_CFA_offset | DW_REG_14); DUV(5);
      /* Extra registers saved for JIT-compiled code. */
      DB(DW_CFA_offset | DW_REG_13); DUV(9); DB(DW_CFA_offset | DW_REG_12);
      DUV(10);
#elif LJ_TARGET_ARM
      {
        int i;
...
void lj_gdbjit_addtrace(jit_State *J, GCtrace *T) {
  ...
  ctx.spadjp =
      CFRAME_SIZE_JIT + (MSize)(parent ? traceref(J, parent)->spadjust : 0);
