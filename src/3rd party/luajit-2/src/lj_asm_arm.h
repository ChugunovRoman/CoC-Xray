...
static MCode *asm_exitstub_gen(ASMState *as, ExitNo group) {
  ...
  *mxp = ARMI_BL |
         ((((MCode *)(void *)lj_vm_exit_handler - mxp) - 2) & 0x00ffffffu);
  mxp++;
...
static void asm_fusexref(ASMState *as, ARMIns ai, Reg rd, IRRef ref,
  ...
  if (ra_noreg(ir->r) && canfuse(as, ir)) {
    ...
    if (ir->o == IR_ADD) {
      int32_t ofs2;
      if (irref_isk(ir->op2) && (ofs2 = ofs + IR(ir->op2)->i) > -lim &&
          ofs2 < lim && (!(!LJ_SOFTFP && (ai & 0x08000000)) || !(ofs2 & 3))) {
        ofs = ofs2;
        ref = ir->op1;
...
static int asm_fusemadd(ASMState *as, IRIns *ir, ARMIns ai, ARMIns air) {
  ...
  if (lref != rref &&
    ...
    Reg right,
        left =
            ra_alloc2(as, irm, rset_exclude(rset_exclude(RSET_FPR, dest), add));
    right = (left >> 8);
    left &= 255;
...
#if LJ_SOFTFP
  if (destused) {
    if (ra_hasspill(ir->s) && ra_hasspill((ir + 1)->s) && (ir->s & 1) == 0 &&
        ir->s + 1 == (ir + 1)->s) {
      int i;
      for (i = 0; i < 2; i++) {
        Reg r = (ir + i)->r;
  ...
  *l_loop = ARMF_CC(ARMI_B, CC_NE) | ((as->mcp - l_loop - 2) & 0x00ffffffu);
  ...
  if (khash == 0) {
    ...
    if (irt_isstr(kt)) { /* Fetch of str->hash is cheaper than ra_allock. */
      ...
      emit_dnm(as, ARMI_SUB | ARMF_SH(ARMSH_ROR, 32 - HASH_ROT3), tmp, tmp,
               tmp + 1);
      emit_lso(as, ARMI_LDR, dest, tab, (int32_t)offsetof(GCtab, node));
      emit_dnm(as,
               ARMI_EOR |
                   ARMF_SH(ARMSH_ROR, 32 - ((HASH_ROT2 + HASH_ROT1) & 31)),
               tmp, tmp + 1, tmp);
      emit_lso(as, ARMI_LDR, RID_TMP, tab, (int32_t)offsetof(GCtab, hmask));
      emit_dnm(as, ARMI_SUB | ARMF_SH(ARMSH_ROR, 32 - HASH_ROT1), tmp + 1,
               tmp + 1, tmp);
...
static void asm_uref(ASMState *as, IRIns *ir) {
  ...
  if (irref_isk(ir->op1)) {
    ...
    if (ir->o == IR_UREFC) {
      ...
      emit_opk(as, ARMI_ADD, dest, uv, (int32_t)offsetof(GCupval, tv),
               RSET_GPR);
...
static void asm_xload(ASMState *as, IRIns *ir) {
  Reg dest =
      ra_dest(as, ir, (!LJ_SOFTFP && irt_isfp(ir->t)) ? RSET_FPR : RSET_GPR);
...
static void asm_ahuvload(ASMState *as, IRIns *ir) {
  ...
  if (ra_used(ir)) {
    lua_assert((LJ_SOFTFP ? 0 : irt_isnum(ir->t)) || irt_isint(ir->t) ||
               irt_isaddr(ir->t));
