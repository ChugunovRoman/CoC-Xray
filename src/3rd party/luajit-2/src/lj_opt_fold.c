...
LJFOLDF(kfold_int64arith) {
  return INT64FOLD(
      kfold_int64arith(ir_k64(fleft)->u64, ir_k64(fright)->u64, (IROp)fins->o));
}
...
LJFOLDF(simplify_band_k) {
  int64_t k =
      fright->o == IR_KINT ? (int64_t)fright->i : (int64_t)ir_k64(fright)->u64;
  if (k == 0) /* i & 0 ==> 0 */
    return RIGHTFOLD;
  if (k == -1) /* i & -1 ==> i */
    return LEFTFOLD;
...
LJFOLDF(simplify_shift1_ki) {
  int64_t k =
      fleft->o == IR_KINT ? (int64_t)fleft->i : (int64_t)ir_k64(fleft)->u64;
  if (k == 0) /* 0 o i ==> 0 */
    return LEFTFOLD;
  return NEXTFOLD;
}
...
LJFOLDF(reassoc_intarith_k64) {
  ...
  if (irk->o == IR_KINT64) {
    uint64_t k =
        kfold_int64arith(ir_k64(irk)->u64, ir_k64(fright)->u64, (IROp)fins->o);
    PHIBARRIER(fleft);
    fins->op1 = fleft->op1;
...
TRef LJ_FASTCALL lj_opt_fold(jit_State *J) {
  ...
  if (LJ_UNLIKELY((J->flags & JIT_F_OPT_MASK) != JIT_F_OPT_DEFAULT)) {
    lua_assert(
        ((JIT_F_OPT_FOLD | JIT_F_OPT_FWD | JIT_F_OPT_CSE | JIT_F_OPT_DSE) |
