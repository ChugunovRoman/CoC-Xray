    ...
    if (c == '\\') {
      ...
      case 'x':
        ...
        while (lj_char_isxdigit(cp_get(cp)))
          c = (c << 4) +
              (lj_char_isdigit(cp->c) ? cp->c - '0' : (cp->c & 15) + 9);
        cp_save(cp, (c & 0xff));
        continue;
      default:
...
#define CPNS_DEFAULT                                                           \
  ((1u << CT_KW) | (1u << CT_TYPEDEF) | (1u << CT_FUNC) | (1u << CT_EXTERN) |  \
   (1u << CT_CONSTVAL))
        ...
        if (!(info & CTF_BOOL)) {
          ...
          if (vsize) { /* Vector size set? */
            ...
            if (vsize >= esize) {
              ...
              info = CTINFO(CT_ARRAY,
                            (info & CTF_QUAL) + CTF_VECTOR + CTALIGN(vsize));
            }
          }
        }
...
static CPscl cp_decl_spec(CPState *cp, CPDecl *decl, CPscl scl) {
  ...
  for (;;) { /* Parse basic types. */
    ...
    if (sz || tdef ||
        (cds &
         (CDF_SHORT | CDF_LONG | CDF_SIGNED | CDF_UNSIGNED | CDF_COMPLEX)))
      break;
    switch (cp->tok) {
    case CTOK_STRUCT:
    ...
    do {
      ...
      if (ctype_isvoid(ct->info))
        ...
        ctypeid = lj_ctype_intern(
            cp->cts, CTINFO(CT_PTR, CTALIGN_PTR | ctype_cid(ct->info)),
            CTSIZE_PTR);
      else if (ctype_isfunc(ct->info))
        ctypeid = lj_ctype_intern(
            cp->cts, CTINFO(CT_PTR, CTALIGN_PTR | ctypeid), CTSIZE_PTR);
      /* Add new parameter. */
