...
static void asm_exitstub_setup(ASMState *as, ExitNo nexits) {
  ...
  *--mxp =
      PPCI_LI | PPCF_T(RID_TMP) | as->T->traceno; /* Read by exit handler. */
  mxp--;
  *mxp = PPCI_BL |
         ((((MCode *)(void *)lj_vm_exit_handler - mxp) & 0x00ffffffu) << 2);
...
static void asm_tointg(ASMState *as, IRIns *ir, Reg left) {
  ...
  emit_lsptr(as, PPCI_LFS, (fbias & 31),
             (void *)lj_ir_k64_find(as->J, U64x(59800004, 59800000)), RSET_GPR);
...
static void asm_hrefk(ASMState *as, IRIns *ir) {
  ...
  Reg dest =
      (ra_used(ir) || ofs > 32736) ? ra_dest(as, ir, RSET_GPR) : RID_NONE;
  ...
  if (irref_isk(ref)) {
    ...
    if (ra_hasreg(irr->r)) {
      ...
      right = irr->r;
               ...
               irref_isk(irr->op2) && checki16(ofs + IR(irr->op2)->i)) {
      ofs += IR(irr->op2)->i;
...
static void asm_ahuvload(ASMState *as, IRIns *ir) {
  ...
  if (irt_isnum(t)) {
    ...
    if (ra_hasreg(dest)) {
      if (ofs == AHUREF_LSX) {
        tmp = ra_scratch(
            as, rset_exclude(rset_exclude(RSET_GPR, (idx & 255)), (idx >> 8)));
...
static void asm_sload(ASMState *as, IRIns *ir) {
  int32_t ofs =
      8 * ((int32_t)ir->op1 - 1) + ((ir->op2 & IRSLOAD_FRAME) ? 0 : 4);
  IRType1 t = ir->t;
...
static void asm_min_max(ASMState *as, IRIns *ir, int ismax) {
  if (irt_isnum(ir->t)) {
    ...
    if (tmp == left || tmp == right)
      tmp = ra_scratch(
          as, rset_exclude(rset_exclude(rset_exclude(RSET_FPR, dest), left),
                           right));
    ...
    if (tmp2 == left || tmp2 == right)
      tmp2 = ra_scratch(
          as, rset_exclude(rset_exclude(rset_exclude(RSET_GPR, dest), left),
                           right));
...
static void asm_comp64(ASMState *as, IRIns *ir) {
  ...
  if ((cc & 3) == (CC_EQ & 3)) {
    asm_guardcc(as, cc);
    emit_tab(as, (cc & 4) ? PPCI_CRAND : PPCI_CROR, (CC_EQ & 3), (CC_EQ & 3),
             4 + (CC_EQ & 3));
  } else {
    asm_guardcc(as, CC_EQ);
    emit_tab(as, PPCI_CROR, (CC_EQ & 3), (CC_EQ & 3), ((cc ^ ~(cc >> 2)) & 1));
    emit_tab(as, (cc & 4) ? PPCI_CRAND : PPCI_CRANDC, (CC_EQ & 3), (CC_EQ & 3),
             4 + (cc & 3));
  }
...
static void asm_loop_fixup(ASMState *as) {
  ...
  if (as->loopinv) { /* Inverted loop branch? */
    ...
    p[-2] =
        (p[-2] & (0xffff0000u & ~PPCF_Y)) | (((target - p + 2) & 0x3fffu) << 2);
  } else {
...
static void asm_tail_fixup(ASMState *as, TraceNo lnk) {
  ...
  if (spadj == 0) {
    ...
    *--p = PPCI_NOP;
    ...
    p[-3] =
        PPCI_ADDI | PPCF_T(RID_TMP) | PPCF_A(RID_SP) | (CFRAME_SIZE + spadj);
