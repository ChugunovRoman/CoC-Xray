...
typedef long (*PNTAVM)(HANDLE handle, void **addr, ULONG zbits, size_t *size,
                       ULONG alloctype, ULONG prot);
static PNTAVM ntavm;
  ...
  if (pad < MAX_REQUEST && is_initialized(m)) {
    ...
    if (m->topsize > pad) {
      ...
      if (sp->size >= extra &&
        ...
        if ((CALL_MREMAP(sp->base, sp->size, newsize, CALL_MREMAP_NOMOVE) !=
             MFAIL) ||
