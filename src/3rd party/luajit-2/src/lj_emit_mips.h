...
static void emit_rotr(ASMState *as, Reg dest, Reg src, Reg tmp,
                      uint32_t shift) {
...
static void emit_branch(ASMState *as, MIPSIns mi, Reg rs, Reg rt,
                        MCode *target) {
  MCode *p = as->mcp;
...
static void emit_spload(ASMState *as, IRIns *ir, Reg r, int32_t ofs) {
  if (r < RID_MAX_GPR)
    ...
    emit_tsi(as, irt_isnum(ir->t) ? MIPSI_LDC1 : MIPSI_LWC1, (r & 31), RID_SP,
             ofs);
}
...
static void emit_spstore(ASMState *as, IRIns *ir, Reg r, int32_t ofs) {
  if (r < RID_MAX_GPR)
    ...
    emit_tsi(as, irt_isnum(ir->t) ? MIPSI_SDC1 : MIPSI_SWC1, (r & 31), RID_SP,
             ofs);
}
