...
static void penalty_pc(jit_State *J, GCproto *pt, BCIns *pc, TraceError e) {
  ...
  for (i = 0; i < PENALTY_SLOTS; i++)
    if (mref(J->penalty[i].pc, const BCIns) == pc) { /* Cache slot found? */
      ...
      val =
          ((uint32_t)J->penalty[i].val << 1) + LJ_PRNG_BITS(J, PENALTY_RNDBITS);
      if (val > PENALTY_MAX) {
...
static void trace_stop(jit_State *J) {
  ...
  L = J->L;
  lj_vmevent_send(L, TRACE, setstrV(L, L->top++, lj_str_newlit(L, "stop"));
  ...
  if (traceno) {
    ...
    lj_vmevent_send(
        ...
        pc = J->pc; while (!isluafunc(frame_func(frame))) {
          pc =
              (frame_iscont(frame) ? frame_contpc(frame) : frame_pc(frame)) - 1;
  ...
  retry:
    ...
    case LJ_TRACE_RECORD:
      ...
      lj_vmevent_send_(
          ...
          TValue savetv2 = J2G(J)->tmptv2; setintV(L->top++, J->cur.traceno);
          setfuncV(L, L->top++, J->fn);
          setintV(L->top++, J->pt ? (int32_t)proto_bcpos(J->pt, J->pc) : -1);
          setintV(L->top++, J->framedepth);, J2G(J)->tmptv = savetv;
