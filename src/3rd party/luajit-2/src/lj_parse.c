...
LJ_NORET static void err_limit(FuncState *fs, uint32_t limit,
                               const char *what) {
  if (fs->linedefined == 0)
...
static void jmp_patchval(FuncState *fs, BCPos list, BCPos vtarget, BCReg reg,
                         BCPos dtarget) {
  while (list != NO_JMP) {
...
static void bcemit_binop(FuncState *fs, BinOpr op, ExpDesc *e1, ExpDesc *e2) {
  if (op <= OPR_POW) {
    ...
    *e1 = *e2;
    ...
    lua_assert(op == OPR_NE || op == OPR_EQ || op == OPR_LT || op == OPR_GE ||
               op == OPR_LE || op == OPR_GT);
  ...
  for (; vg < vl; vg++)
    if (gcrefeq(vg->name, vl->name) && gola_isgoto(vg)) {
      if (vg->slot < vl->slot) {
        ...
        lj_lex_error(ls, 0, LJ_ERR_XGSCOPE, strdata(strref(vg->name)),
                     strdata(name));
      }
      gola_patch(ls, vg, vl);
    }
...
static void fs_fixup_line(FuncState *fs, GCproto *pt, void *lineinfo,
                          BCLine numline) {
