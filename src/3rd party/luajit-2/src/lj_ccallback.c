...
static void callback_mcode_init(global_State *g, uint32_t *page) {
  ...
  *p++ =
      PPCI_ORI | PPCF_A(RID_TMP) | PPCF_T(RID_TMP) | (u32ptr(target) & 0xffff);
...
static void callback_mcode_init(global_State *g, uint32_t *page) {
  ...
  *p++ =
      MIPSI_ORI | MIPSF_T(RID_R3) | MIPSF_S(RID_R3) | (u32ptr(target) & 0xffff);
...
static void callback_mcode_new(CTState *cts) {
  ...
  if (!p)
    lj_err_caller(cts->L, LJ_ERR_FFI_CBACKOV);
#elif LJ_TARGET_POSIX
  p = mmap(NULL, sz, (PROT_READ | PROT_WRITE), MAP_PRIVATE | MAP_ANONYMOUS, -1,
           0);
  if (p == MAP_FAILED)
    lj_err_caller(cts->L, LJ_ERR_FFI_CBACKOV);
#else
...
#define CALLBACK_HANDLE_REGARG                                                 \
  if (isfp && nfpr < CCALL_NARG_FPR) { /* Try to pass argument in FPRs. */     \
    sp =                                                                       \
        (void *)((uint8_t *)&cts->cb.fpr[nfpr] + ((LJ_BE && n == 1) ? 4 : 0)); \
    nfpr++;                                                                    \
    ngpr += n;                                                                 \
    goto done;                                                                 \
  } else { /* Try to pass argument in GPRs. */                                 \
