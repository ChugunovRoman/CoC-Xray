...
static IRRef split_call_l(jit_State *J, IRRef1 *hisubst, IRIns *oir, IRIns *ir,
                          IRCallID id) {
...
static IRRef split_call_li(jit_State *J, IRRef1 *hisubst, IRIns *oir, IRIns *ir,
                           IRCallID id) {
...
static IRRef split_call_ll(jit_State *J, IRRef1 *hisubst, IRIns *oir, IRIns *ir,
                           IRCallID id) {
  IRRef tmp, op1 = ir->op1, op2 = ir->op2;
  J->cur.nins--;
#if LJ_LE
  ...
  ir->prev = tmp = split_emit(J, IRTI(IR_CALLN), tmp, id);
  return split_emit(
      J, IRT(IR_HIOP, (LJ_SOFTFP && irt_isnum(ir->t)) ? IRT_SOFTFP : IRT_INT),
      tmp, tmp);
}
  ...
  for (ref = REF_FIRST; ref < nins; ref++) {
    ...
    if (irt_isnum(ir->t)) {
      ...
      case IR_SLOAD:
        if ((nir->op2 & IRSLOAD_CONVERT)) { /* Convert from int to number. */
          ...
          ir->prev = nref =
              split_emit(J, IRTI(IR_CALLN), nref, IRCALL_softfp_i2d);
      ...
      case IR_PHI:
        ...
        if (hisubst[ir->op1] != hisubst[ir->op2])
          split_emit(J, IRT(IR_PHI, IRT_SOFTFP), hisubst[ir->op1],
                     hisubst[ir->op2]);
        break;
      case IR_HIOP:
      ...
      default:
        ...
        hi = split_emit(J, IRTG(IR_HIOP, IRT_SOFTFP), hisubst[ir->op1],
                        hisubst[ir->op2]);
        break;
      }
    } else
      ...
      case IR_CONV: { /* Conversion to 64 bit integer. Others handled below. */
        ...
        if (st == IRT_NUM) { /* NUM to 64 bit int conv. */
          hi = split_call_l(J, hisubst, oir, ir,
                            irt_isi64(ir->t) ? IRCALL_fp64_d2l
                                             : IRCALL_fp64_d2ul);
    ...
    split_call:
      ...
      if (LJ_SOFTFP ? irt_is64(ir->t) : irt_isint64(ir->t))
        hi =
            split_emit(J,
                       IRT(IR_HIOP, (LJ_SOFTFP && irt_isnum(ir->t)) ? IRT_SOFTFP
                                                                    : IRT_INT),
                       nref, nref);
...
static int split_needsplit(jit_State *J) {
  ...
  for (ref = J->chain[IR_CONV]; ref; ref = IR(ref)->prev) {
    ...
    if ((LJ_SOFTFP && (st == IRT_NUM || st == IRT_FLOAT)) || st == IRT_I64 ||
        st == IRT_U64)
      return 1;
  }
  return 0; /* Nope. */
