...
*/
static void crec_copy_emit(jit_State *J, CRecMemList *ml, MSize mlp, TRef trdst,
                           TRef trsrc) {
  MSize i, j, rwin = 0;
  for (i = 0, j = 0; i < mlp;) {
...
*/
static void crec_fill_emit(jit_State *J, CRecMemList *ml, MSize mlp, TRef trdst,
                           TRef trfill) {
  MSize i;
  for (i = 0; i < mlp; i++) {
    TRef trofs = lj_ir_kintp(J, ml[i].ofs);
...
static TRef crec_tv_ct(jit_State *J, CType *s, CTypeID sid, TRef sp) {
  ...
  if (ctype_isnum(sinfo)) {
    ...
    ptr =
        emitir(IRT(IR_ADD, IRT_PTR), dp, lj_ir_kintp(J, sizeof(GCcdata) + esz));
...
static TRef crec_ct_tv(jit_State *J, CType *d, TRef dp, TRef sp,
                       cTValue *sval) {
  CTState *cts = ctype_ctsG(J2G(J));
  ...
  if (LJ_LIKELY(tref_isinteger(sp))) {
    sid = CTID_INT32;
    svisnz =
        (void *)(intptr_t)(tvisint(sval) ? (intV(sval) != 0) : !tviszero(sval));
  ...
  if (tref_isnumber(idx)) {
    ...
    if (ctype_ispointer(ct->info) && t >= IRT_I8 && t <= IRT_U64) {
      if (ctk->size == 8) {
        ...
        idx =
            emitir(IRT(IR_ADD, IRT_PTR), idx, lj_ir_kintp(J, sizeof(GCcdata)));
...
static TRef crec_call_args(jit_State *J, RecordFFData *rd, CTState *cts,
                           CType *ct) {
...
static TRef crec_arith_ptr(jit_State *J, TRef *sp, CType **s, MMS mm) {
  ...
  }
  {
    ...
    if (t == IRT_NUM || t == IRT_FLOAT)
      ...
      tr =
          emitconv(tr, IRT_INTP, IRT_INT, ((t - IRT_I8) & 1) ? 0 : IRCONV_SEXT);
#else
    if (!tref_typerange(sp[1], IRT_I8, IRT_U32)) {
      tr = emitconv(tr, IRT_INTP, t,
                    (t == IRT_NUM || t == IRT_FLOAT) ? IRCONV_TRUNC | IRCONV_ANY
                                                     : 0);
    }
#endif
    ...
    id = lj_ctype_intern(
        cts, CTINFO(CT_PTR, CTALIGN_PTR | ctype_cid(ctp->info)), CTSIZE_PTR);
    return emitir(IRTG(IR_CNEWI, IRT_CDATA), lj_ir_kint(J, id), tr);
  }
}
  ...
  if (trdst && trlen) {
    ...
    if (tviscdata(&rd->argv[0])) { /* Get alignment of original destination. */
      ...
      if (ctype_isptr(ct->info))
        ct = ctype_rawchild(cts, ct);
      step =
          (1u << ctype_align(lj_ctype_info(cts, ctype_typeid(cts, ct), &sz)));
    }
    trdst = crec_ct_tv(J, ctype_get(cts, CTID_P_VOID), 0, trdst, &rd->argv[0]);
    trlen = crec_toint(J, cts, trlen, &rd->argv[1]);
    if (trfill)
...
void LJ_FASTCALL recff_ffi_typeof(jit_State *J, RecordFFData *rd) {
  if (tref_iscdata(J->base[0])) {
    ...
    J->base[0] =
        emitir(IRTG(IR_CNEWI, IRT_CDATA), lj_ir_kint(J, CTID_CTYPEID), trid);
  } else {
