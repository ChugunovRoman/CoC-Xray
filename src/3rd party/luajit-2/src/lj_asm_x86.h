...
static Reg asm_fuseload(ASMState *as, IRRef ref, RegSet allow) {
  ...
  if (ir->o == IR_KNUM) {
    ...
    lua_assert(allow != RSET_EMPTY);
    if (!(avail & (avail - 1))) { /* Fuse if less than two regs available. */
      as->mrm.ofs = ptr2addr(ref == REF_BASE ? (void *)&J2G(as->J)->jit_base
                                             : (void *)ir_kint64(ir));
      as->mrm.base = as->mrm.idx = RID_NONE;
      return RID_MRM;
    }
    ...
    if (ir->o == IR_SLOAD) {
      if (!(ir->op2 & (IRSLOAD_PARENT | IRSLOAD_CONVERT)) &&
        ...
        as->mrm.ofs =
            8 * ((int32_t)ir->op1 - 1) + ((ir->op2 & IRSLOAD_FRAME) ? 4 : 0);
        as->mrm.idx = RID_NONE;
        return RID_MRM;
      }
...
static int asm_count_call_slots(ASMState *as, const CCallInfo *ci,
                                IRRef *args) {
      ...
      if (LJ_64 ? irt_isu64(ir->t) : irt_isu32(ir->t)) {
        ...
        if (st == IRT_NUM)
          emit_rma(as, XO_ADDSD, tmp,
                   lj_ir_k64_find(as->J, LJ_64 ? U64x(c3f00000, 00000000)
                                               : U64x(c1e00000, 00000000)));
        else
          emit_rma(as, XO_ADDSS, tmp,
                   lj_ir_k64_find(as->J, LJ_64 ? U64x(00000000, df800000)
        ...
        emit_mrm(
            as, op,
            dest |
                ((LJ_64 && (irt_is64(ir->t) || irt_isu32(ir->t))) ? REX_64 : 0),
            left);
      }
    }
...
static void asm_conv_fp_int64(ASMState *as, IRIns *ir) {
  ...
  if (ra_hasreg(dest)) {
    ra_free(as, dest);
    ra_modified(as, dest);
    emit_rmro(as, irt_isnum(ir->t) ? XMM_MOVRM(as) : XO_MOVSS, dest, RID_ESP,
              ofs);
  }
  emit_rmro(as, irt_isnum(ir->t) ? XO_FSTPq : XO_FSTPd,
            irt_isnum(ir->t) ? XOg_FSTPq : XOg_FSTPd, RID_ESP, ofs);
...
static void asm_cnew(ASMState *as, IRIns *ir) {
  ...
  if (ir->o == IR_CNEWI) {
    ...
    if (irref_isk(ir->op2)) {
      IRIns *irk = IR(ir->op2);
      uint64_t k =
          irk->o == IR_KINT64 ? ir_k64(irk)->u64 : (uint64_t)(uint32_t)irk->i;
...
static void asm_obar(ASMState *as, IRIns *ir) {
  ...
  if (irref_isk(ir->op2)) {
    ...
    Reg val =
        ra_alloc1(as, ir->op2, rset_exclude(RSET_SCRATCH & RSET_GPR, obj));
...
static int fpmjoin_pow(ASMState *as, IRIns *ir) {
  ...
  if (irp == ir - 1 && irp->o == IR_MUL && !ra_used(irp)) {
    ...
    if (irpp == ir - 2 && irpp->o == IR_FPMATH && irpp->op2 == IRFPM_LOG2 &&
        !ra_used(irpp)) {
...
static void asm_bitswap(ASMState *as, IRIns *ir) {
  Reg dest = ra_dest(as, ir, RSET_GPR);
  as->mcp = emit_op(XO_BSWAP + ((dest & 7) << 24), REX_64IR(ir, 0), dest, 0,
                    as->mcp, 1);
  ra_left(as, dest, ir->op1);
}
        ...
        if (opisfusableload((IROp)irl->o) &&
          ...
          */
          ...
          if (left == RID_MRM) { /* Fusion succeeded? */
            ...
            emit_i8(as, imm);
            emit_mrm(as,
                     (irt_isi8(origt) || irt_isu8(origt)) ? XO_ARITHib
                                                          : XO_ARITHiw8,
                     r64 + XOg_CMP, RID_MRM);
            return;
          } /* Otherwise handle register case as usual. */
        } else {
          left = asm_fuseloadm(as, lref, irt_isu8(ir->t) ? RSET_GPR8 : RSET_GPR,
                               r64);
        }
        asm_guardcc(as, cc);
...
static const uint8_t map_op1[256] = {
    0x92, 0x92, 0x92, 0x92, 0x52, 0x45, 0x51, 0x51, 0x92, 0x92, 0x92, 0x92,
    0x52, 0x45, 0x51, 0x20, 0x92, 0x92, 0x92, 0x92, 0x52, 0x45, 0x51, 0x51,
    0x92, 0x92, 0x92, 0x92, 0x52, 0x45, 0x51, 0x51, 0x92, 0x92, 0x92, 0x92,
    0x52, 0x45, 0x10, 0x51, 0x92, 0x92, 0x92, 0x92, 0x52, 0x45, 0x10, 0x51,
    0x92, 0x92, 0x92, 0x92, 0x52, 0x45, 0x10, 0x51, 0x92, 0x92, 0x92, 0x92,
    0x52, 0x45, 0x10, 0x51,
#if LJ_64
    0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x14, 0x14, 0x14, 0x14,
    0x14, 0x14, 0x14, 0x14,
#else
...
#endif
    0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51,
    0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x92, 0x92, 0x10, 0x10, 0x12, 0x11,
    0x45, 0x86, 0x52, 0x93, 0x51, 0x51, 0x51, 0x51, 0x52, 0x52, 0x52, 0x52,
    0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52,
    0x93, 0x86, 0x93, 0x93, 0x92, 0x92, 0x92, 0x92, 0x92, 0x92, 0x92, 0x92,
    0x92, 0x92, 0x92, 0x92, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51,
    0x51, 0x51, 0x47, 0x51, 0x51, 0x51, 0x51, 0x51,
#if LJ_64
    0x59, 0x59, 0x59, 0x59, 0x51, 0x51, 0x51, 0x51, 0x52, 0x45, 0x51, 0x51,
    0x51, 0x51, 0x51, 0x51,
#else
    0x55, 0x55, 0x55, 0x55, 0x51, 0x51, 0x51, 0x51, 0x52, 0x45, 0x51, 0x51,
    0x51, 0x51, 0x51, 0x51,
#endif
    0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x05, 0x05, 0x05, 0x05,
    0x05, 0x05, 0x05, 0x05, 0x93, 0x93, 0x53, 0x51, 0x70, 0x71, 0x93, 0x86,
    0x54, 0x51, 0x53, 0x51, 0x51, 0x52, 0x51, 0x51, 0x92, 0x92, 0x92, 0x92,
    0x52, 0x52, 0x51, 0x51, 0x92, 0x92, 0x92, 0x92, 0x92, 0x92, 0x92, 0x92,
    0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x45, 0x45, 0x47, 0x52,
    0x51, 0x51, 0x51, 0x51, 0x10, 0x51, 0x10, 0x10, 0x51, 0x51, 0x63, 0x66,
    0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x92, 0x92};

static const uint8_t map_op2[256] = {
    };
