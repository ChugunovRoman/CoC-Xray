...
static BCReg snap_usedef(jit_State *J, uint8_t *udf, const BCIns *pc,
                         BCReg maxslot) {
  BCReg s;
  GCobj *o;
...
void lj_snap_replay(jit_State *J, GCtrace *T) {
  ...
  setslot:
    J->slot[s] =
        tr | (sn & (SNAP_CONT | SNAP_FRAME)); /* Same as TREF_* flags. */
    ...
    for (n = 0; pass23 && n < nent; n++) {
      ...
      if (regsp_reg(ir->r) == RID_SUNK) {
        ...
        if (LJ_HASFFI && ir->o == IR_CNEWI) {
          ...
          J->slot[snap_slot(sn)] =
              emitir(ir->ot & ~(IRT_MARK | IRT_ISPHI), op1, op2);
        } else {
          IRIns *irs;
...
static void snap_unsink(jit_State *J, GCtrace *T, ExitState *ex, SnapNo snapno,
                        BloomFilter rfilt, IRIns *ir, TValue *o);

/* Restore a value from the trace exit state. */
static void snap_restoreval(jit_State *J, GCtrace *T, ExitState *ex,
                            SnapNo snapno, BloomFilter rfilt, IRRef ref,
                            TValue *o) {
  IRIns *ir = &T->ir[ref];
  IRType1 t = ir->t;
  RegSP rs = ir->prev;
...
static void snap_restoredata(GCtrace *T, ExitState *ex, SnapNo snapno,
                             BloomFilter rfilt, IRRef ref, void *dst,
                             CTSize sz) {
  IRIns *ir = &T->ir[ref];
  RegSP rs = ir->prev;
  int32_t *src;
  ...
  if (ir->o == IR_CNEW || ir->o == IR_CNEWI) {
    ...
    if (ir->o == IR_CNEWI) {
      ...
      if (LJ_32 && sz == 8 && ir + 1 < T->ir + T->nins &&
          (ir + 1)->o == IR_HIOP) {
        snap_restoredata(T, ex, snapno, rfilt, (ir + 1)->op2, LJ_LE ? p + 4 : p,
                         4);
        if (LJ_BE)
          p += 4;
        sz = 4;
      ...
      for (irs = ir + 1; irs < irlast; irs++)
        if (irs->r == RID_SINK && snap_sunk_store(T, ir, irs)) {
          ...
          if (LJ_32 && irs + 1 < T->ir + T->nins && (irs + 1)->o == IR_HIOP) {
            lua_assert(szs == 4);
            snap_restoredata(T, ex, snapno, rfilt, (irs + 1)->op2,
                             LJ_LE ? p + 4 : p, 4);
            if (LJ_BE)
              p += 4;
          }
