...
static TRef fori_load(jit_State *J, BCReg slot, IRType t, int mode) {
  int conv =
      (tvisint(&J->L->base[slot]) != (t == IRT_INT)) ? IRSLOAD_CONVERT : 0;
  ...
  if (ix->val == 0) { /* Indexed load */
    ...
    if (tvisnil(oldv)) { /* Previous value was nil? */
      ...
      if (hasmm)
        ...
        emitir(IRTG(oldv == niltvg(J2G(J)) ? IR_EQ : IR_NE, IRT_P32), xref,
...
noconstify:
  ...
  if (!uvp->closed) {
    ...
    emitir(IRTG(IR_UGT, IRT_P32), emitir(IRT(IR_SUB, IRT_P32), uref, REF_BASE),
    ...
    if (nresults >= 0) { /* Known fixed number of results. */
      ...
      if (nvararg > 0) {
        ...
        for (i = 0; i < nload; i++) {
          ...
          TRef aref =
              emitir(IRT(IR_AREF, IRT_P32), vbase, lj_ir_kint(J, (int32_t)i));
...
void lj_record_ins(jit_State *J) {
  ...
  if (LJ_UNLIKELY(J->postproc != LJ_POST_NONE)) {
    ...
    case LJ_POST_FIXCOMP: /* Fixup comparison. */
      ...
      rec_comp_fixup(J, pc,
                     (!tvistruecond(&J2G(J)->tmptv2) ^ (bc_op(*pc) & 1)));
      /* fallthrough */
...
#define rcv (&ix.keyv)
  ...
  case BCMnum: {
    ...
    copyTV(J->L, rcv, tv);
    ix.key = rc =
        tvisint(tv) ? lj_ir_kint(J, intV(tv)) : lj_ir_knumint(J, numV(tv));
  } break;
  case BCMstr: {
...
void lj_record_setup(jit_State *J) {
  ...
  */
  {
    TRef tr = emitir(IRT(IR_XLOAD, IRT_U8), lj_ir_kptr(J, &J2G(J)->hookmask),
                     IRXLOAD_VOLATILE);
