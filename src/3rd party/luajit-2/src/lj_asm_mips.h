...
static void asm_exitstub_setup(ASMState *as) {
  ...
  *--mxp =
      MIPSI_J | ((((uintptr_t)(void *)lj_vm_exit_handler) >> 2) & 0x03ffffffu);
        ...
        if (gpr <= REGARG_LASTGPR) {
          ...
          if (irt_isfp(ir->t)) {
            ...
            if (irt_isnum(ir->t)) {
              ...
              lua_assert(
                  rset_test(as->freeset, gpr + 1)); /* Already evicted. */
              gpr += 2;
...
static void asm_conv(ASMState *as, IRIns *ir) {
  ...
  if (irt_isfp(ir->t)) {
    ...
    if (stfp) { /* FP to FP conversion. */
      ...
      Reg left = ra_alloc1(as, lref, RSET_GPR);
      emit_fg(as, irt_isfloat(ir->t) ? MIPSI_CVT_S_W : MIPSI_CVT_D_W, dest,
              dest);
    ...
    if (irt_isguard(ir->t)) {
      ...
      if (irt_isu32(ir->t)) {
        ...
        emit_ti(as, MIPSI_LUI, RID_TMP, 0x8000);
        emit_tg(as, MIPSI_MFC1, dest, tmp);
        emit_fg(as, st == IRT_FLOAT ? MIPSI_FLOOR_W_S : MIPSI_FLOOR_W_D, tmp,
                tmp);
        emit_fgh(as, st == IRT_FLOAT ? MIPSI_SUB_S : MIPSI_SUB_D, tmp, left,
                 tmp);
        if (st == IRT_FLOAT)
        ...
        emit_tg(as, MIPSI_MFC1, dest, tmp);
        emit_fg(as, st == IRT_FLOAT ? MIPSI_TRUNC_W_S : MIPSI_TRUNC_W_D, tmp,
                left);
      }
    }
  } else {
...
static void asm_arithov(ASMState *as, IRIns *ir) {
  ...
  tmp = ra_scratch(
      as,
      rset_exclude(rset_exclude(rset_exclude(RSET_GPR, left), right), dest));
...
static void asm_comp(ASMState *as, IRIns *ir) {
  ...
  if (irt_isnum(ir->t)) {
    ...
    if ((op & 4) == 0 && irref_isk(ir->op2) && IR(ir->op2)->i == 0) {
      ...
      right = ra_alloc1(as, ir->op2, rset_exclude(RSET_GPR, left));
      asm_guard(as, ((op ^ (op >> 1)) & 1) ? MIPSI_BNE : MIPSI_BEQ, RID_TMP,
                RID_ZERO);
