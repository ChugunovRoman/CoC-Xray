...
#define NV_FOREACH(i, container)                                               \
  ...
  for (NV_STRING_JOIN2(cont, __LINE__)::PseudoIndex i((container).start());    \
...
struct PseudoIndexWrapper {
  template <typename T> PseudoIndexWrapper(const T &container) {
...
#define NV_FOREACH(i, container)                                               \
  for (PseudoIndexWrapper i(container); !(container).isDone(i(&(container)));  \
 ...
 */
template <typename T> class NVCORE_CLASS Array {
public:
  /// Ctor.
  ...
  void append(const T other[], uint count) {
    if (count > 0) {
      ...
      resize(m_size + count);
      // Must use operator=() to copy elements, in case of side effects (e.g.
      // ref-counting).
...
public:
  ...
  struct Entry {
    ...
    Entry(const Entry &e)
        : next_in_chain(e.next_in_chain), hash_value(e.hash_value), key(e.key),
          value(e.value) {}
