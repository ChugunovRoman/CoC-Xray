...
static void *callerAddress(void *secret) {
  ...
  // How to obtain the instruction pointers in different platforms, from mlton's
  // source code. http://mlton.org/ OpenBSD && NetBSD ucp->sc_eip FreeBSD:
...
struct Win32AssertHandler : public AssertHandler {
  ...
  virtual int assert(const char *exp, const char *file, int line,
