...
inline bool equal(Vector2::Arg v1, Vector2::Arg v2,
...
inline Vector3 normalizeSafe(Vector3::Arg v, Vector3::Arg fallback,
                             float epsilon = NV_EPSILON) {
  float l = length(v);
  if (isZero(l, epsilon)) {
    return fallback;
...
inline bool equal(Vector3::Arg v1, Vector3::Arg v2,
                  float epsilon = NV_EPSILON) {
  return equal(v1.x(), v2.x(), epsilon) && equal(v1.y(), v2.y(), epsilon) &&
         equal(v1.z(), v2.z(), epsilon);
}
...
inline Vector4 normalizeSafe(Vector4::Arg v, Vector4::Arg fallback,
                             float epsilon = NV_EPSILON) {
  float l = length(v);
  if (isZero(l, epsilon)) {
    return fallback;
...
inline bool equal(Vector4::Arg v1, Vector4::Arg v2,
                  float epsilon = NV_EPSILON) {
  return equal(v1.x(), v2.x(), epsilon) && equal(v1.y(), v2.y(), epsilon) &&
...
inline Vector4 min(Vector4::Arg a, Vector4::Arg b) {
  return Vector4(min(a.x(), b.x()), min(a.y(), b.y()), min(a.z(), b.z()),
                 min(a.w(), b.w()));
}

inline Vector4 max(Vector4::Arg a, Vector4::Arg b) {
  return Vector4(max(a.x(), b.x()), max(a.y(), b.y()), max(a.z(), b.z()),
                 max(a.w(), b.w()));
}
