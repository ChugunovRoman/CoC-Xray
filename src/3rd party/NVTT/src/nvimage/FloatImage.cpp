...
FloatImage::FloatImage()
    : m_width(0), m_height(0), m_componentNum(0), m_count(0), m_mem(NULL) {}
...
Image *FloatImage::createImageGammaCorrect(float gamma /*= 2.2f*/) const {
  ...
  for (uint i = 0; i < size; i++) {
    const uint8 r =
        nv::clamp(int(255.0f * pow(rChannel[i], 1.0f / gamma)), 0, 255);
    const uint8 g =
        nv::clamp(int(255.0f * pow(gChannel[i], 1.0f / gamma)), 0, 255);
    const uint8 b =
        nv::clamp(int(255.0f * pow(bChannel[i], 1.0f / gamma)), 0, 255);
...
void FloatImage::scaleBias(uint base_component, uint num, float scale,
                           float bias) {
  const uint size = m_width * m_height;
...
void FloatImage::toLinear(uint base_component, uint num,
                          float gamma /*= 2.2f*/) {
  exponentiate(base_component, num, gamma);
}

/// From linear to gamma space.
void FloatImage::toGamma(uint base_component, uint num,
                         float gamma /*= 2.2f*/) {
...
float FloatImage::sampleNearest(const float x, const float y, const int c,
...
float FloatImage::sampleLinear(const float x, const float y, const int c,
                               const WrapMode wm) const {
  if (wm == WrapMode_Clamp)
...
float FloatImage::sampleNearestClamp(const float x, const float y,
                                     const int c) const {
...
float FloatImage::sampleNearestRepeat(const float x, const float y,
                                      const int c) const {
...
float FloatImage::sampleNearestMirror(const float x, const float y,
                                      const int c) const {
...
/// Downsample applying a 1D kernel separately in each dimension.
FloatImage *FloatImage::downSample(const Filter &filter, WrapMode wm,
                                   uint alpha) const {
  const uint w = max(1, m_width / 2);
  const uint h = max(1, m_height / 2);
...
/// Downsample applying a 1D kernel separately in each dimension.
FloatImage *FloatImage::resize(const Filter &filter, uint w, uint h,
                               WrapMode wm) const {
...
FloatImage *FloatImage::resize(const Filter &filter, uint w, uint h,
...

  {
    ...
    // Process all channels before applying vertical kernel to make sure alpha
    // has been computed.
...
float FloatImage::applyKernel(const Kernel2 *k, int x, int y, uint c,
                              WrapMode wm) const {
  nvDebugCheck(k != NULL);
...
float FloatImage::applyKernelVertical(const Kernel1 *k, int x, int y, uint c,
                                      WrapMode wm) const {
  nvDebugCheck(k != NULL);
...
float FloatImage::applyKernelHorizontal(const Kernel1 *k, int x, int y, uint c,
                                        WrapMode wm) const {
  nvDebugCheck(k != NULL);
...
/// Apply 1D vertical kernel at the given coordinates and return result.
void FloatImage::applyKernelVertical(const PolyphaseKernel &k, int x, uint c,
                                     WrapMode wm,
...
/// Apply 1D horizontal kernel at the given coordinates and return result.
void FloatImage::applyKernelHorizontal(const PolyphaseKernel &k, int y, uint c,
                                       WrapMode wm,
                                       float *__restrict output) const {
  const uint length = k.length();
