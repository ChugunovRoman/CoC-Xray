...
bool nv::ImageIO::saveFloat(const char *fileName, const FloatImage *fimage,
                            uint base_component, uint num_components) {
  const char *extension = Path::extension(fileName);
...
Image *nv::ImageIO::loadTGA(Stream &s) {
  ...
  case TGA_TYPE_INDEXED:
    if (tga.colormap_type != 1 || tga.colormap_size != 24 ||
      ...
      nvDebug("*** ImageIO::loadTGA: Error, only 24bit paletted images are "
              "supported.\n");
      return false;
    }
    pal = true;
  ...
  if (pal) {
    for (int y = 0; y < tga.height; y++) {
      for (int x = 0; x < tga.width; x++) {
        uint8 idx = *src++;
        dst[x].setBGRA(palette[3 * idx + 0], palette[3 * idx + 1],
      ...
      for (uint x = 0; x < width; x++) {
        if (bpp == 8) {
          ...
          if (format == SAMPLEFORMAT_IEEEFP) {
            ...
            dst[x] =
                float(((::uint32 *)buf)[x * spp + c] >> 8) / float(0xFFFFFF);
          }
        }
      }
    }
...
bool nv::ImageIO::saveFloatTIFF(const char *fileName, const FloatImage *fimage,
                                uint base_component, uint num_components) {
...
FloatImage *nv::ImageIO::loadFloatEXR(const char *fileName, Stream &s) {
  ...
  for (Imf::ChannelList::ConstIterator it = channels.begin();
    ...
    frameBuffer.insert(it.name(),
                       Imf::Slice(Imf::FLOAT, (char *)fimage->channel(i),
...
bool nv::ImageIO::saveFloatEXR(const char *fileName, const FloatImage *fimage,
                               uint base_component, uint num_components) {
  nvCheck(fileName != NULL);
  nvCheck(fimage != NULL);
  nvCheck(base_component + num_components <= fimage->componentNum());
  ...
  for (uint c = 0; c < num_components; c++) {
    ...
    frameBuffer.insert(
        channelNames[c],
        Imf::Slice(Imf::FLOAT, channel, sizeof(float), sizeof(float) * w));
  }
