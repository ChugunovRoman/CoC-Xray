...
void nv::Quantize::Truncate(Image *image, uint rsize, uint gsize, uint bsize,
                            uint asize) {
  nvCheck(image != NULL);
...
// Error diffusion. Floyd Steinberg.
void nv::Quantize::FloydSteinberg(Image *image, uint rsize, uint gsize,
                                  uint bsize, uint asize) {
  nvCheck(image != NULL);
  ...
  for (uint y = 0; y < h; y++) {
    for (uint x = 0; x < w; x++) {
      ...
      // Compute new error.
      Vector4 diff(float(int(pixel.r) - r), float(int(pixel.g) - g),
