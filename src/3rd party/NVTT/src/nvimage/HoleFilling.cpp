    ...
    for (int c = 0; c < count; c++) {
      ...
      for (int y = 0; y < h; y++) {
        for (int x = 0; x < w; x++) {
          ...
          int num = west + east + north + south + northwest + northeast +
                    southwest + southeast;
...
static bool downsample(const FloatImage *src, const BitMap *srcMask,
                       const FloatImage **_dst, const BitMap **_dstMask) {
...
void nv::fillPullPush(FloatImage *img, const BitMap *bmap) {
  ...
  while (
      downsample(mipmaps.back(), mipmapMasks.back(), &current, &currentMask)) {
...
void nv::fillQuadraticExtrapolate(int passCount, FloatImage *img, BitMap *bmap,
                                  int coverageIndex /*= -1*/) {
  nvCheck(passCount > 0);
  nvCheck(img != NULL);
  nvCheck(bmap != NULL);
      ...
      for (int yb = 0; yb < h; yb++) {
        for (int xb = 0; xb < w; xb++) {
          ...
          if (lp.doLocalPixelFill()) {
            ...
            if (c == firstChannel) {
              // coverageChannel[idx] /= lp.weight;	// @@ Not sure what this
              // was for, coverageChannel[idx] is always zero.
              newbmap->setBitAt(xb, yb);
            }
          }
        }
